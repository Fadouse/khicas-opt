...
def verify(case, printed):
    ident = case["id"]
    assert "integrate(" not in printed and "diff(" not in printed
    local["surd"] = Root
    if ident == "PC10-D2":
        assert printed.startswith("piecewise(")
        c, a, d, b, e = printed[10:-1].split(",")
        assert c == "0>=x" and d == "x<1"
        assert equal(parse(a), x / (x - 1)) and equal(
            parse(b), x * (x + 2) / (1 + x) ** 2
        )
        f = parse(e)
        pieces = f.atoms(s.Piecewise)
        assert len(pieces) == 1
        p = next(iter(pieces))
        assert p.args[0] == (0, s.Eq(x, 1)) and p.args[1][1] == True
        regular = f.xreplace({p: p.args[1][0]})
        assert equal(regular, s.Rational(3, 4) + 3 * s.sqrt(x - 1) / 2)
        assert f.subs(x, 1) == s.Rational(3, 4)
        return dict(
            exact=True,
            method="Exact interior derivatives and lazy removable endpoint. Original quotients at zero both tend to zero; at one they are (3+2h)/(2*(2+h)) on the left and 3/4+sqrt(h) on the right, both tending to 3/4. Original function values agree; the infinite second derivative is irrelevant to the finite first derivative.",
        )
    a = parse(printed)
    assert isinstance(a, s.Expr)
    if ident == "PC10-D1":
        pieces = a.atoms(s.Piecewise)
        regular = a
        assert any(p.args[0] == (0, s.Eq(x, 0)) for p in pieces), (
            "Missing removable value at zero"
        )
        assert any(
            p.args[0][0] == s.Symbol("undef") and p.args[0][1] == s.Eq(x, 1)
            for p in pieces
        ), "Missing genuine cusp exclusion"
        while regular.has(s.Piecewise):
        ...
        assert equal(denominator, (x * x + y * y) ** 24), (
            "Original zero denominator must remain explicit"
        )
