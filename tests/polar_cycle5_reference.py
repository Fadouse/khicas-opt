...
def verify(case, printed):
    ident = case["id"]
    assert not any(w in printed for w in ["undef", "integrate(", "diff("]), printed[
        :200
    ]
    local["surd"] = Root
    if ident == "PC5-R1":
        ...
        return dict(
            exact=True,
            method="Exact structural preservation. Both powered bases are >=1; reciprocal radicand >=2, equality only at origin. Squared linear forms prove simultaneous reversal and coordinate-reversal symmetry, and the diagonal restriction.",
        )
    if ident == "PC5-D2":
        assert printed.startswith("piecewise(") and printed.endswith(")")
        c1, a, c2, b, c = printed[10:-1].split(",")
        assert c1 == "0>x" and c2 == "x<=1"
        assert (
            equal(parse(a), -1 / (1 - x))
            and equal(parse(b), -1 / (1 + x) ** 2)
            and equal(parse(c), -s.Rational(1, 4) + 2 * (x - 1))
        )
        return dict(
            exact=True,
            method="Exact open-region derivatives; original one-sided difference quotients agree at 0 (-1) and 1 (-1/4). Actual substitution checks inactive poles and logarithm branches.",
        )
    a = parse(printed)
    assert isinstance(a, s.Expr), "Scalar expression returned a list instead of a value"
    ...
    if ident == "PC5-I1":
        ...
        expected = (
            ...
            - s.log(s.Abs(1 + u))
            + s.log(u * u - u + 1) / 2
            - s.sqrt(3) * s.atan((2 * u - 1) / s.sqrt(3))
        )
        assert equal(f, expected)
        for sign in [-1, 1]:
            assert equal(
                s.diff(f.xreplace({s.Abs(1 + u): sign * (1 + u)}), u),
                3 * u**3 / (1 + u**3),
            )
        ...
        assert a.subs(x, 0).subs(Root(0, 3), 0) == 0, (
            "Derivative is undefined at zero before cancellation"
        )
        ...
        assert equal(a.subs(x, 1), 0), (
            "Principal square-root cut endpoint is undefined or incorrect"
        )
