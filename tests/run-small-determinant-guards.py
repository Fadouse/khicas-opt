...
def matrix_text(matrix):
    return (
        "["
        + ",".join("[" + ",".join(giac(v) for v in row) + "]" for row in matrix)
        + "]"
    )
...
for n in (2, 3, 4):
    for powers in (False, True):
        ...
        for diagonal in (
            ...
            list(range(1, n + 1)),
            [0] + list(range(2, n + 1)),
            [0, 0] + list(range(3, n + 1)),
        ):
            for D in (s.Integer(1), (x + y + z) ** 2, x - 1):
                M = [
                    [(diagonal[i] if i == j else 0) + u[i] * v[j] / D for j in range(n)]
                    for i in range(n)
                ]
                ...
                target = (
                    s.prod(diagonal)
                    + sum(
                        u[i] * v[i] * s.prod(diagonal[k] for k in range(n) if k != i)
                        for i in range(n)
                    )
                    / D
                )
...
for n in (2, 3, 4):
    for D in (s.Integer(1), x + y + z):
        N = s.Matrix(
            n,
            n,
            lambda i, j: sum(rng.randint(-3, 3) * v for v in (s.Integer(1), x, y, z)),
        )
        M = [[N[i, j] / D for j in range(n)] for i in range(n)]
        cases.append(
            dict(
                input="det(" + matrix_text(M) + ")",
                target=N.det(method="berkowitz") / D**n,
                den=D,
...
for n in (2, 3, 4):
    D = x + y + z
    M = [[s.Integer(i == j) for j in range(n)] for i in range(n)]
    M[0][1] = 1 / D
    cases.append(
        dict(
            input="det(" + matrix_text(M) + ")",
    ...
    for outer in (False, True):
        for stack in ("normal", "64"):
            ...
            try:
                ...
                assert "det(" not in run.stdout, (
                    "The bounded test requires a computed determinant"
                )
                key = (index, run.stdout)
                if key not in cache:
                    actual = parse(run.stdout.strip())
                    if c["den"] != 1:
                        assert actual.func == s.Piecewise, (
                            "Missing original matrix pole guard"
                        )
                        (value, condition), (regular, otherwise) = actual.args
                        assert (
                            value == s.Symbol("undef")
                            and otherwise == True
                            and isinstance(condition, s.Equality)
                        )
                        lhs = (
                            s.Poly(condition.lhs - condition.rhs, *variables)
                            .sqf_part()
                            .monic()
                        )
                        rhs = s.Poly(c["den"], *variables).sqf_part().monic()
                        assert lhs == rhs, (
                            "Guard must have exactly the original excluded zero set"
                        )
                        actual = regular
                ...
                for point in points:
                    ...
                    if excluded:
                        ...
                        assert r.returncode == 0 and equal(
                            parse(r.stdout.strip()), c["target"].subs(sub)
                        ), (point, r.stdout)
            ...
            if not row["passed"]:
                print(
                    "FAIL", index, outer, stack, row.get("error", "")[:160], flush=True
                )
            a.report.write_text(
                json.dumps(
                    dict(
                        scope="Actual determinant entry and bounded algorithm; independent identities and exact complete pole-set comparison, normal and guarded host stacks.",
