...
def verify(case, printed):
    ...
    if ident == "PC4-D2" and printed.startswith("(("):
        ...
        printed = match[1]
    assert not any(word in printed for word in ("undef", "integrate(", "diff(")), (
        printed[:200]
    )
    if ident == "PC4-R1":
        assert len(printed) < 4096, (
            "Runaway expansion: %d output characters from a 93-character input"
            % len(printed)
        )
        assert parse(printed) == parse(case["input"][9:-1])
        return dict(
            exact=True,
            method="Exact structural preservation; A=2+30*r²+30*r⁴+2*r⁶>=2 for r=x²+y²+z²>=0, hence 64/65<=A⁶/(1+A⁶)<1 and positive denominator.",
        )
    ...
    if ident in ["PC4-I1", "PC4-I2"]:
        ...
        if ident == "PC4-I1":
            ...
            assert equal(f, 3 * u - 3 * s.log(s.Abs(u + 1)))
            for sign in [-1, 1]:
                assert equal(
                    s.diff(f.xreplace({s.Abs(u + 1): sign * (u + 1)}), u),
                    3 * u / (1 + u),
                )
            ...
            f = a.xreplace({Root(x - 2, 5): u})
            assert equal(f, 5 * u**3 / 3 - 5 * u + 5 * s.atan(u))
            assert equal(s.diff(f, u), 5 * u**4 / (1 + u * u))
            assert (
                f.subs(u, 0) == 0
                and s.limit(f / u**5, u, 0, dir="+")
                == s.limit(f / u**5, u, 0, dir="-")
                == 1
            )
