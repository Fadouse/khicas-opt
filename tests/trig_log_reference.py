...
def parse(t):
    return sp.sympify(t.replace("^", "**"), locals=local)
...
def verify_trig_log_reference(case, printed):
    ...
    for p in actual.atoms(sp.polylog):
        for q in expected.atoms(sp.polylog):
            if (
                sp.simplify(sp.expand_trig(sp.expand_complex(p.args[1] - q.args[1])))
                == 0
            ):
                replacements[p] = q
                break
    ...
    phase = (
        sp.S.Zero
        if case["id"] == "TL6"
        else parse(case["phase"])
        + (sp.pi / 2 if case["cosine"] else 0)
        + (sp.pi if case["inside_coefficient"] < 0 else 0)
    )
    for half in range(2):
        ...
        for atom in sorted(d.atoms(sp.floor), key=lambda t: sp.count_ops(t)):
            ...
            index = sp.floor(sp.simplify(argument.subs(v, 1)))
            assert (
                sp.factor(argument - index).is_nonnegative
                and sp.factor(index + 1 - argument).is_positive
            ), (case["id"], argument, index)
            d = d.xreplace({atom: index})
        for atom in sorted(d.atoms(sp.Abs), key=lambda t: sp.count_ops(t)):
            argument = sp.factor(atom.args[0])
            if argument.is_nonnegative:
                d = d.xreplace({atom: argument})
            elif argument.is_nonpositive:
                d = d.xreplace({atom: -argument})
            else:
                raise AssertionError(
                    (case["id"], "unproved absolute-value sign", argument)
                )
    ...
    for k in range(-3, 4):
        for fraction in [
            ...
            if case["id"] == "TL6":
                ...
                a = mp.mpf(str(parse(case["frequency"]).evalf(60)))
                b = mp.mpf(str(parse(case["phase"]).evalf(60)))
                if case["cosine"]:
                    b += mp.pi / 2
                if case["inside_coefficient"] < 0:
                    b += mp.pi
                point = (u - b) / a
            error = abs(mp.diff(value, point) - integrand(point))
            assert error < mp.mpf("1e-40"), (case["id"], point, error)
            errors.append(str(error))
    return {
        "exit": 0,
        "stderr": "CHECK exact: canonical Clausen/Li2 identity with principal-log ramp; independent derivative samples on 28 real subinterval points",
        "derivative_numeric_max_error": max(map(float, errors)),
    }
