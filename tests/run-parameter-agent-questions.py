...
cp = root / "tests/parameter-agent-questions.json"
...
contract = {
    "PARAM-Q1": (["a"], "re(a)>0"),
    "PARAM-Q3": (["s", "c"], "(re(c)>0) & (re(s)>0)"),
    "PARAM-Q4": (["a", "b"], "(re(a)>0) & (re(b)>0)"),
}
rows = []
for c in cases:
    for outer in [False, True]:
        ...
        if c["id"] in contract:
            params, _ = contract[c["id"]]
            e = (
                "["
                + ",".join(["assume(" + v + ",complex)" for v in params] + [e])
                + "]["
                + str(len(params))
                + "]"
            )
        for stack in ["normal", "64"]:
            ...
            try:
                ...
                if c.get("behavior") == "divergent":
                    assert (
                        r.returncode == 3
                        and row["result"] == "undef"
                        and "Divergent improper integral" in r.stderr
                    )
                    row["status"] = "divergence-rejected"
                elif c["id"] in contract:
                    assert r.returncode in [0, 2]
                    value = row["result"]
                    assert value[0] == "(" and value[-1] == ")"
                    guard, branches = value[1:-1].split("?", 1)
                    yes, no = branches.rsplit(":", 1)
                    assert parse(guard) == parse(contract[c["id"]][1])
                    assert (
                        sp.simplify((parse(yes) - parse(c["expected"])).rewrite(sp.exp))
                        == 0
                    )
                    if c["id"] == "PARAM-Q3":
                        assert (
                            "quote(" in no or no.strip().startswith("'integrate(")
                        ) and "integrate(" in no
                    else:
                    ...
                    assert sp.simplify(parse(row["result"]) - parse(c["expected"])) == 0
                    row["status"] = "exact"
            except Exception as error:
                row["status"] = "unresolved-or-failed"
                row["verification_error"] = repr(error)
            rows.append(row)
a.report.write_text(
    json.dumps(
        {
            "scope": "Root independently verifies whole-pipeline answers and complex condition predicates for eight agent-generated cases. A remaining unevaluated integral is not a pass.",
...
for c in cases:
    print(c["id"], sorted({r["status"] for r in rows if r["id"] == c["id"]}))
