...
def verify(case, printed):
    ...
    if ident == "PC13-I1":
        ...
        angles = a.atoms(s.atan)
        assert len(angles) == 1
        angle = next(iter(angles))
        arg = angle.args[0]
        sign = (
            1
            if s.trigsimp(s.simplify(arg - s.sin(x) / (2 + s.sqrt(3) + s.cos(x)))) == 0
            else -1
        )
        assert (
            s.trigsimp(s.simplify(arg - sign * s.sin(x) / (2 + s.sqrt(3) + s.cos(x))))
            == 0
        )
        assert equal(
            a.xreplace({angle: s.Symbol("A")}),
            (x - 2 * sign * s.Symbol("A")) / s.sqrt(3),
        )
        t = s.Symbol("t", real=True)
        den = s.denom(arg).subs(s.sin(x) ** 2, 1 - s.cos(x) ** 2).subs(s.cos(x), t)
        P = s.Poly(den, t)
        assert P.degree() <= 2
        extrema = [s.Integer(-1), s.Integer(1)] + [
            r for r in s.solve(s.diff(den, t), t) if r.is_real and -1 <= r <= 1
        ]
        values = [s.simplify(den.subs(t, r)) for r in extrema]
        assert all(v.is_positive for v in values) or all(
            v.is_negative for v in values
        ), "Rationalization introduced a real zero denominator"
        ...
        for k in range(-3, 4):
            assert a.subs(x, k * s.pi) == 0 and a.subs(x, s.pi / 2 + k * s.pi).has(
                s.Symbol("undef")
            )
        regular = a
        ...
        target = (
            s.cos(x) * s.Abs(s.sin(2 * x))
            + 2 * s.sin(x) * s.cos(2 * x) * s.sign(s.sin(2 * x))
            - s.sin(x)
        )
        ...
        f = a.subs(x, t - 1)
        expected = t ** (1 / (t - 1)) * ((t - 1) / t - s.log(t)) / (t - 1) ** 2
        f = f.replace(
            lambda z: z.is_Pow and z.base == t,
            lambda z: s.expand_power_exp(s.Pow(t, s.apart(z.exp, t), evaluate=False)),
        )
        assert equal(
            s.simplify(f / t ** (1 / (t - 1))), ((t - 1) / t - s.log(t)) / (t - 1) ** 2
        )
