...
def verify(case, printed):
    ident = case["id"]
    assert "integrate(" not in printed and "diff(" not in printed, printed[:200]
    if ident == "PC9-D2":
        m = re.fullmatch(r"\(\(x=1\)\? undef : (piecewise\(.*\))\)", printed)
        assert m, (
            "The function jumps at one even though the neighboring branch slopes agree"
        )
        c, a, d, b, e = m[1][10:-1].split(",")
        assert c == "0>x" and d == "x<=1"
        assert (
            equal(parse(a), s.exp(x))
            and equal(parse(b), 1 / (1 + x))
            and equal(parse(e), s.exp((x - 1) / 2) / 2)
        )
        h = s.Symbol("h", positive=True)
        assert (
            s.limit((s.exp(-h) - 1) / (-h), h, 0) == 1
            and s.limit(s.log(1 + h) / h, h, 0) == 1
        )
        assert s.limit(s.exp(h / 2) / h, h, 0) == s.oo
        return dict(
            exact=True,
            method="Exact branch derivatives. Original difference quotients at zero both tend to one. At one the right quotient exp(h/2)/h diverges because the function values differ by one, despite equal branch slopes; lazy undef rejects the jump.",
        )
    ...
    if ident == "PC9-I1":
        ...
        assert (
            s.limit(f / u**3, u, 0, dir="+") == 0
            and s.limit(f / u**3, u, 0, dir="-") == 0
        )
        ...
        F = -(s.log(t) + 1) / t
        assert (
            equal(s.diff(F, t), s.log(t) / t**2)
            and s.limit(F, t, s.oo) == 0
            and F.subs(t, 1) == -1
        )
        ...
        f = a.xreplace(
            {
                Root(x - 1, 3): u,
                Root(x**3 * (x - 1) ** 4, 3): x * u**4,
                Root(x**3, 3): x,
            }
        ).subs(x, u**3 + 1)
        expected = (
            parse(case["expected"]["expression"])
            .xreplace({Root(x - 1, 3): u})
            .subs(x, u**3 + 1)
        )
        assert equal(f, expected)
        assert equal(a.subs(x, 0).subs(Root(-1, 3), -1).subs(Root(0, 3), 0), 2)
        assert equal(a.subs(x, 1).subs(Root(0, 3), 0), s.cos(1)), (
            "Removable zero at one remains undefined"
        )
        ...
        for atom in a.atoms(s.Pow):
            if atom.exp == s.Rational(1, 2) and equal(atom.base, canonical.base):
                a = a.xreplace({atom: canonical})
        expected = s.log(canonical) - (s.log(x + s.I) - s.log(x - s.I)) / 2
        assert equal(a, expected), (
            "Principal root must retain the exact quotient, not a rectangular formula with a false zero denominator"
        )
