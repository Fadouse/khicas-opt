...
cases = [(2, 1, "sin", n) for n in [1, 2, 3, 4, 8]] + [
    (3, -2, "cos", n) for n in [2, 3, 6]
]
for A, B, w, n in cases:
    wave = s.sin(x) if w == "sin" else s.cos(x)
    f = f"1/({A}+({B})*{w}(x))^{n}"
    for outer in [False, True]:
        ...
        for stack in ["normal", "64"]:
            ...
            if printed not in cache:
                ...
                assert not actual.has(s.tan) and "integrate(" not in printed
                ...
                for power in actual.atoms(s.Pow):
                    if power.exp.is_negative and power.base.has(x):
                        base = power.base.subs(wave, z)
                        P = s.Poly(base, z)
                        assert P.degree() <= 1 and not base.has(x)
                        lo = s.simplify(base.subs(z, -1))
                        hi = s.simplify(base.subs(z, 1))
                        assert (lo.is_positive and hi.is_positive) or (
                            lo.is_negative and hi.is_negative
                        ), (e, base)
                cache[printed] = True
            rows.append(
                dict(
                    ...
                    result=printed,
                    exact=True,
                    method="Independent exact differentiation followed by rational unit-circle substitution proves the derivative identity. Every displayed variable denominator is affine in the selected sine/cosine and has same strict sign at ±1, proving global continuity and no half-angle chart holes.",
                )
            )
a.report.write_text(
    json.dumps(
