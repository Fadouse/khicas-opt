...
for file, signature in [
    ("yintg.cc", "  static bool integrate_parameter_quadratic("),
    ("ksubst.cc", "  static bool simplify_conjugate_roots("),
    ("yderive.cc", "  static bool derive_floor_phase("),
]:
    source += function((source_path(file)).read_text(), signature)
source += """}
...
groups = {
    ...
    "phase": [
        ...
        ("floor(-2*x+1)", True),
        ("x^2*floor(1/x)", True),
        ("floor((2*x+1)/(x-1))", True),
        ("floor(2/(x-1)+1/3)", True),
        ("floor(x^2)", False),
        ("floor(1/x^2)", False),
        ("floor(sin(x))", False),
        ("floor(a*x)", False),
        ("floor(x)+floor(2*x)", False),
        ("floor(1/(x^2-1))", False),
        ("x^2", False),
    ],
    "roots": [
    ...
    "constant": [
        ("3/7", True),
        ("sqrt(3)", True),
        ("(2+sqrt(3))^2-7-4*sqrt(3)", True),
        ("sqrt(3)+1/sqrt(3)", True),
        ("sqrt(2)+sqrt(3)", False),
        ("sqrt(1+sqrt(2))", False),
        ("sqrt(-1)", False),
        ("sqrt(x)", False),
    ...
    "equal": [
        ...
        ("cos((-sqrt(2*pi))^2)-1", True),
        ("x-x", False),
    ],
}
for mode, items in groups.items():
    for expression, accepted in items:
        cases.append((mode, expression, accepted, False))
...
for mode, expression, accepted, complex_mode in cases:
    ...
    rows.append(
        dict(
            ...
            and r.stdout.strip() == ("ACCEPT" if accepted else "DEFER"),
        )
    )
a.report.write_text(
    json.dumps(
        dict(
            scope="Independent direct dispatch contracts. DEFER promises only rejection by this bounded rule, not completion or correctness of the legacy fallback. Constant equality uses exact algebra, never a floating tolerance.",
            source_sha256={
