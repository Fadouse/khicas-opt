...
def verify(case, printed):
    ...
    if ident == "PC16-I1":
        ...
        y = local["y"]
        assert a.atoms(s.tan) == {s.tan(x), s.tan(y), s.tan(x + y)}
        assert equal(
            a, (s.tan(x) + s.tan(y)) / (1 - s.tan(x) * s.tan(y)) - s.tan(x + y)
        )
        numerator, den = s.fraction(s.cancel(a))
        assert equal(den, s.tan(x) * s.tan(y) - 1) or equal(
            den, 1 - s.tan(x) * s.tan(y)
        )
