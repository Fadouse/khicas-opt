...
p.add_argument("--polar-probe", required=True)
p.add_argument("--report", type=Path, required=True)
a = p.parse_args()
X, Y = s.symbols("X Y", real=True)
u = s.Symbol("t", real=True)
cases = [
    ("vertical", "[a*t/(1+t^2),a*t^2/(1+t^2)]", X * X + Y * Y - A * Y, (0, A), A),
    (
        "shifted-parameter",
        "[(a-2)*t/(1+t^2),(a-2)*t^2/(1+t^2)]",
        X * X + Y * Y - (A - 2) * Y,
        (0, A - 2),
        A - 2,
    ),
...
for name, pair, equation, hole, scale in cases:
    ...
    for stack in ("normal", "64"):
        for outer in (False, True):
            ...
            expression = f"({pair},t,[r,theta])"
            row = dict(case=name, input=expression, stack=stack, outer=outer)
            try:
                r = subprocess.run(
                    [a.polar_probe, "param", expression],
                    env=env,
                    capture_output=True,
                    text=True,
                    timeout=10,
                ...
                raw = next(v[4:] for v in r.stdout.splitlines() if v.startswith("RAW "))
                assert len(raw) < 4096
                checked = []
                ...
                for aa in [-2, 0, 2] if scale is not None else [1]:
                    for rr, tt in [
                        ...
                        xx = s.simplify(rr * s.cos(tt))
                        yy = s.simplify(rr * s.sin(tt))
                        sub = {A: aa, X: xx, Y: yy}
                        degenerate = scale is not None and scale.subs(A, aa) == 0
                        expected = (
                            (xx == 0 and yy == 0)
                            if degenerate
                            else s.simplify(equation.subs(sub)) == 0
                        )
                        if expected and not degenerate and hole is not None:
                            expected = not (
                                s.sympify(hole[0]).subs(A, aa) == xx
                                and s.sympify(hole[1]).subs(A, aa) == yy
                            )
