...
def verify(case, printed):
    ...
    if ident in ("PC18-I1", "PC18-I2"):
        ...
        for sign in (-1, 1):
            ...
            for atom in branch.atoms(s.sign):
                ...
                branch = branch.xreplace(
                    {atom: s.Integer(sign if equal(atom.args[0], 1 - t) else -sign)}
                )
        ...
        for sign in (-1, 1):
            ...
            right = target.replace(
                lambda z: z.func == s.Abs, lambda z: sign * (u - 1)
            ).subs(s.sign(u - 1), sign)
            assert equal(left, right)
        method = (
            "On x>0 substitute u=ln(x). Both signs around u=1 give the exact derivative. "
        ...
        method = (
            "Original domain [-1,1], ordinary derivative on (-1,1), including zero. "
        ...
        method = (
            "On the original domain x>=0, x!=1, factor x-1=(t-1)(t+1). "
        ...
        if a.func == s.Piecewise:
            ...
            assert (
                isinstance(condition, s.Equality)
                and condition.rhs == 0
                and equal(condition.lhs, (x + y + z) ** 2)
            )
            a = regular
        assert equal(a, 1 + (x**8 + y**8 + z**8) / (x + y + z) ** 2)
        assert s.count_ops(a) < 256 and len(printed.encode()) <= 65536
        method = (
            "Column multilinearity proves det(I+vv^T)=1+v^Tv: every term using "
...
def actual_checks(case, expression, probe, env):
    ...
    for variables, point, expected in substitutions:
        ...
        if expected == "nonfinite":
            assert run.returncode in (0, 3) and value in (
                "undef",
                "infinity",
                "-infinity",
            ), row
            ...
            assert (
                parse(value).is_finite is True
                if expected == "finite"
                else equal(parse(value), parse(expected))
            ), row
        rows.append(row)
    return rows
