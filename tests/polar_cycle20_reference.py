...
def parse(text):
    ...
    def replace(text):
        while "piecewise(" in text:
            ...
            for k in range(0, len(parts) - 1, 2):
                ...
                condition = (
                    s.Eq(base_parse(bits[0]), base_parse(bits[1]), evaluate=False)
                    if len(bits) == 2
                    else base_parse(parts[k])
                )
                pairs.append((base_parse(parts[k + 1]), condition))
            name = "pc20_piece_" + str(len(values))
            values[name] = s.Piecewise(
                *pairs, (base_parse(parts[-1]), True), evaluate=False
            )
    ...
    try:
        return base_parse(replace(text))
    finally:
        for name in values:
            local.pop(name, None)
...
def regular(e):
    while e.has(s.Piecewise):
        e = e.replace(lambda z: z.func == s.Piecewise, lambda z: z.args[-1][0])
    return e


def verify(case, printed):
    ident = case["id"]
    assert not any(t in printed for t in ("integrate(", "diff(", "rootof("))
    ...
    if ident == "PC20-I1":
        assert a.has(s.Piecewise), (
            "Generic primitive misses a=+/-1 and real pole regimes"
        )
        ...
        for f in leaves:
            if f == s.Symbol("undef"):
                continue
            if f.has(s.atan):
                assert equal(s.diff(f, x), 1 / q)
                types.add("atan")
            elif f.has(s.log):
                for node in list(s.preorder_traversal(f)):
                    if node.func == s.log and node.args[0].func == s.Abs:
                        f = f.xreplace({node: s.log(node.args[0].args[0])})
                assert equal(s.diff(f, x), 1 / q)
                types.add("log")
            else:
                for v in (-1, 1):
                    assert equal(
                        s.diff(f.subs(parameter, v), x), 1 / q.subs(parameter, v)
                    )
                types.add("repeated")
        assert types == {"atan", "log", "repeated"}
        for v in (-1, 1):
            assert lazy(a, {parameter: v, x: 0}).is_finite is True
        ...
        constant = s.simplify((a - target).subs(x, 0))
        assert not constant.has(x) and equal(a, target + constant)
        # Endpoint proof uses the ORIGINAL primitive under x=cos(t).
        transformed = 2 * s.sqrt(2) * s.cos(t / 2) * (s.pi / 2 - t) + 4 * s.sqrt(
            2
        ) * s.sin(t / 2)
        assert s.limit(
            (transformed - s.pi * s.sqrt(2)) / (s.cos(t) - 1), t, 0, dir="+"
        ) == s.pi / (2 * s.sqrt(2))
        ...
        assert lazy(a, {x: 0}) == 1, (
            "Original quotient at accumulation point tends to 1, not 0 or 2"
        )
        assert equal(regular(a), 2 * x * s.floor(1 / x))
        n = s.Symbol("n", integer=True, nonzero=True)
        assert lazy(a, {x: 1 / n}) == s.Symbol("undef"), (
            "All reciprocal-integer jumps must be excluded"
        )
        ...
        for sign in (-1, 1):
            assert lazy(a, {x: 0, y: sign * t}) == s.Symbol("undef")
        ...
        if equal(a, source):
            return dict(
                exact=True,
                normalized=False,
                method="Direct precursor retains all original radicals. Original real domain is x>=|y|; explicit simplify must denest with the domain guard.",
            )
        ...
        assert (
            bad == s.Symbol("undef")
            and otherwise == True
            and isinstance(condition, s.StrictLessThan)
        )
...
def actual_checks(case, expression, probe, env):
    items = {
        "PC20-I1": [
            ...
            ("[x,a]", "[-1,1]", "undef"),
            ("[x,a]", "[1,-1]", "undef"),
            ("[x,a]", "[-2+sqrt(3),2]", "undef"),
        ],
        "PC20-I2": [("x", "1", "finite")],
        "PC20-D1": [("x", "0", "1")]
        + [("x", str(s.Rational(1, n)), "undef") for n in [-7, -2, -1, 1, 2, 7]]
        + [("x", "2/3", "4/3"), ("x", "-2/3", "8/3")],
        "PC20-D2": [
            ("[x,y]", "[0,0]", "0"),
            ("[x,y]", "[2,0]", "0"),
    ...
    for variables, point, expected in items.get(case["id"], []):
        ...
        if expected == "undef":
            ...
            assert parse(row["result"]).is_finite is True, row
        else:
            assert equal(parse(row["result"]), parse(expected)), row
        rows.append(row)
    return rows
