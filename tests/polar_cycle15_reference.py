...
def verify(case, printed):
    ...
    if ident == "PC15-I1":
        ...
        assert (
            equal(a, target)
            and s.trigsimp(s.diff(target, x) - 1 / (5 + 3 * s.cos(x)) ** 3) == 0
        )
        ...
        for point in [-1, 1]:
            assert a.subs(x, point).has(s.Symbol("undef"))
        regular = a
        ...
        y = local["y"]
        assert a == s.Abs(s.log(x / y)) - s.log(s.Abs(x) / s.Abs(y))
        assert (
            equal(a.subs({x: -1, y: -2}), 2 * s.log(2)) and a.subs({x: -2, y: -1}) == 0
        )
        ...
        assert all(("x^2-" + str(j * j) + ")") in printed for j in range(2, 14)), (
            "Canceled product domain lost"
        )
