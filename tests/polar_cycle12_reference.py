...
def verify(case, printed):
    ...
    if ident == "PC12-I1":
        ...
        f = s.expand_log(a.subs(x, u * u), force=True)
        ...
        for k in range(-3, 4):
            assert a.subs(x, k * s.pi) == (s.Symbol("undef") if k % 2 else 0)
        regular = a
        ...
        assert equal(
            regular, 2 * s.acos(s.cos(x)) * s.sin(x) / s.sqrt(1 - s.cos(x) ** 2)
        )
        ...
        y = local["y"]
        assert equal(a, s.Abs(x + y) + s.Abs(x - y) - 2 * s.Abs(x))
        ...
        assert (
            len(printed) < 65536 and len(a.atoms(s.log)) == 16 and s.count_ops(a) < 600
        )
        expected = sum(
            s.log(
                (s.sqrt(1 + (x + k) ** 2) + x + k)
                / (s.sqrt(1 + (x + k - 1) ** 2) + x + k - 1)
            )
            for k in range(1, 17)
        )
        assert a == expected
