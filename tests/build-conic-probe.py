...
from integration_build import ROOT, compiler_options, build, special_source

p = argparse.ArgumentParser()
p.add_argument("--build-dir", type=Path, required=True)
p.add_argument("--simplify-build-dir", type=Path)
p.add_argument("--probe-source", type=Path, default=ROOT / "tests/conic_probe.cc")
a = p.parse_args()
d = a.build_dir
d.mkdir(parents=True, exist_ok=True)
for name in ["equation_normalize.h", "parametric_display.h", "determinant_small.h"]:
...
special_source(d)
extra = [
    str(simplify_dir / name)
    for name in ("derivative.cc", "matrix.cc", "numeric_evalf.cc")
    if (simplify_dir / name).exists()
]
subprocess.run(
    flags
    + [
        str(d / "kconvert.cc"),
        str(a.probe_source),
        str(simplify_dir / "simplify.cc"),
        str(simplify_dir / "normalize.cc"),
        str(d / "special.cc"),
    ]
    + extra
    + libs
    + ["-pthread", "-o", str(d / "probe")],
    check=True,
)
print(d / "probe")
