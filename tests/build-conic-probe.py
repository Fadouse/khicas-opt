...
for name in ['equation_normalize.h','parametric_display.h','determinant_small.h']:(d/name).write_bytes((ROOT/name).read_bytes())
...
special_source(d)
extra=[str(simplify_dir/name) for name in ('derivative.cc','matrix.cc','numeric_evalf.cc') if (simplify_dir/name).exists()]
