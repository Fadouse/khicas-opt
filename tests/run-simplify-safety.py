...
p.add_argument(
    "--probe", type=Path, help="Reuse a probe built with target_simplify=True"
)
args = p.parse_args()
report = {
    "scope": "Repository integration/normalization and FXCG simplify entry points; other dependencies are host Giac. A guarded pthread stack excludes dynamic-loader startup; not CG50 memory or timing.",
...
with tempfile.TemporaryDirectory(prefix="khicas-simplify-safety-") as tmp:
    ...
    text += (
        function((source_path("ksubst.cc")).read_text(), "  gen tsimplify_noexpln(")
        .replace("v1[0]", "checked(v1,0)")
        .replace("v1[1]", "checked(v1,1)")
    )
    ...
    subprocess.run(
        flags + [str(d / "bounds.cc")] + libs + ["-o", str(d / "bounds")], check=True
    )
    ...
    cases = [
        ...
        (
            "closed-absolute-derivative",
            "simplify(diff(simplify(integrate(ln(abs(sin(x))),x)),x)-ln(abs(sin(x))))",
            0,
        ),
        ("elementary-log", "simplify(diff(integrate(ln(x),x),x)-ln(x))", 0),
        (
            "definite-log-sine",
            "simplify(integrate(3*ln(sin(2*x)),x,0,pi/2)+3*pi*ln(2)/2)",
            0,
        ),
        (
            "weighted-definite",
            "simplify(integrate(x*ln(sin(x)),x,0,pi)+pi^2*ln(2)/2)",
            0,
        ),
    ]
    for name, expr, status in cases:
        for stack in ("normal", "64"):
            ...
            result = subprocess.run(
                [str(exe), expr], capture_output=True, text=True, env=env, timeout=20
            )
            row = {
                "id": name,
                "input": expr,
            ...
            if status == 0:
                assert result.stdout.strip() == "0", row
            elif status == "closed":
                assert "integrate(" not in result.stdout and "Li2(" in result.stdout, (
                    row
                )
            else:
                assert "integrate(" in result.stdout, row
            print(name, stack, "PASS", flush=True)
print(
    "PASS: target simplification bounds, closed trigonometric-log integrals and guarded computation stack"
)
