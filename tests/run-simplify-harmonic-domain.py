...
local = {"x": x, "a": aa, "b": b, "i": sp.I, "ln": sp.log}
rows = []
expressions = [
    "sin(x)+cos(x)",
    "3*sin(2*x)+5*cos(2*x)+7",
    "sqrt(3)*sin(x)-sqrt(2)*cos(x)",
    "(sin(x)+cos(x))/2",
    "a*sin(x)+b*cos(x)",
    "sin(x)^2+cos(x)^2",
    "2*sin(x)*cos(x)",
...
for expression in expressions:
    for stack in ["normal", "64"]:
        ...
        r = subprocess.run(
            [str(a.probe), "simplify(" + expression + ")"],
            capture_output=True,
            text=True,
            env=env,
            timeout=5,
        ...
        want = sp.sympify(expression.replace("^", "**"), locals=local)
        assert sp.trigsimp(got - want) == 0, row
        for point in [0, sp.pi, -sp.pi, sp.pi / 2, -sp.pi / 2, 3 * sp.pi]:
...
a.report.write_text(
    json.dumps(
        {
            "scope": "Actual FXCG simplify, exact independent identities and exact values at potential half-angle poles. Nonlinear trig identities retained as regressions.",
            "source_sha256": {
