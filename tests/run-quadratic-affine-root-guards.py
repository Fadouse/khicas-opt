...
(a.build_dir / "equation_normalize.h").write_text(
    (source_path("equation_normalize.h")).read_text()
)
(a.build_dir / "guarded_probe_stack.h").write_text(
    (ROOT / "tests/guarded_probe_stack.h").read_text()
)
out = '#include "giacPCH.h"\n#include "equation_normalize.h"\n#include "guarded_probe_stack.h"\n#include <iostream>\nnamespace giac {\n'
out += function(source, "  void decompose_prod(") + function(
    source, "  gen extract_cst("
)
for name, typ in [
    ("integration_syntax", "gen"),
    ("integration_coefficient", "gen"),
    ("integration_power", "bool"),
    ("integration_square_root", "bool"),
    ("integration_resource_rational", "bool"),
    ("integrate_quadratic_affine_root", "bool"),
]:
    sig = "  static " + typ + " " + name + "("
...
contract = a.build_dir / "probe"
subprocess.run(
    flags + [str(a.build_dir / "probe.cc")] + libs + ["-o", str(contract)], check=True
)
...
dlog = ((2 * u + w) / (u * u + w * u + v) - (2 * u - w) / (u * u - w * u + v)) / (
    4 * v * w
)
datan = (2 * h / (h * h + (2 * u + w) ** 2) + 2 * h / (h * h + (2 * u - w) ** 2)) / (
    2 * v * h
)
proof = s.factor(
    (dlog + datan).subs(h * h, 4 * v - w * w) - 1 / ((u * u + v) ** 2 - w * w * u * u)
)
assert proof == 0
mp.mp.dps = 55
rows = []
...
def gs(e):
    return str(e).replace("**", "^").replace("log(", "ln(").replace("oo", "+infinity")
...
def run(exe, args, stack):
    ...
    return subprocess.run(
        [str(exe)] + args, env=env, capture_output=True, text=True, timeout=10
    )
...
def save():
    a.report.write_text(
        json.dumps(
            dict(
                scope="Exact rational primitive identity; exact production outputs against the derived family; independent 55-digit quadrature cross-check; domain/size/mode deferral contracts. Guarded host stack is not CG50 emulation.",
                proof=str(proof),
                source_sha256={
                    n: hashlib.sha256((source_path(n)).read_bytes()).hexdigest()
                    for n in ("yintg.cc", "equation_normalize.h")
                },
...
for A, B, C, k, b, c in [
    ...
    (2, 3, 4, 2, 3, 3),
    (3, -2, 2, -2, 3, -2),
    (1, 2, 2, 3, -2, s.Rational(2, 3)),
    (1, 0, 1, 1, -3, 1),
]:
    A, B, C, k, b, c = map(s.sympify, (A, B, C, k, b, c))
    Q = A * x * x + B * x + C
    L = k * x + b
    f = c / (Q * s.sqrt(L))
    pp = B * k / A - 2 * b
    qq = b * b - B * b * k / A + C * k * k / A
    vv = s.sqrt(qq)
    ww = s.sqrt(2 * vv - pp)
    hh = s.sqrt(2 * vv + pp)
    ...
    def F(U):
        ...
        if U == 0:
            return s.Integer(0)
        return s.log((U * U + ww * U + vv) / (U * U - ww * U + vv)) / (4 * vv * ww) + (
            s.atan((2 * U + ww) / hh) + s.atan((2 * U - ww) / hh)
        ) / (2 * vv * hh)
    ...
    for U, V in [
        ...
        (s.Integer(2), s.Integer(1)),
    ]:
        lo = (U * U - b) / k
        hi = (s.oo if k > 0 else -s.oo) if V == s.oo else (V * V - b) / k
        expected = 2 * k * c / A * (F(V) - F(U))
        expression = f"integrate({gs(f)},x,{gs(lo)},{gs(hi)})".replace(
            "-+infinity", "-infinity"
        )
        ...
        for stack in ("normal", "64"):
            for outer in (False, True):
                ...
                row = dict(
                    ...
                    quadrature=str(numeric),
                )
                try:
                    r = run(a.probe, [e], stack)
                    row.update(
                        exit=r.returncode, result=r.stdout.strip(), stderr=r.stderr
                    )
                    assert (
                        r.returncode == 0
                        and "Warning" not in r.stderr
                ...
                rows.append(row)
                save()
                print(row["kind"], stack, row["passed"], flush=True)
contracts = [
...
for e, lo, hi, accept, reason, mode in contracts:
    for stack in ("normal", "64"):
        ...
        rows.append(row)
        save()
        print(reason, stack, row["passed"], flush=True)
