...
for name in ("equation_normalize.h", "elliptic_first.h"):
...
for file, sig in [
    ("yintg.cc", "  static bool integrate_elliptic_quartic("),
    ("yderive.cc", "  static bool derive_minmax_contact("),
    ("yderive.cc", "  static bool derive_squared_affine_radical("),
    ("ksubst.cc", "  static bool simplify_minmax_clamp("),
]:
    src += function((source_path(file)).read_text(), sig)
src += """}
...
groups = {
    "elliptic": [
        ("1/sqrt(1-x^4)", True),
        ("1/sqrt(2-3*(x-1)^4)", False),
        ("1/sqrt(-3*x^4+12*x^3-18*x^2+12*x-1)", True),
        ("1/sqrt(1+x^2-x^4)", True),
        ("1/sqrt(1+x^4)", False),
        ("1/sqrt(x^4-1)", False),
        ("1/sqrt(-x^4)", False),
        ("1/sqrt(a-x^4)", False),
        ("1/sqrt(1+x-x^4)", False),
        ("1/sqrt(1-x^8)", False),
    ],
    "minmax": [
        ("max(x^2,a*x)", True),
    ...
    "radical": [
        ...
        ("sqrt(x^2*(x+1))", True),
        ("sqrt(x^3)", True),
        ("sqrt((2*x-1)^2*(3-2*x))", True),
        ("sqrt(x^2)", False),
        ("sqrt(x^4*(x-1))", False),
        ("sqrt(x^2*(x^2+1))", False),
        ("sqrt(x^2*(a*x+1))", False),
        ("sqrt(sin(x)^2*(x-1))", False),
    ],
    "clamp": [
        ("max(a,min(x,b))-min(b,max(x,a))", True),
        ("min(b,x)", False),
        ("max(a,min(x,b))+min(b,max(x,a))", False),
        ("max(a,min(x,b))-min(b,max(x,c))", False),
        ("max(a,min(sin(x),b))-min(b,max(sin(x),a))", False),
    ],
}
rows = []
    ...
    for expression, accept in cases + [(cases[0][0], False)]:
        ...
        rows.append(
            dict(
                ...
                and r.stdout.strip() == ("ACCEPT" if accept else "DEFER"),
            )
        )
mp.mp.dps = 70
for triple in [
    ...
    for scale in [1e-100, 1, 1e100]:
        args = [v * scale for v in triple]
        r = subprocess.run(
            [str(exe), "rf"] + [str(v) for v in args],
            capture_output=True,
            text=True,
            timeout=10,
        )
        row = dict(
            kind="rf", arguments=args, result=r.stdout.strip(), exit=r.returncode
        )
        try:
            assert r.returncode == 0 and r.stdout.startswith("ACCEPT ")
            actual = mp.mpf(r.stdout.split()[1])
            expected = mp.elliprf(*map(mp.mpf, args))
            assert abs(actual - expected) <= mp.mpf("2e-14") * abs(expected), (
                actual,
                expected,
            )
...
for args in [(-1, 1, 1), (0, 0, 1), (0, 0, 0), (math.inf, 1, 1), (math.nan, 1, 1)]:
    ...
    rows.append(
        dict(
            ...
            result=r.stdout.strip(),
            exit=r.returncode,
            passed=r.returncode == 0 and r.stdout.startswith("DEFER"),
        )
    )
a.report.write_text(
    json.dumps(
        dict(
            scope="Extracted production helpers: acceptance/defer is not a guarantee about legacy fallback. RF uses actual production double arithmetic independently of gen tagged-number storage; 70-digit independent references and invalid-domain rejection.",
            source_sha256={
