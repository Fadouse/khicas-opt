...
def checked_status(process, computed=False):
    ...
    status = (
        ...
        if checks
            ...
            process.returncode,
            "computed" if computed and process.returncode == 0 else "error",
        )
    )
...
parser.add_argument(
    "--baseline-ref", default=BASE, help="Git reference used with --compare"
)
parser.add_argument(
    "--corpus",
    type=Path,
    default=ROOT / "tests/calculus-corpus.json",
...
parser.add_argument(
    "--target-simplify",
    action="store_true",
    help="Include repository FXCG simplification entry points",
)
...
if args.target_simplify:
    ...
    report["validation_scope"] = (
        "separate host Giac probe validates the printed target result; no repository integration, normalization or FXCG simplify linked"
    )
...
with tempfile.TemporaryDirectory(prefix="khicas-calculus-") as tmp:
    ...
    for ref in ("baseline", "current") if args.compare else ("current",):
        exe = build(
            Path(tmp) / ref,
            "current" if ref == "current" else args.baseline_ref,
        ...
        for case in cases:
            ...
            try:
                ...
                row = {
                    ...
                    "status": status,
                    "exit": p.returncode,
                    "result": p.stdout.strip()[:4096],
                    "result_characters": len(p.stdout.strip()),
                    "result_sha256": hashlib.sha256(
                        p.stdout.strip().encode()
                    ).hexdigest(),
                ...
                if validator and status == "computed":
                    ...
                    try:
                        ...
                        validated = subprocess.run(
                            [str(validator), p.stdout.strip()] + verification,
                            capture_output=True,
                            text=True,
                        ...
                        validation = {
                            ...
                            "status": checked_status(validated),
                            "result": validated.stdout.strip()[:4096],
                            "stderr": validated.stderr[-2500:],
                        }
                        measured = re.search(
                            r"^SECONDS ([\d.e+-]+)", validated.stderr, re.M
                        )
                        if measured:
                            validation["result_parse_eval_seconds"] = float(measured[1])
                ...
                status = (
                    "validation_timeout"
                    if not validator and re.search(r"^SECONDS ", stderr, re.M)
                    else "integration_timeout"
                )
                row = {
                    "id": case["id"],
                    "status": status,
            ...
            rows.append(row)
            print(
                ref, row["id"], row["status"], f"{row['wall_seconds']:.3f}s", flush=True
            )
...
if args.strict and any(row["status"] != "exact" for row in report["runs"]["current"]):
    raise SystemExit(1)
if args.require_solved and any(
    row["status"] not in ("exact", "sampled", "numeric_constant")
