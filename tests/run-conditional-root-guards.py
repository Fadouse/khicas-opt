...
checks = [
    ("when(x==0,0,1/x)", "((x=0)? 0 : 1/x)"),
    ("piecewise(x==0,0,1/x)", "piecewise(x=0,0,1/x)"),
    ("when(x==0,0,1/x,7)", "7"),
    ("when(0==0,3,1/0)", "3"),
    ("piecewise(1==0,1/0,2==2,7,1/0)", "7"),
    ("x==0", "false"),
    ("when(x==x,7,1/0)", "7"),
]
...
for stack in ["normal", "64"]:
    for e, expected in checks:
        out = run(e, stack)
        assert out == expected, (e, out, expected)
        ...
        for outer in [False, True]:
            ...
            rows.append(
                dict(
                    ...
                    result=out,
                    exact=True,
                    method="Exact real-root substitution and both local logarithm signs",
                )
            )
a.report.write_text(
    json.dumps(
        dict(
            ...
            scope="Actual scalar probe; condition substitution is lazy, odd-root integrals independently differentiated in a real bijective variable.",
            runs=rows,
        ),
        indent=2,
    )
