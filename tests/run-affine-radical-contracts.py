...
cases = [
    ...
    (
        "(4*(3*x+4)-9*(2-x))/(2*sqrt(3*x+4)+3*sqrt(2-x))",
        True,
        "weighted positive roots",
    ),
    (
        "(4*(3*x+4)-9*(2-x))/(-2*sqrt(3*x+4)-3*sqrt(2-x))",
        True,
        "weighted negative roots",
    ),
    ...
    json.dumps(
        dict(
            ...
            source_sha256={
                ...
                for name in ("yintg.cc", "equation_normalize.h")
            },
            runs=rows,
        ),
        indent=2,
