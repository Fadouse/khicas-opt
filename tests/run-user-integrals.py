...
def run_probe(command, phase):
    try:
        ...
        result = {
            ...
            "status": checks[-1]
            if checks
            else {2: "unevaluated", 3: "undefined"}.get(
                r.returncode,
                "computed" if phase == "integration" and r.returncode == 0 else "error",
            ),
            "result": r.stdout.strip(),
            "stderr": r.stderr,
        }
        if r.returncode and result["status"] == "exact":
            result["status"] = "error"
        ...
        timed_out = (
            "validation"
            if phase == "combined" and re.search(r"^SECONDS ", stderr, re.M)
            else phase
        )
        if timed_out == "combined":
            timed_out = "integration"
        result = {
            "status": timed_out + "_timeout",
            "result": output_text(error.stdout).strip(),
            "stderr": stderr,
        }
    measured = re.search(r"^SECONDS ([\d.e+-]+)", result["stderr"], re.M)
    if measured:
        result["seconds"] = float(measured[1])
    return result
...
report = {
    ...
    "timeout_seconds": 30,
    "virtual_address_limit_MiB": 768,
    "cases": [],
}
report["stack_test"] = (
    "64 KiB guarded pthread computation stack; process/library startup uses normal stack"
)
report["validation_scope"] = (
    "same probe process; host Giac outer simplify and repository normalization"
)
if args.target_simplify:
    report["scope"] = (
        "actual yintg, zintgab, normalization and FXCG simplify entry points; other dependencies host Giac; not CG50 timings"
    )
    report["validation_scope"] = (
        "separate host Giac probe; validates the printed target result, with no repository integration, normalization or FXCG simplify linked"
    )
    report["validation_timeout_seconds"] = 30
    report["source_sha256"]["ksubst.cc"] = hashlib.sha256(
        (source_path("ksubst.cc")).read_bytes()
    ).hexdigest()
    if (source_path("equation_normalize.h")).exists():
        report["source_sha256"]["equation_normalize.h"] = hashlib.sha256(
...
with tempfile.TemporaryDirectory(prefix="khicas-user-integrals-") as tmp:
    ...
    validator = (
        build_validation_probe(Path(tmp) / "validation")
        if args.target_simplify
        else None
    )
    for case in cases:
        ...
        for _ in range(args.repeats):
            verification = [case["reference"], case["mode"], case["integrand"]] + case[
                "samples"
            ]
            if validator:
                ...
                if result["status"] == "computed":
                    validation = run_probe(
                        [str(validator), result["result"]] + verification, "validation"
                    )
                    if "seconds" in validation:
                        validation["result_parse_eval_seconds"] = validation.pop(
                            "seconds"
                        )
        ...
        if ts:
            row["median_host_seconds"] = statistics.median(ts)
        try:
            r = subprocess.run(
                [str(exe), case["input"]],
                capture_output=True,
            ...
            row["stack_64KiB"] = {
                ...
                "result": r.stdout.strip(),
                "stderr": r.stderr,
                "same_as_normal_stack": r.stdout.strip()
                == row["runs"][0].get("result"),
            }
            if r.returncode or not row["stack_64KiB"]["same_as_normal_stack"]:
                failed = True
        except subprocess.TimeoutExpired as error:
            row["stack_64KiB"] = {
                "status": "integration_timeout",
                "result": output_text(error.stdout).strip(),
                "stderr": output_text(error.stderr),
            }
            failed = True
        ...
        print(
            case["id"],
            row["runs"][0]["status"],
            row.get("median_host_seconds"),
            row["stack_64KiB"].get("exit"),
            flush=True,
        )
if failed:
    raise SystemExit(1)
print(
    "PASS: all eight exact; all eight complete with identical results under a 64 KiB stack"
)
