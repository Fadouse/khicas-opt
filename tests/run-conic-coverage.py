...
a = p.parse_args()
cp = ROOT / "tests/conic-corpus.json"
...
for c in cases:
    for outer in [False, True]:
        for stack in ["normal", "64"]:
            ...
            args = [str(a.probe), expression] + ([c["setup"]] if "setup" in c else [])
            ...
            try:
                ...
                key = (c["id"], raw)
                if key not in cache:
                    cache[key] = check(c, raw)
                row["verification"] = cache[key]
                if c["branches"]:
                    view = fields["VIEW"]
                    assert (
                        view.count("x(" + c["parameter"] + ")=") == c["branches"]
                        and view.count("y(" + c["parameter"] + ")=") == c["branches"]
                    ), view
                    row["label_nodes"] = int(fields["NODES"].split()[1])
                    row["within_renderer_budget"] = row["label_nodes"] < 256
                row["pass"] = True
            ...
            a.report.write_text(
                json.dumps(
                    {
                        "scope": "Actual conic conversion and repository FXCG simplify. Independent exact residual and full real-chart coverage certificates, not only sample substitution. Display labels inspected; host stack is not CG50 MMU emulation.",
                        "source_sha256": {
