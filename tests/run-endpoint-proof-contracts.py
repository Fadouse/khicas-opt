...
for signature in [
    ...
    "  static bool integration_affine_atom(",
    "  static bool integration_log_endpoint(",
]:
...
cases = [
    ("x*ln(x)", 0, 1, "0"),
    ("ln(x)-ln(x)/(1+x)-ln(1+x)", 0, 1, "0"),
    ("-ln(x)", 0, 1, "+infinity"),
    ("ln(x)", 0, 1, "-infinity"),
    ("ln(x)+1/x", 0, 1, None),
    ("ln(x)-1/x", 0, 1, None),
    ("ln(x)-ln(2*x)", 0, 1, "-ln(2)"),
    ("ln(abs(x))-ln(abs(-3*x))", 0, -1, "-ln(3)"),
    ("ln(abs(x))-ln(abs(-3*x))", 0, 1, "-ln(3)"),
    ("ln(-x)-ln(x)", 0, 1, None),
    ("ln(x)*ln(1+x)", 0, 1, None),
    ("x*ln(-x)", 0, -1, "0"),
    ("x*ln(-x)", 0, 1, None),
    ("x*ln(x^2)", 0, 1, None),
    ("x*ln(a*x)", 0, 1, None),
    ("sin(1/x)*ln(x)", 0, 1, None),
    ("x^2*ln(x)/(1+x^2)", 0, 1, "0"),
    ("ln(x)/(x-1)", 0, 1, "+infinity"),
    ("ln(x)/ln(2)", 0, 1, None),
    ("ln(x)", 1, 1, "0"),
    ("ln(x)", -1, 1, None),
    ("ln(x)", 0, 0, None),
    ("ln(x)+ln(x+1)/x", 0, 1, None),
]
...
for zero in (-2, 0, 3):
    for scale in (-5, 2):
        for direction in (-1, 1):
            cases.append(
                (
                    f"ln(abs(x-({zero})))-ln(abs({scale}*(x-({zero}))))",
                    zero,
                    direction,
                    f"-ln({abs(scale)})",
                )
...
for expression, point, direction, expected in cases:
    r = subprocess.run(
        [str(exe), expression, str(point), str(direction)],
        capture_output=True,
        text=True,
        timeout=5,
    )
    ...
    try:
        ...
        if expected is None:
            assert r.stdout.strip() == "DEFER"
        elif "infinity" in expected:
            assert r.stdout.strip() in [
                "ACCEPT " + expected,
                "ACCEPT " + expected.lstrip("+"),
            ]
        else:
            assert r.stdout.startswith("ACCEPT ") and equal(
                parse(r.stdout[7:].strip()), parse(expected)
            )
        row["passed"] = True
...
a.report.write_text(
    json.dumps(
        dict(
            scope="Direct contract checks of actual endpoint proof helper; DEFER must preserve the generic fallback, not return a guessed finite value.",
