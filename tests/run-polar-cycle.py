  ...
  for stack in ['normal','64']:
   ...
   try:
    if saved:
     ...
     r=subprocess.run([str(a.probe),expression],capture_output=True,text=True,env=env,timeout=10 if c['id'] in ('PC12-R1','PC13-R1','PC14-R1','PC15-R1','PC16-R1') else 12)
    ...
    if points:
     for point in points:
      r=subprocess.run([str(a.probe),'eval(subst('+expression+',x='+str(point)+'))'],capture_output=True,text=True,env=env,timeout=10 if c['id'] in ('PC12-R1','PC13-R1','PC14-R1','PC15-R1','PC16-R1') else 12)
    ...
    if c['id']=='PC10-R1':
     r=subprocess.run([str(a.probe),'eval(subst('+expression+',[x,y],[0,0]))'],capture_output=True,text=True,env=env,timeout=10 if c['id'] in ('PC12-R1','PC13-R1','PC14-R1','PC15-R1','PC16-R1') else 12)
