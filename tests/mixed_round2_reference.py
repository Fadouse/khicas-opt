...
def verify(case, printed):
    ...
    if id == "MR2-I2":
        ...
        return verify_trig_log_reference(
            dict(
                id=id,
                expected=expected,
                f="ln(-3*sin(2*x/3+pi/7))",
                frequency="2/3",
                phase="pi/7",
                inside_coefficient=-3,
                cosine=False,
            ),
            printed,
        )
    if id in ["MR2-I1", "MR2-I3", "MR2-I4"]:
        ...
        assert equal(canonical_dilog(actual, expected), expected)
        if id == "MR2-I1":
            # Both real log magnitudes are positive on every intersection
            # interval. Each independent H phase has its own chain factor.
            u = 3 * x / 4 + sp.pi / 8
            v = x / 2 + 7 * sp.pi / 18
            assert sp.diff(u, x) == sp.Rational(3, 4) and sp.diff(v, x) == sp.Rational(
                1, 2
            )
            assert sp.trigsimp(sp.sin(v) - sp.cos(x / 2 - sp.pi / 9)) == 0
            return {
                "exact": True,
                "method": "canonical two-phase Li2 primitive; exact Clausen derivative and positive-log scale identities on all sign intersections",
            }
        derivative = sp.diff(expected, x).replace(
            lambda z: z.func == sp.polylog and z.args[0] == 1,
            lambda z: -sp.log(1 - z.args[1]),
        )
        if id == "MR2-I3":
            w = x * x + x
            target = (2 * x + 1) * sp.log(1 + w * w) / w
            assert (
                equal(derivative, target)
                and sp.simplify(derivative.subs(x, -sp.Rational(1, 2))) == 0
            )
        else:
        ...
        return {
            "exact": True,
            "method": "canonical primitive and exact real-domain differentiation, including stationary substitution point",
        }
    if id == "MR2-D1":
        ...
        assert equal(
            sp.expand_log(regular, force=True), sp.expand_log(expected, force=True)
        )
        assert sp.simplify(actual.subs(x, 0)) == 0
        return {
            "exact": True,
            "method": "positive magnitude logarithm identity; exact nonlinear phase chain rule; regular value at zero",
        }
    if id in ["MR2-D2", "MR2-D3"]:
        zeros = [0, 1] if id == "MR2-D2" else [0]
        target = (
            -2 * (2 * x - 1) * sp.log(1 + x * x * (x - 1) ** 2) / (x * (x - 1))
            if id == "MR2-D2"
            else 2 * sp.log(1 + x * x) / (x * (1 + x * x))
        )
        ...
        if id == "MR2-D2":
            ...
            assert equal(regular.subs(x, -x), -regular)
        return {
            "exact": True,
            "method": "complete polynomial zero locus, exact branch formulas, all removable values/limits and stationary point or parity",
        }
    assert id == "MR2-D4"
    expected = parse(case["expected"]["expression"])
    assert equal(actual, expected) and actual.subs(x, 0) == 2
    ...
    assert (
        sp.solveset(sp.denom(sp.cancel(actual)), x, domain=sp.S.Reals) == sp.S.EmptySet
    )
    return {
        "exact": True,
        "method": "exact positive-radical algebra and trig identity; real denominator has no roots",
    }
