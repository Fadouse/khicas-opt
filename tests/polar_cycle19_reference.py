...
def verify(case, printed):
    ...
    if ident == "PC19-P1":
        ...
        lhs, rhs = raw.split("=")
        a = parse(lhs) - parse(rhs)
        r, theta = s.symbols("r theta")
        target = r * s.sin(theta) * (r * r - 4 * r * s.cos(theta) + 3)
        assert equal(s.expand_trig(a), s.expand_trig(target))
        return dict(
            exact=True,
            method="Polynomial/trigonometric identity without division. The full r=0 component, arbitrary nonnegative r on sin(theta)=0, both circle branches and their tangencies are retained.",
        )
    a = parse(printed)
    ...
    if ident == "PC19-I1":
        ...
        for sign in (-1, 1):
            ...
            branches.append(b)
        left = s.limit(branches[0], t, 1, dir="-")
        right = s.limit(branches[1], t, 1, dir="+")
        assert equal(left, right), (
            "Primitive branches need one common constant at the included zero"
        )
        assert equal(a.subs(x, 0), left), (
            "Assigned primitive value differs from the common limit"
        )
        ...
        assert (
            s.simplify(s.trigsimp(s.diff(F, u) - u * s.sinh(u) / s.cosh(u) ** 2)) == 0
        )
        ...
        for sign in (-1, 1):
            ...
            target = t ** (2 + s.sin(sign * t)) * (
                s.cos(sign * t) * s.log(t) + (2 + s.sin(sign * t)) / (sign * t)
            )
            assert equal(b, target)
        assert s.simplify(a.subs(x, 0)) == 0, (
            "Original function is differentiable at zero"
        )
        ...
        assert equal(s.simplify(a.subs(x, -t)), 0), (
            "Negative half-line is a constant function"
        )
        ...
        if a.has(s.atan):
            y = s.Symbol("y")
            assert equal(a, s.atan(x) + s.atan(y) - s.atan((x + y) / (1 - x * y)))
            return dict(
                exact=True,
                normalized=False,
                method="Direct arithmetic precursor retains the exact original expression and domain; explicit simplify still must normalize its branches.",
            )
        ...
        assert (
            bad == s.Symbol("undef")
            and otherwise == True
            and isinstance(condition, s.Equality)
        )
        ...
        assert (
            zero == 0
            and otherwise == True
            and isinstance(positive, s.StrictGreaterThan)
        )
        assert equal(positive.lhs - positive.rhs, d) and equal(
            negative, s.pi * s.sign(x + y)
        )
...
def actual_checks(case, expression, probe, env):
    ...
    for variables, point, expected in cases.get(case["id"], []):
        ...
        if expected == "undef":
            assert run.returncode == 3 and row["result"] == "undef", row
        else:
            assert run.returncode == 0, row
            value = parse(row["result"])
            if expected == "finite":
                assert value.is_finite is True, row
                ...
                assert equal(value, parse(expected)), row
        rows.append(row)
    return rows
