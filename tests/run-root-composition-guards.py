...
for stack in ["normal", "64"]:
    for n, m, k, power, A, B, c, d, C in [
        ...
        expected = (
            c * s.diff(C * u**m / (A + B * u**k) ** power, u) / (n * u ** (n - 1))
        )
        for outer in [False, True]:
            e = "diff(" + f + ",x)"
            e = "simplify(" + e + ")" if outer else e
            out = run(e, stack)
            v = parse(out).xreplace({Root(c * x + d, n): u}).subs(x, (u**n - d) / c)
            assert equal(v, expected), (e, out, v, expected)
            point = s.Rational(-d, c)
            value = s.Rational(C * c, A**power) if m == n else s.Integer(0)
            ...
            rows.append(
                dict(
                    ...
                    result=out,
                    endpoint=str(point),
                    endpoint_value=actual,
                    exact=True,
                    method="Exact rational derivative in odd-root variable; at moving zero F(h)/h tends to C*c/A^p if m=n and 0 if m>n. Original denominator zeros remain excluded.",
                )
            )
    for sign in [-1, 1]:
        e = f"sqrt((ln(x)+{sign}*i*pi)^2)-ln(x)-{sign}*i*pi"
        for outer in [False, True]:
            expr = "simplify(" + e + ")" if outer else e
            out = run(expr, stack)
            ...
            rows.append(
                dict(
                    ...
                    exact=True,
                    method="Principal-root syntax preserved; sqrt(-pi²)=i*pi at x=1, with both upper/lower imaginary offsets checked.",
                )
            )
a.report.write_text(
    json.dumps(
