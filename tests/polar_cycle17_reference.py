...
def verify(case, printed):
    ...
    if ident == "PC17-I1":
        ...
        assert equal(a, target) and equal(
            s.diff(target, x), (s.sqrt(1 + x) - s.sqrt(1 - x)) / 2
        )
        ...
        u = x / 2 - s.pi / 4
        target = s.Abs(s.cos(u)) / s.Abs(s.sin(u)) - (1 + s.sin(x)) / s.cos(x)
        a = a.replace(
            lambda z: z.func in (s.sin, s.cos), lambda z: z.func(s.expand(z.args[0]))
        )
        ...
        assert len(printed) < 65536 and s.count_ops(a) < 256, (
            "Unnecessary 1024-monomial expansion violates compact-result requirement"
        )
