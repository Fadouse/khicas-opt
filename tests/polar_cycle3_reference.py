...
def verify(case, printed):
    ...
    if ident == "PC3-D2":
        ...
        return dict(
            exact=True,
            method="Exact nonzero branch; original difference quotient h*sin(1/h) is bounded by |h| and tends to zero. Lazy zero branch is retained; derivative continuity is neither required nor true.",
        )
    a = parse(printed)
    if ident == "PC3-I1":
        ...
        expected = (1 + x * x) ** s.sin(x) * (
            s.cos(x) * s.log(1 + x * x) + 2 * x * s.sin(x) / (1 + x * x)
        )
        ...
        base = 1 + x * x
        canonical = {
            p: s.exp(s.log(base) * s.sin(x)) / base
        ...
        if not equal(a, retained):
            ...
            assert s.rem(
                s.Poly(s.expand(numerator), u), s.Poly(u * u - x * x - 1, u)
            ).is_zero
