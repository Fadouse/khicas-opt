...
def parse(text):
    overrides = {
        ...
        "max": s.Max,
        "min": s.Min,
        "EllipticF": s.elliptic_f,
        "Gamma": s.gamma,
    }
    old = dict(local)
    try:
        local.update(overrides)
        return base_parse(text)
    finally:
        local.clear()
        local.update(old)
...
def polar_tree(text):
    ...
    for i, c in enumerate(text):
        ...
        if depth == 0 and c == "?":
            q = i
        if depth == 0 and c == ":" and q is not None:
            colon = i
            break
    if q is not None:
        return (
            "when",
            polar_tree(text[:q]),
            polar_tree(text[q + 1 : colon]),
            polar_tree(text[colon + 1 :]),
        )
    depth = 0
    for i, c in enumerate(text):
        depth += (c == "(") - (c == ")")
        if depth == 0 and c == "=":
            return s.Eq(parse(text[:i]), parse(text[i + 1 :]), evaluate=False)
    return parse(text)


def verify(case, printed):
    ident = case["id"]
    assert not any(v in printed for v in ("integrate(", "diff(", "rootof("))
    z = s.Symbol("z", positive=True)
    if ident == "PC21-P1":
        raw = next(v[4:] for v in printed.splitlines() if v.startswith("RAW "))
        tree = polar_tree(raw)
        assert (
            tree[0] == "when"
            and tree[1] == s.Eq(A, 0, evaluate=False)
            and tree[2] == s.Eq(R, 0, evaluate=False)
        )
        gate = tree[3]
        assert gate[0] == "when" and gate[2] == s.Symbol("undef")
        assert equal(gate[1].lhs - gate[1].rhs, A * (R * s.sin(T) - A))
        chart = gate[3]
        assert (
            chart[0] == "when"
            and chart[1] == s.Eq(R, 0, evaluate=False)
            and chart[2] == s.Eq(R, 0, evaluate=False)
        )
        ...
        return dict(
            exact=True,
            method="Exact circle identity and inverse t=x/(a-y). On the circle a-y=0 is exactly the missing finite-parameter point (0,a). At a=0 the map is identically the origin; r=0 remains included at every angle. All statements use the specified r>=0 polar convention.",
        )
    value = parse(printed)
    if ident == "PC21-I1":
        for sub, target in [
            (-z, s.Rational(1, 2) + z),
            (z / (1 + z), z * z / (1 + z) ** 2 - z / (1 + z) + s.Rational(1, 2)),
            (1 + z, z + s.Rational(1, 2)),
        ]:
            assert equal(value.subs(A, sub), target)
        assert value.subs(A, 0) == s.Rational(1, 2) and value.subs(A, 1) == s.Rational(
            1, 2
        )
        ...
        assert (
            isinstance(cond, s.Equality)
            and equal(cond.lhs - cond.rhs, x * x - A * x)
            and otherwise == True
        )
        assert (
            contact.func == s.Piecewise
            and contact.args[0][0] == 2 * x
            and contact.args[1][0] == s.Symbol("undef")
        )
        assert equal(contact.args[0][1].lhs - contact.args[0][1].rhs, 2 * x - A)
        assert (
            regular.func == s.Piecewise
            and regular.args[0][0] == 2 * x
            and regular.args[1][0] == A
        )
        assert isinstance(regular.args[0][1], s.StrictGreaterThan) and equal(
            regular.args[0][1].lhs - regular.args[0][1].rhs, x * x - A * x
        )
        ...
        assert value.has(s.Piecewise), (
            "A real derivative must not acquire values outside the original real domain"
        )
        ...
        if value == s.Max(A, s.Min(x, B)) - s.Min(B, s.Max(x, A)):
            return dict(
                exact=True,
                normalized=False,
                method="Original expression retained; explicit simplify must eliminate x.",
            )
...
def actual_checks(case, expression, probe, env):
    items = {
        ...
        "PC21-I2": [
            ("x", "0", "0"),
            ("x", "1", "sqrt(pi)*Gamma(1/4)/(4*Gamma(3/4))"),
            ("x", "-1", "-sqrt(pi)*Gamma(1/4)/(4*Gamma(3/4))"),
        ],
        "PC21-D1": [
            ("[x,a]", "[0,0]", "0"),
        ...
        "PC21-D2": [("x", q, "undef") for q in ["-1", "0", "1/2", "2/3", "1"]]
        + [("x", "2", "2"), ("x", "5", "13/4")],
        "PC21-S1": [
            ("[x,a,b]", str([xx, aa, bb]), str(max(aa - bb, 0)))
            for xx in [-3, 0, 2, 5]
            for aa, bb in [(1, 3), (3, 1), (2, 2)]
        ],
    }
    rows = []
    for variables, point, expected in items.get(case["id"], []):
        r = subprocess.run(
            [str(probe), f"eval(subst({expression},{variables},{point}))"],
            env=env,
            capture_output=True,
            text=True,
            timeout=10,
        ...
        row = dict(
            point=point, expected=expected, exit=r.returncode, result=r.stdout.strip()
        )
        assert r.returncode == (3 if expected == "undef" else 0), row
        if expected == "undef":
            assert row["result"] == "undef", row
        else:
            actual, target = parse(row["result"]), parse(expected)
            if case["id"] == "PC21-I2":
                # Euler reflection at 1/4: Gamma(1/4)Gamma(3/4)=pi*sqrt(2).
                reflection = {
                    s.gamma(s.Rational(3, 4)): s.pi
                    * s.sqrt(2)
                    / s.gamma(s.Rational(1, 4))
                }
                actual = actual.xreplace(reflection)
                target = target.xreplace(reflection)
