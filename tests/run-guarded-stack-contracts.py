...
subprocess.run(
    flags + ["-O0", "-I" + str(ROOT / "tests"), str(src), "-o", str(exe)], check=True
)
rows = []
for stack, depth, expected in [
    (None, 64, 0),
    ("64", 2, 0),
    ("64", 64, -11),
    ("128", 2, 0),
    ...
    r = subprocess.run(
        [str(exe), str(depth)], capture_output=True, text=True, env=env, timeout=5
    )
    ok = r.returncode == expected
    if expected == 0:
        ok = ok and r.stdout.splitlines() == ["LOCAL_DESTRUCTOR", "GLOBAL_DESTRUCTOR"]
    rows.append(
        dict(
            stack=stack,
            depth=depth,
...
a.report.write_text(
    json.dumps(
        dict(
            scope="Host stack harness only: guarded overflow, normal local/global destructors and invalid input. SIGSEGV is required for deliberate overflow, never counted as successful computation.",
            source_sha256=hashlib.sha256(
                (ROOT / "tests/guarded_probe_stack.h").read_bytes()
            ).hexdigest(),
            runs=rows,
        ),
        indent=2,
