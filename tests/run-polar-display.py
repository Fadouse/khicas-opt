...
cases = [
    ...
    (
        "ellipse",
        "cart",
        "(x^2/4+y^2/9=1,[x,y],[r,theta])",
        [6 / s.sqrt(9 * c * c + 4 * sn * sn)],
    ),
    ...
    (
        "shifted-circle",
        "cart",
        "((x-2)^2+y^2=1,[x,y],[r,theta])",
        [(4 * c + s.sqrt(16 * c * c - 12)) / 2, (4 * c - s.sqrt(16 * c * c - 12)) / 2],
    ),
    ...
    for outer in [False, True]:
        for stack in ["normal", "64"]:
            ...
            try:
                ...
                fields = dict(
                    line.split(" ", 1) for line in run.stdout.strip().splitlines()
                )
                assert int(fields["NODES"]) <= 128
                if expected is not None:
                    actual = radius(fields["RAW"])
                    assert len(actual) == len(expected)
                    for aa, bb in zip(actual, expected):
                        assert s.trigsimp(aa - bb) == 0, (aa, bb)
                else:
                    left, right = fields["RAW"].split("=", 1)
                    residual = parse(left) - parse(right)
                    if name == "origin-line":
                        assert (
                            s.trigsimp(residual.subs(r, 0)) == 0
                            and s.trigsimp(residual.subs(t, s.pi / 4)) == 0
                        )
                    elif name == "unknown-factor":
                        assert s.simplify(residual.subs(s.Symbol("a"), 0)) == 0
                    elif name == "degenerate-quadratic":
                        assert s.trigsimp(residual.subs(t, s.pi / 2)) == 0
                    else:
