...
for Q, points in [
    ...
    for c in (1, 2):
        for sign in ("+", "-"):
            for n in (1, 2, 3):
                cases.append((f"1/({c}{sign}sqrt({Q}))^{n}", points))
for den, points in [
    ("sqrt(1+x)+sqrt(1-x)", ["-1", "0", "1"]),
    ("sqrt(2)+sqrt(x)", ["0", "2"]),
    ("sqrt(2)-sqrt(x)", ["0", "2"]),
    ("(1+sqrt(1+x^2))*(2-sqrt(1+x^2))", ["0", "sqrt(3)"]),
]:
    for n in (1, 2):
        cases.append((f"1/({den})^{n}", points))
rows = []
...
a.report.write_text(
    json.dumps(
        dict(
            scope="56 radical denominator families, normal/64 KiB main-thread guarded stack and once/twice simplify. Exact structural retention proves the same radical/denominator domain; actual substitutions separately test conjugate zeros, true poles and root endpoints. Retention is not claimed as canonical simplification.",
            probe_sha256=hashlib.sha256(Path(a.probe).read_bytes()).hexdigest(),
            source_sha256={
                n: hashlib.sha256(source_path(n).read_bytes()).hexdigest()
                for n in ("ksubst.cc", "zusual.cc")
            },
            runs=rows,
        ),
        indent=2,
