...
def run(probe, expression, stack, outer=False):
    ...
    r = subprocess.run(
        [str(probe), expression], env=env, capture_output=True, text=True, timeout=12
    )
    ...
    for outer in [False, True]:
        for stack in ["normal", "64"]:
            ...
            if branches:
                ...
                assert (
                    fields["VIEW"].count("x(t)=") == branches
                    and fields["VIEW"].count("y(t)=") == branches
                )
            rows.append(
                dict(
                    id=id,
...
for rhs in ["0", "1"]:
    ...
    for stack in ["normal", "64"]:
        ...
        rows.append(
            dict(
                id="unknown-pivot-" + rhs,
                stack=stack,
                result=after,
                status="unchanged-fallback",
                display_limit="May exceed 256 nodes; unchanged limitation, not counted as a displayed success.",
            )
        )
...
for n in [3, 4, 5, 7]:
    ...
    for stack in ["normal", "64"]:
        for equation in [False, True]:
            result, err = run(
                a.probe, "simplify(" + ("r=" if equation else "") + expr + ")", stack
            )
            if equation:
            ...
            u, v = sp.symbols("u v")
            old = expected.xreplace({sp.sin(x): u, sp.cos(x): v})
            new = actual.xreplace({sp.sin(x): u, sp.cos(x): v})
            ...
            rows.append(
                dict(
                    id="trig-denominator-" + str(n),
                    stack=stack,
                    equation=equation,
                    result=result,
                    status="exact",
                    verification="rational identity, same denominator zero set, no expression growth",
                )
            )

for phase in ["x", "x^2+pi/4", "-3*x+pi/7", "x^3+pi/8"]:
    u = parse(phase)
    expected = -sp.diff(u, x) * sp.log(2 * sp.Abs(sp.sin(u / 2)))
    expr = "diff(im(Li2(exp(i*(" + phase + ")))),x)"
    for stack in ["normal", "64"]:
        for outer in [False, True]:
            result, err = run(
                a.probe, "simplify(" + expr + ")" if outer else expr, stack
            )
            actual = parse(result)
            assert equal(
                sp.expand_log(actual, force=True), sp.expand_log(expected, force=True)
            )
            ...
            rows.append(
                dict(
                    ...
                    result=result,
                    status="exact",
                    verification="real phase derivative on sine-nonzero intervals; no eager-abs PARI warnings",
                )
            )
        for point in [-2, 0, 2]:
            if phase == "x" and point == 0:
                continue
            result, err = run(
                a.probe, "evalf(subst(" + expr + ",x=" + str(point) + "))", stack
            )
            value = complex(parse(result).evalf())
            reference = complex(expected.subs(x, point).evalf(40))
            assert abs(value - reference) < 2e-11 * (1 + abs(reference)), (
                phase,
                point,
                result,
                reference,
...
a.report.write_text(
    json.dumps(
        {
            "scope": "Independent guard proofs and selected numeric substitutions. Host execution is not CG50 MMU emulation. Unknown-pivot rows prove output preservation only.",
            "source_sha256": {
                n: hashlib.sha256((source_path(n)).read_bytes()).hexdigest()
                for n in ["kconvert.cc", "ksubst.cc", "yderive.cc", "dilogarithm.h"]
            },
            "runs": rows,
        },
        indent=2,
