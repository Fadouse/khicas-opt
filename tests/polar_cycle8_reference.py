...
def verify(case, printed):
    ...
    if ident == "PC8-D2":
        ...
        assert m, (
            "The oscillatory original difference quotient at zero must be excluded"
        )
        c, a, d, b, e = m[1][10:-1].split(",")
        assert c == "0>x" and d == "x<=1"
        assert (
            equal(parse(a), s.sin(1 / x) - s.cos(1 / x) / x)
            and equal(parse(b), 2 * x)
            and parse(e) == 2
        )
        ...
        return dict(
            exact=True,
            method="Exact interior derivatives; original left quotient sin(1/h) has two sequences with limits ±1 at zero, whereas original quotients at one tend to 2 from both sides. Lazy undef excludes precisely zero.",
        )
    ...
    if ident == "PC8-I1":
        ...
        assert equal(
            s.diff(a, x),
            2 * x * s.log(1 + x * x) / ((1 + x * x) * (1 + s.log(1 + x * x))),
        )
        ...
        if not equal(a, canonical):
            z = s.sqrt(x * x - 1) - x
            legacy = -s.pi * s.sign(x + 1) / 2 + (
                2 * s.atan(z) - s.log(s.Abs(z))
            ) / s.sign(x + 1)
            outer = (
                -s.pi
                + 4 * s.atan(z)
                ...
                * s.log(
                    s.sqrt(
                        ...
                        * s.sqrt(s.Abs(x * x - 1))
                        * x
                        * s.cos((s.pi * s.sign(x * x - 1) - s.pi) / 4)
                        + s.Abs(x * x - 1)
                    )
                )
            ) / (2 * s.sign(x + 1))
        ...
        f = f.replace(
            lambda z: z.func == Root and z.args[1] == 3 and equal(z.args[0], v**6),
            lambda z: v * v,
        )
        f = f.replace(
            lambda z: (
                z.func == s.Pow and z.exp == s.Rational(2, 3) and z.base == s.Abs(v**3)
            ),
            lambda z: v * v,
        )
        ...
        assert s.Poly(
            A**32
            - B**32
            - (A - B) * (A + B) * s.prod(A**k + B**k for k in [2, 4, 8, 16]),
            r,
        ).is_zero
