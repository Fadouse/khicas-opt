    ...
    for scale in ["1", "-1", "1/2", "-2/3"]:
        ...
        cases.append(
            (
                ...
                2 * s.diff(T, x) / (1 + T * T),
                {},
                "Principal range asin(2t/(1+t²))=2atan(t), |t|<=1 everywhere.",
            )
        )
for wave, points in [
    ("sin(x)", {"pi/2": "0", "-pi/2": "0", 0: "2"}),
    ("cos(x)", {0: "0", "pi": "0", "pi/2": "-2"}),
]:
    t = wave
    cases.append(
        (
            f"diff(asin(2*{t}/(1+{t}^2)),x)",
            2 * s.diff(parse(t), x) / (1 + parse(t) ** 2),
            points,
            "Original contact quotient is O(h), so the finite derivative is zero.",
        )
    )
for n in [2, 5, 12]:
    for c in [1, 2]:
        ...
        points = {
            str(s.Rational(sign * j, c)): "undef"
            for j in range(2, n + 1)
            for sign in [-1, 1]
        }
        points.update({"0": str(s.Rational(1, (n + 1) ** 2))})
        cases.append(
            (
                e,
                target,
                points,
                "Finite exact factor telescope retains every canceled ±j/c hole; origin allowed.",
            )
        )
for e, target, points, method in cases:
    for outer in [False, True]:
        for stack in ["normal", "64"]:
            expression = f"simplify({e})" if outer else e
            env = dict(os.environ)
            ...
            try:
                ...
                for point, expected in points.items():
                    q = subprocess.run(
                        [a.probe, f"eval(subst({expression},x={point}))"],
                        capture_output=True,
                        text=True,
                        env=env,
                        timeout=10,
