...
def verify(case, printed):
    ...
    if ident == "PC14-R1":
        ...
        printed = printed.replace("x=(floor(x))", "Eq(x,floor(x))").replace(
            "x=floor(x)", "Eq(x,floor(x))"
        )
    a = parse(printed)
    if ident == "PC14-I1":
        assert equal(s.diff(a, x), 1 / (x**4 + 4)) and a.subs(x, 0) == 0
        assert a.atoms(s.atan) == {s.atan(x - 1), s.atan(x + 1)}
        assert {s.expand(v.args[0]) for v in a.atoms(s.log)} == {
            x * x - 2 * x + 2,
            x * x + 2 * x + 2,
        }
        ...
        for k in range(1, 5):
            for sign in [-1, 1]:
                assert a.subs(x, sign * s.sqrt(k * s.pi)) == s.Symbol("undef")
        regular = a
        ...
        assert (
            a.args[2] == (s.Symbol("undef"), s.Eq(x, s.floor(x)))
            and a.args[3][1] == True
        )
        for j in [0, 1, 3]:
            assert equal(a.args[j][0], 20 / (x * (x + 20)))
