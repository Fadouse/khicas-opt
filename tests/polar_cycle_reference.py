...
def verify(case, printed):
    ident = case["id"]
    if ident.startswith("PC21-"):
        from polar_cycle21_reference import verify as next_cycle

        return next_cycle(case, printed)
    if ident.startswith("PC20-"):
        from polar_cycle20_reference import verify as next_cycle

        return next_cycle(case, printed)
    if ident.startswith("PC19-"):
        from polar_cycle19_reference import verify as next_cycle

        return next_cycle(case, printed)
    if ident.startswith("PC18-"):
        from polar_cycle18_reference import verify as next_cycle

        return next_cycle(case, printed)
    if ident.startswith("PC17-"):
        from polar_cycle17_reference import verify as next_cycle

        return next_cycle(case, printed)
    if ident.startswith("PC16-"):
        from polar_cycle16_reference import verify as next_cycle

        return next_cycle(case, printed)
    if ident.startswith("PC15-"):
        from polar_cycle15_reference import verify as next_cycle

        return next_cycle(case, printed)
    if ident.startswith("PC14-"):
        from polar_cycle14_reference import verify as next_cycle
    ...
    if ident.startswith("PC3-"):
        ...
        return next_cycle(case, printed)
    forbidden = (
        ("integrate(", "diff(", "rootof(")
        if ident == "PC1-D4"
        else ("integrate(", "diff(", "undef", "rootof(")
    )
    ...
    if ident == "PC1-D4" and a.has(s.Piecewise):
        ...
        assert excluded == {-1, 1}, (
            "Only the original absolute-value corners are excluded"
        )
        while a.has(s.Piecewise):
            a = a.replace(lambda z: z.func == s.Piecewise, lambda z: z.args[-1][0])
    if ident in ("PC1-I1", "PC1-I2"):
        ...
        if ident.endswith("I1"):
            ...
            assert all(
                s.simplify(s.diff(phi, x).subs(x, p)) == 0
                for p in [-1 / s.sqrt(3), 1 / s.sqrt(3)]
            )
        ...
        target = (
            s.sin(x)
            * s.cos(x)
            / (s.sin(x) ** 4 + 3 * s.sin(x) ** 2 * s.cos(x) ** 2 + 2 * s.cos(x) ** 4)
        )
        ...
        regular = regular.replace(
            lambda z: z.func == s.sin, lambda z: s.sin(s.factor(z.args[0]))
        )
        ...
        target = (s.sin(x) ** 3 * s.cos(x) + s.sin(x) * s.cos(x) ** 3) / (
            s.sin(x) ** 6 + s.cos(x) ** 6
        )
