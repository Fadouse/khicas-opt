...
p.add_argument(
    "--corpus", type=Path, default=ROOT / "tests/mixed-round1-questions.json"
)
...
cache = {}
report = {
    "scope": "Repository integration, derivative engine and FXCG simplify; host dependencies remain, not CG50 hardware/MMU emulation. Independent exact proofs and explicit domain/display checks.",
    "ref": a.ref,
    "corpus_sha256": hashlib.sha256(a.corpus.read_bytes()).hexdigest(),
    "source_sha256": {
        n: hashlib.sha256(source(a.ref, n).encode()).hexdigest()
        for n in [
            "yintg.cc",
            "zintgab.cc",
            "ksubst.cc",
    ...
    "probe_sha256": {
        ...
        for p in [a.probe, a.conic_probe]
    },
    "runs": rows,
}
for c in cases:
    for outer in [False, True]:
        for stack in ["normal", "64"]:
            ...
            try:
                r = subprocess.run(
                    [str(a.conic_probe if conic else a.probe), expression],
                    env=env,
                    capture_output=True,
                    text=True,
                    timeout=12,
                ...
                if conic:
                    ...
                    result = fields["RAW"]
                    row["coordinates"] = result
                    if c["branches"]:
                        assert (
                            fields["VIEW"].count("x(" + c["parameter"] + ")=")
                            == c["branches"]
                            and fields["VIEW"].count("y(" + c["parameter"] + ")=")
                            == c["branches"]
                        )
                        row["label_nodes"] = int(fields["NODES"].split()[1])
                        assert row["label_nodes"] <= 256
                key = (c["id"], result)
                if key not in cache:
                    cache[key] = verify(c, result)
                row["verification"] = cache[key]
                row["pass"] = True
            except Exception as e:
                row.update(error=str(e)[:4000], **{"pass": False})
            rows.append(row)
...
report["cases"] = [
    {"id": c["id"], "all_four_pass": all(r["pass"] for r in rows if r["id"] == c["id"])}
    for c in cases
]
a.report.write_text(json.dumps(report, indent=2) + "\n")
