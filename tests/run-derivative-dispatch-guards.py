...
u = "x"
for k in range(8):
    u = ("sin" if k % 2 else "cos") + "(" + u + ")"
    expressions.append(u)
            ...
            try:
                ...
                if key not in cache:
                    ...
                    target = target.replace(
                        lambda t: t.func == s.sign,
                        lambda t: t.args[0] / s.Abs(t.args[0]),
                    )
                    assert s.simplify(s.expand_trig(value - target)) == 0, (
                        value,
                        target,
                    )
                    cache[key] = True
...
a.report.write_text(
    json.dumps(
        dict(
            scope="38 expression families, direct/outer simplify and normal/64 KiB stack. Exact symbolic derivatives on each original differentiability component; dedicated historical branch/endpoint guards are separate.",
            source_sha256={
                n: hashlib.sha256(source_path(n).read_bytes()).hexdigest()
                for n in ("yderive.cc", "ksubst.cc")
            },
