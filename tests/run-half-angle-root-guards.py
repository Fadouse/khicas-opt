...
for wave in ["sin", "cos"]:
    for sign in [-1, 1]:
        for frequency, shift, scale in [(1, "0", 1), (2, "pi/3", 3), (-3, "1", 2)]:
            hi = f"2*pi/{abs(frequency)}"
            e = f"integrate(sqrt({scale}*(1+({sign})*{wave}({frequency}*x+({shift})))),x,0,{hi})"
...
for e, target in cases:
    for outer in [False, True]:
        for stack in ["normal", "64"]:
            expression = "simplify(" + e + ")" if outer else e
            env = dict(os.environ)
            ...
            try:
                ...
                assert r.returncode == 0 and equal(parse(r.stdout.strip()), target), (
                    r.returncode,
                    r.stdout,
                    r.stderr,
                )
                row.update(
                    passed=True,
                    proof="Nonnegative half-angle square root is sqrt(2A)*absolute sine/cosine. Integral over a complete phase period equals 4sqrt(2A)/|frequency|, independent of phase; reversed bounds reverse sign.",
                )
