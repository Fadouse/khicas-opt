...
p.add_argument(
    "--corpus", type=Path, default=ROOT / "tests/user-challenge-integrals.json"
)
args = p.parse_args()
corpus = args.corpus
cases = json.loads(corpus.read_text())["cases"]
report = {
    "scope": "Repository integration, normalization and FXCG simplification entry points; other dependencies host Giac, not a CG50 emulator or hardware timings",
    ...
    "source_sha256": {
        ...
        if (source_path(n)).exists()
    },
    "runs": [],
}
with tempfile.TemporaryDirectory(prefix="khicas-user-challenge-") as tmp:
    d = Path(tmp)
    target = build(d / "target", target_simplify=True)
    validator = build_validation_probe(d / "validator")
    for case in cases:
        ...
        integral = (
            ...
            + case["f"]
            + ",x"
            + ("," + ",".join(case["bounds"]) if definite else "")
            + ")"
        )
        for outer in (False, True):
            ...
            for stack in ("normal", "64"):
                ...
                try:
                    ...
                    if m:
                        row["host_seconds"] = float(m[1])
                    for label, key in (
                        ("PARSER_CALLS", "parser_calls"),
                        ("MAX_RSS_KB", "host_max_rss_kb"),
                    ):
                    ...
                    verification = [
                        case["expected"],
                        "definite" if definite else "indefinite",
                        case["f"],
                    ]
                    if not definite:
                        verification += case.get("samples", ["1/3", "1", "2"])
                    if (
                        case.get("verification", {}).get("symbolic_proof")
                        == "generic-binomial-parts"
                    ):
                        from compact_reference import verify_binomial_reference

                        row["validation"] = verify_binomial_reference(
                            case, row["result"]
                        )
                    elif (
                        case.get("verification", {}).get("symbolic_proof")
                        ...
                        row["validation"] = verify_dilogarithm_reference(
                            case, row["result"]
                        )
                    elif (
                        case.get("verification", {}).get("symbolic_proof")
                        ...
                        row["validation"] = verify_trig_log_reference(
                            case, row["result"]
                        )
                    else:
                        check = subprocess.run(
                            [str(validator), row["result"]] + verification,
                            capture_output=True,
                            text=True,
                            timeout=30,
                        )
                        ...
                        assert (
                            check.returncode == 0 and "CHECK exact" in check.stderr
                        ), row
                    if stack == "normal":
                        normal_result = row["result"]
                    else:
                        assert row["result"] == normal_result, row
                    row["status"] = "exact"
                    ...
                    print(
                        case["id"],
                        "simplify" if outer else "integrate",
                        stack,
                        "FAILED",
                        flush=True,
                    )
...
if failures:
    raise SystemExit(
        f"FAIL: {failures} of {len(report['runs'])} runs failed; see {args.report}"
    )
print(
    f"PASS: all {len(cases)} exact with/without outer simplify on normal and guarded 64 KiB stacks"
)
