...
def verify(case, printed):
    ...
    if ident == "PC11-D2":
        ...
        local["piecewise"] = lambda *v: s.Piecewise(
            *[(v[j + 1], v[j]) for j in range(0, len(v) - 1, 2)],
            (v[-1], True),
            evaluate=False,
        )
        ...
        def branch(e, point):
            if e.func == s.Piecewise:
                for v, c in e.args:
                    if c == True or c.subs(x, point) == True:
                        return branch(v, point)
            return e

        for point, target in [
            (-1, 1 / (1 + x * x)),
            (s.Rational(1, 2), 1 / (1 + x) ** 2),
            (2, s.Rational(1, 4)),
        ]:
            assert equal(branch(a, point), target)
    ...
    if ident == "PC11-I1":
        ...
        assert a.subs(x, 0) == 0
        for k in [-2, -1, 1, 2]:
            assert a.subs(x, k * s.pi) == s.Symbol("undef")
        regular = a
        while regular.has(s.Piecewise):
            regular = regular.replace(
                lambda z: z.func == s.Piecewise, lambda z: z.args[-1][0]
            )
        target = 2 * x * s.Abs(s.sin(x)) / (1 + x * x) + s.log(1 + x * x) * s.sign(
            s.sin(x)
        ) * s.cos(x)
        assert equal(
            regular.replace(s.sign, lambda z: z / s.Abs(z)),
            target.replace(s.sign, lambda z: z / s.Abs(z)),
        )
        ...
        assert a == parse(case["input"][9:-1]), (
            "Domain-bearing logarithms must remain unless explicit conditions are returned"
        )
        t = s.Symbol("t", positive=True)
        for branch in [t + 1, -t - 1]:
            assert (
                s.simplify(
                    s.expand_log(
                        a.subs(x, branch).replace(s.log, lambda z: s.log(s.factor(z))),
                        force=True,
                    )
                )
                == 0
        ...
        num, den = s.fraction(a)
        assert num == s.factorial(24)
        assert s.Poly(den, x) == s.Poly(s.prod(x + k for k in range(25)), x)
        for j in range(25):
            assert s.diff(den, x).subs(x, -j) == (-1) ** j * s.factorial(
                j
            ) * s.factorial(24 - j)
