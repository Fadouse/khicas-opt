...
def run(e, stack):
    ...
    return subprocess.run(
        [a.probe, e], capture_output=True, text=True, env=env, timeout=10
    )
...
def save():
    a.report.write_text(
        json.dumps(
            dict(
                scope="Independent symbolic derivatives, original contact/domain rules, and high-precision elliptic references; normal/64 KiB host tests. Numerical agreement is not a device-accuracy claim.",
...
def check(e, proof, kind, expected_undef=False):
    for stack in ("normal", "64"):
        ...
        try:
            ...
            if expected_undef:
                assert row["result"] == "undef"
            else:
                proof(parse(row["result"]))
            row["passed"] = True
...
for m in [-1000, -2, -1, 0, s.Rational(1, 2), s.Rational(999999, 1000000)]:
    for phi in [
        ...
        pm = mp.mpf(str(s.N(m, 50)))
        pp = mp.mpf(str(s.N(phi, 50)))
        ref = mp.ellipf(pp, pm)
...
for scale, alpha, beta, center in [
    (1, 1, -1, 0),
    (2, 2, -1, 1),
    (3, s.Rational(3, 2), -s.Rational(1, 3), -2),
    (1, 3, -2, s.Rational(1, 2)),
]:
    scale, alpha, beta, center = map(s.sympify, (scale, alpha, beta, center))
    u = x - center
    Q = scale * (1 - alpha * u * u) * (1 - beta * u * u)
    expected = s.elliptic_f(s.asin(s.sqrt(alpha) * u), beta / alpha) / s.sqrt(
        scale * alpha
    )
    for outer in (False, True):
        e = f"integrate(1/sqrt({gs(Q)}),x)"
...
for fun in (s.Max, s.Min):
    for f, g in [
        ...
        f = s.sympify(f)
        g = s.sympify(g)
        e = f"simplify(diff({gs(fun(f, g, evaluate=False))},x))"
        for aa in [-2, 0, 2]:
            for xx in [-2, 0, 1, 2]:
                sub = {x: s.Integer(xx), A: s.Integer(aa)}
                v1 = f.subs(sub)
                v2 = g.subs(sub)
                d1 = s.diff(f, x).subs(sub)
                d2 = s.diff(g, x).subs(sub)
                bad = v1 == v2 and d1 != d2
                expected = (
                    d1 if (v1 == v2 or (v1 > v2 if fun == s.Max else v1 < v2)) else d2
                )
                check(
                    f"eval(subst({e},[x,a],[{xx},{aa}]))",
                    same(expected),
                    "minmax-contact",
                    bad,
                )
...
for H, L in [(x, x - 1), (x, x + 1), (x, x), (2 * x - 1, 3 - 2 * x), (x + 2, -x - 3)]:
    H = s.sympify(H)
    L = s.sympify(L)
    e = f"simplify(diff(sqrt({gs(H * H * L)}),x))"
    for xx in [-4, -3, -2, -1, 0, s.Rational(1, 2), 1, s.Rational(3, 2), 2, 3]:
        ...
        expected = (
            s.Integer(0)
            if ll == 0
            else s.diff(H, x) * s.sign(hh) * s.sqrt(ll)
            + s.Abs(hh) * s.diff(L, x) / (2 * s.sqrt(ll))
        )
        check(
            f"eval(subst({e},x,{gs(xx)}))", same(expected), "squared-affine-domain", bad
        )
# Clamp order identities, including polynomial thresholds and outer scale.
for left, right, variable, c in [
    (A, B, x, 1),
    (A * A, B + 1, 2 * x - 3, -2),
    (x * x, x * x + 1, A, 3),
    (A, A, B, 1),
    ...
    e = c * (
        s.Max(left, s.Min(variable, right, evaluate=False), evaluate=False)
        - s.Min(right, s.Max(variable, left, evaluate=False), evaluate=False)
    )
    check(
        "simplify(" + gs(e) + ")",
        same(c * (left - right + s.Abs(left - right)) / 2),
        "clamp-order",
    )
