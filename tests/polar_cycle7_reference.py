...
def verify(case, printed):
    ...
    if ident == "PC7-D2":
        ...
        assert (
            equal(parse(a), 2 * x)
            and equal(parse(b), x / s.sqrt(1 - x * x))
            and parse(e) == 0
        )
        ...
        return dict(
            exact=True,
            method="Exact interior derivatives; original difference quotients at zero both tend to zero, at one the left tends to +infinity and right is zero. Explicit lazy undef at one; no inactive-root exclusions.",
        )
    ...
    if ident == "PC7-I1":
        ...
        assert equal(
            s.diff(t, x), (1 - x * x) / ((1 + x * x) * s.sqrt(x**4 + 3 * x * x + 1))
        )
        assert a.subs(x, 0) == 0
        ...
        assert equal(a, parse(case["expected"]["expression"])), (
            "Require a globally regular primitive, not a formula with a hidden midpoint pole"
        )
        assert s.limit(f, u, 0) == 0 and equal(
            s.limit(f, u, s.oo), s.pi * (1 - 1 / s.sqrt(2))
        )
        ...
        for point in [-1, 1]:
            assert equal(a.subs(x, point).subs(Root(0, 3), 0), s.cos(1))
