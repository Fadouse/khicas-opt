...
calls = " || ".join(
    ...
    for name in ("integrate_mobius_cauchy_log", "integrate_log_sine_cosine_sum")
)
text += (
    "bool cycle7_log_rule(const gen &f,const gen &x,const gen &lo,const gen &hi,gen &r,GIAC_CONTEXT){gen e=integration_syntax(f,contextptr);return "
    + calls
    + ";}\n}\n"
)
report = []
with tempfile.TemporaryDirectory(prefix="khicas-cycle7-log-") as tmp:
    ...
    subprocess.run(
        flags
        + [str(d / "rules.cc"), str(ib.ROOT / "tests/cycle7_log_integrals.cc")]
        + libs
        + ["-pthread", "-o", str(unit)],
        check=True,
    )
