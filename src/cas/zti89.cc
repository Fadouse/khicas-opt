...
namespace giac {
  ...
  */
  ...
  static define_unary_function_eval(__ti_not_implemented, &_nop,
                                    _ti_not_implemented_s); // never evaled
  define_unary_function_ptr(at_ti_not_implemented, alias_at_ti_not_implemented,
                            &__ti_not_implemented);
  ...
  gen _seq(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.type == _VECT && g.subtype == _SEQ__VECT && !g._VECTptr->empty()) {
      ...
      if (v.size() >= 2) {
        gen x(v[1]);
        if (is_equal(x) && x._SYMBptr->feuille.type == _VECT &&
            !x._SYMBptr->feuille._VECTptr->empty())
      ...
      if (v.size() == 2 && is_integral((g1 = eval(v[1], eval_level(contextptr), contextptr))) &&
          g1.val >= 0) {
        vecteur res;
  ...
  gen _InputStr(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 1)
      ...
      if (s == 2)
        res = __click.op(
            makevecteur(
                string2gen(eval(v[0], eval_level(contextptr), contextptr).print(contextptr)), v[1],
                1),
            contextptr);
      else
  ...
  */
  ...
  static define_unary_function_eval2_quoted(__InputStr, &_InputStr, _InputStr_s,
                                            &printastifunction);
  define_unary_function_ptr5(at_InputStr, alias_at_InputStr, &__InputStr, _QUOTE_ARGUMENTS,
                             T_RETURN);

  static const char _lis_phrase_s[] = "lis_phrase";
  static define_unary_function_eval2_quoted(__lis_phrase, &_InputStr, _lis_phrase_s,
                                            &printastifunction);
  define_unary_function_ptr5(at_lis_phrase, alias_at_lis_phrase, &__lis_phrase, _QUOTE_ARGUMENTS,
                             T_RETURN);
  ...
  define_unary_function_ptr5(at_resoudre_dans_C, alias_at_resoudre_dans_C, &__resoudre_dans_C,
  ...
  define_unary_function_ptr5(at_factoriser_sur_C, alias_at_factoriser_sur_C, &__factoriser_sur_C, 0,
                             true);
  ...
  gen _nSolve(const gen & g, GIAC_CONTEXT) {
    ...
    gen var = v[1], guess;
    if (var.type == _SYMB &&
        (var._SYMBptr->sommet == at_equal || var._SYMBptr->sommet == at_equal2 ||
         var._SYMBptr->sommet == at_same)) {
      ...
      return newton(remove_equal(v[0]), var, guess, NEWTON_DEFAULT_ITERATION, 1e-5, 1e-12,
                    !complex_mode(contextptr), 1, 0, 1, 0, 1, contextptr);
    } else
      return newton(remove_equal(v[0]), var, 0, NEWTON_DEFAULT_ITERATION, 1e-5, 1e-12,
                    !complex_mode(contextptr), 1, 0, 1, 0, 1, contextptr);
  }
  ...
  gen _randPoly(const gen & g, GIAC_CONTEXT) {
      ...
      if (vs >= 4 && v[2].type == _INT_ && v[3].type == _INT_) {
        ...
        if (calc_mode(contextptr) == 1 || abs_calc_mode(contextptr) == 38) {
          if (v[2] == v[3])
            return gensizeerr(contextptr);
          for (;;) {
            w[0] = int(giac_rand(contextptr) / (rand_max2 + 1.0) * (v[3].val - v[2].val + 1) +
                       v[2].val);
            if (w[0].val)
              break;
          }
        ...
        for (unsigned i = 1; i <= (unsigned)absint(deg); ++i) {
          w[i] =
              int(giac_rand(contextptr) / (rand_max2 + 1.0) * (v[3].val - v[2].val + 1) + v[2].val);
        }
    ...
    return w;
#else
  return (deg >= 1024 || (f.type == _VECT && f._VECTptr->empty())) ? gen(w, _POLY1__VECT)
                                                                   : symb_horner(w, x);
#endif
  }
  ...
  gen _nDeriv(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() > 2)
      step = v[2];
    return evalf(rdiv(subst(v[0], v[1], v[1] + step, false, contextptr) -
  ...
  gen _fMin(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() == 4) // fMin(func,[constraints],[variables],[guess])
      return fmin_cobyla(v[0], gen2vecteur(v[1]), gen2vecteur(v[2]), gen2vecteur(v[3]), undef,
                         undef, contextptr);
    if (v.size() == 5) // fMin(func,[constraints],[variables],[guess],eps)
      return fmin_cobyla(v[0], gen2vecteur(v[1]), gen2vecteur(v[2]), gen2vecteur(v[3]), v[4], undef,
                         contextptr);
    if (v.size() == 6) // fMin(func,[constraints],[variables],[guess],eps,maxiter)
      return fmin_cobyla(v[0], gen2vecteur(v[1]), gen2vecteur(v[2]), gen2vecteur(v[3]), v[4], v[5],
                         contextptr);
  ...
  gen _fMax(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() == 5) // fMax(func,[constraints],[variables],[guess],eps)
      return fmin_cobyla(-v[0], gen2vecteur(v[1]), gen2vecteur(v[2]), gen2vecteur(v[3]), v[4],
                         undef, contextptr);
  ...
  gen _taylor(const gen & g, GIAC_CONTEXT) {
    ...
    return _series(makesequence(v[0], symbolic(at_equal, makesequence(v[1], x0)), v[2]),
                   contextptr);
  }
  ...
  gen arclen(const gen & g, GIAC_CONTEXT) {
    ...
    if (!v.empty() && v.front().is_symb_of_sommet(at_pnt)) {
      ...
      if (g.is_symb_of_sommet(at_curve) && g._SYMBptr->feuille.type == _VECT &&
  ...
  gen frenet(const gen & g, gen & M, gen & T, gen & N, gen & B, gen & kappa, gen & Nsurkappa,
             gen & tau, gen & t, bool compute_torsion, GIAC_CONTEXT) {
    if (g.is_symb_of_sommet(at_pnt)) {
      if (g._SYMBptr->feuille.type == _VECT &&
          g._SYMBptr->feuille._VECTptr->size() >= 2) { // element
        gen arg = (*g._SYMBptr->feuille._VECTptr)[1];
        if (arg.type == _VECT)
          return frenet((*arg._VECTptr)[1], M, T, N, B, kappa, Nsurkappa, tau, t, compute_torsion,
                        contextptr);
      }
      return frenet(remove_at_pnt(g), M, T, N, B, kappa, Nsurkappa, tau, t, compute_torsion,
                    contextptr);
    }
    ...
    if (dim == 2 && T.type == _VECT && T._VECTptr->size() == 2) {
      kappa =
          normal(vitesse[0] * accel[1] - vitesse[1] * accel[0], contextptr) / pow(v2, 2) * sqrtv2;
      Nsurkappa = normal(v2 / (vitesse[0] * accel[1] - vitesse[1] * accel[0]) *
                             makevecteur(-vitesse[1], vitesse[0]),
                         contextptr);
      N = makevecteur(-T._VECTptr->back(), T._VECTptr->front());
    } else {
      gen an = normal(accel - (scalar_product(accel, vitesse, contextptr) / v2) * vitesse,
                      contextptr); // normal accel
    ...
    if (dim != 3 || !compute_torsion)
      ...
      tau = _det(makesequence(vitesse, accel, f3), contextptr) /
            l2norm2(cross(vitesse, accel, contextptr));
    }
    if (!is_undef(x0)) {
  ...
  gen _frenet(const gen & g, GIAC_CONTEXT) {
    bool b = complex_variables(contextptr);
    complex_variables(false, contextptr);
    gen t, M, T, N, B, kappa, Nsurkappa, tau,
        dim = frenet(g, M, T, N, B, kappa, Nsurkappa, tau, t, true, contextptr);
    complex_variables(b, contextptr);
    if (dim.type != _INT_)
      return dim;
    if (dim.val == 2)
  ...
  gen _osculating_circle(const gen & args, GIAC_CONTEXT) {
    ...
    complex_variables(false, contextptr);
    gen t, M, T, N, B, kappa, Nsurkappa, tau,
        dim = frenet(g, M, T, N, B, kappa, Nsurkappa, tau, t, false, contextptr);
    complex_variables(b, contextptr);
    if (dim.type != _INT_)
      return dim;
    if (dim.val == 2)
      return put_attributs(
          _cercle(makesequence(_point(M, contextptr), _point(M + 2 * Nsurkappa, contextptr)),
                  contextptr),
  ...
  define_unary_function_ptr5(at_osculating_circle, alias_at_osculating_circle, &__osculating_circle,
                             0, true);
    ...
    if (g.type == _ZINT) {
      ...
      if (!forme.empty()) {
        ...
        if (digits + 1 < txt.size()) {
          string tmp = txt.substr(0, 1) + "." + txt.substr(1, digits) + "e+" +
                       print_INT_(int(txt.size()) - 1);
          return tmp;
        }
      }
      return txt;
  ...
  static gen shiftrotate(const gen & g, bool right) {
    ...
    if (a.type == _VECT) {
      ...
      if (right)
        return gen(mergevecteur(vecteur(itend - nbre, itend), vecteur(it, itend - nbre)),
                   a.subtype);
  ...
  gen _list2exp(const gen & g, GIAC_CONTEXT) {
    ...
    if (res.is_symb_of_sommet(at_ou) && res._SYMBptr->feuille.type == _VECT &&
        res._SYMBptr->feuille._VECTptr->size() == 1)
    ...
    if (g.type == _VECT) {
      ...
      if (v.size() == 4 && (v[2].type != _INT_ || v[3].type != _INT_)) {
        ...
        if (is_zero(derive(vecteur(lv.begin() + 1, lv.end()), n, contextptr), contextptr)) {
          ...
          fxnd(v[0], p1, p2);
          return simplify(
              product(gen2polynome(p1, int(lv.size())), lv, n, v[2], v[3], contextptr) /
                  product(gen2polynome(p2, int(lv.size())), lv, n, v[2], v[3], contextptr),
              contextptr);
        }
      }
  ...
  gen _int(const gen & g, GIAC_CONTEXT) {
    if (g.type == _STRNG && g.subtype == -1)
      return g;
    if (g.type == _VECT && g.subtype == _SEQ__VECT && g._VECTptr->size() == 2 &&
        g._VECTptr->front().type == _STRNG && g._VECTptr->back().type == _INT_) {
  ...
  gen _Fill(const gen & g, GIAC_CONTEXT) {
    ...
    if (dim.type == _INT_)
      return sto(vecteur(dim.val, eval(v[0], eval_level(contextptr), contextptr)), v[1],
                 contextptr);
    if (dim.type == _VECT) {
      ...
      if (w.size() != 2 || w.front().type != _INT_ || w.back().type != _INT_)
        return gensizeerr(contextptr);
      return sto(
          vecteur(w[0].val, vecteur(w[1].val, eval(v[0], eval_level(contextptr), contextptr))),
          v[1], contextptr);
    }
    return gensizeerr(contextptr);
  }
  ...
  gen _LU(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() == 5)
      v.pop_back();
    if (v.size() != 4 || !is_squarematrix(res = eval(v[0], eval_level(contextptr), contextptr)) ||
        v[1].type != _IDNT || v[2].type != _IDNT || v[3].type != _IDNT)
      return gentypeerr(contextptr);
    res = lu(res, contextptr); // P,L,U
    if (is_undef(res) || res.type != _VECT || res.subtype != _SEQ__VECT ||
        res._VECTptr->size() != 3)
      return res;
  ...
  gen _QR(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() == 4)
      v.pop_back();
    gen res;
    if (v.size() != 3 || !is_squarematrix(res = eval(v[0], eval_level(contextptr), contextptr)) ||
        v[1].type != _IDNT || v[2].type != _IDNT)
      return gentypeerr(contextptr);
  ...
  gen _ref(const gen & a_orig, GIAC_CONTEXT) {
    ...
    mrref(a, res, pivots, det, 0, int(a.size()), 0, int(a.front()._VECTptr->size()),
          /* fullreduction */ 0, 0, true, 1, 0, contextptr);
    if (keep_pivot)
      return res;
  ...
  gen _subMat(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() > 2 && v[1].type == _VECT && v[1]._VECTptr->size() == 2 && v[2].type == _VECT &&
        v[2]._VECTptr->size() == 2) {
      ...
      if (is_integral(f1) && is_integral(f2) && f1.type == _INT_ && f2.type == _INT_ &&
          is_integral(f3) && is_integral(f4) && f3.type == _INT_ && f4.type == _INT_) {
        lignedeb = giacmax(1, f1.val + !shift);
    ...
    return matrice_extract(*v[0]._VECTptr, lignedeb - 1, colonnedeb - 1, lignefin - lignedeb + 1,
                           colonnefin - colonnedeb + 1);
  }
...
unary_function_eval __getKey(0, &_getKey, _getKey_s);
  ...
  gen _Line(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() == 5 && v[4].val == 0)
      couleur = FL_WHITE;
    return _couleur(
        makesequence(_segment(makesequence(v[0] + cst_i * v[1], v[2] + cst_i * v[3]), contextptr),
                     couleur),
        contextptr);
  }
  ...
  gen _LineHorz(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() == 2 && v[1].val == 0)
      couleur = FL_WHITE;
    return _couleur(
        makesequence(_droite(makesequence(cst_i * v[0], 1 + cst_i * v[0]), contextptr), couleur),
        contextptr);
  }
  ...
  gen _LineVert(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() == 2 && v[1].val == 0)
      couleur = FL_WHITE;
    return _couleur(makesequence(_droite(makesequence(v[0], cst_i + v[0]), contextptr), couleur),
                    contextptr);
  }
  ...
  gen _Circle(const gen & g, GIAC_CONTEXT) {
    ...
    return _couleur(makesequence(_cercle(makesequence(centre, v[2]), contextptr), couleur),
                    contextptr);
  }
  ...
  gen _NewPic(const gen & g, GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      if (it->_VECTptr->size() != 2)
        return gensizeerr(contextptr);
      *it = symbolic(
          at_pnt, makevecteur(it->_VECTptr->front() + cst_i * it->_VECTptr->back(), int(FL_BLACK)),
          _PNT__VECT);
    }
  ...
  static define_unary_function_eval2(__droite_tangente, &_LineTan, _droite_tangente_s,
                                     &printastifunction);
  define_unary_function_ptr5(at_droite_tangente, alias_at_droite_tangente, &__droite_tangente, 0,
                             true);
  ...
  gen _RandSeed(const gen & g, GIAC_CONTEXT) {
    if (g.type == _STRNG && g.subtype == -1)
      return g;
#if defined(NSPIRE_NEWLIB) || defined KHICAS || defined(VISUALC) || defined(__MINGW_H) ||          \
    defined BESTA_OS || defined EMCC || defined EMCC2 || defined NSPIRE
    srand(g.val);
#else
#ifndef GNUWINCE
    srandom(g.val);
  ...
  */
  ...
  static define_unary_function_eval2_quoted(__Unarchiv, &_Unarchiv, _Unarchiv_s,
                                            &printastifunction);
  define_unary_function_ptr5(at_Unarchiv, alias_at_Unarchiv, &__Unarchiv, _QUOTE_ARGUMENTS,
                             T_RETURN);
  ...
  static define_unary_function_eval2_quoted(__sialorssinon, &_ifte, _sialorssinon_s,
                                            &printassialorssinon);
  define_unary_function_ptr5(at_sialorssinon, alias_at_sialorssinon, &__sialorssinon,
                             _QUOTE_ARGUMENTS, T_IFTE);
  ...
  static string printaspour(const gen & feuille, const char * sommetstr, GIAC_CONTEXT) {
    ...
    if (is_zero(*it, contextptr) && is_zero(*(it + 2), contextptr)) {
      ...
      if ((it->type != _SYMB) || (it->_SYMBptr->sommet != at_sto) || ((it + 2)->type != _SYMB) ||
          ((it + 2)->_SYMBptr->sommet != at_sto) ||
          (it->_SYMBptr->feuille._VECTptr->back() !=
           (it + 2)->_SYMBptr->feuille._VECTptr->back())) {
        res = localize("pour", l) + " (";
        res += it->print(contextptr) + ';';
        ++it;
        res += it->print(contextptr) + ';';
  ...
  define_unary_function_ptr5(at_tantque, alias_at_tantque, &__tantque, _QUOTE_ARGUMENTS,
                             T_MUPMAP_WHILE);
...
#if defined RTOS_THREADX || defined BESTA_OS || defined EMCC || defined EMCC2 ||                   \
    defined __MINGW_H || defined KHICAS
...
static gen ti_decode_identificateur(octet *& ptr, GIAC_CONTEXT) {
  ...
  if (pos > 0 && pos + 1 < l) {
    ...
    res = symb_double_deux_points(
        makevecteur(make_symbol(s1, contextptr), make_symbol(s2, contextptr)));
  } else
...
static gen ti_decode_binary(octet *& ptr, const unary_function_ptr * u, bool normal_order = true,
                            GIAC_CONTEXT = 0) {
  --ptr;
...
static gen ti_decode_for(octet *& ptr, GIAC_CONTEXT) {
  ...
  if (ck_is_strictly_positive(gstep, contextptr))
    return symb_for(symb_sto(gdebut, gcompteur), symb_inferieur_egal(gcompteur, gfin),
                    symb_sto(gcompteur + gstep, gcompteur), symb_bloc(arg));
  else
    return symb_for(symb_sto(gdebut, gcompteur), symb_superieur_egal(gcompteur, gfin),
                    symb_sto(gcompteur + gstep, gcompteur), symb_bloc(arg));
}
...
static gen ti_decode_local(octet *& ptr, GIAC_CONTEXT) {
  ...
  for (;;) {
    ti_decode_newline(ptr, contextptr);
    if ((*(ptr - 1) == ENDPRGM_ITAG || *(ptr - 1) == ENDFUNC_ITAG) && *ptr == COMMAND_TAG)
      break;
    res.push_back(ti_decode_tag(ptr, contextptr));
  }
...
static gen ti_command_tag(octet *& ptr, GIAC_CONTEXT) {
  ...
  case ENDTRY_ITAG:
  case ENDWHILE_ITAG:
    return gensizeerr(gettext("End structure ") + print_INT_(*ptr) + " " + print_INT_(*(ptr - 2)) +
                      " " + print_INT_(*(ptr - 1)));
  case EXIT_ITAG:
    ptr -= 3; // skip offset
...
gen ti_decode_tag(octet *& ptr, GIAC_CONTEXT) {
  ...
  case SUBSCRIPT_TAG:
    ...
    if (res._VECTptr->size() == 2)
      return symbolic(at_at,
                      makesequence(res._VECTptr->front(), (res._VECTptr->back() - plus_one)));
    else
      return symbolic(
          at_at,
          makesequence(res._VECTptr->front(),
                       gen(vecteur(res._VECTptr->begin() + 1, res._VECTptr->end()), _SEQ__VECT) -
                           gen(vecteur(res._VECTptr->size() - 1, plus_one), _SEQ__VECT)));
  case RAND_TAG:
  ...
  case USERFUNC_TAG:
    ...
    if (res.type != _SYMB || res._SYMBptr->feuille.type != _VECT)
      return gensizeerr(contextptr);
    res = res._SYMBptr->feuille;
    return symbolic(at_of, makesequence(res._VECTptr->front(),
                                        gen(vecteur(res._VECTptr->begin() + 1, res._VECTptr->end()),
                                            _SEQ__VECT)));
  case FIG_TAG:
...
gen _unarchive_ti(const gen & g, GIAC_CONTEXT) {
  ...
  if (lu[6] == 'P') { // package/group file
    ...
    for (unsigned int i = 0; i < nprogs; ++i) {
      ...
      parser_filename(gname.print(contextptr), contextptr);
      unsigned int tt =
          buf[0x4e + 0x10 * i] * 65536 + buf[0x4d + 0x10 * i] * 256 + buf[0x4c + 0x10 * i] + 4;
      if (tt > s)
        return res;
      unsigned int ttt = buf[tt] * 256 + buf[tt + 1];
      if (s < ttt + tt + 2)
  ...
  define_unary_function_ptr5(at_ftantque, alias_at_ftantque, &__ftantque, _QUOTE_ARGUMENTS,
                             T_BLOC_END);

  static const char _ffonction_s[] = "ffonction";
  static define_unary_function_eval_quoted(__ffonction, &_for, _ffonction_s);
  define_unary_function_ptr5(at_ffonction, alias_at_ffonction, &__ffonction, _QUOTE_ARGUMENTS,
                             T_BLOC_END);
#endif
