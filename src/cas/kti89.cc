...
namespace giac {
  ...
  gen _logb(const gen & g, GIAC_CONTEXT) {
    ...
    int n = 0;
    gen e1(g._VECTptr->front()), b(g._VECTptr->back()), q;
  ...
  gen _taylor(const gen & g, GIAC_CONTEXT) {
    ...
    return _series(
        makesequence(v[0], symb_equal(v[1], x0), v[2]),
        contextptr); //_series(makesequence(v[0],symbolic(at_equal,makesequence(v[1],x0)),v[2]),contextptr);
  }
