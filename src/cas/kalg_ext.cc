  ...
  */
  ...
  gen fminmax(const gen & g, int minmax, GIAC_CONTEXT) {
    ...
    if (range == makevecteur(minus_inf, plus_inf))
      ...
      giac_assume(
          symb_and(symb_superieur_egal(var, range[0]), symb_inferieur_egal(var, range[1])),
          contextptr); // giac_assume(symbolic(at_and,makevecteur(symb_superieur_egal(var,range[0]),symb_inferieur_egal(var,range[1]))),contextptr);
      w = solve(df, var, 2, contextptr);
      if (savevar == var)
    ...
    if (g.type == _SYMB) {
      ...
      if ((s == at_sin) || (s == at_cos)) {
        a = vecteur(
            1,
            makeline(minus_one,
                     plus_one)); // a=vecteur(1,gen(makevecteur(minus_one,plus_one),_LINE__VECT));
        return 1;
      }
    }
    a = vecteur(
        1, makeline(minus_inf,
                    plus_inf)); // a=vecteur(1,gen(makevecteur(minus_inf,plus_inf),_LINE__VECT));
    return 1;
  }
  ...
  int sturmsign(const gen & g0, bool strict, GIAC_CONTEXT) {
    ...
    if (g.is_symb_of_sommet(at_prod)) {
      ...
      default:
        tmp = insturmsign(symb_prod(w), strict, contextptr);
        return tmp == -2 ? -2 : res * tmp;
      }
    }
