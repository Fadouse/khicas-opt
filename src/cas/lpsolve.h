...
*/
  ...
  enum lp_results { _LP_SOLVED, _LP_INFEASIBLE, _LP_UNBOUNDED, _LP_ERROR };

  enum lp_parsing_errors { _LP_ERR_SIZE = 1, _LP_ERR_TYPE = 2, _LP_ERR_DIM = 3 };

  enum lp_precision_types { _LP_EXACT, _LP_INEXACT, _LP_PROB_DEPENDENT };

  enum lp_relation_types { _LP_LEQ = -1, _LP_EQ = 0, _LP_GEQ = 1 };
  ...
  struct lp_range {
    gen lbound;
    gen ubound;
    inline void tighten_lbound(const gen & l, GIAC_CONTEXT) {
      lbound = max(lbound, l, contextptr);
    }
    inline void tighten_ubound(const gen & u, GIAC_CONTEXT) {
      ubound = min(ubound, u, contextptr);
    }
    inline bool is_unrestricted_below() {
      return is_inf(lbound);
    }
    inline bool is_unrestricted_above() {
      return is_inf(ubound);
    }
    lp_range();
  };
  ...
  struct lp_constraints {
    ...
    inline int ncols() {
      return lhs.empty() ? 0 : lhs.front()._VECTptr->size();
    }
  ...
  struct lp_node {
    ...
    ints cut_indices;
    inline bool is_integer_feasible() {
      return is_zero(infeas);
    }
