...
inline int smod(int a, int b, int c, int m) {
  ...
  gen a_root(const vecteur & v, const complex_double & c0, double eps) {
    ...
    if (0 && debug_infolevel > 2)
      COUT << "Aroot init " << c0 << " after renormalization: " << v_d << endl
           << "Diff " << dv_d << endl;
#endif
...
#define CLOCKS_PER_SEC 1e6
  ...
  int rref_reduce(std_matrix<gen> & M, vecteur & pivots, vector<int> & permutation, gen & det,
                  ...
                  bool step_rref, const vecteur & lv, bool convert_internal, GIAC_CONTEXT) {
    int linit = l;
  ...
  int mrref(const matrice & a, matrice & res, vecteur & pivots, gen & det, int l, int lmax, int c,
    ...
    int status =
        rref_reduce(M, pivots, permutation, det, detnum, algorithm, l, lmax, c, cmax,
                    ...
                    eps, step_rref, lv, convert_internal, contextptr);
  ...
  void hessenberg_schur(std_matrix<gen> & H, std_matrix<gen> & P, int maxiter, double eps,
    ...
    if (francis_schur(H, 0, n_orig, P, maxiter, std::sqrt(double(n_orig)) * eps, false, !real, true,
                      true, contextptr)) {
      return;
    }
  ...
  vecteur mpcar_int(const matrice & A, bool krylov, GIAC_CONTEXT, bool compute_pmin) {
    ...
    for (; pipd < (testvalue = logbound * charpol.size() / (n + 1.0));) {
      ...
      if (n > 10 && dbglevel < 2 && CLOCK() - initial_clock > 60 * CLOCKS_PER_SEC)
        dbglevel = 2;
      if (dbglevel > 1)
        CERR << CLOCK() << " " << 100 * pipd / testvalue << " % done"
             << (currentprob < proba ? ", stable." : ", unstable.") << endl;
  ...
  bool egv(const matrice & m0, matrice & p, vecteur & d, GIAC_CONTEXT, bool jordan,
    ...
    if (numeric_matrix) {
      ...
      for (int i = dim - 2; i >= 0; --i) {
        if (is_strictly_greater(abs(H[i + 1][i], contextptr), 10 * dim * dim * eps * sumabs,
                                contextptr)) {
          // shift H by cst_i
          shift = sumabs * cst_i;
          for (int j = 0; j < dim; ++j) {
      ...
      return ans;
#else
    bool ans = francis_schur(H, 0, dim, P, SOLVER_MAX_ITERATE, dim * eps, false, true, true, true,
                             contextptr);
    std_matrix_gen2matrice_destroy(P, p);
    std_matrix_gen2matrice_destroy(H, d);
