...
namespace giac {
  ...
  struct pf1 {
    ...
    pf1(const vecteur & n, const vecteur & d, const vecteur & f, int m)
        : num(n), den(d), fact(f), mult(m) {};
    pf1(const polynome & n, const polynome & d, const polynome & f, int m)
        : num(polynome2poly1(n, 1)), den(polynome2poly1(d, 1)), fact(polynome2poly1(f, 1)),
          mult(m) {}
  };

  gen complex_subst(const gen & e, const vecteur & substin, const vecteur & substout,
                    GIAC_CONTEXT) {
    ...
    gen res =
        simplifier(eval(subst(e, substin, substout, false, contextptr), 1, contextptr), contextptr);
  ...
  static gen nop_pow(const gen & e, GIAC_CONTEXT) {
    ...
    if ((e._VECTptr->back().type != _INT_) || (e._VECTptr->back().val >= 0) ||
        ((e._VECTptr->front().type == _SYMB) && (e._VECTptr->front()._SYMBptr->sommet == at_exp)))
      return symbolic(at_pow, change_subtype(e, _SEQ__VECT));
    return nop_inv(
        symbolic(at_pow, gen(makevecteur(e._VECTptr->front(), -e._VECTptr->back()), _SEQ__VECT)),
        contextptr);
  }
  ...
  void decompose_plus(const vecteur & arg, const gen & x, vecteur & non_constant,
                      gen & plus_constant, GIAC_CONTEXT) {
    non_constant.clear();
    plus_constant = zero;
  ...
  void decompose_prod(const vecteur & arg, const gen & x, vecteur & non_constant,
                      gen & prod_constant, bool signcst, GIAC_CONTEXT) {
    non_constant.clear();
  ...
  gen extract_cst(gen & u, const gen & x, GIAC_CONTEXT) {
    ...
    gen prod_constant = 1;
    decompose_prod(*u._SYMBptr->feuille._VECTptr, x, non_constant, prod_constant, false,
                   contextptr);
  ...
  gen linear_apply(const gen & e, const gen & x, gen & remains, int intmode, GIAC_CONTEXT,
                   gen (*f)(const gen &, const gen &, gen &, int, const context *)) {
    ...
    if (e.type != _SYMB)
      return gensizeerr(gettext("in linear_apply"));
    ...
    if (u == at_prod) {
      ...
      if (non_constant.empty())
        return gensizeerr(gettext("in linear_apply 2")); // otherwise the product would be constant
      if (non_constant.size() == 1)
  ...
  static gen substconj_(const gen & N, const gen & X, const gen & x, const gen & dephasage_,
                        bool residue_only, GIAC_CONTEXT) {
    ...
    gen res = normal_norootof(rb, contextptr) *
              symbolic(at_ln, pow(X, 2) + ratnormal(-2 * c * x, contextptr) * X +
                                  x.squarenorm(contextptr));
    gen atanterm = pi / cst_pi * symbolic(at_atan, (X - c * x) / (s * x));
    if (X.is_symb_of_sommet(at_tan))
      atanterm += pi * sign(s * x, contextptr) *
                  symbolic(at_floor, X._SYMBptr->feuille / pi + plus_one_half);
    res = res + N2 * atanterm;
    return res;
  }

  static gen substconj(const gen & N, const gen & X, const gen & x, const gen & dephasage,
                       bool residue_only, GIAC_CONTEXT) {
    if (has_i(N)) {
      gen Nr, Ni;
      reim(N, Nr, Ni, contextptr);
      return substconj_(Nr, X, x, dephasage, residue_only, contextptr) +
  ...
  gen _surd(const gen & args, GIAC_CONTEXT) {
    ...
    if (is_equal(a)) {
      ...
      return symbolic(at_equal, makesequence(_surd(makesequence(a0, b), contextptr),
                                             _surd(makesequence(a1, b), contextptr)));
    }
    if (is_undef(a))
      return a;
    if (is_undef(b))
    ...
    if (!has_evalf(a, aa, 1, contextptr)) {
      if (c.type == _INT_ && c == b &&
          (c.val % 2 == 0 || (a.is_symb_of_sommet(at_pow) && a._SYMBptr->feuille[1].type == _INT_ &&
                              a._SYMBptr->feuille[1].val % c.val == 0)))
  ...
  static gen makelnatan(const gen & N, const gen & X, const gen & c0, int n, bool residue_only,
                        GIAC_CONTEXT) {
    gen c(c0), res(0);
    if (n % 2) {
    ...
    if (is_positive(c, contextptr)) {
      if (n == 2)
        ...
        c = pow(c, inv(n, contextptr), contextptr);
      if (!residue_only)
        res += normal_norootof(subst(N, X, c, false, contextptr), contextptr) *
                   lnabs2(X - c, X, contextptr) +
               normal_norootof(subst(N, X, -c, false, contextptr), contextptr) *
                   lnabs2(X + c, X, contextptr);
      for (int i = 1; i < n / 2; ++i)
        res += substconj(N, X, c, gen(2 * i) / n * cst_pi, residue_only, contextptr);
    } else {
  ...
  static bool integrate_rothstein_trager(const polynome & num, const vecteur & v, const vecteur & l,
                                         const gen & X, gen & res, int intmode, GIAC_CONTEXT) {
            ...
            if (reel) {
              ...
              if (is_zero(raci, contextptr))
                ...
                if (wj < w.size()) {
                  ...
                  resadd += racr * symb_ln(gcdr * gcdr + gcdi * gcdi) -
                            2 * raci * ln2sumatan(gcdr, gcdi, l, contextptr);
                  w.erase(w.begin() + wj);
                } else
  ...
  static bool integrate_deno_length_2(const polynome & num, const vecteur & v, const vecteur & l,
                                      const vecteur & lprime, gen & res, bool residue_only,
      ...
      if (it == itend) { // ok!
        ...
        if ((intmode & 2) == 0)
          gprintf(
              step_ratfrac,
              gettext(
                  "Integration of a rational fraction with denominator %gen\nroots are obtained by "
                  "solving the 2nd order equation %gen=0 then extracting nth-roots"),
              makevecteur(a * symb_pow(vx_var, 2 * n) + b * symb_pow(vx_var, n) + c,
                          a * symb_pow(vx_var, 2) + b * vx_var + c),
              contextptr);
        ...
        if (is_zero(im(a, contextptr)) && is_zero(im(b, contextptr)) &&
            is_zero(im(c, contextptr))) {
          if (!is_positive(-delta, contextptr)) {
            ...
            for (int i = 0; i < d; ++i)
              res += substconj_(N / c2, X, moduled, (argument + 2 * i * cst_pi) / d, residue_only,
                                contextptr);
          }
          return true;
        }
    ...
    if (n % d)
      return residue_only ? false
                          : integrate_rothstein_trager(num, v, l, X, res, intmode, contextptr);
    if (d != n) {
      ...
      if (!deg)
        return residue_only ? false
                            : integrate_rothstein_trager(num, v, l, X, res, intmode, contextptr);
      if ((intmode & 2) == 0)
        gprintf(step_cyclotomic,
                gettext("Integrate rational fraction with denominator a cyclotomic polynomial, "
                        "roots are primitive roots of %gen=0"),
                makevecteur(a * symb_pow(vx_var, deg) + b), contextptr);
      ...
      // 1/vprime=Vprime/d
      gen N = normal(
          _quorem(makesequence(r2e(num, l, contextptr) * horner(r2e(Vprime, lprime, contextptr), X),
                               horner(r2e(v, lprime, contextptr), X), X),
                  contextptr)[1] /
              r2e(dd, lprime, contextptr),
          contextptr);
      ...
      if ((intmode & 2) == 0)
        gprintf(step_nthroot,
                gettext("Integrate rational fraction with denominator %gen=0\nroots are deduced "
                        "from nth-roots of unity"),
                makevecteur(a * symb_pow(vx_var, n) + b), contextptr);
      // int(num/(a*X^n+b),X)=sum(x=rootof(-b/a),num*x/(-n*b)*ln(X-x))
  ...
  static void xtoinvx(const gen & a, const modpoly & n, const modpoly & d, modpoly & newn,
                      modpoly & newd, bool dx) {
    int ns = int(n.size());
    int nd = int(d.size());
    newn = vecteur(ns);
    newd = vecteur(nd);
  ...
  static gen integrate_rational(const gen & e, const gen & x, gen & remains_to_integrate,
                                gen & xvar, int intmode, GIAC_CONTEXT);

  static void solve_aPprime_plus_P(const gen & anum, const gen & aden, const vecteur & Q,
                                   vecteur & R, gen & Pden) {
  ...
  static gen integrate_linearizable(const gen & e, const gen & gen_x, gen & remains_to_integrate,
                                    int intmode, bool do_risch, GIAC_CONTEXT) {
    // exp linearization
    vecteur vexp;
    gen res;
    ...
    if ((intmode & 2) == 0) {
      ...
      if (vexp.size() > 2 || !is_zero(ratnormal(tmp - e, contextptr)))
        gprintf(step_linearizable, gettext("Integrate linearizable expression %gen -> %gen"),
                makevecteur(e, tmp), contextptr);
    }
    ...
    for (; it != itend;) {
      ...
      if (!is_quadratic_wrt(reaxb, gen_x, rex2, rea, reb, contextptr)) {
        ...
        if (lv.size() == 1) {
          ...
          if (C.type == _VECT && C._VECTptr->size() > 2) {
            ...
            gen a = -Cv[1] / (n * c);
            // must be c*(x-a)^n
            if (C == _coeff(makesequence(c * pow(gen_x - a, n, contextptr), gen_x), contextptr) &&
                ((n % 2) || is_positive(-c, contextptr))) {
              ...
              if (C.type == _VECT) {
                ...
                for (int b = 0; b <= m; ++b) {
                  ires +=
                      Cv[m - b] *
                      _lower_incomplete_gamma(
                          makesequence(gen(b + 1) / gen(n), c * pow(gen_x - a, n)), contextptr) /
                      pow(ca, b + 1, contextptr);
                }
                if (n % 2 == 0) {
      ...
      if (!is_zero(rex2)) {
        if (1
          ...
          gen ppart = subst(r2e(poly12polynome(vres, 1, les_vars), les_var, contextptr), gen_x,
                            gen_x + decal, false, contextptr) /
                      r2e(vresden, vecteur(les_var.begin() + 1, les_var.end()), contextptr) *
                      exp(reaxb, contextptr);
          // add erf part from the last coeff vres[vs]
          gen a = -rex2; // r2e(-ina,les_var,contextptr);
          gen sqrta = sqrt_noabs(a, contextptr);
          gen erfpart = r2e(in_coeffnumv[vs], cdr_VECT(les_var), contextptr) *
                        symbolic(at_sqrt, cst_pi) / sqrta * exp(cst, contextptr) / 2 *
                        _erf(sqrta * (gen_x + decal), contextptr);
      ...
      if ((intmode & 2) == 0) {
        ...
        if (vtrig.size() > 2 || !is_zero(ratnormal(tmp - coeff, contextptr)))
          gprintf(step_triglinearizable,
                  gettext("Integrate trigonometric linearizable expression %gen -> %gen"),
                  makevecteur(coeff, tmp), contextptr);
      }
      ...
      for (; jt != jtend;) {
        ...
        gen imc;
        bool quad = imaxb.type == _SYMB &&
                    is_quadratic_wrt(imaxb._SYMBptr->feuille, gen_x, ima, imb, imc, contextptr);
        if (!coeffnotpoly && quad && !is_zero(ima) && angle_radian(contextptr)) {
          imc = _trig2exp(coeff * exp(reaxb, contextptr) * imaxb, contextptr);
          res +=
              integrate_linearizable(imc, gen_x, remains_to_integrate, intmode, true, contextptr);
          continue;
        }
        if (coeffnotpoly || (imaxb.type == _SYMB && !is_linear_wrt(imaxb._SYMBptr->feuille, gen_x,
                                                                   ima, imb, contextptr))) {
          if (trig_type)
        ...
        if (trig_type)
          ...
          if (is_zero(rea)) {
            ...
            res = res + exp(reb, contextptr) *
                            integrate_rational(coeff, gen_x, tmprem, xvar, intmode, contextptr);
        ...
        resplus = rdiv(
            r2e(poly12polynome(resnum, 1, les_vars), les_var, contextptr),
            r2e(poly12polynome(vecteur(1, resden * in_coeffden), 1, les_vars), les_var, contextptr),
            contextptr);
        if (step_infolevel(contextptr)) {
          gprintf(step_polyexp,
                  gettext("Primitive of %gen is polynomial of same degree*same exponential %gen"),
                  makevecteur(coeff * symb_exp(reaxb + cst_i * imaxb),
                              resplus * symb_exp(reaxb + cst_i * imaxb)),
                  contextptr);
        }
        if (!trig_type) {
        ...
        if (coeff_is_real) {
          ...
          if (trig_type == 1)
            res = res + exp(reaxb, contextptr) *
                            (resim * cos(imaxb, contextptr) + resre * sin(imaxb, contextptr));
          else
            res = res + exp(reaxb, contextptr) *
                            (resre * cos(imaxb, contextptr) - resim * sin(imaxb, contextptr));
          continue;
        }
        fxnd(in_rea - cst_i * in_ima, in_anum, in_aden);
        solve_aPprime_plus_P(in_anum, in_aden, in_coeffnumv, resnum, resden);
        gen resmoins = rdiv(
            ...
            contextptr);
        if (trig_type == 1)
          res = res + exp(reaxb, contextptr) * rdiv(resplus * exp(cst_i * imaxb, contextptr) -
                                                        resmoins * exp(-cst_i * imaxb, contextptr),
                                                    plus_two * cst_i, contextptr);
        else
          res = res + exp(reaxb, contextptr) * rdiv(resplus * exp(cst_i * imaxb, contextptr) +
                                                        resmoins * exp(-cst_i * imaxb, contextptr),
                                                    plus_two, contextptr);
  ...
  gen linear_integrate_nostep(const gen & e, const gen & x, gen & remains_to_integrate, int intmode,
                              GIAC_CONTEXT);
  ...
  static int integrate_sqrt(gen & e, const gen & gen_x, const vecteur & rvar, gen & res,
                            gen & remains_to_integrate, int intmode,
                            GIAC_CONTEXT) { // x and a power
    ...
    if ((exposant.type == _FRAC) && (exposant._FRACptr->num.type == _INT_) &&
        (exposant._FRACptr->den.type == _INT_)) {
      ...
      if (is_linear_wrt(argument, gen_x, a, b, contextptr)) {
        ...
        remains_to_integrate =
            rdiv(d, a, contextptr) * complex_subst(tmprem, gen_x, fnc_inverse, contextptr);
      ...
      if (is_linear_wrt(fr_n, gen_x, a, b, contextptr) &&
          is_linear_wrt(fr_d, gen_x, ap, bp, contextptr)) {
        // argument=(a*x+b)/(ap*x+bp)=t^d
        // -> x=(bp*t^d-b)/(a-ap*t^d)
        // -> dx= d*(b*ap-a*bp)*t^(d-1)/(a-ap*t^d)^2
        vecteur substin(makevecteur(argument, gen_x));
        vecteur substout(makevecteur(
            pow(gen_x, d), rdiv(bp * pow(gen_x, d) - b, a - ap * pow(gen_x, d), contextptr)));
        tmpe = complex_subst(e, substin, substout, contextptr) *
               rdiv(pow(gen_x, d - 1), pow(a - ap * pow(gen_x, d), 2), contextptr);
        tmpres = linear_integrate_nostep(tmpe, gen_x, tmprem, intmode, contextptr);
        gen fnc_inverse =
            pow(rdiv(a * gen_x + b, ap * gen_x + bp, contextptr), fraction(1, d), contextptr);
      ...
      if (frdconst) {
        ...
        if (is_constant_wrt(num, tmpx, contextptr)) {
          res =
              linear_integrate_nostep(num / den, gen_x, remains_to_integrate, intmode, contextptr);
          return 1;
        }
	 ...
	 */
	  ...
	  */
          ...
          alpha = integrate_rational(alpha / den, gen_x, remains_to_integrate, xvar, intmode,
                                     contextptr);
          if (is_undef(alpha)) {
            res = alpha;
            return 2;
	  ...
	  */
          ...
          if (tmpv.type != _VECT) {
            if (tmpv.type == _FRAC) {
              ...
              if (tmpv._FRACptr->num.type == _VECT)
                tmpv =
                    multvecteur(inv(tmpv._FRACptr->den, contextptr), *tmpv._FRACptr->num._VECTptr);
            }
            if (tmpv.type != _VECT)
          ...
          if (is_quadratic_wrt(argument, gen_x, a, b, c, contextptr)) {
            if (!is_positive(-a, contextptr)) {
              ...
              gen m(id_m);
              tmpe = eval(rdiv(complex_subst(e * sqrt(argument, contextptr), argument,
                                             pow(m + sqrta * gen_x, 2), contextptr),
                               b - plus_two * sqrta * m, contextptr),
                          1, contextptr);
              tmpe = ratnormal(complex_subst(tmpe, gen_x,
                                             rdiv(m * m - c, b - plus_two * sqrta * m, contextptr),
                                             contextptr),
                               contextptr);
              // factor?
              ...
              remains_to_integrate =
                  remains_to_integrate + complex_subst(plus_two * tmprem, m,
                                                       sqrt(argument, contextptr) - sqrta * gen_x,
                                                       contextptr);
              res = alpha + complex_subst(plus_two * tmpres, m,
                                          sqrt(argument, contextptr) - sqrta * gen_x, contextptr);
              return 2;
            } else {
              gen D = b * b - gen(4) * a * c;
		...
		( *			dx/sqrt[y]=-2*du/[sqrt[-a]*[1+u^2]] * )
	      */
              ...
              gen u(id_u), uu(u);
              gen uasx = rdiv(D - plus_two * sqrta * sqrt(argument, contextptr),
                              plus_two * a * gen_x + b, contextptr);
              tmpe = ratnormal(e * sqrt(argument, contextptr), contextptr);
              tmpe = complex_subst(
                  tmpe, gen_x,
                  rdiv(rdiv(plus_two * u * D, 1 + u * u, contextptr) - b, plus_two * a, contextptr),
                  contextptr);
              ...
              // -> asin(a*x+b/2)
              vecteur vin(makevecteur(u, symbolic(at_atan, u))),
                  vout(makevecteur(
                      uasx, inv(-2, contextptr) * sD *
                                asin(ratnormal((-2 * a * gen_x - b) / D, contextptr), contextptr)));
              remains_to_integrate =
                  remains_to_integrate + complex_subst(tmprem, vin, vout, contextptr);
      ...
      if (fr_dp.type == _POLY)
        dendeg = fr_dp._POLYptr->lexsorted_degree();
      if (dendeg > 0 || fr_np.type != _POLY)
        return 1;
  ...
  static gen integrate_piecewise(gen & e, const gen & piece, const gen & gen_x,
                                 gen & remains_to_integrate, GIAC_CONTEXT, int intmode) {
  ...
  static gen integrate_trig_fraction(gen & e, const gen & gen_x, vecteur & var,
                                     const gen & coeff_trig, int trig_fraction,
    ...
    for (; vart != vartend; ++vart) {
      ...
      if (n.type != _INT_)
        return gensizeerr(gettext("trig_fraction"));
    ...
    if (trig_fraction == 4) { // everything depends on exp(x)
      ...
      if ((intmode & 2) == 0)
        gprintf(step_ratfracexp,
                gettext("Integrate rational fraction of exponential %gen by %gen change of "
                        "variable, leading to integral of %gen"),
                makevecteur(e, exp(gen_x, contextptr), f), contextptr);
      tmpres = linear_integrate_nostep(f, gen_x, tmprem, intmode, contextptr);
      gen expx = exp(coeff_trig * gen_x + coeff_cst, contextptr);
      if ((intmode & 2) == 0)
        gprintf(step_backsubst, gettext("Back substitution %gen->%gen in %gen"),
                makevecteur(gen_x, expx, tmprem), contextptr);
    ...
    f = complex_subst(f, tan(rdiv(gen_x, plus_two, contextptr), contextptr), gen_x, contextptr) *
        inv(plus_one + pow(gen_x, 2), contextptr);
    if ((intmode & 2) == 0)
      gprintf(step_ratfractrig,
              gettext("Integrate rational fraction of trigonometric %gen by %gen change of "
                      "variable, leading to integral of %gen"),
              makevecteur(e, tanxsur2, f), contextptr);
    vecteur vf(1, gen_x);
    rlvarx(f, gen_x, vf);
    if (vf.size() <= 1)
      ...
      if ((intmode & 2) == 0)
        gprintf(step_backsubst, gettext("Back substitution %gen->%gen in %gen"),
                makevecteur(gen_x, tanxsur2, tmpres), contextptr);
    ...
    if (tmpres == 0)
      ...
      remains_to_integrate =
          rdiv(plus_two, coeff_trig, contextptr) * tmprem * (1 + pow(tanxsur2, 2));
  ...
  bool intgab_ratfrac(const gen & e, const gen & x, gen & value, GIAC_CONTEXT) {
    ...
    if (r_den.type != _POLY) { // not convergent
      if (num.lexsorted_degree() % 2)
        ...
        value = subst(r2e(r_num, l, contextptr), x, 1, false, contextptr) /
                r2e(r_den, l, contextptr) * plus_inf;
      return true;
    }
    ...
    if (integrate_deno_length_2(num, vtmp, l, lprime, value, true, 2 /* no step info*/,
                                contextptr)) {
    ...
    for (; it != itend; ++it) {
      ...
      case 2: // 2nd order
        ...
        value += rdiv(r2e(atannum, lprime, contextptr),
                      (r2e(alpha, lprime, contextptr)) * sqrtdelta, contextptr);
        break;
      default: // divide a*it->num =b*it->den.derivative()+c
        ...
        if (!c.coord.empty()) {
          ...
          if (!integrate_deno_length_2(c, vtmp, l, lprime, value, true, 2 /* no step info*/,
                                       contextptr))
            return false;
        }
        break;
  ...
  static gen integrate_rational_end(vector<pf<gen>>::iterator & it,
                                    vector<pf<gen>>::const_iterator & itend, const gen & x,
                                    const gen & xvar, const vecteur & l, const vecteur & lprime,
                                    const polynome & ipnum, const polynome & ipden,
                                    const gen & ratpart, gen & remains_to_integrate, int intmode,
                                    GIAC_CONTEXT) {
    gen lnpart(0), deuxaxplusb, sqrtdelta;
    int s = ipnum.dim;
    polynome a(s), b(s), c(s);
    ...
    for (; it != itend; ++it) {
      ...
      switch (deg) {
      case 1: // 1st order
        lnpart = lnpart + rdiv(r2e(it->num, l, contextptr), r2e(firstcoeff(it->den), l, contextptr),
                               contextptr) *
                              lnabs2(r2e(it->fact, l, contextptr), xvar, contextptr);
        break;
      case 2: // 2nd order
        findabcdelta(it->fact, a, b, c, delta);
        Delta = r2e(delta, lprime, contextptr);
        ...
        lnpart = lnpart +
                 rdiv(r2e(d, lprime, contextptr), r2e(lnpartden, lprime, contextptr), contextptr) *
                     gen(uselog ? lnabs2(r2e(it->fact, l, contextptr), xvar, contextptr)
                                : symbolic(at_ln, r2e(it->fact, l, contextptr)));
        ...
        if (uselog) {
          ...
          simplify(atannum, alpha);
          lnpart = lnpart +
                   rdiv(r2e(atannum, lprime, contextptr),
                        (r2e(alpha, lprime, contextptr)) * sqrtdelta, contextptr) *
                       lnabs2(rdiv(deuxaxplusb - sqrtdelta, deuxaxplusb + sqrtdelta, contextptr),
                              xvar, contextptr);
        } else {
          ...
          if (tmpatan.is_symb_of_sommet(at_tan))
            ...
            gen beta, gamma;
            if ( //0 &&
                xvar.is_symb_of_sommet(at_tan) &&
                is_linear_wrt(tmpatan, xvar, beta, gamma, contextptr) &&
                is_strictly_greater(4 * beta, gamma * gamma, contextptr)) {
              gen argtan = ratnormal(2 * xvar._SYMBptr->feuille, contextptr);
              gen si = symbolic(at_sin, argtan), ci = symbolic(at_cos, argtan);
              tmpatan = symbolic(at_atan, ratnormal(((beta - 1) * si + gamma * (1 + ci)) /
                                                        (1 + beta + gamma * si + (1 - beta) * ci),
                                                    contextptr));
              ...
              if (xvar.is_symb_of_sommet(at_tan)) {
                if (do_lnabs(contextptr)) {
                  ...
                  residue = cst_pi * sign(residue, contextptr) *
                            _floor((xvar._SYMBptr->feuille / cst_pi + plus_one_half), contextptr);
                }
              } else {
                ...
                if (xvar.type != _IDNT) {
                  ...
                  residue = ratnormal(limit(tmpatan, *x._IDNTptr, 0, -1, contextptr) -
                                          limit(tmpatan, *x._IDNTptr, 0, 1, contextptr),
                                      contextptr);
          ...
          tmpatan += residue;
          lnpart = lnpart + rdiv(r2e(atannum, lprime, contextptr),
                                 (r2e(alpha, lprime, contextptr)) * sqrtdelta, contextptr) *
                                tmpatan;
        } // end else uselof
        break;
      default: // divide a*it->num =b*it->den.derivative()+c
        ...
        if (!c.coord.empty()) {
          ...
          if (!integrate_deno_length_2(c, vtmp, l, lprime, lnpart, false, intmode, contextptr))
            remains_to_integrate +=
                r2sym(vector<pf<gen>>(1, pf<gen>(c, a * it->den, it->fact, 1)), l, contextptr);
        }
        // extract log part b/a*ln[fact]
        simplify(b, a);
        if (!is_zero(b))
          lnpart = lnpart + rdiv(r2e(b, l, contextptr), r2e(a, l, contextptr), contextptr) *
                                lnabs(r2e(it->fact, l, contextptr), contextptr);
        break;
      }
    }
    return rdiv(r2e(ipnum.integrate(), l, contextptr), r2e(ipden, l, contextptr), contextptr) +
           ratpart + lnpart;
  }
  ...
  static gen integrate_rational(const gen & e, const gen & x, gen & remains_to_integrate,
                                gen & xvar, int intmode, GIAC_CONTEXT) {
    if (x.type != _IDNT)
      return gensizeerr(contextptr); // see limit
    if (has_num_coeff(e)) {
    ...
    vecteur lprime(l);
    if (lprime.front().type != _VECT)
      return gensizeerr(gettext("in integrate_rational"));
    lprime.front() = cdr_VECT(*(lprime.front()._VECTptr));
    gen r = e2r(e, l, contextptr);
    // cout << "Int " << r << '\n';
    gen r_num, r_den;
    ...
    vecteur vtmp;
    if (den.coord.size() == 2 && den.lexsorted_degree() != 1 &&
        num.lexsorted_degree() < den.lexsorted_degree() && den.coord.back().index.is_zero() &&
        xvar.type == _IDNT) {
      polynome2poly1(den, 1, vtmp);
      r = 0;
    ...
    if (aa > 1) { // Apply
      if ((intmode & 2) == 0)
        gprintf(step_ratfracpow,
                gettext("Integrate rational fraction %gen, change of variable %gen"),
                makevecteur(e, symb_equal(x, symb_pow(x, aa))), contextptr);
      int k = 0;
      k = den_deg % aa;
      ...
      xvar = pow(xvar, aa);
      return integrate_rational(r2e(fraction(num, den), l, contextptr), x, remains_to_integrate,
                                xvar, intmode, contextptr);
    }
    ...
    if ((intmode & 2) == 0)
      gprintf(step_ratfracsqrfree, gettext("Integrate rational fraction %gen"),
              makevecteur(_sqrfree(e, contextptr)), contextptr);
    ...
    for (; it != itend; ++it) {
      ...
      if ((it->mult > 1) && (intmode & 2) == 0) {
        ...
        gprintf(
            step_ratfrachermite,
            gettext("Partial fraction %gen -> Hermite reduction -> integrate squarefree part %gen"),
            makevecteur(inv(r2sym(fact2, l, contextptr), contextptr) * r2e(it->num, l, contextptr) /
                            fact1,
                        r2e(single.num, l, contextptr) / r2e(single.den, l, contextptr)),
            contextptr);
      }
    ...
    if ((intmode & 2) == 0)
      gprintf(step_ratfracfinal, gettext("Partial fraction integration of %gen"),
              makevecteur(r2sym(finaldecomp, l_orig, contextptr)), contextptr);
    it = finaldecomp.begin();
    itend = finaldecomp.end();
    ...
    if (xvar.type == _IDNT && tmp1.type == _VECT && tmp1._VECTptr->size() == 2) {
      gen tmp2 =
          _quorem(makesequence(tmp1._VECTptr->front(), tmp1._VECTptr->back(), xvar), contextptr);
    ...
    return integrate_rational_end(it, itend, x, xvar, l, lprime, ipnum, ipden, ratpart,
                                  remains_to_integrate, intmode, contextptr);
  }
  ...
  static gen int_atan(const gen & x, GIAC_CONTEXT) {
    if (angle_radian(contextptr))
      ...
      return x * atan(x, contextptr) * deg2rad_e -
             rdiv(ln(pow(x, 2) + 1, contextptr), plus_two, contextptr);
    //grad
    else
      return x * atan(x, contextptr) * grad2rad_e -
             rdiv(ln(pow(x, 2) + 1, contextptr), plus_two, contextptr);
  }
  ...
  static gen int_atanh(const gen & x, GIAC_CONTEXT) {
    return x * atan(x, contextptr) -
           rdiv(ln(abs(pow(x, 2) - 1, contextptr), contextptr), plus_two, contextptr);
  }

  static const gen_op_context primitive_tab_primitive[] = {
      int_sin,  int_cos,  int_tan,  int_exp, int_sinh,  int_cosh,  int_tanh,
      int_asin, int_acos, int_atan, xln_x,   int_asinh, int_acosh, int_atanh};
...
#endif
  bool is_rewritable_as_f_of0(const gen & fu, const gen & u, gen & fx, const gen & gen_x,
                              GIAC_CONTEXT);

  static bool in_is_rewritable_as_f_of(const gen & fu, const gen & u, gen & fx, const gen & gen_x,
                                       GIAC_CONTEXT) {
    if (fu.type == _VECT) {
      vecteur res;
    ...
    if ((u._SYMBptr->sommet == at_pow) && (u._SYMBptr->feuille._VECTptr->back().type == _INT_) &&
        (u._SYMBptr->feuille._VECTptr->front() == f._VECTptr->front())) {
  ...
  bool is_rewritable_as_f_of0(const gen & fu, const gen & u, gen & fx, const gen & gen_x,
    ...
    if (u.type == _SYMB) {
        ...
        if (tmpu.type == _VECT && tmpu._VECTptr->size() == 2) {
          ...
          if (expo.type == _INT_) {
            if (is_linear_wrt(tmpu, gen_x, a, b, contextptr)) {
              fx = complex_subst(
                  fu, makevecteur(u, gen_x),
                  makevecteur(gen_x, rdiv(pow(gen_x, expo, contextptr) - b, a, contextptr)),
                  contextptr);
              return true;
            }
      ...
      if (u._SYMBptr->sommet == at_prod) {
        ...
        if (non_constant.empty())
          return false; // setsizeerr(gettext("in is_rewritable_as_f_of_f"));
        if (!is_one(alpha)) {
          gen tmpu, tmpfx;
      ...
      if (u._SYMBptr->sommet == at_plus) {
        if (u._SYMBptr->feuille.type != _VECT)
          return is_rewritable_as_f_of0(fu, u._SYMBptr->feuille, fx, gen_x, contextptr);
        if (_is_polynomial(makesequence(fu, gen_x), contextptr) == 1 &&
            _is_polynomial(makesequence(u, gen_x), contextptr) == 1) {
        ...
        if (non_constant.empty())
          return false; // setsizeerr(gettext("in is_rewritable_as_f_of_f 2"));
        if (!is_zero(alpha)) {
          gen tmpu, tmpfx;
  ...
  bool is_rewritable_as_f_of(const gen & fu_, const gen & u, gen & fx, const gen & gen_x,
                             GIAC_CONTEXT) {
  ...
  gen integrate_gen_rem(const gen & e_orig, const gen & x_orig, gen & remains_to_integrate,
                        int intmode, GIAC_CONTEXT) {
  ...
  static bool integration_exponential_zero(const gen & g, const gen & x, gen & root, GIAC_CONTEXT) {
    if (complex_mode(contextptr) || complex_variables(contextptr) || taille(g, 65) > 64)
      return false;
    vecteur atoms = lop(g, at_exp);
    if (atoms.size() != 1)
      return false;
    gen A, B, a, b;
    gen atom = atoms[0];
    bool reciprocal = false;
    if (!is_linear_wrt(g, atom, A, B, contextptr) || !equation_rational(A) ||
        !equation_rational(B)) {
      vecteur inverses = mergevecteur(lop(g, at_inv), lop(g, at_pow));
      bool found = false;
      for (unsigned j = 0; j < inverses.size(); ++j) {
        ...
        if ((inverses[j].is_symb_of_sommet(at_inv) && f == atoms[0]) ||
            (inverses[j].is_symb_of_sommet(at_pow) && f.type == _VECT && f._VECTptr->size() == 2 &&
             f[0] == atoms[0] && f[1] == -1)) {
          atom = inverses[j];
          found = true;
          break;
      ...
      if (!found || !is_linear_wrt(g, atom, A, B, contextptr))
        return false;
      reciprocal = true;
    }
    if (!equation_rational(A) || is_zero(A) || !equation_rational(B) ||
        !is_linear_wrt(atoms[0]._SYMBptr->feuille, x, a, b, contextptr) || !equation_rational(a) ||
        is_zero(a) || !equation_rational(b))
      return false;
    gen ratio = -B / A;
    if (!is_strictly_positive(ratio, contextptr))
      return false;
    root = ((reciprocal ? -ln(ratio, contextptr) : ln(ratio, contextptr)) - b) / a;
    return true;
  }

  static bool integrate_step0(gen & e, const gen & gen_x, vecteur & l1, vecteur & m1, gen & res,
                              gen & remains_to_integrate, GIAC_CONTEXT, int intmode) {
    const identificateur & id_x = *gen_x._IDNTptr;
    vecteur l2, m2, l3, l4;
    ...
    *logptr(contextptr) << gettext("Warning, integration of abs or sign assumes constant sign by "
                                   "intervals (correct if the argument is real):\nCheck ")
                        << l1 << '\n';
    ...
    for (int j = 0; j < i; ++j) {
      ...
      if (val.is_symb_of_sommet(at_sign)) {
        ...
        bool warn = true;
        bool affine = is_linear_wrt(val2, gen_x, a, b, contextptr) && !is_zero(a) &&
                      ((has_evalf(a, r, 1, contextptr) && has_evalf(b, r, 1, contextptr)) ||
                       lvar(res) == lidnt(res));
        if (affine)
          r = -b / a;
        ...
        gen limsup = dolim ? limit(res, *l3.front()._IDNTptr, l5.front(), 0, contextptr)
                           : subst(res, l3, l5, false, contextptr);
        l5[j] = -1;
        gen liminf = dolim ? limit(res, *l3.front()._IDNTptr, l5.front(), 0, contextptr)
                           : subst(res, l3, l5, false, contextptr);
#endif
          gen tmp = ratnormal((limit(liminf, id_x, r, -1, contextptr) -
                               limit(limsup, id_x, r, 1, contextptr)) /
                                  2,
                              contextptr) *
                    val;
          if (is_undef(tmp) || is_inf(tmp))
            *logptr(contextptr) << gettext("Unable to cancel step at ") + r.print(contextptr) +
                                       " of "
                                << limsup << "-" << liminf << '\n';
          else
            resadd += tmp;
        ...
        if (warn)
          *logptr(contextptr) << gettext("Discontinuities at zeroes of ") << val._SYMBptr->feuille
                              << " were not checked" << '\n';
      }
    }
  ...
  static bool detect_inv_trigln(gen & e, vecteur & rvar, const gen & gen_x, gen & res,
                                gen & remains_to_integrate, bool additional_check, int intmode,
                                GIAC_CONTEXT) {
    ...
    for (; rvt != rvtend; ++rvt) {
      ...
      if (rvtt > 7) {
          ...
          for (unsigned tmpi = 0; tmpi < tmpv.size(); ++tmpi) {
            ...
            if (tmpvi.type == _VECT && tmpvi._VECTptr->size() == 2) {
              ...
              if (ratnormal(tmpvi0 - 1 + pow(a * gen_x + b, 2, contextptr), contextptr) == 0) {
                ...
                substout.push_back(pow(symbolic(rvtt == 8 ? at_cos : at_sin, gen_x),
                                       2 * tmpvi._VECTptr->back(), contextptr));
              }
            }
          }
        }
        ...
        if ((intmode & 2) == 0)
          gprintf(step_ratfracchgvar,
                  gettext("Integrate %gen, change of variable %gen->%gen, new integral %gen"),
                  makevecteur(e, gen_x, xt, tmpe), contextptr);
        tmpres = linear_integrate_nostep(tmpe, gen_x, tmprem, intmode, contextptr);
        if ((intmode & 2) == 0)
          gprintf(step_backsubst, gettext("Back substitution %gen->%gen in %gen"),
                  makevecteur(gen_x, *rvt, tmpres), contextptr);
        remains_to_integrate =
            complex_subst(rdiv(tmprem, dxt, contextptr), gen_x, *rvt, contextptr);
  ...
  gen integrate_id_rem(const gen & e_orig, const gen & gen_x, gen & remains_to_integrate,
                       GIAC_CONTEXT) {
  ...
  void surd2pow(const gen & e, vecteur & subst1, vecteur & subst2, GIAC_CONTEXT) {
    ...
    for (unsigned i = 0; i < l1NTHROOT.size(); ++i) {
      ...
      if (g._SYMBptr->feuille.type == _VECT && g._SYMBptr->feuille._VECTptr->size() == 2) {
        vecteur gv = *g._SYMBptr->feuille._VECTptr;
#if defined GIAC_GGB
        gv = makevecteur(subst(gv[1], l1NTHROOT, l2NTHROOT, false, contextptr),
                         inv(gv[0], contextptr));
#else
  ...
  static bool small_polynomial(const gen & e, const gen & x, unsigned & budget) {
    if (!budget)
      return false;
    --budget;
    if (e == x || e.type == _INT_ || e.type == _ZINT)
      return true;
    if (e.type == _FRAC)
    ...
    if (op == at_neg)
      return small_polynomial(f, x, budget);
    if (f.type != _VECT)
      return false;
    ...
    if (op != at_plus && op != at_prod)
      return false;
    for (unsigned i = 0; i < v.size(); ++i)
      if (!small_polynomial(v[i], x, budget))
        return false;
    return true;
  }
  ...
  static int continuous_parity(const gen & e, const gen & x, unsigned & budget) {
    ...
    const vecteur & v = *f._VECTptr;
    if (op == at_pow) {
      if (v.size() != 2 || v[1].type != _INT_ || v[1].val < 0)
        return -1;
      int p = continuous_parity(v[0], x, budget);
      return p < 0 ? -1 : (p && v[1].val % 2);
    }
    if ((op != at_plus && op != at_prod) || v.empty())
      return -1;
    int p = continuous_parity(v[0], x, budget);
  ...
  static bool small_sparse_polynomial(const gen & e, const gen & x, sparse_poly1 & out,
                                      GIAC_CONTEXT) {
    out.clear();
    const vecteur * terms = e.is_symb_of_sommet(at_plus) && e._SYMBptr->feuille.type == _VECT
                                ? e._SYMBptr->feuille._VECTptr
                                : 0;
    unsigned count = terms ? terms->size() : 1;
    if (count > 8)
      return false;
    for (unsigned i = 0; i < count; ++i) {
      gen t = terms ? (*terms)[i] : e, c = 1;
      int degree = 0;
      if (t.is_symb_of_sommet(at_neg)) {
        c = -1;
        t = t._SYMBptr->feuille;
      }
      ...
      if (c.type != _INT_ && c.type != _ZINT && c.type != _FRAC)
        return false;
      unsigned budget = 3;
      if (!small_polynomial(c, x, budget))
        return false;
      unsigned j = 0;
      for (; j < out.size(); ++j)
        if (out[j].exponent.val == degree) {
          out[j].coeff += c;
          break;
        }
      if (j == out.size())
        out.push_back(monome(c, degree));
    }
    return true;
  }
  ...
  static bool integrate_sparse_atan(const gen & e, const gen & x, gen & primitive, GIAC_CONTEXT) {
    ...
    if (!f[di].is_symb_of_sommet(at_inv))
      return false;
    sparse_poly1 u, den;
    ...
    if (!large || !small_sparse_polynomial(f[di]._SYMBptr->feuille, x, den, contextptr))
      return false;
    for (unsigned i = 0; i < u.size(); ++i)
    ...
    for (unsigned i = 0; i < den.size(); ++i) {
      if (den[i].exponent.val == 0)
        a = den[i].coeff;
      else if (!is_zero(den[i].coeff))
        return false;
    }
  ...
  static bool integrate_trig_periods(const gen & e, const gen & x, const gen & lo, const gen & hi,
    ...
    if (absolute)
      ...
      if (v.size() != 2 || v[1].type != _INT_ || v[1].val < 32 || v[1].val > 8192 || v[1].val % 2)
        return false;
      n = v[1];
      f = v[0];
    }
    if (f.is_symb_of_sommet(at_neg))
      f = f._SYMBptr->feuille;
    if (!f.is_symb_of_sommet(at_sin) && !f.is_symb_of_sommet(at_cos))
      return false;
    gen a, b;
    if (!is_linear_wrt(f._SYMBptr->feuille, x, a, b, contextptr) || is_zero(a))
      return false;
    gen realvals = evalf_double(makevecteur(a, b, lo, hi), 1, contextptr);
    if (realvals.type != _VECT)
      return false;
    for (unsigned i = 0; i < realvals._VECTptr->size(); ++i) {
      const gen & v = (*realvals._VECTptr)[i];
      if (v.type != _DOUBLE_ || is_inf(v) || is_undef(v))
        return false;
    }
    gen periods = ratnormal(a * (hi - lo) / cst_pi, contextptr);
    if (periods.type != _INT_ && periods.type != _ZINT)
      return false;
    res = c * (hi - lo) *
          (absolute ? rdiv(2, cst_pi, contextptr)
                    : rdiv(comb(n, n / 2, contextptr), pow(2, n, contextptr), contextptr));
    return true;
  }
  ...
  static bool integrate_large_power(const gen & e, const gen & x, gen & primitive, GIAC_CONTEXT) {
    ...
    if (!product.is_symb_of_sommet(at_prod) || product._SYMBptr->feuille.type != _VECT)
      return false;
    unsigned budget = 64;
    if (!small_polynomial(product, x, budget))
      return false;
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      ...
      if (p[1].val < 32 || p[0].type != _SYMB)
        continue;
      gen d = derive(p[0], x, contextptr), rest = 1;
      if (is_zero(d) || is_undef(d))
        continue;
      for (unsigned j = 0; j < v.size(); ++j)
        if (j != i)
          rest = rest * v[j];
      gen dc = 1, rc = 1;
      if (d.is_symb_of_sommet(at_neg)) {
        dc = -1;
        d = d._SYMBptr->feuille;
      }
      if (rest.is_symb_of_sommet(at_neg)) {
        rc = -1;
        rest = rest._SYMBptr->feuille;
      }
      dc = dc * extract_cst(d, x, contextptr);
      rc = rc * extract_cst(rest, x, contextptr);
      if (d != rest || is_zero(dc))
        continue;
      gen n = p[1] + 1;
  ...
  static bool integration_rational(const gen & g) {
    ...
    return g.type == _FRAC && (g._FRACptr->num.type == _INT_ || g._FRACptr->num.type == _ZINT) &&
           (g._FRACptr->den.type == _INT_ || g._FRACptr->den.type == _ZINT) &&
           !is_zero(g._FRACptr->den);
  }
  ...
  static gen integration_syntax(const gen & g, GIAC_CONTEXT) {
    ...
    if (f.type != _VECT) {
      ...
      if (op == at_inv) {
        if (a.is_symb_of_sommet(at_prod) && a._SYMBptr->feuille.type == _VECT) {
          gen r = 1;
          const vecteur & p = *a._SYMBptr->feuille._VECTptr;
          for (unsigned i = 0; i < p.size(); ++i)
            r = r * inv(p[i], contextptr);
          return r;
        }
    ...
    for (unsigned i = 0; i < v.size(); ++i)
      v[i] = integration_syntax(v[i], contextptr);
  ...
  static bool integration_power(const gen & g, gen & base, int n) {
    if (n == -1 && g.is_symb_of_sommet(at_inv)) {
      base = g._SYMBptr->feuille;
      return true;
    }
    if (g.is_symb_of_sommet(at_inv))
      return integration_power(g._SYMBptr->feuille, base, -n);
  ...
  static bool integration_monomial(gen g, const gen & x, gen & c, gen & n, GIAC_CONTEXT) {
    ...
    if (g.is_symb_of_sommet(at_inv)) {
      gen d;
      if (!integration_monomial(g._SYMBptr->feuille, x, d, n, contextptr) || is_zero(d))
        return false;
      c = c / d;
      n = -n;
      return true;
    }
    if (g.is_symb_of_sommet(at_sqrt) && g._SYMBptr->feuille == x) {
      n = gen(1) / 2;
      return true;
    }
    if (!g.is_symb_of_sommet(at_pow) || g._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *g._SYMBptr->feuille._VECTptr;
    if (v.size() != 2 || !integration_rational(v[1]))
      return false;
    if (v[0] != x) {
      gen d, m;
      if (v[1].type != _INT_ || v[1].val < -16 || v[1].val > 16 ||
          !integration_monomial(v[0], x, d, m, contextptr) || is_zero(d))
        return false;
      c = c * pow(d, v[1].val);
      n = m * v[1];
      return true;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_binomial_chain(const gen & e, const gen & x, gen & res,
                                       bool polynomial_only, GIAC_CONTEXT) {
    if (!e.is_symb_of_sommet(at_prod) || e._SYMBptr->feuille.type != _VECT)
      return false;
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      vecteur p;
      if (v[i].is_symb_of_sommet(at_sqrt))
        p = makevecteur(v[i]._SYMBptr->feuille, gen(1) / 2);
      else if (v[i].is_symb_of_sommet(at_pow) && v[i]._SYMBptr->feuille.type == _VECT)
        p = *v[i]._SYMBptr->feuille._VECTptr;
      else
        continue;
      // Evaluation may express u^(2/3) as (u^(1/3))^2. Only unwrap an
      // integer outer power, which preserves principal-power branches.
      if (p.size() == 2 && p[1].type == _INT_ && p[0].is_symb_of_sommet(at_pow) &&
          p[0]._SYMBptr->feuille.type == _VECT) {
        const vecteur inner = *p[0]._SYMBptr->feuille._VECTptr;
        if (inner.size() == 2 && integration_rational(inner[1])) {
          p[0] = inner[0];
          p[1] = p[1] * inner[1];
        }
      }
      ...
      if (!integration_monomial(u[0], x, c0, k0, contextptr) ||
          !integration_monomial(u[1], x, c1, k1, contextptr))
        continue;
      if (is_zero(k0)) {
        a = c0;
        b = c1;
        ...
        continue;
      if (is_zero(b) || !is_strictly_positive(k, contextptr))
        continue;
      gen rest = 1, c, m;
      for (unsigned j = 0; j < v.size(); ++j)
        if (i != j)
          rest = rest * v[j];
      if (!integration_monomial(rest, x, c, m, contextptr))
        continue;
      gen q = (m + 1) / k;
      if (q.type != _INT_ || q.val < 1 || q.val > 8)
        continue;
      if (polynomial_only &&
          (k.type != _INT_ || m.type != _INT_ || m.val < 0 || p[1].type != _INT_))
        continue;
      res = 0;
  ...
  static bool integration_quadratic(const gen & e, const gen & x, gen & a, gen & b, GIAC_CONTEXT) {
    sparse_poly1 p;
    if (!small_sparse_polynomial(e, x, p, contextptr))
      return false;
    a = 0;
    b = 0;
    for (unsigned i = 0; i < p.size(); ++i) {
  ...
  static bool integrate_reciprocal_quartic(const gen & e, const gen & x, gen & res, GIAC_CONTEXT) {
    ...
    if (v.size() != 3)
      return false;
    gen A, B, radical, numerator = 1;
    bool have_q = false, have_r = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base;
      if (integration_power(v[i], base, -1)) {
        if (!have_q && integration_quadratic(base, x, A, B, contextptr)) {
          have_q = true;
          continue;
        }
        if (base.is_symb_of_sommet(at_sqrt)) {
          radical = base._SYMBptr->feuille;
          have_r = true;
          continue;
        }
        if (base.is_symb_of_sommet(at_pow) && base._SYMBptr->feuille.type == _VECT &&
            base._SYMBptr->feuille._VECTptr->size() == 2 &&
            base._SYMBptr->feuille[1] == gen(1) / 2) {
          radical = base._SYMBptr->feuille[0];
          have_r = true;
          continue;
        }
      }
      if (v[i].is_symb_of_sommet(at_pow) && v[i]._SYMBptr->feuille.type == _VECT &&
          v[i]._SYMBptr->feuille._VECTptr->size() == 2 &&
          v[i]._SYMBptr->feuille[1] == -gen(1) / 2) {
        radical = v[i]._SYMBptr->feuille[0];
        have_r = true;
        continue;
      }
    ...
    gen N, M;
    if (!have_q || !have_r || !integration_quadratic(numerator, x, N, M, contextptr))
      return false;
    gen d = B / A;
    if (is_zero(d) || M != -N * d)
      return false;
    sparse_poly1 p;
    if (!small_sparse_polynomial(radical, x, p, contextptr))
      return false;
    gen r4 = 0, r2 = 0, r0 = 0;
    for (unsigned i = 0; i < p.size(); ++i) {
      if (p[i].exponent == 4)
        r4 += p[i].coeff;
      else if (p[i].exponent == 2)
        r2 += p[i].coeff;
      else if (p[i].exponent == 0)
        r0 += p[i].coeff;
      else if (!is_zero(p[i].coeff))
    ...
    if (!is_strictly_positive(r4, contextptr) || r0 != r4 * d * d)
      return false;
    if (is_strictly_positive(-d, contextptr)) {
      gen K = r2 / r4 - 2 * d;
      ...
      res = -N * atanh(scale * x / sqrt(radical / r4, contextptr), contextptr) /
            (A * sqrt(r4, contextptr) * scale);
      return true;
    }
    if (!is_strictly_positive(d, contextptr) || !is_strictly_positive(r2 / r4 + 2 * d, contextptr))
      return false;
    gen m = 2 * d - r2 / r4, u = x / (pow(x, 2) + d), primitive;
    if (is_zero(m))
      primitive = -u;
    else if (is_strictly_positive(m, contextptr)) {
      gen s = sqrt(m, contextptr);
      primitive = -asin(s * u, contextptr) / s;
    } else {
      gen s = sqrt(-m, contextptr);
      primitive = -asinh(s * u, contextptr) / s;
    }
    res = N * primitive / (A * sqrt(r4, contextptr));
    return true;
  }
  ...
  static bool integrate_quartic_trig(const gen & e, const gen & x, gen & res, GIAC_CONTEXT) {
    ...
    if (!angle_radian(contextptr) || !integration_power(e, den, -1) ||
        !den.is_symb_of_sommet(at_plus) || den._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *den._SYMBptr->feuille._VECTptr;
    gen s, t, a, b;
    if (v.size() != 2 || !integration_power(v[0], s, 4) || !integration_power(v[1], t, 4))
      return false;
    if (s.is_symb_of_sommet(at_cos))
      swapgen(s, t);
    if (!s.is_symb_of_sommet(at_sin) || !t.is_symb_of_sommet(at_cos) ||
        s._SYMBptr->feuille != t._SYMBptr->feuille)
      return false;
    gen u = s._SYMBptr->feuille;
    if (!is_linear_wrt(u, x, a, b, contextptr) || !integration_rational(a) ||
        !integration_rational(b) || is_zero(a))
      return false;
    ...
    res = (root * u -
           atan(sin(4 * u, contextptr) / (3 + 2 * root + cos(4 * u, contextptr)), contextptr) /
               root) /
          a;
    return true;
  }
  ...
  static bool integrate_high_frequency_trig(const gen & e, const gen & x, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || taille(e, 65) > 64 || !e.is_symb_of_sommet(at_prod) ||
    ...
    for (unsigned j = 0; j < 2; ++j) {
      ...
      if (base.is_symb_of_sommet(at_pow)) {
        ...
        if (p[1].type != _INT_ || p[1].val < 1 || p[1].val > 32765)
          continue;
        n = p[1].val;
        base = gen(p[0]);
      }
      ...
      if ((!sine && !base.is_symb_of_sommet(at_cos)) ||
          !carrier.is_symb_of_sommet(sine ? at_sin : at_cos))
        continue;
      ...
      if (!is_linear_wrt(u, x, a, b, contextptr) || !integration_rational(a) || is_zero(a) ||
          ...
          !integration_rational(A) || is_inf(b) || is_inf(B) || is_undef(b) || is_undef(B))
        continue;
      int sign = 1;
      if (!is_zero(A - (n + 2) * a)) {
        if (!is_zero(A + (n + 2) * a))
      ...
      if (!is_zero(ratnormal(B - sign * (n + 2) * b, contextptr)))
        continue;
  ...
  static bool integrate_composed_binomial(const gen & e, const gen & x, gen & res, GIAC_CONTEXT) {
    if (taille(e, 129) > 128 || !e.is_symb_of_sommet(at_prod) || e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() > 4)
      return false;
    for (unsigned i = 0; i < v.size(); ++i) {
      ...
      if (p.size() != 2 || p[1].type != _INT_ || p[1].val < 1 || p[1].val > 8192 ||
          !p[0].is_symb_of_sommet(at_plus) || p[0]._SYMBptr->feuille.type != _VECT)
        continue;
      const vecteur & sum = *p[0]._SYMBptr->feuille._VECTptr;
      if (sum.size() != 2)
        continue;
      unsigned ci = integration_resource_rational(sum[0]) ? 0 : 1;
      gen a = sum[ci], term = sum[1 - ci];
      if (!integration_resource_rational(a))
        continue;
      gen b = integration_syntax(integration_coefficient(term, x, contextptr), contextptr);
      if (!integration_resource_rational(b) || is_zero(b) || !term.is_symb_of_sommet(at_pow) ||
          term._SYMBptr->feuille.type != _VECT)
        continue;
      const vecteur & kp = *term._SYMBptr->feuille._VECTptr;
      if (kp.size() != 2 || kp[1].type != _INT_ || kp[1].val < 1 || kp[1].val > 64 || kp[0] == x ||
          taille(kp[0], 33) > 32)
        continue;
      gen u = kp[0];
      int k = kp[1].val, m = 0;
      sparse_poly1 up;
      if (!small_sparse_polynomial(u, x, up, contextptr) || up.empty() || up.size() > 4)
        continue;
      bool ok = true;
      unsigned derivative_terms = 0;
      for (unsigned j = 0; j < up.size(); ++j) {
        if (up[j].exponent.val > 8 || !integration_resource_rational(up[j].coeff))
          ok = false;
        if (up[j].exponent.val && !is_zero(up[j].coeff))
          ++derivative_terms;
      }
      if (!ok || !derivative_terms)
        continue;
      gen rest = 1;
      for (unsigned j = 0; j < v.size(); ++j)
        if (i != j) {
          ...
          if (v[j].is_symb_of_sommet(at_pow) && v[j]._SYMBptr->feuille.type == _VECT) {
            ...
            if (mp.size() == 2 && mp[0] == u) {
              if (mp[1].type != _INT_ || mp[1].val < 0 || mp[1].val > 32767) {
                ok = false;
                break;
              }
              m += mp[1].val;
      ...
      if (!ok || m > 32767 || (m + 1) % k || (m + 1) / k > 8)
        continue;
      sparse_poly1 rp;
      if (!small_sparse_polynomial(rest, x, rp, contextptr))
        continue;
      gen scale = undef;
      unsigned matches = 0;
      for (unsigned j = 0; j < rp.size(); ++j) {
        if (is_zero(rp[j].coeff))
          continue;
        if (!integration_resource_rational(rp[j].coeff)) {
          ok = false;
          break;
        }
        unsigned l = 0;
        for (; l < up.size(); ++l)
          if (up[l].exponent.val == rp[j].exponent.val + 1 && !is_zero(up[l].coeff))
            break;
        if (l == up.size()) {
          ok = false;
          break;
      ...
      if (!ok || matches != derivative_terms || is_undef(scale) ||
          !integration_resource_rational(scale))
        continue;
      int q = (m + 1) / k;
      gen answer = 0;
      for (int j = 0; j < q; ++j) {
        gen exponent = p[1] + j + 1;
        answer += comb(q - 1, j, contextptr) * pow(-a, q - 1 - j) *
                  symbolic(at_pow, makesequence(p[0], exponent)) / exponent;
      }
      answer = scale * answer / (gen(k) * pow(b, q));
      if (taille(answer, 513) > 512)
        continue;
      res = answer;
      return true;
    }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integration_dilog_form(const gen & e, const gen & x, gen & u, gen & scale,
                                     bool & extra, GIAC_CONTEXT) {
    if (taille(e, 97) > 96)
      return false;
    gen f = e, coefficient = integration_coefficient(f, x, contextptr), weight = 1, argument;
    if (!integration_resource_rational(coefficient))
      return false;
    vecteur v = f.is_symb_of_sommet(at_prod) && f._SYMBptr->feuille.type == _VECT
                    ? *f._SYMBptr->feuille._VECTptr
                    : vecteur(1, f);
    if (v.size() > 4)
      return false;
    bool found = false;
    ...
    if (!found)
      return false;
    gen other, a, b;
    if (integration_one_plus(argument, other))
      u = -other;
    else {
      if (!is_linear_wrt(argument, x, a, b, contextptr) || !integration_resource_rational(a) ||
          !integration_resource_rational(b) || is_zero(a))
        return false;
      u = 1 - argument;
    }
    // The extra logarithmic pole has a separate log-square primitive.
    vecteur weights = weight.is_symb_of_sommet(at_prod) && weight._SYMBptr->feuille.type == _VECT
                          ? *weight._SYMBptr->feuille._VECTptr
                          : vecteur(1, weight);
    weight = 1;
    for (unsigned i = 0; i < weights.size(); ++i) {
      gen base;
      if (!extra && integration_power(weights[i], base, -1) && base == argument)
        extra = true;
      else
        weight = weight * weights[i];
    }
    gen c, q, w, n;
    if (integration_monomial(u, x, c, q, contextptr) && integration_resource_rational(c) &&
        !is_zero(c) && integration_resource_rational(q) && !is_zero(q) &&
        !is_strictly_greater(abs(q, contextptr), 32, contextptr)) {
      if (q.type == _FRAC && (q._FRACptr->den.type != _INT_ || q._FRACptr->den.val > 16))
        return false;
      if (!integration_monomial(weight, x, w, n, contextptr) || n != -1 ||
          !integration_resource_rational(w))
        return false;
      scale = coefficient * w / q;
      return true;
    }
    gen base = u;
    gen uc = integration_coefficient(base, x, contextptr);
    if (base.is_symb_of_sommet(at_exp) && integration_resource_rational(uc) && !is_zero(uc) &&
        ...
        (integration_resource_rational(a) ||
         (taille(a, 17) <= 16 && integration_resource_rational(re(a, contextptr)) &&
          integration_resource_rational(im(a, contextptr)))) &&
        !is_zero(a) && integration_resource_rational(b) &&
        integration_monomial(weight, x, w, n, contextptr) && is_zero(n) &&
        integration_resource_rational(w)) {
      scale = coefficient * w / a;
      return true;
    }
    if (!is_linear_wrt(u, x, a, b, contextptr) || !integration_resource_rational(a) || is_zero(a) ||
        !integration_resource_rational(b))
      return false;
    w = integration_coefficient(weight, x, contextptr);
    if (!integration_resource_rational(w) || !integration_power(weight, base, -1))
      return false;
    gen A, B;
    if (!is_linear_wrt(base, x, A, B, contextptr) || !integration_resource_rational(A) ||
        is_zero(A) || !integration_resource_rational(B) || B * a != b * A)
      return false;
    scale = coefficient * w / A;
    return true;
  }
  ...
  static bool integrate_dilog_primitive(const gen & e, const gen & x, gen & res, GIAC_CONTEXT) {
    gen u, scale;
    bool extra;
    if (!integration_dilog_form(e, x, u, scale, extra, contextptr))
      return false;
    res = -scale * gen(symbolic(at_Li2, u));
    if (extra) {
      gen L = ln(1 - u, contextptr);
      res -= scale * L * L / 2;
    }
    return true;
  }
  ...
  static bool integrate_log_trig_primitive(const gen & e, const gen & x, gen & res, GIAC_CONTEXT) {
    ...
    if (e.is_symb_of_sommet(at_plus) && e._SYMBptr->feuille.type == _VECT) {
      ...
      for (unsigned j = 0; j < terms.size(); ++j) {
        ...
        if (!integration_resource_rational(c) ||
            !integrate_log_trig_primitive(t, x, primitive, contextptr))
          return false;
        total += c * primitive;
      }
      res = total;
    ...
    if (e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT) {
      ...
      bool found = false;
      for (unsigned j = 0; j < v.size(); ++j) {
        if (v[j].is_symb_of_sommet(at_ln)) {
          if (found)
            return false;
          logarithm = v[j];
          found = true;
    ...
    if (!logarithm.is_symb_of_sommet(at_ln))
      return false;
    gen f = logarithm._SYMBptr->feuille, c = integration_coefficient(f, x, contextptr), u, a, b;
    bool absolute = f.is_symb_of_sommet(at_abs);
    if (absolute) {
      if (!is_strictly_positive(c, contextptr))
        return false;
      f = gen(f._SYMBptr->feuille);
      c = c * integration_coefficient(f, x, contextptr);
      c = abs(c, contextptr);
    }
    if (!integration_resource_rational(c) || is_zero(c) ||
        (!f.is_symb_of_sommet(at_sin) && !f.is_symb_of_sommet(at_cos)))
      return false;
    u = f._SYMBptr->feuille;
    bool linear =
        is_linear_wrt(u, x, a, b, contextptr) && integration_resource_rational(a) && !is_zero(a);
    gen scale;
    if (!is_one(weight)) {
      // Match coefficients of weight = scale*u', without dividing by a
      // variable derivative and losing regular stationary points.
      sparse_poly1 up, wp;
      gen phase = integration_syntax(u, contextptr),
          phase_scale =
              integration_syntax(integration_coefficient(phase, x, contextptr), contextptr);
      if (!integration_resource_rational(phase_scale) || is_zero(phase_scale) ||
          ...
          !small_sparse_polynomial(weight, x, wp, contextptr) || wp.size() > 4)
        return false;
      scale = undef;
      ...
      for (unsigned j = 0; j < wp.size(); ++j) {
        if (is_zero(wp[j].coeff))
          continue;
        if (!integration_resource_rational(wp[j].coeff))
          return false;
        unsigned k = 0;
        for (; k < up.size(); ++k)
          if (up[k].exponent.val == wp[j].exponent.val + 1 && !is_zero(up[k].coeff))
            break;
        if (k == up.size())
          return false;
      ...
      if (!count || count != matches || !integration_resource_rational(scale))
        return false;
    } else {
      if (!linear)
        return false;
    ...
    gen pa, pb;
    if (is_one(weight) &&
        (!is_linear_wrt(b, cst_pi, pa, pb, contextptr) || !integration_resource_rational(pa) ||
         !integration_resource_rational(pb)))
      return false;
    if (f.is_symb_of_sommet(at_cos))
      u += cst_pi / 2;
    if (is_strictly_positive(-c, contextptr)) {
      c = -c;
      u += cst_pi;
    ...
    res = (is_one(weight) ? x : scale * u) * ln(c / 2, contextptr) -
          scale * gen(symbolic(at_im, dilog)) / 2;
    if (!absolute) {
  ...
  static bool integrate_global_trig_power(const gen & e, const gen & x, gen & res, GIAC_CONTEXT) {
    ...
    if (den.is_symb_of_sommet(at_inv)) {
      ...
      if (den.is_symb_of_sommet(at_pow) && den._SYMBptr->feuille.type == _VECT &&
          den._SYMBptr->feuille._VECTptr->size() == 2) {
        const gen & f = den._SYMBptr->feuille;
        if (f[1].type != _INT_)
          return false;
        n = f[1].val;
        den = gen(f[0]);
      }
    } else if (den.is_symb_of_sommet(at_pow) && den._SYMBptr->feuille.type == _VECT &&
               den._SYMBptr->feuille._VECTptr->size() == 2) {
      const gen & f = den._SYMBptr->feuille;
      if (f[1].type != _INT_ || f[1].val >= 0)
        return false;
      n = -f[1].val;
      den = gen(f[0]);
    } else
    ...
    if (n < 1 || n > 8)
      return false;
    vecteur waves = mergevecteur(lop(den, at_sin), lop(den, at_cos));
    if (waves.size() != 1)
      return false;
    gen wave = waves[0], phase = wave._SYMBptr->feuille, a, b, slope, shift, pa, pb;
    if (!is_linear_wrt(den, wave, b, a, contextptr) || !integration_resource_rational(a) ||
        !integration_resource_rational(b) || is_zero(b) || !is_strictly_positive(a, contextptr) ||
        !is_strictly_positive(a * a - b * b, contextptr) ||
        !is_linear_wrt(phase, x, slope, shift, contextptr) ||
        !integration_resource_rational(slope) || is_zero(slope) ||
        !is_linear_wrt(shift, cst_pi, pa, pb, contextptr) || !integration_resource_rational(pa) ||
        !integration_resource_rational(pb))
      return false;
  ...
  static bool integrate_quadratic_affine_root(const gen &, const gen &, const gen &, const gen &,
                                              gen &, GIAC_CONTEXT);
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_affine_trig_square(const gen & g, const gen & x, gen & result,
                                           GIAC_CONTEXT) {
    if (complex_mode(contextptr) || complex_variables(contextptr) || !angle_radian(contextptr) ||
        taille(g, 65) > 64)
      return false;
    vecteur factors = g.is_symb_of_sommet(at_prod) && g._SYMBptr->feuille.type == _VECT
                          ? *g._SYMBptr->feuille._VECTptr
                          : makevecteur(g);
    if (factors.size() > 4)
      return false;
    gen wave, P = 1;
    ...
    for (unsigned j = 0; j < factors.size(); ++j) {
      gen base = factors[j];
      int exponent = -2;
      if (base.is_symb_of_sommet(at_inv)) {
        base = gen(base._SYMBptr->feuille);
        exponent = 2;
      }
      bool square = base.is_symb_of_sommet(at_pow) && base._SYMBptr->feuille.type == _VECT &&
                    base._SYMBptr->feuille._VECTptr->size() == 2 &&
                    base._SYMBptr->feuille[1] == exponent;
      if (square) {
        base = gen(base._SYMBptr->feuille[0]);
        if (!found && (base.is_symb_of_sommet(at_sin) || base.is_symb_of_sommet(at_cos))) {
          wave = base;
          found = true;
          continue;
        }
      }
      if (!equation_polynomial_bound(factors[j], budget, 0, bound) || bound.degree > 1 ||
          bound.terms > 4)
        return false;
      P = P * factors[j];
    }
    ...
    if (!found || !is_linear_wrt(P, x, A, B, contextptr) || !equation_rational(A) ||
        !is_linear_wrt(B, cst_pi, ba, bb, contextptr) || !equation_rational(ba) ||
        !equation_rational(bb) || !is_linear_wrt(wave._SYMBptr->feuille, x, k, shift, contextptr) ||
        !equation_rational(k) || is_zero(k) || !is_linear_wrt(shift, cst_pi, pa, pb, contextptr) ||
        !equation_rational(pa) || !equation_rational(pb))
      return false;
    ...
    result = gen(sine ? -1 : 1) * P * cross / (k * wave) +
             A * symbolic(at_ln, symbolic(at_abs, wave)) / (k * k);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_affine_trig_square_interval(const gen & g, const gen & x, const gen & lo,
                                                    const gen & hi, gen & result, GIAC_CONTEXT) {
    if (is_inf(lo) || is_inf(hi) || taille(lo, 17) > 16 || taille(hi, 17) > 16)
      return false;
    gen primitive;
    if (!integrate_affine_trig_square(g, x, primitive, contextptr))
      return false;
    vecteur waves = mergevecteur(lop(g, at_sin), lop(g, at_cos));
    if (waves.size() != 1)
      return false;
    gen phase = waves[0]._SYMBptr->feuille,
        offset = waves[0].is_symb_of_sommet(at_cos) ? plus_one_half : gen(0), periods[2];
    for (unsigned j = 0; j < 2; ++j) {
      gen q =
          ratnormal(subst(phase, x, j ? hi : lo, false, contextptr) / cst_pi + offset, contextptr);
      if (!equation_rational(q))
        return false;
      gen numerator = q.type == _FRAC ? q._FRACptr->num : q,
          denominator = q.type == _FRAC ? q._FRACptr->den : gen(1);
      if (numerator.type != _INT_ || numerator.val < -512 || numerator.val > 512 ||
          denominator.type != _INT_ || denominator.val > 24)
        return false;
      periods[j] = _floor(q, contextptr);
      if (q == periods[j])
        return false; // endpoint pole, including an affine zero of the numerator
    }
    if (periods[0] != periods[1])
      return false; // at least one interior pole
    ...
    if (is_undef(upper) || is_undef(lower) || is_inf(upper) || is_inf(lower))
      return false;
    result = ratnormal(upper - lower, contextptr);
    return true;
  }
  ...
  static bool integrate_elliptic_quartic(const gen & g, const gen & x, gen & result, GIAC_CONTEXT) {
    if (complex_mode(contextptr) || complex_variables(contextptr) || !angle_radian(contextptr) ||
        !g.is_symb_of_sommet(at_inv))
      return false;
    const gen & root = g._SYMBptr->feuille;
    gen Q;
    if (root.is_symb_of_sommet(at_sqrt))
      Q = root._SYMBptr->feuille;
    else if (root.is_symb_of_sommet(at_pow) && root._SYMBptr->feuille.type == _VECT &&
             root._SYMBptr->feuille._VECTptr->size() == 2 &&
             root._SYMBptr->feuille[1] == plus_one_half)
      Q = root._SYMBptr->feuille[0];
    else
      return false;
    unsigned budget = 96;
    equation_polynomial_budget bound;
    if (!equation_polynomial_bound(Q, budget, 0, bound) || bound.degree != 4 || bound.terms > 16)
      return false;
    vecteur vars = lidnt(Q);
    if (vars.size() != 1 || vars[0] != x)
      return false;
    gen c[5];
    for (unsigned j = 0; j < 5; ++j) {
      c[j] = _coeff(makesequence(Q, x, int(j)), contextptr);
      if (!equation_rational(c[j]))
        return false;
    }
    if (!is_strictly_positive(-c[4], contextptr))
      return false;
    ...
    if (!is_zero(c[1]) || !is_zero(c[3]) || !is_strictly_positive(c[0], contextptr))
      return false;
    ...
    gen alpha = ratnormal((-c[2] + disc) / (2 * c[0]), contextptr),
        beta = ratnormal((-c[2] - disc) / (2 * c[0]), contextptr);
    gen scale = sqrt(alpha, contextptr), phi = symbolic(at_asin, scale * (x - center));
    result = symbolic(at_EllipticF, makesequence(phi, ratnormal(beta / alpha, contextptr))) /
             (sqrt(c[0], contextptr) * scale);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_parameter_quadratic(const gen & g, const gen & x, gen & result,
                                            GIAC_CONTEXT) {
    if (complex_mode(contextptr) || complex_variables(contextptr) || !angle_radian(contextptr) ||
        !g.is_symb_of_sommet(at_inv))
      return false;
    ...
    if (taille(Q, 65) > 64)
      return false;
    vecteur vars = lidnt(Q);
    if (vars.size() < 2 || vars.size() > 3 || !equalposcomp(vars, x))
      return false;
    unsigned budget = 96;
    equation_polynomial_budget bound;
    if (!equation_polynomial_bound(Q, budget, 0, bound) || bound.degree > 4 || bound.terms > 16 ||
        !is_zero(im(Q, contextptr)))
      return false;
    gen A, B, C;
    if (!is_quadratic_wrt(Q, x, A, B, C, contextptr) || contains(C, x))
      return false;
    gen linear;
    if (is_zero(B))
      linear = x / C;
      ...
      if (!equation_rational(B))
        linear = symbolic(at_when, makesequence(symb_equal(B, 0), x / C, linear));
    }
    ...
    if (!is_zero(A)) {
      ...
      if (is_zero(delta))
        ...
        gen hyperbolic =
            symbolic(at_ln, symbolic(at_abs, (U - negative_root) / (U + negative_root))) /
            negative_root;
        if (equation_rational(delta))
          primitive = is_strictly_positive(delta, contextptr) ? elliptic : hyperbolic;
        else
          primitive = symbolic(
              at_when, makesequence(symb_equal(delta, 0), repeated,
                                    symbolic(at_when, makesequence(symbolic(at_superieur_strict,
                                                                            makesequence(delta, 0)),
                                                                   elliptic, hyperbolic))));
      }
      if (!equation_rational(A))
        primitive = symbolic(at_when, makesequence(symb_equal(A, 0), linear, primitive));
    }
    result = symbolic(at_when, makesequence(symb_equal(Q, 0), undef, primitive));
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_compact_primitive(const gen & input, const gen & x, gen & res,
                                          GIAC_CONTEXT) {
    if (taille(input, 129) > 128 || complex_mode(contextptr) || complex_variables(contextptr))
      return false;
    if (integrate_affine_radical(input, x, res, contextptr))
      return true;
    gen e = integration_syntax(input, contextptr), c = integration_coefficient(e, x, contextptr);
    gen p;
    if (integration_resource_rational(c) &&
        integrate_quadratic_affine_root(e, x, undef, undef, p, contextptr)) {
      res = c * p;
      return true;
    }
    if (integration_resource_rational(c) && integrate_affine_trig_square(e, x, p, contextptr)) {
      res = c * p;
      return true;
    }
    if (integration_resource_rational(c) && integrate_elliptic_quartic(e, x, p, contextptr)) {
      res = c * p;
      return true;
    }
    if (integration_resource_rational(c) && integrate_parameter_quadratic(e, x, p, contextptr)) {
      res = c * p;
      return true;
    }
    if (integration_resource_rational(c) && integrate_logarithmic_span(e, x, p, contextptr)) {
      res = c * p;
      return true;
    }
    if (integration_resource_rational(c) && integrate_global_trig_power(e, x, p, contextptr)) {
      res = c * p;
      return true;
    }
    if (!is_undef(c) && !is_inf(c) && integrate_high_frequency_trig(e, x, p, contextptr)) {
      res = c * p;
      return true;
    }
    ...
    if (angle_radian(contextptr) && has_op(e, *at_ln) && taille(e, 97) <= 96 &&
        taille(c, 17) <= 16 && !contains(c, x) &&
      ...
      if (logs.size() == 1 && e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT) {
        ...
        for (unsigned j = 0; j < roots.size(); ++j) {
          gen Q;
          if (roots[j].is_symb_of_sommet(at_sqrt))
            Q = roots[j]._SYMBptr->feuille;
          else {
            const gen & f = roots[j]._SYMBptr->feuille;
            if (f.type != _VECT || f._VECTptr->size() != 2 || f[1] != gen(1) / 2)
              continue;
            Q = f[0];
          }
          sparse_poly1 polynomial;
          if (!small_sparse_polynomial(Q, x, polynomial, contextptr) || polynomial.empty() ||
              polynomial.front().exponent.val > 4)
            continue;
          bool bounded = true;
          for (unsigned h = 0; h < polynomial.size(); ++h)
            if (!integration_resource_rational(polynomial[h].coeff))
              bounded = false;
          vecteur variables = lvar(arg);
          for (unsigned h = 0; h < variables.size(); ++h)
            if (variables[h] != x && variables[h] != roots[j])
              bounded = false;
          if (bounded) {
            ...
            for (unsigned k = 0; k < chain.size(); ++k)
              if (chain[k] == t) {
                gen weight = 1;
                for (unsigned h = 0; h < chain.size(); ++h)
                  if (h != k)
                    weight = weight * chain[h];
          ...
          if (polynomial.front().exponent.val > 2)
            continue;
          gen C, L, a, b;
          if (!is_linear_wrt(arg, roots[j], C, L, contextptr) || taille(C, 17) > 16 ||
              contains(C, x) || !integration_resource_rational(normal(C * C, contextptr)) ||
              !is_strictly_positive(C, contextptr) || !is_linear_wrt(L, x, a, b, contextptr) ||
              !integration_resource_rational(a) || !integration_resource_rational(b))
            continue;
          gen gap = normal(Q - pow(L / C, 2), contextptr);
          if (!integration_resource_rational(gap) || !is_strictly_positive(gap, contextptr))
            continue;
          const vecteur & factors = *e._SYMBptr->feuille._VECTptr;
          for (unsigned k = 0; k < factors.size(); ++k) {
            gen den, A, B;
            if (!integration_power(factors[k], den, -1) ||
                !is_linear_wrt(den, pow(t, 2), B, A, contextptr) ||
                !integration_resource_rational(A) || !integration_resource_rational(B) ||
                !is_strictly_positive(A, contextptr) || !is_strictly_positive(B, contextptr))
              continue;
            gen weight = 1;
            for (unsigned h = 0; h < factors.size(); ++h)
              if (h != k)
                weight = weight * factors[h];
            gen dt = derive(t, x, contextptr);
            if (is_zero(dt) || taille(dt, 97) > 96)
              continue;
            gen scale = normal(c * weight / dt, contextptr);
            if (taille(scale, 17) > 16 || contains(scale, x) ||
                !integration_resource_rational(normal(scale * scale, contextptr)) ||
                !is_zero(im(scale, contextptr)))
              continue;
            res = scale * atan(t * sqrt(B / A, contextptr), contextptr) / sqrt(A * B, contextptr);
            return true;
          }
        }
      }
    ...
    if (e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT &&
        e._SYMBptr->feuille._VECTptr->size() == 2) {
      const vecteur & v = *e._SYMBptr->feuille._VECTptr;
      for (unsigned j = 0; j < 2; ++j) {
        gen R, power, root = v[j];
        bool reciprocal = root.is_symb_of_sommet(at_inv);
        if (reciprocal)
          root = gen(root._SYMBptr->feuille);
        if (root.is_symb_of_sommet(at_sqrt)) {
          R = root._SYMBptr->feuille;
          power = gen(1) / 2;
        } else if (root.is_symb_of_sommet(at_pow) && root._SYMBptr->feuille.type == _VECT &&
                   root._SYMBptr->feuille._VECTptr->size() == 2) {
          R = root._SYMBptr->feuille[0];
          power = root._SYMBptr->feuille[1];
        } else
          continue;
        if (reciprocal)
          power = -power;
        ...
        if (power == gen(1) / 2 && angle_radian(contextptr) && taille(R, 17) <= 16 &&
            lop(R, at_pow).empty()) {
          ...
          num = nd[0];
          div = nd[1];
          if (integration_power(v[1 - j], den, -1) && is_linear_wrt(num, x, a, b, contextptr) &&
              is_linear_wrt(div, x, d, f, contextptr) && is_linear_wrt(den, x, h, k, contextptr) &&
              integration_resource_rational(a) && integration_resource_rational(b) &&
              integration_resource_rational(d) && integration_resource_rational(f) &&
              integration_resource_rational(h) && integration_resource_rational(k)) {
            gen delta = a * f - b * d, E = k * a - h * b, F = h * f - k * d, J = a * F + d * E;
            if (!is_zero(a) && !is_zero(E) && !is_zero(J) && !is_zero(delta)) {
              gen u = v[j];
              res = 2 * c * delta / J *
                    (integrate_real_quadratic(-d / a, u, contextptr) -
                     integrate_real_quadratic(F / E, u, contextptr));
              return true;
            }
          }
        }
        gen den;
        if (!integration_resource_rational(power) || is_zero(power) ||
            is_strictly_greater(abs(power, contextptr), 8, contextptr) ||
            !integration_power(v[1 - j], den, -1))
          continue;
        gen ratio = integration_syntax(R, contextptr),
            factor = integration_syntax(integration_coefficient(ratio, x, contextptr), contextptr);
        if (!integration_resource_rational(factor) || is_zero(factor) ||
            !ratio.is_symb_of_sommet(at_prod) || ratio._SYMBptr->feuille.type != _VECT ||
            ratio._SYMBptr->feuille._VECTptr->size() != 2)
          continue;
        const vecteur & rv = *ratio._SYMBptr->feuille._VECTptr;
        unsigned k = rv[0].is_symb_of_sommet(at_inv) ? 0 : 1;
        gen B;
        if (!integration_power(rv[k], B, -1))
          continue;
        gen A = rv[1 - k];
        sparse_poly1 polys[3];
        if (!small_sparse_polynomial(A, x, polys[0], contextptr) ||
            !small_sparse_polynomial(B, x, polys[1], contextptr) ||
            !small_sparse_polynomial(den, x, polys[2], contextptr))
          continue;
        gen coeff[3][3];
        bool valid = true;
        for (unsigned row = 0; row < 3; ++row)
          for (unsigned col = 0; col < polys[row].size(); ++col) {
            const monome & term = polys[row][col];
            int degree = term.exponent.val;
            if (degree > (row == 2 ? 2 : 1) || !integration_resource_rational(term.coeff)) {
              valid = false;
              break;
            }
        ...
        if (is_zero(delta))
          continue;
        gen product[3] = {coeff[0][0] * coeff[1][0],
                          coeff[0][1] * coeff[1][0] + coeff[0][0] * coeff[1][1],
                          coeff[0][1] * coeff[1][1]};
        gen scale = undef;
        for (unsigned col = 0; col < 3; ++col) {
          if (is_zero(product[col])) {
            if (!is_zero(coeff[2][col]))
              valid = false;
            continue;
          }
        ...
        if (!valid || !integration_resource_rational(scale) || is_zero(scale))
          continue;
        res = c * v[j] / (power * delta * scale);
        return true;
      }
    }
    if (!integrate_log_trig_primitive(e, x, p, contextptr) &&
        ...
        !integrate_reciprocal_quartic(e, x, p, contextptr) &&
        !integrate_quartic_trig(e, x, p, contextptr))
      return false;
    res = c * p;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_sine_dirichlet(const vecteur & v, const gen & x, const gen & lo,
                                       const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || v.size() != 2)
      return false;
    unsigned i = v[0].is_symb_of_sommet(at_inv) ? 0 : 1;
    gen base;
    if (!integration_power(v[i], base, -1) || base != x)
      return false;
    gen f = v[1 - i];
    int n = 1;
    if (f.is_symb_of_sommet(at_pow) && f._SYMBptr->feuille.type == _VECT) {
      const vecteur & p = *f._SYMBptr->feuille._VECTptr;
      if (p.size() != 2 || p[1].type != _INT_ || p[1].val < 1 || p[1].val > 31 || p[1].val % 2 == 0)
        return false;
      n = p[1].val;
      f = p[0];
    }
    ...
    if (!f.is_symb_of_sommet(at_sin) || !is_linear_wrt(f._SYMBptr->feuille, x, a, b, contextptr) ||
        !integration_rational(a) || is_zero(a) || !is_zero(b))
      return false;
    int halves =
        (lo == minus_inf && hi == plus_inf)
            ? 2
            : ((is_zero(lo) && hi == plus_inf) || (lo == minus_inf && is_zero(hi)) ? 1 : 0);
    if (!halves)
      return false;
    int sign = is_strictly_positive(a, contextptr) ? 1 : -1;
    res = gen(sign * halves) * cst_pi * comb(n - 1, (n - 1) / 2, contextptr) / pow(gen(2), n);
    return true;
  }
  ...
  static bool integrate_logistic_moment(const vecteur & v, const gen & x, gen & res, GIAC_CONTEXT) {
    ...
    for (unsigned i = 0; i < 2; ++i) {
      gen den;
      if (!integration_power(v[i], den, -1) || !den.is_symb_of_sommet(at_plus) ||
          den._SYMBptr->feuille.type != _VECT)
        continue;
      const vecteur & d = *den._SYMBptr->feuille._VECTptr;
      if (d.size() != 3)
        continue;
      gen sum = 0, offset_sum = 0, freq = 0, offset = 0;
      unsigned count = 0;
      bool valid = true;
      for (unsigned j = 0; j < 3; ++j) {
        if (d[j].is_symb_of_sommet(at_exp)) {
          gen a, b;
          if (!is_linear_wrt(d[j]._SYMBptr->feuille, x, a, b, contextptr) ||
              !integration_rational(a) || is_zero(a) || !integration_rational(b)) {
            valid = false;
            break;
          }
          sum += a;
      ...
      gen c, n;
      if (!valid || count != 2 || !is_zero(sum) || !is_zero(offset_sum) ||
          !integration_monomial(v[1 - i], x, c, n, contextptr) || n.type != _INT_ || n.val < 0 ||
          n.val > 16)
        continue;
      ...
      for (int j = 0; j <= n.val; j += 2) {
        if (is_zero(mu) && j != n.val)
          continue;
        gen moment =
            j ? (pow(gen(2), j) - 2) * abs(_bernoulli(j, contextptr), contextptr) * pow(cst_pi, j)
              : gen(1);
        res += comb(n.val, j, contextptr) * (j == n.val ? gen(1) : pow(mu, n.val - j)) * moment /
               pow(a, j + 1);
      }
      res = c * res;
      return true;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_weighted_reflection(const vecteur & v, const gen & x, const gen & lo,
                                            const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || (v.size() != 2 && v.size() != 3) || is_inf(lo) || is_inf(hi))
      return false;
    if (v.size() == 2) {
      gen den, weight = v[0];
      if (!integration_power(v[1], den, -1)) {
        weight = v[1];
        if (!integration_power(v[0], den, -1))
          return false;
      }
      if (!den.is_symb_of_sommet(at_plus) || den._SYMBptr->feuille.type != _VECT ||
          den._SYMBptr->feuille._VECTptr->size() != 2)
        return false;
      gen sine = den._SYMBptr->feuille[0], cosine = den._SYMBptr->feuille[1];
      gen sc = integration_coefficient(sine, x, contextptr),
          cc = integration_coefficient(cosine, x, contextptr);
      if (sine.is_symb_of_sommet(at_cos)) {
        swapgen(sine, cosine);
        swapgen(sc, cc);
      }
      if (!sine.is_symb_of_sommet(at_sin) || !cosine.is_symb_of_sommet(at_cos) || sc != cc ||
          !integration_rational(sc) || is_zero(sc) ||
          sine._SYMBptr->feuille != cosine._SYMBptr->feuille)
        return false;
      gen a, b, p, q;
      if (!is_linear_wrt(sine._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
          is_zero(a) || !is_linear_wrt(weight, x, p, q, contextptr) || !integration_rational(p) ||
          !integration_rational(q))
        return false;
      gen l = ratnormal((a * lo + b) / cst_pi, contextptr),
          r = ratnormal((a * hi + b) / cst_pi, contextptr);
      if (!((is_zero(l) && r == gen(1) / 2) || (is_zero(r) && l == gen(1) / 2)))
        return false;
      gen root = sqrt(gen(2), contextptr);
      res = (p * (lo + hi) / 2 + q) * root * ln(1 + root, contextptr) / (sc * abs(a, contextptr));
      return true;
    }
    gen weight = 1, arg, den;
    bool have_sin = false, have_den = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      if (v[i].is_symb_of_sommet(at_sin)) {
        arg = v[i]._SYMBptr->feuille;
        have_sin = true;
      } else if (integration_power(v[i], den, -1))
        have_den = true;
      else
        weight = weight * v[i];
    }
    ...
    if (!have_sin || !have_den || !is_linear_wrt(arg, x, a, b, contextptr) ||
        ...
        !is_linear_wrt(weight, x, p, q, contextptr) || !integration_rational(p) ||
        !integration_rational(q))
      return false;
    gen l = ratnormal(a * lo + b, contextptr), r = ratnormal(a * hi + b, contextptr);
    if (!((is_zero(l) && r == cst_pi) || (is_zero(r) && l == cst_pi)))
      return false;
    if (!den.is_symb_of_sommet(at_plus) || den._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & d = *den._SYMBptr->feuille._VECTptr;
    ...
    gen A = 0, B = 0;
    for (unsigned i = 0; i < 2; ++i) {
      if (integration_rational(d[i])) {
        A += d[i];
        continue;
      }
      gen t = d[i], c = integration_coefficient(t, x, contextptr), u;
      if (!integration_rational(c) || !integration_power(t, u, 2) || !u.is_symb_of_sommet(at_cos) ||
          u._SYMBptr->feuille != arg)
        return false;
      B += c;
    }
    if (!is_strictly_positive(A, contextptr) || !is_strictly_positive(A + B, contextptr) ||
        is_zero(B))
      return false;
    ...
    res = (p * (lo + hi) / 2 + q) * gen(2) * factor / abs(a, contextptr);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_decay_transform(const gen & e, const gen & x, const gen & lo,
                                        const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !e.is_symb_of_sommet(at_prod) ||
        e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() < 2 || v.size() > 3)
      return false;
    gen exponent, arg, rest = 1;
    bool exponential = false, trig = false, cosine = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      if (v[i].is_symb_of_sommet(at_exp) && !exponential) {
        exponent = v[i]._SYMBptr->feuille;
        exponential = true;
      } else if (!trig && (v[i].is_symb_of_sommet(at_sin) || v[i].is_symb_of_sommet(at_cos))) {
        arg = v[i]._SYMBptr->feuille;
        cosine = v[i].is_symb_of_sommet(at_cos);
        trig = true;
      } else
        rest = rest * v[i];
    }
    gen b, c, a, d;
    if (!exponential || !trig || !is_linear_wrt(arg, x, b, c, contextptr) ||
        !is_zero(im(b, contextptr)) || contains(b, x) || taille(b, 33) > 32 ||
        !integration_rational(c))
      return false;
    gen denominator;
    if (is_zero(lo) && hi == plus_inf && !cosine && is_zero(c) &&
        integration_power(rest, denominator, -1) && denominator == x &&
        is_linear_wrt(exponent, x, a, d, contextptr) && is_zero(im(a, contextptr)) &&
        !contains(a, x) && taille(a, 33) <= 32 && integration_rational(d) &&
        is_strictly_positive(-a, contextptr)) {
      res = exp(d, contextptr) * atan(b / (-a), contextptr);
      return true;
    }
    if (lo != minus_inf || hi != plus_inf || !is_one(rest))
      return false;
    sparse_poly1 p;
    if (!small_sparse_polynomial(exponent, x, p, contextptr))
      return false;
    gen q = 0, l = 0, k = 0;
    for (unsigned i = 0; i < p.size(); ++i) {
      if (p[i].exponent == 2)
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_atan_square(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                    gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || !integration_rational(hi) ||
        !is_strictly_positive(hi, contextptr) || !e.is_symb_of_sommet(at_prod) ||
        e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() != 3)
      return false;
    gen a = 0, b = 0, ra = 0, rb = 0, ta = 0, tb = 0, k = 0;
    bool root = false, den = false, trig = false, reciprocal = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base, radical;
      if (v[i].is_symb_of_sommet(at_atan)) {
        if (trig)
          return false;
        base = v[i]._SYMBptr->feuille;
        k = ratnormal(integration_coefficient(base, x, contextptr), contextptr);
        gen inverse;
        if (integration_power(base, inverse, -1)) {
          base = inverse;
          reciprocal = true;
        }
        if (!integration_rational(k) || !integration_square_root(base, radical) ||
            !integration_quadratic(radical, x, ta, tb, contextptr))
          return false;
        trig = true;
        continue;
      }
      ...
      if (integration_square_root(base, radical)) {
        if (root || !integration_quadratic(radical, x, ra, rb, contextptr))
          return false;
        root = true;
      } else {
        if (den || !integration_quadratic(base, x, a, b, contextptr))
          return false;
        den = true;
      }
    }
    if (!trig || !root || !den || ta != ra || tb != rb || !is_strictly_positive(ra, contextptr) ||
        !is_strictly_positive(k, contextptr) || is_zero(a) ||
        !is_strictly_positive(b / a, contextptr))
      return false;
    gen B = b / a;
    if (rb != 2 * ra * B || (reciprocal ? k * k != ra * hi * hi : k * k * ra * hi * hi != 1))
      return false;
    gen t = atan(hi / sqrt(B, contextptr), contextptr);
    gen square = t * t;
    res =
        (reciprocal ? square
                    : cst_pi * atan(hi / sqrt(hi * hi + 2 * B, contextptr), contextptr) - square) /
        (2 * B * a * sqrt(ra, contextptr));
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_log_trig(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                 gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || is_inf(lo) || is_inf(hi) || !is_zero(im(lo, contextptr)) ||
        !is_zero(im(hi, contextptr)))
      return false;
    gen f = e, mean = 1;
    bool weighted = false;
    if (f.is_symb_of_sommet(at_prod) && f._SYMBptr->feuille.type == _VECT) {
      ...
      gen a, b;
      if (!is_linear_wrt(v[1 - j], x, a, b, contextptr) || !integration_rational(a) ||
          !integration_rational(b))
        return false;
      mean = a * (lo + hi) / 2 + b;
      f = gen(v[j]);
      weighted = true;
    ...
    if (!cosine && !t.is_symb_of_sommet(at_sin))
      return false;
    gen a, b;
    if (!is_linear_wrt(t._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
        !integration_rational(b) || is_zero(a))
      return false;
    gen l = ratnormal((a * lo + b) / cst_pi, contextptr),
        r = ratnormal((a * hi + b) / cst_pi, contextptr);
    if (is_strictly_greater(l, r, contextptr))
      swapgen(l, r);
    gen periods = ratnormal(r - l, contextptr);
    if (absolute && (periods.type == _INT_ || periods.type == _ZINT)) {
      gen center = ratnormal(l + r, contextptr);
      if (weighted && center.type != _INT_ && center.type != _ZINT)
        return false;
      res = -mean * (hi - lo) * ln(gen(2), contextptr);
      return true;
    }
    if (cosine) {
      l += gen(1) / 2;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_thermal_moment(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                       gen & res, GIAC_CONTEXT) {
    ...
    for (unsigned i = 0; i < 2; ++i) {
      ...
      if (!d[j].is_symb_of_sommet(at_exp) || (d[1 - j] != 1 && d[1 - j] != -1))
        continue;
      gen a, b, c, n;
      if (!is_linear_wrt(d[j]._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
          !is_zero(b) || !is_strictly_positive(a, contextptr) ||
          !integration_monomial(v[1 - i], x, c, n, contextptr) || n.type != _INT_ || n.val < 0 ||
          n.val > 64)
        continue;
      bool fermi = d[1 - j] == 1;
      if (n.val == 0) {
        res = c * (fermi ? ln(gen(2), contextptr) / a : plus_inf);
        return true;
      }
      res = c * factorial(n.val) * _Zeta(n + 1, contextptr) / pow(a, n.val + 1);
      if (fermi)
        res = res * (1 - inv(pow(gen(2), n.val), contextptr));
      return true;
    }
    return false;
  }
  ...
  static gen integration_beta_psi(gen z, unsigned order, GIAC_CONTEXT) {
    ...
    for (unsigned step = 0; step < 32 && is_strictly_greater(z, 1, contextptr); ++step) {
      z -= 1;
      correction += scale / pow(z, int(order) + 1);
    }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static void integration_beta_partitions(unsigned remaining, unsigned k, const gen & coefficient,
                                          vecteur & factors, const vecteur & cumulant,
                                          vecteur & terms, GIAC_CONTEXT) {
    if (!remaining) {
      vecteur product(factors);
      if (!is_one(coefficient) || product.empty())
        product.push_back(coefficient);
      terms.push_back(product.size() == 1 ? product[0] : symbolic(at_prod, gen(product)));
      return;
    }
    if (!k)
      return;
    integration_beta_partitions(remaining, k - 1, coefficient, factors, cumulant, terms,
                                contextptr);
    if (is_zero(cumulant[k]))
      return;
    ...
    for (unsigned count = 1; count * k <= remaining; ++count) {
      ...
      factors.push_back(count == 1 ? cumulant[k]
                                   : symbolic(at_pow, makesequence(cumulant[k], int(count))));
      integration_beta_partitions(remaining - count * k, k - 1, coefficient / denominator, factors,
                                  cumulant, terms, contextptr);
      factors.pop_back();
    }
  }
  ...
  static unsigned integration_beta_terms(const gen & g, unsigned limit) {
    if (is_zero(g))
      return 0;
    if (g.type == _FRAC)
      return integration_beta_terms(g._FRACptr->num, limit);
    if (g.is_symb_of_sommet(at_neg))
      return integration_beta_terms(g._SYMBptr->feuille, limit);
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      unsigned next = integration_beta_terms(v[i], limit);
      count = product ? count * next : count + next;
      if (count >= limit)
        return limit;
    }
    return count;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static gen integration_cumulant_moment(const vecteur & cumulant, unsigned n, bool & compact,
                                         GIAC_CONTEXT) {
    vecteur moment(n + 1);
    moment[0] = 1;
    compact = false;
    for (unsigned j = 1; j <= n; ++j) {
      ...
      for (unsigned k = 1; k <= j; ++k) {
        expansion +=
            integration_beta_terms(cumulant[k], 65) * integration_beta_terms(moment[j - k], 65);
        if (expansion > 64)
          break;
      }
      if (expansion > 64) {
      ...
      unsigned choose = 1;
      for (unsigned k = 1; k <= j; ++k) {
        if (!is_zero(cumulant[k]) && !is_zero(moment[j - k]))
          sum += int(choose) * cumulant[k] * moment[j - k];
        choose = choose * (j - k) / k;
      }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static gen integration_beta_moment(const gen & p, const gen & q, const gen & u, const gen & v,
                                     const gen & shift, unsigned n, bool & compact, GIAC_CONTEXT) {
    ...
    for (unsigned k = 1; k <= n; ++k) {
      ...
      vp = vp * v;
      sp = sp * (u + v);
      cumulant[k] = ratnormal(up * integration_beta_psi(p, k - 1, contextptr) +
                                  vp * integration_beta_psi(q, k - 1, contextptr) -
                                  sp * integration_beta_psi(p + q, k - 1, contextptr),
                              contextptr);
      if (k == 1)
        cumulant[k] += shift;
    }
  ...
  static bool integration_mellin_monomial(const gen & g, const gen & x, gen & c, gen & n,
                                          unsigned & budget, GIAC_CONTEXT) {
    if (!budget)
      return false;
    --budget;
    if (integration_monomial(g, x, c, n, contextptr))
      return true;
    ...
    if (g.is_symb_of_sommet(at_pow) && g._SYMBptr->feuille.type == _VECT) {
      ...
      if (outer.size() == 2 && outer[1].type == _INT_ && outer[1].val >= -64 &&
          outer[1].val <= 64 && outer[0].is_symb_of_sommet(at_pow) &&
          outer[0]._SYMBptr->feuille.type == _VECT) {
        const vecteur & inner = *outer[0]._SYMBptr->feuille._VECTptr;
        if (inner.size() == 2 && inner[0] == x && integration_rational(inner[1]) &&
            (inner[1].type != _FRAC ||
             (inner[1]._FRACptr->den.type == _INT_ && inner[1]._FRACptr->den.val > 0 &&
              inner[1]._FRACptr->den.val <= 64))) {
          c = 1;
          n = inner[1] * outer[1];
          return true;
        }
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      gen a, b;
      if (!integration_mellin_monomial(v[i], x, a, b, budget, contextptr))
        return false;
      c = c * a;
      n += b;
    }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_mellin_log(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                   gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || hi != plus_inf)
      return false;
    vecteur singleton;
    const vecteur * terms = 0;
    if (e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT)
    ...
    const vecteur & v = *terms;
    if (v.size() > 8)
      return false;
    gen m = 0, c = 1, A = 0, B = 0, q = 0, r = 0;
    unsigned logs = 0, budget = 64;
    bool denominator = false;
    gen logbase = 0, denbase = 0;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base = v[i], power = 1;
      if (!integration_outer_power(base, power))
        return false;
      if (base.is_symb_of_sommet(at_ln)) {
        ...
        if (v[i] != base && (!v[i].is_symb_of_sommet(at_pow) || v[i]._SYMBptr->feuille[0] != base ||
                             v[i]._SYMBptr->feuille[1].type != _INT_))
          return false;
        if (power.type != _INT_ || power.val < 0 || power.val > 16)
          return false;
        if (logs && logbase != base._SYMBptr->feuille)
          return false;
        logbase = base._SYMBptr->feuille;
        logs += power.val;
        if (logs > 16 || (logbase != x && logs > 4))
          return false;
        continue;
      }
      if (base.is_symb_of_sommet(at_plus) && is_strictly_positive(-power, contextptr)) {
        ...
        if (base._SYMBptr->feuille.type != _VECT || base._SYMBptr->feuille._VECTptr->size() != 2)
          return false;
        const vecteur & d = *base._SYMBptr->feuille._VECTptr;
        unsigned j = integration_rational(d[0]) ? 0 : 1;
        if (!integration_rational(d[j]) ||
            !integration_mellin_monomial(d[1 - j], x, B, q, budget, contextptr))
          return false;
        A = d[j];
        r = -power;
        denbase = base;
      ...
      gen f, n;
      if (!integration_mellin_monomial(v[i], x, f, n, budget, contextptr))
        return false;
      c = c * f;
      m += n;
    }
    gen p = m + 1;
    if (!denominator || !is_strictly_positive(A, contextptr) ||
        !is_strictly_positive(B, contextptr) || !is_strictly_positive(q, contextptr) ||
        !is_strictly_positive(p, contextptr) || !is_strictly_positive(q * r - p, contextptr) ||
        is_strictly_greater(r, 16, contextptr))
      return false;
    if (logs && logbase != x && logbase != denbase)
      return false;
    ...
    gen a = p / q, b = r - a;
    res = c * pow(A, -r, contextptr) * pow(A / B, a, contextptr) * Gamma(a, contextptr) *
          Gamma(b, contextptr) / (q * Gamma(r, contextptr));
    bool compact = false;
    if (logs)
      res = res * integration_beta_moment(a, b, base_log ? gen(0) : inv(q, contextptr),
                                          base_log ? gen(-1) : -inv(q, contextptr),
                                          base_log ? ln(A, contextptr) : ln(A / B, contextptr) / q,
                                          logs, compact, contextptr);
    if (!compact)
  ...
  static bool integration_beta_weight(const gen & g, const gen & x, gen & p, gen & q,
                                      const gen & power, unsigned & budget, GIAC_CONTEXT) {
    if (!budget--)
      return false;
    if (is_one(g))
      return true;
    ...
    gen a, b;
    if (g.is_symb_of_sommet(at_plus) && is_linear_wrt(g, x, a, b, contextptr) && a == -1 &&
        b == 1) {
      q += power;
      return true;
    }
    if (g.is_symb_of_sommet(at_inv))
      return integration_beta_weight(g._SYMBptr->feuille, x, p, q, -power, budget, contextptr);
    if (g.is_symb_of_sommet(at_sqrt))
      return integration_beta_weight(g._SYMBptr->feuille, x, p, q, power / 2, budget, contextptr);
    if (g.is_symb_of_sommet(at_pow) && g._SYMBptr->feuille.type == _VECT) {
      const vecteur & v = *g._SYMBptr->feuille._VECTptr;
      return v.size() == 2 && integration_rational(v[1]) &&
             integration_beta_weight(v[0], x, p, q, power * v[1], budget, contextptr);
    }
    if (!g.is_symb_of_sommet(at_prod) || g._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *g._SYMBptr->feuille._VECTptr;
    for (unsigned i = 0; i < v.size(); ++i)
      if (!integration_beta_weight(v[i], x, p, q, power, budget, contextptr))
        return false;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static gen integration_beta_joint(const gen * u, const gen * v, unsigned n, const gen & p,
                                    const gen & q, bool & compact, GIAC_CONTEXT) {
    ...
    for (unsigned i = 0; i < n; ++i)
      for (unsigned j = i + 1; j < n; ++j) {
        gen covariance =
            u[i] * u[j] * pp[1] + v[i] * v[j] * pq[1] - (u[i] + v[i]) * (u[j] + v[j]) * ps[1];
        answer += n == 2 ? covariance : covariance * mean[3 - i - j];
      }
    if (n == 3)
      answer += u[0] * u[1] * u[2] * pp[2] + v[0] * v[1] * v[2] * pq[2] -
                (u[0] + v[0]) * (u[1] + v[1]) * (u[2] + v[2]) * ps[2];
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_beta_log(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                 gen & res, GIAC_CONTEXT) {
    ...
    if (v.size() > 8)
      return false;
    gen p = 1, q = 1, u = 0, w = 0, ud[3], vd[3];
    unsigned order = 0, stored = 0, budget = 64;
    bool directional = false, mixed = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base = v[i], power = 1;
      if (!integration_outer_power(base, power))
        return false;
      if (base.is_symb_of_sommet(at_ln)) {
        if (v[i] != base && (!v[i].is_symb_of_sommet(at_pow) || v[i]._SYMBptr->feuille[0] != base ||
                             v[i]._SYMBptr->feuille[1].type != _INT_))
          return false;
        if (power.type != _INT_ || power.val < 1 || power.val > 16)
          return false;
        gen a = 0, b = 0;
        unsigned argbudget = 32;
        if (!integration_beta_weight(base._SYMBptr->feuille, x, a, b, 1, argbudget, contextptr) ||
            (is_zero(a) && is_zero(b)))
          return false;
        for (int j = 0; j < power.val && stored < 3; ++j) {
          ud[stored] = a;
          vd[stored] = b;
          ++stored;
        }
        ...
        if (order > 16)
          return false;
      } else if (!integration_beta_weight(v[i], x, p, q, 1, budget, contextptr))
        return false;
    }
    if (!order || !is_strictly_positive(p, contextptr) || !is_strictly_positive(q, contextptr) ||
        is_strictly_greater(p, 16, contextptr) || is_strictly_greater(q, 16, contextptr))
      return false;
  ...
  static bool integration_harmonic_real(const gen & g, GIAC_CONTEXT) {
    if (taille(g, 65) > 64 || is_undef(g) || is_inf(g) || !is_zero(im(g, contextptr)))
      return false;
  ...
  static bool integration_harmonic_product(const gen & left, const gen & right, const gen & x,
                                           vecteur & out, GIAC_CONTEXT) {
    if (out.size() >= 32)
      return false;
    gen a = left, b = right,
        scale =
            integration_coefficient(a, x, contextptr) * integration_coefficient(b, x, contextptr);
    if (is_constant_wrt(a, x, contextptr)) {
      out.push_back(scale * a * b);
      return true;
    }
    if (is_constant_wrt(b, x, contextptr)) {
      out.push_back(scale * a * b);
      return true;
    }
    bool sa = a.is_symb_of_sommet(at_sin), sb = b.is_symb_of_sommet(at_sin);
    if ((!sa && !a.is_symb_of_sommet(at_cos)) || (!sb && !b.is_symb_of_sommet(at_cos)) ||
        out.size() > 30)
      return false;
    gen sum = ratnormal(a._SYMBptr->feuille + b._SYMBptr->feuille, contextptr),
        difference = ratnormal(a._SYMBptr->feuille - b._SYMBptr->feuille, contextptr);
  ...
  static bool integration_harmonic_terms(const gen & g, const gen & x, vecteur & out,
                                         unsigned & budget, GIAC_CONTEXT) {
    if (!budget || out.size() >= 32)
      return false;
    --budget;
    if (g.is_symb_of_sommet(at_sin) || g.is_symb_of_sommet(at_cos)) {
      ...
      if (!is_linear_wrt(g._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
          !integration_harmonic_real(b, contextptr))
        return false;
      out.push_back(symbolic(g._SYMBptr->sommet, a * x + b));
      return true;
    }
    ...
    if (g.is_symb_of_sommet(at_neg)) {
      vecteur v;
      if (!integration_harmonic_terms(g._SYMBptr->feuille, x, v, budget, contextptr) ||
          out.size() + v.size() > 32)
        return false;
      for (unsigned i = 0; i < v.size(); ++i)
        out.push_back(-v[i]);
    ...
    if (g.is_symb_of_sommet(at_plus)) {
      for (unsigned i = 0; i < v.size(); ++i)
        if (!integration_harmonic_terms(v[i], x, out, budget, contextptr))
          return false;
      return true;
    }
    bool power = g.is_symb_of_sommet(at_pow);
    if (power && (v.size() != 2 || v[1].type != _INT_ || v[1].val < 0 || v[1].val > 4))
      return false;
    if (!power && !g.is_symb_of_sommet(at_prod))
      return false;
    vecteur result(1, 1), base;
    if (power && !integration_harmonic_terms(v[0], x, base, budget, contextptr))
      return false;
    unsigned count = power ? v[1].val : v.size();
    for (unsigned i = 0; i < count; ++i) {
      if (!power) {
        base.clear();
        if (!integration_harmonic_terms(v[i], x, base, budget, contextptr))
          return false;
      }
      vecteur next;
      for (unsigned j = 0; j < result.size(); ++j)
        for (unsigned k = 0; k < base.size(); ++k)
          if (!integration_harmonic_product(result[j], base[k], x, next, contextptr))
            return false;
      result.swap(next);
    }
    if (out.size() + result.size() > 32)
      return false;
  ...
  static bool integration_gaussian_quadratic(const gen & g, const gen & x, gen & q, gen & l,
                                             gen & constant, GIAC_CONTEXT) {
    const vecteur * terms = g.is_symb_of_sommet(at_plus) && g._SYMBptr->feuille.type == _VECT
                                ? g._SYMBptr->feuille._VECTptr
                                : 0;
    unsigned count = terms ? terms->size() : 1;
    if (count > 8)
    ...
    constant = 0;
    for (unsigned i = 0; i < count; ++i) {
      gen t = terms ? (*terms)[i] : g, scale = integration_coefficient(t, x, contextptr), base, a,
          b;
      if (!integration_rational(scale))
        return false;
      if (integration_power(t, base, 2)) {
        if (!is_linear_wrt(base, x, a, b, contextptr) || !integration_rational(a) ||
            !integration_rational(b))
          return false;
        q += scale * a * a;
        l += 2 * scale * a * b;
        ...
        if (!is_linear_wrt(t, x, a, b, contextptr) || !integration_rational(a) ||
            !integration_rational(b))
          return false;
        l += scale * a;
        constant += scale * b;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_gaussian_erf(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                     gen & res, GIAC_CONTEXT) {
    bool full = lo == minus_inf && hi == plus_inf, half = is_zero(lo) && hi == plus_inf;
    if ((!full && !half) || !angle_radian(contextptr) || !e.is_symb_of_sommet(at_prod) ||
        e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() < 2 || v.size() > 3)
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      ...
      gen term = v[i], base, a, b;
      unsigned multiplicity = 1;
      if (integration_power(term, base, 2)) {
        term = gen(base);
        multiplicity = 2;
      }
      if (!term.is_symb_of_sommet(at_erf) || slopes.size() + multiplicity > 2 ||
          !is_linear_wrt(term._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
          !integration_rational(b))
        return false;
      for (unsigned j = 0; j < multiplicity; ++j) {
        slopes.push_back(a);
        shifts.push_back(b);
      }
    }
    gen q, l, constant;
    if (!gaussian || slopes.empty() ||
        !integration_gaussian_quadratic(exponent, x, q, l, constant, contextptr) ||
        !is_strictly_positive(-q, contextptr))
      return false;
    gen a = -q, mean = -l / (2 * q), factor = exp(constant - l * l / (4 * q), contextptr);
    if (half && !is_zero(mean))
      return false;
    for (unsigned i = 0; i < slopes.size(); ++i)
      shifts[i] += slopes[i] * mean;
    if (slopes.size() == 1) {
      gen b = slopes[0];
      if (full)
        res = factor * sqrt(cst_pi / a, contextptr) *
              erf(shifts[0] * sqrt(a / (a + b * b), contextptr), contextptr);
      else {
        if (!is_zero(shifts[0]))
          return false;
    ...
    if (!is_zero(shifts[0]) || !is_zero(shifts[1]))
      return false;
    gen b = slopes[0], c = slopes[1];
    res = (full ? 2 : 1) * factor *
          asin(b * c / sqrt((a + b * b) * (a + c * c), contextptr), contextptr) /
          sqrt(cst_pi * a, contextptr);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_laplace_difference(const gen & e, const gen & x, const gen & lo,
                                           const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || hi != plus_inf ||
        !e.is_symb_of_sommet(at_prod) || e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() < 3 || v.size() > 6)
      return false;
    gen a = 0, d = 0, numerator = 1;
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      ...
      if (v[i].is_symb_of_sommet(at_exp)) {
        if (exponential || !is_linear_wrt(v[i]._SYMBptr->feuille, x, a, d, contextptr) ||
            !is_zero(im(a, contextptr)) || contains(a, x) || taille(a, 33) > 32 ||
            !integration_rational(d) || !is_strictly_positive(-a, contextptr))
          return false;
        exponential = true;
      } else if (integration_power(v[i], base, -1) && base == x) {
        if (inverse)
          return false;
        inverse = 1;
      } else if (integration_power(v[i], base, -2) && base == x) {
        if (inverse)
          return false;
        inverse = 2;
      } else
    ...
    if (!numerator.is_symb_of_sommet(at_plus) || numerator._SYMBptr->feuille.type != _VECT)
      return false;
    ...
    for (unsigned i = 0; i < terms.size(); ++i) {
      ...
      if (is_constant_wrt(t, x, contextptr)) {
        at_zero += subst(c * t, x, 0, false, contextptr).eval(1, contextptr);
        continue;
      }
      ...
      if (inverse == 2 && t == x) {
        ...
        if (!integration_harmonic_real(c, contextptr))
          return false;
        derivative_zero += c;
        continue;
      }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_inverse_gaussian(const gen & e, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!is_zero(lo) || hi != plus_inf)
      return false;
    const gen * exponential = &e;
    gen weight = 1;
    if (e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT) {
      const vecteur & v = *e._SYMBptr->feuille._VECTptr;
      if (v.size() != 2)
    ...
    if (!exponential->is_symb_of_sommet(at_exp))
      return false;
    gen c, n;
    if (!integration_monomial(weight, x, c, n, contextptr))
      return false;
    const gen & arg = exponential->_SYMBptr->feuille;
    vecteur terms;
    if (arg.is_symb_of_sommet(at_plus) && arg._SYMBptr->feuille.type == _VECT)
      terms = *arg._SYMBptr->feuille._VECTptr;
    else
      terms.push_back(arg);
    if (terms.size() > 3)
      return false;
    gen a = 0, b = 0, d = 0, k = 0;
    bool expanded_square = false;
    for (unsigned i = 0; i < terms.size(); ++i) {
      gen coefficient, p;
      if (!integration_monomial(terms[i], x, coefficient, p, contextptr)) {
        ...
        if (expanded_square || !integration_rational(scale) || !integration_power(core, sum, 2) ||
            ...
            sum._SYMBptr->feuille._VECTptr->size() != 2)
          return false;
        const vecteur & v = *sum._SYMBptr->feuille._VECTptr;
        gen c1, p1, c2, p2;
        if (!integration_monomial(v[0], x, c1, p1, contextptr) ||
            !integration_monomial(v[1], x, c2, p2, contextptr) || is_zero(p1) || p1 + p2 != 0)
          return false;
        coefficient = 2 * scale * c1 * c2;
        p = 0;
      ...
      bool positive = is_strictly_positive(p, contextptr);
      gen degree = positive ? p : -p;
      if (is_zero(k))
        k = degree;
      else if (degree != k)
        return false;
    ...
    if (is_zero(k) || !is_strictly_positive(a, contextptr) || !is_strictly_positive(b, contextptr))
      return false;
    ...
    gen order = ratnormal((n + 1) / k, contextptr);
    bool negative = is_strictly_positive(-order, contextptr);
    if (negative)
      order = -order;
    gen m = ratnormal(order - gen(1) / 2, contextptr);
    if (m.type != _INT_ || m.val < 0 || m.val > 16)
      return false;
    ...
    res = c * sqrt(cst_pi / (negative ? b : a), contextptr) * exp(d - z, contextptr) *
          pow(sqrt(scale, contextptr), m.val) * polynomial / k;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_log_zeta(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                 gen & res, GIAC_CONTEXT) {
    ...
    if (v.size() < 2 || v.size() > 3)
      return false;
    bool inverse = false, kernel = false;
    gen u = 0, logarg = 0, b = 0, q = 0;
    int m = 0, sign = 0;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen t = v[i], base;
      if (integration_power(t, base, -1) && base == x) {
        if (inverse)
          return false;
        inverse = true;
        continue;
      ...
      if (t.is_symb_of_sommet(at_pow) && t._SYMBptr->feuille.type == _VECT) {
        const vecteur & p = *t._SYMBptr->feuille._VECTptr;
        if (p.size() != 2 || p[1].type != _INT_ || p[1].val < 1 || p[1].val > 8)
          return false;
        n = p[1].val;
        t = gen(p[0]);
      }
      ...
      if (integration_one_plus(t._SYMBptr->feuille, other)) {
        if (kernel || n != 1 || !integration_monomial(other, x, b, q, contextptr) || is_zero(b) ||
            !is_strictly_positive(q, contextptr) || is_strictly_greater(q, 32, contextptr))
          return false;
        sign = is_strictly_positive(b, contextptr) ? 1 : -1;
        u = sign * other;
        b = sign * b;
        kernel = true;
      } else {
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_hyperbolic_log(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                       gen & res, GIAC_CONTEXT) {
    if (!e.is_symb_of_sommet(at_prod) || e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    ...
    for (unsigned i = 0; i < 2; ++i) {
      ...
      if (!t.is_symb_of_sommet(at_ln) || !t._SYMBptr->feuille.is_symb_of_sommet(at_cosh))
        continue;
      const gen & ch = t._SYMBptr->feuille;
      gen p = 0, d = v[1 - i];
      if (d.is_symb_of_sommet(at_inv)) {
        p = 1;
        d = gen(d._SYMBptr->feuille);
      }
      if (d.is_symb_of_sommet(at_pow) && d._SYMBptr->feuille.type == _VECT) {
        const vecteur & w = *d._SYMBptr->feuille._VECTptr;
        if (w.size() != 2 || !integration_rational(w[1]))
          continue;
        p = is_one(p) ? w[1] : -w[1];
        d = gen(w[0]);
      }
      if (d != ch || !is_strictly_positive(p, contextptr) || is_strictly_greater(p, 16, contextptr))
        continue;
      gen a, b;
      ...
      res = sqrt(cst_pi, contextptr) * Gamma(A, contextptr) /
            (abs(a, contextptr) * Gamma(B, contextptr));
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_atan_log_moment(const gen & e, const gen & x, const gen & lo,
                                        const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || hi != plus_inf ||
        !e.is_symb_of_sommet(at_prod) || e._SYMBptr->feuille.type != _VECT)
      return false;
    ...
    if (v.size() != 3)
      return false;
    bool inverse = false, denominator = false, angle = false;
    gen a = 0, A = 0, B = 0;
    int n = 0;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen t = v[i], base;
      if (integration_power(t, base, -1)) {
        if (base == x) {
          ...
          if (denominator || !integration_quadratic(base, x, A, B, contextptr))
            return false;
          denominator = true;
        }
        continue;
      ...
      gen b;
      if (angle || !t.is_symb_of_sommet(at_atan) ||
          !is_linear_wrt(t._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
          is_zero(a) || !is_zero(b))
        return false;
      angle = true;
    }
    if (!inverse || !denominator || !angle || !integration_rational(A) ||
        !integration_rational(B) || !is_strictly_positive(B, contextptr) ||
        !is_zero(ratnormal(A - B * a * a, contextptr)))
      return false;
    gen L = cst_pi / 2, sum = 0, c = 1;
    int m = n - 1, s = 3;
    while (m > 0) {
    ...
    if (!is_strictly_positive(a, contextptr) && n % 2)
      res = -res;
    return true;
  }

  // Read a bounded affine expression in one cosine without polynomial expansion.
  static bool integration_beta_affine_weight(const gen & g, const gen & x, const gen & lo,
                                             const gen & hi, gen & p, gen & q, gen & coefficient,
                                             const gen & power, unsigned & budget, GIAC_CONTEXT) {
    if (!budget)
      return false;
    --budget;
    ...
    if (is_strictly_greater(power, 32, contextptr) || is_strictly_greater(-power, 32, contextptr))
      return false;
    if (integration_rational(g)) {
      if (!is_strictly_positive(g, contextptr))
        return false;
      coefficient = coefficient * pow(g, power, contextptr);
      return true;
    }
    if (g.is_symb_of_sommet(at_inv))
      return integration_beta_affine_weight(g._SYMBptr->feuille, x, lo, hi, p, q, coefficient,
                                            -power, budget, contextptr);
    if (g.is_symb_of_sommet(at_sqrt))
      return integration_beta_affine_weight(g._SYMBptr->feuille, x, lo, hi, p, q, coefficient,
                                            power / 2, budget, contextptr);
    if (g.is_symb_of_sommet(at_pow) && g._SYMBptr->feuille.type == _VECT) {
      const vecteur & v = *g._SYMBptr->feuille._VECTptr;
      return v.size() == 2 && integration_rational(v[1]) &&
             integration_beta_affine_weight(v[0], x, lo, hi, p, q, coefficient, power * v[1],
                                            budget, contextptr);
    }
    if (g.is_symb_of_sommet(at_prod) && g._SYMBptr->feuille.type == _VECT) {
      const vecteur & v = *g._SYMBptr->feuille._VECTptr;
      for (unsigned i = 0; i < v.size(); ++i)
        if (!integration_beta_affine_weight(v[i], x, lo, hi, p, q, coefficient, power, budget,
                                            contextptr))
          return false;
      return true;
    }
    ...
    if (g == x)
      ...
      for (unsigned i = 0; i < v.size(); ++i) {
        gen c, n;
        if (!integration_monomial(v[i], x, c, n, contextptr) || n.type != _INT_ || n.val < 0 ||
            n.val > 2)
          return false;
        poly[n.val] += c;
      }
    ...
    gen right = ratnormal(poly[0] + hi * (poly[1] + hi * poly[2]), contextptr), width = hi - lo,
        scale;
    if (is_zero(poly[2])) {
      if (is_zero(left) && is_strictly_positive(right, contextptr)) {
        p += power;
        scale = right;
      } else if (is_zero(right) && is_strictly_positive(left, contextptr)) {
        q += power;
        scale = left;
      } else
        return false;
    } else {
      if (!is_zero(left) || !is_zero(right) || !is_strictly_positive(-poly[2], contextptr))
        return false;
      p += power;
      q += power;
      scale = -poly[2] * width * width;
    }
    coefficient = coefficient * pow(scale, power, contextptr);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_beta_affine_log(const gen & e, const gen & x, const gen & lo,
                                        const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !integration_rational(lo) || !integration_rational(hi) ||
        !is_strictly_positive(hi - lo, contextptr) || (is_zero(lo) && hi == 1))
      return false;
    vecteur singleton;
    const vecteur * terms;
    if (e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT)
    ...
    if (contains(e, t) || eval(t, 1, contextptr) != t)
      return false;
    gen p = 0, q = 0, c = 1, mapped = 1;
    unsigned budget = 64, logs = 0;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen f = v[i], base;
      int order = 1;
      if (f.is_symb_of_sommet(at_pow) && f._SYMBptr->feuille.type == _VECT) {
        const vecteur & power = *f._SYMBptr->feuille._VECTptr;
        if (power.size() == 2 && power[0].is_symb_of_sommet(at_ln) && power[1].type == _INT_) {
          order = power[1].val;
          f = gen(power[0]);
        }
      }
      if (f.is_symb_of_sommet(at_ln)) {
        if (order < 1 || order > 16 || (logs += order) > 16)
          return false;
        gen a = 0, b = 0, scale = 1;
        if (!integration_beta_affine_weight(f._SYMBptr->feuille, x, lo, hi, a, b, scale, 1, budget,
                                            contextptr) ||
            !is_zero(ratnormal(scale - 1, contextptr)))
          return false;
        gen argument = pow(t, a, contextptr) * pow(1 - t, b, contextptr);
        mapped = mapped * pow(symbolic(at_ln, argument), order);
        continue;
      }
      if (!integration_beta_affine_weight(v[i], x, lo, hi, p, q, c, 1, budget, contextptr))
        return false;
    }
    if (!logs)
      return false;
    mapped = mapped * pow(t, p, contextptr) * pow(1 - t, q, contextptr);
    if (!integrate_beta_log(mapped, t, 0, 1, res, contextptr))
      return false;
    res = (hi - lo) * c * res;
    return true;
  }
  ...
  static gen integration_acos_circle_value(const gen & q, GIAC_CONTEXT) {
    ...
    return 2 * cst_pi * h - 2 * h * h - 2 * cst_pi * atan(sqrt(1 + 2 * q, contextptr), contextptr) +
           cst_pi * cst_pi / 2;
  }

  static bool integration_arc_binomial(gen g, const gen & x, gen & constant, gen & coefficient,
                                       gen & degree, unsigned & budget, GIAC_CONTEXT) {
    gen scale = integration_coefficient(g, x, contextptr);
    if (!integration_rational(scale) || !g.is_symb_of_sommet(at_plus) ||
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      gen c, n;
      if (!integration_mellin_monomial(v[i], x, c, n, budget, contextptr))
        return false;
      if (is_zero(n))
        constant += scale * c;
      else {
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_acos_monomial_pullback(const gen & e, const gen & x, const gen & lo,
                                               const gen & hi, gen & res, GIAC_CONTEXT) {
    ...
    if (v.size() > 5)
      return false;
    gen angle, denominator, weight = 1;
    unsigned budget = 64;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base;
      if (v[i].is_symb_of_sommet(at_acos)) {
        if (!is_zero(angle))
          return false;
        angle = v[i];
    ...
    gen E, d, k, c, m;
    if (is_zero(angle) || is_zero(denominator) ||
        !integration_arc_binomial(denominator, x, E, d, k, budget, contextptr) ||
        !is_strictly_positive(E, contextptr) || !is_strictly_positive(d, contextptr) ||
        !is_strictly_positive(k, contextptr) ||
        !integration_mellin_monomial(weight, x, c, m, budget, contextptr) || m + 1 != k / 2)
      return false;
    d = d / E;
    c = c / E;
    gen arg = angle._SYMBptr->feuille, outer = integration_coefficient(arg, x, contextptr),
        num = outer, den;
    if (!integration_rational(outer) || !arg.is_symb_of_sommet(at_prod) ||
        arg._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & a = *arg._SYMBptr->feuille._VECTptr;
    if (a.size() > 4)
      return false;
    for (unsigned i = 0; i < a.size(); ++i) {
      gen base;
      if (integration_power(a[i], base, -1) && !integration_rational(base)) {
        if (!is_zero(den))
          return false;
        den = base;
      } else
    ...
    if (is_zero(den) || !integration_arc_binomial(num, x, A, B, kn, budget, contextptr) ||
        !integration_arc_binomial(den, x, C, D, kd, budget, contextptr) || kn != k || kd != k ||
        B != D)
      return false;
    gen unit = (C - A) / 2;
    if (is_zero(unit) || B != -d * unit)
      return false;
    gen q = (A + C) / (4 * unit);
    if (!is_strictly_positive(q, contextptr) ||
        !is_zero(ratnormal(pow(hi, k, contextptr) * d - q, contextptr)))
      return false;
    res = c * integration_acos_circle_value(q, contextptr) / (k * sqrt(d, contextptr));
    return true;
  }

  static bool integration_affine_cosine(const gen & g, const gen & x, gen & arg, gen & constant,
                                        gen & coefficient, GIAC_CONTEXT) {
    gen core = g, scale = ratnormal(integration_coefficient(core, x, contextptr), contextptr);
    if (!integration_rational(scale))
      return false;
    vecteur v;
    if (core.is_symb_of_sommet(at_plus) && core._SYMBptr->feuille.type == _VECT)
      v = *core._SYMBptr->feuille._VECTptr;
    else
      v = makevecteur(core);
    if (v.size() > 4)
      return false;
    ...
    coefficient = 0;
    for (unsigned i = 0; i < v.size(); ++i) {
      if (integration_rational(v[i])) {
        constant += scale * v[i];
        continue;
      }
      gen t = v[i], c = ratnormal(integration_coefficient(t, x, contextptr), contextptr);
      if (!integration_rational(c) || !t.is_symb_of_sommet(at_cos))
        return false;
      if (is_undef(arg))
        arg = t._SYMBptr->feuille;
      else if (arg != t._SYMBptr->feuille)
        return false;
      coefficient += scale * c;
    }
    return true;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_acos_circle(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                    gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !e.is_symb_of_sommet(at_acos))
      return false;
    gen f = e._SYMBptr->feuille,
        c = ratnormal(integration_coefficient(f, x, contextptr), contextptr);
    if (!integration_rational(c))
      return false;
    if (!f.is_symb_of_sommet(at_prod) || f._SYMBptr->feuille.type != _VECT)
      return false;
    ...
    if (!integration_affine_cosine(numerator, x, arg, A, B, contextptr) ||
        !integration_affine_cosine(denominator, x, arg, C, D, contextptr) || is_undef(arg) ||
        B != C)
      return false;
    gen delta = B - A;
    if (is_zero(delta) || D - B != delta)
      return false;
    gen q = B / delta;
    if (!is_strictly_positive(q, contextptr))
      return false;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_atan_frullani(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                      gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || hi != plus_inf ||
        !e.is_symb_of_sommet(at_prod) || e._SYMBptr->feuille.type != _VECT)
      return false;
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      ...
      if (t.is_symb_of_sommet(at_ln) && t._SYMBptr->feuille == x) {
        logs += n;
        if (logs > 2)
          return false;
        continue;
      }
      if (!is_zero(numerator) || !v[i].is_symb_of_sommet(at_plus))
        return false;
      numerator = v[i];
    }
    if (!inverse || !numerator.is_symb_of_sommet(at_plus) ||
        numerator._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & terms = *numerator._SYMBptr->feuille._VECTptr;
    if (terms.size() < 2 || terms.size() > 8)
      return false;
    ...
    for (unsigned j = 0; j < terms.size(); ++j) {
      ...
      a = integration_coefficient(argument, x, contextptr);
      unsigned monomial_budget = 16;
      gen multiplier;
      if (!integration_mellin_monomial(argument, x, multiplier, b, monomial_budget, contextptr) ||
          is_zero(b))
        return false;
      a = a * multiplier;
      if (is_zero(degree))
      ...
      if (!logs) {
        if (!j)
          ...
          result += c * ln(a / reference, contextptr);
      } else {
    ...
    res = cst_pi * result / (2 * abs(degree, contextptr) * pow(degree, int(logs)));
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_loglog_mellin(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                      gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || hi != 1 || !e.is_symb_of_sommet(at_prod) ||
        e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() > 8)
      return false;
    gen q = 0, h = 0, c = 1, m = 0;
    bool denominator = false, logarithm = false;
    unsigned budget = 64;
    for (unsigned i = 0; i < v.size(); ++i) {
      ...
      if (t.is_symb_of_sommet(at_ln)) {
        ...
        if (!inner.is_symb_of_sommet(at_ln) ||
            !integration_mellin_monomial(inner._SYMBptr->feuille, x, d, s, budget, contextptr) ||
            d != 1)
          return false;
        h = -k * s;
        if (taille(h, 33) > 32 || is_inf(h) || !is_zero(im(h, contextptr)) ||
            !is_strictly_positive(h, contextptr))
          return false;
        logarithm = true;
        continue;
      }
      if (integration_power(t, base, -1) && integration_one_plus(base, other)) {
        gen d, n;
        if (denominator || !integration_mellin_monomial(other, x, d, n, budget, contextptr) ||
            d != 1)
          return false;
        q = n / 2;
        denominator = true;
      ...
      gen d, n;
      if (!integration_mellin_monomial(t, x, d, n, budget, contextptr))
        return false;
      c = c * d;
      m += n;
    }
    if (!denominator || !logarithm || !integration_rational(q) ||
        !is_strictly_positive(q, contextptr) || m + 1 != q)
      return false;
    gen quarter = gen(1) / 4;
    gen base =
        cst_pi * ln(2 * cst_pi, contextptr) / 4 +
        cst_pi * ln(Gamma(3 * quarter, contextptr) / Gamma(quarter, contextptr), contextptr) / 2;
    res = c * (base + cst_pi * ln(h / q, contextptr) / 4) / q;
    return true;
  }
  ...
  static bool integration_log_slope(gen e, const gen & x, gen & slope, unsigned & budget,
                                    GIAC_CONTEXT) {
    gen k = integration_coefficient(e, x, contextptr), a, p;
    if (!e.is_symb_of_sommet(at_ln) ||
        !integration_mellin_monomial(e._SYMBptr->feuille, x, a, p, budget, contextptr) || a != 1)
      return false;
    slope = k * p;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_loglog_frullani(const gen & e, const gen & x, const gen & lo,
                                        const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!is_zero(lo) || hi != 1 || !e.is_symb_of_sommet(at_prod) ||
        e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() > 8)
      return false;
    gen numerator, den = 0, h = 1, scale = 1, shift = 0;
    unsigned logs = 0, budget = 64;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen t = v[i], base, slope;
      if (integration_power(t, base, -1) &&
          integration_log_slope(base, x, slope, budget, contextptr)) {
        if (!is_zero(den))
          return false;
        den = -slope;
        continue;
      ...
      if (t.is_symb_of_sommet(at_ln)) {
        if (logs || !integration_log_slope(t._SYMBptr->feuille, x, slope, budget, contextptr) ||
            !is_strictly_positive(-slope, contextptr))
          return false;
        logs = n;
        h = -slope;
        continue;
      ...
      gen a, p;
      if (!integration_mellin_monomial(v[i], x, a, p, budget, contextptr))
        return false;
      scale = scale * a;
      shift += p;
    }
    if (is_zero(den) || !numerator.is_symb_of_sommet(at_plus) ||
        numerator._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & terms = *numerator._SYMBptr->feuille._VECTptr;
    if (terms.size() < 2 || terms.size() > 8)
    ...
    for (unsigned j = 0; j < terms.size(); ++j) {
      gen c, m;
      if (!integration_mellin_monomial(terms[j], x, c, m, budget, contextptr) ||
          !is_strictly_positive(m + shift + 1, contextptr))
        return false;
      gen L = ln(m + shift + 1, contextptr);
      sum += c;
      S1 += c * L;
      if (logs)
        S2 += c * L * L;
    ...
    if (logs) {
      ...
      if (logs == 1)
        ...
        value = -S3 / 3 - cst_euler_gamma * S2 -
                (cst_euler_gamma * cst_euler_gamma + cst_pi * cst_pi / 6) * S1 + 2 * L * first -
                L * L * S1;
    }
    res = scale * value / den;
    return true;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_arc_rational_circle(const gen & e, const gen & x, const gen & lo,
                                            const gen & hi, gen & res, GIAC_CONTEXT) {
    ...
    if (!sine && !v[j].is_symb_of_sommet(at_acos))
      return false;
    gen den, A, B;
    if (!integration_power(v[1 - j], den, -1) || !integration_quadratic(den, x, A, B, contextptr) ||
        A != B || is_zero(A))
      return false;
    gen f = v[j]._SYMBptr->feuille,
        c = ratnormal(integration_coefficient(f, x, contextptr), contextptr);
    if (!integration_rational(c) || !f.is_symb_of_sommet(at_prod) ||
        f._SYMBptr->feuille.type != _VECT)
      return false;
    ...
    if (!integration_rational(nc) || !integration_rational(dc) || is_zero(dc))
      return false;
    gen n2, n0, d2, d0;
    if (!integration_quadratic(numerator, x, n2, n0, contextptr) ||
        !integration_quadratic(denominator, x, d2, d0, contextptr))
      return false;
    n2 = n2 * nc;
    n0 = n0 * nc;
    d2 = d2 * dc;
    d0 = d0 * dc;
    if (is_zero(n2) || n2 != d2 || d0 - n0 != -2 * n2)
      return false;
    gen q = -(n0 + d0) / (4 * n2);
    if (!is_strictly_positive(q, contextptr) ||
        !is_zero(ratnormal(ratnormal(hi * hi - q, contextptr), contextptr)))
      return false;
    gen h = atan(sqrt(q, contextptr), contextptr);
  ...
  static bool integration_period_real_bound(const gen & g, GIAC_CONTEXT) {
    ...
    if (taille(g, 65) > 64)
      return false;
    gen a, b;
    return is_linear_wrt(g, cst_pi, a, b, contextptr) && integration_rational(a) &&
           integration_rational(b);
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_reciprocal_trig_period(const gen & e, const gen & x, const gen & lo,
                                               const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !integration_period_real_bound(lo, contextptr) ||
        !integration_period_real_bound(hi, contextptr))
      return false;
    gen f = e, c = ratnormal(integration_coefficient(f, x, contextptr), contextptr), den;
    if (!integration_rational(c) || !integration_power(f, den, -1))
      return false;
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      ...
      bool cosine = t.is_symb_of_sommet(at_cos);
      if (!integration_rational(k) || (!cosine && !t.is_symb_of_sommet(at_sin)))
        return false;
      if (is_undef(arg))
        arg = t._SYMBptr->feuille;
      else if (arg != t._SYMBptr->feuille)
    ...
    if (!is_strictly_positive(discriminant, contextptr) ||
        ...
        !integration_period_real_bound(b, contextptr))
      return false;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_mellin_two_binomials(const gen & e, const gen & x, const gen & lo,
                                             const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || hi != plus_inf ||
        !e.is_symb_of_sommet(at_prod) || e._SYMBptr->feuille.type != _VECT)
      return false;
    ...
    if (v.size() < 2 || v.size() > 3)
      return false;
    gen m = 0, c = 1, q = 0, a = 0, b = 0, scale = 1;
    unsigned denominators = 0;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base;
      if (integration_power(v[i], base, -1) && base.is_symb_of_sommet(at_plus) &&
          base._SYMBptr->feuille.type == _VECT) {
        const vecteur & terms = *base._SYMBptr->feuille._VECTptr;
        if (denominators == 2 || terms.size() != 2)
          return false;
        ...
        if (!integration_rational(A) || !is_strictly_positive(A, contextptr) ||
            ...
            !is_strictly_positive(B, contextptr) || !is_strictly_positive(n, contextptr))
          return false;
        if (!denominators) {
          q = n;
          a = B / A;
        ...
        gen C, n;
        if (!integration_monomial(v[i], x, C, n, contextptr))
          return false;
        c = c * C;
        m += n;
      }
    }
    if (denominators != 2 || a == b)
      return false;
    gen s = (m + 1) / q;
    if (!is_strictly_positive(s, contextptr) || !is_strictly_positive(2 - s, contextptr))
      return false;
    if (is_one(s))
      res = c * ln(a / b, contextptr) / (scale * q * (a - b));
    else
      res = c * cst_pi * (pow(a, 1 - s, contextptr) - pow(b, 1 - s, contextptr)) /
            (scale * q * (a - b) * sin(cst_pi * s, contextptr));
    return true;
  }
  ...
  static bool integration_exp_affine(const gen & g, const gen & x, gen & rate, gen & shift,
                                     GIAC_CONTEXT) {
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_exp_difference(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                       gen & res, GIAC_CONTEXT) {
    if (!is_zero(lo) || hi != plus_inf || !e.is_symb_of_sommet(at_prod) ||
        e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & factors = *e._SYMBptr->feuille._VECTptr;
    if (factors.size() < 2 || factors.size() > 6)
      return false;
    gen numerator = 1, envelope_rate = 0, envelope_shift = 0;
    gen frequency = 0;
    bool oscillatory = false;
    bool envelope = false, have_numerator = false;
    unsigned inverse = 0, logs = 0;
    for (unsigned i = 0; i < factors.size(); ++i) {
      gen base, rate, shift;
      if (integration_power(factors[i], base, -1) && base == x) {
        if (inverse)
          return false;
        inverse = 1;
      } else if (integration_power(factors[i], base, -2) && base == x) {
        if (inverse)
          return false;
        inverse = 2;
      } else if (factors[i].is_symb_of_sommet(at_ln) && factors[i]._SYMBptr->feuille == x) {
        if (++logs > 2)
          return false;
      } else if (integration_power(factors[i], base, 2) && base.is_symb_of_sommet(at_ln) &&
                 base._SYMBptr->feuille == x) {
        logs += 2;
        if (logs > 2)
          return false;
      } else if (factors[i].is_symb_of_sommet(at_cos)) {
        if (oscillatory || !angle_radian(contextptr) ||
            !is_linear_wrt(factors[i]._SYMBptr->feuille, x, frequency, shift, contextptr) ||
            !integration_resource_rational(frequency) || !is_zero(shift))
          return false;
        oscillatory = true;
    ...
    if (!inverse || !have_numerator || !numerator.is_symb_of_sommet(at_plus) ||
        numerator._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & terms = *numerator._SYMBptr->feuille._VECTptr;
    if (terms.size() < 2 || terms.size() > 8)
      return false;
    vecteur rates, coefficients;
    gen sum = 0, first = 0, common_shift = 0;
    for (unsigned i = 0; i < terms.size(); ++i) {
      gen term = terms[i], c = ratnormal(integration_coefficient(term, x, contextptr), contextptr),
          rate, shift;
      if (!integration_rational(c) || !integration_exp_affine(term, x, rate, shift, contextptr))
        return false;
      rate += envelope_rate;
      shift += envelope_shift;
      if (!is_strictly_positive(rate, contextptr))
        return false;
      if (!i)
        common_shift = shift;
    ...
    if (!is_zero(sum) || (inverse == 2 && !is_zero(first)))
      return false;
    if (oscillatory) {
      if (inverse != 1 || logs)
        return false;
      ...
      for (unsigned i = 0; i < rates.size(); ++i)
        answer -= coefficients[i] * ln(rates[i] * rates[i] + frequency * frequency, contextptr) / 2;
      res = exp(common_shift, contextptr) * answer;
      return true;
    }
    ...
    gen S1 = 0, S2 = 0, S3 = 0;
    for (unsigned i = 0; i < rates.size(); ++i) {
      gen L = ln(rates[i], contextptr),
          weight = coefficients[i] * (inverse == 2 ? rates[i] : gen(1));
      S1 += weight * L;
      if (logs)
        S2 += weight * L * L;
      if (logs == 2)
    ...
    if (logs == 1)
      answer = inverse == 2 ? (1 - gamma) * S1 - S2 / 2 : S2 / 2 + gamma * S1;
    if (logs == 2)
      answer = inverse == 2 ? S3 / 3 + (gamma - 1) * S2 +
                                  (2 - 2 * gamma + gamma * gamma + cst_pi * cst_pi / 6) * S1
                            : -S3 / 3 - gamma * S2 - (gamma * gamma + cst_pi * cst_pi / 6) * S1;
    res = exp(common_shift, contextptr) * answer;
    return true;
  }

  static bool integration_chain_add(vecteur & powers, vecteur & coefficients, const gen & p,
                                    const gen & c) {
    ...
    if (numerator.type != _INT_ || numerator.val < -64 || numerator.val > 64)
      return false;
    if (p.type == _FRAC &&
        (p._FRACptr->den.type != _INT_ || p._FRACptr->den.val < 1 || p._FRACptr->den.val > 8))
      return false;
    if (is_zero(c))
      return true;
  ...
  static bool integration_chain_terms(const gen & g, const gen & x, vecteur & powers,
                                      vecteur & coefficients, unsigned & budget, GIAC_CONTEXT) {
    if (!budget)
      return false;
    --budget;
    if (integration_rational(g))
      return integration_chain_add(powers, coefficients, 0, g);
    if (g == x)
      return integration_chain_add(powers, coefficients, 1, 1);
    if (g.is_symb_of_sommet(at_neg)) {
      vecteur p, c;
      if (!integration_chain_terms(g._SYMBptr->feuille, x, p, c, budget, contextptr))
        return false;
      for (unsigned i = 0; i < p.size(); ++i)
        if (!integration_chain_add(powers, coefficients, p[i], -c[i]))
          return false;
      return true;
    }
    gen base = g, power = 1;
    ...
    if (g.is_symb_of_sommet(at_plus)) {
      for (unsigned i = 0; i < v.size(); ++i)
        if (!integration_chain_terms(v[i], x, powers, coefficients, budget, contextptr))
          return false;
      return true;
    }
    if (!g.is_symb_of_sommet(at_prod) || v.size() > 8)
      return false;
    vecteur rp(1, 0), rc(1, 1);
    for (unsigned i = 0; i < v.size(); ++i) {
      vecteur p, c, np, nc;
      if (!integration_chain_terms(v[i], x, p, c, budget, contextptr) || p.size() * rp.size() > 64)
        return false;
      for (unsigned j = 0; j < p.size(); ++j)
        for (unsigned k = 0; k < rp.size(); ++k)
          if (!integration_chain_add(np, nc, p[j] + rp[k], c[j] * rc[k]))
            return false;
      rp.swap(np);
      rc.swap(nc);
    }
    for (unsigned i = 0; i < rp.size(); ++i)
      if (!integration_chain_add(powers, coefficients, rp[i], rc[i]))
        return false;
    return true;
  }

  static bool integration_chain_endpoint(const vecteur & powers, const vecteur & coefficients,
                                         const gen & bound, gen & value, GIAC_CONTEXT) {
    if (bound != plus_inf &&
        (!integration_rational(bound) || is_strictly_positive(-bound, contextptr)))
      return false;
    if (!is_zero(bound) && bound != plus_inf) {
      gen u = 0;
      for (unsigned i = 0; i < powers.size(); ++i)
        u += coefficients[i] * pow(bound, powers[i], contextptr);
      value = erf(u, contextptr);
      return true;
    }
    bool found = false;
    gen dominant = 0, coefficient = 0, constant = 0;
    for (unsigned i = 0; i < powers.size(); ++i) {
      if (is_zero(coefficients[i]))
        continue;
      if (is_zero(powers[i]))
        constant += coefficients[i];
      if (!found || (bound == plus_inf ? is_strictly_greater(powers[i], dominant, contextptr)
                                       : is_strictly_greater(dominant, powers[i], contextptr))) {
        dominant = powers[i];
        coefficient = coefficients[i];
        found = true;
      }
    }
    if (found && (bound == plus_inf ? is_strictly_positive(dominant, contextptr)
                                    : is_strictly_positive(-dominant, contextptr)))
      value = is_strictly_positive(coefficient, contextptr) ? 1 : -1;
    else
      value = erf(constant, contextptr);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_erf_chain(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                  gen & res, GIAC_CONTEXT) {
    if (taille(e, 129) > 128)
      return false;
    gen f = e, outside = integration_coefficient(f, x, contextptr);
    if (!integration_rational(outside) || !f.is_symb_of_sommet(at_prod) ||
        f._SYMBptr->feuille.type != _VECT)
      return false;
    if ((!is_zero(lo) && (!integration_rational(lo) || !is_strictly_positive(lo, contextptr))) ||
        (hi != plus_inf && (!integration_rational(hi) || !is_strictly_greater(hi, lo, contextptr))))
      return false;
    const vecteur & v = *f._SYMBptr->feuille._VECTptr;
    if (v.size() > 8)
      return false;
    gen u, exponent, weight = 1;
    unsigned order = 0;
    bool gaussian = false, found = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      if (v[i].is_symb_of_sommet(at_exp)) {
        if (gaussian)
          return false;
      ...
      if (base.is_symb_of_sommet(at_pow) && base._SYMBptr->feuille.type == _VECT &&
          base._SYMBptr->feuille._VECTptr->size() == 2) {
        power = base._SYMBptr->feuille[1];
        base = gen(base._SYMBptr->feuille[0]);
      }
      if (base.is_symb_of_sommet(at_erf)) {
        if (found || power.type != _INT_ || power.val < 1 || power.val > 16)
          return false;
        found = true;
        order = power.val;
        u = base._SYMBptr->feuille;
      } else
        weight = weight * v[i];
    }
    if (!gaussian || !found)
    ...
    vecteur up, uc, wp, wc;
    unsigned budget = 64;
    if (!integration_chain_terms(u, x, up, uc, budget, contextptr) || up.size() > 4 ||
        !integration_chain_terms(weight, x, wp, wc, budget, contextptr))
      return false;
    // Match exp(-u^2) structurally first, then allow its bounded expanded form.
    gen square = exponent;
    gen scale = integration_coefficient(square, x, contextptr), inside;
    bool matched = scale == -1 && integration_power(square, inside, 2) && inside == u;
    if (!matched) {
      vecteur ep, ec;
      if (!integration_chain_terms(exponent, x, ep, ec, budget, contextptr))
        return false;
      for (unsigned i = 0; i < up.size(); ++i)
        for (unsigned j = 0; j < up.size(); ++j)
          if (!integration_chain_add(ep, ec, up[i] + up[j], uc[i] * uc[j]))
            return false;
    ...
    vecteur dp, dc;
    for (unsigned i = 0; i < up.size(); ++i)
      if (!integration_chain_add(dp, dc, up[i] - 1, uc[i] * up[i]))
        return false;
    gen multiplier = 0;
    bool ratio = false;
    for (unsigned i = 0; i < dp.size(); ++i) {
      ...
      if (!ratio) {
        multiplier = w / dc[i];
        ratio = true;
      } else if (w != multiplier * dc[i])
        return false;
    }
    if (!ratio || is_zero(multiplier))
      return false;
    for (unsigned j = 0; j < wp.size(); ++j) {
      gen d = 0;
      for (unsigned i = 0; i < dp.size(); ++i)
        if (wp[j] == dp[i])
          d += dc[i];
      if (wc[j] != multiplier * d)
        return false;
    }
    gen a, b;
    if (!integration_chain_endpoint(up, uc, lo, a, contextptr) ||
        !integration_chain_endpoint(up, uc, hi, b, contextptr))
      return false;
    res = outside * multiplier * sqrt(cst_pi, contextptr) *
          (pow(b, int(order + 1), contextptr) - pow(a, int(order + 1), contextptr)) /
          gen(2 * int(order + 1));
    return true;
  }

  static bool integration_rectangle_term(gen e, const gen & x, gen & side, gen & radial,
                                         gen & common, gen & factor, GIAC_CONTEXT) {
    gen coefficient = integration_coefficient(e, x, contextptr);
    if (!integration_rational(coefficient) || !e.is_symb_of_sommet(at_prod) ||
        e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() != 3)
      return false;
    gen a = 0, b = 0, ra = 0, rb = 0, ta = 0, tb = 0;
    bool root = false, den = false, trig = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base, radical;
      if (v[i].is_symb_of_sommet(at_atan)) {
        if (trig)
        ...
        trig = true;
        base = v[i]._SYMBptr->feuille;
        side = integration_coefficient(base, x, contextptr);
        if (!integration_rational(side) || !integration_power(base, radical, -1) ||
            !integration_square_root(radical, base) ||
            !integration_quadratic(base, x, ta, tb, contextptr))
          return false;
      } else {
        if (!integration_power(v[i], base, -1))
          return false;
    ...
    if (!root || !den || !trig || ta != ra || tb != rb || !is_strictly_positive(ra, contextptr) ||
        !is_strictly_positive(rb, contextptr) || !is_strictly_positive(side, contextptr) ||
        is_zero(a) || 2 * b * ra != a * rb)
      return false;
    radial = ra;
    common = rb / 2;
    factor = coefficient * sqrt(ra, contextptr) / a;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_atan_rectangle_pair(const gen & e, const gen & x, const gen & lo,
                                            const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || !integration_rational(hi) ||
        !is_strictly_positive(hi, contextptr) || !e.is_symb_of_sommet(at_plus) ||
        e._SYMBptr->feuille.type != _VECT || e._SYMBptr->feuille._VECTptr->size() != 2)
      return false;
    gen s1, r1, c1, f1, s2, r2, c2, f2;
    if (!integration_rectangle_term(e._SYMBptr->feuille[0], x, s1, r1, c1, f1, contextptr) ||
        !integration_rectangle_term(e._SYMBptr->feuille[1], x, s2, r2, c2, f2, contextptr) ||
        c1 != c2 || s1 * s1 != r2 * hi * hi || s2 * s2 != r1 * hi * hi ||
        !is_zero(ratnormal(f1 - f2, contextptr)))
      return false;
    res = f1 * atan(s1 / sqrt(c1, contextptr), contextptr) *
          atan(s2 / sqrt(c1, contextptr), contextptr) / c1;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_gaussian_erf_exp(const gen & e, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (lo != minus_inf || hi != plus_inf || !e.is_symb_of_sommet(at_exp) || taille(e, 97) > 96)
      return false;
    const gen & exponent = e._SYMBptr->feuille;
    const vecteur * terms =
        exponent.is_symb_of_sommet(at_plus) && exponent._SYMBptr->feuille.type == _VECT
            ? exponent._SYMBptr->feuille._VECTptr
            : 0;
    unsigned count = terms ? terms->size() : 1;
    if (count > 8)
      return false;
    gen a = 0, b = 0, c = 0, remainder = 0;
    bool found = false;
    for (unsigned i = 0; i < count; ++i) {
      ...
      if (t.is_symb_of_sommet(at_erf)) {
        if (found || !integration_rational(coefficient) ||
            !is_linear_wrt(t._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
            !integration_rational(b) || is_zero(a))
          return false;
        c = coefficient;
    ...
    gen q, l, k;
    if (!integration_gaussian_quadratic(remainder, x, q, l, k, contextptr) ||
        !is_strictly_positive(-q, contextptr))
      return false;
    if (!found) {
      // The c=0 limit contains no erf after arithmetic normalization.
      res = exp(k - l * l / (4 * q), contextptr) * sqrt(cst_pi / (-q), contextptr);
      return true;
    }
    if (q != -a * a || l != -2 * a * b)
      return false;
    gen scale = exp(k + b * b, contextptr) * sqrt(cst_pi, contextptr) / abs(a, contextptr);
    res = is_zero(c) ? scale : scale * sinh(c, contextptr) / c;
    return true;
  }
...
// B>0 and A+B>0 guarantee ordinary real convergence without any poles.
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_reciprocal_cosh(const gen & e, const gen & x, const gen & lo,
                                        const gen & hi, gen & res, GIAC_CONTEXT) {
    bool full = lo == minus_inf && hi == plus_inf, half = is_zero(lo) && hi == plus_inf;
    if ((!full && !half) || !angle_radian(contextptr) || taille(e, 65) > 64)
      return false;
    gen f = e, outside = integration_syntax(integration_coefficient(f, x, contextptr), contextptr),
        den;
    if (!integration_rational(outside) || !integration_power(f, den, -1))
      return false;
    const vecteur * terms = den.is_symb_of_sommet(at_plus) && den._SYMBptr->feuille.type == _VECT
                                ? den._SYMBptr->feuille._VECTptr
                                : 0;
    unsigned count = terms ? terms->size() : 1;
    if (count > 4)
      return false;
    ...
    for (unsigned i = 0; i < count; ++i) {
      ...
      if (integration_rational(t)) {
        A += t;
        continue;
      }
      gen coefficient = integration_syntax(integration_coefficient(t, x, contextptr), contextptr);
      if (found || !integration_rational(coefficient) || !t.is_symb_of_sommet(at_cosh) ||
          !is_linear_wrt(t._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
          !integration_rational(b) || is_zero(a))
        return false;
      B = coefficient;
    ...
    if (!found || !is_strictly_positive(B, contextptr) ||
        !is_strictly_positive(A + B, contextptr) || (half && !is_zero(b)))
      return false;
    gen value;
    if (A == B)
      value = inv(B, contextptr);
    else if (is_strictly_positive(A - B, contextptr)) {
      gen root = sqrt(A * A - B * B, contextptr);
      value = ln((A + root) / B, contextptr) / root;
    } else
      value = 2 * atan(sqrt((B - A) / (B + A), contextptr), contextptr) /
              sqrt(B * B - A * A, contextptr);
    res = (full ? 2 : 1) * outside * value / abs(a, contextptr);
    return true;
  }
...
// No repeated differentiation or expansion of erfc is needed; n<=8.
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_gaussian_atan_moment(const gen & e, const gen & x, const gen & lo,
                                             const gen & hi, gen & res, GIAC_CONTEXT) {
    bool full = lo == minus_inf && hi == plus_inf, half = is_zero(lo) && hi == plus_inf;
    if ((!full && !half) || !angle_radian(contextptr) || taille(e, 97) > 96)
      return false;
    gen f = e, outside = integration_syntax(integration_coefficient(f, x, contextptr), contextptr);
    if (!integration_rational(outside) || !f.is_symb_of_sommet(at_prod) ||
        f._SYMBptr->feuille.type != _VECT)
    ...
    if (v.size() > 5)
      return false;
    gen exponent, b = 0, weight = outside;
    bool gaussian = false, angle = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base = v[i], power = 1;
      if (integration_outer_power(base, power) && base.is_symb_of_sommet(at_exp)) {
        if (gaussian)
          return false;
        gaussian = true;
        exponent = integration_syntax(power * base._SYMBptr->feuille, contextptr);
      } else if (v[i].is_symb_of_sommet(at_atan)) {
        gen intercept;
        if (angle || !is_linear_wrt(v[i]._SYMBptr->feuille, x, b, intercept, contextptr) ||
            !integration_rational(b) || is_zero(b) || !is_zero(intercept))
          return false;
        angle = true;
      } else
        weight = weight * v[i];
    }
    gen coefficient, degree, q, l, d;
    unsigned budget = 32;
    if (!gaussian || !angle ||
        !integration_mellin_monomial(weight, x, coefficient, degree, budget, contextptr) ||
        degree.type != _INT_ || degree.val < 1 || degree.val > 17 || degree.val % 2 != 1 ||
        !integration_gaussian_quadratic(exponent, x, q, l, d, contextptr) || !is_zero(l) ||
        !is_strictly_positive(-q, contextptr))
      return false;
    gen a = -q, bb = b * b, j0 = 1, jg = 0, gaussian_moment = gen(1) / 2;
    gen i0 = b / (2 * a), ig = 0;
    unsigned n = (degree.val - 1) / 2;
    for (unsigned k = 1; k <= n; ++k) {
      j0 = -j0 / bb;
      jg = (gaussian_moment - jg) / bb;
    ...
    res = (full ? 2 : 1) * coefficient * exp(d, contextptr) *
          (i0 * kernel + ig * sqrt(cst_pi, contextptr) / root);
    return true;
  }
...
// a,q>0 and N>0 prove ordinary real convergence, also for s=-1 at zero.
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_exponential_log_moment(const gen & e, const gen & x, const gen & lo,
                                               const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!is_zero(lo) || hi != plus_inf || taille(e, 97) > 96)
      return false;
    gen f = e, outside = integration_syntax(integration_coefficient(f, x, contextptr), contextptr);
    if (!integration_rational(outside))
      return false;
    const vecteur * factors = f.is_symb_of_sommet(at_prod) && f._SYMBptr->feuille.type == _VECT
                                  ? f._SYMBptr->feuille._VECTptr
                                  : 0;
    unsigned count = factors ? factors->size() : 1;
    if (count > 5)
      return false;
    gen argument, weight = outside;
    bool found = false;
    for (unsigned i = 0; i < count; ++i) {
      const gen & t = factors ? (*factors)[i] : f;
      if (t.is_symb_of_sommet(at_ln)) {
        if (found)
          return false;
        found = true;
        argument = t._SYMBptr->feuille;
      } else
        weight = weight * t;
    }
    gen other;
    if (!found || !integration_one_plus(argument, other))
      return false;
    gen sign = integration_syntax(integration_coefficient(other, x, contextptr), contextptr),
        power = 1;
    if ((sign != 1 && sign != -1) || !integration_outer_power(other, power) ||
        !other.is_symb_of_sommet(at_exp))
      return false;
    gen rate, q, coefficient, m;
    unsigned budget = 32;
    if (!integration_mellin_monomial(other._SYMBptr->feuille, x, rate, q, budget, contextptr) ||
        !integration_mellin_monomial(weight, x, coefficient, m, budget, contextptr))
      return false;
    gen a = -rate * power;
    if (!is_strictly_positive(a, contextptr) || !is_strictly_positive(q, contextptr) ||
        is_strictly_greater(q, 16, contextptr))
      return false;
    gen N = (m + 1) / q;
    if (N.type != _INT_ || N.val < 1 || N.val > 12)
      return false;
    gen factor = sign == 1 ? 1 - inv(pow(gen(2), N.val), contextptr) : gen(-1);
    res = coefficient * factorial(N.val - 1) * factor * _Zeta(N.val + 1, contextptr) /
          (q * pow(a, N.val, contextptr));
    return true;
  }

  // H(n*pi/4)=integral_0^(n*pi/4) log|sin(u)| du
  // =-n*pi*log(2)/4-G*sigma(n)/2, sigma=(0,1,0,-1).
  static int integration_quarter_sigma(int n) {
    n %= 4;
    if (n < 0)
      n += 4;
    return n == 1 ? 1 : (n == 3 ? -1 : 0);
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_log_trig_quarters(const gen & e, const gen & x, const gen & lo,
                                          const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || is_inf(lo) || is_inf(hi) || taille(e, 129) > 128)
      return false;
    ...
    bool absolute = t.is_symb_of_sommet(at_abs);
    if (absolute)
      t = gen(t._SYMBptr->feuille);
    bool cosine = t.is_symb_of_sommet(at_cos);
    if (!cosine && !t.is_symb_of_sommet(at_sin))
      return false;
    gen a, b, A, B;
    if (!is_linear_wrt(t._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
        !integration_rational(b) || is_zero(a) || !is_linear_wrt(weight, x, A, B, contextptr) ||
        !integration_rational(A) || !integration_rational(B))
      return false;
    if (cosine)
      b += cst_pi / 2;
    gen l = ratnormal((a * lo + b) / cst_pi, contextptr),
    ...
    if (is_zero(A)) {
      gen ql = 4 * l, qr = 4 * r;
      if (ql.type != _INT_ || qr.type != _INT_ || ql.val < -128 || qr.val > 128)
        return false;
      int wave = ql.val >= 0 ? ql.val / 4 : (ql.val - 3) / 4;
      if (!absolute && (wave % 2 || qr.val > 4 * (wave + 1)))
        return false;
      int delta = integration_quarter_sigma(qr.val) - integration_quarter_sigma(ql.val);
      gen value = -(r - l) * cst_pi * ln(gen(2), contextptr);
      if (delta) {
        gen G = (gen(symbolic(at_Psi, makesequence(gen(1) / 4, 1))) -
                 gen(symbolic(at_Psi, makesequence(gen(3) / 4, 1)))) /
                16;
        value -= delta * G / 2;
      }
      res = outside * B * value / a;
      if (reversed)
        res = -res;
      return true;
    }
    gen twice = 2 * l;
    if (twice.type != _INT_ || twice.val < -64 || twice.val > 64)
      return false;
    ...
    l -= wave;
    r -= wave;
    if (!((l == 0 && r == gen(1) / 2) || (l == gen(1) / 2 && r == 1) || (l == 0 && r == 1)))
      return false;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_log_product_zeta(const gen & e, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!is_zero(lo) || !integration_rational(hi) || !is_strictly_positive(hi, contextptr) ||
        taille(e, 129) > 128)
      return false;
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      ...
      gen other, c, n;
      if (!integration_one_plus(t._SYMBptr->feuille, other) ||
          !integration_monomial(other, x, c, n, contextptr) || !integration_rational(c) ||
          !integration_rational(n) || is_zero(c) || !is_strictly_positive(n, contextptr) ||
          is_strictly_greater(n, 32, contextptr))
        return false;
      bool minus = is_strictly_positive(-c, contextptr);
      if (minus) {
        negative += count;
        c = -c;
      }
    ...
    if (!inverse || logs != 2 || !is_zero(ratnormal(b * pow(hi, q, contextptr) - 1, contextptr)))
      return false;
    gen factor = negative == 2 ? gen(2) : (negative == 1 ? -gen(5) / 8 : gen(1) / 4);
    res = outside * factor * _Zeta(3, contextptr) / q;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_log_trig_sum(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                     gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || is_inf(lo) || is_inf(hi) || taille(e, 129) > 128)
      return false;
    gen f = e, outside = integration_coefficient(f, x, contextptr);
    if (!integration_rational(outside) || !f.is_symb_of_sommet(at_ln))
      return false;
    gen arg = f._SYMBptr->feuille, scale = integration_coefficient(arg, x, contextptr);
    if (!integration_rational(scale) || !is_strictly_positive(scale, contextptr) ||
        !arg.is_symb_of_sommet(at_plus) || arg._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *arg._SYMBptr->feuille._VECTptr;
    if (v.size() != 2)
      return false;
    gen A = 0, B = 0, a = 0, b = 0;
    bool found = false, cosine = false;
    gen square_arg;
    bool squared = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      if (!contains(v[i], x) && is_zero(im(v[i], contextptr))) {
        A += v[i];
        continue;
      }
      gen t = v[i], coefficient = integration_coefficient(t, x, contextptr);
      gen base;
      bool square = integration_power(t, base, 2);
      if (square)
        t = base;
      if (!is_zero(im(coefficient, contextptr)) ||
          !(t.is_symb_of_sommet(at_sin) || t.is_symb_of_sommet(at_cos)))
        return false;
      if (!is_linear_wrt(t._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
          !integration_rational(b) || is_zero(a))
        return false;
      if (square) {
        if (found && (!squared || t._SYMBptr->feuille != square_arg))
          return false;
        square_arg = t._SYMBptr->feuille;
        squared = true;
        A += coefficient / 2;
        B += (t.is_symb_of_sommet(at_cos) ? coefficient : -coefficient) / 2;
        a = 2 * a;
        b = 2 * b;
        cosine = true;
      } else {
        if (found)
          return false;
        B = coefficient;
        cosine = t.is_symb_of_sommet(at_cos);
      }
      found = true;
    }
    if (!found || !is_strictly_positive(A, contextptr))
      return false;
    if (is_strictly_positive(A - abs(B, contextptr), contextptr)) {
      ...
      bool half = cosine && l.type == _INT_ && r.type == _INT_ && l.val >= -128 && l.val <= 128 &&
                  r.val >= -128 && r.val <= 128;
      if (!whole && !half)
        return false;
      gen mean = squared ? 2 * ln(sqrt(scale, contextptr) *
                                      (sqrt(A + B, contextptr) + sqrt(A - B, contextptr)) / 2,
                                  contextptr)
                         : ln(scale * (A + sqrt(A * A - B * B, contextptr)) / 2, contextptr);
      res = outside * (hi - lo) * mean;
      return true;
    }
    if (B != A && B != -A)
      return false;
    gen l = ratnormal(2 * (a * lo + b) / cst_pi, contextptr),
        r = ratnormal(2 * (a * hi + b) / cst_pi, contextptr);
    if (l.type != _INT_ || r.type != _INT_ || l.val < -32 || l.val > 32 || r.val < -32 ||
        r.val > 32 || l == r)
      return false;
    int offset = cosine ? (B == A ? 2 : 0) : (B == A ? 1 : 3);
    int delta =
        integration_quarter_sigma(r.val + offset) - integration_quarter_sigma(l.val + offset);
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_atan_log_measure(const gen & e, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !integration_rational(lo) || !integration_rational(hi) ||
        lo == hi || taille(e, 129) > 128)
      return false;
    gen f = e, outside = integration_coefficient(f, x, contextptr);
    if (!integration_rational(outside) || !f.is_symb_of_sommet(at_prod) ||
        f._SYMBptr->feuille.type != _VECT)
    ...
    if (integration_monomial(u, x, c, q, contextptr) && is_strictly_positive(c, contextptr) &&
        is_strictly_positive(q, contextptr)) {
      if (is_strictly_greater(q, 32, contextptr) || is_strictly_positive(-lo, contextptr) ||
          is_strictly_positive(-hi, contextptr))
        return false;
      gen wc, wn;
      if (!integration_monomial(weight, x, wc, wn, contextptr) || wn != q - 1)
        return false;
      if (den._SYMBptr->feuille.type != _VECT || den._SYMBptr->feuille._VECTptr->size() != 2)
        return false;
      const vecteur & d = *den._SYMBptr->feuille._VECTptr;
      for (unsigned i = 0; i < 2; ++i) {
        gen dc, dn;
        if (!integration_monomial(d[i], x, dc, dn, contextptr))
          return false;
        if (is_zero(dn))
          A += dc;
        else if (dn == q)
      ...
      if (is_zero(A) || B != A * c)
        return false;
      dl = c * pow(lo, q, contextptr);
      dh = c * pow(hi, q, contextptr);
      multiplier = wc / (c * q * A);
    } else {
      gen wa, wb;
      if (!is_linear_wrt(u, x, a, b, contextptr) || !integration_rational(a) ||
          !integration_rational(b) || is_zero(a) || !is_linear_wrt(weight, x, wa, wb, contextptr) ||
          !is_zero(wa) || !integration_rational(wb) || !is_linear_wrt(den, x, A, B, contextptr) ||
          !integration_rational(A) || !integration_rational(B) || is_zero(A) ||
          B * a != A * (1 + b))
        return false;
      dl = a * lo + b;
    ...
    dl = ratnormal(dl, contextptr);
    dh = ratnormal(dh, contextptr);
    if (!((is_zero(dl) && dh == 1) || (dl == 1 && is_zero(dh))))
      return false;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_mixed_mellin_log(const gen & e, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || hi != plus_inf || taille(e, 129) > 128)
      return false;
    vecteur factors;
    if (e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT)
      factors = *e._SYMBptr->feuille._VECTptr;
    else
      factors.push_back(e);
    if (factors.size() > 8)
      return false;
    gen coefficient = 1, power = 0, a[2], k[2];
    unsigned count = 0, logs = 0, budget = 64;
    for (unsigned i = 0; i < factors.size(); ++i) {
      ...
      if (integration_power(factors[i], base, -1) && integration_one_plus(base, other)) {
        if (count >= 2 ||
            !integration_mellin_monomial(other, x, a[count], k[count], budget, contextptr) ||
            !integration_rational(a[count]) || !integration_rational(k[count]) ||
            !is_strictly_positive(a[count], contextptr) ||
            !is_strictly_positive(k[count], contextptr))
          return false;
        ++count;
        continue;
      }
      gen c, p;
      if (!integration_mellin_monomial(factors[i], x, c, p, budget, contextptr) ||
          !integration_rational(c))
        return false;
    ...
    if (k[1] != 2 * k[0] || a[1] != a[0] * a[0])
      return false;
    gen s = (power + 1) / k[0];
    if (!integration_rational(s) || !is_strictly_positive(s, contextptr) ||
        !is_strictly_positive(3 - s, contextptr))
      return false;
    ...
    if (s.type == _FRAC && (s._FRACptr->den.type != _INT_ || s._FRACptr->den.val > 12))
      return false;
    gen L = logs && !is_one(a[0]) ? ln(a[0], contextptr) : gen(0);
    gen value = integration_mixed_mellin_value(s, logs, L, contextptr);
    res = coefficient * pow(a[0], -s, contextptr) * value / pow(k[0], int(logs + 1));
    return true;
  }
#if defined(__GNUC__) && !defined(__clang__)
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_oscillatory_cancellation(const gen & e, const gen & x, const gen & lo,
                                                 const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || taille(e, 129) > 128)
      return false;
    int halves = (lo == minus_inf && hi == plus_inf)                                     ? 2
                 : ((is_zero(lo) && hi == plus_inf) || (lo == minus_inf && is_zero(hi))) ? 1
                                                                                         : 0;
    if (!halves)
      return false;
    vecteur factors;
    ...
    if (factors.size() > 6)
      return false;
    gen coefficient = 1, power = 0, numerator;
    ...
    for (unsigned i = 0; i < factors.size(); ++i) {
      ...
      gen c, p;
      if (!integration_monomial(factors[i], x, c, p, contextptr) || !integration_rational(c))
        return false;
      coefficient = coefficient * c;
      power += p;
    }
    if (!have_numerator || (power != -2 && power != -3) ||
        numerator._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & terms = *numerator._SYMBptr->feuille._VECTptr;
    if (terms.size() < 2 || terms.size() > 16)
      return false;
    ...
    for (unsigned i = 0; i < terms.size(); ++i) {
      ...
      if (!integration_rational(c))
        return false;
      vecteur atoms;
      if (term.is_symb_of_sommet(at_prod) && term._SYMBptr->feuille.type == _VECT)
        atoms = *term._SYMBptr->feuille._VECTptr;
      else
        atoms.push_back(term);
      if (atoms.size() > 4)
        return false;
      ...
      for (unsigned j = 0; j < atoms.size(); ++j) {
        ...
        if (atom.is_symb_of_sommet(at_sin) || atom.is_symb_of_sommet(at_cos)) {
          gen shift;
          if (have_trig ||
              !is_linear_wrt(atom._SYMBptr->feuille, x, frequency, shift, contextptr) ||
              !integration_rational(frequency) || !is_zero(shift))
            return false;
          sine = atom.is_symb_of_sommet(at_sin);
          have_trig = true;
          continue;
        }
        gen d, q;
        if (!integration_monomial(atom, x, d, q, contextptr) || !integration_rational(d))
          return false;
        c = c * d;
        p += q;
      }
      ...
      if (power == -2) {
        ...
        if (sine) {
          if (!is_zero(p))
            return false;
          cancellation += c * frequency;
          value -= c * frequency * magnitude / 4;
        } else {
          if (p != 1)
            return false;
          cancellation += c;
    ...
    if (!is_zero(cancellation))
      return false;
    res = halves * coefficient * cst_pi * value;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_erf_tail_product(const gen & e, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!is_zero(lo) || hi != plus_inf)
      return false;
    vecteur single;
    ...
    if (v->size() > 6)
      return false;
    gen slope[2], degree[2], weight = 1, m = 0;
    bool complement[2];
    unsigned count = 0, budget = 48;
    for (unsigned i = 0; i < v->size(); ++i) {
      const gen & term = (*v)[i];
      gen base = term;
      unsigned repeat = 1;
      if (term.is_symb_of_sommet(at_pow) && term._SYMBptr->feuille.type == _VECT &&
          term._SYMBptr->feuille._VECTptr->size() == 2 && term._SYMBptr->feuille[1] == 2) {
        base = term._SYMBptr->feuille[0];
        repeat = 2;
      }
      ...
      if (integration_one_plus(base, other)) {
        ...
        if (c == -1 && other.is_symb_of_sommet(at_erf))
          base = symbolic(at_erfc, other._SYMBptr->feuille);
      }
      if (base.is_symb_of_sommet(at_erf) || base.is_symb_of_sommet(at_erfc)) {
        ...
        if (!integration_mellin_monomial(base._SYMBptr->feuille, x, a, q, budget, contextptr) ||
            !integration_rational(a) || is_zero(a) || !integration_rational(q) ||
            !is_strictly_positive(q, contextptr))
          return false;
        while (repeat--) {
          slope[count] = a;
          degree[count] = q;
          complement[count] = base.is_symb_of_sommet(at_erfc);
          ++count;
        }
      } else {
        gen a, q;
        if (!integration_mellin_monomial(term, x, a, q, budget, contextptr) ||
            !integration_rational(a))
          return false;
        weight = weight * a;
        m += q;
      }
    }
    if (count != 2 || degree[0] != degree[1] || m + 1 != degree[0] ||
        (!complement[0] && !complement[1]))
      return false;
    if (!complement[1]) {
      swapgen(slope[0], slope[1]);
      bool t = complement[0];
      complement[0] = complement[1];
      complement[1] = t;
    }
    gen a = slope[0], b = slope[1];
    if (!is_strictly_positive(b, contextptr) ||
        (complement[0] && !is_strictly_positive(a, contextptr)))
      return false;
    gen h = sqrt(a * a + b * b, contextptr);
    res = weight * (complement[0] ? a + b - h : h - b) /
          (degree[0] * a * b * sqrt(cst_pi, contextptr));
    return true;
  }
  ...
  static gen integration_cauchy_log_moment(unsigned n, GIAC_CONTEXT) {
    if (n % 2)
      return (gen(symbolic(at_Psi, makesequence(gen(1) / 4, int(n)))) -
              gen(symbolic(at_Psi, makesequence(gen(3) / 4, int(n))))) /
             pow(gen(4), int(n + 1));
    ...
    for (unsigned j = 2; j <= n; j += 2) {
      ...
      for (unsigned k = 0; k < j; ++k) {
        if (!(k % 2))
          E[j] -= choose * E[k];
        choose = choose * int(j - k) / int(k + 1);
      }
    }
    return gen(E[n] < 0 ? -E[n] : E[n]) * pow(cst_pi, int(n + 1)) / pow(gen(2), int(n + 2));
  }

  static bool integration_log_affine_ratio(gen f, const gen & x, gen & scale, gen & n1, gen & n0,
                                           gen & d1, gen & d0, GIAC_CONTEXT) {
    scale = integration_syntax(integration_coefficient(f, x, contextptr), contextptr);
    if (!integration_rational(scale) || is_zero(scale) || !f.is_symb_of_sommet(at_prod) ||
        f._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *f._SYMBptr->feuille._VECTptr;
    if (v.size() != 2)
    ...
    return found && is_linear_wrt(numerator, x, n1, n0, contextptr) &&
           is_linear_wrt(denominator, x, d1, d0, contextptr) && integration_rational(n1) &&
           integration_rational(n0) && integration_rational(d1) && integration_rational(d0);
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_mobius_cauchy_log(const gen & e, const gen & x, const gen & lo,
                                          const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !integration_rational(lo) || !integration_rational(hi) ||
        lo == hi || taille(e, 129) > 128)
      return false;
    ...
    for (unsigned i = 0; i < 2; ++i) {
      ...
      if (t.is_symb_of_sommet(at_pow) && t._SYMBptr->feuille.type == _VECT &&
          t._SYMBptr->feuille._VECTptr->size() == 2) {
        power = t._SYMBptr->feuille[1];
        t = gen(t._SYMBptr->feuille[0]);
      }
      if (found || !t.is_symb_of_sommet(at_ln) || power.type != _INT_ || power.val < 1 ||
          power.val > 8)
        return false;
      found = true;
      order = power.val;
    ...
    if (!found || !inverse)
      return false;
    gen A, B, C;
    if (!integration_gaussian_quadratic(den, x, A, B, C, contextptr) || !integration_rational(A) ||
        !integration_rational(B) || !integration_rational(C) || is_zero(A))
      return false;
    gen center = -B / (2 * A), width2 = C / A - center * center;
    if (!is_strictly_positive(width2, contextptr))
      return false;
    gen width = sqrt(width2, contextptr);
    if (!integration_rational(width))
      return false;
    gen l = ratnormal((lo - center) / width, contextptr),
        r = ratnormal((hi - center) / width, contextptr);
    bool reverse = false;
    if (is_strictly_greater(l, r, contextptr)) {
      swapgen(l, r);
      reverse = true;
    }
    if (!((l == -1 && (r == 0 || r == 1)) || (l == 0 && r == 1)))
      return false;
    gen scale, n1, n0, d1, d0;
    if (!integration_log_affine_ratio(logarg, x, scale, n1, n0, d1, d0, contextptr))
      return false;
    gen nc = n1 * center + n0, dc = d1 * center + d0;
    if (is_zero(nc) || is_zero(dc))
      return false;
    gen sign = n1 * width / nc;
    if ((sign != 1 && sign != -1) || d1 * width / dc != -sign)
      return false;
    gen k = scale * nc / dc;
    if (!is_strictly_positive(k, contextptr))
      return false;
    gen shift = ln(k, contextptr), answer = 0;
    int choose = 1;
    for (unsigned j = 0; j <= order; ++j) {
      ...
      if (side && (!is_zero(shift) || j == order)) {
        ...
        if (sign == -1 && j % 2)
          term = -term;
        if (j != order)
          term = term * pow(shift, int(order - j), contextptr);
        answer += term;
      }
      choose = choose * int(order - j) / int(j + 1);
    }
    res = outside * answer / (A * width);
    if (reverse)
      res = -res;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_log_sine_cosine_sum(const gen & e, const gen & x, const gen & lo,
                                            const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || is_inf(lo) || is_inf(hi) || taille(e, 129) > 128)
      return false;
    gen f = e, outside = integration_syntax(integration_coefficient(f, x, contextptr), contextptr);
    if (!integration_rational(outside) || !f.is_symb_of_sommet(at_ln))
      return false;
    gen arg = f._SYMBptr->feuille;
    bool absolute = arg.is_symb_of_sommet(at_abs);
    if (absolute)
      arg = gen(arg._SYMBptr->feuille);
    gen scale = integration_syntax(integration_coefficient(arg, x, contextptr), contextptr);
    if (!integration_rational(scale) || is_zero(scale) || !arg.is_symb_of_sommet(at_plus) ||
        arg._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *arg._SYMBptr->feuille._VECTptr;
    if (v.size() != 2)
      return false;
    gen A = 0, B = 0, angle;
    bool sine = false, cosine = false;
    for (unsigned i = 0; i < 2; ++i) {
      gen t = v[i], c = integration_syntax(integration_coefficient(t, x, contextptr), contextptr);
      if (!integration_rational(c))
        return false;
      c = c * scale;
      if (t.is_symb_of_sommet(at_sin)) {
        if (sine)
          return false;
        sine = true;
        A = c;
      } else if (t.is_symb_of_sommet(at_cos)) {
        if (cosine)
          return false;
        cosine = true;
        B = c;
        ...
        return false;
      if (i && t._SYMBptr->feuille != angle)
        return false;
      angle = t._SYMBptr->feuille;
    }
    if (!sine || !cosine || is_zero(A) || (A != B && A != -B))
      return false;
    gen a, b;
    if (!is_linear_wrt(angle, x, a, b, contextptr) || !integration_rational(a) || is_zero(a) ||
        (!integration_rational(b) && !integration_rational(ratnormal(b / cst_pi, contextptr))))
      return false;
    int phase = is_strictly_positive(A, contextptr)
                    ? (is_strictly_positive(B, contextptr) ? 1 : -1)
                    : (is_strictly_positive(B, contextptr) ? 3 : -3);
    gen l = ratnormal(4 * (a * lo + b) / cst_pi, contextptr),
        r = ratnormal(4 * (a * hi + b) / cst_pi, contextptr);
    if (l.type != _INT_ || r.type != _INT_ || l.val < -64 || l.val > 64 || r.val < -64 ||
        r.val > 64 || l == r)
      return false;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_gamma_log_moment(const gen & e, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || hi != plus_inf)
      return false;
    gen input = e,
        outside = integration_syntax(integration_coefficient(input, x, contextptr), contextptr);
    if (!integration_rational(outside))
      return false;
    vecteur singleton;
    const vecteur * v;
    if (input.is_symb_of_sommet(at_prod) && input._SYMBptr->feuille.type == _VECT)
      v = input._SYMBptr->feuille._VECTptr;
    else {
      singleton.push_back(input);
      v = &singleton;
    }
    if (v->size() > 8)
      return false;
    ...
    for (unsigned i = 0; i < v->size(); ++i) {
      const gen & term = (*v)[i];
      gen base = term, power = 1;
      if (!integration_outer_power(base, power))
        return false;
      if (base.is_symb_of_sommet(at_exp)) {
        if (exponential || !integration_rational(power) ||
            is_strictly_greater(abs(power, contextptr), 16, contextptr))
          return false;
        gen a;
        if (!integration_mellin_monomial(base._SYMBptr->feuille, x, a, q, budget, contextptr) ||
            !integration_rational(a))
      ...
      gen logbase = base;
      bool negative_log =
          base.is_symb_of_sommet(at_neg) && base._SYMBptr->feuille.is_symb_of_sommet(at_ln);
      if (negative_log)
        base = gen(base._SYMBptr->feuille);
      if (base.is_symb_of_sommet(at_ln)) {
        if (logs || power.type != _INT_ || power.val < 1 || power.val > 4 ||
            (term != logbase &&
             (!term.is_symb_of_sommet(at_pow) || term._SYMBptr->feuille[0] != logbase ||
              term._SYMBptr->feuille[1].type != _INT_)))
          return false;
        if (!integration_mellin_monomial(base._SYMBptr->feuille, x, logscale, logpower, budget,
                                         contextptr) ||
            !integration_rational(logscale) || !integration_rational(logpower) ||
            !is_strictly_positive(logscale, contextptr) || is_zero(logpower))
          return false;
        logs = power.val;
        if (negative_log && logs % 2)
          coefficient = -coefficient;
        continue;
      }
      gen a, p;
      if (!integration_mellin_monomial(term, x, a, p, budget, contextptr) ||
          !integration_rational(a))
        return false;
      coefficient = coefficient * a;
      m += p;
    }
    if (!exponential || !integration_rational(q) || !is_strictly_positive(q, contextptr) ||
        !is_strictly_positive(rate, contextptr))
      return false;
    gen s = (m + 1) / q;
    if (!integration_rational(s) || !is_strictly_positive(s, contextptr) ||
        is_strictly_greater(s, 16, contextptr) ||
        (s.type == _FRAC && (s._FRACptr->den.type != _INT_ || s._FRACptr->den.val > 16)))
      return false;
    ...
    gen ratio = logpower / q, scale = 1;
    vecteur cumulant(logs + 1);
    for (unsigned k = 1; k <= logs; ++k) {
      scale = scale * ratio;
      cumulant[k] = scale * integration_beta_psi(s, k - 1, contextptr);
      if (k == 1)
        cumulant[k] += ln(logscale, contextptr) - ratio * ln(rate, contextptr);
    }
    bool compact = false;
    res = value * integration_cumulant_moment(cumulant, logs, compact, contextptr);
    if (!compact)
      res = ratnormal(res, contextptr);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_atan_log_cauchy(const gen & e, const gen & x, const gen & lo,
                                        const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || taille(e, 129) > 128)
      return false;
    gen f = e, outside = integration_syntax(integration_coefficient(f, x, contextptr), contextptr);
    ...
    if (v.size() < 3 || v.size() > 5)
      return false;
    gen u, logarg, den, weight = 1;
    bool angle = false, logarithm = false, inverse = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base;
      if (v[i].is_symb_of_sommet(at_atan)) {
        if (angle)
        ...
        angle = true;
        u = v[i]._SYMBptr->feuille;
      } else if (v[i].is_symb_of_sommet(at_ln)) {
        if (logarithm)
          return false;
        logarithm = true;
        logarg = v[i]._SYMBptr->feuille;
      } else if (integration_power(v[i], base, -1) && base.is_symb_of_sommet(at_plus)) {
        if (inverse)
          return false;
        inverse = true;
    ...
    if (!angle || !logarithm || !inverse)
      return false;
    gen c, q, a, b, dl, dh, multiplier, shift, power;
    if (integration_monomial(u, x, c, q, contextptr) && is_strictly_positive(c, contextptr) &&
        is_strictly_positive(q, contextptr)) {
      if (is_strictly_greater(q, 32, contextptr) || (!integration_rational(lo) && lo != plus_inf) ||
          (!integration_rational(hi) && hi != plus_inf) || is_strictly_positive(-lo, contextptr) ||
          is_strictly_positive(-hi, contextptr))
        return false;
      gen wc, wn, lc, ln;
      if (!integration_monomial(weight, x, wc, wn, contextptr) || wn != q - 1 ||
          !integration_monomial(logarg, x, lc, ln, contextptr) ||
          !is_strictly_positive(lc, contextptr))
        return false;
      if (den._SYMBptr->feuille.type != _VECT || den._SYMBptr->feuille._VECTptr->size() != 2)
        return false;
      gen A = 0, B = 0;
      const vecteur & terms = *den._SYMBptr->feuille._VECTptr;
      for (unsigned i = 0; i < 2; ++i) {
        gen dc, dn;
        if (!integration_monomial(terms[i], x, dc, dn, contextptr))
          return false;
        if (is_zero(dn))
          A += dc;
        else if (dn == 2 * q)
      ...
      if (is_zero(A) || B != A * c * c)
        return false;
      power = ln / q;
      shift = giac::ln(lc, contextptr) - power * giac::ln(c, contextptr);
      multiplier = wc / (A * c * q);
      dl = lo == plus_inf ? plus_inf : c * pow(lo, q, contextptr);
      dh = hi == plus_inf ? plus_inf : c * pow(hi, q, contextptr);
    } else {
      gen wa, wb, A, B, C, la, lb;
      if (!is_linear_wrt(u, x, a, b, contextptr) || !integration_rational(a) ||
          !integration_rational(b) || is_zero(a) || !is_linear_wrt(weight, x, wa, wb, contextptr) ||
          !is_zero(wa) || !integration_rational(wb) ||
          !integration_gaussian_quadratic(den, x, A, B, C, contextptr) || is_zero(A) ||
          B * a != 2 * A * b || C * a * a != A * (1 + b * b) ||
          !is_linear_wrt(logarg, x, la, lb, contextptr) || !integration_rational(la) ||
          !integration_rational(lb) || la * b != lb * a ||
          !is_strictly_positive(la / a, contextptr))
        return false;
      power = 1;
      shift = ln(la / a, contextptr);
      multiplier = wb * a / A;
      dl = ratnormal(a * lo + b, contextptr);
      dh = ratnormal(a * hi + b, contextptr);
    }
    if (is_strictly_greater(power, 32, contextptr) || is_strictly_greater(-power, 32, contextptr))
      return false;
    bool reverse = dl == plus_inf || (dl == 1 && dh == 0);
    if (reverse)
      swapgen(dl, dh);
    bool lower = dl == 0 && dh == 1, upper = dl == 1 && dh == plus_inf,
         full = dl == 0 && dh == plus_inf;
    if (!lower && !upper && !full)
      return false;
    gen result = 0;
    if (!is_zero(shift))
      result = shift * cst_pi * cst_pi * (full ? gen(1) / 8 : (upper ? gen(3) / 32 : gen(1) / 32));
    if (!is_zero(power)) {
    ...
    res = outside * multiplier * result;
    if (reverse)
      res = -res;
    return true;
  }

  static bool integration_trig_unit_log(const gen & f, const gen & x, gen & scale, gen & angle,
                                        bool & cosine, int & sign, GIAC_CONTEXT) {
    if (!f.is_symb_of_sommet(at_ln))
      return false;
    gen arg = f._SYMBptr->feuille,
        outer = integration_syntax(integration_coefficient(arg, x, contextptr), contextptr);
    if (!integration_rational(outer) || !arg.is_symb_of_sommet(at_plus) ||
        arg._SYMBptr->feuille.type != _VECT || arg._SYMBptr->feuille._VECTptr->size() != 2)
      return false;
    const vecteur & v = *arg._SYMBptr->feuille._VECTptr;
    gen A = 0, B = 0;
    bool found = false;
    for (unsigned i = 0; i < 2; ++i) {
      ...
      gen t = v[i], c = integration_syntax(integration_coefficient(t, x, contextptr), contextptr);
      if (found || !integration_rational(c) ||
          !(t.is_symb_of_sommet(at_sin) || t.is_symb_of_sommet(at_cos)))
        return false;
      found = true;
      B = c;
    ...
    if (!found || is_zero(A) || (B != A && B != -A))
      return false;
    scale = outer * A;
    if (!is_strictly_positive(scale, contextptr))
      return false;
    sign = B == A ? 1 : -1;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_opposite_trig_logs(const gen & e, const gen & x, const gen & lo,
                                           const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || is_inf(lo) || is_inf(hi) || taille(e, 129) > 128)
      return false;
    gen f = e, outside = integration_syntax(integration_coefficient(f, x, contextptr), contextptr);
    ...
    const vecteur & v = *f._SYMBptr->feuille._VECTptr;
    gen A, B, theta, other;
    bool cosine, othercosine;
    int sign, othersign;
    if (!integration_trig_unit_log(v[0], x, A, theta, cosine, sign, contextptr) ||
        !integration_trig_unit_log(v[1], x, B, other, othercosine, othersign, contextptr) ||
        theta != other || cosine != othercosine || sign != -othersign)
      return false;
    gen a, b;
    if (!is_linear_wrt(theta, x, a, b, contextptr) || !integration_rational(a) || is_zero(a) ||
        (!integration_rational(b) && !integration_rational(ratnormal(b / cst_pi, contextptr))))
      return false;
    gen l = ratnormal(2 * (a * lo + b) / cst_pi, contextptr),
        r = ratnormal(2 * (a * hi + b) / cst_pi, contextptr);
    ...
    int offset = cosine ? (sign == 1 ? 2 : 0) : (sign == 1 ? 1 : 3),
        otheroffset = cosine ? (sign == 1 ? 0 : 2) : (sign == 1 ? 3 : 1);
    int delta =
        integration_quarter_sigma(r.val + offset) - integration_quarter_sigma(l.val + offset);
    int otherdelta = integration_quarter_sigma(r.val + otheroffset) -
                     integration_quarter_sigma(l.val + otheroffset);
  ...
  static bool integration_scaled_chain_terms(const gen & g, const gen & x, vecteur & powers,
                                             vecteur & coefficients, unsigned & budget,
                                             GIAC_CONTEXT) {
    const vecteur * terms = g.is_symb_of_sommet(at_plus) && g._SYMBptr->feuille.type == _VECT
                                ? g._SYMBptr->feuille._VECTptr
                                : 0;
    ...
    if (count > 4)
      return false;
    for (unsigned i = 0; i < count; ++i) {
      gen t = integration_syntax(terms ? (*terms)[i] : g, contextptr),
          c = integration_syntax(integration_coefficient(t, x, contextptr), contextptr);
      if (!integration_rational(c))
        return false;
      vecteur p, v;
      if (!integration_chain_terms(t, x, p, v, budget, contextptr))
        return false;
      for (unsigned j = 0; j < p.size(); ++j)
        if (!integration_chain_add(powers, coefficients, p[j], c * v[j]))
          return false;
    }
    return true;
  }
...
// moments. Only scalar rational recurrence coefficients are retained.
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_monomial_gaussian_erf(const gen & e, const gen & x, const gen & lo,
                                              const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!is_zero(lo) || hi != plus_inf || taille(e, 97) > 96)
      return false;
    gen f = e, outside = integration_syntax(integration_coefficient(f, x, contextptr), contextptr);
    ...
    if (v.size() > 6)
      return false;
    gen exponent, argument, weight = outside;
    bool gaussian = false, error = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base = v[i], power = 1;
      if (integration_outer_power(base, power) && base.is_symb_of_sommet(at_exp)) {
        if (gaussian)
    ...
    if (!gaussian || !error)
      return false;
    vecteur ep, ec, ap, ac, wp, wc;
    unsigned budget = 64;
    if (!integration_scaled_chain_terms(exponent, x, ep, ec, budget, contextptr) || ep.size() > 2 ||
        !integration_scaled_chain_terms(argument, x, ap, ac, budget, contextptr) ||
        ap.size() != 1 || !integration_scaled_chain_terms(weight, x, wp, wc, budget, contextptr) ||
        wp.size() != 1)
      return false;
    gen a = 0, q = 0, d = 0;
    ...
    if (!is_strictly_positive(a, contextptr) || !is_strictly_positive(q, contextptr) ||
        is_strictly_greater(q, 16, contextptr) || q != 2 * ap[0] || is_zero(ac[0]))
      return false;
    gen N = (wp[0] + 1) / q;
    if (N.type != _INT_ || N.val < 1 || N.val > 8)
      return false;
    gen b = ac[0], v0 = a + b * b, polynomial = 1, moment = 1;
    for (int j = 1; j < N.val; ++j) {
      moment = moment * gen(2 * j - 1) / (2 * v0);
      polynomial = gen(j) * polynomial / a + moment;
    }
    res = wc[0] * exp(d, contextptr) * b * polynomial / (q * a * sqrt(v0, contextptr));
    return true;
  }
...
// power k leaves the finite harmonic polynomial cos(t)^(2*k-2).
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_atan_cauchy_power(const gen & e, const gen & x, const gen & lo,
                                          const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || hi != plus_inf || taille(e, 97) > 96)
      return false;
    gen f = e, outside = integration_syntax(integration_coefficient(f, x, contextptr), contextptr);
    if (!integration_rational(outside) || !f.is_symb_of_sommet(at_prod) ||
        f._SYMBptr->feuille.type != _VECT)
    ...
    if (v.size() > 6)
      return false;
    gen argument, denominator, weight = outside;
    int n = 0, k = 0;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen angle = v[i];
      int order = 1;
      ...
      gen angle_coefficient =
          integration_syntax(integration_coefficient(angle, x, contextptr), contextptr);
      if (angle.is_symb_of_sommet(at_atan)) {
        if (n || !integration_rational(angle_coefficient))
          return false;
        n = order;
        argument = angle._SYMBptr->feuille;
        weight = weight * pow(angle_coefficient, order);
        continue;
      }
      gen base = v[i], power = 1;
      if (integration_outer_power(base, power) && base.is_symb_of_sommet(at_plus) &&
          power.type == _INT_ && power.val <= -1 && power.val >= -4) {
        if (k)
          return false;
        k = -power.val;
        denominator = base;
    ...
    if (!n || !k)
      return false;
    vecteur ap, ac, dp, dc, wp, wc;
    unsigned budget = 64;
    if (!integration_scaled_chain_terms(argument, x, ap, ac, budget, contextptr) ||
        ap.size() != 1 ||
        !integration_scaled_chain_terms(denominator, x, dp, dc, budget, contextptr) ||
        dp.size() != 2 || !integration_scaled_chain_terms(weight, x, wp, wc, budget, contextptr) ||
        wp.size() != 1)
      return false;
    gen a = ac[0], q = ap[0], A = 0, B = 0;
    if (is_zero(a) || !is_strictly_positive(q, contextptr) ||
        is_strictly_greater(q, 16, contextptr) || wp[0] != q - 1)
      return false;
    for (unsigned i = 0; i < dp.size(); ++i) {
      if (is_zero(dp[i]))
        A += dc[i];
    ...
    if (!is_strictly_positive(A, contextptr) || B != A * a * a)
      return false;
    int h = k - 1;
    gen L = cst_pi / 2, divisor = pow(gen(4), h);
    gen value = comb(2 * h, h, contextptr) * pow(L, n + 1) / (gen(n + 1) * divisor);
    for (int j = 1; j <= h; ++j) {
      gen omega2 = gen(4 * j * j), sign = j % 2 ? gen(-1) : gen(1), previous = 0,
          current = (sign - 1) / omega2;
      for (int m = 2; m <= n; ++m) {
        gen next = gen(m) * pow(L, m - 1) * sign / omega2 - gen(m * (m - 1)) * previous / omega2;
        previous = current;
        current = next;
      }
    ...
    if (is_strictly_positive(-a, contextptr) && n % 2)
      value = -value;
    res = ratnormal(wc[0] * value / (q * abs(a, contextptr) * pow(A, k)), contextptr);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_exponential_beta(const gen & e, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!is_zero(lo) || hi != plus_inf || taille(e, 97) > 96 || !e.is_symb_of_sommet(at_prod) ||
        e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() != 2)
    ...
    for (unsigned i = 0; i < 2; ++i) {
      gen base = v[i], n = 1;
      if (!integration_outer_power(base, n) || !integration_resource_rational(n) ||
          !is_strictly_greater(n, -1, contextptr) || is_strictly_greater(n, 8192, contextptr))
        continue;
      gen other;
      if (!integration_one_plus(base, other))
        continue;
      ...
      if (sign != -1)
        continue;
      gen bp = 1;
      if (!integration_outer_power(other, bp) || !other.is_symb_of_sommet(at_exp))
        continue;
      gen slope, phase;
      if (!is_linear_wrt(other._SYMBptr->feuille, x, slope, phase, contextptr) || !is_zero(phase))
        continue;
      gen b = -slope * bp;
      if (!integration_resource_rational(b) || !is_strictly_positive(b, contextptr))
        continue;
      gen carrier = v[1 - i], ap = 1;
      if (!integration_outer_power(carrier, ap) || !carrier.is_symb_of_sommet(at_exp))
        continue;
      if (!is_linear_wrt(carrier._SYMBptr->feuille, x, slope, phase, contextptr) || !is_zero(phase))
        continue;
      gen a = -slope * ap;
      if (!integration_resource_rational(a) || !is_strictly_positive(a, contextptr))
        continue;
      gen r = a / b;
      if (r.type != _INT_ || r.val < 1 || r.val > 16)
        continue;
      gen answer = gen(1) / (n + 1);
      for (int j = 2; j <= r.val; ++j)
        answer = answer * gen(j - 1) / (n + j);
      res = answer / b;
      return true;
    }
    return false;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_complementary_ratio(const gen & e, const gen & x, const gen & lo,
                                            const gen & hi, gen & res, GIAC_CONTEXT) {
    ...
    if (integration_power(v[0], denominator, -1))
      numerator = v[1];
    else if (integration_power(v[1], denominator, -1))
      numerator = v[0];
    else
      return false;
    if (!denominator.is_symb_of_sommet(at_plus) || denominator._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & terms = *denominator._SYMBptr->feuille._VECTptr;
    if (terms.size() != 2 || (numerator != terms[0] && numerator != terms[1]))
      return false;
    gen left = terms[0], right = terms[1], p = 1, q = 1;
    if (!integration_outer_power(left, p) || !integration_outer_power(right, q) || p != q ||
        p.type != _INT_ || p.val < 1 || p.val > 64)
      return false;
    gen a, b, c, d;
    bool reflected = false;
    if (is_linear_wrt(left, x, a, b, contextptr) && is_linear_wrt(right, x, c, d, contextptr) &&
        integration_rational(a) && integration_rational(b) && integration_rational(c) &&
        integration_rational(d) && integration_rational(lo) && integration_rational(hi) &&
        !is_zero(a) && c == -a && d == a * (lo + hi) + b) {
      ...
      if (left.is_symb_of_sommet(at_cos))
        swapgen(left, right);
      if (!left.is_symb_of_sommet(at_sin) || !right.is_symb_of_sommet(at_cos) ||
          left._SYMBptr->feuille != right._SYMBptr->feuille ||
          !is_linear_wrt(left._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
          is_zero(a) || !integration_period_real_bound(b, contextptr) ||
          !integration_period_real_bound(lo, contextptr) ||
          !integration_period_real_bound(hi, contextptr))
        return false;
      gen l = ratnormal(2 * (a * lo + b) / cst_pi, contextptr),
          r = ratnormal(2 * (a * hi + b) / cst_pi, contextptr);
    ...
    if (!reflected)
      return false;
    res = ratnormal((hi - lo) / 2, contextptr);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_cauchy_fourier(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                       gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || taille(e, 65) > 64)
      return false;
    ...
    if (factors.size() > 2)
      return false;
    gen frequency = 0, phase, denominator;
    bool have_den = false, have_cos = false;
    for (unsigned i = 0; i < factors.size(); ++i) {
      gen base;
      if (factors[i].is_symb_of_sommet(at_cos)) {
        if (have_cos ||
            !is_linear_wrt(factors[i]._SYMBptr->feuille, x, frequency, phase, contextptr) ||
            !is_zero(phase) || contains(frequency, x) || !is_zero(im(frequency, contextptr)))
          return false;
        have_cos = true;
      } else if (!have_den && integration_power(factors[i], base, -1)) {
        denominator = base;
        have_den = true;
      } else
        return false;
    }
    if (!have_den || !denominator.is_symb_of_sommet(at_plus) ||
        denominator._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & terms = *denominator._SYMBptr->feuille._VECTptr;
    if (terms.size() != 2)
      return false;
    gen A = 0, C = 0;
    for (unsigned i = 0; i < terms.size(); ++i) {
      if (!contains(terms[i], x)) {
        C += terms[i];
        continue;
      }
      gen term = terms[i], coefficient = integration_coefficient(term, x, contextptr), base;
      if (!integration_power(term, base, 2) || base != x || contains(coefficient, x))
        return false;
      A += coefficient;
    }
    if (!is_zero(im(A, contextptr)) || !is_zero(im(C, contextptr)) ||
        !is_strictly_positive(A, contextptr) || !is_strictly_positive(C, contextptr))
      return false;
    gen scale = sqrt(C / A, contextptr);
    res = gen(halves) * cst_pi * exp(-scale * abs(frequency, contextptr), contextptr) /
          (2 * A * scale);
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_unit_log_arc(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                     gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !is_zero(lo) || !integration_resource_rational(hi) ||
        !is_strictly_positive(hi, contextptr) || !e.is_symb_of_sommet(at_prod) ||
        e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    if (v.size() < 2 || v.size() > 4)
      return false;
    gen arg, weight = 1, den, rad;
    bool arc = false, logarithm = false, have_den = false, have_rad = false;
    for (unsigned i = 0; i < v.size(); ++i) {
      gen base;
      if (v[i].is_symb_of_sommet(at_atan) || v[i].is_symb_of_sommet(at_ln)) {
        if (arc || logarithm)
          return false;
        arc = v[i].is_symb_of_sommet(at_atan);
        logarithm = !arc;
        arg = v[i]._SYMBptr->feuille;
      } else if (integration_power(v[i], base, -1) && base.is_symb_of_sommet(at_plus)) {
        if (have_den)
        ...
        have_den = true;
                 ...
                 (v[i].is_symb_of_sommet(at_pow) && v[i]._SYMBptr->feuille.type == _VECT &&
                  v[i]._SYMBptr->feuille._VECTptr->size() == 2 &&
                  v[i]._SYMBptr->feuille[1] == -gen(1) / 2 &&
                  (rad = v[i]._SYMBptr->feuille[0], true))) {
        if (have_rad)
          return false;
        have_rad = true;
      } else
    ...
    gen c, q, w, n;
    if ((!arc && !logarithm) || !integration_monomial(arg, x, c, q, contextptr) ||
        !integration_monomial(weight, x, w, n, contextptr) || !integration_resource_rational(c) ||
        !integration_resource_rational(q) || !integration_resource_rational(w) || is_zero(c) ||
        !is_strictly_positive(q, contextptr) || is_strictly_greater(q, 32, contextptr))
      return false;
    if (q.type == _FRAC && (q._FRACptr->den.type != _INT_ || q._FRACptr->den.val > 16))
      return false;
    if (arc && n == -1 && !have_den) {
      if (have_rad) {
        gen other, rc, rn;
        if (!integration_one_plus(rad, other) ||
            !integration_monomial(other, x, rc, rn, contextptr) || rn != 2 * q ||
            !is_strictly_positive(-rc, contextptr) ||
            !is_zero(ratnormal(-rc * pow(hi, 2 * q, contextptr) - 1, contextptr)))
          return false;
        gen b = c / sqrt(-rc, contextptr), value = asinh(abs(b, contextptr), contextptr);
        if (is_strictly_positive(-c, contextptr))
          value = -value;
        res = w * cst_pi * value / (2 * q);
        return true;
      }
    ...
    if (logarithm && have_den && !have_rad && n == q - 1 && is_strictly_positive(c, contextptr)) {
      gen other, dc, dn;
      if (!integration_one_plus(den, other) ||
          !integration_monomial(other, x, dc, dn, contextptr) || dn != 2 * q || dc != c * c ||
          !is_zero(ratnormal(c * pow(hi, q, contextptr) - 1, contextptr)))
        return false;
      gen G = (gen(symbolic(at_Psi, makesequence(gen(1) / 4, 1))) -
               gen(symbolic(at_Psi, makesequence(gen(3) / 4, 1)))) /
              16;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_positive_cosine_kernel(const gen & e, const gen & x, const gen & lo,
                                               const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || !integration_period_real_bound(lo, contextptr) ||
        !integration_period_real_bound(hi, contextptr) || taille(e, 129) > 128)
      return false;
    gen kernel = e, harmonic;
    bool have_harmonic = false;
    if (e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT) {
      const vecteur & v = *e._SYMBptr->feuille._VECTptr;
      if (v.size() != 2)
        return false;
      unsigned j = v[0].is_symb_of_sommet(at_cos) ? 0 : 1;
      if (!v[j].is_symb_of_sommet(at_cos))
        return false;
      harmonic = v[j]._SYMBptr->feuille;
      have_harmonic = true;
      kernel = v[1 - j];
    }
    gen den;
    bool logarithm = kernel.is_symb_of_sommet(at_ln);
    if (logarithm)
      den = kernel._SYMBptr->feuille;
    else if (!integration_power(kernel, den, -1))
      return false;
    if (!den.is_symb_of_sommet(at_plus) || den._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *den._SYMBptr->feuille._VECTptr;
    ...
    for (unsigned i = 0; i < v.size(); ++i) {
      ...
      if (found || !is_zero(im(c, contextptr)) || !t.is_symb_of_sommet(at_cos))
        return false;
      B = c;
      u = t._SYMBptr->feuille;
      found = true;
    }
    if (!found)
      return false;
    ...
    if (have_harmonic) {
      gen c, d;
      if (!is_linear_wrt(harmonic, x, c, d, contextptr))
        return false;
      n = ratnormal(c / a, contextptr);
      if (taille(n, 17) > 16 || !is_assumed_integer(n, contextptr) ||
          !is_zero(ratnormal(d - n * b, contextptr)))
        return false;
      n = abs(n, contextptr);
    }
    gen l = ratnormal((a * lo + b) / cst_pi, contextptr),
        r = ratnormal((a * hi + b) / cst_pi, contextptr),
        periods = ratnormal((r - l) / 2, contextptr);
    bool whole = periods.type == _INT_ && periods.val >= -64 && periods.val <= 64;
    bool half = l.type == _INT_ && r.type == _INT_ && l.val >= -128 && l.val <= 128 &&
                r.val >= -128 && r.val <= 128;
    if (!whole && !half)
      return false;
    gen q = ratnormal(-B / 2, contextptr), root, mean;
    bool poisson = is_zero(ratnormal(A - 1 - q * q, contextptr)) &&
                   is_strictly_positive(1 - q, contextptr) &&
                   is_strictly_positive(1 + q, contextptr);
    if (poisson) {
      root = 1 - q * q;
      mean = 0;
      ...
      if (!is_strictly_positive(A - abs(B, contextptr), contextptr))
        return false;
      root = sqrt(A * A - B * B, contextptr);
      q = -B / (A + root);
      if (logarithm && is_zero(n))
        mean = ln((A + root) / 2, contextptr);
    }
    if (logarithm && !is_zero(n) && !is_strictly_positive(n, contextptr))
      return false;
    if (logarithm)
      res = is_zero(n) ? (hi - lo) * mean : -(hi - lo) * pow(q, n, contextptr) / n;
    else
      res = (hi - lo) * (is_zero(n) ? gen(1) : pow(q, n, contextptr)) / root;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_dilog_definite(const gen & e, const gen & x, const gen & lo, const gen & hi,
                                       gen & res, GIAC_CONTEXT) {
    if (!integration_resource_rational(lo) || !integration_resource_rational(hi) ||
        is_strictly_positive(-lo, contextptr) || is_strictly_positive(-hi, contextptr))
      return false;
    ...
    if (is_zero(lo) && is_strictly_positive(hi, contextptr) && taille(e, 97) <= 96) {
      vecteur terms = e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT
                          ? *e._SYMBptr->feuille._VECTptr
                          : vecteur(1, e);
      gen argument, weight = 1;
      unsigned logs = 0;
      if (terms.size() <= 4) {
        for (unsigned j = 0; j < terms.size(); ++j) {
          if (terms[j].is_symb_of_sommet(at_ln)) {
            argument = terms[j]._SYMBptr->feuille;
            ++logs;
          } else
            weight = weight * terms[j];
        }
        gen other, d, q, w, n;
        if (logs == 1 && integration_one_plus(argument, other) &&
            integration_monomial(other, x, d, q, contextptr) && integration_resource_rational(d) &&
            !is_zero(d) && integration_resource_rational(q) &&
            is_strictly_positive(q, contextptr) && !is_strictly_greater(q, 32, contextptr) &&
            integration_monomial(weight, x, w, n, contextptr) && integration_resource_rational(w) &&
            !is_zero(w) && integration_resource_rational(n) &&
            !is_strictly_positive(q + n + 1, contextptr)) {
          gen endpoint = 1 + d * pow(hi, q, contextptr);
          if (is_zero(endpoint) || is_strictly_positive(endpoint, contextptr)) {
            res = is_strictly_positive(w * d, contextptr) ? plus_inf : minus_inf;
            return true;
          }
        }
      }
    ...
    if (!is_zero(im(scale, contextptr)))
      return false;
    gen c, q;
    if (integration_monomial(u, x, c, q, contextptr) && !is_strictly_positive(q, contextptr))
      return false;
    gen l = subst(u, x, lo, false, contextptr).eval(1, contextptr),
        r = subst(u, x, hi, false, contextptr).eval(1, contextptr);
    if (!is_zero(im(l, contextptr)) || !is_zero(im(r, contextptr)) ||
        (!is_zero(1 - l) && !is_strictly_positive(1 - l, contextptr)) ||
        (!is_zero(1 - r) && !is_strictly_positive(1 - r, contextptr)))
      return false;
    if (extra && (is_one(l) || is_one(r)))
      return false;
    res = scale * (_Li2(l, contextptr) - _Li2(r, contextptr));
    if (extra) {
      gen L = ln(1 - l, contextptr), R = ln(1 - r, contextptr);
      res += scale * (L * L - R * R) / 2;
    }
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_compact_definite(const gen & e, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (integrate_dilog_definite(e, x, lo, hi, res, contextptr) ||
        ...
        integrate_oscillatory_cancellation(e, x, lo, hi, res, contextptr))
      return true;
    ...
    if (integrate_gaussian_erf_exp(e, x, lo, hi, res, contextptr) ||
        ...
        integrate_atan_rectangle_pair(e, x, lo, hi, res, contextptr))
      return true;

    if (integrate_reciprocal_trig_period(e, x, lo, hi, res, contextptr))
      return true;
    if (integrate_exp_difference(e, x, lo, hi, res, contextptr))
      return true;
    if (integrate_mellin_two_binomials(e, x, lo, hi, res, contextptr))
      return true;
    if (integrate_gaussian_erf(e, x, lo, hi, res, contextptr))
      return true;
    if (integrate_arc_rational_circle(e, x, lo, hi, res, contextptr) ||
        ...
        integrate_loglog_mellin(e, x, lo, hi, res, contextptr))
      return true;
    if (integrate_log_zeta(e, x, lo, hi, res, contextptr) ||
        integrate_hyperbolic_log(e, x, lo, hi, res, contextptr) ||
        integrate_atan_log_moment(e, x, lo, hi, res, contextptr) ||
        integrate_atan_square(e, x, lo, hi, res, contextptr) ||
        integrate_mellin_log(e, x, lo, hi, res, contextptr) ||
        integrate_beta_log(e, x, lo, hi, res, contextptr) ||
        integrate_laplace_difference(e, x, lo, hi, res, contextptr) ||
        integrate_inverse_gaussian(e, x, lo, hi, res, contextptr) ||
        integrate_decay_transform(e, x, lo, hi, res, contextptr) ||
        integrate_log_trig(e, x, lo, hi, res, contextptr) ||
        integrate_thermal_moment(e, x, lo, hi, res, contextptr))
      return true;
    gen primitive;
    if (!is_inf(lo) && !is_inf(hi) && integrate_binomial_chain(e, x, primitive, true, contextptr)) {
      res = subst(primitive, x, hi, false, contextptr) - subst(primitive, x, lo, false, contextptr);
      return true;
    }
    if (lo == minus_inf && hi == plus_inf && e.is_symb_of_sommet(at_inv) &&
        integrate_logistic_moment(makevecteur(1, e), x, res, contextptr))
      return true;
    if (!e.is_symb_of_sommet(at_prod) || e._SYMBptr->feuille.type != _VECT)
      return false;
    const vecteur & v = *e._SYMBptr->feuille._VECTptr;
    return integrate_sine_dirichlet(v, x, lo, hi, res, contextptr) ||
           (lo == minus_inf && hi == plus_inf &&
            integrate_logistic_moment(v, x, res, contextptr)) ||
  ...
  static bool integrate_real_root(const gen & e, const gen & x, gen & res, int mode, GIAC_CONTEXT) {
    if (complex_mode(contextptr) || complex_variables(contextptr) || taille(e, 129) > 128 ||
        (!has_op(e, *at_surd) && !has_op(e, *at_NTHROOT)))
      return false;
    ...
    if (index.type != _INT_ || index.val < -9 || index.val > 9)
      return false;
    ...
    if (roots.size() == 1 && index.val > 0) {
      gen base = integration_syntax(e, contextptr),
          coefficient =
              integration_syntax(integration_coefficient(base, x, contextptr), contextptr),
          den, A, B;
      if (integration_resource_rational(coefficient) && integration_power(base, den, -1) &&
        ...
        for (int j = 0; j < n - 1; ++j)
          total += pow(q, j) * pow(u, n - 1 - j) / (n - 1 - j);
        if (!is_zero(B))
          total += pow(q, n - 1) * symbolic(at_ln, symbolic(at_abs, A * u + B));
        res = coefficient * gen(n) * total / (a * A);
        return true;
      }
    }
    ...
    for (unsigned i = 0; i < powers.size(); ++i) {
      ...
      if (p.type != _VECT || p._VECTptr->size() != 2 || p[1].type != _INT_ || p[1].val < -16 ||
          p[1].val > 16)
        return false;
    }
    gen t(identificateur(" khicas_root"));
    if (contains(lidnt(e), t) || eval(t, 1, contextptr) != t)
      return false;
    ...
    if (!is_zero(remainder) || is_undef(primitive) || has_op(primitive, *at_integrate))
      return false;
  ...
  static bool integrate_residue_kernel(const gen & e, const gen & x, gen & res, GIAC_CONTEXT) {
    gen den, r;
    if (!integration_power(e, den, -1) || !integration_one_plus(den, r) ||
        !integration_power(r, r, -2))
      return false;
    vecteur terms;
    if (r.is_symb_of_sommet(at_plus) && r._SYMBptr->feuille.type == _VECT)
      terms = *r._SYMBptr->feuille._VECTptr;
    else
      terms.push_back(r);
    if (terms.empty() || terms.size() > 8)
      return false;
    gen total = 0;
    int sign = 0;
    for (unsigned i = 0; i < terms.size(); ++i) {
  ...
  bool integration_rational_tail(const gen & g, const gen & x, int & degree, gen & leading,
                                 unsigned & budget, unsigned depth, GIAC_CONTEXT) {
    if (!budget || depth > 8)
      return false;
    --budget;
    if (integration_resource_rational(g)) {
      if (is_zero(g))
        return false;
      degree = 0;
      leading = g;
    ...
    if (g.is_symb_of_sommet(at_neg) || g.is_symb_of_sommet(at_inv)) {
      if (!integration_rational_tail(f, x, degree, leading, budget, depth + 1, contextptr))
        return false;
      if (g.is_symb_of_sommet(at_neg))
        leading = -leading;
      else {
        degree = -degree;
        leading = inv(leading, contextptr);
      }
    ...
    if (g.is_symb_of_sommet(at_pow)) {
      if (v.size() != 2 || v[1].type != _INT_ || v[1].val < -64 || v[1].val > 64 ||
          !integration_rational_tail(v[0], x, degree, leading, budget, depth + 1, contextptr))
        return false;
      int n = v[1].val;
      if (n && (degree > 128 / (n < 0 ? -n : n) || degree < -128 / (n < 0 ? -n : n)))
        return false;
      degree *= n;
      leading = pow(leading, n, contextptr);
      return integration_resource_rational(leading);
    }
    bool product = g.is_symb_of_sommet(at_prod), sum = g.is_symb_of_sommet(at_plus);
    if ((!product && !sum) || v.empty() || v.size() > 16)
      return false;
    degree = product ? 0 : -129;
    leading = product ? 1 : 0;
    for (unsigned i = 0; i < v.size(); ++i) {
      if (is_zero(v[i])) {
        if (product)
      ...
      int d;
      gen c;
      if (!integration_rational_tail(v[i], x, d, c, budget, depth + 1, contextptr))
        return false;
      if (product) {
        degree += d;
        leading = leading * c;
        ...
        leading += c;
      if (degree > 128 || degree < -128 || !integration_resource_rational(leading))
        return false;
    }
  ...
  // Keep these bounded dispatch paths compact in the calculator ROM.
  static bool integration_finite_poly_add(sparse_poly1 & out, const gen & c, int degree,
                                          unsigned & budget) {
    if (!budget || degree > 64)
      return false;
    --budget;
  ...
  static bool integration_finite_poly_product(const sparse_poly1 & left, const sparse_poly1 & right,
                                              sparse_poly1 & out, unsigned & budget) {
    if (left.size() * right.size() > budget)
      return false;
    for (unsigned i = 0; i < left.size(); ++i)
      for (unsigned j = 0; j < right.size(); ++j) {
        int degree = left[i].exponent.val + right[j].exponent.val;
        if (degree > 64 ||
            !integration_finite_poly_add(out, left[i].coeff * right[j].coeff, degree, budget))
          return false;
      }
    return true;
  }

  static bool integration_finite_poly_terms(const gen & g, const gen & x, sparse_poly1 & out,
                                            unsigned & budget, unsigned depth, GIAC_CONTEXT) {
    if (!budget || depth > 8)
      return false;
    --budget;
    if (integration_rational(g))
      return integration_finite_poly_add(out, g, 0, budget);
    if (g == x)
      return integration_finite_poly_add(out, 1, 1, budget);
    if (g.is_symb_of_sommet(at_inv) && integration_rational(g._SYMBptr->feuille) &&
        !is_zero(g._SYMBptr->feuille))
    ...
    if (g.type == _FRAC && integration_rational(g._FRACptr->den) && !is_zero(g._FRACptr->den)) {
      sparse_poly1 p;
      if (!integration_finite_poly_terms(g._FRACptr->num, x, p, budget, depth + 1, contextptr))
        return false;
      for (unsigned i = 0; i < p.size(); ++i)
        if (!integration_finite_poly_add(out, p[i].coeff / g._FRACptr->den, p[i].exponent.val,
                                         budget))
          return false;
      return true;
    }
    if (g.is_symb_of_sommet(at_neg)) {
      sparse_poly1 p;
      if (!integration_finite_poly_terms(g._SYMBptr->feuille, x, p, budget, depth + 1, contextptr))
        return false;
      for (unsigned i = 0; i < p.size(); ++i)
        if (!integration_finite_poly_add(out, -p[i].coeff, p[i].exponent.val, budget))
          return false;
      return true;
    }
    if (g.type != _SYMB || g._SYMBptr->feuille.type != _VECT)
    ...
    if (g.is_symb_of_sommet(at_plus)) {
      if (v.size() > 32)
        return false;
      for (unsigned i = 0; i < v.size(); ++i)
        if (!integration_finite_poly_terms(v[i], x, out, budget, depth + 1, contextptr))
          return false;
      return true;
    }
    bool power = g.is_symb_of_sommet(at_pow);
    if (power && (v.size() != 2 || v[1].type != _INT_ || v[1].val < 0 || v[1].val > 64))
      return false;
    if (!power && (!g.is_symb_of_sommet(at_prod) || v.size() > 8))
      return false;
    sparse_poly1 result(1, monome(1, 0)), base;
    if (power) {
      if (!integration_finite_poly_terms(v[0], x, base, budget, depth + 1, contextptr))
        return false;
      unsigned n = v[1].val;
      while (n) {
        if (n & 1) {
          sparse_poly1 next;
          if (!integration_finite_poly_product(result, base, next, budget))
            return false;
          result.swap(next);
        }
        n >>= 1;
        if (n) {
          sparse_poly1 next;
          if (!integration_finite_poly_product(base, base, next, budget))
            return false;
          base.swap(next);
        }
      }
      ...
      for (unsigned i = 0; i < v.size(); ++i) {
        base.clear();
        if (!integration_finite_poly_terms(v[i], x, base, budget, depth + 1, contextptr))
          return false;
        sparse_poly1 next;
        if (!integration_finite_poly_product(result, base, next, budget))
          return false;
    ...
    for (unsigned i = 0; i < result.size(); ++i)
      if (!integration_finite_poly_add(out, result[i].coeff, result[i].exponent.val, budget))
        return false;
    return true;
  }
...
// Direct endpoint powers therefore replace generic assumptions and limits.
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_finite_polynomial(const gen & e, const gen & x, const gen & lo,
                                          const gen & hi, gen & res, GIAC_CONTEXT) {
    if (!integration_rational(lo) || !integration_rational(hi) || taille(e, 129) > 128)
      return false;
    sparse_poly1 polynomial;
    unsigned budget = 512;
    if (!integration_finite_poly_terms(e, x, polynomial, budget, 0, contextptr))
      return false;
    gen answer = 0;
    for (unsigned i = 0; i < polynomial.size(); ++i) {
      if (is_zero(polynomial[i].coeff))
        continue;
      int degree = polynomial[i].exponent.val + 1;
      answer += polynomial[i].coeff * (pow(hi, degree, contextptr) - pow(lo, degree, contextptr)) /
                gen(degree);
    }
    res = answer;
    return true;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integration_polynomial_radical(const gen & C, const gen & A, const gen & x,
                                             bool inverse, gen & res, GIAC_CONTEXT) {
    unsigned budget = 128;
    equation_polynomial_budget cb, ab;
    if (!equation_polynomial_bound(C, budget, 0, cb) || cb.degree > 8 ||
        ...
        cb.denominator_bits + 10 * (ab.numerator_bits + ab.denominator_bits) > 2048)
      return false;
    vecteur variables = lvar(C);
    if (variables.size() && (variables.size() != 1 || variables[0] != x))
      return false;
    gen a, b;
    if (!is_linear_wrt(A, x, a, b, contextptr) || !integration_rational(a) ||
        !integration_rational(b))
      return false;
    sparse_poly1 polynomial;
    budget = 256;
    if (!integration_finite_poly_terms(C, x, polynomial, budget, 0, contextptr))
      return false;
    if (is_zero(a)) {
      if (!is_strictly_positive(b, contextptr))
        return false;
      ...
      for (unsigned j = 0; j < polynomial.size(); ++j) {
        int n = polynomial[j].exponent.val + 1;
        P += polynomial[j].coeff * pow(x, n) / gen(n);
      }
      gen root = sqrt(b, contextptr);
      res = inverse ? P / root : P * root;
      return true;
    }
    ...
    res = sqrt(A, contextptr) * (inverse ? P : A * P) / a;
    return true;
  }
  ...
  static bool integrate_affine_radical(const gen & input, const gen & x, gen & res, GIAC_CONTEXT) {
    if (taille(input, 129) > 128 || complex_mode(contextptr) || complex_variables(contextptr))
      return false;
    gen f = integration_syntax(input, contextptr), C = 1, root, A;
    int mode = 0;
    vecteur factors = f.is_symb_of_sommet(at_prod) ? *f._SYMBptr->feuille._VECTptr : makevecteur(f);
    for (unsigned j = 0; j < factors.size(); ++j) {
      const gen & v = factors[j];
      gen rad;
      int candidate = 0;
      if (integration_square_root(v, rad))
        candidate = 1;
      else if (v.is_symb_of_sommet(at_inv)) {
        if (integration_square_root(v._SYMBptr->feuille, rad))
          candidate = 2;
        else if (v._SYMBptr->feuille.is_symb_of_sommet(at_plus) &&
                 v._SYMBptr->feuille._SYMBptr->feuille.type == _VECT &&
                 v._SYMBptr->feuille._SYMBptr->feuille._VECTptr->size() == 2)
          candidate = 3;
      }
      if (candidate) {
        if (mode)
        ...
        unsigned budget = 128;
        equation_polynomial_budget bound;
        if (!equation_polynomial_bound(v, budget, 0, bound) || bound.degree > 9)
          return false;
        C = C * v;
      }
    }
    if (!mode || is_zero(C))
      return false;
    if (mode < 3)
      return integration_polynomial_radical(C, A, x, mode == 2, res, contextptr);
    const vecteur & terms = *root._SYMBptr->feuille._SYMBptr->feuille._VECTptr;
    gen U = terms[0], V = terms[1], c = equation_numeric_factor(U), d = equation_numeric_factor(V),
        B;
    if (!integration_rational(c) || !integration_rational(d) ||
        !is_strictly_positive(c * d, contextptr) || !integration_square_root(U, A) ||
        !integration_square_root(V, B))
      return false;
    gen aa, ab, ba, bb;
    unsigned budget = 128;
    equation_polynomial_budget bound;
    if (!equation_polynomial_bound(A, budget, 0, bound) || bound.degree > 1 ||
        !equation_polynomial_bound(B, budget, 0, bound) || bound.degree > 1 ||
        !is_linear_wrt(A, x, aa, ab, contextptr) || !is_linear_wrt(B, x, ba, bb, contextptr) ||
        !integration_rational(aa) || !integration_rational(ab) || !integration_rational(ba) ||
        !integration_rational(bb))
      return false;
    // Verify a nonempty real interior before any conjugate reduction.
    if ((is_zero(aa) && !is_strictly_positive(ab, contextptr)) ||
        (is_zero(ba) && !is_strictly_positive(bb, contextptr)))
      return false;
    if (is_strictly_positive(-aa * ba, contextptr)) {
      ...
      if (!is_strictly_positive(upper - lower, contextptr))
        return false;
    }
    ...
    if (is_zero(aa * bb - ba * ab) && !(is_zero(aa) && is_strictly_positive(ab, contextptr)) &&
        !(is_zero(ba) && is_strictly_positive(bb, contextptr)))
      return false;
    gen delta = ratnormal(c * c * A - d * d * B, contextptr);
    if (is_zero(delta))
      return false;
    budget = 128;
    if (!equation_polynomial_bound(C, budget, 0, bound) || bound.degree > 9)
      return false;
    ...
    if (!integration_polynomial_radical(quotient, A, x, false, left, contextptr) ||
        !integration_polynomial_radical(quotient, B, x, false, right, contextptr))
      return false;
    res = c * left - d * right;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_quadratic_affine_root(const gen & input, const gen & x, const gen & lo,
                                              const gen & hi, gen & res, GIAC_CONTEXT) {
    ...
    const gen & f = input;
    if (!f.is_symb_of_sommet(at_prod) || f._SYMBptr->feuille.type != _VECT ||
        f._SYMBptr->feuille._VECTptr->size() != 2 || complex_mode(contextptr) ||
        complex_variables(contextptr) || !angle_radian(contextptr) || taille(input, 65) > 64)
      return false;
    const vecteur & v = *f._SYMBptr->feuille._VECTptr;
    gen L, Q, d;
    bool matched = false;
    for (unsigned j = 0; j < 2; ++j) {
      if (integration_power(v[j], d, -1) && integration_square_root(d, L) &&
          integration_power(v[1 - j], Q, -1)) {
        matched = true;
        break;
      }
    }
    ...
    if (!equation_polynomial_bound(L, budget, 0, bound) || bound.degree > 1 || bound.terms > 4 ||
        !equation_polynomial_bound(Q, budget, 0, bound) || bound.degree > 2 || bound.terms > 8)
      return false;
    gen k, b, A, B, C;
    if (!is_linear_wrt(L, x, k, b, contextptr) || !integration_resource_rational(k) || is_zero(k) ||
        !integration_resource_rational(b) || !is_quadratic_wrt(Q, x, A, B, C, contextptr) ||
        !integration_resource_rational(A) || !integration_resource_rational(B) ||
        !integration_resource_rational(C) || !is_strictly_positive(A, contextptr) ||
        !is_strictly_positive(4 * A * C - B * B, contextptr))
      return false;
    bool definite = !is_undef(lo);
    gen endpoints[2];
    if (definite) {
      for (unsigned j = 0; j < 2; ++j) {
        const gen & z = j ? hi : lo;
        if (is_inf(z)) {
          if (z != (is_strictly_positive(k, contextptr) ? plus_inf : minus_inf))
            return false;
          endpoints[j] = plus_inf;
        } else {
          if (!integration_resource_rational(z))
            return false;
          gen rad = k * z + b;
          if (!is_zero(rad) && !is_strictly_positive(rad, contextptr))
            return false;
    ...
    gen root = sqrt(q, contextptr), w = sqrt(2 * root - p, contextptr),
        h = sqrt(2 * root + p, contextptr);
    gen values[2];
    unsigned count = definite ? 2 : 1;
    for (unsigned j = 0; j < count; ++j) {
      gen u = definite ? endpoints[j] : sqrt(L, contextptr);
      if (u == plus_inf) {
        values[j] = cst_pi / (2 * root * h);
        continue;
      }
      if (is_zero(u)) {
        values[j] = 0;
      ...
      values[j] = symbolic(at_ln, (base + w * u) / (base - w * u)) / (4 * root * w) +
                  (symbolic(at_atan, (2 * u + w) / h) + symbolic(at_atan, (2 * u - w) / h)) /
                      (2 * root * h);
    }
    res = 2 * k / A * (definite ? values[1] - values[0] : values[0]);
    return true;
  }
  ...
  static bool integration_finite_elementary_bound(const gen & g, GIAC_CONTEXT) {
    if (taille(g, 33) > 32)
      return false;
    if (g.is_symb_of_sommet(at_exp) && integration_rational(g._SYMBptr->feuille))
      return true;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_finite_elementary(const gen & e, const gen & x, const gen & lo,
                                          const gen & hi, gen & res, GIAC_CONTEXT) {
    if (taille(e, 97) > 96 || !integration_finite_elementary_bound(lo, contextptr) ||
        !integration_finite_elementary_bound(hi, contextptr))
      return false;
    ...
    if (integration_square_root(f, arg) && integration_power(arg, square, 2)) {
      gen u = square;
      if (u.is_symb_of_sommet(at_sin) || u.is_symb_of_sommet(at_cos))
        u = gen(u._SYMBptr->feuille);
      if (is_linear_wrt(u, x, a, b, contextptr) && integration_rational(a) &&
          integration_finite_elementary_bound(b, contextptr))
        f = symbolic(at_abs, square);
    }
    if (f.is_symb_of_sommet(at_abs)) {
      const gen & u = f._SYMBptr->feuille;
      if ((u.is_symb_of_sommet(at_sin) || u.is_symb_of_sommet(at_cos)) &&
          angle_radian(contextptr) && is_linear_wrt(u._SYMBptr->feuille, x, a, b, contextptr) &&
          integration_rational(a) && !is_zero(a) &&
          integration_finite_elementary_bound(b, contextptr)) {
        gen periods = ratnormal(a * (hi - lo) / cst_pi, contextptr);
        if (periods.type == _INT_) {
          res = ratnormal(2 * c * (hi - lo) / cst_pi, contextptr);
          return true;
        }
      }
    }
    bool sine = f.is_symb_of_sommet(at_sin), cosine = f.is_symb_of_sommet(at_cos),
         exponential = f.is_symb_of_sommet(at_exp), logarithm = f.is_symb_of_sommet(at_ln),
         arctangent = f.is_symb_of_sommet(at_atan), absolute = f.is_symb_of_sommet(at_abs);
    if (sine || cosine || exponential || logarithm || arctangent || absolute) {
      if ((sine || cosine || arctangent) && !angle_radian(contextptr))
        return false;
      if (!is_linear_wrt(f._SYMBptr->feuille, x, a, b, contextptr) || !integration_rational(a) ||
          is_zero(a) || !integration_finite_elementary_bound(b, contextptr))
        return false;
      lower = ratnormal(a * lo + b, contextptr);
      upper = ratnormal(a * hi + b, contextptr);
      if (sine)
        res = c * (cos(lower, contextptr) - cos(upper, contextptr)) / a;
      else if (cosine)
        res = c * (sin(upper, contextptr) - sin(lower, contextptr)) / a;
      else if (exponential)
        res = c * (exp(upper, contextptr) - exp(lower, contextptr)) / a;
      else if (absolute)
        res = c * (upper * abs(upper, contextptr) - lower * abs(lower, contextptr)) / (2 * a);
      else if (arctangent)
        res = c *
              (upper * atan(upper, contextptr) - lower * atan(lower, contextptr) -
               ln((1 + upper * upper) / (1 + lower * lower), contextptr) / 2) /
              a;
      else {
        if ((!is_zero(lower) && !is_strictly_positive(lower, contextptr)) ||
            (!is_zero(upper) && !is_strictly_positive(upper, contextptr)))
          return false;
    ...
    if (integration_power(f, arg, -1) && integration_quadratic(arg, x, a, b, contextptr) &&
        is_strictly_positive(a, contextptr) && is_strictly_positive(b, contextptr)) {
      if (!angle_radian(contextptr))
        return false;
      gen scale = sqrt(a / b, contextptr);
      res = c * (atan(scale * hi, contextptr) - atan(scale * lo, contextptr)) /
            sqrt(a * b, contextptr);
      return true;
    }
    ...
    arg = f;
    gen p = 1;
    bool wrapped = false, fractional = false;
    for (unsigned depth = 0; depth < 8; ++depth) {
      if (arg.is_symb_of_sommet(at_inv)) {
        p = -p;
        arg = gen(arg._SYMBptr->feuille);
        wrapped = true;
        continue;
      }
      if (arg.is_symb_of_sommet(at_sqrt)) {
        p = p / 2;
        arg = gen(arg._SYMBptr->feuille);
        wrapped = true;
        fractional = true;
      ...
      if (arg.is_symb_of_sommet(at_pow) && arg._SYMBptr->feuille.type == _VECT) {
        ...
        if (n.type != _INT_ || n.val < -64 || n.val > 64 ||
            (v[1].type == _FRAC &&
             (v[1]._FRACptr->den.type != _INT_ || v[1]._FRACptr->den.val > 16)))
          return false;
        if (v[1].type != _INT_)
          fractional = true;
        p = p * v[1];
    ...
    if (!wrapped || !integration_rational(p) || !is_linear_wrt(arg, x, a, b, contextptr) ||
        !integration_rational(a) || is_zero(a) || !integration_rational(b))
      return false;
    const gen & numerator = p.type == _FRAC ? p._FRACptr->num : p;
    if (numerator.type != _INT_ || numerator.val < -64 || numerator.val > 64 ||
        (p.type == _FRAC && (p._FRACptr->den.type != _INT_ || p._FRACptr->den.val > 16)))
      return false;
    lower = ratnormal(a * lo + b, contextptr);
    upper = ratnormal(a * hi + b, contextptr);
    bool lp = is_strictly_positive(lower, contextptr), up = is_strictly_positive(upper, contextptr);
    if (fractional && ((!lp && !is_zero(lower)) || (!up && !is_zero(upper))))
      return false;
    if (!is_strictly_positive(p + 1, contextptr)) {
      bool negative =
          is_strictly_positive(-lower, contextptr) && is_strictly_positive(-upper, contextptr);
      if (!(lp && up) && (fractional || !negative))
        return false;
    }
    if (p == -1)
      res = c * ln(abs(upper / lower, contextptr), contextptr) / a;
    else
      res = c * (pow(upper, p + 1, contextptr) - pow(lower, p + 1, contextptr)) / (a * (p + 1));
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_parameter_kernel(const gen & input, const gen & x, const gen & lo,
                                         const gen & hi, gen & res, GIAC_CONTEXT) {
    if (taille(input, 97) > 96)
      return false;
    gen e = input, coefficient = integration_coefficient(e, x, contextptr);
    if (contains(coefficient, x) || taille(coefficient, 17) > 16 || is_zero(coefficient))
      return false;
    if (is_zero(lo) && hi == plus_inf && e.is_symb_of_sommet(at_plus) &&
        e._SYMBptr->feuille.type == _VECT) {
      const vecteur & terms = *e._SYMBptr->feuille._VECTptr;
      if (terms.size() > 4)
        return false;
      gen sum = 0;
      bool complex_rate = false;
      for (unsigned i = 0; i < terms.size(); ++i) {
        gen term = terms[i], c = integration_coefficient(term, x, contextptr), a, b;
        if (!term.is_symb_of_sommet(at_exp) ||
            !is_linear_wrt(term._SYMBptr->feuille, x, a, b, contextptr) || !is_zero(b) ||
            contains(a, x) || contains(c, x) || taille(a, 17) > 16 || taille(c, 17) > 16)
          return false;
        gen realrate = re(-a, contextptr);
        if (contains(realrate, *at_re) || !is_strictly_positive(realrate, contextptr))
          return false;
        complex_rate = complex_rate || !is_zero(im(a, contextptr));
        sum -= c / a;
      }
      if (!complex_rate)
        return false;
      res = ratnormal(coefficient * sum, contextptr);
      return true;
    }
    vecteur factors = e.is_symb_of_sommet(at_prod) && e._SYMBptr->feuille.type == _VECT
                          ? *e._SYMBptr->feuille._VECTptr
                          : vecteur(1, e);
    if (factors.size() > 2)
      return false;
    ...
    vecteur required;
    bool power = false, exponential = false, inverse = false, trig = false, sine = false;
    gen frequency;
    if (is_zero(lo) && hi == plus_inf) {
      for (unsigned i = 0; i < factors.size(); ++i) {
        gen base;
        const gen & f = factors[i];
        if (f.is_symb_of_sommet(at_exp)) {
          gen a, b;
          if (exponential || !is_linear_wrt(f._SYMBptr->feuille, x, a, b, contextptr) ||
              !is_zero(b) || contains(a, x))
            return false;
          exponential = true;
          rate = -a;
        } else if (f.is_symb_of_sommet(at_sin) || f.is_symb_of_sommet(at_cos)) {
          gen phase;
          if (trig || !angle_radian(contextptr) ||
              !is_linear_wrt(f._SYMBptr->feuille, x, frequency, phase, contextptr) ||
              !is_zero(phase) || contains(frequency, x) || !is_zero(im(frequency, contextptr)))
            return false;
          trig = true;
          sine = f.is_symb_of_sommet(at_sin);
        } else if (integration_power(f, base, -1) && base.is_symb_of_sommet(at_plus)) {
          if (inverse)
            return false;
          inverse = true;
          den = base;
        } else if (f.is_symb_of_sommet(at_pow) && f._SYMBptr->feuille.type == _VECT &&
                   f._SYMBptr->feuille._VECTptr->size() == 2 && f._SYMBptr->feuille[0] == x) {
          if (power || contains(f._SYMBptr->feuille[1], x))
            return false;
          power = true;
          shape = ratnormal(f._SYMBptr->feuille[1] + 1, contextptr);
        } else if (f == x) {
          if (power)
            return false;
          power = true;
      ...
      if (exponential && !inverse) {
        ...
        if (trig) {
          ...
          answer = inv(rate, contextptr);
        } else {
          required.push_back(re(shape, contextptr));
          answer = symbolic(at_Gamma, shape) /
                   (is_one(rate) ? gen(1) : gen(symbolic(at_pow, makesequence(rate, shape))));
        ...
        gen other, c, q;
        if (!integration_one_plus(den, other) ||
            !integration_monomial(other, x, c, q, contextptr) || !is_one(c) ||
            !integration_resource_rational(q) || !is_strictly_positive(q, contextptr) ||
            is_strictly_greater(q, 16, contextptr))
          return false;
        if (q.type == _FRAC && (q._FRACptr->den.type != _INT_ || q._FRACptr->den.val > 16))
          return false;
        required.push_back(re(shape, contextptr));
        required.push_back(q - re(shape, contextptr));
        answer = cst_pi / (q * sin(cst_pi * shape / q, contextptr));
      } else
        return false;
    } else {
      if (factors.size() != 2 || !integration_resource_rational(lo) ||
          !integration_resource_rational(hi) || !is_strictly_positive(hi - lo, contextptr))
        return false;
      gen alpha = 0, beta = 0, left = 0, right = 0;
      for (unsigned i = 0; i < 2; ++i) {
        const gen & f = factors[i];
        gen base = f, exponent = 1;
        if (f.is_symb_of_sommet(at_pow) && f._SYMBptr->feuille.type == _VECT &&
            f._SYMBptr->feuille._VECTptr->size() == 2) {
          base = f._SYMBptr->feuille[0];
          exponent = f._SYMBptr->feuille[1];
        }
        gen a, b;
        if (contains(exponent, x) || taille(exponent, 17) > 16 ||
            !is_linear_wrt(base, x, a, b, contextptr) || !integration_resource_rational(a) ||
            !integration_resource_rational(b))
          return false;
        if (is_strictly_positive(a, contextptr) && is_zero(a * lo + b) && is_zero(left)) {
          left = a * (hi - lo);
          alpha = ratnormal(exponent + 1, contextptr);
        } else if (is_strictly_positive(-a, contextptr) && is_zero(a * hi + b) && is_zero(right)) {
          right = -a * (hi - lo);
          beta = ratnormal(exponent + 1, contextptr);
        } else
          return false;
      }
      if (is_zero(left) || is_zero(right) ||
          (integration_rational(alpha) && integration_rational(beta)))
        return false;
      required.push_back(re(alpha, contextptr));
      required.push_back(re(beta, contextptr));
      gen leftscale = is_one(left) ? gen(1) : gen(symbolic(at_pow, makesequence(left, alpha - 1)));
      gen rightscale =
          is_one(right) ? gen(1) : gen(symbolic(at_pow, makesequence(right, beta - 1)));
      answer = (hi - lo) * leftscale * rightscale * symbolic(at_Gamma, alpha) *
               symbolic(at_Gamma, beta) / symbolic(at_Gamma, alpha + beta);
    }
    for (unsigned i = 0; i < required.size(); ++i) {
      const gen & v = required[i];
      bool unknown_real_part = contains(v, *at_re);
      if (!unknown_real_part && is_strictly_positive(v, contextptr))
        continue;
      if (!unknown_real_part && (is_zero(v) || is_strictly_positive(-v, contextptr))) {
        if (is_undef(fallback) || i > 0) {
          *logptr(contextptr) << "Divergent improper integral (parameter convergence boundary)"
                              << '\n';
          res = undef;
          return true;
        }
      ...
      gen next = symb_superieur_strict(v, 0);
      condition = is_one(condition) ? next : symb_and(condition, next);
    }
    answer = coefficient * answer;
    res = is_one(condition) ? answer
                            : gen(symbolic(at_when, makesequence(condition, answer, fallback)));
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_real_definite(const gen & input, const gen & x, gen lo, gen hi, gen & res,
                                      GIAC_CONTEXT) {
    if (taille(input, 129) > 128)
      return false;
    ...
    if (integrate_parameter_kernel(e, x, lo, hi, res, contextptr)) {
      if (reverse)
        res = -res;
      return true;
    }
    ...
    if (!integration_rational(c) || is_zero(c))
      return false;
    if (integrate_quadratic_affine_root(e, x, lo, hi, res, contextptr)) {
      res = (reverse ? -c : c) * res;
      return true;
    }
    if (integrate_affine_trig_square_interval(e, x, lo, hi, res, contextptr)) {
      res = (reverse ? -c : c) * res;
      return true;
    }
    ...
    if ((is_inf(lo) || is_inf(hi)) && angle_radian(contextptr) &&
      ...
      gen a, b;
      if (is_linear_wrt(e._SYMBptr->feuille, x, a, b, contextptr) &&
          integration_resource_rational(a) && !is_zero(a) &&
          integration_period_real_bound(b, contextptr)) {
      ...
      if (factors[ei].is_symb_of_sommet(at_exp)) {
        ...
        if (integration_rational(q)) {
          ...
          if (full && square && is_strictly_positive(-q, contextptr) &&
              ...
              h.is_symb_of_sommet(at_exp) &&
              is_linear_wrt(h._SYMBptr->feuille, x, a, b, contextptr) && integration_rational(a) &&
              is_zero(b)) {
            res = (reverse ? -c : c) * sqrt(cst_pi / (-q), contextptr) / 2;
            return true;
          }
          if (integration_power(factors[1 - ei], base, -2) && base == x && is_zero(lo) &&
              hi == plus_inf) {
            if (integration_power(exponent, base, -2) && base == x &&
                is_strictly_positive(-q, contextptr)) {
              res = (reverse ? -c : c) * sqrt(cst_pi / (-q), contextptr) / 2;
              return true;
            }
            if (square) {
              res = (reverse ? -c : c) * plus_inf;
              return true;
            }
          }
        }
    ...
    if (factors.size() == 2 && is_one(lo) &&
        (hi == plus_inf || (integration_rational(hi) && is_strictly_greater(hi, 1, contextptr)))) {
      for (unsigned i = 0; i < 2; ++i) {
        gen base;
        if (!integration_power(factors[i], base, -1) || !base.is_symb_of_sommet(at_ln) ||
            base._SYMBptr->feuille != x)
          continue;
        if (integration_power(factors[1 - i], base, -2) && base == x) {
          res = (reverse ? -c : c) * plus_inf;
          return true;
        }
      }
    }
    if (!angle_radian(contextptr) || is_inf(lo) || is_inf(hi))
      return false;
    ...
    if (factors.size() == 2) {
      ...
      if (factors[li].is_symb_of_sommet(at_ln) && integration_power(factors[1 - li], den, -1) &&
          ...
          is_linear_wrt(u, x, a, b, contextptr) && integration_rational(a) &&
          integration_rational(b) && !is_zero(a)) {
        gen l = ratnormal(a * lo + b, contextptr), r = ratnormal(a * hi + b, contextptr);
        if ((is_zero(l) && is_one(r)) || (is_one(l) && is_zero(r))) {
          gen ka, kb;
          if (!is_linear_wrt(factors[li]._SYMBptr->feuille, x, ka, kb, contextptr) ||
              !integration_rational(ka) || !integration_rational(kb))
            return false;
          gen k = ka / a;
          if (kb == k * (1 + b) && is_strictly_positive(k, contextptr)) {
            res = (reverse ? -c : c) * (r - l) / a * cst_pi *
                  (ln(k, contextptr) / 4 + ln(gen(2), contextptr) / 8);
            return true;
          }
        }
      }
  ...
  gen integrate_id_rem(const gen & e_orig, const gen & gen_x, gen & remains_to_integrate,
    ...
    *logptr(contextptr) << gettext("integrate id_rem ") << e_orig << '\n';
    ...
    if (e.is_symb_of_sommet(at_ln) && !angle_mode(contextptr)) {
      ...
      if (trig.is_symb_of_sommet(at_abs))
        trig = gen(trig._SYMBptr->feuille);
    ...
    if (!lpiece.empty())
      lpiece = lvarx(lpiece, gen_x);
    if (!lpiece.empty()) {
      *logptr(contextptr) << gettext("Warning: piecewise indefinite integration does not return a "
                                     "continuous antiderivative")
                          << '\n';
    ...
    *logptr(contextptr) << gettext("integrate step 0 ") << e << '\n';
    ...
    if (((intmode & 4) == 0) && u == at_pow && f[0].is_symb_of_sommet(at_pow)) {
      e = symbolic(at_pow,
                   makesequence(f[0]._SYMBptr->feuille[0], f[0]._SYMBptr->feuille[1] * f[1]));
      return integrate_id_rem(e, gen_x, remains_to_integrate, contextptr, intmode);
    }
    if ((u == at_pow) && is_constant_wrt(f._VECTptr->back(), gen_x, contextptr) &&
        is_linear_wrt(f._VECTptr->front(), gen_x, a, b, contextptr)) {
      if ((intmode & 2) == 0)
        gprintf(step_linear,
                gettext("Integrate %gen, a linear expression u=%gen to a constant power "
                        "n=%gen,\nIf n=-1 then ln(u)/a else u^(n+1)/((n+1)*%gen)"),
                makevecteur(e, a * gen_x + b, f._VECTptr->back(), a), contextptr);
      b = f._VECTptr->back();
      if (is_minus_one(b))
    ...
    if ((u == at_surd) && is_constant_wrt(f._VECTptr->back(), gen_x, contextptr) &&
        is_linear_wrt(f._VECTptr->front(), gen_x, a, b, contextptr)) {
      if ((intmode & 2) == 0)
        gprintf(step_linear,
                gettext("Integrate %gen, a linear expression u=%gen to a constant power "
                        "n=1/%gen,\nIf n=-1 then ln(u)/a else u^(n+1)/((n+1)*%gen)"),
                makevecteur(e, a * gen_x + b, f._VECTptr->front(), a), contextptr);
      b = f._VECTptr->back();
      if (is_minus_one(b))
        return rdiv(lnabs(f._VECTptr->front(), contextptr), a, contextptr);
      return f._VECTptr->front() * symbolic(at_surd, f) / (a + a / b);
    }
    if ((u == at_NTHROOT) && is_constant_wrt(f._VECTptr->front(), gen_x, contextptr) &&
        is_linear_wrt(f._VECTptr->back(), gen_x, a, b, contextptr)) {
      if ((intmode & 2) == 0)
        gprintf(step_linear,
                gettext("Integrate %gen, a linear expression u=%gen to a constant power "
                        "n=1/%gen,\nIf n=-1 then ln(u)/a else u^(n+1)/((n+1)*%gen)"),
    ...
    *logptr(contextptr) << gettext("integrate step 1 ") << e << '\n';
    ...
    if (u == at_sum && f.type == _VECT && f._VECTptr->size() == 4) {
      ...
      if (!is_zero(remains_to_integrate))
        remains_to_integrate =
            _sum(makesequence(remains_to_integrate, fv[1], fv[2], fv[3]), contextptr);
      return res;
    }
    // unary op only
    ...
    if (s && is_linear_wrt(f, gen_x, a, b, contextptr)) {
      if ((intmode & 2) == 0)
        gprintf(
            step_funclinear,
            gettext(
                "Integrate %gen: function %gen applied to a linear expression u=%gen, result %gen"),
            makevecteur(e, primitive_tab_op[s - 1], a * gen_x + b,
                        primitive_tab_primitive[s - 1](a * gen_x + b, contextptr) / a),
            contextptr);
    ...
    if (!allsame && curgcd == 1) { // translate if exists b!=0 with f(x+b)
      for (int i = 1; i < v.size(); ++i) {
        ...
        if (is_linear_wrt(vf, gen_x, a, b, contextptr) && (a == 1 || a == -1)) {
          ...
          remains_to_integrate =
              complex_subst(remains_to_integrate, gen_x, a * gen_x + b, contextptr);
    ...
    if (!allsame && curgcd != 0 && curgcd != 1) {
      ...
      if (v != vrep)
        e1 = complex_subst(e1, v, vrep, contextptr);
      gen E1 = integrate_id_rem(e1, gen_x, remains_to_integrate, contextptr, intmode);
    ...
    if (rvarsize <= TRY_FU_UPRIME) { // otherwise no hope
      ...
      for (int pos = 1; pos <= v.size(); ++it, ++pos) {
        ...
        if (u.is_symb_of_sommet(at_atan)
          ...
          if (is_linear_wrt(argatan, gen_x, a, b, contextptr)) {
            ...
            gen ck = subst(fu, makevecteur(u, gen_x, sqrt(pow(a * gen_x + b, 2) + 1, contextptr)),
                           makevecteur(gen_x,
                                       (symbolic(at_sin, gen_x) / symbolic(at_cos, gen_x) - b) / a,
                                       symb_inv(symb_cos(gen_x))),
                           false, contextptr);
        ...
        if (tst && (issin || iscos)) {
          ...
          if (is_rewritable_as_f_of(fx1, pow(gen_x, 2), fx2, gen_x,
                                    contextptr)) { // attempt with cos^2 or sin^2
            fx2 = recursive_ratnormal(fx2, contextptr);
            gen fx21 =
                recursive_ratnormal(subst(-fx2, gen_x, 1 - gen_x, false, contextptr), contextptr);
            int t1 = taille(fx2, 256), t2 = taille(fx21, 256);
            if (t2 < 0.7 * t1) {
              if (issin)
                u = symb_cos(u._SYMBptr->feuille);
              else
                u = symb_sin(u._SYMBptr->feuille);
              fx2 = fx21;
            }
            u = pow(u, 2);
            fx = fx2;
        ...
        if (tst) {
          if ((intmode & 2) == 0)
            gprintf(step_fuuprime,
                    gettext("Integration of %gen: f(u)*u' where f=%gen->%gen and u=%gen"),
                    makevecteur(e, gen_x, fx, u), contextptr);
#if 0
	  ...
	  return e;
          ...
          remains_to_integrate =
              remains_to_integrate + complex_subst(tmprem, gen_x, u, contextptr) * df;
        ...
        if (pos < v.size() && u.is_symb_of_sommet(at_pow)) { // no check with u=gen_x^2
          ...
          if (tst) {
            if ((intmode & 2) == 0)
              gprintf(step_fuuprime,
                      gettext("Integration of %gen: f(u)*u' where f=%gen->%gen and u=%gen"),
                      makevecteur(e, gen_x, fx, uit), contextptr);
            e = linear_integrate_nostep(fx, gen_x, tmprem, intmode, contextptr);
            remains_to_integrate =
                remains_to_integrate + complex_subst(tmprem, gen_x, *it, contextptr) * df;
        ...
        // a simpler integral
        gen A, B, C;
        if (rvarsize == 2 && is_quadratic_wrt(inv(df, contextptr), gen_x, A, B, C, contextptr) &&
          ...
          if (is_linear_wrt(C, gen_x, a, b, contextptr)) { // always true
            ...
            e = subst(e, makevecteur(gen_x, f),
                      makevecteur((B * gen_x - b) / (-A * gen_x + a), gen_x), false, contextptr) *
                (-A * b + B * a) / pow(gen_x * A - a, 2);
            e = linear_integrate_nostep(e, gen_x, tmprem, intmode, contextptr);
            remains_to_integrate =
                remains_to_integrate + complex_subst(tmprem, gen_x, f, contextptr);
            return complex_subst(e, gen_x, f, contextptr);
          }
        }
        fu = recursive_ratnormal(
            rdiv(e, df, contextptr),
            contextptr); // changed from ratnormal, made 30/06/2021 for int((4/x^5)*cos((1/x^4)-6) );
        if (is_rewritable_as_f_of(fu, f, fx, gen_x, contextptr) && !is_undef(fx)) {
          if ((intmode & 2) == 0)
            gprintf(step_fuuprime,
                    gettext("Integration of %gen: f(u)*u' where f=%gen->%gen and u=%gen"),
                    makevecteur(e, gen_x, fx, f), contextptr);
          e = linear_integrate_nostep(fx, gen_x, tmprem, intmode, contextptr);
          remains_to_integrate =
              remains_to_integrate + complex_subst(tmprem, gen_x, f, contextptr) * df;
          return complex_subst(e, gen_x, f, contextptr);
        }
      }
    }
    ...
    *logptr(contextptr) << gettext("integrate step 2 ") << e << '\n';
    ...
    if (e._SYMBptr->sommet == at_pow) {
      ...
      fu = pow(e._SYMBptr->feuille._VECTptr->front(),
               e._SYMBptr->feuille._VECTptr->back() - plus_one, contextptr);
      est_puissance = true;
    } else {
    ...
    for (int i = 0; (i < TRY_FU_UPRIME) && (vt != vtend); ++vt, ++i) {
          ...
          if (vtexpo.type == _INT_ && vtexpo.val == 4) {
            ...
            if (evenodd == 1) {
              ...
              if (is_even_odd(sroot, gen_x, contextptr)) {
                ...
                tmp = -linear_integrate_nostep(tmp * pow(gen_x, -2, contextptr), gen_x, tmprem,
                                               intmode | 2 | 8, contextptr);
      ...
      bool recur =
          rvarsize > 2 && gen_x.type == _IDNT && strcmp(gen_x._IDNTptr->id_name, "t_nostep") == 0 &&
          u.is_symb_of_sommet(
              at_pow); // workaround for some stupid integrals like integrate(sin((d*x + c)^(2/3)*b + a)/(f*x + E),x);
      if (!recur && (is_rewritable_as_f_of(fu, u, fx, gen_x, contextptr) ||
                     is_rewritable_as_f_of(simplifier(fu, contextptr), simplifier(u, contextptr),
                                           fx, gen_x, contextptr))) {
        fx = cst * fx;
        if ((intmode & 2) == 0)
          gprintf(step_fuuprime,
...
#else
      remains_to_integrate = remains_to_integrate + complex_subst(tmprem, gen_x, u, contextptr) *
                                                        derive(u, gen_x, contextptr);
      return complex_subst(e1, gen_x, u, contextptr);
#endif
      }
      if (vt->is_symb_of_sommet(at_pow)) {
        ...
        if (vtexpo.type == _INT_ && vtexpo.val % 2) {
          ...
          u = linear_integrate_nostep(pow(vtbase, vtexpo.val / 2, contextptr), gen_x, tmprem,
                                      intmode | 2, contextptr);
          if (is_undef(u) || !is_zero(tmprem))
            continue;
          if (!est_puissance) {
            vecteur vv(v);
          ...
          if (is_rewritable_as_f_of(fu, u, fx, gen_x, contextptr)) {
            ...
            remains_to_integrate =
                remains_to_integrate +
                complex_subst(tmprem, gen_x, u, contextptr) * derive(u, gen_x, contextptr);
    ...
    *logptr(contextptr) << gettext("integrate step 3 ") << e << '\n';
    ...
    if (has_op(rvar, *at_LambertW)) {
      ...
      if (vw.size() == 1 && is_linear_wrt(vw[0]._SYMBptr->feuille, gen_x, a, b, contextptr)) {
        ...
        gen tmpe = complex_subst(e, substin, substout, contextptr) * (gen_x + 1) *
                   symbolic(at_exp, gen_x) / a,
            tmprem;
        gen tmpres = linear_integrate_nostep(tmpe, gen_x, tmprem, intmode, contextptr);
        substout[1] = symbolic(at_exp, gen_x);
        substin[1] = (a * gen_x + b) / vw[0];
    ...
    // square roots
    if ((rvarsize == 2) && (rvar.back().type == _SYMB) &&
        (rvar.back()._SYMBptr->sommet == at_pow)) {
      ...
      if (integrate_sqrt(e, gen_x, rvar, res, remains_to_integrate, intmode, contextptr) == 2) {
        ...
        if ((intmode & 1) == 0 && is_zero(res)) {
          ...
          gen e2 =
              normal(-complex_subst(e, gen_x, inv(gen_x, contextptr), contextptr) / gen_x / gen_x,
                     contextptr);
          gen remains_to_integrate2,
              res2 = integrate_id_rem(e2, gen_x, remains_to_integrate2, contextptr, 1);
          if (!is_zero(res2)) {
            res = complex_subst(res2, gen_x, inv(gen_x, contextptr), contextptr);
            remains_to_integrate =
                -complex_subst(remains_to_integrate2, gen_x, inv(gen_x, contextptr), contextptr) /
                gen_x / gen_x;
            return res;
          }
          remains_to_integrate = e;
    ...
    // integration by part?
    if ((e._SYMBptr->sommet == at_prod) && (e._SYMBptr->feuille.type == _VECT)) {
      const_iterateur ibp = e._SYMBptr->feuille._VECTptr->begin(),
                      ibpend = e._SYMBptr->feuille._VECTptr->end();
      for (int j = 0; ibp != ibpend; ++ibp, ++j) {
        ...
        if ((ibp->_SYMBptr->sommet == at_pow) &&
          ...
          test = equalposcomp(inverse_tab_op,
                              ibp->_SYMBptr->feuille._VECTptr->front()._SYMBptr->sommet);
        else
        ...
        if (is_zero(tmp)) {
          ...
          for (; tmpi < tmpv.size(); ++tmpi) {
            if (tmpv[tmpi].type == _SYMB &&
                equalposcomp(inverse_tab_op, tmpv[tmpi]._SYMBptr->sommet))
              break;
          }
          if (tmpi == tmpv.size()) {
            if ((intmode & 2) == 0)
              gprintf(step_bypart, gettext("Integration of %gen: by part, u*v'=%gen*(%gen)'"),
                      makevecteur(e, *ibp, tmpprimitive), contextptr);
      ...
      int test;
      if ((e._SYMBptr->sommet == at_pow) && (e._SYMBptr->feuille._VECTptr->front().type == _SYMB) &&
          (e._SYMBptr->feuille._VECTptr->back().type == _INT_) &&
          (e._SYMBptr->feuille._VECTptr->back().val > 0))
      ...
      if (test) {
        if ((intmode & 2) == 0)
          gprintf(step_bypart1,
                  gettext("Integration of %gen by part of u*v' where u=1 and v=%gen'"),
                  makevecteur(e, e), contextptr);
        gen tmpres, tmprem;
    ...
    if (!is_elementary(rvar, gen_x) &&
        detect_inv_trigln(e, rvar, gen_x, res, remains_to_integrate, false, intmode, contextptr))
      return res;

    // rewrite inv(exp)
    vector<const unary_function_ptr *> vsubstin(1, at_inv);
    vector<gen_op_context> vsubstout(1, invexptoexpneg);
    e = subst(e, vsubstin, vsubstout, true,
              contextptr); // changed to true for numint of programs (otherwise e becomes undef)
    ...
    if (!has_nop_var(v)) {
      ...
      if (!has_nop_var(v)) {
        ...
        vecteur vx = lvarx(v, gen_x);
        if (vx.empty() || vx == vecteur(1, gen_x))
          return integrate_linearizable(etmp, gen_x, remains_to_integrate, intmode, true,
                                        contextptr);
      }
    }
      ...
      if (trig_fraction) { // trig of linear?
        ...
        if (is_zero(coeff_trig))
          ...
          if (quotient.type == _INT_)
            continue;
          if ((quotient.type == _FRAC) && (quotient._FRACptr->num.type == _INT_) &&
              (quotient._FRACptr->den.type == _INT_)) {
            coeff_trig =
                ratnormal(rdiv(coeff_trig, quotient._FRACptr->den, contextptr), contextptr);
            continue;
          }
          if ((quotient.type == _SYMB) && (quotient._SYMBptr->sommet == at_inv) &&
              (quotient._SYMBptr->feuille.type == _INT_)) {
            coeff_trig =
                ratnormal(rdiv(coeff_trig, quotient._SYMBptr->feuille, contextptr), contextptr);
            continue;
          }
          trig_fraction = false;
        }
    ...
    if (trig_fraction) {
      ...
      if (has_i(e))
        do_lnabs(false, contextptr);
      res = integrate_trig_fraction(e, gen_x, var, coeff_trig, trig_fraction, remains_to_integrate,
                                    intmode, contextptr);
      do_lnabs(b, contextptr);
      return res;
    }
    ...
    if ((intmode & 2) == 0)
      gprintf(step_risch, gettext("Integrate %gen, no heuristic found, running Risch algorithm"),
              makevecteur(e), contextptr);
  ...
  gen linear_integrate(const gen & e, const gen & x, gen & remains_to_integrate, int intmode,
                       GIAC_CONTEXT) {
  ...
  static gen integrate0_(const gen & e, const identificateur & x, gen & remains_to_integrate,
                         GIAC_CONTEXT) {
    if (step_infolevel(contextptr))
      gprintf(step_integrate_header,
              gettext("===== Step/step primitive of %gen with respect to %gen ====="),
              makevecteur(e, x), contextptr);
    if (e.type == _VECT) {
      vecteur w;
      vecteur::const_iterator it = e._VECTptr->begin(), itend = e._VECTptr->end();
      for (; it != itend; ++it)
    ...
    if (step_infolevel(contextptr) && is_zero(remains_to_integrate))
      gprintf(gettext("Hence primitive of %gen with respect to %gen is %gen"),
              makevecteur(e, x, res), contextptr);
    return res;
  }

  static gen integrate0(const gen & e, const identificateur & x, gen & remains_to_integrate,
                        GIAC_CONTEXT) {
  ...
  bool adjust_int_sum_arg(vecteur & v, int & s) {
    ...
    if ((s == 2) && (v[1].type == _SYMB) &&
        (v[1]._SYMBptr->sommet == at_equal || v[1]._SYMBptr->sommet == at_equal2 ||
         v[1]._SYMBptr->sommet == at_same)) {
  ...
  gen ck_int_numerically(const gen & f, const gen & x, const gen & a, const gen & b,
                         const gen & exactvalue, GIAC_CONTEXT) {
    ...
    *logptr(contextptr)
        << gettext("Error while checking exact value with approximate value, returning both!")
        << '\n';
  ...
  void ggb_num_coeff(gen & g) {
    if (g.type != _FRAC || g._FRACptr->den.type != _ZINT)
      return;
    mpz_t t;
    mpz_init_set(t, *g._FRACptr->den._ZINTptr);
...
gen protect_integrate(const gen & args, GIAC_CONTEXT) {
    ...
    for (int i = 0; i < v.size(); ++i) {
      ...
      if (expo != 0) {
        ...
        if (is_assumed_integer(expo, contextptr))
          continue;
        if (expo.type == _FRAC && expo._FRACptr->den.type == _INT_ &&
            (expo._FRACptr->den.val % 2 == 1))
          continue;
          ...
          if (!lid.empty()) {
            ...
            if (addi || varval == var) {
              if (var.type == _IDNT && is_linear_wrt(base, var, a, b, contextptr) && !is_zero(a)) {
                ...
                if (as == 0) {
                  av = lidnt(a);
                  if (av.size() == 1 && !equalposcomp(res, av[0]) &&
                      is_linear_wrt(a, av[0], aa, ab, contextptr)) {
                ...
                if (as) {
                  av = lidnt(b);
                  if (av.size() == 1 && !equalposcomp(res, av[0]) &&
                      is_linear_wrt(b, av[0], aa, ab, contextptr)) {
                    as = fastsign(aa, contextptr);
                    if (as == 1)
                      hyp = symb_superieur_strict(av[0], -ab / aa);
                    else if (as == -1)
                ...
                if (var.type == _IDNT && is_quadratic_wrt(base, var, a, b, c, contextptr) &&
                    !is_zero(a)) {
                  int as = fastsign(a, contextptr);
                  gen avar, aa, ab;
                  vecteur av;
                ...
                if (varbase.size() == 1 && lidnt(base).size() == 1) {
                  ...
                  if (var0.type == _SYMB) {
                    ...
                    if (varf.type != _VECT &&
                        is_linear_wrt(
                            varf, var, a, b,
                            contextptr)) { // f(a*x+b), if f is trig assume in a*x+b in a period
                      int as = fastsign(a, contextptr);
                      if (as) {
                        ...
                        if (addhyp != 0) {
                          if (as == 1)
                            addhyp = symb_and(symb_superieur_strict(var, (-addhyp - b) / a),
                                              symb_inferieur_strict(var, (addhyp - b) / a));
                          else if (as == -1)
                            addhyp = symb_and(symb_inferieur_strict(var, (-addhyp - b) / a),
                                              symb_superieur_strict(var, (addhyp - b) / a));
                        }
                        dosolve = true;
                      }
                    }
            ...
            if (hyp.type == _VECT) {
              ...
              for (int j = hypv.size() - 1; j >= 0; --j) {
                ...
                if (u != at_and && u != at_ou && u != at_superieur_strict &&
                    u != at_superieur_egal && u != at_inferieur_strict && u != at_inferieur_egal)
                  continue;
  ...
  gen curviligne(const vecteur & w, const vecteur & v, const gen & curve, const gen & V,
                 const gen & tmin_, const gen & tmax_, GIAC_CONTEXT) {
    if (curve.type == _VECT) {
      gen S = 0;
    ...
    if (!is_undef(V))
      return subst(V, v, subst(vt, t, tmax, false, contextptr), false, contextptr) -
             subst(V, v, subst(vt, t, tmin, false, contextptr), false, contextptr);
  ...
  gen curviligne(const vecteur & w, const vecteur & v, const gen & curve, const gen & tmin,
                 const gen & tmax, GIAC_CONTEXT) {
    gen V;
  ...
  static bool integration_affine_atom(const gen & g, const gen & atom, gen & a, gen & b,
    ...
    const gen & f = g._SYMBptr->feuille;
    if (g.is_symb_of_sommet(at_neg)) {
      if (!integration_affine_atom(f, atom, a, b, budget, depth + 1))
        return false;
      a = -a;
      b = -b;
      return true;
    ...
    if (g.is_symb_of_sommet(at_division) && v.size() == 2 && !contains(v[1], atom)) {
      if (!integration_affine_atom(v[0], atom, a, b, budget, depth + 1))
        return false;
      a = a / v[1];
      b = b / v[1];
      return true;
    ...
    for (unsigned j = 0; j < v.size(); ++j) {
      gen c, d;
      if (!integration_affine_atom(v[j], atom, c, d, budget, depth + 1))
        return false;
      if (product) {
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integration_log_endpoint(const gen & g, const gen & x, const gen & point,
                                       int direction, gen & result, GIAC_CONTEXT) {
    if (complex_mode(contextptr) || complex_variables(contextptr) || !integration_rational(point) ||
        (direction != 1 && direction != -1) || taille(g, 129) > 128 || has_i(g) ||
        !has_op(g, *at_ln))
      return false;
    vecteur logs = lop(g, at_ln);
    if (logs.empty() || logs.size() > 4)
      return false;
    vecteur variables = lvar(g), powers = lop(g, at_pow);
    for (unsigned j = 0; j < variables.size(); ++j)
      if (variables[j] != x && !equalposcomp(logs, variables[j]))
        return false;
    for (unsigned j = 0; j < powers.size(); ++j) {
      const gen & f = powers[j]._SYMBptr->feuille;
      if (f.type != _VECT || f._VECTptr->size() != 2 || f[1].type != _INT_ || f[1].val < -16 ||
          f[1].val > 16)
        return false;
    }
    gen singular = symbolic(at_ln, gen(direction) * (x - point));
    bool found = false;
    vecteur replacements;
    for (unsigned j = 0; j < logs.size(); ++j) {
      gen u = logs[j]._SYMBptr->feuille;
      bool magnitude = u.is_symb_of_sommet(at_abs);
      if (magnitude)
        u = gen(u._SYMBptr->feuille);
        ...
        const vecteur & f = *u._SYMBptr->feuille._VECTptr;
        gen scale = 1, inner;
        for (unsigned k = 0; k < f.size(); ++k) {
          if (integration_rational(f[k]))
            scale = scale * f[k];
          else if (!magnitude && f[k].is_symb_of_sommet(at_abs)) {
            magnitude = true;
            inner = f[k]._SYMBptr->feuille;
          } else
            return false;
        }
        if (!magnitude || !is_strictly_positive(scale, contextptr))
          return false;
        u = scale * inner;
      }
      gen a, b;
      unsigned budget = 64;
      if (!integration_affine_atom(u, x, a, b, budget, 0) || !integration_rational(a) ||
          !integration_rational(b))
        return false;
      gen value = a * point + b;
      if (is_zero(value)) {
        if (is_zero(a) || (!magnitude && !is_strictly_positive(a * gen(direction), contextptr)))
          return false;
        // All vanishing affine logs on this side share ln|x-point|.
        // Collect coefficients before classifying infinity-infinity.
        replacements.push_back(singular +
                               ln(is_strictly_positive(a, contextptr) ? a : -a, contextptr));
        found = true;
      } else {
    ...
    if (is_undef(result) || is_inf(result))
      return false;
    if (!is_zero(log_coefficient))
      result = is_strictly_positive(log_coefficient, contextptr) ? minus_inf : plus_inf;
    return true;
  }
  ...
  static bool integration_punctured_guard(const gen & g, const gen & x, GIAC_CONTEXT) {
    if (!g.is_symb_of_sommet(at_equal) || g._SYMBptr->feuille.type != _VECT ||
        g._SYMBptr->feuille._VECTptr->size() != 2)
      return false;
    gen D = g._SYMBptr->feuille[0] - g._SYMBptr->feuille[1];
    vecteur atoms;
    unsigned budget = 192;
    if (taille(D, 129) > 128 || !logarithmic_span_entire(D, x, atoms, budget, 0))
      return false;
    for (unsigned order = 0; order <= 4; ++order) {
      if (taille(D, 129) > 128)
        return false;
      gen value = eval(subst(D, x, 0, false, contextptr), 1, contextptr);
      if (equation_rational(value) && !is_zero(value))
        return true;
      if (is_undef(value) || is_inf(value) || order == 4)
        return false;
  ...
  static gen integration_punctured_primitive(const gen & g, const gen & x, GIAC_CONTEXT) {
    gen current = g;
    for (unsigned j = 0; j < 4; ++j) {
      if (!current.is_symb_of_sommet(at_when) || current._SYMBptr->feuille.type != _VECT ||
          current._SYMBptr->feuille._VECTptr->size() != 3)
        break;
      const gen & f = current._SYMBptr->feuille;
      if (f[1] != undef || !integration_punctured_guard(f[0], x, contextptr))
        break;
      current = gen(f[2]);
    }
    return current;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static gen integration_primitive_limit(const gen & g, const gen & x, const gen & point,
                                         int direction, GIAC_CONTEXT) {
    gen current = (!is_inf(point) && !is_undef(point))
                      ? integration_punctured_primitive(g, x, contextptr)
                      : g;
    gen value;
    if (integration_log_endpoint(current, x, point, direction, value, contextptr))
      return value;
    return limit(current, *x._IDNTptr, point, direction, contextptr);
  }

  static bool integrate_affine_abs(const gen & f, const gen & x, gen lo, gen hi, gen & res,
                                   GIAC_CONTEXT) {
    unsigned budget = 3;
    if (!is_constant_wrt(lo, x, contextptr) || !small_polynomial(lo, x, budget))
      return false;
    budget = 3;
    if (!is_constant_wrt(hi, x, contextptr) || !small_polynomial(hi, x, budget) ||
        taille(f, 128) > 127)
      return false;
    ...
    for (unsigned i = 0; i < terms.size(); ++i) {
      ...
      if (!is_linear_wrt(arg, x, a, b, contextptr))
        return false;
      budget = 3;
      if (!small_polynomial(a, x, budget))
        return false;
      budget = 3;
      if (!small_polynomial(b, x, budget))
        return false;
      arguments.push_back(arg);
      if (is_zero(a))
        continue;
      gen r = rdiv(-b, a, contextptr);
      if (!is_strictly_greater(r, lo, contextptr) || !is_strictly_greater(hi, r, contextptr))
        continue;
      for (unsigned j = 1; j < points.size(); ++j) {
        if (r == points[j])
          break;
        if (is_strictly_greater(points[j], r, contextptr)) {
          points.insert(points.begin() + j, r);
          break;
        }
      }
    }
    ...
    for (unsigned i = 1; i < points.size(); ++i) {
      ...
      for (unsigned j = 0; j < arguments.size(); ++j) {
        ...
        replacements.push_back(is_strictly_positive(value, contextptr) ? arguments[j]
                                                                       : -arguments[j]);
      }
    ...
    *logptr(contextptr) << gettext("integrate begin") << '\n';
    ...
    if (v.size() == 1) {
      ...
      if (is_algebraic_program(c, a, b) && a.type != _VECT)
        return symbolic(
            at_program,
            makesequence(a, 0, _integrate(gen(makevecteur(b, a), _SEQ__VECT), contextptr)));
    ...
    if (s == 2) {
      ...
      if (v0.type == _VECT && v0._VECTptr->size() == 2 &&
          (t1 == 2 || v1.is_symb_of_sommet(at_union)))
        return curviligne(*v0._VECTptr, makevecteur(x__IDNT_e, y__IDNT_e), v1, undef, undef,
                          contextptr);
    }
    if (s == 7) {
      for (int i = 0; i < s; ++i)
      ...
      if (v[0].type == _VECT && v[1].type == _VECT && v[0]._VECTptr->size() == 2 &&
          v[1]._VECTptr->size() == 2) {
      ...
      gen xy = makevecteur(x, y);
      gen xyt = makevecteur(xt, yt), V;
      if (is_potential(makevecteur(M, N), *xy._VECTptr, V, contextptr))
        return subst(V, xy, subst(xyt, t, tmax, false, contextptr), false, contextptr) -
               subst(V, xy, subst(xyt, t, tmin, false, contextptr), false, contextptr);
    ...
    if (v.back() != at_assume && (s == 3 || s == 5)) {
      ...
      if (v[0].type == _VECT) {
        ...
        if (v[1].type == _VECT && v[0]._VECTptr->size() == v[1]._VECTptr->size()) {
          ...
          // integrate([M,N],[x,y],G)
          return curviligne(*v[0]._VECTptr, *v[1]._VECTptr, v[2], s == 3 ? undef : v[3],
                            s == 3 ? undef : v[4], contextptr);
        }
      }
    ...
    if (storcl_38 && x.type == _IDNT &&
        storcl_38(x, 0, x._IDNTptr->id_name, undef, false, contextptr, NULL, false)) {
    ...
    gen period_result;
    if (s == 4 && !approx_mode(contextptr) && !has_num_coeff(v[0]) && !has_num_coeff(v[2]) &&
        !has_num_coeff(v[3]) &&
        (integrate_real_definite(v[0], x, eval(v[2], 1, contextptr), eval(v[3], 1, contextptr),
                                 period_result, contextptr) ||
         integrate_trig_periods(v[0], x, eval(v[2], 1, contextptr), eval(v[3], 1, contextptr),
                                period_result, contextptr) ||
         integrate_affine_abs(v[0], x, eval(v[2], 1, contextptr), eval(v[3], 1, contextptr),
                              period_result, contextptr)))
    ...
    if (s >= 4) { // take care of boundaries when evaluating
      if (v.back() == at_assume) {
        ...
        gen a(v[2]), b(v[3]);
        if (evalf_double(a, 1, contextptr).type == _DOUBLE_ &&
            evalf_double(b, 1, contextptr).type == _DOUBLE_) {
          bool neg = false;
          ...
          if (lv.size() == 1 && v[1].type == _IDNT) {
            ...
            for (int i = 0; i < int(lv.size()); ++i) {
              if (is_strictly_greater(lv[i], a, contextptr) &&
                  is_strictly_greater(b, lv[i], contextptr)) {
                v[3] = lv[i];
  ...
  try {
    ...
    if (s == 4) {
#ifndef POCKETCAS
      if ((has_num_coeff(v[0]) || v[2].type == _FLOAT_ || v[2].type == _DOUBLE_ ||
           v[2].type == _REAL || v[3].type == _FLOAT_ || v[3].type == _DOUBLE_ ||
           v[3].type == _REAL)) {
        vecteur ld = makevecteur(unsigned_inf, cst_pi);
        // should first remove mute variables inside embedded sum/int/fsolve
        lidnt(makevecteur(true_lidnt(v[0]), evalf_double(v[2], 1, contextptr),
                          evalf_double(v[3], 1, contextptr)),
              ld, false);
        ld.erase(ld.begin());
        ld.erase(ld.begin());
        if (ld == vecteur(1, v[1]) || ld.empty())
          return intnum(gen(makevecteur(v[0], v[1], v[2], v[3]), _SEQ__VECT), false, contextptr,
                        true);
      }
#endif
      v0orig = v[0];
      ...
      if (!is_inf(borne_inf) && !is_inf(borne_sup) && is_zero(borne_inf + borne_sup) &&
      ...
      if (!lfloor.empty()) {
        ...
        if (lvarx(cond, x).size() > 1 || !is_linear_wrt(cond, x, a, b, contextptr)) {
          *logptr(contextptr)
              << gettext("Floor definite integration: can only handle linear < or > condition")
              << '\n';
        ...
        if (is_inf(borne_inf) || is_inf(borne_sup)) {
          *logptr(contextptr)
              << gettext("Floor definite integration: unable to handle infinite boundaries")
              << '\n';
        } else {
      ...
      if (!lpiece.empty()) {
        ...
        for (int i = 0; i < vs / 2; ++i) {
          ...
          if (is_one(cond)) {
            ...
            res +=
                _integrate(gen(makevecteur(tmp, x, borne_inf, borne_sup), _SEQ__VECT), contextptr);
          ...
          if (cond.is_symb_of_sommet(at_different)) {
            *logptr(contextptr) << gettext("Assuming true condition ") << cond << '\n';
            ...
            res +=
                _integrate(gen(makevecteur(v[0], x, borne_inf, borne_sup), _SEQ__VECT), contextptr);
            return ck_int_numerically(v0orig, x, aorig, borig, (chsign ? -res : res), contextptr);
          }
          if (cond.is_symb_of_sommet(at_superieur_strict) ||
              cond.is_symb_of_sommet(at_superieur_egal)) {
            cond = cond._SYMBptr->feuille[0] - cond._SYMBptr->feuille[1];
            unable = false;
          }
          if (cond.is_symb_of_sommet(at_inferieur_strict) ||
              cond.is_symb_of_sommet(at_inferieur_egal)) {
        ...
        return ck_int_numerically(v0orig, x, aorig, borig, (chsign ? -res : res),
                                  contextptr); // return chsign?-res:res;
      } // end piecewise
      if (intgab(v[0], x, borne_inf, borne_sup, res, contextptr)) {
        ...
        if (calc_mode(contextptr) == 1 || abs_calc_mode(contextptr) == 38) {
          ...
          int sps = int(sp.size());
          for (int i = 0; i < sps; i++) {
            if ((ordonne && is_strictly_greater(sp[i], borne_inf, contextptr) &&
                 is_strictly_greater(borne_sup, sp[i], contextptr)) ||
                (!ordonne && is_strictly_greater(sp[i], borne_sup, contextptr) &&
                 is_strictly_greater(borne_inf, sp[i], contextptr))) {
    ...
    if (s == 4 && !is_inf(borne_sup) && !is_inf(borne_inf)) {
      ...
      if (is_zero(v0ab)) {
        ...
        gen a, b;
        for (; i < l.size(); ++i) {
          if (l[i].type == _SYMB && is_linear_wrt(l[i]._SYMBptr->feuille, x, a, b, contextptr) &&
              !is_zero(a) && !is_zero(b))
            break;
        }
        if (i < l.size()) {
          vecteur vin = makevecteur(x, l[i]),
                  vout = makevecteur(x - b / a, l[i]._SYMBptr->sommet(a * x, contextptr));
    ...
    if (ordonne) {
      gen xval = assumeeval(x, contextptr);
      giac_assume(symb_and(symb_superieur_egal(x, borne_inf), symb_inferieur_egal(x, borne_sup)),
                  contextptr);
      primitive = integrate_chknum(v[0], x, rem, contextptr);
      primitive = eval(primitive, 1, contextptr);
      restorepurge(xval, x, contextptr);
      res = integration_primitive_limit(primitive, x, borne_sup, -1, contextptr) -
            integration_primitive_limit(primitive, x, borne_inf, 1, contextptr);
    } else {
      if ((desordonne = is_greater(borne_inf, borne_sup, contextptr))) {
        gen xval = assumeeval(x, contextptr);
        giac_assume(symb_and(symb_superieur_egal(x, borne_sup), symb_inferieur_egal(x, borne_inf)),
                    contextptr);
        primitive = integrate_chknum(v[0], x, rem, contextptr);
        primitive = eval(primitive, 1, contextptr);
        restorepurge(xval, x, contextptr);
        ...
        primitive = integrate_chknum(v[0], x, rem, contextptr);
        res = integration_primitive_limit(primitive, x, borne_sup, 0, contextptr) -
              integration_primitive_limit(primitive, x, borne_inf, 0, contextptr);
      }
    }
#else
  try {
    ...
    *logptr(contextptr) << "Error trying to find limit of " << primitive << '\n';
    ...
    if (is_undef(res)) {
      if (res.type == _STRNG && abs_calc_mode(contextptr) == 38)
        return res;
      res = subst(primitive, *x._IDNTptr, borne_sup, false, contextptr) -
            subst(primitive, *x._IDNTptr, borne_inf, false, contextptr);
    }
    ...
    if (sp.size() > 1) {
      *logptr(contextptr) << gettext("No checks were made for singular points of antiderivative ") +
                                 primitive.print(contextptr) +
                                 gettext(" for definite integration in [") +
                                 borne_inf.print(contextptr) + "," + borne_sup.print(contextptr) +
                                 "]"
                          << '\n';
      sp.clear();
    } else {
      if ((is_inf(borne_inf) || evalf_double(borne_inf, 1, contextptr).type == _DOUBLE_) &&
        ...
        if (is_greater(borne_sup, borne_inf, contextptr))
          giac_assume(
              symb_and(symb_superieur_egal(x, borne_inf), symb_inferieur_egal(x, borne_sup)),
              contextptr);
        else
          giac_assume(
              symb_and(symb_superieur_egal(x, borne_sup), symb_inferieur_egal(x, borne_inf)),
              contextptr);
        sp = protect_find_singularities(primitive, *x._IDNTptr, 2, contextptr);
        restorepurge(xval, x, contextptr);
        if (!lidnt(evalf_double(sp, 1, contextptr)).empty())
          return gensizeerr("Unable to handle singularities of " + primitive.print(contextptr) +
                            " at " + gen(sp).print(contextptr));
      } else
    ...
    for (int i = 0; i < sps; i++) {
      if (sp[i].type == _DOUBLE_ || sp[i].type == _REAL || has_op(sp[i], *at_rootof)) {
        *logptr(contextptr)
            << gettext("Unable to handle approx. or algebraic extension singular point ") +
      ...
      if ((ordonne && is_strictly_greater(sp[i], borne_inf, contextptr) &&
           is_strictly_greater(borne_sup, sp[i], contextptr)) ||
          (desordonne && is_strictly_greater(sp[i], borne_sup, contextptr) &&
           is_strictly_greater(borne_inf, sp[i], contextptr)))
        res += limit(primitive, *x._IDNTptr, sp[i], -1, contextptr) -
               limit(primitive, *x._IDNTptr, sp[i], 1, contextptr);
    }
    if (!is_zero(rem)) {
      if (is_inf(res))
  ...
  gen _integrate(const gen & args_, GIAC_CONTEXT) {
    ...
    if (complex_variables(contextptr))
      *logptr(contextptr)
          << gettext("Warning, complex variables is set, this can lead to fairly complex answers. "
                     "It is recommended to switch off complex variables in the settings or by "
                     "complex_variables:=0; and declare individual variables to be complex by e.g. "
                     "assume(a,complex).")
          << '\n';
    vecteur ass;
    ...
    if (!ass.empty()) {
      *logptr(contextptr) << "Run purge(" << ass
                          << "); or purge(unquote(assumptions)) to clear auto-assumptions\n";
    ...
    if (args.type == _VECT && args._VECTptr->size() == 4
      ...
      if (x.type == _IDNT && a.type != _DOUBLE_ && b.type != _DOUBLE_) {
        ...
        vecteur lv = lidnt(v0);
        if (lv.size() == 1 && lv[0] == x && has_evalf(a, tmp, 1, contextptr) &&
            has_evalf(b, tmp, 1, contextptr) &&
            tegral(v[0], x, a, b, 1e-6, (1 << 10), resapprox, false, contextptr)) {
  ...
  static string texprintasintegrate(const gen & g, const char * s_orig, GIAC_CONTEXT) {
    ...
    if (l == 4)
      return s + "_{" + gen2tex(v[2], contextptr) + "}^{" + gen2tex(v[3], contextptr) + "}" +
             gen2tex(v.front(), contextptr) + "\\, d" + gen2tex(v[1], contextptr);
    return s;
  }
  static define_unary_function_eval4_quoted(__integrate, &_integrate, _integrate_s, 0,
                                            &texprintasintegrate);
  define_unary_function_ptr5(at_integrate, alias_at_integrate, &__integrate, _QUOTE_ARGUMENTS,
                             true);

  // called by approx_area
  double rombergo(const gen & f0, const gen & x, const gen & a_orig, const gen & b_orig, int n,
                  GIAC_CONTEXT) {
  ...
  double rombergt(const gen & f, const gen & x, const gen & a, const gen & b, int n, GIAC_CONTEXT) {
    ...
    if (h == 0)
      return 0;
    //h est la longueur de la subdivision
    at = subst(f, x, b, false, contextptr).evalf(1, contextptr) +
         subst(f, x, a, false, contextptr).evalf(1, contextptr);
  ...
  static bool tegral_util(const gen & f, const gen & x, const gen & a, const gen & b, gen & i30,
                          gen & i30abs, gen & err, GIAC_CONTEXT) {
    gen h = evalf_double(b - a, 1, contextptr), i14, i6;
    //int s30=15,s14=14,s6=6;
    long_double c30[] = {-0.98799251802048542849, -0.93727339240070590430, -0.84820658341042721620,
                         -0.72441773136017004742, -0.57097217260853884754, -0.39415134707756336990,
                         -0.20119409399743452230, 0.00000000000000000000,  0.20119409399743452230,
                         0.39415134707756336990,  0.57097217260853884754,  0.72441773136017004742,
                         0.84820658341042721620,  0.93727339240070590430,  0.98799251802048542849};
    long_double b30[] = {
        0.15376620998058634177e-1, 0.35183023744054062355e-1, 0.53579610233585967506e-1,
        0.69785338963077157224e-1, 0.83134602908496966777e-1, 0.93080500007781105513e-1,
        0.99215742663555788228e-1, 0.10128912096278063644,    0.99215742663555788228e-1,
        0.93080500007781105514e-1, 0.83134602908496966777e-1, 0.69785338963077157224e-1,
        0.53579610233585967507e-1, 0.35183023744054062355e-1, 0.15376620998058634177e-1};
    long_double b14[] = {
        0.21474028217339757006e-1, 0.14373155100418764102e-1, 0.92599218105237092609e-1,
        0.11827741709315709983e-1, 0.15847003639679458478,    0.38429189419875016111e-2,
        0.19741290152890658991,    0.19741290152890658991,    0.38429189419875016111e-2,
        0.15847003639679458478,    0.11827741709315709983e-1, 0.92599218105237092608e-1,
        0.14373155100418764102e-1, 0.21474028217339757006e-1};
    long_double b6[] = {
        0.10715760948621577132, 0.31130901929813818033e-1, 0.36171148858397041065, 0,
        0.36171148858397041065, 0.31130901929813818033e-1, 0.10715760948621577132};
    vecteur v30(15), v30abs(15);
    for (int i = 0; i < 15; i++) {
      v30[i] = evalf_double(
          eval(subst(f, x, ((a + b) + double(c30[i]) * h) / 2, false, contextptr), 1, contextptr),
          1, contextptr);
      v30abs[i] = _l2norm(v30[i], contextptr);
      if (v30abs[i].type != _DOUBLE_)
        return false;
  ...
  bool tegral(const gen & f, const gen & x, const gen & a_, const gen & b_, const gen & eps,
              int nmax, gen & value, bool exactcheck, GIAC_CONTEXT) {
    ...
    if (exactcheck) {
      ...
      if (0 && vf.size() <= 1) { // dangerous
        ...
        value = _limit(makesequence(F, x, exact(b, contextptr), -1), contextptr) -
                _limit(makesequence(F, x, exact(a, contextptr), 1), contextptr);
    ...
    for (; int(v.size()) < nmax;) {
      ...
      if (A == C || B == C) {
        ...
        if (is_greater(1e-4, ERR / I30ABS, contextptr)) {
          *logptr(contextptr)
              << "Low accuracy, error estimate " << ERR / I30ABS
  ...
  gen romberg(const gen & f0, const gen & x0, const gen & a, const gen & b, const gen & eps,
              int nmax, GIAC_CONTEXT) {
    return evalf_int(f0, x0, a, b, eps, nmax, true, contextptr, false);
  }
  gen evalf_int(const gen & f0, const gen & x0, const gen & a, const gen & b, const gen & eps,
                int nmax, bool romberg_method, GIAC_CONTEXT, bool exactcheck) {
    gen x(x0), f(f0);
    if (x.type != _IDNT) {
    ...
    if (!romberg_method)
      *logptr(contextptr)
          << "Adaptive method failure, will try with Romberg, last approximation was " << value
          << '\n';
    // a, b and eps should be evalf-ed, and eps>0
    gen h = b - a;
    vecteur old_line, cur_line;
#ifdef NO_STDEXCEPT
    old_line.push_back(evalf(
        h * (limit(f, *x._IDNTptr, a, 1, contextptr) + limit(f, *x._IDNTptr, b, -1, contextptr)) /
            2,
        eval_level(contextptr), contextptr));
#else
  try {
    old_line.push_back(evalf(
        h * (limit(f, *x._IDNTptr, a, 1, contextptr) + limit(f, *x._IDNTptr, b, -1, contextptr)) /
    ...
    if (is_inf(old_line[0]) || is_undef(old_line[0]) ||
        !lop(old_line[0], at_bounded_function).empty()) {
      // FIXME middle point in arbitrary precision
      *logptr(contextptr)
          << gettext("Infinity or undefined limit at bounds.\nUsing middle point Romberg method")
          << '\n';
      gen y = (a + b) / 2;
      ...
      for (int i = 0; i < nmax; ++i) {
        ...
        if (i > nmax / 2 &&
            (ck_is_greater(eps, err, contextptr) ||
      ...
      if (calc_mode(contextptr) == 1)
        return undef;
      *logptr(contextptr) << gettext("Unable to find numeric integral using Romberg method, "
                                     "returning the last approximations")
                          << '\n';
      cur_line = is_undef(value) ? makevecteur(old_line.back(), cur_line.back())
                                 : makevecteur(cur_line.back(), value);
      return cur_line;
  ...
  gen intnum(const gen & args, bool romberg_method, GIAC_CONTEXT, bool exactcheck) {
    ...
    if (x.type != _IDNT || (a.type != _DOUBLE_ && a.type != _REAL) ||
...
**********************************************************************/
  ...
  bool rational_sum(const gen & e, const gen & x, gen & res, gen & remains_to_sum, bool allow_psi,
                    GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      ...
      if (jt == jtend)
        ...
        for (int j = 0; j < k; ++j) {
          if (decal > 0)
            res = res - rdiv(r2e(poly12polynome(it_num, 1, vdim), v, contextptr),
                             r2e(poly12polynome(it_den, 1, vdim), v, contextptr), contextptr);
    ...
    for (; jt != jtend; ++jt) {
      ...
      if (jtfact.size() != 2) { // add to remains_to_sum
        remains_to_sum =
            remains_to_sum + rdiv(r2e(poly12polynome(jt->num, 1, vdim), v, contextptr),
                                  r2e(poly12polynome(jt->den, 1, vdim), v, contextptr), contextptr);
      } else {
        ...
        for (int i = 0; i < jt->mult; ++i) {
          if (i >= decal) {
            if (!allow_psi)
              return false;
            res =
                res + r2e(rdiv(dec[i - decal], coeff * jtden.front(), contextptr), vv, contextptr) *
                          Psi(x - r2e(racine, vv, contextptr), i, contextptr);
          }
          coeff = gen(-i - 1) * coeff;
        }
      }
  ...
  void AB2PQR(const polynome & A, const polynome & B, polynome & P, polynome & Q, polynome & R) {
    ...
    std::vector<facteur<tensor<gen>>>::const_iterator itA = vA.begin(), itAend = vA.end(),
                                                      itB = vB.begin(), itBend = vB.end();
    vecteur racines;
    for (; itA != itAend; ++itA) {
      for (; itB != itBend; ++itB) {
  ...
  bool gosper(const polynome & P, const polynome & Q, const polynome & R, polynome & Y, gen & deno,
              GIAC_CONTEXT) {
  ...
  bool is_hypergeometric(const gen & e, const identificateur & x, vecteur & v, polynome & P,
                         polynome & Q, polynome & R, GIAC_CONTEXT) {
    v = lvarx(e, x);
    ...
    for (int i = 0; i < vs; ++i) {
      if (v[i].is_symb_of_sommet(at_exp) &&
          !is_linear_wrt(v[i]._SYMBptr->feuille, x, a, b, contextptr))
        return false;
    }
  ...
  static gen inner_sum(const gen & e, const gen & x, gen & remains_to_sum, int intmode,
                       GIAC_CONTEXT) {
    gen res;
    ...
    remains_to_sum = zero;
    gen facteur = r2e(Y * R, v, contextptr) / r2e(P, v, contextptr) /
                  r2e(deno, vecteur(v.begin() + 1, v.end()), contextptr);
  ...
  gen sum(const gen & e, const gen & x, gen & remains_to_sum, GIAC_CONTEXT) {
    ...
    if (!v.empty()) {
      ...
      return _simplify(
          _evalc(linear_apply(e1, x, remains_to_sum, 0, contextptr, inner_sum), contextptr),
          contextptr);
    } else
...
#endif
    gen tmp1 = ((is_inf(b) && x.type == _IDNT) ? limit(res, *x._IDNTptr, b, 0, contextptr)
                                               : subst(res, x, b + 1, false, contextptr));
    gen tmp2 = (is_inf(a) && x.type == _IDNT) ? limit(res, *x._IDNTptr, a, 0, contextptr)
                                              : subst(res, x, a, false, contextptr);
    res = tmp1 - tmp2;
  ...
  gen prodsum(const gen & g, bool isprod) {
    ...
    if (isprod) {
      ...
      if (v[0].type == _STRNG)
        res = string2gen("", false); // fix for string joining (L.Marohnić)
      for (--debut; debut < fin; ++debut) {
  ...
  gen seqprod(const gen & g, int type, GIAC_CONTEXT) {
    ...
    if (v.size() < 4) {
      ...
      if (v.size() == 3 && v[1].is_symb_of_sommet(at_equal) &&
          v[1]._SYMBptr->feuille[1].is_symb_of_sommet(at_interval)) {
        gen f = v[1]._SYMBptr->feuille;
        gen v1 = f[0];
        gen v2 = f[1]._SYMBptr->feuille[0], v3 = f[1]._SYMBptr->feuille[1];
        return change_subtype(seqprod(makevecteur(v[0], v1, v2, v3, v[2]), type, contextptr),
                              _SEQ__VECT);
      }
      if (v.size() == 3 && !v[1].is_symb_of_sommet(at_equal) && g.subtype == _SEQ__VECT)
        return change_subtype(seqprod(gen(makevecteur(symb_interval(v[0], v[1]), v[2]), _SEQ__VECT),
                                      type, contextptr),
                              0);
      if (type == 0)
    ...
    if (is_strictly_greater(debut, fin, contextptr)) {
      ...
      if (!is_greater(fin, debut, contextptr))
        return gensizeerr((gettext("Unable to sort boundaries ") + debut.print(contextptr)) +
                          ("," + fin.print(contextptr)));
      if (is_positive(-step, contextptr))
        step = -step;
    ...
    if (v[1].type != _IDNT) {
      if (is_equal(v[1]) && v[1]._SYMBptr->feuille.type == _VECT) {
        ...
        if (tmp.size() == 2) {
          ...
          v[1] = symbolic(at_equal,
                          gen(makevecteur(tmp[0], eval(tmp[1], eval_level(contextptr), contextptr)),
                              _SEQ__VECT));
        }
      } else
  ...
  static bool sum_affine_telescope(const vecteur & v, gen & res, GIAC_CONTEXT) {
    if (v.size() != 4 || v[1].type != _IDNT || v[2].type != _INT_ || v[3].type != _INT_ ||
        ...
        complex_mode(contextptr) || complex_variables(contextptr) || taille(v[0], 33) > 32)
      return false;
    ...
    if (variables.size() > 2)
      return false;
    for (unsigned j = 0; j < variables.size(); ++j)
      if (variables[j].type != _IDNT)
        return false;
    for (unsigned j = 0; j < powers.size(); ++j) {
      const gen & f = powers[j]._SYMBptr->feuille;
      if (f.type != _VECT || f._VECTptr->size() != 2 || f[1].type != _INT_ || f[1].val < -2 ||
          f[1].val > 2)
        return false;
    }
    gen nd = fxnd(e);
    if (nd.type != _VECT || nd._VECTptr->size() != 2 || !integration_resource_rational(nd[0]) ||
        is_zero(nd[0]))
      return false;
    if (taille(nd[1], 65) > 64)
      return false;
    gen A, B, C;
    if (!is_quadratic_wrt(nd[1], v[1], A, B, C, contextptr) || !integration_resource_rational(A) ||
        is_zero(A) || !is_zero(ratnormal(B * B - A * A - 4 * A * C, contextptr)))
      return false;
    gen z = ratnormal((B / A - 1) / 2, contextptr);
    if (contains(z, v[1]) || taille(z, 17) > 16 || !is_zero(im(z, contextptr)))
      return false;
    variables = lvar(z);
    gen slope, shift;
    if (variables.size() > 1 ||
        (variables.size() == 1 &&
         (variables[0].type != _IDNT || !is_linear_wrt(z, variables[0], slope, shift, contextptr) ||
          !integration_resource_rational(slope) || !integration_resource_rational(shift))))
      return false;
    ...
    res = symbolic(at_piecewise,
                   gen(makevecteur(symbolic(at_inferieur_strict, makesequence(t, -N)), answer,
                                   symbolic(at_superieur_strict, makesequence(t, 0)), answer,
                                   symb_equal(t, symbolic(at_floor, t)), undef, answer),
                       _SEQ__VECT));
    return true;
  }
  ...
  gen _sum(const gen & args, GIAC_CONTEXT) {
    ...
    int s = int(v.size());
    gen telescope;
    if (sum_affine_telescope(v, telescope, contextptr))
      return telescope;
    ...
    if (s == 4) {
      ...
      if (v[1].type == _IDNT && (is_inf(af) || af.type == _DOUBLE_) &&
          (is_inf(bf) || bf.type == _DOUBLE_)) {
        vecteur w;
      ...
      try {
        ...
        for (unsigned i = 0; i < w.size(); ++i) {
          if (is_integer(w[i]) && is_greater((v[3] - w[i]) * (w[i] - v[2]), 0, contextptr)) {
            gen v0w = limit(v0, *v[1]._IDNTptr, w[i], 0,
                            contextptr); // gen v0w=subst(v0,v[1],w[i],false,contextptr);
      ...
      if (is_zero(v[2] - v[3] - 1))
        return zero;
      bool numeval = (!is_integer(v[2]) && v[2].type != _FRAC) ||
                     (!is_integer(v[3]) && v[3].type != _FRAC) || approx_mode(contextptr);
      if (is_integral(v[2])) {
      ...
      if (is_positive(v[2] - v[3] - 1, contextptr))
        return -_sum(gen(makevecteur(v[0], v[1], v[3] + (numeval ? gen(1.0) : plus_one), v[2] - 1),
                         _SEQ__VECT),
                     contextptr);
      if (is_strictly_positive(-v[2], contextptr) && is_positive(-v[3], contextptr)) {
        gen tmp = quotesubst(v[0], v[1], -v[1], contextptr);
        return _sum(gen(makevecteur(tmp, v[1], -v[3],
                                    (numeval ? evalf_double(-v[2], 1, contextptr) : -v[2])),
                        args.subtype),
                    contextptr);
      }
      if (v[2].type == _INT_ && v[3].type == _INT_ &&
          absint(v[3].val - v[2].val) < max_sum_add(contextptr)) {
        gen res = seqprod(v, 2, contextptr);
        return (numeval || has_num_coeff(res)) ? evalf(res, 1, contextptr)
                                               : ratnormal(res, contextptr);
      }
    } // end if s==4
    const_iterateur it = v.begin(), itend = v.end();
    gen f = *it;
  ...
  define_unary_function_ptr5(at_wz_certificate, alias_at_wz_certificate, &__wz_certificate, 0,
                             true);
  ...
  */
  ...
  static int gsl_odesolve_jacobian(double t, const double y[], double * dfdy, double dfdt[],
                                   void * params) {
    // compute
    odesolve_param * par = (odesolve_param *)params;
#ifndef NO_STDEXCEPT
  ...
  double rk_error(const vecteur & v, const vecteur & w_final, const vecteur & w_init,
                  GIAC_CONTEXT) {
    double err = 0, derr;
  ...
  gen odesolve(const gen & t0orig, const gen & t1orig, const gen & f, const gen & y0orig,
               double tstep, bool return_curve, double * ymin, double * ymax, int maxstep,
               GIAC_CONTEXT) {
    ...
    if ((t0_e.type != _DOUBLE_ && t0_e.type != _CPLX) ||
        (t1_e.type != _DOUBLE_ && t1_e.type != _CPLX))
      return gensizeerr(contextptr);
  ...
  gen tolerance = epsilon(contextptr) > 1e-12 ? epsilon(contextptr) : 1e-12;
    ...
    double butcher_b4[] = {5179. / 57600,    0,           7571. / 16695, 393. / 640,
    ...
    for (int nstep = 0; do_while && nstep < maxstep && temps < temps_total; ++nstep) {
      ...
      for (int j = 1; j <= 6; j++) {
        ...
        if (dim == 1) {
          gen & yt10 = yt1[0];
          for (int k = 0; k < j; k++) {
            type_operator_plus_times(butcher_a[butcher_a_shift + k], butcher_k[k]._VECTptr->front(),
                                     yt10);
          }
        } else {
          for (int k = 0; k < j; k++) {
      ...
      if (is_undef(hopt))
        break;
      if (debug_infolevel > 5)
        CERR << nstep << ":" << t_e << ",y5=" << y_final5 << ",y4=" << y_final4 << " " << tstep
             << " tstep (optimal)=" << hopt << " err=" << err << '\n';
      if (is_strictly_greater(err, tolerance, contextptr)) {
        ...
        if (!iscomplex) {
          ...
          for (int i = 0; i < dim; ++i) {
            // CERR << y[i] << '\n';
            if (ymin && ymax &&
                (y_final5[i]._DOUBLE_val < ymin[i] || y_final5[i]._DOUBLE_val > ymax[i]))
              do_while = false;
          }
        }
      }
  ...
  static gen odesolve(const vecteur & w, GIAC_CONTEXT) {
    ...
    if (v[1].type == _VECT && v[2].type == _VECT &&
        v[2]._VECTptr->size() == v[1]._VECTptr->size() && vs > 3) {
      if (v[1]._VECTptr->size() < 2)
        return gendimerr(contextptr);
      t0 = v[2]._VECTptr->front();
      t1 = v[3];
      gen newv1 =
          symbolic(at_equal, v[1]._VECTptr->front(), symb_interval(v[2]._VECTptr->front(), v[3]));
    ...
    if (t0t.is_symb_of_sommet(at_interval)) { // functional form
      ...
      vstart = 1;
      read_tmintmaxtstep(v, t, vstart, tmin, tmax, tstep, tminmax_defined, tstep_defined,
                         contextptr);
      if (t0 != t1) {
        if (tstep == 0)
  ...
  gen fourier_an(const gen & f, const gen & x, const gen & T, const gen & n, const gen & a,
                 GIAC_CONTEXT) {
    gen primi, iT = inv(T, contextptr);
    gen omega = ratnormal(2 * cst_pi * iT, contextptr);
    fourier_assume(n, contextptr);
    primi = _integrate(
        gen(makevecteur(f * cos(omega * n * x, contextptr), x, a, ratnormal(a + T, contextptr)),
            _SEQ__VECT),
        contextptr);
    gen an = iT * primi;
    if (n != 0)
  ...
  gen fourier_bn(const gen & f, const gen & x, const gen & T, const gen & n, const gen & a,
                 GIAC_CONTEXT) {
    fourier_assume(n, contextptr);
    gen primi, iT = inv(T, contextptr);
    gen omega = ratnormal(2 * cst_pi * iT, contextptr);
    primi = _integrate(
        gen(makevecteur(f * sin(omega * n * x, contextptr), x, a, ratnormal(a + T, contextptr)),
            _SEQ__VECT),
        contextptr);
    gen an = 2 * iT * primi;
    return has_num_coeff(an) ? an : recursive_normal(an, contextptr);
  ...
  gen fourier_cn(const gen & f, const gen & x, const gen & T, const gen & n, const gen & a,
                 GIAC_CONTEXT) {
    fourier_assume(n, contextptr);
    gen primi, iT = inv(T, contextptr);
    gen omega = ratnormal(2 * cst_pi * iT, contextptr);
    primi = _integrate(gen(makevecteur(f * exp(-cst_i * omega * n * x, contextptr), x, a,
                                       ratnormal(a + T, contextptr)),
                           _SEQ__VECT),
                       contextptr);
    gen cn = iT * primi;
