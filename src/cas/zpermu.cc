...
namespace giac {
  ...
  gen _hadamard(const gen & args, GIAC_CONTEXT) {
    ...
    if (ckmatrix(args) || args[0][0].type != _VECT) {
      ...
      return sqrt(min(square_hadamard_bound(m), square_hadamard_bound(mtran(m)), contextptr),
                  contextptr);
    }
    if ((args.type != _VECT) || (args._VECTptr->size() != 2))
      return gentypeerr(contextptr);
    ...
    vecteur c;
    for (int k = 0; k < n; k++) {
      if ((v1[k].type != _VECT) || (v2[k].type != _VECT))
        return gentypeerr(contextptr);
      ...
      if (opt.is_symb_of_sommet(at_equal) &&
        ...
        if (coord.type == _VECT && coord._VECTptr->size() == 3) {
          if (opt._SYMBptr->feuille[1] == at_sphere) {
            gen r = coord[0], r2 = pow(r, 2, contextptr), t = coord[1], s = sin(t, contextptr),
                p = coord[2];
  ...
  define_unary_function_ptr5(at_laplacian, alias_at_laplacian, &__laplacian, _QUOTE_ARGUMENTS,
                             true);
  ...
  gen _divergence(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args._VECTptr->size() == 3) {
      ...
      if (opt.is_symb_of_sommet(at_equal) &&
        ...
        gen coord = (*args._VECTptr)[1];
        if (g.type == _VECT && coord.type == _VECT && g._VECTptr->size() == 3 &&
            coord._VECTptr->size() == 3) {
          if (opt._SYMBptr->feuille[1] == at_sphere) {
            // spheric: 1/r^2*d_r(r^2*A_r)+1/r/sin(theta)*d_theta(sin(theta)*A_theta)+1/r*sin(theta)*d_phi(A_phi)
            gen Ar = g[0], At = g[1], Ap = g[2], r = coord[0], r2 = pow(r, 2, contextptr),
                t = coord[1], s = sin(t, contextptr), p = coord[2];
            return derive(r2 * Ar, r, contextptr) / r2 +
                   (derive(s * At, t, contextptr) + derive(Ap, p, contextptr)) / (r * s);
          }
          if (opt._SYMBptr->feuille[1] == at_cylindre) {
            // cylindric: 1/r*d_r(r*A_r)+1/r*d_theta(A_theta)+d_z(A_z)
            gen Ar = g[0], At = g[1], Az = g[2], r = coord[0], t = coord[1], z = coord[2];
            return (derive(r * Ar, r, contextptr) + derive(At, t, contextptr)) / r +
                   derive(Az, z, contextptr);
          }
        }
      }
    }
  ...
  define_unary_function_ptr5(at_divergence, alias_at_divergence, &__divergence, _QUOTE_ARGUMENTS,
                             true);
        ...
        if (g.type == _VECT && coord.type == _VECT && g._VECTptr->size() == 3 &&
          ...
          if (opt._SYMBptr->feuille[1] == at_sphere) {
            ...
            gen Ar = g[0], At = g[1], Ap = g[2], r = coord[0], r2 = pow(r, 2, contextptr),
                t = coord[1], s = sin(t, contextptr), p = coord[2];
            return makevecteur(
                (derive(s * Ap, t, contextptr) - derive(At, p, contextptr)) / (r * s),
                derive(Ar, p, contextptr) / (r * s) - derive(r * Ap, r, contextptr) / r,
                (derive(r * At, r, contextptr) - derive(Ar, t, contextptr)) / r);
          }
          if (opt._SYMBptr->feuille[1] == at_cylindre) {
            // cylindric (1/r*d_theta(A_z)-d_z(A_theta)),d_z(A_r)-d_r(A_z),1/r*d_r(r*A_theta)-d_theta(A_r))
            gen Ar = g[0], At = g[1], Az = g[2], r = coord[0], t = coord[1], z = coord[2];
            return makevecteur(derive(Az, t, contextptr) / r - derive(At, z, contextptr),
                               derive(Ar, z, contextptr) - derive(Az, r, contextptr),
                               (derive(r * At, r, contextptr) - derive(Ar, t, contextptr)) / r);
          }
        }
      }
  ...
  bool hnf(const matrice & Aorig, const gen & var, matrice & U, matrice & A, matrice & V,
           bool do_smith, GIAC_CONTEXT) {
  ...
  gen _hermite(const gen & args, GIAC_CONTEXT) {
    ...
    if (!find_n_x(args, n, x, a)) {
      ...
      if (args.type == _VECT && args._VECTptr->size() == 2 && ckmatrix(args._VECTptr->front()) &&
          hnf(*args._VECTptr->front()._VECTptr, args._VECTptr->back(), U, A, V, false, contextptr))
        return makesequence(U, A);
      return gensizeerr(contextptr);
    }
  ...
  gen _smith(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args._VECTptr->size() == 2 && ckmatrix(args._VECTptr->front()) &&
        hnf(*args._VECTptr->front()._VECTptr, args._VECTptr->back(), U, A, V, true, contextptr))
      return makesequence(U, A, V);
    return gensizeerr(contextptr);
  }
  ...
  gen tchebyshev_eval(const gen & n, const gen & x, const vecteur & v, GIAC_CONTEXT) {
    ...
    if (res.size() == 1)
      ...
      matrice A(
          makevecteur(makevecteur(res[1], res[0]), makevecteur(-res[0], 2 * res[0] * x + res[1])));
  ...
  gen _tchebyshev1(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args._VECTptr->size() == 2 && is_integer(args._VECTptr->front()) &&
      ...
      return tchebyshev_eval(args._VECTptr->front(), args._VECTptr->back(),
                             makevecteur(1, args._VECTptr->back()), contextptr);
    if (!find_n_x(args, n, x, a))
      return gensizeerr(contextptr);
    if (n == 0)
      return 1;
  ...
  gen _tchebyshev2(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args._VECTptr->size() == 2 && is_integer(args._VECTptr->front()) &&
        args._VECTptr->back().type != _IDNT)
      return tchebyshev_eval(args._VECTptr->front() + 1, args._VECTptr->back(), makevecteur(0, 1),
                             contextptr);
    int n;
    gen a, x;
    if (!find_n_x(args, n, x, a))
  ...
  gen _sum_riemann(const gen & args, GIAC_CONTEXT) {
    ...
    if (is_zero(tmp) && is_inf(tmp2))
      return string2gen(gettext("Probably not a Riemann sum"), false);
    gen res =
        recursive_normal(tmp * _series(makesequence(an, v2[0], plus_inf), contextptr), contextptr);
