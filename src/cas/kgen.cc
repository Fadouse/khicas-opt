...
#define ALLOC48 16 * 32
  ...
  static void deletefast(void * obj) {
    if (((size_t)obj >= (size_t)&tab24[0]) && ((size_t)obj < (size_t)&tab24[ALLOC24])) {
      int pos = ((size_t)obj - ((size_t)&tab24[0])) / sizeof(six_int);
      freeslot24[pos / 32] |= (1u << (pos % 32));
      if (unsigned(pos / 32) < first_free24)
        first_free24 = unsigned(pos / 32);
      return;
    }
    if (((size_t)obj >= (size_t)&tab16[0]) && ((size_t)obj < (size_t)&tab16[ALLOC16])) {
      *(unsigned *)obj = 0;
      int pos = ((size_t)obj - ((size_t)&tab16[0])) / sizeof(four_int);
      freeslot16[pos / 32] |= (1u << (pos % 32));
      if (unsigned(pos / 32) < first_free16)
        first_free16 = unsigned(pos / 32);
      return;
    }
#ifdef ALLOC48
    if (((size_t)obj >= (size_t)&tab48[0]) && ((size_t)obj < (size_t)&tab48[ALLOC48])) {
      *(unsigned *)obj = 0;
      int pos = ((size_t)obj - ((size_t)&tab48[0])) / sizeof(twelve_int);
      freeslot48[pos / 32] |= (1u << (pos % 32));
      if (unsigned(pos / 32) < first_free48)
        first_free48 = unsigned(pos / 32);
      return;
    }
#endif
#ifdef ALLOC32
    if (((size_t)obj >= (size_t)&tab32[0]) && ((size_t)obj < (size_t)&tab32[ALLOC32])) {
      int pos = ((size_t)obj - ((size_t)&tab32[0])) / sizeof(eight_int);
      freeslot32[pos / 32] |= (1u << (pos % 32));
      if (unsigned(pos / 32) < first_free32)
        first_free32 = unsigned(pos / 32);
      return;
    }
#endif
    free(obj);
    ...
    if (!dbg->sst_at_stack.empty()) {
      ...
      if (pos.type == _VECT && pos._VECTptr->size() >= 2) {
        ...
        if (v.size() == 1) {
          ...
          tmp = pos._VECTptr->front().print(contextptr) + "(" +
                gen(v, _SEQ__VECT).print(contextptr) + ")";
      } else
...
void divisionby0err(const gen & e, GIAC_CONTEXT) {
  throw(std::runtime_error(last_evaled_function(contextptr) + gettext("Division of ") +
                           e.print(contextptr) + gettext(" by 0")));
}

void cksignerr(const gen & e, GIAC_CONTEXT) {
  throw(std::runtime_error(last_evaled_function(contextptr) + gettext("Unable to check sign: ") +
                           e.print(contextptr)));
}

void invalidserieserr(const string & s, GIAC_CONTEXT) {
  throw(std::runtime_error(last_evaled_function(contextptr) +
                           gettext("Invalid series expansion: ") + s));
}
...
void maxordererr(GIAC_CONTEXT) {
  throw(std::runtime_error(last_evaled_function(contextptr) + gettext("Max order (") +
...
void setstabilityerr(GIAC_CONTEXT) {
  throw(std::runtime_error(last_evaled_function(contextptr) +
                           gettext("calculation size limit exceeded")));
}
#endif // NO_STDEXCEPT
  ...
  gen gendivisionby0err(const gen & e, GIAC_CONTEXT) {
    return undeferr(last_evaled_function(contextptr) + gettext("Error: Division of ") +
                    e.print(contextptr) + gettext(" by 0"));
  }

  gen gencksignerr(const gen & e, GIAC_CONTEXT) {
    return undeferr(last_evaled_function(contextptr) + gettext("Error: Unable to check sign: ") +
                    e.print(contextptr));
  }

  gen geninvalidserieserr(const string & s, GIAC_CONTEXT) {
    *logptr(contextptr) << undeferr(last_evaled_function(contextptr) +
                                    gettext("Error: Invalid series expansion: ") + s)
                        << endl;
    return undef;
  }
  ...
  gen genstabilityerr(GIAC_CONTEXT) {
    return undeferr(last_evaled_function(contextptr) +
                    gettext("Error: calculation size limit exceeded"));
  }
...
ref_vecteur * new_ref_vecteur(const vecteur & v) {
  ...
  *((unsigned *)&ptr->v + 6) = 0;
...
#endif
    if (v.size() > 1 && ((((size_t)_VECTptr) & stack_mask) > stackptr ||
  ...
  __VECTptr = vptr;
    ...
    type = _VECT;
    subtype = (signed char)s;
    if (_VECTptr->size() > 1 && ((((size_t)_VECTptr) & stack_mask) > stackptr ||
                                 (((size_t)_VECTptr->begin()) & stack_mask) > stackptr))
      ctrl_c = interrupted = true;
  }
...
ref_symbolic * new_ref_symbolic(const symbolic & s) {
  ...
  *(((unsigned *)&ptr->s.feuille) + 1) = 0;
  ...
  __SYMBptr = new_ref_symbolic(s);
    ...
    type = _SYMB;
    subtype = 0;
    if (_SYMBptr->sommet != at_restart && _SYMBptr->sommet != at_purge &&
        (((size_t)_SYMBptr) & stack_mask) > stackptr)
      ctrl_c = interrupted = true;
  }
  ...
  *(((unsigned *)&ptr->im) + 1) = 0;
  ...
  complex<double> gen2complex_d(const gen & e) {
    if (e.type == _CPLX) {
      ...
      if (ee.type == _DOUBLE_)
        return complex<double>(ee._DOUBLE_val, 0);
      if (ee.type != _CPLX) {
#ifndef NO_STDEXCEPT
  ...
  gen & gen::operator=(const gen & a) {
    ...
    if (a.type > _DOUBLE_ && a.type != _FLOAT_ && a.type != _FUNC && a.__ZINTptr) {
  ...
  *((ulonglong *)this) = *((ulonglong *)&a);
    ...
    if (type_save > _DOUBLE_ && type_save != _FLOAT_ && type_save != _FUNC)
...
#ifdef DEBUG_SUPPORT
    if (e.type != _FRAC)
      setsizeerr(gettext("gen.cc/_FRAC2_SYMB"));
#endif
  ...
  bool check_not_assume(const gen & not_evaled, gen & evaled, bool evalf_after,
    ...
    if (evaled.type == _VECT && evaled.subtype == _ASSUME__VECT) {
      ...
      if (not_evaled.type == _IDNT && evaled._VECTptr->size() == 1 &&
          evaled._VECTptr->front().type == _INT_) {
        gen tmp = not_evaled;
  ...
  static bool in_eval_vect(const gen & g, gen & evaled, int level, GIAC_CONTEXT) {
    ...
    if (g.subtype == _INTERVAL__VECT && g._VECTptr->size() == 2) {
      ...
      gen l = eval(g._VECTptr->front(), level, contextptr),
          u = eval(g._VECTptr->back(), level, contextptr);
      ...
      if ((l.type == _DOUBLE_ && u.type == _DOUBLE_) || (l.type == _REAL && u.type == _REAL)) {
        mpfi_t interv;
        mpfi_init(interv);
    ...
    if (g.subtype == _FOLDER__VECT || g.subtype == _RGBA__VECT)
      return false;
    if ((g.subtype == _SEQ__VECT) && (!g._VECTptr->empty()) &&
        (g._VECTptr->front().type == _SYMB) &&
        (g._VECTptr->front().is_symb_of_sommet(at_comment)) && (g._VECTptr->back().type == _SYMB) &&
    ...
    if (g._VECTptr->size() == 1 && g._VECTptr->front().is_symb_of_sommet(at_interval))
      return in_eval_vect(
          gen(makevecteur(g._VECTptr->front()[1], g._VECTptr->front()[2]), _INTERVAL__VECT), evaled,
          1, contextptr);
  ...
  bool gen::in_eval(int level, gen & evaled, const context * contextptr) const {
    ...
    case _SYMB:
        ...
        return false;
      {
        ...
        bool is_ifte = false, is_of_local_ifte_bloc = false, is_plus = Sommet == at_plus,
             is_prod = false, is_pow = false;
...
#endif // pthread
        {
          ...
          if ((is_plus || is_prod || is_pow) && feuille.type == _VECT &&
            ...
            if (a.type != _VECT || a.subtype != _SEQ__VECT) {
              ...
              if (evaled.type != _VECT || evaled.subtype != _SEQ__VECT) {
                if (is_plus)
                  evaled = operator_plus(a, evaled, contextptr);
                else {
                  if (is_prod)
                    evaled = operator_times(a, evaled, contextptr);
                  else
                    evaled = pow(a, evaled, contextptr);
                }
                elevel = slevel;
                return true;
              }
  ...
  polynome apply(const polynome & p, const context * contextptr,
                 gen (*f)(const gen &, const context *)) {
    polynome res(p.dim);
    std::vector<monomial<gen>>::const_iterator it = p.coord.begin(), itend = p.coord.end();
    res.coord.reserve(itend - it);
    for (; it != itend; ++it) {
  ...
  gen accurate_evalf(const gen & g, int nbits) {
    if (g.type == _FRAC && g._FRACptr->num.type == _VECT)
      return inv(accurate_evalf(g._FRACptr->den, nbits), context0) *
             accurate_evalf(g._FRACptr->num, nbits);
    if (g.type == _VECT)
...
#ifdef BF2GMP_H
      mpfr_set_str(euler_gamma,
                   "0."
                   "5772156649015328606065120900824024310421593359399235988057672348848677267776646"
                   "7093694706329174674951463144724980708248096050401448654283622417399764492353625"
                   "3500333742937337737673942792595258247094916008735203948165670853233151776611528"
                   "6211995015079847937450857057400299213547861466940296043254215190587755352",
                   nbits, MPFR_RNDN);
#else
  ...
  bool has_evalf(const gen & g, gen & res, int level, GIAC_CONTEXT) {
    ...
    if (g.type == _EXT) {
      gen a, b;
      if (has_evalf(*g._EXTptr, a, level, contextptr) &&
          has_evalf(*(g._EXTptr + 1), b, level, contextptr)) {
  ...
  bool gen::in_evalf(int level, gen & evaled, const context * contextptr) const {
    ...
    case _SYMB:
      ...
      if (_SYMBptr->sommet == at_integrate ||
          (_SYMBptr->sommet == at_int && xcas_mode(contextptr) != 3)) {
...
#ifdef HAVE_LIBMPFR
      if (decimal_digits(contextptr) > 14)
        evaled = _FRACptr->num.evalf(level, contextptr) *
                 inv(_FRACptr->den.evalf(level, contextptr), contextptr);
      else
#endif
      {
        if (is_zero(_FRACptr->num.im(contextptr)) && is_zero(_FRACptr->den.im(contextptr)))
          ...
          evaled = gen(
              re(contextptr).evalf(1, contextptr),
              im(contextptr)
                  .evalf(
                      1,
                      contextptr)); // re(contextptr).evalf(1,contextptr)+cst_i*im(contextptr).evalf(1,contextptr);
      }
      return true;
#ifdef HAVE_LIBMPFR
      if (decimal_digits(contextptr) <= 14)
...
#endif
        evaled = rdiv(_FRACptr->num.evalf(level, contextptr),
                      _FRACptr->den.evalf(level, contextptr), contextptr);
      return true;
    case _FUNC:
      return in_eval_func(*this, &evaled, contextptr);
    case _MOD:
  ...
  gen evalf2double_nock(const gen & g0, int level, const context * contextptr) {
    ...
    if (g0.type == _CPLX) {
      ...
      return gen(
          evalf2double_nock(*g0._CPLXptr, 1, contextptr),
          evalf2double_nock(
              *(g0._CPLXptr + 1), 1,
              contextptr)); //evalf2double_nock(*g0._CPLXptr,1,contextptr)+cst_i*evalf2double_nock(*(g0._CPLXptr+1),1,contextptr);
    }
    gen g = evalf(g0, level, contextptr);
    if (g.type == _CPLX)
      return gen(
          evalf2double_nock(*g._CPLXptr, 1, contextptr),
          evalf2double_nock(
              *(g._CPLXptr + 1), 1,
              contextptr)); //evalf2double_nock(*g._CPLXptr,1,contextptr)+cst_i*evalf2double_nock(*(g._CPLXptr+1),1,contextptr);
    return g;
  }
  ...
  static void symb_reim(const symbolic & s, gen & r, gen & i, GIAC_CONTEXT) {
    ...
    if (u == at_integrate) {
      ...
      vecteur v = *f._VECTptr;
      if (v.size() <= 2 || (v.size() >= 4 && is_exactly_zero(im(v[2], contextptr)) &&
                            is_exactly_zero(im(v[3], contextptr)))) {
        f = v[0];
        reim(f, ref, imf, contextptr);
        v[0] = ref;
    ...
    if ((u == at_asin || u == at_acos) && is_zero(imf, contextptr) &&
        is_greater(1, f, contextptr) && is_greater(f, -1, contextptr)) {
      r = s;
      i = 0;
      return;
    }
  ...
  static gen symb_re(const symbolic & s, GIAC_CONTEXT) {
    ...
    if (u == at_pow) {
      ...
      if (is_zero(im(expo, contextptr), contextptr) && is_zero(im(e, contextptr), contextptr)) {
        gen sgn = atan_tan_no_floor(contextptr)
                      ? 1
                      : sign(e, contextptr); // workaround for int(sqrt(x+sqrt(x)))
        if (!is_integer(expo)) {
          if (sgn == -1)
  ...
  gen gen::re(GIAC_CONTEXT) const {
    ...
    case _CPLX:
      return *_CPLXptr;
    case _VECT:
      return gen(subtype == _POLY1__VECT ? trim(_VECTre(*_VECTptr, contextptr), 0)
                                         : _VECTre(*_VECTptr, contextptr),
                 subtype);
    case _MAP:
  ...
  gen gen::im(GIAC_CONTEXT) const {
    ...
    case _CPLX:
      return *(_CPLXptr + 1);
    case _VECT:
      return gen(subtype == _POLY1__VECT ? trim(_VECTim(*_VECTptr, contextptr), 0)
                                         : _VECTim(*_VECTptr, contextptr),
                 subtype);
    case _MAP:
  ...
  gen arg_CPLX(const gen & a, GIAC_CONTEXT) {
    ...
    if (is_zero_or_contains(imagpart, contextptr))
      return (1 - sign(realpart, contextptr)) * cst_pi_over_2 +
             atan(imagpart / realpart, contextptr);
    if ((realpart.type == _DOUBLE_) || (imagpart.type == _DOUBLE_))
      return eval(atan(rdiv(imagpart, realpart, contextptr), contextptr) +
                      (1 - sign(realpart, contextptr)) * sign(imagpart, contextptr) *
                          evalf_double(cst_pi_over_2, 1, contextptr),
                  1, contextptr);
    else
  ...
  gen makemod(const gen & a, const gen & b) {
    ...
    if (a.type == _POLY) {
      ...
      vector<monomial<gen>>::const_iterator it = a._POLYptr->coord.begin(),
                                            itend = a._POLYptr->coord.end();
      res.coord.reserve(itend - it);
      for (; it != itend; ++it) {
  ...
  static gen modinv(const gen & a) {
    ...
    if (((modu.type == _INT_) || (modu.type == _ZINT)) && a._MODptr->is_cinteger())
  ...
  gen operator_plus(const gen & a, const gen & b, unsigned t, GIAC_CONTEXT) {
    ...
    case _VECT__VECT:
      ...
      if (warnextend && a.subtype == 0 && b.subtype == 0 && python_compat(contextptr)) {
        warnextend = false;
        alert(
            gettext("Warning + is vector addition, run list1.extend(list2) for list concatenation"),
            contextptr);
      }
    ...
    case _CPLX__CPLX: {
      ...
      if (aptr->type == _DOUBLE_ && (aptr + 1)->type == _DOUBLE_ && bptr->type == _DOUBLE_ &&
        ...
        return adjust_complex_display(gen(aptr->_DOUBLE_val + bptr->_DOUBLE_val,
                                          (aptr + 1)->_DOUBLE_val + (bptr + 1)->_DOUBLE_val),
                                      a, b);
    ...
    case _MOD__MOD:
#ifdef SMARTPTR64
      return modadd((ref_modulo *)(*((ulonglong *)&a) >> 16),
                    (ref_modulo *)(*((ulonglong *)&b) >> 16));
#else
  ...
  static gen symbolic_plot_makevecteur(const unary_function_ptr & u, const gen & e, bool project,
                                       GIAC_CONTEXT) {
  ...
  gen sym_add(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    if (a.is_symb_of_sommet(at_unit)) {
      ...
      if (b.is_symb_of_sommet(at_unit)) {
        ...
        if (is_undef(tmp))
          return tmp;
        return new_ref_symbolic(symbolic(
            at_unit,
            makenewvecteur(operator_plus(va[0], operator_times(g, vb[0], contextptr), contextptr),
                           va[1])));
      }
      if (lidnt(b).empty()) {
    ...
    if ((a.type == _SYMB) && equalposcomp(plot_sommets, a._SYMBptr->sommet)) {
      if ((b.type == _SYMB) && equalposcomp(plot_sommets, b._SYMBptr->sommet))
        ...
        if (a.is_symb_of_sommet(at_curve) && a._SYMBptr->feuille.type == _VECT &&
            a._SYMBptr->feuille._VECTptr->size() == 2 &&
            a._SYMBptr->feuille._VECTptr->front().type == _VECT) {
          ...
          if (v.size() >= 6) {
            v[5] = subst(v[5], makevecteur(x__IDNT_e, y__IDNT_e),
                         makevecteur(x__IDNT_e - re(b, contextptr), y__IDNT_e - im(b, contextptr)),
                         false, contextptr);
            a_ = symbolic(at_curve,
                          gen(makevecteur(gen(v, a._SYMBptr->feuille._VECTptr->front().subtype),
                                          a._SYMBptr->feuille._VECTptr->back()),
                              a._SYMBptr->feuille.subtype));
          }
        }
        return symbolic_plot_makevecteur(a_._SYMBptr->sommet, a_._SYMBptr->feuille + b, true,
                                         contextptr);
      }
    }
    if ((b.type == _SYMB) && equalposcomp(plot_sommets, b._SYMBptr->sommet)) {
      ...
      return symbolic_plot_makevecteur(b._SYMBptr->sommet, b._SYMBptr->feuille + a, true,
                                       contextptr);
    }
    ...
    if (is_algebraic_program(a, var1, res1)) {
      ...
      return symbolic(at_program,
                      gen(makevecteur(var1, 0, operator_plus(res1, b, contextptr)), _SEQ__VECT));
    }
    if (is_algebraic_program(b, var2, res2)) {
      if (!is_constant_wrt(a, var2, contextptr))
        *logptr(contextptr) << "Warning constant+function with constant dependent of mute variable"
                            << '\n';
      return symbolic(at_program,
                      gen(makevecteur(var2, 0, operator_plus(a, res2, contextptr)), _SEQ__VECT));
    }
    if (a.type == _VECT) {
        ...
        if (b.type == _VECT && b._VECTptr->size() == 2) {
          ...
          if (b.subtype == _VECTOR__VECT && res.front() == bv.back())
            return _vector(
                ...
                contextptr);
          return _vector(
              gen(makenewvecteur(res.front(), res.back() + bv.back() - bv.front()), _SEQ__VECT),
              contextptr);
        }
    ...
    if (ia) {
      ...
      if (ia == ib || (ia == 1 && ib)) {
        ...
        vecteur & vb = *b._SYMBptr->feuille._VECTptr;
        return new_ref_symbolic(symbolic(
            b._SYMBptr->sommet, makesequence(va.front() + vb.front(), va.back() + vb.back())));
      }
      if (ia == 1 || !ib) // = +
        return new_ref_symbolic(
            symbolic(a._SYMBptr->sommet, makesequence(va.front() + b, va.back() + b)));
      if ((ia == 5 && ib == 6) || (ia == 6 && ib == 5)) {
        vecteur & vb = *b._SYMBptr->feuille._VECTptr;
        return new_ref_symbolic(symbolic(
            at_superieur_strict, makesequence(va.front() + vb.front(), va.back() + vb.back())));
      }
    }
    if (ib)
      return b + a;
    if (a.is_symb_of_sommet(at_interval)) {
      ...
      if (f.type == _VECT && f._VECTptr->size() == 2) {
        ...
        return new_ref_symbolic(
            symbolic(at_interval, gen(makenewvecteur(b + v[0], b + v[1]), _SEQ__VECT)));
      }
    }
    if (b.is_symb_of_sommet(at_interval))
      return b + a;
    ...
    if ((a.type == _SYMB) && (b.type == _SYMB)) {
      if (a._SYMBptr->sommet == at_plus) {
        if (b._SYMBptr->sommet == at_plus)
          return new_ref_symbolic(symbolic(
              at_plus,
              gen(mergevecteur(*(a._SYMBptr->feuille._VECTptr), *(b._SYMBptr->feuille._VECTptr)),
                  _SEQ__VECT)));
        else
  ...
  gen operator_minus(const gen & a, const gen & b, unsigned t, GIAC_CONTEXT) {
    ...
    case _REAL__CPLX:
      return gen(a - *b._CPLXptr, -*(b._CPLXptr + 1));
    case _CPLX__CPLX:
      return adjust_complex_display(
          gen(*a._CPLXptr - *b._CPLXptr, *(a._CPLXptr + 1) - *(b._CPLXptr + 1)), a, b);
    case _POLY__POLY:
      return subpoly(a, b);
    case _FRAC__FRAC:
      return (*a._FRACptr) - (*b._FRACptr);
    ...
    case _MOD__MOD:
#ifdef SMARTPTR64
      return modsub((ref_modulo *)(*((ulonglong *)&a) >> 16),
                    (ref_modulo *)(*((ulonglong *)&b) >> 16));
#else
    ...
    */
    ...
    if ((a.type == _SYMB) && (b.type == _SYMB)) {
      if (a._SYMBptr->sommet == at_plus) {
        if (b._SYMBptr->sommet == at_plus)
          return new_ref_symbolic(
              symbolic(at_plus, gen(mergevecteur(*(a._SYMBptr->feuille._VECTptr),
                                                 negvecteur(*(b._SYMBptr->feuille._VECTptr))),
                                    _SEQ__VECT)));
        else
          return new_ref_symbolic(symbolic(*a._SYMBptr, -b));
      } else {
  ...
  gen operator-(const gen & a) {
    ...
    case _SYMB:
      ...
      if (a._SYMBptr->sommet == at_unit) {
        ...
        return new_ref_symbolic(
            symbolic(at_unit, makenewvecteur(-a._SYMBptr->feuille._VECTptr->front(),
                                             a._SYMBptr->feuille._VECTptr->back())));
      }
      if (a._SYMBptr->sommet == at_plus)
      ...
      if (a._SYMBptr->sommet == at_interval && a._SYMBptr->feuille.type == _VECT &&
          a._SYMBptr->feuille._VECTptr->size() == 2) {
        return symb_interval(
            -a._SYMBptr->feuille._VECTptr->back(),
            -a._SYMBptr->feuille._VECTptr
                 ->front()); // (symbolic(at_interval,gen(makenewvecteur(-a._SYMBptr->feuille._VECTptr->back(),-a._SYMBptr->feuille._VECTptr->front()),_SEQ__VECT)));
      }
      ...
      if (a._SYMBptr->sommet == at_equal || a._SYMBptr->sommet == at_equal2 ||
        ...
        return (symbolic(a._SYMBptr->sommet, makesequence(-a._SYMBptr->feuille._VECTptr->front(),
                                                          -a._SYMBptr->feuille._VECTptr->back())));
      if (is_inequality(a))
        return (symbolic(a._SYMBptr->sommet, makesequence(-a._SYMBptr->feuille._VECTptr->back(),
  ...
  static gen mulpoly(const gen & th, const gen & other) {
    ...
    if (c1 == 1)
      return other._POLYptr->shift(th._POLYptr->coord.front().index,
                                   th._POLYptr->coord.front().value);
    int c2 = int(other._POLYptr->coord.size());
    if (c2 == 1)
      return th._POLYptr->shift(other._POLYptr->coord.front().index,
                                other._POLYptr->coord.front().value);
    ref_polynome * resptr = new ref_polynome(th._POLYptr->dim);
  ...
  void type_operator_plus_times(const gen & a, const gen & b, gen & c) {
    ...
    if (c.type == _EXT && a.type == _EXT && b.type == _EXT) {
      if ((c._EXTptr + 1)->type == _VECT && *(a._EXTptr + 1) == *(c._EXTptr + 1) &&
          *(b._EXTptr + 1) == *(c._EXTptr + 1) && a._EXTptr->type == _VECT &&
          b._EXTptr->type == _VECT && c._EXTptr->type == _VECT) {
        ...
        if (v.size() == 3 && v[0] == 1 && v[1] == 0 && a._EXTptr->_VECTptr->size() == 2 &&
          ...
          gen a1 = a._EXTptr->_VECTptr->front(), a0 = a._EXTptr->_VECTptr->back(),
              b1 = b._EXTptr->_VECTptr->front(), b0 = b._EXTptr->_VECTptr->back(),
              c1 = c._EXTptr->_VECTptr->front(), c0 = c._EXTptr->_VECTptr->back();
    ...
    if (c.type == _VECT && c.ref_count() == 1 && a.type == _VECT && b.type == _VECT &&
        ...
        is_int_zint_vecteur(*a._VECTptr) && is_int_zint_vecteur(*b._VECTptr) &&
        is_int_zint_vecteur(*c._VECTptr)) {
      add_mulmodpoly(a._VECTptr->begin(), a._VECTptr->end(), b._VECTptr->begin(), b._VECTptr->end(),
                     0, *c._VECTptr);
      return;
    }
    gen g;
  ...
  static gen mult_cplx(const gen & a, const gen & b, GIAC_CONTEXT) {
    gen *aptr = a._CPLXptr, *bptr = b._CPLXptr;
    unsigned t =
        (aptr->type | ((aptr + 1)->type << 8) | (bptr->type << 16) | ((bptr + 1)->type << 24));
    if (t == (_DOUBLE_ | (_DOUBLE_ << 8) | (_DOUBLE_ << 16) | (_DOUBLE_ << 24))) {
      double ar = aptr->_DOUBLE_val, ai = (aptr + 1)->_DOUBLE_val, br = bptr->_DOUBLE_val,
             bi = (bptr + 1)->_DOUBLE_val;
    ...
    if (t == (_REAL | (_REAL << 8) | (_REAL << 16) | (_REAL << 24))) {
      ...
      if (!dynamic_cast<real_interval *>(&ax) || !dynamic_cast<real_interval *>(&ay) ||
          !dynamic_cast<real_interval *>(&bx) || !dynamic_cast<real_interval *>(&by)) {
        mpfr_t axbx, ayby, r;
  ...
  static gen operator_times(const gen & a, const gen & b, unsigned t, GIAC_CONTEXT) {
    ...
    case _INT___VECT:
      ...
      if (warnpy && a.val >= 0 && python_compat(contextptr)) {
        alert(gettext("Python compatibility, integer*list will do vector multiplication, run "
                      "list*integer to duplicate list"),
              contextptr);
        warnpy = false;
      }
    case _ZINT__VECT:
    ...
    case _VECT__VECT: {
      ...
      if (A.subtype == _SET__VECT && B.subtype == _SET__VECT) {
        ...
        const_iterateur at = A._VECTptr->begin(), aend = A._VECTptr->end(),
                        bt = B._VECTptr->begin(), bend = B._VECTptr->end();
        for (; at != aend; ++at) {
      ...
      if (A.is_approx() && !is_fully_numeric(B)) {
        ...
        if (!A._VECTptr->empty() && A._VECTptr->back().type == _VECT &&
            !A._VECTptr->back()._VECTptr->empty() &&
            A._VECTptr->back()._VECTptr->front().type == _REAL) {
          if (real_interval * ptr =
                  dynamic_cast<real_interval *>(A._VECTptr->back()._VECTptr->front()._REALptr)) {
      ...
      if ( // abs_calc_mode(contextptr)==38 &&
          (A.subtype == _MATRIX__VECT || B.subtype == _MATRIX__VECT) && ckmatrix(A) &&
          ckmatrix(B)) {
    ...
    case _MOD__MOD:
#ifdef SMARTPTR64
      return modmul((ref_modulo *)(*((ulonglong *)&a) >> 16),
                    (ref_modulo *)(*((ulonglong *)&b) >> 16));
#else
    ...
    if (exponent.is_symb_of_sommet(at_prod)) {
      ...
      if (f.type == _VECT) {
        ...
        if (allow_recursion && perhapsone) {
          ...
          if (!is_undef(num1) && num1 != num)
            return minus1pow(symb_prod(num1, symb_inv(den)), contextptr, false);
        }
#endif
        if (even && perhapsone)
          return 1;
  ...
  gen pow(const gen & base, const gen & exponent, GIAC_CONTEXT) {
      ...
      return exponent;
    {
      ...
      if (is_algebraic_program(base, a, b))
        return symbolic(at_program,
                        gen(makevecteur(a, 0, pow(b, exponent, contextptr)), _SEQ__VECT));
    ...
    if (base.type == _VECT && base.subtype != _POLY1__VECT && !is_squarematrix(base)) {
      *logptr(contextptr) << gettext("Warning, ^ is ambiguous on non square matrices. Use .^ to "
      ...
      if (base.subtype != _LIST__VECT && (exponent.type == _INT_ && exponent.val % 2 == 0))
        return pow(dotvecteur(*base._VECTptr, *base._VECTptr, contextptr), exponent.val / 2,
                   contextptr);
    ...
    if (exponent.type == _FRAC) {
      ...
      if (exponent._FRACptr->num == 1 && exponent._FRACptr->den == 2 && base.type == _SYMB) {
        ...
        if (v.size() == 1 && v.front().is_symb_of_sommet(at_pow) &&
          ...
          if (is_linear_wrt(base, v.front(), b, a, contextptr) &&
              (is_integer(a) || a.type == _FRAC) && (is_integer(b) || b.type == _FRAC)) {
            gen d = a * a - b * b * c;
      ...
      return pow(
          base,
          new_ref_symbolic(symbolic(
              at_prod, makesequence(exponent._FRACptr->num, symb_inv(exponent._FRACptr->den)))),
          contextptr);
    }
    if (is_inf(base)) {
    ...
    if (base.type == _SYMB) {
      ...
      if (u == at_unit) {
        ...
        if (w.size() == 1)
          ...
          v1 = symbolic(at_prod, gen(w, _SEQ__VECT));
        return new_ref_symbolic(
            symbolic(at_unit, makenewvecteur(pow(v[0], exponent, contextptr), v1)));
      }
      ...
      if (u == at_pow && !has_i(base)) {
        ...
        if (v1.type == _INT_ && v1.val % 2 == 0 && (new_exp.type != _INT_ || new_exp.val % 2) &&
      ...
      if (u == at_equal || u == at_equal2) {
        ...
        return new_ref_symbolic(
            symbolic(base._SYMBptr->sommet, makesequence(pow(vb.front(), exponent, contextptr),
    ...
    if (abs_calc_mode(contextptr) == 38 && !complex_mode(contextptr) && is_exactly_zero(base) &&
        is_exactly_zero(
            exponent)) // was is_zero(,.contextptr) changed so that MINREAL^0 is not undef but 1
      return undef;
    ...
    case _DOUBLE___INT_:
      ...
      return std::pow(base._DOUBLE_val, exponent.val);
        ...
        return (exponent.val % 2 ? -1 : 1) *
               std::pow(-base._DOUBLE_val,
                        exponent.val); //exp(exponent*log(-base,contextptr),contextptr);
    case _INT___DOUBLE_:
#ifdef _SOFTMATH_H
    ...
    default:
      ...
      if (exponent.is_symb_of_sommet(at_prod) && exponent._SYMBptr->feuille.type == _VECT &&
        ...
        if (e1.is_symb_of_sommet(at_ln) && e2.is_symb_of_sommet(at_inv) &&
            e2._SYMBptr->feuille.is_symb_of_sommet(at_ln) &&
            base == e2._SYMBptr->feuille._SYMBptr->feuille)
      ...
      if (base.type == _REAL || base.type == _DOUBLE_ ||
          ...
          base.type == _FLOAT_ ||
          ((base.type < _POLY || base.type == _FLOAT_) &&
           (exponent.type == _REAL || exponent.type == _DOUBLE_ || exponent.type == _FLOAT_)))
        return exp(exponent * log(base, contextptr), contextptr);
      /*
	...
	if (abs_calc_mode(contextptr)==38 && !complex_mode(contextptr))
      ...
      if ((exponent.type == _SYMB) && (exponent._SYMBptr->sommet == at_prod)) {
        ...
        gen superexponent(1);
        const_iterateur it = exponent._SYMBptr->feuille._VECTptr->begin(),
                        itend = exponent._SYMBptr->feuille._VECTptr->end();
        for (; it != itend; ++it) {
          if (it->type == _INT_) {
        ...
        if (superexponent.type != _INT_ ||
          ...
          return new_ref_symbolic(
              symbolic(at_pow, gen(makenewvecteur(base, exponent), _SEQ__VECT)));
        if (subexponent_deno.type != _INT_ ||
            (absint(subexponent_deno.val) > MAX_ALG_EXT_ORDER_SIZE && !lidnt(base).empty())) {
          if (is_one(superexponent))
            return new_ref_symbolic(symbolic(
                at_pow, gen(makenewvecteur(base, _FRAC2_SYMB(subexponent_num, subexponent_deno)),
                            _SEQ__VECT)));
          return new_ref_symbolic(symbolic(
              at_pow, gen(makenewvecteur(
                              new_ref_symbolic(symbolic(
                                  at_pow, gen(makenewvecteur(base, _FRAC2_SYMB(subexponent_num,
                                                                               subexponent_deno)),
                                              _SEQ__VECT))),
                              superexponent),
                          _SEQ__VECT)));
        ...
        if (r) {
          if (complex_mode(contextptr) &&
            ...
            res = new_ref_symbolic(
                      symbolic(at_pow, gen(makenewvecteur(pow(base1, subexponent_num, contextptr),
                                                          inv(subexponent_deno, contextptr)),
                                           _SEQ__VECT))) *
                  res;
          } else
            res = new_ref_symbolic(
                symbolic(at_pow, gen(makenewvecteur(pow(base, subexponent_num, contextptr),
                                                    inv(subexponent_deno, contextptr)),
                                     _SEQ__VECT)));
          if (r != 1)
        ...
        if (q == 1) {
          if (is_one(subexponent_num))
            return res * base;
          return res * new_ref_symbolic(symbolic(
                           at_pow, gen(makenewvecteur(base, subexponent_num), _SEQ__VECT)));
        }
        if (q == -1)
          return res * inv(pow(base, subexponent_num, contextptr), contextptr);
        return res * new_ref_symbolic(symbolic(
                         at_pow, gen(makenewvecteur(pow(base, subexponent_num, contextptr), q),
                                     _SEQ__VECT)));
      }
      gen var1, var2, res1, res2;
      if (is_algebraic_program(base, var1, res1) && is_algebraic_program(exponent, var2, res2)) {
        ...
        if (var1 == var2)
          return symbolic(at_program,
                          gen(makevecteur(var1, 0, pow(res1, res2, contextptr)), _SEQ__VECT));
      }
      if (exponent.type == _ZINT) {
        if (base.type == _USER)
  ...
  gen sym_mult(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    if (is_inequality(a) && !is_equal(a)) {
      ...
      if (bs == -1)
        return new_ref_symbolic(
            symbolic(a._SYMBptr->sommet, makesequence(a._SYMBptr->feuille._VECTptr->back() * b,
                                                      a._SYMBptr->feuille._VECTptr->front() * b)));
      if (bs == 1)
        return new_ref_symbolic(
            symbolic(a._SYMBptr->sommet, makesequence(a._SYMBptr->feuille._VECTptr->front() * b,
    ...
    if (is_inequality(b) && !is_equal(b)) {
      ...
      if (bs == -1)
        return new_ref_symbolic(
            symbolic(b._SYMBptr->sommet, makesequence(a * b._SYMBptr->feuille._VECTptr->back(),
                                                      a * b._SYMBptr->feuille._VECTptr->front())));
      if (bs == 1)
        return new_ref_symbolic(
            symbolic(b._SYMBptr->sommet, makesequence(a * b._SYMBptr->feuille._VECTptr->front(),
    ...
    if (b.is_symb_of_sommet(at_unit)) {
      ...
      if (lidnt(a).empty()) {
        vecteur & v = *b._SYMBptr->feuille._VECTptr;
        return new_ref_symbolic(
            symbolic(at_unit, makenewvecteur(operator_times(a, v[0], contextptr), v[1])));
      }
    }
    gen var1, var2, res1, res2;
    if (is_algebraic_program(a, var1, res1)) {
      ...
      if (!is_constant_wrt(b, var1, contextptr))
        *logptr(contextptr) << "Warning function*constant with constant dependent of mute variable"
                            << '\n';
      return symbolic(at_program,
                      gen(makevecteur(var1, 0, operator_times(res1, b, contextptr)), _SEQ__VECT));
    }
    if (is_algebraic_program(b, var2, res2)) {
      if (!is_constant_wrt(a, var2, contextptr))
        *logptr(contextptr) << "Warning constant*function with constant dependent of mute variable"
                            << '\n';
      return symbolic(at_program,
                      gen(makevecteur(var2, 0, operator_times(a, res2, contextptr)), _SEQ__VECT));
    }
    if (is_inf(a)) {
    ...
    if ((a.type == _SYMB) && equalposcomp(plot_sommets, a._SYMBptr->sommet)) {
      ...
      return symbolic_plot_makevecteur(a._SYMBptr->sommet, a._SYMBptr->feuille * b, false,
                                       contextptr);
    }
    if ((b.type == _SYMB) && equalposcomp(plot_sommets, b._SYMBptr->sommet)) {
      ...
      if (b_.is_symb_of_sommet(at_curve) && b_._SYMBptr->feuille.type == _VECT &&
          b_._SYMBptr->feuille._VECTptr->size() == 2 &&
          b_._SYMBptr->feuille._VECTptr->front().type == _VECT) {
        // adjust param and cartesian eq
        ...
        if (v.size() >= 6) {
          ...
          v[5] =
              subst(v[5], makevecteur(x__IDNT_e, y__IDNT_e),
                    makevecteur(ax * x__IDNT_e - ay * y__IDNT_e, ax * y__IDNT_e + ay * x__IDNT_e),
                    false, contextptr);
          b_ = symbolic(at_curve,
                        gen(makevecteur(gen(v, b_._SYMBptr->feuille._VECTptr->front().subtype),
                                        b_._SYMBptr->feuille._VECTptr->back()),
                            b_._SYMBptr->feuille.subtype));
        }
      }
      return symbolic_plot_makevecteur(b_._SYMBptr->sommet, b_._SYMBptr->feuille * a, false,
                                       contextptr);
    }
    if (a.is_symb_of_sommet(at_neg)) {
      if (b.is_symb_of_sommet(at_neg))
    ...
    if (is_equal(a)) {
      ...
      if (is_equal(b)) {
        ...
        return new_ref_symbolic(symbolic(
            a._SYMBptr->sommet,
            gen(makenewvecteur(va.front() * vb.front(), va.back() * vb.back()), _SEQ__VECT)));
      } else
        return new_ref_symbolic(symbolic(
            a._SYMBptr->sommet, gen(makenewvecteur(va.front() * b, va.back() * b), _SEQ__VECT)));
    }
    if (is_equal(b)) {
      vecteur & vb = *b._SYMBptr->feuille._VECTptr;
      return new_ref_symbolic(symbolic(
          b._SYMBptr->sommet, gen(makenewvecteur(a * vb.front(), a * vb.back()), _SEQ__VECT)));
    }
    if ((a.type == _SYMB) && (b.type == _SYMB)) {
    ...
    if (a.type == _SYMB) {
      if (a._SYMBptr->sommet == at_prod)
        ...
        return new_ref_symbolic(
            symbolic(at_prod, gen(b.type == _INT_ ? makenewvecteur(b, a) : makenewvecteur(a, b),
                                  _SEQ__VECT)));
    }
    if ((a.type == _IDNT) || (b.type == _IDNT))
      return new_ref_symbolic(symbolic(at_prod, gen(makenewvecteur(a, b), _SEQ__VECT)));
  ...
  gen inv(const gen & a, GIAC_CONTEXT) {
    ...
    case _CPLX:
      ...
      if (a._CPLXptr->type == _DOUBLE_ || a._CPLXptr->type == _FLOAT_ ||
          a._CPLXptr->type == _REAL || (a._CPLXptr + 1)->type == _DOUBLE_ ||
          (a._CPLXptr + 1)->type == _FLOAT_ || (a._CPLXptr + 1)->type == _REAL) {
        gen a2 = no_context_evalf(a.squarenorm(contextptr));
        if (is_inf(a2)) {
          gen theta = arg(a, contextptr);
          theta = gen(cos(theta, contextptr),
                      -sin(theta, contextptr)); //cos(theta,contextptr)-cst_i*sin(theta,contextptr);
    ...
    case _SYMB:
      ...
      if (a.is_symb_of_sommet(at_unit)) {
        ...
        return new_ref_symbolic(symbolic(
            at_unit,
            makenewvecteur(inv(a._SYMBptr->feuille._VECTptr->front(), contextptr),
      ...
      if (a._SYMBptr->sommet == at_neg)
        ...
        if (a._SYMBptr->sommet == at_prod)
          return new_ref_symbolic(
              symbolic(at_prod, gen(inv__VECT(*(a._SYMBptr->feuille._VECTptr), contextptr),
                                    a._SYMBptr->feuille.subtype)));
        else
  ...
  */
  ...
  gen gen::inverse(GIAC_CONTEXT) const {
    return inv(*this, contextptr);
  }
  ...
  gen pow(const gen & base, unsigned long int exponent) {
    ...
    case _SYMB:
      ...
      if (base._SYMBptr->sommet == at_pow) {
        ...
        return pow((base._SYMBptr->feuille)._VECTptr->front(), gen((longlong)(exponent)) * res,
                   context0);
      }
  ...
  gen pow(const gen & base, int exponent) {
    ...
    if (base.type <= _ZINT && has_evalf(base, b, 0, context0) && !is_inf(b) &&
        is_greater(abs(exponent * log(abs(b, context0), context0), context0), powlog2float,
                   context0)) {
  ...
  gen rdiv(const gen & a, const gen & b, GIAC_CONTEXT) {
...
#ifdef HAVE_LIBMPFR
        return rdiv(*a._CPLXptr, b, contextptr) +
               cst_i *
                   rdiv(real_object(*(a._CPLXptr + 1), mpfr_get_prec(a._CPLXptr->_REALptr->inf)), b,
                        contextptr);
#else
      return rdiv(*a._CPLXptr, b, contextptr) +
             cst_i * rdiv(real_object(*(a._CPLXptr + 1)), b, contextptr);
#endif
      }
      if ((a._CPLXptr + 1)->type == _REAL) {
#ifdef HAVE_LIBMPFR
        return rdiv(real_object(*a._CPLXptr, mpfr_get_prec((a._CPLXptr + 1)->_REALptr->inf)), b,
                    contextptr) +
               cst_i * rdiv(*(a._CPLXptr + 1), b, contextptr);
#else
    ...
    case _CPLX__CPLX:
      if (a.subtype == 3 || b.subtype == 3)
        return a * inv(b, contextptr);
      return adjust_complex_display(
          rdiv(a * conj(b, contextptr), b.squarenorm(contextptr), contextptr), a, b);
    case _DOUBLE___CPLX:
    case _FLOAT___CPLX:
    case _INT___CPLX:
    case _ZINT__CPLX:
    ...
    case _VECT__VECT:
      ...
      if (is_squarematrix(b)) {
        if (abs_calc_mode(contextptr) == 38) {
          *logptr(contextptr) << gettext("Warning: A/B with B a square matrix is a misleading "
                                         "notation interpreted as inv(B)*A")
                              << '\n';
          return inv(b, contextptr) * a;
        }
        *logptr(contextptr) << gettext("Warning, pointwise division of a by b. For matrix "
                                       "division, please use inv(b)*a or a*inv(b)")
                            << '\n';
      }
    ...
    case _FRAC__FRAC:
      if (a._FRACptr->num.type == _CPLX || a._FRACptr->den.type == _CPLX ||
        ...
        return (a._FRACptr->num * (b._FRACptr->den / d)) /
               ((a._FRACptr->den / d) * b._FRACptr->num);
      }
      return (*a._FRACptr) / (*b._FRACptr);
    case _SPOL1__SPOL1:
    ...
    default:
        ...
        return gensizeerr("string /");
      {
        ...
        if (is_algebraic_program(a, var1, res1)) {
          ...
          if (!is_constant_wrt(b, var1, contextptr))
            *logptr(contextptr)
                << "Warning function/constant with constant dependent of mute variable" << '\n';
          return symbolic(at_program,
                          gen(makevecteur(var1, 0, rdiv(res1, b, contextptr)), _SEQ__VECT));
        }
        if (is_algebraic_program(b, var2, res2)) {
          if (!is_constant_wrt(a, var2, contextptr))
            *logptr(contextptr)
                << "Warning constant/function with constant dependent of mute variable" << '\n';
          return symbolic(at_program,
                          gen(makevecteur(var2, 0, rdiv(a, res2, contextptr)), _SEQ__VECT));
        }
      }
      if (a.type == _FLOAT_)
      ...
      if (is_equal(a)) {
        ...
        if (is_equal(b)) {
          vecteur & vb = *b._SYMBptr->feuille._VECTptr;
          return new_ref_symbolic(
              symbolic(a._SYMBptr->sommet, makesequence(rdiv(va.front(), vb.front(), contextptr),
                                                        rdiv(va.back(), vb.back(), contextptr))));
        } else
          return new_ref_symbolic(
              symbolic(a._SYMBptr->sommet, makesequence(rdiv(va.front(), b, contextptr),
                                                        rdiv(va.back(), b, contextptr))));
      }
      if (is_equal(b)) {
        vecteur & vb = *b._SYMBptr->feuille._VECTptr;
        return new_ref_symbolic(
            symbolic(b._SYMBptr->sommet, makesequence(rdiv(a, vb.front(), contextptr),
  ...
  bool is_positive(const gen & a, GIAC_CONTEXT) {
    ...
    case _FRAC:
      return (is_positive(a._FRACptr->num, contextptr) &&
              is_positive(a._FRACptr->den, contextptr)) ||
             (is_positive(-a._FRACptr->num, contextptr) &&
              is_positive(-a._FRACptr->den, contextptr));
    case _EXT:
      return false;
    case _SYMB:
  ...
  bool operator_equal(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    case _INT___CPLX:
    case _ZINT__CPLX:
      return (operator_equal(a, re(b, contextptr), contextptr) &&
              is_zero(im(b, contextptr), contextptr));
    case _CPLX__ZINT:
    case _CPLX__INT_:
      return (operator_equal(re(a, contextptr), b, contextptr) &&
    ...
    case _DOUBLE___DOUBLE_:
      ...
      return absdouble(a._DOUBLE_val - b._DOUBLE_val) <
             absdouble(a._DOUBLE_val) * epsilon(contextptr);
    case _IDNT__IDNT:
    ...
    case _EXT__EXT:
      return (change_subtype(*a._EXTptr, _POLY1__VECT) ==
                  change_subtype(*b._EXTptr, _POLY1__VECT) &&
    ...
    default: // Check pointers, type subtype
      ...
      if (a.type == _INT_ && a.subtype == _INT_TYPE && b.type == _FUNC) {
        if (a == _STRNG && b == at_string)
          return true;
        if (a == _VECT && b == at_vector)
          return true;
        if (a == _FLOAT_ && b == at_float)
          return true;
        if (a == _DOUBLE_ && b == at_real)
          return true;
        if (a == _CPLX && b == at_complex)
          return true;
        if ((a == _INT_ || a == _ZINT) && b == at_int)
          return true;
      }
  ...
  bool operator==(const gen & a, const identificateur & i) {
    return a.type == _IDNT &&
           (a._IDNTptr->id_name == i.id_name || !strcmp(a._IDNTptr->id_name, i.id_name));
  }

  bool identificateur::operator==(const gen & i) {
    return i.type == _IDNT &&
           (id_name == i._IDNTptr->id_name || !strcmp(id_name, i._IDNTptr->id_name));
  }
  ...
  gen sign(const gen & a, GIAC_CONTEXT) {
    ...
    if (eps > 1e-6)
      *logptr(contextptr)
          << gettext(
                 "Warning, sign might return 0 incorrectly because the value of eps is too large ")
          << eps << endl;
    switch (a.type) {
    case _INT_:
    case _ZINT:
  ...
  static gen sym_is_greater(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    if (is_equal(a) && is_equal(b)) {
      ...
      if (af.type == _VECT && bf.type == _VECT && af._VECTptr->size() == 2 &&
          bf._VECTptr->size() == 2 && af._VECTptr->front() == bf._VECTptr->front())
  ...
  gen superieur_strict(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    case _FRAC__FRAC:
      if (is_positive(a._FRACptr->den, contextptr) && is_positive(b._FRACptr->den, contextptr))
        return superieur_strict(a._FRACptr->num * b._FRACptr->den,
                                a._FRACptr->den * b._FRACptr->num, contextptr);
      break;
    case _FRAC__INT_:
...
#if 1
    if ((a.type == _REAL && b.type <= _REAL) || (b.type == _REAL && a.type <= _REAL)) {
  ...
  gen gen::operator_at(const gen & i, GIAC_CONTEXT) const {
      ...
      if (i._SYMBptr->sommet == at_interval || ideuxpoints) {
        ...
        if (is_integral(i1) && is_integral(i2)) {
          ...
          if (debut < 0 || step * double(fin - debut) < 0)
            return (type == _STRNG) ? string2gen("", false)
                                    : gen(vecteur(0), subtype); // swap(debut,fin);
          if (step == 1) {
            if (type == _STRNG)
    ...
    if (i.type == _VECT) {
      ...
      for (; it != itend; ++it) {
        ...
        if ((it->type == _SYMB) && (it->_SYMBptr->sommet == at_interval || itdeuxpoints) &&
            (it + 1 != itend)) {
          // submatrix extraction
          gen i1 = it->_SYMBptr->feuille._VECTptr->front(),
              iback = it->_SYMBptr->feuille._VECTptr->back();
          int step = 1;
  ...
  */
  ...
  gen gen::operator()(const gen & i, const gen & progname, const context * contextptr) const {
    bool isprog = type == _FUNC || this->is_symb_of_sommet(at_program) ||
                  this->is_symb_of_sommet(*at_program);
    if (!isprog) {
    ...
    if (type == _SYMB) {
      ...
      if (_SYMBptr->sommet == at_function_diff || _SYMBptr->sommet == at_of ||
      ...
      if (lid.size() == 1 && !has_algebraic_program(*this)) {
        if (lid.front() == vx_var || lid.front() == t__IDNT_e)
          ...
          *logptr(contextptr) << "Warning, evaluating univariate expression of x(value) like if "
                                 "expression was a function.\nYou should write subst("
                              << *this << "," << lid.front() << "," << i << ")" << endl;
        else
          return gensizeerr("Expression used like a function " + this->print(contextptr) +
                            "\nYou should write subst(" + this->print(contextptr) + "," +
                            lid.front().print(contextptr) + "," + i.print(contextptr) + ")");
      ...
      if (warn)
        *logptr(contextptr) << gettext("Warning, evaluating (") << *this << ")(" << i
                            << ") as a function not as a product" << endl;
        ...
        if (i.is_symb_of_sommet(at_interval)) {
          ...
          if (ife.type == _VECT && ife._VECTptr->size() == 2) {
            ...
            if (if1.type == _INT_ && if2.type == _INT_ && if1.val >= 1 && if2.val >= 1 &&
                if1.val <= if2.val && if1.val <= int(_VECTptr->size()) &&
                if2.val <= int(_VECTptr->size()))
              return gen(vecteur(_VECTptr->begin() + if1.val - 1, _VECTptr->begin() + if2.val),
                         subtype);
          }
        }
  ...
  static gen monomial_degree(const gen & a) {
    ...
    if (a.type == _IDNT) {
      ...
      if (strlen(a._IDNTptr->id_name) >= 3 && a._IDNTptr->id_name[0] == 'c' &&
          a._IDNTptr->id_name[1] == '_')
        return 0;
      return 1;
    }
    if (a.type != _SYMB)
  ...
  static bool is_monomial(const gen & a) {
    ...
    if (a._SYMBptr->sommet == at_pow)
      return true;
    if (a._SYMBptr->sommet != at_prod && a._SYMBptr->sommet != at_plus &&
        a._SYMBptr->sommet != at_neg && a._SYMBptr->sommet != at_inv)
      return false;
    gen af = a._SYMBptr->feuille;
    if (af.type != _VECT)
  ...
  static bool islesscomplexthanf2(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    if (is_monomial(a) && is_monomial(b)) {
      ...
      if (da != db)
        return increasing_power(contextptr) ? is_greater(db, da, contextptr)
                                            : is_greater(da, db, contextptr);
    }
    if (a.type == b.type)
      return a.islesscomplexthan(b);
  ...
  struct f_compare_context {
    ...
    f_compare_context(bool (*f_)(const gen & a, const gen & b, GIAC_CONTEXT), GIAC_CONTEXT)
        : f(f_), ptr(contextptr) {}
    inline bool operator()(const gen & a, const gen & b) {
      return f(a, b, ptr);
    }
  ...
  void gen_sort_f_context(iterateur it, iterateur itend,
                          bool (*f)(const gen & a, const gen & b, GIAC_CONTEXT), GIAC_CONTEXT) {
  ...
  public:
    modified_compare() {}
    bool operator()(const gen & a, const gen & b) {
      return modified_islesscomplexthanf(a, b);
    }
  };
  ...
  static vecteur terme2unitaire(const gen & x, bool sorted, GIAC_CONTEXT) {
    ...
    if (x._SYMBptr->sommet == at_prod && (tmp = x._SYMBptr->feuille).type == _VECT &&
        !tmp._VECTptr->empty()) {
      ...
      for (unsigned i = 0; i < vtmp.size(); ++i) {
        ...
        if (vtmp[i].is_symb_of_sommet(at_inv) &&
            vtmp[i]._SYMBptr->feuille.is_symb_of_sommet(at_pow)) {
    ...
    if (x._SYMBptr->sommet == at_pow)
      return makevecteur(plus_one,
                         x._SYMBptr->sommet(collect(x._SYMBptr->feuille, contextptr), contextptr));
  ...
  static vecteur fusionliste(const vecteur & v) {
    ...
    for (; it != itend; ++it) {
      if ((*it)[1].type != current.type ||
          (current.type == _DOUBLE_ && current._DOUBLE_val != (*it)[1]._DOUBLE_val) ||
          (*it)[1] != current) {
  ...
  struct tri_context {
    const context * contextptr;
    bool operator()(const gen & a, const gen & b) {
      return islesscomplexthanf2(a, b, contextptr);
    }
...
bool my_mpz_invert(mpz_t & ainv, const mpz_t & a, const mpz_t & m) {
  ...
  static gen symgcd(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    if (a == b)
      return a;
    if ((a.type == _MOD) && (b.type == _MOD) && (a._MODptr->type <= _CPLX) &&
        (b._MODptr->type <= _CPLX))
      return chkmod(plus_one, a);
    if (a.type == _MOD || b.type == _MOD || a.type == _DOUBLE_ || a.type == _FLOAT_ ||
        a.type == _REAL || b.type == _DOUBLE_ || b.type == _FLOAT_ || b.type == _REAL)
      return plus_one;
    if ((a.type == _POLY) && (b.type == _POLY))
      return polygcd(*a._POLYptr, *b._POLYptr);
    if ((a.type == _EXT) && (b.type == _EXT)) {
      if ((*(a._EXTptr + 1) != *(b._EXTptr + 1)) || (a._EXTptr->type != _VECT) ||
          (b._EXTptr->type != _VECT))
        return plus_one;
  ...
  gen simplify(gen & n, gen & d) {
    if ((d.type == _DOUBLE_) || ((d.type == _CPLX) && ((d._CPLXptr->type == _DOUBLE_) ||
    ...
    if ((n.type == _DOUBLE_) || ((n.type == _CPLX) && ((n._CPLXptr->type == _DOUBLE_) ||
  ...
  gen invmod(const gen & a, const gen & modulo) {
    ...
    if (a.type == _MOD) {
      if (*(a._MODptr + 1) != modulo)
        return gensizeerr("Incompatible modulo " + a.print(context0) + "," +
                          modulo.print(context0));
      return inv(a, context0);
    }
    if (a.type == _CPLX) {
      gen r = re(a, context0), i = im(a, context0); // ok
      gen n = invmod(r * r + i * i, modulo);
      return gen(smod(r * n, modulo),
                 -smod(i * n, modulo)); //smod(r*n,modulo)-cst_i*smod(i*n,modulo);
    }
    if (a.type == _POLY)
      return fraction(1, a);
    ref_mpz_t * res;
  ...
  bool in_fracmod(const gen & m, const gen & a, mpz_t & d, mpz_t & d1, mpz_t & absd1, mpz_t & u,
  ...
  bool alloc_fracmod(const gen & a_orig, const gen & modulo, gen & res, mpz_t & d, mpz_t & d1,
                     mpz_t & absd1, mpz_t & u, mpz_t & u1, mpz_t & ur, mpz_t & q, mpz_t & r,
                     mpz_t & sqrtm, mpz_t & tmp) {
    ...
    if (a_orig.type == _POLY) {
      vector<monomial<gen>>::const_iterator it = a_orig._POLYptr->coord.begin(),
                                            itend = a_orig._POLYptr->coord.end();
    ...
    if (a_orig.type == _CPLX) {
      gen reres, imres;
      if (!alloc_fracmod(*a_orig._CPLXptr, modulo, reres, d, d1, absd1, u, u1, ur, q, r, sqrtm,
                         tmp) ||
          !alloc_fracmod(*(a_orig._CPLXptr + 1), modulo, imres, d, d1, absd1, u, u1, ur, q, r,
                         sqrtm, tmp))
        return false;
  ...
  static void _ZINTpowmod(const gen & base, const gen & expo, const gen & modulo,
                          ref_mpz_t *& res) {
    ref_mpz_t *aptr, *bptr;
    if (base.type)
#ifdef SMARTPTR64
  ...
  longlong ll = strtoll(s, &endchar, base);
    ...
    if (!errno) {
      ...
      if (ll == int(ll) || ((base == 2 || base == 8 || base == 16) && ll == (unsigned int)(ll)))
        return gen(int(ll));
      else
...
#ifdef SMARTPTR64
    *((ulonglong *)&res) =
        ulonglong(new ref_string(remove_ss_quotes ? ss.substr(1, ss.size() - 2) : ss)) << 16;
#else
          ...
          if (!v.empty()) {
            ...
            for (i = 0; i < vs; i++) {
              ...
              if (contains(args._VECTptr->front(), i__IDNT_e)) {
                *logptr(contextptr)
                    << gettext("Warning, i is usually sqrt(-1), I'm using a symbolic variable "
                               "instead but you should check your input")
                    << endl;
                return res;
              }
            }
  ...
  static gen aplatir_plus(const gen & g) {
    ...
    if (g.is_symb_of_sommet(at_plus) && g._SYMBptr->feuille.type == _VECT) {
      iterateur it = g._SYMBptr->feuille._VECTptr->begin(),
  ...
  gen aplatir_fois_plus(const gen & g) {
    ...
    if (g._SYMBptr->sommet == at_prod && f.type == _VECT && f._VECTptr->size() == 2)
      return sym_mult(aplatir_fois_plus(f._VECTptr->front()), aplatir_fois_plus(f._VECTptr->back()),
                      context0);
  ...
  static gen aplatir_plus_only(const gen & g) {
    ...
    return gen((symbolic(g._SYMBptr->sommet, aplatir_plus_only(g._SYMBptr->feuille))))
        .change_subtype(g.subtype);
  }
  ...
  */
  ...
  gen maptoarray(const gen_map & m, GIAC_CONTEXT) {
    ...
    for (unsigned i = 0; i < ps; ++i) {
      res.push_back(symb_interval(
          pv[i] + array_start(contextptr),
          lv[i] +
              array_start(
  ...
  static string print_EQW(const eqwdata & e) {
    string s;
    s += "eqwdata(position" + print_INT_(e.x) + "," + print_INT_(e.y) + ",dxdy," +
         print_INT_(e.dx) + "," + print_INT_(e.dy);
  ...
  string gen::print(GIAC_CONTEXT) const {
      ...
      if (*complex_display_ptr(*this) & 1) {
        ...
        if (_CPLXptr->type == _FLOAT_ && (_CPLXptr + 1)->type == _FLOAT_)
          ...
          return abs(*this, contextptr).print(contextptr) + "\xe2\x88\xa1" +
                 print_FLOAT_(atan2f(_CPLXptr->_FLOAT_val, (_CPLXptr + 1)->_FLOAT_val,
                                     angle_radian(contextptr)
                                         ? AMRad
                                         : (angle_degree(contextptr) ? AMDeg : AMGrad)),
                              contextptr);
#else
          return abs(*this, contextptr).print(contextptr) + "\xe2\x88\xa1" +
                 print_FLOAT_(atan2f(_CPLXptr->_FLOAT_val, (_CPLXptr + 1)->_FLOAT_val,
    ...
    if ((_MODptr->type == _SYMB && _MODptr->_SYMBptr->sommet != at_pow) ||
        (_MODptr->type == _VECT && _MODptr->subtype == _SEQ__VECT))
      return "(" + _MODptr->print(contextptr) + ")" +
             (python_compat(contextptr) ? " mod " : " % ") + (*(_MODptr + 1)).print(contextptr);
    ...
    case _FUNC:
      ...
      if (rpn_mode(contextptr) || _FUNCptr->ptr()->printsommet == &printastifunction ||
          subtype == 0)
    ...
    case _POINTER_: {
      ...
      if (u < (1U << 31))
        return "pointer(" + hexa_print_INT_(int((alias_type)_POINTER_val)) + "," +
               print_INT_(subtype) + ")";
    }
    default:
#ifndef NO_STDEXCEPT
...
#ifndef NSPIRE
  ostream & operator<<(ostream & os, const gen & a) {
...
istream & operator>>(istream & is, gen & a) {
  ...
  real_object::real_object(const gen & g, unsigned int precision) {
    ...
    set_decimal_digits(giacmax(20, std::ceil(precision * 0.30102999566398119
                                             //M_LN2/M_LN10
                                             )),
                       context0);
  ...
  gen real_object::addition(const gen & g, GIAC_CONTEXT) const {
    ...
    return gensizeerr(gettext("real_object + gen") + this->print(contextptr) + "," +
                      g.print(contextptr));
  }
  ...
  static void compile_with_mpfr() {
    setsizeerr(gettext(
        "Compile with MPFR or USE_GMP_REPLACEMENTS if you want transcendental long float support"));
  }
  ...
  gen real_object::multiply(const gen & g, GIAC_CONTEXT) const {
    ...
    case _REAL:
      return *this * *g._REALptr;
    case _CPLX:
      return gen(this->multiply(*g._CPLXptr, contextptr),
                 this->multiply(*(g._CPLXptr + 1), contextptr));
    case _FRAC:
      if (!is_integer(g._FRACptr->num) || !is_integer(g._FRACptr->den))
  ...
  g = equaltosto(g, &C);
  if (g.type == _VECT && !g._VECTptr->empty() &&
      g._VECTptr->front().is_symb_of_sommet(at_set_language)) {
    vecteur v = *g._VECTptr;
  ...
  if (push) {
    ...
    if (calc_mode(&C) != 1 &&
        (last.is_symb_of_sommet(at_pnt) || last.is_symb_of_sommet(at_pixon))) {
      ...
      if (is3d(last)) {
        ...
        if (worker == -1)
          return "gl3d_not_supported";
        if (worker)
          return "gl3d_not_supported_if_workers_are_enabled";
      ...
      if (last.is_symb_of_sommet(at_pnt)) {
        ...
        if (os_shell) {
          if (islogo(g))
            ...
            if ((g.type == _SYMB || (warn_symb_program_sto && g.type == _VECT)) &&
                taille(g, 256) <= 256)
              g = xcas::eqw(g, true, &C);
          }
        }
