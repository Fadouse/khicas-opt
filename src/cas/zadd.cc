...
inline int os_draw_string_small_(int x, int y, const char * s) {
  return os_draw_string_small(x, y, FL_BLACK, FL_WHITE, s, false);
}

inline int os_draw_string_(int x, int y, const char * s) {
  return os_draw_string(x, y, 0, 65535, s, false);
}
...
namespace giac {
  ...
  define_unary_function_ptr5(at_current_sheet, alias_at_current_sheet, &__current_sheet,
...
unsigned short mmind_col[] = {COLOR_BLUE,  COLOR_RED,  COLOR_MAGENTA,
                              COLOR_GREEN, COLOR_CYAN, COLOR_YELLOW};

void mastermind_disp(const vector<int> & solution, const vector<vector<int>> & essais,
                     const vector<int> & essai, bool fulldisp, GIAC_CONTEXT) {
  int x0 = 30, y0 = 30;
  if (fulldisp)
  ...
  if (fulldisp) {
    ...
    for (int c = 0; c < essais.size(); ++c) {
      ...
      for (int i = 0; i < 4; ++i) {
        draw_filled_circle(x0 + 20 * c + 10, y0 + 20 * i + 10, 10, mmind_col[essai[i]], true, true,
                           contextptr);
      }
      // resultats
      ...
      if (essai.size() == 4) {
        ...
        if (essais.size() == nbessais) {
          ...
          for (int i = 0; i < solution.size(); ++i)
            draw_filled_circle(30 + 20 * i + 10, 190 + 20, 10, mmind_col[solution[i]], true, true,
                               contextptr);
          confirm4(lang == 1 ? "Vous avez perdu." : "Game over.",
                   lang == 1 ? "La solution etait" : "Solution was", false, 140);
          return -1;
        }
      }
    }
...
complex<double> horner_newton(const vector<std::complex<double>> & p,
...
complex<double> horner_newton(const vector<double> & p, const std::complex<double> & x) {
  ...
  int n = itend - it - 1;
  complex<double> num = *it * x + *(it + 1),
                  den = (double(n) * (*it)) * x + double(n - 1) * (*(it + 1));
  for (it += 2, n -= 2; n; --n, ++it) {
    num *= x;
    den *= x;
    num += *it;
    ...
    for (int y = 0; y < Y; ++y) {
      ...
      if (mandel) {
        ...
        for (int x = 0; x < X; ++x) {
          ...
          if (nrac < Z.size()) {
            int r_, g_, b_;
            arc_en_ciel(25 * nrac + j, r_, g_, b_);
...
int fractale(GIAC_CONTEXT) {
  ...
  double d = 16;
  if (inputdouble(lang == 1 ? "Nombre d'iterations? (defaut 16)"
                            : "Number of iterations? (default 16)",
                  d, contextptr) &&
      d >= 1 && d <= 20)
    Nmax = d;
...
void drawAtom(uint8_t id) {
  ...
  if (atomsdefs[id].y >= 7) {
    ...
    stroke_rectangle(6 + atomsdefs[id].x * 17, 15 + atomsdefs[id].y * 17, 18, 18,
                     rgb24to16(0x525552));
    ...
    stroke_rectangle(6 + atomsdefs[id].x * 17, 6 + atomsdefs[id].y * 17, 18, 18,
                     rgb24to16(0x525552));
...
int periodic_table(const char *& name, const char *& symbol, char * protons, char * nucleons,
                   char * mass, char * electroneg) {
...
#else
      os_draw_string_small_(0, 200,
                            lang == 1 ? ("EXE: tout, P:protons, N:nucleons, M:mass, E:khi")
                                      : ("EXE: all, P:protons, N:nucleons, M:mass, E:khi"));
#endif
      for (int i = 0; i < ATOM_NUMS; i++) {
        drawAtom(i);
      }
      if (atomsdefs[cursor_pos].y >= 7) {
        stroke_rectangle(6 + atomsdefs[cursor_pos].x * 17, 15 + atomsdefs[cursor_pos].y * 17, 18,
                         18, 0x000000);
        stroke_rectangle(7 + atomsdefs[cursor_pos].x * 17, 16 + atomsdefs[cursor_pos].y * 17, 16,
                         16, 0x000000);
      } else {
...
int finance(int mode, GIAC_CONTEXT) { // mode==-1 pret, 1 placement
  ...
  while (1) {
    ...
    char * tab[6] = {(char *)pvs1.c_str(),   (char *)fvs1.c_str(), (char *)irs1.c_str(),
                     (char *)irpys1.c_str(), (char *)pms1.c_str(), (char *)nbs1.c_str()};
    for (int i = 0; i < 6; i++)
      smallmenuitems[i].text = tab[i];
    smallmenuitems[6].text = (char *)((lang == 1) ? "Quitter " : "Quit ");
    os_draw_string(0, 200, solved ? giac::_GREEN : giac::_MAGENTA, _WHITE,
                   "F1 solve |  EXE change value  | F6 help");
    ...
    if (sres == KEY_CTRL_CATALOG || sres == KEY_BOOK || sres == KEY_CTRL_F6) { // Help
      ...
      elem[0].s =
          (lang == 1)
              ? "Deplacez le curseur sur une ligne, tapez EXE/OK pour entrer une nouvelle valeur "
                "ou tapez sur Ans pour resoudre."
              : "Move cursor on a line, type EXE/OK to enter a new value or type Ans to solve";
      elem[0].newLine = 0;
      if (mode == -1)
        elem[1].s =
            (lang == 1)
                ? "Par exemple entrez le montant de l'emprunt en 1, 0 en 2, le taux d'interet, le "
                  "nombre d'annees puis placez le curseur en 5 et tapez F1 ou Ans."
                : "For example, enter due amount in 1, 0 in 2, interest rate, number of years then "
                  "move cursor on 5 and type F1 or Ans";
      else
        elem[1].s = (lang == 1)
                        ? "Pour calculer l'evolution d'un placement, entrer le montant place au "
                          "debut, le taux d'interet, le nombre d'annees, 0 en 5 (paiement) puis "
                          "deplacez le curseur en 2 et tapez F1 ou Ans"
                        : "For example, enter current value in 0, interest rate, number of years, "
                          "0 in 5 (payment) then move cursor in 2 and type F1 or Ans";
      elem[1].newLine = 1;
    ...
    if (sres == KEY_CHAR_ANS || sres == KEY_CTRL_F1) {
      ...
      if (choix == 2) { // solve for T
        giac::gen sol =
            un - pow(1 + vx_var, n, contextptr) * (u0 - gen(r) / vx_var) - gen(r) / vx_var;
    ...
    int keynumber = -1;
    if (sres >= KEY_CHAR_0 && sres <= KEY_CHAR_9)
      keynumber = sres - KEY_CHAR_0;
...
int khicas_addins_menu(GIAC_CONTEXT) {
  ...
  smallmenuitems[5].text =
      (char *)((lang == 1) ? "Exemple simple: Syracuse" : "Simple example; Syracuse");
  smallmenuitems[6].text =
      (char *)((lang == 1) ? "Exemple de jeu: Mastermind" : "Game example: Mastermind");
  ...
  smallmenuitems[smallmenu.numitems - 1].text =
      (char *)((lang == 1) ? "Revenir au shell" : "Back to shell");
  while (1) {
    int sres = doMenu(&smallmenu);
    if (sres == MENU_RETURN_SELECTION || sres == KEY_CTRL_EXE) {
      ...
      if (smallmenu.selection == 6) {
        ...
        int i;
        for (;;) {
          inputdouble(lang == 1 ? ("Suite de Syracuse. u0?") : "Syracuse sequence, u9?", d,
                      contextptr);
          i = (d);
          if (i == d)
            break;
          confirm(lang == 1 ? ("u0 doit etre entier!") : "u0 must be an integer!",
                  gettext("Recommencez"));
        }
        i = max(i, 1);
...
void flash_info(const char * buf, std::vector<fileinfo_t> & v, size_t & first_modif, bool modif,
                int initpos, GIAC_CONTEXT) {
  Menu smallmenu;
...
#else
      string msg2 =
          string("Mode: ") + (ptr + 104) + string(" mtime ") + print_INT_(fromstring8(ptr + 136));
#endif
...
int flash_from_ram(const char * buf, size_t & first_modif, GIAC_CONTEXT) {
  ...
  int n =
      fileBrowser(filename, "py", (lang == 1 ? "Choisir fichier a copier" : "Select file to copy"));
  if (n == 0)
    return 0;
...
void handle_flash(GIAC_CONTEXT) {
  const char flash_fr[] =
      "Cette application, disponible hors mode examen, permet de sauvegarder et gerer des scripts "
      "en memoire flash. Elle a besoin de 70K de memoire RAM, lancez-la tout de suite apres avoir "
      "ouvert KhiCAS.\nPour eviter une usure trop rapide de la flash, il est conseille de "
      "l'utiliser le moins souvent possible et de ne pas vider la corbeille avant que cela ne soit "
      "necessaire (ainsi les nouveaux fichiers s'ecriront sur d'autres secteurs).\nL'auteur "
      "decline toute responsabilite en cas d'usure prematuree de votre memoire flash.";
  const char flash_en[] =
      "This app (not available if exam mode is on) lets you save and handle scripts in flash "
      "memory. It requires 70K of free RAM, you should run it immediatly after launching "
      "KhiCAS.\nIn order to avoid premature wear of your flash, run this app only when required. "
      "Don't empty the trash unless it's necessary (that way new files will be written in other "
      "sectors).\nThe author declines all responsability in the event of premature wear of your "
      "flash memory.";
  textArea text;
  text.editable = false;
  text.clipline = -1;
  ...
  if (buf64k == 0) {
    confirm(lang == 1 ? "Pas assez de memoire RAM." : "RAM Memory full",
            lang == 1 ? "Purgez et relancez KhiCAS" : "Purge and restart KhiCAS");
    return;
  }
#ifndef NUMWORKS
  char * freeptr = 0;
  ...
  while (1) {
    ...
    if (sres == MENU_RETURN_SELECTION || sres == KEY_CTRL_EXE) {
      ...
      if (smallmenu.selection == 4) {
        if (numworks_maxtarsize - first_modif > 65536 &&
            do_confirm(lang == 1 ? "Il reste de la place, etes-vous sur?"
                                 : "There's still room, are you sure?"))
    ...
    for (int j = t.disp_col_begin; j < J; ++j) {
      ...
      if (vj.type == _VECT && vj._VECTptr->size() == 3) {
        ...
        if (dx < col_width)
          ...
          s = s.substr(0, 8) + "...";
          os_draw_string_small(x + 2, y + 2, _BLACK, rev ? color_gris : _WHITE, s.c_str(),
                               false); // draw
        }
      }
      x += col_width + 4;
  ...
  if (t.cmd_row >= 0 && t.cmd_pos >= 0 && t.cmd_pos <= s.size()) {
    ...
    if (has_sel) {
      ...
      if (t.cmd_row != t.cur_row || t.cmd_col != t.cur_col) {
        xend = os_draw_string(xend, sheety, _BLACK, color_gris,
                              printcell(t.cur_row, t.cur_col).c_str(), false);
    ...
    xend =
        os_draw_string(xend, sheety, _BLACK, _WHITE, printcell(t.cur_row, t.cur_col).c_str()) + 5;
...
void resizesheet(tableur & t) {
  ...
  if (nr != cur_r || nc != cur_c) {
    if (do_confirm(((lang == 1 ? "Redimensionner " : "Resize ") + print_INT_(cur_r) + "x" +
                    print_INT_(cur_c) + "->" + print_INT_(nr) + "x" + print_INT_(nc))
                       .c_str())) {
      vecteur fill(3, 0);
...
void sheet_menu_setup(tableur & t, GIAC_CONTEXT) {
  ...
  while (1) {
    ...
    if (sres == MENU_RETURN_SELECTION || sres == KEY_CTRL_EXE) {
      ...
      if (smallmenu.selection == 2) {
        double d = t.nrows;
        if (inputdouble((lang == 1 ? "Nombre de lignes?" : "Rows?"), d, contextptr) &&
            d == int(d) && d > 0) {
          t.nrows = d;
        }
        continue;
      ...
      if (smallmenu.selection == 3) {
        double d = t.ncols;
        if (inputdouble((lang == 1 ? "Nombre de colonnes?" : "Colonnes?"), d, contextptr) &&
            d == int(d) && d > 0) {
          t.ncols = d;
        }
        continue;
...
int sheet_menu_menu(tableur & t, GIAC_CONTEXT) {
...
#endif
  smallmenuitems[0].text =
      (char *)(lang == 1 ? "Sauvegarde tableur (shift sto)" : "Save sheet (shift sto)");
  ...
  smallmenuitems[smallmenu.numitems - 1].text =
      (char *)(lang == 1 ? "Quitter tableur" : "Leave sheet");
  while (1) {
    int sres = doMenu(&smallmenu);
    if (sres == MENU_RETURN_EXIT)
      return -1;
    if (sres == MENU_RETURN_SELECTION || sres == KEY_CTRL_EXE) {
      ...
      if (smallmenu.selection == 3 && !inexammode) {
        char filename[128];
        if (fileBrowser(filename, (char *)"*.tab",
                        (lang == 1 ? "Fichiers tableurs" : "Sheet files"))) {
          if (t.changed &&
              do_confirm(lang == 1 ? "Sauvegarder le tableur actuel?" : "Save current sheet?"))
            save_sheet(t, contextptr);
...
void sheet_cmd(tableur & t, const char * ans) {
  ...
  if (t.sel_row_begin >= 0) {
    t.cmdline = "";
    s = "=" + s + "matrix(" + print_INT_(absint(t.sel_row_begin - t.cur_row) + 1) + "," +
        print_INT_(absint(t.sel_col_begin - t.cur_col) + 1) + "," +
    ...
    if (i < t.nrows && j < t.ncols) {
      ...
      do_confirm(
          (lang == 1 ? "Impossible de trouver une cellule libre" : "Could not find an empty cell"));
      return;
    }
  }
  activate_cmdline(t);
...
void sheet_help_insert(tableur & t, int exec, GIAC_CONTEXT) {
  ...
  if (back >= t.cmd_pos) {
    t.cmdline = t.cmdline.substr(0, t.cmd_pos - back) +
                t.cmdline.substr(t.cmd_pos, t.cmdline.size() - t.cmd_pos);
    t.cmd_pos -= back;
  }
  if (!adds.empty())
...
giac::gen sheet(GIAC_CONTEXT) {
  ...
  for (;;) {
    ...
    case KEY_CTRL_RIGHT:
      if (t.cmd_pos >= 0 && t.cmd_row == t.cur_row && t.cmd_col == t.cur_col &&
          t.sel_row_begin == -1) {
        ++t.cmd_pos;
    ...
    translate_fkey(key);
    if ((key >= KEY_CTRL_F1 && key <= KEY_CTRL_F3) || (key >= KEY_CTRL_F7 && key <= KEY_CTRL_F20)) {
