...
namespace giac {
  ...
  inline bool logarithmic_span_entire(const gen & g, const gen & x, vecteur & atoms,
    ...
    if (equation_rational(g))
      return equation_rational_budget(g, bound);
    if (g.type != _SYMB)
      return false;
    ...
    if (g.is_symb_of_sommet(at_sin) || g.is_symb_of_sommet(at_cos) || g.is_symb_of_sommet(at_exp)) {
      if (!logarithmic_span_entire(f, x, atoms, budget, depth + 1))
        return false;
      if (!equalposcomp(atoms, g))
        atoms.push_back(g);
      return atoms.size() <= 8;
    }
    if (g.is_symb_of_sommet(at_neg))
      return logarithmic_span_entire(f, x, atoms, budget, depth + 1);
    if (f.type != _VECT)
      return false;
    if (g.is_symb_of_sommet(at_pow) && f._VECTptr->size() == 2)
      return f[1].type == _INT_ && f[1].val > 0 && f[1].val <= 8 &&
             logarithmic_span_entire(f[0], x, atoms, budget, depth + 1);
    if (!g.is_symb_of_sommet(at_prod) && !g.is_symb_of_sommet(at_plus))
      return false;
    for (unsigned j = 0; j < f._VECTptr->size(); ++j)
      if (!logarithmic_span_entire(f[j], x, atoms, budget, depth + 1))
        return false;
    return true;
  }
...
// a formal identity is sufficient even when it misses other identities.
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool integrate_logarithmic_span(const gen & input, const gen & x, gen & result,
                                         GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || taille(input, 97) > 96 || x.type != _IDNT)
      return false;
    vecteur factors = input.is_symb_of_sommet(at_prod) && input._SYMBptr->feuille.type == _VECT
                          ? *input._SYMBptr->feuille._VECTptr
                          : makevecteur(input);
    gen numerator = 1;
    vecteur denominators;
    for (unsigned j = 0; j < factors.size(); ++j) {
      if (factors[j].is_symb_of_sommet(at_inv)) {
        const gen & d = factors[j]._SYMBptr->feuille;
        if (equation_rational(d) && !is_zero(d)) {
        ...
        if (d.is_symb_of_sommet(at_prod) && d._SYMBptr->feuille.type == _VECT) {
          ...
          for (unsigned k = 0; k < v.size(); ++k)
            denominators.push_back(v[k]);
        } else
          denominators.push_back(d);
      } else
        numerator = numerator * factors[j];
    }
    if (denominators.empty() || denominators.size() > 3)
      return false;
    vecteur atoms;
    unsigned budget = 192;
    if (!logarithmic_span_entire(numerator, x, atoms, budget, 0))
      return false;
    gen D = 1;
    for (unsigned j = 0; j < denominators.size(); ++j) {
      if (!logarithmic_span_entire(denominators[j], x, atoms, budget, 0))
        return false;
      D = D * denominators[j];
    }
    if (atoms.empty())
      return false; // ordinary rational integration is cheaper
    vecteur basis = makevecteur(D);
    for (unsigned j = 0; j < denominators.size(); ++j) {
      gen term = derive(denominators[j], x, contextptr);
      for (unsigned k = 0; k < denominators.size(); ++k)
        if (k != j)
          term = term * denominators[k];
      basis.push_back(term);
    }
    for (unsigned j = 0; j < basis.size(); ++j) {
      budget = 192;
      if (!logarithmic_span_entire(basis[j], x, atoms, budget, 0))
        return false;
    }
    ...
    for (unsigned j = 0; j < atoms.size(); ++j) {
      ...
      do {
        if (serial >= 128)
          return false;
        name = identificateur(" logarithmic_span_" + print_INT_(int(serial++)));
      } while (contains(atoms, name));
      names.push_back(name);
    }
    basis.push_back(numerator);
    unsigned columns = basis.size() - 1;
    ...
    for (unsigned col = 0; col < basis.size(); ++col) {
      ...
      budget = 256;
      equation_polynomial_budget bound;
      if (!equation_polynomial_bound(masked, budget, 0, bound) || bound.terms > 64 ||
          bound.degree > 16)
        return false;
      fraction f = sym2r(basis[col], atoms, contextptr);
      if (!equation_rational(f.den) || is_zero(f.den))
        return false;
      if (f.num.type == _POLY) {
        const polynome & poly = *f.num._POLYptr;
        if (poly.coord.size() > 64)
          return false;
        for (unsigned k = 0; k < poly.coord.size(); ++k) {
          const monomial<gen> & m = poly.coord[k];
          if (!equation_rational(m.value))
            return false;
          index_t exponent(m.index.begin(), m.index.end());
          vecteur & row = coefficients[exponent];
          if (row.empty())
            row.resize(columns + 1, 0);
          row[col] = m.value / f.den;
        }
      } else {
        if (!equation_rational(f.num))
          return false;
        vecteur & row = coefficients[index_t(atoms.size(), 0)];
        if (row.empty())
          row.resize(columns + 1, 0);
        row[col] = f.num / f.den;
      }
      if (coefficients.size() > 64)
        return false;
    }
    std::vector<vecteur> rows;
    for (coefficient_table::const_iterator it = coefficients.begin(); it != coefficients.end();
         ++it)
    ...
    for (unsigned col = 0; col < columns && rank < rows.size(); ++col) {
      unsigned pivot = rank;
      while (pivot < rows.size() && is_zero(rows[pivot][col]))
        ++pivot;
      if (pivot == rows.size())
        continue;
      std::swap(rows[pivot], rows[rank]);
      gen divisor = rows[rank][col];
      for (unsigned k = col; k <= columns; ++k) {
        rows[rank][k] = rows[rank][k] / divisor;
        equation_polynomial_budget bound;
        if (!equation_rational_budget(rows[rank][k], bound))
          return false;
      }
      for (unsigned i = 0; i < rows.size(); ++i)
        if (i != rank && !is_zero(rows[i][col])) {
          gen scale = rows[i][col];
          for (unsigned k = col; k <= columns; ++k) {
            rows[i][k] -= scale * rows[rank][k];
            equation_polynomial_budget bound;
            if (!equation_rational_budget(rows[i][k], bound))
              return false;
          }
        }
    ...
    for (unsigned i = rank; i < rows.size(); ++i)
      if (!is_zero(rows[i][columns]))
        return false;
    vecteur weights(columns, 0);
    for (unsigned i = 0; i < rank; ++i)
      weights[pivots[i]] = rows[i][columns];
    result = weights[0] * x;
    for (unsigned j = 0; j < denominators.size(); ++j)
      if (!is_zero(weights[j + 1]))
