  ...
  vecteur mkisom(const gen & n_orig, int b, GIAC_CONTEXT) {
    ...
    if (d >= 3) {
      ...
      if (d1 == 2) {
        ...
        //cout<<A<<'\n';
        gen theta = n[1];
        R = *(recursive_normal(multvecteur(cos(theta, contextptr), midn(3)) +
                                   multvecteur(1 - cos(theta, contextptr), ntn) +
                                   multvecteur(sin(theta, contextptr), A),
                               contextptr)
                  ._VECTptr);
      }
      if (b == 1) {
