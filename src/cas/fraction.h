...
#else
template <class T> std::ostream & operator<<(std::ostream & os, const Tfraction<T> & f) {
  os << f.num << "/" << f.den << " ";
  return os;
}
#endif

  template <class T> void Tfraction<T>::dbgprint() {
    COUT << num << "/" << den << " ";
  }
  ...
  template <class T> class facteur {
  public:
    T fact;
    int mult;
...
#ifdef NSPIRE
    template <class I>
    friend nio::ios_base<I> & operator<<(nio::ios_base<I> & os, const facteur<T> & m) {
