...
namespace giac {
      ...
      if ((temp__SYMB.feuille.type == _IDNT) && (temp__SYMB.sommet != at_abs)) {
        ...
        if (!temp__SYMB.sommet.ptr()->series_expansion) {
          *logptr(contextptr) << gettext("no taylor method for ")
                              << temp__SYMB.sommet.ptr()->print(contextptr) << '\n';
          return false;
        }
        gen shift_coeff;
      ...
      if (temp__SYMB.sommet == at_of) {
        ...
        if (tf.type == _VECT && tf._VECTptr->size() == 2) {
          ...
          for (int i = 1; i <= ordre; i++) {
            ...
            if (i == 1)
              fn = symbolic(at_of,
                            makesequence(symbolic(at_function_diff, tff), image_of_lim_point));
            else
              fn = symbolic(
                  at_of,
                  makesequence(
                      symbolic(at_of, makesequence(symbolic(at_composepow,
                                                            makesequence(at_function_diff, i)),
                                                   tff)),
                      image_of_lim_point));
  ...
  void rlvarx(const gen & e, const gen & xgen, vecteur & res) {
    ...
    for (; it != itend; ++it) {
      ...
      if (it->is_symb_of_sommet(at_derive) && it->_SYMBptr->feuille.type == _VECT &&
        ...
        for (--n; n > 1; --n) {
          res.push_back(symbolic(at_derive, makesequence(it->_SYMBptr->feuille._VECTptr->front(),
                                                         (*it->_SYMBptr->feuille._VECTptr)[1], n)));
        }
        res.push_back(symbolic(at_derive, makesequence(it->_SYMBptr->feuille._VECTptr->front(),
                                                       (*it->_SYMBptr->feuille._VECTptr)[1])));
      }
      if (it->type == _SYMB) {
  ...
  */
  ...
  static gen in_limit(const gen & e0, const identificateur & x, const gen & lim_point,
    ...
    if (_about(x, contextptr) != x) {
      ...
      for (int i = 0;; ++i) {
        ...
        if (_about(X, contextptr) != X)
          continue;
        return in_limit(quotesubst(e0, x, X, contextptr), *X._IDNTptr, lim_point, direction,
                        contextptr);
      }
      // never reached
      gen xprime("x" + print_INT_(giac_rand(contextptr)), contextptr);
      return in_limit(quotesubst(e0, x, xprime, contextptr), *xprime._IDNTptr, lim_point, direction,
                      contextptr);
    }
    ...
    if (vsign.empty()) {
      ...
      if (!contains(lidnt(first_try), unsigned_inf)) {
        ...
        first_try = recursive_ratnormal(first_try, contextptr);
        gen chk;
        if (has_evalf(first_try, chk, 1, contextptr) && !is_zero(chk) && !is_undef(chk) &&
            !is_inf(chk))
	  ...
	  */
      ...
      if (!is_undef(first_try) && !is_undef(numtry)) {
        ...
        if (first_try != unsigned_inf && numtry != unsigned_inf &&
            is_strictly_greater(epsilon(contextptr), abs(numtry - first_try, contextptr),
                                contextptr))
          return first_try;
      }
    } // end if vsign.empty()
  ...
  static bool mrv(const gen & e, const identificateur & x, vecteur & faster_var, vecteur & coeff_ln,
    ...
    for (; it != itend; ++it) {
      ...
      if (temp._SYMBptr->sommet == at_pow)
        temp = symbolic(at_exp, (*(temp._SYMBptr->feuille._VECTptr))[1] *
  ...
  gen series(const gen & e_, const identificateur & x, const gen & lim_point, int ordre,
             int direction, GIAC_CONTEXT) {
    gen e(e_);
    int save_series_flags = series_flags(contextptr);
    series_flags(save_series_flags | 8, contextptr);
