  ...
  */
              ...
              if (tmp3.type <= _CPLX && tmp2.type <= _CPLX && tmp2 != tmp3) {
                if (!vzero.empty() && !tst)
                  *logptr(contextptr) << gettext("Warning, choosing root of ") << f
                                      << gettext(" at parameters values ") << vzero << endl;
  ...
  static gen in_normalize_sqrt(const gen & e, vecteur & L, bool keep_abs, GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      ...
      lout.push_back(pow(simpl * out, nover2, contextptr) * pow(doubl, expnum, contextptr) *
                     pow(recursive_normal(r2e(s, lv, contextptr), contextptr), nover2, contextptr) *
                     pow(tmpd, expnum, contextptr) * pow(tmparg, -expnum, contextptr));
    }
  ...
  gen normal(const gen & e, bool distribute_div, GIAC_CONTEXT) {
    ...
    if (is_equal(e)) {
      ...
      return symb_equal(
          normal(v.front(), distribute_div, contextptr),
          normal(
              v.back(), distribute_div,
              contextptr)); // symbolic(at_equal,makesequence(normal(v.front(),distribute_div,contextptr),normal(v.back(),distribute_div,contextptr)));
    }
