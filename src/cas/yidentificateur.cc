...
gen vx_var(x__IDNT_e);
  ...
  const gen * const tab_one_letter_idnt[] = {
      &a__IDNT_e, &b__IDNT_e, &c__IDNT_e, &d__IDNT_e, &e__IDNT_e, &f__IDNT_e, &g__IDNT_e,
      &h__IDNT_e, &i__IDNT_e, &j__IDNT_e, &k__IDNT_e, &l__IDNT_e, &m__IDNT_e, &n__IDNT_e,
      &o__IDNT_e, &p__IDNT_e, &q__IDNT_e, &r__IDNT_e, &s__IDNT_e, &t__IDNT_e, &u__IDNT_e,
      &v__IDNT_e, &w__IDNT_e, &x__IDNT_e, &y__IDNT_e, &z__IDNT_e};
  ...
  gen identificateur::eval(int level, const gen & orig, const context * contextptr) {
    ...
    if (level <= 0) {
      ...
      if (storcl_38 && abs_calc_mode(contextptr) == 38 &&
          storcl_38(evaled, NULL, id_name, undef, false, contextptr, NULL, false))
        return evaled;
#endif
      if (contextptr) {
        sym_tab::const_iterator it = contextptr->tabptr->find(id_name),
  ...
  bool identificateur::in_eval(int level, const gen & orig, gen & evaled,
                               const context * contextptr, bool No38Lookup) {
    ...
    if (contextptr) { // Look for local variables...
...
#ifndef FXCG
      if (storcl_38 != NULL && !No38Lookup && abs_calc_mode(contextptr) == 38 &&
          storcl_38(evaled, NULL, id_name, undef, false, contextptr, NULL, true))
        return true;
#endif
      ...
      // now at global level
      // check for quoted
      if (cur->quoted_global_vars && !cur->quoted_global_vars->empty() &&
          equalposcomp(*cur->quoted_global_vars, orig))
        return false;
    ...
    if (quoted && *quoted & 1)
      return false;
#ifndef FXCG
    if (current_folder_name.type == _IDNT && current_folder_name._IDNTptr->value &&
        current_folder_name._IDNTptr->value->type == _VECT) {
  ...
  const char * identificateur::print(GIAC_CONTEXT) const {
    ...
    if (
        ...
        abs_calc_mode(contextptr) == 38 && !strcmp(id_name, string_infinity))
      return "±∞";
...
#ifdef NSPIRE
  template <class T>
  nio::ios_base<T> & operator<<(nio::ios_base<T> & os, const identificateur & s) {
    return os << s.print(context0);
  }
#else
ostream & operator<<(ostream & os, const identificateur & s) {
  return os << s.print(context0);
}
#endif
