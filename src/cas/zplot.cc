...
#include <sys/wait.h>
    ...
    defined GIAC_GGB || defined USE_GMP_REPLACEMENTS || defined KHICAS
inline bool is_graphe(const giac::gen & g, std::string & disp_out, const giac::context *) {
  return false;
}
inline giac::gen _graph_vertices(const giac::gen & g, const giac::context *) {
  return g;
}
inline giac::gen _is_planar(const giac::gen & g, const giac::context *) {
  return g;
}
#else
...
int PARAM_STEP = 20;
  ...
  const int _GROUP__VECT_subtype[] = {
      _GROUP__VECT, _LINE__VECT, _HALFLINE__VECT, _VECTOR__VECT, _POLYEDRE__VECT, _POINT__VECT, 0};
#ifdef IPAQ
  double gnuplot_xmin = -4, gnuplot_xmax = 4, gnuplot_ymin = -4, gnuplot_ymax = 4,
         gnuplot_zmin = -5, gnuplot_zmax = 5, gnuplot_tmin = -6, gnuplot_tmax = 6,
         gnuplot_tstep = 0.3;
#else
double gnuplot_xmin = -5, gnuplot_xmax = 5, gnuplot_ymin = -5, gnuplot_ymax = 5, gnuplot_zmin = -5,
       gnuplot_zmax = 5, gnuplot_tmin = -6, gnuplot_tmax = 6, gnuplot_tstep = 0.1;
#endif
  double global_window_xmin(gnuplot_xmin), global_window_xmax(gnuplot_xmax),
      global_window_ymin(gnuplot_ymin), global_window_ymax(gnuplot_ymax);
  ...
  */
    ...
    for (; it != itend; ++it) {
      ...
      if ((p.type == _SYMB) && (p._SYMBptr->sommet == at_pnt)) {
        ...
        if ((p.type != _VECT || p.subtype == _POINT__VECT) &&
            ((p.type != _SYMB) || (!equalposcomp(not_point_sommets, p._SYMBptr->sommet))))
  ...
  bool find_best(vector<int> & selected, const gen & p, double eps, int try_perp_history_pos,
                 int & pnt_pos, int & history_position, gen & res, GIAC_CONTEXT) {
    vecteur v = selection2vecteur(selected, contextptr),
            w = selection2vecteureval(selected, contextptr);
    ...
    if (pnt_pos < 0) { // no point nearby, try to make one
      ...
      if (nsegments) { // near middle point?
        ...
        if (try_perp_history_pos >= 0) {
          ...
          if (is_greater(eps * abs(pm, contextptr) * abs(wv, contextptr),
                         scalar_product(pm, wv, contextptr), contextptr)) {
            history_position = erase_pos(contextptr) + selected[n1];
            gen l1 = remove_sto(history_in(contextptr)[history_position]);
            res = symb_perpendiculaire(
                makevecteur(remove_sto(history_in(contextptr)[try_perp_history_pos]), l1));
  ...
  void terminal_stream_replot(const char * terminal, FILE * stream, int i,
                              const char * file_extension) {
    terminal_stream_replot(
        terminal, stream,
        (gnuplot_filename + print_INT_(i) + "." + string(file_extension)).c_str());
  }
...
#endif
        bool done = false;
        if (v[i].is_symb_of_sommet(at_prod) &&
            v[i]._SYMBptr->feuille.type == _VECT) { // hack for polarplot using re(rho)
...
inline int density(double z, double fmin, double fmax) {
  ...
  static vecteur densityscale(double xmin, double xmax, double ymin, double ymax, double fmin,
                              double fmax, int n, GIAC_CONTEXT) {
    vecteur res;
    if (n < 10)
      n = 10;
    if (n > 100)
...
#endif
      res.push_back(pnt_attrib(
          gen((i ? makevecteur(D, A, B, C, D) : makevecteur(B, C, D, A, B)), _GROUP__VECT), attrib,
          contextptr));
    }
    return res;
  }
  ...
  if (regular) {
    ...
    gen rect = pnt_attrib(gen(makevecteur(gen(xmin, ymin), gen(xmax, ymin), gen(xmax, ymax),
                                          gen(xmin, ymax), gen(xmin, ymin)),
                              _GROUP__VECT),
                          vecteur(1, _FILL_POLYGON + 256), contextptr);
    ...
    gen res = plot_array(fij, r, c, xmin, xmax, dx, ymin, ymax, dy, lz, makevecteur(attr, legendes),
                         true, contextptr);
    if (res.type == _VECT) {
    ...
    for (int i = 1; i < r - 1; ++i) {
      ...
      for (int j = 1; j < c - 1; ++j) {
        ...
        if (m[2].type == _DOUBLE_) {
          ...
          res.push_back(pnt_attrib(gen(makevecteur(A, B, C, D, A), _GROUP__VECT),
                                   vecteur(1, e + _FILL_POLYGON), contextptr));
        }
      }
    }
    return res;
  ...
  gen plotfunc(const gen & f_, const gen & vars, const vecteur & attributs, int densityplot,
               double function_xmin, double function_xmax, double function_ymin,
               double function_ymax, double function_zmin, double function_zmax, int nstep,
               int jstep, bool showeq, const context * contextptr) {
    ...
    if (densityplot == 0 && are_inequations(L)) {
      ...
      if (poly_ineq_plot(L, x__IDNT_e, y__IDNT_e, function_xmin, function_xmax, function_ymin,
                         function_ymax, attributs, res, contextptr))
        return gen(res, _SEQ__VECT);
      if (jstep == 0 && nstep <= 401)
        nstep *= 16;
      return plotfunc(f_, makevecteur(x__IDNT_e, y__IDNT_e), attributs, 2, function_xmin,
                      function_xmax, function_ymin, function_ymax, -1, 2, nstep, jstep, false,
                      contextptr);
    }
    if (f_.is_symb_of_sommet(at_equal)
        //|| is_inequation(f_)
    ) {
      return plotimplicit(equal2diff(f_), x__IDNT_e, y__IDNT_e, function_xmin, function_xmax,
                          function_ymin, function_ymax, nstep, jstep, epsilon(contextptr),
                          attributs, false, true, contextptr, 3);
      return string2gen("Try plot([" + f_._SYMBptr->feuille.print(contextptr) + "]," +
                            vars.print(contextptr) + "). (In)equations can not be plotted.",
                        false);
    }
    ...
    if (step <= 0 || (function_xmax - function_xmin) / step > max_nstep)
      return gensizeerr(gettext("Plotfunc: unable to discretize: xmin, xmax, step=") +
                        print_DOUBLE_(function_xmin, 12) + "," + print_DOUBLE_(function_xmax, 12) +
                        "," + print_DOUBLE_(step, 12) + gettext("\nTry a larger value for xstep"));
    vecteur res;
    ...
    if (f.type == _VECT) { // multi-plot
      ...
      for (unsigned i = 0; i < s; ++i) {
        ...
        gen tmp =
            plotfunc(vf[i], vars, cur_attributs, false, function_xmin, function_xmax, function_ymin,
                     function_ymax, function_zmin, function_zmax, nstep, jstep, showeq, contextptr);
        if (tmp.type == _VECT)
    ...
    if (vars.type == _IDNT) { // function plot
      ...
      if (taille(f, 100) <= 100 && fast_is_linear_wrt(f, vars, a, b, contextptr))
        return put_attributs(
            _segment(makesequence(function_xmin + cst_i * (a * gen(function_xmin) + b),
                                  function_xmax + cst_i * (a * gen(function_xmax) + b)),
                     contextptr),
            attributs, contextptr); // replace segment by droite so that legend=... works?
      vecteur lpiece(lop(f, at_piecewise));
      if (!lpiece.empty())
        lpiece = lvarx(lpiece, vars);
      if (!lpiece.empty()) {
        ...
        if (piecef.type == _VECT) {
          ...
          for (i = 0; i < vs / 2; ++i) {
            ...
            if (cond.is_symb_of_sommet(at_different)) {
              *logptr(contextptr) << gettext("Assuming true condition ") << cond << '\n';
              ...
              return plotfunc(f, vars, attributs, densityplot, function_xmin, function_xmax,
                              function_ymin, function_ymax, function_zmin, function_zmax, nstep,
                              jstep, showeq, contextptr);
            }
            bool unable = true;
            ...
            if (ck_is_greater(l, function_xmax, contextptr)) {
              ...
              gen curres = plotfunc(tmp, vars, attributs, densityplot, function_xmin, function_xmax,
                                    function_ymin, function_ymax, function_zmin, function_zmax,
                                    nstep, jstep, showeq, contextptr);
            ...
            if (positif) {
              ...
              gen curres = plotfunc(tmp, vars, attributs, densityplot, l._DOUBLE_val, function_xmax,
                                    function_ymin, function_ymax, function_zmin, function_zmax,
                                    nstep, jstep, showeq, contextptr);
              if (curres.type == _VECT)
                res = mergevecteur(res, *curres._VECTptr);
            ...
            gen curres = plotfunc(tmp, vars, attributs, densityplot, function_xmin, l._DOUBLE_val,
                                  function_ymin, function_ymax, function_zmin, function_zmax, nstep,
                                  jstep, showeq, contextptr);
            if (curres.type == _VECT)
              res = mergevecteur(res, *curres._VECTptr);
          ...
          if (i == vs / 2) {
            if (vs % 2) {
              ...
              gen curres = plotfunc(f, vars, attributs, densityplot, function_xmin, function_xmax,
                                    function_ymin, function_ymax, function_zmin, function_zmax,
                                    nstep, jstep, showeq, contextptr);
              if (curres.type == _VECT)
                res = mergevecteur(res, *curres._VECTptr);
      ...
      for (; !ctrl_c && !interrupted && i < xmax; i += step) {
        ...
        if (yy.type != _DOUBLE_) {
          ...
          if (!chemin.empty())
            res.push_back(pnt_attrib(
                symb_curve(gen(makevecteur(vars + cst_i * f, vars, xmin, i, showeq), _PNT__VECT),
                           gen(chemin, _GROUP__VECT)),
                attributs.empty() ? color : attributs, contextptr));
          xmin = i;
          chemin.clear();
          continue;
        }
      ...
      if (!chemin.empty()) {
        if (debug_infolevel)
          CERR << "curve " << chemin.size() << " " << chemin.front() << " .. " << chemin.back()
               << '\n';
        res.push_back(pnt_attrib(
            symb_curve(gen(makevecteur(vars + cst_i * f, vars, xmin, i - step, showeq), _PNT__VECT),
                       gen(chemin, _GROUP__VECT)),
            attributs.empty() ? color : attributs, contextptr));
      }
    ...
    if (s == 2) {
	  ...
	  if (!push) continue;
          ...
          for (int j = 0; j <= nv; ++j, y += dy) {
            ...
            if (mode) {
              // jstart->j
              vecteur rect;
              double xleft = x - dx / 2, xright = x + dx / 2, ystartleft = ystart - dy / 2,
                     yleft = y - dy / 2;
#if 0
		...
		if (J>=0 && J<=j) ystartleft += (J-jstart)*dy;
...
#endif
              rect =
                  makevecteur(gen(xleft, ystartleft), gen(xright, ystart - dy / 2),
                              gen(xright, y - dy / 2), gen(xleft, yleft), gen(xleft, ystartleft));
          ...
          if (mode && i) {
            vecteur rect(makevecteur(gen(x - dx / 2, ystart - dy / 2),
                                     gen(x + dx / 2, ystart - dy / 2), gen(x + dx / 2, y - dy / 2),
                                     gen(x - dx / 2, y - dy / 2),
                                     gen(x - dx / 2, ystart - dy / 2)));
      ...
      if (densityplot) {
        ...
        r = mergevecteur(density(values, function_zmin, function_zmax, true, contextptr),
                         densityscale(function_xmin + dx, function_xmax - dx, function_ymin - dy,
                                      function_ymin - 2 * dy, function_zmin, function_zmax, 20,
                                      contextptr));
      } else
        r = pnt_attrib(
            hypersurface(gen(makevecteur(makevecteur(var1, var2, f), makevecteur(var1, var2),
                                         makevecteur(function_xmin, function_ymin),
                                         makevecteur(function_xmax, function_ymax),
                                         gen(values, _GROUP__VECT)),
                             _PNT__VECT),
                         z__IDNT_e - f, makevecteur(var1, var2, z__IDNT_e)),
            attributs.empty() ? color : attributs, contextptr);
    }
    if (!has_gnuplot)
      return r;
    ...
    int out_handle;
    bool clrplot = false;
    FILE *gnuplot_out_readstream,
        *stream = open_gnuplot(clrplot, gnuplot_out_readstream, out_handle);
    int ng = 0;
#ifdef IPAQ
    ...
    // usleep(50000);
    r.subtype = gnuplot_fileno, ++gnuplot_fileno;
    return r;
  ...
  static gen bit_orv(const gen & a, const vecteur & v) {
    ...
    if (a.type == _VECT) {
      vecteur res;
      const_iterateur it = a._VECTptr->begin(), itend = a._VECTptr->end(), jt = v.begin(),
                      jtend = v.end();
  ...
  static void read_option(const vecteur & v, double xmin, double xmax, double ymin, double ymax,
                          double zmin, double zmax, vecteur & attributs, int & nstep, int & jstep,
                          int & kstep, bool unfactored, GIAC_CONTEXT) {
    read_attributs(v, attributs, contextptr);
    const_iterateur it = v.begin(), itend = v.end();
    for (; it != itend; ++it) {
      if (it->type == _VECT) {
        read_option(*it->_VECTptr, xmin, xmax, ymin, ymax, zmin, zmax, attributs, nstep, jstep,
                    kstep, contextptr);
        continue;
      }
  ...
  void read_option(const vecteur & v, double xmin, double xmax, double ymin, double ymax,
                   double zmin, double zmax, vecteur & attributs, int & nstep, int & jstep,
                   int & kstep, GIAC_CONTEXT) {
    bool unfactored = false;
    read_option(v, xmin, xmax, ymin, ymax, zmin, zmax, attributs, nstep, jstep, kstep, unfactored,
                contextptr);
  }

  static void read_option(const vecteur & v, double xmin, double xmax, double ymin, double ymax,
                          vecteur & attributs, int & nstep, int & jstep, GIAC_CONTEXT) {
  ...
  gen funcplotfunc(const gen & args, int densityplot, const context * contextptr) {
    double xmin = gnuplot_xmin, xmax = gnuplot_xmax, ymin = gnuplot_ymin, ymax = gnuplot_ymax,
           zmin = gnuplot_zmin, zmax = gnuplot_zmax;
    bool showeq = false;
    if (densityplot)
    ...
    if (densityplot == 0 && args.type != _VECT) {
      ...
      if (equalposcomp(lname, vx_var) && equalposcomp(lname, y__IDNT_e))
        return funcplotfunc(makesequence(args, makevecteur(vx_var, y__IDNT_e)), densityplot,
                            contextptr);
    }
    ...
    bool newsyntax;
    if (e1.type != _VECT) {
      newsyntax = readrange(e1, gnuplot_xmin, gnuplot_xmax, e1, xmin, xmax, contextptr) &&
                  (is_equal(vargs[1]) || s < 4);
    } else {
      if (e1._VECTptr->size() != 2)
        return setplotfuncerr();
      vecteur v(*e1._VECTptr);
      newsyntax = readrange(v[0], gnuplot_xmin, gnuplot_xmax, v[0], xmin, xmax, contextptr) &&
                  readrange(v[1], gnuplot_ymin, gnuplot_ymax, v[1], ymin, ymax, contextptr);
      if (newsyntax)
        e1 = v;
    }
    if (!newsyntax) { // plotfunc(fonction,var,min,max[,zminmax,attribut])
      ...
      gen e3 = vargs[3];
      if (e1.type == _VECT) {
        if ((e2.type != _VECT) || (e3.type != _VECT) || (e2._VECTptr->size() != 2) ||
            (e3._VECTptr->size() != 2))
        ...
        if ((e21.type != _DOUBLE_) || (e22.type != _DOUBLE_) || (e31.type != _DOUBLE_) ||
            (e32.type != _DOUBLE_))
      ...
      return plotfunc(vargs.front(), e1, vecteur(1, attribut), densityplot, xmin, xmax, ymin, ymax,
                      zmin, zmax, nstep, jstep, showeq, contextptr);
    }
    ...
    return plotfunc(vargs[0], e1, attributs, densityplot, xmin, xmax, ymin, ymax, zmin, zmax, nstep,
                    jstep, showeq, contextptr);
  }
  gen _plotfunc(const gen & args, const context * contextptr) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    int nd;
    if ((nd = is_distribution(args)) || (args.type == _VECT && !args._VECTptr->empty() &&
                                         (nd = is_distribution(args._VECTptr->front())))) {
  ...
  define_unary_function_ptr5(at_plotdensity, alias_at_plotdensity, &__plotdensity, _QUOTE_ARGUMENTS,
                             true);
  ...
  define_unary_function_ptr5(at_plotmatrix, alias_at_plotmatrix, &__plotmatrix, _QUOTE_ARGUMENTS,
                             true);
  ...
  define_unary_function_ptr5(at_densityplot, alias_at_densityplot, &__densityplot, _QUOTE_ARGUMENTS,
                             true);
  ...
  bool readrange(const gen & g, double defaultxmin, double defaultxmax, gen & x, double & xmin,
                 double & xmax, GIAC_CONTEXT) {
    xmin = defaultxmin;
    xmax = defaultxmax;
    if (g.type == _IDNT) {
      x = g;
  ...
  static gen pointonoff(const gen & args, const vecteur & attributs, GIAC_CONTEXT) {
    ...
    if ((a.is_real(contextptr)) || (a.type == _CPLX)) {
      if (attributs.size() <= 1)
        ...
        e = symb_pnt_name(args.eval(eval_level(contextptr), contextptr), attributs[0], attributs[1],
                          contextptr);
    } else {
      if (args.type != _VECT)
  ...
  void merge_pixon(const vecteur & v, vecteur & w) {
    ...
    for (; it != itend; ++it) {
      ...
      if (lastcolor == c.val && lastxmax == lastxmin && lastxmax == x.val &&
          lastymax + 1 == y.val) {
        ++lastymax;
        continue;
      }
      if (lastcolor == c.val && lastymax == lastymin && lastymax == y.val &&
          lastxmax + 1 == x.val) {
        ++lastxmax;
        continue;
      }
      ...
      if (lastcolor != -1) {
        if (lastxmax == lastxmin) {
          if (lastymax == lastymin)
            ...
            w.push_back(symbolic(
                at_pixon, makesequence(lastxmin, lastymin, lastcolor, lastymax + 1 - lastymin)));
        } else {
          if (lastymax == lastymin)
            w.push_back(symbolic(
                at_pixon, makesequence(lastxmin, lastymin, lastcolor, lastxmin - lastxmax - 1)));
        }
      }
#else
    if (lastcolor != -1) {
      if (lastxmax == lastxmin) {
        if (lastymax == lastymin)
          w.push_back(symbolic(
              at_pnt,
              makesequence(symbolic(at_pixon, makesequence(lastxmin, lastymin, lastcolor)), 0)));
        else
          w.push_back(symbolic(
              at_pnt, makesequence(symbolic(at_pixon, makesequence(lastxmin, lastymin, lastcolor,
                                                                   lastymax + 1 - lastymin)),
                                   0)));
      } else {
        if (lastymax == lastymin)
  ...
  static gen _droite_segment(const gen & args, int subtype, const vecteur & attributs,
                             GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    gen e;
      ...
      *it = remove_at_pnt(*it);
      ...
      if (it->type == _VECT) {
        if (it->_VECTptr->size() == 2) {
          if (ispnt) {
            if (it->subtype == _LINE__VECT || it->subtype == _HALFLINE__VECT ||
                it->subtype == _VECTOR__VECT)
    ...
    if (res.size() == 2 && is_zero(res.front() - res.back(), contextptr) &&
        subtype != _GROUP__VECT && subtype != _VECTOR__VECT)
      return undef;
  ...
  gen droite_by_equation(const vecteur & v, bool est_plan, GIAC_CONTEXT) {
    ...
    if (s >= 2 && is_equal(v[1])) {
      ...
      if (is_undef(v0) || is_undef(v1))
        return v0 + v1;
      gen A;
      if (!is_zero(derive(v0, vxyz, contextptr), contextptr) ||
          !is_zero(derive(v1, vxyz, contextptr), contextptr))
        return gensizeerr(gettext("Non linear equations"));
      vecteur directeur(
          *normal(cross(*v0._VECTptr, *v1._VECTptr, contextptr), contextptr)._VECTptr);
      ...
      if (is_zero(directeur[2], contextptr)) {   // z is constant on the line
        if (is_zero(directeur[1], contextptr)) { // fix x=0 and solve for y and z
          gen sol = solve(
              makevecteur(subst(eq, x, 0, false, contextptr), subst(eq1, x, 0, false, contextptr)),
              makevecteur(y, z), 0, contextptr);
          ...
          // fix y=0 for A, solve
          gen sol = solve(
              makevecteur(subst(eq, y, 0, false, contextptr), subst(eq1, y, 0, false, contextptr)),
              makevecteur(x, z), 0, contextptr);
          if (is_undef(sol) || sol.type != _VECT || sol._VECTptr->size() != 1)
            return gensizeerr(contextptr);
          sol = sol._VECTptr->front();
        ...
        gen sol = solve(
            makevecteur(subst(eq, z, 0, false, contextptr), subst(eq1, z, 0, false, contextptr)),
            makevecteur(x, y), 0, contextptr);
        if (is_undef(sol) || sol.type != _VECT || sol._VECTptr->size() != 1)
          return gensizeerr(contextptr);
        sol = sol._VECTptr->front();
    ...
    if (est_plan) {
      ...
      return pnt_attrib(symbolic(at_hyperplan, gen(makevecteur(B, A), _SEQ__VECT)), attributs,
                        contextptr);
    }
    // 2-d line
    if (is_zero(eqy, contextptr)) {
  ...
  gen _droite(const gen & args0, GIAC_CONTEXT) {
    ...
    return _droite_segment(gen(makevecteur(v0, v1), _SEQ__VECT), _LINE__VECT, attributs,
                           contextptr);
  }
  ...
  gen _vector(const gen & args, GIAC_CONTEXT) {
    ...
    if (s >= 2 && s <= 3 && v[0].type != _VECT && v[1].type != _VECT &&
        !v[0].is_symb_of_sommet(at_pnt) && !v[1].is_symb_of_sommet(at_pnt) &&
        is_zero(im(v[0], contextptr), contextptr) && is_zero(im(v[1], contextptr), contextptr)) {
      if (s == 2) {
        v[1] = v[0] + cst_i * v[1];
        v[0] = 0;
      } else {
        if (v[2].type != _VECT && !v[2].is_symb_of_sommet(at_pnt) &&
            is_zero(im(v[2], contextptr), contextptr)) {
          v[1] = v;
  ...
  gen _segment(const gen & args, GIAC_CONTEXT) {
    ...
    if (s == 4) {
      ...
      if (name.type != _IDNT)
        ...
        w.push_back(eval(symb_sto(_droite_segment(seg, _GROUP__VECT, attributs, contextptr), name),
                         contextptr));
      return gen(w, _GROUP__VECT);
  ...
  int gnuplot_show_pnt(const symbolic & e, GIAC_CONTEXT) {
    ...
    if (f.type == _VECT && !f._VECTptr->empty()) {
      ...
      double les3 = gnuplot_xmax - gnuplot_xmin + gnuplot_ymax - gnuplot_ymin + gnuplot_zmax -
                    gnuplot_zmin,
             deltat = gnuplot_tmax - gnuplot_tmin, milieut = (gnuplot_tmax + gnuplot_tmin) / 2;
      ...
      if (p.type == _VECT && p.subtype != _POINT__VECT) {
        ...
        if (p.subtype == _POLYEDRE__VECT) {
              ...
              if (s >= 4) {
                for (int j = 0; j < s / 2; ++j) {
                  if (w[j].type != _VECT || w[j]._VECTptr->size() != 3)
                    ...
                    tmpfile << evalf(ww[0], 1, contextptr) << " " << evalf(ww[1], 1, contextptr)
                            << " " << evalf(ww[2], 1, contextptr) << '\n';
                  }
                }
                tmpfile << '\n';
          ...
          if (v.size() == 2) {
            ...
            tmp_eq = tmp_eq + 1e-3 * makevecteur(gnuplot_xmax - gnuplot_xmin,
                                                 gnuplot_ymax - gnuplot_ymin,
                                                 gnuplot_zmax - gnuplot_zmin);
            ...
            for (; it != itend; ++it) {
              if (it->type == _VECT && it->_VECTptr->size() == 3) {
                ...
                tmpfile << evalf(w[0], 1, contextptr) << " " << evalf(w[1], 1, contextptr) << " "
                        << evalf(w[2], 1, contextptr) << '\n';
                doplot = true;
              }
            }
            with_point = 0;
      ...
      if (p.is_symb_of_sommet(at_hypersphere)) {
        ...
        if (f.type == _VECT && f._VECTptr->size() >= 2) {
          ...
          if (v.front().type == _VECT && v.front()._VECTptr->size() == 3) {
            ...
            vecteur dir1(makevecteur(1, 0, 0)), dir2(makevecteur(0, 1, 0)),
                dir3(makevecteur(0, 0, 1));
            ...
            gen par_eq = v.front() + r * (cos(uu, contextptr) * (cos(vv, contextptr) * dir1 +
                                                                 sin(vv, contextptr) * dir2) +
                                          sin(uu, contextptr) * dir3);
            par_eq.subtype = _SEQ__VECT;
  ...
  gen symb_segment(const gen & x, const gen & y, const vecteur & c, int type, GIAC_CONTEXT) {
    gen e;
    if (c.empty())
      e = symbolic(at_pnt, gen(makevecteur(gen(makevecteur(x, y), type), default_color(contextptr)),
                               _PNT__VECT));
  ...
  static string printaspnt(const gen & feuille, const char * sommetstr, GIAC_CONTEXT) {
	...
	return "vector("+f0.print(contextptr)+")";
      ...
      */
      ...
      if (f0.type != _VECT) {
        if (f0.type == _SYMB &&
            (f0._SYMBptr->sommet == at_hyperplan || f0._SYMBptr->sommet == at_hypersphere)) {
          return f0.print(contextptr);
        }
  ...
  bool centre_rayon(const gen & cercle, gen & centre, gen & rayon, bool absrayon, GIAC_CONTEXT,
    ...
    if ((c.type != _SYMB) || (c._SYMBptr->sommet != at_cercle)) {
      if (detect_conic && c.is_symb_of_sommet(at_curve)) {
        ...
        int ctype =
            conique_reduite(eq, undef, makevecteur(x__IDNT_e, y__IDNT_e), x0, y0, V0, V1, propre,
                            equation_reduite, param_curves, ratparam, true, contextptr, &a, &b);
        if (ctype < 2)
          return false;
        a = inv(a, contextptr);
        b = inv(b, contextptr);
        if (ctype == 3) {
          ...
          rayon = makevecteur(symb_equal(a__IDNT_e, a), symb_equal(b__IDNT_e, b),
                              symb_equal(c__IDNT_e, c), symb_equal(e__IDNT_e, c / a));
          return true;
        }
        if (ctype == 4) {
          if (is_positive(-a, contextptr)) {
            ...
            rayon = makevecteur(symb_equal(a__IDNT_e, b), symb_equal(b__IDNT_e, a),
                                symb_equal(c__IDNT_e, c), symb_equal(e__IDNT_e, c / b));
            return true;
          }
          return false;
        }
    ...
    if (args.type == _VECT) {
      ...
      if (s == 2) {
        if (args._VECTptr->front().type == _VECT || args._VECTptr->back().type == _VECT)
          return gensizeerr(contextptr);
        return pointonoff(args._VECTptr->front() + cst_i * (*args._VECTptr)[1], attributs,
                          contextptr);
      }
  ...
  static gen point2d3d(const gen & args, bool espace, GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      w.push_back(symbolic(
          at_sto, makesequence(symbolic(at_pnt, gen(makevecteur(rand_2d3d(espace), attributs[0]),
                                                    _PNT__VECT)),
                               *it)));
    }
  ...
  gen _affixe(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    if (args.type == _VECT && args._VECTptr->size() == 2 &&
        !args._VECTptr->front().is_symb_of_sommet(at_pnt))
  ...
  gen coordonnees(const gen & args, bool in, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT) {
      ...
      if (args._VECTptr->size() == 2 && args.subtype == _VECTOR__VECT) {
        ...
        a = remove_at_pnt(a);
        b = remove_at_pnt(b);
        if ((a.type == _VECT && b.type != _VECT) && (b.type == _VECT && a.type != _VECT))
          return gentypeerr(contextptr);
        gen c = b - a;
        if (c.type == _VECT)
          return c;
        return gen(makevecteur(re(c, contextptr), im(c, contextptr)),
                   calc_mode(contextptr) == 1 ? _GGB__VECT : 0);
      }
    ...
    return gen(makevecteur(re(P, contextptr), im(P, contextptr)),
               calc_mode(contextptr) == 1 ? _GGB__VECT : 0);
  }
  ...
  static define_unary_function_eval(__coordonnees_polaires, &_coordonnees_polaires,
                                    _coordonnees_polaires_s);
  define_unary_function_ptr5(at_coordonnees_polaires, alias_at_coordonnees_polaires,
                             &__coordonnees_polaires, 0, true);
  ...
  static define_unary_function_eval(__coordonnees_rectangulaires, &_coordonnees_rectangulaires,
                                    _coordonnees_rectangulaires_s);
  define_unary_function_ptr5(at_coordonnees_rectangulaires, alias_at_coordonnees_rectangulaires,
                             &__coordonnees_rectangulaires, 0, true);
  ...
  static inline gen divide_by_2(const gen & ra, GIAC_CONTEXT) {
    if (ra.type == _DOUBLE_ || (ra.type == _CPLX && (ra._CPLXptr->type == _DOUBLE_ ||
                                                     (ra._CPLXptr + 1)->type == _DOUBLE_)))
      return ra / gen(2.0);
    else
  ...
  gen _cercle(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args.subtype == _PNT__VECT)
    ...
    bool diam = true;
    if (e.is_symb_of_sommet(at_equal) && e._SYMBptr->feuille.type == _VECT &&
    ...
    if (d0.type == _VECT) {
      ...
      return plotparam3d(ce + cos(t, contextptr) * ra + sin(t, contextptr) * d02, makevecteur(t, u),
                         gnuplot_xmin, gnuplot_xmax, gnuplot_ymin, gnuplot_ymax, gnuplot_zmin,
                         gnuplot_zmax, 0, 2 * M_PI, 0, 0, false, false, attributs, M_PI / 30, 0,
                         undef, makevecteur(t, u), contextptr);
    ...
    gen res = pnt_attrib(
        new_ref_symbolic(symbolic(at_cercle, gen(makenewvecteur(diametre, a1, a2), _PNT__VECT))),
  ...
  gen perpendiculaire(const gen & args, bool suppl, GIAC_CONTEXT) {
    ...
    if (f.is_symb_of_sommet(at_hyperplan)) {
      ...
      if (e.type == _VECT && e._VECTptr->size() == 2) {
        ...
        if (suppl) { // -> plan defined by line and n
          return pnt_attrib(symbolic(at_hyperplan, makesequence(cross(e, n, contextptr), M)),
                            attributs,
                            contextptr); // _plan(makevecteur(cross(e,n,contextptr),M),contextptr);
        }
    ...
    if (f.type == _VECT) {
      ...
      if (suppl)
        return pnt_attrib(symbolic(at_hyperplan, makesequence(f, e)), attributs,
                          contextptr); // _plan(makevecteur(f,e),contextptr);
  ...
  gen _mediatrice(const gen & args, GIAC_CONTEXT) {
    ...
    reim(e, ex, ey, contextptr);
    reim(f, fx, fy, contextptr);
    mx = (ex + fx) / 2;
    my = (ey + fy) / 2;
    efy = ex - fx;
    efx = fy - ey; // perpendicular to ef
    ax = mx - efx;
    ay = my - efy;
    bx = mx + efx;
    by = my + efy;
    return symb_segment(makecomplex(ax, ay), makecomplex(bx, by), attributs, _LINE__VECT,
                        contextptr);
    /*
  ...
  define_unary_function_ptr5(at_perpendiculaire, alias_at_perpendiculaire, &__perpendiculaire, 0,
                             true);
  ...
  gen _parallele(const gen & args, GIAC_CONTEXT) {
    ...
    if (!b.is_symb_of_sommet(at_pnt)) {
      ...
      if (b.type != _VECT)
        b = symbolic(at_pnt, gen(makevecteur(gen(makevecteur(0, b), _LINE__VECT), attributs[0]),
                                 _PNT__VECT));
    }
    if (s > 2) {
      b = remove_at_pnt(b);
      d = remove_at_pnt(args._VECTptr->back());
      if (a.type == _VECT && a._VECTptr->size() == 3 && b.type == _VECT &&
          b._VECTptr->size() == 2 && d.type == _VECT && d._VECTptr->size() == 2) {
        gen n = cross(b._VECTptr->front() - b._VECTptr->back(),
                      d._VECTptr->front() - d._VECTptr->back(), contextptr);
        return pnt_attrib(symbolic(at_hyperplan, makesequence(n, a)), attributs,
                          contextptr); // _plan(makevecteur(n,a),contextptr);
      }
    }
    ...
    if (!ahyper && b.type == _VECT) {
      ...
      if (b.is_symb_of_sommet(at_hyperplan)) { // hyperplan, point
        return pnt_attrib(symbolic(at_hyperplan, makesequence(hyperplan_normal(b), a)), attributs,
                          contextptr); // _plan(makevecteur(hyperplan_normal(b),a),contextptr);
      }
      if (b.type == _VECT && b._VECTptr->size() == 2 && b.subtype != _POINT__VECT) { // b is a line
        if (a.type == _VECT && a._VECTptr->size() == 2 &&
            a.subtype != _POINT__VECT) { // a is a line -> d
          gen M = a._VECTptr->front();
          gen n =
              cross(M - a._VECTptr->back(), b._VECTptr->front() - b._VECTptr->back(), contextptr);
          return pnt_attrib(symbolic(at_hyperplan, makesequence(n, M)), attributs,
                            contextptr); // _plan(makevecteur(n,M),contextptr);
        }
    ...
    if (d.type != _VECT || d._VECTptr->size() != 2 ||
        (b.type == _VECT && b.subtype != _POINT__VECT && b._VECTptr->size() != 3))
      return gensizeerr(contextptr);
  ...
  static void get_rectangle_args(const vecteur & v, gen & e, gen & f, gen & g, gen & tmp,
                                 GIAC_CONTEXT) {
  ...
  static define_unary_function_eval_quoted(__triangle_rectangle, &_triangle_rectangle,
                                           _triangle_rectangle_s);
  define_unary_function_ptr5(at_triangle_rectangle, alias_at_triangle_rectangle,
                             &__triangle_rectangle, _QUOTE_ARGUMENTS, true);
  ...
  define_unary_function_ptr5(at_rectangle, alias_at_rectangle, &__rectangle, _QUOTE_ARGUMENTS,
                             true);

  gen _parallelogramme(const gen & args0, GIAC_CONTEXT) {
    if (args0.type == _STRNG && args0.subtype == -1)
      return args0;
    gen args(args0);
    if (args.type != _VECT)
    ...
    if (s < 3)
      return gendimerr(contextptr);
    gen e = remove_at_pnt(eval(v[0], contextptr)), f = remove_at_pnt(eval(v[1], contextptr)),
        d = remove_at_pnt(eval(v[2], contextptr));
    e = remove_at_pnt(get_point(e, 0, contextptr));
    if (f.type == _VECT && f.subtype == _VECTOR__VECT && f._VECTptr->size() == 2)
  ...
  static define_unary_function_eval_quoted(__parallelogramme, &_parallelogramme,
                                           _parallelogramme_s);
  define_unary_function_ptr5(at_parallelogramme, alias_at_parallelogramme, &__parallelogramme,
                             _QUOTE_ARGUMENTS, true);
  ...
  static define_unary_function_eval_quoted(__triangle_isocele, &_triangle_isocele,
                                           _triangle_isocele_s);
  define_unary_function_ptr5(at_triangle_isocele, alias_at_triangle_isocele, &__triangle_isocele,
                             _QUOTE_ARGUMENTS, true);
  ...
  static define_unary_function_eval_quoted(__triangle_equilateral, &_triangle_equilateral,
                                           _triangle_equilateral_s);
  define_unary_function_ptr5(at_triangle_equilateral, alias_at_triangle_equilateral,
                             &__triangle_equilateral, _QUOTE_ARGUMENTS, true);
  ...
  gen _isopolygone(const gen & args, GIAC_CONTEXT) {
    ...
    w.push_back(f);
    for (int i = 1; i < n; ++i) {
      w.push_back(e + ef * cos(2 * i * cst_pi / n, contextptr) +
                  nd * sin(2 * i * cst_pi / n, contextptr));
    }
    w.push_back(f);
    //grad
  ...
  gen _hexagone(const gen & args, GIAC_CONTEXT) {
    ...
    j = e + ecenter - ef;
    gen res =
        pnt_attrib(gen(makevecteur(e, f, g, h, i, j, e), _GROUP__VECT), attributs, contextptr);
    if (s == dim)
      return res;
    vecteur w(1, res);
    w.push_back(eval(symb_sto(_point(g, contextptr), v[dim]), contextptr));
  ...
  define_unary_function_ptr5(at_polygone_ouvert, alias_at_polygone_ouvert, &__polygone_ouvert, 0,
                             true);

  static bool gen23points(const gen & args, gen & e, gen & f, gen & g, vecteur & attributs,
                          GIAC_CONTEXT) {
    if (args.type != _VECT)
      return false; // setsizeerr(gettext("plot.cc/gen23points"));
    vecteur v(*args._VECTptr);
    int s = read_attributs(v, attributs, contextptr);
    if (s < 2)
  ...
  gen _hauteur(const gen & args, GIAC_CONTEXT) {
    ...
    if (f.type == _VECT && f._VECTptr->size() == 3) {
      ...
      gen h = _projection(
          gen(makevecteur(_droite(gen(makevecteur(f, g), _SEQ__VECT), contextptr), e), _SEQ__VECT),
          contextptr);
  ...
  gen _mediane(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    if (args.type == _VECT && args.subtype != _SEQ__VECT)
      return _median(args, contextptr);
    unsigned s = 0;
    if ((args.type != _VECT) || ((s = unsigned(args._VECTptr->size())) < 2))
      return gensizeerr(contextptr);
    if (s == 2 && args._VECTptr->front().type == _VECT &&
        args._VECTptr->front()._VECTptr->size() > 3)
      return _median(args, contextptr);
    gen e, f, g;
    vecteur attributs(1, default_color(contextptr));
    if (!gen23points(args, e, f, g, attributs, contextptr))
  ...
  static vecteur inter2droites2(const gen & a1, const gen & a2, const gen & b1, const gen & b2,
                                int asub, int bsub, GIAC_CONTEXT) {
    // 2-d line intersection
    gen v(a2 - a1), w(b2 - b1);
    gen x = (b1 - a1).re(contextptr), y = (b1 - a1).im(contextptr), vx = v.re(contextptr),
        vy = v.im(contextptr);
    gen wx = w.re(contextptr), wy = w.im(contextptr);
    gen alpha = rdiv(wy * x - wx * y, wy * vx - wx * vy, contextptr);
    return remove_not_in_segment(
        b1, b2, bsub,
        remove_not_in_segment(a1, a2, asub,
                              makevecteur(symb_pnt(normal(a1 + alpha * v, contextptr),
                                                   default_color(contextptr), contextptr)),
                              contextptr),
        contextptr);
  }

  static vecteur inter2droites3(const gen & a1, const gen & a2, const gen & b1, const gen & b2,
                                int asub, int bsub, GIAC_CONTEXT) {
    ...
    if (perpendiculaire_commune(makevecteur(a1, a2), makevecteur(b1, b2), M, N, n, contextptr)) {
      if (is_zero(ratnormal(M - N, contextptr), contextptr))
        return remove_not_in_segment(
            b1, b2, bsub,
            remove_not_in_segment(a1, a2, asub,
                                  makevecteur(symb_pnt(M, default_color(contextptr), contextptr)),
                                  contextptr),
            contextptr);
    }
    return vecteur(0);
  ...
  gen _circonscrit(const gen & arg_orig, GIAC_CONTEXT) {
    if (arg_orig.type == _STRNG && arg_orig.subtype == -1)
      return arg_orig;
    vecteur attributs(1, default_color(contextptr));
    ...
    gen ef = (e + f) / 2;
    gen fg = (f + g) / 2;
    vecteur w(inter2droites2(ef, ef + cst_i * (f - e), fg, fg + cst_i * (g - f), _LINE__VECT,
                             _LINE__VECT, contextptr));
    if (w.empty())
      return gensizeerr(contextptr);
    h = remove_at_pnt(w.front());
    return pnt_attrib(
        symbolic(at_cercle, gen(makevecteur(makevecteur(e, 2 * h - e), 0, 2 * cst_pi), _PNT__VECT)),
        attributs, contextptr);
  ...
  static gen inscrit(const gen & args, const vecteur & attributs, bool exinscrit, GIAC_CONTEXT) {
    ...
    if (est_aligne(A, B, C, contextptr) || is_undef(A) || is_undef(B) || is_undef(B))
      return undef;
    gen a2 = distance2pp(B, C, contextptr), b2 = distance2pp(C, A, contextptr),
        c2 = distance2pp(A, B, contextptr);
    ...
    return pnt_attrib(
        symbolic(at_cercle, gen(makevecteur(makevecteur(I - r, I + r), 0, 2 * cst_pi), _PNT__VECT)),
        attributs, contextptr);
  ...
  bool on(const gen & e_orig, const gen & f, gen & t, GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      if (is_zero(
              normal(subst(im(ee[0], contextptr), tt, *it, false, contextptr) - im(f, contextptr),
                     contextptr),
              contextptr) &&
  ...
  gen projection(const gen & e, const gen & pp, GIAC_CONTEXT) {
    ...
    if ((sur.type == _VECT) && (sur._VECTptr->size() >= 2)) {
      ...
      for (int i = 0; i < s - 1; ++i) {
        ...
        if (!is_undef(c) &&
            (i == 0 || is_strictly_greater(dist, (cur_dist = distance2pp(p, cur_proj, contextptr)),
                                           contextptr))) {
    ...
    if ((sur.type == _SYMB) && (sur._SYMBptr->sommet == at_curve)) {
      ...
      try {
        ...
        if (numereq) {
          ...
          if (tmp.type != _VECT)
            ...
            if (is_zero(im(d1, contextptr))) {
              ...
              eq = d * ((x1 * d - d1 * x) * mx + (y1 * d - d1 * y) * my) + (x * x + y * y) * d1 -
                   d * (x1 * x + y1 * y);
              if (!conique)
        ...
        if (has_num_coeff(eq) && !conique) {
          ...
          if (sol.empty()) {
            vecteur eqv(makevecteur(eq, symb_equal(v[1], symb_interval(v[2], v[3])),
                                    symb_equal(change_subtype(_NSTEP, _INT_PLOT), 30), 1e-4));
            sol = gen2vecteur(in_fsolve(eqv, contextptr));
          }
        } else {
          if (approxafter && lvarxwithinv(eq, tt, contextptr).size() < 2) {
            sol = gen2vecteur(_proot(makesequence(evalf(eq, 1, contextptr), tt),
                                     contextptr)); //eq=evalf(eq,1,contextptr);
          } else
...
#ifdef NO_STDEXCEPT
        distance2_found =
            distance2pp(limit(v[0], *v[1]._IDNTptr, v[2], 0, contextptr), p, contextptr);
#else
    try {
      if (conique)
        distance2_found =
            distance2pp(subst(v[0], *v[1]._IDNTptr, v[2], false, contextptr), p, contextptr);
      else
        distance2_found =
            distance2pp(limit(v[0], *v[1]._IDNTptr, v[2], 0, contextptr), p, contextptr);
    } catch (std::runtime_error) {
      last_evaled_argptr(contextptr) = NULL;
        ...
        for (; it != itend; ++it) {
          ...
          if (conique)
            cur_distance2 =
                distance2pp(subst(v[0], *v[1]._IDNTptr, *it, false, contextptr), p, contextptr);
          else
            cur_distance2 =
                distance2pp(limit(v[0], *v[1]._IDNTptr, *it, 0, contextptr), p, contextptr);
          if (is_undef(cur_distance2))
            continue;
  ...
  gen cercle2curve(const gen & f, GIAC_CONTEXT) {
    ...
    return symb_curve(
        gen(makevecteur(centre + normal(rayon, contextptr) * symb_exp(cst_i * t__IDNT_e), t__IDNT_e,
                        zero, cst_two_pi),
            _PNT__VECT),
        f);
  }
  ...
  gen line2curve(const gen & f) {
    ...
    return symb_curve(
        gen(makevecteur(ratnormal(t * B + (1 - t) * A, context0), t, tmin, tmax), _PNT__VECT), f);
  }
  ...
  static gen distance2sp(const_iterateur it, const const_iterateur itend, const gen & p0,
                         int subtype, GIAC_CONTEXT) {
    ...
    for (;;) {
      ...
      if (subtype == _LINE__VECT ||
          (ck_is_positive(t, contextptr) &&
           (subtype == _HALFLINE__VECT || ck_is_greater(1, t, contextptr))))
  ...
  gen distance2pp(const gen & ee, const gen & ff, GIAC_CONTEXT) {
    ...
    if (e.type == _VECT) {
      if (e.subtype == _POINT__VECT ||
          (f.type == _VECT && f.subtype == _POINT__VECT && e.subtype != _LINE__VECT)) { // n-d point
      ...
      if (e._VECTptr->size() == 2) { // e of type line
        ...
        if (f.type == _VECT && f._VECTptr->size() == 2) { // line to line
          ...
          if (e._VECTptr->front().type != _VECT) { // 2-d
            ...
            return pow(dotvecteur(n, makevecteur(re(C - A, contextptr), im(C - A, contextptr))), 2,
                       contextptr) /
                   dotvecteur(n, n);
          } else { // 3-d
    ...
    if ((e.type == _SYMB) && (e._SYMBptr->sommet == at_cercle)) {
      if ((f.type == _SYMB) && (f._SYMBptr->sommet == at_cercle)) {
        ...
        if (!centre_rayon(e, centre1, rayon1, true, contextptr) ||
  ...
  gen distance2(const gen & f1, const gen & f2, GIAC_CONTEXT) {
    ...
    if (e2.is_symb_of_sommet(at_equal)) {
      ...
      if (!pnt && e1.type == _SYMB)
        pnt = e1._SYMBptr->sommet != at_curve && e1._SYMBptr->sommet != at_cercle &&
              e1._SYMBptr->sommet != at_hypersphere && e1._SYMBptr->sommet != at_hypersurface &&
              e1._SYMBptr->sommet != at_hyperplan;
      if (pnt) {
        gen e3 = e2._SYMBptr->feuille;
        if (e3.type == _VECT && e3._VECTptr->size() == 2) {
          e3 = e3._VECTptr->back() - e3._VECTptr->front();
          gen a, A, b, c;
          if (is_linear_wrt(e3, x__IDNT_e, a, A, contextptr) &&
              is_linear_wrt(A, y__IDNT_e, b, c, contextptr) && (a != 0 || b != 0)) {
      ...
      e2 = plotimplicit(equal2diff(e2), x__IDNT_e, y__IDNT_e, gnuplot_xmin, gnuplot_xmax,
                        gnuplot_ymin, gnuplot_ymax, 20 * gnuplot_pixels_per_eval, 0,
                        epsilon(contextptr), vecteur(1, default_color(contextptr)), false,
                        true /* cklinear */, contextptr, 3);
  ...
  gen _longueur(const gen & args, GIAC_CONTEXT) {
    ...
    if (e1.type == _CPLX && e1.subtype == 3 && e2.type == _CPLX && e2.subtype == 3) {
      double x = e1._CPLXptr->_DOUBLE_val - e2._CPLXptr->_DOUBLE_val,
             y = (e1._CPLXptr + 1)->_DOUBLE_val - (e2._CPLXptr + 1)->_DOUBLE_val;
  ...
  gen approx_area(const gen & f, const gen & x, const gen & a_, const gen & b_, int n, int method,
                  GIAC_CONTEXT) {
    gen a(a_), b(b_);
  ...
  gen _aire(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && !args._VECTptr->empty() &&
        args._VECTptr->front().is_symb_of_sommet(at_pnt) &&
        args._VECTptr->back().is_symb_of_sommet(at_pnt)) {
    ...
    if (g.is_symb_of_sommet(at_cercle)) {
      ...
      if (g._SYMBptr->feuille.type == _VECT && g._SYMBptr->feuille._VECTptr->size() >= 3)
        return normal(((*g._SYMBptr->feuille._VECTptr)[2] - (*g._SYMBptr->feuille._VECTptr)[1]) *
                          (rayon * conj(rayon, contextptr)) / 2,
                      contextptr);
	...
	if (v[3].type==_DOUBLE_ && v[3]==2*M_PI)
	  v[3]=cst_two_pi;
        ...
        gen I2 =
            (subst(x, v[1], v[3], false, contextptr) - subst(x, v[1], v[2], false, contextptr)) *
            (subst(y, v[1], v[2], false, contextptr) + subst(y, v[1], v[3], false, contextptr)) / 2;
        return I1 + I2;
      }
    }
    ...
    if (s == 2 && (v[1].is_symb_of_sommet(at_equal) || v[1].is_symb_of_sommet(at_interval))) {
      ...
      if (tmp.is_symb_of_sommet(at_equal) && tmp._SYMBptr->feuille.type == _VECT &&
          tmp._SYMBptr->feuille._VECTptr->size() == 2) {
        x = tmp._SYMBptr->feuille[0];
        tmp = tmp._SYMBptr->feuille[1];
      }
      if (tmp.is_symb_of_sommet(at_interval) && tmp._SYMBptr->feuille.type == _VECT &&
          tmp._SYMBptr->feuille._VECTptr->size() == 2) {
    ...
    for (int i = 3; i < s; ++i) {
      ...
      if (cote1.type == _VECT && cote2.type == _VECT)
        ...
        res += im(cote2, contextptr) * re(cote1, contextptr) -
               re(cote2, contextptr) * im(cote1, contextptr);
    }
  ...
  gen _perimetre(const gen & args, GIAC_CONTEXT) {
    ...
    if (g.is_symb_of_sommet(at_cercle)) {
      ...
      if (!centre_rayon(g, centre, rayon, true, contextptr))
        return gensizeerr(contextptr);
      if (g._SYMBptr->feuille.type == _VECT && g._SYMBptr->feuille._VECTptr->size() >= 3)
        return recursive_normal(
            ((*g._SYMBptr->feuille._VECTptr)[2] - (*g._SYMBptr->feuille._VECTptr)[1]) * rayon,
            contextptr);
    ...
    if (g.is_symb_of_sommet(at_curve)) { // -> arclen
      ...
      if (f.type == _VECT && f._VECTptr->size() >= 4) {
        ...
        return _integrate(makesequence(symbolic(at_sqrt, x * x + y * y), v[1], v[2], v[3]),
                          contextptr);
      }
    }
    if (g.type != _VECT)
  ...
  gen angle(const vecteur & v1, const vecteur & v2, GIAC_CONTEXT) {
    return acos(
        simplify(dotvecteur(v1, v2) / sqrt(dotvecteur(v1, v1) * dotvecteur(v2, v2), contextptr),
                 contextptr),
        contextptr);
  }
  ...
  gen _angle(const gen & args, GIAC_CONTEXT) {
    ...
    if (e.type == _VECT && e.subtype == _GGBVECT && f.type == _VECT && f.subtype == _GGBVECT) {
      if (e._VECTptr->size() == 2 && f._VECTptr->size() == 2)
        return arg((f._VECTptr->front() + cst_i * f._VECTptr->back()) /
                       (e._VECTptr->front() + cst_i * e._VECTptr->back()),
                   contextptr);
    ...
    if (f.is_symb_of_sommet(at_hyperplan)) {
      ...
      if (e.type == _VECT && e._VECTptr->size() == 3 && f.type == _VECT &&
          f._VECTptr->size() == 3 && g.type == _VECT && g._VECTptr->size() == 3) {
        vecteur v1(*(f - e)._VECTptr), v2(*(g - e)._VECTptr);
        res = angle(v1, v2, contextptr);
      } else
        res = recursive_normal(
            arg(simplify(conj(f - e, contextptr) * (g - e), contextptr), contextptr), contextptr);
    }
    gen c, c2;
    ...
    if (montrer) {
      ...
      if (is_zero(evalf(abs(res, contextptr) - cst_pi_over_2, 1, contextptr), contextptr)) {
        ...
        c = symbolic(at_cercle,
                     gen(makevecteur(makevecteur(e - ef, e + ef), 0, res, 1), _PNT__VECT));
    ...
    if (montrer && c.is_symb_of_sommet(at_pnt) && c._SYMBptr->feuille.type == _VECT &&
        c._SYMBptr->feuille._VECTptr->size() == 2) {
  ...
  static bool is_near(const gen & a, const gen & b0, double eps, GIAC_CONTEXT) {
    ...
    if (b.is_symb_of_sommet(at_curve) && b._SYMBptr->feuille.type == _VECT &&
        b._SYMBptr->feuille._VECTptr->size() > 1) {
      gen tmp = (*b._SYMBptr->feuille._VECTptr)[1];
      return is_near(a, tmp, eps, contextptr);
    }
    if (b.is_symb_of_sommet(at_hypersurface) && b._SYMBptr->feuille.type == _VECT &&
        b._SYMBptr->feuille._VECTptr->size() >= 2) {
      ...
      for (int i = 0; it != itend; ++it, ++i) {
        ...
        for (; jt != jtend; ++jt) {
          ...
          if ((g.type == _SYMB) && equalposcomp(plot_sommets, g._SYMBptr->sommet) &&
              !g.is_symb_of_sommet(at_parameter)) {
            qf = remove_at_pnt(evalf(g, 1, contextptr));
            if ((qf.is_symb_of_sommet(at_curve) || qf.is_symb_of_sommet(at_hypersurface) ||
                 lidnt(qf).empty()) &&
                is_near(pf, qf, eps, contextptr)) {
              if (is_segment(qf))
      ...
      if (res.type == _VECT) {
        if (vs == 4 && res._VECTptr->size() == 2) {
          ...
          if (tmp[1].is_symb_of_sommet(at_sto)) {
            ...
            if (f.type == _VECT && f._VECTptr->size() == 2) {
              tmp[1] = symb_sto(string2gen(f._VECTptr->front().print(contextptr), false),
                                f._VECTptr->back());
...
unary_function_eval __click(1, &_click, _click_s);
  ...
  static gen in_parameter2point(const vecteur & v, GIAC_CONTEXT) {
    ...
    gen res;
    if (v.size() < 2)
      return gensizeerr(gettext("plot.cc/parameter2point"));
    ...
    gen attribut = default_color(contextptr);
    if (v0.is_symb_of_sommet(at_pnt) && v0._SYMBptr->feuille.type == _VECT &&
        v0._SYMBptr->feuille._VECTptr->size() > 1)
    ...
    if (geo_obj.type == _VECT) {
      ...
      if (t.type != _VECT && ligne.size() == 2) {
        if ((geo_obj.subtype == _GROUP__VECT || geo_obj.subtype == _HALFLINE__VECT) &&
            is_positive(-t, contextptr))
          t = 0;
      ...
      if (n < 0)
        return symb_pnt(ligne.front(), attribut, contextptr);
    ...
    if ((geo_obj.type == _SYMB) && (geo_obj._SYMBptr->sommet == at_cercle)) {
      ...
      if (!centre_rayon(geo_obj, centre, rayon, true, contextptr))
        return gensizeerr(contextptr);
      return symb_pnt(centre + normal(rayon, contextptr) * exp(cst_i * t, contextptr), attribut,
                      contextptr);
    }
  ...
  static bool reeval_with_1arg_quoted(const gen & x, gen & res, gen & def_x, GIAC_CONTEXT) {
    ...
    def_x = undef;
    const_iterateur it0 = history_in(contextptr).begin() - 1, itend = history_in(contextptr).end(),
                    itpos, it;
    it = itend - 1;
    vecteur localvars;
    for (;; --it) {
  ...
  gen _as_function_of(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    if (rpn_mode(contextptr) || (args.type != _VECT) || (args._VECTptr->size() != 2) ||
        (args._VECTptr->back().type != _IDNT))
  ...
  define_unary_function_ptr5(at_as_function_of, alias_at_as_function_of, &__as_function_of,
                             _QUOTE_ARGUMENTS, true);
  ...
  // 2 lines and circles
  // >2 all conics
  bool equation2geo2d(const gen & f0, const gen & x, const gen & y, gen & g, double tmin,
                      double tmax, double tstep, const gen & pointon, int allowed,
    ...
    if (fx0 && fy0)
      return false;
    gen fxx(derive(fx, x, contextptr)), fxy(derive(fx, y, contextptr)),
        fyy(derive(fy, y, contextptr));
    if (is_undef(fx) || is_undef(fy) || is_undef(fxx) || is_undef(fxy) || is_undef(fyy))
      return false;
    if (is_zero(derive(fxx, x, contextptr), contextptr) &&
        is_zero(derive(fxy, x, contextptr), contextptr) &&
        is_zero(derive(fyy, x, contextptr), contextptr) &&
        is_zero(derive(fxx, y, contextptr), contextptr) &&
        is_zero(derive(fxy, y, contextptr), contextptr) &&
        is_zero(derive(fyy, y, contextptr), contextptr)) {
      vecteur vxy(makevecteur(x, y)), v0(2, 0);
      gen c = ratnormal(subst(f, vxy, v0, false, contextptr), contextptr);
      fxx = ratnormal(fxx, contextptr);
      fyy = ratnormal(fyy, contextptr);
      fxy = ratnormal(fxy, contextptr);
      if (is_zero(fxy, contextptr)) {
        if (is_zero(fxx, contextptr) && is_zero(fyy, contextptr)) {
          gen d = gcd(fx, fy);
          fx = normal(fx / d, contextptr);
          fy = normal(fy / d, contextptr);
          c = normal(c / d, contextptr);
        ...
        if (is_zero(fxx - fyy, contextptr)) {
          ...
          g = symbolic(at_cercle, gen(makevecteur(gen(makevecteur(centre - rayon, centre + rayon),
                                                      _GROUP__VECT),
                                                  0, cst_two_pi),
                                      _PNT__VECT));
          return true;
      ...
      vecteur V0, V1, param_curves;
      if (conique_reduite(eq, pointon, makevecteur(x__IDNT_e, y__IDNT_e), x0, y0, V0, V1, propre,
                          equation_reduite, param_curves, ratparam, true, contextptr) <= 0)
        return false;
      vecteur res;
  ...
  bool find_curve_parametrization(const gen & geo_obj, gen & m, const gen & gen_t, double T,
                                  gen & tmin, gen & tmax, bool tminmax_defined, GIAC_CONTEXT) {
    if (gen_t.type != _IDNT)
      return false;
        ...
        if (fg.type == _VECT && fg._VECTptr->size() > 3) {
          ...
          if (lv.size() == 3) {
            ...
            if (lv == sincost) {
              ...
              m = subst(m, sincost, makevecteur(2 * gen_t / t2p1, (1 - gen_t * gen_t) / t2p1),
                        false, contextptr);
              if (!tminmax_defined) {
                tmin = -T;
                tmax = T;
              ...
              if (lv == sincost) {
                m = subst(m, sincost,
                          makevecteur((gen_t - inv(gen_t, contextptr)) / 2,
                                      (gen_t + inv(gen_t, contextptr)) / 2),
                          false, contextptr);
                if (!tminmax_defined) {
                  tmin = -T;
                  tmax = T;
    ...
    if (geo_obj.type == _VECT && geo_obj._VECTptr->size() == 2) {
      m = geo_obj._VECTptr->front() +
          gen_t * (geo_obj._VECTptr->back() - geo_obj._VECTptr->front());
      if (!tminmax_defined) {
        tmin = 0;
        tmax = 1;
  ...
  static bool doublify(const gen & tmin, const gen & tmax, double T, double & tmin_d,
                       double & tmax_d, GIAC_CONTEXT) {
    tmin_d = gnuplot_tmin;
    tmax_d = gnuplot_tmax;
  ...
  void read_tmintmaxtstep(vecteur & vargs, gen & t, int vstart, double & tmin, double & tmax,
                          double & tstep, bool & tminmax_defined, bool & tstep_defined,
                          GIAC_CONTEXT) {
    tstep = gnuplot_tstep;
    ...
    for (int i = vstart; i < vs; ++i) {
      ...
      if (vargs[i].is_symb_of_sommet(at_equal) && vargs[i]._SYMBptr->feuille.type == _VECT &&
          vargs[i]._SYMBptr->feuille._VECTptr->front().type == _INT_) {
  ...
  gen _lieu(const gen & args, GIAC_CONTEXT) {
    ...
    double Tmin, Tmax;
    gen tcopy(t__IDNT_e);
    read_tmintmaxtstep(vargs, tcopy, 2, Tmin, Tmax, tstep, tminmax_defined, tstep_defined,
                       contextptr);
    if (tminmax_defined) {
      tmin = Tmin;
      tmax = Tmax;
    ...
    try {
      ...
      if (tt.type == _IDNT) {
        ...
        const_iterateur it0 = history_out(contextptr).begin() - 1,
                        itend = history_out(contextptr).end(), it;
        it = itend - 1;
        for (;; --it) {
        ...
        // type checking
        if ((tt.type != _SYMB) || (tt._SYMBptr->sommet != at_pnt) ||
            (tt._SYMBptr->feuille.type != _VECT))
          return gensizeerr(contextptr);
        ...
        if (!find_curve_parametrization(geo_obj, m, gen_t, T, tmin, tmax, tminmax_defined,
                                        contextptr))
      ...
      if (!is_zero(N, contextptr)) {
        ...
        if (N.type == _VECT && N._VECTptr->size() == 2) {
          ...
          gen ap(derive(a, gen_t, contextptr)), bp(derive(b, gen_t, contextptr)),
              cp(derive(c, gen_t, contextptr));
        ...
        lieu_eq = _factor(lieu_eq, contextptr);
          ...
          if (equation2geo2d(lieu_eq, x, y, geoobj, tmin_d, tmax_d, tstep_d, undef, 3,
                             contextptr)) {
            vecteur lieu_vect;
        ...
        return _plotparam(gen(makevecteur(N, symb_equal(gen_t, symb_interval(tmin, tmax)),
                                          symb_equal(ntstep, gtstep)),
                              _SEQ__VECT),
                          contextptr);
      }
      if (w.size() < 2)
  ...
  gen apply3d(const gen & e1, const gen & e2, const context * contextptr,
              gen (*f)(const gen &, const gen &, const context *)) {
    if (is_undef(e2))
      return e2;
  ...
  gen curve_surface_apply(const gen & elem, const gen & b,
                          gen (*func)(const gen &, const gen &, const context *), GIAC_CONTEXT) {
    if (func && (b.is_symb_of_sommet(at_curve) || b.is_symb_of_sommet(at_hypersurface)) &&
  ...
  static gen symetrie_droite(const gen & w, const gen & w0, const gen & b, GIAC_CONTEXT) {
    ...
    return b -
           plus_two * w *
               rdiv(scalar_product(b - w0, w, contextptr), w.squarenorm(contextptr), contextptr);
  }
  ...
  static gen rotation(const vecteur & v, int s, GIAC_CONTEXT) {
    if (s == 3) {
      ...
      if (b.type == _VECT && b.subtype == _VECTOR__VECT && b._VECTptr->size() == 2)
        return _vector(
            gen(makevecteur(
                    rotation(makevecteur(centre, angle, b._VECTptr->front()), s, contextptr),
                    rotation(makevecteur(centre, angle, b._VECTptr->back()), s, contextptr)),
                _SEQ__VECT),
            contextptr);
      ...
      if (centre.type != _VECT) // 2-d rotation
        return symb_pnt(rotation(centre, angle, b, contextptr), default_color(contextptr),
                        contextptr);
  ...
  static gen symetrie(const gen & aa, const gen & bb, GIAC_CONTEXT) {
    ...
    if (b.type == _VECT && b.subtype == _VECTOR__VECT && b._VECTptr->size() == 2)
      return _vector(gen(makevecteur(symetrie(aa, b._VECTptr->front(), contextptr),
                                     symetrie(aa, b._VECTptr->back(), contextptr)),
                         _SEQ__VECT),
                     contextptr);
    if (a.is_symb_of_sommet(at_hyperplan)) { // symmetry of b wrt plan
      ...
      if (b.type == _VECT)
        return symb_pnt(apply3d(elem, b, contextptr, symetrieplan), default_color(contextptr),
                        contextptr);
      ...
      if (b.is_symb_of_sommet(at_hypersphere)) { // hypersphere wrt plan
        ...
        if (!centre_rayon(b, c, r, false, contextptr))
          return gensizeerr(contextptr);
        return _sphere(makevecteur(c - scalar_product(n, c - P, contextptr) / n2 * n, r),
                       contextptr);
      }
  ...
  static gen symetrie(const vecteur & v, int s, GIAC_CONTEXT) {
    ...
    if (s == 1) {
      return symb_program(x__IDNT_e, zero,
                          symbolic(at_symetrie, gen(makevecteur(v[0], x__IDNT_e), _SEQ__VECT)),
                          contextptr);
    }
    return gentypeerr(contextptr);
  }
  ...
  static gen projectionpoint(const gen & aa, const gen & bb, GIAC_CONTEXT) {
    gen b = remove_at_pnt(bb);
    if (b.type == _VECT && b.subtype == _VECTOR__VECT && b._VECTptr->size() == 2)
      return _vector(gen(makevecteur(projectionpoint(aa, b._VECTptr->front(), contextptr),
                                     projectionpoint(aa, b._VECTptr->back(), contextptr)),
                         _SEQ__VECT),
                     contextptr);
    if (aa.is_symb_of_sommet(at_hypersphere)) {
      ...
      if (f.type == _VECT && f._VECTptr->size() >= 2) {
        const vecteur & v = *f._VECTptr;
        gen c = v.front();
        if (c.type == _VECT && c._VECTptr->size() == 3 && b.type == _VECT &&
            b._VECTptr->size() == 3) {
  ...
  static gen projection(const vecteur & v, int s, GIAC_CONTEXT) {
    ...
    if (s == 1)
      return symb_program(x__IDNT_e, zero,
                          symbolic(at_projection, gen(makevecteur(v[0], x__IDNT_e), _SEQ__VECT)),
                          contextptr);
    return gentypeerr(contextptr);
  }
  ...
  static gen homothetie(const vecteur & v, int s, GIAC_CONTEXT) {
    if (s == 3) {
      ...
      if (b.type == _VECT && b.subtype == _VECTOR__VECT && b._VECTptr->size() == 2)
        return _vector(
            gen(makevecteur(
                    homothetie(makevecteur(centre, rapport, b._VECTptr->front()), s, contextptr),
                    homothetie(makevecteur(centre, rapport, b._VECTptr->back()), s, contextptr)),
                _SEQ__VECT),
            contextptr);
  ...
  define_unary_function_ptr5(at_est_coplanaire, alias_at_est_coplanaire, &__est_coplanaire, 0,
                             true);
  ...
  bool est_cocyclique(const gen & a, const gen & b, const gen & c, const gen & d, GIAC_CONTEXT) {
    ...
    if (a.type == _VECT && a._VECTptr->size() == 3) {
      if (!est_coplanaire(a, b, c, d, contextptr))
        return false;
      return (est_aligne(a + ab / abs_norm2(ab, contextptr), a + ac / abs_norm2(ac, contextptr),
                         a + ad / abs_norm2(ad, contextptr), contextptr)) != 0;
    }
  ...
  define_unary_function_ptr5(at_est_cocyclique, alias_at_est_cocyclique, &__est_cocyclique, 0,
                             true);
  ...
  gen _est_parallele(const gen & args, GIAC_CONTEXT) {
    ...
    if ((args.type == _VECT) && (args._VECTptr->size() == 2)) {
      ...
      if (b.is_symb_of_sommet(at_hyperplan) && av)
        return is_zero(
            simplify(scalar_product(hyperplan_normal(b), a[0] - a[1], contextptr), contextptr),
            contextptr);
      if (!av || !bv)
        return gensizeerr(contextptr);
  ...
  gen est_dans(const gen & a_, const gen & b_, GIAC_CONTEXT) {
    ...
    if (M.type <= _CPLX || M.type == _SYMB || M.type == _IDNT || M.type == _FLOAT_ ||
        M.type == _FRAC) {
      if (b.is_symb_of_sommet(at_cercle)) {
        gen c, r;
      ...
      if (b.type == _VECT && b._VECTptr->size() >= 3 &&
          is_zero(b._VECTptr->front() - b._VECTptr->back(), contextptr)) {
        // point in a polygon?
  ...
  static gen inversion(const gen & centre, const gen & rapport, const gen & argument,
                       GIAC_CONTEXT) {
    ...
    if (b.type == _VECT && b.subtype != _POINT__VECT) {
      ...
      if (est_aligne(centre, c, d, contextptr)) {
        if (b.subtype == _LINE__VECT)
          return b;
        return gen(makevecteur(inversion(centre, rapport, c, contextptr),
                               inversion(centre, rapport, d, contextptr)),
                   b.subtype);
      }
      ...
      if (is_undef(image))
        return image;
      if (b.subtype == _LINE__VECT)
        return remove_at_pnt(_cercle(
            gen(makevecteur((centre + image) / 2, (image - centre) / 2), _SEQ__VECT), contextptr));
      ...
      return remove_at_pnt(
          _cercle(gen(makevecteur(p, abs(image - centre, contextptr) / 2, a1, a2), _SEQ__VECT),
                  contextptr));
    }
  ...
  gen inversion(const vecteur & v, int s, GIAC_CONTEXT) {
    if (s == 3) {
      ...
      if ((centre.type == _VECT && centre.subtype != _POINT__VECT) ||
          (centre.type == _SYMB && centre._SYMBptr->sommet == at_cercle))
        return gensizeerr(contextptr);
      if (b.type == _VECT) {
        const_iterateur it = b._VECTptr->begin(), itend = b._VECTptr->end();
        vecteur res;
      ...
      return symb_pnt(inversion(centre, rapport, b, contextptr), default_color(contextptr),
                      contextptr);
    }
    if (s == 2) {
      return symb_program(
          x__IDNT_e, zero,
          symbolic(at_inversion, gen(makevecteur(v[0], v[1], x__IDNT_e), _SEQ__VECT)), contextptr);
    }
    return gentypeerr(contextptr);
  }
  ...
  static gen similitude(const vecteur & v, int s, GIAC_CONTEXT) {
    if (s == 4) {
      ...
      if (b.type == _VECT && b.subtype == _VECTOR__VECT && b._VECTptr->size() == 2)
        return _vector(
            gen(makevecteur(similitude(makevecteur(centre, rapport, angle, b._VECTptr->front()), s,
                                       contextptr),
                            similitude(makevecteur(centre, rapport, angle, b._VECTptr->back()), s,
                                       contextptr)),
                _SEQ__VECT),
            contextptr);
      if (centre.type != _VECT)
        return symb_pnt(centre + rapport * exp(cst_i * angletorad(angle, contextptr), contextptr) *
                                     (b - centre),
                        default_color(contextptr), contextptr);
  ...
  static gen translationpoint(const gen & a, const gen & b, GIAC_CONTEXT) {
    if (has_i(a) || (a.type == _VECT && a._VECTptr->size() > 3)) {
      ...
      if (!is_analytic(b) && !b.is_symb_of_sommet(at_cercle) &&
          evalf(b, 1, contextptr).type == _SYMB)
        return gensizeerr(contextptr);
    }
    return a + b;
  }
  ...
  static gen translation(const vecteur & v, int s, GIAC_CONTEXT) {
    if (s == 2) {
      ...
      if (a.is_symb_of_sommet(at_pnt)) {
        a = remove_at_pnt(a);
        if (a.type == _VECT && (a.subtype == _VECTOR__VECT || a.subtype == _GROUP__VECT) &&
            a._VECTptr->size() == 2) {
          return translation(makevecteur(a._VECTptr->back() - a._VECTptr->front(), v[1]), s,
                             contextptr);
        }
    ...
    if (s == 1) {
      return symb_program(x__IDNT_e, zero,
                          symbolic(at_translation, gen(makevecteur(v[0], x__IDNT_e), _SEQ__VECT)),
                          contextptr);
    }
    return gentypeerr(contextptr);
  }
  ...
  static string printascurve(const gen & feuille, const char * sommetstr, GIAC_CONTEXT) {
    ...
    return string(sommetstr) +
           ('(' + feuille._VECTptr->front().print(contextptr) + string(",undef)"));
  }
  /*
  ...
  */
  ...
  gen plotparam(const gen & f, const gen & vars, const vecteur & attributs, int densityplot,
                double function_xmin, double function_xmax, double function_ymin,
                double function_ymax, double function_tmin, double function_tmax,
                double function_tstep, const gen & equation, const gen & parameq,
                const gen & vparam, const context * contextptr) {
    if (function_tstep <= 0 || (function_tmax - function_tmin) / function_tstep > max_nstep ||
        function_tmax == function_tmin)
      return gensizeerr(gettext("Plotparam: unable to discretize: tmin, tmax, tstep=") +
                        print_DOUBLE_(function_tmin, 12) + "," + print_DOUBLE_(function_tmax, 12) +
                        "," + print_DOUBLE_(function_tstep, 12) +
    ...
    for (int count = 0; !ctrl_c && !interrupted && count <= nstep; ++count, t += function_tstep) {
      ...
      if (joindre)
        ...
        if (!chemin.empty())
          res.push_back(symb_pnt(
              symb_curve(gen(makevecteur(fC, vars, function_tmin, t, 0, equation, parameq, vparam),
                             _PNT__VECT),
                         gen(chemin, _GROUP__VECT)),
              attribut, contextptr));
        function_tmin = t;
    ...
    if (!chemin.empty())
      res.push_back(symb_pnt(symb_curve(gen(makevecteur(fC, vars, function_tmin, function_tmax, 0,
                                                        equation, parameq, vparam),
                                            _PNT__VECT),
                                        gen(chemin, _GROUP__VECT)),
                             attribut, contextptr));
    leave(protect, localvar, newcontextptr);
  ...
  gen plotparam(const gen & f, const gen & vars, const vecteur & attributs, int densityplot,
                ...
                double function_tstep, const gen & equation, const gen & parameq,
                const context * contextptr) {
    return plotparam(f, vars, attributs, densityplot, function_xmin, function_xmax, function_ymin,
                     function_ymax, function_tmin, function_tmax, function_tstep, equation, parameq,
                     undef, contextptr);
  }
  ...
  gen paramplotparam(const gen & args, int densityplot, const context * contextptr) {
    ...
    */
    ...
    if (s > 7)
      vparam = vargs[7];
    double umin = gnuplot_tmin, vmin = gnuplot_tmin, umax = gnuplot_tmax, vmax = gnuplot_tmax,
           tmin = gnuplot_tmin, tmax = gnuplot_tmax, xmin = gnuplot_xmin, xmax = gnuplot_xmax,
           ymin = gnuplot_ymin, ymax = gnuplot_ymax, zmin = gnuplot_zmin, zmax = gnuplot_zmax,
           tstep = -1, ustep = -1, vstep = -1;
    if (param2d && s >= 3) {
      gen trange = vargs[2].evalf_double(eval_level(contextptr), contextptr);
      if ((trange.type == _SYMB) && (trange._SYMBptr->sommet == at_interval)) {
        tmin = evalf_double(trange._SYMBptr->feuille._VECTptr->front(), eval_level(contextptr),
                            contextptr)
                   ._DOUBLE_val;
        tmax = evalf_double(trange._SYMBptr->feuille._VECTptr->back(), eval_level(contextptr),
                            contextptr)
                   ._DOUBLE_val;
        if (s > 3)
    ...
    if (f.type == _VECT && f._VECTptr->size() == 3 && vars.type != _VECT) { // 3-d curve
      if (s == 4) {
        if (vargs[2].is_symb_of_sommet(at_interval)) {
          ...
          vars =
              symbolic(at_equal,
                       makesequence(vars, symbolic(at_interval, makesequence(vargs[2], vargs[3]))));
      }
      vars = makevecteur(
          vars, symbolic(at_equal,
                         makesequence(v__IDNT_e, symbolic(at_interval, makesequence(vmin, vmax)))));
      curve3d = true;
    }
    if (tstep < 0)
    ...
    if (vars.type == _VECT && vars._VECTptr->size() >= 2) {
      ...
      if (v.size() != 2)
        return gensizeerr(contextptr);
      if (readrange(v[0], tmin, tmax, v[0], umin, umax, contextptr) &&
          readrange(v[1], tmin, tmax, v[1], vmin, vmax, contextptr)) {
        if (ustep < 0)
          ustep = (umax - umin) /
                  (curve3d ? gnuplot_pixels_per_eval : std::sqrt(double(gnuplot_pixels_per_eval)));
        if (vstep < 0)
          vstep = (vmax - vmin) / std::sqrt(double(gnuplot_pixels_per_eval));
        return plotparam3d(f, makevecteur(v[0], v[1]), xmin, xmax, ymin, ymax, zmin, zmax, umin,
                           umax, vmin, vmax, densityplot, f_autoscale, attributs, ustep, vstep,
                           undef, vecteur(0), contextptr);
      }
      return gensizeerr(contextptr);
    }
    if (!readrange(vars, tmin, tmax, vars, tmin, tmax, contextptr))
      return gensizeerr(gettext("2nd arg must be a free variable"));
    return plotparam(f, vars, attributs, densityplot, xmin, xmax, ymin, ymax, tmin, tmax, tstep, eq,
                     parameq, vparam, contextptr);
  }
  ...
  define_unary_function_ptr5(at_plotparam, alias_at_plotparam, &__plotparam, _QUOTE_ARGUMENTS,
                             true);

  static const char _courbe_parametrique_s[] = "courbe_parametrique";
  static define_unary_function_eval_quoted(__courbe_parametrique, &_plotparam,
                                           _courbe_parametrique_s);
  define_unary_function_ptr5(at_courbe_parametrique, alias_at_courbe_parametrique,
                             &__courbe_parametrique, _QUOTE_ARGUMENTS, true);
  ...
  define_unary_function_ptr5(at_paramplot, alias_at_paramplot, &__paramplot, _QUOTE_ARGUMENTS,
                             true);
  ...
  gen _plot(const gen & g, const context * contextptr) {
    ...
    if (g.type == _VECT) {
      ...
      if (s) {
        ...
        if (a == s - 1 && attributs != vecteur(1, default_color(contextptr))) {
          gen res = _plot(a == 1 ? v.front() : gen(vecteur(v.begin(), v.begin() + a), g.subtype),
                          contextptr);
          if (res.type == _VECT) {
            ...
            for (int i = 0; i < w.size(); ++i) {
              if (w[i].is_symb_of_sommet(at_pnt)) {
                ...
                if (attributs.size() == 1 && attributs[0].type == _VECT &&
                    attributs[0]._VECTptr->size() > i)
      ...
      if (s && v[0].type == _FUNC && (nd = is_distribution(v[0])) &&
          1 + (nargs = distrib_nargs(nd)) <= s) {
        if (is_discrete_distribution(nd))
        ...
        if (d.type == _FUNC) {
          if (nargs == 1)
            ...
            v[0] = symbolic(*d._FUNCptr,
                            gen(vecteur(v.begin() + 1, v.begin() + 1 + nargs), _SEQ__VECT));
          s -= nargs;
    ...
    if (s > 1 && v0.type == _VECT && v1.type == _VECT &&
        v0._VECTptr->size() == v1._VECTptr->size()) {
      *logptr(contextptr) << gettext("Assuming you want to run polygonplot") << '\n';
      vecteur w0 = *v0._VECTptr, w1 = *v1._VECTptr;
      int ss = w0.size(), i;
      for (i = 0; i < ss; ++i) {
        if ((w0[i].type > _REAL && w0[i].type != _FRAC) ||
            (w1[i].type > _REAL && w1[i].type != _FRAC))
          break;
      }
      if (i == ss) {
        // polygonplot
    ...
    if (g.subtype != _SEQ__VECT && s == 3) {
      if (v[2].type == _IDNT)
        return plotparam(v[0] + cst_i * v[1], v[2], attributs, true, gnuplot_xmin, gnuplot_xmax,
                         gnuplot_ymin, gnuplot_ymax, gnuplot_tmin, gnuplot_tmax, gnuplot_tstep,
                         undef, undef, contextptr); // FIX equation?
      if (v[2].is_symb_of_sommet(at_equal)) {       // parametric plot
        ...
        if (gw.type == _VECT && gw._VECTptr->size() == 2) {
          ...
          if (gx.type == _IDNT && chk_double_interval(gw._VECTptr->back(), inf, sup, contextptr))
            return plotparam(v[0] + cst_i * v[1], gx, attributs, true, gnuplot_xmin, gnuplot_xmax,
                             gnuplot_ymin, gnuplot_ymax, inf, sup, gnuplot_tstep, undef, undef,
                             contextptr); // FIX EQUATION?
        }
      }
    }
    if (s < 1)
    ...
    return plotfunc(v[0], xvar, attributs, false, xmin, xmax, ymin, ymax, zmin, zmax, nstep, jstep,
                    showeq, contextptr);
  }
  ...
  gen _plotpolar(const gen & args, const context * contextptr) {
    ...
    vargs.front() = makevecteur(rho * cos(angletorad(theta, contextptr), contextptr),
                                rho * sin(angletorad(theta, contextptr), contextptr));
  ...
  define_unary_function_ptr5(at_plotpolar, alias_at_plotpolar, &__plotpolar, _QUOTE_ARGUMENTS,
                             true);

  static const char _polarplot_s[] = "polarplot";
  static define_unary_function_eval_quoted(__polarplot, &_plotpolar, _polarplot_s);
  define_unary_function_ptr5(at_polarplot, alias_at_polarplot, &__polarplot, _QUOTE_ARGUMENTS,
                             true);
  ...
  define_unary_function_ptr5(at_courbe_polaire, alias_at_courbe_polaire, &__courbe_polaire,
                             _QUOTE_ARGUMENTS, true);
  ...
  void ck_parameter(const gen & g, GIAC_CONTEXT) {
    ...
    if ((g.type == _IDNT) && (g.evalf(1, contextptr) != g))
      *logptr(contextptr) << gettext("Variable ") + g.print(contextptr) +
                                 gettext(" should be purged")
                          << '\n';
#endif
  }
  ...
  gen _parameq(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.is_symb_of_sommet(at_equal)) {
      ...
      gen x0, y0, propre, equation_reduite, ratparam;
      vecteur V0, V1, param_curves;
      if (conique_reduite(args, undef, makevecteur(vx_var, y__IDNT_e), x0, y0, V0, V1, propre,
                          equation_reduite, param_curves, ratparam, false, contextptr) > 1)
        return ratparam;
      return gensizeerr(contextptr);
    }
    vecteur v;
    ...
    gen t = v[1], u, par_eq;
    if (e.type == _SYMB &&
        (e._SYMBptr->sommet == at_hyperplan || e._SYMBptr->sommet == at_hypersphere)) {
      if (s < 2) {
      ...
      if (e._SYMBptr->sommet == at_hypersphere) {
        ...
        if (v.size() != 3)
          return gendimerr(contextptr);
        par_eq = centre + makevecteur(rayon * cos(t, contextptr) * cos(u, contextptr),
                                      rayon * cos(t, contextptr) * sin(u, contextptr),
                                      rayon * sin(t, contextptr));
      } else {
        vecteur P, n;
  ...
  gen rationalparam2equation(const gen & at_orig, const gen & t_orig, const gen & x, const gen & y,
                             GIAC_CONTEXT) {
    if (t_orig == x || t_orig == y) {
      gen t(identificateur(t_orig.print(contextptr) + "_"));
      return rationalparam2equation(subst(at_orig, t_orig, t, false, contextptr), t, x, y,
                                    contextptr);
    }
    ...
    if (at_orig.is_symb_of_sommet(at_plus) && at_orig._SYMBptr->feuille.type == _VECT) {
      ...
      if (v.size() == 2 && v[0] == t_orig && v[1].is_symb_of_sommet(at_prod) &&
          v[1]._SYMBptr->feuille.type == _VECT) {
  ...
  static gen equation(const gen & arg, const gen & x, const gen & y, const gen & z, GIAC_CONTEXT) {
    ...
    if (e.is_symb_of_sommet(at_hyperplan)) {
      ...
      if (ckmatrix(f) && f._VECTptr->size() == 2) {
        return symbolic(
            at_equal,
            makesequence(normal(dotvecteur(*f._VECTptr->front()._VECTptr,
                                           subvecteur(vxyz, *f._VECTptr->back()._VECTptr)),
                                contextptr),
                         zero));
      }
    }
    ...
    if ((e.type == _SYMB) && (e._SYMBptr->sommet == at_cercle)) {
      ...
      return symbolic(
          at_equal,
          makesequence(pow(x - re(centre, contextptr), 2) + pow(y - im(centre, contextptr), 2), r));
    }
    if ((e.type == _VECT) && (e._VECTptr->size() == 2)) {
      gen A = e._VECTptr->front();
      gen B = e._VECTptr->back();
      ...
      if (v.type == _VECT) { // 3-d line?
        ...
        if (is_zero(vv[0], contextptr) && is_zero(vv[1], contextptr))
          return gen(makevecteur(symbolic(at_equal, makesequence(x, vA[0])),
                                 symbolic(at_equal, makesequence(y, vA[1]))),
                     _SEQ__VECT);
        ...
        return gen(
            makevecteur(
                symbolic(at_equal, makesequence(normal(dotvecteur(v1, xyz), contextptr), zero)),
                symbolic(at_equal, makesequence(normal(dotvecteur(v2, xyz), contextptr), zero))),
            _SEQ__VECT);
      }
    ...
    try {
      ...
      if (!is_zero(b, contextptr))
        return symbolic(
            at_equal, makesequence(y, normal(-a / b, contextptr) * x + normal(c / b, contextptr)));
      return symbolic(
          at_equal,
          makesequence(x, normal(c / a, contextptr))); //symbolic(at_equal,makesequence(a*x+b*y,c));
    }
    if ((e.type == _SYMB) && (e._SYMBptr->sommet == at_curve)) {
      vecteur v = *e._SYMBptr->feuille._VECTptr->front()._VECTptr;
      if (v.size() >= 6 && !is_undef(v[5]))
        return symbolic(at_equal,
                        makesequence(subst(v[5], makevecteur(x__IDNT_e, y__IDNT_e, z__IDNT_e),
                                           makevecteur(x, y, z), false, contextptr),
                                     0));
      if (v[1].type != _IDNT)
      ...
      if (v[0].type != _VECT) {
        ...
        if (lvarxpow(makevecteur(xt, yt), v[1]).size() <= 1) {
          ...
          return symbolic(at_equal,
                          makesequence(rationalparam2equation(v[0], v[1], x, y, contextptr), 0));
        }
      ...
      if (v[0].type == _VECT && v[0]._VECTptr->size() == 3) {
        ...
        if (lvarxpow(makevecteur(xt, yt, zt), v[1]).size() <= 1) {
          return makevecteur(_resultant(makesequence(xt - x, zt - z, id), contextptr),
                             _resultant(makesequence(yt - y, zt - z, id), contextptr));
        }
      }
      return gensizeerr(contextptr);
    }
  ...
  vecteur remove_not_in_segment(const gen & a, const gen & b, int subtype, const vecteur & v,
                                GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      if (is_undef(*it))
        continue;
      gen t = scalar_product(remove_at_pnt(*it) - a, ab, contextptr) /
              scalar_product(ab, ab, contextptr);
      if (is_undef(t))
        continue;
  ...
  vecteur remove_not_in_arc(const vecteur & v_, const gen & g, GIAC_CONTEXT) {
    ...
    if (g.type == _VECT && g._VECTptr->size() == 2)
      return remove_not_in_segment(g._VECTptr->front(), g._VECTptr->back(), g.subtype, v,
                                   contextptr);
    if (!g.is_symb_of_sommet(at_cercle))
      return v;
    gen & f = g._SYMBptr->feuille;
    ...
    if (fv.front().type != _VECT || fv.front()._VECTptr->size() != 2)
      return v;
    gen a = fv.front()._VECTptr->front(), b = fv.front()._VECTptr->back(), alpha = fv[1],
        beta = fv[2];
  ...
  vecteur interdroitecercle(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    gen I1(recursive_normal(pr + d_a * a_pr_perp, contextptr)),
        I2(recursive_normal(pr - d_a * a_pr_perp, contextptr));
    if (I1.type == _VECT)
      I1.subtype = _POINT__VECT;
    if (I2.type == _VECT)
      I2.subtype = _POINT__VECT;
    return remove_not_in_arc(
        remove_not_in_segment(a1, a2, a.subtype,
                              makevecteur(symb_pnt(I1, default_color(contextptr), contextptr),
                                          symb_pnt(I2, default_color(contextptr), contextptr)),
                              contextptr),
        b, contextptr);
  }
  ...
  vecteur interpolygone(const vecteur & p, const gen & bb, GIAC_CONTEXT) {
    ...
    for (++it; it != itend; ++it) {
      gen tmp = symbolic(
          at_pnt, gen(makevecteur(gen(makevecteur(*(it - 1), *it), _GROUP__VECT), 0), _PNT__VECT));
  ...
  vecteur inter2cercles_or_spheres(const gen & centre_a, const gen & rayon_a2, const gen & centre_b,
                                   const gen & rayon_b2, bool a2d, GIAC_CONTEXT) {
    gen ab(centre_b - centre_a);
    gen ab2(abs_norm2(ab, contextptr));
    gen ab2minusr2 = ab2 - rayon_a2 - rayon_b2;
    if (is_strictly_greater(ab2minusr2, 0, contextptr) &&
        is_strictly_greater(pow(ab2minusr2, 2) - 4 * rayon_a2 * rayon_b2, 0, contextptr))
      return vecteur(0); // empty
    gen ababs(sqrt(ab2, contextptr));
    if (!a2d && (centre_a.type != _VECT || centre_a._VECTptr->size() != 3 ||
                 centre_b.type != _VECT || centre_b._VECTptr->size() != 3))
      return vecteur(1, gensizeerr(contextptr));
    ...
    if (a2d) { // circle inter circle = 2 points (or 1)
      ...
      return makevecteur(symb_pnt(ratnormal(ab4 + d_perp * ab_perp, contextptr),
                                  default_color(contextptr), contextptr),
                         symb_pnt(ratnormal(ab4 - d_perp * ab_perp, contextptr),
                                  default_color(contextptr), contextptr));
      ...
      gen eq = ab4 + d_perp / abs_norm(v1, contextptr) * symb_cos(vx_var) * v1 +
               d_perp / abs_norm(v2, contextptr) * symb_sin(vx_var) * v2;
  ...
  vecteur curveintercircle(const gen & curve, const gen & circle, bool iscircle, GIAC_CONTEXT) {
    ...
    if (vf.size() > 5 && !is_undef(vf[5])) {
      ...
      if (find_curve_parametrization(circle, m, intervart, T, tmin, tmax, false, contextptr)) {
        ...
        carteq = subst(carteq, makevecteur(x__IDNT_e, y__IDNT_e), makevecteur(circlex, circley),
                       false, contextptr);
    ...
    try {
      ...
      giac_assume(symb_and(symb_superieur_egal(solvex, tmin), symb_inferieur_egal(solvex, tmax)),
                  contextptr);
  ...
  static vecteur equationintercurve(const gen & at_orig, const gen & t, const gen & b,
                                    const gen & bu_orig, const gen & u, GIAC_CONTEXT) {
    ...
    if (u.type != _IDNT)
      return vecteur(1, gensizeerr(contextptr));
    gen x("x__" + print_INT_(std_rand()), contextptr),
        y("y__" + print_INT_(std_rand()), contextptr);
    ...
    eq = subst(eq, makevecteur(x, y), makevecteur(re(bu1, contextptr), im(bu1, contextptr)), false,
               contextptr);
    vln = lop(eq, at_innertln);
    wln = vln;
    for (int i = 0; i < int(vln.size()); ++i) {
      if (vln[i].type == _SYMB)
  ...
  bool intercartesianparametric(const gen & a0, const gen & b0, vecteur & res, GIAC_CONTEXT) {
    ...
    if (is_undef(a))
      return false;
    gen m, mx, my, t(t__IDNT_e), tmin, tmax;
    double T = -1e300;
    bool tminmax = false;
    vecteur v;
  ...
  vecteur inter(const gen & aa, const gen & bb, GIAC_CONTEXT) {
    ...
    if (a.type == _VECT) {
      ...
      if (as == 2) {
        if (b.type == _VECT) {
          ...
          if (bs == 2) { // D inter D'
            gen a1 = a._VECTptr->front(), a2 = a._VECTptr->back(), b1 = b._VECTptr->front(),
                b2 = b._VECTptr->back();
            gen v(a2 - a1), w(b2 - b1);
            if (v.type == _VECT) {
    ...
    if ((ac && bc) || (asp && bsp)) {
      ...
      if (!centre_rayon(a, centre_a, rayon_a, false, contextptr) ||
          !centre_rayon(b, centre_b, rayon_b, false, contextptr))
        return vecteur(1, gensizeerr(contextptr));
      vecteur res = inter2cercles_or_spheres(centre_a, abs_norm2(rayon_a, contextptr), centre_b,
                                             abs_norm2(rayon_b, contextptr), ac, contextptr);
      return (ac && bc) ? remove_not_in_arc(remove_not_in_arc(res, a, contextptr), b, contextptr)
                        : res;
    }
    ...
    vecteur res;
    if (a.is_symb_of_sommet(at_equal) && b.is_symb_of_sommet(at_curve) &&
        intercartesianparametric(a, b, res, contextptr))
      return res;
    if (b.is_symb_of_sommet(at_equal) && a.is_symb_of_sommet(at_curve) &&
        intercartesianparametric(b, a, res, contextptr))
      return res;
    if ((a.type == _SYMB) && (a._SYMBptr->sommet == at_curve) && (b.type == _SYMB) &&
        (b._SYMBptr->sommet == at_curve)) { // curve inter curve
      gen fa, fb;
      if ((fa = a._SYMBptr->feuille).type != _VECT || (fb = b._SYMBptr->feuille).type != _VECT ||
          fa._VECTptr->empty() || fb._VECTptr->empty() || fa._VECTptr->front().type != _VECT ||
          fb._VECTptr->front().type != _VECT)
        return vecteur(1, gensizeerr(contextptr));
      ...
      gen m, tmin, tmax;
      double T = 1e300;
      if (find_curve_parametrization(ac ? remove_at_pnt(aa) : a, m, va[1], T, tmin, tmax, false,
                                     contextptr)) {
        return equationintercurve(m, va[1], b, vb[0], vb[1], contextptr);
        // va[0]=m;
      }
      if (find_curve_parametrization(bc ? remove_at_pnt(bb) : b, m, vb[1], T, tmin, tmax, false,
                                     contextptr)) {
      ...
      try {
        ...
        itend = res.end();
        for (; it != itend; ++it) {
          sol.push_back(symb_pnt(
              subst((exchanged ? va : vb)[0], (exchanged ? va : vb)[1], *it, false, contextptr),
              default_color(contextptr), contextptr));
        }
        return sol;
#ifndef NO_STDEXCEPT
  ...
  static gen tangent(const gen & args, GIAC_CONTEXT) {
    ...
    if (s == 1 && argv[0].type == _SYMB && !argv[0].is_symb_of_sommet(at_pnt)) {
      ...
      if (t.type != _VECT) {
        ...
        if (der.type == _VECT && der._VECTptr->size() == 2) {
          ...
          vecteur sol = gsolve(makevecteur(droite, argv0), *xy._VECTptr, false,
                               (approx_mode(contextptr) ? 1 : 0), contextptr);
          if (!is_undef(sol)) {
            vecteur res;
            for (unsigned i = 0; i < sol.size(); ++i) {
              gen soli = normal(sol[i], contextptr);
              gen derp = subst(makevecteur(-der._VECTptr->back(), der._VECTptr->front()), xy, soli,
                               false, contextptr);
    ...
    for (int i = 0; i < s; ++i) {
      ...
      if (curve.type == _VECT && !curve._VECTptr->empty() &&
          curve._VECTptr->back().is_symb_of_sommet(at_pnt))
      ...
      if ((curve.type == _SYMB) &&
          (curve._SYMBptr->sommet == at_cercle || curve._SYMBptr->sommet == at_hypersphere)) {
        gen centre, rayon;
        if (!centre_rayon(curve, centre, rayon, false, contextptr))
          continue;
        rayon = ratnormal(rayon, contextptr);
        if (t.type == _VECT || t.is_symb_of_sommet(at_pnt) ||
            !is_zero(im(t, contextptr), contextptr)) {
          ...
          if (is_zero(rayonb2 / rayon2, contextptr)) {
            if (OA.type == _VECT)
              ...
              result.push_back(
                  _droite(makesequence(t, t + im(OA, contextptr) - cst_i * re(OA, contextptr)),
                          contextptr));
            continue;
          }
          ...
          if (OA.type == _VECT)
            return gensizeerr(contextptr);
          vecteur v = inter2cercles_or_spheres(
              centre, rayon2, t, rayonb2, true,
              contextptr); // inter(_cercle(makevecteur(symb_pnt(t,contextptr),sqrt(rayonb2)),contextptr),curve,contextptr);
          if (!v.empty()) {
            if (is_undef(v.front()))
              return v.front();
      ...
      if ((t.type == _SYMB) && (t._SYMBptr->sommet == at_pnt)) {
          ...
          if (tv.size() >= 2) {
            ...
            if (M.type == _VECT && M._VECTptr->size() == 2) {
              ...
              if (remove_at_pnt(Mon) == curve) {
                ...
                return _droite_segment(gen(makevecteur(M, M + direction), _SEQ__VECT), _LINE__VECT,
                                       vecteur(1, default_color(contextptr)), contextptr);
              }
            }
          }
        }
        gen M = remove_at_pnt(t);
        if (M.is_symb_of_sommet(at_curve))
          return gensizeerr(contextptr);
        if (v.size() > 7 && v[7].type == _VECT) {
          ...
          if (v7.size() == 7 && v7.front() == at_ellipse) {
            ...
            gen A = a * d * yM - a * f * xM + b * d * xM - d * d,
                B = 2 * a * f * yM + 2 * c * d * xM - 2 * d * f,
                C = b * f * yM - c * d * yM + c * f * xM - f * f;
            gen D = B * B - 4 * A * C;
            if (is_positive(D, contextptr)) {
              ...
              return makevecteur(_droite(makesequence(M, t1), contextptr),
                                 _droite(makesequence(M, t2), contextptr));
            }
            return vecteur(0);
          }
        }
        ...
        double T = 1;
        if (v[1].type == _IDNT &&
            find_curve_parametrization(curve, m, v[1], T, tmin, tmax, false, contextptr)) {
          gen rem;
          if (m.is_symb_of_sommet(at_plus) && m._SYMBptr->feuille.type == _VECT &&
              m._SYMBptr->feuille._VECTptr->size() == 2 &&
              m._SYMBptr->feuille._VECTptr->front() == v[1] &&
              !has_i(rem = _simplifier(-cst_i * m._SYMBptr->feuille._VECTptr->back(), contextptr)))
            rem = v[1];
          else
            rem = re(m, contextptr);
          if (rem == v[1]) {
            ...
            if (M.is_symb_of_sommet(at_plus) && M._SYMBptr->feuille.type == _VECT &&
                M._SYMBptr->feuille._VECTptr->size() == 2 &&
                !has_i(r = _simplifier(-cst_i * M._SYMBptr->feuille._VECTptr->back(), contextptr)))
          ...
          if (mv.empty() || (mv.size() == 1 && mv.front() == v[1])) {
            ...
            vecteur sols = gsolve(makevecteur(eq, eq2), makevecteur(x, y), false,
                                  (approx_mode(contextptr) ? 1 : 0), contextptr);
            if (is_undef(sols))
              return sols;
            // build tangents
        ...
        gen xt(re(v06, contextptr)), yt(im(v06, contextptr)), xp(re(direction, contextptr)),
            yp(im(direction, contextptr));
        gen var = v[1];
        ...
        gen test1 = recursive_normal(subst(yt, var, re(M, contextptr), false, contextptr) -
                                         im(M, contextptr),
                                     contextptr),
            test2 = recursive_normal(subst(xt, var, re(M, contextptr), false, contextptr) -
                                         re(M, contextptr),
                                     contextptr);
  ...
  gen _ellipse(const gen & args, GIAC_CONTEXT) {
    ...
    if (!foyers_a(vecteur(args._VECTptr->begin(), args._VECTptr->begin() + s), F1, F2, a,
                  contextptr))
      return gensizeerr(contextptr);
    if (aorig.is_symb_of_sommet(at_pnt) || a.type == _VECT ||
        !is_zero(im(a, contextptr), contextptr)) {
    ...
    if (n.type == _VECT) { // 3-d
      ...
      res = O + a * symb_cos(theta) * F1F2 / abs_norm(F1F2, contextptr) +
            b * symb_sin(theta) * n / abs_norm(n, contextptr);
    } else { // 2-d
...
#else
    return _paramplot(gen(makevecteur(res, symb_equal(theta_orig, symb_interval(0, 2 * cst_pi)),
                                      symb_equal(nstep, 120), symb_equal(ustep, M_PI / 60),
                                      symbolic(at_equal, makesequence(at_display, attributs[0])),
                                      eq, parameq, vparam),
                          _SEQ__VECT),
                      contextptr);
#endif
  ...
  gen _hyperbole(const gen & args, GIAC_CONTEXT) {
    ...
    if (n.type == _VECT) {
      n = n - O;
      n = cross(cross(F1F2, n, contextptr), F1F2, contextptr);
      res1 = a * cosh(theta, contextptr) * F1F2 / abs_norm(F1F2, contextptr) +
             b * sinh(theta, contextptr) * n / abs_norm(n, contextptr);
      res2 = -a * cosh(theta, contextptr) * F1F2 / abs_norm(F1F2, contextptr) +
             b * sinh(theta, contextptr) * n / abs_norm(n, contextptr);
    } else {
      gen xF1F2 = re(F1F2, contextptr), yF1F2 = im(F1F2, contextptr);
      gen eitheta(xy2eitheta(xF1F2, yF1F2, contextptr));
...
#else
    return makevecteur(
        _paramplot(gen(makevecteur(res1, symb_equal(theta, symb_interval(-3, 3)),
                                   symb_equal(nstep, 60), symb_equal(ustep, 0.1),
                                   symbolic(at_equal, makesequence(at_display, attributs[0])), eq,
                                   parameq, vparam),
                       _SEQ__VECT),
                   contextptr),
        _paramplot(gen(makevecteur(res2, symb_equal(theta, symb_interval(-3, 3)),
                                   symb_equal(nstep, 60), symb_equal(ustep, 0.1),
                                   symbolic(at_equal, makesequence(at_display, attributs[0])), eq,
                                   parameq, vparam),
                       _SEQ__VECT),
                   contextptr)); // should be -inf..inf
#endif
  }
...
#else
    res =
        _paramplot(gen(makevecteur(res, symb_equal(theta, symb_interval(-12, 12)),
                                   symb_equal(nstep, 60), symb_equal(ustep, 0.15),
                                   symbolic(at_equal, makesequence(at_display, attributs[0])), eq),
                       _SEQ__VECT),
                   contextptr);
#endif
  ...
  gen put_attributs(const gen & lieu_g, const vecteur & attributs, GIAC_CONTEXT) {
    ...
    if (!lieu_g.is_symb_of_sommet(at_pnt) && lieu_geo.type == _VECT &&
        (lieu_geo.subtype < _LINE__VECT || lieu_geo.subtype > _HALFLINE__VECT)) {
  ...
  gen _conique(const gen & args, GIAC_CONTEXT) {
    ...
    if (s == 5) {
      ...
      if (is_undef(base) || base.empty() || base.front().type != _VECT ||
          base.front()._VECTptr->size() != 6)
      ...
      gen g;
      if (equation2geo2d(eq, x, y, g, gnuplot_tmin, gnuplot_tmax, gnuplot_tstep, w[0], 3,
                         contextptr))
  ...
  gen _legende(const gen & a, GIAC_CONTEXT) {
    ...
    if (position.type == _VECT && position._VECTptr->size() == 2)
      return symb_pnt(symbolic(at_legende, makesequence(position, string2gen(s, false), couleur)),
                      couleur, contextptr);
    position = remove_at_pnt(v[0]);
    if (position.type == _SYMB) {
      if (position._SYMBptr->sommet == at_cercle)
        position = position._SYMBptr->feuille;
      else if (position._SYMBptr->sommet == at_curve) {
    ...
    if (v.size() < 2)
      ...
      return symb_pnt_name(position + decal, makevecteur(couleur + _POINT_POINT, v[1], sg), sg,
                           contextptr);
  }
  ...
  gen _distanceat(const gen & args, GIAC_CONTEXT) {
    ...
    if (s < 3)
      return gentypeerr(contextptr);
    gen l = sqrt(_longueur2(gen(makevecteur(eval(v[0], eval_level(contextptr), contextptr),
                                            eval(v[1], eval_level(contextptr), contextptr)),
                                _SEQ__VECT),
                            contextptr),
                 contextptr);
    int save_digits = decimal_digits(contextptr);
  ...
  define_unary_function_ptr5(at_distanceat, alias_at_distanceat, &__distanceat, _QUOTE_ARGUMENTS,
                             true);
  ...
  define_unary_function_ptr5(at_perimeterat, alias_at_perimeterat, &__perimeterat, _QUOTE_ARGUMENTS,
                             true);
  ...
  define_unary_function_ptr5(at_perimeteratraw, alias_at_perimeteratraw, &__perimeteratraw, 0,
                             true);

  gen _extract_measure(const gen & valeur, GIAC_CONTEXT) {
    if (valeur.type == _STRNG && valeur.subtype == -1)
      return valeur;
    gen tmp = valeur;
  ...
  define_unary_function_ptr5(at_extract_measure, alias_at_extract_measure, &__extract_measure, 0,
                             true);
  ...
  gen _angleat(const gen & args, GIAC_CONTEXT) {
    ...
    if (s < 4)
      return gentypeerr(contextptr);
    gen l = _angle(gen(makevecteur(eval(v[0], eval_level(contextptr), contextptr),
                                   eval(v[1], eval_level(contextptr), contextptr),
                                   eval(v[2], eval_level(contextptr), contextptr)),
                       _SEQ__VECT),
                   contextptr);
    int save_digits = decimal_digits(contextptr);
    decimal_digits(contextptr) = 3;
  ...
  gen _couleur(const gen & a, GIAC_CONTEXT) {
    ...
    if (a._VECTptr->size() == 3 && c.type == _INT_ && (b = a._VECTptr->front()).type == _INT_ &&
        (*a._VECTptr)[1].type == _INT_) {
      // 565 color
      int d = (((b.val * 32) / 256) << 11) | ((((*a._VECTptr)[1].val * 64) / 256) << 5) |
              ((c.val * 32) / 256);
      if (d > 0 && d < 512) {
        d += (1 << 11);
      }
  ...
  static gen plotode(const vecteur & w, GIAC_CONTEXT) {
    ...
    if (vars.type == _VECT && vars._VECTptr->size() == 2 &&
        vars._VECTptr->back().is_symb_of_sommet(at_equal))
      vars = makevecteur(vars._VECTptr->front(), vars._VECTptr->back()._SYMBptr->feuille[0]);
    v[1] = vars;
    vecteur f = makevecteur(
        fp, (t.is_symb_of_sommet(at_equal) ? t._SYMBptr->feuille._VECTptr->front() : t), vars);
  ...
  int maxstep = 500;
    ...
    for (; it != itend; ++it) {
      ...
      if (dim3)
        res.push_back(
            gen(makevecteur(it->_VECTptr->front(), tmp.front(), tmp.back()), _POINT__VECT));
      else
  ...
  gen plotfield(const gen & xp, const gen & yp, const gen & x, const gen & y, double xmin,
                double xmax, double xstep, double ymin, double ymax, double ystep, double scaling,
                vecteur & attributs, bool normalize, const context * contextptr) {
    if (xstep <= 0 || ystep <= 0)
    ...
    for (double curx = xmin; curx <= xmax; curx += scaling * xstep) {
      ...
      for (double cury = ymin; cury <= ymax; cury += scaling * ystep) {
        if (ctrl_c || interrupted)
          break;
        curxcury[1] = cury;
        xp_eval = subst(xp, xy, curxcury, false, contextptr)
                      .evalf2double(eval_level(contextptr), contextptr);
        yp_eval = subst(yp, xy, curxcury, false, contextptr)
                      .evalf2double(eval_level(contextptr), contextptr);
        if ((xp_eval.type == _DOUBLE_) && (yp_eval.type == _DOUBLE_)) {
          ...
          res.push_back(pnt_attrib(
              gen(makevecteur(origine, origine + echelle * xp_eval + echelle * cst_i * yp_eval),
  ...
  static bool read_plotfield_args(const gen & args, gen & xp, gen & yp, gen & x, gen & y,
                                  double & xmin, double & xmax, double & xstep, double & ymin,
                                  double & ymax, double & ystep, vecteur & attributs,
    ...
    if (s && v[0].is_symb_of_sommet(at_equal)) {
      ...
      if (f.type == _VECT && f._VECTptr->size() == 2 && f._VECTptr->front().type == _INT_) {
        *logptr(contextptr) << "Warning, replacing plotfield of " << v[0] << " by "
                            << f._VECTptr->back() << '\n';
        v[0] = f._VECTptr->back();
      }
    }
    for (int i = 0; i < s; ++i) {
      ...
      if (v[i].type == _VECT && v[i]._VECTptr->size() == 1 &&
          v[i]._VECTptr->front().type == _VECT) {
      ...
      if (v[i].is_symb_of_sommet(at_equal) && v[i]._SYMBptr->feuille.type == _VECT &&
          ...
          v[i]._SYMBptr->feuille._VECTptr->front() == at_plotode) {
    ...
    vecteur tmp;
    read_option(*args._VECTptr, xmin, xmax, ymin, ymax, gnuplot_zmin, gnuplot_zmax, tmp, nstep,
                jstep, kstep, contextptr);
  ...
  gen _plotfield(const gen & args, const context * contextptr) {
    ...
    bool normalize;
    vecteur initcondv;
    if (!read_plotfield_args(args, xp, yp, x, y, xmin, xmax, xstep, ymin, ymax, ystep, attributs,
                             normalize, initcondv, contextptr))
      return gensizeerr(contextptr);
    int s = initcondv.size();
    if (s >= 2) {
      lastplotode0 = initcondv[0];
      lastplotode1 = initcondv[1];
    }
    double scaling = 2;
    if (s > 2) {
      vecteur res;
      res.push_back(plotfield(xp, yp, x, y, xmin, xmax, xstep / scaling, ymin, ymax,
                              ystep / scaling, scaling, attributs, normalize, contextptr));
      vecteur argu(3);
      argu[0] = initcondv[0];
      argu[1] = initcondv[1];
      for (int i = 2; i < s; ++i) {
    ...
    return plotfield(xp, yp, x, y, xmin, xmax, xstep / scaling, ymin, ymax, ystep / scaling,
                     scaling, attributs, normalize, contextptr);
  }
...
#if defined(WIN32) || !defined(HAVE_SIGNAL_H_OLD)
      res.push_back(_plotode(
          gen(makevecteur(f, vars, makevecteur(0, x0, y0), xminmax, yminmax, at_plan), _SEQ__VECT),
          contextptr));
#else
    if (thread_eval_status(contextptr) == 1)
      res.push_back(_plotode(
      ...
      res.push_back(_signal(
          _plotode(gen(makevecteur(f, vars, makevecteur(0, x0, y0), xminmax, yminmax, at_plan),
                       _SEQ__VECT),
                   contextptr),
          contextptr));
#endif
  ...
  static define_unary_function_eval(__interactive_plotode, &_interactive_plotode,
                                    _interactive_plotode_s);
  define_unary_function_ptr5(at_interactive_plotode, alias_at_interactive_plotode,
                             &__interactive_plotode, 0, true);

  static const char _interactive_odeplot_s[] = "interactive_odeplot";
  static define_unary_function_eval(__interactive_odeplot, &_interactive_plotode,
                                    _interactive_odeplot_s);
  define_unary_function_ptr5(at_interactive_odeplot, alias_at_interactive_odeplot,
                             &__interactive_odeplot, 0, true);
      ...
      if (op < 0) {
        ...
        std::pair<charptr_gen *, charptr_gen *> p =
            equal_range(builtin_lexer_functions_begin(), builtin_lexer_functions_end(),
                        std::pair<const char *, gen>(s.c_str(), 0), tri);
  ...
  static symbolic unarchive_SYMB(istream & is, GIAC_CONTEXT) {
    ...
    gen f = unarchive(is, contextptr);
    if (e.type == _FUNC) {
      if (e == at_neg && f.is_symb_of_sommet(at_prod) && f._SYMBptr->feuille.type == _VECT &&
          !f._SYMBptr->feuille._VECTptr->empty() &&
          f._SYMBptr->feuille._VECTptr->front().type == _ZINT) {
        mpz_neg(*f._SYMBptr->feuille._VECTptr->front()._ZINTptr,
                *f._SYMBptr->feuille._VECTptr->front()._ZINTptr);
        return *f._SYMBptr;
      }
  ...
  */
  ...
  gen unarchive_session_string(const std::string & s, int level, const gen & replace,
                               GIAC_CONTEXT) {
#ifdef HAVE_SSTREAM
    istringstream is(s);
    if (!is)
  ...
  gen _archive(const gen & args, GIAC_CONTEXT) {
    ...
    if (s == 3) { // new binary archive format
      ...
      if (!archive_save(f, b, contextptr))
        return gensizeerr(gettext("Error writing ") + b.print(contextptr) + " in file " +
                          a.print(contextptr));
      fclose(f);
      return b;
    }
  ...
  gen xyztrange(double xmin, double xmax, double ymin, double ymax, double zmin, double zmax,
                double tmin, double tmax, double wxmin, double wxmax, double wymin, double wymax,
                int axes, double class_min, double class_size, bool gnuplot_hidden3d,
                bool gnuplot_pm3d) {
    vecteur v;
    v.push_back(xmin);
    v.push_back(xmax);
  ...
  gen _xyztrange(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type != _VECT || args._VECTptr->size() < 4)
      return makevecteur(
          ...
          makevecteur(string2gen("wy+", false), global_window_ymax)); //gensizeerr(contextptr);
    vecteur w = *args._VECTptr;
    int s = int(w.size());
    if (s < 12) {
      if (s < 5)
        w.push_back(gnuplot_zmin);
      if (s < 6)
        w.push_back(gnuplot_zmax);
      if (s < 7)
        w.push_back(gnuplot_tmin);
      if (s < 8)
        w.push_back(gnuplot_zmin);
      if (s < 9)
        w.push_back(w[0]);
...
unary_function_eval __xyztrange(0, &_xyztrange, _xyztrange_s);
  ...
  gen _switch_axes(const gen & args, GIAC_CONTEXT) {
    ...
    return eval(xyztrange(gnuplot_xmin, gnuplot_xmax, gnuplot_ymin, gnuplot_ymax, gnuplot_zmin,
                          gnuplot_zmax, gnuplot_tmin, gnuplot_tmax, global_window_xmin,
                          global_window_xmax, global_window_ymin, global_window_ymax,
                          show_axes(contextptr), class_minimum, class_size,
#ifdef WITH_GNUPLOT
  ...
  gen plotseq(const gen & f, const gen & x, double x0, double xmin, double xmax, int niter,
              const vecteur & attributs, const context * contextptr, bool print) {
    if (xmin > xmax)
      swapdouble(xmin, xmax);
    double Xmin = xmin;
    if (xmin < 0 && xmax > 0)
    ...
    if (print)
      gprintf("======== %gen=%gen), %gen=%gen",
              makevecteur(symb_at(u__IDNT_e, n__IDNT_e + 1, contextptr),
                          subst(f, x, symb_at(u__IDNT_e, n__IDNT_e, contextptr), false, contextptr),
                          symb_at(u__IDNT_e, 0, contextptr), x0),
              1, contextptr);
    ...
    vecteur g(
        gen2vecteur(_plotfunc(gen(makevecteur(f, symb_equal(x, symb_interval(xmin, xmax))
#ifdef NUMWORKS
                                                     ,
                                  ...
                                  _SEQ__VECT),
                              contextptr)));
    g.push_back(pnt_attrib(gen(makevecteur(gen(xmin, xmin), gen(xmax, xmax)), _LINE__VECT),
                           attributs, contextptr));
#ifdef GIAC_HAS_STO_38
    int color = FL_BLACK;
#else
  int color = FL_MAGENTA;
    ...
    g.push_back(symb_pnt(gen(makevecteur(gen(x0, x0), gen(x0, Xmin)), _VECTOR__VECT),
                         color | _DASH_LINE, contextptr));
    return g; // gen(g,_SEQ__VECT);
  }
  int find_plotseq_args(const gen & args, gen & expr, gen & x, double & x0d, double & xmin,
                        double & xmax, int & niter, vecteur & attributs, GIAC_CONTEXT,
                        bool & print) {
    vecteur v = gen2vecteur(args);
    print = true;
#if 0
  ...
  gen _plotseq(const gen & args, const context * contextptr) {
    ...
    vecteur attributs;
    bool print;
    if (find_plotseq_args(args, expr, var, x0d, xmin, xmax, niter, attributs, contextptr, print) <
        0)
      return gentypeerr(contextptr);
  ...
  static gen in_plotimplicit(const gen & f_orig, const gen & x, const gen & y, double xmin,
                             double xmax, double ymin, double ymax, int nxstep, int nystep,
                             double eps, const vecteur & attributs, int ckgeo2d,
                             const context * contextptr) {
#ifdef RTOS_THREADX
    return undef;
#else
  if (f_orig.type == _VECT) {
    ...
    int vs = int(v.size());
    for (int i = 0; i < vs; ++i) {
      w.push_back(in_plotimplicit(v[i], x, y, xmin, xmax, ymin, ymax, nxstep, nystep, eps,
                                  attributs, ckgeo2d, contextptr));
    }
    return gen(w, _SEQ__VECT);
  }
  if (f_orig.is_symb_of_sommet(at_inv) || (is_zero(derive(f_orig, x, contextptr), contextptr) &&
                                           is_zero(derive(f_orig, y, contextptr), contextptr)))
  ...
  if (f_orig.is_symb_of_sommet(at_prod) && f_orig._SYMBptr->feuille.type == _VECT) {
    ...
    int s = int(fv.size());
    for (int i = 0; i < s; ++i) {
      gen tmp = in_plotimplicit(fv[i], x, y, xmin, xmax, ymin, ymax, nxstep, nystep, eps, attributs,
                                ckgeo2d, contextptr);
      if (!is_undef(tmp))
  ...
  if (f_orig.is_symb_of_sommet(at_pow)) {
    ...
    if (farg.type == _VECT && farg._VECTptr->size() == 2) {
      ...
      if (ck_is_positive(expo, contextptr))
        return in_plotimplicit(arg, x, y, xmin, xmax, ymin, ymax, nxstep, nystep, eps, attributs,
                               ckgeo2d, contextptr);
      else
        return vecteur(0); // gen(vecteur(0),_SEQ__VECT);
    }
  ...
  gen attribut = attributs.empty() ? default_color(contextptr) : attributs[0];
  gen lieu_geo;
  if (ckgeo2d && equation2geo2d(f_orig, x, y, lieu_geo, gnuplot_tmin, gnuplot_tmax, gnuplot_tstep,
                                undef, ckgeo2d, contextptr)) {
  ...
  if (is_undef(dfx) || is_undef(dfy))
    return dfx + dfy;
  {
    gen x1("x1", contextptr);
    sto(-dfy, x1, contextptr);
    gen y1("y1", contextptr);
    sto(dfx, y1, contextptr);
    gen tmp(string2gen("implicit", false));
    gen x2("x2", contextptr);
    sto(tmp, x2, contextptr);
    gen y2("y2", contextptr);
    sto(tmp, y2, contextptr);
  }
...
#endif
  vector<vector<flott>> fxy(nxstep + 1, vector<flott>(nystep + 1)),
      dfxorig(nxstep + 1, vector<flott>(0)), dfyorig(nxstep + 1, vector<flott>(0)),
      dfxyorig_abs(nxstep + 1, vector<flott>(0)), xorig(nxstep + 1, vector<flott>(0)),
      yorig(nxstep + 1, vector<flott>(0));
  gen gtmp;
  ...
  if (xy1 == xy) {
    ...
    singular_points = gsolve(*exact(makevecteur(f_orig, dfx, dfy), contextptr)._VECTptr, xy, false,
                             (approx_mode(contextptr) ? 1 : 0), contextptr);
    for (int k = 0; k < int(singular_points.size()); k++) {
      gen sp = singular_points[k];
      gen spd = evalf_double(sp, 1, contextptr);
      if (sp.type != _VECT || sp._VECTptr->size() != 2 || spd[0].type != _DOUBLE_ ||
          spd[1].type != _DOUBLE_)
        continue;
      ...
      if (has_op(sp, *at_rootof))
        sp = spd;
      // find all tangents starting from sp
      for (int order = 2; order < 10; ++order) {
        gen tays = series(subst(f_orig, xy, xy + sp, false, contextptr), xy, makevecteur(0, 0),
                          order, 0, contextptr);
        if (!is_zero(tays, contextptr)) {
          ...
          gen sqfftays = _quo(
              gen(makevecteur(tays,
                              _gcd(gen(makevecteur(tays, derive(tays, t, contextptr)), _SEQ__VECT),
                                   contextptr),
                              t),
                  _SEQ__VECT),
              contextptr);
...
#ifndef GIAC_HAS_STO_38
          *logptr(contextptr) << gettext("Near ") << sp << ", 1/epsilon^2*f(" << sp
                              << "+epsilon*[1,t])=" << subst(tays, t, t__IDNT_e, false, contextptr)
                              << " roots " << r << '\n';
#endif
          if (r.type == _VECT) {
            int total = 0;
            for (unsigned kr = 0; kr < r._VECTptr->size(); ++kr) {
              ...
              for (++total;; ++mult, ++total) {
                ...
                if (quo.type != _VECT || quo._VECTptr->size() != 2 ||
                    !is_zero(quo._VECTptr->back(), contextptr))
                  break;
          ...
          for (int sing = 0; sing < int(singular_points_tangents.size()); ++sing) {
            ...
            if (tmp.type == _VECT && tmp._VECTptr->size() >= 6) {
              ...
              double xcurrent =
                  evalf_double(sp[0] + xstep / 3 * (*tmp._VECTptr)[3], 1, contextptr)._DOUBLE_val;
              double ycurrent =
                  evalf_double(sp[1] + xstep / 3 * (*tmp._VECTptr)[4], 1, contextptr)._DOUBLE_val;
              ...
              if (is_greater(abs(tmp[4], contextptr), abs(tmp[3], contextptr), contextptr)) {
                ...
                int iszero = -1;
                vecteur v =
                    bisection_solver(fx, x, xcurrent - xstep, xcurrent + xstep, iszero, contextptr);
                ...
                int iszero = -1;
                vecteur v =
                    bisection_solver(fy, y, ycurrent - ystep, ycurrent + ystep, iszero, contextptr);
                for (unsigned vi = 0; vi < v.size(); ++vi) {
                  gen sol = v[vi];
                  if (sol.type != _DOUBLE_)
                    continue;
    ...
    if (!singular_points.empty())
      *logptr(contextptr)
          << gettext("Singular points directions: [cell_i, cell_j, singularity, next solution] ")
          << singular_points_directions << '\n';
#endif
  }
  bool pathfound;
  vecteur res;
  ...
  for (;;) {
    ...
    if (!pathfound) {
      ...
      if (visited[i][j] || (((j && visited[i][j - 1]) || visited[i][j + 1]) &&
      ...
      if (tmp < 0) {
        ...
        if (tmp < 0) {
          ...
          ycurrent = yy;
          gen fx = subst(f_orig, y, ycurrent, false, contextptr);
          gen sol = _fsolve(gen(makevecteur(fx, x, makevecteur(xcurrent - xstep, xcurrent),
                                            _BISECTION_SOLVER, 100 * eps),
                                _SEQ__VECT),
                            contextptr);
          if (!get_sol(sol, contextptr))
    ...
    gtmp = subst(dfx, xy, makevecteur(xcurrent, ycurrent), false, contextptr)
               .evalf2double(eval_level(contextptr), contextptr);
    if (gtmp.type == _DOUBLE_)
      dfxorig[iorig][jorig] = gtmp._DOUBLE_val;
    else
      dfxorig[iorig][jorig] = oldi ? (fxy[oldi][oldj] - fxy[oldi - 1][oldj]) / xstep
                                   : (fxy[oldi + 1][oldj] - fxy[oldi][oldj]) / xstep;
    gtmp = subst(dfy, xy, makevecteur(xcurrent, ycurrent), false, contextptr)
               .evalf2double(eval_level(contextptr), contextptr);
    if (gtmp.type == _DOUBLE_)
      dfyorig[iorig][jorig] = gtmp._DOUBLE_val;
    else
      dfyorig[iorig][jorig] = oldj ? (fxy[oldi][oldj] - fxy[oldi][oldj - 1]) / ystep
                                   : (fxy[oldi][oldj + 1] - fxy[oldi][oldj]) / ystep;
    ...
    for (int count = 0; count < nxstep * nystep;) {
      ...
      gtmp = subst(dfx, xy, xycurrent, false, contextptr)
                 .evalf2double(eval_level(contextptr), contextptr);
      ...
      if (gtmp.type == _DOUBLE_)
        ...
        use_newton = false;
        dfxcurrent = oldi ? (fxy[oldi][oldj] - fxy[oldi - 1][oldj]) / xstep
                          : (fxy[oldi + 1][oldj] - fxy[oldi][oldj]) / xstep;
      }
      gtmp = subst(dfy, xy, xycurrent, false, contextptr)
                 .evalf2double(eval_level(contextptr), contextptr);
      if (gtmp.type == _DOUBLE_)
        dfycurrent = gtmp._DOUBLE_val;
      else {
        dfycurrent = oldj ? (fxy[oldi][oldj] - fxy[oldi][oldj - 1]) / ystep
                          : (fxy[oldi][oldj + 1] - fxy[oldi][oldj]) / ystep;
        use_newton = false;
      }
      if (sign == -1) {
      ...
      if (was_not_singular) {
        double pascoeff =
            std::min(xstep, ystep) / std::sqrt(dfycurrent * dfycurrent + dfxcurrent * dfxcurrent);
        gen pas = gen(pascoeff) * makevecteur(dfycurrent, -dfxcurrent);
        newxy = xycurrent + pas;
        gtmp = subst(dfx, xy, newxy, false, contextptr)
                   .evalf2double(eval_level(contextptr), contextptr);
        if (gtmp.type == _DOUBLE_) {
          double d1 = gtmp._DOUBLE_val;
          gtmp = subst(dfy, xy, newxy, false, contextptr)
                     .evalf2double(eval_level(contextptr), contextptr);
          if (gtmp.type == _DOUBLE_) {
      ...
      if (was_not_singular && dfxyabs2 < 0.1 && !is_zero(last_direction, contextptr)) {
        ...
        for (; k < int(singular_points.size()); k++) {
          ...
          if (sp.type == _VECT && sp._VECTptr->size() == 2 &&
              is_greater(2 * xstep, abs(sp[0] - newxy[0], contextptr), contextptr) &&
              is_greater(2 * ystep, abs(sp[1] - newxy[1], contextptr), contextptr))
            break;
        }
      ...
      double deltax = evalf_double(re(direction, contextptr), eval_level(contextptr), contextptr)
                          ._DOUBLE_val,
             deltay = evalf_double(im(direction, contextptr), eval_level(contextptr), contextptr)
                          ._DOUBLE_val;
	  ...
	  if (!sol1.empty()){
	    ...
	    sol=undef;
#else
        if (is_positive(subst(fy, y, ycurrent - ystep, false, contextptr) *
                            subst(fy, y, ycurrent + ystep, false, contextptr),
                        contextptr) ||
            use_newton) {
          sol = _fsolve(gen(makevecteur(fy, y, ycurrent, _NEWTON_SOLVER), _SEQ__VECT), contextptr);
        } else {
          sol = _fsolve(gen(makevecteur(fy, y, makevecteur(ycurrent - ystep, ycurrent + ystep),
                                        _BISECTION_SOLVER, 100 * eps),
                            _SEQ__VECT),
                        contextptr);
	  ...
	  if (!sol1.empty()){
	    ...
	    sol=undef;
#else
        if (is_positive(subst(fx, x, xcurrent - xstep, false, contextptr) *
                            subst(fx, x, xcurrent + xstep, false, contextptr),
                        contextptr) ||
            use_newton) {
          sol = _fsolve(gen(makevecteur(fx, x, xcurrent, _NEWTON_SOLVER), _SEQ__VECT), contextptr);
        } else {
          sol = _fsolve(gen(makevecteur(fx, x, makevecteur(xcurrent - xstep, xcurrent + xstep),
                                        _BISECTION_SOLVER, 100 * eps),
                            _SEQ__VECT),
                        contextptr);
          get_sol(sol, contextptr);
      ...
      if (visited[icur][jcur]) {
        if (0.1 * dfxyorig_abs[icur][jcur] * dfxycurrent_abs >
            fabs(dfxcurrent * dfyorig[icur][jcur] - dfycurrent * dfxorig[icur][jcur])) {
          if (count < 2)
            chemin_ok = false;
          else {
            // join to this point
            if (is_greater(xstep, abs(re(chemin.front() - xycurrent, contextptr), contextptr),
                           contextptr) &&
                is_greater(ystep, abs(im(chemin.front() - xycurrent, contextptr), contextptr),
                           contextptr))
  ...
  gen plotimplicit(const gen & f_orig, const gen & x, const gen & y, double xmin, double xmax,
                   double ymin, double ymax, int nxstep, int nystep, double eps,
                   const vecteur & attributs, bool unfactored, bool cklinear,
                   const context * contextptr, int ckgeo2d) {
    ...
    if (cklinear && is_linear_wrt(f_orig, y, a, b, contextptr) && a != 0) {
      ...
      // y=-b/a
      return plotfunc(-b / a, x, attributs, 0, xmin, xmax, ymin, ymax, -5, 5, nxstep, 0, false,
                      contextptr);
    }
    if (cklinear && is_linear_wrt(f_orig, x, a, b, contextptr) && a != 0) {
      ...
      return symetrie(d,
                      plotfunc(-b / a, y, attributs, 0, xmin, xmax, ymin, ymax, -5, 5, nxstep, 0,
                               false, contextptr),
                      contextptr);
    }
    ...
    gen res = in_plotimplicit(ff, x, y, xmin, xmax, ymin, ymax, nxstep, nystep, eps, attributs,
                              ckgeo2d, contextptr);
    if (cplx)
  ...
  gen _plotimplicit(const gen & args, const context * contextptr) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    if (args.type != _VECT)
      return plotimplicit(remove_equal(args), vx_var, y__IDNT_e, gnuplot_xmin, gnuplot_xmax,
                          gnuplot_ymin, gnuplot_ymax, 20 * gnuplot_pixels_per_eval, 0,
                          epsilon(contextptr), vecteur(1, default_color(contextptr)), false, true,
                          contextptr, 3);
    ...
    read_option(v, xmin, xmax, ymin, ymax, zmin, zmax, attributs, nstep, jstep, kstep, unfactored,
                contextptr);
    if (dim3)
      return plotimplicit(remove_equal(v[0]), x, y, z, xmin, xmax, ymin, ymax, zmin, zmax, nstep,
                          jstep, kstep, epsilon(contextptr), attributs, unfactored, cklinear,
                          contextptr);
    else
      return plotimplicit(remove_equal(v[0]), x, y, xmin, xmax, ymin, ymax, nstep, jstep,
                          epsilon(contextptr), attributs, unfactored, cklinear, contextptr, 3);
  }
  ...
  static bool is_approx0(const gen & a, double dx, double dy) {
    if (a.type == _CPLX)
      return (fabs(a._CPLXptr->_DOUBLE_val) < 1e-6 * dx) &&
             (fabs((a._CPLXptr + 1)->_DOUBLE_val) < 1e-6 * dy);
    if (a.type == _REAL)
      return (fabs(a._CPLXptr->_DOUBLE_val) < 1e-6 * dx);
    return is_zero(a);
  }
...
static void glue_components(vecteur & v, double dx, double dy) {
  ...
  for (int i = 0; i < s - 1; ++i) {
    ...
    for (int j = i + 1; j < s; ++j) {
      gen & next = v[j];
      if (cur.type == _VECT && next.type == _VECT && !cur._VECTptr->empty() &&
          !next._VECTptr->empty()) {
        if (is_approx0(cur._VECTptr->front() - next._VECTptr->back(), dx, dy) ||
            is_approx0(cur._VECTptr->front() - next._VECTptr->front(), dx, dy))
...
gen plot_array(const vector<vector<double>> & fij, int imax, int jmax, double xmin, double xmax,
               double dx, double ymin, double ymax, double dy, const vecteur & lz,
               const vecteur & attributs, bool contour, GIAC_CONTEXT) {
  ...
  for (int i = 0; i < imax - 1; ++i) {
    ...
    for (int j = 0; j < jmax - 1; ++j) {
      ...
      for (int k = 0; k < nz; k++) {
        ...
        if (d == z)
          d += z ? z * eps : eps;
        bool ab = (a - z) * (b - z) <= 0, ca = (a - z) * (c - z) <= 0, db = (d - z) * (b - z) <= 0,
             cd = (d - z) * (c - z) <= 0;
        gen AB, CA, DB, CD;
        // intercepts
        if (ab)
      ...
      for (int n = 0; n < ncomp; n++) {
        ...
        if (is_approx0(begin - end, dx, dy)) {
          ...
          if (fij[itmp][jtmp] > Z[k]) {
            ...
            res[k][n] =
                mergevecteur(mergevecteur(vecteur(cv.begin(), cv.begin() + pos + 1),
                                          makevecteur(xymin, gen(xmax, ymin), gen(xmax, ymax),
                                                      gen(xmin, ymax), xymin)),
                             vecteur(cv.begin() + pos, cv.end()));
          }
          continue;
        }
        ...
        if ((bxmin || bxmax || bymin || bymax) && (exmin || exmax || eymin || eymax)) {
          ...
          for (; m < ncomp; m++) {
            ...
            if (begin2.type == _DOUBLE_) {
              b2x = begin2._DOUBLE_val;
              b2y = 0;
            } else {
              if (begin2.type != _CPLX)
                continue;
              b2x = begin2._CPLXptr->_DOUBLE_val;
              b2y = (begin2._CPLXptr + 1)->_DOUBLE_val;
            }
            if (end2.type == _DOUBLE_) {
              e2x = end2._DOUBLE_val;
              e2y = 0;
            } else {
              if (end2.type != _CPLX)
                continue;
...
gen plotcontour(const gen & f0, bool contour, GIAC_CONTEXT) {
  ...
  if (!s)
    return gensizeerr(contextptr);
  gen f = v[0];
  double xmin = gnuplot_xmin,
         xmax = gnuplot_xmax * (1 + 1e-6); // small shift 1e-6 for plotinequation(x^2+y^2<=4)
  ...
  for (int i = 0; i < imax; ++i) {
    ...
    for (int j = 0; !ctrl_c && !interrupted && j < jmax; ++j) {
      xyval[1] = ymin + j * dy;
      gen f1 = evalf_double(
          evalf(quotesubst(f, xy, xyval, contextptr), eval_level(contextptr), contextptr),
  ...
  return plot_array(fij, imax, jmax, xmin, xmax, dx, ymin, ymax, dy, lz, attributs, contour,
                    contextptr);
}
#endif
  gen _plotcontour(const gen & f0, GIAC_CONTEXT) {
    if (f0.type == _STRNG && f0.subtype == -1)
      return f0;
  ...
  define_unary_function_ptr5(at_plotcontour, alias_at_plotcontour, &__plotcontour, _QUOTE_ARGUMENTS,
                             true);
  ...
  define_unary_function_ptr5(at_contourplot, alias_at_contourplot, &__contourplot, _QUOTE_ARGUMENTS,
                             true);
  ...
  define_unary_function_ptr5(at_plotinequation, alias_at_plotinequation, &__plotinequation,
                             _QUOTE_ARGUMENTS, true);
  ...
  define_unary_function_ptr5(at_inequationplot, alias_at_inequationplot, &__inequationplot,
                             _QUOTE_ARGUMENTS, true);
  ...
  gen _inter_droite(const gen & args, GIAC_CONTEXT) {
    ...
    if (s < 2 || s > 3)
      return gendimerr(contextptr);
    gen res = _inter(gen(makevecteur(args._VECTptr->front(), (*args._VECTptr)[1]), _SEQ__VECT),
                     contextptr);
    if (res.type == _VECT && !res._VECTptr->empty()) {
      if (s ==
          3) { // either a point (find nearest point in res) or a list (find 1st point not in list
        vecteur v = *res._VECTptr;
        ...
        if (other.type == _VECT) {
          ...
          for (i = 0; i < vs; ++i) {
            for (j = 0; j < ws; ++j) {
              if (is_zero(evalf(recursive_normal(distance2(other[j], v[i], contextptr), contextptr),
                                1, contextptr),
                          contextptr))
                break;
            }
  ...
  */
  ...
  void papier_lignes(vecteur & res, double xmin, double xmax, double ymin, double ymax,
                     double angle, double deltax, double deltay, double pente,
                     const vecteur & attributs, GIAC_CONTEXT) {
    res.push_back(
        pnt_attrib(gen(makevecteur(xmin + ymin * cst_i, xmin + ymax * cst_i), _GROUP__VECT),
                   attributs, contextptr));
    res.push_back(
        pnt_attrib(gen(makevecteur(xmax + ymax * cst_i, xmin + ymax * cst_i), _GROUP__VECT),
                   attributs, contextptr));
    res.push_back(
        pnt_attrib(gen(makevecteur(xmax + ymin * cst_i, xmax + ymax * cst_i), _GROUP__VECT),
                   attributs, contextptr));
    res.push_back(
        pnt_attrib(gen(makevecteur(xmax + ymin * cst_i, xmin + ymin * cst_i), _GROUP__VECT),
                   attributs, contextptr));
    //const double cst_pi;
    double pi = evalf_double(cst_pi, 1, contextptr)._DOUBLE_val;
    if (angle == pi / 2) {
      for (double x = xmin; x <= xmax; x += deltax) {
        res.push_back(pnt_attrib(gen(makevecteur(x + ymin * cst_i, x + ymax * cst_i), _GROUP__VECT),
                                 attributs, contextptr));
      }
      //return res;
    }
    ...
    if (angle < pi / 2) {
      ...
      for (double x = xmin - q * deltax; x <= xmin; x += deltax) {
        ...
        if (Y1 < ymax) {
          res.push_back(
              pnt_attrib(gen(makevecteur(xmin + Y2 * cst_i, xmax + Y1 * cst_i), _GROUP__VECT),
                         attributs, contextptr));
        }
        if (X1 < xmax) {
          res.push_back(
              pnt_attrib(gen(makevecteur(xmin + Y2 * cst_i, X1 + ymax * cst_i), _GROUP__VECT),
                         attributs, contextptr));
        }
      }
      for (double x = xmin; x <= xmax; x += deltax) {
        ...
        if (Y1 < ymax) {
          res.push_back(
              pnt_attrib(gen(makevecteur(x + ymin * cst_i, xmax + Y1 * cst_i), _GROUP__VECT),
                         attributs, contextptr));
        }
        if (X1 < xmax) {
          res.push_back(
              pnt_attrib(gen(makevecteur(x + ymin * cst_i, X1 + ymax * cst_i), _GROUP__VECT),
                         attributs, contextptr));
        }
      }
      //return res;
    ...
    if (angle > pi / 2) {
      ...
      while (x <= xmax) {
        ...
        if (Y1 < ymax) {
          res.push_back(
              pnt_attrib(gen(makevecteur(x + ymin * cst_i, xmin + Y1 * cst_i), _GROUP__VECT),
                         attributs, contextptr));
        }
        if (X1 > xmin) {
          res.push_back(
      ...
      while (x <= xmax - q * deltax) {
        ...
        if (Y1 < ymax) {
          res.push_back(
              pnt_attrib(gen(makevecteur(xmax + Y2 * cst_i, xmin + Y1 * cst_i), _GROUP__VECT),
                         attributs, contextptr));
        }
        if (X1 > xmin) {
          res.push_back(
              pnt_attrib(gen(makevecteur(xmax + Y2 * cst_i, X1 + ymax * cst_i), _GROUP__VECT),
                         attributs, contextptr));
        }
        x = x + deltax;
      }
  ...
  static gen Papier_pointe_quadrillage(const gen & args, int quadrillage, GIAC_CONTEXT) {
    double xmin = gnuplot_xmin, xmax = gnuplot_xmax, ymin = gnuplot_ymin, ymax = gnuplot_ymax;
    double deltax = (xmax - xmin) / 20, deltay = (ymax - ymin) / 20,
           angle = evalf_double(cst_pi / 2, 1, contextptr)._DOUBLE_val;
    vecteur attributs(1, default_color(contextptr));
    if (args.type == _VECT) {
      vecteur & w = *args._VECTptr;
      int s = int(w.size());
    ...
    if (quadrillage == 0 || quadrillage == 1) {
      for (double y = ymin; y <= ymax; y += deltay) {
        res.push_back(pnt_attrib(gen(makevecteur(xmin + y * cst_i, xmax + y * cst_i), _GROUP__VECT),
                                 attributs, contextptr));
      }
      //papier_lignes(res,xmin,xmax,ymin,ymax,0,deltax,deltay,pente,attributs,contextptr);
      papier_lignes(res, xmin, xmax, ymin, ymax, angle, deltax, deltay, pente, attributs,
                    contextptr);
    }
    if (quadrillage == 1) {
      ...
      if (u1 - deltax > 0) {
        angle = std::atan(deltay / (u1 - deltax));
      }
      if (u1 - deltax < 0) {
        angle = std::atan(deltay / (u1 - deltax)) + M_PI;
      }
      papier_lignes(res, xmin, xmax, ymin, ymax, angle, deltax, deltay, std::tan(angle), attributs,
                    contextptr);
  ...
  define_unary_function_ptr5(at_triangle_paper, alias_at_triangle_paper, &__triangle_paper, 0,
                             true);
  ...
  gen _DrawInv(const gen & g, const context * contextptr) {
    ...
    return _plotparam(gen(makevecteur(y + cst_i * x, x, gnuplot_xmin, gnuplot_xmax), _SEQ__VECT),
                      contextptr);
  }
  ...
  logo_turtle vecteur2turtle(const vecteur & v) {
    int s = int(v.size());
    if (s >= 5 && v[0].type == _DOUBLE_ && v[1].type == _DOUBLE_ && v[2].type == _DOUBLE_ &&
        v[3].type == _INT_ && v[4].type == _INT_) {
      logo_turtle t;
      t.x = v[0]._DOUBLE_val;
      t.y = v[1]._DOUBLE_val;
  ...
  gen turtle2gen(const logo_turtle & turtle) {
    return gen(makevecteur(turtle.x, turtle.y, double(turtle.theta), turtle_status(turtle),
                           turtle.radius, turtle.s),
               _LOGO__VECT);
  }
  ...
  static define_unary_function_eval2(__tourne_droite, &_tourne_droite, _tourne_droite_s,
                                     &printastifunction);
  ...
  static define_unary_function_eval2(__tourne_gauche, &_tourne_gauche, _tourne_gauche_s,
                                     &printastifunction);
  ...
  static define_unary_function_eval2(__leve_crayon, &_leve_crayon, _leve_crayon_s,
                                     &printastifunction);
  ...
  static define_unary_function_eval2(__baisse_crayon, &_baisse_crayon, _baisse_crayon_s,
                                     &printastifunction);
  ...
  static define_unary_function_eval2(__pas_de_cote, &_pas_de_cote, _pas_de_cote_s,
                                     &printastifunction);
  ...
  static define_unary_function_eval2(__cache_tortue, &_cache_tortue, _cache_tortue_s,
                                     &printastifunction);
  ...
  static define_unary_function_eval2(__montre_tortue, &_montre_tortue, _montre_tortue_s,
                                     &printastifunction);
  ...
  gen _crayon(const gen & g, GIAC_CONTEXT) {
    if (g.type == _STRNG && g.subtype == -1)
      return g;
    if (g.type == _STRNG)
      return _crayon(gen(*g._STRNGptr, contextptr), contextptr);
    // logo instruction
    if (g.type == _VECT && g._VECTptr->size() == 3)
  ...
  static define_unary_function_eval2(__efface_logo, &_efface_logo, _efface_logo_s,
                                     &printastifunction);
  ...
  static void turtle_move(int r, int theta2, GIAC_CONTEXT) {
    ...
    (*turtleptr).x +=
        r * (std::cos(M_PI / 180 * (theta2 + theta0)) - std::cos(M_PI / 180 * theta0));
    (*turtleptr).y +=
        r * (std::sin(M_PI / 180 * (theta2 + theta0)) - std::sin(M_PI / 180 * theta0));
  ...
  static define_unary_function_eval2(__disque_centre, &_disque_centre, _disque_centre_s,
                                     &printastifunction);
  ...
  static define_unary_function_eval2(__polygone_rempli, &_polygone_rempli, _polygone_rempli_s,
                                     &printastifunction);
  define_unary_function_ptr5(at_polygone_rempli, alias_at_polygone_rempli, &__polygone_rempli, 0,
                             T_LOGO);
  ...
  static define_unary_function_eval2(__rectangle_plein, &_rectangle_plein, _rectangle_plein_s,
                                     &printastifunction);
  define_unary_function_ptr5(at_rectangle_plein, alias_at_rectangle_plein, &__rectangle_plein, 0,
                             T_LOGO);
  ...
  static define_unary_function_eval2(__triangle_plein, &_triangle_plein, _triangle_plein_s,
                                     &printastifunction);
  define_unary_function_ptr5(at_triangle_plein, alias_at_triangle_plein, &__triangle_plein, 0,
                             T_LOGO);
  ...
  static define_unary_function_eval2(__dessine_tortue, &_dessine_tortue, _dessine_tortue_s,
                                     &printastifunction);
  define_unary_function_ptr5(at_dessine_tortue, alias_at_dessine_tortue, &__dessine_tortue, 0,
                             T_LOGO);
  ...
  */
  gen _arc(const gen & args, GIAC_CONTEXT) {
    ...
    if (segment_cercle) {
      ...
      res = pnt_attrib(symbolic(at_cercle, gen((s == 4 && v[3].type < _IDNT)
                                                   ? makevecteur(diametre, zero, g, v[3])
                                                   : makevecteur(diametre, zero, g),
                                               _PNT__VECT)),
                       attributs, contextptr);
    }
  ...
  int est_rect(const gen & a, const gen & b, const gen & c, const gen & d, GIAC_CONTEXT) {
    ...
    if (is_zero(simplify(e, contextptr), contextptr) &&
        is_zero(simplify(f, contextptr), contextptr)) {
  ...
  int est_parallelogramme(const gen & a, const gen & b, const gen & c, const gen & d,
                          GIAC_CONTEXT) {
    gen e(a - b + c - d);
    if (is_zero(simplify(e, contextptr), contextptr)) {
  ...
  bool est_carre(const gen & a, const gen & b, const gen & c, const gen & d, GIAC_CONTEXT) {
    ...
    gen h(dotvecteur(d - a, b - a, contextptr));
    return is_zero(simplify(e, contextptr), contextptr) &&
           is_zero(simplify(g, contextptr), contextptr) &&
           is_zero(simplify(h, contextptr), contextptr);
  }
  ...
  bool est_equilateral(const gen & a, const gen & b, const gen & c, GIAC_CONTEXT) {
    ...
    gen f(abs_norm2(c - a, contextptr));
    gen e(abs_norm2(b - c, contextptr));
    return is_zero(simplify(d - f, contextptr), contextptr) &&
           is_zero(simplify(e - f, contextptr), contextptr);
  }
  ...
  gen _est_equilateral(const gen & args, GIAC_CONTEXT) {
    ...
    vecteur v = sommet(args);
    if (v.size() == 3) {
      return est_equilateral(remove_at_pnt(v[0]), remove_at_pnt(v[1]), remove_at_pnt(v[2]),
                             contextptr);
    }
  ...
  define_unary_function_ptr5(at_est_equilateral, alias_at_est_equilateral, &__est_equilateral, 0,
                             true);
  ...
  gen _est_parallelogramme(const gen & args, GIAC_CONTEXT) {
    ...
    vecteur v = sommet(args);
    if (v.size() == 4) {
      return est_parallelogramme(remove_at_pnt(v[0]), remove_at_pnt(v[1]), remove_at_pnt(v[2]),
                                 remove_at_pnt(v[3]), contextptr);
    }
    return symbolic(at_est_parallelogramme, args);
  }
  static const char _est_parallelogramme_s[] = "is_parallelogram";
  static define_unary_function_eval(__est_parallelogramme, &_est_parallelogramme,
                                    _est_parallelogramme_s);
  define_unary_function_ptr5(at_est_parallelogramme, alias_at_est_parallelogramme,
                             &__est_parallelogramme, 0, true);
  ...
  gen _est_carre(const gen & args, GIAC_CONTEXT) {
    ...
    vecteur v = sommet(args);
    if (v.size() == 4) {
      return est_carre(remove_at_pnt(v[0]), remove_at_pnt(v[1]), remove_at_pnt(v[2]),
                       remove_at_pnt(v[3]), contextptr);
    }
  ...
  gen _est_losange(const gen & args, GIAC_CONTEXT) {
    ...
    if (v.back() == v.front())
      v.pop_back();
    if (v.size() == 4) {
      return est_losange(remove_at_pnt(v[0]), remove_at_pnt(v[1]), remove_at_pnt(v[2]),
                         remove_at_pnt(v[3]), contextptr);
    }
  ...
  gen _est_rectangle(const gen & args, GIAC_CONTEXT) {
    ...
    if (v.back() == v.front())
      v.pop_back();
    if (v.size() == 3) {
      return est_trianglerect(remove_at_pnt(v[0]), remove_at_pnt(v[1]), remove_at_pnt(v[2]),
                              contextptr);
    }
    if (v.size() == 4) {
      return est_rect(remove_at_pnt(v[0]), remove_at_pnt(v[1]), remove_at_pnt(v[2]),
                      remove_at_pnt(v[3]), contextptr);
    }
  ...
  gen _birapport(const gen & args, GIAC_CONTEXT) {
    ...
    if ((args.type == _VECT) && (args._VECTptr->size() == 4)) {
      vecteur v(*args._VECTptr);
      gen a(remove_at_pnt(v[0])), b(remove_at_pnt(v[1])), c(remove_at_pnt(v[2])),
          d(remove_at_pnt(v[3]));
  ...
  gen _axe_radical(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT) {
      ...
      gen k = ratnormal((abs_norm2(R1, contextptr) - abs_norm2(R2, contextptr)) /
                            abs_norm2(c1 - c2, contextptr),
                        contextptr);
  ...
  gen equation_homogene(const gen & eq, GIAC_CONTEXT) {
    ...
    alg_lvar(eq, v);
    gen v0 = v.front();
    if (v0.type != _VECT || v0._VECTptr->size() < 3 || v0[0] != x__IDNT_e || v0[1] != y__IDNT_e ||
        v0[2] != z__IDNT_e)
      return gensizeerr(contextptr);
  ...
  gen _polaire(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT) {
      ...
      if (!centre_rayon(C, c, R, false, contextptr)) {
        ...
        gen res = x0 * derive(eq, x__IDNT_e, contextptr) + y0 * derive(eq, y__IDNT_e, contextptr) +
                  derive(eq, z__IDNT_e, contextptr);
        if (is_undef(res))
          return res;
        return _droite(recursive_normal(subst(res, z__IDNT_e, 1, false, contextptr), contextptr),
                       contextptr);
      }
    ...
    return change_subtype(_coordonnees_polaires(args, contextptr),
                          _SEQ__VECT); //return symbolic(at_polaire,args);
    //return gensizeerr(contextptr);
  }
  ...
  gen _polaire_reciproque(const gen & args, GIAC_CONTEXT) {
    ...
    if (a.type == _VECT) {
      ...
      for (; it != itend; ++it) {
        ...
        a = remove_at_pnt(a);
        if (a.type == _VECT)
          res.push_back(put_attributs(_pole(gen(makevecteur(c, a), _SEQ__VECT), contextptr),
                                      attributs, contextptr));
        else
          res.push_back(put_attributs(_polaire(gen(makevecteur(c, a), _SEQ__VECT), contextptr),
                                      attributs, contextptr));
      }
    ...
    a = remove_at_pnt(a);
    if (a.type == _VECT)
      return put_attributs(_pole(gen(makevecteur(c, a), _SEQ__VECT), contextptr), attributs,
                           contextptr);
    return put_attributs(_polaire(gen(makevecteur(c, a), _SEQ__VECT), contextptr), attributs,
                         contextptr);
  }
  static const char _polaire_reciproque_s[] = "reciprocation";
  static define_unary_function_eval(__polaire_reciproque, &_polaire_reciproque,
                                    _polaire_reciproque_s);
  define_unary_function_ptr5(at_polaire_reciproque, alias_at_polaire_reciproque,
                             &__polaire_reciproque, 0, true);

  //teste si deux cercles C1 centre c1 rayon R1 et C2  centre c2 rayon R2
  //sont orthogonaux
  bool est_orthogonal(const gen & c1, const gen & R1, const gen & c2, const gen & R2,
                      GIAC_CONTEXT) {
    gen res = simplify(-abs_norm2(R1, contextptr) - abs_norm2(R2, contextptr) +
                           abs_norm2(c1 - c2, contextptr),
                       contextptr);
    ...
    if (args.type == _VECT && args._VECTptr->size() == 2) {
      ...
      if (a.is_symb_of_sommet(at_hyperplan)) {
        ...
        if (ckmatrix(b) && b._VECTptr->size() == 2)
          return est_parallele_vecteur(hyperplan_normal(a),
                                       *(b._VECTptr->back() - b._VECTptr->front())._VECTptr,
                                       contextptr);
        if (b.is_symb_of_sommet(at_hyperplan))
      ...
      if ((a.type != _VECT) || (a._VECTptr->size() != 2) || (b.type != _VECT) ||
          (b._VECTptr->size() != 2))
        return gensizeerr(contextptr);
      //on a 2 droites
  ...
  define_unary_function_ptr5(at_est_orthogonal, alias_at_est_orthogonal, &__est_orthogonal, 0,
                             true);
  ...
  static define_unary_function_eval(__est_perpendiculaire, &_est_perpendiculaire,
                                    _est_perpendiculaire_s);
  define_unary_function_ptr5(at_est_perpendiculaire, alias_at_est_perpendiculaire,
                             &__est_perpendiculaire, 0, true);
  ...
  gen _est_conjugue(const gen & args, GIAC_CONTEXT) {
    ...
    if (s == 3) {
      ...
      if ((c.type != _VECT) && (b.type != _VECT)) // on a 2 points
        return _est_orthogonal(makesequence(v[0], _cercle(makesequence(v[1], v[2]), contextptr)),
                               contextptr);
      if (b.type == _VECT) {
  ...
  gen _est_harmonique(const gen & args, GIAC_CONTEXT) {
    ...
    if ((args.type == _VECT) && (args._VECTptr->size() == 4)) {
      vecteur v(*args._VECTptr);
      return est_harmonique(remove_at_pnt(v[0]), remove_at_pnt(v[1]), remove_at_pnt(v[2]),
                            remove_at_pnt(v[3]), contextptr);
    }
  ...
  define_unary_function_ptr5(at_est_harmonique, alias_at_est_harmonique, &__est_harmonique, 0,
                             true);
  ...
  define_unary_function_ptr5(at_conj_harmonique, alias_at_conj_harmonique, &__conj_harmonique, 0,
                             true);
  ...
  gen _point_div(const gen & args, GIAC_CONTEXT) {
    ...
    vecteur v(*args._VECTptr);
    gen a = remove_at_pnt(eval(v[0], contextptr)), b = remove_at_pnt(eval(v[1], contextptr)),
        k = eval(v[2], contextptr);
    //if (! is_zero(im(k,contextptr),contextptr)) return gensizeerr(contextptr);
    gen c;
    k = normal(1 - k, contextptr);
    if (is_zero(k, contextptr))
      return gensizeerr(contextptr);
    //{c=unsigned_inf;}
  ...
  int est_faisceau_cercle(const gen & c1, const gen & R1, const gen & c2, const gen & R2,
                          const gen & c3, const gen & R3, GIAC_CONTEXT) {
    ...
    if (!est_aligne(c1, c2, c3, contextptr))
      return 0;
    //les centres sont alignes
    gen v = _axe_radical(makesequence(_cercle(makesequence(c1, R1), contextptr),
                                      _cercle(makesequence(c2, R2), contextptr)),
                         contextptr);
    gen w = _axe_radical(makesequence(_cercle(makesequence(c1, R1), contextptr),
                                      _cercle(makesequence(c3, R3), contextptr)),
                         contextptr);
    v = remove_at_pnt(v);
  ...
  static define_unary_function_eval(__est_faisceau_cercle, &_est_faisceau_cercle,
                                    _est_faisceau_cercle_s);
  define_unary_function_ptr5(at_est_faisceau_cercle, alias_at_est_faisceau_cercle,
                             &__est_faisceau_cercle, 0, true);
  ...
  int est_faisceau_droite(const gen & a, const gen & b, const gen & c, GIAC_CONTEXT) {
    ...
    if (v.type == _VECT && !v._VECTptr->empty() &&
        est_element(v[0], _droite(c, contextptr), contextptr))
      return 1;
    else
      return 0;
  }
  ...
  static define_unary_function_eval(__est_faisceau_droite, &_est_faisceau_droite,
                                    _est_faisceau_droite_s);
  define_unary_function_ptr5(at_est_faisceau_droite, alias_at_est_faisceau_droite,
                             &__est_faisceau_droite, 0, true);
  ...
  define_unary_function_ptr5(at_div_harmonique, alias_at_div_harmonique, &__div_harmonique,
                             _QUOTE_ARGUMENTS, true);
  ...
  gen _enveloppe(const gen & args, GIAC_CONTEXT) {
    ...
    if (var.type != _VECT) {
      ...
      xvar = vx_var;
      yvar = y__IDNT_e;
    } else if (var._VECTptr->size() == 1) {
      tvar = var[0];
      xvar = vx_var;
      yvar = y__IDNT_e;
    } else if (var._VECTptr->size() == 3) {
      xvar = var[0];
      if (xvar.type != _IDNT)
        return gentypeerr(contextptr);
      yvar = var[1];
  ...
  int graph_output_type(const gen & g) {
    ...
    if (g.type == _VECT && g.subtype == _LOGO__VECT && g._VECTptr->size() == 6) {
      ...
      if (v[0].type == _DOUBLE_ && v[1].type == _DOUBLE_ && v[2].type == _DOUBLE_ &&
          v[3].type == _INT_ && v[4].type == _INT_ && v[5].type == _STRNG)
        return 4;
    }
    if (g.type == _VECT && !g._VECTptr->empty())
  ...
  gen _Ox_2d_unit_vector(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    vecteur v =
        makevecteur(_point(makevecteur(0, 0), contextptr), _point(makevecteur(1, 0), contextptr));
    if (args.type == _VECT)
  ...
  define_unary_function_ptr5(at_Ox_2d_unit_vector, alias_at_Ox_2d_unit_vector, &__Ox_2d_unit_vector,
                             0, true);
  ...
  gen _Oy_2d_unit_vector(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    vecteur v =
        makevecteur(_point(makevecteur(0, 0), contextptr), _point(makevecteur(0, 1), contextptr));
    if (args.type == _VECT)
      v = mergevecteur(v, *args._VECTptr);
    else
      v.push_back(args);
  ...
  define_unary_function_ptr5(at_Oy_2d_unit_vector, alias_at_Oy_2d_unit_vector, &__Oy_2d_unit_vector,
                             0, true);
  ...
  gen _Ox_3d_unit_vector(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    vecteur v = makevecteur(_point(makevecteur(0, 0, 0), contextptr),
                            _point(makevecteur(1, 0, 0), contextptr));
    if (args.type == _VECT)
      v = mergevecteur(v, *args._VECTptr);
    else
      v.push_back(args);
  ...
  define_unary_function_ptr5(at_Ox_3d_unit_vector, alias_at_Ox_3d_unit_vector, &__Ox_3d_unit_vector,
                             0, true);
  ...
  gen _Oy_3d_unit_vector(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    vecteur v = makevecteur(_point(makevecteur(0, 0, 0), contextptr),
                            _point(makevecteur(0, 1, 0), contextptr));
    if (args.type == _VECT)
      v = mergevecteur(v, *args._VECTptr);
    else
      v.push_back(args);
  ...
  define_unary_function_ptr5(at_Oy_3d_unit_vector, alias_at_Oy_3d_unit_vector, &__Oy_3d_unit_vector,
                             0, true);
  ...
  gen _Oz_3d_unit_vector(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    vecteur v = makevecteur(_point(makevecteur(0, 0, 0), contextptr),
                            _point(makevecteur(0, 0, 1), contextptr));
    if (args.type == _VECT)
      v = mergevecteur(v, *args._VECTptr);
    else
      v.push_back(args);
  ...
  define_unary_function_ptr5(at_Oz_3d_unit_vector, alias_at_Oz_3d_unit_vector, &__Oz_3d_unit_vector,
                             0, true);
  ...
  gen _frame_3d(const gen & args, GIAC_CONTEXT) {
    ...
    res.push_back(_vector(gen(v, _SEQ__VECT), contextptr));
    v = makevecteur(_point(makevecteur(0, 0, 0), contextptr),
                    _point(makevecteur(0, 1, 0), contextptr));
  ...
  gen _plot3d(const gen & g, const context * contextptr) {
    ...
    if (v[0].type == _VECT) {
      ...
      if (v[1].is_symb_of_sommet(at_interval)) {
        if (!chk_double_interval(v[1], tmin, tmax, contextptr) ||
            !chk_double_interval(v[2], smin, smax, contextptr))
          return gensizeerr(contextptr);
        vecteur vars(makevecteur(s__IDNT_e, t__IDNT_e));
        return plotparam3d(v[0](gen(vars, _SEQ__VECT), contextptr), vars, gnuplot_xmin,
                           gnuplot_xmax, gnuplot_ymin, gnuplot_ymax, gnuplot_zmin, gnuplot_xmax,
                           tmin, tmax, smin, smax, true, autoscale, attributs, gnuplot_tstep,
                           gnuplot_tstep, undef, vecteur(0), contextptr);
      } else {
        if (!readrange(v[1], gnuplot_tmin, gnuplot_tmax, v[1], tmin, tmax, contextptr) ||
            !readrange(v[2], gnuplot_tmin, gnuplot_tmax, v[2], smin, smax, contextptr))
          return gensizeerr(contextptr);
        return plotparam3d(v[0], makevecteur(v[1], v[2]), gnuplot_xmin, gnuplot_xmax, gnuplot_ymin,
                           gnuplot_ymax, gnuplot_zmin, gnuplot_xmax, tmin, tmax, smin, smax, true,
                           autoscale, attributs, gnuplot_tstep, gnuplot_tstep, undef, vecteur(0),
                           contextptr);
      }
    } else {
      double xmin, xmax, ymin, ymax;
      if (v[1].is_symb_of_sommet(at_interval)) {
        if (!chk_double_interval(v[1], xmin, xmax, contextptr) ||
            !chk_double_interval(v[2], ymin, ymax, contextptr))
          return gensizeerr(contextptr);
        gen vars(makevecteur(x__IDNT_e, y__IDNT_e), _SEQ__VECT);
        return plotfunc(v[0](vars, contextptr), vars, attributs, false, xmin, xmax, ymin, ymax,
                        gnuplot_zmin, gnuplot_zmax, gnuplot_pixels_per_eval, 0, false, contextptr);
      } else {
        if (!readrange(v[1], gnuplot_xmin, gnuplot_xmax, v[1], xmin, xmax, contextptr) ||
            !readrange(v[2], gnuplot_ymin, gnuplot_ymax, v[2], ymin, ymax, contextptr))
          return gensizeerr(contextptr);
        return plotfunc(v[0], makevecteur(v[1], v[2]), attributs, false, xmin, xmax, ymin, ymax,
                        gnuplot_zmin, gnuplot_zmax, gnuplot_pixels_per_eval, 0, false, contextptr);
      }
    }
  }
  ...
  gen _inter(const gen & args, GIAC_CONTEXT) {
    ...
    if (ww.size() == 1 && ww.front().is_symb_of_sommet(at_inter)) {
      ...
      try {
        ...
        iterateur it = sol.begin(), itend = sol.end();
        for (; it != itend; ++it) {
          *it = gen(makevecteur(*it, normal(subst(a, x, *it, false, contextptr), contextptr)),
                    _GGB__VECT);
        }
        return sol;
#ifndef NO_STDEXCEPT
  ...
  gen _bezier(const gen & args, GIAC_CONTEXT) {
    ...
    if (parameq) {
      gen peq = _parameq(
          gen(makevecteur(symbolic(at_Bezier, gen(v, _GROUP__VECT)), t__IDNT_e), _SEQ__VECT),
          contextptr);
      if (d3)
        return plotparam3d(peq, makevecteur(t__IDNT_e, v__IDNT_e), -1e300, 1e300, -1e300, 1e300,
                           -1e300, 1e300, 0, 1, 0, 1, false, true, attributs, 0.01, 0.01, undef,
                           vecteur(0), contextptr);
      return plotparam(peq, t__IDNT_e, attributs, false, -1e300, 1e300, -1e300, 1e300, 0, 1, 0.01,
                       undef, peq, contextptr);
    }
...
#if defined(GIAC_GENERIC_CONSTANTS) || (defined(VISUALC) && !defined(RTOS_THREADX)) ||             \
    defined(x86_64)
  unary_function_ptr plot_sommets[] = {*at_pnt,   *at_parameter, *at_cercle,
                                       *at_curve, *at_animation, 0};
  unary_function_ptr not_point_sommets[] = {*at_cercle,      *at_curve,        *at_hyperplan,
                                            *at_hypersphere, *at_hypersurface, 0};
  unary_function_ptr notexprint_plot_sommets[] = {*at_funcplot,
                                                  *at_paramplot,
                                                  *at_polarplot,
                                                  *at_implicitplot,
                                                  *at_contourplot,
                                                  ...
                                                  *at_parabole,
                                                  0};
  unary_function_ptr implicittex_plot_sommets[] = {
      *at_plot,        *at_plot3d,    *at_plotfunc,
      *at_plotparam,   *at_plotpolar, *at_plotimplicit,
      *at_plotcontour, *at_DrawInv,   *at_DrawFunc,
      *at_DrawParm,    *at_DrawPol,   *at_DrwCtour,
      *at_plotode,     *at_plotfield, *at_interactive_plotode,
      *at_plotseq,     *at_Graph,     0};
  unary_function_ptr point_sommet_tab_op[] = {
      *at_point, *at_element, *at_inter_unique, *at_centre, *at_isobarycentre, *at_barycentre, 0};
  unary_function_ptr nosplit_polygon_function[] = {*at_inter_unique,
                                                   *at_inter,
                                                   *at_distanceat,
                                                   *at_distanceatraw,
                                                   *at_rotation,
                                                   ...
                                                   *at_cercle,
                                                   0};
  unary_function_ptr measure_functions[] = {*at_angleat,
                                            *at_angleatraw,
                                            *at_areaat,
                                            *at_areaatraw,
                                            *at_perimeterat,
                                            ...
                                            *at_distanceatraw,
                                            0};
  unary_function_ptr transformation_functions[] = {
      *at_projection, *at_rotation,           *at_translation,
      *at_homothetie, *at_similitude,         *at_inversion,
      *at_symetrie,   *at_polaire_reciproque, 0};

#else
const size_t plot_sommets_alias[] = {(size_t)&__pnt,   (size_t)&__parameter, (size_t)&__cercle,
                                     (size_t)&__curve, (size_t)&__animation, 0};
const unary_function_ptr * const plot_sommets = (const unary_function_ptr *)plot_sommets_alias;

const size_t not_point_sommets_alias[] = {(size_t)&__cercle,       (size_t)&__curve,
                                          (size_t)&__hyperplan,    (size_t)&__hypersphere,
                                          (size_t)&__hypersurface, 0};
const unary_function_ptr * const not_point_sommets = (const unary_function_ptr *)plot_sommets_alias;

const size_t notexprint_plot_sommets_alias[] = {(size_t)&__funcplot,
                                                (size_t)&__paramplot,
                                                (size_t)&__polarplot,
                                                (size_t)&__implicitplot,
                                                (size_t)&__contourplot,
                                                (size_t)&__odeplot,
                                                (size_t)&__interactive_odeplot,
                                                (size_t)&__fieldplot,
                                                (size_t)&__seqplot,
                                                (size_t)&__ellipse,
                                                (size_t)&__hyperbole,
                                                (size_t)&__parabole,
                                                0};
const unary_function_ptr * const notexprint_plot_sommets =
    (const unary_function_ptr *)notexprint_plot_sommets_alias;

const size_t implicittex_plot_sommets_alias[] = {
    (size_t)&__plot,        (size_t)&__plot3d,    (size_t)&__plotfunc,
    (size_t)&__plotparam,   (size_t)&__plotpolar, (size_t)&__plotimplicit,
    (size_t)&__plotcontour, (size_t)&__DrawInv,   (size_t)&__DrawFunc,
    (size_t)&__DrawParm,    (size_t)&__DrawPol,   (size_t)&__DrwCtour,
    (size_t)&__plotode,     (size_t)&__plotfield, (size_t)&__interactive_plotode,
    (size_t)&__plotseq,     (size_t)&__Graph,     0};
const unary_function_ptr * const implicittex_plot_sommets =
    (const unary_function_ptr *)implicittex_plot_sommets_alias;

const size_t point_sommet_tab_op_alias[] = {(size_t)&__point,
                                            (size_t)&__element,
                                            (size_t)&__inter_unique,
                                            (size_t)&__centre,
                                            (size_t)&__isobarycentre,
                                            (size_t)&__barycentre,
                                            0};
const unary_function_ptr * const point_sommet_tab_op =
    (const unary_function_ptr *)point_sommet_tab_op_alias;

const size_t nosplit_polygon_function_alias[] = {(size_t)&__inter_unique,
                                                 (size_t)&__inter,
                                                 (size_t)&__distanceatraw,
                                                 (size_t)&__distanceat,
                                                 (size_t)&__rotation,
                                                 (size_t)&__projection,
                                                 (size_t)&__symetrie,
                                                 (size_t)&__polaire_reciproque,
                                                 (size_t)&__areaat,
                                                 (size_t)&__areaatraw,
                                                 (size_t)&__perimeterat,
                                                 (size_t)&__perimeteratraw,
                                                 (size_t)&__slopeat,
                                                 (size_t)&__slopeatraw,
                                                 (size_t)&__tangent,
                                                 (size_t)&__cercle,
                                                 0};
const unary_function_ptr * const nosplit_polygon_function =
    (const unary_function_ptr *)nosplit_polygon_function_alias;

const size_t measure_functions_alias[] = {(size_t)&__angleat,
                                          (size_t)&__angleatraw,
                                          (size_t)&__areaat,
                                          (size_t)&__areaatraw,
                                          (size_t)&__perimeterat,
                                          ...
                                          (size_t)&__distanceatraw,
                                          0};
const unary_function_ptr * const measure_functions =
    (const unary_function_ptr *)measure_functions_alias;

const size_t transformation_functions_alias[] = {
    (size_t)&__projection, (size_t)&__rotation,           (size_t)&__translation,
    (size_t)&__homothetie, (size_t)&__similitude,         (size_t)&__inversion,
    (size_t)&__symetrie,   (size_t)&__polaire_reciproque, 0};
const unary_function_ptr * const transformation_functions =
    (const unary_function_ptr *)transformation_functions_alias;
#endif
