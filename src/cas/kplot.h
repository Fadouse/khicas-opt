...
std::vector<logo_turtle> & turtle_stack();
  ...
  bool find_curve_parametrization(const gen & geo_obj, gen & m, const gen & gen_t, double T,
                                  gen & tmin, gen & tmax, gen & tstep, GIAC_CONTEXT);
  // test if a point f is on a parametric curve e
  // compute t if true
  bool on(const gen & e_orig, const gen & f, gen & t, GIAC_CONTEXT);

  gen plotfunc(const gen & f, const gen & vars, const vecteur & attributs, bool clrplot,
...
extern unary_function_eval __click;
  ...
  gen plotimplicit(const gen & f_orig, const gen & x, const gen & y, double xmin, double xmax,
                   ...
                   const vecteur & attributs, bool unfactored, const context * contextptr,
                   int ckgeo2d);
