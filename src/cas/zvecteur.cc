...
inline int smod(int a, int b, int c, int m) {
  ...
  vecteur makevecteur(const gen & a, const gen & b, const gen & c, const gen & d, const gen & e,
                      const gen & f, const gen & g, const gen & h) {
    vecteur v(8);
    v[0] = a;
    v[1] = b;
    v[2] = c;
  ...
  vecteur makevecteur(const gen & a, const gen & b, const gen & c, const gen & d, const gen & e,
                      const gen & f, const gen & g, const gen & h, const gen & i) {
    vecteur v(9);
    v[0] = a;
    v[1] = b;
    v[2] = c;
  ...
  vecteur makevecteur(const gen & a, const gen & b, const gen & c, const gen & d, const gen & e,
                      const gen & f, const gen & g, const gen & h, const gen & i, const gen & j) {
    vecteur v(10);
    v[0] = a;
    v[1] = b;
    v[2] = c;
  ...
  vecteur makevecteur(const gen & a, const gen & b, const gen & c, const gen & d, const gen & e,
                      const gen & f, const gen & g, const gen & h, const gen & i, const gen & j,
                      const gen & k, const gen & l, const gen & m) {
    vecteur v(13);
    v[0] = a;
    v[1] = b;
    v[2] = c;
  ...
  vecteur makevecteur(const gen & a, const gen & b, const gen & c, const gen & d, const gen & e,
                      const gen & f, const gen & g, const gen & h, const gen & i, const gen & j,
                      const gen & k, const gen & l, const gen & m, const gen & n) {
    vecteur v(14);
    v[0] = a;
    v[1] = b;
    v[2] = c;
  ...
  ref_vecteur * makenewvecteur(const gen & a, const gen & b, const gen & c, const gen & d,
                               const gen & e, const gen & f, const gen & g) {
    ref_vecteur * vptr = new_ref_vecteur(0);
    vptr->v.reserve(7);
    vptr->v.push_back(a);
    vptr->v.push_back(b);
  ...
  void transpose_double(const matrix_double & a, int r0, int r1, int c0, int c1,
                        matrix_double & at) {
  ...
  gen spread_convert(const gen & g, int g_row, int g_col, GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      if (iscell(*it, c, r, contextptr)) {
        sub_in.push_back(*it);
        sub_out.push_back(
            symbolic(at_cell, makevecteur(makevecteur(r - g_row), makevecteur(c - g_col))));
      }
    }
    // absolute cell
    l = lop(g, at_dollar);
    ...
    for (it = l.begin(); it != itend; ++it) {
      ...
      if (v.size() == 2) {
        ...
        if ((a.type == _SYMB) && (a._SYMBptr->sommet == at_dollar) &&
            (a._SYMBptr->feuille.type == _IDNT)) {
  ...
  gen pos2cell(const gen & g, int i, int j, int r, int c, int r2, int c2) {
    if (g.is_symb_of_sommet(at_deuxpoints) && g._SYMBptr->feuille.type == _VECT) {
      ...
      return symbolic(at_deuxpoints, makevecteur(pos2cell(gf[0], i, j, r, c, r, c),
                                                 pos2cell(gf[1], i, j, r2, c2, r2, c2)));
    }
    vecteur v;
    if ((g.type == _SYMB) && (g._SYMBptr->sommet == at_cell))
      v = *g._SYMBptr->feuille._VECTptr;
  ...
  matrice matrice_insert(const matrice & m, int insert_row, int insert_col, int nrows, int ncols,
                         const gen & fill_, GIAC_CONTEXT) {
        ...
        for (; it != itend; ++it) {
          ...
          if (type) {
            ...
            if (cell_c2 >= insert_col)
              cell_c2 += ncols;
            sub_out.push_back(
                pos2cell(*it, i + decal_i, j + decal_j, cell_r, cell_c, cell_r2, cell_c2));
          } else
  ...
  // erase nrows/ncols
  matrice matrice_erase(const matrice & m, int insert_row, int insert_col, int nrows, int ncols,
                        GIAC_CONTEXT) {
    int r, c, cell_r, cell_c;
    int decal_i = 0, decal_j;
    mdims(m, r, c);
  ...
  static gen evaldeuxpoints(const gen & args, const matrice * mptr, int cr, int cc, int & x,
                            int & y, int & X, int & Y, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args._VECTptr->size() == 2) {
      ...
      if (!mptr) {
        if (w[0].is_symb_of_sommet(at_cell) && w[1].is_symb_of_sommet(at_cell)) {
          if (w[0]._SYMBptr->feuille.type != _VECT ||
              w[0]._SYMBptr->feuille._VECTptr->size() != 2 ||
              w[1]._SYMBptr->feuille.type != _VECT || w[1]._SYMBptr->feuille._VECTptr->size() != 2)
...
#ifndef __APPLE__
        if (debug_infolevel > 2)
          CERR << "proot (j=" << j << "i=" << i << "), z'=" << newc << " f(z')=" << newfc
               << " f(z)=" << fc << " " << prefact << '\n';
#endif
  ...
  static void minmax(const vector<double> & lnval, const vector<int> & lndeg, double shift,
                     double & maxval, int & maxdeg, double & minval, int & mindeg) {
    maxdeg = 0;
    maxval = 0;
    mindeg = 0;
    minval = 0;
  ...
  static gen balance(vecteur & v, double & eps, GIAC_CONTEXT) {
    ...
    gen v0 = abs(v[0], contextptr), lnv0 = ln(v0, contextptr),
        lnv0d = evalf_double(accurate_evalf(ln(v0, contextptr), 60), 1, contextptr);
    ...
    for (int d = 1; d <= deg; ++d) {
      ...
      if (!is_zero(vd)) {
        ratio = evalf_double(accurate_evalf(ln(abs(vd, contextptr), contextptr), 60) - lnv0, 1,
                             contextptr);
  ...
  bool diagonal_mult(const vector<giac_double> & d, bool invert, const vector<giac_double> & source,
                     vector<giac_double> & target) {
    int n = int(d.size());
    if (source.size() != n)
      return false;
    target.resize(n);
  ...
  static bool schur_eigenvalues(matrix_double & H1, vecteur & res, double eps, GIAC_CONTEXT) {
    ...
    if (debug_infolevel > 2) {
      ...
      for (int i = 2; i < dim; ++i) {
        *logptr(contextptr) << i << ": " << string(i - 2, '*') << H1[i][i - 2] << ","
                            << H1[i][i - 1] << "=" << H1[i][i] << "=";
        if (i < dim - 1)
    ...
    for (int i = 0; i < dim; ++i) {
      if (i < dim - 1 && std::sqrt(eps) > absdouble(H1[i + 1][i])) {
        if (dim * eps < absdouble(H1[i + 1][i]) &&
            (i == 0 || dim * eps < absdouble(H1[i][i - 1]))) {
      ...
      double test = 0;
      if (i < dim - 2)
        test = absdouble(H1[i + 2][i + 1]) / (absdouble(H1[i + 1][i + 1]) + absdouble(H1[i][i]) +
                                              absdouble(H1[i + 1][i]) + absdouble(H1[i][i + 1]));
  ...
  static bool proot_real1(const vecteur & v, double eps, int rprec, vecteur & res, GIAC_CONTEXT) {
    ...
    if (!francis_schur(H, 0, dim, P, 2 * SOLVER_MAX_ITERATE, dim * eps, false, complex_schur, false,
                       false, contextptr))
  ...
  static bool in_proot(const vecteur & w, double & eps, int & rprec, vecteur & res,
                       bool isolaterealroot, GIAC_CONTEXT) {
#ifdef EMCC
    if (eps < 1e-300)
      eps = 1e-11;
  ...
  static vecteur proot(const vecteur & v, double & eps, int & rprec, bool ck_exact) {
    ...
    if (vsize < 2)
      return vecteur(0);
    if (vsize == 2)
      return vecteur(1, rprec <= 50 ? evalf(-v[1] / v[0], 1, context0)
                                    : accurate_evalf(-v[1] / v[0], rprec)); // ok
    ...
    if (0 && v.size() >= 64 &&
        pari_polroots(accurate_evalf(v, rprec), crystalball, giacmax(rprec, 53), contextptr) &&
        !is_undef(crystalball)) {
        ...
        for (unsigned k = 0; int(k) < SOLVER_MAX_ITERATE; ++k) {
          ...
          if (is_greater(2.5 * abs(decal, contextptr), mindist, contextptr)) {
            ...
            if (dd <= distances.size() / 3) {
              ...
              for (unsigned i = 0; i < dists.size(); ++i) {
                ...
                if (dists[i] <= distances[dd]) {
                  ...
                  if (i + 1 < dists.size() && add_conjugate &&
                      !is_exactly_zero(im(crystalball[i], contextptr))) {
              ...
              if (debug_infolevel)
                CERR << CLOCK() * 1e-6 << "Entering generalized Bairstow " << dd << " roots "
                     << positions << '\n';
            ...
            if (is_greater(mindist2, 3 * abs(ratio, contextptr), contextptr)) {
              ...
              if (crystalball[j] == conj(crystalball[j + 1], contextptr))
                k2 = j + 1;
              ...
              for (; k < SOLVER_MAX_ITERATE; ++k) {
                ...
                current[1] += dc1;
                current[2] += dc2;
                if (is_greater(eps * deg * 10,
                               abs(dc1 / current[1], contextptr) +
                                   abs(dc2 / current[2], contextptr),
                               contextptr)) {
            ...
            if (is_greater(3 * abs(decal, contextptr), mindist, contextptr)) {
              ...
              CERR << "Bad conditionned root j= " << j << " value " << crystalball[j] << " ratio "
                   << evalf_double(abs(ratio, contextptr), 1, contextptr) << " mindist " << mindist
                   << '\n';
              break;
            }
          }
  ...
  //  eps=1e-5;
    ...
    for (int i = 0;; ++i, eps *= 1.1) {
      ...
      for (; j < SOLVER_MAX_ITERATE * vsize2; j++) {
        if (!(j % vsize2)) {
          ...
          // random restart
            ...
            r = accurate_evalf(
                j / vsize *
                    complex_double(std_rand() * 1.0 / RAND_MAX, std_rand() * 1.0 / RAND_MAX),
                nbits);
          oldval = horner(cur_v, r);
          prefact = accurate_evalf(plus_one, nbits);
        }
        ...
        newr = r - prefact * dr;
        if (is_zero(dr) ||
            is_positive(-rprec - ln(abs(dr, contextptr) / abs(r, contextptr), contextptr) /
                                     std::log(2.0),
                        contextptr)) {
          r = newr;
          break;
      ...
      if (j == vsize)
        return vecteur(
            1, gensizeerr(gettext("Proot error : no root found for ") + gen(v).print(contextptr)));
      if (debug_infolevel)
        CERR << "Root found " << evalf_double(r, 1, contextptr) << '\n';
      if (add_conjugate && is_greater(abs(im(r, contextptr), contextptr), eps, contextptr)) { // ok
        res.push_back(rprec < 53 ? evalf_double(conj(r, contextptr), 1, contextptr)
                                 : conj(accurate_evalf(r, rprec), contextptr)); // ok
        if (!crystalball.empty()) {
    ...
    for (; it != itend; ++it) {
      ...
      if (s < 0)
        ...
        if (s != cur_s)
          return false;
        if (s &&
            (it->_VECTptr->front().type == _VECT &&
             it->_VECTptr->front().subtype != _POLY1__VECT) &&
            !allow_embedded_vect)
          return false;
        if (ckundef) {
          for (int i = 0; i < s; ++i)
  ...
  bool is_fully_numeric(const gen & a, int withfracint) {
    ...
    case _REAL:
      return true;
    case _CPLX:
      return is_fully_numeric(*a._CPLXptr, withfracint) &&
             is_fully_numeric(*(a._CPLXptr + 1), withfracint);
    case _VECT:
    ...
    case _FRAC:
      return (withfracint & num_mask_withfrac) && is_fully_numeric(a._FRACptr->num, withfracint) &&
             is_fully_numeric(a._FRACptr->den, withfracint);
    default:
      return false;
    }
  }
...
#define GIAC_DETBLOCK // current bloc implementation for det is slower
  ...
  void tran_int(const vector<vector<int>> & a, vector<vector<int>> & res, int r1 = 0, int r2 = 0,
                int c1 = 0, int c2 = 0) {
    vector<vector<int>>::const_iterator it = a.begin() + r1,
                                        itend = r2 > r1 ? it + (r2 - r1) : a.end();
    ...
    vector<int>::const_iterator * itr =
        (vector<int>::const_iterator *)alloca(ncolres * sizeof(vector<int>::const_iterator));
#else
  ...
  giac_double dotvecteur_double(const std::vector<giac_double> & a,
                                const std::vector<giac_double> & b) {
    return dotvecteur(a, b);
  }

  // H*w->v, assumes correct sizes (v already initialized)
  void multmatvecteur(const matrix_double & H, const std::vector<giac_double> & w,
  ...
  complex_double dotvecteur(const std::vector<complex_double> & a,
                            const std::vector<complex_double> & b) {
  ...
  static define_unary_function_eval(__blockmatrix_mult_size, &_blockmatrix_mult_size,
                                    _blockmatrix_mult_size_s);
  define_unary_function_ptr5(at_blockmatrix_mult_size, alias_at_blockmatrix_mult_size,
                             &__blockmatrix_mult_size, 0, true);

  // multiply a[a0..a1,i0+delta..i1+delta] with bloc btran[b0..b1,i0..i1]
  // and adds or subtracts to c[a0+c0..a1+c0,b0+c1..b1+c1]
  void mmult_double_block(const matrix_double & A, int a0, int a1, const matrix_double & Btran,
                          int b0, int b1, matrix_double & C, int c0, int c1, int i0, int i1,
                          int delta = 0, bool add = true) {
    for (int a = a0; a < a1; ++a) {
      ...
      for (; it <= itend;) {
        ...
        const giac_double * j0 = &(*it)[i0];
        ++it;
        const giac_double * j1 = &(*it)[i0];
        ++it;
        const giac_double * j2 = &(*it)[i0];
        ++it;
        const giac_double * j3 = &(*it)[i0];
        ++it;
        const giac_double * j4 = &(*it)[i0];
        ++it;
#if 1
  ...
  static void mmult_mod_block(const vector<vector<int>> & A, int a0, int a1,
                              const vector<vector<int>> & Btran, int b0, int b1,
                              vector<vector<int>> & C, int c0, int c1, int i0, int i1, int p,
                              int delta = 0, bool add = true) {
    for (int a = a0; a < a1; ++a) {
  ...
  // or -= if add=false
  void in_mmult_mod(const vector<vector<int>> & A, const vector<vector<int>> & Btran,
                    vector<vector<int>> & C, int c0, int c1, int p, int Ar0, int Ar1, int Ac0,
                    int Ac1, bool add, int Br0 = 0, int Br1 = 0, int Bc0 = 0) {
    ...
    for (int i = 0; i < n; i += mmult_int_blocksize) {
      ...
      for (int k = 0; k < resrows; k += mmult_int_blocksize) {
        ...
        for (int j = 0; j < rescols; j += mmult_int_blocksize) {
          ...
          if (jend > rescols)
            jend = rescols;
          mmult_mod_block(A, k + Ar0, kend + Ar0, Btran, Br0 + j, Br0 + jend, C, c0 - Ar0, c1 - Br0,
                          Bc0 + i, Bc0 + iend, p, Ac0 - Bc0, add);
        }
      }
    }
  }
    ...
    for (int k = kstart; k < resrows; k += mmult_double_blocksize) {
      ...
      for (int i = 0; i < n; i += mmult_double_blocksize) {
        ...
        for (int j = 0; j < rescols; j += mmult_double_blocksize) {
          ...
          if (jend > rescols)
            jend = rescols;
          mmult_double_block(a, k + Ar0, kend + Ar0, btran, Br0 + j, Br0 + jend, c, c0 - Ar0,
                             c1 - Br0, Bc0 + i, Bc0 + iend, Ac0 - Bc0, ptr->add);
        }
      }
    }
    return ptr;
  ...
  void in_mmult_double(const matrix_double & A, const matrix_double & Btran, matrix_double & C,
                       int c0, int c1, int Ar0, int Ar1, int Ac0, int Ac1, bool add, int Br0 = 0,
                       int Br1 = 0, int Bc0 = 0) {
    int resrows = Ar1 > Ar0 ? Ar1 - Ar0 : A.size(), rescols = Btran.size();
    if (Br1 > Br0)
      rescols = Br1 - Br0;
    ...
    if (nthreads > 1) {
      ...
      for (int j = 0; j < nthreads; ++j) {
        thread_mmult_double_t tmp = {&A,  &Btran, &C,  0,   0,  n,  rescols,
                                     Ar0, Br0,    Ac0, Bc0, c0, c1, add};
        multdparam[j] = tmp;
      }
      ...
      for (int j = 0; j < nthreads; ++j) {
        ...
        bool res = true;
        if (j < nthreads - 1)
          res = pthread_create(&tab[j], (pthread_attr_t *)NULL, do_thread_mmult_double,
                               (void *)&multdparam[j]);
        if (res)
      ...
      for (int k = 0; k < resrows; k += mmult_double_blocksize) {
        ...
        for (int j = 0; j < rescols; j += mmult_double_blocksize) {
          ...
          if (jend > rescols)
            jend = rescols;
          mmult_double_block(A, k + Ar0, kend + Ar0, Btran, Br0 + j, Br0 + jend, C, c0 - Ar0,
                             c1 - Br0, Bc0 + i, Bc0 + iend, Ac0 - Bc0, add);
        }
      }
    }
  }
  ...
  void mmult_mod(const vector<vector<int>> & A, const vector<vector<int>> & Btran,
                 vector<vector<int>> & C, int p, int Ar0, int Ar1, int Ac0, int Ac1, int Brbeg,
                 int Brend, int Bcbeg, int Crbeg, int Ccbeg, bool add) {
    int resrows, rescols;
    ...
    if (
        //0 &&
        resrows > strassen_limit && rescols > strassen_limit && Acols > strassen_limit &&
        Crbeg == 0 && Ccbeg == 0) {
      if (p != 0) {
      ...
      double nstep =
          std::ceil(std::log(giacmin(resrows, rescols) / double(strassen_limit)) / std::log(2.0));
  ...
  void add_mod(bool add, const vector<vector<int>> & A, const vector<vector<int>> & B,
               vector<vector<int>> & C, int p, int Ar0 = 0, int Ar1 = 0, int Ac0 = 0, int Ac1 = 0,
               int Br0 = 0, int Bc0 = 0, int Cr0 = 0, int Cc0 = 0) {
    if (Ar1 <= Ar0)
      Ar1 = A.size() + Ar0;
    if (!A.empty() && Ac1 <= Ac0)
      Ac1 = A.front().size() + Ac0;
    vector<vector<int>>::const_iterator at = A.begin() + Ar0, atend = A.begin() + Ar1,
                                        bt = B.begin() + Br0;
    ...
    for (; at != atend; ++ct, ++bt, ++at) {
      ...
      vector<int>::const_iterator it = ai.begin() + Ac0, itend = ai.begin() + Ac1,
                                  jt = bi.begin() + Bc0;
  ...
  void sub_mod(const vector<vector<int>> & A, const vector<vector<int>> & B,
               vector<vector<int>> & C, int p, int Ar0 = 0, int Ar1 = 0, int Ac0 = 0, int Ac1 = 0,
               int Br0 = 0, int Bc0 = 0, int Cr0 = 0, int Cc0 = 0) {
    if (Ar1 <= Ar0)
      Ar1 = A.size() + Ar0;
  ...
  define_unary_function_ptr5(at_strassen_limit, alias_at_strassen_limit, &__strassen_limit, 0,
                             true);
  ...
  void strassen_mod(bool skip_reduce, bool add, const vector<vector<int>> & A,
                    const vector<vector<int>> & Btran, vector<vector<int>> & C, int p, int arbeg,
                    int arend, int acbeg, int acend, int brbeg, int brend, int bcbeg) {
    ...
    if (a <= strassen_limit || ac <= strassen_limit || b <= strassen_limit) {
      if (p && skip_reduce) {
    ...
    if (ac % 2 || a % 2 || b % 2) { // add missing 0 to get even dimensions
      ...
      for (int i = 0; i < b; ++i) {
        Btran_[i] =
            vector<int>(Btran[brbeg + i].begin() + bcbeg, Btran[brbeg + i].begin() + bcbeg + ac);
        if (ac % 2)
    ...
    t1.reserve(giacmax(a, b));
    t1.resize(b);
    alloc(t1, ac, acb);
    // vector< vector<int> > t1(b,vector<int>(ac));
    sub_mod(Btran, Btran, t1, skip_reduce ? 0 : p, brbeg + b, brbeg + 2 * b, bcbeg, bcbeg + ac,
            brbeg, bcbeg);
    // t2=b22-t1=btran22-t1
    vector<vector<int>> t2(b, vector<int>(ac));
    sub_mod(Btran, t1, t2, skip_reduce ? 0 : p, brbeg + b, brbeg + 2 * b, bcbeg + ac,
            bcbeg + 2 * ac, 0, 0);
    // t3=b22-b12=btran22-btran21
    vector<vector<int>> t3(b, vector<int>(ac));
    sub_mod(Btran, Btran, t3, skip_reduce ? 0 : p, brbeg + b, brbeg + 2 * b, bcbeg + ac,
            bcbeg + 2 * ac, brbeg + b, bcbeg);
    ...
    // p4=s2*t2
    alloc(s1, b, b);
    vector<vector<int>> & p4 = s1;
    // vector< vector<int> > p4(a,vector<int>(b));
    strassen_mod(skip_reduce, false, s2, t2, p4, p);
    // p5=s3*t3
    alloc(s2, b, b);
    vector<vector<int>> & p5 = s2;
    // vector< vector<int> > p5(a,vector<int>(b));
    strassen_mod(skip_reduce, false, s3, t3, p5, p);
    // p6=s4*b22
    alloc(s3, b, b);
    vector<vector<int>> & p6 = s3;
    // vector< vector<int> > p6(a,vector<int>(b));
    strassen_mod(skip_reduce, false, s4, Btran, p6, p, 0, 0, 0, 0, brbeg + b, brbeg + 2 * b,
                 bcbeg + ac);
    // p7=a22*t4
    alloc(s4, b, b);
    vector<vector<int>> & p7 = s4;
    // vector< vector<int> > p7(a,vector<int>(b));
    strassen_mod(skip_reduce, false, A, t4, p7, p, arbeg + a, arbeg + 2 * a, acbeg + ac,
                 acbeg + 2 * ac, 0, b, 0);
    // p2=a12*b21=a12*btran12
    t1.resize(a);
    alloc(t1, b, b);
    vector<vector<int>> & p2 = t1;
    // vector< vector<int> > p2(a,vector<int>(b));
    strassen_mod(skip_reduce, false, A, Btran, p2, p, arbeg, arbeg + a, acbeg + ac, acbeg + 2 * ac,
                 brbeg, brbeg + b, bcbeg + ac);
    // p1=a11*b11
    t2.resize(a);
    alloc(t2, b, b);
    vector<vector<int>> & p1 = t2;
    // vector< vector<int> > p1(a,vector<int>(b));
    strassen_mod(skip_reduce, false, A, Btran, p1, p, arbeg, arbeg + a, acbeg, acbeg + ac, brbeg,
                 brbeg + b, bcbeg);
    t3.clear();
    t4.clear();
    if (debug_infolevel > 2)
  ...
  static bool ichinrem_inplace(matrice & a, const matrice & b, const gen & amod, int bmod,
                               int fullreduction) {
    gen U, v, d;
  ...
  bool mmult_int(const matrice & a, const matrice & btran, matrice & c) {
    ...
    if (resinf.type == _INT_) {
      vecteur2vectvector_int(a, 0, ai);
      vecteur2vectvector_int(btran, 0, btrani);
    ...
    if ((a2 < 128) || (b2 < 128) || (a2 / b2 < 1.4 && b2 / a2 < 1.4)) {
      ...
      for (int i = 0; i < resrows; ++i) {
        ...
        for (int j = 0; j < rescols; ++j) {
          gen tmp;
          tmp.uncoerce();
          const_iterateur it = a[i]._VECTptr->begin(), itend = a[i]._VECTptr->end(),
                          jt = btran[j]._VECTptr->begin();
          for (; it != itend; ++jt, ++it) {
            if (it->type == _INT_) {
              if (jt->type == _INT_) {
    ...
    if (nthreads > 1) {
...
#ifdef __clang__
      vector<vector<int>> *tabai =
                              (vector<vector<int>> *)alloca(nthreads * sizeof(vector<vector<int>>)),
                          *tabbtrani =
                              (vector<vector<int>> *)alloca(nthreads * sizeof(vector<vector<int>>)),
                          *tabci =
                              (vector<vector<int>> *)alloca(nthreads * sizeof(vector<vector<int>>));
      ...
      for (; i <= (nsteps / nthreads) * nthreads;) {
        for (j = 0; j < nthreads; ++j) {
          ...
          bool res = true;
          if (j < nthreads - 1)
            res = pthread_create(&tab[j], (pthread_attr_t *)NULL, thread_mmult_mod,
                                 (void *)&multmodparam[j]);
          if (res)
    ...
    for (; i <= nsteps; ++i) {
      p = nextprime(p + 1);
      vecteur2vectvector_int(a, p.val, ai);
      vecteur2vectvector_int(btran, p.val, btrani);
  ...
  bool mmult_double(const matrice & a, const matrice & btran, matrice & res) {
    ...
    if (matrice2std_matrix_double(a, ad, true) && matrice2std_matrix_double(btran, btrand, true)) {
...
#if 1 // ndef GIAC_HAS_STO_38
      int n;
      if (!ad.empty() && resrows >= 2 * mmult_double_blocksize &&
          rescols >= 2 * mmult_double_blocksize &&
          (n = ad.front().size()) >= mmult_double_blocksize) {
	   ...
	   */
          ...
          double beta = 0.0;
#ifdef POCKETCAS
          cblas_dgemm(CblasColMajor, CblasNoTrans, CblasTrans, M, N, K, alpha, A, /*LDA*/ M, B,
                      /*LDB*/ N, beta, C, /*LDC*/ M);
#else
          dgemm_((char *)"N", (char *)"T", &M, &N, &K, &alpha, A, /*LDA*/ &M, B, /*LDB*/ &N, &beta,
                 C, /*LDC*/ &M);
#endif
    ...
    if (!matrice2std_matrix_complex_double(a, zad, true) ||
        !matrice2std_matrix_complex_double(btran, zbtrand, true))
      return false;
  ...
  void linear_combination(const gen & c1, const vecteur & v1, const gen & c2, const vecteur & v2,
                          const gen & c, const gen & cinv, vecteur & v, double eps, int cstart) {
    if (!is_one(cinv)) {
      if (cinv.type == _FRAC)
        linear_combination(c1 * cinv._FRACptr->num, v1, c2 * cinv._FRACptr->num, v2,
                           cinv._FRACptr->den, 1, v, eps, cstart);
      else
    ...
    if (it1 == jt1) {
      if (is_one(c)) {
        ...
        for (; jt1 != it1end; ++jt1, ++it2) {
#if !defined USE_GMP_REPLACEMENTS && !defined BF2GMP_H
          if (t == _ZINT && jt1->type == _ZINT && c.type == _ZINT && it2->type == _ZINT &&
              jt1->ref_count() == 1) {
...
#endif
          if (t == _EXT && jt1->type == _EXT && it2->type == _EXT &&
              *(jt1->_EXTptr + 1) == *(c1._EXTptr + 1) &&
              *(it2->_EXTptr + 1) == *(c1._EXTptr + 1)) {
            gen tmp = change_subtype(*c1._EXTptr, _POLY1__VECT) * (*jt1->_EXTptr) +
                      change_subtype(*c2._EXTptr, _POLY1__VECT) * (*it2->_EXTptr);
  ...
  void modlinear_combination(vecteur & v1, const gen & c2, const vecteur & v2, const gen & modulo,
                             int cstart, int cend) {
  ...
  // v=c1*v1+c2*v2
  void double_linear_combination(double c1, const std::vector<giac_double> & v1, double c2,
                                 const std::vector<giac_double> & v2, std::vector<giac_double> & v,
                                 int cstart) {
  ...
  // v1=v1+c2*v2
  void double_linear_combination(std::vector<giac_double> & v1, double c2,
                                 const std::vector<giac_double> & v2, int cstart, int cend) {
    if (c2) {
  ...
  void double_multilinear_combination(std::vector<giac_double> & v1, giac_double c1,
                                      std::vector<giac_double> & v2, giac_double c2,
                                      std::vector<giac_double> & v3, giac_double c3,
                                      std::vector<giac_double> & v4, giac_double c4,
                                      const std::vector<giac_double> & w, int cstart, int cend) {
    if (!c1 && !c2 && !c3 && !c4)
      return;
    std::vector<giac_double>::iterator it1 = v1.begin() + cstart, it1end = v1.end(),
                                       it2 = v2.begin() + cstart, it3 = v3.begin() + cstart,
                                       it4 = v4.begin() + cstart;
    if (cend && cend >= cstart && cend < it1end - v1.begin())
      it1end = v1.begin() + cend;
  ...
  inline void pseudo_mod4(Vec4q & a, int b, const Vec4q & c, const Vec4q & p4, const Vec4q & invp4,
                          unsigned nbits) {
  ...
  // v1 += c1*w % p, v2 += c2*w %p, v3 += c3*w % p, v4 += c4*w % p;
  void int_multilinear_combination(std::vector<int> & v1, int c1, std::vector<int> & v2, int c2,
                                   std::vector<int> & v3, int c3, std::vector<int> & v4, int c4,
                                   const std::vector<int> & w, int p, int cstart, int cend) {
    c1 %= p;
    c2 %= p;
    c3 %= p;
    c4 %= p;
    int *it1 = &*(v1.begin() + cstart), *it1end = &*(v1.end()), *it2 = &*(v2.begin() + cstart),
        *it3 = &*(v3.begin() + cstart), *it4 = &*(v4.begin() + cstart), *it1_;
  ...
  void LL_multilinear_combination(std::vector<longlong> & v1, int c1, std::vector<longlong> & v2,
                                  int c2, std::vector<longlong> & v3, int c3,
                                  std::vector<longlong> & v4, int c4,
                                  const std::vector<longlong> & w, int p, int cstart, int cend) {
    c1 %= p;
    c2 %= p;
    c3 %= p;
    c4 %= p;
    longlong *it1 = &*(v1.begin() + cstart), *it1end = &*(v1.end()), *it2 = &*(v2.begin() + cstart),
             *it3 = &*(v3.begin() + cstart), *it4 = &*(v4.begin() + cstart), *it1_;
    if (cend && cend >= cstart && cend < it1end - &v1.front())
      it1end = &*(v1.begin() + cend);
    it1_ = it1 - 4;
  ...
  bool find_multi_linear_combination(vector<vector<int>> & N, int l0, int & l1, int & l2, int & l3,
                                     int pivotcol, int lexcluded, int lmax) {
    if (l0 >= lmax - 3)
      return false;
    l1 = l0 + 1;
    for (; l1 < lmax; ++l1) {
  ...
  bool find_multi_linear_combination(vector<vector<longlong>> & N, int l0, int & l1, int & l2,
                                     int & l3, int pivotcol, int lexcluded, int lmax) {
    if (l0 >= lmax - 3)
      return false;
    vector<longlong> * ptr;
    l1 = l0 + 1;
  ...
  void * do_thread_double_linv(void * ptr) {
    ...
    // n^3/6 operations
    for (; i <= end - 4; i += 4) {
      giac_double *col0 = &m[i][n], *col1 = &m[i + 1][n], *col2 = &m[i + 2][n],
                  *col3 = &m[i + 3][n];
      for (int k = 0; k < i + 4; ++k) {
        col0[k] = 0.0;
        col1[k] = 0.0;
      ...
      for (int k = i + 4; k < n; ++k) {
        ...
        giac_double *mkj = &m[k][i], *col0j = col0 + i, *col1j = col1 + i, *col2j = col2 + i,
                    *col3j = col3 + i, *col0end = col0 + k;
  ...
  define_unary_function_ptr5(at_invd_blocksize, alias_at_invd_blocksize, &__invd_blocksize, 0,
                             true);
  ...
  void * do_thread_double_lu2inv(void * ptr) {
    ...
    if (n <= 200 || invd_blocksize <= 1) {
      for (; i <= end - 6; i += 6) {
        giac_double *col0 = &m[i][n], *col1 = &m[i + 1][n], *col2 = &m[i + 2][n],
                    *col3 = &m[i + 3][n], *col4 = &m[i + 4][n], *col5 = &m[i + 5][n];
        for (int k = n - 1; k >= 0; --k) {
          giac_double res0 = col0[k], res1 = col1[k], res2 = col2[k], res3 = col3[k],
                      res4 = col4[k], res5 = col5[k];
          int lastnon0pos = n - 1;
          if (lastnon0posv)
            lastnon0pos = (*lastnon0posv)[k];
          giac_double *mkj = &m[k][lastnon0pos], *col0j = col0 + lastnon0pos, *colend = col0 + k,
                      *col1j = col1 + lastnon0pos, *col2j = col2 + lastnon0pos,
                      *col3j = col3 + lastnon0pos, *col4j = col4 + lastnon0pos,
                      *col5j = col5 + lastnon0pos;
        ...
        for (; kstart >= 0; kstart = kend - 1) {
          ...
          for (; i <= iend - 6; i += 6) {
            ...
            for (int k = kstart; k >= kend; --k) {
              giac_double res0 = col0[k], res1 = col1[k], res2 = col2[k], res3 = col3[k],
                          res4 = col4[k], res5 = col5[k];
              giac_double *mkj = &m[k][cstart], *col0j = col0 + cstart, *colend = col0 + cend,
                          *col1j = col1 + cstart, *col2j = col2 + cstart, *col3j = col3 + cstart,
                          *col4j = col4 + cstart, *col5j = col5 + cstart;
              for (; col0j >= colend; --mkj, --col5j, --col4j, --col3j, --col2j, --col1j, --col0j) {
                giac_double tmp = *mkj;
                if (!tmp)
                  continue;
  ...
  void double_lu2inv(matrix_double & m, const vector<int> & permu) {
    ...
    if (nthreads > 1 && n > 40) {
      ...
      for (int j = 0; j < nthreads; ++j) {
        ...
        bool res = true;
        if (j < nthreads - 1)
          res = pthread_create(&tab[j], (pthread_attr_t *)NULL, do_thread_double_linv,
                               (void *)&param[j]);
        if (res)
    ...
    if (nthreads > 1 && n > 40) {
      ...
      for (int j = 0; j < nthreads; ++j) {
        ...
        bool res = true;
        if (j < nthreads - 1)
          res = pthread_create(&tab[j], (pthread_attr_t *)NULL, do_thread_double_lu2inv,
                               (void *)&param[j]);
        if (res)
  ...
  void int_linsolve_l(const vector<vector<int>> & m, int l, int c, const vector<int> & y,
                      vector<int> & a, int p) {
  ...
  void int_linsolve_l4(const vector<vector<int>> & m, int l, int c, const vector<int> & y0,
                       const vector<int> & y1, const vector<int> & y2, const vector<int> & y3,
                       vector<int> & a0, vector<int> & a1, vector<int> & a2, vector<int> & a3,
                       int p) {
    int n = y0.size();
    a0.resize(n);
    a1.resize(n);
  ...
  // solve triangular system l*a=y where l is the lower part of a lu decomp in m[l..][c..]
  void double_linsolve_l(const matrix_double & m, int l, int c, const vector<double> & y,
                         vector<double> & a) {
    // l*a=y: a1=y1, a2=y2-m_21*a1, ..., ak=yk-sum_{j=1..k-1}(m_kj*aj)
    int n = y.size();
    a.resize(n);
  ...
  // solve triangular system l*a=y where l is the lower part of a lu decomp in m
  void double_linsolve_l4(const matrix_double & m, int l, int c, const vector<double> & y0,
                          const vector<double> & y1, const vector<double> & y2,
                          const vector<double> & y3, vector<double> & a0, vector<double> & a1,
                          vector<double> & a2, vector<double> & a3) {
    int n = y0.size();
    a0.resize(n);
    a1.resize(n);
    a2.resize(n);
  ...
  */
  ...
  void int_linsolve_u(const vector<vector<int>> & m, int l, int c, const vector<int> & y,
                      vector<longlong> & a, int p) {
  ...
  void int_linsolve_u4(const vector<vector<int>> & m, int l, int c, const vector<int> & y0,
                       const vector<int> & y1, const vector<int> & y2, const vector<int> & y3,
                       vector<longlong> & a0, vector<longlong> & a1, vector<longlong> & a2,
                       vector<longlong> & a3, int p) {
    // a_1=y_1/m_11, a_2=(y_2-m_12*a_1)/m_22, , ak=(yk-sum_{j<k} m_jk*aj)/m_kk
    int n = y0.size();
    // initialize a to y
  ...
  void double_linsolve_u(const matrix_double & m, int l, int c, const vector<double> & y,
                         vector<double> & a) {
    // a_1=y_1/m_11, a_2=(y_2-m_12*a_1)/m_22, , ak=(yk-sum_{j<k} m_jk*aj)/m_kk
    int n = y.size();
    // initialize a to y
  ...
  void double_linsolve_u4(const matrix_double & m, int l, int c, const vector<double> & y0,
                          const vector<double> & y1, const vector<double> & y2,
                          const vector<double> & y3, vector<double> & a0, vector<double> & a1,
                          vector<double> & a2, vector<double> & a3) {
    // a_1=y_1/m_11, a_2=(y_2-m_12*a_1)/m_22, , ak=(yk-sum_{j<k} m_jk*aj)/m_kk
  ...
  void int_lu2inv(vector<vector<int>> & m, int p, const vector<int> & permu) {
    ...
    for (; i <= n - 4; i += 4) {
      ...
      col0[i + 3] = (-m[i + 3][i] - longlong(col0[i + 1]) * m[i + 3][i + 1] -
                     longlong(col0[i + 2]) * m[i + 3][i + 2]) %
                    p;
      col1[i + 1] = 1;
      col1[i + 2] = -m[i + 2][i + 1];
      ...
      for (int k = i + 4; k < n; ++k) {
        ...
        int *mkj = &m[k][i], *col0j = col0 + i, *col1j = col1 + i, *col2j = col2 + i,
            *col3j = col3 + i, *col0end = col0 + k;
        for (; col0j < col0end; ++mkj, ++col3j, ++col2j, ++col1j, ++col0j) {
          longlong tmp = (*mkj);
          if (!tmp)
    ...
    for (; i <= n - 4; i += 4) {
      ...
      for (int k = n - 1; k >= 0; --k) {
        ...
        int *mkj = &m[k][n - 1], *col0j = col0 + n - 1, *colend = col0 + k, *col1j = col1 + n - 1,
            *col2j = col2 + n - 1, *col3j = col3 + n - 1;
  ...
  // v1=v1+c2*v2 smod modulo
  void modlinear_combination(vector<int> & v1, int c2, const vector<int> & v2, int modulo,
...
#if defined(PSEUDO_MOD) &&                                                                         \
    !(defined(VISUALC) || defined(BESTA_OS) || defined(OSX) || defined(OSXIOS) ||                  \
      defined(FIR_LINUX) || defined(FIR_ANDROID) || defined(ANDROID))
      c2 %= modulo;
  ...
  // v1=v1+c2*v2
  void modlinear_combination(vector<longlong> & v1, int c2, const vector<longlong> & v2, int modulo,
                             int cstart, int cend) {
    if (c2) {
  ...
  static void print_debug_info(const gen & pivot) {
    if ((pivot.type == _POLY) && !pivot._POLYptr->coord.empty())
      CERR << "poly(" << sum_degree(pivot._POLYptr->coord.front().index) << ","
           << pivot._POLYptr->coord.size() << ") ";
    else
      CERR << pivot << " ";
  }
  ...
  bool is_integer_vecteur(const vecteur & m, bool intonly) {
    ...
    for (; it != itend; ++it) {
      ...
      if (it->type == _ZINT)
        continue;
      if (it->type == _CPLX && is_integer(*it->_CPLXptr) && is_exactly_zero(*(it->_CPLXptr + 1)))
        continue;
      return false;
  ...
  bool is_integer_matrice(const matrice & m, bool intonly) {
    const_iterateur it = m.begin(), itend = m.end();
    for (; it != itend; ++it)
      if (it->type != _VECT || !is_integer_vecteur(*it->_VECTptr, intonly))
        return false;
    return true;
  }
  ...
  bool is_fraction_vecteur(const vecteur & m) {
    const_iterateur it = m.begin(), itend = m.end();
    for (; it != itend; ++it)
      if (it->type != _FRAC && !is_integer(*it))
        return false;
    return true;
  }
  ...
  bool is_fraction_matrice(const matrice & m) {
    const_iterateur it = m.begin(), itend = m.end();
    for (; it != itend; ++it)
      if (it->type != _VECT || !is_fraction_vecteur(*it->_VECTptr))
        return false;
    return true;
  }
  ...
  void * thread_modrref(void * ptr_) {
    ...
    ptr->success =
        in_modrref(*ptr->aptr, *ptr->Nptr, *ptr->resptr, *ptr->pivotsptr, ptr->det, ptr->l,
                   ptr->lmax, ptr->c, ptr->cmax, ptr->fullreduction, ptr->dont_swap_below,
                   ptr->Modulo, ptr->carac, ptr->rref_or_det_or_lu, ptr->mult_by_det_mod_p,
                   ptr->inverting, ptr->no_initial_mod, ptr->workptr);
    return ptr;
  }
...
#define CLOCKS_PER_SEC 1e6
...
#ifndef GIAC_HAS_STO_38
  static int mrref_int(const matrice & a, matrice & res, vecteur & pivots, gen & det, int l,
                       int lmax, int c, int cmax, int fullreduction, int dont_swap_below,
                       bool convert_internal, int algorithm, int rref_or_det_or_lu, int modular,
                       vector<int> & permutation, GIAC_CONTEXT) {
...
#ifdef __clang__
    vector<vector<int>> * Nptr =
        (vector<vector<int>> *)alloca((nthreads - 1) * sizeof(vector<vector<int>>));
    matrice * resptr = (matrice *)alloca((nthreads - 1) * sizeof(matrice));
    vecteur * pivotsptr = (vecteur *)alloca((nthreads - 1) * sizeof(vecteur));
    smallmodrref_temp_t * work =
        (smallmodrref_temp_t *)alloca(nthreads * sizeof(smallmodrref_temp_t));
#else
...
#ifdef __clang__
    thread_modrref_t * modrrefparam =
        (thread_modrref_t *)alloca((nthreads - 1) * sizeof(thread_modrref_t));
#else
    ...
    for (int i = 0; i < nthreads - 1; ++i) {
      ...
      thread_modrref_t tmp = {
          &a,        &Nptr[i], &resptr[i], &pivotsptr[i], &work[i],        0, 1,  l,
          lmax,      c,        cmax,       fullreduction, dont_swap_below, 0, -1, rref_or_det_or_lu,
          inverting, false,    false};
      modrrefparam[i] = tmp;
    }
#endif
    if (!failure) {
      ...
      if (!done) {
        pi_p = p;
        if (!in_modrref(a, N, res, pivots, det, l, lmax, c, cmax, 0 /* fullreduction */,
                        dont_swap_below, p.val, -1 /* carac*/, 1 /* det */, 1 /* mult by 1*/,
                        false /* inverting */, true /* no initial mod */,
#ifdef HAVE_LIBPTHREAD
                        &work[nthreads - 1]
#else
                        0
      ...
      for (; is_strictly_greater(h2, pi_p * pi_p, contextptr);) {
        ...
        for (int j = 0; j < nthreads - 1; j++) {
          ...
          bool res = pthread_create(&tab[j], (pthread_attr_t *)NULL, thread_modrref,
                                    (void *)&modrrefparam[j]);
          if (res)
...
#endif
        p = nextp(p + 1, factdet);
        gen current_estimate = evalf_double(
            _evalf(
                gen(makevecteur(200 * ln(pi_p, contextptr) / ln(h2, contextptr), 20), _SEQ__VECT),
                contextptr),
            1, contextptr);
        if (as > 10 && dbglevel < 2 && CLOCK() - initial_clock > min_proba_time * CLOCKS_PER_SEC)
          dbglevel = 2;
        if (as > 10 && dbglevel > 1) {
          CERR << CLOCK() * 1e-6 << " detrref, % done " << current_estimate << ", prime " << p
               << (proba < 1e-10 ? " stable" : " unstable");
          if (dbglevel > 3)
            CERR << ", det/lif=" << det;
          CERR << '\n';
        }
        if (!in_modrref(a, N, res, pivots, det_mod_p, l, lmax, c, cmax, 0 /* fullreduction */,
                        dont_swap_below, p.val, -1 /*carac*/, 1 /* det */, 1 /* mult by 1*/,
                        false /* inverting */, true /* no initial mod */,
#ifdef HAVE_LIBPTHREAD
                        &work[nthreads - 1]
        ...
        if (old_det == det)
          ...
          proba = 1.0;
        if (proba < proba_epsilon(contextptr) && is_greater(70, current_estimate, contextptr) &&
            CLOCK() - initial_clock > min_proba_time * CLOCKS_PER_SEC)
          break;
      } // end loop h2>pi_p^2
      ...
      if (rref_or_det_or_lu == 1) {
        if (is_strictly_greater(h2, pi_p * pi_p, contextptr))
          *logptr(contextptr)
              << gettext("Probabilistic algorithm for determinant\n(run proba_epsilon:=0 for a "
                         "deterministic answer, this is slower).\nError probability is less than ")
              << proba << '\n';
        return 1;
      }
      h2 = h20;
    ...
    if (!failure) {
      ...
      pi_p = p;
      gen det1;
      if (!in_modrref(a, N, res, pivots, det1, l, lmax, c, cmax, fullreduction, dont_swap_below,
                      p.val, -1 /*carac*/, rref_or_det_or_lu,
                      (inverting || rref_or_det_or_lu == 0) ? det : 1, true /* inverting */,
                      true /* no initial mod */, 0 /*workptr*/))
        return 0;
#if 1
      ...
      for (; is_strictly_greater(h2, pi_p * pi_p, contextptr);) {
        ...
        if (p.type != _INT_)
          break;
        if (!in_modrref(a, N, res_mod_p, pivots_mod_p, det_mod_p, l, lmax, c, cmax, fullreduction,
                        dont_swap_below, p.val, -1 /*carac*/, rref_or_det_or_lu,
                        (inverting || rref_or_det_or_lu == 0) ? det : 1, true /* inverting */,
                        true /* no initial mod */, 0 /*workptr*/))
          return 0;
#ifdef HAVE_LIBPTHREAD
        ...
        if (as > 10 && debug_infolevel > 1)
          CERR << CLOCK() * 1e-6 << " modrref, % done "
               << evalf_double(
                      _evalf(gen(makevecteur(200 * ln(pi_p, contextptr) / ln(h2, contextptr), 20),
                                 _SEQ__VECT),
                             contextptr),
                      1, contextptr)
               << ", prime " << p << '\n';
	...
	*/
	...
	res=*ichinrem(gen(res),gen(res_mod_p),pi_p,p)._VECTptr;
        ...
        if (inverting) {
          ...
          if (is_greater(pi_p, 2 * abs(det, contextptr), contextptr) &&
              is_greater(pi_p, 2 * linfnorm(res, contextptr) * linfa, contextptr)) {
            if (debug_infolevel > 2)
  ...
  int rref_reduce(std_matrix<gen> & M, vecteur & pivots, vector<int> & permutation, gen & det,
                  gen & detnum, int algorithm, int l, int lmax, int c, int cmax,
                  int dont_swap_below, int rref_or_det_or_lu, int fullreduction, double eps,
                  bool step_rref, const vecteur & lv, bool convert_internal, bool num_mat,
                  GIAC_CONTEXT) {
    int linit = l;
    ...
    for (; (l < lmax) && (c < cmax);) {
        ...
        if (rref_or_det_or_lu == 3) { // LU without line permutation
          ...
          for (int ltemp = l + 1; ltemp < lmax; ++ltemp) {
            ...
            if (!is_exactly_zero(temp) &&
                (is_exactly_zero(pivot) || betterpivot(temp, pivot, num_mat, contextptr))) {
              pivot = temp;
              pivotline = ltemp;
            }
          }
      ...
      if (!is_exactly_zero(pivot)
        ...
        if (step_rref) {
          ...
          gprintf(step_rrefpivot, gettext("%gen\nReduce column %gen with pivot %gen at row %gen"),
                  makevecteur(res, c + 1, pivot1, pivotline + 1), contextptr);
        }
        ...
        if (l != pivotline) {
          if (step_rref) {
            ...
            if (convert_internal)
              res = *r2sym(res, lv, contextptr)._VECTptr;
            gprintf(step_rrefexchange, gettext("Exchange row %gen and row %gen"),
                    makevecteur(l + 1, pivotline + 1), contextptr);
          }
        ...
        if (fullreduction) { // should be done after for efficiency
          for (int ltemp = linit; ltemp < lmax; ++ltemp) {
            ...
            if (step_rref && l != ltemp) {
              ...
              gprintf(step_rrefpivot0, gettext("L%gen <- (%gen)*L%gen-(%gen)*L%gen on %gen"),
                      makevecteur(ltemp + 1, coeff1, ltemp + 1, coeff2, l + 1, res), contextptr);
            }
            if (ltemp != l) {
              if (algorithm !=
                  RREF_GAUSS_JORDAN) // M[ltemp] = rdiv( pivot * M[ltemp] - M[ltemp][pivotcol]* M[l], bareiss);
                linear_combination(pivot, M[ltemp], -M[ltemp][pivotcol], M[l], bareiss, invbareiss,
                                   M[ltemp], eps, 0);
              else // M[ltemp]=M[ltemp]-rdiv(M[ltemp][pivotcol],pivot)*M[l];
                linear_combination(plus_one, M[ltemp], -rdiv(M[ltemp][pivotcol], pivot, contextptr),
                                   M[l], plus_one, plus_one, M[ltemp], eps, 0);
            }
          }
          ...
          for (int ltemp = l + 1; ltemp < lmax; ++ltemp) {
            ...
            if (algorithm != RREF_GAUSS_JORDAN)
              linear_combination(pivot, M[ltemp], -M[ltemp][pivotcol], M[l], bareiss, invbareiss,
                                 M[ltemp], eps, (c + 1) * (rref_or_det_or_lu > 0));
            else {
              gen coeff = M[ltemp][pivotcol] / pivot;
              linear_combination(plus_one, M[ltemp], -coeff, M[l], plus_one, plus_one, M[ltemp],
                                 eps, (c + 1) * (rref_or_det_or_lu > 0));
              if (rref_or_det_or_lu == 2 || rref_or_det_or_lu == 3) {
                M[ltemp][pivotcol] = 0;
  ...
  int mrref(const matrice & a, matrice & res, vecteur & pivots, gen & det, int l, int lmax, int c,
            int cmax, int fullreduction_, int dont_swap_below, bool convert_internal,
            int algorithm_, int rref_or_det_or_lu, GIAC_CONTEXT) {
    if (!ckmatrix(a))
      return 0;
    double eps = epsilon(contextptr);
    ...
    if (fullreduction_ >= 0 && algorithm == RREF_GUESS && rref_or_det_or_lu == 0 && as > 10 &&
        as == a0s - 1 && int(as) == lmax && int(a0s) == cmax)
      modular = 2;
...
#ifndef GIAC_HAS_STO_38
    // modular algorithm
    if (((algorithm == RREF_GUESS && (fullreduction == 2 || rref_or_det_or_lu == 1)) || modular) &&
        is_integer_matrice(a) && as <= a0s && as >= 20) {
      int Res = mrref_int(a, res, pivots, det, l, lmax, c, cmax, fullreduction, dont_swap_below,
                          convert_internal, algorithm, rref_or_det_or_lu, modular, permutation,
                          contextptr);
      if (Res >= 0)
        return Res;
    }
#if 1 // modular algo not fast enough and p-adic already used
    if (as >= GIAC_PADIC && (rref_or_det_or_lu == 1 || fullreduction == 2) &&
        (algorithm == RREF_GUESS || modular) && is_fraction_matrice(a)) {
      res = a;
      gen detden = 1;
      vecteur lcms(as);
      ...
      matrice res_;
      int ok = mrref(res, res_, pivots, det, l, lmax, c, cmax, fullreduction, dont_swap_below,
                     convert_internal, algorithm, rref_or_det_or_lu, contextptr);
      det = det / detden;
      swap(res, res_);
      if (ok == 2) {
    ...
    if (lidnt(a).empty() && tmp.type == _VECT && !tmp._VECTptr->empty()) {
      ...
      if (tmp.type == _MOD) {
        ...
        if (!modrref(unmoda, res, pivots, det, l, lmax, c, cmax, fullreduction, dont_swap_below,
                     modulo, true /*ckprime*/, rref_or_det_or_lu)) {
          if (!mrref(unmoda, res, pivots, det, l, lmax, c, cmax, fullreduction, dont_swap_below,
                     convert_internal, algorithm, rref_or_det_or_lu, contextptr))
            return 0;
        }
    ...
    if (num_mat) {
      ...
      if (eps >= 1e-16 && num_mat && matrice2std_matrix_double(res, N, true)) {
        ...
        doublerref(N, pivots, permutation, maxrankcols, ddet, l, lmax, c, cmax, fullreduction,
                   dont_swap_below, rref_or_det_or_lu, eps);
        if (rref_or_det_or_lu != 1) {
          std_matrix<gen> RES;
        ...
        for (; i < res.size(); ++i) {
          if (res[i].type == _VECT && res[i].ref_count() == 1) {
            ...
            for (; j < resi.size(); ++j) {
              ...
              if (resij.type == _POLY && resij._POLYptr->dim == 0 &&
                  resij._POLYptr->coord.size() == 1) {
                gen tmp = resij._POLYptr->coord.front().value;
                if (tmp.type < _POLY)
                  continue;
                if (tmp.type == _EXT && tmp._EXTptr->type == _VECT &&
                    (tmp._EXTptr + 1)->type == _VECT &&
                    is_integer_vecteur(*tmp._EXTptr->_VECTptr) &&
                    is_integer_vecteur(*(tmp._EXTptr + 1)->_VECTptr))
                  continue;
              }
              break;
            }
    ...
    if (algorithm != RREF_GAUSS_JORDAN) {
      ...
      if (lvs == 1 && lv.front().type == _VECT && lv.front()._VECTptr->empty() &&
          (rref_or_det_or_lu == 1 || modular) && is_integer_matrice(res) && as <= a0s) {
        matrice res1;
        if (!mrref(res, res1, pivots, det, l, lmax, c, cmax, fullreduction, dont_swap_below, false,
                   algorithm, rref_or_det_or_lu, contextptr))
          return 0;
        res = res1;
      ...
      bool fullreductionafter =
          rref_or_det_or_lu == 0 && dont_swap_below == 0 && c == 0 && linit == 0 && cmax >= lmax &&
          step_infolevel(contextptr) == 0 && fullreduction &&
          algorithm !=
              RREF_GAUSS_JORDAN; // insure all pivots are normalized to be = to the determinant
      if (((rref_or_det_or_lu == 1 && as == a0s) || fullreductionafter) && as > 4 &&
          algorithm == RREF_GUESS &&
          ((convert_internal && lvs == 1 && lv.front().type == _VECT) ||
           res.front()._VECTptr->front().type == _POLY)) {
        ...
        int polydim = res.front()._VECTptr->front().type == _POLY
                          ? res.front()._VECTptr->front()._POLYptr->dim
                          : int(lv.front()._VECTptr->size());
      ...
      if (algorithm == RREF_LAGRANGE &&
          ((rref_or_det_or_lu == 1 && as == a0s) || fullreductionafter)) {
        vecteur lva = lvar(a);
        if ((!convert_internal && lva.empty()) || (lvs == 1 && lv.front() == lva)) {
          ...
          if (polydim) {
            ...
            for (; x <= totaldeg; ++x) {
              ...
              if (dbg)
                CERR << CLOCK() * 1e-6 << " det: begin horner dim " << polydim << " pos " << x
                     << " from" << totaldeg << '\n';
              for (unsigned int i = 0; i < as; ++i) {
                vecteur resxi;
              ...
              if (dbg)
                CERR << CLOCK() * 1e-6 << " det: end horner dim " << polydim << " pos " << x
                     << " from" << totaldeg << '\n';
              matrice res1;
              if (!mrref(resx, res1, pivots, det, l, lmax, c, cmax, -fullreduction, dont_swap_below,
                         false, algorithm_, rref_or_det_or_lu, contextptr))
                return 0;
              Y[x] = det;
              if (fullreduction) {
                if (is_zero(det))
    ...
    bool fullreductionafter = rref_or_det_or_lu == 0 && dont_swap_below == 0 &&
                              cmax - c >= lmax - linit && step_infolevel(contextptr) == 0 &&
                              fullreduction && algorithm != RREF_GAUSS_JORDAN;
    gen detnumsave = detnum;
    int status =
        rref_reduce(M, pivots, permutation, det, detnum, algorithm, l, lmax, c, cmax,
                    dont_swap_below, rref_or_det_or_lu, (fullreductionafter ? 0 : fullreduction),
                    eps, step_rref, lv, convert_internal, num_mat, contextptr);
    ...
    if (fullreductionafter) {
      ...
      if (status == 3 || is_exactly_zero(det)) {
        ...
        pivots.clear();
        matrice2std_matrix_gen(res, M);
        det = detnum = detnumsave; // this should be commented but some outputs are more complicated
        rref_reduce(M, pivots, permutation, det, detnum, algorithm, l, lmax, c, cmax,
                    dont_swap_below, rref_or_det_or_lu, fullreduction, eps, step_rref, lv,
                    convert_internal, num_mat, contextptr);
      } else {
        ...
        for (l = lmax - 2; l >= linit; --l) {
          ...
          for (int j = lmax - 1; j >= l + 1; --j) {
            linear_combination(plus_one, Ml, -Mlcopy[shift + j], M[j], plus_one, 1, Ml, eps,
                               shift + lmax);
          }
  ...
  void vect_vecteur_2_vect_vector_int(const std_matrix<gen> & M, int modulo,
                                      vector<vector<int>> & N) {
  ...
  void dotvector_int(const vector<int> & v0, const vector<int> & v1, const vector<int> & v2,
                     const vector<int> & v3, const vector<int> & w, longlong & res0,
                     longlong & res1, longlong & res2, longlong & res3) {
    vector<int>::const_iterator it = w.begin(), itend = w.end(), it1, jt0 = v0.begin(),
                                jt1 = v1.begin(), jt2 = v2.begin(), jt3 = v3.begin();
    unsigned n = unsigned(itend - it);
  ...
  bool multvectvector_int_vector_int(const vector<vector<int>> & M, const vector<int> & v,
                                     int modulo, vector<int> & Mv) {
...
#if 1
  void smallmodrref_lower(vector<vector<int>> & N, int lstart, int l, int lmax, int c, int cmax,
                          const vector<int> & pivots, int modulo, bool debuginfo) {
    ...
    for (int L = l; L < lmax; ++L) {
      if (debuginfo) {
        ...
        if (L % 500 == 499) {
          CERR << CLOCK() * 1e-6 << " remaining " << lmax - L << '\n';
        }
      }
...
void smallmodrref_lower(vector<vector<int>> & N, int lstart, int l, int lmax, int c, int cmax,
  ...
  effcmin = effcmax;
  int nslice = std::ceil(ps * double(cmax - effcmin) * sizeof(int) /
                         32768); // lower matrix size/L1 cache size
  if (nslice <= 0)
    nslice = 1;
  ...
  void smallmodrref_lower_pivots(vector<vector<int>> & N, int l, int lmax, int c, int cmax,
                                 vector<int> & pivots, int modulo) {
    pivots.clear();
    int L = l, C = c, k;
    for (; L < lmax && C < cmax;) {
  ...
  void * do_thread_smallmodrref_upper(void * ptr_) {
    ...
    in_thread_smallmodrref_upper(*ptr->N, ptr->l, ptr->lpivot, ptr->lmax, ptr->c, ptr->cmax,
                                 ptr->modulo, 1);
    return ptr;
  }
  ...
  void in_thread_smallmodrref_upper(vector<vector<int>> & N, int l, int lpivot, int lmax, int c,
                                    int cmax, int modulo, int parallel) {
#ifdef HAVE_LIBPTHREAD
    if (parallel != 1) {
      ...
      for (int j = 0; j < parallel; ++j) {
        bool res = true;
        if (j < parallel - 1)
          res = pthread_create(&tab[j], (pthread_attr_t *)NULL, do_thread_smallmodrref_upper,
                               (void *)&upperparam[j]);
        if (res)
  ...
  void thread_smallmodrref_upper(vector<vector<int>> & K, int l, int lmax, int c, int cmax,
                                 int modulo, int parallel) {
    ...
    if (cmax - pivotcol < 16 * parallel)
      ...
      if (debug_infolevel > 1)
        CERR << CLOCK() * 1e-6 << " rref_upper " << lmax << "*" << pivotcol + 1 << ","
             << cmax - pivotcol - 1 << '\n';
      int l1 = (l + lmax) / 2;
      int pc = smallmodrref_lastpivotcol(K, l1);
      if (cmax - pc < 16 * parallel || lmax - l < 16)
        ...
        if (debug_infolevel > 1)
          CERR << CLOCK() * 1e-6 << " rref_upper_parallel " << l1 - l << "*" << pivotcol - pc << "/"
               << pivotcol - c << '\n';
  ...
  void do_modular_reduction(vector<vector<int>> & N, int l, int pivotcol, int pivotval, int linit,
                            int lmax, int c, int effcmax, int rref_or_det_or_lu, int modulo) {
#ifndef GIAC_HAS_STO_38
    int l1, l2, l3;
#endif
    ...
    for (int ltemp = linit; ltemp < lmax; ++ltemp) {
      ...
      if (!ludecomp && find_multi_linear_combination(N, ltemp, l1, l2, l3, pivotcol, l, lmax)) {
        int_multilinear_combination(N[ltemp], -N[ltemp][pivotcol], N[l1], -N[l1][pivotcol], N[l2],
                                    -N[l2][pivotcol], N[l3], -N[l3][pivotcol], N[l], modulo, c,
                                    effcmax);
        ltemp = l3;
        continue;
      }
      if (ludecomp && ltemp <= lmax - 4 && !N[ltemp + 1].empty() && N[ltemp + 1][pivotcol] &&
          !N[ltemp + 2].empty() && N[ltemp + 2][pivotcol] && !N[ltemp + 3].empty() &&
          N[ltemp + 3][pivotcol]) {
        ...
        int_multilinear_combination(N[ltemp], -N[ltemp][pivotcol], N[ltemp + 1],
                                    -N[ltemp + 1][pivotcol], N[ltemp + 2], -N[ltemp + 2][pivotcol],
                                    N[ltemp + 3], -N[ltemp + 3][pivotcol], N[l], modulo,
                                    (rref_or_det_or_lu > 0) ? (c + 1) : c, effcmax);
        ltemp += (4 - 1);
        continue;
      }
#endif
      if (ludecomp)
        N[ltemp][pivotcol] = (N[ltemp][pivotcol] * longlong(pivotval)) % modulo;
      modlinear_combination(N[ltemp], -N[ltemp][pivotcol], N[l], modulo,
                            (rref_or_det_or_lu > 0) ? (c + 1) : c, effcmax, true /* pseudomod */);
    }
  }

  void LL_modular_reduction(vector<vector<longlong>> & N, int l, int pivotcol, int pivotval,
                            int linit, int lmax, int c, int effcmax, int rref_or_det_or_lu,
                            int modulo) {
#ifndef GIAC_HAS_STO_38
    int l1, l2, l3;
#endif
    ...
    for (int ltemp = linit; ltemp < lmax; ++ltemp) {
      ...
      if (!ludecomp && find_multi_linear_combination(N, ltemp, l1, l2, l3, pivotcol, l, lmax)) {
        ...
        LL_multilinear_combination(N[ltemp], -coeff0, N[l1], -coeff1, N[l2], -coeff2, N[l3],
                                   -coeff3, N[l], modulo, c, effcmax);
        ltemp = l3;
        continue;
      }
#endif
      ...
      modlinear_combination(N[ltemp], -coeff, N[l], modulo, (rref_or_det_or_lu > 0) ? (c + 1) : c,
                            effcmax);
    }
  }
  ...
  void * do_thread_modular_reduction(void * ptr_) {
    ...
    do_modular_reduction(*ptr->Nptr, ptr->l, ptr->pivotcol, ptr->pivotval, ptr->linit, ptr->lmax,
                         ptr->c, ptr->effcmax, ptr->rref_or_det_or_lu, ptr->modulo);
    return ptr;
  }

  void * do_thread_lower_reduction(void * ptr_) {
    thread_modular_reduction_t * ptr = (thread_modular_reduction_t *)ptr_;
    smallmodrref_lower(*ptr->Nptr, ptr->linit, ptr->l, ptr->lmax, ptr->c, ptr->effcmax,
                       *ptr->pivotcols, ptr->modulo, ptr->debuginfo);
    return ptr;
  }
  ...
  bool LLsmallmodrref(vector<vector<int>> & Nint, int l_, int lmax_, int c_, int cmax_,
                      vecteur & pivots, vector<int> & permutation, vector<int> & maxrankcols,
                      longlong & idet, int fullreduction, int dont_swap_below, int modulo,
                      int carac, int rref_or_det_or_lu) {
    // return false;
    bool inverting = fullreduction == 2;
    ...
    return rref_or_det_or_lu != 0 || l_ == 0 ||
           fullreduction ==
               0; //true; // we can not return true for fullreduction, because the upper lines are not reduced
  }
  ...
  void smallmodrref(int nthreads, vector<vector<int>> & N, vecteur & pivots,
                    vector<int> & permutation, vector<int> & maxrankcols, longlong & idet, int l,
                    int lmax, int c, int cmax, int fullreduction, int dont_swap_below, int modulo,
                    int rref_or_det_or_lu, bool reset, smallmodrref_temp_t * workptr,
                    bool allow_block, int carac) {
    bool inverting = fullreduction == 2;
    int linit = l; //,previous_l=l;
    // Reduction
    int pivot, temp = 0;
    ...
    if (allow_block &&
      ...
      if (
        ...
        smallmodrref(nthreads, N, pivots, permutation, maxrankcols, idet, l, l + halfl, c, cmax,
                     0 /*fullreduction*/, 0, modulo, 0, false, workptr, true, carac);
        // use first half for second half
        vector<int> pivotcols;
        smallmodrref_lower_pivots(N, l, effl, c, cmax, pivotcols, modulo);
        bool reduction_done = false;
        if (debug_infolevel > 2)
          CERR << CLOCK() * 1e-6 << " rref_lower begin " << effl << ".." << lmax << "/" << c << ".."
               << cmax << '\n';
        // CERR << pivotcols << '\n';
#ifdef HAVE_LIBPTHREAD
        if (nthreads > 1 && double(lmax - effl) * (cmax - c) > 1e5) {
          ...
          if (nthreads > 64)
            nthreads = 64;
          for (int j = 0; j < nthreads; ++j) {
            thread_modular_reduction_t tmp = {
                &N,   &pivotcols, effl, pivotcol,          temp,   l,
                lmax, c,          cmax, rref_or_det_or_lu, modulo, j == 0 && debug_infolevel > 2};
            redparam[j] = tmp;
          }
          ...
          for (int j = 0; j < nthreads; ++j) {
            ...
            bool res = true;
            if (j < nthreads - 1)
              res = pthread_create(&tab[j], (pthread_attr_t *)NULL, do_thread_lower_reduction,
                                   (void *)&redparam[j]);
            if (res)
        ...
        if (debug_infolevel > 2)
          CERR << CLOCK() * 1e-6 << " rref_lower end " << effl << ".." << lmax << "/" << c << ".."
               << cmax << '\n';
        // reduce second half
        //cerr << N <<'\n';
        smallmodrref(nthreads, N, pivots, permutation, maxrankcols, idet, l + halfl, lmax,
                     c + (idet && rref_or_det_or_lu == 1 ? halfl : 0), cmax, 0 /*fullreduction*/, 0,
                     modulo, 0, false, workptr, true, carac);
        if (debug_infolevel > 2)
    ...
    bool blocktest = allow_block && rref_or_det_or_lu != 0 && lmax - l >= 2 * mmult_int_blocksize &&
    ...
    if (blocktest) {
      ...
      for (int i = l; i < lmax; ++i) {
        ...
        vector<int>::const_iterator it = N[l].begin() + c,
                                    itend = N[l].begin() + giacmin(cmax, N[l].size());
        for (; it != itend; ++it) {
          if (!*it)
            ++count;
        }
    ...
    if ( //0 &&
        rref_or_det_or_lu == 2 &&
        giacmax(lmax - l, cmax - c) * double(modulo) * modulo < (1ULL << 63) && blocktest) {
      ...
      smallmodrref(nthreads, N, pivots, permutation, maxrankcols, idet, l, l + taille, c,
                   c + taille, false, false, modulo, 2, false, 0, true, carac);
      if (!idet) {
        ...
        for (; i <= cmax - c - taille - 4; i += 4) {
          ...
          int_linsolve_l4(N, l, c, tmpptr->y, tmpptr->y1, tmpptr->y2, tmpptr->y3, tmpptr->Ainv[i],
                          tmpptr->Ainv[i + 1], tmpptr->Ainv[i + 2], tmpptr->Ainv[i + 3], modulo);
          // copy into N
          for (int j = 0; j < taille; j++) {
        ...
        for (i = 0; i <= lmax - l - taille - 4; i += 4) {
          ...
          int_linsolve_u4(N, l, c, tmpptr->y, tmpptr->y1, tmpptr->y2, tmpptr->y3, tmpptr->z,
                          tmpptr->z1, tmpptr->z2, tmpptr->z3, modulo);
          for (int j = 0; j < taille; j++) {
        ...
        in_mmult_mod(N, tmpptr->Ainv, N, l + taille, c + taille, modulo, l + taille, lmax, c,
                     c + taille, false);
        // final lu decomposition
        smallmodrref(nthreads, N, pivots, permutation, maxrankcols, idet, l + taille, lmax,
                     c + taille, cmax, false, false, modulo, 2, false, 0, true, carac);
        if (debug_infolevel > 2)
    ...
    bool tryblock =
        blocktest && rref_or_det_or_lu == 1 &&
        giacmax(mmult_int_blocksize, det_blocksize) * double(modulo) * modulo < ((1ULL << 63)) &&
        lmax - l >= 3 * det_blocksize && cmax - c >= 3 * det_blocksize;
    ...
    for (; (l < lmax) && (c < cmax);) {
#ifdef GIAC_DETBLOCK
      if (tryblock && lmax - l >= 3 * det_blocksize && cmax - c >= 3 * det_blocksize &&
          l % det_blocksize == 0 && c % det_blocksize == 0) {
        ...
        // lu
        tmpptr->permblock.clear();
        tmpptr->maxrankblock.clear();
        tmpptr->pivblock.clear();
        longlong idetblock;
        if (debug_infolevel > 2)
          CERR << CLOCK() * 1e-6 << "block reduction mod " << modulo << " size " << det_blocksize
               << " " << workptr << '\n';
        smallmodrref(nthreads, tmpptr->Ainv, tmpptr->pivblock, tmpptr->permblock,
                     tmpptr->maxrankblock, idetblock, 0, det_blocksize, 0, det_blocksize, 0, false,
                     modulo, 2, true, 0, true, carac);
        if (idetblock) {
          ...
          mmult_mod(N, tmpptr->Ainvtran, tmpptr->CAinv, modulo, l + det_blocksize, lmax, c,
                    c + det_blocksize);
          // transpose B
      ...
      if ( //0 &&
          ...
          (carac > 0 || (lmax - l >= 32 && cmax - c >= 32)) &&
          (lmax - l) * double(modulo) * double(modulo) < (1ULL << 63)) {
        //double(lmax-l)*(cmax-c)*sizeof(longlong)<128e3 &&
        if (LLsmallmodrref(N, l, lmax, c, cmax, pivots, permutation, maxrankcols, idet,
                           fullreduction, dont_swap_below, modulo, carac, rref_or_det_or_lu))
          break;
      }
      pivot = N[l].empty() ? 0 : (N[l][c] %= modulo);
      if (rref_or_det_or_lu == 3 && !pivot) {
      ...
      if (gcd(pivot, modulo) == 1) {
        ...
        if (0 && pivot == 1 && allow_block)
          temp =
              1; // can not be activated because pseudo-mod expect reducing line to be smaller than p
        else {
          temp = invmod(pivot, modulo);
          // multiply det
          idet = (idet * pivot) % modulo;
        ...
        if (nthreads > 1 && double(lmax - effl) * (effcmax - c) > 1e5) {
          ...
          for (int j = 0; j < nthreads; ++j) {
            thread_modular_reduction_t tmp = {
                &N,   0, l,       pivotcol,          temp,   linit,
                lmax, c, effcmax, rref_or_det_or_lu, modulo, j == 0 && debug_infolevel > 2};
            redparam[j] = tmp;
          }
          int kstep = int(std::ceil((lmax - effl) / double(nthreads))), k = effl;
          for (int j = 0; j < nthreads; ++j) {
            ...
            bool res = true;
            if (j < nthreads - 1)
              res = pthread_create(&tab[j], (pthread_attr_t *)NULL, do_thread_modular_reduction,
                                   (void *)&redparam[j]);
            if (res)
...
#endif
        if (do_reduction)
          do_modular_reduction(N, l, pivotcol, temp, effl, lmax, c, effcmax, rref_or_det_or_lu,
                               modulo);
        // increment column number if swap was allowed
        if (l >= dont_swap_below)
          ++c;
  ...
  // 3 for lu without permutation
  // fullreduction=0 or 1, use 2 if the right part of a is idn
  void in_doublerref(matrix_double & N, vecteur & pivots, vector<int> & permutation,
                     vector<int> & maxrankcols, double & idet, int l, int lmax, int c, int cmax,
...
#ifndef GIAC_HAS_STO_38
    bool blocktest =
        lmax - l >= 2 * mmult_double_blocksize && cmax - c >= 2 * mmult_double_blocksize;
    if (blocktest) {
      // count 0 in N[l->lmax][c->cmax]
      // if matrix is sparse, then block operations is not faster
      double count = 0;
    ...
    if ( //0 &&
      ...
      in_doublerref(N, pivots, permutation, maxrankcols, idet, l, lmax, c, c + taille, false, false,
                    eps, 2, false, 0);
      // find U3: L1*U3=P1*B, cmax-c-taille systems to solve, each has taille unknowns
      int i = 0;
      for (; i <= cmax - c - taille - 4; i += 4) {
        ...
        double_linsolve_l4(N, l, c, tmpptr->y, tmpptr->y1, tmpptr->y2, tmpptr->y3, tmpptr->Ainv[i],
                           tmpptr->Ainv[i + 1], tmpptr->Ainv[i + 2], tmpptr->Ainv[i + 3]);
        // copy into N
        for (int j = 0; j < taille; j++) {
          N[l + j][i + c + taille] = tmpptr->Ainv[i][j];
          N[l + j][i + 1 + c + taille] = tmpptr->Ainv[i + 1][j];
      ...
      // subtract L3*U3 from D
      in_mmult_double(N, tmpptr->Ainv, N, l + taille, c + taille, l + taille, lmax, c, c + taille,
                      false);
      // final lu decomposition
      in_doublerref(N, pivots, permutation, maxrankcols, idet, l + taille, lmax, c + taille, cmax,
                    false, false, eps, 2, false, 0);
      if (debug_infolevel > 2)
        ...
        if (fullreduction) {
          for (int ltemp = linit; ltemp < lmax; ++ltemp) {
            ...
            if (((ltemp <= l - 4) || (ltemp > l && ltemp <= lmax - 4))) {
              double_multilinear_combination(
                  N[ltemp], -N[ltemp][pivotcol], N[ltemp + 1], -N[ltemp + 1][pivotcol],
                  N[ltemp + 2], -N[ltemp + 2][pivotcol], N[ltemp + 3], -N[ltemp + 3][pivotcol],
                  N[l], (use_cstart ? c : cmax), effcmax);
              ltemp += (4 - 1);
            } else
#endif
              double_linear_combination(N[ltemp], -N[ltemp][pivotcol], N[l],
                                        (use_cstart ? c : cmax), effcmax);
          }
        } else {
          for (int ltemp = l + 1; ltemp < lmax; ++ltemp) {
            ...
            if (ltemp < lmax - 4) {
              ...
              double_multilinear_combination(
                  N[ltemp], -N[ltemp][pivotcol], N[ltemp + 1], -N[ltemp + 1][pivotcol],
                  N[ltemp + 2], -N[ltemp + 2][pivotcol], N[ltemp + 3], -N[ltemp + 3][pivotcol],
                  N[l], (rref_or_det_or_lu > 0) ? (c + 1) : (use_cstart ? c : cmax), effcmax);
              ltemp += (4 - 1);
            } else
#endif
              double_linear_combination(N[ltemp], -N[ltemp][pivotcol], N[l],
                                        (rref_or_det_or_lu > 0) ? (c + 1) : (use_cstart ? c : cmax),
                                        effcmax);
          }
        } // end else
        // increment column number if swap was allowed
  ...
  void doublerref(matrix_double & N, vecteur & pivots, vector<int> & permutation,
                  vector<int> & maxrankcols, double & idet, int l, int lmax, int c, int cmax,
                  int fullreduction, int dont_swap_below, int rref_or_det_or_lu, double eps) {
    in_doublerref(N, pivots, permutation, maxrankcols, idet, l, lmax, c, cmax, fullreduction,
                  dont_swap_below, eps, rref_or_det_or_lu, true, 0);
  }

  bool in_modrref(const matrice & a, vector<vector<int>> & N, matrice & res, vecteur & pivots,
                  gen & det, int l, int lmax, int c, int cmax, int fullreduction,
                  int dont_swap_below, int Modulo, int carac, int rref_or_det_or_lu,
                  const gen & mult_by_det_mod_p, bool inverting, bool no_initial_mod,
                  smallmodrref_temp_t * workptr) {
#ifndef GIAC_HAS_STO_38
    if (no_initial_mod) {
      const_iterateur it = a.begin(), itend = a.end();
    ...
    smallmodrref(1, N, pivots, permutation, maxrankcol, idet, l, lmax, c, cmax, fullreduction,
                 dont_swap_below, Modulo, rref_or_det_or_lu, true, workptr, true, carac);
#ifndef GIAC_HAS_STO_38
    if (inverting) {
  ...
  // 3 for lu without permutation
  // fullreduction=0 or 1, use 2 if the right part of a is idn
  bool modrref(const matrice & a, matrice & res, vecteur & pivots, gen & det, int l, int lmax,
               int c, int cmax, int fullreduction, int dont_swap_below, const gen & modulo,
               bool ckprime, int rref_or_det_or_lu) {
    if (ckprime && !is_probab_prime_p(modulo)) {
      if (rref_or_det_or_lu == 1) {
        ...
        if (currentp.type == _INT_) {
          res.clear();
          N.clear();
          if (!in_modrref(a, N, res, pivots, det, l, lmax, c, cmax, fullreduction, dont_swap_below,
                          currentp.val, v[0].val /* carac */, rref_or_det_or_lu, 1,
                          false /* inverting */, false /* no initial mod */, 0 /*workptr*/))
            return false;
        } else {
          if (v[1] != 1 || !modrref(a, res, pivots, det, l, lmax, c, cmax, fullreduction,
                                    dont_swap_below, v[0], false, rref_or_det_or_lu))
            return false;
        }
        gen pip = currentp;
        for (int i = 2; i < v.size(); i += 2) {
          ...
          if (currentp.type == _INT_) {
            res.clear();
            N.clear();
            if (!in_modrref(a, N, res, pivots, curdet, l, lmax, c, cmax, fullreduction,
                            dont_swap_below, currentp.val, v[i].val /* carac */, rref_or_det_or_lu,
                            1, false /* inverting */, false /* no initial mod */, 0 /*workptr*/))
              return false;
          } else {
            if (v[i + 1] != 1)
              return false;
            if (!modrref(a, res, pivots, curdet, l, lmax, c, cmax, fullreduction, dont_swap_below,
                         v[i], false, rref_or_det_or_lu))
              return false;
          }
	...
	modulo.val<46340 &&
      ...
      res.clear();
      vector<vector<int>> N;
      return in_modrref(a, N, res, pivots, det, l, lmax, c, cmax, fullreduction, dont_swap_below,
                        modulo.val, -1 /*carac*/, rref_or_det_or_lu, 1, false /* inverting */,
                        false /* no initial mod */, 0 /*workptr*/);
    }
    // bool use_cstart=!c;
    // bool inverting=fullreduction==2;
    det = 1;
      ...
      if (!is_exactly_zero(pivot)) {
        ...
        if (fullreduction) {
          ...
          for (int ltemp = l + 1; ltemp < lmax; ++ltemp) {
            ...
            modlinear_combination(M[ltemp], -M[ltemp][pivotcol], M[l], modulo,
                                  (c + 1) * (rref_or_det_or_lu > 0));
          }
        } // end else
        // increment column number if swap was allowed
        if (l >= dont_swap_below)
  ...
  bool mrref(const matrice & a, matrice & res, vecteur & pivots, gen & det, GIAC_CONTEXT) {
    return mrref(a, res, pivots, det, 0, int(a.size()), 0, int(a.front()._VECTptr->size()),
                 /* fullreduction */ 1, 0, true, 1, 0, contextptr) != 0;
  }
  ...
  bool modrref(const matrice & a, matrice & res, vecteur & pivots, gen & det, const gen & modulo) {
    return modrref(a, res, pivots, det, 0, int(a.size()), 0, int(a.front()._VECTptr->size()),
                   true /* full reduction */, 0 /* dont_swap_below*/, modulo, true /*ckprime*/,
                   0 /* rref */);
  }
  ...
  bool modinv(const matrice & a, matrice & res, const gen & modulo, gen & det_mod_p) {
    ...
    int s = int(a.size());
    vecteur pivots;
    if (!modrref(arref, res, pivots, det_mod_p, 0, s, 0, 2 * s, 2 /* full reduction*/,
                 0 /*dont_swap_below*/, modulo, true /*ckprime*/, 0 /* rref */))
      return false;
    return remove_identity(res);
  }

  bool smallmodinv(const vector<vector<int>> & a, vector<vector<int>> & res, int modulo,
                   longlong & det_mod_p) {
    res = a;
    add_identity(res);
    int s = int(a.size());
...
#ifndef GIAC_HAS_STO_38
    smallmodrref(1, res, pivots, permutation, rankcols, det_mod_p, 0, s, 0, s, 0, false, modulo, 2,
                 true, 0, true, -1);
    if (det_mod_p == 0)
      return false;
  ...
  smallmodrref(1, res, pivots, permutation, rankcols, det_mod_p, 0, s, 0, 2 * s,
               2 /* full reduction*/, 0 /*dont_swap_below*/, modulo, 0 /* rref */, true, 0, false,
               -1);
  ...
  void dotvecteur_int_(vector<vector<int>>::const_iterator at, const vector<int> & b,
                       vector<longlong> & res) {
  ...
  void multmatvecteur_int(const vector<vector<int>> & a, const vector<int> & b,
                          vector<longlong> & res) {
  ...
  static void dotvecteur_int_(vector<vector<int>>::const_iterator at, const vecteur & b,
                              iterateur it, int p, gen *& yptr) {
    vector<int>::const_iterator ita0 = at->begin(), ita0end = at->end();
    vector<int>::const_iterator ita1 = (at + 1)->begin();
    vector<int>::const_iterator ita2 = (at + 2)->begin();
    vector<int>::const_iterator ita3 = (at + 3)->begin();
  ...
  static void multmatvecteur_int(const matrice & a, const vector<vector<int>> & A,
                                 const vecteur & b, bool smallint, vecteur & res, int p,
                                 gen * yptr) {
#ifndef GIAC_HAS_STO_38
    if (smallint) {
#if 0
  ...
  vecteur padic_linsolve_c(const matrice & a, const vecteur & b, const matrice & c, unsigned n,
                           const gen & p, unsigned reconstruct) {
    unsigned bsize = unsigned(b.size()), asize = unsigned(a.size()); // should be the same
    if (reconstruct && reconstruct < bsize)
      bsize = reconstruct;
    vecteur res(bsize), y(b), x, tmp; // initialize y_0=b
    int resbits = n * nbits(p);
    uncoerce(res, resbits);
    int smallint = 0;
    if (p.type == _INT_ && ((ulonglong)p.val * p.val < ((ulonglong)1 << 63) / asize))
      smallint = 1;
  ...
  static void extract(const vector<vector<int>> & source, const vector<int> & lines, int rang,
                      vector<vector<int>> & target, vector<vector<int>> & excluded) {
  ...
  static void extract1(const matrice & source, const vector<int> & lines, int rang,
                       matrice & target, vector<vecteur> & excluded) {
    int s = int(lines.size()), i;
    target.clear();
    target.reserve(rang);
    for (i = 0; i < rang; ++i) {
  ...
  static void extract2(const matrice & source, const vector<int> & lines, int rang,
                       matrice & target, vector<vecteur> & k_excluded,
  ...
  int padic_linsolve_prepare(const matrice & a, gen & p, vector<int> & ranklines,
                             vector<int> & rankcols, matrice & asub, matrice & ainv,
    ...
    for (;;) { // break loop as soon as a good p is found
      ...
      Nsub = N;
      int nstep = int(giacmin(nrows, ncols) * std::log(evalf_double(h2, 1, context0)._DOUBLE_val) /
                      2 / std::log(double(p.val))) +
                  1;
      vecteur pivots;
      longlong idet;
      smallmodrref(1, Nsub, pivots, ranklines, rankcols, idet, 0, nrows, 0, ncols,
                   0 /* fullreduction*/, 0 /* dont_swap_below */, p.val, 0 /* rref_or_det_or_lu */,
                   true, 0, true, -1);
  ...
  // solve
  bool padic_linsolve_solve(const matrice & a, const gen & p, const vector<int> & ranklines,
                            const vector<int> & rankcols, const matrice & asub,
                            const matrice & ainv, const vecteur & compat, const vecteur & b,
                            vecteur & sol) {
    ...
    int nstep = int(rang * std::log(evalf_double(h2, 1, context0)._DOUBLE_val) / 2 /
                    std::log(double(p.val))) +
                1;
  ...
  define_unary_function_ptr5(at_padic_linsolve, alias_at_padic_linsolve, &__padic_linsolve, 0,
                             true);
  ...
  int padic_linsolve(const matrice & A, const vecteur & b, vecteur & res, gen & p, gen & det_mod_p,
                     gen & h2, unsigned reconstruct, int maxtry) {
#ifdef GIAC_HAS_STO_38
    return 0;
#endif
    ...
    for (unsigned j = 0; j < s; ++j) {
      if (j) {
        ...
        if (is_strictly_positive(sqrtpn + tmp, context0) &&
            is_strictly_positive(sqrtpn - tmp, context0)) {
          if (0) { // debug
  ...
  matrice mrref(const matrice & a, GIAC_CONTEXT) {
    ...
    if (!mrref(a, res, pivots, det, 0, int(a.size()), 0, int(a.front()._VECTptr->size()),
               /* fullreduction */ 1, 0, true, 1, 0, contextptr))
      return vecteur(1, vecteur(1, gendimerr(contextptr)));
    return res;
  }

  bool read_reduction_options(const gen & a_orig, matrice & a, bool & convert_internal,
                              int & algorithm, bool & minor_det, bool & keep_pivot,
                              int & last_col) {
    ...
    if (ckmatrix(a_orig)) {
      ...
      if (s <= 3 && v[0].is_symb_of_sommet(at_pnt)) {
        ...
        if (ckmatrix(v))
          return read_reduction_options(v, a, convert_internal, algorithm, minor_det, keep_pivot,
                                        last_col);
      }
      if (!s || !ckmatrix(v[0]))
        return false;
  ...
  gen _rref(const gen & a_orig, GIAC_CONTEXT) {
    if (a_orig.type == _STRNG && a_orig.subtype == -1)
      return a_orig;
    matrice a;
    bool convert_internal, minor_det, keep_pivot;
    int algorithm, last_col;
    if (!read_reduction_options(a_orig, a, convert_internal, algorithm, minor_det, keep_pivot,
                                last_col))
      return gensizeerr(contextptr);
    if (minor_det)
    ...
    if (!mrref(a, res, pivots, det, 0, int(a.size()), 0, ncols,
               /* fullreduction */ 1, 0, convert_internal, algorithm, 0, contextptr))
      return gendimerr(contextptr);
    if (!keep_pivot) {
    ...
    if (has_num_coeff(v)) {
      ...
      for (; it != itend; ++it) {
        if (has_evalf(*it, tmp, 1, contextptr) &&
            is_greater((tmp = abs(tmp, contextptr)), vmax, contextptr))
          vmax = tmp;
      }
      it = v.begin();
  ...
  void vranm(int n, const gen & F, vecteur & res, GIAC_CONTEXT) {
    ...
    if (f.is_symb_of_sommet(at_interval) && f._SYMBptr->feuille.type == _VECT) {
      gen x = evalf(f._SYMBptr->feuille._VECTptr->front(), 1, contextptr),
          y = evalf(f._SYMBptr->feuille._VECTptr->back(), 1, contextptr);
    ...
    if (f.is_symb_of_sommet(at_uniform) || f.is_symb_of_sommet(at_uniformd)) {
      ...
      if (f.type != _VECT || f._VECTptr->size() != 2 || f._VECTptr->front().type != _DOUBLE_ ||
          f._VECTptr->back().type != _DOUBLE_) {
    ...
    if (f.is_symb_of_sommet(at_exp) || f.is_symb_of_sommet(at_randexp) ||
        f.is_symb_of_sommet(at_exponential) || f.is_symb_of_sommet(at_exponentiald)) {
    ...
    if (f.is_symb_of_sommet(at_geometric) || f.is_symb_of_sommet(at_randgeometric)) {
      ...
      for (int i = 0; i < n; ++i)
        res.push_back(
            int(std::ceil(std::log(1 - giac_rand(contextptr) / (rand_max2 + 1.0)) / lambda)));
      return;
    }
    if (f == at_normald || f == at_NORMALD || f == at_normal || f == at_randNorm ||
        f == at_randnormald)
      f = symbolic(at_normald, makesequence(0, 1));
    if ((f.is_symb_of_sommet(at_normald) || f.is_symb_of_sommet(at_NORMALD) ||
         f.is_symb_of_sommet(at_normal) || f.is_symb_of_sommet(at_randNorm) ||
         f.is_symb_of_sommet(at_randnormald)) &&
    ...
    if ((f.is_symb_of_sommet(at_fisher) || f.is_symb_of_sommet(at_fisherd) ||
         f.is_symb_of_sommet(at_randfisherd) || f.is_symb_of_sommet(at_snedecor) ||
         f.is_symb_of_sommet(at_randfisher)) &&
        f._SYMBptr->feuille.type == _VECT && f._SYMBptr->feuille._VECTptr->size() == 2) {
      gen g1(f._SYMBptr->feuille._VECTptr->front()), g2(f._SYMBptr->feuille._VECTptr->back());
      if (is_integral(g1) && g1.type == _INT_ && g1.val > 0 && g1.val <= 1000 && is_integral(g2) &&
          g2.type == _INT_ && g2.val > 0 && g2.val <= 1000) {
    ...
    if ((f.is_symb_of_sommet(at_chisquare) || f.is_symb_of_sommet(at_randchisquare) ||
         f.is_symb_of_sommet(at_chisquared) || f.is_symb_of_sommet(at_randchisquared)) &&
        f._SYMBptr->feuille.type == _INT_ && f._SYMBptr->feuille.val > 0 &&
        f._SYMBptr->feuille.val <= 1000) {
    ...
    if ((f.is_symb_of_sommet(at_cauchy) || f.is_symb_of_sommet(at_cauchyd)) &&
        f._SYMBptr->feuille.type == _VECT && f._SYMBptr->feuille._VECTptr->size() == 2) {
      gen g1(f._SYMBptr->feuille._VECTptr->front()), g2(f._SYMBptr->feuille._VECTptr->back());
    ...
    if (f.is_symb_of_sommet(at_student) || f.is_symb_of_sommet(at_randstudent) ||
      ...
      if (f._SYMBptr->feuille.type == _INT_ && f._SYMBptr->feuille.val > 0 &&
          f._SYMBptr->feuille.val <= 1000) {
        int k = f._SYMBptr->feuille.val;
        for (int i = 0; i < n; ++i)
    ...
    if (f.is_symb_of_sommet(at_multinomial) && f._SYMBptr->feuille.type == _VECT) {
      ...
      if (P._VECTptr->size() == 2 && P._VECTptr->front().type == _VECT) {
        if (P._VECTptr->back().type != _VECT ||
            P._VECTptr->front()._VECTptr->size() != P._VECTptr->back()._VECTptr->size()) {
          res = vecteur(1, gensizeerr(contextptr));
          return;
        }
    ...
    if ((f.is_symb_of_sommet(at_binomial) || f.is_symb_of_sommet(at_BINOMIAL)) &&
      ...
      f = evalf_double(f._SYMBptr->feuille._VECTptr->back(), 1, contextptr);
      if (!is_integral(N) || N.type != _INT_ || N.val <= 0 || f.type != _DOUBLE_ ||
          f._DOUBLE_val <= 0 || f._DOUBLE_val >= 1) {
        res = vecteur(1, gensizeerr(contextptr));
        return;
      }
      ...
      if (double(Nv) * n > 5 * Nv + n * std::ceil(std::log(double(Nv)) / std::log(2.0))) {
        ...
        for (int k = 0; k < Nv; ++k) {
          // recompute current from time to time
          if (k % 32 == 0)
            current = std::exp(lngamma(Nv + 1) - lngamma(k + 1) - lngamma(Nv - k + 1) +
                               k * std::log(p) + (Nv - k) * std::log(1 - p));
          cumul += current;
          tableau[k + 1] = cumul;
      ...
      if (Nv > 1000) {
        for (int i = 0; i < n; ++i)
          res.push_back(
              binomial_icdf(Nv, p, double(giac_rand(contextptr)) / rand_max2, contextptr));
      } else {
        p *= rand_max2;
        for (int i = 0; i < n; ++i) {
          int ok = 0;
    ...
    case _VECT:
      ...
      if (e._VECTptr->size() >= 2) {
        ...
        if (e._VECTptr->size() == 4) {
          ...
          if (loi.type == _INT_ && e._VECTptr->back().type == _INT_) {
            ...
            for (int j = 0; j < n; ++j) {
              ...
              for (int k = 0; k < m; ++k) {
                (*res._VECTptr)[k] =
                    (a + int((b - a + 1) * (giac_rand(contextptr) / (rand_max2 + 1.0))));
              }
              M[j] = res;
            }
        ...
        if (e._VECTptr->size() > 4) {
          ...
          if (loi.type == _FUNC) {
            if (loi == at_multinomial)
              loi = symbolic(at_multinomial,
                             gen(vecteur(e._VECTptr->begin() + 3, e._VECTptr->end()), _SEQ__VECT));
            else
              loi = loi(gen(vecteur(e._VECTptr->begin() + 3, e._VECTptr->end()), _SEQ__VECT),
                        contextptr);
          } else
            loi =
                symb_of(loi, gen(vecteur(e._VECTptr->begin() + 3, e._VECTptr->end()), _SEQ__VECT));
          return gen(mranm(n, m, loi, contextptr), _MATRIX__VECT);
        }
      ...
      if (e._VECTptr->size() >= 2) {
        ...
        if (e._VECTptr->size() > 3) {
          if (loi.type == _FUNC) {
            if (loi == at_multinomial)
              loi = symbolic(at_multinomial,
                             gen(vecteur(e._VECTptr->begin() + 2, e._VECTptr->end()), _SEQ__VECT));
            else
              loi = loi(gen(vecteur(e._VECTptr->begin() + 2, e._VECTptr->end()), _SEQ__VECT),
                        contextptr);
          } else
            loi =
                symb_of(loi, gen(vecteur(e._VECTptr->begin() + 2, e._VECTptr->end()), _SEQ__VECT));
        }
  ...
  bool minv(const matrice & a, matrice & res, bool convert_internal, int algorithm, GIAC_CONTEXT) {
    ...
    int ok = mrref(arref, res, pivots, det, 0, s, 0, 2 * s,
                   /* fullreduction */ 2 * 256, 0, convert_internal, algorithm, 0, contextptr);
    if (!ok)
      return false;
    if (debug_infolevel)
  ...
  gen _det(const gen & a_orig, GIAC_CONTEXT) {
    gen compact;
    if (determinant_small(a_orig, compact, contextptr))
      return compact;
    if (a_orig.type == _STRNG && a_orig.subtype == -1)
      return a_orig;
    matrice a;
    ...
    int s = int(a.size());
    if (!mrref(a, res, pivots, determinant, 0, s, 0, s,
               /* fullreduction */ 0, 0, convert_internal, algorithm, 1 /* det */, contextptr))
      return gendimerr(contextptr);
    return determinant;
  }
  ...
  void mhessenberg(vector<vector<int>> & H, vector<vector<int>> & P, int modulo, bool compute_P) {
    ...
    for (int m = 0; m < n - 2; ++m) {
      ...
      for (i = m + 2; i < n; ++i) {
        ...
        if (debug_infolevel > 3)
          CERR << "// i=" << i << " " << u << '\n';
        modlinear_combination(Hi, -u, Hmp1, modulo, m, 0,
                              false); // H[i]=H[i]-u*H[m+1]; COULD START at m
        // column operation
        for (int j = 0; j < n; ++j) {
          vector<int> & Hj = H[j];
#ifdef _I386_
	...
	for (int j=0;j<n;++j){
      ...
      */
      ...
      for (i = m + 2; i < n; ++i) {
        ...
        if (debug_infolevel > 2)
          CERR << "// i=" << i << " " << u << '\n';
        linear_combination(plus_one, H[i], -u, H[m + 1], plus_one, 1, vtemp, 1e-12,
                           0); // H[i]=H[i]-u*H[m+1];
        swap(H[i], vtemp);
        linear_combination(plus_one, P[i], -u, P[m + 1], plus_one, 1, vtemp, 1e-12,
                           0); // H[i]=H[i]-u*H[m+1];
        swap(P[i], vtemp);
        // column operation
        for (int j = 0; j < n; ++j) {
  ...
  // a*A+c*C->A
  // c*A-a*C->C
  void bi_linear_combination_AC(giac_double a, vector<giac_double> & A, giac_double c,
                                vector<giac_double> & C, int cstart = 0, int cend = -1) {
  ...
  // a*A+c*C->C
  // c*A-a*C->A
  void bi_linear_combination_CA(giac_double a, vector<giac_double> & A, giac_double c,
                                vector<giac_double> & C, int cstart, int cend) {
    giac_double * Aptr = &A.front() + cstart;
  ...
  void tri_linear_combination(giac_double c11, vector<giac_double> & x1, giac_double c12,
                              vector<giac_double> & x2, giac_double c13, vector<giac_double> & x3,
                              giac_double c22, giac_double c23, giac_double c33, int cstart = 0,
                              int cend = -1) {
    vector<giac_double>::iterator it1 = x1.begin() + cstart, it2 = x2.begin() + cstart,
                                  it3 = x3.begin() + cstart,
                                  it3end = cend < 0 ? x3.end() : x3.begin() + cend;
  ...
  void tri_linear_combination(giac_double a, vector<giac_double> & A, giac_double b,
                              vector<giac_double> & B, giac_double aprime, vector<giac_double> & C,
                              giac_double bprime, int cstart, int cend = 0) {
    giac_double *Aptr = &A.front() + cstart, *Cptr = &C.front() + cstart;
    giac_double *Bptr = &B.front() + cstart,
                *Bend = Bptr + (cend <= 0 ? (B.size() - cstart) : cend - cstart);
    Bend -= 8;
  ...
  void qr_givens_p(matrix_double & P, int Pstart, int Pend, int cstart, int n, int lastcol,
                   const vector<giac_double> & coeffs) {
    ...
    for (int m = lastcol - 1; m >= cstart; --m) {
      ...
      for (int i = n - 1; i > m - cstart; --i) {
        ...
        if (i > m - cstart + 1) {
          ...
          if (U != 0) {
            tri_linear_combination(un, P[i], tn, P[m - cstart], U, P[i - 1], T,
                                   giacmax(m - cstart, Pstart), Pend);
            --i;
            pos -= 2;
            continue;
          }
        }
        bi_linear_combination_CA(un, P[i], tn, P[m - cstart], giacmax(m - cstart, Pstart),
                                 Pend); // givens_linear_combination(un,P[i],tn,P[m],m);
      }
    } // end for m
  }
  ...
  void * do_thread_qr_givens_p(void * ptr_) {
    ...
    qr_givens_p(*ptr->Pptr, ptr->Pstart, ptr->Pend, ptr->cstart, ptr->n, ptr->lastcol,
                *ptr->coeffsptr);
    return ptr;
  }
  ...
  void qr_givens(matrix_double & H, int rstart, matrix_double & P, bool computeP, bool Pidn,
                 bool transpose, int cstart = 0, int cend = 0, bool recurse = true) {
    ...
    if (recurse && n >= c && cend - cstart > 200) {
      ...
      qr_givens(H, rstart, P, true, true, false, cstart, cstart + c1,
                /* recurse*/ false); // P is Q1
      ...
      matrix_double tranA2;
      tranA2.reserve(giacmax(c2, n - c1));
    ...
    coeffs.reserve(lastcol * (2 * n - lastcol));
    if (debug_infolevel)
      ...
      for (i = m - cstart + 1; i < n; ++i) {
        ...
        if (i + 1 < n) {
          ...
          if (U != 0) {
            ...
            coeffs.push_back(T);
            coeffs.push_back(U);
            tri_linear_combination(un, H[i + rstart], tn, H[m - cstart + rstart], U,
                                   H[i + rstart + 1], T, m, cend);
            ++i;
            continue;
          }
        }
        // H[m]=un*H[i]+tn*H[m] and H[i]=tn*H[i]-un*H[m];
        bi_linear_combination_CA(un, H[i + rstart], tn, H[m - cstart + rstart], m,
                                 cend); // givens_linear_combination(un,H[i],tn,H[m],m);
      }
    }
    if (!computeP)
      return;
    ...
    if (Pidn) {
      ...
      if (nthreads > 1) {
        ...
        for (int j = 0; j < nthreads; ++j) {
          ...
          bool res = true;
          if (j < nthreads - 1)
            res = pthread_create(&tab[j], (pthread_attr_t *)NULL, do_thread_qr_givens_p,
                                 (void *)&multdparam[j]);
          if (res)
      ...
      for (int m = cstart; m < lastcol; ++m) {
        ...
        for (i = m + 1; i < n; ++i) {
          ...
          bi_linear_combination_CA(un, P[i], tn, P[m], 0,
                                   cP); // givens_linear_combination(un,P[i],tn,P[m],0);
        }
      }
      if (!transpose)
        transpose_double(P);
  ...
  void qr_rq(std_matrix<gen> & H, std_matrix<gen> & P, const gen & shift, int n, int & nitershift0,
             GIAC_CONTEXT) {
  ...
  bool std_matrix_gen2std_matrix_complex_double(const std_matrix<gen> & H,
                                                matrix_complex_double & H1, bool crunch) {
    int n = int(H.size());
    H1.resize(n);
    for (int i = 0; i < n; ++i) {
      if (!convert(H[i], H1[i], crunch))
  ...
  bool std_matrix_complex_double2std_matrix_gen(const matrix_complex_double & H,
                                                std_matrix<gen> & H1) {
    int n = int(H.size());
    H1.resize(n);
    for (int i = 0; i < n; ++i) {
  ...
  void linear_combination(const gen & c1, const vecteur & v1, const gen & c2, const vecteur & v2,
                          vecteur & v, int cstart, double eps) {
    eps = 0;
    if (cstart < 0)
      cstart = 0;
    const_iterateur it1 = v1.begin() + cstart, it1end = v1.end(), it2 = v2.begin() + cstart;
  ...
  void hessenberg_ortho(std_matrix<gen> & H, std_matrix<gen> & P, int firstrow, int n,
                        bool compute_P, int already_zero, double eps, GIAC_CONTEXT) {
...
#ifdef HAVE_LIBMPFR
  gen tri_linear_combination(const gen & c1, const gen & x1, const gen & c2, const gen & x2,
                             const gen & c3, const gen & x3, mpfr_t & tmp1, mpfr_t & tmp2) {
    if (c1.type != _REAL || x1.type != _REAL || c2.type != _REAL || x2.type != _REAL ||
        c3.type != _REAL || x3.type != _REAL)
  ...
  void tri_linear_combination(const gen & c1, const vecteur & x1, const gen & c2,
                              const vecteur & x2, const gen & c3, const vecteur & x3, vecteur & y) {
  ...
  void francis_schur_iterate(std_matrix<gen> & H, double eps, const gen & l1, int n_orig, int n1,
                             int n2, std_matrix<gen> & P, bool compute_P, GIAC_CONTEXT) {
    ...
    gen c11 = x, c12 = conj(y, contextptr), c21 = y, c22 = -x, tmp1, tmp2;
  ...
  void francis_schur_iterate_real(std_matrix<gen> & H, int n_orig, int n1, int n2,
                                  std_matrix<gen> & P, bool compute_P, GIAC_CONTEXT) {
    ...
    gen ha = H[n1][n1], hb = H[n1][n1 + 1], hd = H[n1 + 1][n1], he = H[n1 + 1][n1 + 1],
        hh = H[n1 + 2][n1 + 1];
    gen x = hb * hd + ha * (ha - s) + p, y = hd * (he - s + ha), z = hd * hh;
    // normalize, subtract [1,0,0] and normalize again
    gen xyz = sqrt(x * conj(x, contextptr) + y * conj(y, contextptr) + z * conj(z, contextptr),
                   contextptr);
    ...
    if (fabs(evalf_double(re(x, contextptr) - 1, 1, contextptr)._DOUBLE_val) < 0.5)
      ...
      x -= 1;
    xyz = sqrt(x * conj(x, contextptr) + y * conj(y, contextptr) + z * conj(z, contextptr),
               contextptr);
    x = x / xyz;
    y = y / xyz;
    z = z / xyz;
    ...
    gen c11 = 1 - 2 * x * conj(x, contextptr), c12 = -2 * x * conj(y, contextptr),
        c13 = -2 * x * conj(z, contextptr);
    gen c21 = -2 * y * conj(x, contextptr), c22 = 1 - 2 * y * conj(y, contextptr),
        c23 = -2 * y * conj(z, contextptr);
    gen c31 = -2 * z * conj(x, contextptr), c32 = -2 * z * conj(y, contextptr),
        c33 = 1 - 2 * z * conj(z, contextptr);
  ...
  bool francis_schur(std_matrix<gen> & H, int n1, int n2, std_matrix<gen> & P, int maxiter,
                     double eps, bool is_hessenberg, bool complex_schur, bool compute_P,
                     bool no_lapack, GIAC_CONTEXT) {
    vecteur eigenv;
    ...
    if (!is_hessenberg) {
      ...
      hessenberg_ortho(H, P, 0, n_orig, compute_P, 0, 0.0,
                       contextptr); // insure Hessenberg form (on the whole matrix)
    }
    if (n2 - n1 <= 1)
    ...
    if (n2 - n1 == 2) { // 2x2 submatrix, we know how to diagonalize
      ...
      if (eigenval2(H, n2, l1, l2, contextptr) || complex_schur) {
        ...
        if (is_greater(abs(H[n1][n1] - l1, contextptr), abs(H[n1][n1] - l2, contextptr),
                       contextptr))
    ...
    for (int niter = 0; n2 - n1 > 2 && niter < maxiter; niter++) {
      ...
      for (int i = n1; i < n2 - 1; ++i) {
        ...
        if (ratio.type == _DOUBLE_ && fabs(ratio._DOUBLE_val) < eps) {
          ...
          if (!francis_schur(H, n1, i + 1, P, maxiter, eps, true, complex_schur, compute_P, true,
                             contextptr))
            return false;
          return francis_schur(H, i + 1, n2, P, maxiter, eps, true, complex_schur, compute_P, true,
                               contextptr);
        }
      }
      ...
      if (complex_schur) {
        ...
        if (n2 - n1 >= 2) {
          ...
          if (is_greater(abs(l1 - H[n2 - 1][n2 - 1], contextptr),
                         abs(l2 - H[n2 - 1][n2 - 1], contextptr), contextptr))
            l1 = l2;
        }
  ...
  void hessenberg_schur(std_matrix<gen> & H, std_matrix<gen> & P, int maxiter, double eps,
                        GIAC_CONTEXT) {
    ...
    if (is_double) {
      ...
      if (francis_schur(H, 0, n_orig, P, maxiter, std::sqrt(double(n_orig)) * eps, false, !real,
                        true, true, contextptr)) {
        return;
      }
    }
    ...
    for (int niter = 0; n > 1 && niter < maxiter; niter++) {
      ...
      if (Small)
        ...
        if (n == 2 ||
            is_strictly_greater(0.01, (ratio = abs(H[n - 2][n - 3] / H[n - 2][n - 2], contextptr)),
                                contextptr)) {
      ...
      ratio = test / abs(H[n - 1][n - 1], contextptr);
      if (is_strictly_greater(gen(eps) / oldratio, 1, contextptr) &&
          is_strictly_greater(gen(eps) / ratio, 1, contextptr)) {
  ...
  bool mhessenberg(const matrice & M, matrice & h, matrice & p, int modulo, int maxiter, double eps,
                   GIAC_CONTEXT) {
    int n = int(M.size());
    if (!n || n != mcols(M))
  ...
  bool mod_pcar(vector<vector<int>> & N, int modulo, bool & krylov, vector<int> & res, GIAC_CONTEXT,
    ...
    if (krylov) { // try Krylov pmin
      ...
      smallmodrref(1, ttemp, pivots, permutation, maxrankcol, det, 0, n, 0, n + 1,
                   false /*full reduction */, 0, modulo, 2 /* LU */, true, 0, true, -1);
      if (debug_infolevel > 2)
  ...
  bool mod_pcar(const matrice & A, vector<vector<int>> & N, int modulo, bool & krylov,
                vector<int> & res, GIAC_CONTEXT, bool compute_pmin) {
    if (debug_infolevel > 2)
  ...
  vecteur mpcar_int(const matrice & A, bool krylov, GIAC_CONTEXT, bool compute_pmin) {
    ...
    double logbound = n / (1 + 1.0 / std::sqrt(double(n)) / Bd) *
                          (std::log10(double(n)) / 2 + std::log10(Bd)) +
                      n * std::log10(2.0),
           testvalue;
    ...
    for (; pipd < (testvalue = logbound * charpol.size() / (n + 1.0));) {
      if (currentprob < proba && pipd < testvalue / 1.33 &&
          CLOCK() - initial_clock > min_proba_time * CLOCKS_PER_SEC)
        break;
      if (n > 10 && dbglevel < 2 && CLOCK() - initial_clock > 60 * CLOCKS_PER_SEC)
        dbglevel = 2;
      if (dbglevel > 1)
        CERR << CLOCK() * 1e-6 << " " << 100 * pipd / testvalue << " % done"
             << (currentprob < proba ? ", stable." : ", unstable.") << '\n';
    ...
    if (pipd < testvalue)
      *logptr(contextptr)
          << gettext("Probabilistic answer. Run proba_epsilon:=0 for a certified result. Error <")
          << proba << '\n';
    return charpol;
  ...
  define_unary_function_ptr5(at_pcar_hessenberg, alias_at_pcar_hessenberg, &__pcar_hessenberg, 0,
                             true);
  ...
  dense_POLY1 mpcar(const matrice & a, vecteur & Bv, bool compute_Bv, bool convert_internal,
                    GIAC_CONTEXT) {
    int n = int(a.size());
  ...
  define_unary_function_ptr5(at_rat_jordan_block, alias_at_rat_jordan_block, &__rat_jordan_block, 0,
                             true);
    ...
    for (int j = 1; j < n; ++j) {
      ...
      for (int l = 1; l <= d; ++l) {
        ...
        for (int m = 1; m <= mmax; ++m) {
          QAvj0 = addvecteur(QAvj0, multvecteur(q[l] * comb((unsigned long)l, (unsigned long)m),
                                                *res[(j - m) * d + (l - m)]._VECTptr));
        }
      }
      // shift
      ...
      for (int l = 1; l < d; ++l) {
        ...
        int mmax = giacmin(l, j);
        for (int m = 1; m <= mmax; ++m)
          tmp = subvecteur(tmp, multvecteur(comb((unsigned long)l, (unsigned long)m),
                                            *res[(j - m) * d + (l - m)]._VECTptr));
        res[j * d + l] = tmp;
      }
    }
  ...
  bool lapack_schur(matrix_double & H, matrix_double & P, bool compute_P, vecteur & eigenvalues) {
    ...
    */
    ...
    doublef2c_real *Wreal = new doublef2c_real[n], *Wimag = new doublef2c_real[n],
                   *work = new doublef2c_real[lwork];
    logical * bwork = new logical[n];
    char ch[2] = {0, 0};
    ch[0] = (compute_P ? 'v' : 'n');
    matrice_double2lapack(H, Hlapack);
    char ch2[] = "n";
    dgees_(ch, ch2, 0, &n, Hlapack, &n, &sdim, Wreal, Wimag, Plapack, &ldvs, work, &lwork, bwork,
           &info);
  ...
  bool lapack_schur(std_matrix<gen> & H, std_matrix<gen> & P, bool compute_P, vecteur & eigenvalues,
                    GIAC_CONTEXT) {
    if (!CAN_USE_LAPACK)
      return false;
    ...
    */
    integer n = H.size(), sdim, ldvs = n, lwork = max(20, n) * n, info;
    doublef2c_complex *Hlapack = new doublef2c_complex[n * n],
                      *Plapack = new doublef2c_complex[n * n];
    ...
    ch[0] = (compute_P ? 'v' : 'n');
    char ch2[] = "n";
    zgees_(ch, ch2, 0, &n, Hlapack, &n, &sdim, W, Plapack, &ldvs, work, &lwork, rwork, bwork,
           &info);
...
bool lapack_schur(std_matrix<gen> & H, std_matrix<gen> & P, bool compute_P, vecteur & eigenvalues,
  ...
  bool egv(const matrice & m0, matrice & p, vecteur & d, GIAC_CONTEXT, bool jordan,
           bool rational_jordan_form, bool eigenvalues_only) {
    matrice m = m0;
    if (m.size() == 1) {
    ...
    if (numeric_matrix) {
      ...
      if (std_matrix_gen2std_matrix_giac_double(H, H1, false)) {
        bool ans =
            francis_schur(H1, 0, dim, P1, 2 * SOLVER_MAX_ITERATE, eps, false, !eigenvalues_only);
        if (eigenvalues_only) {
          vecteur res;
          ...
          ans = francis_schur(H, 0, dim, P, 2 * SOLVER_MAX_ITERATE, eps, true, true, true, true,
                              contextptr);
          std_matrix_gen2matrice_destroy(P, p);
        ...
        if (matrice2std_matrix_complex_double(m, H2)) {
          ...
          ans = francis_schur(H, 0, dim, P, SOLVER_MAX_ITERATE, dim * eps, false, true, true, true,
                              contextptr);
        std_matrix_gen2matrice_destroy(P, p);
        std_matrix_gen2matrice_destroy(H, d);
        return ans && schur_eigenvectors(p, d, eps, contextptr);
    ...
    matrice mr = *(e2r(numeric_matrix ? exact(m, contextptr) : m, lv, contextptr)
                       ._VECTptr); // convert to internal form
    // vecteur lv;
    // matrice mr = m;
    matrice m_adj;
    vecteur p_car;
    ...
    if (has_mod_coeff(p_car, modulo)) {
      ...
      if (ddf(pc, qmat, &env, vpc)) {
        ...
        *logptr(contextptr) << "Warning! Automatic extension not implemented. You can try to "
                               "diagonalize the matrix * a non trivial element of GF("
                            << modulo << ",lcm of degrees of factor(" << symb_horner(p_car, vx_var)
                            << "))" << '\n';
    }
#ifndef NO_RTTI
    ...
    if (!factor(ppcar, p_content, f, false, rational_jordan_form ? false : withsqrt(contextptr),
                //false,
                complex_mode(contextptr), 1, extra_div))
      return false;
    ...
    for (; f_it != f_itend; ++f_it) {
      ...
      if (s >= 3 && rational_jordan_form) {
        ...
        for (; char_found < n;) {
          ...
          if (!mrref(Ccopy, C, pivots, det, 0, int(Ccopy.size()), 0, taille,
                     /* fullreduction */ 1, char_line, true, 1, 0, contextptr))
            return false;
          ...
          tmp = mergematrice(
              tmp, vecteur(qdeg * cycle_size, vecteur(taille - total_char_found - taille_bloc)));
          if (is_undef(tmp))
            return false;
      ...
      if (s >= 3) { // recompute cur_m_adj using new extensions
        ...
        if (v.size() != w.size() - 1) {
          ...
          if (m0num.type == _VECT && is_numericm(*m0num._VECTptr)
              // && lidnt(m0num).empty()
          ) {
        ...
        if (jordan) {
          ...
          for (; char_found < n;) {
            if (!mrref(char_m_copy, char_m, pivots, det, 0, taille, 0, taille,
                       /* fullreduction */ 1, egv_found, true, 1, 0, contextptr))
              return false;
            if (sym)
            ...
            int j = 0;
            for (; j < egv_found; ++j)
              char_m_copy.push_back(
                  vecteur(char_m[j]._VECTptr->begin(), char_m[j]._VECTptr->end() - taille));
            for (; j < taille; ++j) {
              vegv = vecteur(char_m[j]._VECTptr->begin(), char_m[j]._VECTptr->begin() + taille);
              if (is_zero(vegv, contextptr) ||
                  (numeric_matrix && evalf(abs(vegv, contextptr), 1, contextptr)._DOUBLE_val <
                                         10 * taille * epsilon(contextptr)))
                break;
              // cycle found!
              ...
              char_m_copy.push_back(
                  vecteur(char_m[j]._VECTptr->begin(), char_m[j]._VECTptr->end() - taille));
              // Store cycle
              const_iterateur c_it = char_m[j]._VECTptr->begin(),
                              c_itend = char_m[j]._VECTptr->end();
            ...
            for (; j < taille; ++j) {
              char_m_copy.push_back(
                  vecteur(char_m[j]._VECTptr->begin() + taille, char_m[j]._VECTptr->end()));
            }
          }
        } // end if (jordan)
        else {
          ...
          if (sym) {
            ...
            m_egv =
                gramschmidt(matrice(m_egv.begin(), m_egv.begin() + f_it->mult), false, contextptr);
          }
  ...
  matrice megv(const matrice & e, GIAC_CONTEXT) {
    ...
    if (!egv(e, m, d, contextptr, false, false, false))
      *logptr(contextptr) << gettext("Low accuracy or not diagonalizable at some eigenvalue. Try "
                                     "jordan if the matrix is exact.")
                          << '\n';
  ...
  gen jordan(const gen & a, bool rational_jordan, GIAC_CONTEXT) {
    if (a.type == _VECT && a.subtype == _SEQ__VECT && a._VECTptr->size() == 2 &&
        is_squarematrix(a._VECTptr->front())) {
  ...
  bool mker(vector<vector<int>> & res, vector<vector<int>> & v, int modulo) {
    v.clear();
    longlong det;
    vector<int> permutation(res.size()), maxrankcols;
    vecteur pivots;
    for (int i = 0; i < res.size(); ++i)
      permutation[i] = i;
#if 1
    if (debug_infolevel)
      CERR << CLOCK() * 1e-6 << " rref lower\n";
    smallmodrref(1, res, pivots, permutation, maxrankcols, det, 0, int(res.size()), 0,
                 int(res.front().size()), /* fullreduction */ 0, 0, modulo, 0, false, NULL, true,
                 modulo);
    int usedcount = 0;
    if (debug_infolevel)
      CERR << CLOCK() * 1e-6 << " rref upper\n";
    smallmodrref_upper(res, 0, int(res.size()), 0, int(res.front().size()), modulo);
    if (debug_infolevel)
      CERR << CLOCK() * 1e-6 << " rref end\n";
#else
  smallmodrref(1, res, pivots, permutation, maxrankcols, det, 0, int(res.size()), 0,
               int(res.front().size()), /* fullreduction */ 1, 0, modulo, 0, false, NULL, true,
               modulo);
#endif
  ...
  bool mker(const matrice & a, vecteur & v, int algorithm, GIAC_CONTEXT) {
    ...
    matrice res;
    if (!mrref(a, res, pivots, det, 0, int(a.size()), 0, int(a.front()._VECTptr->size()),
               /* fullreduction */ 1, 0, true, algorithm, 0, contextptr))
      return false;
    mdividebypivot(res, -1, contextptr);
    // put zero lines in res at their proper place, so that
    // non zero pivot are on the diagonal
  ...
  bool mimage(const matrice & a, vecteur & v, GIAC_CONTEXT) {
    ...
    vecteur pivots;
    matrice res;
    if (!mrref(atran, res, pivots, det, 0, int(atran.size()), 0,
               int(atran.front()._VECTptr->size()),
               /* fullreduction */ 1, 0, true, 1, 0, contextptr))
      return false;
    matrice::const_iterator it = res.begin(), itend = res.end();
    for (int i = 0; it != itend; ++it, ++i) {
  ...
  gen cross(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    if (g1.type != _VECT || g2.type != _VECT) {
      ...
      if (g1._VECTptr->size() == 2 && g2._VECTptr->size() == 2)
        return g1._VECTptr->front() * g2._VECTptr->back() -
               g1._VECTptr->back() * g2._VECTptr->front();
  ...
  */
  ...
  bool mlu(const matrice & a0, vecteur & P, matrice & L, matrice & U, GIAC_CONTEXT) {
    ...
    if (!ckmatrix(a)) { // activate non-square matrix (instead of is_squarematrix)
      if (a.front().type == _VECT && !a.front()._VECTptr->empty() &&
          (a.back() == at_irem || a.back() == at_ichinrem)) {
        modular = true;
        a = *a.front()._VECTptr;
      }
      if (!ckmatrix(a))
  ...
  gen qr(const gen & args_orig, GIAC_CONTEXT) {
    ...
    method = -3;
#endif
    if ((args_orig.type == _VECT) && (args_orig._VECTptr->size() == 2) &&
        (args_orig._VECTptr->back().type == _INT_)) {
    ...
    if (method < 0 || !is_fully_numeric(evalf_double(args, 1, contextptr)) ||
        (cplx = !is_zero(im(args, contextptr), contextptr))) {
      matrice r;
      ...
      matrice res(gramschmidt(*_trn(args, contextptr)._VECTptr, r,
                              cplx || method == -1 || method == -3, contextptr));
      if (method <= -3)
  ...
  gen _svd(const gen & args_orig, GIAC_CONTEXT) {
    if (args_orig.type == _STRNG && args_orig.subtype == -1)
      return args_orig;
    gen args;
    ...
    if (real && method >= 0 && is_fully_numeric((argsf = evalf_double(args, 1, contextptr)))) {
      ...
      // first call to determine optimum work vector size lwork
      dgesvd_(&jobU, &jobVT, &rows, &cols, A, &rows, S, U, &rows, VT, &cols, &worktmp, &lwork,
              &info);
      if (info) {
        delete[] A;
        delete[] S;
      ...
      if (1) {
        ...
        gsl_matrix_free(x);
        matrice U(gsl_matrix2matrice(u)), S(gsl_vector2vecteur(s)),
            V(gsl_matrix2matrice(v)); // A=U*S*tran(V)
        gsl_matrix_free(u);
        gsl_matrix_free(v);
        gsl_vector_free(s);
  ...
  gen smalleps = s * pow(eps, inv(smallsvl ? smallsvl : 1, contextptr), contextptr) * svdmax2;
    ...
    if (smallsvl)
      *logptr(contextptr) << "Warning, ill-conditionned matrix, " << smallsvl
                          << " small singular values were replaced by 0. Result is probably wrong."
                          << '\n';
    if (method == -2) {
      if (transposed) {
    ...
    if (tmp.type != _VECT || tmp._VECTptr->size() != 3 || !ckmatrix(tmp._VECTptr->front()) ||
        !ckmatrix(tmp[1]))
      return gensizeerr(contextptr);
    u = *tmp[0]._VECTptr;
    mtran(u, Mp);
  ...
  gen _cholesky(const gen & _args, GIAC_CONTEXT) {
    if (_args.type == _STRNG && _args.subtype == -1)
      return _args;
...
*/
  ...
  matrice gramschmidt(const matrice & m, matrice & r, bool normalize, GIAC_CONTEXT) {
    ...
    for (int i = 1; i < s; ++i) {
      ...
      for (int j = 0; j < i; ++j) {
        coeff =
            rdiv(dotvecteur(*conj(v[j], contextptr)._VECTptr, *v[i]._VECTptr), sc[j], contextptr);
        cl = cl + coeff * v[j];
        rcol[j] = coeff;
      }
      v[i] = v[i] - cl;
  ...
  matrice lll(const matrice & M, matrice & L, matrice & O, matrice & A, GIAC_CONTEXT) {
    ...
    for (; k < n;) {
      ...
      for (int j = 0; j < k; ++j) {
        ...
        tmp[j] =
            dotvecteur(conj(O[j], contextptr), Otmp) / dotvecteur(conj(O[j], contextptr), O[j]);
      ...
      for (int j = k - 1; j >= 0; --j) {
        gen alpha =
            dotvecteur(conj(O[j], contextptr), res[k]) / dotvecteur(conj(O[j], contextptr), O[j]);
      ...
      gen lastalpha = dotvecteur(conj(O[k - 1], contextptr), res[k]) /
                      dotvecteur(conj(O[k - 1], contextptr), O[k - 1]);
      if (ck_is_greater(dotvecteur(conj(O[k], contextptr), O[k]),
                        (gen(3) / 4 - lastalpha * conj(lastalpha, contextptr)) *
                            dotvecteur(conj(O[k - 1], contextptr), O[k - 1]),
                        contextptr)) {
  ...
  void modlinear_combination(const gen & c1, const vecteur & v1, const gen & c2, const vecteur & v2,
                             const gen & c, vecteur & w, environment * env, int cstart, int cend) {
  ...
  bool hermite(const std_matrix<gen> & Aorig, std_matrix<gen> & U, std_matrix<gen> & A,
               environment * env, GIAC_CONTEXT) {
    A = Aorig;
    int n = int(A.size());
    if (!n)
      return false;
  ...
  bool smith(const std_matrix<gen> & Aorig, std_matrix<gen> & U, std_matrix<gen> & A,
             std_matrix<gen> & V, environment * env, GIAC_CONTEXT) {
    A = Aorig;
    int n = int(A.size());
    if (!n)
      return false; // setsizeerr();
  ...
  bool eigenval2(matrix_double & H, int n2, giac_double & l1, giac_double & l2) {
    giac_double a = H[n2 - 2][n2 - 2], b = H[n2 - 2][n2 - 1], c = H[n2 - 1][n2 - 2],
                d = H[n2 - 1][n2 - 1];
  ...
  // v=(c1*v1+c2*v2), begin at cstart
  void linear_combination(giac_double c1, const vector<giac_double> & v1, giac_double c2,
                          const vector<giac_double> & v2, vector<giac_double> & v, int cstart,
                          double eps) {
    eps = 0;
    if (cstart < 0)
      cstart = 0;
    vector<giac_double>::const_iterator it1 = v1.begin() + cstart, it1end = v1.end(),
                                        it2 = v2.begin() + cstart;
  ...
  void householder_mult2(const matrix_double & H, const std::vector<giac_double> & w,
                         vector<giac_double> & v, vector<giac_double> & vprime, int k,
                         bool is_k_hessenberg) {
    int n = int(H.size());
    for (int j = 0; j < n; ++j)
      vprime[j] = 0;
    int j = 0;
    for (; j <= n - 4; j += 4) {
      const giac_double *H0jk = &H[j].front(), *H0jk1 = H0jk + k, *H0jkend = H0jk + n,
                        *wk = &w.front() + k;
      ...
      giac_double res0 = 0.0, res1 = 0.0, res2 = 0.0, res3 = 0.0, wj0 = w[j], wj1 = w[j + 1],
                  wj2 = w[j + 2], wj3 = w[j + 3];
  ...
  // H*w->v, assumes correct sizes (v already initialized)
  // assumes w[0]=w[1]=...=w[k-1]=0
  void householder_mult(const matrix_double & H, const std::vector<giac_double> & w,
                        vector<giac_double> & v, int k) {
    int n = int(H.size());
    for (int j = 0; j < n; ++j) {
      vector<giac_double>::const_iterator it = H[j].begin() + k, itend = H[j].end(),
                                          jt = w.begin() + k;
      giac_double res = 0.0;
      for (; it != itend; ++jt, ++it)
        res += (*it) * (*jt);
  ...
  // assumes w[0]=w[1]=...=w[k-1]=0
  void householder_mult(const std::vector<giac_double> & w, const matrix_double & H,
                        vector<giac_double> & v, int k, bool is_k_hessenberg, int jstart, int jend,
                        int deltarow = 0, int cstart = 0, int cend = 0) {
    ...
    for (; j <= jend - 8; j += 8) {
      giac_double wj0 = w[j], wj1 = w[j + 1], wj2 = w[j + 2], wj3 = w[j + 3], wj4 = w[j + 4],
                  wj5 = w[j + 5], wj6 = w[j + 6], wj7 = w[j + 7];
      const giac_double *Hjk0 = &H[j + deltarow][cstart], *Hjk1 = &H[j + deltarow + 1][cstart],
                        *Hjk2 = &H[j + deltarow + 2][cstart], *Hjk3 = &H[j + deltarow + 3][cstart],
                        *Hjk4 = &H[j + deltarow + 4][cstart], *Hjk5 = &H[j + deltarow + 5][cstart],
                        *Hjk6 = &H[j + deltarow + 6][cstart], *Hjk7 = &H[j + deltarow + 7][cstart];
      ...
      vkend -= 8;
      for (; vk <= vkend;) {
        *vk += wj0 * (*Hjk0) + wj1 * (*Hjk1) + wj2 * (*Hjk2) + wj3 * (*Hjk3) + wj4 * (*Hjk4) +
               wj5 * (*Hjk5) + wj6 * (*Hjk6) + wj7 * (*Hjk7);
        ;
        ++vk;
        ++Hjk0;
        ++Hjk1;
  ...
  // assumes w[0]=w[1]=...=w[k-1]=0
  void householder_mult(const std::vector<giac_double> & w, const matrix_double & H,
                        vector<giac_double> & v, int k, bool is_k_hessenberg) {
    int n = int(H.size());
  ...
  void hessenberg_p_pass1(matrix_double & P, const vector<giac_double> & w,
                          const vector<giac_double> & qprime, int j, int jend, int n,
    ...
    for (; j <= jend - 3; j += 3) {
      ...
      giac_double *Pj0k = &P[j + deltarow][qstart + deltacol],
                  *Pj1k = &P[j + deltarow + 1][qstart + deltacol],
                  *Pj2k = &P[j + deltarow + 2][qstart + deltacol], *Pj0kend = Pj0k + (n - qstart);
  ...
  void householder_idn_mult(const matrix_double & H, const std::vector<giac_double> & w,
                            vector<giac_double> & v, int k) {
    v.resize(w.size());
    int n = int(H.size());
    ...
    int j = k;
    for (; j <= n - 4; j += 4) {
      vector<giac_double>::const_iterator it0 = H[j].begin() + k, it1 = H[j + 1].begin() + k,
                                          it2 = H[j + 2].begin() + k, it3 = H[j + 3].begin() + k,
                                          itend = H[j].end(), jt = w.begin() + k;
      giac_double res0 = 0.0, res1 = 0.0, res2 = 0.0, res3 = 0.0;
  ...
  void householder_idnt_mult2(const matrix_double & P, const std::vector<giac_double> & w1,
                              vector<giac_double> & w2, vector<giac_double> & Pw1,
                              vector<giac_double> & Pw2, int k) {
    Pw1.resize(w1.size());
    Pw2.resize(w2.size());
    int n = int(P.size());
...
#if 1
    for (; j <= n - 2; j += 2) {
      vector<giac_double>::const_iterator it0 = P[j].begin() + k, it1 = P[j + 1].begin() + k,
                                          itend = P[j].end(), jt1 = w1.begin() + k,
                                          jt2 = w2.begin() + k;
...
#endif
    for (; j < n; ++j) {
      vector<giac_double>::const_iterator it = P[j].begin() + k, itend = P[j].end(),
                                          jt1 = w1.begin() + k, jt2 = w2.begin() + k;
  ...
  // P->P-Pw1 w1* - Pw2 w2*
  void hessenberg_idnt_2p(matrix_double & P, const std::vector<giac_double> & Pw1,
                          vector<giac_double> & Pw2, vector<giac_double> & w1,
                          vector<giac_double> & w2) {
    int qstart = 0;
...
#if 1
    for (; j <= jend - 2; j += 2) {
      const giac_double wj10 = 2 * Pw1[j], wj11 = 2 * Pw1[j + 1], wj20 = 2 * Pw2[j],
                        wj21 = 2 * Pw2[j + 1];
  ...
  void qr_householder(matrix_double & H, int rstart, matrix_double & P, bool computeP, bool Pidn,
                      bool transpose, int cstart = 0, int cend = 0, bool recurse = true,
                      bool thin = true) {
    ...
    if (recurse && n >= c && cend - cstart > 200) {
      ...
      // Q12 has n-c1 rows, Q2 has n-c2 rows
      int c1 = (cend - cstart) / 2, c2 = cend - cstart - c1;
      qr_householder(H, rstart, P, true, true, true, cstart, cstart + c1, /* recurse*/ false,
                     /* thin */ false); // P is Q1
      //transpose_double(P); // P is tran(Q1)
      // R11 is in place in H, R21=0 also
      // temporary storage to compute tran(Q1)*A2
  ...
  void hessenberg_householder(matrix_double & H, matrix_double & P, bool compute_P) {
    ...
    householder_mult(w, H, qprime, m + 1, true);
      ...
      for (; j <= m - 2; j += 3) {
        ...
        giac_double *Hjk = &H[j][m + 1], *Hjk1 = &H[j + 1][m + 1], *Hjk2 = &H[j + 2][m + 1],
                    *wk = &w[m + 1], *wkend = wk + (n - m - 1);
      ...
      for (; j <= n - 3; j += 3) {
        giac_double wj0 = 2 * w[j], wj1 = 2 * w[j + 1], qj0 = 2 * q[j], qj1 = 2 * q[j + 1],
                    wj2 = 2 * w[j + 2], qj2 = 2 * q[j + 2], tmpq, tmpw;
        giac_double *Hj0k = &H[j][m], *Hj1k = &H[j + 1][m], *Hj2k = &H[j + 2][m], *wk = &w[m],
                    *wkend = wk + (n - m), *qprimek = &qprime[m];
        wkend -= 8;
        for (; wk <= wkend;) {
          tmpq = *qprimek;
          tmpw = *wk;
  ...
  void bi_tri_linear_combination(giac_double u, giac_double t, giac_double u2, giac_double t2,
                                 giac_double U, giac_double T, giac_double U2, giac_double T2,
                                 vector<giac_double> & p1, vector<giac_double> & p2,
                                 vector<giac_double> & p3, vector<giac_double> & P3, int cstart,
                                 int cend) {
    ...
    vector<giac_double>::iterator p1p = p1.begin() + cstart,
                                  p1e = cend <= 0 ? p1.end() : p1.begin() + cend,
                                  p2p = p2.begin() + cstart, p3p = p3.begin() + cstart,
                                  P3p = P3.begin() + cstart;
    p1e -= 2;
    for (; p1p <= p1e;) {
    ...
    while (opindex <= int(oper.size()) - 3) {
      ...
      if (optype == 2) {
        for (int m = firstrow; m < n - 2; opindex += 2, ++m)
          bi_linear_combination_CA(oper[opindex], P[m + 2], oper[opindex + 1], P[m + 1], cstart,
                                   cend);
        continue;
      }
      if (optype == -3) {
        tri_linear_combination(oper[opindex], P[n], oper[opindex + 1], P[n + 1], oper[opindex + 2],
                               P[n + 2], oper[opindex + 3], oper[opindex + 4], oper[opindex + 5],
                               cstart, cend);
        opindex += 6;
        continue;
      }
  ...
  void hessenberg_ortho3_flush_p(matrix_double & P, bool compute_P, vector<giac_double> & oper,
                                 bool force_flush) {
    if (oper.empty())
      return;
    if (!compute_P) {
    ...
    if (nthreads > 1 && nH * oper.size() > 1e6) {
      ...
      for (int j = 0; j < nthreads; ++j) {
        ...
        bool res = true;
        if (j < nthreads - 1)
          res = pthread_create(&tab[j], (pthread_attr_t *)NULL, do_hessenberg_p,
                               (void *)&hessenbergparam[j]);
        if (res)
  ...
  void hessenberg_ortho(matrix_double & H, matrix_double & P, int firstrow, int n, bool compute_P,
                        int already_zero, vector<giac_double> & oper) {
  ...
  void hessenberg_ortho3(matrix_double & H, matrix_double & P, int firstrow, int n, bool compute_P,
...
#if 1
    for (; j < n - 4; j += 3) {
      giac_double *Hj = &H[j].front(), *Hjend = Hj + n - 2, *Hj1 = &H[j + 1].front(),
                  *Hj2 = &H[j + 2].front();
      opindex = opstart;
      if (j > firstrow) {
  ...
  void tri_linear_combination(const giac_double & c1, const vector<giac_double> & x1,
                              const giac_double & c2, const vector<giac_double> & x2,
                              const giac_double & c3, const vector<giac_double> & x3,
                              vector<giac_double> & y) {
    vector<giac_double>::const_iterator it1 = x1.begin(), it2 = x2.begin(), it3 = x3.begin(),
                                        it3end = x3.end();
  ...
  template <class T> nio::ios_base<T> & operator<<(nio::ios_base<T> & os, const matrix_double & m) {
    int s = int(m.size());
    for (int i = 0; i < s; ++i)
      os << m[i] << '\n';
    return os;
  ...
  void matrix_double::dbgprint() const {
    COUT << *this << '\n';
  }
...
ostream & operator<<(ostream & os, const matrix_complex_double & m) {
  ...
  void matrix_complex_double::dbgprint() const {
    COUT << *this << '\n';
  }

  void francis_iterate1(matrix_double & H, int n1, int n2, matrix_double & P, double eps,
                        bool compute_P, giac_double l1, bool finish, vector<giac_double> & oper) {
    if (debug_infolevel > 2)
  ...
  bool in_francis_schur(matrix_double & H, int n1, int n2, matrix_double & P, int maxiter,
                        double eps, bool compute_P, matrix_double & Haux, matrix_double & T,
                        bool in_recursion, vector<giac_double> & oper);

  void do_francis_iterate2(matrix_double & H, int n1, int n2, giac_double s, giac_double p,
                           matrix_double & P, bool compute_P, vector<giac_double> & oper) {
    // compute (H-l2)(H-l1)=(H-s)*H+p on n1-th basis vector (if n1==0, on [1,0,...,0])
    giac_double ha = H[n1][n1], hb = H[n1][n1 + 1], hd = H[n1 + 1][n1], he = H[n1 + 1][n1 + 1],
                hh = H[n1 + 2][n1 + 1];
    giac_double x = hb * hd + ha * (ha - s) + p, y = hd * (he - s + ha), z = hd * hh, c11, c12, c13,
  ...
  void francis_iterate2(matrix_double & H, int n1, int n2, matrix_double & P, double eps,
                        bool compute_P, matrix_double & Haux, matrix_double & T, bool in_recursion,
                        vector<giac_double> & oper) {
    // now H is proper hessenberg (indices n1 to n2-1)
    if (debug_infolevel > 2)
    ...
    if (
      ...
      giac_double savetest, smallest = -1;
      if (ok < limite)
        k = n2 - 1;
      else
        ok = limite;
      for (int k0 = n2 - 2; k0 > n2 - 200 && k0 > (0.2 * n1 + 0.8 * n2); --k0) {
      ...
      if (k > n1 + 2 && k < n2 - 2) {
        ...
        if (debug_infolevel > 2 && d >= 3) {
          if (n1k)
            CERR << CLOCK() * 1e-6 << " ok=" << ok << ", test=" << savetest
                 << ", recursive call dim " << d << " n1 " << n1 << " on ... [" << T[d - 2][d - 3]
                 << "," << T[d - 2][d - 2] << "," << T[d - 2][d - 1] << " ],[" << T[d - 1][d - 2]
                 << "," << T[d - 1][d - 1] << "]" << '\n';
          else
            CERR << CLOCK() * 1e-6 << " ok=" << ok << ", test=" << savetest
                 << ", recursive call dim " << d << " n2 " << n2 << " on ... [" << T[d - 2][d - 3]
                 << "," << T[d - 2][d - 2] << "," << T[d - 2][d - 1] << " ],[" << T[d - 1][d - 2]
                 << "," << T[d - 1][d - 1] << "]" << '\n';
        }
        ...
        if (in_francis_schur(T, 0, d, TP, 25, eps, false /* TP not computed*/, Haux, T,
                             true /* in_recursion */, oper_recursive)) {
          debug_infolevel = save_debug_infolevel;
          if (debug_infolevel > 2) {
            CERR << CLOCK() * 1e-6 << " end recursive call on ... [" << T[d - 2][d - 3] << ","
                 << T[d - 2][d - 2] << "," << T[d - 2][d - 1] << " ][0," << T[d - 1][d - 2] << ","
                 << T[d - 1][d - 1] << "]" << '\n';
            if (debug_infolevel > 3) {
    ...
    if (p == s * s / 4 ||
        (absdouble(H[n2 - 2][n2 - 2]) < eps && absdouble(H[n2 - 1][n2 - 1]) < eps)) {
      // multiple root
  ...
  bool in_francis_schur(matrix_double & H, int n1, int n2, matrix_double & P, int maxiter,
    ...
    for (int niter = 0; n2 - n1 > 2 && niter < maxiter; niter++) {
      ...
      for (int i = n2 - 2; i >= n1; --i) {
        ...
        ratio = absdouble(H[i + 1][i]) /
                (absdouble(H[i][i]) + (i < n2 - 2 ? absdouble(H[i + 2][i + 1]) : 0));
        ...
        if (ratio < coeff * eps) {
          ...
          if (debug_infolevel > 2)
            CERR << '\n'
                 << CLOCK() * 1e-6 << " Francis split double "
                 << giacmin((i + 1) - n1, n2 - (i + 1)) << " [" << n1 << " " << i + 1 << " " << n2
                 << "]" << '\n';
#ifdef GIAC_SCHUR_RECURSE_ALL
          if (!in_francis_schur(H, n1, i + 1, P, maxiter, eps, compute_P, Haux, T, in_recursion,
                                oper)) {
        ...
        if (in_recursion && n2 - (i + 1) <= 2)
          return true;
        if (!in_recursion && !in_francis_schur(H, n1, i + 1, P, maxiter, eps, compute_P, Haux, T,
                                               in_recursion, oper)) {
          in_francis_schur(H, i + 1, n2, P, maxiter, eps, compute_P, Haux, T, in_recursion, oper);
          return false;
        }
#endif
          return in_francis_schur(H, i + 1, n2, P, maxiter, eps, compute_P, Haux, T, in_recursion,
                                  oper);
        }
  ...
  bool balanced_eigenvalues(matrix_double & H, vecteur & res, int maxiter, double eps,
                            bool is_hessenberg, GIAC_CONTEXT) {
  ...
  bool francis_schur(matrix_double & H, int n1, int n2, matrix_double & P, int maxiter, double eps,
                     bool is_hessenberg, bool compute_P) {
    vecteur eigenv;
    if (n1 == 0
        // && n2<400
  ...
  // conj(a)*A+conj(c)*C->C
  // c*A-a*C->A
  void bi_linear_combination(complex_double a, vector<complex_double> & A, complex_double c,
                             vector<complex_double> & C, int cstart, int cend) {
  ...
  void hessenberg_ortho(matrix_complex_double & H, matrix_complex_double & P, int firstrow, int n,
                        bool compute_P, int already_zero) {
    int nH = int(H.size());
    if (n < 0 || n > nH)
      n = nH;
    if (firstrow < 0 || firstrow > n)
  ...
  // a*A+c*C->A
  // c*A-a*C->C
  void bi_linear_combination(double a, vector<complex_double> & A, complex_double c,
                             vector<complex_double> & C) {
  ...
  void francis_iterate1(matrix_complex_double & H, int n1, int n2, matrix_complex_double & P,
                        double eps, bool compute_P, complex_double l1, bool finish) {
    if (debug_infolevel > 2)
      CERR << CLOCK() * 1e-6 << " iterate1 " << n1 << " " << n2 << '\n';
    int n_orig = int(H.size());
    ...
    if (finish) {
      // [[a,b],[c,d]] -> [b,l1-a] or [l1-d,c] as first eigenvector
      complex_double a = H[n2 - 2][n2 - 2], b = H[n2 - 2][n2 - 1], c = H[n2 - 1][n2 - 2],
                     d = H[n2 - 1][n2 - 1];
  ...
  bool in_francis_schur(matrix_complex_double & H, int n1, int n2, matrix_complex_double & P,
                        int maxiter, double eps, bool compute_P, matrix_complex_double & Haux,
                        bool only_one);

  void francis_iterate2(matrix_complex_double & H, int n1, int n2, matrix_complex_double & P,
                        double eps, bool compute_P, matrix_complex_double & Haux, bool only_one) {
    // int n_orig(H.size());
    // now H is proper hessenberg (indices n1 to n2-1)
    if (debug_infolevel > 2)
      CERR << CLOCK() * 1e-6 << " iterate2 " << n1 << " " << n2 << '\n';
    ...
    if (
      ...
      if (k >= n1 && k < n2 - 2) {
        ...
        if (debug_infolevel > 2)
          CERR << CLOCK() * 1e-6 << " recursive call dim " << d << " on ... [" << T[d - 2][d - 3]
               << "," << T[d - 2][d - 2] << "," << T[d - 2][d - 1] << " ][" << T[d - 1][d - 2]
               << "," << T[d - 1][d - 1] << "]" << '\n';
        int save_debug_infolevel = debug_infolevel;
        debug_infolevel = 0;
      ...
      if (n2 - n1 == 2 ||
          (ok > 1e-1 && n2 - n1 > 2 &&
           complex_abs(H[n2 - 2][n2 - 3]) < 1e-2 * complex_abs(H[n2 - 2][n2 - 2]))) {
        complex_double a = H[n2 - 2][n2 - 2], b = H[n2 - 2][n2 - 1], c = H[n2 - 1][n2 - 2],
                       d = H[n2 - 1][n2 - 1];
    ...
    for (int niter = 0; n2 - n1 > 1 && niter < maxiter; niter++) {
      ...
      for (int i = n2 - 2; i >= n1; --i) {
        ...
        if (ratio < coeff * eps) {
          ...
          if (debug_infolevel > 2)
            CERR << '\n'
                 << CLOCK() * 1e-6 << " Francis split complex "
                 << giacmin((i + 1) - n1, n2 - (i + 1)) << " [" << n1 << " " << i + 1 << " " << n2
                 << "]" << '\n';
          if (only_one && n2 - (i + 1) <= 2)
            return true;
          if (!only_one &&
              !in_francis_schur(H, n1, i + 1, P, maxiter, eps, compute_P, Haux, only_one)) {
  ...
  // Francis algorithm on submatrix rows and columns n1..n2-1
  // Invariant: trn(P)*H*P=orig matrix, complex_schur not used for giac_double coeffs
  bool francis_schur(matrix_complex_double & H, int n1, int n2, matrix_complex_double & P,
                     int maxiter, double eps, bool is_hessenberg, bool compute_P) {
    vecteur eigenv;
    int n_orig = int(H.size()); //,nitershift0=0;
    if (!is_hessenberg) {
      if (debug_infolevel > 0)
      ...
      hessenberg_householder(H,P,compute_P);
#else
      hessenberg_ortho(H, P, 0, n_orig, compute_P,
                       0); // insure Hessenberg form (on the whole matrix)
#endif
      if (debug_infolevel > 0)
