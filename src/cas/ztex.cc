...
namespace giac {
  ...
  bool is_clipped(double a, double xmin, double xmax, double b, double ymin, double ymax, double c,
  ...
  bool clip_line(double x1, double y1, double x2, double y2, double xmin, double ymin, double xmax,
                 double ymax, int mode, double & xa, double & ya, double & xb, double & yb) {
  ...
  static bool in_autoscale(const gen & g, vector<double> & vx, vector<double> & vy,
                           vector<double> & vz, GIAC_CONTEXT, bool curve = false) {
    ...
    if (g.type == _VECT) {
      if (!curve && g.subtype == _GROUP__VECT && !g._VECTptr->empty() &&
          g._VECTptr->front().type != _VECT) { // "compressed" hypersurface?
        vecteur v = *evalf_double(g, 1, contextptr)._VECTptr;
        int s = v.size();
        if (s % 3 == 0) {
          int i;
    ...
    if (g.is_symb_of_sommet(at_hyperplan)) {
      ...
      if (g0.type == _VECT && g0._VECTptr->size() >= 2) {
        ...
        if (n.type == _VECT && n._VECTptr->size() == 3 && P.type == _VECT &&
            P._VECTptr->size() == 3) {
          vecteur & N = *n._VECTptr;
          vecteur & M = *P._VECTptr;
          if (M[0].type == _DOUBLE_ && M[1].type == _DOUBLE_ && M[2].type == _DOUBLE_ &&
              N[0].type == _DOUBLE_ && N[1].type == _DOUBLE_ && N[2].type == _DOUBLE_) {
            double d = M[0]._DOUBLE_val,
                   r = (fabs(N[0]._DOUBLE_val) + fabs(N[1]._DOUBLE_val) + fabs(N[2]._DOUBLE_val)) /
                       3;
            vx.push_back(d - r);
            vx.push_back(d + r);
            d = M[1]._DOUBLE_val;
