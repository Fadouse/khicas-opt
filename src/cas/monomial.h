...
std::ostream & operator<<(std::ostream & os, const std::pair<T, U> & p) {
  ...
  inline bool is_zero(const longlong & a) {
    return a == 0;
  }
  inline bool is_zero(const long & a) {
    return a == 0;
  }
  inline bool is_zero(const int & a) {
    return a == 0;
  }
  ...
  inline void type_operator_plus_times_reduce(const double & a, const double & b, double & c,
                                              int reduce) {
    c += a * b;
  }
  ...
  inline void type_operator_plus_times_reduce(const longlong & a, const longlong & b, longlong & c,
                                              int reduce) {
    c += a * b;
    if (reduce)
      c %= reduce;
  ...
  inline void type_operator_reduce(const longlong & a, const longlong & b, longlong & c,
                                   int reduce) {
    c = a * b;
    if (reduce)
      c %= reduce;
  ...
  inline void type_operator_plus_times_reduce_nock(const int & b, const int & c, int & a,
                                                   int reduce) {
#ifdef _I386_ // a<-a+b*c mod m
  ...
  inline bool has_denominator(int a) {
    return false;
  }
  inline bool has_denominator(longlong a) {
    return false;
  }
  ...
  public:
    std::pointer_to_binary_function<const monomial<T> &, const monomial<T> &, bool>
        strictly_greater;
    sort_helper(
        const std::pointer_to_binary_function<const monomial<T> &, const monomial<T> &, bool>
            is_strictly_greater)
        : strictly_greater(is_strictly_greater) {};
    sort_helper()
        : strictly_greater(std::ptr_fun<const monomial<T> &, const monomial<T> &, bool>(
              m_lex_is_strictly_greater<T>)) {};
    bool operator()(const monomial<T> & a, const monomial<T> & b) {
      return strictly_greater(a, b);
    }
  };
...
#else
template <class T> std::ostream & operator<<(std::ostream & os, const monomial<T> & m) {
  return os << m.print();
}
#endif

  template <class T> bool operator==(const monomial<T> & a, const monomial<T> & b) {
  ...
  template <class T> monomial<T> Untrunc1(const T & t, int j = 0) {
    index_m new_i;
    new_i.push_back(j);
...
template <class T> std::ostream & operator<<(std::ostream & os, const std::vector<T> v) {
  ...
  */
  ...
  void Mul(typename std::vector<monomial<T>>::const_iterator & a,
           typename std::vector<monomial<T>>::const_iterator & a_end, const T & fact,
  ...
  template <class T>
  std::vector<monomial<T>> operator+(const std::vector<monomial<T>> & v,
                                     const std::vector<monomial<T>> & w) {
    typename std::vector<monomial<T>>::const_iterator a = v.begin(), a_end = v.end();
  ...
  struct ltindex {
    ...
    ltindex(bool (*my_is_strictly_greater)(const index_m &, const index_m &))
        : is_strictly_greater(my_is_strictly_greater) {};
  };
  ...
  void Mul(typename std::vector<monomial<T>>::const_iterator & ita,
    ...
    for (; it_a_cur != ita_end; ++it_a_cur) {
      ...
      for (it_b_cur = itb; it_b_cur != itb_end; ++it_b_cur) {
        ...
        for (it_aindex = it_aindexbeg; it_aindex != it_aindexend;
             ++it_bindex, ++it_sum_index, ++it_aindex)
    ...
    typedef std::map<index_t, T,
                     const std::pointer_to_binary_function<const index_m &, const index_m &, bool>>
        application;
    ...
    for (; ita_cur != ita_end; ++ita_cur) {
      ...
      for (itb_cur = itb; itb_cur != itb_end; ++itb_cur) {
        ...
        for (itaindex = itaindexbeg; itaindex != itaindexend;
             ++itbindex, ++itsommeindex, ++itaindex)
  ...
  if (debug_infolevel)
    CERR << "// " << CLOCK() << " Mul degree " << aa << "+" << bb << " size " << asize << "*"
         << bsize << "=" << asize * bsize << " max " << r << std::endl;
  new_coord.clear();
  ...
  std::vector<monomial<T>> operator*(const std::vector<monomial<T>> & v,
    ...
    typename std::vector<monomial<T>>::const_iterator b = w.begin(), b_end = w.end();
    std::vector<monomial<T>> res;
    Mul(a, a_end, b, b_end, res, i_lex_is_strictly_greater,
        std::ptr_fun<const monomial<T> &, const monomial<T> &, bool>(
            (m_lex_is_strictly_greater<T>)));
    return res;
  }

  template <class T>
  std::vector<monomial<T>> & operator*=(std::vector<monomial<T>> & v,
                                        const std::vector<monomial<T>> & w) {
    typename std::vector<monomial<T>>::const_iterator a = v.begin(), a_end = v.end();
    typename std::vector<monomial<T>>::const_iterator b = w.begin(), b_end = w.end();
    Mul(a, a_end, b, b_end, v, i_lex_is_strictly_greater,
        std::ptr_fun<const monomial<T> &, const monomial<T> &, bool>(
            (m_lex_is_strictly_greater<T>)));
    return v;
  }
  ...
  template <class T>
  void Shift(const std::vector<monomial<T>> & v, const index_m & i,
             std::vector<monomial<T>> & new_coord) {
    new_coord.clear();
  ...
  template <class T>
  void Shift(const std::vector<monomial<T>> & v, const index_m & i, const T & fois,
             std::vector<monomial<T>> & new_coord) {
    new_coord.clear();
    typename std::vector<monomial<T>>::const_iterator itend = v.end();
    if (is_one(fois)) {
  ...
  template <class T>
  void Shift(const std::vector<monomial<T>> & v, const T & fois, const index_m & i,
             std::vector<monomial<T>> & new_coord) {
    new_coord.clear();
    typename std::vector<monomial<T>>::const_iterator itend = v.end();
    for (typename std::vector<monomial<T>>::const_iterator it = v.begin(); it != itend; ++it)
  ...
  template <class T> T Content(const std::vector<monomial<T>> & v) {
  ...
  template <class T> T ppz(std::vector<monomial<T>> & p) {
    T n = Content(p);
    p = p / n;
    return n;
  }
  ...
  template <class T>
  void Nextcoeff(typename std::vector<monomial<T>>::const_iterator & it,
  ...
  template <class T> void Trunc1(const std::vector<monomial<T>> & c, std::vector<monomial<T>> & v) {
    v.reserve(c.size());
  ...
  template <class T> void Untruncn(std::vector<monomial<T>> & c, int j) {
  ...
  template <class T>
  void Apply(typename std::vector<monomial<T>>::const_iterator & it,
             const typename std::vector<monomial<T>>::const_iterator & itend, T (*f)(const T &),
             std::vector<monomial<T>> & v) {
    v.reserve(itend - it);
    T temp;
    for (; it != itend; ++it) {
  ...
  template <class T>
  bool Findpivot(std::vector<monomial<T>> & v, int rows, int cols, index_t & permut, int & pivotr,
                 int & pivotc, std::vector<monomial<T>> & pivotline, T * pivotcol, T & pivotcoeff) {
    bool found = false;
    T zero(0);
  ...
  template <class T>
  void Rref(std::vector<monomial<T>> & v, int rows, int cols, index_t & permut,
            bool dobareiss = true) {
    T bareisscoeff = 1;
    permut.clear();
    ...
    for (int pivotc = 1; pivotc <= cols; pivotc++) {
      ...
      typename std::vector<monomial<T>>::const_iterator it = v.begin(), itend = v.end(), tmpit,
                                                        tmpitend;
      while (it != itend) {
        int r = it->index.front();
  ...
  template <class T>
  void Findbeginofrows(typename std::vector<monomial<T>>::const_iterator & it,
                       const typename std::vector<monomial<T>>::const_iterator & itend,
                       const int & cols, typename std::vector<monomial<T>>::const_iterator * beg) {
  ...
  template <class T>
  void Findbeginofrows(typename std::vector<monomial<T>>::iterator & it,
                       const typename std::vector<monomial<T>>::const_iterator & itend,
                       const int & cols, typename std::vector<monomial<T>>::iterator * beg) {
    for (int j = 0; j <= cols; j++)
      beg[j] = 0;
    for (; it != itend;) {
      int row = it->index.front();
  ...
  template <class T>
  void Normalrref(std::vector<monomial<T>> & v, int rows, int cols, index_t & permut,
                  bool dobareiss = true) {
    ...
    typename std::vector<monomial<T>>::const_iterator * beg =
        new typename std::vector<monomial<T>>::const_iterator[cols + 1];
