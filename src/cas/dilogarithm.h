...
// directly to avoid losing z in the subtraction 1-z.
  ...
  static std::complex<double> khicas_dilog_numeric(khicas_dilog_complex z) {
    ...
    if (z == khicas_dilog_complex(1, 0))
      return khicas_dilog_complex(pi * pi / 6, 0);
    double radius = std::abs(z);
    if (radius <= 0.25) {
      khicas_dilog_complex sum = z, power = z;
      for (int n = 2; n <= 48; ++n) {
        power *= z;
        khicas_dilog_complex term = power / double(n * n);
        sum += term;
        if (std::abs(term) <= 2e-17 * std::abs(sum))
          break;
      }
      return sum;
    }
    if (radius > 1) {
      khicas_dilog_complex L = khicas_dilog_log(-z);
      return -khicas_dilog_numeric(khicas_dilog_complex(1, 0) / z) - pi * pi / 6. - L * L / 2.;
    }
    if (z.real() > 0.5)
      return pi * pi / 6. - khicas_dilog_log(z) * khicas_dilog_log(khicas_dilog_complex(1, 0) - z) -
             khicas_dilog_numeric(khicas_dilog_complex(1, 0) - z);
    static const double coefficients[] = {
        0.027777777777777776,    -0.00027777777777777778, 4.7241118669690098e-06,
        -9.1857730746619641e-08, 1.8978869988971001e-09,  -4.0647616451442256e-11,
        8.9216910204564523e-13,  -1.9939295860721074e-14, 4.5189800296199183e-16,
        -1.0356517612181247e-17, 2.395218621026187e-19,   -5.581785874325009e-21};
    khicas_dilog_complex w = -khicas_dilog_log(khicas_dilog_complex(1, 0) - z), w2 = w * w,
                         power = w * w2, sum = w - w2 / 4.;
  ...
  static gen d_Li2(const gen & z, GIAC_CONTEXT) {
    ...
    if (z.type <= _REAL || z.is_symb_of_sommet(at_exp))
      return regular;
  ...
  gen _Li2(const gen & argument, GIAC_CONTEXT) {
    if (argument.type == _STRNG && argument.subtype == -1)
      return argument;
    ...
    if (z.type == _VECT)
      return apply(z, _Li2, contextptr);
    if (is_undef(z))
      return z;
    if (is_zero(z))
      return z;
    if (taille(z, 129) <= 128 && has_num_coeff(z)) {
      gen value = evalf_double(z, 1, contextptr);
      khicas_dilog_complex input;
      if (value.type == _DOUBLE_)
        input = khicas_dilog_complex(value._DOUBLE_val, 0);
      else if (value.type == _CPLX) {
        gen r = evalf_double(re(value, contextptr), 1, contextptr),
            i = evalf_double(im(value, contextptr), 1, contextptr);
        if (r.type != _DOUBLE_ || i.type != _DOUBLE_)
          return symbolic(at_Li2, z);
        input = khicas_dilog_complex(r._DOUBLE_val, i._DOUBLE_val);
      } else
        return symbolic(at_Li2, z);
      if (!(std::abs(input.real()) <= 1.7976931348623157e308) ||
          !(std::abs(input.imag()) <= 1.7976931348623157e308))
        return undef;
      khicas_dilog_complex result = khicas_dilog_numeric(input);
      if (input.imag() == 0 && input.real() <= 1)
        return gen(result.real());
      return gen(result.real(), result.imag());
    }
    if (is_one(z))
      return cst_pi * cst_pi / 6;
    if (z == -1)
      return -cst_pi * cst_pi / 12;
    if (z == gen(1) / 2) {
      gen L = ln(gen(2), contextptr);
      return cst_pi * cst_pi / 12 - L * L / 2;
    }
    return symbolic(at_Li2, z);
  }
  static const char _Li2_s[] = "Li2";
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_quoted(__Li2, &_Li2, (size_t)&D_at_Li2unary_function_ptr,
                                            _Li2_s);
#else
