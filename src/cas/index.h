...
namespace giac {
  ...
  inline index_t operator*(int fois, const index_t & a) {
    return a * fois;
  }
  ...
  inline index_t index_min(const index_t & a, const index_t & b) {
    return index_gcd(a, b);
  }
  inline index_t index_max(const index_t & a, const index_t & b) {
  ...
  template <class T> bool lex_is_greater(const std::vector<T> & v1, const std::vector<T> & v2);
  template <class T>
  ...
  public:
    ...
    ref_index_t(index_t::const_iterator it, index_t::const_iterator itend)
        : ref_count(1), i(it, itend) {}
  };
...
const int POLY_VARS_OTHER = 0;
  ...
  const int POLY_VARS = POLY_VARS_DIRECT + POLY_VARS_OTHER - 1;
    ...
    defined __clang__ //|| defined FXCG // || defined(NSPIRE)
  class index_m {
  public:
    ref_index_t * riptr;
    // construct
    ...
    index_t::reverse_iterator rbegin() {
      return riptr->i.rbegin();
    }
    index_t::reverse_iterator rend() {
      return riptr->i.rend();
    }
    index_t::const_reverse_iterator rbegin() const {
      return riptr->i.rbegin();
    }
    index_t::const_reverse_iterator rend() const {
      return riptr->i.rend();
    }
#endif
    deg_t & front() {
    ...
    deg_t & operator[](size_t pos) {
      return *(begin() + pos);
    }
    deg_t operator[](size_t pos) const {
      return *(begin() + pos);
    }
    void clear() {
      riptr->i.clear();
...
#ifdef NSPIRE
    template <class T>
    friend nio::ios_base<T> & operator<<(nio::ios_base<T> & os, const index_m & m) {
      os << ":index_m:[ ";
...
public:
    ...
    if (im.taille % 2) {
      ...
      *(((size_t *)other) + 1) = *(((size_t *)im.other) + 1);
      ...
      size_t *target = (size_t *)other,
             *end = target + POLY_VARS_OTHER / (sizeof(size_t) / sizeof(deg_t));
  ...
  */
  ...
  public:
    size_t operator()(const index_t & v) const {
  ...
  inline index_m operator*(int fois, const index_m & a) {
    return a * fois;
  }
  index_m operator/(const index_m & a, int divisepar);
  inline int operator/(const index_m & a, const index_m & b) {
  ...
  inline int total_degree(const index_m & v1) {
  ...
  inline bool i_16var_is_strictly_greater(const index_m & v1, const index_m & v2) {
    return !i_16var_is_greater(v2, v1);
  }
  inline bool i_32var_is_strictly_greater(const index_m & v1, const index_m & v2) {
    return !i_32var_is_greater(v2, v1);
  }
  inline bool i_64var_is_strictly_greater(const index_m & v1, const index_m & v2) {
    return !i_64var_is_greater(v2, v1);
  }
  inline bool i_11var_is_strictly_greater(const index_m & v1, const index_m & v2) {
    return !i_11var_is_greater(v2, v1);
  }
  inline bool i_7var_is_strictly_greater(const index_m & v1, const index_m & v2) {
    return !i_7var_is_greater(v2, v1);
  }
  inline bool i_3var_is_strictly_greater(const index_m & v1, const index_m & v2) {
