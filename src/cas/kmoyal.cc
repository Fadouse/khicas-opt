...
namespace giac {
  ...
  gen Beta(const gen & a, const gen & b, GIAC_CONTEXT) {
    if (a.type == _DOUBLE_ || b.type == _DOUBLE_ || a.type == _CPLX || b.type == _CPLX) {
  ...
  */
  ...
  gen betad_icdf(const gen & alpha_orig, const gen & beta_orig, const gen & t_orig, GIAC_CONTEXT) {
    ...
    identificateur x(" x");
    return newton(symb_Beta(alpha, beta, x, 1) - y, x, x0, NEWTON_DEFAULT_ITERATION, 1e-5, 1e-12,
                  true, 1, 0, 1, 0, prefactor, contextptr);
  }
