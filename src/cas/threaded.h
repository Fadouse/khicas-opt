...
namespace giac {
  ...
  template <class U> inline unsigned sizeinbase2(U n) {
    unsigned i = 0;
    for (; n; ++i) {
      n >>= 1;
    }
  ...
  inline void type_operator_reduce(const mpz_class & a, const mpz_class & b, mpz_class & c,
                                   int reduce) {
    mpz_mul(c.get_mpz_t(), a.get_mpz_t(), b.get_mpz_t());
    if (reduce) {
      c %= reduce;
  ...
  inline void type_operator_plus_times_reduce(const mpz_class & a, const mpz_class & b,
                                              mpz_class & c, int reduce) {
    // c+=a*b % reduce;
    mpz_addmul(c.get_mpz_t(), a.get_mpz_t(), b.get_mpz_t());
    if (reduce) {
      c %= reduce;
  ...
  inline void type_operator_reduce(const int128_t & a, const int128_t & b, int128_t & c,
                                   int reduce) {
    c = a * b;
    if (reduce)
      c %= reduce;
  ...
  inline void type_operator_plus_times_reduce(const int128_t & a, const int128_t & b, int128_t & c,
                                              int reduce) {
    c += a * b;
    if (reduce)
      c %= reduce;
  ...
  void mulsmall(const std::vector<int>::const_iterator & ita0,
                const std::vector<int>::const_iterator & ita_end,
                const std::vector<int>::const_iterator & itb0,
                const std::vector<int>::const_iterator & itb_end, int modulo,
                std::vector<int> & new_coord);
  void mulext(const std::vector<int> & a, const std::vector<int> & b, const std::vector<int> & pmin,
              int modulo, std::vector<int> & res);
  ...
  inline void type_operator_times(const std::vector<int> & a, const std::vector<int> & b,
                                  std::vector<int> & c) {
    c.clear();
#ifndef NO_STDEXCEPT
  ...
  inline void type_operator_plus_times(const std::vector<int> & a, const std::vector<int> & b,
                                       std::vector<int> & c) {
    c.clear();
#ifndef NO_STDEXCEPT
  ...
  public:
    ...
    my_mpz(long l) {
      mpz_init_set_si(ptr, l);
    }
    my_mpz(const my_mpz & z) {
      mpz_init_set(ptr, z.ptr);
    }
    ~my_mpz() {
      mpz_clear(ptr);
    }
    my_mpz operator-() const {
      my_mpz tmp(*this);
      mpz_neg(tmp.ptr, tmp.ptr);
      return tmp;
    }
    my_mpz & operator=(const my_mpz & a) {
      mpz_set(ptr, a.ptr);
      return *this;
    }
  };
  ...
  inline void type_operator_plus_times_reduce(const my_mpz & a, const my_mpz & b, my_mpz & c,
                                              int reduce) {
    // c+=a*b % reduce;
    mpz_addmul(c.ptr, a.ptr, b.ptr);
    if (reduce)
  ...
  template <class T, class U> struct T_unsigned {
    T g;
    U u;
    T_unsigned(const T & myg, const U & myu) : g(myg), u(myu) {};
    T_unsigned() : g(0), u(0) {};
    bool operator==(const T_unsigned<T, U> & tu) const {
      return g == tu.g && u == tu.u;
    }
    bool operator!=(const T_unsigned<T, U> & tu) const {
      return g == tu.g && u != tu.u;
    }
...
std::ostream & operator<<(std::ostream & os, const std::vector<T_unsigned<T, U>> & v) {
  ...
  template <class T> class vector_size64 : public std::vector<T> {};
  template <class T> class vector_size32 : public std::vector<T> {};
#else
// "smart" vector, T must be a type of size 64bits, sizeof(T)=8
// and vector must be of size 192 bits, sizeof=24
template <class T> class vector_size64 {
public:
  longlong taille;
  T v0;
  T v1;
...
template <class T> struct mytab {
  T * tab;
  unsigned int _size;
  unsigned int _capacity;
};

template <class T> class vector_size32 {
public:
  struct {
    unsigned int taille;
    T v0;
  ...
  public:
    ...
    inline size_t operator()(unsigned x) const {
      return x;
    }
    inline size_t operator()(long long unsigned x) const {
      return x % 12345701;
    }
    inline size_t operator()(long long x) const {
      return x % 12345701;
    }
    hash_function_unsigned_object() {};
  };

  bool mod_gcd(const std::vector<T_unsigned<int, hashgcd_U>> & p0,
               const std::vector<T_unsigned<int, hashgcd_U>> & q0, int modulo,
               std::vector<T_unsigned<int, hashgcd_U>> & pgcd,
               std::vector<T_unsigned<int, hashgcd_U>> & pcof,
               std::vector<T_unsigned<int, hashgcd_U>> & qcof, const std::vector<hashgcd_U> & vars,
               bool compute_cofactors, int nthreads);

  bool mod_gcd(const std::vector<T_unsigned<int, hashgcd_U>> & p_orig,
               const std::vector<T_unsigned<int, hashgcd_U>> & q_orig, int modulo,
               std::vector<T_unsigned<int, hashgcd_U>> & d,
               std::vector<T_unsigned<int, hashgcd_U>> & pcofactor,
               std::vector<T_unsigned<int, hashgcd_U>> & qcofactor,
               const std::vector<hashgcd_U> & vars, bool compute_pcofactor, bool compute_qcofactor,
               int nthreads);

  bool gcd(const std::vector<T_unsigned<gen, hashgcd_U>> & p_orig,
           const std::vector<T_unsigned<gen, hashgcd_U>> & q_orig,
           std::vector<T_unsigned<gen, hashgcd_U>> & d,
           std::vector<T_unsigned<gen, hashgcd_U>> & pcofactor,
           std::vector<T_unsigned<gen, hashgcd_U>> & qcofactor, const std::vector<hashgcd_U> & vars,
           bool compute_cofactors, int nthreads = 1);

  bool gcd_ext(const std::vector<T_unsigned<gen, hashgcd_U>> & p_orig,
               const std::vector<T_unsigned<gen, hashgcd_U>> & q_orig,
               std::vector<T_unsigned<gen, hashgcd_U>> & d,
               std::vector<T_unsigned<gen, hashgcd_U>> & pcofactor,
               std::vector<T_unsigned<gen, hashgcd_U>> & qcofactor,
               const std::vector<hashgcd_U> & vars, bool compute_cofactors, int nthreads = 1);

  template <class T, class U>
  void smalladd(const std::vector<T_unsigned<T, U>> & v1, const std::vector<T_unsigned<T, U>> & v2,
  ...
  void smalladd(const std::vector<T_unsigned<T, U>> & v1, const std::vector<T_unsigned<T, U>> & v2,
                std::vector<T_unsigned<T, U>> & v, const R & reduce) {
    if (&v1 == &v || &v2 == &v) {
      std::vector<T_unsigned<T, U>> tmp;
  ...
  template <class T, class U>
  void smallsub(const std::vector<T_unsigned<T, U>> & v1, const std::vector<T_unsigned<T, U>> & v2,
                std::vector<T_unsigned<T, U>> & v) {
    if (&v1 == &v || &v2 == &v) {
      std::vector<T_unsigned<T, U>> tmp;
      smallsub(v1, v2, tmp);
  ...
  template <class T, class U>
  void smallmult(const T & g, const std::vector<T_unsigned<T, U>> & v1,
                 std::vector<T_unsigned<T, U>> & v) {
    if (is_zero(g)) {
      v.clear();
      return;
  ...
  template <class T, class U>
  void smalldiv(const std::vector<T_unsigned<T, U>> & v1, const T & g,
                std::vector<T_unsigned<T, U>> & v) {
    typename std::vector<T_unsigned<T, U>>::const_iterator it1 = v1.begin(), it1end = v1.end();
    if (&v1 == &v) {
      typename std::vector<T_unsigned<T, U>>::iterator it1 = v.begin(), it1end = v.end();
  ...
  template <class T, class U>
  void smallshift(const std::vector<T_unsigned<T, U>> & v1, U shift,
                  std::vector<T_unsigned<T, U>> & v) {
    typename std::vector<T_unsigned<T, U>>::const_iterator it1 = v1.begin(), it1end = v1.end();
    if (&v1 == &v) {
      typename std::vector<T_unsigned<T, U>>::iterator it1 = v.begin(), it1end = v.end();
  ...
  template <class T, class U, class R>
  void smallhorner(const std::vector<T_unsigned<T, U>> & v1, const T & g,
                   const std::vector<U> & vars, std::vector<T_unsigned<T, U>> & v,
                   const R & reduce) {
  ...
  template <class T, class U, class R>
  void smallmult(const std::vector<T_unsigned<T, U>> & v1, const std::vector<T_unsigned<T, U>> & v2,
                 std::vector<T_unsigned<T, U>> & v, const R & reduce, size_t possible_size);

  template <class T, class U, class R>
  void smallmulpoly_interpolate(const std::vector<T_unsigned<T, U>> & v1,
                                const std::vector<T_unsigned<T, U>> & v2,
                                std::vector<T_unsigned<T, U>> & v, const std::vector<U> & vars,
                                const index_t & vdeg, const R & reduce) {
  ...
  void smallmulpoly_interpolate(const std::vector<T_unsigned<T, U>> & v1,
                                const std::vector<T_unsigned<T, U>> & v2,
                                std::vector<T_unsigned<T, U>> & v, const index_t & vdeg,
                                const R & reduce) {
  ...
  template <class U> struct u_pair_index {
    U u;
    unsigned i1, i2;
    u_pair_index(unsigned _i1, unsigned _i2, U _u) : u(_u), i1(_i1), i2(_i2) {}
    u_pair_index() : u(0), i1(0), i2(0) {}
  };
  template <class U> inline bool operator<(const u_pair_index<U> & p1, const u_pair_index<U> & p2) {
    return p1.u < p2.u;
  }
  ...
  template <class T> void in_out_heap(T * tab, size_t size, T value) {
  ...
  template <class U> inline bool inverse_order(const U & u1, const U & u2) {
    return u1 < u2;
  }

  template <class U> struct U_unsigned {
    U u;
    unsigned v;
    U_unsigned(U _u, unsigned _v) : u(_u), v(_v) {}
    U_unsigned() : u(0) {}
  };
  template <class U> inline bool operator<(const U_unsigned<U> & p1, const U_unsigned<U> & p2) {
    return p1.u < p2.u;
  }
  ...
  void smallmult(const std::vector<T_unsigned<T, U>> & v1, const std::vector<T_unsigned<T, U>> & v2,
    ...
    typename std::vector<T_unsigned<T, U>>::const_iterator it1beg = v1.begin(), it1 = v1.begin(),
                                                           it1end = v1.end(), it2beg = v2.begin(),
                                                           it2, it2end = v2.end();
    T g1, g;
    ...
    if (heap_mult >= 0 && possible_size && u12 < 512e6 / sizeof(T) &&
        std::log(double(std::min(v1s, v2s))) / std::log(2.0) > 1 + 2 * u12 / v1v2) {
      ...
      for (u = 0; u <= u12; ++u)
        prod[u] = T(0);
      typename std::vector<T_unsigned<T, U>>::const_iterator slice_it2beg = it2beg,
                                                             slice_it2end = it2end;
    ...
    if (heap_mult < 0 || use_heap) {
      ...
      if (heap_mult == -1 || use_heap) {
        ...
        for (it1 = it1beg, heap0 = heap; heap0 != heapend; ++heap0, ++it1) {
          ...
          vindex[it1 - it1beg] = std::vector<std::pair<unsigned, unsigned>>(
              1, std::pair<unsigned, unsigned>(unsigned(it1 - it1beg), 0));
        ...
        for (; heapbeg != heapend;) {
          ...
          while (heapend != heapbeg && topu == heapbeg->u) {
            ...
            for (; it != itend; ++it) {
              ...
              type_operator_plus_times_reduce((it1beg + it->first)->g, (it2beg + its)->g, g,
                                              reduce);
      ...
      if (heap_mult == -2) {
        ...
        for (it1 = it1end - 1;;) {
          Mit = M.insert(
              Mit, std::pair<U, std::pair<unsigned, unsigned>>(
                       it1->u + u2, std::pair<unsigned, unsigned>(unsigned(it1 - it1beg), 0)));
          if (it1 == it1beg)
            break;
          --it1;
        }
        ...
        for (; !M.empty();) {
          ...
          if (Mitbeg->first != deg)
            ++Mitbeg;
          Mit = Mitbeg;
          type_operator_reduce((it1beg + Mit->second.first)->g, (it2beg + Mit->second.second)->g,
                               coeff, reduce);
          for (++Mit; Mit != Mend; ++Mit) {
            type_operator_plus_times_reduce((it1beg + Mit->second.first)->g,
                                            (it2beg + Mit->second.second)->g, coeff, reduce);
          }
          if (!is_zero(coeff))
          ...
          for (; vit != vitend; ++vit)
            M.insert(std::pair<U, std::pair<unsigned, unsigned>>(
                (it1beg + vit->first)->u + (it2beg + vit->second)->u, *vit));
        }
        return;
      }
      if (heap_mult == -3) { // using map of U -> chains
        ...
        for (it1 = it1end - 1;;) {
          Mit =
              M.insert(Mit, std::pair<U, std::vector<std::pair<unsigned, unsigned>>>(
                                it1->u + u2,
        ...
        while (!M.empty()) {
          ...
          std::vector<std::pair<unsigned, unsigned>> nouveau;
          for (; it != itend; ++it) {
            type_operator_plus_times_reduce((it1beg + it->first)->g, (it2beg + it->second)->g, g,
                                            reduce);
            ++it->second;
            if (it->second != v2s)
              nouveau.push_back(*it);
          ...
          for (; it != itend; ++it) {
            ...
            if (Mit != M.end())
              ...
              M[u] = std::vector<std::pair<unsigned, unsigned>>(
                  1, std::pair<unsigned, unsigned>(it->first, it->second));
          }
        }
        return;
      }
      ...
      for (; heapbeg != heapend;) {
        if (!v.empty() && v.back().u == heapbeg->u) {
          type_operator_plus_times_reduce((it1beg + heapbeg->i1)->g, (it2beg + heapbeg->i2)->g,
                                          v.back().g, reduce);
          if (is_zero(v.back().g))
            v.pop_back();
        } else {
  ...
  template <class T, class U, class R> struct threadmult_t {
  ...
  template <class T, class U, class R> void * do_threadmult(void * ptr) {
    ...
    typename std::vector<T_unsigned<T, U>>::const_iterator it1 = v1->begin(), it1beg = v1->begin(),
                                                           it1end = v1->end(), it2beg, it2, it2end;
    T g1, g;
    U u1, u2, u, degdiv = argptr->degdiv;
    int d1 = 0, d2, v2deg = v2ptrs->size() - 2, v1deg = argptr->v1ptrs->size() - 2,
        d = argptr->current_deg;
    T * prod = argptr->prod;
    if (prod) {
      for (int slice_d2 = 0; slice_d2 <= v2deg; ++slice_d2) {
        ...
        typename std::vector<T_unsigned<T, U>>::const_iterator slice_it2beg = (*v2ptrs)[slice_d2],
                                                               slice_it2end =
                                                                   (*v2ptrs)[slice_d2 + 1];
        ...
        for (; slice_it2beg < slice_it2end; slice_it2beg = it2end) {
          ...
          for (it1 = it1beg; it1 != it1end; ++it1) {
            ...
            if ((it1 + 3) < it1end) {
              ...
              T *prod0 = prod + u1, *prod1 = prod + (it1 + 1)->u, *prod2 = prod + (it1 + 2)->u,
                *prod3 = prod + (it1 + 3)->u;
    ...
    if ( //false
      ...
      for (it1 = it1beg, heap0 = heap; it1 != it1end; ++it1) {
        ...
        if (smallindex) {
          ...
          (*vsmallindexptr)[count].push_back(
              std::pair<unsigned short, unsigned short>(it1 - it1beg, 0));
        } else {
  ...
  template <class T, class U, class R>
  bool threadmult(const std::vector<T_unsigned<T, U>> & v1,
                  const std::vector<T_unsigned<T, U>> & v2, std::vector<T_unsigned<T, U>> & v,
                  U degdiv, const R & reduce, size_t possible_size = 100) {
    if (!threads_allowed)
      return false;
    v.clear();
    ...
    U u12 = u1 + u2; // size of the array for array multiplication
    // compare u12 and v1v2*ln(v1v2)
    if (heap_mult >= 0 && possible_size > 100 && u12 < 512e6 / sizeof(T) &&
        u12 < v1v2 * std::log(double(possible_size)) * 2) {
      if (debug_infolevel > 20)
        CERR << "array multiplication, v1 size " << v1s << " v2 size " << v2s << " u1+u2 " << u12
             << std::endl;
      // array multiplication
    ...
    if (
      ...
      std::vector<vector_size64<std::pair<unsigned, unsigned>>> * vindexptr =
          smallindex ? 0 : (new std::vector<vector_size64<std::pair<unsigned, unsigned>>>(v1si));
      std::vector<vector_size32<std::pair<unsigned short, unsigned short>>> * vsmallindexptr =
          smallindex
              ? (new std::vector<vector_size32<std::pair<unsigned short, unsigned short>>>(v1si))
              : 0;
      for (; i >= 0; --i) {
        arg[i].v1ptr = &v1;
        arg[i].v1ptrs = &v1it;
      ...
      for (int j = 0; i >= 0 && j < nthreads; --i, ++j) {
        ...
        if (use_heap) {
          ...
          arg[i].vindexptr =
              smallindex ? 0
                         : (new std::vector<vector_size64<std::pair<unsigned, unsigned>>>(v1si));
          arg[i].vsmallindexptr =
              smallindex
                  ? (new std::vector<vector_size32<std::pair<unsigned short, unsigned short>>>(
                        v1si))
                  : 0;
        } else {
          arg[i].use_heap = false;
        ...
        res = pthread_create(&tab[i], (pthread_attr_t *)NULL, do_threadmult<T, U, R>,
                             (void *)&arg[i]);
        if (res) {
    ...
    if (prod) {
      ...
      for (int i = deg1v; i >= 0; --i) {
        typename std::vector<T_unsigned<T, U>>::const_iterator it = arg[i].vptr->begin(),
                                                               itend = arg[i].vptr->end();
        for (; it != itend; ++it) {
          v.push_back(*it);
        }
        delete arg[i].vptr;
...
#else // PTHREAD
template <class T, class U, class R>
bool threadmult(const std::vector<T_unsigned<T, U>> & v1, const std::vector<T_unsigned<T, U>> & v2,
                std::vector<T_unsigned<T, U>> & v, U degdiv, const R & reduce,
                size_t possible_size = 100) {
  return false;
}

#endif // PTHREAD

  template <class U> void partial_degrees2vars(const index_t & d, std::vector<U> & vars) {
    int dim = int(d.size());
    vars[dim - 1] = 1;
  ...
  template <class U> void partial_degrees(U u, const std::vector<U> & vars, index_t & res) {
  ...
  template <class T, class U>
  void partial_degrees(const std::vector<T_unsigned<T, U>> & a, const std::vector<U> & vars,
                       index_t & res) {
  ...
  template <class T, class U>
  void convert(std::vector<T_unsigned<T, U>> & a, const std::vector<U> & source,
  ...
  inline bool hashdivrem_finish_later(longlong a) {
    return false;
  }
  inline bool hashdivrem_finish_later(double a) {
    return false;
  }
  inline bool hashdivrem_finish_later(int a) {
    return false;
  }
  inline bool hashdivrem_finish_later(const vecteur & v) {
    return false;
  }
  inline bool hashdivrem_finish_later(const std::vector<int> & v) {
    return false;
  }
#ifdef INT128
  inline bool hashdivrem_finish_later(int128_t a) {
    return false;
  }
#endif

  inline bool hashdivrem_finish_later(const gen & a) {
    return true;
  }
  inline bool hashdivrem_finish_later(const my_mpz & a) {
    return true;
  }
#ifdef HAVE_GMPXX_H
  inline bool hashdivrem_finish_later(const mpz_class & a) {
    return true;
  }
#endif

  template <class U> inline bool one_index_smaller(U u, U v, const std::vector<int> & varsshift) {
    if (u < v)
      return true;
    return false;
    /*
  ...
  template <class T, class U, class R>
  int hashdivrem(const std::vector<T_unsigned<T, U>> & a, const std::vector<T_unsigned<T, U>> & b,
                 std::vector<T_unsigned<T, U>> & q, std::vector<T_unsigned<T, U>> & r,
                 const std::vector<U> & vars, const R & reduce, double qmax, bool allowrational,
                 int quo_only = 0) {
    ...
    typename std::vector<T_unsigned<T, U>>::const_iterator it2, it2end,
        itbbeg = b.begin(), itbend = b.end(), ita = a.begin(), itaend = a.end(), cit, citend, qbeg;
    T binv = b.front().g;
    if (!is_zero(reduce))
    ...
    if ((!quo_only || quo_only == 3) &&
      ...
      if (debug_infolevel > 1)
        CERR << CLOCK() * 1e-6 << " array division, a size " << a.size() << " b size " << b.size()
             << " u " << umax << std::endl;
      // array division
      ...
      for (rdeg = adeg; rdeg >= bdeg; --rdeg) {
        ...
        if (vars.size() == 1) {
          ...
          T res;
          if (!is_zero(reduce))
            type_operator_reduce(maincoeff.front().g, binv, res,
                                 reduce); // res=smod(maincoeff.front().g*binv,reduce);
          else {
          ...
          int recdivres =
              hashdivrem(maincoeff, lcoeffb, quo, tmp, std::vector<U>(vars.begin() + 1, vars.end()),
                         reduce, qmax, allowrational);
          if (recdivres < 1) {
            delete[] rem;
            return recdivres;
          }
    ...
    bool use_heap = false && (heap_mult > 0 && v1v2 >= heap_mult);
#if 1 // heap division
    if (heap_mult < 0 || use_heap || (quo_only && quo_only < 3)) {
...
#endif
      if (debug_infolevel > 1)
        CERR << CLOCK() * 1e-6 << " heap division, a size " << a.size() << " b size " << b.size()
             << " vars " << vars << std::endl;
      // heap division:
      ...
      std::vector<std::vector<std::pair<unsigned, unsigned>>> vindex(
          bs); // ,std::vector< std::pair<unsigned,unsigned> >(4));
            ...
            if (heapu >= bu) {
...
#endif
                  unsigned & its = it->second;
                  type_operator_plus_times_reduce((itbbeg + it->first)->g, (qbeg + its)->g, g,
                                                  reduce);
                  // increment 2nd poly index of the elements of the top chain
                  ++its;
                  if (its < qsize) {
              ...
              } // while heapend!=heapbeg && heapu==
              {
                ...
                for (; it != itend; ++it) {
                  ...
                  if (norem && one_index_smaller(u, bu, varsshift))
                    continue;
                  // check if u is in the path to the root of the heap
                  unsigned holeindex = unsigned(heapend - heapbeg), parentindex;
                  if (holeindex && u == heapbeg->u) {
...
#ifdef HEAP_STATS
      if (debug_infolevel)
        CERR << "chain " << chain << ", nochain " << nochain << ", type_op_reduce " << typeopreduce
             << " null quotients" << nullq << std::endl;
#endif
      // r still empty
      ...
      for (; heapbeg != heapend;) {
        ...
        while (heapbeg != heapend && heapbeg->u == heapu) {
          ...
          for (; it != itend; ++it) {
            unsigned & its = it->second;
            type_operator_plus_times_reduce((itbbeg + it->first)->g, (q.begin() + its)->g, g,
                                            reduce);
            // increment 2nd poly index of the elements of the top chain
            ++its;
            if (its < q.size())
  ...
  std::vector<hash_prod> produit(adeg + 1);
    ...
    for (rdeg = adeg; rdeg >= bdeg; --rdeg) {
      ...
      std::vector<T_unsigned<T, U>> maincoeff, quo, tmp;
      U ushift = U(rdeg) << mainvar;
      for (prod_it = produit[rdeg].begin(), prod_itend = produit[rdeg].end(); prod_it != prod_itend;
           ++prod_it) {
    ...
    for (int i = bdeg - 1; i >= 0; --i) {
      for (prod_it = produit[i].begin(), prod_itend = produit[i].end(); prod_it != prod_itend;
           ++prod_it) {
        if (!is_zero(gu.g = prod_it->second)) {
          gu.u = prod_it->first;
          r.push_back(gu);
  ...
  template <class T, class U>
  T peval_x1_xn(typename std::vector<T_unsigned<T, U>>::const_iterator it,
                typename std::vector<T_unsigned<T, U>>::const_iterator itend,
                const typename std::vector<T> & v, const typename std::vector<U> & vars,
                const T & reduce) {
    if (vars.empty())
      return it->g;
  ...
  void peval_x2_xn(const typename std::vector<T_unsigned<T, U>> & p,
                   const typename std::vector<T> & v, const std::vector<U> & vars,
                   std::vector<T_unsigned<T, U>> & res, const T & reduce) {
