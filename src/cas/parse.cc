...
static void python_import(string & cur, int cs, int posturtle, int poscmath, int posmath,
  ...
  if (posmatplotlib >= 0 && posmatplotlib < cs) {
    cur += "np:=numpy:;xlim(a,b):=gl_x=a..b:;ylim(a,b):=gl_y=a..b:;scatter:=scatterplot:;bar:=bar_"
           "plot:;text:=legend:;xlabel:=gl_x_axis_name:;ylabel:=gl_y_axis_name:;arrow:=vector:;"
           "boxplot:=moustache:;";
  ...
  if (posturtle >= 0 && posturtle < cs) {
    ...
    if (alertturtle) {
      alertturtle = false;
      alert("pu:=penup;up:=penup; pd:=pendown;down:=pendown; fd:=forward;bk:=backward; rt:=right; "
    ...
    for (pos = 0; pos < int(cur.size()); ++pos) {
      ...
      if (!instring && pythoncompat && ch == '{' && (pos == 0 || cur[pos - 1] != '\\')) {
        ...
        if (p < cs
          ...
          cur = cur.substr(0, pos) + "{/" +
                replace_deuxpoints_egal(cur.substr(pos + 1, p - 1 - pos)) + "/}" +
                cur.substr(p + 1, cs - pos - 1);
        }
      }
      if (!instring && pythoncompat && ch == '\'' && pos < cur.size() - 2 && cur[pos + 1] != '\\' &&
          (pos == 0 ||
           (cur[pos - 1] != '\\' && cur[pos - 1] != '\''))) { // workaround for '' string delimiters
      ...
      if (ch == 'i' && pos + 7 < int(cur.size()) && cur.substr(pos, 7) == "import ") {
        ...
        if (posp >= posi || posp < 0)
          posp = posi;
        if (posi > pos + 5 && posi < int(cur.size())) {
          cur = cur.substr(posi + 4, cur.size() - posi - 4) +
                ":=" + cur.substr(pos + 7, posp - (pos + 7)) + ';';
        } else
      ...
      if (ch == 'l' && pos + 6 < int(cur.size()) && cur.substr(pos, 6) == "lambda" &&
        ...
        if (posdot > pos + 7 && posdot < int(cur.size()) - 1 && cur[posdot + 1] != '=' &&
            cur[posdot + 1] != ';') {
          pythonmode = true;
          cur = cur.substr(0, pos) + "(" + cur.substr(pos + 6, posdot - pos - 6) + ")->" +
                cur.substr(posdot + 1, cur.size() - posdot - 1);
        }
      }
