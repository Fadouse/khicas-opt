...
struct tinymt32_t {
  ...
  uint32_t mat2;
  uint32_t tmat;
  tinymt32_t(uint32_t seed = 1234567) {
    tinymt32_init(this, seed);
  }
};
...
#endif
  t0 ^= t1;
  conv.u = ((t0 ^ (-((int32_t)(t1 & 1)) & random->tmat)) >> 9) | uint32_t(0x3f800000);
  return conv.f;
}
...
#endif
  t0 ^= t1;
  conv.u = ((t0 ^ (-((int32_t)(t1 & 1)) & random->tmat)) >> 9) | uint32_t(0x3f800001);
  return conv.f;
}
