...
namespace giac {
  ...
  gen csturm_seq(modpoly & r0, modpoly & r1, vecteur & listquo, vecteur & coeffP, vecteur & coeffR,
                 GIAC_CONTEXT) {
    listquo.clear();
    coeffP.clear();
    coeffR.clear();
  ...
  static gen csturm_horner(const modpoly & p, const gen & a) {
    if (p.size() == 1 && p.front().type == _POLY && p.front()._POLYptr->dim == 1) {
      ...
      vector<monomial<gen>>::const_iterator it = p.front()._POLYptr->coord.begin(),
                                            itend = p.front()._POLYptr->coord.end();
  ...
  static int csturm_vertex_ab(const modpoly & r0, const modpoly & r1, const vecteur & listquo,
                              const vecteur & coeffP, const vecteur & coeffR, const gen & a,
                              int start, GIAC_CONTEXT) {
  ...
  static int csturm_realroots(const modpoly & S, const modpoly & R, const vecteur & listquo,
                              const vecteur & coeffP, const vecteur & coeffR, const gen & a,
                              const gen & b, const gen & t0, const gen & t1, vecteur & realroots,
                              double eps, GIAC_CONTEXT) {
    if (is_inf(t0)) // replace with max(R)
      return csturm_realroots(S, R, listquo, coeffP, coeffR, a, b, -linfnorm(R, contextptr), t1,
                              realroots, eps, contextptr);
    if (is_inf(t1)) // replace with max(R)
      return csturm_realroots(S, R, listquo, coeffP, coeffR, a, b, t0, linfnorm(R, contextptr),
                              realroots, eps, contextptr);
    ...
    */
    if (n == 1) {
      ...
      int s2;
      gen delta =
          evalf_double(log((T1 - T0) * abs(b, contextptr) / eps, contextptr) / log(2., contextptr),
                       1, contextptr);
    ...
    if (csturm_realroots(S, R, listquo, coeffP, coeffR, a, b, T0, t01, realroots, eps,
                         contextptr) == -2)
      return -2;
    if (csturm_realroots(S, R, listquo, coeffP, coeffR, a, b, t01, T1, realroots, eps,
                         contextptr) == -2)
      return -2;
    return n;
  }
  ...
  static vecteur csturm_segment_seq(const modpoly & P, const gen & a, const gen & b,
                                    vecteur & realroots, double eps, vecteur & horiz_sturm,
                                    vecteur & vert_sturm, GIAC_CONTEXT) {
    ...
    */
    ...
    int typeseq = -1;
    if (debug_infolevel)
      *logptr(contextptr) << "segment " << a << ".." << b << ", im/re:" << S << "|" << R
                          << ", gcd:" << g << '\n';
    if (g.type == _VECT && g._VECTptr->size() == P.size()) {
      ...
      typeseq = csturm_realroots(S, R, listquo, coeffP, coeffR, a, b - a, 0, 1, realroots, eps,
                                 contextptr);
      if (typeseq == -2)
        return realroots;
    }
  ...
  static bool csturm_square_seq(const modpoly & P, const gen & a0, const gen & b0, const gen & a1,
                                const gen & b1, gen & pgcd, vecteur & realroots, double eps,
                                vecteur & seq1, vecteur & seq2, vecteur & seq3, vecteur & seq4,
                                vecteur & horiz_sturm, vecteur & vert_sturm, GIAC_CONTEXT) {
  ...
  // returns -1 on error
  static int csturm_square(const modpoly & P, const gen & a0, const gen & b0, const gen & a1,
                           const gen & b1, const vecteur & seq1, const vecteur & seq2,
                           const vecteur & seq3, const vecteur & seq4, GIAC_CONTEXT) {
    int ind, tmp;
    ind = 0;
    gen A = a0 + cst_i * b0, B = a1 + cst_i * b0;
  ...
  static void csturm_normalize(modpoly & p, const gen & a0, const gen & b0, const gen & a1,
                               const gen & b1, vecteur & roots) {
    int n = int(p.size()) - 1;
    ...
    if (n % 2) {
      ...
      if (n1) {
        ...
        if (is_greater(a, a0, context0) && is_greater(b, b0, context0) &&
            is_greater(a1, a, context0) && is_greater(b1, b, context0))
  ...
  void ab2a0b0a1b1(const gen & a, const gen & b, gen & a0, gen & b0, gen & a1, gen & b1,
                   GIAC_CONTEXT) {
    a0 = re(a, contextptr);
    b0 = im(a, contextptr);
    a1 = re(b, contextptr);
    b1 = im(b, contextptr);
    if (ck_is_greater(a0, a1, contextptr))
      swapgen(a0, a1);
    if (ck_is_greater(b0, b1, contextptr))
      swapgen(b0, b1);
  }
  ...
  int csturm_square(const gen & p, const gen & a, const gen & b, gen & pgcd, GIAC_CONTEXT) {
    ...
    if (!realroots.empty()) {
      ...
      if ((is_zero(ry) && (rx == a0 || rx == a1)) || (is_zero(rx) && (ry == b0 || ry == b1)))
        ;
      else
        evident = 1;
    }
    ...
    if (!csturm_square_seq(P, a0, b0, a1, b1, pgcd, realroots, 0.0, seq1, seq2, seq3, seq4,
                           horiz_seq, vert_seq, contextptr)) {
      if (pgcd.type != _VECT)
        return -1;
  ...
  static void complex_roots(const modpoly & P, const gen & a0, const gen & b0, const gen & a1,
                            const gen & b1, vecteur & realroots, vecteur & complexroots,
                            double eps);

  static bool complex_roots_split(const modpoly & P, const gen & pgcd, const gen & a0,
                                  const gen & b0, const gen & a1, const gen & b1,
                                  vecteur & realroots, vecteur & complexroots, double eps) {
    if (pgcd.type != _VECT)
      return false;
    modpoly g = (*pgcd._VECTptr) / pgcd[0];
    // true factorization found, restart with each factor
  ...
  void in_round2(gen & x, const gen & deuxn, int n) {
    ...
    if (x.type == _FRAC && x._FRACptr->den.type == _CPLX)
      x = fraction(x._FRACptr->num * conj(x._FRACptr->den, context0),
                   x._FRACptr->den.squarenorm(context0));
    if (x.type == _FRAC && x._FRACptr->den.type == _ZINT &&
        (x._FRACptr->num.type == _ZINT ||
         (x._FRACptr->num.type == _CPLX && x._FRACptr->num._CPLXptr->type == _ZINT &&
          (x._FRACptr->num._CPLXptr + 1)->type == _ZINT))) {
  ...
  bool newton_complex_1root(const modpoly & P, const gen & a0, const gen & b0, const gen & a1,
                            const gen & b1, vecteur & complexroots, double eps) {
    if (is_positive(a1 - a0 - 0.01, context0) || is_positive(b1 - b0 - 0.01, context0))
      return false;
  ...
  static int complex_roots(const modpoly & P, const gen & a0, const gen & b0, const gen & a1,
                           const gen & b1, const vecteur & seq1, const vecteur & seq2,
                           const vecteur & seq3, const vecteur & seq4, vecteur & realroots,
                           vecteur & complexroots, double eps, vecteur & horiz_sturm,
                           vecteur & vert_sturm) {
    ...
    if (n <= 0)
      return 2 * n;
    if (eps <= 0) {
      *logptr(context0) << gettext("Bad precision, using 1e-12 instead of ") +
                               print_DOUBLE_(eps, 14)
                        << '\n';
      eps = 1e-12;
    }
    if (is_strictly_greater(eps, a1 - a0, context0) &&
        is_strictly_greater(eps, b1 - b0, context0)) {
     ...
     */
    int n1 = complex_roots(P, a0, b0, a01, b01, seq1, seqvert, seqhoriz, seq4, realroots,
                           complexroots, eps, horiz_sturm, vert_sturm),
        nadd;
    if (n1 == -2)
      return -2;
    if (n1 == n)
      return n;
    n1 += (nadd = complex_roots(P, a01, b0, a1, b01, seq1, seq2, seqhoriz, seqvert, realroots,
                                complexroots, eps, horiz_sturm, vert_sturm));
    if (nadd == -2)
      return -2;
    if (n1 == n)
      return n;
    n1 += (nadd = complex_roots(P, a01, b01, a1, b1, seqhoriz, seq2, seq3, seqvert, realroots,
                                complexroots, eps, horiz_sturm, vert_sturm));
    if (nadd == -2)
      return -2;
    if (n1 == n)
      return n;
    n1 += (nadd = complex_roots(P, a0, b01, a01, b1, seqhoriz, seqvert, seq3, seq4, realroots,
                                complexroots, eps, horiz_sturm, vert_sturm));
    if (nadd == -2)
      return -2;
    return n;
  ...
  static void complex_roots(const modpoly & P, const gen & a0, const gen & b0, const gen & a1,
    ...
    gen pgcd;
    if (!csturm_square_seq(P, a0, b0, a1, b1, pgcd, realroots, eps, Seq1, Seq2, Seq3, Seq4,
                           horiz_sturm, vert_sturm, context0))
      complex_roots_split(P, pgcd, a0, b0, a1, b1, realroots, complexroots, eps);
    else
      complex_roots(P, a0, b0, a1, b1, Seq1, Seq2, Seq3, Seq4, realroots, complexroots, eps,
                    horiz_sturm, vert_sturm);
  }

  // Find complex roots of P in a0,b0 -> a1,b1
  bool complex_roots(const modpoly & P, const gen & a0, const gen & b0, const gen & a1,
                     const gen & b1, gen & pgcd, vecteur & roots, double eps) {
  ...
  vecteur keep_in_rectangle(const vecteur & croots, const gen A0, const gen & B0, const gen & A1,
                            const gen & B1, bool embed, GIAC_CONTEXT) {
    vecteur roots;
    const_iterateur it = croots.begin(), itend = croots.end();
    for (; it != itend; ++it) {
      gen a = re(*it, contextptr), b = im(*it, contextptr);
      if (is_greater(a, A0, contextptr) && is_greater(A1, a, contextptr) &&
          is_greater(b, B0, contextptr) && is_greater(B1, b, contextptr))
  ...
  bool newton_improve(const vecteur & P, const vecteur & P1, bool Preal, vecteur & v,
                      vecteur & vradius, int i, int kmax, int n, int epsn, const gen & epsg2surdeg2,
                      const gen & epsg) {
    gen r = v[i];
    bool nextconj = false;
    ...
    if (r.type == _CPLX && r._CPLXptr->type == _REAL &&
        mpfr_get_prec(r._CPLXptr->_REALptr->inf) > N)
    ...
    for (int k = 0; k < kmax; ++k) {
      ...
      if (is_greater(epsg2surdeg2, delta, context0)) {
        ...
        if (!is_exactly_zero(vradius[i]))
          vradius[i] = min(
              epsg,
              pow(plus_two, int(evalf_double(ln(vradius[i], context0), 1, context0)._DOUBLE_val /
                                std::log(2.)) +
                                1),
              context0);
        if (debug_infolevel)
      ...
      in_round2(Pr, deuxN, N);
      in_round2(P1r, deuxN, N); // round2(Pr,deuxN,context0); round2(P1r,deuxN,context0);
      r -= Pr / P1r;
    } // end for k
#endif
  ...
  vecteur complex_roots(const modpoly & P, const gen & a0, const gen & b0, const gen & a1,
                        const gen & b1, bool complexe, double eps, bool use_proot) {
    if (P.empty())
      return P;
    eps = absdouble(eps);
    if (eps > 1e-6)
          ...
          for (unsigned j = 0; j < v.size(); ++j) {
            if (Preal && is_zero(im(v[j], context0))) {
              if (is_exactly_zero(vradius[j]) || vradius[j] == -1) {
                if (is_greater(v[j], a0, context0) && is_greater(a1, v[j], context0) &&
                    is_greater(0, b0, context0) && is_greater(b1, 0, context0))
                  res.push_back(makevecteur(v[j], 1));
                continue;
              }
              gen P1 = horner(P, v[j] - vradius[j], 0, false),
                  P2 = horner(P, v[j] + vradius[j], 0, false);
              if (P1.type == _FRAC)
                P1 = P1._FRACptr->num;
              if (P2.type == _FRAC)
                P2 = P2._FRACptr->num;
              if (is_strictly_positive(-P1 * P2, context0)) {
                if (is_greater(v[j], a0, context0) && is_greater(a1, v[j], context0) &&
                    is_greater(0, b0, context0) && is_greater(b1, 0, context0))
                  res.push_back(makevecteur(
                      eval(change_subtype(makevecteur(v[j] - vradius[j], v[j] + vradius[j]),
                                          _INTERVAL__VECT),
                           1, context0),
                      1));
                continue;
            ...
            if (is_greater(R, a0, context0) && is_greater(a1, R, context0) &&
                is_greater(I, b0, context0) && is_greater(b1, I, context0)) {
              if (is_exactly_zero(vradius[j]))
                ...
                res.push_back(makevecteur(
                    eval(change_subtype(makevecteur(a - vradius[j], a + vradius[j]),
                                        _INTERVAL__VECT),
                         1, context0) +
                        cst_i * eval(change_subtype(makevecteur(b - vradius[j], b + vradius[j]),
                                                    _INTERVAL__VECT),
                                     1, context0),
                    1));
#else
              res.push_back(makevecteur(makevecteur(ratnormal(v[j] - vradius[j] * (1 + cst_i)),
                                                    ratnormal(v[j] + vradius[j] * (1 + cst_i))),
                                        1));
#endif
              }
            }
  ...
  gen complexroot(const gen & g, bool complexe, GIAC_CONTEXT) {
    ...
    if (use_vas && vas(symb2poly_num(v[0], contextptr), a0, a1, isolation ? 1e300 : eps, vas_res,
                       true, contextptr))
      return vas_res;
    vecteur l, l0;
    lidnt(p, l0, false);
  ...
  gen _realroot(const gen & g, GIAC_CONTEXT) {
    if (g.type == _STRNG && g.subtype == -1)
      return g;
    gen res;
    bool evalf_after = false;
    if (g.type == _VECT && !g._VECTptr->empty() && g._VECTptr->back() == at_evalf) {
      res = complexroot(gen(vecteur(g._VECTptr->begin(), g._VECTptr->end() - 1), g.subtype), false,
                        contextptr);
      evalf_after = true;
    } else
...
#define M_LN2 0.6931471805599454
  ...
  static bool compute_lnabsmantexpo(const vecteur & cl, vector<double> & cllnabsmant,
                                    vector<long int> & clexpo, vector<short int> & clsign,
                                    GIAC_CONTEXT) {
    int k = int(cl.size());
      ...
      if (clsign[m - 1] == -1) { // is_strictly_positive(-cl[m-1],contextptr)
        ...
        for (int n = k; n > m; --n) {
          if (clsign[n - 1] == 1) { // is_strictly_positive(cl[n-1],contextptr)
            gen temp = (cllnabsmant[m - 1] - cllnabsmant[n - 1] +
                        (clexpo[m - 1] - clexpo[n - 1] + timesused[n - 1]) * M_LN2) /
                       (n - m); // LMQ_evalf(cl[m-1],cl[n-1],timesused[n-1],n-m,contextptr);
      ...
      if (clsign[m] == -1) { // is_strictly_positive(-cl[m],contextptr)
        ...
        for (int n = 0; n < m; ++n) {
          if (clsign[n] == 1) { // is_strictly_positive(cl[n],contextptr)
            ...
            // temp=evalf(temp,1,contextptr);
            gen temp =
                (cllnabsmant[m] - cllnabsmant[n] + (clexpo[m] - clexpo[n] + timesused[n]) * M_LN2) /
                (m - n); // LMQ_evalf(cl[m],cl[n],timesused[n],m-n,contextptr);
            ++timesused[n];
            if (is_strictly_greater(tempmin, temp, contextptr))
              tempmin = temp;
          }
  ...
  vecteur VAS_positive_roots(const modpoly & P, const gen & ap, const gen & bp, const gen & cp,
                             const gen & dp, GIAC_CONTEXT) {
    matrice Pascal;
  ...
  static bool bisection_newton(const modpoly & P, const modpoly & Pprime, const gen & a0,
                               const gen & a1, gen & x0, double eps, gen & eps2, GIAC_CONTEXT) {
    if (is_greater(bisection_newton_eps, x0 - a0, contextptr) ||
        is_greater(bisection_newton_eps, a1 - x0, contextptr))
  ...
  gen bisection(const modpoly & p, const gen & a0, const gen & b0, double eps, GIAC_CONTEXT) {
    int nsteps =
        int(std::ceil(std::log(evalf_double(b0 - a0, 1, contextptr)._DOUBLE_val / eps) / M_LN2));
  ...
  int multiplicity(const factorization & f, const gen & interval, GIAC_CONTEXT) {
    ...
    if (interval.type == _VECT && interval._VECTptr->size() == 2) {
      for (; it != itend; ++it) {
        if (is_strictly_positive(-it->fact(interval._VECTptr->front()) *
                                     it->fact(interval._VECTptr->back()),
                                 contextptr))
          return it->mult;
      }
      for (it = f.begin(); it != itend; ++it) {
        if (is_positive(-it->fact(interval._VECTptr->front()) * it->fact(interval._VECTptr->back()),
                        contextptr))
          return it->mult;
      }
    } else {
  ...
  static void add_vasres(vecteur & vasres, const gen & a, const gen & a0, const gen & b0, int mult,
                         bool with_mult, GIAC_CONTEXT) {
  ...
  bool vas(const modpoly & P, const gen & a0, const gen & b0, double eps, vecteur & vasres,
           bool with_mult, GIAC_CONTEXT) {
    if (P.size() <= 3) {
      if (P.size() < 2)
        return true;
    ...
    for (; it != itend; ++it) {
      ...
      if (with_mult)
        ...
        vasres.push_back(
            (interval.type == _VECT && interval._VECTptr->size() == 2)
                ? evalf((interval._VECTptr->front() + interval._VECTptr->back()) / 2, 1, contextptr)
                : interval);
    }
    return true;
  }
