...
namespace giac {
  ...
  vecteur sortsubst(const vecteur & v, const vecteur & i, const vecteur & newi, bool quotesubst,
                    GIAC_CONTEXT); // assumes that i is sorted
  ...
  template <class T> int equalposcomp(const std::vector<T> & v, const T & w) {
    int n = 1;
  ...
  gen subst(const gen & e, const std::vector<const unary_function_ptr *> & v,
            const std::vector<gen_op_context> & w, bool quotesubst, GIAC_CONTEXT);
  gen subst(const gen & e, const unary_function_ptr * v, const gen_op_context * w, bool quotesubst,
            GIAC_CONTEXT, bool recursive_nonrat = true);

  gen subst(const gen & e, const std::vector<const unary_function_ptr *> & v,
            const std::vector<gen (*)(const gen &)> & w, bool quotesubst, GIAC_CONTEXT);
