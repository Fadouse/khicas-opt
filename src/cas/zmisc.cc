...
inline giac::gen _graph_charpoly(const giac::gen & g, const giac::context *) {
  return g;
}
#else
...
namespace giac {
  ...
  define_unary_function_ptr5(at_scalar_product, alias_at_scalar_product, &__scalar_product, 0,
                             true);
  ...
  gen interpolate_xi(int j, const gen & coeff) {
    ...
    if (galois_field * gf = dynamic_cast<galois_field *>(coeff._USERptr)) {
      ...
      g.a = _revlist(
          _convert(makesequence(j, change_subtype(_BASE, _INT_MAPLECONVERSION), gf->p), context0),
          context0);
      return g;
    }
#endif
  ...
  bool interpolable(int d1, gen & coefft, bool extend, GIAC_CONTEXT) {
    ...
    if (tt == _MOD) {
      ...
      int n = int(
          std::ceil(std::log(d1 + 20.0) / std::log(evalf_double(m, 1, contextptr)._DOUBLE_val)));
#ifdef NO_RTTI
      return false;
#else
  ...
  define_unary_function_ptr5(at_adjoint_matrix, alias_at_adjoint_matrix, &__adjoint_matrix, 0,
                             true);
  ...
  static gen epsilon2zero(const gen & g, GIAC_CONTEXT) {
    ...
    case _DOUBLE_:
      if (fabs(g._DOUBLE_val) < epsilon(contextptr))
        ...
        return g;
    case _CPLX:
      return epsilon2zero(re(g, contextptr), contextptr) +
             cst_i * epsilon2zero(im(g, contextptr), contextptr);
    case _SYMB:
    ...
    if (v.size() == 3 && v[1].type == _INT_ && v[2].type == _INT_) {
      ...
      if (v[0].type == _VECT) {
        ...
        if (i1 >= int(w.size()) || i2 >= int(w.size()))
          return gendimerr(contextptr);
        return gen(
            mergevecteur(vecteur(w.begin(), w.begin() + i1), vecteur(w.begin() + i2 + 1, w.end())),
            v[0].subtype);
      }
      if (v[0].type == _STRNG) {
...
gen _show_pixels(const gen & args, GIAC_CONTEXT) {
  ...
  gen _truncate(const gen & args, GIAC_CONTEXT) {
    ...
    if ((den.type == _POLY) && (den._POLYptr->lexsorted_degree()))
      return gensizeerr(contextptr);
    if (num.type == _POLY) {
      vector<monomial<gen>>::const_iterator it = num._POLYptr->coord.begin(),
                                            itend = num._POLYptr->coord.end();
  ...
  gen _canonical_form(const gen & args, GIAC_CONTEXT) {
    ...
    if (is_algebraic_program(args, a, b))
      return symbolic(
          at_program,
          makesequence(a, 0, _canonical_form(gen(makevecteur(b, a[0]), _SEQ__VECT), contextptr)));
    if (args.type != _VECT) {
      p = args;
      x = ggb_var(p);
    } else {
  ...
  define_unary_function_ptr5(at_canonical_form, alias_at_canonical_form, &__canonical_form, 0,
                             true);
  ...
  gen _taux_accroissement(const gen & args, GIAC_CONTEXT) {
    ...
    if (v[1].type != _IDNT)
      return gentypeerr(contextptr);
    return (subst(v.front(), v[1], v[3], false, contextptr) -
            subst(v.front(), v[1], v[2], false, contextptr)) /
           (v[3] - v[2]);
  }
  static const char _taux_accroissement_s[] = "taux_accroissement";
  static define_unary_function_eval(__taux_accroissement, &_taux_accroissement,
                                    _taux_accroissement_s);
  define_unary_function_ptr5(at_taux_accroissement, alias_at_taux_accroissement,
                             &__taux_accroissement, 0, true);
  ...
  gen _roots(const gen & g_, GIAC_CONTEXT) {
    if (g_.type == _STRNG && g_.subtype == -1)
      return g_;
    gen eq = 0;
    gen g(g_);
    if (g.type == _VECT && g._VECTptr->size() == 3 && g._VECTptr->back() == at_equal &&
  ...
  gen _float2rational(const gen & g, GIAC_CONTEXT) {
    ...
    case _REAL:
      return float2rational(evalf_double(g, 1, contextptr)._DOUBLE_val, epsilon(contextptr),
                            contextptr);
    case _CPLX:
      return _float2rational(re(g, contextptr), contextptr) +
             cst_i * _float2rational(im(g, contextptr), contextptr);
    case _SYMB:
  ...
  define_unary_function_ptr5(at_float2rational, alias_at_float2rational, &__float2rational, 0,
                             true);
  ...
  gen _gramschmidt(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() == 2) {
      ...
      for (int i = 1; i < s; ++i) {
        gen cl;
        for (int j = 0; j < i; ++j) {
          gen tmp = rdiv(scalaire(gen(makevecteur(lv[i], lv[j]), _SEQ__VECT), contextptr), sc[j],
                         contextptr) *
                    lv[j];
          cl = cl + tmp;
        }
  ...
  */
  ...
  static gen pmin(const matrice & m, GIAC_CONTEXT) {
    ...
    mrref(res, r, v, det, 0, s + 1, 0, s * s,
          /* fullreduction */ 0, 1, true, 1, 0, contextptr);
  ...
  bool is_potential(const vecteur & fv, const vecteur & xv, gen & res, GIAC_CONTEXT) {
    ...
    for (int i = 0; i < s; ++i) {
      for (int j = i + 1; j < s; ++j) {
        if (!is_zero(simplify(derive(fv[i], xv[j], contextptr) - derive(fv[j], xv[i], contextptr),
                              contextptr)))
          return false;
      }
    }
    for (int i = 0; i < s; ++i) {
      res = res + integrate_gen(simplify(fv[i] - derive(res, xv[i], contextptr), contextptr), xv[i],
                                contextptr);
    }
    return true;
  }
  ...
  gen _vpotential(const gen & g, GIAC_CONTEXT) {
    ...
    gen F = simplify(fv[0] + derive(res[1], xv[2], contextptr) + derive(res[2], xv[1], contextptr),
                     contextptr);
  ...
  define_unary_function_ptr5(at_vpotential, alias_at_vpotential, &__vpotential, _QUOTE_ARGUMENTS,
                             true);
  ...
  gen _l2norm(const gen & g0, GIAC_CONTEXT) {
    ...
    vecteur v;
    if (g._VECTptr->size() == 2 && g._VECTptr->front().type == _VECT &&
        g._VECTptr->back() == at_vector) {
  ...
  define_unary_function_ptr5(at_frobenius_norm, alias_at_frobenius_norm, &__frobenius_norm, 0,
                             true);
  ...
  gen _diag(const gen & g, GIAC_CONTEXT) {
    ...
    if (l == 3 && v[0].type == _VECT && v[1].type == _VECT && v[2].type == _VECT &&
        v[0]._VECTptr->size() + 1 == v[1]._VECTptr->size() &&
        v[0]._VECTptr->size() == v[2]._VECTptr->size()) {
...
#endif
    int res = inputline("EXE: ok, EXIT: break", args.type == _STRNG ? (*args._STRNGptr) : "input ?",
                        s, false, 142);
#ifdef OLD_CONSOLE
    if (!freeze)
      line_start -= 3;
  ...
  define_unary_function_ptr5(at_textinput, alias_at_textinput, &__textinput, _QUOTE_ARGUMENTS,
                             true);
  ...
  gen _coeff(const gen & g, GIAC_CONTEXT) {
    ...
    if (xcas_mode(contextptr) == 1 && g.type == _VECT && g._VECTptr->size() == 2 &&
  ...
  static gen freq_quantile(const matrice & v, double d, GIAC_CONTEXT) {
    ...
    for (int i = 0; i < s; ++i) {
      ...
      if (partial_sum == sigma && i < s)
        return (i == s - 1 || (calc_mode(contextptr) != 1 && abs_calc_mode(contextptr) != 38))
                   ? data[i]
                   : (data[i] + data[i + 1]) / 2;
    }
    return undef;
  ...
  gen _median(const gen & g, GIAC_CONTEXT) {
    ...
    if (!ckmatrix(v)) {
      ...
      if ((calc_mode(contextptr) == 1 || abs_calc_mode(contextptr) == 38) && !v.empty() &&
          !(v.size() % 2))
  ...
  gen _quartiles(const gen & g, GIAC_CONTEXT) {
    ...
    vecteur v(gen2vecteur(g));
    if (g.type == _VECT && g.subtype == _SEQ__VECT && v.size() == 2)
      return makevecteur(freq_quantile(v, 0.0, contextptr), freq_quantile(v, 0.25, contextptr),
                         freq_quantile(v, 0.5, contextptr), freq_quantile(v, 0.75, contextptr),
                         freq_quantile(v, 1.0, contextptr));
    if (!ckmatrix(v)) {
      if (!is_fully_numeric(evalf(v, 1, contextptr))) {
        islesscomplexthanf_sort(v.begin(), v.end());
        int s = int(v.size());
        return makevecteur(v[0], v[int(std::ceil(s / 4.)) - 1], v[int(std::ceil(s / 2.)) - 1],
                           v[int(std::ceil(3 * s / 4.)) - 1], v[s - 1]);
      }
      v = ascsort(mtran(vecteur(1, v)), true);
    } else
      v = ascsort(v, true);
  ...
  gen _moustache(const gen & g_orig, GIAC_CONTEXT) {
    ...
    if (horizontal) {
      ...
      for (int i = 0; i < s; ++i) {
        ...
        res.push_back(symb_segment(current[0] + y_middle * cst_i, current[1] + y_middle * cst_i,
                                   attr, _GROUP__VECT, contextptr));
        // rectangle
        res.push_back(
            pnt_attrib(gen(makevecteur(current[3] + y_down * cst_i, current[1] + y_down * cst_i,
                                       current[1] + y_up * cst_i, current[3] + y_up * cst_i,
                                       current[3] + y_down * cst_i),
                           _GROUP__VECT),
                       attr, contextptr));
        // mediane
        res.push_back(symb_segment(current[2] + y_down * cst_i, current[2] + y_up * cst_i, attr,
                                   _GROUP__VECT, contextptr));
        // trait 3eme quartile -> fin
        res.push_back(symb_segment(current[3] + y_middle * cst_i, current[4] + y_middle * cst_i,
                                   attr, _GROUP__VECT, contextptr));
      }
      ...
      for (int i = 0; i < s; ++i) {
        ...
        if (current.size() != 5)
          continue;
        // trait min -> 1er quartile
        res.push_back(symb_segment(x_middle + current[0] * cst_i, current[1] * cst_i + x_middle,
                                   attr, _GROUP__VECT, contextptr));
        // rectangle
        res.push_back(
            pnt_attrib(gen(makevecteur(current[1] * cst_i + x_up, current[1] * cst_i + x_down,
                                       current[3] * cst_i + x_down, current[3] * cst_i + x_up,
                                       current[1] * cst_i + x_up),
                           _GROUP__VECT),
                       attr, contextptr));
        // mediane
        res.push_back(symb_segment(current[2] * cst_i + x_down, current[2] * cst_i + x_up, attr,
                                   _GROUP__VECT, contextptr));
        // trait 3eme quartile -> fin
        res.push_back(symb_segment(current[3] * cst_i + x_middle, current[4] * cst_i + x_middle,
                                   attr, _GROUP__VECT, contextptr));
      }
    }
  ...
  static gen stddevmean(const vecteur & v, int withstddev, int xcol, int freqcol, GIAC_CONTEXT) {
    ...
    if (withstddev) {
      ...
      if (s.type != _VECT && is_greater(1, s, contextptr) && withstddev == 2)
        *logptr(contextptr)
            << "stddevp called with N<=1, perhaps you are misusing this command with frequencies"
            << '\n';
  ...
  gen _mean(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.type == _VECT && !g._VECTptr->empty() && g._VECTptr->front().type == _FUNC &&
        (nd = is_distribution(g._VECTptr->front()))) {
      return _mean(symbolic(*g._VECTptr->front()._FUNCptr,
                            gen(vecteur(g._VECTptr->begin() + 1, g._VECTptr->end()), _SEQ__VECT)),
                   contextptr);
    }
    if (g.type == _VECT && g.subtype == _SEQ__VECT)
  ...
  define_unary_function_ptr5(at_ecart_type_population, alias_at_ecart_type_population,
                             &__ecart_type_population, 0, true);
  ...
  gen _stddev(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.type == _SYMB && (nd = is_distribution(g))) {
      ...
      if (nd == 5) {
        if (f.type < _IDNT && is_greater(1, f, contextptr))
          return undef;
        if (f.type < _IDNT && is_greater(2, f, contextptr))
          return plus_inf;
        return sqrt(f / (f - 2), contextptr);
      }
      if (nd == 6)
        return (f[1].type < _IDNT && is_greater(4, f[1], contextptr))
                   ? undef
                   : f[1] / (f[1] - 2) *
                         sqrt(2 * (f[0] + f[1] - 2) / f[0] / (f[1] - 4), contextptr);
      if (nd == 8)
        return f[1] * sqrt(Gamma(1 + gen(2) / f[0], contextptr) -
                               pow(Gamma(1 + gen(1) / f[0], contextptr), 2, contextptr),
                           contextptr);
      if (nd == 9)
    ...
    if (g.type == _VECT && !g._VECTptr->empty() && g._VECTptr->front().type == _FUNC &&
        (nd = is_distribution(g._VECTptr->front()))) {
      return _stddev(symbolic(*g._VECTptr->front()._FUNCptr,
                              gen(vecteur(g._VECTptr->begin() + 1, g._VECTptr->end()), _SEQ__VECT)),
                     contextptr);
    }
    if (g.type == _VECT && g.subtype == _SEQ__VECT)
  ...
  static vecteur covariance_correlation(const gen & g, const gen & u1, const gen & u2, int xcol,
                                        int ycol, int freqcol, GIAC_CONTEXT) {
    if (is_undef(g))
    ...
    gen correlation =
        ...
        sqrt((n * sigmax2 - sigmax * sigmax) * (n * sigmay2 - sigmay * sigmay), contextptr);
    return makevecteur(covariance, correlation);
  }

  static void find_xyfreq(const gen & g, gen & gv, int & xcol, int & ycol, int & freqcol,
                          GIAC_CONTEXT) {
    xcol = 0;
    ycol = 1;
    freqcol = -1;
  ...
  static define_unary_function_eval(__covariance_correlation, &_covariance_correlation,
                                    _covariance_correlation_s);
  define_unary_function_ptr5(at_covariance_correlation, alias_at_covariance_correlation,
                             &__covariance_correlation, 0, true);
  ...
  define_unary_function_ptr5(at_interval2center, alias_at_interval2center, &__interval2center, 0,
                             true);
  ...
  void Mult4(double * colmat, double * vect, double * res) {
    ...
    res[2] =
        colmat[2] * vect[0] + colmat[6] * vect[1] + colmat[10] * vect[2] + colmat[14] * vect[3];
    res[3] =
        colmat[3] * vect[0] + colmat[7] * vect[1] + colmat[11] * vect[2] + colmat[15] * vect[3];
  }
  ...
  gen function_regression(const gen & g, const gen & u1, const gen & u2, gen & a, gen & b,
                          double & xmin, double & xmax, gen & correl2, GIAC_CONTEXT) {
    gen gv, freq;
    int xcol, ycol, freqcol;
    xmin = 1e300;
    xmax = -xmin;
    ...
    if (u1 == at_sin) { // trig regression a*sin(omega*x+phi)+b
      ...
      while (1) {
        ...
        omega -= delta[2];
        phi -= delta[3];
        if (fabs(a_ ? delta[0] / a_ : delta[0]) + fabs(b_ ? delta[1] / b_ : delta[1]) +
                fabs(omega ? delta[2] / omega : delta[2]) + fabs(phi ? delta[3] / phi : delta[3]) <
            1e-7)
          break;
      }
      a = makesequence(a_, b_, omega, phi);
#else
    gen E, A(identificateur("A")), B(identificateur("B")), O(identificateur("omega")),
        P(identificateur("phi"));
    for (int i = 0; i < n; ++i) {
      double x = trig[i][0];
      double y = trig[i][1];
    ...
    a = _fsolve(makesequence(symb_equal(derive(E, vars, contextptr), 0), vars,
                             makevecteur(a_, b_, omega, phi)),
                contextptr);
#endif
      b = undef;
    }
  ...
  define_unary_function_ptr5(at_linear_regression, alias_at_linear_regression, &__linear_regression,
                             0, true);
  ...
  define_unary_function_ptr5(at_sin_regression, alias_at_sin_regression, &__sin_regression, 0,
                             true);
  ...
  static define_unary_function_eval(__exponential_regression, &_exponential_regression,
                                    _exponential_regression_s);
  define_unary_function_ptr5(at_exponential_regression, alias_at_exponential_regression,
                             &__exponential_regression, 0, true);
  ...
  static define_unary_function_eval(__logarithmic_regression, &_logarithmic_regression,
                                    _logarithmic_regression_s);
  define_unary_function_ptr5(at_logarithmic_regression, alias_at_logarithmic_regression,
                             &__logarithmic_regression, 0, true);
  ...
  define_unary_function_ptr5(at_power_regression, alias_at_power_regression, &__power_regression, 0,
                             true);

  gen regression_plot_attributs(const gen & g, vecteur & attributs, bool & eq, bool & r,
                                GIAC_CONTEXT) {
    gen res = g;
    r = false;
    eq = false;
  ...
  gen _linear_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    if (is_undef(errcode))
      return errcode;
    xmax += (xmax - xmin);
    gen ad(evalf_double(a, 1, contextptr)), bd(evalf_double(b, 1, contextptr)),
        cd(evalf_double(correl2, 1, contextptr));
    if (ad.type == _DOUBLE_ && bd.type == _DOUBLE_ && cd.type == _DOUBLE_) {
      string eqs =
          "y=" + print_DOUBLE_(ad._DOUBLE_val, 3) + "*x+" + print_DOUBLE_(bd._DOUBLE_val, 3);
    ...
    return makesequence(
        _scatterplot(g, contextptr),
        put_attributs(_droite(makesequence(b * cst_i, 1 + (b + a) * cst_i), contextptr), attributs,
                      contextptr));
  }
  static const char _linear_regression_plot_s[] = "linear_regression_plot";
  static define_unary_function_eval(__linear_regression_plot, &_linear_regression_plot,
                                    _linear_regression_plot_s);
  define_unary_function_ptr5(at_linear_regression_plot, alias_at_linear_regression_plot,
                             &__linear_regression_plot, 0, true);
  ...
  gen _exponential_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    gen ad(evalf_double(a, 1, contextptr)), bd(evalf_double(b, 1, contextptr)),
        cd(evalf_double(correl2, 1, contextptr));
    if (ad.type == _DOUBLE_ && bd.type == _DOUBLE_ && cd.type == _DOUBLE_) {
      string eqs = "y=" + print_DOUBLE_(std::exp(ad._DOUBLE_val), 3) + "^x*" +
                   print_DOUBLE_(std::exp(bd._DOUBLE_val), 3);
      string R2s = " , R2=" + print_DOUBLE_(cd._DOUBLE_val, 3);
      *logptr(contextptr) << eqs << R2s << '\n';
      string s;
      if (eq)
    ...
    return makesequence(
        _scatterplot(g, contextptr),
        put_attributs(_plotfunc(gen(makevecteur(evalf(exp(b, contextptr), 1, contextptr) *
                                                    exp(a * vx_var, contextptr),
                                                symb_equal(vx_var, symb_interval(xmin, xmax))),
                                    _SEQ__VECT),
                                contextptr),
                      attributs, contextptr));
  }
  static const char _exponential_regression_plot_s[] = "exponential_regression_plot";
  static define_unary_function_eval(__exponential_regression_plot, &_exponential_regression_plot,
                                    _exponential_regression_plot_s);
  define_unary_function_ptr5(at_exponential_regression_plot, alias_at_exponential_regression_plot,
                             &__exponential_regression_plot, 0, true);
  ...
  gen _logarithmic_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    if (ad.type == _DOUBLE_ && bd.type == _DOUBLE_ && cd.type == _DOUBLE_) {
      string eqs =
          "y=" + print_DOUBLE_(ad._DOUBLE_val, 3) + "*ln(x)+" + print_DOUBLE_(bd._DOUBLE_val, 3);
      string R2s = " , R2=" + print_DOUBLE_(cd._DOUBLE_val, 3);
      *logptr(contextptr) << eqs << R2s << '\n';
      string s;
      if (eq)
    ...
    return makesequence(
        _scatterplot(g, contextptr),
        put_attributs(_plotfunc(gen(makevecteur(a * ln(vx_var, contextptr) + b,
                                                symb_equal(vx_var, symb_interval(xmin, xmax))),
                                    _SEQ__VECT),
                                contextptr),
                      attributs, contextptr));
  ...
  static define_unary_function_eval(__logarithmic_regression_plot, &_logarithmic_regression_plot,
                                    _logarithmic_regression_plot_s);
  define_unary_function_ptr5(at_logarithmic_regression_plot, alias_at_logarithmic_regression_plot,
                             &__logarithmic_regression_plot, 0, true);
  ...
  gen _sin_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    res = a;
    if (is_undef(res) || res.type != _VECT || res._VECTptr->size() != 4)
      return res;
    res = res[0] * symb_sin(res[2] * vx_var + res[3]) + res[1];
    string eqs = "y=" + res.print(contextptr);
    *logptr(contextptr) << eqs << '\n';
    return makesequence(_scatterplot(g, contextptr),
                        put_attributs(_plot(makesequence(res, vx_var, xmin, xmax), contextptr),
                                      attributs, contextptr));
  }
  static const char _sin_regression_plot_s[] = "sin_regression_plot";
  static define_unary_function_eval(__sin_regression_plot, &_sin_regression_plot,
                                    _sin_regression_plot_s);
  define_unary_function_ptr5(at_sin_regression_plot, alias_at_sin_regression_plot,
                             &__sin_regression_plot, 0, true);
  ...
  gen _power_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    gen ad(evalf_double(a, 1, contextptr)), bd(evalf_double(b, 1, contextptr)),
        cd(evalf_double(correl2, 1, contextptr));
    if (ad.type == _DOUBLE_ && bd.type == _DOUBLE_ && cd.type == _DOUBLE_) {
      string eqs = "y=" + print_DOUBLE_(exp(bd, contextptr)._DOUBLE_val, 3) + "*x^" +
                   print_DOUBLE_(ad._DOUBLE_val, 3);
      string R2s = " , R2=" + print_DOUBLE_(cd._DOUBLE_val, 3);
      *logptr(contextptr) << eqs << R2s << '\n';
      string s;
      if (eq)
    ...
    return makesequence(
        _scatterplot(g, contextptr),
        put_attributs(_plotfunc(gen(makevecteur(exp(b, contextptr) * pow(vx_var, a, contextptr),
                                                symb_equal(vx_var, symb_interval(xmin, xmax))),
                                    _SEQ__VECT),
                                contextptr),
                      attributs, contextptr));
  ...
  static define_unary_function_eval(__power_regression_plot, &_power_regression_plot,
                                    _power_regression_plot_s);
  define_unary_function_ptr5(at_power_regression_plot, alias_at_power_regression_plot,
                             &__power_regression_plot, 0, true);

  static gen polynomial_regression(const gen & g, int d, const gen & u1, const gen & u2,
                                   double & xmin, double & xmax, GIAC_CONTEXT) {
  ...
  static gen polynomial_regression(const gen & g, double & xmin, double & xmax, GIAC_CONTEXT) {
    if (g.type == _VECT && g._VECTptr->size() == 3) {
      vecteur & v = *g._VECTptr;
      if (v[0].type == _VECT && v[1].type == _VECT &&
          v[0]._VECTptr->size() == v[1]._VECTptr->size())
        return polynomial_regression(makevecteur(mtran(makevecteur(v[0], v[1])), v[2]), xmin, xmax,
                                     contextptr);
    }
    if (g.type != _VECT || g._VECTptr->size() != 2)
      return gensizeerr(contextptr);
    ...
    if (last.type != _INT_)
      return gensizeerr(contextptr);
    return polynomial_regression(g._VECTptr->front(), absint(last.val), zero, zero, xmin, xmax,
                                 contextptr);
  }
  ...
  static define_unary_function_eval(__polynomial_regression, &_polynomial_regression,
                                    _polynomial_regression_s);
  define_unary_function_ptr5(at_polynomial_regression, alias_at_polynomial_regression,
                             &__polynomial_regression, 0, true);
  gen _polynomial_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    return put_attributs(
        _plotfunc(gen(makevecteur(res, symb_equal(vx_var, symb_interval(xmin, xmax))), _SEQ__VECT),
                  contextptr),
        attributs, contextptr);
  }
  static const char _polynomial_regression_plot_s[] = "polynomial_regression_plot";
  static define_unary_function_eval(__polynomial_regression_plot, &_polynomial_regression_plot,
                                    _polynomial_regression_plot_s);
  define_unary_function_ptr5(at_polynomial_regression_plot, alias_at_polynomial_regression_plot,
                             &__polynomial_regression_plot, 0, true);
  ...
  static gen logistic_regression(const gen & g, double & xmin, double & xmax, gen & r,
                                 GIAC_CONTEXT) {
    if (g.type != _VECT)
      return gensizeerr(contextptr);
    vecteur & v = *g._VECTptr;
    ...
    *logptr(contextptr) << gettext("Pinstant=") << a << gettext("*Pcumul+") << b << '\n'
    ...
    if (is_positive(-urr, contextptr))
      return gensizeerr(gettext("Data not suited for logistic regression (instant production "
                                "should be a decreasing function of cumulated production)"));
    ...
    return makevecteur(urr / (1 + exp(b2 * vx_var + bt0, contextptr)),
                       urr * b / 2 / (1 + cosh(b2 * vx_var + bt0, contextptr)), urr, urr * b / 4,
                       -bt0 / b2, r);
  }
  ...
  static define_unary_function_eval(__logistic_regression, &_logistic_regression,
                                    _logistic_regression_s);
  define_unary_function_ptr5(at_logistic_regression, alias_at_logistic_regression,
                             &__logistic_regression, 0, true);
  ...
  static define_unary_function_eval(__logistic_regression_plot, &_logistic_regression_plot,
                                    _logistic_regression_plot_s);
  define_unary_function_ptr5(at_logistic_regression_plot, alias_at_logistic_regression_plot,
                             &__logistic_regression_plot, 0, true);
  ...
  static gen gen_interpolate(const gen & g, int deg, GIAC_CONTEXT) {
    ...
    if (s < 4)
      return gensizeerr(contextptr);
    gen m = evalf_double(v[0], 1, contextptr), Xmin = evalf_double(v[1], 1, contextptr),
        Xmax = evalf_double(v[2], 1, contextptr), Xstep = evalf_double(v[3], 1, contextptr);
    if (!ckmatrix(m) || m._VECTptr->size() != 2 || Xmin.type != _DOUBLE_ || Xmax.type != _DOUBLE_ ||
        Xstep.type != _DOUBLE_)
      return gensizeerr(contextptr);
    ...
    matrice res;
    int pos = 0;
    gen Mcur = Xmin, Mnext = M[1]._VECTptr->front(), Ycur = M[0]._VECTptr->back(),
        Ynext = M[1]._VECTptr->back();
    double ycur;
    if (deg == 1) {
      for (double xcur = xmin; xcur <= xmax; xcur += xstep) {
        ...
        if (pos >= Ms - 1) { // use Ycur
          ...
          ycur = Ycur._DOUBLE_val + (xcur - Mcur._DOUBLE_val) /
                                        (Mnext._DOUBLE_val - Mcur._DOUBLE_val) *
                                        (Ynext._DOUBLE_val - Ycur._DOUBLE_val);
          res.push_back(makevecteur(xcur, ycur));
        }
      }
    } else {
  ...
  static define_unary_function_eval(__linear_interpolate, &_linear_interpolate,
                                    _linear_interpolate_s);
  define_unary_function_ptr5(at_linear_interpolate, alias_at_linear_interpolate,
                             &__linear_interpolate, 0, true);
  ...
  static define_unary_function_eval(__parabolic_interpolate, &_parabolic_interpolate,
                                    _parabolic_interpolate_s);
  define_unary_function_ptr5(at_parabolic_interpolate, alias_at_parabolic_interpolate,
                             &__parabolic_interpolate, 0, true);
    ...
    if (ckmatrix(data)) {
      if (!data.empty() && data.front()._VECTptr->size() > 1) {
        ...
        for (; kbegin <= kend; ++kbegin) {
          ...
          res.push_back(
              makevecteur(symbolic(at_interval, makesequence(min_class, max_class)), effectif));
        }
        return res;
      }
  ...
  static vecteur centres2intervalles(const vecteur & centres, double class_min, bool with_class_min,
                                     GIAC_CONTEXT) {
    if (centres.size() < 2)
      return vecteur(1, gensizeerr(contextptr));
    double d0 = evalf_double(centres[0], 1, contextptr)._DOUBLE_val,
           d1 = evalf_double(centres[1], 1, contextptr)._DOUBLE_val;
  ...
  define_unary_function_ptr5(at_center2interval, alias_at_center2interval, &__center2interval, 0,
                             true);

  static gen histogram(const vecteur & v, double class_minimum, double class_size,
  ...
  int s = read_attributs(args, attributs, contextptr);
    ...
    if (s >= 2) {
      ...
      if (ckmatrix(data) && arg1.type == _DOUBLE_) { // [ [center, effectif] ... ], min
        ...
        data = mtran(data);
        gen g = data[0][0];
        if (g.is_symb_of_sommet(at_interval) && g._SYMBptr->feuille.type == _VECT &&
    ...
    for (int k = 1; k < s; ++k) {
      ...
      for (; it != itend; ++it) {
        ...
        if (v.front().is_symb_of_sommet(at_interval)) {
          ...
          tmp =
              v.front(); // +class_s/2; // FIX 30/11/2012 for e.g. cumulated_frequencies([[1,0.3],[2,0.5],[3,0.2]])
        tmp = evalf_double(tmp, 1, contextptr);
        if (tmp.type != _DOUBLE_)
          return gensizeerr(contextptr);
  ...
  static define_unary_function_eval(__cumulated_frequencies, &_cumulated_frequencies,
                                    _cumulated_frequencies_s);
  define_unary_function_ptr5(at_cumulated_frequencies, alias_at_cumulated_frequencies,
                             &__cumulated_frequencies, 0, true);
      ...
      for (it = v.begin(); it != itend; ++it) {
        ...
        if (tmp.type == _STRNG && attributs[1].type == _STRNG && *attributs[1]._STRNGptr == nullstr)
          ...
          if (is_equal(tmp))
            ...
            if (scatter)
              vres.push_back(symb_pnt_name(
                  it->_VECTptr->front() + cst_i * tmp, attributs[0],
                  string2gen(((it == v.begin() && !polygone) ? gen2string(attributs[1]) : ""),
                             false),
                  contextptr));
            if (bar)
              vres.push_back(symb_segment(it->_VECTptr->front(),
                                          it->_VECTptr->front() + cst_i * tmp, attributs,
                                          _GROUP__VECT, contextptr));
          }
        }
      }
  ...
  define_unary_function_ptr5(at_ligne_polygonale, alias_at_ligne_polygonale, &__ligne_polygonale, 0,
                             true);
  ...
  static define_unary_function_eval(__polygonscatterplot, &_polygonscatterplot,
                                    _polygonscatterplot_s);
  define_unary_function_ptr5(at_polygonscatterplot, alias_at_polygonscatterplot,
                             &__polygonscatterplot, 0, true);

  static const char _ligne_polygonale_pointee_s[] = "ligne_polygonale_pointee";
  static define_unary_function_eval(__ligne_polygonale_pointee, &_polygonscatterplot,
                                    _ligne_polygonale_pointee_s);
  define_unary_function_ptr5(at_ligne_polygonale_pointee, alias_at_ligne_polygonale_pointee,
                             &__ligne_polygonale_pointee, 0, true);
  ...
  static gen read_camembert_args(const gen & g, vecteur & vals, vecteur & names,
                                 vecteur & attributs, GIAC_CONTEXT) {
    if (g.type != _VECT)
      return gensizeerr(contextptr);
    attributs = vecteur(1, default_color(contextptr) | _FILL_POLYGON);
    int s = read_attributs(*g._VECTptr, attributs, contextptr);
    gen args = (s == 1) ? g._VECTptr->front()
                        : gen(vecteur(g._VECTptr->begin(), g._VECTptr->begin() + s), g.subtype);
    if (ckmatrix(args)) {
  ...
  gen _diagramme_batons(const gen & g_, GIAC_CONTEXT) {
    ...
    int ncamemberts = int(vals.size()), s = int(vals.front()._VECTptr->size()),
        t = int(attr.size());
    ...
    for (const_iterateur it = vals.begin(); it != vals.end(); ++it) {
      if (it->type == _VECT)
        allvals = mergevecteur(
            allvals, vecteur(it->_VECTptr->begin() + (it->_VECTptr->front().type == _STRNG ? 1 : 0),
                             it->_VECTptr->end()));
    }
    ...
    for (int j = 0; j < ncamemberts; j++) {
      ...
      if (Vals[0].type == _STRNG) {
        // add title
        res.push_back(
            symb_pnt_name(xy - largeur - dsc * cst_i, _POINT_INVISIBLE, Vals[0], contextptr));
        ++i;
      }
      for (; i < s; ++i) {
        gen tmp, xpos;
        if (names[i].type != _STRNG && has_evalf(names[i], xpos, 1, contextptr)) {
          tmp = gen(makevecteur(xpos + largeur + cst_i * Vals[i], xpos + largeur, xpos - largeur,
                                xpos - largeur + cst_i * Vals[i], xpos + largeur + cst_i * Vals[i]),
                    _LINE__VECT);
          res.push_back(symb_pnt(tmp, i < t ? attr[i] : (c | _FILL_POLYGON), contextptr));
        } else {
          tmp = gen(makevecteur(xy + i - 1 + largeur + cst_i * Vals[i], xy + i - 1 + largeur,
                                xy + i - 1 - largeur, xy + i - 1 - largeur + cst_i * Vals[i],
                                xy + i - 1 + largeur + cst_i * Vals[i]),
                    _LINE__VECT);
          res.push_back(
              symb_pnt(tmp, i < t ? attr[i] : ((i == 7 ? 0 : i) | _FILL_POLYGON), contextptr));
          res.push_back(symb_pnt_name(xy + i - 1 + largeur + cst_i * (Vals[i] + padding),
                                      _POINT_INVISIBLE | _QUADRANT2, names[i], contextptr));
        }
      }
    }
#else
    ...
    for (; i < s; ++i) {
      ...
      if (names[i].type != _STRNG && has_evalf(names[i], xpos, 1, contextptr)) {
        tmp = gen(makevecteur(xpos + largeur + cst_i * Vals[i], xpos + largeur, xpos - largeur,
                  ...
                  _LINE__VECT);
        res.push_back(
            symb_pnt(tmp, i < t ? attr[i] : (c | _FILL_POLYGON | _QUADRANT2), contextptr));
      } else {
        tmp =
            gen(makevecteur(xy + i + largeur + cst_i * Vals[i], xy + i + largeur, xy + i - largeur,
                            xy + i - largeur + cst_i * Vals[i], xy + i + largeur + cst_i * Vals[i]),
                _LINE__VECT);
        res.push_back(
            symb_pnt_name(tmp, i < t ? attr[i] : ((i == 7 ? 0 : i) | _FILL_POLYGON | _QUADRANT2),
                          names[i], contextptr));
      }
    }
  }
  ...
  define_unary_function_ptr5(at_diagramme_batons, alias_at_diagramme_batons, &__diagramme_batons, 0,
                             true);
  ...
  define_unary_function_ptr5(at_diagrammebatons, alias_at_diagrammebatons, &__diagrammebatons, 0,
                             true);
    ...
    for (; i < s; ++i) {
      ...
      if (da100 > 0) {
        ...
        if (is_positive(a - 3 * cst_pi / 2, contextptr))
          pos = _QUADRANT4;
        gen tmp = symbolic(at_cercle, gen(makevecteur(diametre, a, a + da), _PNT__VECT));
        res.push_back(symb_pnt_name(
            tmp, i < t ? attr[i] : (i % 7 | _FILL_POLYGON | pos),
            string2gen(gen2string(names[i]) + ":" + string(ss) + "%", false), contextptr));
        a = a + da;
      }
    }
  }
  ...
  static bool graham_sort_function(const gen & a, const gen & b) {
    ...
    return is_strictly_greater(w[1], v[1], context0) ||
           (v[1] == w[1] && is_strictly_greater(w[2], v[2], context0));
  }
  ...
  gen _convexhull(const gen & g, GIAC_CONTEXT) {
    ...
    for (int j = 1; j < s; ++j) {
      zcur = l[j];
      ycur = im(zcur, contextptr);
      xcur = re(zcur, contextptr);
    ...
    for (int j = 0; j < s; ++j) {
      zcur = l[j];
      if (zcur != zmin) {
        ls.push_back(makevecteur(zcur, arg(zcur - zmin, contextptr),
                                 (zcur - zmin) * conj(zcur - zmin, contextptr)));
      }
    }
  ...
  define_unary_function_ptr5(at_simplex_reduce, alias_at_simplex_reduce, &__simplex_reduce, 0,
                             true);
#else
...
matrice simplex_reduce(const matrice & m_orig, vecteur & bfs, gen & optimum, bool max_pb,
                       bool choose_first, GIAC_CONTEXT) {
  matrice m(m_orig);
  int nr = int(m.size());
  ...
  for (;;) {
    ...
    for (int j = 0; j < nc - 1; ++j) {
      if (is_strictly_greater((max_pb ? mincoeff : last[j]), (max_pb ? last[j] : mincoeff),
                              contextptr)) {
        J = j;
        mincoeff = last[j];
        if (choose_first)
    ...
    if (J == -1) { // Find bfs and optimum
      ...
      for (int i = 0; i < nc - 1 && counter > 0; ++i) {
        ...
        if (cur_col >= 0 &&
            is_one(mt[i][cur_col])) { // BUGFIX by Luka Marohnić: a proper check for idn line
          --counter;
...
define_unary_function_ptr5(at_simplex_reduce, alias_at_simplex_reduce, &__simplex_reduce, 0, true);
  ...
  gen _hamdist(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.type == _VECT && g.subtype == _SEQ__VECT && g._VECTptr->size() == 2 &&
        ...
        g._VECTptr->front().subtype != _LIST__VECT && g._VECTptr->back().subtype != _LIST__VECT) {
  ...
  gen _plotarea(const gen & g, GIAC_CONTEXT) {
    ...
    if (s >= 2 && v[0].type != _VECT) {
      ...
      if (is_equal(tmp) && tmp._SYMBptr->feuille.type == _VECT &&
      ...
      if (s >= 2) {
        ...
        gen graph = funcplotfunc(gen(vecteur(v.begin(), v.begin() + s1 + 1), _SEQ__VECT), false,
                                 contextptr); // must be a graph of fcn
        if (is_undef(graph))
          return graph;
        // extract polygon
          ...
          if (graphev.size() > 1) {
            ...
            if (polyg.type == _VECT) {
              if (s == 2) {
                ...
                attributs[1] =
                    string2gen(_gaussquad(gen(makevecteur(v[0], v[1]), _SEQ__VECT), contextptr)
                                   .print(contextptr),
                               false);
              ...
              if (s >= 3 && v[2].type == _INT_) {
                ...
                if (s >= 4 && v[3].type == _INT_)
                  method = v[3].val;
                if (method == _RECTANGLE_DROIT || method == _RECTANGLE_GAUCHE ||
                    method == _POINT_MILIEU) {
                ...
                return gen(
                    makevecteur(
                        gen(makevecteur(pnt_attrib(res, attributs, contextptr), graph), _SEQ__VECT),
                        _couleur(makevecteur(graph, _RED + _DASH_LINE + _LINE_WIDTH_3),
                                 contextptr)),
                    _SEQ__VECT);
              } // end if (s>=3)
  ...
  define_unary_function_ptr5(at_remove_language, alias_at_remove_language, &__remove_language, 0,
                             true);
  ...
  gen plotproba(const gen & args, const vecteur & positions, const vecteur & attributs,
                GIAC_CONTEXT) {
    if (args.type != _VECT)
      return gensizeerr(contextptr);
    ...
    if (int(positions.size()) == ms) {
      ...
      for (int i = 0; i < ms; ++i) {
        ...
        if (tmp[i].type < _POLY) {
          ...
          if (p.is_symb_of_sommet(at_pnt) && p._SYMBptr->feuille.type == _VECT &&
              p._SYMBptr->feuille._VECTptr->size() > 1) {
    ...
    if (!attributs.empty() && attributs[0].type == _VECT &&
        int(attributs[0]._VECTptr->size()) == ms)
    ...
    for (int i = 0; i < ms; ++i) {
      for (int j = 0; j < ms; ++j) {
        ...
        if (mij != 0) {
          ...
          res.push_back(
              _arc(gen(makevecteur(l[i], l[j], 0.6, 2, legende, aff), _SEQ__VECT), contextptr));
        }
      }
    }
    return res;
  ...
  gen _plotproba(const gen & args, GIAC_CONTEXT) {
    ...
    if (s == 2 && v.front().type == _VECT && is_squarematrix(v.front()) &&
        !v[1]._VECTptr->empty() && v[1]._VECTptr->front().type == _STRNG) {
  ...
  define_unary_function_ptr5(at_plotproba, alias_at_plotproba, &__plotproba, _QUOTE_ARGUMENTS,
                             true);
#endif
  ...
  gen conjugate_gradient(const matrice & A, const vecteur & b_orig, const vecteur & x0, double eps,
                         int maxiter, GIAC_CONTEXT) {
    int n = int(A.size());
    ...
    *logptr(contextptr)
        << gettext("Warning! Leaving conjugate gradient algorithm after dimension of matrix "
                   "iterations. Check that your matrix is hermitian/symmetric definite.")
        << '\n';
    return xk;
  }

  // Ax=b where A=D+B, Dx_{n+1}=b-B*x_n
  gen jacobi_linsolve(const matrice & A, const vecteur & b_orig, const vecteur & x0, double eps,
                      int maxiter, GIAC_CONTEXT) {
    int n = int(A.size());
    matrice B(A);
    vecteur D(n);
    ...
    *logptr(contextptr) << gettext(
                               "Warning! Leaving Jacobi iterative algorithm after maximal number "
                               "of iterations. Check that your matrix is diagonal dominant.")
                        << '\n';
    return xn;
  }
  ...
  gen gauss_seidel_linsolve(const matrice & A, const vecteur & b_orig, const vecteur & x0,
                            double omega, double eps, int maxiter, GIAC_CONTEXT) {
    int n = int(A.size());
    ...
    *logptr(contextptr) << gettext(
                               "Warning! Leaving Gauss-Seidel iterative algorithm after maximal "
                               "number of iterations. Check that your matrix is diagonal dominant.")
                        << '\n';
    return xn;
  }
  ...
  gen iterative_solver(const gen & args, int method, GIAC_CONTEXT) {
    ...
    if (!is_integral(niter) || niter.val < 1)
      return gentypeerr(contextptr);
    if (method == 1)
      return creux ? sparse_jacobi_linsolve(*A._MAPptr, *b._VECTptr, x0, eps._DOUBLE_val, niter.val,
                                            contextptr)
                   : jacobi_linsolve(*A._VECTptr, *b._VECTptr, x0, eps._DOUBLE_val, niter.val,
                                     contextptr);
    if (method == 2)
      return creux ? sparse_gauss_seidel_linsolve(*A._MAPptr, *b._VECTptr, x0, omega,
                                                  eps._DOUBLE_val, niter.val, contextptr)
                   : gauss_seidel_linsolve(*A._VECTptr, *b._VECTptr, x0, omega, eps._DOUBLE_val,
                                           niter.val, contextptr);
    if (method == 4)
      return creux ? sparse_conjugate_gradient(*A._MAPptr, *b._VECTptr, x0, eps._DOUBLE_val,
                                               niter.val, contextptr)
                   : conjugate_gradient(*A._VECTptr, *b._VECTptr, x0, eps._DOUBLE_val, niter.val,
                                        contextptr);
    return gensizeerr(contextptr);
  }
  // params: matrix A, vector b, optional init value x0, optional precision eps
  ...
  static define_unary_function_eval(__conjugate_gradient, &_conjugate_gradient,
                                    _conjugate_gradient_s);
  define_unary_function_ptr5(at_conjugate_gradient, alias_at_conjugate_gradient,
                             &__conjugate_gradient, 0, true);
  ...
  define_unary_function_ptr5(at_jacobi_linsolve, alias_at_jacobi_linsolve, &__jacobi_linsolve, 0,
                             true);
  ...
  static define_unary_function_eval(__gauss_seidel_linsolve, &_gauss_seidel_linsolve,
                                    _gauss_seidel_linsolve_s);
  define_unary_function_ptr5(at_gauss_seidel_linsolve, alias_at_gauss_seidel_linsolve,
                             &__gauss_seidel_linsolve, 0, true);
  ...
  void strongconnect(const vector<vector<unsigned>> & G, vector<vertex> & V, int & index,
                     vector<unsigned> & S, vector<bool> & inS, vector<vector<unsigned>> & SCC,
                     unsigned v) {
    V[v].index = index;
    V[v].lowlink = index;
    ++index;
  ...
  void classify_scc(const vector<vector<unsigned>> & G, vector<vector<unsigned>> & SCC,
                    vector<vector<unsigned>> & SCCrec, vector<vector<unsigned>> & SCCtrans) {
  ...
  static define_unary_function_eval_quoted(__add_autosimplify, &_add_autosimplify,
                                           _add_autosimplify_s);
  define_unary_function_ptr5(at_add_autosimplify, alias_at_add_autosimplify, &__add_autosimplify,
  ...
  bool is_periodic(const gen & f, const gen & x, gen & periode, GIAC_CONTEXT) {
    ...
    for (unsigned i = 0; i < vx.size(); ++i) {
      if (vx[i].type != _SYMB ||
          (vx[i]._SYMBptr->sommet != at_exp && vx[i]._SYMBptr->sommet != at_sin &&
           vx[i]._SYMBptr->sommet != at_cos && vx[i]._SYMBptr->sommet != at_tan)) {
        if (f.type == _SYMB)
  ...
  bool convert_polynom(const gen & f, const gen & x, const gen & x0, vecteur & v, gen & a,
                       int & order, GIAC_CONTEXT) {
    v.clear();
    ...
    res = l.empty() ? tmp2
                    : ((tmp2.type == _FRAC && tmp2._FRACptr->den.type == _VECT &&
                        tmp2._FRACptr->den._VECTptr->size() > 1)
                           ? gen(fraction(r2e(tmp2._FRACptr->num, l, contextptr),
                                          r2e(tmp2._FRACptr->den, l, contextptr)))
                           : r2e(tmp2, l, contextptr));
  ...
  gen try_limit_undef(const gen & f, const identificateur & x, const gen & x0, int direction,
                      GIAC_CONTEXT) {
    gen res;
  ...
  int step_param_(const gen & f, const gen & g, const gen & t, gen & tmin, gen & tmax,
                  vecteur & poi, vecteur & tvi, bool printtvi, bool exactlegende, GIAC_CONTEXT) {
    if (t.type != _IDNT)
      return 0;
    gprintf(gettext("====================\nParametric plot (%gen,%gen), variable %gen"),
            makevecteur(f, g, t), 1, contextptr);
    ...
    int eof = 0, eog = 0;
    if (tmin == -tmax && (eof = is_even_odd(f, t, contextptr)) &&
        (eog = is_even_odd(g, t, contextptr))) {
      if (eof == 1) {
        if (eog == 1)
          gprintf(gettext("Even functions."), vecteur(0), 1, contextptr);
        else
          gprintf(gettext("Even function %gen, odd function %gen. Reflection Ox"),
                  makevecteur(f, g), 1, contextptr);
      } else {
        if (eog == 1)
          gprintf(gettext("Odd function %gen, even function %gen. Reflection Oy"),
                  makevecteur(f, g), 1, contextptr);
        else
    ...
    gen f1 = _factor(derive(f, t, contextptr), contextptr),
        g1 = _factor(derive(g, t, contextptr), contextptr);
    ...
    if (!lidnt(evalf(c, 1, contextptr)).empty()) {
      *logptr(contextptr) << gettext(
                                 "Infinite number of critical points. Try with optional argument ")
                          << t << "=tmin..tmax" << '\n';
      purgenoassume(t, contextptr);
      return 0;
    }
    ...
    for (; it != itend; ++it) {
      ...
      *it = recursive_normal(*it, contextptr);
      if (in_domain(df, t, *it, contextptr) && is_greater(*it, tmin, contextptr) &&
          is_greater(tmax, *it, contextptr)) {
        crit.push_back(*it);
	  ...
	  ay=recursive_normal(ay,contextptr); */
          ...
          while (ordre <= 20 && o1 == 0) {
            if (ctrl_c || interrupted)
              return 0;
            // series expansion
            if (!convert_polynom(series(f, xid, *it, ordre, contextptr), xid, *it, vx, ax, ox,
                                 contextptr))
              break;
            if (!convert_polynom(series(g, xid, *it, ordre, contextptr), xid, *it, vy, ay, oy,
                                 contextptr))
              break;
            o1 = ox;
            if (ox < oy)
          ...
          gprintf(gettext("Singular point %gen, point %gen direction %gen kind (%gen,%gen)\nTaylor "
                          "expansions %gen"),
                  makevecteur(symb_equal(t__IDNT_e, *it), makevecteur(fx, gx), makevecteur(ax, ay),
                              o1, o2, makevecteur(vx, vy)),
                  1, contextptr);
        ...
        if (!is_undef(fx) && !is_inf(fx) && !is_undef(gx) && !is_inf(gx)) {
          gen pnt = _point(
              makesequence(fx, gx, write_legende(makevecteur(fx, gx), exactlegende, contextptr),
                           symb_equal(at_couleur, equalposcomp(infl, *it) ? _RED : _MAGENTA)),
              contextptr);
          poi.push_back(pnt);
          if (singp) {
            vecteur ve = makevecteur(_point(makesequence(fx, gx), contextptr),
                                     makevecteur(ax / n, ay / n), symb_equal(at_couleur, _BLUE));
      ...
      if (is_inf(fx)) {
        if (!is_inf(fy)) {
          ...
          if (is_greater(fy, ymax, contextptr))
            ymax = fy;
          gprintf(gettext("Horizontal asymptote at %gen : %gen"), makevecteur(*it, equ), 1,
                  contextptr);
          gen dr = _droite(makesequence(equ, write_legende(equ, exactlegende, contextptr),
                                        symb_equal(at_couleur, _RED)),
                           contextptr);
        ...
        if (is_inf(a)) {
          ...
          continue;
        if (is_zero(a)) {
          gprintf(gettext("Horizontal parabolic asymptote at %gen"), vecteur(1, *it), 1,
                  contextptr);
          continue;
        }
        ...
        if (is_undef(b))
          continue;
        if (is_inf(b)) {
          gprintf(gettext("Parabolic asymptote direction at %gen: %gen"),
                  makevecteur(*it, symb_equal(y__IDNT_e, a * x__IDNT_e)), 1, contextptr);
          continue;
    ...
    for (size_t i = 0; i < ep.size(); ++i) {
      if (is_greater(ep[i], tmin0, contextptr) && is_greater(tmax0, ep[i], contextptr) &&
          in_domain(df, t, ep[i], contextptr))
        tvx.push_back(ep[i]);
    }
    comprim(tvx);
    ...
    vecteur tvidf = makevecteur(symb_equal(symbolic(at_derive, x__IDNT_e), f1),
                                try_limit_undef(f1, xid, nextt, 1, contextptr));
    vecteur tvidg = makevecteur(symb_equal(symbolic(at_derive, y__IDNT_e), g1),
                                try_limit_undef(g1, xid, nextt, 1, contextptr));
    vecteur tviconv = makevecteur(
        symbolic(at_derive, x__IDNT_e) * symbolic(at_derive, symbolic(at_derive, y__IDNT_e)) -
            symbolic(at_derive, y__IDNT_e) * symbolic(at_derive, symbolic(at_derive, x__IDNT_e)),
        try_limit_undef(conv, xid, nextt, 1, contextptr));
  ...
  int step_param(const gen & f, const gen & g, const gen & t, gen & tmin, gen & tmax, vecteur & poi,
                 vecteur & tvi, bool printtvi, bool exactlegende, GIAC_CONTEXT) {
    bool c = complex_mode(contextptr);
    int st = step_infolevel(contextptr), s = 0;
    if (t == x__IDNT_e || t == y__IDNT_e)
      *logptr(contextptr)
          << gettext("Warning, using x or y as variable in parametric plot may lead to confusion!")
          << '\n';
    step_infolevel(0, contextptr);
#ifdef NO_STDEXCEPT
  ...
  int step_func_(const gen & f, const gen & x, gen & xmin, gen & xmax, vecteur & poi, vecteur & tvi,
                 gen & periode, vecteur & asym, vecteur & parab, vecteur & crit, vecteur & infl,
                 bool printtvi, bool exactlegende, GIAC_CONTEXT, int do_inflex_tabsign) {
    if (x.type != _IDNT)
      return 0;
    if (do_inflex_tabsign != 2)
      gprintf(gettext("====================\nFunction plot %gen, variable %gen"), makevecteur(f, x),
              1, contextptr);
    ...
    iterateur it = df1._VECTptr->begin(), itend = df1._VECTptr->end();
    for (; it != itend; ++it) {
      if (in_domain(dflarge, x, *it, contextptr) && is_greater(*it, xmin, contextptr) &&
          is_greater(xmax, *it, contextptr)) {
        sing.push_back(*it);
      }
    }
    // Extremas
    ...
    try {
      ...
      c2 = (!(do_inflex_tabsign & 1) || is_zero(f2))
               ? gen(vecteur(0))
               : solve(_numer(f2, contextptr), x, periode == 0 ? 2 : 0, contextptr);
      c = c1;
#ifndef NO_STDEXCEPT
    } catch (std::runtime_error & err) {
      calc_mode(cm, contextptr);
  ...
  if (c.type != _VECT) {
    *logptr(contextptr) << gettext("Unable to find critical points") << '\n';
    ...
    for (; it != itend; ++it) {
      ...
      if (!lop(*it, at_rootof).empty())
        *it = re(evalf(*it, 1, contextptr), contextptr);
      if (in_domain(df, x, *it, contextptr) && is_greater(*it, xmin, contextptr) &&
          is_greater(xmax, *it, contextptr)) {
        crit.push_back(*it);
        gen fx = try_limit_undef(f, xid, *it, 0, contextptr);
        fx = recursive_normal(fx, contextptr);
        if (!is_undef(fx) && !is_inf(fx)) {
          if (1 || exactlegende)
            poi.push_back(_point(
                makesequence(*it, fx, write_legende(makevecteur(*it, fx), exactlegende, contextptr),
                             symb_equal(at_couleur, equalposcomp(infl, *it) ? _GREEN : _MAGENTA)),
                contextptr));
          else {
            ...
            if (abscisse.type == _DOUBLE_ && ordonnee.type == _DOUBLE_)
              poi.push_back(
                  _point(makesequence(
                             *it, fx,
                             write_legende(string2gen(print_DOUBLE_(abscisse._DOUBLE_val, 3) + "," +
                                                          print_DOUBLE_(ordonnee._DOUBLE_val, 3),
                                                      false),
                                           exactlegende, contextptr),
                             symb_equal(at_couleur, _MAGENTA)),
                         contextptr));
          }
        }
      }
    ...
    for (; do_inflex_tabsign != 2 && it != itend; ++it) {
      ...
      if (!has_inf_or_undef(*it)) { // vertical
        ...
        if (is_inf(l)) {
          ...
          poi.push_back(
              _droite(makesequence(*it, *it + cst_i, write_legende(equ, exactlegende, contextptr),
                                   symb_equal(at_couleur, _RED)),
                      contextptr));
          if (eo && *it != 0) {
            equ = symb_equal(x__IDNT_e, -*it);
            ...
            poi.push_back(_droite(makesequence(-*it, -*it + cst_i,
                                               write_legende(equ, exactlegende, contextptr),
                                               symb_equal(at_couleur, _RED)),
                                  contextptr));
          }
        }
      ...
      if (!is_inf(l)) {
        ...
        gen dr = _droite(makesequence(l * cst_i, l * cst_i + 1,
                                      write_legende(equ, exactlegende, contextptr),
                                      symb_equal(at_couleur, _RED)),
                         contextptr);
        if (!equalposcomp(poi, dr))
        ...
        if (eo == 2 && *it != 0 && l != 0) {
          ...
          dr = _droite(makesequence(-l * cst_i, -l * cst_i + 1,
                                    write_legende(equ, exactlegende, contextptr),
                                    symb_equal(at_couleur, _RED)),
                       contextptr);
          if (!equalposcomp(poi, dr))
      ...
      if (is_inf(b)) {
        parab.push_back(makevecteur(*it, a));
        gprintf(gettext("Parabolic asymptote direction %gen at infinity"),
                vecteur(1, symb_equal(y__IDNT_e, a * x__IDNT_e)), 1, contextptr);
        continue;
      } else if (!lidnt(evalf(b, 1, contextptr)).empty())
        continue;
    ...
    vecteur ep = endpoints(df);
    for (size_t i = 0; i < ep.size(); ++i) {
      if (is_greater(ep[i], xmin0, contextptr) && is_greater(xmax0, ep[i], contextptr) &&
          in_domain(df, x, ep[i], contextptr))
        tvx.push_back(ep[i]);
    }
    // add sign/abs
    ...
    if (!lsignabs.empty()) {
      ...
      for (size_t i = 0; i < lsignabs.size(); ++i) {
        tvx = mergevecteur(
            tvx, solve(lsignabs[i]._SYMBptr->feuille, x, periode == 0 ? 2 : 0, contextptr));
      }
    }
    comprim(tvx);
    gen tmp = _sort(tvx, contextptr);
    ...
    gen nothing = string2gen(" ", false);
    vecteur tvidf =
        makevecteur(do_inflex_tabsign == 2 ? f1 : symb_equal(symbolic(at_derive, yof), f1),
                    try_limit_undef(f1, xid, nextx, 1, contextptr));
    vecteur tvidf2;
    if ((do_inflex_tabsign & 1))
      tvidf2 = makevecteur(symbolic(at_derive, symbolic(at_derive, yof)),
                           try_limit_undef(f2, xid, nextx, 1, contextptr));
    int tvs = int(tvx.size());
    for (int i = 1; i < tvs; ++i) {
      if (ctrl_c || interrupted)
        return 0;
      ...
      if (i < tvs - 1 && equalposcomp(sing, nextx)) {
        ...
        y = try_limit_undef(f, xid, nextx, -1, contextptr);
        if (0 && !is_inf(nextx) &&
            !is_zero(recursive_normal(y - try_limit_undef(f, xid, nextx, 1, contextptr),
                                      contextptr))) // should not happen
          y = undef;
        y = recursive_normal(y, contextptr);
        if (!has_inf_or_undef(y) && is_greater(ymin, y, contextptr))
          ymin = y;
        ...
        gen ysecond;
        if (!is_inf(nextx) &&
            !is_zero(recursive_normal(
                y - (ysecond = try_limit_undef(f1, xid, nextx, 1, contextptr)), contextptr)))
    ...
    gprintf(gettext(do_inflex_tabsign == 2 ? "Sign %gen\n%gen" : "Variations %gen\n%gen"),
            makevecteur(f, tvi), 1, contextptr);
#if !defined(EMCC) && !defined(EMCC2)
    if (printtvi && step_infolevel(contextptr) == 0)
      *logptr(contextptr) << tvi << '\n';
#endif
  ...
  int step_func(const gen & f, const gen & x, gen & xmin, gen & xmax, vecteur & poi, vecteur & tvi,
                gen & periode, vecteur & asym, vecteur & parab, vecteur & crit, vecteur & inflex,
                bool printtvi, bool exactlegende, GIAC_CONTEXT, int do_inflex_tabsign) {
    bool c = complex_mode(contextptr);
    int st = step_infolevel(contextptr), s = 0;
    step_infolevel(0, contextptr);
#ifdef NO_STDEXCEPT
    s = step_func_(f, x, xmin, xmax, poi, tvi, periode, asym, parab, crit, inflex, printtvi,
                   exactlegende, contextptr, do_inflex_tabsign);
#else
  try {
    s = step_func_(f, x, xmin, xmax, poi, tvi, periode, asym, parab, crit, inflex, printtvi,
                   exactlegende, contextptr, do_inflex_tabsign);
  ...
  int plot = 0;
    ...
    int periodic = 0;
    if (param)
      periodic = step_param(f._VECTptr->front(), f._VECTptr->back(), x, xmin, xmax, poi, tvi, false,
                            exactlegende, contextptr);
    else {
      gen periode;
      vecteur asym, parab, crit, inflex;
      periodic = step_func(f, x, xmin, xmax, poi, tvi, periode, asym, parab, crit, inflex, false,
                           exactlegende, contextptr, do_inflex_tabsign);
    }
    // round floats in tvi
  ...
  gen _ggbalt(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.is_symb_of_sommet(at_pnt)) {
      ...
      if (x.type == _VECT && x.subtype == _POINT__VECT && x._VECTptr->size() == 3) {
        ...
        return arg(sqrt(pow(v[0], 2, contextptr) + pow(v[1], 2, contextptr), contextptr) +
                       cst_i * v[2],
                   contextptr);
      }
  ...
  gen _ggbsort(const gen & args, GIAC_CONTEXT) {
    if (args.type != _VECT || args._VECTptr->empty() || args._VECTptr->front().type == _VECT)
      return args;
      ...
      while (!finished) {
        ...
        if (s[pos + 1] == '*') {
          ...
          if (g.type == _VECT && g._VECTptr->size() >= 5) {
            txt = "assume(" + g[0].print(contextptr) + "=[" + g[1].print(contextptr) + "," +
                  g[2].print(contextptr) + "," + g[3].print(contextptr) + "," +
                  g[4].print(contextptr) + "])";
          }
        }
        res += txt;
        pos = nextpos;
  ...
  gen _strip(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args._VECTptr->size() == 2 && args._VECTptr->front().type == _STRNG &&
      ...
      return string2gen(strip(*args._VECTptr->front()._STRNGptr, *args._VECTptr->back()._STRNGptr),
                        false);
    return gensizeerr(contextptr);
  }
  ...
  if (is_integral(a)) {
    ...
    if (vs >= 2) {
      const gen & x = v.front();
      const gen & y = v[1];
      if (x.type == _INT_ && x.val >= 0 && x.val < pixel_cols && y.type == _INT_ && y.val >= 0 &&
          y.val < pixel_lines) {
...
void set_pixel(double x, double y, int c, GIAC_CONTEXT) {
  ...
  void draw_line(int x1, int y1, int x2, int y2, int color, GIAC_CONTEXT) {
    if ((absint(x1) & 0x7ffff000) || x1 == -2147483648 || (absint(x2) & 0x7ffff000) ||
        x2 == -2147483648 || (absint(y1) & 0x7ffff000) || y1 == -2147483648 ||
  ...
  void draw_filled_polygon(vector<vector<int>> & L, int xmin, int xmax, int ymin, int ymax,
                           int color, GIAC_CONTEXT) {
    int n = L.size();
  ...
  gen _draw_polygon(const gen & a, GIAC_CONTEXT) {
    ...
    if (!ckmatrix(g) || g._VECTptr->front()._VECTptr->size() != 2 ||
        !vecteur2vectvector_int(*g._VECTptr, 0, v1))
      return gensizeerr(contextptr);
  ...
  void draw_filled_arc(int x, int y, int rx, int ry, int theta1_deg, int theta2_deg, int color,
                       int xmin, int xmax, int ymin, int ymax, bool segment, GIAC_CONTEXT) {
  ...
  void draw_arc(int xc, int yc, int rx, int ry, int color, double t1, double t2, bool q1, bool q2,
                bool q3, bool q4, GIAC_CONTEXT) {
    ...
    while (x <= y) {
      ...
      if (q4) {
        if (y >= -x * t2 && y <= -x * t1)
          set_pixel(xc + x, yc + yeff, color, contextptr);
        if (x >= -y * t2 && x <= -y * t1)
          set_pixel(xc + y, yc + xeff, color, contextptr);
      }
      if (q3) {
        if (y >= x * t1 && y <= x * t2)
          set_pixel(xc - x, yc + yeff, color, contextptr);
      ...
      if (x0.type == _INT_ && y0.type == _INT_ && r.type == _INT_) {
        if (arc & 1) {
          ...
          if ((arc & 2) || (attr & 0x40000000))
            draw_filled_arc(x0.val, y0.val, r.val, ry.val,
                            int(theta1._DOUBLE_val * 180 / M_PI + .5),
                            int(theta2._DOUBLE_val * 180 / M_PI + .5), attr & 0xffff, 0, pixel_cols,
                            0, pixel_lines, false, contextptr);
          draw_arc(x0.val, y0.val, r.val, ry.val, attr & 0xffff, theta1._DOUBLE_val,
                   theta2._DOUBLE_val, contextptr);
        } else {
          if ((arc & 2) || (attr & 0x40000000))
  ...
  static define_unary_function_eval(__draw_filled_circle, &_draw_filled_circle,
                                    _draw_filled_circle_s);
  define_unary_function_ptr5(at_draw_filled_circle, alias_at_draw_filled_circle,
                             &__draw_filled_circle, 0, true);
  ...
  define_unary_function_ptr5(at_draw_filled_arc, alias_at_draw_filled_arc, &__draw_filled_arc, 0,
                             true);
    ...
    if (vs >= 4) {
      ...
      if (x0.type == _INT_ && y0.type == _INT_ && x1.type == _INT_ && y1.type == _INT_) {
        if (rect) {
          ...
          draw_line(x0.val, y0.val, x1.val, y1.val,
                    vs == 4 ? 0 : remove_at_display(v[4], contextptr).val, contextptr);
        return 1;
      }
    }
    return gensizeerr(contextptr);
  ...
  define_unary_function_ptr5(at_draw_rectangle, alias_at_draw_rectangle, &__draw_rectangle, 0,
                             true);
...
#if 1 // def KHICAS
  os_draw_string(
      v[1].val, v[2].val, v.size() > 3 ? remove_at_display(v[3], contextptr).val : _BLACK,
      v.size() > 4 ? remove_at_display(v[4], contextptr).val : _WHITE, s._STRNGptr->c_str());
  return 1;
#else
  v.erase(v.begin());
  v.push_back(s);
  ...
  gen _rgb(const gen & args, GIAC_CONTEXT) {
    ...
    if (a.type == _INT_ && b.type == _INT_ && c.type == _INT_ && a.val >= 0 && b.val >= 0 &&
        c.val >= 0) {
  ...
  gen prediction(const gen & args, int type, GIAC_CONTEXT) {
    ...
    if (type == 0 || type == 2) {
      if (type == 0 &&
          (is_strictly_greater(25, n, contextptr) || is_strictly_greater(.2, p, contextptr) ||
           is_strictly_greater(p, .8, contextptr)))
    ...
    if (type == 1) {
      ...
      if (is_strictly_greater(30, n, contextptr) || is_greater(5, n * p, contextptr) ||
          is_greater(5, n * (1 - p), contextptr))
        return gensizeerr("Unable to predict");
      return makevecteur(max(p - b, 0, contextptr), min(p + b, 1, contextptr));
    }
    return undef;
  ...
  void locate_files(const char * dirname, const char * ext_, vector<string> & v, bool recurse,
                    GIAC_CONTEXT) {
    DIR * dp;
    struct dirent * ep;
    string ext(".");
...
#include <emscripten/fetch.h>
  ...
  string fetch(const string & url) {
    ...
    emscripten_fetch_t * fetch =
        emscripten_fetch(&attr, url.c_str()); // Blocks here until the operation is complete.
