...
namespace giac {
  ...
  static bool variables(const gen & g) {
    return pair(g) && g._VECTptr->front().type == _IDNT && g._VECTptr->back().type == _IDNT &&
  ...
  static int curve_sign(const gen & g, unsigned depth, GIAC_CONTEXT) {
    if (depth > 12 || taille(g, 65) > 64 || is_undef(g) || is_inf(g))
      return 2;
    if (is_zero(g))
      return 0;
    ...
    if (numeric || g.type == _IDNT)
      return is_strictly_greater(g, 0, contextptr)    ? 1
             : is_strictly_greater(-g, 0, contextptr) ? -1
                                                      : 2;
    if (g.is_symb_of_sommet(at_neg)) {
      int s = curve_sign(g._SYMBptr->feuille, depth + 1, contextptr);
      return s == 2 ? 2 : -s;
    }
    ...
    if (g.is_symb_of_sommet(at_pow) && pair(g._SYMBptr->feuille)) {
      ...
      if (sign == 2)
        return 2;
      if (v[1].type == _INT_ && v[1].val) {
        if (!sign)
          return v[1].val > 0 ? 0 : 2;
        return (v[1].val % 2) ? sign : 1;
      }
      if (sign == 1 && curve_rational(v[1]))
        return 1;
      return 2;
    }
    ...
    if (g.is_symb_of_sommet(at_plus) && g._SYMBptr->feuille.type == _VECT) {
      ...
      for (unsigned j = 0; j < powers.size(); ++j) {
        ...
        if (!pair(f) || f._VECTptr->back().type != _INT_ || f._VECTptr->back().val < 0 ||
            f._VECTptr->back().val > 8) {
          bounded = false;
          break;
        }
      }
      if (bounded) {
        gen factored = _factor(g, contextptr);
        if (!is_undef(factored) && factored != g)
          return curve_sign(factored, depth + 1, contextptr);
      }
      const vecteur & v = *g._SYMBptr->feuille._VECTptr;
      bool nonnegative = true, nonpositive = true, positive = false, negative = false;
      for (unsigned j = 0; j < v.size(); ++j) {
        int t = curve_sign(v[j], depth + 1, contextptr);
        positive = positive || t == 1;
        negative = negative || t == -1;
  ...
  static gen curve_coefficient(const curve_polynomial_terms & p, unsigned x, unsigned y) {
    for (unsigned i = 0; i < p.size(); ++i)
      if (p[i].px == x && p[i].py == y)
        return p[i].coefficient;
    return 0;
  }
  static bool curve_add_term(curve_polynomial_terms & p, unsigned x, unsigned y, const gen & c,
                             unsigned & work) {
    if (!work || x + y > 8)
      return false;
    --work;
    ...
    for (unsigned i = 0; i < p.size(); ++i)
      if (p[i].px == x && p[i].py == y) {
        p[i].coefficient += c;
        if (is_zero(p[i].coefficient))
          p.erase(p.begin() + i);
        return true;
      }
    if (p.size() >= 64)
      return false;
    p.push_back(curve_term(x, y, c));
    return true;
  }
  static bool curve_multiply(const curve_polynomial_terms & a, const curve_polynomial_terms & b,
    ...
    for (unsigned i = 0; i < a.size(); ++i)
      for (unsigned j = 0; j < b.size(); ++j)
        if (!curve_add_term(p, a[i].px + b[j].px, a[i].py + b[j].py,
                            a[i].coefficient * b[j].coefficient, work))
          return false;
    return true;
  }
  static bool curve_collect_polynomial(const gen & g, const gen & x, const gen & y,
                                       curve_polynomial_terms & out, unsigned depth,
                                       unsigned & work, GIAC_CONTEXT) {
    if (!work || depth > 16)
      return false;
    --work;
    out.clear();
    if (g == x) {
      out.push_back(curve_term(1, 0, 1));
      return true;
    }
    if (g == y) {
      out.push_back(curve_term(0, 1, 1));
      return true;
    }
    if (!depends(g, x) && !depends(g, y)) {
      if (!is_zero(g))
        out.push_back(curve_term(0, 0, g));
      return true;
    }
    if (g.type != _SYMB)
      return false;
    ...
    if (g.is_symb_of_sommet(at_neg)) {
      if (!curve_collect_polynomial(f, x, y, out, depth + 1, work, contextptr))
        return false;
      for (unsigned i = 0; i < out.size(); ++i)
        out[i].coefficient = -out[i].coefficient;
      return true;
    }
    if (f.type != _VECT)
      return false;
    ...
    if (g.is_symb_of_sommet(at_division) && v.size() == 2) {
      if (depends(v[1], x) || depends(v[1], y))
        return false;
      gen denominator = ratnormal(v[1], contextptr);
      if (!curve_rational(denominator) || is_zero(denominator) ||
          !curve_collect_polynomial(v[0], x, y, out, depth + 1, work, contextptr))
        return false;
      for (unsigned i = 0; i < out.size(); ++i)
        out[i].coefficient = out[i].coefficient / denominator;
      return true;
    }
    if (g.is_symb_of_sommet(at_plus)) {
      ...
      for (unsigned i = 0; i < v.size(); ++i) {
        ...
        if (!curve_collect_polynomial(v[i], x, y, a, depth + 1, work, contextptr))
          return false;
        for (unsigned j = 0; j < a.size(); ++j)
          if (!curve_add_term(out, a[j].px, a[j].py, a[j].coefficient, work))
            return false;
      }
      return true;
    }
    if (g.is_symb_of_sommet(at_prod)) {
      ...
      for (unsigned i = 0; i < v.size(); ++i) {
        curve_polynomial_terms a, p;
        if (!curve_collect_polynomial(v[i], x, y, a, depth + 1, work, contextptr) ||
            !curve_multiply(out, a, p, work))
          return false;
        out.swap(p);
      }
      return true;
    ...
    if (g.is_symb_of_sommet(at_pow) && v.size() == 2 && v[1].type == _INT_ && v[1].val >= 0 &&
        v[1].val <= 8) {
      curve_polynomial_terms a;
      if (!curve_collect_polynomial(v[0], x, y, a, depth + 1, work, contextptr))
        return false;
      out.push_back(curve_term(0, 0, 1));
      for (int i = 0; i < v[1].val; ++i) {
        curve_polynomial_terms p;
        if (!curve_multiply(out, a, p, work))
          return false;
        out.swap(p);
      }
      return true;
  ...
  static bool curve_polynomial(const gen & g, const gen & x, const gen & y,
                               curve_polynomial_terms & out, GIAC_CONTEXT) {
    if (taille(g, 513) > 512)
      return false;
    unsigned work = 4096;
    return curve_collect_polynomial(g, x, y, out, 0, work, contextptr);
  }
  static bool curve_folium_param(const curve_polynomial_terms & p, const gen & t, gen & out,
                                 GIAC_CONTEXT) {
    if (p.size() != 2 && p.size() != 3)
      return false;
    gen L = ratnormal(curve_coefficient(p, 3, 0), contextptr);
    if (!curve_rational(L) || is_zero(L) ||
        !is_zero(ratnormal(curve_coefficient(p, 0, 3) - L, contextptr)))
      return false;
    gen K = -curve_coefficient(p, 1, 1);
    if (is_zero(K)) {
      if (p.size() != 2)
        return false;
      out = vecteur(1, makevecteur(t, -t));
      return true;
    }
    if (p.size() != 3 || curve_proved_sign(K, contextptr) == 2)
      return false;
  ...
  static bool curve_lemniscate_param(const curve_polynomial_terms & p, const gen & t, gen & out,
                                     GIAC_CONTEXT) {
    if (p.size() != 3 && p.size() != 5)
      return false;
    gen L = curve_coefficient(p, 4, 0);
    int sl = curve_proved_sign(L, contextptr);
    if (sl == 0 || sl == 2)
      return false;
    if (!is_zero(ratnormal(curve_coefficient(p, 0, 4) - L, contextptr)) ||
        !is_zero(ratnormal(curve_coefficient(p, 2, 2) - 2 * L, contextptr)) ||
        !is_zero(ratnormal(curve_coefficient(p, 2, 0) + curve_coefficient(p, 0, 2), contextptr)))
      return false;
    gen K = ratnormal(curve_coefficient(p, 0, 2) / L, contextptr);
    if (is_zero(K)) {
      if (p.size() != 3)
        return false;
      out = vecteur(1, makevecteur(0, 0));
      return true;
    }
    if (p.size() != 5 || !angle_radian(contextptr))
      return false;
    int sk = curve_proved_sign(K, contextptr);
    if (sk == 2)
      return false;
    bool rotated = sk == -1;
    gen s = sin(t, contextptr), c = cos(t, contextptr),
        X = sqrt(rotated ? -K : K, contextptr) * c / (1 + s * s);
    out = vecteur(1, rotated ? makevecteur(X * s, X) : makevecteur(X, X * s));
    return true;
  }

  static bool curve_origin_pencil(const curve_polynomial_terms & p, const gen & t, gen & out,
                                  GIAC_CONTEXT) {
    if (p.empty())
      return false;
    unsigned low = 9, high = 0;
    for (unsigned i = 0; i < p.size(); ++i) {
      if (!curve_rational(p[i].coefficient))
        return false;
      unsigned n = p[i].px + p[i].py;
      if (n < low)
        low = n;
      if (n > high)
        high = n;
    }
    if (high < 2 || high > 8 || low + 1 != high)
      return false;
    gen P = 0, Q = 0;
    for (unsigned i = 0; i < p.size(); ++i) {
      gen term = p[i].coefficient * pow(t, int(p[i].py));
      if (p[i].px + p[i].py == high)
        P += term;
      else
        Q += term;
    }
    ...
    if (is_undef(common) || depends(common, t))
      return false;
    ...
    if (is_zero(verticalP) && is_zero(verticalQ))
      branches.push_back(makevecteur(0, t));
    else if (!is_zero(verticalP) && !is_zero(verticalQ))
      branches.push_back(makevecteur(0, -verticalQ / verticalP));
    bool origin_at_zero =
        !is_zero(curve_coefficient(p, high, 0)) && is_zero(curve_coefficient(p, high - 1, 0));
    if (!origin_at_zero && !(is_zero(verticalP) && is_zero(verticalQ)))
      branches.push_back(makevecteur(0, 0));
    *logptr(contextptr) << "Rational parametrization: retain nonzero denominator conditions; "
                           "constant branches include missing points.\n";
    out = branches;
    return true;
  }
...
// Complete real trigonometric/hyperbolic charts have no missing endpoints.
...
#endif
  static bool curve_conic_param(const curve_polynomial_terms & p, const gen & t, gen & out,
                                GIAC_CONTEXT) {
    if (!angle_radian(contextptr) || p.empty() || p.size() > 6)
      return false;
    for (unsigned j = 0; j < p.size(); ++j)
      if (p[j].px + p[j].py > 2 || taille(p[j].coefficient, 33) > 32 ||
          !is_zero(im(p[j].coefficient, contextptr)))
        return false;
    gen A = curve_coefficient(p, 2, 0), B = curve_coefficient(p, 1, 1),
        C = curve_coefficient(p, 0, 2);
    gen D = curve_coefficient(p, 1, 0), E = curve_coefficient(p, 0, 1),
        F = curve_coefficient(p, 0, 0);
    ...
    if (is_zero(det)) {
      ...
      if (sl == 1 || sl == -1) {
        gen U = t - D / (2 * A),
            V = ratnormal((-A * t * t + D * D / (4 * A) - F) / linear, contextptr);
    ...
    int sd = curve_proved_sign(det, contextptr);
    if (sd == 2)
      return false;
    gen h = ratnormal((B * E - 2 * C * D) / det, contextptr),
        k = ratnormal((B * D - 2 * A * E) / det, contextptr);
    ...
    if (is_zero(A) && is_zero(C)) {
      ...
      if (is_zero(rho)) {
        out = makevecteur(makevecteur(h + t, k), makevecteur(h, k + t));
        return true;
      }
      gen product = ratnormal(rho / B, contextptr);
      int sp = curve_proved_sign(product, contextptr);
      if (sp == 0 || sp == 2)
        return false;
      bool positive = sp == 1;
      gen radius = sqrt(positive ? product : -product, contextptr), X = radius * exp(t, contextptr),
          Y = (positive ? 1 : -1) * radius * exp(-t, contextptr);
      out = makevecteur(makevecteur(h + X, k + Y), makevecteur(h - X, k - Y));
      return true;
    }
    ...
    if (!is_zero(B)) {
      ...
      if (!simple_rotation && (sa == 1 || sa == -1)) {
        l1 = A;
        l2 = ratnormal(det / (4 * A), contextptr);
        s = ratnormal(B / (2 * A), contextptr);
        sheared = true;
      } else {
        // Exact orthonormal eigenvectors; no atan quadrant or angle ambiguity.
        int sb = curve_proved_sign(B, contextptr);
        if (sb == 0 || sb == 2)
          return false;
        if (!simple_rotation)
          gap = sqrt((A - C) * (A - C) + B * B, contextptr);
    ...
    if (is_zero(rho)) {
      if (sd == 1) {
        out = vecteur(1, swapped ? makevecteur(k, h) : makevecteur(h, k));
        return true;
      }
      gen slope = sqrt(-l1 / l2, contextptr);
      charts.push_back(makevecteur(t, slope * t));
      charts.push_back(makevecteur(t, -slope * t));
    } else {
      ...
      if (pos1 && pos2)
        charts.push_back(makevecteur(sqrt(r1, contextptr) * cos(t, contextptr),
                                     sqrt(r2, contextptr) * sin(t, contextptr)));
      else if (neg1 && neg2) {
        out = vecteur(0);
        return true;
      } else if ((pos1 && neg2) || (neg1 && pos2)) {
        gen U =
            sqrt(pos1 ? r1 : -r1, contextptr) * (pos1 ? cosh(t, contextptr) : sinh(t, contextptr));
        gen V =
            sqrt(pos2 ? r2 : -r2, contextptr) * (pos2 ? cosh(t, contextptr) : sinh(t, contextptr));
        charts.push_back(makevecteur(U, V));
        charts.push_back(pos1 ? makevecteur(-U, V) : makevecteur(U, -V));
      } else
        return false;
    }
    vecteur branches;
    branches.reserve(charts.size());
    for (unsigned j = 0; j < charts.size(); ++j) {
      const vecteur & v = *charts[j]._VECTptr;
      gen X = ratnormal(h + c * v[0] - s * v[1], contextptr),
          Y = ratnormal(k + (sheared ? v[1] : s * v[0] + c * v[1]), contextptr);
  ...
  static bool curve_constant_linear_graph(const gen & f, const vecteur & xy, const gen & t,
                                          int first, gen & out, GIAC_CONTEXT) {
    for (int j = 0; j < 2; ++j) {
      int i = j ? 1 - first : first;
      gen a, b;
      if (!is_linear_wrt(f, xy[i], a, b, contextptr) || !curve_rational(a) || is_zero(a))
        continue;
      gen value = subst(ratnormal(-b / a, contextptr), xy[1 - i], t, false, contextptr);
      out = vecteur(1, i ? makevecteur(t, value) : makevecteur(value, t));
      return true;
    }
    return false;
  }
  ...
  static bool curve_odd_polar_radius(const gen & f, const gen & r, const gen & theta, const gen & t,
                                     gen & out, GIAC_CONTEXT) {
    if (taille(f, 129) > 128)
      return false;
    vecteur terms;
    if (f.is_symb_of_sommet(at_plus) && f._SYMBptr->feuille.type == _VECT)
      terms = *f._SYMBptr->feuille._VECTptr;
    else
      terms.push_back(f);
    if (terms.size() > 8)
      return false;
    gen rhs = 0, coefficient = 0;
    int n = 0;
    for (unsigned i = 0; i < terms.size(); ++i) {
      ...
      if (!g.is_symb_of_sommet(at_pow) || g._SYMBptr->feuille.type != _VECT)
        return false;
      const vecteur & v = *g._SYMBptr->feuille._VECTptr;
      if (v.size() != 2 || v[0] != r || v[1].type != _INT_ || v[1].val < 3 || v[1].val > 9 ||
          !(v[1].val % 2))
        return false;
      if (n && n != v[1].val)
        return false;
    ...
    if (!n || !curve_rational(coefficient) || is_zero(coefficient))
      return false;
    ...
    if (!is_zero(im(value, contextptr)))
      return false;
    ...
    if (curve_rational(value))
      ...
      radius = gen(symbolic(at_sign, value)) *
               gen(symbolic(at_pow, makesequence(gen(symbolic(at_abs, value)), gen(1) / n)));
    out = vecteur(1, makevecteur(radius * cos(t, contextptr), radius * sin(t, contextptr)));
    return true;
  }

  static bool curve_polar_nonzero(const gen & g, GIAC_CONTEXT) {
    int s = curve_proved_sign(g, contextptr);
    return s == 1 || s == -1;
  }
  ...
  static bool curve_conic_polar(const curve_polynomial_terms & p, const gen & r, const gen & theta,
                                gen & result, GIAC_CONTEXT) {
    if (p.empty() || p.size() > 6)
      return false;
    for (unsigned j = 0; j < p.size(); ++j)
      if (p[j].px + p[j].py > 2 || taille(p[j].coefficient, 33) > 32 ||
    ...
    if (is_zero(A) && is_zero(B) && is_zero(C)) {
      ...
      if ((is_zero(D) && is_zero(E)) || !curve_polar_nonzero(F, contextptr))
        return false;
      result = symb_equal(r, ratnormal(-F / L, contextptr));
      *logptr(contextptr) << "Real polar radius: denominator != 0.\n";
      return true;
    }
    gen H = is_zero(B) ? (A == C ? A : A * cs * cs + C * sn * sn)
                       : A * cs * cs + B * cs * sn + C * sn * sn;
    ...
    bool definite = curve_proved_sign(ratnormal(4 * A * C - B * B, contextptr), contextptr) == 1 &&
                    curve_polar_nonzero(A, contextptr);
    if (is_zero(D) && is_zero(E)) {
      if (!definite && !curve_polar_nonzero(F, contextptr))
        return false;
      if (definite && curve_proved_sign(-F / A, contextptr) == -1) {
        result = vecteur(0);
        return true;
      }
      result = symb_equal(r, curve_polar_sqrt(ratnormal(-F / H, contextptr), contextptr));
      return true;
    }
    if (!definite)
      return false;
    if (is_zero(F)) {
      ...
      result = symb_equal(r, ratnormal(-L / H, contextptr));
      return true;
    }
    ...
    result = first == second ? first : gen(makevecteur(first, second));
    return true;
  }

  static bool curve_compact_polar(const gen & g, const gen & x, const gen & y, const gen & r,
                                  const gen & theta, gen & result, GIAC_CONTEXT) {
    curve_polynomial_terms p;
    if (!curve_polynomial(g, x, y, p, contextptr))
      return false;
    if (curve_conic_polar(p, r, theta, result, contextptr))
      return true;
    ...
    if (curve_polar_nonzero(leading, contextptr) && curve_coefficient(p, 0, 4) == leading &&
      ...
      for (unsigned i = 0; i < p.size(); ++i) {
        ...
        if (!((t.px == 4 && t.py == 0) || (t.px == 0 && t.py == 4) || (t.px == 2 && t.py == 2) ||
              (t.px == 2 && t.py == 0) || (t.px == 0 && t.py == 2)))
          shape = false;
      }
      if (shape) {
        gen k = -xx / leading;
        if (is_undef(k) || is_inf(k))
          return false;
        result = symb_equal(r, curve_polar_sqrt(k * cos(2 * theta, contextptr), contextptr));
        *logptr(contextptr)
            << "Same Cartesian curve; origin retained. Angle labels at the origin may differ.\n";
        return true;
      }
    }
    leading = curve_coefficient(p, 3, 0);
    if (!curve_polar_nonzero(leading, contextptr) || curve_coefficient(p, 0, 3) != leading)
      return false;
    for (unsigned i = 0; i < p.size(); ++i) {
      const curve_term & t = p[i];
      if (!((t.px == 3 && t.py == 0) || (t.px == 0 && t.py == 3) || (t.px == 1 && t.py == 1)))
        return false;
    }
    gen k = -curve_coefficient(p, 1, 1) / leading;
    if (is_undef(k) || is_inf(k))
      return false;
    gen sine = sin(theta, contextptr), cosine = cos(theta, contextptr),
        denominator = pow(cosine, 3) + pow(sine, 3);
    if (curve_polar_nonzero(k, contextptr)) {
      result = symb_equal(r, k * sine * cosine / denominator);
      *logptr(contextptr) << "Same Cartesian curve; origin retained. Explicit radius excludes "
                          << denominator << "=0.\n";
    } else {
      ...
      *logptr(contextptr) << "Same Cartesian curve; origin retained. Implicit radius preserves "
  ...
  static gen equation(const gen & g, GIAC_CONTEXT) {
    if (is_undef(g))
      return g;
    gen result;
    if (equation_primitive(symb_equal(g, 0), result, contextptr))
      return result;
  ...
  gen _cart2polar(const gen & args, GIAC_CONTEXT) {
    ...
    if (!angle_radian(contextptr))
      return gensizeerr("Equation conversion requires radians");
    ...
    if (curve_compact_polar(input, xy[0], xy[1], w[0], w[1], compact, contextptr))
      return compact;
    gen converted =
        subst(input, xy, makevecteur(w[0] * cos(w[1], contextptr), w[0] * sin(w[1], contextptr)),
              false, contextptr);
  ...
  gen _cart2param(const gen & args, GIAC_CONTEXT) {
    ...
    if (depends(v[0], v[2]))
      return gensizeerr("Parameter already occurs in input");
    const vecteur & xy = *v[1]._VECTptr;
    gen f = residual(v[0]);
    ...
    gen direct;
    if (curve_constant_linear_graph(f, xy, v[2], 1, direct, contextptr))
      return direct;
    ...
    if (bounded_polynomial && (curve_conic_param(polynomial, v[2], direct, contextptr) ||
                               ...
                               curve_folium_param(polynomial, v[2], direct, contextptr)))
      return direct;
    ...
    for (unsigned i = 0; i < factors.size(); ++i) {
      ...
      if (part.is_symb_of_sommet(at_pow) && pair(part._SYMBptr->feuille)) {
        ...
        if (power[1].type == _INT_) {
          if (power[1].val <= 0)
            continue; // denominator exclusions stay with the input
          part = power[0];
        }
      }
      if (depends(part, xy[0]) || depends(part, xy[1]))
    ...
    if (components.size() > 1) {
      ...
      for (unsigned i = 0; i < components.size(); ++i) {
        gen branches =
            _cart2param(makesequence(symb_equal(components[i], 0), v[1], v[2]), contextptr);
        if (is_undef(branches) || branches.type != _VECT)
          return branches;
    ...
    if (bounded_polynomial) {
      ...
      for (unsigned i = 0; i < polynomial.size(); ++i) {
        if (polynomial[i].px > dx)
          dx = polynomial[i].px;
        if (polynomial[i].py > dy)
          dy = polynomial[i].py;
      }
      if (dx && (!dy || dx < dy))
        solved = 0;
    }
    if (curve_constant_linear_graph(f, xy, v[2], solved, direct, contextptr))
      return direct;
    if (bounded_polynomial && curve_origin_pencil(polynomial, v[2], direct, contextptr))
      return direct;
  ...
  gen _polar2param(const gen & args, GIAC_CONTEXT) {
    ...
    if (depends(v[0], v[2]))
      return gensizeerr("Parameter already occurs in input");
    gen direct;
    if (curve_odd_polar_radius(residual(v[0]), rt[0], rt[1], v[2], direct, contextptr))
      return direct;
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool curve_quadratic_image(const vecteur & p, const gen & t, const gen & rt, gen & result,
                                    GIAC_CONTEXT) {
    if (complex_mode(contextptr) || complex_variables(contextptr) || taille(gen(p), 97) > 96)
      return false;
    gen N[2], D[2];
    unsigned budget = 192;
    ...
    for (unsigned j = 0; j < 2; ++j) {
      vecteur vars = lvar(p[j]);
      for (unsigned k = 0; k < vars.size(); ++k)
        if (vars[k].type != _IDNT)
          return false;
      ...
      for (unsigned k = 0; k < powers.size(); ++k) {
        ...
        if (f.type != _VECT || f._VECTptr->size() != 2 || f[1].type != _INT_ || f[1].val < -2 ||
          ...
          return false;
        if (f[1].val < 0)
          excluded.push_back(f[0]);
      }
      vecteur invs = mergevecteur(lop(p[j], at_inv), lop(p[j], at_division));
      for (unsigned k = 0; k < invs.size(); ++k)
        excluded.push_back(invs[k].is_symb_of_sommet(at_inv) ? invs[k]._SYMBptr->feuille
                                                             : invs[k]._SYMBptr->feuille[1]);
      if (!determinant_fraction(p[j], N[j], D[j], budget, 0))
        return false;
      if (!equation_polynomial_bound(N[j], budget, 0, bound) || bound.degree > 4 ||
          bound.terms > 16 || !equation_polynomial_bound(D[j], budget, 0, bound) ||
          bound.degree > 2 || bound.terms > 4)
        return false;
    }
    if (excluded.size() > 4)
      return false;
    gen den = D[0] == D[1] ? D[0] : D[0] * D[1];
    budget = 128;
    if (!equation_polynomial_bound(den, budget, 0, bound) || bound.degree > 2)
      return false;
    vecteur vars = lidnt(den);
    if (!vars.empty() && (vars.size() != 1 || vars[0] != t))
      return false;
    for (unsigned j = 0; j < excluded.size(); ++j) {
      if (!equation_polynomial_bound(excluded[j], budget, 0, bound) || bound.degree > 2)
        return false;
      gen quotient = ratnormal(den / excluded[j], contextptr);
      if (!equation_polynomial_bound(quotient, budget, 0, bound))
        return false;
    }
    gen rows[3] = {ratnormal(N[0] * (den / D[0]), contextptr),
                   ratnormal(N[1] * (den / D[1]), contextptr), den};
    gen M[3][3], adj[3][3];
    for (unsigned j = 0; j < 3; ++j) {
      for (unsigned k = 0; k < 3; ++k)
        M[j][k] = _coeff(makesequence(rows[j], t, int(2 - k)), contextptr);
      if (!is_zero(ratnormal(rows[j] - M[j][0] * t * t - M[j][1] * t - M[j][2], contextptr)))
        return false;
      for (unsigned k = 0; k < 3; ++k)
        if (contains(M[j][k], t) || !is_zero(im(M[j][k], contextptr)))
          return false;
    }
    for (unsigned j = 0; j < 3; ++j)
      for (unsigned k = 0; k < 3; ++k)
        adj[j][k] = ratnormal(M[(k + 1) % 3][(j + 1) % 3] * M[(k + 2) % 3][(j + 2) % 3] -
                                  M[(k + 1) % 3][(j + 2) % 3] * M[(k + 2) % 3][(j + 1) % 3],
                              contextptr);
    gen det =
        ratnormal(M[0][0] * adj[0][0] + M[0][1] * adj[1][0] + M[0][2] * adj[2][0], contextptr);
    if (is_zero(det))
      return false;
    gen parameter, root, point;
    ...
    if (!equation_rational(det)) {
      ...
      vars = lidnt(det);
      budget = 128;
      if (vars.size() != 1 || !equation_polynomial_bound(det, budget, 0, bound) || bound.degree > 4)
        return false;
      parameter = vars[0];
      int degree = int(bound.degree);
      gen leading = _coeff(makesequence(det, parameter, degree), contextptr);
      if (!equation_rational(leading) || is_zero(leading))
        return false;
      root =
          -_coeff(makesequence(det, parameter, degree - 1), contextptr) / (gen(degree) * leading);
      if (!equation_rational(root) ||
          !is_zero(
              ratnormal(det - leading * pow(parameter - root, degree, contextptr), contextptr)))
        return false;
      gen sample, divisor;
      for (int n = 0; n < 3; ++n) {
        sample = n - 1;
        divisor = eval(subst(den, t, sample, false, contextptr), 1, contextptr);
        if (equation_rational(divisor) && !is_zero(divisor))
          break;
      }
      if (!equation_rational(divisor) || is_zero(divisor))
        return false;
      ...
      for (unsigned j = 0; j < 2; ++j) {
        ...
        if (!is_zero(ratnormal(numerator - c[j] * den, contextptr)))
          return false;
      }
      point = is_zero(c[0]) && is_zero(c[1])
                  ? symb_equal(rt[0], 0)
                  : symb_and(symb_equal(rt[0] * cos(rt[1], contextptr), c[0]),
                             symb_equal(rt[0] * sin(rt[1], contextptr), c[1]));
      degenerates = true;
    }
    gen xy =
        makevecteur(gen(identificateur(" khicas_curve_x")), gen(identificateur(" khicas_curve_y")));
    gen q[3];
    for (unsigned j = 0; j < 3; ++j)
      q[j] = adj[j][0] * xy[0] + adj[j][1] * xy[1] + adj[j][2];
    ...
    chart = symbolic(at_when,
                     makesequence(symb_equal(rt[0], 0),
                                  is_zero(origin) ? symb_equal(rt[0], 0) : symb_equal(origin, 0),
                                  chart));
    gen exclusion = ratnormal(
        subst(q[2], xy, makevecteur(rt[0] * cos(rt[1], contextptr), rt[0] * sin(rt[1], contextptr)),
              false, contextptr),
        contextptr);
    result = symbolic(at_when, makesequence(symb_equal(exclusion, 0), undef, chart));
    if (degenerates)
      result = symbolic(at_when, makesequence(symb_equal(parameter, root), point, result));
    return true;
  }
  ...
  gen _param2polar(const gen & args, GIAC_CONTEXT) {
    ...
    if (v.size() == 3) {
      ...
      if (depends(v[0], rt[0]) || depends(v[0], rt[1]))
        return gensizeerr("Output coordinates already occur in input");
      gen image;
      if (curve_quadratic_image(p, v[1], v[2], image, contextptr))
        return image;
      gen xy = makevecteur(gen(identificateur(" khicas_curve_x")),
                           gen(identificateur(" khicas_curve_y")));
  ...
  gen _param2cart(const gen & args, GIAC_CONTEXT) {
    ...
    if (!rational_in(a, t) || !rational_in(b, t)) {
      if (!angle_radian(contextptr))
        return gensizeerr("Trigonometric conversion requires radians");
      ...
      if (depends(v[0], u))
        return gensizeerr("Internal parameter collision");
    ...
    *logptr(contextptr)
        << "Algebraic closure: retain parameter range and denominator exclusions.\n";
