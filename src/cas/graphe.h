...
namespace giac {
  ...
  public:
    ...
    public:
      ...
      const gen & label() const {
        return m_label;
      }
      void set_label(const gen & s) {
        m_label = s;
      }
      void set_subgraph(int s) {
        m_subgraph = s;
      }
      int subgraph() const {
        return m_subgraph;
      }
      void set_embedded(bool yes) {
        m_embedded = yes;
      }
      ...
      void set_prelim(double val) {
        m_prelim = val;
      }
      double prelim() const {
        return m_prelim;
      }
      void set_modifier(double val) {
        m_modifier = val;
      }
      double modifier() const {
        return m_modifier;
      }
      const attrib & attributes() const {
        return m_attributes;
      }
      attrib & attributes() {
        return m_attributes;
      }
      void set_attribute(int key, const gen & val) {
        m_attributes[key] = val;
      }
      void set_attributes(const attrib & attr) {
        copy_attributes(attr, m_attributes);
      }
      const ivector & neighbors() const {
        return m_neighbors;
      }
    ...
    public:
      ...
      const attrib & vertex_attributes() const {
        return vertex_attr;
      }
      const attrib & edge_attributes() const {
        return edge_attr;
      }
      const attrib & chain_attributes() const {
        return chain_attr;
      }
      ...
      void incr() {
        ++pos;
        if (int(m_chain.size()) <= pos)
          m_chain.resize(pos + 1, 0);
      }
      void clear_chain() {
        pos = 0;
        m_chain.resize(1);
      ...
      bool chain_completed() {
        return m_chain.back() != 0;
      }
      bool chain_empty() {
        return pos == 0 && m_chain.front() == 0;
      }
    };
    ...
    class matching_maximizer {
      ...
      void append_non_blossom_adjacents(int v, std::map<int, ivector>::const_iterator bit,
                                        ivector & lst);
      ivector adjacent(int v);
      ipair make_edge(int i, int j) {
        return std::make_pair(i < j ? i : j, i < j ? j : i);
      }
  ...
  private:
    ...
    static void multiply_sparse_matrices(const sparsemat & A, const sparsemat & B, sparsemat & P,
                                         int ncols, bool symmetric = false);
    ...
    void tomita_recurse(const ivector & R, const ivector & P_orig, const ivector & X_orig,
                        ivectors & cliques) const;
    ...
    static bool embed_rectangles(const std::vector<rectangle> & rectangles, dpairs & embedding,
                                 double ew, double eh, double eps);
    static bool segments_crossing(const point & p, const point & r, const point & q,
                                  const point & s, point & crossing);
    static bool point2segment_projection(const point & p, const point & q, const point & r,
                                         point & proj);
    void accumulate_repulsive_force(const point & p, const point & q, double R, double K,
                                    double eps, point & force, bool sq_dist = false);
    void force_directed_placement(layout & x, double K, double R = DBL_MAX, double tol = 0.001,
                                  bool ac = true);
    ...
    static void append_segment(vecteur & drawing, const point & p, const point & q, int color,
                               int width, bool arrow = false);
    ...
    bool edges_crossing(const ipair & e1, const ipair & e2, const layout & x,
                        point & crossing) const;
  ...
  public:
    ...
    int rand_integer(int n) const {
      return giac::giac_rand(ctx) % n;
    }
    double rand_uniform() const {
      return giac::giac_rand(ctx) / (RAND_MAX + 1.0);
    }
    double rand_normal() const {
      return giac::randNorm(ctx);
    }
    static bool is_real_number(const gen & g);
    static gen to_binary(int number, int chars);
    const context * giac_context() const {
      return ctx;
    }
    ...
    void copy_nodes(const std::vector<vertex> & V) {
      nodes = std::vector<vertex>(V.begin(), V.end());
    }
    ...
    const ivector & get_marked_nodes() const {
      return marked_nodes;
    }
    void get_marked_nodes(vecteur & V) const;
    void get_marked_nodes_in_subgraph(int s, ivector & m) const;
    void copy_marked_nodes(const ivector & mv) {
      marked_nodes = ivector(mv.begin(), mv.end());
    }
    void mark_node(int v);
    void mark_node(const gen & v) {
      mark_node(node_index(v));
    }
    bool unmark_node(int v);
    bool unmark_node(const gen & v) {
      return unmark_node(node_index(v));
    }
    void unmark_all_nodes() {
      marked_nodes.clear();
    }
    void sort_marked_nodes() {
      std::sort(marked_nodes.begin(), marked_nodes.end());
    }
    gen to_gen() const;
    ...
    void dfs(int root, bool rec = true, bool clr = true, ivector * D = NULL, int sg = -1,
             bool skip_embedded = false);
    void bfs(int root, bool rec = true, bool clr = true, ivector * D = NULL, int sg = -1,
             bool skip_embedded = false);
    const ivector & get_discovered_nodes() const {
    ...
    int add_node(const gen & v, const attrib & attr) {
      int i = add_node(v);
      nodes[i].set_attributes(attr);
      return i;
    }
    ...
    const vertex & node(int i) const {
      return nodes[i];
    }
    const gen & node_label(int i) const {
    ...
    const attrib & graph_attributes() const {
      return attributes;
    }
    ...
    void add_edge(const ipair & edge) {
      add_edge(edge.first, edge.second);
    }
    void add_edge(const ipair & edge, const attrib & attr) {
    ...
    bool remove_edge(const ipair & p) {
      return remove_edge(p.first, p.second);
    }
    bool has_edge(int i, int j) const;
    bool has_edge(ipair p) const {
      return has_edge(p.first, p.second);
    }
    ...
    void set_graph_attribute(int key, const gen & val) {
      attributes[key] = val;
    }
    void set_graph_attributes(const attrib & attr) {
      copy_attributes(attr, attributes);
    }
    ...
    void set_directed(bool yes) {
      set_graph_attribute(_GT_ATTRIB_DIRECTED, boole(yes));
    }
    void set_weighted(bool yes) {
      set_graph_attribute(_GT_ATTRIB_WEIGHTED, boole(yes));
    }
    ...
    void make_circular_layout(layout & x, const ivector & outer_face, bool planar = false,
                              double tol = 0.001);
    ...
    void connected_components(ivectors & components, int sg = -1, bool skip_embedded = false,
                              int * count = NULL);
    ...
    void dijkstra(int src, const ivector & dest, vecteur & path_weights,
                  ivectors * cheapest_paths = NULL);
