...
#ifdef HAVE_UNISTD_H
#include <unistd.h>
#if !defined(NSPIRE) && !defined FXCG && !defined(__VISUALC__) &&                                  \
    !defined(NUMWORKS) // #ifndef NSPIRE
#include <dirent.h>
#ifndef __MINGW_H
#include <pwd.h>
#endif // MINGW
...
namespace giac {
  ...
  static define_unary_function_eval4_index(10, __division, &_division, _division_s,
                                           &printasdivision, &texprintsommetasoperator);
  ...
  gen _EXPM1(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.type == _DOUBLE_ && g._DOUBLE_val < 0.01 && g._DOUBLE_val > -0.01) {
      ...
      double r =
          (((((x / 5040 + 1. / 720) * x + 1. / 120) * x + 1. / 24) * x + 1. / 6) * x + .5) * x + 1;
      return x * r;
    }
...
#endif
  static define_unary_function_eval(
      __polar_complex, &_polar_complex,
      _polar_complex_s); //,&printsommetasoperator,&texprintsommetasoperator);
  define_unary_function_ptr5(at_polar_complex, alias_at_polar_complex, &__polar_complex, 0,
                             T_UNARY_OP);     // T_MOD
