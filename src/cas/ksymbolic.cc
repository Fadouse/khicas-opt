...
namespace giac {
  ...
  const unary_function_ptr * archive_function_tab() {
    static const unary_function_ptr archive_function_tab_ptr[] = {*at_plus,
                                                                  *at_neg,
                                                                  *at_binary_minus,
                                                                  *at_prod,
                                                                  *at_division,
                                                                  ...
                                                                  *at_try_catch,
                                                                  0};
    archive_function_tab_length =
        sizeof(archive_function_tab_ptr) / sizeof(const unary_function_ptr *);
...
static const size_t archive_function_tab_alias[] = {
    ...
    // 72
    alias_at_inferieur_egal, alias_at_superieur_strict, alias_at_superieur_egal, alias_at_different,
    alias_at_equal,
    // 82
    alias_at_rpn_prog, alias_at_local, alias_at_return, alias_at_Dialog,
    alias_at_double_deux_points,
    // 92
    alias_at_pointprod, alias_at_pointdivision, alias_at_pointpow, alias_at_hash, alias_at_pourcent,
    // 102
    alias_at_tilocal, alias_at_break, alias_at_continue, alias_at_ampersand_times,
    alias_at_maple_lib,
    // 112
    ...
    // 172
    alias_at_pointminus, alias_at_struct_dot, alias_at_try_catch, 0};
const unary_function_ptr * _archive_function_tab =
    (const unary_function_ptr *)&archive_function_tab_alias;
const unary_function_ptr * archive_function_tab() {
...
int archive_function_tab_length = sizeof(archive_function_tab_alias) / sizeof(size_t);
  ...
  static string & add_print_plus(string & s, const symbolic & g, GIAC_CONTEXT) {
    ...
    for (int i = 1; i < l; ++i) {
      ...
      if (v[i].type == _SYMB) {
        ...
        if (u == at_superieur_egal || u == at_superieur_strict || u == at_inferieur_strict ||
            u == at_inferieur_egal)
          par = true;
      }
      if (par)
  ...
  static string & add_print_neg(string & s, const gen & feuille, GIAC_CONTEXT) {
    ...
    if (feuille.type != _CPLX) {
      if (feuille.type != _SYMB ||
          (feuille._SYMBptr->sommet == at_inv || feuille._SYMBptr->sommet == at_prod) ||
          !need_parenthesis(feuille)) {
  ...
  static string & add_print_inv(string & s, const gen & feuille, GIAC_CONTEXT) {
    ...
    bool bt =
        f.type == _SYMB && (f._SYMBptr->sommet == at_plus || f._SYMBptr->sommet == at_inv ||
                            f._SYMBptr->sommet == at_prod || need_parenthesis(f._SYMBptr->sommet));
  ...
  static string & add_print_prod(string & s, const symbolic & g, GIAC_CONTEXT) {
    gen n0, d0;
    if (print_rewrite_prod_inv && rewrite_prod_inv(g.feuille, n0, d0)) {
    ...
    for (int i = 0; i < l; ++i) {
      ...
      if (e.type != _SYMB) {
        if (i)
          s += '*';
        if ((e.type == _CPLX && need_parenthesis(e)) || e.type == _MOD) {
          s += "(";
        ...
        if (e._SYMBptr->sommet == at_inv) {
          ...
          if (i) {
            if ((f.type == _CPLX) || (f.type == _MOD) ||
                ((f.type == _SYMB) &&
                 ((f._SYMBptr->sommet == at_plus) || (f._SYMBptr->sommet == at_prod) ||
                  need_parenthesis(f._SYMBptr->sommet) || f._SYMBptr->sommet == at_inv ||
                  (f._SYMBptr->sommet == at_pow &&
                   f._SYMBptr->feuille[0].is_symb_of_sommet(at_inv))))) {
              s += "/(";
              add_print(s, f, contextptr);
          ...
          if (i)
            s += '*';
          if (e._SYMBptr->sommet == at_plus || e._SYMBptr->sommet == at_binary_minus ||
              e._SYMBptr->sommet == at_neg || is_inequation(e)) {
            s += "(";
  ...
  static string & add_print_pow(string & s, const symbolic & g, GIAC_CONTEXT) {
...
#endif
    bool argpar = ((arg.type > _CPLX && arg.type != _FLOAT_) || !is_positive(arg, contextptr)) &&
                  arg.type != _IDNT;
#ifdef EMCC
    ...
    if (arg.type == _IDNT || (arg.type == _SYMB && arg._SYMBptr->sommet != at_neg &&
                              (arg._SYMBptr->sommet != at_exp || calc_mode(contextptr) != 1) &&
                              !arg._SYMBptr->sommet.ptr()->printsommet)) {
      argpar = arg.is_symb_of_sommet(at_inv) ||
               (arg.is_symb_of_sommet(at_exp) && abs_calc_mode(contextptr) == 38);
      if (argpar)
        s += '(';
  ...
  s += python_compat(contextptr) ? "**" : __pow.s;
#endif
    bool puipar = pui.type == _SYMB || pui.type == _FRAC || pui.type == _CPLX ||
                  (pui.type == _VECT && pui.subtype == _SEQ__VECT);
    if (puipar)
      s += '(';
    add_print(s, pui, contextptr);
    if (puipar)
  ...
  static string & add_print_symbolic(string & s, const symbolic & g, GIAC_CONTEXT) {
    ...
    if (g.sommet.ptr()->printsommet && g.sommet != at_plus && g.sommet != at_prod &&
        g.sommet != at_pow) {
    ...
    if (g.feuille.type != _VECT ||
        (g.sommet != at_prod && g.feuille._VECTptr->front().type == _VECT)) {
      s += g.sommet.ptr()->print(contextptr);
      s += '(';
      add_print(s, g.feuille, contextptr);
      s += ')';
    ...
    for (int i = 0;; ++i) {
      add_print(s, (*(g.feuille._VECTptr))[i],
                contextptr); // s += (*(feuille._VECTptr))[i].print(contextptr);
      if (i == l - 1) {
        break;
      }
      s += ',';
  ...
  const string & tmp = g.print(contextptr);
    ...
    if (l && s[l - 1] == '+') {
      ...
      if (tmp.size() > 3 && (unsigned char)tmp[0] == 226 && (unsigned char)tmp[1] == 136 &&
          (unsigned char)tmp[2] == 146) { // -30, -120, -110
        s[l - 1] = '-';
  ...
  static void eval_sto_pnt_symb(const gen & feuille, gen & e, GIAC_CONTEXT) {
    ...
    if (e.type == _SYMB && e.ref_count() <= 2 && e._SYMBptr->feuille.type == _VECT &&
        e._SYMBptr->feuille.ref_count() == 1) {
      ...
      e = (symbolic(at_pnt,
                    gen(makevecteur(e._SYMBptr->feuille._VECTptr->front(),
                                    e._SYMBptr->feuille._VECTptr->back(),
                                    string2gen(feuille._VECTptr->back().print(contextptr), false)),
                        _PNT__VECT)));
  }
    ...
    for (int pos = 0; it != itend; ++pos, ++it) {
      if ((it->type == _SYMB) && (it->_SYMBptr->sommet == at_pnt) &&
        ...
        if (legende.type == _IDNT && 0 && abs_calc_mode(contextptr) == 38) {
          ...
          *it = (symbolic(at_pnt, gen(makevecteur(it->_SYMBptr->feuille._VECTptr->front(),
                                                  it->_SYMBptr->feuille._VECTptr->back(), legende),
                                      _PNT__VECT)));
        } else {
          *it = (symbolic(at_pnt, gen(makevecteur(it->_SYMBptr->feuille._VECTptr->front(),
                                                  it->_SYMBptr->feuille._VECTptr->back(),
                                                  string2gen(legende.print(contextptr), false)),
                                      _PNT__VECT)));
        }
      }
    }
  ...
  static gen eval_sto(const gen & feuille, int level, GIAC_CONTEXT) { // autoname function
    ...
    if (feuille.type == _VECT && feuille.subtype == _SORTED__VECT && feuillev.size() == 2 &&
        feuillev.front().is_symb_of_sommet(at_program)) {
      ...
      if (prog.type == _VECT && prog._VECTptr->size() == 3 &&
        ...
        if (feuilleback.type == _IDNT &&
            feuilleback._IDNTptr->eval(1, feuilleback, contextptr).type == _VECT) {
          prog = symb_of(
              feuilleback,
              prog._VECTptr
                  ->front()); // symbolic(at_of,makesequence(feuilleback,prog._VECTptr->front()));
          return eval_sto(gen(makevecteur(val, prog), _SORTED__VECT), level, contextptr);
        }
      }
    }
    if (feuilleback.type == _SYMB &&
        (feuilleback._SYMBptr->sommet == at_unquote || feuilleback._SYMBptr->sommet == at_hash)) {
    ...
    if (quotearg)
      ...
      if (feuilleback.type == _VECT && feuillevfront->type == _VECT &&
          feuilleback._VECTptr->size() == feuillevfront->_VECTptr->size()) {
        gen tmp;
    ...
    if (e.type == _SYMB && e._SYMBptr->sommet == at_pnt && e._SYMBptr->feuille.type == _VECT &&
        e._SYMBptr->feuille._VECTptr->size() == 2 &&
        (contextptr ? !contextptr->previous : !protection_level))
      eval_sto_pnt_symb(feuille, e, contextptr);
    if (e.type == _VECT && !e._VECTptr->empty() && e._VECTptr->back().type == _SYMB &&
        e._VECTptr->back()._SYMBptr->sommet == at_pnt &&
        (contextptr ? !contextptr->previous : !protection_level))
  ...
  gen symbolic::eval(int level, const context * contextptr) const {
    ...
    try {
      ...
      if (feuille.type == _VECT && sommet == at_pnt && feuille._VECTptr->size() == 3) {
        ans = (*sommet.ptr())(
            gen(vecteur(feuille._VECTptr->begin(), feuille._VECTptr->begin() + 2), feuille.subtype)
                    .in_eval(level, ans, contextptr)
                ? ans
                : feuille,
            contextptr);
        last = save_last;
      ...
      if (feuille.type == _IDNT && sommet == at_neg &&
          !strcmp(feuille._IDNTptr->id_name, string_infinity)) {
        last = save_last;
        lastarg = save_lastarg;
        return minus_inf;
      }
  ...
  bool rewrite_prod_inv(const gen & arg, gen & n, gen & d) {
    ...
    if (arg.type == _VECT && !arg._VECTptr->empty() &&
        arg._VECTptr->back().is_symb_of_sommet(at_inv)) {
  ...
  gen symbolic::evalf(int level, const context * contextptr) const {
    ...
    if (sommet == at_sto) { // autoname function
      ...
      if ((e.type == _SYMB) && (e._SYMBptr->sommet == at_pnt) &&
        ...
        e = (symbolic(
            at_pnt, gen(makevecteur(e._SYMBptr->feuille._VECTptr->front(),
                                    e._SYMBptr->feuille._VECTptr->back(),
                                    string2gen(feuille._VECTptr->back().print(contextptr), false)),
                        _PNT__VECT)));
      if ((e.type == _VECT) && (e._VECTptr->size()) && (e._VECTptr->back().type == _SYMB) &&
          (e._VECTptr->back()._SYMBptr->sommet == at_pnt)) {
        vecteur v = *e._VECTptr;
        iterateur it = v.begin(), itend = v.end();
        for (; it != itend; ++it) {
          if ((it->type == _SYMB) && (it->_SYMBptr->sommet == at_pnt) &&
            ...
            *it = (symbolic(
                at_pnt,
                gen(makevecteur(it->_SYMBptr->feuille._VECTptr->front(),
                                it->_SYMBptr->feuille._VECTptr->back(),
                                string2gen(feuille._VECTptr->back().print(contextptr), false)),
                    _PNT__VECT)));
        }
    ...
    if (sommet == at_EllipticF && feuille.type == _VECT && feuille._VECTptr->size() == 2) {
      ans = _EllipticF(
          makesequence(feuille[0].evalf(level, contextptr), feuille[1].eval(level, contextptr)),
          contextptr);
      last = save_last;
      lastarg = save_lastarg;
      return ans;
    ...
    if (sommet.quoted() && sommet != at_and && !equalposcomp(plot_sommets, sommet)) {
      ...
      if ((sommet == at_neg) && (feuille.type == _IDNT) &&
          !strcmp(feuille._IDNTptr->id_name, string_infinity)) {
        last = save_last;
        lastarg = save_lastarg;
        return minus_inf;
