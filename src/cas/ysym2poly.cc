...
namespace giac {
  ...
  static bool is_algebraic_EXTension(const gen & e) {
    ...
    if ((e._SYMBptr->sommet == at_sqrt) || (e._SYMBptr->sommet == at_rootof))
      return true;
    if ((e._SYMBptr->sommet == at_pow) && (e._SYMBptr->feuille._VECTptr->back().type == _FRAC) &&
        (e._SYMBptr->feuille._VECTptr->back()._FRACptr->den.type == _INT_) &&
        (absint(e._SYMBptr->feuille._VECTptr->back()._FRACptr->den.val) <= MAX_ALG_EXT_ORDER_SIZE))
      return true;
    return false;
  }
  ...
  */
  ...
  gen simplify3(gen & n, gen & d) {
    ...
    if (n.type == _POLY) {
      gen l;
      vector<monomial<gen>>::const_iterator it = n._POLYptr->coord.begin(),
                                            itend = n._POLYptr->coord.end();
      if (it < itend - 1) {
  ...
  static void _FRACadd(const gen & n1, const gen & d1, const gen & n2, const gen & d2, gen & num,
                       gen & den) {
    // COUT << n1 << "/" << d1 << "+" << n2 << "/" << d2 << "=";
    if (is_one(d1)) {
      num = n1 * d2 + n2;
  ...
  static void _FRACmul(const gen & n1, const gen & d1, const gen & n2, const gen & d2, gen & num,
                       gen & den) {
    // COUT << n1 << "/" << d1 << "*" << n2 << "/" << d2 << "=";
    if (is_one(d1)) {
      num = n1;
  ...
  // symbolic to tensor
  //**********************************
  static bool sym2radd(vecteur::const_iterator debut, vecteur::const_iterator fin, const gen & iext,
                       const vecteur & l, const vecteur & lv, const vecteur & lvnum,
                       const vecteur & lvden, int l_size, gen & num, gen & den, GIAC_CONTEXT,
                       bool sequentially) {
    ...
    if (sequentially || fin - debut < 4) {
      ...
      den = plus_one;
      for (; debut != fin; ++debut) {
        totally_converted = totally_converted &&
                            sym2r(*debut, iext, l, lv, lvnum, lvden, l_size, n1, d1, contextptr);
        n2 = num;
        d2 = den;
      ...
      gen n1, d1, n2, d2;
      totally_converted = totally_converted && sym2radd(debut, milieu, iext, l, lv, lvnum, lvden,
                                                        l_size, n1, d1, contextptr, sequentially);
      totally_converted = totally_converted && sym2radd(milieu, fin, iext, l, lv, lvnum, lvden,
                                                        l_size, n2, d2, contextptr, sequentially);
      _FRACadd(n1, d1, n2, d2, num, den);
    }
  ...
  */
  ...
  static bool sym2rmul(vecteur::const_iterator debut, vecteur::const_iterator fin, const gen & iext,
                       const vecteur & l, const vecteur & lv, const vecteur & lvnum,
                       const vecteur & lvden, int l_size, gen & num, gen & den, GIAC_CONTEXT) {
    bool totally_converted = true;
    // First check for a "normal" monomial
      ...
      for (; debut != fin; ++debut) {
        if (debut->type < _IDNT)
          ...
          if (debut->is_symb_of_sommet(at_inv)) {
            ...
            if (debut->is_symb_of_sommet(at_pow) &&
                debut->_SYMBptr->feuille._VECTptr->back().type == _INT_) {
              tmp = debut->_SYMBptr->feuille._VECTptr->front();
              pui = debut->_SYMBptr->feuille._VECTptr->back().val;
            } else {
              tmp = *debut;
    ...
    if (fin - debut < 4) {
      ...
      totally_converted = totally_converted && sym2rmul(debut, milieu, iext, l, lv, lvnum, lvden,
                                                        l_size, n1, d1, contextptr);
      totally_converted = totally_converted && sym2rmul(milieu, fin, iext, l, lv, lvnum, lvden,
                                                        l_size, n2, d2, contextptr);
  ...
  static bool sym2rxrootnum(vecteur & v, const vecteur & lv, gen & num, gen & tmpden,
                            GIAC_CONTEXT) {
    ...
    for (f_it = f.begin(); f_it != f_itend; ++f_it) {
      ...
      if (deg == 1) {
        ...
        if (!hasevalf) {
          *logptr(contextptr) << gettext("Warning, checking for positivity of a root depending of "
                                         "parameters might return wrong sign: ")
                              << numlv << "\n";
          v = vcur;
          num = numcur;
          break;
    ...
    if (lowest_degree > 1) {
      ...
      if (num.type == _EXT) {
        ...
        int k;
        vecteur lvadd(lv);
        lvadd.insert(lvadd.begin(), vecteur(0));
  ...
  static bool sym2rxroot(gen & num, gen & den, int n, int d, const vecteur & l, GIAC_CONTEXT) {
    ...
    */
    ...
    if (is_atomic(num)) {
      ...
      if (0 && // disable for expressions with mixed rootof/fracpow?
          embeddings == 1 && it->type == _VECT && it->_VECTptr->empty())
        embeddings = 0;
      else {
        for (int j = 0; j < embeddings; ++it, ++j) {
          if (it->type != _VECT) {
            string s("sym2rxroot error num=" + num.print(contextptr) +
                     " den=" + den.print(contextptr) + " l=" + gen(l).print(contextptr));
            CERR << s << "\n";
#ifndef NO_STDEXCEPT
            setsizeerr(s);
#endif
  ...
  static bool sym2r(const symbolic & s, const gen & iext, const vecteur & l, const vecteur & lv,
                    const vecteur & lvnum, const vecteur & lvden, int l_size, gen & num, gen & den,
                    GIAC_CONTEXT) {
    if (s.sommet == at_plus) {
      if (s.feuille.type != _VECT) {
    ...
    if (s.sommet == at_neg) {
      bool totally_converted =
          sym2r(s.feuille, iext, l, lv, lvnum, lvden, l_size, num, den, contextptr);
      num = -num;
      return totally_converted;
    }
    if (s.sommet == at_inv) {
      ...
      if (num.type == _EXT) {
        ...
        if ((num.type == _POLY) && (num._POLYptr->dim == 0) && (!num._POLYptr->coord.empty()) &&
            (num._POLYptr->coord.front().value.type == _EXT)) {
    ...
    if (s.sommet == at_pow) {
      if ((*s.feuille._VECTptr)[1].type == _INT_) {
        bool totally_converted = sym2r(s.feuille._VECTptr->front(), iext, l, lv, lvnum, lvden,
                                       l_size, num, den, contextptr);
    ...
    if (s.sommet == at_rootof) {
      bool totally_converted = sym2r(s.feuille._VECTptr->front(), iext, l, lv, lvnum, lvden, l_size,
                                     num, den, contextptr);
      gen pmin_num, pmin_den;
      totally_converted = totally_converted && sym2r(s.feuille._VECTptr->back(), iext, l, lv, lvnum,
                                                     lvden, l_size, pmin_num, pmin_den, contextptr);
      if (!is_one(pmin_den)) {
#ifndef NO_STDEXCEPT
  ...
  static bool sym2r(const fraction & f, const gen & iext, const vecteur & l, const vecteur & lv,
                    const vecteur & lvnum, const vecteur & lvden, int l_size, gen & num, gen & den,
                    GIAC_CONTEXT) {
    gen dent, numt;
    bool totally_converted = sym2r(f.num, iext, l, lv, lvnum, lvden, l_size, num, dent, contextptr);
    totally_converted =
        totally_converted && sym2r(f.den, iext, l, lv, lvnum, lvden, l_size, den, numt, contextptr);
    num = num * numt;
    den = den * dent;
    return totally_converted;
  }

  bool sym2r(const vecteur & v, const gen & iext, const vecteur & l, const vecteur & lv,
             const vecteur & lvnum, const vecteur & lvden, int l_size, gen & num, gen & den,
             GIAC_CONTEXT) {
    den = plus_one;
    if (v.empty()) {
    ...
    for (; it != itend; ++it) {
      totally_converted =
          totally_converted && sym2r(*it, iext, l, lv, lvnum, lvden, l_size, num, den, contextptr);
  ...
  bool sym2r(const vecteur & v, const vecteur & l, const vecteur & lv, const vecteur & lvnum,
             const vecteur & lvden, int l_size, gen & num, gen & den, GIAC_CONTEXT) {
    return sym2r(v, 1, l, lv, lvnum, lvden, l_size, num, den, contextptr);
  }

  static bool sym2rmod(const gen * gptr, const gen & iext, const vecteur & l, const vecteur & lv,
                       const vecteur & lvnum, const vecteur & lvden, int l_size, gen & num,
                       gen & den, GIAC_CONTEXT) {
    gen modnum, modden, modulo;
    bool totally_converted =
        sym2r(*(gptr + 1), iext, l, lv, lvnum, lvden, l_size, modnum, modden, contextptr);
    modulo = rdiv(modnum, modden, contextptr);
    totally_converted =
        totally_converted && sym2r(*gptr, iext, l, lv, lvnum, lvden, l_size, num, den, contextptr);
  ...
  bool sym2r(const gen & e, const gen & iext, const vecteur & l, const vecteur & lv,
             const vecteur & lvnum, const vecteur & lvden, int l_size, gen & num, gen & den,
             GIAC_CONTEXT) {
    int n = 0;
    switch (e.type) {
    ...
    case _IDNT:
    case _SYMB:
      if (e.is_symb_of_sommet(at_rootof) && e._SYMBptr->feuille.type == _VECT &&
          e._SYMBptr->feuille._VECTptr->size() == 2) {
        gen r = e._SYMBptr->feuille._VECTptr->back();
        for (int i = 0; i < lv.size() && i < lvnum.size(); ++i) {
          if (lv[i].is_symb_of_sommet(at_rootof) && lv[i]._SYMBptr->feuille.type == _VECT &&
              lv[i]._SYMBptr->feuille._VECTptr->size() == 2) {
            gen gl = lv[i]._SYMBptr->feuille._VECTptr->front();
            if (gl.type == _VECT && gl._VECTptr->size() == 2 && gl._VECTptr->front() == 1 &&
                gl._VECTptr->back() == 0) {
  ...
  static vecteur common_algext(const vecteur & a, const vecteur & l, GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      ...
      for (int i = 0; i < s; ++i) {
        ...
        if (oldminpoly.type == _FRAC) {
          gen resn, resd;
          hornerfrac(*alg_extoutnum[i]._VECTptr, oldminpoly._FRACptr->num, oldminpoly._FRACptr->den,
                     resn, resd);
    ...
    for (it = a.begin(); it != itend; ++it) {
      ...
      if (!is_one(alg_extoutden[n])) {
        ...
        if (resn.type == _VECT && minpoly.type == _VECT &&
            resn._VECTptr->size() >= minpoly._VECTptr->size()) {
  ...
  static bool compute_lv_lvnum_lvden(const vecteur & l, vecteur & lv, vecteur & lvnum,
                                     vecteur & lvden, bool & totally_converted, int l_size,
                                     GIAC_CONTEXT) {
    gen num, den;
    ...
    for (; it != itend; ++it) {
      algext = algext || (it->type == _SYMB &&
                          (it->_SYMBptr->sommet == at_pow || it->_SYMBptr->sommet == at_rootof));
      totally_converted =
          totally_converted && sym2r(*it, 0, l, lv, lvnum, lvden, l_size, num, den, contextptr);
      lvnum.push_back(num);
      lvden.push_back(den);
    }
  ...
  bool in_find_extension(const gen & extension, vecteur & already, vector<int> & embed, gen & iext,
                         GIAC_CONTEXT) {
    iext = 0;
  ...
  gen rootof_extract(const gen & e, GIAC_CONTEXT) {
    if (e.type == _VECT && e._VECTptr->size() == 2)
      return e._VECTptr->front() *
             symb_rootof(gen(makevecteur(1, 0), _POLY1__VECT), e._VECTptr->back(), contextptr);
  ...
  bool sym2r(const gen & e, const vecteur & l, int l_size, gen & num, gen & den, GIAC_CONTEXT) {
    ...
    totally_converted =
        totally_converted && sym2r(e, iext, l, lv, lvnum, lvden, l_size, num, den, contextptr);
    if (is_inf(num) && !is_inf(den))
      den = 1;
    if (is_inf(den) && !is_inf(num))
      num = 1;
    ...
    if (den.type == _POLY && !den._POLYptr->coord.empty() &&
        den._POLYptr->coord.front().value.type == _EXT) {
  ...
  gen r2sym(const gen & e, const index_m & i, const vecteur & l, GIAC_CONTEXT) {
    ...
    if (i.size() != l.size())
      return gensizeerr(
          gettext("sym2poly/r2sym(const gen & e,const index_m & i,const vecteur & l)"));
    ...
    for (; it != itend; ++it, ++l_it) {
      if ((*it))
        res->v.push_back(
            pow(*l_it, *it, contextptr)); // change for normal(abs(z)^2), was pow(*l_it,*it)
    }
    if (res->v.empty()) {
      delete_ref_vecteur(res);
      return e;
  ...
  gen r2sym(const fraction & f, const vecteur & l, GIAC_CONTEXT) {
    ...
    if (fn.is_symb_of_sommet(at_prod) && fn._SYMBptr->feuille.type == _VECT && fd.type == _INT_ &&
        absint(fd.val) > 1) {
      gen c = 1;
  ...
  static gen r2sym(const gen & p, const const_iterateur & lt, const const_iterateur & ltend,
                   GIAC_CONTEXT);

  static gen solve_deg2(const gen & p, const gen & pmin, const const_iterateur & lt,
                        const const_iterateur & ltend, GIAC_CONTEXT) {
    ...
    if (is_zero(b_over_2))
      return p._VECTptr->front() *
                 sqrt(r2sym(-w.back() / w.front(), lt, ltend, contextptr), contextptr) +
             p._VECTptr->back();
    gen x;
    if (b_over_2.type != _FRAC) {
      ...
      gen delta_prime =
          r2sym(pow(b_over_2, 2, contextptr) - w.front() * w.back(), lt, ltend, contextptr);
  ...
  */
  ...
  gen minimal_polynomial(const gen & pp, bool minonly, GIAC_CONTEXT) {
    ...
    if (pm.type == _VECT) {
      ...
      if ((d = int(mk.size())) <= 5 && d != 4) {
        ...
        if (r.type != _DOUBLE_ && r.type != _CPLX && !w.empty())
          return w.front();
  ...
  static gen r2sym(const gen & p, const const_iterateur & lt, const const_iterateur & ltend,
    ...
    if (p.type == _FRAC) {
      ...
      if (p._FRACptr->den.type == _CPLX)
        res =
            rdiv(r2sym(p._FRACptr->num * conj(p._FRACptr->den, contextptr), lt, ltend, contextptr),
                 r2sym(p._FRACptr->den * conj(p._FRACptr->den, contextptr), lt, ltend, contextptr),
                 contextptr);
      else
        res = rdiv(r2sym(p._FRACptr->num, lt, ltend, contextptr),
                   r2sym(p._FRACptr->den, lt, ltend, contextptr), contextptr);
      return (p._FRACptr->num.type == _EXT) ? ratnormal(res, contextptr) : res;
    }
    if (p.type == _MOD)
      return makemodquoted(r2sym(*p._MODptr, lt, ltend, contextptr),
                           r2sym(*(p._MODptr + 1), lt, ltend, contextptr));
    if (p.type == _VECT) {
        ...
        try {
          ...
          for (int ntry = 0; ntry < 10; ++ntry, tst = false) {
            ...
            if (!lvar(makevecteur(tmp00, tmp10)).empty())
              break;
            if (tmp10.type == _VECT &&
                _size(_gcd(makesequence(tmp10, derivative(*tmp10._VECTptr)), contextptr),
                      contextptr) > 1)
            ...
            if (tmp00.type <= _CPLX) {
              ...
              if (tmp3.type <= _CPLX && tmp2.type <= _CPLX && tmp2 != tmp3) {
                if (!vzero.empty() && !tst)
                  *logptr(contextptr) << gettext("Warning, choosing root of ") << f
                                      << gettext(" at parameters values ") << vzero << "\n";
                if (is_greater(abs(tmp3 - tmp00, contextptr), abs(tmp2 - tmp00, contextptr),
                               contextptr))
                  return w.back();
                else
                  return w.front();
	...
	if (tmp0.type==_DOUBLE_){
      ...
      if (f == v) {
        ...
        return symb_rootof(r2sym(*(pp._EXTptr), lt, ltend, contextptr),
                           r2sym(f, lt, ltend, contextptr), contextptr);
    ...
    res.reserve(itend - it);
    for (; it != itend; ++it) {
      res.push_back(r2sym(r2sym(it->value, lt + 1, ltend, contextptr), it->index, *(lt->_VECTptr),
                          contextptr));
    }
    if (res.size() == 1)
      return res.front();
  ...
  gen r2sym(const gen & p, const vecteur & l, GIAC_CONTEXT) {
    ...
    if (p.type == _MOD)
      return makemodquoted(r2sym(*p._MODptr, l, contextptr),
                           r2sym(*(p._MODptr + 1), l, contextptr));
    if (p.type == _EXT) {
      ...
      if (f == v)
        ...
        return symb_rootof(r2sym(*(pp._EXTptr), l, contextptr), r2sym(f, l, contextptr),
                           contextptr);
  ...
  gen r2sym(const vector<pf<gen>> & pfde_VECT, const vecteur & l, GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      res = res + rdiv(r2sym(gen(it->num), l, contextptr),
                       (r2sym(gen(it->den / pow(it->fact, it->mult)), l, contextptr) *
                        pow(r2sym(gen(it->fact), l, contextptr), it->mult)),
                       contextptr);
    }
    return res;
  }
  ...
  static vecteur lop_pow(const gen & g) {
    ...
    for (; it != itend; ++it) {
      ...
      if (cur.is_symb_of_sommet(at_pow) && cur._SYMBptr->feuille._VECTptr->back().type == _INT_)
        cur =
            cur._SYMBptr->feuille._VECTptr
                ->front(); // was continue, changed for integrate((b^2*x^2 + a*b*x*2 + a^2)^(-3/2) , x) ;
  ...
  static bool is_sqrtxy(const gen & g_, gen & res, GIAC_CONTEXT) {
    gen g(g_);
    vecteur l = lop_pow(g);
    if (l.size() == 2 && l[0]._SYMBptr->feuille[1] == plus_one_half &&
        l[1]._SYMBptr->feuille[1] == plus_one_half) {
      gen c, d, e, f;
      ...
      // rewrite g
      l = vecteur(1,
                  symb_pow(l[0]._SYMBptr->feuille[0] * l[1]._SYMBptr->feuille[0], plus_one_half));
      g = d + e * l[0];
    }
    ...
    if (!is_greater(a, b, contextptr))
      *logptr(contextptr) << "Warning, assuming " << a - b
                          << " is positive. Hint: run assume to make assumptions on a variable\n";
    return true;
  }
  ...
  static gen in_normalize_sqrt(const gen & e, vecteur & L, bool keep_abs, GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      ...
      if (complex_mode(contextptr) == false && var.type == _IDNT &&
          var._IDNTptr->eval(1, var, contextptr) == var) {
      ...
      if (nd.type == _INT_ || nd.type == _ZINT) {
        ...
        lout.push_back(pow(doubl / tmparg, expnum, contextptr) *
                       pow(simpl * out, nover2, contextptr));
        continue;
      }
      if (nd.type != _POLY)
        continue;
      ...
      gen z = fast_icontent(simpl), zsimpl, zdoubl;
      bool zpos;
      ...
      lout.push_back(prev * pow(simpl * out, nover2, contextptr) * pow(doubl, expnum, contextptr) *
                     lspow.back() * pow(tmpd, expnum, contextptr) *
                     pow(tmparg, -expnum, contextptr));
    }
  ...
  static bool has_embedded_fractions(const gen & g) {
    if (g.type == _POLY) {
      vector<monomial<gen>>::const_iterator it = g._POLYptr->coord.begin(),
                                            itend = g._POLYptr->coord.end();
      for (; it != itend; ++it) {
  ...
  gen normal(const gen & e, bool distribute_div, bool allow_embeded_recursion, GIAC_CONTEXT) {
    ...
    if (is_equal(e)) {
      ...
      return symbolic(at_equal, makesequence(normal(v.front(), distribute_div, contextptr),
    ...
    try {
      ...
      vecteur LL;
      gen EE = in_normalize_sqrt(inner_sqrt(e), LL, true, contextptr);
      ...
      if (is_undef(tmp)) {
        ...
        *logptr(contextptr)
            << gettext(
                   ...
                   "simplification only. This might return a wrong answer if simplifying 0/0!")
            << "\n";
        l = lvar(ee);
        tmp = e2r(ee, l, contextptr);
    ...
    if (!distribute_div) {
      ...
      if (f.num.type == _POLY && !f.num._POLYptr->coord.empty() &&
          is_positive(-f.num._POLYptr->coord.front())) {
        f.num = -f.num;
        gen res = r2sym(f, l, contextptr);
        purgeassumelist(L, contextptr);
  ...
  static int is_program(const gen & g) {
    ...
    if (g.type == _IDNT) {
      ...
      if (strlen(idname) == 2 &&
          (idname[0] == 'F' || idname[0] == 'R' || idname[0] == 'X' || idname[0] == 'Y') &&
          idname[1] >= '0' && idname[1] <= '9')
        return 1;
      else
        return -1;
    }
  ...
  bool is_algebraic_program(const gen & g, gen & f1, gen & f3) {
    if (g.type == _FUNC) {
      if (g == at_equal || g == at_superieur_strict || g == at_superieur_egal ||
          g == at_inferieur_strict || g == at_inferieur_egal)
        return false;
    ...
    if (!g.is_symb_of_sommet(at_program) || g._SYMBptr->feuille.type != _VECT ||
        g._SYMBptr->feuille._VECTptr->size() != 3) {
      if (is_program(g) != 1)
        return false;
      identificateur _tmpi(" x");
      f1 = _tmpi;
    ...
    bool res = f3.type <= _IDNT || f3.type == _FRAC || f3.type == _VECT ||
               (f3.type == _SYMB && !f3.is_symb_of_sommet(at_bloc));
    if (res)
  ...
  gen recursive_normal(const gen & e, bool distribute_div, GIAC_CONTEXT) {
    ...
    if (is_inequation(e) && e._SYMBptr->feuille.type == _VECT) {
      ...
      if (is_integer(l) || l.type == _FRAC) {
        ...
        if (e2.is_symb_of_sommet(at_plus) && e2._SYMBptr->feuille.type == _VECT &&
            e2._SYMBptr->feuille._VECTptr->size() == 2)
          e2 = makesequence(e2._SYMBptr->feuille._VECTptr->front(),
                            -e2._SYMBptr->feuille._VECTptr->back());
        else
    ...
    for (; it != itend; ++it) {
      ...
      if (it->_SYMBptr->feuille._VECTptr->front().type == _INT_ &&
          it->_SYMBptr->feuille._VECTptr->back() == plus_one_half)
        continue;
      ...
      if (is_zero(base) || num.type != _INT_ || den.type != _INT_ ||
        ...
        *it = pow(base, rdiv(num, den, contextptr), contextptr);
        ...
        if (den.val > 1 && (is_integer(base) || base.type == _FRAC) &&
          ...
          gen tmp;
          if (vtmp.size() > 2)
            tmp = r2e(algebraic_EXTension(makevecteur(1, 0), vtmp), vecteur(1, vecteur(0)),
                      contextptr);
          else
            tmp = -vtmp.back() / vtmp.front();
          *it = pow(base, num.val / den.val, contextptr) * tmp;
        } else
          *it = pow(base, num.val / den.val, contextptr) *
                pow(base, rdiv(num.val % den.val, den), contextptr);
      }
    }
    if (l != l_subst)
  ...
  gen _recursive_normal(const gen & e, GIAC_CONTEXT) {
    ...
    gen var, res;
    if (is_equal(e))
      return apply_to_equal(
          e, recursive_normal,
          contextptr); // symb_equal(_recursive_normal(equal2diff(e),contextptr),0);
  ...
  gen ratnormal(const gen & e, GIAC_CONTEXT) {
    ...
    lvar(e, l);
    if (l.size() > 1)
      gen_sort_f(l.begin(), l.end(), sort_func);
  ...
  gen recursive_ratnormal(const gen & e, GIAC_CONTEXT) {
    ...
    for (unsigned i = 0; i < l.size(); ++i) {
      if (l[i].type == _SYMB)
        l[i] = l[i]._SYMBptr->sommet(recursive_ratnormal(l[i]._SYMBptr->feuille, contextptr),
                                     contextptr);
    }
    if (fg.type == _FRAC && fg._FRACptr->num.type == _FRAC) {
      fraction f(fg._FRACptr->num._FRACptr->num, fg._FRACptr->den * fg._FRACptr->num._FRACptr->den);
  ...
  static gen factor(const polynome & p, const vecteur & l, bool fixed_order, bool with_sqrt,
                    gen divide_an_by, gen & extra_div, GIAC_CONTEXT);

  static gen factor_multivar(const polynome & p, const vecteur & l, bool fixed_order,
                             bool with_sqrt, gen divide_an_by, gen & extra_div, GIAC_CONTEXT) {
    polynome pp(p);
  ...
  static gen factor(const polynome & p, const vecteur & l, bool fixed_order, bool with_sqrt,
    ...
    if (is_one(p_content))
      ...
      if ((tmp.type > _POLY || (tmp.type == _POLY && !tmp._POLYptr->coord.empty() &&
                                tmp._POLYptr->coord.front().value.type == _USER)) &&
          !v.empty() && v.front().mult == 1 &&
          (v.size() == 1 || v.front().fact.degree(0) == 0)) { // for GF factorization
        v.front().fact =
            tmp.type == _POLY ? (*tmp._POLYptr * v.front().fact) : (tmp * v.front().fact);
        return r2sym(v, l, contextptr) / r2sym(extra_div, l, contextptr);
      }
  ...
  static gen var_factor(const gen & e, const vecteur & l, bool fixed_order, bool with_sqrt,
                        const gen & divide_an_by, GIAC_CONTEXT) {
    if (e.type != _POLY)
  ...
  gen ordered_factor(const gen & ee, vecteur & l, bool with_sqrt, GIAC_CONTEXT) {
    ...
    fxnd(f, f_num, f_den);
    return rdiv(var_factor(f_num, l, true, with_sqrt, 1, contextptr),
                var_factor(f_den, l, true, with_sqrt, 1, contextptr));
  }
  ...
  gen ratfactor(const gen & ee, bool with_sqrt, GIAC_CONTEXT) {
    ...
    if (with_sqrt)
      l = vecteur(1, l);
    gen tmp = rdiv(var_factor(f_num, l, false, with_sqrt, 1, contextptr),
                   var_factor(f_den, l, false, with_sqrt, 1, contextptr));
    return tmp;
  }
  ...
  gen partfrac(const gen & e_, const vecteur & l, bool with_sqrt, GIAC_CONTEXT) {
    ...
    factor(ext * f_den, p_content, vden, false, with_sqrt, complex_mode(contextptr), ext,
           extra_div);
  ...
  static define_unary_function_eval(__non_recursive_normal, (const gen_op_context)normal,
                                    _non_recursive_normal_s);
  define_unary_function_ptr5(at_non_recursive_normal, alias_at_non_recursive_normal,
                             &__non_recursive_normal, 0, true);
  ...
  gen _factor(const gen & args, GIAC_CONTEXT) {
    ...
    *logptr(contextptr) << "Run ifactor(" << args << ") for integer factorization." << "\n";
    ...
    if (args.type == _VECT && args._VECTptr->size() == 2 && is_equal(args._VECTptr->front())) {
      ...
      return symb_equal(_factor(makesequence(a, x), contextptr),
                        _factor(makesequence(b, x), contextptr));
    }
    gen var, res;
  ...
  gen _collect(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args.subtype == _SEQ__VECT && args._VECTptr->size() >= 2 &&
        args._VECTptr->front().type != _VECT) {
  ...
  gen _partfrac(const gen & args_, GIAC_CONTEXT) {
    ...
    return partfrac(args._VECTptr->front(), args._VECTptr->back(), withsqrt(contextptr),
                    contextptr);
  }
  ...
  gen _resultant(const gen & args, GIAC_CONTEXT) {
    ...
    if (v.back() == at_sylvester || v.back() == at_det)
      return _det(_sylvester(gen(vecteur(args._VECTptr->begin(), args._VECTptr->begin() + s - 1),
                                 _SEQ__VECT),
                             contextptr),
                  contextptr);
      ...
      if (w.size() > 1) {
        ...
        if (rargs.back() != at_lagrange || w.size() == 2) {
          ...
          if ((f1_num.type == _POLY) && (f2_num.type == _POLY)) {
            ...
            gen coeff;
            if (!interpolable_resultant(pp, d, coeff, true, contextptr) ||
                !interpolable_resultant(qp, d, coeff, true, contextptr))
              return gensizeerr(
                  gettext("Not enough elements in field to interpolate. Try in a field extension"));
            if (coeff.type == _USER)
              j = 0;
            int dim = pp.dim;
            ...
            gen den = pow(r2sym(f1_den, l, contextptr), qp.lexsorted_degree(), contextptr) *
                      pow(r2sym(f2_den, l, contextptr), pp.lexsorted_degree(), contextptr);
            ...
            for (int i = 0; i <= d; ++i, ++j) {
              ...
              gen gq = horner(vq, xi);
              if (debug_infolevel && i % 16 == 0)
                CERR << CLOCK() * 1e-6 << " interp resultant " << j << ", "
                     << 100 * double(i) / (d + 1) << "% done" << "\n";
              if (gp.type == _POLY && gq.type == _POLY) {
                if (0 && m >= GIAC_PADIC / 2 && n >= GIAC_PADIC / 2)
        ...
        for (int i = 0; i <= d; ++i, ++j) {
          ...
          for (;; ++j) {
            // find evaluation preserving degree in x
            if (!is_zero(subst(pl, y, j, false, contextptr)) &&
    ...
    if ((f1_num.type == _POLY) && (f2_num.type == _POLY))
      return r2sym(gen(resultant(*f1_num._POLYptr, *f2_num._POLYptr)), l, contextptr) /
             pow(r2sym(f1_den, l, contextptr), f2_num._POLYptr->lexsorted_degree(), contextptr) /
             pow(r2sym(f2_den, l, contextptr), f1_num._POLYptr->lexsorted_degree(), contextptr);
    if (is_zero(f1))
      return f1;
    if (is_zero(f2))
      return f2;
  ...
  gen _discriminant(const gen & args, GIAC_CONTEXT) {
    ...
    if (v[0].type == _VECT) {
      ...
      if (n < 0)
        return undef;
      gen r = _resultant(makesequence(v[0], derivative(*v[0]._VECTptr)), contextptr) /
              v[0]._VECTptr->front();
    ...
    gen r =
        _resultant(makesequence(p1, p2, x), contextptr) / _lcoeff(makesequence(p1, x), contextptr);
    r = ratnormal(r, contextptr);
...
gen read1arg_from_stream(istream & inf, GIAC_CONTEXT)
  ...
  static void remove_ext_copies(gen & g, const gen & ext1, const gen & ext2) {
    if (g.type == _POLY) {
      vector<monomial<gen>>::iterator it = g._POLYptr->coord.begin(),
                                      itend = g._POLYptr->coord.end();
      for (; it != itend; ++it)
