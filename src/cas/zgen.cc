...
#ifdef ALLOC44
    if (((size_t)obj >= (size_t)&tab44[0]) && ((size_t)obj < (size_t)&tab44[ALLOC44])) {
      *(unsigned *)obj = 0;
  ...
  *((ulonglong *)this) = *((ulonglong *)&a);
  ...
  gen sym_add(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    if (ia) {
      ...
      if (ia == ib || (ia == 1 && ib)) {
        ...
        return (symbolic(b._SYMBptr->sommet,
                         makesequence(va.front() + vb.front(), va.back() + vb.back())));
      }
      if (ia == 1 || !ib) // = +
        ...
        if (b.is_symb_of_sommet(at_interval)) {
          ...
          if (g.type == _VECT && g._VECTptr->size() == 2) {
            ...
            return symb_interval(
                w[0] + v[0],
                w[1] +
                    v[1]); //(symbolic(at_interval,gen(makenewvecteur(w[0]+v[0],w[1]+v[1]),_SEQ__VECT)));
          }
        }
        return symb_interval(
            b + v[0],
            b + v[1]); //(symbolic(at_interval,gen(makenewvecteur(b+v[0],b+v[1]),_SEQ__VECT)));
      }
    }
    if (b.is_symb_of_sommet(at_interval))
      return b + a;
  ...
  gen sym_sub(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    */
    ...
    if ((a.type == _SYMB) && (b.type == _SYMB)) {
      if (a._SYMBptr->sommet == at_plus) {
        if (b._SYMBptr->sommet == at_plus)
          return symb_plus(mergevecteur(*(a._SYMBptr->feuille._VECTptr),
  ...
  gen pow(const gen & base, const gen & exponent, GIAC_CONTEXT) {
    ...
    if (exponent.type == _FRAC) {
      ...
      return pow(base, (symb_prod2(exponent._FRACptr->num, symb_inv(exponent._FRACptr->den))),
                 contextptr);
    }
    if (is_inf(base)) {
	...
	if (abs_calc_mode(contextptr)==38 && !complex_mode(contextptr))
      ...
      if ((exponent.type == _SYMB) && (exponent._SYMBptr->sommet == at_prod)) {
        ...
        if (subexponent_deno.type != _INT_) {
          ...
          return symb_pow(symb_pow(base, _FRAC2_SYMB(subexponent_num, subexponent_deno)),
                          superexponent);
        }
        ...
        if (r) {
          if (complex_mode(contextptr) &&
            ...
            res = symb_pow(pow(base1, subexponent_num, contextptr),
                           inv(subexponent_deno, contextptr)) *
                  res;
          } else
            res =
                symb_pow(pow(base, subexponent_num, contextptr), inv(subexponent_deno, contextptr));
          if (r != 1)
            res = symb_pow(res, r);
        }
        if (!q)
  ...
  gen sym_mult(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    if (is_equal(a)) {
      ...
      if (is_equal(b)) {
        ...
        return (symbolic(a._SYMBptr->sommet,
                         gen(makenewvecteur(va.front() * b, va.back() * b), _SEQ__VECT)));
    }
    if (is_equal(b)) {
      vecteur & vb = *b._SYMBptr->feuille._VECTptr;
      return (symbolic(b._SYMBptr->sommet,
                       gen(makenewvecteur(a * vb.front(), a * vb.back()), _SEQ__VECT)));
    }
    if ((a.type == _SYMB) && (b.type == _SYMB)) {
  ...
  gen inv(const gen & a, GIAC_CONTEXT) {
    ...
    case _SYMB:
      ...
      if (a._SYMBptr->sommet == at_neg)
        ...
        if (a._SYMBptr->sommet == at_prod)
          return (symb_prod(gen(inv__VECT(*(a._SYMBptr->feuille._VECTptr), contextptr),
    ...
    default:
        ...
        return gensizeerr("string /");
      {
        ...
        if (is_algebraic_program(a, var1, res1)) {
          ...
          if (!is_constant_wrt(b, var1, contextptr))
            *logptr(contextptr)
                << "Warning function/constant with constant dependant of mute variable" << endl;
          return symb_prog3(var1, 0, rdiv(res1, b, contextptr));
        }
        if (is_algebraic_program(b, var2, res2)) {
          if (!is_constant_wrt(a, var2, contextptr))
            *logptr(contextptr)
                << "Warning constant/function with constant dependant of mute variable" << endl;
      ...
      if (is_equal(a)) {
        ...
        if (is_equal(b)) {
          ...
          return (symbolic(a._SYMBptr->sommet, makesequence(rdiv(va.front(), b, contextptr),
                                                            rdiv(va.back(), b, contextptr))));
      }
      if (is_equal(b)) {
        vecteur & vb = *b._SYMBptr->feuille._VECTptr;
        return (symbolic(b._SYMBptr->sommet, makesequence(rdiv(a, vb.front(), contextptr),
