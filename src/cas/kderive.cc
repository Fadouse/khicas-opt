...
namespace giac {
  ...
  static gen derive_SYMB(const gen & g_orig, const identificateur & i, GIAC_CONTEXT) {
    ...
    if (s.sommet == at_derive) {
      if (s.feuille.type != _VECT)
        return symb_derive(
            s.feuille, vx_var,
            2); // symbolic(at_derive,gen(makevecteur(s.feuille,vx_var,2),_SEQ__VECT));
      if (s.feuille._VECTptr->size() == 2) { // derive(f,x)
        ...
        if (*othervar._IDNTptr == i) { // _FUNCnd derivative
          ...
          nderiv.push_back(1);
          return symb_derive(
              (*s.feuille._VECTptr)[0], var,
              nderiv); //symbolic(at_derive,gen(makevecteur((*s.feuille._VECTptr)[0],var,nderiv),_SEQ__VECT));
        }
      } else { // derive(f,x,n)
        ...
        if (othervar.type == _IDNT) {
          ...
          return symb_derive(
              (*s.feuille._VECTptr)[0], othervar,
              nderiv); //symbolic(at_derive,gen(makevecteur((*s.feuille._VECTptr)[0],othervar,nderiv),_SEQ__VECT));
        }
      }
    }
  ...
  gen derive(const gen & e, const gen & vars, const gen & nderiv, GIAC_CONTEXT) {
    ...
    if (nderiv.type == _INT_) {
      ...
      return symb_derive(
          ecopie, vars,
          n + 1 - j); //symbolic(at_derive,gen(makevecteur(ecopie,vars,n+1-j),_SEQ__VECT));
    }
  ...
  static define_unary_function_eval4_quoted(__derive, &_derive, _derive_s, printasderive,
  ...
  gen _function_diff(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.is_symb_of_sommet(at_function_diff)) {
      ...
      return symb_of(
          gen(symbolic(at_composepow, makesequence(at_function_diff, 2))),
          f); //symbolic(at_of,makesequence(gen(symbolic(at_composepow,makesequence(at_function_diff,2))),f));
    }
      ...
      if (f.type == _VECT && f._VECTptr->size() == 2) {
        ...
        if (f1.is_symb_of_sommet(at_composepow)) {
          ...
          if (f1f.type == _VECT && f1f._VECTptr->size() == 2 &&
            ...
            return symb_of(
                gen(symbolic(at_composepow,
                             makesequence(at_function_diff, f1f._VECTptr->back() + 1))),
                f2); // symbolic(at_of,makesequence(gen(symbolic(at_composepow,makesequence(at_function_diff,f1f._VECTptr->back()+1))),f2));
          }
        }
      }
    }
  ...
  void domain(const gen & f, const gen & x, vecteur & eqs, vecteur & excluded, int mode,
    ...
    for (int i = 0; i < int(v.size()); ++i) {
      ...
      if (u == at_asin || u == at_acos || u == at_atanh) {
        if (mode == 1)
          excluded = mergevecteur(
              excluded, gen2vecteur(_solve(makesequence(symb_equal(pow(gf, 2, contextptr), 0), x),
                                           contextptr)));
        else
