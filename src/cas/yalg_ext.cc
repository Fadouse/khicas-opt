...
namespace giac {
  ...
  bool comparegen::operator()(const gen & a, const gen & b) const {
    if (a.type == _INT_ && b.type == _INT_)
      return a.val < b.val;
    gen A1, A2, B1, B2;
    if (a.type == _VECT && a._VECTptr->size() == 2 && (A1 = a._VECTptr->front()).type == _INT_ &&
        (A2 = a._VECTptr->back()).type == _INT_ && b.type == _VECT && b._VECTptr->size() == 2 &&
        (B1 = b._VECTptr->front()).type == _INT_ && (B2 = b._VECTptr->back()).type == _INT_) {
...
static void rootof_unlock() {}
  ...
  vecteur galoisconj(const vecteur & v, GIAC_CONTEXT) {
    ...
    if (pari_galoisconj(g, res, contextptr))
      return res;
#endif
    if (int(v.size()) > MAX_COMMON_ALG_EXT_ORDER_SIZE)
      return res;
  ...
  bool proot_cache(const vecteur & v, double eps, const vecteur & res) {
    ...
    rootmap::iterator ritend = proot_list().end(), rit = proot_list().find(v);
    if (rit != ritend) {
      if (rit->second.type != _VECT || rit->second._VECTptr->size() != 2 ||
          rit->second._VECTptr->front().type != _VECT ||
          rit->second._VECTptr->back().type != _DOUBLE_ ||
          rit->second._VECTptr->back()._DOUBLE_val > eps)
  ...
  gen in_select_root(const vecteur & a, bool reel, GIAC_CONTEXT, double eps) {
    ...
    gen current(a.front());
    double max_re(re(current, contextptr).evalf_double(1, contextptr)._DOUBLE_val),
        max_im(im(current, contextptr).evalf_double(1, contextptr)._DOUBLE_val);
    const_iterateur it = a.begin(), itend = a.end();
    for (; it != itend; ++it) {
      double cur_re(re(*it, contextptr).evalf_double(1, contextptr)._DOUBLE_val),
          cur_im(im(*it, contextptr).evalf_double(1, contextptr)._DOUBLE_val);
  ...
  gen alg_evalf(const gen & a, const gen & b, GIAC_CONTEXT) {
    if (a.type == _FRAC)
      return rdiv(alg_evalf(a._FRACptr->num, b, contextptr),
                  alg_evalf(a._FRACptr->den, b, contextptr), contextptr);
  ...
  gen common_minimal_POLY(const gen & ga, const gen & gb, gen & a, gen & b, int & k,
                          const vecteur * lvptr, GIAC_CONTEXT) {
    ...
    for (k = k_init;; ++k) {
      ...
      for (int j = 0; j <= na * nb; ++j) {
        ...
        p.TDivRem(pb, tmpq, tmpr, true);
        p.coord.swap(tmpr.coord); // p=p%pb; //
        p.reorder(transposition(0, 1, 2));
        // division by a after because b might depend on a
        p.TDivRem(pa, tmpq, tmpr, true);
        p.coord.swap(tmpr.coord); // p=p%pa; //
      }
    ...
    if (!rootof_trylock()) {
      ...
      if (rit == ritend) {
        ...
        gen gaa, gbb;
        if (is_known_rootof(va, lvptr, gaa, contextptr) &&
            is_known_rootof(vb, lvptr, gbb, contextptr)) {
          if (!rootof_trylock()) {
            if (lvptr) {
  ...
  gen common_EXT(gen & a, gen & b, const vecteur * l, GIAC_CONTEXT) {
    ...
    if (is_one(a__VECT[0]) && is_one(b__VECT[0]) && is_zero(a__VECT[as - 1] - b__VECT[bs - 1])) {
      int i = 1;
      for (; i < as - 1; ++i) {
        if (!is_zero(a__VECT[i]))
          break;
      }
      if (i == as - 1) {
        for (i = 1; i < bs - 1; ++i) {
          if (!is_zero(b__VECT[i]))
            break;
        }
        if (i == bs - 1) {
          ...
          c[0] = 1;
          c[gs] = a__VECT[as - 1];
          polynome cp = poly12polynome(c), cpc;
          factorization f;
          gen an, extra;
    ...
    if (!trouve) {
      ...
      if (innerdim) {
        ...
        *logptr(contextptr) << gettext("Warning, need to choose a branch for the root of a "
                                       "polynomial with parameters. This might be wrong.")
                            << '\n';
        if (l && l->size() >= 2) {
        ...
        find_good_eval(
            px, pb,
            vb); // find_good_eval does not take care of assumptions, but vb should be ok (loop above)
        if (vb == vb0)
          *logptr(contextptr) << gettext("The choice was done assuming ") << params << "=" << vb
                              << '\n';
        else
          *logptr(contextptr) << gettext("Non regular value ") << vb0
                              << gettext(" was discarded and replaced randomly by ") << params
                              << "=" << vb << '\n';
        ...
        racines =
            proot(*evalf(b__VECT, 1, contextptr)
                       ._VECTptr); // evalf to avoid recursion if computing exact roots of b__VECT
      if (is_undef(racines))
        return gensizeerr(contextptr);
    ...
    if (!deep_emb && !trouve && !is_undef(racine_max)) { // select root for b
      ...
      double min_abs = 0,
             racine_max_d = evalf_double(abs(racine_max, contextptr), 1, contextptr)._DOUBLE_val;
      int ndig = 14;
      while (!trouve) {
        ...
        if (min_abs > 1e-4 * racine_max_d) {
          *logptr(contextptr) << "Precision problem choosing root in common_EXT, current precision "
                              << ndig << '\n';
          trouve = false;
          ndig = 2 * ndig;
          f_it = f.begin();
  ...
  bool has_rootof_value(const gen & Pmin, gen & value, GIAC_CONTEXT) {
    ...
    if (contextptr && contextptr->globalcontextptr->rootofs) {
      ...
      for (unsigned i = 0; i < r.size(); ++i) {
        gen ri = r[i];
        if (ri.type == _VECT && ri._VECTptr->size() == 2 && Pmin.type == _VECT &&
            ri._VECTptr->front().type == _VECT &&
            *Pmin._VECTptr == *ri._VECTptr->front()._VECTptr) {
  ...
  static string printasrootof(const gen & g, const char * s, GIAC_CONTEXT) {
    if (contextptr && g.type == _VECT && g._VECTptr->size() == 2) {
      ...
      if (g._VECTptr->front().type == _VECT &&
          has_rootof_value(g._VECTptr->back(), value, contextptr)) {
  ...
  gen symb_rootof(const gen & p, const gen & pmin, GIAC_CONTEXT) {
    ...
    gen value(undef);
    if (!rootof_trylock()) {
      rootmap::iterator it = symbolic_rootof_list().find(pmin),
                        itend = symbolic_rootof_list().end();
      if (it != itend)
        value = it->second;
      rootof_unlock();
    }
  ...
  gen rootof(const gen & e, GIAC_CONTEXT) {
    ...
    if (e._VECTptr->size() == 2 && e._VECTptr->front().type != _VECT) {
      ...
      if (v.size() != 1)
        return gentypeerr(gettext("rootof"));
      return rootof(
          makesequence(_symb2poly(makesequence(e._VECTptr->front(), v.front()), contextptr),
                       _symb2poly(makesequence(e._VECTptr->back(), v.front()), contextptr)),
          contextptr);
    }
    ...
    if (e.type == _VECT && e._VECTptr->size() == 2 && e._VECTptr->back().type == _VECT) {
      ...
      return symbolic(
          at_rootof,
          gen(makevecteur(e._VECTptr->front(), gen(v2, e._VECTptr->back().subtype)), e.subtype));
    }
  ...
  */
  ...
  gen set_timeout(const gen & args, GIAC_CONTEXT) {
    ...
    if (g.type == _VECT && g._VECTptr->size() == 2 && g._VECTptr->front().type == _INT_ &&
        g._VECTptr->back().type == _INT_) {
#ifdef TIMEOUT
      caseval_maxtime = giacmax(2, g._VECTptr->front().val);
      caseval_n = 0;
      caseval_mod = giacmax(2, g._VECTptr->back().val);
      string S = "Max eval time set to " + print_INT_(caseval_maxtime) + ", check frequency 1/" +
                 print_INT_(caseval_mod);
  ...
  static int number_of_sign_changes(const vecteur & v, const gen & a0, int & global_sign,
                                    GIAC_CONTEXT) {
  ...
  static int sturmab(const gen & g, const gen & x, const gen & a, const gen & b, bool remove_b_root,
                     GIAC_CONTEXT) {
    if (g.type == _VECT) {
#ifdef NO_STDEXCEPT
      return -2;
    ...
    for (; it != itend; ++it) {
      if (it->type == _VECT) {
        res += number_of_sign_changes(*it->_VECTptr, a, global_sign, contextptr) -
               number_of_sign_changes(*it->_VECTptr, b, dontcare, contextptr);
  ...
  void recompute_minmax(const vecteur & w, const vecteur & range, const gen & expr, const gen & var,
                        gen & resmin, gen & resmax, vecteur & xmin, vecteur & xmax, int direction,
                        GIAC_CONTEXT) {
    const_iterateur it = w.begin(), itend = w.end();
    for (; it != itend; ++it) {
      if (ck_is_strictly_greater(*it, range[1], contextptr) ||
          ck_is_strictly_greater(range[0], *it, contextptr))
        continue;
#ifdef NO_STDEXCEPT
  ...
  gen fminmax(const gen & g, int minmax, GIAC_CONTEXT) {
    ...
    if (expr.type == _SYMB) {
      ...
      if (u == at_exp || u == at_ln || u == at_atan || u == at_abs) {
        ...
        if (u == at_ln && tmp.type == _VECT && tmp._VECTptr->size() == 2 &&
            is_positive(-tmp._VECTptr->front(), contextptr))
    ...
    if (range == makevecteur(minus_inf, plus_inf))
      ...
      giac_assume(symbolic(at_and, makevecteur(symb_superieur_egal(var, range[0]),
                                               symb_inferieur_egal(var, range[1]))),
                  contextptr);
      w = solve(df, var, 2, contextptr);
      if (savevar == var)
    ...
    recompute_minmax(vecteur(1, range[0]), range, expr, var, resmin, resmax, xmin, xmax, 1,
                     contextptr);
    recompute_minmax(vecteur(1, range[1]), range, expr, var, resmin, resmax, xmin, xmax, -1,
                     contextptr);
    // Singularities
    ...
    if (g.type == _SYMB) {
      ...
      try {
        ...
        for (unsigned i = 0; i < lv0.size(); ++i) {
          if (evalf_double(lv0[i], 1, contextptr).type !=
              _DOUBLE_) // if (!is_constant_idnt(lv0[i]))
            lv.push_back(lv0[i]);
        }
        if (!lv.empty()) {
    ...
    if (g.is_symb_of_sommet(at_prod)) {
      ...
      for (int i = 0; i < s; ++i) {
        if (v[i].is_symb_of_sommet(at_sqrt) &&
            sturmsign(v[i]._SYMBptr->feuille, strict, contextptr) == 1)
          continue;
      ...
      case 0:
        return res;
      case 1:
        tmp = insturmsign(w.front(), strict, contextptr);
        return tmp == -2 ? -2 : res * tmp;
      default:
        tmp = insturmsign(symbolic(at_prod, w), strict, contextptr);
        return tmp == -2 ? -2 : res * tmp;
      }
    }
