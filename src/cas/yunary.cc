...
namespace giac {
...
#else
  bool unary_function_ptr::quoted() const {
    return (ptr()->index_quoted_function & 0x1) || (((size_t)_ptr) & 0x1);
  }
#endif
  ...
  */
  ...
  unary_function_ptr::unary_function_ptr(const unary_function_eval * myptr, int myquoted,
                                         int parser_token)
      : _ptr((size_t)myptr) {
    if (myquoted)
      *((size_t *)&_ptr) |= 0x1;
  ...
  unary_function_ptr::unary_function_ptr(const alias_unary_function_eval * myptr, int myquoted,
                                         int parser_token)
      : _ptr((size_t)myptr) {
    if (myquoted)
      *((size_t *)&_ptr) |= 0x1;
  ...
  */
  const char * unary_function_eval::print(GIAC_CONTEXT) const {
    ...
    multimap<string, localized_string>::iterator it = back_lexer_localization_map().find(s),
                                                 backend = back_lexer_localization_map().end(),
                                                 itend =
                                                     back_lexer_localization_map().upper_bound(s);
    if (it != backend) {
      for (; it != itend; ++it) {
...
unary_function_ptr::unary_function_ptr(const unary_function_abstract * myptr, int myquoted,
                                       int parser_token)
    : _ptr(myptr) {
  if (myquoted)
    *((size_t *)&_ptr) |= 0x1;
...
unary_function_ptr::unary_function_ptr(const alias_unary_function_eval * myptr, int myquoted,
                                       int parser_token)
    : _ptr((const unary_function_abstract *)myptr) {
  if (myquoted)
    *((size_t *)&_ptr) |= 0x1;
  if (parser_token)
    if (!lexer_functions_register(*this, myptr->s, parser_token))
...
const char * unary_function_abstract::print(GIAC_CONTEXT) const {
  ...
  multimap<string, localized_string>::iterator it = back_lexer_localization_map().find(s),
                                               backend = back_lexer_localization_map().end(),
                                               itend = back_lexer_localization_map().upper_bound(s);
  if (it != backend) {
    for (; it != itend; ++it) {
      if (it->second.language == lang)
        return it->second.chaine.c_str();
...
#ifdef NSPIRE
  template <class T>
  nio::ios_base<T> & operator<<(nio::ios_base<T> & os, const unary_function_eval & o) {
    return os << o.s;
  }
#else
  ostream & operator<<(ostream & os, const unary_function_eval & o) {
    return os << o.s;
  }
#endif
#else
...
// I/O
ostream & operator<<(ostream & os, const unary_function_abstract & o) {
  return os << o.s;
}
ostream & operator<<(ostream & os, const unary_function_unary & o) {
  return os << o.s;
}
ostream & operator<<(ostream & os, const unary_function_eval & o) {
  return os << o.s;
...
unary_function_user * unary_function_user::recopie() const {
  unary_function_user * ptr =
      new unary_function_user(index_quoted_function, f, s, printsommet, texprint, cprint);
  ptr->D = D;
  return ptr;
}
...
unary_function_compose::unary_function_compose(unsigned u,
                                               const vector<unary_function_ptr> & myop_v)
    : unary_function_abstract(u, 0) {
...
unary_function_list::unary_function_list(unsigned u, const vector<unary_function_ptr> & myop_v)
    : unary_function_abstract(u, 0) {
  string * sptr = new string("{ ");
  vector<unary_function_ptr>::const_iterator it = myop_v.begin(), itend = myop_v.end();
  for (; it != itend; ++it) {
...
ostream & operator<<(ostream & os, const unary_function_compose & p) {
  return os << p.s;
}
ostream & operator<<(ostream & os, const unary_function_list & p) {
  return os << p.s;
}
ostream & operator<<(ostream & os, const unary_function_constant & c) {
  return os << c.s;
}
ostream & operator<<(ostream & os, const unary_function_innerprod & i) {
  return os << i.s;
}
  ...
  string printsommetasoperator(const gen & feuille, const char * sommetstr_orig, GIAC_CONTEXT) {
    ...
    if (itb->type == _FRAC && sommetstr != "=")
      ...
      if (sommetstr == "=" || itb->type == _IDNT ||
          (itb->type <= _CPLX && is_positive(*itb, contextptr)))
        s = itb->print(contextptr);
      else
        s = '(' + itb->print(contextptr) + ')';
    }
    ...
    for (;;) {
      if (itb == itend)
        return s;
      if ((itb->type == _SYMB || itb->type == _FRAC || itb->type == _CPLX ||
           (itb->type == _VECT && itb->subtype == _SEQ__VECT)) &&
          sommetstr != " . ")
  ...
  partial_derivative::partial_derivative(gen (*mydf)(const gen & argss, const context * contextptr))
      : df(unary_function_ptr(new unary_function_eval(0, mydf, ""))) {}

#else
partial_derivative_onearg::partial_derivative_onearg(gen (*mydf)(const gen & args))
    : df(unary_function_ptr(new unary_function_unary(0, mydf, ""))) {}
partial_derivative_onearg::partial_derivative_onearg(gen (*mydf)(const gen & args,
