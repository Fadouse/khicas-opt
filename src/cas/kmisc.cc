...
int inputline(const char * msg1, const char * msg2, ustl::string & s, bool numeric,
              int ypos); // dConsole.cpp
...
namespace giac {
      ...
      for (it = v.begin(); it != itend; ++it) {
        ...
        if (tmp.type == _STRNG && attributs[1].type == _STRNG && *attributs[1]._STRNGptr == nullstr)
          ...
          if (is_equal(tmp))
            ...
            if (bar)
              vres.push_back(
                  symb_segment(it->_VECTptr->front(), tmp, attributs, _GROUP__VECT, contextptr));
          }
        }
      }
      if (polygone)
  ...
  gen _gramschmidt(const gen & g, GIAC_CONTEXT) {
    ...
    if (v.size() == 2) {
      ...
      for (int i = 1; i < s; ++i) {
        ...
        for (int j = 0; j < i; ++j)
          cl = cl +
               rdiv(scalaire(makesequence(lv[i], lv[j]), contextptr), sc[j], contextptr) * lv[j];
        lv[i] = lv[i] - cl;
  ...
  */
  ...
  define_unary_function_ptr5(at_potential, alias_at_potential, &__potential, _QUOTE_ARGUMENTS,
                             true);
  ...
  int step_param_(const gen & f, const gen & g, const gen & t, gen & tmin, gen & tmax,
    ...
    vecteur tvidf = makevecteur(symb_equal(symb_derive(x__IDNT_e), f1),
                                try_limit_undef(f1, xid, nextt, 1, contextptr));
    vecteur tvidg = makevecteur(symb_equal(symb_derive(y__IDNT_e), g1),
                                try_limit_undef(g1, xid, nextt, 1, contextptr));
    vecteur tviconv;
    if (do_inflex)
      tviconv = makevecteur(symb_derive(x__IDNT_e) * symb_derive(symb_derive(y__IDNT_e)) -
                                symb_derive(y__IDNT_e) * symb_derive(symb_derive(x__IDNT_e)),
  ...
  int step_param(const gen & f, const gen & g, const gen & t, gen & tmin, gen & tmax, vecteur & poi,
                 vecteur & tvi, bool printtvi, bool exactlegende, GIAC_CONTEXT, bool do_inflex) {
...
int step_param(const gen & f, const gen & g, const gen & t, gen & tmin, gen & tmax, vecteur & poi,
  ...
  int step_func_(const gen & f, const gen & x, gen & xmin, gen & xmax, vecteur & poi, vecteur & tvi,
    ...
    gen c2 = (!(do_inflex_tabsign & 1) || is_zero(f2))
                 ? gen(vecteur(0))
  ...
  if (c.type != _VECT) {
    *logptr(contextptr) << gettext("Unable to find critical points") << endl;
    ...
    if ((do_inflex_tabsign & 1))
      tvidf2 =
          makevecteur(string2gen("y''", false), try_limit_undef(f2, xid, nextx, 1, contextptr));
    int tvs = int(tvx.size());
    for (int i = 1; i < tvs; ++i) {
      if (ctrl_c || interrupted)
        return 0;
      ...
      if (i < tvs - 1 && equalposcomp(sing, nextx)) {
        ...
        y = try_limit_undef(f, xid, nextx, -1, contextptr);
        if (0 && !is_inf(nextx) &&
            !is_zero(recursive_ratnormal(y - try_limit_undef(f, xid, nextx, 1, contextptr),
  ...
  try {
    s = step_func_(f, x, xmin, xmax, poi, tvi, periode, asym, parab, crit, inflex, printtvi,
                   exactlegende, contextptr, do_inflex);
  } catch (std::runtime_error & e) {
    last_evaled_argptr(contextptr) = NULL;
    s = 0;
  }
  ...
  int do_inflex_tabsign = 1;
    ...
    if (param)
      periodic = step_param(f._VECTptr->front(), f._VECTptr->back(), x, xmin, xmax, poi, tvi, false,
                            exactlegende, contextptr, do_inflex_tabsign == 1);
    else {
      gen periode;
  ...
  static vecteur covariance_correlation(const gen & g, const gen & u1, const gen & u2, int xcol,
    ...
    gen correlation =
        tmp / sqrt((n * sigmax2 - sigmax * sigmax) * (n * sigmay2 - sigmay * sigmay), contextptr);
  ...
  gen _linear_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    return put_attributs(_droite(makesequence(b * cst_i, 1 + (b + a) * cst_i), contextptr),
  ...
  gen _exponential_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    return put_attributs(_plotfunc(makesequence(evalf(exp(b, contextptr), 1, contextptr) *
  ...
  gen _logarithmic_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    return put_attributs(_plotfunc(makesequence(a * ln(vx_var, contextptr) + b,
                                                symb_equal(vx_var, symb_interval(xmin, xmax))),
                                   contextptr),
                         attributs, contextptr);
  }
  ...
  gen _power_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    return put_attributs(_plotfunc(makesequence(exp(b, contextptr) * pow(vx_var, a, contextptr),
                                                symb_equal(vx_var, symb_interval(xmin, xmax))),
                                   contextptr),
                         attributs, contextptr);
  }
  ...
  gen _polynomial_regression_plot(const gen & g, GIAC_CONTEXT) {
    ...
    return put_attributs(
        _plotfunc(makesequence(res, symb_equal(vx_var, symb_interval(xmin, xmax))), contextptr),
        attributs, contextptr);
  }
    ...
    for (int j = 0; j < ncamemberts; j++) {
      ...
      for (; i < s; ++i) {
        ...
        if (names[i].type != _STRNG && has_evalf(names[i], xpos, 1, contextptr)) {
          ...
          res.push_back(
              symb_pnt(tmp, i < t ? attr[i] : (i * 8192 | _FILL_POLYGON | _QUADRANT2), contextptr));
        } else {
          xpos = xy + i;
          gen xposl(xpos + largeur), xposml(xpos - largeur);
          tmp = xposl + ivals;
          tmp = gen(makevecteur(tmp, xposl, xposml, xposml + ivals, tmp), _LINE__VECT);
          res.push_back(
              symb_pnt_name(tmp, i < t ? attr[i] : (color_list[i % 8] | _FILL_POLYGON | _QUADRANT2),
                            names[i], contextptr));
        }
      }
    }
      ...
      for (; i < s; ++i) {
        ...
        if (da100 > 0) {
          ...
          res.push_back(symb_pnt_name(
              tmp, i < t ? attr[i] : (color_list[i % 8] | _FILL_POLYGON | pos),
  ...
  void draw_filled_arc(int x, int y, int rx, int ry, int color, int theta1_deg, int theta2_deg,
                       int xmin, int xmax, int ymin, int ymax, bool segment) {
    ...
    if (vs >= 3) {
      ...
      if (x0.type == _INT_ && y0.type == _INT_ && r.type == _INT_) {
        if (arc) {
          ...
          if (attr & 0x40000000)
            draw_filled_arc(x0.val, y0.val, r.val, ry.val, attr & 0xffff,
                            int(theta1._DOUBLE_val * 180 / M_PI + .5),
                            int(theta2._DOUBLE_val * 180 / M_PI + .5), 0, 384, 0, 218);
  ...
  gen _rgb(const gen & args, GIAC_CONTEXT) {
    if (args.type == _VECT && args._VECTptr->size() == 3) {
      ...
      if (a.type == _INT_ && b.type == _INT_ && c.type == _INT_ && a.val >= 0 && a.val < 256 &&
          b.val >= 0 && b.val < 256 && c.val >= 0 && c.val < 256) {
        int d = 0;
        d = (((giacmin(a.val, 255) * 32) / 256) << 11) | (((giacmin(b.val, 255) * 64) / 256) << 5) |
            ((giacmin(c.val, 255) * 32) / 256);
        if (d > 0 && d < 512)
          d += (1 << 11);
        return d;
      }
  ...
  gen _plotarea(const gen & g, GIAC_CONTEXT) {
    ...
    if (v[0].type == _VECT) {
      ...
      return makesequence(pnt_attrib(v[0], attributs, contextptr),
                          _legende(makesequence(makevecteur(30, 30), attributs[1]), contextptr));
    }
            ...
            if (polyg.type == _VECT) {
              ...
              if (s >= 3 && v[2].type == _INT_) {
                ...
                *logptr(contextptr) << "Approx area " << A << endl;
                return makesequence(
                    A, pnt_attrib(res, attributs, contextptr),
                    _couleur(makesequence(graph, _RED + _DASH_LINE + _LINE_WIDTH_3), contextptr));
              } // end if (s>=3)
