...
namespace giac {
  ...
  public:
    ...
    std::pointer_to_binary_function<const monomial<T> &, const monomial<T> &, bool>
        m_is_strictly_greater;
    // constructors
    tensor(const tensor<T> & t)
        : dim(t.dim), coord(t.coord), is_strictly_greater(t.is_strictly_greater),
          m_is_strictly_greater(t.m_is_strictly_greater) {}
    tensor(const tensor<T> & t, const std::vector<monomial<T>> & v)
        : dim(t.dim), coord(v), is_strictly_greater(t.is_strictly_greater),
          m_is_strictly_greater(t.m_is_strictly_greater) {}
    ...
    tensor()
        : dim(0), is_strictly_greater(i_lex_is_strictly_greater),
          m_is_strictly_greater(std::ptr_fun<const monomial<T> &, const monomial<T> &, bool>(
              m_lex_is_strictly_greater<T>)) {}
    explicit tensor(int d)
        : dim(d), is_strictly_greater(i_lex_is_strictly_greater),
          m_is_strictly_greater(std::ptr_fun<const monomial<T> &, const monomial<T> &, bool>(
              m_lex_is_strictly_greater<T>)) {}
    explicit tensor(int d, const tensor<T> & t)
        : dim(d), is_strictly_greater(t.is_strictly_greater),
          m_is_strictly_greater(t.m_is_strictly_greater) {}
    tensor(const monomial<T> & v)
        : dim(int(v.index.size())), is_strictly_greater(i_lex_is_strictly_greater),
          m_is_strictly_greater(std::ptr_fun<const monomial<T> &, const monomial<T> &, bool>(
              m_lex_is_strictly_greater<T>)) {
      coord.push_back(v);
    }
    tensor(const T & v, int d)
        : dim(d), is_strictly_greater(i_lex_is_strictly_greater),
          m_is_strictly_greater(std::ptr_fun<const monomial<T> &, const monomial<T> &, bool>(
              m_lex_is_strictly_greater<T>)) {
      if (!is_zero(v))
    ...
    tensor(int d, const std::vector<monomial<T>> & c)
        : dim(d), coord(c), is_strictly_greater(i_lex_is_strictly_greater),
          m_is_strictly_greater(std::ptr_fun<const monomial<T> &, const monomial<T> &, bool>(
              m_lex_is_strictly_greater<T>)) {}
    ~tensor() {
      coord.clear();
    ...
    bool TDivRem(const tensor<T> & other, tensor<T> & quo, tensor<T> & rem,
                 bool allowrational = true) const; // this=quo*other+rem
    bool TDivRemHash(const tensor<T> & b, tensor<T> & quo, tensor<T> & r,
                     bool allowrational = false, int exactquo = 0, double qmax = 0.0)
        const; // same as TDivRem but allowrationnal=false *and* poly with 1 main variable
    bool TDivRem1(const tensor<T> & b, tensor<T> & quo, tensor<T> & r, bool allowrational = false,
                  int exactquo = 0) const;
    bool Texactquotient(const tensor<T> & other, tensor<T> & quo,
                        bool allowrational = false) const; // this=quo*other+rem, rem must be 0
    bool TPseudoDivRem(const tensor<T> & other, tensor<T> & quo, tensor<T> & rem,
                       tensor<T> & a) const; // a*this=quo*other+rem
  ...
  void convert(const tensor<T> & p, const index_t & deg, std::vector<T_unsigned<T, U>> & v) {
    ...
    U u;
    int nterms;
    index_t::const_iterator itit, itcur, idxcur, idxend, ditbeg = deg.begin(), ditend = deg.end(),
                                                         dit;
    for (; it != itend; ++it) {
      u = 0;
      ...
      if (itstop->index.back())
        continue;
      for (idxcur = itstop->index.begin(), idxend = itstop->index.end() - 1; idxcur != idxend;
           ++idxcur, ++itcur) {
        if (*idxcur != *itcur)
          break;
      }
  ...
  template <class T> bool operator==(const tensor<T> & p, const tensor<T> & q) {
    if (p.dim != q.dim || p.coord.size() != q.coord.size())
      return false;
    typename vector<monomial<T>>::const_iterator it = p.coord.begin(), jt = q.coord.begin(),
                                                 itend = p.coord.end();
    for (; it != itend; ++it, ++jt) {
      if (*it != *jt)
        return false;
  ...
  template <class T> void tensor<T>::insert_monomial(const monomial<T> & c) {
    coord.push_back(c);
    this->tsort(); // sort(coord.begin(),coord.end(),m_is_strictly_greater);
  }

  template <class T> int tensor<T>::degree(int n) const {
  ...
  template <class T> int tensor<T>::total_degree() const {
    typename std::vector<monomial<T>>::const_iterator it = this->coord.begin();
    typename std::vector<monomial<T>>::const_iterator it_end = this->coord.end();
    int res = 0;
    for (; it != it_end; ++it) {
  ...
  template <class T> int tensor<T>::partial_degree(int vars) const {
    typename std::vector<monomial<T>>::const_iterator it = this->coord.begin();
    typename std::vector<monomial<T>>::const_iterator it_end = this->coord.end();
    int res = 0;
    for (; it != it_end; ++it) {
  ...
  template <class T> int tensor<T>::valuation(int n) const {
    typename std::vector<monomial<T>>::const_iterator it = this->coord.begin();
    typename std::vector<monomial<T>>::const_iterator it_end = this->coord.end();
    if (it == it_end)
      return 0;
  ...
  template <class T> index_t tensor<T>::degree() const {
  ...
  template <class T> void tensor<T>::append(const tensor<T> & p) {
    if (p.coord.empty())
      return;
    if (this->coord.empty()) {
      this->dim = p.dim;
  ...
  template <class T> tensor<T> tensor<T>::multiplydegrees(int d) const {
    tensor<T> res(dim);
  ...
  template <class T> tensor<T> tensor<T>::dividedegrees(int d) const {
    tensor<T> res(dim);
    typename std::vector<monomial<T>>::const_iterator it = coord.begin(), it_end = coord.end();
    for (; it != it_end; ++it) {
      index_t i = it->index.iref();
  ...
  template <class T> tensor<T> tensor<T>::dividealldegrees(int d) const {
    tensor<T> res(dim);
    typename std::vector<monomial<T>>::const_iterator it = coord.begin(), it_end = coord.end();
    for (; it != it_end; ++it) {
      index_t i = it->index.iref();
  ...
  template <class T> int tensor<T>::position(const index_m & v) const {
  ...
  template <class T> index_m tensor<T>::vector_int(int position) const {
    return (coord[position].index);
  }

  template <class T> T & tensor<T>::operator()(const index_m & v) {
    int p = position(v);
    if (p != -1)
  ...
  template <class T> void lexsort(std::vector<monomial<T>> & v) {
#if 1 // def NSPIRE
    sort_helper<T> M(
        std::ptr_fun<const monomial<T> &, const monomial<T> &, bool>(m_lex_is_strictly_greater<T>));
  ...
  template <class T> int tensor<T>::gcddeg(int k) const {
    assert(k < dim);
    int res = 0;
    typename std::vector<monomial<T>>::const_iterator it = coord.begin(), itend = coord.end();
    for (; it != itend; ++it) {
  ...
  template <class T> index_t tensor<T>::gcddeg() const {
    typename std::vector<monomial<T>>::const_iterator it = coord.begin(), itend = coord.end();
    assert(itend != it);
  ...
  */
  ...
  template <class T> tensor<T> tensor<T>::operator()(const T & x0) const {
    if (coord.empty())
      return *this;
    if (!dim) {
      return *this;
    ...
    for (; it != horner_coord.end(); ++it) {
      if (pui.front() == it->index.front()) {
        ...
        rem.TAdd(add_rem, rem);
        rem *= pow(x0, pui.front() - it->index.front());
#else
  ...
  template <class T> void tensor<T>::TAdd(const tensor<T> & other, tensor<T> & result) const {
  ...
  */
  ...
  template <class T> void tensor<T>::TSub(const tensor<T> & other, tensor<T> & result) const {
    typename std::vector<monomial<T>>::const_iterator a = coord.begin();
    typename std::vector<monomial<T>>::const_iterator a_end = coord.end();
    typename std::vector<monomial<T>>::const_iterator b = other.coord.begin();
    typename std::vector<monomial<T>>::const_iterator b_end = other.coord.end();
  ...
  */
  ...
  template <class T> tensor<T> Tpow(const tensor<T> & x, int n) {
#ifndef NO_STDEXCEPT
    if (n < 0)
  ...
  */
  ...
  template <class T> tensor<T> & tensor<T>::operator*=(const T & fact) {
    // Tensor constant multiplication
    if (is_one(fact))
      return *this;
    if (is_zero(fact)) {
  ...
  */
  ...
  template <class T>
  tensor<T> Tnextcoeff(typename std::vector<monomial<T>>::const_iterator & it,
                       const typename std::vector<monomial<T>>::const_iterator & itend) {
    if (it == itend)
      return tensor<T>(0);
  ...
  template <class T>
  tensor<T> Tlastcoeff(const typename std::vector<monomial<T>>::const_iterator & itbeg,
                       const typename std::vector<monomial<T>>::const_iterator & itend) {
    assert(itbeg != itend);
  ...
  template <class T>
  bool tensor<T>::TDivRem1(const tensor<T> & b, tensor<T> & quo, tensor<T> & r, bool allowrational,
                           int exactquo) const {
    ...
    */
    ...
    int bdeg = it->index.front(), rdeg = r.lexsorted_degree(),
        qdeg = rdeg - bdeg; // int bval=(itend-1)->index.front();
  ...
  template <class T>
  bool tensor<T>::TDivRemHash(const tensor<T> & b, tensor<T> & quo, tensor<T> & r,
                              bool allowrational, int exactquo, double qmax) const {
    const tensor<T> & a = *this;
    quo.coord.clear();
    quo.dim = a.dim;
    r.dim = a.dim;
    ...
    if (ddeg > 2 && bs > 10) {
      ...
      if (ans < RAND_MAX) {
        ...
        convert(*this, d, p1);
        convert(b, d, p2);
        if (hashdivrem<T, unsigned>(p1, p2, quot, remain, vars, /* reduce */ 0, qmax,
                                    allowrational)) {
          convert(quot, d, quo);
          convert(remain, d, r);
          return true;
      ...
      if (ans / RAND_MAX < RAND_MAX) {
        ...
        convert(*this, d, p1);
        convert(b, d, p2);
        if (hashdivrem<T, ulonglong>(p1, p2, quot, remain, vars, /* reduce */ 0, qmax,
                                     allowrational)) {
          convert(quot, d, quo);
          convert(remain, d, r);
          return true;
  ...
  template <class T> void tensor<T>::Tcoeffs(std::vector<tensor<T>> & v) const {
  ...
  template <class T> std::vector<tensor<T>> tensor<T>::Tcoeffs() const {
    int current_deg = lexsorted_degree();
  ...
  template <class T>
  bool tensor<T>::TDivRem(const tensor<T> & other, tensor<T> & quo, tensor<T> & rem,
                          bool allowrational) const {
    //printf("K");
    ...
    if (bsize == 1) {
      ...
      if (b_max == b_max * 0) {
        if (is_one(b))
          ...
          for (typename std::vector<monomial<T>>::const_iterator it = coord.begin(); it != itend;
               ++it) {
            T q = rdiv(it->value, b);
    ...
    while (a_max >= b_max) {
      ...
      if (!allowrational) {
        if (has_denominator(q) || (!is_zero(q * b - rem.coord.front().value)))
          return false;
      }
      // end error trapping
  ...
  template <class T>
  bool tensor<T>::TPseudoDivRem(const tensor<T> & other, tensor<T> & quo, tensor<T> & rem,
                                tensor<T> & a) const {
...
#endif // GIAC_VECTOR
#endif // NSPIRE
      typename std::vector<monomial<T>>::const_iterator it = rem.coord.begin(),
                                                        itend = rem.coord.end();
  ...
  template <class T> T tensor<T>::constant_term() const {
    if (!((*this).coord.size()))
      return T(0);
#if 1
  ...
  template <class T> tensor<T> tensor<T>::shift(const index_m & i, const T & fois) const {
  ...
  template <class T> tensor<T> tensor<T>::shift(const T & fois, const index_m & i) const {
    tensor<T> res(dim, *this);
    res.coord.reserve(coord.size());
  ...
  template <class T> tensor<T> tensor<T>::shift(const index_m & i) const {
    tensor<T> res(dim);
    res.coord.reserve(coord.size());
  ...
  template <class T> void tensor<T>::reverse() {
    typename std::vector<monomial<T>>::const_iterator itend = coord.end();
  ...
  template <class T> void tensor<T>::reorder(const std::vector<int> & permutation) {
    typename std::vector<monomial<T>>::const_iterator itend = coord.end();
    for (typename std::vector<monomial<T>>::iterator it = coord.begin(); it != itend; ++it)
  ...
  template <class T> T tensor<T>::norm() const {
    T res(0);
    typename std::vector<monomial<T>>::const_iterator itend = coord.end();
  ...
  template <class T> bool Tlistmax(const tensor<T> & p, T & n) {
    n = T(1);
    for (typename std::vector<monomial<T>>::const_iterator it = p.coord.begin();
         it != p.coord.end(); ++it) {
  ...
  template <class T> void Tapply(const tensor<T> & p, T (*f)(const T &), tensor<T> & pgcd) {
    typename std::vector<monomial<T>>::const_iterator it = p.coord.begin(), itend = p.coord.end();
    Apply(it, itend, f, pgcd.coord);
  }

  template <class T> tensor<T> Tapply(const tensor<T> & p, T (*f)(const T &)) {
    tensor<T> res(p.dim);
    Tapply(p, f, res);
    return res;
  }

  template <class T> void Tlgcd(const tensor<T> & p, tensor<T> & pgcd) {
    if (!p.dim) {
      pgcd = p;
      return;
    }
  ...
  template <class T> tensor<T> Tlgcd(const tensor<T> & p) {
    if (p.dim == 1) {
  ...
  template <class T> void Tcommonlgcd(const tensor<T> & p, const tensor<T> & q, tensor<T> & pgcd) {
    if (!p.dim) {
      pgcd = p;
      return;
    }
  ...
  template <class T> tensor<T> Tcarcomp(const tensor<T> & p) {
    typename std::vector<monomial<T>>::const_iterator it = p.coord.begin();
    int n = it->index.front();
    tensor<T> res(p.dim);
  ...
  template <class T> tensor<T> Tfirstcoeff(const tensor<T> & p) {
    typename std::vector<monomial<T>>::const_iterator it = p.coord.begin();
    typename std::vector<monomial<T>>::const_iterator itend = p.coord.end();
    if (it == itend)
      return p;
  ...
  template <class T> void Tswap(tensor<T> *& a, tensor<T> *& b) {
    tensor<T> * c = a;
    a = b;
    b = c;
  }
  ...
  template <class T>
  void Tcontentgcd(const tensor<T> & p, const tensor<T> & q, tensor<T> & cont, tensor<T> & prim,
                   int gcddeg) {
    if (p.coord.empty()) {
      cont = Tlgcd(q);
      prim = q / Tlgcd(q);
    ...
    while (!a.coord.empty() && !ctrl_c && !interrupted) {
  ...
  template <class T>
  void Tsturm_seq(const giac::tensor<T> & p, giac::tensor<T> & cont,
                  std::vector<tensor<T>> & sturm_seq) {
  ...
  template <class T> tensor<T> Tgcdpsr(const tensor<T> & p, const tensor<T> & q, int gcddeg = 0) {
  ...
  template <class T> tensor<T> Tresultant(const tensor<T> & p, const tensor<T> & q) {
    assert(p.dim == q.dim);
    if (p.coord.empty())
      return p;
    if (q.coord.empty())
    ...
    while (n) {
      ...
      ptmp.TPseudoDivRem(qtmp, tmp2, rem,
                         a); // (ptmp*pow(tmp1,ddeg+1)).TDivRem1(qtmp,tmp2,rem,false);
  ...
  template <class T>
  void Tegcdpsr(const tensor<T> & p1, const tensor<T> & p2, tensor<T> & u, tensor<T> & v,
                tensor<T> & d) {
  ...
  template <class T>
  void TegcdTlgcd(const tensor<T> & p1, const tensor<T> & p2, tensor<T> & u, tensor<T> & v,
                  tensor<T> & d) {
    assert(p1.dim == p2.dim);
    // p1=cp1*a p2=cp2*b
    // a*u+b*v=d
  ...
  template <class T>
  void Tegcdtoabcuv(const tensor<T> & a, const tensor<T> & b, const tensor<T> & c, tensor<T> & u,
                    tensor<T> & v, tensor<T> & d, tensor<T> & C) {
  ...
  template <class T>
  void Tabcuv(const tensor<T> & a, const tensor<T> & b, const tensor<T> & c, tensor<T> & u,
              tensor<T> & v, tensor<T> & C) {
    tensor<T> d(a.dim);
    Tegcdpsr(a, b, u, v, d); // a*u+b*v=d
    Tegcdtoabcuv(a, b, c, u, v, d, C);
  }

  template <class T> tensor<T> tensor<T>::derivative() const {
    if (coord.empty())
      return *this;
    tensor<T> res(dim);
    if (dim == 0)
  ...
  template <class T> tensor<T> tensor<T>::integrate() const {
    if (coord.empty())
      return *this;
    tensor<T> res(dim);
    res.coord.reserve(coord.size());
  ...
  template <class T>
  void Tfracadd(const tensor<T> & n1, const tensor<T> & d1, const tensor<T> & n2,
                const tensor<T> & d2, tensor<T> & num, tensor<T> & den) {
    // COUT << n1 << "/" << d1 << "+" << n2 << "/" << d2 << "=";
    if (Tis_one(d1)) {
  ...
  template <class T>
  void Tfracmul(const tensor<T> & n1, const tensor<T> & d1, const tensor<T> & n2,
                const tensor<T> & d2, tensor<T> & num, tensor<T> & den) {
    // COUT << n1 << "/" << d1 << "*" << n2 << "/" << d2 << "=";
    if (Tis_one(d1)) {
      num = n1;
  ...
  */
  ...
  template <class T> T Tproduct(const std::vector<facteur<T>> & v) {
    assert(!v.empty());
    ...
    ++it;
    for (; it != itend; ++it)
      prod *= it->mult == 1 ? it->fact
                            : Tpow(it->fact, it->mult); //  prod=prod*Tpow(it->fact,it->mult);
    return prod;
  }
  ...
  T Tproduct(typename std::vector<facteur<T>>::const_iterator it,
    ...
    for (; it != itend; ++it)
      prod *=
          it->mult == 1 ? it->fact : Tpow(it->fact, it->mult); // prod=prod*Tpow(it->fact,it->mult);
    return prod;
  }

  // square-free factorization of a polynomial
  // T must be a 0 characteristic field
  template <class T> std::vector<facteur<tensor<T>>> Tsqff_char0(const tensor<T> & p) {
  ...
  public:
    ...
    pf(const tensor<T> & n, const tensor<T> & d, const tensor<T> & f, int m)
        : num(n), fact(f), den(d), mult(m) {};
  };

  template <class T> std::ostream & operator<<(std::ostream & os, const pf<T> & v) {
    os << v.num << "/" << v.den;
    return os;
  }
...
#else
template <class T> std::ostream & operator<<(std::ostream & os, const std::vector<pf<T>> & v) {
  typename std::vector<pf<T>>::const_iterator itend = v.end();
  for (typename std::vector<pf<T>>::const_iterator it = v.begin();;) {
    os << *it;
    ++it;
  ...
  template <class T> tensor<T> TsimplifybyTlgcd(tensor<T> & a, tensor<T> & b) {
  ...
  template <class T>
  void Tpartfrac(const tensor<T> & num, const tensor<T> & den,
                 /* const tensor<T> & dendiff ,*/ const std::vector<facteur<tensor<T>>> & w, int n,
                 int m, std::vector<pf<T>> & pfdecomp) {
    if (m == n)
      return;
    if (m - n == 1) {
  ...
  template <class T>
  void Tpartfrac(const tensor<T> & num, const tensor<T> & den,
                 const std::vector<facteur<tensor<T>>> & v, std::vector<pf<T>> & pfdecomp,
                 tensor<T> & ipnum, tensor<T> & ipden) {
    int n = int(v.size());
    pfdecomp.reserve(n);
  ...
  template <class T> pf<T> Tlaplace_reduce_pf(const pf<T> & p_cst, tensor<T> & laplacevar) {
    pf<T> p(p_cst);
    assert(p.mult > 0);
    if (p.mult == 1)
      return p_cst;
  ...
  template <class T> pf<T> Tintreduce_pf(const pf<T> & p_cst, std::vector<pf<T>> & intdecomp) {
    assert(p_cst.mult > 0);
    if (p_cst.mult == 1)
      return p_cst;
    pf<T> p(p_cst);
    ...
    tensor<T> resnum(fprime.dim), resden(T(1), fprime.dim), numtemp(fprime.dim),
        dentemp(fprime.dim);
    Tegcdpsr(p.fact, fprime, u, v, d); // f*u+f'*v=d
    tensor<T> usave(u), vsave(v);
    int initial_mult = p.mult - 1;
  ...
  template <class T> std::vector<T> makevector(const T & a, const T & b) {
    std::vector<T> v;
    v.push_back(a);
    v.push_back(b);
    return v;
  ...
  template <class T> std::vector<T> makevector(const T & a, const T & b, const T & c) {
    std::vector<T> v;
    v.push_back(a);
    v.push_back(b);
    v.push_back(c);
  ...
  template <class T> std::vector<T> makevector(const T & a, const T & b, const T & c, const T & d) {
    std::vector<T> v;
    v.push_back(a);
    v.push_back(b);
    v.push_back(c);
  ...
  template <class T> std::vector<T> merge(const std::vector<T> & a, const std::vector<T> & b) {
    std::vector<T> v(a);
    int as = a.size();
    int bs = b.size();
    v.reserve(as + bs);
