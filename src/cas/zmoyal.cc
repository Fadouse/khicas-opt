...
namespace giac {
  ...
  gen incomplete_beta(double a, double b, double p, bool regularize) { // regularize=true by default
	...
	return r;
        ...
        if (regularize)
          return res / a *
                 std::exp(a * std::log(p) + (b - 1) * std::log(1 - p) + lngamma(a + b) -
                          lngamma(a) - lngamma(b));
  ...
  gen Beta(const gen & a, const gen & b, GIAC_CONTEXT) {
    if (a.type == _DOUBLE_ || b.type == _DOUBLE_ || a.type == _FLOAT_ || b.type == _FLOAT_ ||
    ...
    gen n;
    if (a.type == _FRAC && b.type == _FRAC && is_positive(a, contextptr) &&
        is_positive(b, contextptr) && is_integer((n = a + b))) {
      gen res = 1, a_(a), b_(b);
  ...
  gen _Beta(const gen & args, GIAC_CONTEXT) {
    ...
    if (s > 2 && (v[0].type == _DOUBLE_ || v[1].type == _DOUBLE_ || v[2].type == _DOUBLE_ ||
                  v[0].type == _REAL || v[1].type == _REAL || v[2].type == _REAL)) {
      gen tmp = evalf_double(v, 1, contextptr);
      if (tmp.type == _VECT)
        v = *tmp._VECTptr;
      s = int(v.size());
    }
    if ((s == 3 || s == 4) && v[0].type == _DOUBLE_ && v[1].type == _DOUBLE_ &&
        v[2].type == _DOUBLE_) {
      return incomplete_beta(v[0]._DOUBLE_val, v[1]._DOUBLE_val, v[2]._DOUBLE_val,
                             s == 4 && !is_zero(v[3]));
    }
    if (s < 2 || s > 4)
  ...
  gen _upper_incomplete_gamma(const gen & args, GIAC_CONTEXT) {
    ...
    if ((s == 2 || s == 3) && v[0].type == _DOUBLE_ && v[1].type == _DOUBLE_) {
      double res = upper_incomplete_gammad(v[0]._DOUBLE_val, v[1]._DOUBLE_val,
                                           s == 3 ? !is_zero(v[2]) : false);
      if (res == -1) {
        if (s == 3 && !is_zero(v[2]))
          return 1 - lower_incomplete_gamma(v[0]._DOUBLE_val, v[1]._DOUBLE_val, true, contextptr);
        return Gamma(v[0]._DOUBLE_val, contextptr) -
               lower_incomplete_gamma(v[0]._DOUBLE_val, v[1]._DOUBLE_val, false, contextptr);
    ...
    if (s == 3) {
      if (is_zero(v[2]))
        return Gamma(v[0], contextptr) -
               symbolic(at_lower_incomplete_gamma, makesequence(v[0], v[1]));
      return symbolic(at_Gamma, makesequence(v[0], v[1], 1));
    }
    return symbolic(at_upper_incomplete_gamma, args);
  }
  static const char _upper_incomplete_gamma_s[] = "ugamma";
  static define_unary_function_eval(__upper_incomplete_gamma, &_upper_incomplete_gamma,
                                    _upper_incomplete_gamma_s);
  define_unary_function_ptr5(at_upper_incomplete_gamma, alias_at_upper_incomplete_gamma,
                             &__upper_incomplete_gamma, 0, true);
  ...
  define_unary_function_ptr5(at_randchisquared, alias_at_randchisquared, &__randchisquared, 0,
                             true);
  ...
  static gen normald(const gen & m, const gen & s, const gen & x, GIAC_CONTEXT) {
    gen v(s * s);
    return inv(sqrt(2 * cst_pi * v, contextptr), contextptr) *
           exp(-pow(x - m, 2) / (2 * v), contextptr);
  }
...
#else
    if (x > 5) {
      double p = .3275911, a1 = .254829592, a2 = -.284496736, a3 = 1.421413741, a4 = -1.453152027,
             a5 = 1.061405429;
      double t = 1.0 / (1 + p * x);
  ...
  gen _normal_cdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 4) {
      ...
      return normal_cdf((v[3] - v[0]) / v[1], contextptr) -
             normal_cdf((v[2] - v[0]) / v[1], contextptr);
    }
    return gensizeerr(contextptr);
  }
  ...
  static gen utpn_inverse(double y, GIAC_CONTEXT) {
    identificateur x(" x");
    return newton(erf(x / std::sqrt(2.0), contextptr) + 2 * y - 1, x, utpn_initial_guess(y),
                  NEWTON_DEFAULT_ITERATION, 1e-5, 1e-12, true, 1, 0, 1, 0, .5, contextptr);
  }
  ...
  gen _normal_icdf(const gen & g, GIAC_CONTEXT) {
    ...
    vecteur v = *g._VECTptr;
    int s = v.size();
    if (s == 2 && (v.back() == at_left || v.back() == at_right || v.back() == at_centre ||
                   v.back() == at_tail)) {
  ...
  gen apply3rd(const gen & e1, const gen & e2, const gen & e3, const context * contextptr,
               gen (*f)(const gen &, const gen &, const gen &, const context *)) {
    if (e3.type != _VECT)
  ...
  gen binomial(const gen & N, const gen & K, const gen & P, GIAC_CONTEXT) {
	...
	return Gamma(nd+1,contextptr)/Gamma(kd+1,contextptr)/Gamma(nd-kd+1,contextptr)*pow(pd,kd,contextptr)*pow(1-pd,nd-kd,contextptr);
      ...
      if (nd.type == _DOUBLE_ && kd.type == _DOUBLE_ && pd.type == _DOUBLE_) {
        ...
        return std::exp(lngamma(ndd + 1) - lngamma(kdd + 1) - lngamma(nk + 1) +
                        kdd * std::log(pdd) + nk * std::log(1 - pdd));
      }
#endif
    }
  ...
  gen _binomial(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 2) {
      ...
      if (v0.type != _DOUBLE_ || v1.type != _DOUBLE_)
        return _factorial(v[0], contextptr) /
               (_factorial(v[1], contextptr) * _factorial(v[0] - v[1], contextptr));
  ...
  gen _multinomial(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 3) {
      ...
      if (!is_zero(1 - _plus(P, contextptr), contextptr))
        swapgen(K, P);
      if (_plus(K, contextptr) != n || K.type != _VECT || P.type != _VECT ||
          K._VECTptr->size() != P._VECTptr->size())
        return gensizeerr(contextptr);
  ...
  gen _randmultinomial(const gen & args, GIAC_CONTEXT) {
    ...
    if (args._VECTptr->size() == 2 && g1.type == _VECT) {
      ...
      if (g2.type != _VECT || g2._VECTptr->size() != g1._VECTptr->size() ||
          !is_zero(1 - _sum(g1, contextptr)))
        return gensizeerr(contextptr);
      double u = giac_rand(contextptr) / (rand_max2 + 1.0);
      gen somme = 0;
  ...
  define_unary_function_ptr5(at_randmultinomial, alias_at_randmultinomial, &__randmultinomial, 0,
                             true);
  ...
  gen _negbinomial(const gen & g, GIAC_CONTEXT) {
    ...
    int s = int(v.size());
    if (s == 2)
      return symbolic(at_negbinomial, g); // for random
    if (s == 3) {
  ...
  gen _negbinomial_cdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 4) {
      ...
      return _Beta(makesequence(n, k2 + 1, p, 1), contextptr) -
             _Beta(makesequence(n, k1 + 1, p, 1), contextptr);
    }
    return gensizeerr(contextptr);
  }
  ...
  define_unary_function_ptr5(at_negbinomial_cdf, alias_at_negbinomial_cdf, &__negbinomial_cdf, 0,
                             true);
  ...
  define_unary_function_ptr5(at_negbinomial_icdf, alias_at_negbinomial_icdf, &__negbinomial_icdf, 0,
                             true);
  ...
  gen binomial_cdf(const gen & n, const gen & p, const gen & x0, const gen & x, GIAC_CONTEXT) {
    ...
    gen pd = p;
    if (pd.type == _FLOAT_)
      pd = evalf_double(p, 1, contextptr);
    if (n.type == _INT_ && pd.type == _DOUBLE_ && (fx.val - fx0.val) > 100) {
      ...
      if (fx0.val <= 0) {
        ...
        if (fx.val >= n.val)
          ...
          res = incomplete_beta(fx0.val, n.val - fx0.val + 1, p) -
                incomplete_beta(fx.val + 1, n.val - fx.val, p);
      }
      if (!is_undef(res))
        return res;
      // Other formula
    ...
    if (n.type == _INT_ && pd.type == _DOUBLE_ && last.type == _DOUBLE_ && last._DOUBLE_val != 0) {
      ...
      for (int i = fx0.val + 1; i <= fx.val; ++i) {
        if (i % 25 == 0)
          lastd = evalf_double(binomial(n, i, pd, contextptr), 1, contextptr)
                      ._DOUBLE_val; // avoid loss of precision
        else
  ...
  gen binomial_icdf(const gen & n0, const gen & p0, const gen & x_orig, GIAC_CONTEXT) {
    ...
    if (is_strictly_greater(p, 1, contextptr) || is_strictly_greater(0, p, contextptr))
      return gensizeerr(contextptr);
    if (n.type != _INT_ || p.type != _DOUBLE_ || x.type != _DOUBLE_ || x._DOUBLE_val < 0 ||
        x._DOUBLE_val > 1)
    ...
    if (N * P >= 30 && N * (1 - P) >= 30) {
      ...
      gen g = _floor(
          _normal_icdf(makesequence(n * p, sqrt(n * p * (1 - p), contextptr), x), contextptr),
          contextptr);
      int G = g.val;
      ...
      long_double current = std::exp(lngamma(N + 1) - lngamma(G + 1) - lngamma(N - G + 1) +
                                     G * std::log(P) + (N - G) * std::log(1 - P));
    ...
    if (last.type == _DOUBLE_) {
      ...
      for (; k <= n.val; ++k) {
        if (x._DOUBLE_val <= bd)
          return k - 1;
        if (k % 25 == 0)
          lastd = evalf_double(binomial(n, k, p, contextptr), 1, contextptr)
                      ._DOUBLE_val; // avoid loss of precision
        else
  ...
  gen _binomial_icdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 4)
      return binomial_icdf(v[0], v[1], v[3], contextptr) -
             binomial_icdf(v[0], v[1], v[2], contextptr);
    return gensizeerr(contextptr);
  }
  ...
  gen _randbinomial(const gen & g, GIAC_CONTEXT) {
    ...
    if (!is_integral(n) || n.type != _INT_ || n.val <= 0 ||
        ck_is_strictly_greater(0, p, contextptr) || ck_is_strictly_greater(p, 1, contextptr))
      return gensizeerr(contextptr);
  ...
  gen poisson(const gen & m, const gen & k, GIAC_CONTEXT) {
    ...
    if (M.type == _DOUBLE_) {
      ...
      if (K.type == _DOUBLE_)
        return std::exp(-M._DOUBLE_val + K._DOUBLE_val * std::log(M._DOUBLE_val) -
                        lngamma(K._DOUBLE_val + 1));
    }
  ...
  gen student(const gen & n0, const gen & x, GIAC_CONTEXT) {
    ...
    if (!is_integral(n) || n.val < 1)
      return gensizeerr(contextptr);
    return Gamma(rdiv(n + 1, 2, contextptr), contextptr) /
           Gamma(rdiv(n, 2, contextptr), contextptr) / sqrt(n * cst_pi, contextptr) *
           pow((1 + pow(x, 2) / n), -rdiv(n + 1, 2, contextptr), contextptr);
  }
  ...
  */
  ...
  gen UTPT(const gen & n_orig, const gen & x0, GIAC_CONTEXT) {
    ...
    if (0 && dof >= 100) {
      ...
      y = a * y;
      double res =
          (((((-.4 * y - 3.3) * y - 24) * y - 85.5) / (.8 * y * y + 100 + b) + y + 3) / b + 1) *
          std::sqrt(y);
      if (x < 0)
        res = -res;
  ...
  gen student_icdf(const gen & m0, const gen & t_orig, GIAC_CONTEXT) {
    gen t = evalf_double(t_orig, 1, contextptr);
    gen m(m0);
    if (!is_integral(m) || m.val < 1 || t.type != _DOUBLE_ || t._DOUBLE_val < 0 ||
        t._DOUBLE_val > 1)
    ...
    identificateur x(" x");
    gen res = newton(_student_cdf(makesequence(m, x), contextptr) - y, x, x0,
                     NEWTON_DEFAULT_ITERATION, 1e-5, 1e-12, true, 1, 0, 1, 0, .5, contextptr);
    if (!is_undef(res))
      return res;
  ...
  gen chisquare(const gen & n, const gen & x, GIAC_CONTEXT) {
    ...
    gen n2 = n / 2;
    return rdiv(pow(x, n2 - 1, contextptr) * exp(-x / 2, contextptr),
                Gamma(n2, contextptr) * pow(2, n2, contextptr), contextptr);
  }
  ...
  */
  ...
  define_unary_function_ptr5(at_chisquared_cdf, alias_at_chisquared_cdf, &__chisquared_cdf, 0,
                             true);
  ...
  gen chisquare_icdf(const gen & m0, const gen & t_orig, GIAC_CONTEXT) {
    ...
    // FIXME
    identificateur x(" z");
    return newton(1 - UTPC(m, x, contextptr) - t, x, x0, NEWTON_DEFAULT_ITERATION, 1e-5, 1e-12,
                  true, 1, 0, 1, 0, .5, contextptr);
  }
  gen _chisquare_icdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 3) {
      ...
      if (v[2] == at_centre)
        return makevecteur(chisquare_icdf(v[0], (1 - v[1]) / 2, contextptr),
                           chisquare_icdf(v[0], (1 + v[1]) / 2, contextptr));
      if (v[2] == at_tail)
        return makevecteur(chisquare_icdf(v[0], (v[1]) / 2, contextptr),
                           chisquare_icdf(v[0], 1 - v[1] / 2, contextptr));
  ...
  define_unary_function_ptr5(at_chisquare_icdf, alias_at_chisquare_icdf, &__chisquare_icdf, 0,
                             true);
  ...
  define_unary_function_ptr5(at_chisquared_icdf, alias_at_chisquared_icdf, &__chisquared_icdf, 0,
                             true);
  ...
  gen snedecor(const gen & a, const gen & b, const gen & x, GIAC_CONTEXT) {
    ...
    if (is_positive(-x, contextptr))
      return zero;
    return pow(a / b, a / 2, contextptr) / Beta(a / 2, b / 2, contextptr) *
           pow(x, a / 2 - 1, contextptr) * pow(1 + a / b * x, -(a + b) / 2, contextptr);
  }
  ...
  */
  gen UTPF(const gen & num, const gen & den, const gen & x0, GIAC_CONTEXT) {
    ...
    if (!is_integral(gndf) || !is_integral(gddf) || gx.type != _DOUBLE_)
      return symbolic(at_UTPF,
                      gen(makevecteur(num, den, x0), _SEQ__VECT)); // gensizeerr(contextptr);
    if (gx._DOUBLE_val <= 0)
      return plus_one;
  ...
  gen _snedecor_cdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 4)
      return snedecor_cdf(v[0], v[1], v[3], contextptr) -
             snedecor_cdf(v[0], v[1], v[2], contextptr);
    return gensizeerr(contextptr);
  }
  ...
  static double utpf_initial_guess(int num, int den, double y, GIAC_CONTEXT) {
    ...
    double w = xp * std::sqrt(h + lambda) / h -
               (lambda + 5.0 / 6.0 - 2 / (3 * h)) * fabs(1.0 / (num - 1) - 1.0 / (den - 1));
    return std::exp(2 * w);
  }
  ...
  gen snedecor_icdf(const gen & num0, const gen & den0, const gen & t_orig, GIAC_CONTEXT) {
    ...
    if (!is_integral(num) || !is_integral(den) || num.val < 0 || den.val < 0 ||
        t.type != _DOUBLE_ || t._DOUBLE_val < 0 || t._DOUBLE_val > 1)
      return gensizeerr(contextptr);
    if (t._DOUBLE_val == 0)
      return zero;
    if (t._DOUBLE_val == 1)
    ...
    // FIXME
    identificateur x(" z");
    return newton(1 - UTPF(num, den, x, contextptr) - t, x, x0, NEWTON_DEFAULT_ITERATION, 1e-5,
                  1e-12, true, 0, 1.79769313486e+308, 1, 0, .5, contextptr);
  }
  gen _snedecor_icdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 4) {
      ...
      if (v[3] == at_centre)
        return makevecteur(snedecor_icdf(v[0], v[1], (1 - v[2]) / 2, contextptr),
                           snedecor_icdf(v[0], v[1], (1 + v[2]) / 2, contextptr));
      if (v[3] == at_tail)
        return makevecteur(snedecor_icdf(v[0], v[1], (v[2]) / 2, contextptr),
                           snedecor_icdf(v[0], v[1], 1 - v[2] / 2, contextptr));
      return snedecor_icdf(v[0], v[1], v[3], contextptr) -
             snedecor_icdf(v[0], v[1], v[2], contextptr);
    }
    return gensizeerr(contextptr);
  }
  ...
  define_unary_function_ptr5(at_snedecord_icdf, alias_at_snedecord_icdf, &__snedecord_icdf, 0,
                             true);
  ...
  gen weibull_cdf(const gen & k, const gen & lambda, const gen & theta, const gen & x,
                  GIAC_CONTEXT) {
    gen tmp = (x - theta) / lambda;
  ...
  gen _weibull_cdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 5)
      return weibull_cdf(v[0], v[1], v[2], v[4], contextptr) -
             weibull_cdf(v[0], v[1], v[2], v[3], contextptr);
    return gensizeerr(contextptr);
  }
  ...
  gen weibull_icdf(const gen & k, const gen & lambda, const gen & theta, const gen & y,
                   GIAC_CONTEXT) {
  ...
  gen _weibull_icdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 5)
      return weibull_icdf(v[0], v[1], v[2], v[4], contextptr) -
             weibull_icdf(v[0], v[1], v[2], v[3], contextptr);
    return gensizeerr(contextptr);
  }
  ...
  gen _randweibulld(const gen & args, GIAC_CONTEXT) {
    ...
    if (is_positive(-k, contextptr) || is_positive(-lambda, contextptr) || k.type != _DOUBLE_ ||
        lambda.type != _DOUBLE_)
      return gensizeerr(contextptr);
  ...
  define_unary_function_ptr5(at_weibullvariate, alias_at_weibullvariate, &__weibullvariate, 0,
                             true);
  ...
  gen betad(const gen & alpha, const gen & beta, const gen & x, GIAC_CONTEXT) {
    ...
    return pow(x, alpha - 1, contextptr) * pow(1 - x, beta - 1, contextptr) /
           Beta(alpha, beta, contextptr);
  }
  ...
  gen _betad_cdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 4)
      return _Beta(makesequence(v[0], v[1], v[3], 1), contextptr) -
             _Beta(makesequence(v[0], v[1], v[2], 1), contextptr);
    return gensizeerr(contextptr);
  }
  ...
  gen betad_icdf(const gen & alpha_orig, const gen & beta_orig, const gen & t_orig, GIAC_CONTEXT) {
    ...
    if (alpha.type != _DOUBLE_ || beta.type != _DOUBLE_ || t.type != _DOUBLE_ ||
        alpha._DOUBLE_val <= 0 || beta._DOUBLE_val <= 0 || t._DOUBLE_val < 0 || t._DOUBLE_val > 1)
      return gensizeerr(
          contextptr); // symbolic(at_betad_icdf,makesequence(alpha_orig,beta_orig,t_orig));
    double y = t._DOUBLE_val;
    if (y <= 1e-13) {
    ...
    identificateur x(" x");
    return newton(symbolic(at_Beta, makesequence(alpha, beta, x, 1)) - y, x, x0,
                  NEWTON_DEFAULT_ITERATION, 1e-5, 1e-12, true, 1, 0, 1, 0, prefactor, contextptr);
  }
  ...
  gen _randbetad(const gen & args, GIAC_CONTEXT) {
    ...
    if (is_positive(-alpha, contextptr) || is_positive(-beta, contextptr) ||
        alpha.type != _DOUBLE_ || beta.type != _DOUBLE_)
      return gensizeerr(contextptr);
  ...
  gen gammad(const gen & alpha, const gen & beta, const gen & x, GIAC_CONTEXT) {
    ...
    if (x == plus_inf)
      return 0;
    return pow(x, alpha - 1, contextptr) * exp(-beta * x, contextptr) *
           pow(beta, alpha, contextptr) / Gamma(alpha, contextptr);
  }
  ...
  gen _gammad_cdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (s == 4)
      return _lower_incomplete_gamma(makesequence(v[0], v[1] * v[3], 1), contextptr) -
             _lower_incomplete_gamma(makesequence(v[0], v[1] * v[2], 1), contextptr);
    return gensizeerr(contextptr);
  }
  ...
  gen gammad_icdf(const gen & alpha_orig, const gen & beta_orig, const gen & t_orig, GIAC_CONTEXT) {
    ...
    if (alpha.type != _DOUBLE_ || beta.type != _DOUBLE_ || t.type != _DOUBLE_ ||
        alpha._DOUBLE_val <= 0 || beta._DOUBLE_val <= 0 || t._DOUBLE_val < 0 || t._DOUBLE_val > 1)
      return gensizeerr(
          contextptr); // symbolic(at_gammad_icdf,makesequence(alpha_orig,beta_orig,t_orig));
    double y = t._DOUBLE_val;
    if (y <= 1e-13) {
    ...
    return newton(symbolic(at_lower_incomplete_gamma, makesequence(alpha, x)) -
                      y * Gamma(alpha, contextptr),
                  x, x0, NEWTON_DEFAULT_ITERATION, 1e-5, 1e-12, true, 1, 0, 1, 0, prefactor,
                  contextptr) /
           beta;
  }
...
double rgamma(double a, double scale, GIAC_CONTEXT) {
  ...
  if (x > 0.0) {
    ...
    if (fabs(v) <= 0.25)
      q = q0 +
          0.5 * t * t * ((((((a7 * v + a6) * v + a5) * v + a4) * v + a3) * v + a2) * v + a1) * v;
    else
  ...
  define_unary_function_ptr5(at_exponential_cdf, alias_at_exponential_cdf, &__exponential_cdf, 0,
                             true);
  ...
  define_unary_function_ptr5(at_exponentiald_cdf, alias_at_exponentiald_cdf, &__exponentiald_cdf, 0,
                             true);
  ...
  define_unary_function_ptr5(at_exponential_icdf, alias_at_exponential_icdf, &__exponential_icdf, 0,
                             true);
  ...
  define_unary_function_ptr5(at_exponentiald_icdf, alias_at_exponentiald_icdf, &__exponentiald_icdf,
                             0, true);
  ...
  define_unary_function_ptr5(at_geometric_icdf, alias_at_geometric_icdf, &__geometric_icdf, 0,
                             true);

  gen _randgeometric(const gen & g, GIAC_CONTEXT) {
    if (g.type == _STRNG && g.subtype == -1)
      return g;
    return _ceil(gen(std::log(1 - giac_rand(contextptr) / (rand_max2 + 1.0))) /
                     ln(1 - g, contextptr),
                 contextptr);
  }
  ...
  */
  ...
  gen _kolmogorovt(const gen & g, GIAC_CONTEXT) {
    ...
    if (nd) {
      ...
      if (is_discrete_distribution(nd)) {
        ...
        return makevecteur(string2gen("D=", false), d, string2gen("K=", false), ks,
                           string2gen("1-kolmogorovd(K)=", false), 1 - kolmogorovd(ks));
      }
      for (int i = 0; i < n; ++i) {
        fback = double(X[i]);
        gen tmp = evalf_double(f, 1, contextptr);
  ...
  gen _wilcoxonp(const gen & args, GIAC_CONTEXT) {
    ...
    if (n.type == _VECT && n._VECTptr->size() == 2) {
      ...
      if (!is_integral(M) || M.type != _INT_ || M.val < 1 || !is_integral(N) || N.type != _INT_ ||
          N.val < 1 || M.val + N.val > 400)
        return gendimerr(contextptr);
  ...
  gen _wilcoxont(const gen & g, GIAC_CONTEXT) {
    ...
    if (m.type == _VECT) {
      ...
      *logptr(contextptr) << "\nranksum " << w << ", shifted ranksum " << w - M * (M + 1.) / 2
      ...
      if (p.type != _VECT || p._VECTptr->size() < double(M) * N)
        return gensizeerr(contextptr);
      ...
      if (typetest == 1)
        P = 1 - P;
      *logptr(contextptr) << "P-value " << P << " (" << evalf_double(P, 1, contextptr)
                          << "), alpha=" << alpha;
    ...
    *logptr(contextptr) << m << "\n";
    ...
    if (typetest == 0)
      total = 2 * total;
    *logptr(contextptr) << gettext("Wilcoxon statistic: ") << w << gettext(", p-value: ") << total
                        << gettext(", confidence level: ") << alpha << "\n";
  ...
  gen _chisquaret(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.subtype != _SEQ__VECT && is_integer_vecteur(v)) {
      ...
      if (X.size() >= 50 && int(X.size()) > 5 * (M - m)) {
        *logptr(contextptr) << gettext("Guessing data is a list of values, adequation to uniform "
                                       "discret distribution between ")
                            << m << gettext(" and ") << M << "\n";
        return _chisquaret(makesequence(g, vecteur(M - m + 1, 1. / (M - m))), contextptr);
      }
      *logptr(contextptr) << gettext("Guessing data is the list of number of elements in each "
                                     "class, adequation to uniform distribution")
                          << "\n";
    ...
    if (!yproba && y.type == _VECT) {
      ...
      if (is_integer_vecteur(*x._VECTptr)) {
        ...
        *logptr(contextptr) << s << gettext(" samples Chi2 test result ") << res
                            << gettext(",\nreject adequation if superior to chisquare_icdf(")
                            << k - 1 << ",0.95)=" << chisquare_icdf(k - 1, 0.95, contextptr)
                            << " or chisquare_icdf(" << k - 1 << ",1-alpha) if alpha!=5%" << "\n";
        return res;
      }
    }
    if (yproba && x.type == _VECT && !x._VECTptr->empty() && is_integer_vecteur(*x._VECTptr)) {
      ...
      if (N == J) { // X is directly the effectifs
        ...
        for (int i = 0; i < N; ++i) {
          if (X[i] < shift || X[i] >= J + shift)
            return gensizeerr(gettext("Data should be a class number between 0 and ") +
                              print_INT_(J));
          ++eff[X[i]];
        }
      ...
      *logptr(contextptr) << gettext("Sample adequation to a finite discrete probability "
                                     "distribution\nChi2 test result ")
                          << res << gettext(",\nreject adequation if superior to chisquare_icdf(")
                          << J - 1 << ",0.95)=" << chisquare_icdf(J - 1, 0.95, contextptr)
                          << " or chisquare_icdf(" << J - 1 << ",1-alpha) if alpha!=5%" << "\n";
      return res;
    }
    int nd;
    if ((nd = is_distribution(y))) {
      ...
      if (is_integer_vecteur(*xorig._VECTptr)) {
        ...
        if (X.size() >= 50 && int(X.size()) >= 5 * Xclasses) {
          ...
          moyenne = m1 / efftotal;
          ecart = std::sqrt(efftotal / (efftotal - 1)) *
                  sqrt(m2 / efftotal - moyenne * moyenne, contextptr);
        }
      }
      if (is_undef(moyenne)) {
        x = evalf_double(x, 1, contextptr);
      ...
      if (s == nargs - 2) { // 2 params estimation
        ...
        case 1:
          w.push_back(moyenne);
          w.push_back(ecart);
          *logptr(contextptr) << gettext("Normal density, estimating mean and stddev from data ")
                              << moyenne << " " << ecart << "\n";
          break;
        case 2: {
        ...
        case 3: {
          ...
          *logptr(contextptr) << gettext("Negative binomial: estimating n and p from data ") << n
                              << " " << p << "\n";
          w.push_back(n);
          w.push_back(p);
          break;
        ...
        case 13: { // moyenne +/- sqrt(3)*ecart
          ...
          *logptr(contextptr) << gettext(
                                     "Uniform density, estimating interval boundaries from data ")
                              << a << " " << b << "\n";
          break;
        }
        default:
      ...
      if (s == nargs - 1) { // 1 params estimation
        ...
        case 1: { // normald with 1 parameter known, w[0] default is stddev
          ...
          if (w[0].is_symb_of_sommet(at_equal)) {
            if (w[0][1] == at_mean) {
              *logptr(contextptr) << gettext("Normal density, estimating std deviation from data ")
                                  << ecart << "\n";
              w.push_back(ecart);
            }
            w[0] = w[0][2];
        ...
        case 4:
          w.push_back(moyenne);
          *logptr(contextptr) << gettext(
                                     "Poisson distribution, estimating parameter from data mean ")
                              << moyenne << "\n";
          break;
        default:
          return gensizeerr(contextptr);
      ...
      if (is_integer_vecteur(*xorig._VECTptr)) {
        if (classdef) {
          // not yet supported...
          *logptr(contextptr)
              << "Warning, using default class minimum=0 and class_size=1 for discrete distribution"
              << "\n";
        }
        dof = Xclasses - 1 - dof;
        gen res = 0;
        ...
        *logptr(contextptr) << gettext("Sample adequation to ") << loi
                            << gettext(", Chi2 test result ") << res
                            << gettext(",\nreject adequation if superior to chisquare_icdf(") << dof
                            << ",0.95)=" << chisquare_icdf(dof, 0.95, contextptr)
                            << gettext(" or chisquare_icdf(") << dof
                            << gettext(",1-alpha) if alpha!=5%") << "\n";
        return res;
      ...
      dof = int(m.size()) - dof;
      if (dof < 2)
        return gensizeerr(gettext("Not enough degree of freedom with default values. Add "
                                  "classes,classmin,classsize parameters"));
      efftotal = 0.0;
      ...
      for (unsigned i = 0; i < m.size(); ++i) {
        ...
        gen tmp0 = evalf_double(tmp._SYMBptr->feuille[0], 1, contextptr),
            tmp1 = evalf_double(tmp._SYMBptr->feuille[1], 1, contextptr), tmp2 = m[i][1];
      ...
      *logptr(contextptr) << gettext("Sample adequation to ") << loi
                          ...
                          << gettext(",\nreject adequation if superior to chisquare_icdf(") << dof
                          << ",0.95)=" << chisquare_icdf(dof, 0.95, contextptr)
                          << " or chisquare_icdf(" << dof << ",1-alpha) if alpha!=5%" << "\n";
      return res;
    }
    return undef;
  }
  ...
  gen zttest(const gen & g, bool ztest, GIAC_CONTEXT) {
    ...
    if ((n0 = double(v0.size())) == 2 && is_integer(v0[1])) {
      ...
      dof += int(n0);
      proportion = ztest && is_integer(v0[0]) && MU1.type == _DOUBLE_ && MU1._DOUBLE_val > 0 &&
                   MU1._DOUBLE_val < 1 && v[2].type == _FUNC;
      if (proportion)
    ...
    gen Falpha;
    if (test == 0)
      Falpha = ztest ? _normal_icdf(makesequence(0, 1, 1 - alpha / 2), contextptr)
                     : _student_icdf(makesequence(dof, 1 - alpha), contextptr);
    else
      Falpha = ztest ? _normal_icdf(makesequence(0, 1, 1 - alpha), contextptr)
                     : _student_icdf(makesequence(dof, 1 - alpha), contextptr);
    ...
    *logptr(contextptr) << gettext("*** TEST RESULT ") << (ok ? "1 ***" : "0 ***") << "\n"
                        << "Summary " << (ztest ? "Z-Test" : "T-Test") << " null hypothesis H0 "
                        << (proportion ? "p1=p2" : "mu1=mu2") << ", alt. hyp. H1 mu1"
                        << (test == -1 ? "<" : (test == 0 ? "!=" : ">")) << "mu2." << "\n";
    *logptr(contextptr) << gettext("Test returns 0 if probability to observe data is less than ")
                        << alpha << "\n";
    *logptr(contextptr)
        << gettext("(null hyp. mu1=mu2 rejected with less than alpha probability error)") << "\n";
    *logptr(contextptr) << gettext("Test returns 1 otherwise (can not reject null hypothesis)")
                        << "\n";
    *logptr(contextptr) << gettext("Data mean mu1=") << mu0 << gettext(", population mean mu2=")
                        << mu1;
    if (!ztest)
      *logptr(contextptr) << gettext(", degrees of freedom ") << dof;
    *logptr(contextptr) << "\n"
                        << "alpha level " << alpha
                        << ", multiplier*stddev/sqrt(sample size)= " << falpha << "*" << sigma
                        << "/" << sqrtn0 << "\n";
    return (ok ? 1 : 0);
  }
  ...
  gen distribution(int nd) {
    ...
    if (!d_static)
      d_static =
          new vecteur(makevecteur(at_normald, at_binomial, at_negbinomial, at_poisson, at_studentd,
                                  at_fisherd, at_cauchyd, at_weibulld, at_betad, at_gammad,
                                  at_chisquared, at_geometric, at_uniformd, at_exponentiald));
  ...
  gen _mgf(const gen & g, GIAC_CONTEXT) {
    ...
    if (nd == 1)
      return exp(v[1] * t + plus_one_half * pow(v[2], 2, contextptr) * pow(t, 2, contextptr),
                 contextptr);
    if (nd == 2)
  ...
  gen icdf(int n) {
    ...
    if (!icdf_static)
      icdf_static = new vecteur(makevecteur(
          at_normald_icdf, at_binomial_icdf, undef, at_poisson_icdf, at_studentd_icdf,
          at_fisherd_icdf, at_cauchyd_icdf, at_weibulld_icdf, at_betad_icdf, at_gammad_icdf,
          at_chisquared_icdf, at_geometric_icdf, at_uniformd_icdf, at_exponentiald_icdf));
  ...
  gen cdf(int n) {
    ...
    if (!cdf_static)
      cdf_static = new vecteur(
          makevecteur(at_normald_cdf, at_binomial_cdf, undef, at_poisson_cdf, at_studentd_cdf,
                      at_fisherd_cdf, at_cauchyd_cdf, at_weibulld_cdf, at_betad_cdf, at_gammad_cdf,
                      at_chisquared_cdf, at_geometric_cdf, at_uniformd_cdf, at_exponentiald_cdf));
  ...
  gen _cdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (!(nd = is_distribution(w.front()))) {
      ...
      if (s == 2 && ckmatrix(w0)) { // list of classes/effectifs
        ...
        for (int i = 0; i < s; ++i) {
          gen mi0 = m[i][0];
          if (mi0.is_symb_of_sommet(at_interval) && mi0._SYMBptr->feuille.type == _VECT &&
              mi0._SYMBptr->feuille._VECTptr->size() == 2)
            mi0 =
                (mi0._SYMBptr->feuille._VECTptr->front() + mi0._SYMBptr->feuille._VECTptr->back()) /
                2;
  ...
  gen _icdf(const gen & g, GIAC_CONTEXT) {
    ...
    if (plot) {
      ...
      return _symetrie(
          makesequence(_droite(makesequence(0, 1 + cst_i), contextptr), _cdf(g, contextptr)),
          contextptr);
    }
    return res;
  }
...
#include "input_parser.h"
  ...
  complex<double> LambertW(complex<double> z, int n) {
    ...
    if (n == 0 && std::abs(z - .5) <= .5)
      w = (0.35173371 * (0.1237166 + 7.061302897 * z)) /
          (2. + 0.827184 * (1. + 2. * z)); // (1,1) Pade approximant for W(z,0)
    if (n == -1 && std::abs(z - .5) < .5)
      w = -((complex<double>(2.2591588985, 4.22096) *
             (complex<double>(-14.073271, -33.767687754) * z -
              complex<double>(12.7127, -19.071643) * (1. + 2. * z))) /
            (2. - complex<double>(17.23103, -10.629721) * (1. + 2. * z))); // (1,1) Pade
