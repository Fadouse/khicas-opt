  ...
  static gen derive_SYMB(const gen & g_orig, const identificateur & i, GIAC_CONTEXT) {
    ...
    if (s.sommet == at_prod) {
      ...
      if (s.feuille.type == _VECT && s.feuille._VECTptr->size() == 2 &&
        ...
        return (derive(u, i, contextptr) * v - u * derive(v, i, contextptr)) /
               pow(v, 2, contextptr);
      }
      if (do_step)
      ...
      if (do_step)
        gprintf(gettext("Hence derivative of %gen by product rule is %gen"),
                makevecteur(g_orig, res), contextptr);
      return res;
    }
    if (s.sommet == at_neg)
      return -derive(s.feuille, i, contextptr);
    if (s.sommet == at_pow) {
      ...
      if (step_infolevel(contextptr) > 1) {
        if (is_constant_wrt(exponent, i, contextptr))
          gprintf(
              gettext("Derivative of a power: (%gen)'=(%gen)*(%gen)'*%gen"),
              makevecteur(symb_pow(base, exponent), exponent, base, symb_pow(base, exponent - 1)),
              contextptr);
        else
          gprintf(
              gettext("Derivative of a power: (%gen)'=%gen*(%gen)'*ln(%gen)+(%gen)*(%gen)'*%gen"),
    ...
    if (range.size() >= 1 && range.front().type == _VECT) {
      ...
      if (range.size() == 2 && range[0] == minus_inf && range[1] == plus_inf) {
        ...
        if (is_periodic(f, x, periode, contextptr)) {
          gen hyp = symb_and(symbolic(at_superieur_egal, makesequence(x, 0)),
                             symbolic(at_inferieur_egal, makesequence(x, periode)));
