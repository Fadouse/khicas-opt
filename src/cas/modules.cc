...
void displaylogo() {
  ...
  while (1) {
    ...
    unsigned int key;
    ck_getkey((int *)&key);
    if (key == KEY_CTRL_EXIT || key == KEY_CTRL_AC || key == KEY_CTRL_MENU || key == KEY_CTRL_EXE ||
        key == KEY_CTRL_VARS)
      break;
    if (key == KEY_CTRL_UP) {
      t.turtley += 10;
...
void fl_pie(int x, int y, int rx, int ry, int theta1_deg, int theta2_deg, int c = _BLACK,
            bool segment = false) {
...
#ifdef FX
void text_print(int fontsize, const char * s, int x, int y, int c = _BLACK, int bg = _WHITE,
                int mode = 0) {
  ...
  if (fontsize >= 6 && ss == 2 && s[0] == char(0xe5) &&
      (s[1] == char(0xea) ||
    ...
    if (l > 0) {
      ...
      if (t.visible) {
        ...
        fl_line(deltax + x + Dy, deltay + LCD_HEIGHT_PX - (y - Dx), deltax + x + 3 * Dx,
                deltay + LCD_HEIGHT_PX - (y + 3 * Dy), 0); // always display turtle
        fl_line(deltax + x - Dy, deltay + LCD_HEIGHT_PX - (y + Dx), deltax + x + 3 * Dx,
                deltay + LCD_HEIGHT_PX - (y + 3 * Dy), 0);
      }
    }
    return;
  } // End logo mode
...
void Turtle::draw() {
  ...
  if (turtleptr &&
    ...
    text_print(18, buf, deltax + horizontal_pixels, deltay + (2 * COORD_SIZE) / 3 - 2, COLOR_BLACK,
               COLOR_YELLOW);
    sprintf(buf, "y %i   ", int(turtle.y + .5));
    text_print(18, buf, deltax + horizontal_pixels, deltay + (4 * COORD_SIZE) / 3 - 3, COLOR_BLACK,
               COLOR_YELLOW);
