...
// Real nonnegative arguments only; scaling prevents intermediate overflow.
  ...
  static bool elliptic_first_rf(double x, double y, double z, double & value) {
    ...
    if (z > scale)
      scale = z;
    if (!(x >= 0 && y >= 0 && z >= 0 && scale > 0 && scale <= 1.7976931348623157e308))
      return false;
    ...
    for (unsigned n = 0; n < 64; ++n) {
      ...
      if (std::abs(dx) < 0.002 && std::abs(dy) < 0.002 && std::abs(dz) < 0.002) {
        ...
        double p = 1 - e2 / 10 + e3 / 14 + e2 * e2 / 24 - 3 * e2 * e3 / 44 -
                   5 * e2 * e2 * e2 / 208 + 3 * e3 * e3 / 104 + e2 * e2 * e3 / 16;
        value = p / std::sqrt(mean) / std::sqrt(scale);
        return true;
      }
  ...
  gen _EllipticF(const gen & argument, GIAC_CONTEXT) {
    if (argument.type == _STRNG && argument.subtype == -1)
      return argument;
    if (argument.type != _VECT || argument._VECTptr->size() != 2)
      return gensizeerr("EllipticF(phi,m): parameter m, radians");
    if (!angle_radian(contextptr))
      return gensizeerr("EllipticF requires radians");
    const gen &phi = argument[0], &m = argument[1];
    if (is_undef(phi) || is_undef(m))
      return undef;
    if (is_zero(phi))
      return 0;
    if (is_zero(m))
    ...
    if (taille(argument, 65) <= 64 && has_num_coeff(argument)) {
      ...
      if (p.type == _DOUBLE_ && q.type == _DOUBLE_ && std::abs(p._DOUBLE_val) <= 1000000 &&
          q._DOUBLE_val > 0) {
    ...
    if (m == -1 && (phi == cst_pi / 2 || phi == -cst_pi / 2)) {
      gen k = sqrt(cst_pi, contextptr) * Gamma(gen(1) / 4, contextptr) /
              (4 * Gamma(gen(3) / 4, contextptr));
