...
#undef HAVE_GMPXX_H
  ...
  struct ref_mpz_t {
    ...
    mpz_t z;
    ref_mpz_t() : ref_count(1) {
      mpz_init(z);
    }
    ref_mpz_t(size_t nbits) : ref_count(1) {
      mpz_init2(z, int(nbits));
    }
    ref_mpz_t(const mpz_t & Z) : ref_count(1) {
      mpz_init_set(z, Z);
    }
    ~ref_mpz_t() {
      mpz_clear(z);
...
#else
typedef std::map<gen, gen, const std::pointer_to_binary_function<const gen &, const gen &, bool>>
    gen_map;
#endif
  struct ref_gen_map;
...
class unary_function_abstract;
  ...
  gen(double d);
    ...
    gen(const char * s, GIAC_CONTEXT) {
      type = 0;
      *this = gen(std::string(s), contextptr);
    };
    ...
    inline const char * printcharptr(GIAC_CONTEXT) const {
      return print(contextptr).c_str();
    };
    ...
    inline const char * printcharptr() const {
      return print().c_str();
    };
...
#else
  ref_gen_map(const std::pointer_to_binary_function<const gen &, const gen &, bool> & p)
      : ref_count(1), m(p) {}
#endif
    ref_gen_map(const gen_map & M) : ref_count(1), m(M) {}
  };

  struct alias_ref_fraction {
...
#ifdef SMARTPTR64
#define define_alias_gen(name, type, subtype, ptr)                                                 \
  alias_gen name = {(ulonglong(ptr) << 16) | (subtype << 8) | type};
#define define_alias_ref_symbolic(name, sommet, type, subtype, ptr)                                \
  alias_ref_symbolic name = {-1, (unary_function_eval *)sommet,                                    \
                             (ulonglong(ptr) << 16) | (subtype << 8) | type};
#define define_alias_ref_fraction(name, numtype, numsubtype, numptr, dentype, densubtype, denptr)  \
  alias_ref_fraction name = {-1,                                                                   \
                             {(ulonglong(numptr) << 16) | (numsubtype << 8) | numtype},            \
                             {(ulonglong(denptr) << 16) | (densubtype << 8) | dentype}};
#define define_alias_ref_complex(name, retype, resubtype, reptr, imtype, imsubtype, imptr)         \
  alias_ref_complex name = {-1,                                                                    \
                            0,                                                                     \
                            {(ulonglong(reptr) << 16) | (resubtype << 8) | retype},                \
                            {(ulonglong(imptr) << 16) | (imsubtype << 8) | imtype}};
#define define_tab2_alias_gen(name, retype, resubtype, reptr, imtype, imsubtype, imptr)            \
  alias_gen name[] = {{(ulonglong(reptr) << 16) | (resubtype << 8) | retype},                      \
                      {(ulonglong(imptr) << 16) | (imsubtype << 8) | imtype}};
#else // SMARTPTR64
#ifdef DOUBLEVAL
#define define_alias_gen(name, type, subtype, ptr)                                                 \
  alias_gen name = {type, subtype, 0, ulonglong(ptr)};
#define define_alias_ref_symbolic(name, sommet, type, subtype, ptr)                                \
  alias_ref_symbolic name = {-1, (unary_function_eval *)sommet, type, subtype, 0, ulonglong(ptr)};
#define define_alias_ref_fraction(name, numtype, numsubtype, numptr, dentype, densubtype, denptr)  \
  alias_ref_fraction name = {-1,                                                                   \
                             {numtype, numsubtype, 0, ulonglong(numptr)},                          \
                             {dentype, densubtype, 0, ulonglong(denptr)}};
#define define_alias_ref_complex(name, retype, resubtype, reptr, imtype, imsubtype, imptr)         \
  alias_ref_complex name = {                                                                       \
      -1, 0, {retype, resubtype, 0, ulonglong(reptr)}, {imtype, imsubtype, 0, ulonglong(imptr)}};
#define define_tab2_alias_gen(name, retype, resubtype, reptr, imtype, imsubtype, imptr)            \
  alias_gen name[] = {{retype, resubtype, 0, ulonglong(reptr)},                                    \
                      {imtype, imsubtype, 0, ulonglong(imptr)}};
#else // DOUBLEVAL
#define define_alias_gen(name, type, subtype, ptr)                                                 \
  alias_gen name = {long(ptr), 0, subtype, type}; //{type,subtype,0,long(ptr)};
#define define_alias_ref_symbolic(name, sommet, type, subtype, ptr)                                \
  alias_ref_symbolic name = {-1,  (unary_function_eval *)sommet, long(ptr), 0, subtype,            \
                             type}; //{-1,(unary_function_eval *)sommet,type,subtype,0,long(ptr)};
#define define_alias_ref_fraction(name, numtype, numsubtype, numptr, dentype, densubtype, denptr)  \
  alias_ref_fraction name = {                                                                      \
      -1,                                                                                          \
      {long(numptr), 0, numsubtype, numtype},                                                      \
      {long(denptr), 0, densubtype,                                                                \
       dentype}}; //{-1,{numtype,numsubtype,0,long(numptr)},{dentype,densubtype,0,long(denptr)}};
#define define_alias_ref_complex(name, retype, resubtype, reptr, imtype, imsubtype, imptr)         \
  alias_ref_complex name = {                                                                       \
      -1,                                                                                          \
      0,                                                                                           \
      {long(reptr), 0, resubtype, retype},                                                         \
      {long(imptr), 0, imsubtype,                                                                  \
       imtype}}; //{-1,0,{retype,resubtype,0,long(reptr)},{imtype,imsubtype,0,long(imptr)}};
#define define_tab2_alias_gen(name, retype, resubtype, reptr, imtype, imsubtype, imptr)            \
  alias_gen name[] = {                                                                             \
      {long(reptr), 0, resubtype, retype},                                                         \
      {long(imptr), 0, imsubtype,                                                                  \
       imtype}}; //{{retype,resubtype,0,long(reptr)},{imtype,imsubtype,0,long(imptr)}};
#endif           // DOUBLEVAL
#endif           // SMARTPTR64

  // ? #ifdef __GNUC__
#ifdef IMMEDIATE_VECTOR
  struct alias_ref_vecteur {
    ref_count_t ref_count;
    const int _taille;
    const alias_gen * end_immediate_vect;
    const alias_gen * begin_immediate_vect;
    void * ptr;
  };
...
struct alias_ref_vecteur {
  ref_count_t ref_count;
  void * ptr;
  const alias_gen * begin;
  unsigned size;
  unsigned capacity;
};
#define define_alias_ref_vecteur(name, b)                                                          \
  alias_ref_vecteur name = {-1, 0, (const alias_gen *)b, sizeof(gen), sizeof(gen)};
//#define define_alias_ref_vecteur2(name,b) alias_ref_vecteur name={-1,&b[0],&b[2],&b[2],0};
#define define_alias_ref_vecteur2(name, b)                                                         \
  alias_ref_vecteur name = {-1, 0, &b[0], 2 * sizeof(gen), 2 * sizeof(gen)};
#endif
  ...
  struct ref_complex {
    ...
    int display;
    gen re, im;
    ref_complex(const std::complex<double> & c)
        : ref_count(1), display(0), re(real(c)), im(imag(c)) {}
    ref_complex(const gen & R, const gen & I) : ref_count(1), display(0), re(R), im(I) {}
    ref_complex(const gen & R, const gen & I, int display_mode)
        : ref_count(1), display(display_mode), re(R), im(I) {}
    ~ref_complex() {
      re = im = 0;
    }
  };
  ...
  inline bool is_exactly_one(const gen & a) {
    return is_one(a);
  }
  ...
  gen operator*(const gen & a, const gen & b);
  inline gen operator*(int a, const gen & b) {
    return gen(a) * b;
  }
  inline gen operator*(double a, const gen & b) {
    return gen(a) * b;
  }
  ...
  inline wchar_t * wprint(const gen & g, GIAC_CONTEXT) {
    return g.wprint(contextptr);
  }
  ...
  */
  ...
  bool operator!=(const gen & a, const gen & b);
  inline bool operator!=(const gen & a, const identificateur & b) {
    return !(a == b);
  }
  ...
  inline gen eval(const gen & e, int level, const context * contextptr) {
    return e.eval(level, contextptr);
  };
  inline gen eval(const gen & e, const context * contextptr) {
    return e.eval(eval_level(contextptr), contextptr);
  };
  ...
  inline gen evalf_double(const gen & e, int level, const context * contextptr) {
    return e.evalf_double(level, contextptr);
  };
  ...
  inline std::string print(const gen & e, context * contextptr) {
    return e.print(contextptr);
  }
  inline bool is_real(const gen & g, GIAC_CONTEXT) {
    return g.is_real(contextptr);
  }
  inline bool is_cinteger(const gen & g) {
    return g.is_cinteger();
  };
  inline bool is_integer(const gen & g) {
    return g.is_integer();
  };
  double int2double(int i);
  inline bool is_constant(const gen & g) {
    return g.is_constant();
  };
  inline bool is_approx(const gen & g) {
    return g.is_approx();
  };
  ...
  public:
    virtual gen_user * memory_alloc() const {
      gen_user * ptr = new gen_user(*this);
      return ptr;
    }
    virtual ~gen_user() {};
    // redefine operations if it makes sense.
    // You can redefine gen_user + gen_user for speed
    virtual gen operator+(const gen &) const {
      return gensizeerr(gettext("+ not redefined"));
    }
    virtual gen operator+(const gen_user & a) const {
      return (*this) + gen(a);
    }
    virtual gen operator-(const gen &) const {
      return gensizeerr(gettext("Binary - not redefined"));
    }
    virtual gen operator-(const gen_user & a) const {
      return (*this) - gen(a);
    }
    virtual gen operator-() const {
      return gensizeerr(gettext("Unary - not redefined"));
    }
    virtual gen operator*(const gen &) const {
      return gensizeerr(gettext("Binary * not redefined"));
    }
    ...
    virtual gen operator/(const gen & a) const {
      return gensizeerr(gettext("Binary / not redefined"));
    }
    virtual bool is_zero() const {
#ifndef NO_STDEXCEPT
    ...
    virtual gen inv() const {
      return gensizeerr(gettext("Inv not redefined"));
    }
    virtual gen conj(GIAC_CONTEXT) const {
      return gensizeerr(gettext("Conj not redefined"));
    }
    virtual gen re(GIAC_CONTEXT) const {
      return gensizeerr(gettext("Real part not redefined"));
    }
    virtual gen im(GIAC_CONTEXT) const {
      return gensizeerr(gettext("Imaginary part not redefined"));
    }
    virtual gen abs(GIAC_CONTEXT) const {
      return gensizeerr(gettext("Abs not redefined"));
    }
    virtual gen arg(GIAC_CONTEXT) const {
      return gensizeerr(gettext("Arg not redefined"));
    }
    virtual gen sqrt(GIAC_CONTEXT) const {
      return gensizeerr(gettext("Sqrt not redefined"));
    }
    ...
    virtual bool operator==(const gen_user & a) const {
      return (*this) == gen(a);
    }
    ...
    virtual gen operator>(const gen_user & a) const {
      return superieur_strict(*this, gen(a), 0);
    }
    virtual gen operator<=(const gen &) const {
      return gensizeerr(gettext("<= not redefined"));
    }
    virtual gen operator<=(const gen_user & a) const {
      return inferieur_egal(*this, gen(a), 0);
    }
    ...
    virtual gen gcd(const gen &) const {
      return gensizeerr(gettext("gcd not redefined"));
    }
    virtual gen gcd(const gen_user & a) const {
      return gcd(gen(a));
    }
    virtual std::string print(GIAC_CONTEXT) const {
    ...
    virtual std::string texprint(GIAC_CONTEXT) const {
      return "Nothing_to_print_tex";
    }
    virtual gen eval(int level, const context * contextptr) const {
      return *this;
    };
    virtual gen evalf(int level, const context * contextptr) const {
      return *this;
    };
    virtual gen makegen(int i) const {
      return string2gen("makegen not redefined");
    };
    virtual gen rand(GIAC_CONTEXT) const {
      return string2gen("rand not redefined");
    };
  };
  struct ref_gen_user {
    volatile ref_count_t ref_count;
...
inline stdostream & operator<<(stdostream & os, const gen & a) {
  return os << a.print(context0);
}
#endif
...
#ifdef NSPIRE
  template <class T> nio::ios_base<T> & operator<<(nio::ios_base<T> & os, const monome & m) {
    return os << m.print();
  }
#else
std::ostream & operator<<(std::ostream & os, const monome & m);
#endif
  inline bool operator==(const monome & a, const monome & b) {
  ...
  std::string calc_endlines_positions(const vecteur & history_in, const vecteur & history_out,
                                      int nchar, std::vector<int> & endlines,
  ...
  struct eqwdata {
    ...
    bool modifiable;
    int baseline;
    eqwdata(int dxx, int dyy, int xx, int yy, const attributs & a, const gen & gg)
        : g(gg), eqw_attributs(a), x(xx), y(yy), dx(dxx), dy(dyy), selected(false), active(false),
          hasbaseline(false), modifiable(true), baseline(0) {};
    eqwdata(int dxx, int dyy, int xx, int yy, const attributs & a, const gen & gg, int mybaseline)
        : g(gg), eqw_attributs(a), x(xx), y(yy), dx(dxx), dy(dyy), selected(false), active(false),
          hasbaseline(true), modifiable(true), baseline(mybaseline) {};
    const char * dbgprint() {
#if !defined(NSPIRE) && !defined(FXCG)
      CERR << g << ":" << dx << "," << dy << "+" << x << "," << y << "," << baseline << ","
           << eqw_attributs.fontsize << "," << eqw_attributs.background << ","
           << eqw_attributs.text_color << std::endl;
#endif
      return "Done";
    }
  };
  ...
  public:
    ...
    explicit identificateur(
        const char * s,
        bool
            StringIsNowYours); // creates identifier. if StringIsNowYours, then the string will be freed when id is destroyed...
#endif
    ...
    bool in_eval(
        int level, const gen & orig, gen & evaled, const context * context_ptr,
        bool No38Lookup = false); // if No38Lookup, does not check if HP38 knows about this name...
    ...
    inline bool operator!=(const identificateur & i) {
      return !(*this == i);
    }
    inline bool operator!=(const gen & i) {
      return !(*this == i);
    }
    void
    MakeCopyOfNameIfNotLocal(); ///< if the name is not dynamicaly allocated, create a copy for that id.
  };
  ...
  struct symbolic {
    ...
    symbolic(const unary_function_ptr & o, const gen & e1, const gen & e2)
        : sommet(o), feuille(makevecteur(e1, e2)) {};
    symbolic(const unary_function_ptr & o, const gen & e1, const gen & e2, const gen & e3)
        : sommet(o), feuille(makevecteur(e1, e2, e3)) {};
    symbolic(const unary_function_ptr & o, const gen & e1, const gen & e2, const gen & e3,
             const gen & e4)
        : sommet(o), feuille(makevecteur(e1, e2, e3, e4)) {};
    symbolic(const unary_function_ptr * o, const gen & e) : sommet(*o), feuille(e) {};
    symbolic(const unary_function_ptr * o, const gen & e1, const gen & e2)
        : sommet(*o), feuille(makevecteur(e1, e2)) {};
    symbolic(const unary_function_ptr * o, const gen & e1, const gen & e2, const gen & e3)
...
#ifdef SMARTPTR64
  inline mpz_t * gen::ref_ZINTptr() const {
    return &((ref_mpz_t *)(*(ulonglong *)this >> 16))->z;
  }
  inline real_object * gen::ref_REALptr() const {
    return &((ref_real_object *)(*(ulonglong *)this >> 16))->r;
  }
  inline gen * gen::ref_CPLXptr() const {
    return &((ref_complex *)(*(ulonglong *)this >> 16))->re;
  }
  inline gen * gen::ref_MODptr() const {
    return &((ref_modulo *)(*(ulonglong *)this >> 16))->n;
  }
  inline gen * gen::ref_EXTptr() const {
    return &((ref_algext *)(*(ulonglong *)this >> 16))->P;
  }
  inline vecteur * gen::ref_VECTptr() const {
    return &((ref_vecteur *)(*(ulonglong *)this >> 16))->v;
  }
  inline sparse_poly1 * gen::ref_SPOL1ptr() const {
    return &((ref_sparse_poly1 *)(*(ulonglong *)this >> 16))->s;
  }
  inline std::string * gen::ref_STRNGptr() const {
    return &((ref_string *)(*(ulonglong *)this >> 16))->s;
  }
  inline gen_user * gen::ref_USERptr() const {
    return ((ref_gen_user *)(*(ulonglong *)this >> 16))->u;
  }
  inline gen_map * gen::ref_MAPptr() const {
    return &((ref_gen_map *)(*(ulonglong *)this >> 16))->m;
  }
  inline eqwdata * gen::ref_EQWptr() const {
    return &((ref_eqwdata *)(*(ulonglong *)this >> 16))->e;
  }
  inline grob * gen::ref_GROBptr() const {
    return &((ref_grob *)(*(ulonglong *)this >> 16))->g;
  }
  inline void * gen::ref_POINTER_val() const {
    return ((ref_void_pointer *)(*(ulonglong *)this >> 16))->p;
  }
  inline Tfraction<gen> * gen::ref_FRACptr() const {
    return &((ref_fraction *)(*(ulonglong *)this >> 16))->f;
  }
  inline polynome * gen::ref_POLYptr() const {
    return &((ref_polynome *)(*(ulonglong *)this >> 16))->t;
  }
  inline identificateur * gen::ref_IDNTptr() const {
    return &((ref_identificateur *)(*(ulonglong *)this >> 16))->i;
  }
  inline symbolic * gen::ref_SYMBptr() const {
    return &((ref_symbolic *)(*(ulonglong *)this >> 16))->s;
  }
  inline unary_function_ptr * gen::ref_FUNCptr() const {
    return &((ref_unary_function_ptr *)(*(ulonglong *)this >> 16))->u;
  }
#else  // SMARTPTR64
inline mpz_t * gen::ref_ZINTptr() const {
  return &__ZINTptr->z;
...
inline unary_function_ptr * gen::ref_FUNCptr() const {
  return (unary_function_ptr *)&_FUNC_;
}
#endif // SMARTPTR64
...
#define ALLOCA(type, var, size)                                                                    \
  Calloca alloca##var(size);                                                                       \
  type * var = (type *)(alloca##var.ram)
#else
