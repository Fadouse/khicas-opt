...
namespace giac {
  ...
  static define_unary_function_eval2_index(83, __rpn_prog, &return_undef, _rpn_prog_s,
                                           &printasconstant);
...
static const char _RPN_LOCAL_s[] = "RPN_LOCAL";
static define_unary_function_eval2_index(130, __RPN_LOCAL, &_RPN_LOCAL, _RPN_LOCAL_s,
                                         &printasRPN_LOCAL);
define_unary_function_ptr5(at_RPN_LOCAL, alias_at_RPN_LOCAL, &__RPN_LOCAL, 0, T_RPN_OP);
...
gen _RPN_FOR(const gen & args, const context * contextptr) {
  ...
  vecteur res;
  for (; !is_zero(test.eval(eval_level(newcontextptr), newcontextptr)
                      .evalf(eval_level(contextptr), newcontextptr));
       sto(eval(controle[0] + controle[1], eval_level(newcontextptr), newcontextptr), controle[0],
           newcontextptr)) {
    res = rpn_eval(prog, *args._VECTptr, newcontextptr);
  }
  leave(protect, names, newcontextptr);
...
static string printasRPN_WHILE(const gen & feuille, const char * sommetstr, GIAC_CONTEXT) {
  ...
  return "WHILE " +
         printinner_VECT(*feuille._VECTptr->front()._VECTptr, _RPN_FUNC__VECT, contextptr) +
         " REPEAT " +
         printinner_VECT(*feuille._VECTptr->back()._VECTptr, _RPN_FUNC__VECT, contextptr) + " END ";
}
...
static const char _RPN_WHILE_s[] = "RPN_WHILE";
static define_unary_function_eval2_index(134, __RPN_WHILE, &_RPN_WHILE, _RPN_WHILE_s,
                                         &printasRPN_WHILE);
define_unary_function_ptr5(at_RPN_WHILE, alias_at_RPN_WHILE, &__RPN_WHILE, 0, T_RPN_OP);
...
static const char _RPN_CASE_s[] = "RPN_CASE";
static define_unary_function_eval2_index(128, __RPN_CASE, &_RPN_CASE, _RPN_CASE_s,
                                         &printasRPN_CASE);
define_unary_function_ptr5(at_RPN_CASE, alias_at_RPN_CASE, &__RPN_CASE, 0, T_RPN_OP);
...
static string printasRPN_UNTIL(const gen & feuille, const char * sommetstr, GIAC_CONTEXT) {
  ...
  return "DO " + printinner_VECT(*feuille._VECTptr->front()._VECTptr, _RPN_FUNC__VECT, contextptr) +
         " UNTIL " +
         printinner_VECT(*feuille._VECTptr->back()._VECTptr, _RPN_FUNC__VECT, contextptr) + " END ";
}
...
gen _RPN_UNTIL(const gen & args, const context * contextptr) {
  ...
  for (;;) {
    ...
    args._VECTptr->pop_back();
    if (!is_zero(
            tmp.eval(eval_level(contextptr), contextptr).evalf(eval_level(contextptr), contextptr)))
      break;
  }
  return gen(res, _RPN_STACK__VECT);
}
  ...
  for (; it != itend; ++it) {
    if (it->type == _FUNC) {
      ...
      if (nargs > signed(pile.size()))
        return vecteur(
            1, gentoofewargs(it->print(contextptr) + ": stack " + gen(pile).print(contextptr)));
      if (nargs == 1) {
      ...
      const unary_function_ptr control_op[] = {*at_RPN_LOCAL, *at_RPN_FOR,   *at_IFTE, *at_RPN_CASE,
                                               *at_RPN_WHILE, *at_RPN_UNTIL, 0};
...
static int int_one(const struct dirent * unused) {
  ...
  gen _VARS(const gen & args, const context * contextptr) {
    ...
    if (contextptr) {
      if (contextptr->globalcontextptr && contextptr->globalcontextptr->tabptr) {
        sym_tab::const_iterator it = contextptr->globalcontextptr->tabptr->begin(),
        ...
        for (; it != itend; ++it) {
          ...
          if (keywordsptr == 0 || !equalposcomp(*keywordsptr, g)) {
            ...
            if (strngeq) {
              ...
              if ((t == _SYMB && it->second._SYMBptr->sommet != at_program) || t == _FRAC ||
                  t <= _REAL || t == _VECT)
  ...
  gen purgenoassume(const gen & args, const context * contextptr) {
    ...
    if (it->second.type == _POINTER_ && it->second.subtype == _THREAD_POINTER)
      return gentypeerr(args.print(contextptr) + " is locked by thread " +
                        it->second.print(contextptr));
#if 0
  ...
  static string printasdivision(const gen & feuille, const char * s, GIAC_CONTEXT) {
    ...
    if ((f.type == _SYMB && (f._SYMBptr->sommet == at_plus || f._SYMBptr->sommet == at_prod ||
                             f._SYMBptr->sommet == at_division || f._SYMBptr->sommet == at_inv ||
                             need_parenthesis(f._SYMBptr->sommet))) ||
        (f.type == _CPLX) || (f.type == _MOD) || (f.type == _FRAC)) {
      res += '(';
  ...
  static string texprintasdivision(const gen & feuille, const char * s, GIAC_CONTEXT) {
    if (feuille.type != _VECT || feuille._VECTptr->size() != 2)
      return "invalid /";
    return "\\frac{" + gen2tex(feuille._VECTptr->front(), contextptr) + "}{" +
           gen2tex(feuille._VECTptr->back(), contextptr) + "}";
  }
  ...
  static define_unary_function_eval4_index(10, __division, &_division, _division_s,
                                           &printasdivision, &texprintasdivision);
  ...
  static define_unary_function_eval4_index(6, __binary_minus, &_binary_minus, _binary_minus_s,
  ...
  static define_unary_function_eval4(__MOD, &_MODULO, _MOD_s, &printsommetasoperator,
  ...
  define_unary_function_ptr5(at_ADDROW, alias_at_ADDROW, &__ADDROW, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_ADDCOL, alias_at_ADDCOL, &__ADDCOL, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_SCALEADD, alias_at_SCALEADD, &__SCALEADD, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_SWAPCOL, alias_at_SWAPCOL, &__SWAPCOL, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_DELCOL, alias_at_DELCOL, &__DELCOL, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);

  static const char _DELROW_s[] = "DELROW";
  static define_unary_function_eval_quoted(__DELROW, &_delrows, _DELROW_s);
  define_unary_function_ptr5(at_DELROW, alias_at_DELROW, &__DELROW, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);

  static const char _SWAPROW_s[] = "SWAPROW";
  static define_unary_function_eval_quoted(__SWAPROW, &_rowSwap, _SWAPROW_s);
  define_unary_function_ptr5(at_SWAPROW, alias_at_SWAPROW, &__SWAPROW, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  gen _SUB(const gen & args, GIAC_CONTEXT) {
    ...
    if (ckmatrix(v[1]))
      return sto(_subMat(gen(makevecteur(v[1], v[2], v[3]), _SEQ__VECT), contextptr), v0,
                 contextptr);
    int shift =
        array_start(contextptr); //xcas_mode(contextptr)!=0 || abs_calc_mode(contextptr)==38;
  ...
  define_unary_function_ptr5(at_RANDMAT, alias_at_RANDMAT, &__RANDMAT, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_REPLACE, alias_at_REPLACE, &__REPLACE, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_MAKELIST, alias_at_MAKELIST, &__MAKELIST, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  gen _INT(const gen & g, GIAC_CONTEXT) {
    ...
    if (is_positive(g, contextptr))
      ...
      return sg *
             _floor(
                 g * sg,
                 contextptr); //symbolic(at_when,makesequence(symbolic(at_superieur_egal,makesequence(g,0)),symbolic(at_floor,g),symbolic(at_ceil,g)));
    }
  }
  ...
  gen _HPDIFF(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args.subtype == _SEQ__VECT && args._VECTptr->size() >= 2) {
      ...
      ++count;
      gen arg0 = eval(subst(args._VECTptr->front(), var, newx, false, contextptr),
  ...
  define_unary_function_ptr5(at_HPDIFF, alias_at_HPDIFF, &__HPDIFF, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_TAYLOR, alias_at_TAYLOR, &__TAYLOR, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_POLYCOEF, alias_at_POLYCOEF, &__POLYCOEF, _QUOTE_ARGUMENTS,
                             T_UNARY_OP);
  ...
  static string printasPOLYFORM(const gen & feuille, const char * sommetstr, GIAC_CONTEXT) {
    if (feuille.type == _VECT && feuille._VECTptr->size() == 2 &&
        feuille._VECTptr->back().type == _FUNC) {
      const char * s = feuille._VECTptr->back()._FUNCptr->ptr()->s;
      return feuille._VECTptr->front().print(contextptr) + "\xe2\x96\xba" +
             ((strcmp(s, "eval") == 0) ? "\xe2\x96\xba" : s);
    }
  ...
  gen _POLYFORM(const gen & args, GIAC_CONTEXT) {
    ...
    tmp = evalfunc(tmp, contextptr);
    return _recursive_normal(
        tmp,
        context0); // return symb_quote(_partfrac(args,contextptr)); // return symb_quote(recursive_normal(args,1,contextptr)); // symb_quote(simplify(eval(args,1,context0),context0));
  }
  ...
  define_unary_function_ptr5(at_POLYFORM, alias_at_POLYFORM, &__POLYFORM, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_POLYROOT, alias_at_POLYROOT, &__POLYROOT, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_ISOLATE, alias_at_ISOLATE, &__ISOLATE, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  gen _IS_LINEAR(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT && args._VECTptr->size() == 2) {
      ...
      if (v[1].type == _IDNT && is_linear_wrt(v[0], v[1], a, b, contextptr))
        return makevecteur(eval(a, eval_level(contextptr), contextptr),
                           eval(b, eval_level(contextptr), contextptr));
      else
        return 0;
    }
    return gentypeerr(contextptr);
  ...
  define_unary_function_ptr5(at_IS_LINEAR, alias_at_IS_LINEAR, &__IS_LINEAR, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);

  static const char _FNROOT_s[] = "FNROOT";
  static define_unary_function_eval_quoted(__FNROOT, &_fsolve, _FNROOT_s);
  define_unary_function_ptr5(at_FNROOT, alias_at_FNROOT, &__FNROOT, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  gen _COND(const gen & args0, GIAC_CONTEXT) {
    ...
    if (args0.type == _VECT && args0._VECTptr->size() == 2) {
      ...
      if (args0._VECTptr->back() == 2) {
        ...
        epsilon(
            0.0,
            contextptr); // otherwise small singular values are cancelled and condition is infinity
  ...
  define_unary_function_ptr5(at_PERCENTCHANGE, alias_at_PERCENTCHANGE, &__PERCENTCHANGE, 0,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_PERCENTTOTAL, alias_at_PERCENTTOTAL, &__PERCENTTOTAL, 0,
                             T_UNARY_OP_38);
  ...
  gen _INPUT(const gen & args, GIAC_CONTEXT) {
    ...
    // check types
    if (v[0].is_symb_of_sommet(at_double_deux_points) && v[0]._SYMBptr->feuille.type == _VECT &&
        v[0]._SYMBptr->feuille._VECTptr->size() == 2)
      v[0] = v[0]._SYMBptr->feuille._VECTptr->back();
    for (int i = 1; i <= 3; i++) {
      if (v[i].type != _STRNG)
        v[i] = eval(v[i], 1, contextptr);
      if (v[i].type != _STRNG)
        v[i] = string2gen(v[i].print(contextptr));
    }
  ...
  define_unary_function_ptr5(at_ITERATE, alias_at_ITERATE, &__ITERATE, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  define_unary_function_ptr5(at_RECURSE, alias_at_RECURSE, &__RECURSE, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
#endif
  ...
  gen _MAKEMAT(const gen & args, GIAC_CONTEXT) {
    ...
    for (int i = 1; i <= l; ++i) {
      ...
      for (int j = 1; j <= c; ++j) {
        IJval[1] = j;
        ligne[j - 1] =
            eval(subst(v0, IJ, IJval, false, contextptr), eval_level(contextptr), contextptr);
      }
      res.push_back(ligne);
    }
    return res;
  ...
  define_unary_function_ptr5(at_MAKEMAT, alias_at_MAKEMAT, &__MAKEMAT, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
  ...
  gen _LSQ(const gen & args, GIAC_CONTEXT) {
    ...
    if (vs == 4) {
      gen v2 = v[2], v3 = v[3];
      if (v0.type == _VECT && v1.type == _VECT && !v0._VECTptr->empty() &&
          v0._VECTptr->size() == v1._VECTptr->size() && v2.type == _VECT && v3.type == _VECT) {
        ...
        if (!constraints.empty() &&
            (!ckmatrix(constraints) || constraints.front()._VECTptr->size() != 3))
          return gensizeerr(contextptr);
        vecteur & x = *v0._VECTptr;
        vecteur & y = *v1._VECTptr;
        unsigned M = unsigned(expr.size()), K = unsigned(constraints.size()),
                 N = unsigned(v0._VECTptr->size());
        ...
        if (res.size() != 1 || res.front().type != _VECT ||
            res.front()._VECTptr->size() != M + K + 1)
    ...
    if (A.size() > 20)
      *logptr(contextptr) << "LSQ: exact data, running Gramschmidt instead of qr, this is much "
  ...
  gen _NTHROOT(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type != _VECT || args._VECTptr->size() != 2)
      return gensizeerr(contextptr);
    return _surd(gen(makevecteur(args._VECTptr->back(), args._VECTptr->front()), _SEQ__VECT),
                 contextptr);
  }
  static const char _NTHROOT_s[] = "NTHROOT";
  static define_unary_function_eval4(__NTHROOT, &_NTHROOT, _NTHROOT_s, &printasNTHROOT,
                                     &texprintasNTHROOT);
...
unsigned long long mpz_get_ull(mpz_t n) {
  ...
  int is_known_name_home_38(const char * idname) {
    ...
    if (s == 2 && !strcmp(idname, "θ"))
      return 3;
    if (s == 2 && (idname[0] == 'Z' || idname[0] == 'L' || idname[0] == 'M') && idname[1] >= '0' &&
        idname[1] <= '9')
      return 3;
      ...
      if (g.is_symb_of_sommet(at_sto)) {
        ...
        if (f.type == _VECT && f._VECTptr->size() == 2) {
          return symbolic(at_sto,
                          gen(makevecteur(subst(f._VECTptr->front(), vin, vout, true, contextptr),
                                          f._VECTptr->back()),
                              _SEQ__VECT));
        }
      }
...
const gen & U9_idnt = *(gen *)&alias_U9_idnt;
  ...
  define_unary_function_ptr5(at_CHOOSE, alias_at_CHOOSE, &__CHOOSE, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
...
#define CyrilleFnc(n)                                                                              \
  static const char _##n##_s[] = #n;                                                               \
  static define_unary_function_eval_quoted(__##n, &giac::_##n, _##n##_s);                          \
  define_unary_function_ptr5(at_##n, alias_at_##n, &__##n, 0, T_UNARY_OP_38);
  ...
  gen _LINE(const gen & args, GIAC_CONTEXT) {
    ...
    vecteur & v = *args._VECTptr;
    return _droite(gen(makevecteur(v[0] + cst_i * v[1], v[2] + cst_i * v[3]), _SEQ__VECT),
                   contextptr);
  }
  ...
  gen _RECT(const gen & args, GIAC_CONTEXT) {
    ...
    gen res = pnt_attrib(gen(makevecteur(e, f, h, g, e), _GROUP__VECT), vecteur(1, int(FL_BLACK)),
                         contextptr);
    return res;
  }
...
#endif
  static define_unary_function_eval(__Celsius2Fahrenheit, &_Celsius2Fahrenheit,
                                    _Celsius2Fahrenheit_s);
  define_unary_function_ptr5(at_Celsius2Fahrenheit, alias_at_Celsius2Fahrenheit,
                             &__Celsius2Fahrenheit, 0, T_UNARY_OP);
...
static const char _Fahrenheit2Celsius_s[] = "Fahrenheit2Celsius";
#endif
  static define_unary_function_eval(__Fahrenheit2Celsius, &_Fahrenheit2Celsius,
                                    _Fahrenheit2Celsius_s);
  define_unary_function_ptr5(at_Fahrenheit2Celsius, alias_at_Fahrenheit2Celsius,
                             &__Fahrenheit2Celsius, 0, T_UNARY_OP);
  ...
  gen _polar_complex(const gen & g, GIAC_CONTEXT) {
    ...
    if (angle.type == _FLOAT_ && res.type == _FLOAT_) {
      HP_Real a, r, s, c;
      fExpand(gen2HP(angle), &a);
      fExpand(gen2HP(res), &r);
      fisin(&a, &s, angle_radian(contextptr) ? AMRad : (angle_degree(contextptr) ? AMDeg : AMGrad));
      ficos(&a, &c,
            angle_radian(contextptr) ? AMRad : (angle_degree(contextptr) ? AMDeg : AMGrad)); //grad
      fimul_L(&s, &r, &s);
      fimul_L(&c, &r, &c);
...
#endif
  static define_unary_function_eval4(__polar_complex, &_polar_complex, _polar_complex_s,
                                     &printsommetasoperator, &texprintsommetasoperator);
...
//    EditMat(i?i-1:9);
  ...
  define_unary_function_ptr5(at_EDITMAT, alias_at_EDITMAT, &__EDITMAT, _QUOTE_ARGUMENTS,
                             T_UNARY_OP_38);
