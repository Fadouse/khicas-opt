...
namespace giac {
  ...
  gen frac_neg_out(const gen & g, GIAC_CONTEXT) {
    if ((is_integer(g) && is_strictly_positive(-g, contextptr)) ||
        (g.type == _FRAC && (g._FRACptr->num.type <= _DOUBLE_) &&
         is_strictly_positive(-g._FRACptr->num, contextptr)))
  ...
  static bool is_multiple_of_pi_over_12(const gen & a, int & l, GIAC_CONTEXT) {
    ...
    if (angle_radian(contextptr)) {
      ...
      if (is_undef(k) || !is_constant_wrt(k, cst_pi, contextptr) ||
          !is_zero(ratnormal(a - k * cst_pi, contextptr)))
        return false;
      k = (trig_deno / 2) * k;
      if (k.type == _SYMB)
  ...
  */
  ...
  gen apply_to_equal(const gen & g, const gen_op & f) {
    if (g.type != _SYMB || (g._SYMBptr->sommet != at_equal && g._SYMBptr->sommet != at_equal2) ||
  ...
  gen apply_to_equal(const gen & g, gen (*f)(const gen &, GIAC_CONTEXT), GIAC_CONTEXT) {
    ...
    return symbolic(g._SYMBptr->sommet,
                    makesequence(f(v.front(), contextptr), f(v.back(), contextptr)));
  }
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(4, __neg, &_neg, (size_t)&D_at_negunary_function_ptr,
                                           _neg_s);
#else
static define_unary_function_eval3_index(4, __neg, &_neg, D_at_neg, _neg_s);
  ...
  gen ln(const gen & e, GIAC_CONTEXT) {
    ...
    if (!escape_real(contextptr) && !complex_mode(contextptr) && (e.type <= _POLY) &&
        !is_positive(e, contextptr))
      return gensizeerr(contextptr);
#if 0
    if (e.type==_FLOAT_){
#ifdef BCD
    ...
    if (e.is_symb_of_sommet(at_pow) && e._SYMBptr->feuille.type == _VECT &&
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(18, __ln, &ln, (size_t)&D_at_lnunary_function_ptr,
                                           _ln_s);
#else
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3(__log10, &log10, (size_t)&D_at_log10unary_function_ptr,
                                     _log10_s);
#else
static define_unary_function_eval3(__log10, &log10, D_at_log10, _log10_s);
  ...
  gen atan(const gen & e0, GIAC_CONTEXT) {
    ...
    if (tmp.type == _DOUBLE_) {
      ...
      if (absdouble(edh._DOUBLE_val) < 1e-7 &&
        ...
        if (res % 2)
          return (ed > 0 ? res : -res) *
                 (angle_radian(contextptr) ? cst_pi / 10
                                           : (angle_degree(contextptr) ? gen(18) : gen(20))); //grad
        else
          return (ed > 0 ? res / 2 : -res / 2) *
                 (angle_radian(contextptr) ? cst_pi / 5
                                           : (angle_degree(contextptr) ? gen(36) : gen(40))); //grad
      }
      ...
      if (absdouble(edh._DOUBLE_val) < 1e-7) {
        ...
        den /= g;
        return (ed > 0 ? res : -res) *
               (angle_radian(contextptr)
                    ? cst_pi / den
                    : (angle_degree(contextptr) ? gen(15 * g)
                                                : rdiv(50, 3) * gen(g))); //grad   50/3*g grads
      }
      ...
      if (absdouble(edh._DOUBLE_val) < 1e-7 &&
        ...
        return (ed > 0 ? res : -res) *
               (angle_radian(contextptr)
                    ? cst_pi / 8
                    : (angle_degree(contextptr) ? gen(45) / 2 : gen(25))); //grad
      }
    }
  ...
  static gen taylor_atan(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                         int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor_index(42, __atan, &atan,
                                                 (size_t)&D_at_atanunary_function_ptr, &taylor_atan,
                                                 _atan_s);
#else
static define_unary_function_eval_taylor_index(42, __atan, &atan, D_at_atan, &taylor_atan, _atan_s);
  ...
  gen exp(const gen & e0, GIAC_CONTEXT) {
    ...
    if (e.type == _CPLX) {
      ...
      if (e._CPLXptr->type == _REAL || e._CPLXptr->type == _FLOAT_) {
        //grad
        int mode = get_mode_set_radian(contextptr);
        gen res = exp(*e._CPLXptr, contextptr) *
                  gen(cos(*(e._CPLXptr + 1), contextptr), sin(*(e._CPLXptr + 1), contextptr));
        angle_mode(mode, contextptr);
    ...
    if (e.type == _SYMB && e._SYMBptr->sommet == at_ln)
      return e._SYMBptr->feuille;
    if (e.type == _SYMB && e._SYMBptr->sommet == at_neg && e._SYMBptr->feuille.type == _SYMB &&
        e._SYMBptr->feuille._SYMBptr->sommet == at_ln)
  ...
  static gen taylor_exp(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                        int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0; // no symbolic preprocessing
    shift_coeff = 0;
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor2_index(16, __exp, &exp,
                                                  (size_t)&D_at_expunary_function_ptr, &taylor_exp,
                                                  _exp_s, 0, &texprintsommetasoperator);
#else
static define_unary_function_eval_taylor2_index(16, __exp, &exp, D_at_exp, &taylor_exp, _exp_s, 0,
                                                &texprintsommetasoperator);
#endif
  ...
  void zint2simpldoublpos(const gen & e, gen & simpl, gen & doubl, bool & pos, int d,
                          GIAC_CONTEXT) {
    simpl = 1;
    doubl = 1;
    if (!is_integer(e)) {
  ...
  gen sqrt(const gen & e, GIAC_CONTEXT) {
    ...
    if (e.type == _CPLX || has_i(e)) {
      ...
      if (taille(e, 129) <= 128 && !lidnt(e).empty())
      ...
      a = re(e, contextptr);
      b = ratnormal(im(e, contextptr), contextptr);
    ...
    if ((is_zero(e) && !e.is_symb_of_sommet(at_unit)) || is_undef(e) || (e == plus_inf) ||
        (e == unsigned_inf))
      return e;
    ...
    if (e.type == _SYMB) {
      ...
      if (e.is_symb_of_sommet(at_pow) && e._SYMBptr->feuille.type == _VECT &&
        ...
        if (power.type == _INT_ && power.val > 0 && power.val % 4 == 3)
          return symbolic(at_sqrt, e);
      }
      vecteur v = lvar(e);
      if (v.size() == 1 && v.front().is_symb_of_sommet(at_pow) &&
          v.front()._SYMBptr->feuille[1] == plus_one_half &&
          is_integer(v.front()._SYMBptr->feuille[0])) {
        gen a, b, c = v.front()._SYMBptr->feuille[0];
        if (is_linear_wrt(e, v.front(), b, a, contextptr) && (is_integer(a) || a.type == _FRAC) &&
      ...
      for (unsigned i = 0; i < v.size(); ++i) {
        ...
        if (vi.is_symb_of_sommet(at_cos)) {
          ...
          if (is_linear_wrt(e, vi, a, b, contextptr)) {
            if (a == b)
              return sqrt(2 * a, contextptr) *
                     symbolic(at_abs, cos(vi._SYMBptr->feuille / 2, contextptr));
            if (a == -b)
              return sqrt(-2 * a, contextptr) *
                     symbolic(at_abs, sin(vi._SYMBptr->feuille / 2, contextptr));
          }
        }
        if (vi.is_symb_of_sommet(at_sin)) {
          ...
          if (is_linear_wrt(e, vi, a, b, contextptr)) {
            if (a == b)
              return sqrt(2 * a, contextptr) *
                     symbolic(at_abs, cos(vi._SYMBptr->feuille / 2 - cst_pi / 4, contextptr));
            if (a == -b)
              return sqrt(-2 * a, contextptr) *
                     symbolic(at_abs, sin(vi._SYMBptr->feuille / 2 - cst_pi / 4, contextptr));
          }
        }
      } // end loop on vars
    }
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval5(__sqrt, &sqrt, (size_t)&D_at_sqrtunary_function_ptr, _sqrt_s,
                                     &printassqrt, &texprintsommetasoperator);
#else
static define_unary_function_eval5(__sqrt, &sqrt, D_at_sqrt, _sqrt_s, &printassqrt,
                                   &texprintsommetasoperator);
#endif
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(158, __sq, (const gen_op_context)_sq,
                                           (size_t)&D_at_squnary_function_ptr, _sq_s);
#else
  ...
  gen cos(const gen & e0, GIAC_CONTEXT) {
    ...
    if (e.type == _CPLX) {
      ...
      if (e._CPLXptr->type == _REAL || e._CPLXptr->type == _FLOAT_) {
        ...
        int mode = get_mode_set_radian(contextptr);
        e1 = gen(
            cos(e1, contextptr) * cosh(e2, contextptr),
            -sinh(e2, contextptr) *
                sin(e1,
                    contextptr)); //cos(e1,contextptr)*cosh(e2,contextptr)-cst_i*sinh(e2,contextptr)*sin(e1,contextptr);
        angle_mode(mode, contextptr);
        return e1;
      }
#endif
    ...
    if (angle_radian(contextptr)) {
      if (simplify_sincosexp_pi && contains(e, cst_pi) &&
          is_linear_wrt(e, cst_pi, a, b, contextptr)) {
        if (is_zero(a)) {
          if (is_zero(b))
            return 1;
        } else {
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(34, __cos, &cos, (size_t)&D_at_cosunary_function_ptr,
                                           _cos_s);
#else
  ...
  gen sin(const gen & e0, GIAC_CONTEXT) {
    ...
    if (e.type == _CPLX) {
      ...
      if (e._CPLXptr->type == _REAL || e._CPLXptr->type == _FLOAT_) {
        ...
        int mode = get_mode_set_radian(contextptr);
        gen res = gen(
            sin(e1, contextptr) * cosh(e2, contextptr),
            sinh(e2, contextptr) *
                cos(e1,
                    contextptr)); //sin(e1,contextptr)*cosh(e2,contextptr)+cst_i*sinh(e2,contextptr)*cos(e1,contextptr);
        angle_mode(mode, contextptr);
        return res;
      }
#endif
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(32, __sin, &sin, (size_t)&D_at_sinunary_function_ptr,
                                           _sin_s);
#else
  ...
  gen tan(const gen & e0, GIAC_CONTEXT) {
    ...
    if (!approx_mode(contextptr)) {
      ...
      if (is_multiple_of_pi_over_12(2 * e, k, contextptr)) //grad
        ...
        if (is_rational(kk, n, d)) {
          if (d == 10)
            return inv(tan((angle_radian(contextptr) ? cst_pi / 2
                                                     : (angle_degree(contextptr) ? 90 : 100)) -
                               e,
                           contextptr),
                       contextptr); //grad
          if (d == 5) {
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(36, __tan, &tan, (size_t)&D_at_tanunary_function_ptr,
                                           _tan_s);
#else
static define_unary_function_eval3_index(36, __tan, &tan, D_at_tan, _tan_s);
  ...
  gen * normal_sin_pi_12_ptr_() {
    static gen * ans = 0;
    if (!ans)
      ans = new gen(normal(sin_pi_12, context0));
    return ans;
  }
  gen * normal_cos_pi_12_ptr_() {
    static gen * ans = 0;
    if (!ans)
      ans = new gen(normal(cos_pi_12, context0));
    return ans;
  }
  gen asin(const gen & e0, GIAC_CONTEXT) {
    if ((calc_mode(contextptr) == 38 || !escape_real(contextptr)) && !complex_mode(contextptr) &&
        (e0.type <= _POLY) &&
        (!is_positive(e0 + 1, contextptr) || !is_positive(1 - e0, contextptr)))
      return gensizeerr(contextptr);
    if (e0.type == _SPOL1) {
    ...
    if (edg.type == _DOUBLE_) {
      ...
      if (absdouble(edh._DOUBLE_val) < 1e-9 &&
        ...
        return (ed > 0 ? res : -res) *
               (angle_radian(contextptr)
                    ? cst_pi / den
                    : (angle_degree(contextptr) ? gen(45 * g) / 2 : gen(25))); //grad
      }
    }
    if (is_undef(e))
      return e;
    ...
    if (lidnt(e).empty() && is_positive(e * e - 1, contextptr))
      return (angle_radian(contextptr) ? 1 : (angle_degree(contextptr) ? rad2deg_g : rad2grad_g)) *
             asinasln(e, contextptr);
    return symb_asin(e);
  }
  ...
  static gen taylor_asin(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                         int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0; // no symbolic preprocessing
    if (is_one(lim_point)) {
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor_index(38, __asin, &asin,
                                                 (size_t)&D_at_asinunary_function_ptr, &taylor_asin,
                                                 _asin_s);
#else
static define_unary_function_eval_taylor_index(38, __asin, &asin, D_at_asin, &taylor_asin, _asin_s);
  ...
  gen acos(const gen & e0, GIAC_CONTEXT) {
    ...
    if (e.type == _DOUBLE_ || (e.type == _CPLX && (e.subtype))) {
      gen res =
          cst_pi / 2 -
          asinasln(e,
                   contextptr); // -cst_i*no_context_evalf(ln(sqrt(e*e-1,contextptr)+e,contextptr));
      if (angle_radian(contextptr))
        return res;
      else if (angle_degree(contextptr))
  ...
  static gen taylor_acos(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                         int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0; // no symbolic preprocessing
    if (is_one(lim_point)) {
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor_index(40, __acos, &acos,
                                                 (size_t)&D_at_acosunary_function_ptr, &taylor_acos,
                                                 _acos_s);
#else
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(44, __sinh, &sinh, (size_t)&D_at_sinhunary_function_ptr,
                                           _sinh_s);
#else
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(46, __cosh, &cosh, (size_t)&D_at_coshunary_function_ptr,
                                           _cosh_s);
#else
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(48, __tanh, &tanh, (size_t)&D_at_tanhunary_function_ptr,
                                           _tanh_s);
#else
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(50, __asinh, &asinh,
                                           (size_t)&D_at_asinhunary_function_ptr, _asinh_s);
#else
static define_unary_function_eval3_index(50, __asinh, &asinh, D_at_asinh, _asinh_s);
  ...
  static gen acoshasln(const gen & x, GIAC_CONTEXT) {
    if (re(x, contextptr) == x)
      return ln(x + sqrt(x * x - 1, contextptr),
                contextptr); // avoid multiple sqrt but it's the opposite for example for x non real
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(52, __acosh, &acosh,
                                           (size_t)&D_at_acoshunary_function_ptr, _acosh_s);
#else
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3_index(54, __atanh, &atanh,
                                           (size_t)&D_at_atanhunary_function_ptr, _atanh_s);
#else
static define_unary_function_eval3_index(54, __atanh, &atanh, D_at_atanh, _atanh_s);
  ...
  gen quote(const gen & args, GIAC_CONTEXT) {
    if (args.type == _VECT && args.subtype == _SEQ__VECT && !args._VECTptr->empty() &&
        args._VECTptr->front().type == _FUNC) {
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval5_quoted(__quote, &quote, (size_t)&D_at_quoteunary_function_ptr,
                                            _quote_s, &printasquote, 0);
#else
static define_unary_function_eval5_quoted(__quote, &quote, D_at_quote, _quote_s, &printasquote, 0);
  ...
  define_partial_derivative_onearg_genop(D_at_unquote, " D_at_unquote",
                                         (const gen_op_context)unquote);
  static const char _unquote_s[] = "unquote";
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3(__unquote, (const gen_op_context)unquote,
                                     (size_t)&D_at_unquoteunary_function_ptr, _unquote_s);
#else
static define_unary_function_eval3(__unquote, (const gen_op_context)unquote, D_at_unquote,
                                   _unquote_s);
#endif
  ...
  gen order_size(const gen & arg, GIAC_CONTEXT) {
    if (arg.type == _SPOL1 && arg._SPOL1ptr->size() == 1) {
      ...
      if (expo != 1)
        *logptr(contextptr)
            << "order_size argument should always be the series variable name. This means that O("
            << sv << "^" << expo << ") should be written " << sv << "^" << expo << "*order_size("
            << sv << ")" << endl;
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3(__order_size, &order_size,
                                     (size_t)&D_at_order_sizeunary_function_ptr, _order_size_s);
#else
static define_unary_function_eval3(__order_size, &order_size, D_at_order_size, _order_size_s);
  ...
  static define_unary_function_eval4(__re, (const gen_op_context)re, _re_s, 0,
                                     &texprintsommetasoperator);
  ...
  static define_unary_function_eval4(__im, (const gen_op_context)im, _im_s, 0,
                                     &texprintsommetasoperator);
  ...
  static define_unary_function_eval4(__conj, (const gen_op_context)conj, _conj_s, 0,
                                     &texprintsommetasoperator);
  define_unary_function_ptr5(at_conj, alias_at_conj, &__conj, 0, true);

  static gen taylor_sign(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                         int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0; // no symbolic preprocessing
    shift_coeff = 0;
    if (is_strictly_positive(lim_point, contextptr) ||
        (is_zero(lim_point, contextptr) && direction == 1))
    ...
    if (!is_zero(lim_point))
      return makevecteur(symbolic(at_sign, lim_point));
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__sign, _sign, (size_t)&D_at_signunary_function_ptr,
                                           &taylor_sign, _sign_s);
#else
static define_unary_function_eval_taylor(__sign, _sign, D_at_sign, &taylor_sign, _sign_s);
  ...
  static gen taylor_abs(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                        int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0; // no symbolic preprocessing
    shift_coeff = 0;
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor_index(20, __abs, &_abs,
                                                 (size_t)&D_at_absunary_function_ptr, &taylor_abs,
                                                 _abs_s);
#else
static define_unary_function_eval_taylor_index(20, __abs, &_abs, D_at_abs, &taylor_abs, _abs_s);
  ...
  string printassto(const gen & feuille, const char * sommetstr, GIAC_CONTEXT) {
    ...
    if (feuille.subtype == _SORTED__VECT &&
        feuille._VECTptr->front().is_symb_of_sommet(at_program)) {
      gen prog = feuille._VECTptr->front()._SYMBptr->feuille;
      if (prog.type == _VECT && prog._VECTptr->size() == 3 &&
          (prog._VECTptr->front() != _VECT || prog._VECTptr->front()._VECTptr->size() == 2)) {
        ...
        prog = symb_of(feuille._VECTptr->back(),
                       arg); //symbolic(at_of,makesequence(feuille._VECTptr->back(),arg));
    ...
    if (v.back().is_symb_of_sommet(at_of) && feuille.subtype != _SORTED__VECT) {
      ...
      if (f.type == _VECT && f._VECTptr->size() == 2) {
        return f._VECTptr->front().print(contextptr) + "[[" + f._VECTptr->back().print(contextptr) +
               "]] " + stos + " " + v.front().print(contextptr);
      }
    }
#endif
...
#ifdef GIAC_HAS_STO_38
  bool do_storcl_38(gen & value, const char * name_space, const char * idname, gen indice,
                    bool at_of, GIAC_CONTEXT, gen const * sto, bool OnlyLocal);
  bool (*storcl_38)(gen & value, const char * name_space, const char * idname, gen indice,
                    bool at_of, GIAC_CONTEXT, gen const * sto, bool OnlyLocal) = do_storcl_38;
#else
bool (*storcl_38)(gen & value, const char * name_space, const char * idname, gen indice, bool at_of,
                  GIAC_CONTEXT, gen const * sto, bool OnlyLocal) = 0;
#endif
  ...
  static bool in_stomap(gen_map & m, const gen & indice, const gen & a) {
    if (indice.is_symb_of_sommet(*at_interval) && indice._SYMBptr->feuille.type == _VECT &&
        indice._SYMBptr->feuille._VECTptr->size() == 2) {
    ...
    if (ligne.is_symb_of_sommet(*at_interval) && ligne._SYMBptr->feuille.type == _VECT &&
        ligne._SYMBptr->feuille._VECTptr->size() == 2) {
      gen deb = ligne._SYMBptr->feuille._VECTptr->front();
      gen fin = ligne._SYMBptr->feuille._VECTptr->back();
      if (!is_integral(deb) || !is_integral(fin) || deb.type != _INT_ || fin.type != _INT_)
        return false;
      bool both = col.is_symb_of_sommet(*at_interval) && col._SYMBptr->feuille.type == _VECT &&
                  col._SYMBptr->feuille._VECTptr->size() == 2;
      int shift = 0;
      if (both) {
        gen coldeb = col._SYMBptr->feuille._VECTptr->front();
        gen colend = col._SYMBptr->feuille._VECTptr->back();
        if (!is_integral(coldeb) || !is_integral(colend) || coldeb.type != _INT_ ||
            colend.type != _INT_ || colend.val - coldeb.val != fin.val - deb.val)
          return false;
    ...
    if (col.is_symb_of_sommet(*at_interval) && col._SYMBptr->feuille.type == _VECT &&
        col._SYMBptr->feuille._VECTptr->size() == 2) {
  ...
  gen sto(const gen & a, const gen & b, bool in_place, const context * contextptr_) {
    if (a.type == _STRNG && is_undef(a))
      return a;
    if ((a.type == _IDNT || a.is_symb_of_sommet(at_at)) && b.is_symb_of_sommet(at_rootof) &&
        contextptr_) {
      ...
      if (Pmin.type != _VECT || Pmin._VECTptr->size() != 2 ||
          Pmin._VECTptr->front() != makevecteur(1, 0))
    ...
    if (b.type == _SYMB) {
      ...
      if (b._SYMBptr->sommet == at_double_deux_points) {
...
#endif
        if (a1.type == _INT_ && a1.subtype == 0 && a1.val >= 0 &&
            a1.val < (signed)context_list().size()) {
        ...
        if (context_names) {
          map<string, context *>::iterator it = context_names->find(a1.print()),
                                           itend = context_names->end();
          if (it != itend) {
    ...
    if (b.type == _IDNT) {
        ...
        if (name[bl - 2] == '_') {
          ...
          case 'd':
            if (a.type != _INT_ && a.type != _DOUBLE_ && a.type != _FRAC)
              return gensizeerr(gettext("Unable to convert to float (in sto) ") +
                                a.print(contextptr));
            break;
          case 'f':
            if (a.type == _FRAC)
          ...
          case 'l':
            if (a.type == _DOUBLE_ && a._DOUBLE_val <= RAND_MAX && a._DOUBLE_val >= -RAND_MAX) {
              ...
              if (i != a._DOUBLE_val)
                *logptr(contextptr) << gettext("Converting ") << a._DOUBLE_val
                                    << gettext(" to integer ") << i << endl;
              return sto(i, b, in_place, contextptr);
            }
            if (a.type != _INT_) {
              if (a.type != _ZINT || mpz_sizeinbase(*a._ZINTptr, 2) > 62)
                return gensizeerr(gettext("Unable to convert to integer (in sto) ") +
                                  a.print(contextptr));
            }
            break;
          case 'v':
            if (a.type != _VECT)
              return gensizeerr(gettext("Unable to convert to vector (in sto) ") +
                                a.print(contextptr));
            break;
          case 's':
            if (a.type != _STRNG)
      ...
      gen aa(a);
      if (strcmp(name, string_pi) == 0 || strcmp(name, string_infinity) == 0 ||
          strcmp(name, string_undef) == 0
#ifdef GIAC_HAS_STO_38
          || name[0] == '_'
#endif
      )
      ...
      if (contextptr) {
        ...
        if (!done) { // store b globally
          if (contains(lidnt(a), b)) {
            if (a.is_symb_of_sommet(at_when) || a.is_symb_of_sommet(at_ifte) ||
                a.is_symb_of_sommet(at_program))
          ...
          sym_tab * symtabptr = contextptr->globalcontextptr ? contextptr->globalcontextptr->tabptr
                                                             : contextptr->tabptr;
      ...
      if (b._IDNTptr->localvalue && !b._IDNTptr->localvalue->empty() &&
          (b.subtype != _GLOBAL__EVAL))
    ...
    if ((b.type == _SYMB) && (b._SYMBptr->sommet == at_at || b._SYMBptr->sommet == at_of)) {
      ...
      if (destination.is_symb_of_sommet(at_at) && b._SYMBptr->sommet == at_at) {
        destination =
            symbolic(at_at, makesequence(destination._SYMBptr->feuille[0],
                                         makesequence(destination._SYMBptr->feuille[1],
                                                      b._SYMBptr->feuille._VECTptr->back())));
      ...
      gen ret;
      if (storcl_38 && destination.type == _IDNT &&
          storcl_38(ret, 0, destination._IDNTptr->id_name, b, true, contextptr, &a, false))
        return ret;
      if (destination.type == _IDNT && destination._IDNTptr->quoted)
        *destination._IDNTptr->quoted |= 2; // set dirty bit
      gen valeur;
      if (!contextptr && in_place && destination.type == _IDNT &&
          destination._IDNTptr->localvalue && !destination._IDNTptr->localvalue->empty() &&
          local_eval(contextptr))
      ...
      if (valeur.type == _INT_ && valeur.val == 0 && destination.type == _IDNT &&
          destination._IDNTptr->localvalue && !destination._IDNTptr->localvalue->empty())
      ...
      is_integral(indice);
      if (b._SYMBptr->sommet == at_of && valeur.type == _VECT &&
          (1 || abs_calc_mode(contextptr) ==
                    38)) { // matrices and vector indices in HP38 compatibility mode
        if (indice.type == _INT_)
          indice -= 1;
        if (indice.type == _VECT)
          indice = indice - vecteur(indice._VECTptr->size(), 1);
      }
      if ((destination.type != _IDNT && !destination.is_symb_of_sommet(at_double_deux_points)) ||
          (valeur.type != _VECT && valeur.type != _MAP && valeur.type != _IDNT &&
           valeur.type != _STRNG && valeur.type != _SYMB)) {
        string endstring = " not allowed.";
        if (b.is_symb_of_sommet(at_at))
          endstring +=
              " Run purge if you want to create a sparse matrix in " + b[1].print(contextptr) + ".";
        return gentypeerr(gettext("sto ") + b.print(contextptr) + ":=" + valeur.print(contextptr) +
                          endstring);
      }
      ...
      if (valeur.type == _STRNG) {
        ...
        if ((indice.is_symb_of_sommet(*at_interval) || indicedeuxpoints) &&
            indice._SYMBptr->feuille.type == _VECT &&
            indice._SYMBptr->feuille._VECTptr->size() == 2) {
          gen deb = indice._SYMBptr->feuille._VECTptr->front();
          gen fin =
              indice._SYMBptr->feuille._VECTptr->back() + (indicedeuxpoints ? minus_one : zero);
          if (!is_integral(deb) || !is_integral(fin) || deb.type != _INT_ || fin.type != _INT_ ||
              a.type != _STRNG)
            return gendimerr();
        ...
        if (indice.type != _INT_ || a.type != _STRNG || a._STRNGptr->empty())
          return gensizeerr(contextptr);
        if (indice.val < 0)
          indice += (int)valeur._STRNGptr->size();
      ...
      if (valeur.type == _SYMB) {
            ...
            if (v.type == _SYMB) {
              if (itend->val == 0) {
                if (a.type == _FUNC)
                  ...
                  v = symb_of(
                      a,
                      v._SYMBptr->feuille); //symbolic(at_of,makesequence(a,v._SYMBptr->feuille));
              } else {
        ...
        if (indice.val == 0) {
          if (a.type == _FUNC)
            ...
            nvaleur = symb_of(
                a, valeur._SYMBptr
                       ->feuille); // symbolic(at_of,makesequence(a,valeur._SYMBptr->feuille));
        } else {
      ...
      if ((indice.is_symb_of_sommet(*at_interval) || indicedeuxpoints) &&
        ...
        if (!is_integral(deb) || !is_integral(fin) || deb.type != _INT_ || fin.type != _INT_ ||
            fin.val >= LIST_SIZE_LIMIT)
          return gendimerr();
        if (deb.val < 0)
          deb.val += int(vptr->size());
        if (fin.val < 0)
          fin.val += int(vptr->size());
      ...
      if (itend - it == 2) {
        ...
        if ((it->is_symb_of_sommet(*at_interval) || itdeuxpoints) &&
            it->_SYMBptr->feuille.type == _VECT && it->_SYMBptr->feuille._VECTptr->size() == 2) {
          ...
          if ((i2.is_symb_of_sommet(*at_interval) || i2deuxpoints) &&
              i2._SYMBptr->feuille.type == _VECT && i2._SYMBptr->feuille._VECTptr->size() == 2) {
            gen deb2 = i2._SYMBptr->feuille._VECTptr->front();
            gen fin2 = i2._SYMBptr->feuille._VECTptr->back() + (i2deuxpoints ? minus_one : zero);
            if (!is_integral(deb2) || !is_integral(fin2) || deb2.type != _INT_ ||
                fin2.type != _INT_)
              return gendimerr(contextptr);
            if (deb2.val < 0)
              deb2.val += cols;
        ...
        if ((i2.is_symb_of_sommet(*at_interval) || i2deuxpoints) &&
          ...
          if (!is_integral(deb2) || !is_integral(fin2) || deb2.type != _INT_ || fin2.type != _INT_)
            return gendimerr(contextptr);
          if (deb2.val < 0)
            deb2.val += int(vptr->front()._VECTptr->size());
          if (fin2.val < 0)
            fin2.val += int(vptr->front()._VECTptr->size());
          if (deb2.val < 0 || fin2.val < deb2.val ||
              fin2.val >= int(vptr->front()._VECTptr->size()))
            return gendimerr(contextptr);
          if (a.type == _VECT) {
  ...
  define_unary_function_eval4_index(30, __sto, &_sto, _sto_s, &printassto,
                                    &texprintsommetasoperator);
  ...
  define_unary_function_ptr5(at_array_sto, alias_at_array_sto, &__array_sto, _QUOTE_ARGUMENTS,
                             true);
  ...
  static gen in_increment3(const gen & prev, const gen & val, const gen & var, int mult,
                           GIAC_CONTEXT) {
    if (mult == 0)
  ...
  static gen increment(const gen & var, const gen & val_orig, bool negatif, int mult,
                       GIAC_CONTEXT) {
  ...
  static define_unary_function_eval4_index(151, __increment, &_increment, _increment_s,
                                           &printasincrement, &texprintsommetasoperator);
  define_unary_function_ptr5(at_increment, alias_at_increment, &__increment, _QUOTE_ARGUMENTS,
                             true);
  ...
  static define_unary_function_eval4_index(153, __decrement, &_decrement, _decrement_s,
                                           &printasdecrement, &texprintsommetasoperator);
  define_unary_function_ptr5(at_decrement, alias_at_decrement, &__decrement, _QUOTE_ARGUMENTS,
                             true);
  ...
  static define_unary_function_eval4_index(155, __multcrement, &_multcrement, _multcrement_s,
                                           &printasmultcrement, &texprintsommetasoperator);
  define_unary_function_ptr5(at_multcrement, alias_at_multcrement, &__multcrement, _QUOTE_ARGUMENTS,
                             true);
  ...
  static define_unary_function_eval4_index(157, __divcrement, &_divcrement, _divcrement_s,
                                           &printasdivcrement, &texprintsommetasoperator);
  ...
  define_unary_function_ptr5(at_rotatesto, alias_at_rotatesto, &__rotatesto, _QUOTE_ARGUMENTS,
                             true);
  ...
  bool is_assumed_real(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.subtype == _GLOBAL__EVAL && contextptr) {
      sym_tab::const_iterator it = contextptr->globalcontextptr->tabptr->find(g._IDNTptr->id_name),
                              itend = contextptr->globalcontextptr->tabptr->end();
      if (it != itend)
        tmp = it->second;
    }
  ...
  gen doubleassume_and(const vecteur & v, const gen & a, int direction, bool or_assumption,
                       GIAC_CONTEXT) {
    ...
    if (v_intervalle.empty() || or_assumption) {
      if (direction > 0)
        ...
        v_intervalle.push_back(
            makeline(minus_inf, a)); // gen(makevecteur(minus_inf,a),_LINE__VECT));
      if (or_assumption)
  ...
  gen assumesymbolic(const gen & a, gen idnt_must_be, GIAC_CONTEXT) {
    ...
    if ((arg0.type != _IDNT || arg0 == cst_pi) && arg1.type == _IDNT) {
      ...
      if (s == at_superieur_strict || s == at_superieur_egal || s == at_inferieur_strict ||
          s == at_inferieur_egal)
        return assumesymbolic(symbolic(s, swapped), idnt_must_be, contextptr);
    }
  ...
  static void purge_assume(const gen & a, GIAC_CONTEXT) {
    if (a.type == _SYMB &&
        (a._SYMBptr->sommet == at_and || a._SYMBptr->sommet == at_ou ||
         a._SYMBptr->sommet == at_inferieur_strict || a._SYMBptr->sommet == at_inferieur_egal ||
         a._SYMBptr->sommet == at_superieur_egal || a._SYMBptr->sommet == at_superieur_strict ||
         a._SYMBptr->sommet == at_equal)) {
  ...
  gen giac_assume(const gen & a, GIAC_CONTEXT) {
    ...
    gen a_;
    if (a.type == _SYMB) {
      if (a._SYMBptr->sommet == at_and || a._SYMBptr->sommet == at_ou ||
          a._SYMBptr->sommet == at_inferieur_strict || a._SYMBptr->sommet == at_inferieur_egal ||
          a._SYMBptr->sommet == at_superieur_egal || a._SYMBptr->sommet == at_superieur_strict ||
          a._SYMBptr->sommet == at_equal)
        a_ = a;
  ...
  static define_unary_function_eval_quoted(giac__additionally, &giac_additionally,
                                           giac_additionally_s);
  define_unary_function_ptr5(at_additionally, alias_at_additionally, &giac__additionally,
  ...
  gen symb_plus(const gen & a, const gen & b) {
    if (a.is_symb_of_sommet(at_plus) && !is_inf(a._SYMBptr->feuille)) {
      if (b.is_symb_of_sommet(at_plus) && !is_inf(b._SYMBptr->feuille))
        return symb_plus(
            mergevecteur(*(a._SYMBptr->feuille._VECTptr), *(b._SYMBptr->feuille._VECTptr)));
      else
  ...
  inline bool plus_idnt_symb(const gen & a) {
    return (a.type == _IDNT && strcmp(a._IDNTptr->id_name, "undef") &&
            strcmp(a._IDNTptr->id_name, "infinity")) ||
           (a.type == _SYMB && !is_inf(a) &&
            (a._SYMBptr->sommet == at_prod || a._SYMBptr->sommet == at_pow ||
             a._SYMBptr->sommet == at_neg));
  }

  inline bool idnt_symb_int(const gen & b) {
    return (b.type == _INT_ && b.val != 0) || b.type == _ZINT ||
           (b.type == _SYMB && !is_inf(b) && b._SYMBptr->sommet != at_unit &&
            b._SYMBptr->sommet != at_equal && b._SYMBptr->sommet != at_equal2 &&
            !equalposcomp(plot_sommets, b._SYMBptr->sommet) &&
            !equalposcomp(inequality_tab, b._SYMBptr->sommet)) ||
           (b.type == _IDNT && strcmp(b._IDNTptr->id_name, "undef") &&
            strcmp(b._IDNTptr->id_name, "infinity"));
  }
  ...
  gen _plus(const gen & args, GIAC_CONTEXT) {
    ...
    if (it == itend) {
      ...
      if (idnt_symb_int(b) && plus_idnt_symb(a)) {
        ...
        if (!b.is_symb_of_sommet(at_program) && !b.is_symb_of_sommet(at_plus))
      ...
      if (idnt_symb_int(a) && plus_idnt_symb(b)) {
        ...
        if (a.is_symb_of_sommet(at_neg) && b == a._SYMBptr->feuille)
          return chkmod(zero, b);
        if (!a.is_symb_of_sommet(at_program) && !a.is_symb_of_sommet(at_plus))
          return (symb_plus(args));
      }
    ...
    for (; it != itend; ++it) {
      if (sum.type == _SYMB && sum._SYMBptr->sommet == at_plus &&
          sum._SYMBptr->feuille.type == _VECT && sum._SYMBptr->feuille._VECTptr->size() > 1) {
        ...
        for (; it != itend; ++it) {
          if (it->type == _SYMB && it->_SYMBptr->sommet == at_plus &&
            ...
            iterateur jt = it->_SYMBptr->feuille._VECTptr->begin(),
                      jtend = it->_SYMBptr->feuille._VECTptr->end();
            for (; jt != jtend; ++jt) {
              vptr->v.push_back(*jt);
            }
            continue;
          ...
          if (it->type == _DOUBLE_ ||
              (it->type <= _POLY &&
               vptr->v.back().type <=
                   _POLY)) // N.B. _DOUBLE_ special case bad for f(x):= 6. + 3.*x + 2.*x^2;g(x) := 12. ; expand(f(x)+g(x));
            vptr->v.back() = operator_plus(vptr->v.back(), *it, contextptr);
          else {
            if (is_inf(*it) || is_undef(*it) ||
                (it->type == _SYMB && it->_SYMBptr->sommet == at_plus))
              break;
  ...
  */
  ...
  static define_unary_function_eval2_index(170, __pointplus, &_pointplus, _pointplus_s,
  ...
  static define_unary_function_eval2_index(172, __pointminus, &_pointminus, _pointminus_s,
                                           &printsommetasoperator);
  ...
  inline bool prod_idnt_symb(const gen & a) {
    return (a.type == _IDNT && strcmp(a._IDNTptr->id_name, "undef") &&
            strcmp(a._IDNTptr->id_name, "infinity")) ||
           (a.type == _SYMB && !is_inf(a) &&
            (a._SYMBptr->sommet == at_plus || a._SYMBptr->sommet == at_pow ||
             a._SYMBptr->sommet == at_neg));
  }
  ...
  */
  ...
  gen _pow(const gen & args, GIAC_CONTEXT) {
    ...
    if (b.type == _INT_) {
      ...
      if (a.type == _SYMB && !is_inf(a) &&
          (a._SYMBptr->sommet == at_plus || a._SYMBptr->sommet == at_prod)) {
        return b.val ? symb_pow(args) : gen(1);
      }
    }
...
#define __pow (*((unary_function_eval *)&unary__pow))
  ...
  static define_unary_function_eval4_index(70, __inferieur_strict, &_inferieur_strict,
                                           _inferieur_strict_s, &printsommetasoperator,
                                           &texprintsommetasoperator);
  ...
  static define_unary_function_eval4_index(72, __inferieur_egal, &_inferieur_egal,
                                           _inferieur_egal_s, &printsommetasoperator,
                                           &texprintsommetasoperator);
  ...
  static define_unary_function_eval4_index(74, __superieur_strict, &_superieur_strict,
                                           _superieur_strict_s, &printsommetasoperator,
                                           &texprintsommetasoperator);
  ...
  static define_unary_function_eval4_index(76, __superieur_egal, &_superieur_egal,
                                           _superieur_egal_s, &printsommetasoperator,
                                           &texprintsommetasoperator);
  ...
  static define_unary_function_eval2_index(78, __different, &_different, _different_s,
                                           &printsommetasoperator);
  ...
  static string printasof_(const gen & feuille, const char * sommetstr, int format, GIAC_CONTEXT) {
    ...
    string s =
        print_with_parenthesis_if_required(feuille._VECTptr->front(), format, contextptr) + '(';
  ...
  static bool warn_implicit(const gen & a, const gen & b, GIAC_CONTEXT) {
    if (contains(lidnt(b), i__IDNT_e))
      ...
      *logptr(contextptr) << gettext("Warning : using implicit multiplication for (")
                          << a.print(contextptr) << ")(" << b.print(contextptr) << ')' << endl;
    return true;
  }
  gen check_symb_of(const gen & a, const gen & b0, GIAC_CONTEXT) {
    ...
    if (a.type < _IDNT) {
      if (!warn_implicit(a, b, contextptr))
        return gensizeerr("Invalid implicit multiplication for (" + a.print(contextptr) + ")(" +
                          b.print(contextptr) + ')');
      return a * b;
    }
    vecteur va(lvar(a));
    ...
    if (vab.size() == va.size() + vb.size()) {
      vecteur lvarxb;
      if (va.size() != 1 || va.front().type != _IDNT || (lvarxb = lvarx(b, va.front())).empty() ||
          !lop(lvarxb, at_of).empty())
        return symb_of(a, b);
    }
    if (!warn_implicit(a, b, contextptr))
      return gensizeerr("Invalid implicit multiplication for (" + a.print(contextptr) + ")(" +
    ...
    if (b.type == _VECT && b.subtype == _SEQ__VECT && b._VECTptr->size() == 1)
      return symbolic(
          ...
          makesequence(
              a,
              b._VECTptr
                  ->front())); //symbolic(at_of,gen(makevecteur(a,b._VECTptr->front()),_SEQ__VECT));
  ...
  gen _of(const gen & args, const context * contextptr) {
    ...
    */
    if (storcl_38) {
      ...
      if (qf.is_symb_of_sommet(at_double_deux_points)) {
        ...
        if (f.type == _VECT && (*f._VECTptr)[0].type == _IDNT && (*f._VECTptr)[1].type == _IDNT) {
          if (storcl_38(value, (*f._VECTptr)[0]._IDNTptr->id_name,
                        (*f._VECTptr)[1]._IDNTptr->id_name, b, true, contextptr, NULL, false)) {
            return value;
          }
        }
      }
    ...
    if (f.is_symb_of_sommet(at_struct_dot) && f._SYMBptr->feuille.type == _VECT &&
  ...
  static define_unary_function_eval4_index(163, __of, &_of, _of_s, &printasof,
                                           &texprintsommetasoperator);
  ...
  string print_with_parenthesis_if_required(const gen & g, int format, GIAC_CONTEXT) {
    if (g.type == _SYMB || g.type == _FRAC || g.type == _CPLX ||
        (g.type == _VECT && g.subtype == _SEQ__VECT))
  ...
  static string printasat_(const gen & feuille, const char * sommetstr, int format, GIAC_CONTEXT) {
    ...
    if (v.back().type != _STRNG &&
        array_start(contextptr)) { //(xcas_mode(contextptr) > 0 || abs_calc_mode(contextptr)==38)){
      gen indice;
      ...
      string s;
      return print_with_parenthesis_if_required(v.front(), format, contextptr) + '[' +
             gen2string(indice, format, contextptr) + ']';
    } else
      return print_with_parenthesis_if_required(feuille._VECTptr->front(), format, contextptr) +
             '[' + gen2string(feuille._VECTptr->back(), format, contextptr) + ']';
  }
...
#ifdef GIAC_HAS_STO_38
      alert(gettext("Python compatibility enabled. List index will start at 0, run index:=1 or "
                    "of:=1 to disable Python compatibility."),
            contextptr);
#else
    *logptr(contextptr) << gettext(
                               "// Python compatibility enabled. List index will start at 0, run "
                               "index:=1 or python_compat(0) to disable Python compatibility.")
                        << endl;
#endif
    }
    if (storcl_38) {
      if (v.front().type == _IDNT) {
        gen value;
        if (storcl_38(
                value, 0, v.front()._IDNTptr->id_name, v.back(), false, contextptr, NULL,
                false)) { //CdB v.back() is actually never used because the at_of paramter is false. Is that intended?
          return value;
        }
      }
      if (v.front().is_symb_of_sommet(at_double_deux_points)) {
        ...
        if (f[0].type == _IDNT && f[1].type == _IDNT) {
          gen value;
          if (storcl_38(
                  value, f[0]._IDNTptr->id_name, f[1]._IDNTptr->id_name, v.back(), false,
                  contextptr, NULL,
                  false)) { //CdB v.back() is actually never used because the at_of paramter is false. Is that intended?
            return value;
          }
  ...
  static define_unary_function_eval4_index(165, __at, &_at, _at_s, &printasat,
                                           &texprintsommetasoperator);
    ...
    for (; it != itend; ++it) {
      if (is_equal(*it) || it->is_symb_of_sommet(at_deuxpoints)) {
        ...
        if (f.type == _VECT && f._VECTptr->size() == 2) {
          ...
          gen bb = w.front();
          if (array_start(
                  contextptr)) { //(xcas_mode(contextptr) || abs_calc_mode(contextptr)==38)){
            if (bb.type == _VECT)
  ...
  gen _and(const gen & arg, GIAC_CONTEXT) {
    ...
    if (arg.type == _VECT && arg.subtype == _SEQ__VECT && arg._VECTptr->size() == 2 &&
      ...
      return apply(equaltosame(arg._VECTptr->front()).eval(eval_level(contextptr), contextptr),
  ...
  static define_unary_function_eval4_index(67, __and, &_and, _and_s, &printasand,
                                           &texprintsommetasoperator);
  ...
  gen _ou(const gen & arg, GIAC_CONTEXT) {
    ...
    if (arg.type == _VECT && arg.subtype == _SEQ__VECT && arg._VECTptr->size() == 2 &&
        arg._VECTptr->front().type == _VECT)
      return apply(equaltosame(arg._VECTptr->front()).eval(el, contextptr),
                   equaltosame(arg._VECTptr->back()).eval(el, contextptr), ou2);
    if (arg.type != _VECT || arg._VECTptr->empty())
  ...
  static define_unary_function_eval4_index(69, __ou, &_ou, _ou_s, &printasor,
                                           &texprintsommetasoperator);
...
static const char _xor_s[] = " xor ";
#endif
  static define_unary_function_eval4_index(117, __xor, &_xor, _xor_s, &printsommetasoperator,
                                           &texprintsommetasoperator);
  ...
  gen _min(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type != _VECT)
      return args;
    if (args.type == _POLY) {
      vector<monomial<gen>>::const_iterator it = args._POLYptr->coord.begin(),
                                            itend = args._POLYptr->coord.end();
      if (it == itend)
        return undef;
      gen m(it->value);
      for (++it; it != itend; ++it) {
  ...
  gen _gcd(const gen & args, GIAC_CONTEXT) {
    ...
    vecteur::const_iterator it = args._VECTptr->begin(), itend = args._VECTptr->end();
    if (ckmatrix(args) && itend - it == 2 && it->subtype != _POLY1__VECT &&
        (it + 1)->subtype != _POLY1__VECT)
  ...
  gen _equal(const gen & a, GIAC_CONTEXT) {
    ...
    if (a.subtype == _SEQ__VECT && calc_mode(contextptr) == 1)
      return symb_equal(a._VECTptr->front(),
                        gen(vecteur(a._VECTptr->begin() + 1, a._VECTptr->end()), a.subtype));
    return equal(gen(vecteur(a._VECTptr->begin(), a._VECTptr->end() - 1), a.subtype),
                 a._VECTptr->back(), contextptr);
  }
  static const char _equal_s[] = "=";
  static define_unary_function_eval4_index(80, __equal, &_equal, _equal_s, &printasequal,
                                           &texprintsommetasoperator);
  ...
  static define_unary_function_eval4_index(168, __equal2, &_equal2, _equal2_s,
                                           &printsommetasoperator, &texprintsommetasoperator);
  ...
  gen _same(const gen & a, GIAC_CONTEXT) {
    ...
    if (a._VECTptr->front().type == _SYMB || a._VECTptr->back().type == _SYMB) {
      if (!is_inf(a._VECTptr->front()) && !is_undef(a._VECTptr->front()) &&
          !is_inf(a._VECTptr->back()) && !is_undef(a._VECTptr->back()) &&
          a._VECTptr->front().type != _VECT && a._VECTptr->back().type != _VECT) {
        if (same_warning) {
          string s = autosimplify(contextptr);
          if (unlocalize(s) != "'simplify'") {
            *logptr(contextptr)
                << gettext("Warning, the test a==b is performed by checking\nthat the internal "
                           "representation of ")
                << s
                << gettext("(a-b) is not 0.\nTherefore a==b may return false even if a and b are "
                           "mathematically equal,\nif they have different internal "
                           "representations.\nYou can explicitly call a simplification function "
                           "like simplify(a-b)==0 to avoid this.")
                << endl;
            same_warning = false;
          }
        }
  ...
  static define_unary_function_eval4_index(148, __same, &_same, _same_s, &printassame,
                                           &texprintsommetasoperator);
  ...
  gen _irem(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    if (args.type == _VECT && args._VECTptr->size() == 2 && args._VECTptr->front().type == _INT_ &&
        args._VECTptr->back().type == _INT_) {
    ...
    if (args.type == _VECT && args._VECTptr->size() > 1 && args._VECTptr->front().type == _STRNG) {
      ...
      if (s == 4) {
        ...
        if (v1.type != _DOUBLE_ && v1.type != _STRNG)
          return gentypeerr(contextptr);
        gen v2 = evalf_double(v[2], 1, contextptr);
        if (v2.type != _DOUBLE_ && v2.type != _STRNG)
          return gentypeerr(contextptr);
        gen v3 = evalf_double(v[3], 1, contextptr);
        if (v3.type != _DOUBLE_ && v3.type != _STRNG)
          return gentypeerr(contextptr);
        if (v1.type == _DOUBLE_) {
          if (v2.type == _DOUBLE_) {
            if (v3.type == _DOUBLE_)
          ...
          if (v2.type == _DOUBLE_) {
            ...
            if (v3.type == _DOUBLE_)
              ...
              sprintf(buf, fmt, v1._STRNGptr->c_str(), v2._STRNGptr->c_str(),
                      v3._STRNGptr->c_str());
          }
        }
        return string2gen(buf, false);
    ...
    if (v.front().type == _SYMB) {
      ...
      if (v.front()._SYMBptr->sommet == at_pow && arg.type == _VECT && arg._VECTptr->size() == 2) {
        if (is_integer(arg._VECTptr->front()) && is_integer(arg._VECTptr->back()))
          return powmod(_irem(makesequence(arg._VECTptr->front(), v.back()), contextptr),
                        arg._VECTptr->back(), v.back());
        return pow(_irem(makesequence(arg._VECTptr->front(), v.back()), contextptr),
                   arg._VECTptr->back(), contextptr);
      }
      if (v.front()._SYMBptr->sommet == at_neg)
        return _irem(makesequence(simplifier((v.back() - 1) * arg, contextptr), v.back()),
                     contextptr);
      if (v.front()._SYMBptr->sommet == at_prod || v.front()._SYMBptr->sommet == at_plus) {
        return v.front()._SYMBptr->sommet(_irem(makesequence(arg, v.back()), contextptr),
                                          contextptr);
      }
  ...
  static define_unary_function_eval4_index(120, __quote_pow, &_quote_pow, _quote_pow_s,
                                           &printsommetasoperator, &texprintsommetasoperator);
  ...
  static gen symb_quorem(const gen & a, const gen & b) {
    ...
    if ((a.front().type == _POLY) && (a[1].type == _POLY)) {
      ...
      if (a.size() == 3 && a.back().type == _INT_) {
        ...
        if (!divrem1(*a.front()._POLYptr, *a[1]._POLYptr, quo, rem, args._VECTptr->back().val))
          return gensizeerr(gettext("Unable to divide, perhaps due to rounding error") +
                            a.front().print(contextptr) + " / " + a.back().print(contextptr));
    ...
    if ((aan.type == _POLY) && (bbn.type == _POLY)) {
      if (a.size() >= 3 && a.back().type == _INT_) {
        ...
        if (!divrem1(*aan._POLYptr, *bbn._POLYptr, quo, rem, args._VECTptr->back().val))
          return gensizeerr(gettext("Unable to divide, perhaps due to rounding error") +
                            aan.print(contextptr) + " / " + bbn.print(contextptr));
      ...
      u = rdiv(r2e(bbd, lv, contextptr), ad, contextptr) *
          symb_horner(*r2e(un, lvprime, contextptr)._VECTptr, lv.front());
  ...
  gen apply_unit(const gen & args, const gen_op_context & f, GIAC_CONTEXT) {
    return symbolic(at_unit, gen(makevecteur(f(args._SYMBptr->feuille[0], contextptr),
                                             args._SYMBptr->feuille[1]),
                                 _SEQ__VECT));
  }
    ...
    */
    ...
    if (chk.type == _IDNT && is_linear_wrt(args, chk, a, b, contextptr)) {
      ...
      if ((g2.type == _VECT) && (g2.subtype == _ASSUME__VECT)) {
        ...
        if ((v.size() == 3) &&
            (v.front() == vecteur(0) || v.front() == _DOUBLE_ || v.front() == _ZINT ||
             v.front() == _SYMB || v.front() == 0) &&
            (v[1].type == _VECT && v[1]._VECTptr->size() == 1 &&
             v[1]._VECTptr->front().type == _VECT)) {
          ...
          if (v1.size() == 2) {
            ...
            if (Af == Bf)
              return Af;
            if (Af == Bf + 1 && is_zero(ratnormal(A - Af, contextptr)) && v[2].type == _VECT &&
                equalposcomp(*v[2]._VECTptr, v1[0]))
              return Bf;
  ...
  static gen taylor_floor(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                          int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0; // no symbolic preprocessing
    shift_coeff = 0;
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__floor, &_floor, (size_t)&D_at_signunary_function_ptr,
                                           &taylor_floor, _floor_s);
#else
static define_unary_function_eval_taylor(__floor, &_floor, D_at_sign, &taylor_floor, _floor_s);
  ...
  static gen taylor_ceil(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                         int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0; // no symbolic preprocessing
    shift_coeff = 0;
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__ceil, &_ceil, (size_t)&D_at_signunary_function_ptr,
                                           &taylor_ceil, _ceil_s);
#else
static define_unary_function_eval_taylor(__ceil, &_ceil, D_at_sign, &taylor_ceil, _ceil_s);
  ...
  static gen taylor_round(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                          int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0; // no symbolic preprocessing
    shift_coeff = 0;
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__round, &_round, (size_t)&D_at_signunary_function_ptr,
                                           &taylor_round, _round_s);
#else
static define_unary_function_eval_taylor(__round, &_round, D_at_sign, &taylor_round, _round_s);
  ...
  if (args.type == _STRNG && args.subtype == -1)
    return args;
  if (debug_infolevel && (args.type == _IDNT) && args._IDNTptr->localvalue &&
      (!args._IDNTptr->localvalue->empty()))
    *logptr(contextptr) << gettext("Local var protected ")
                        << (*args._IDNTptr->localvalue)[args._IDNTptr->localvalue->size() - 2].val
                        << endl;
  gen tmp = args.eval(eval_level(contextptr), contextptr);
  string nl("\n"), sep(",");
  ...
  if (tmp.type == _VECT && !tmp._VECTptr->empty() &&
      tmp._VECTptr->front() == gen("Unquoted", contextptr)) {
    vecteur & v = *tmp._VECTptr;
    int s = int(v.size());
    for (int i = 1; i < s; ++i)
      *logptr(contextptr) << (v[i].type == _STRNG ? (*v[i]._STRNGptr)
                                                  : unquote(v[i].print(contextptr)));
  } else {
...
const unary_function_eval __print(1, &_print, _print_s, &printasprint);
  ...
  gen _is_prime(const gen & args0, GIAC_CONTEXT) {
    ...
    int certif = 0;
#ifdef HAVE_LIBPARI
    if (args0.type == _VECT && args0.subtype == _SEQ__VECT && args0._VECTptr->size() == 2 &&
        args0._VECTptr->back().type == _INT_) {
  ...
  define_unary_function_ptr5(at_is_pseudoprime, alias_at_is_pseudoprime, &__is_pseudoprime, 0,
                             true);
  ...
  define_unary_function_ptr5(at_legendre_symbol, alias_at_legendre_symbol, &__legendre_symbol, 0,
                             true);
  ...
  gen ichinrem2(const gen & a_orig, const gen & b_orig) {
    ...
    if (l.empty()) {
      if (!check_2d_vecteur(a) || !check_2d_vecteur(b))
    ...
    if (a.type != _VECT || b.type != _VECT) {
      ...
      gen ax = _e2r(makevecteur(a_orig, x), context0),
          bx = _e2r(makevecteur(b_orig, x), context0); // ok
      ...
      if (b0.type == _MOD)
        ...
        return gensizeerr(gettext("Expecting modular coeff"));
      gen mn = lcm(m, n);
      const_iterateur it = ax._VECTptr->begin(), itend = ax._VECTptr->end(),
                      jt = bx._VECTptr->begin();
      vecteur res;
      for (; as > bs; --as, ++it) {
    ...
    if (a.type == _VECT && a._VECTptr->size() == 2 && b.type == _VECT && b._VECTptr->size() == 2) {
      ...
      gen ax = _e2r(makevecteur(a._VECTptr->front(), x), context0),
          bx = _e2r(makevecteur(b._VECTptr->front(), x), context0); // ok
      if (ax.type != _VECT || bx.type != _VECT)
  ...
  gen _ichinrem(const gen & args, GIAC_CONTEXT) {
    ...
    if (res.type == _VECT && res._VECTptr->size() == 2 && is_integer(res._VECTptr->front()) &&
        is_integer(res._VECTptr->back()))
      res._VECTptr->front() =
          _irem(makesequence(res._VECTptr->front() + res._VECTptr->back(), res._VECTptr->back()),
                contextptr);
    return res;
  }
  ...
  gen _chinrem(const gen & args, GIAC_CONTEXT) {
    ...
    if (!check_2d_vecteur(a) || !check_2d_vecteur(b))
      return gensizeerr(contextptr);
    if ((a._VECTptr->front().type != _VECT) || (a._VECTptr->back().type != _VECT) ||
        (b._VECTptr->front().type != _VECT) || (b._VECTptr->back().type != _VECT)) {
      vecteur lv;
    ...
    return makevecteur(gen(chinrem(*a._VECTptr->front()._VECTptr, *b._VECTptr->front()._VECTptr,
                                   *a._VECTptr->back()._VECTptr, *b._VECTptr->back()._VECTptr, 0),
                           _POLY1__VECT),
                       gen(produit, _POLY1__VECT));
  }
  ...
  static define_unary_function_eval5(__factorial, &_factorial, D_at_factorial, _factorial_s,
                                     &printasfactorial, 0);
  ...
  gen _perm(const gen & args, GIAC_CONTEXT) {
    ...
    if ((args._VECTptr->front().type != _INT_) || (args._VECTptr->back().type != _INT_))
      return _factorial(args._VECTptr->front(), contextptr) /
             _factorial(args._VECTptr->front() - args._VECTptr->back(), contextptr);
    if (args._VECTptr->front().val < args._VECTptr->back().val)
      return zero;
    if (args._VECTptr->front().val < 0)
      return undef;
    return perm((unsigned long int)args._VECTptr->front().val,
                (unsigned long int)args._VECTptr->back().val);
  }
  ...
  bool need_workaround(const gen & g) {
    if (g.type == _REAL ||
        (g.type == _CPLX && g._CPLXptr->type == _REAL && (g._CPLXptr + 1)->type == _REAL))
      return false;
    if (g.type <= _CPLX)
      return g != 0 && g / g != 1;
  ...
  gen _evalf(const gen & a_orig, GIAC_CONTEXT) {
    ...
    if (a.type == _STRNG && a.subtype == -1)
      return a;
    if (is_equal(a) && a._SYMBptr->feuille.type == _VECT &&
        a._SYMBptr->feuille._VECTptr->size() == 2) {
      vecteur & v(*a._SYMBptr->feuille._VECTptr);
      return symb_equal(
          evalf(v.front(), 1, contextptr),
          evalf(
              v.back(), 1,
              contextptr)); //return symbolic(at_equal,gen(makevecteur(evalf(v.front(),1,contextptr),evalf(v.back(),1,contextptr)),_SEQ__VECT));
    }
    gen res;
    int ndigits = decimal_digits(contextptr);
    if (a.type == _VECT && a.subtype == _SEQ__VECT && a._VECTptr->size() == 2 &&
        a._VECTptr->back().type == _INT_) {
  ...
  gen _eval(const gen & a, GIAC_CONTEXT) {
    ...
    if (is_equal(a) && a._SYMBptr->feuille.type == _VECT &&
      ...
      vecteur & v(*a._SYMBptr->feuille._VECTptr);
      return symb_equal(
          eval(v.front(), eval_level(contextptr), contextptr),
          eval(
              v.back(), eval_level(contextptr),
              contextptr)); // symbolic(at_equal,gen(makevecteur(eval(v.front(),eval_level(contextptr),contextptr),eval(v.back(),eval_level(contextptr),contextptr)),_SEQ__VECT));
    }
    if (a.type == _VECT && a.subtype == _SEQ__VECT && a._VECTptr->size() == 2) {
      gen a1 = a._VECTptr->front(), a2 = a._VECTptr->back();
      if (a2.type == _INT_)
  ...
  static define_unary_function_eval4_index(108, __ampersand_times, &_ampersand_times,
                                           _ampersand_times_s, &printsommetasoperator,
                                           &texprintsommetasoperator);
    ...
    if (s == 2) {
      ...
      if (e.type == _VECT) {
        ...
        for (; it != itend; ++it) {
          if (it->type != _SYMB)
            continue;
          if (it->_SYMBptr->sommet != at_equal && it->_SYMBptr->sommet != at_equal2 &&
              it->_SYMBptr->sommet != at_same)
            continue;
      ...
      if (e.type != _SYMB)
        return gentypeerr(contextptr);
      if (e._SYMBptr->sommet != at_equal && e._SYMBptr->sommet != at_equal2 &&
          e._SYMBptr->sommet != at_same)
        return gensizeerr(contextptr);
      return subst(v.front(), e._SYMBptr->feuille._VECTptr->front(),
                   e._SYMBptr->feuille._VECTptr->back(), false, contextptr);
    }
    if (s < 3)
  ...
  string version() {
    return string("giac ") + GIAC_VERSION +
           string(", (c) B. Parisse and R. De Graeve, Institut Fourier, Universite de Grenoble I");
  }
  gen _version(const gen & a, GIAC_CONTEXT) {
    if (a.type == _STRNG && a.subtype == -1)
      return a;
    return string2gen(gettext("Powered by Giac 1.x, B. Parisse and R. De Graeve, Institut Fourier, "
  ...
  bool need_parenthesis(const gen & g) {
    ...
    if (u == at_pow || u == at_division || u == at_prod)
      return false;
    if (u == at_neg || u == at_inv || u == at_minus || u == at_and || u == at_ou || u == at_xor ||
        u == at_same || u == at_equal || u == at_equal2 || u == at_superieur_egal ||
        u == at_superieur_strict || u == at_inferieur_egal || u == at_inferieur_strict)
      return true;
  ...
  static const double LC1 = 0.08333333333333333, LC2 = -0.002777777777777778,
                      LC3 = 7.936507936507937E-4, LC4 = -5.952380952380953E-4;
  static const double L9[] = {0.99999999999980993227684700473478,
...
static const long_double LC1 = 0.08333333333333333L, LC2 = -0.002777777777777778L,
                         LC3 = 7.936507936507937E-4L, LC4 = -5.952380952380953E-4L;
static const long_double L9[] = {0.99999999999980993227684700473478L,
  ...
  double lngamma(double X) {
    ...
    res = std::log(M_PIL) - std::log(std::sin(M_PIL * x)) - lngamma(1.L - x);
      ...
      if (x < 20) {
        ...
        long_double a = L9[0] + L9[1] / (x + 1.0) + L9[2] / (x + 2.0) + L9[3] / (x + 3.0) +
                        L9[4] / (x + 4.0) + L9[5] / (x + 5.0) + L9[6] / (x + 6.0) +
                        L9[7] / (x + 7.0) + L9[8] / (x + 8.0);
//	long_double a = L9[0];
//	for (int i = 1; i < 9; ++i) {
...
//	}
      ...
      res = (LN_SQRT2PI + std::log(a) - 7.L) + (x + .5L) * (std::log(x + 7.5L) - 1.L);
        ...
        long_double
          ...
          r1 = 1.L / x,
#endif
            r2 = r1 * r1, r3 = r1 * r2, r5 = r2 * r3, r7 = r3 * r3 * r1;
#ifndef HAVE_LONG_DOUBLE
...
#ifdef FREERTOS
      res = std::log(M_PI) -
            std::log(complex_long_double(std::sin(M_PI * x.real()) * std::cosh(M_PI * x.imag()),
                                         std::cos(M_PI * x.real()) * std::sinh(M_PI * x.imag()))) -
            lngamma(1. - x);
#else
  ...
  static gen taylor_lnGamma_minus(const gen & lim_point, const int ordre,
                                  const unary_function_ptr & f, int direction, gen & shift_coeff,
                                  GIAC_CONTEXT) {
    if (ordre < 0)
      return 0;
  ...
  define_partial_derivative_onearg_genop(D_at_lnGamma_minus, " D_at_lnGamma_minus",
                                         &d_lnGamma_minus);
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__lnGamma_minus, &_lnGamma_minus,
                                           (size_t)&D_at_lnGamma_minusunary_function_ptr,
                                           &taylor_lnGamma_minus, _lnGamma_minus_s);
#else
static define_unary_function_eval_taylor(__lnGamma_minus, &_lnGamma_minus, D_at_lnGamma_minus,
                                         &taylor_lnGamma_minus, _lnGamma_minus_s);
#endif
  ...
  static gen Gamma_replace(const gen & g, GIAC_CONTEXT) {
    return symb_exp((g + minus_one_half) * symb_ln(g) - g) *
           symb_exp(_lnGamma_minus(g, contextptr));
  }
  static gen taylor_Gamma(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                          int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0) {
  ...
  gen Gamma(const gen & x, GIAC_CONTEXT) {
    if (x.type == _VECT && x.subtype == _SEQ__VECT && x._VECTptr->size() >= 2) {
      ...
      if (s.type == _DOUBLE_ && z.type == _DOUBLE_) {
        ...
        if (res == -1) {
          return regu ? 1
                      : Gamma(s._DOUBLE_val, contextptr) -
                            lower_incomplete_gamma(s._DOUBLE_val, z._DOUBLE_val, regu, contextptr);
          //return gensizeerr(contextptr);
        }
        return res;
      }
...
#endif
    if (x.type == _DOUBLE_ ||
        (x.type == _CPLX && (x._CPLXptr->type == _DOUBLE_ || (x._CPLXptr + 1)->type == _DOUBLE_))) {
#if 1
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__Gamma, &_Gamma, (size_t)&D_at_Gammaunary_function_ptr,
                                           &taylor_Gamma, _Gamma_s);
#else
static define_unary_function_eval_taylor(__Gamma, &_Gamma, D_at_Gamma, &taylor_Gamma, _Gamma_s);
  ...
  double upper_incomplete_gammad(double s, double z, bool regularize) {
    ...
    if (s == int(s) && s > 0)
      return regularize ? poisson_cdf(z, int(s - 1))
                        : poisson_cdf(z, int(s - 1)) * std::exp(lngamma(s));
#if 0 // not tested
    ...
    long_double Pm2 = 0, Pm1 = 1, Pm, Qm2 = 1, Qm1 = z, Qm, a2m1 = 1, a2m = 1 - s, b2m1 = z,
                b2m = 1, pmqm;
  ...
  gen gammaetoile(const gen & a, const gen & z, GIAC_CONTEXT) {
    ...
    double eps2 = epsilon(contextptr);
    eps2 = eps2 * eps2;
    if (eps2 <= 0)
      eps2 = 1e-14;
    for (int n = 0;;) {
  ...
  gen lower_incomplete_gamma(double s, double z, bool regularize,
                             GIAC_CONTEXT) { // regularize=true by default
  ...
  gen _lower_incomplete_gamma(const gen & args, GIAC_CONTEXT) {
    ...
    if ((s == 2 || s == 3) && v[0].type == _DOUBLE_ && v[1].type == _DOUBLE_)
      return lower_incomplete_gamma(v[0]._DOUBLE_val, v[1]._DOUBLE_val,
  ...
  static define_unary_function_eval(__lower_incomplete_gamma, &_lower_incomplete_gamma,
                                    _lower_incomplete_gamma_s);
  define_unary_function_ptr5(at_lower_incomplete_gamma, alias_at_lower_incomplete_gamma,
                             &__lower_incomplete_gamma, 0, true);
  ...
  // diGamma function
  static gen taylor_Psi_minus_ln(const gen & lim_point, const int ordre,
                                 const unary_function_ptr & f, int direction, gen & shift_coeff,
                                 GIAC_CONTEXT) {
    if (ordre < 0)
      return 0;
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__Psi_minus_ln, &_Psi_minus_ln,
                                           (size_t)&D_at_Psi_minus_lnunary_function_ptr,
                                           &taylor_Psi_minus_ln, _Psi_minus_ln_s);
#else
static define_unary_function_eval_taylor(__Psi_minus_ln, &_Psi_minus_ln, D_at_Psi_minus_ln,
                                         &taylor_Psi_minus_ln, _Psi_minus_ln_s);
#endif
  ...
  static gen taylor_Psi(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                        int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0) {
      return 0; // statically handled now
  ...
  gen Psi(const gen & x, GIAC_CONTEXT) {
    ...
    if (x.type == _FRAC) {
      ...
      if (num.type == _INT_ && den.type == _INT_ && den.val < 13) {
        ...
        res = k * res - cst_euler_gamma - ln(2 * k, contextptr) -
              cst_pi / 2 / tan(m * cst_pi / k, contextptr);
        gen res1;
        for (int n = 1; n <= (k - 1) / 2; n++) {
          res1 += cos(2 * n * m * cst_pi / k, contextptr) *
                  ln(sin(n * cst_pi / k, contextptr), contextptr);
        }
        return res + 2 * res1;
      }
    }
  ...
  double bernoulli_tab[] = {
      1.000000000000000,      -0.50000000000000000, 0.1666666666666667,    0.0000000000000000,
      -0.3333333333333333e-1, 0.0000000000000000,   0.2380952380952381e-1, 0.0000000000000000,
      -0.3333333333333333e-1, 0.0000000000000000,   0.7575757575757576e-1, 0.0000000000000000,
      -0.2531135531135531,    0.0000000000000000,   1.166666666666667,     0.0000000000000000,
      -7.092156862745098,     0.0000000000000000,   0.5497117794486216e2,  0.0000000000000000,
      -0.5291242424242424e3,  0.0000000000000000,   0.6192123188405797e4,  0.0000000000000000,
      -0.8658025311355311e5,  0.0000000000000000,   0.1425517166666667e7,  0.0000000000000000,
      -0.2729823106781609e8,  0.0000000000000000,   0.6015808739006424e9};
  ...
  gen Psi(const gen & x, int n, GIAC_CONTEXT) {
    ...
    if ((x.type == _INT_) && (x.val < 10000)) {
      ...
      if (n % 2)
        return factorial(n) *
               (Zeta(n + 1, contextptr) - sum_loop(pow(tt, -n - 1), tt, 1, x.val - 1, contextptr));
      else
        return -factorial(n) *
               (Zeta(n + 1, contextptr) - sum_loop(pow(tt, -n - 1), tt, 1, x.val - 1, contextptr));
    }
...
#ifdef GIAC_HAS_STO_38
  define_unary_function_eval_taylor(__Psi, &_Psi, (size_t)&D_at_Psiunary_function_ptr, &taylor_Psi,
                                    _Psi_s);
#else
define_unary_function_eval_taylor(__Psi, &_Psi, D_at_Psi, &taylor_Psi, _Psi_s);
  ...
  gen _normalmod(const gen & g, GIAC_CONTEXT) {
    ...
    if (is_equal(f))
      return symb_equal(
          _normalmod(makevecteur(f._SYMBptr->feuille[0], g._VECTptr->back()), contextptr),
    ...
    if (f.type == _MOD) {
      if (warnmod) {
        *logptr(contextptr)
            << "// Warning: a % b returns the class of a in Z/bZ. Use irem(a,b) for remainder"
            << endl;
        warnmod = false;
      }
      f = *f._MODptr;
...
static const char _normalmod_s[] = "%";
#endif
  static define_unary_function_eval4_index(166, __normalmod, &_normalmod, _normalmod_s,
                                           &printsommetasnormalmod, &texprintsommetasoperator);
  ...
  gen alternate_series(const gen & a, const gen & x, int n, GIAC_CONTEXT) {
    gen d = normal((pow(3 + 2 * sqrt(2, contextptr), n) + pow(3 - 2 * sqrt(2, contextptr), n)) / 2,
                   contextptr);
    gen p = 1;
    gen c = d - p;
    ...
    if (s.type == _DOUBLE_ || (s.type == _CPLX)) {
      ...
      if (rx._DOUBLE_val < 0.5) {
        if (ndiff) {
          identificateur id(" ");
          gen t(id), zeta;
          zeta = derive((1 - pow(2, 1 - t, contextptr)) * pow(2 * cst_pi, t, contextptr) / cst_pi *
                            sin(cst_pi * t / 2, contextptr) * symbolic(at_Gamma, 1 - t) *
                            symbolic(at_Zeta, 1 - t),
                        t, ndiff, contextptr);
          zeta = subst(zeta, t, s, false, contextptr);
          return zeta;
        }
        gen zeta1 = Eta(1 - s, 0, contextptr) / (1 - pow(2, s, contextptr));
        gen zetas = pow(2, s, contextptr) * pow(cst_pi, s - 1, contextptr) *
                    sin(cst_pi * s / 2, contextptr) * Gamma(1 - s, contextptr) * zeta1;
      ...
      double n = (std::log10(3 * (1 + 2 * y) * std::exp(y * M_PI / 2)) + ndigits) /
                 std::log10(3. + std::sqrt(8.));
      identificateur idx(" ");
      gen x(idx);
      gen res = alternate_series(inv(pow(idx + 1, s, contextptr), contextptr) *
                                     pow(-ln(idx + 1, contextptr), ndiff, contextptr),
                                 idx, int(std::ceil(n)), contextptr);
  ...
  gen Zeta(const gen & x, int ndiff, GIAC_CONTEXT) {
    ...
    if (x.type == _DOUBLE_ || (x.type == _CPLX && x.subtype == _DOUBLE_)) {
      ...
      if (rex._DOUBLE_val < 0.5) {
        ...
        zeta = derive(pow(2 * cst_pi, t, contextptr) / cst_pi * sin(cst_pi * t / 2, contextptr) *
                          symbolic(at_Gamma, 1 - t) * symbolic(at_Zeta, 1 - t),
                      t, ndiff, contextptr);
  ...
  gen Zeta(const gen & x, GIAC_CONTEXT) {
    ...
    if ((x.type == _INT_)) {
      ...
      if (n % 2)
        ...
        return pow(cst_pi, n) * ratnormal(abs(bernoulli(x), contextptr) *
                                              rdiv(pow(plus_two, n - 1), factorial(n), contextptr),
                                          contextptr);
    }
#ifdef HAVE_LIBGSL
    if (x.type == _DOUBLE_)
  ...
  static gen taylor_Zeta(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                         int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0; // no symbolic preprocessing
    if (is_one(lim_point)) {
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__Zeta, &_Zeta, (size_t)&D_at_Zetaunary_function_ptr,
                                           &taylor_Zeta, _Zeta_s);
#else
...
static define_unary_function_eval3(__Eta, &_Eta, D_at_Eta, _Eta_s);
  ...
  // error function
  static gen taylor_erfs(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                         int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0;
    if (!is_inf(lim_point))
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__erfs, &_erfs, (size_t)&D_at_erfsunary_function_ptr,
                                           &taylor_erfs, _erfs_s);
#else
static define_unary_function_eval_taylor(__erfs, &_erfs, D_at_erfs, &taylor_erfs, _erfs_s);
  ...
  static gen taylor_erf(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                        int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0) {
      return 0; // statically handled now
  ...
  static gen erf0(const gen & x, gen & erfc, GIAC_CONTEXT) {
    ...
    int newprec, nbitsz = int(z._DOUBLE_val * z._DOUBLE_val / std::log(2.)),
                 prec2 = int(prec * std::log(10.0) / std::log(2.0) + .5);
    if (nbitsz > prec2) {
      // use asymptotic expansion at z=inf
      z = accurate_evalf(inv(x, contextptr), prec2);
      gen z2 = z * z / 2, res = 0, pi = inv(accurate_evalf(plus_two, prec2), contextptr),
          eps = accurate_evalf(pow(10, -prec, contextptr), prec2) / 2;
      for (int n = 0;; ++n) {
        res += pi;
        pi = -(2 * n + 1) * z2 * pi;
      ...
      erfc = evalf(2 * inv(sqrt(cst_pi, contextptr), contextptr), 1, contextptr) *
             exp(-inv(z * z, contextptr), contextptr) * z * res;
      return 1 - erfc;
    }
    if (z._DOUBLE_val > 1)
...
#ifdef GIAC_HAS_STO_38
  define_unary_function_eval_taylor(__erf, &_erf, (size_t)&D_at_erfunary_function_ptr, &taylor_erf,
                                    _erf_s);
#else
...
static define_unary_function_eval3(__erfc, &_erfc, D_at_erfc, _erfc_s);
  ...
  static bool sici(const gen & z0, gen & siz, gen & ciz, int prec, int mode, GIAC_CONTEXT) {
	...
	*/
      ...
      if (Z > 8 && Z <= 12) {
        ...
        long_double tabsi[] = {
            -0.1189416530979229549237628888274e-25, 0.1535274533580010929112764295251e-23,
            0.5949595042899632105631168585106e-23,  -0.8443813569533766615075729254715e-21,
            -0.2314493549701736156628360858149e-20, 0.3873999840160633357506392314227e-18,
            0.6314325721978121436699115557946e-18,  -0.1454512706077874699091092895960e-15,
            -0.7966560468972510895796615830931e-16, 0.4362654888706402638259447137657e-13,
            -0.2168961216095219762135154593351e-13, -0.1013156299801104705978428503741e-10,
            0.1511362807589119975552764377684e-10,  0.1746079592768605042057230051441e-8,
            -0.4215112103088864749315276215891e-8,  -0.2100827763936543847478539598343e-6,
            0.6768578499749603731655034337648e-6,   0.1604768832104729109406272571796e-4,
            -0.6129221770634575410555873017014e-4,  -0.6629459359846050378314018176664e-3,
            0.2619937628043294083259734576549e-2,   0.1168258324144788179314042496964e-1,
            -0.3923347089937577354591945908199e-1,  -0.5440211108893698134047476618518e-1,
            1.658347594218874049330971879387};
        // evalf(symb2poly(subst(d,X,10),h))
        long_double tabci[] = {
            -0.1031363603561483377414206719978e-24, 0.1612636587877966055062771505636e-24,
            0.3224608155602632232346779331477e-22,  0.1692359618263088728225313523766e-21,
            -0.1902123006508838353447776226346e-19, -0.3515542663511577547829464092398e-19,
            0.7651991655644681341759387392123e-17,  0.8949035335940300076443987350711e-17,
            -0.2601535596364695291068633323882e-14, 0.1492297097331336833098183955735e-16,
            0.6873241144777191906667850002758e-12,  -0.6815990267379124633604933192924e-12,
            -0.1385917806100726110447949877846e-9,  0.2729216217490738797188995072400e-9,
            0.2012042413927794669971423802132e-7,   -0.5698511913235728873777759634867e-7,
            -0.1960205632344228431569991958688e-5,  0.6982250568929225912819532196480e-5,
            0.1127699306487082097807313715561e-3,   -0.4465373163022154950275303555869e-3,
            -0.3158611974102019356519036678335e-2,  0.1189143127195082400887895227495e-1,
            0.3139641318985075293153170283171e-1,   -0.8390715290764524522588639478252e-1,
            -0.4545643300445537263453282995265e-1};
      ...
      if (Z > 12 && Z <= 16) {
        ...
        long_double tabsi[] = {
            0.4669904672048171207530336926784e-25,  -0.9121786080411940331362079921979e-24,
            -0.2670914489796227617771058426181e-22, 0.5191793400822084981037748064947e-21,
            0.1272661806026797328866077155393e-19,  -0.2467115711789321804079825604360e-18,
            -0.4949985288801387245191790228644e-17, 0.9598983157925822187983801948972e-16,
            0.1531267131408434243475192526878e-14,  -0.2983793513482513341175920391396e-13,
            -0.3640741847215956508751181538961e-12, 0.7178248462975320846576276463699e-11,
            0.6346880693851116027919629277690e-10,  -0.1280755925139665180236732107401e-8,
            -0.7574841879361234814393864898316e-8,  0.1596987755895423889046710494666e-6,
            0.5558236361973162161308791480329e-6,   -0.1276894967030880532682787806556e-4,
            -0.2074774698099097315147678882137e-4,  0.5764575129603710060263581799849e-3,
            0.2308201450150731015736777254153e-3,   -0.1190515482960545313209358846732e-1,
            0.2356412497996938805126656809673e-2,   0.7075766826391930770538248600113e-1,
            1.556211050077665053703631892805};
        // evalf(symb2poly(subst(d,X,14),h))
        long_double tabci[] = {
            0.3590233410769916717046880365555e-25,  0.1141530173081541381259185089975e-23,
            -0.2223749846955296806257123587014e-22, -0.5971404695883450494036609310213e-21,
            0.1158813189176036949355044721895e-19,  0.2578286744261932540063357506560e-18,
            -0.4996649806240905822071761629155e-17, -0.8975849912962755765138097664122e-16,
            0.1743615446376990117868764420851e-14,  0.2446425362039524592303128232180e-13,
            -0.4789979360061655431004102341364e-12, -0.5015267234200784104185754752298e-11,
            0.9985346701639551763361641926310e-10,  0.7310382166844782587890268654904e-9,
            -0.1502609970889405144661999732289e-7,  -0.6957715037844896183079492333545e-7,
            0.1519752625305293353227780696896e-5,   0.3762397782384872898110463611961e-5,
            -0.9310463095669218193046370118801e-4,  -0.8685445941902185669380116006632e-4,
            0.2944299062831149098901196099828e-2,   0.7349281020023392469738413739405e-4,
            -0.3572765356616331098087728605326e-1,  0.9766944157702399589209205476559e-2,
            0.6939635592758454727438326824349e-1};
        unsigned N = sizeof(tabsi) / sizeof(long_double);
        for (unsigned i = 0; i < N; i++) {
          ress *= Zl;
          ress += tabsi[i];
      ...
      if (Z > 16 && Z <= 20) {
        ...
        long_double tabsi[] = {
            -0.5458114686729331288343465771243e-25, -0.8535297797031477670863377809864e-25,
            0.3197608273507048053143023106525e-22,  0.1246720240695204431925986220740e-22,
            -0.1566961070193290350813150654958e-19, 0.1120194984050557421284180749579e-19,
            0.6303732529750142701943836944779e-17,  -0.1093865529886308098492342051935e-16,
            -0.2034127294195017035840150503185e-14, 0.5391577496552481117289518608625e-14,
            0.5113326950563000131064845650605e-12,  -0.1755022270416406807283368583049e-11,
            -0.9642855115888351354955178064729e-10, 0.3896583006063678015119758986001e-9,
            0.1297996094042326342264874814159e-7,   -0.5741523749720224155210168829004e-7,
            -0.1165550911079956021485851292181e-5,  0.5260587329402175094599529167340e-5,
            0.6336730653896082851113120333026e-4,   -0.2682059741680869878136005839481e-3,
            -0.1788149401756335521591446074162e-2,  0.6231324073036488917300258847156e-2,
            0.1950106172093382517043203223869e-1,   -0.4172151370953756131945311580259e-1,
            1.536608096861185462361173893885};
        // evalf(symb2poly(subst(d,X,18),h))
        long_double tabci[] = {
            0.4605275954646862944758091918129e-26,  -0.1349519299011741936395476560680e-23,
            -0.1307682310402805091861215069192e-23, 0.7246129197780950670155731691447e-21,
            -0.1246271694903983843859628582537e-21, -0.3225645499768452910431606017148e-18,
            0.3989357101823634351098507496171e-18,  0.1165949042798889880769685702457e-15,
            -0.2573970823221046721180373208376e-15, -0.3334412559655945734048458981847e-13,
            0.1020420669194920098922521974402e-12,  0.7298982331099322669182907795178e-11,
            -0.2745289334776461767357052328935e-10, -0.1171271721735576258954174760357e-8,
            0.4994621772350737506214065546293e-8,   0.1300541789133113153488950327832e-6,
            -0.5864843122180159884437825091296e-6,  -0.9221666449442426870101941344121e-5,
            0.4080390556697611593508057267335e-4,   0.3702810510394427353858531043357e-3,
            -0.1453024604178293371013986047647e-2,  -0.6848923209258520415117450659078e-2,
            0.1984174958896001500414615659760e-1,   0.3668426156911556360089444693281e-1,
            -0.4347510299950100478344114920850e-1};
        unsigned N = sizeof(tabsi) / sizeof(long_double);
        for (unsigned i = 0; i < N; i++) {
          ress *= Zl;
          ress += tabsi[i];
      ...
      if (Z > 20 && Z <= 24) {
        ...
        long_double tabsi[] = {
            0.3374532347188926046754549614410e-25,  0.9070719831057349388641148466696e-24,
            -0.2050470901003743543468128570765e-22, -0.4594235790411492575575248293744e-21,
            0.1043077621302187913107676015839e-19,  0.1910932463670378689024003307389e-18,
            -0.4360804036884663718921536467101e-17, -0.6379227769542456535760586922387e-16,
            0.1464704096058402216999762277897e-14,  0.1660737164480043388814198356103e-13,
            -0.3842072518362213364190884929092e-12, -0.3249870299707868231630032306411e-11,
            0.7591535353403568501001826557431e-10,  0.4554066973680817003059315739181e-9,
            -0.1077692372064168550494073939261e-7,  -0.4274452798917825988601610781581e-7,
            0.1030486252719749122401833140117e-5,   0.2434192199039423608811016740448e-5,
            -0.6042868558775171616279520355687e-4,  -0.7128407780774990405916452478794e-4,
            0.1868111001271415320776084256504e-2,   0.7554565401849909491334584321080e-3,
            -0.2271723850350373234098156405564e-1,  -0.4023322404729034509859207643533e-3,
            1.616083736594366543114431027190};
        // evalf(symb2poly(subst(d,X,22),h))
        long_double tabci[] = {
            -0.3765941636516127667565314425887e-25, 0.8496429623966734299562910340895e-24,
            0.2089658856058364573723209071877e-22,  -0.4733667956308362832271784627271e-21,
            -0.9616010057686047674500629395344e-20, 0.2188568968552838678620763986263e-18,
            0.3594653788547495746845572205294e-17,  -0.8227046014999096676822143059749e-16,
            -0.1063984308386789623300816541173e-14, 0.2451691569424535439026086961327e-13,
            0.2414081477262495987003288682969e-12,  -0.5610174912354327213432820922583e-11,
            -0.4025699100579353159380991679438e-10, 0.9460092423484752010231070364358e-9,
            0.4662816039604651635851886085410e-8,   -0.1112697436829371328490149410190e-6,
            -0.3461508105965530058056263010906e-6,  0.8452332929171272742294005025613e-5,
            0.1452920166854777888625357916022e-4,   -0.3688187418989882360609440618036e-3,
            -0.2737432060552935755550251671918e-3,  0.7538061305932840665075723970069e-2,
            0.1234183502875539666044793790453e-2,   -0.4545276483611986938428066996417e-1,
            0.1640691915737749726680980654224e-2};
        unsigned N = sizeof(tabsi) / sizeof(long_double);
        for (unsigned i = 0; i < N; i++) {
          ress *= Zl;
          ress += tabsi[i];
      ...
      if (Z > 24 && Z <= 28) {
        ...
        long_double tabsi[] = {
            0.1887595283929249736721564257313e-26,  -0.1139208683110194949664423805669e-23,
            0.1279293933254064768731875653817e-24,  0.5994732455107313947881755894338e-21,
            -0.6938324133833276535897299455340e-21, -0.2609102593630429565352138793600e-18,
            0.5435603049161089391681629274496e-18,  0.9202351759037677282055983009108e-16,
            -0.2642554055043397588710384653172e-15, -0.2565196994817985742348447471878e-13,
            0.8979673506711721488705329079164e-13,  0.5477157384399615846158662911243e-11,
            -0.2160870416302321163457598611639e-10, -0.8604342982500329851062267981807e-9,
            0.3594354283136494037733432184009e-8,   0.9424490157133423171849887794118e-7,
            -0.3925808731949007274318185773343e-6,  -0.6671455253428628462019837043580e-5,
            0.2584931618742702225630929467268e-4,   0.2717002055000108101387898426941e-3,
            -0.8869394862544894803875105833456e-3,  -0.5192726828102161497754294326711e-2,
            0.1187673367608361403346165544076e-1,   0.2932917117229241298137781896944e-1,
            1.544868862986338557887737260292};
        // evalf(symb2poly(subst(d,X,26),h))
        long_double tabci[] = {
            0.4649965583842175529710094230187e-25,  0.2092492486523365857112019931102e-25,
            -0.2673016958726137563529919009844e-22, 0.1734025691834661144821904431335e-22,
            0.1282183044327200389255292828905e-19,  -0.2098089236197822391981775009963e-19,
            -0.5037728169303807883904931875640e-17, 0.1257389975122490684981594653098e-16,
            0.1585106167468127577346565423777e-14,  -0.5084215538447167537343863000543e-14,
            -0.3884144934780823340053700189354e-12, 0.1455636906316532474122453840240e-11,
            0.7154603949717881354941340050201e-10,  -0.2926061246791173118887623808558e-9,
            -0.9458827111752210232753218600667e-8,  0.3976537604156996965393015686438e-7,
            0.8424410070112619898093729646951e-6,   -0.3418064228754065522708009442873e-5,
            -0.4606856152608972144114035752184e-4,  0.1664083688146614131038736660077e-3,
            0.1330470954446840977471854832752e-2,   -0.3758634727512557357034117912936e-2,
            -0.1514307620917034875601537686776e-1,  0.2488151239725539779697630391881e-1,
            0.2829515103175713190842112993963e-1};
        unsigned N = sizeof(tabsi) / sizeof(long_double);
        for (unsigned i = 0; i < N; i++) {
          ress *= Zl;
          ress += tabsi[i];
      ...
      if (Z > 28 && Z <= 32) {
        ...
        long_double tabsi[] = {
            -0.3146268636172325352644123013773e-25, 0.7254967655706034123075144269188e-24,
            0.1720174348300531678711926627452e-22,  -0.3968569600567226603419285560888e-21,
            -0.7804624742706385979212734054562e-20, 0.1797226284123711388607362992709e-18,
            0.2882267004838688287625265808775e-17,  -0.6604428381371346662942676361200e-16,
            -0.8462601728339625717076212469620e-15, 0.1921641824294710456184856743277e-13,
            0.1918622280824819669976604364077e-12,  -0.4293140602943259867747088385429e-11,
            -0.3236373134197171392533933286615e-10, 0.7078744765376845042933221167302e-9,
            0.3867645142776632643256131482060e-8,   -0.8169087758905646352536906937371e-7,
            -0.3060269540778692505740670587310e-6,  0.6120146081774563343983258719578e-5,
            0.1450591123346014175404634002192e-4,   -0.2651270545919250186145646630982e-3,
            -0.3497315371034554476595429181296e-3,  0.5419736490383548422011597468850e-2,
            0.3119763955955768506415340725399e-2,   -0.3293438746976205966625829690981e-1,
            1.566756540030351110983731309007};
        // evalf(symb2poly(subst(d,X,30),h))
        long_double tabci[] = {
            -0.2905250884383236235247049545527e-25, -0.7522147039999421611567058749977e-24,
            0.1735426584530328263231524611399e-22,  0.3754699661541031707840824496312e-21,
            -0.8657191669382300006764470296916e-20, -0.1541017153772568372597216447575e-18,
            0.3541395808613865455763442962592e-17,  0.5090911414277503198761655016250e-16,
            -0.1161952195659956230574302196387e-14, -0.1318846041657448769184055840206e-13,
            0.2975305748369348193892361410833e-12,  0.2592737707940046163669864700563e-11,
            -0.5742839188863978180610659896436e-10, -0.3707322050098237760097147226680e-9,
            0.7983406320009368395835407645496e-8,   0.3641426564991240108255042664073e-7,
            -0.7507713655866267126314943513395e-6,  -0.2264698987507883174282691040657e-5,
            0.4355811042213145491501868952153e-4,   0.7862739829137060637224202271609e-4,
            -0.1341741499597397210639678491289e-2,  -0.1220985799040877684843355198131e-2,
            0.1638149848494348313828544726235e-1,   0.5141714996252801690622071553807e-2,
            -0.3303241728207114377922644096301e-1};
        unsigned N = sizeof(tabsi) / sizeof(long_double);
        for (unsigned i = 0; i < N; i++) {
          ress *= Zl;
          ress += tabsi[i];
      ...
      if (Z > 32 && Z <= 36) {
        ...
        long_double tabsi[] = {
            0.3942701136493758442738611783193e-25,  0.2833482488121121562882295508384e-25,
            -0.2240325735608330926855387141189e-22, 0.7077167897405254555239797927716e-23,
            0.1062083455080499454129483304028e-19,  -0.1296740970607253649730617065007e-19,
            -0.4125642950551060755694922602219e-17, 0.8186731227770529065956573976157e-17,
            0.1284716025190091972930389282765e-14,  -0.3331328985339278802607691455602e-14,
            -0.3121425078659463413394477246734e-12, 0.9467354869456215167864364128202e-12,
            0.5717466510263820214761281907592e-10,  -0.1880717150280359445242629986190e-9,
            -0.7546218335244881072513219000215e-8,  0.2525355992897561612710992980882e-7,
            0.6743126364121930403816188890288e-6,   -0.2149417013042573939863424458069e-5,
            -0.3721263582524267372627099974480e-4,  0.1039917503622257770319033100678e-3,
            0.1091628590022319276482516968274e-2,   -0.2344369704089296833875653244402e-2,
            -0.1270781662145181668046346804335e-1,  0.1556125547411834767154373923623e-1,
            1.595256185182468624967114677624};
        // evalf(symb2poly(subst(d,X,34),h))
        long_double tabci[] = {
            -0.1989238045881366884396486216401e-26, 0.9603929081698055330298655453613e-24,
            0.1785462486600632168852339199134e-24,  -0.4994884203374138675491408864852e-21,
            0.3922698307156298349251565849186e-21,  0.2148764364742271875693500554094e-18,
            -0.3483461372134677428776900288130e-18, -0.7495914823918311062321675163567e-16,
            0.1730727343462677961269223481865e-15,  0.2069644169058104778527335300702e-13,
            -0.5867510845332297156886886169438e-13, -0.4387405297943553775874545604808e-11,
            0.1397446669253207042207953256760e-10,  0.6866413079078539001893686720856e-9,
            -0.2296062969528945671977337418042e-8,  -0.7526095978772233458886342141369e-7,
            0.2479954929300904010747076408018e-6,   0.5360278479932581967324034110246e-5,
            -0.1619607535570418039830425866209e-4,  -0.2210046023539758078033314699734e-3,
            0.5534219043710011378918782268888e-3,   0.4305022897404827350706361160583e-2,
            -0.7413599071494898236031397261835e-2,  -0.2495794925837074078235212022679e-1,
            0.1626491643735576698165635194377e-1};
        unsigned N = sizeof(tabsi) / sizeof(long_double);
        for (unsigned i = 0; i < N; i++) {
          ress *= Zl;
          ress += tabsi[i];
      ...
      if (Z > 36 && Z <= 40) {
        ...
        long_double tabsi[] = {
            -0.2392585617788018962646757828598e-25, -0.6575431727115741067484987610228e-24,
            0.1413726322474858257725781544417e-22,  0.3273737185831529184312733883924e-21,
            -0.6971366411141976840382089119809e-20, -0.1343722941531172977052740901750e-18,
            0.2818131926670801219167357117818e-17,  0.4456155806753828158414772784641e-16,
            -0.9138685919934995126194762439509e-15, -0.1164855661769990237307373295310e-13,
            0.2314363358966319503074680596234e-12,  0.2327043421113577898681844454632e-11,
            -0.4423649753121148492913255999601e-10, -0.3413423247744706973417929454576e-9,
            0.6100985644119355599936971303758e-8,   0.3483915381970249998364550866465e-7,
            -0.5705724285037609250439734410714e-6,  -0.2292102947589923998615699771400e-5,
            0.3301272634561039972744957647445e-4,   0.8640908538565720173919303844142e-4,
            -0.1017258860929286693108579223376e-2,  -0.1518531271099188526195110204881e-2,
            0.1246413777530741664504711921590e-1,   0.7799173123931192562955174996028e-2,
            1.545492937235698740561891130750};
        // evalf(symb2poly(subst(d,X,38),h))
        long_double tabci[] = {
            0.2755791623901057620946655522961e-25,  -0.5942947240530035802859253027110e-24,
            -0.1501343221496874685931095691537e-22, 0.3214487908378114008198384990138e-21,
            0.6802473399014152745014932500302e-20,  -0.1438706210063382233790032141791e-18,
            -0.2516685599977182931504499600334e-17, 0.5224619763598943054009172687575e-16,
            0.7435302907498417390873416469940e-15,  -0.1502856747601093160946258165352e-13,
            -0.1706516669026401836883958369119e-12, 0.3322517447592247618873995073660e-11,
            0.2938003484322757578999393723335e-10,  -0.5429671029677216915133045537076e-9,
            -0.3623244387376009573032456389105e-8,  0.6223561267305907587303733846902e-7,
            0.3003453535122368377351197880341e-6,   -0.4643099720179236845576657726334e-5,
            -0.1523777445337208725039230269865e-4,  0.2008948838914583162973945866431e-3,
            0.4061768073150514090668429199326e-3,   -0.4114703864552309315370227396080e-2,
            -0.4230290732342083420531097274815e-2,  0.2513351694861302256806674303698e-1,
            0.7129761801971379713551376511546e-2};
        unsigned N = sizeof(tabsi) / sizeof(long_double);
        for (unsigned i = 0; i < N; i++) {
          ress *= Zl;
          ress += tabsi[i];
    ...
    int newprec, nbitsz = int(z._DOUBLE_val / std::log(2.)),
                 prec2 = int(prec * std::log(10.0) / std::log(2.0) + .5);
    if (nbitsz > prec2) {
      // use asymptotic expansion at z=inf
    ...
    gen si = 1, ci = 0, z2 = z * z, pi = 1,
        eps = accurate_evalf(pow(10, -prec, contextptr), newprec) / 2;
    for (int n = 1;; n++) {
      pi = pi * z2 / (2 * n * (2 * n - 1));
      if (is_greater(eps, abs(pi, contextptr), contextptr))
        break;
  ...
  static gen taylor_SiCi_f(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                           int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0;
    if (!is_inf(lim_point))
  ...
  gen _SiCi_f(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _DOUBLE_ || args.type == _REAL)
      return _Ci(args, contextptr) * sin(args, contextptr) +
             (evalf(cst_pi / 2, 1, contextptr) - _Si(args, contextptr)) * cos(args, contextptr);
    return symbolic(at_SiCi_f, args);
  }
  static const char _SiCi_f_s[] = "SiCi_f";
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__SiCi_f, &_SiCi_f,
                                           (size_t)&D_at_SiCi_funary_function_ptr, &taylor_SiCi_f,
                                           _SiCi_f_s);
#else
static define_unary_function_eval_taylor(__SiCi_f, &_SiCi_f, D_at_SiCi_f, &taylor_SiCi_f,
                                         _SiCi_f_s);
#endif
  define_unary_function_ptr5(at_SiCi_f, alias_at_SiCi_f, &__SiCi_f, 0, true);

  static gen taylor_SiCi_g(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                           int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0;
    if (!is_inf(lim_point))
  ...
  gen _SiCi_g(const gen & args, GIAC_CONTEXT) {
    ...
    if (is_undef(args))
      return args;
    if (args.type == _DOUBLE_ || args.type == _REAL)
      return -_Ci(args, contextptr) * cos(args, contextptr) +
             (evalf(cst_pi / 2, 1, contextptr) - _Si(args, contextptr)) * sin(args, contextptr);
    return symbolic(at_SiCi_g, args);
  }
  static const char _SiCi_g_s[] = "SiCi_g";
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__SiCi_g, &_SiCi_g,
                                           (size_t)&D_at_SiCi_gunary_function_ptr, &taylor_SiCi_g,
                                           _SiCi_g_s);
#else
static define_unary_function_eval_taylor(__SiCi_g, &_SiCi_g, D_at_SiCi_g, &taylor_SiCi_g,
                                         _SiCi_g_s);
#endif
  define_unary_function_ptr5(at_SiCi_g, alias_at_SiCi_g, &__SiCi_g, 0, true);

  static gen Si_replace(const gen & g, GIAC_CONTEXT) {
    return cst_pi_over_2 - _SiCi_f(g, contextptr) * cos(g, contextptr) -
           _SiCi_g(g, contextptr) * sin(g, contextptr);
  }
  static gen taylor_Si(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                       int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0) {
      return 0; // statically handled now
  ...
  gen _Si(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    if (args.type == _VECT)
      return apply(args, _Si, contextptr);
    if (is_zero(args, contextptr))
      return args;
    if (is_undef(args))
      return args;
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__Si, &_Si, (size_t)&D_at_Siunary_function_ptr,
                                           &taylor_Si, _Si_s);
#else
static define_unary_function_eval_taylor(__Si, &_Si, D_at_Si, &taylor_Si, _Si_s);
  ...
  static gen Ci_replace(const gen & g, GIAC_CONTEXT) {
    return _SiCi_f(g, contextptr) * sin(g, contextptr) -
           _SiCi_g(g, contextptr) * cos(g, contextptr);
  }
  ...
  static gen taylor_Ci(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                       int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0) {
      return 0; // statically handled now
  ...
  gen _Ci(const gen & args, GIAC_CONTEXT) {
    if (args.type == _STRNG && args.subtype == -1)
      return args;
    if (args.type == _VECT)
      return apply(args, _Ci, contextptr);
    if (is_zero(args, contextptr))
      return minus_inf;
    if (is_undef(args))
      return args;
...
#ifdef GIAC_HAS_STO_38
  define_unary_function_eval_taylor(__Ci, &_Ci, (size_t)&D_at_Ciunary_function_ptr, &taylor_Ci,
                                    _Ci_s);
#else
define_unary_function_eval_taylor(__Ci, &_Ci, D_at_Ci, &taylor_Ci, _Ci_s);
  ...
  static gen taylor_Ci0(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                        int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0;
    if (!is_zero(lim_point, contextptr))
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__Ci0, &_Ci0, (size_t)&D_at_Ci0unary_function_ptr,
                                           &taylor_Ci0, _Ci0_s);
#else
static define_unary_function_eval_taylor(__Ci0, &_Ci0, D_at_Ci0, &taylor_Ci0, _Ci0_s);
#endif
  define_unary_function_ptr5(at_Ci0, alias_at_Ci0, &__Ci0, 0,
                             true); /* FIXME should not registered */

  gen _Ei_f(const gen & args, GIAC_CONTEXT);
  static gen taylor_Ei_f(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                         int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0;
    if (!is_inf(lim_point))
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__Ei_f, &_Ei_f, (size_t)&D_at_Ei_funary_function_ptr,
                                           &taylor_Ei_f, _Ei_f_s);
#else
static define_unary_function_eval_taylor(__Ei_f, &_Ei_f, D_at_Ei_f, &taylor_Ei_f, _Ei_f_s);
  ...
  static gen taylor_Ei(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                       int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0) {
      return 0; // statically handled now
  ...
  gen Ei(const gen & args, GIAC_CONTEXT) {
	...
	*/
    ...
    if (z._DOUBLE_val > 1)
      ...
      newprec = prec2 + 2;
    gen ei = 0, pi = 1,
        eps = accurate_evalf(pow(10, -2 * prec, contextptr) *
                                 exp(-2 * abs(z, contextptr), contextptr),
                             newprec) /
              4,
        r, i;
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__Ei, &_Ei, (size_t)&D_at_Eiunary_function_ptr,
                                           &taylor_Ei, _Ei_s);
#else
static define_unary_function_eval_taylor(__Ei, &_Ei, D_at_Ei, &taylor_Ei, _Ei_s);
  ...
  static gen taylor_Ei0(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                        int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0;
    if (!is_zero(lim_point, contextptr))
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__Ei0, &_Ei0, (size_t)&D_at_Ei0unary_function_ptr,
                                           &taylor_Ei0, _Ei0_s);
#else
static define_unary_function_eval_taylor(__Ei0, &_Ei0, D_at_Ei0, &taylor_Ei0, _Ei0_s);
#endif
  define_unary_function_ptr5(at_Ei0, alias_at_Ei0, &__Ei0, 0,
                             true); /* FIXME should not registered */
  ...
  static gen ws4(const gen x, GIAC_CONTEXT) {
    ...
    return l1 - l2 + l2 / l1 + l2 * (-2 + l2) / 2 / l1 / l1 +
           l2 * (6 - 9 * l2 + 2 * l2 * l2) / 6 / l1 / l1 / l1 +
           l2 * (-12 + 36 * l2 - 22 * l2 * l2 + 3 * l2 * l2 * l2) / 12 / l1 / l1 / l1 / l1;
  }
  static gen taylor_LambertWs(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                              int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0)
      return 0;
    if (!is_inf(lim_point))
  ...
  static gen d_LambertWs(const gen & args, GIAC_CONTEXT) {
    return derive(_LambertW(exp(args, contextptr), contextptr) - ws4(args, contextptr), args,
                  contextptr);
  }
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval_taylor(__LambertWs, &_LambertWs,
                                           (size_t)&D_at_LambertWsunary_function_ptr,
                                           &taylor_LambertWs, _LambertWs_s);
#else
  static define_unary_function_eval_taylor(__LambertWs, &_LambertWs, D_at_LambertWs,
                                           &taylor_LambertWs, _LambertWs_s);
#endif
  ...
  static gen taylor_LambertW(const gen & lim_point, const int ordre, const unary_function_ptr & f,
                             int direction, gen & shift_coeff, GIAC_CONTEXT) {
    if (ordre < 0) {
      return 0; // statically handled now
    }
  ...
  gen _LambertW(const gen & args, GIAC_CONTEXT) {
    if (args.type == _VECT && args._VECTptr->size() == 2) {
      ...
      if (x.type == _CPLX && args.subtype == 3)
        return LambertW(complex<double>(x._CPLXptr->_DOUBLE_val, (x._CPLXptr + 1)->_DOUBLE_val),
                        n.val);
#ifdef HAVE_LIBMPFR
    ...
    if (args.type == _DOUBLE_)
      return LambertW(args._DOUBLE_val);
    if (args.type == _CPLX && args.subtype == 3)
      return LambertW(
          complex<double>(args._CPLXptr->_DOUBLE_val, (args._CPLXptr + 1)->_DOUBLE_val));
#ifdef HAVE_LIBMPFR
    if (args.type == _REAL || (args.type == _CPLX && args._CPLXptr->type == _REAL))
      return LambertW(args, 0);
#endif
    if (args == 0 || args == plus_inf)
      return args;
    if (args == symbolic(at_exp, 1))
      return 1;
    if (args == 2 * symb_ln(2) || args == symb_ln(4))
      return symb_ln(2);
    if (-inv(args, contextptr) == symbolic(at_exp, 1))
      return -1;
  ...
  define_unary_function_eval_taylor(__LambertW, &_LambertW,
                                    (size_t)&D_at_LambertWunary_function_ptr, &taylor_LambertW,
                                    _LambertW_s);
#else
  define_unary_function_eval_taylor(__LambertW, &_LambertW, D_at_LambertW, &taylor_LambertW,
                                    _LambertW_s);
#endif
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3(__Dirac, &_Dirac, (size_t)&D_at_Diracunary_function_ptr,
                                     _Dirac_s);
#else
...
#ifdef GIAC_HAS_STO_38
  static define_unary_function_eval3(__Heaviside, &_Heaviside,
                                     (size_t)&D_Heavisideunary_function_ptr, _Heaviside_s);
#else
static define_unary_function_eval3(__Heaviside, &_Heaviside, D_Heaviside, _Heaviside_s);
  ...
  gen fast_icontent(const gen & g) {
    ...
    if (g._SYMBptr->sommet == at_prod) {
      ...
      const_iterateur it = g._SYMBptr->feuille._VECTptr->begin(),
                      itend = g._SYMBptr->feuille._VECTptr->end();
      for (; it != itend; ++it) {
  ...
  gen fast_divide_by_icontent(const gen & g, const gen & z) {
    ...
    if (g._SYMBptr->sommet == at_inv)
      return symbolic(g._SYMBptr->sommet,
                      fast_divide_by_icontent(g._SYMBptr->feuille, inv(z, context0)));
    if (g._SYMBptr->sommet == at_pow && is_integer(g._SYMBptr->feuille[1])) {
      ...
      return pow(z1, g._SYMBptr->feuille[1], context0) / z *
             pow(g1, g._SYMBptr->feuille[1], context0);
    }
  ...
  // #endif
#if defined(                                                                                       \
    GIAC_GENERIC_CONSTANTS) // || (defined(VISUALC) && !defined(RTOS_THREADX)) || defined( x86_64)
  const gen zero(0);
...
const define_alias_gen(alias_rad2deg_g, _DOUBLE_, (*(ulonglong *)&rad2deg_d) >> 8,
                       (*(ulonglong *)&rad2deg_d) >> 16);
const gen & rad2deg_g = *(const gen *)&alias_rad2deg_g;
const define_alias_gen(alias_deg2rad_g, _DOUBLE_, (*(ulonglong *)&deg2rad_d) >> 8,
                       (*(ulonglong *)&deg2rad_d) >> 16);
const gen & deg2rad_g = *(const gen *)&alias_deg2rad_g;
//grad
const define_alias_gen(alias_rad2grad_g, _DOUBLE_, (*(ulonglong *)&rad2grad_d) >> 8,
                       (*(ulonglong *)&rad2grad_d) >> 16);
const gen & rad2grad_g = *(const gen *)&alias_rad2grad_g;
const define_alias_gen(alias_grad2rad_g, _DOUBLE_, (*(ulonglong *)&grad2rad_d) >> 8,
                       (*(ulonglong *)&grad2rad_d) >> 16);
const gen & grad2rad_g = *(const gen *)&alias_grad2rad_g;
  ...
  gen cos_pi_12(_FRAC2_SYMB(symbolic(at_plus, gen(makevecteur(plus_sqrt6, plus_sqrt2), _SEQ__VECT)),
                            4));
  gen minus_cos_pi_12(
      _FRAC2_SYMB(symbolic(at_plus, gen(makevecteur(minus_sqrt6, minus_sqrt2), _SEQ__VECT)), 4));
  gen sin_pi_12(
      _FRAC2_SYMB(symbolic(at_plus, gen(makevecteur(plus_sqrt6, minus_sqrt2), _SEQ__VECT)), 4));
  gen minus_sin_pi_12(
      _FRAC2_SYMB(symbolic(at_plus, gen(makevecteur(plus_sqrt2, minus_sqrt6), _SEQ__VECT)), 4));
  ...
  const gen * const table_cos[trig_deno + 1] = {
      &minus_one,      &minus_cos_pi_12, &minus_sqrt3_2, &minus_sqrt2_2,
      &minus_one_half, &minus_sin_pi_12, &zero,          &sin_pi_12,
      &plus_one_half,  &plus_sqrt2_2,    &plus_sqrt3_2,  &cos_pi_12,
      &plus_one,       &cos_pi_12,       &plus_sqrt3_2,  &plus_sqrt2_2,
      &plus_one_half,  &sin_pi_12,       &zero,          &minus_sin_pi_12,
      &minus_one_half, &minus_sqrt2_2,   &minus_sqrt3_2, &minus_cos_pi_12,
      &minus_one};
...
const define_alias_ref_symbolic(cst_two_pi_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &cst_two_pi_refv);
...
const define_alias_ref_symbolic(inv_3_symb, alias_at_inv, _INT_, 0, 3) const
    define_alias_gen(alias_inv_3, _SYMB, 0, &inv_3_symb);
const gen & gen_inv_3 = *(const gen *)&alias_inv_3;

const define_alias_ref_symbolic(inv_4_symb, alias_at_inv, _INT_, 0, 4) const
    define_alias_gen(alias_inv_4, _SYMB, 0, &inv_4_symb);
...
const define_alias_ref_symbolic(cst_pi_over_2_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &cst_pi_over_2_refv);
...
const define_tab2_alias_gen(alias_minus_sqrt3_2_tab, _SYMB, 0, &minus_sqrt3_symb, _SYMB, 0,
                            &inv_2_symb);
const define_alias_ref_vecteur2(minus_sqrt3_2_refv, alias_minus_sqrt3_2_tab);
const define_alias_ref_symbolic(minus_sqrt3_2_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &minus_sqrt3_2_refv);
...
const define_tab2_alias_gen(alias_minus_sqrt2_2_tab, _SYMB, 0, &minus_sqrt2_symb, _SYMB, 0,
                            &inv_2_symb);
const define_alias_ref_vecteur2(minus_sqrt2_2_refv, alias_minus_sqrt2_2_tab);
const define_alias_ref_symbolic(minus_sqrt2_2_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &minus_sqrt2_2_refv);
...
const define_tab2_alias_gen(alias_minus_sqrt3_3_tab, _SYMB, 0, &minus_sqrt3_symb, _SYMB, 0,
                            &inv_3_symb);
const define_alias_ref_vecteur2(minus_sqrt3_3_refv, alias_minus_sqrt3_3_tab);
const define_alias_ref_symbolic(minus_sqrt3_3_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &minus_sqrt3_3_refv);
...
const define_tab2_alias_gen(alias_plus_sqrt3_2_tab, _SYMB, 0, &plus_sqrt3_symb, _SYMB, 0,
                            &inv_2_symb);
const define_alias_ref_vecteur2(plus_sqrt3_2_refv, alias_plus_sqrt3_2_tab);
const define_alias_ref_symbolic(plus_sqrt3_2_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &plus_sqrt3_2_refv);
const define_alias_gen(alias_plus_sqrt3_2, _SYMB, 0, &plus_sqrt3_2_symb);
const gen & plus_sqrt3_2 = *(const gen *)&alias_plus_sqrt3_2;

const define_tab2_alias_gen(alias_plus_sqrt2_2_tab, _SYMB, 0, &plus_sqrt2_symb, _SYMB, 0,
                            &inv_2_symb);
const define_alias_ref_vecteur2(plus_sqrt2_2_refv, alias_plus_sqrt2_2_tab);
const define_alias_ref_symbolic(plus_sqrt2_2_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &plus_sqrt2_2_refv);
const define_alias_gen(alias_plus_sqrt2_2, _SYMB, 0, &plus_sqrt2_2_symb);
const gen & plus_sqrt2_2 = *(const gen *)&alias_plus_sqrt2_2;

const define_tab2_alias_gen(alias_plus_sqrt3_3_tab, _SYMB, 0, &plus_sqrt3_symb, _SYMB, 0,
                            &inv_3_symb);
const define_alias_ref_vecteur2(plus_sqrt3_3_refv, alias_plus_sqrt3_3_tab);
const define_alias_ref_symbolic(plus_sqrt3_3_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &plus_sqrt3_3_refv);
const define_alias_gen(alias_plus_sqrt3_3, _SYMB, 0, &plus_sqrt3_3_symb);
const gen & plus_sqrt3_3 = *(const gen *)&alias_plus_sqrt3_3;

const define_tab2_alias_gen(alias_cos_pi_12_4_tab, _SYMB, 0, &plus_sqrt6_symb, _SYMB, 0,
                            &plus_sqrt2_symb);
const define_alias_ref_vecteur2(cos_pi_12_4_refv, alias_cos_pi_12_4_tab);
const define_alias_ref_symbolic(cos_pi_12_4_symb, alias_at_plus, _VECT, _SEQ__VECT,
                                &cos_pi_12_4_refv);

const define_tab2_alias_gen(alias_cos_pi_12_tab, _SYMB, 0, &cos_pi_12_4_symb, _SYMB, 0,
                            &inv_4_symb);
...
const define_tab2_alias_gen(alias_minus_cos_pi_12_4_tab, _SYMB, 0, &minus_sqrt6_symb, _SYMB, 0,
                            &minus_sqrt2_symb);
const define_alias_ref_vecteur2(minus_cos_pi_12_4_refv, alias_minus_cos_pi_12_4_tab);
const define_alias_ref_symbolic(minus_cos_pi_12_4_symb, alias_at_plus, _VECT, _SEQ__VECT,
                                &minus_cos_pi_12_4_refv);
const define_tab2_alias_gen(alias_minus_cos_pi_12_tab, _SYMB, 0, &minus_cos_pi_12_4_symb, _SYMB, 0,
                            &inv_4_symb);
const define_alias_ref_vecteur2(minus_cos_pi_12_refv, alias_minus_cos_pi_12_tab);
const define_alias_ref_symbolic(minus_cos_pi_12_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &minus_cos_pi_12_refv);
...
const define_tab2_alias_gen(alias_sin_pi_12_4_tab, _SYMB, 0, &plus_sqrt6_symb, _SYMB, 0,
                            &minus_sqrt2_symb);
const define_alias_ref_vecteur2(sin_pi_12_4_refv, alias_sin_pi_12_4_tab);
const define_alias_ref_symbolic(sin_pi_12_4_symb, alias_at_plus, _VECT, _SEQ__VECT,
                                &sin_pi_12_4_refv);
const define_tab2_alias_gen(alias_sin_pi_12_tab, _SYMB, 0, &sin_pi_12_4_symb, _SYMB, 0,
                            &inv_4_symb);
...
const define_tab2_alias_gen(alias_minus_sin_pi_12_4_tab, _SYMB, 0, &plus_sqrt2_symb, _SYMB, 0,
                            &minus_sqrt6_symb);
const define_alias_ref_vecteur2(minus_sin_pi_12_4_refv, alias_minus_sin_pi_12_4_tab);
const define_alias_ref_symbolic(minus_sin_pi_12_4_symb, alias_at_plus, _VECT, _SEQ__VECT,
                                &minus_sin_pi_12_4_refv);
const define_tab2_alias_gen(alias_minus_sin_pi_12_tab, _SYMB, 0, &minus_sin_pi_12_4_symb, _SYMB, 0,
                            &inv_4_symb);
const define_alias_ref_vecteur2(minus_sin_pi_12_refv, alias_minus_sin_pi_12_tab);
const define_alias_ref_symbolic(minus_sin_pi_12_symb, alias_at_prod, _VECT, _SEQ__VECT,
                                &minus_sin_pi_12_refv);
...
const define_alias_ref_symbolic(tan_5pi_12_symb, alias_at_plus, _VECT, _SEQ__VECT,
                                &tan_5pi_12_refv);
...
const define_alias_ref_symbolic(deg2rad_e_symb, (size_t)&__prod, _VECT, _SEQ__VECT,
                                &deg2rad_e_refv);
...
const define_alias_ref_symbolic(rad2grad_e_symb, (size_t)&_prod, _VECT, _SEQ__VECT,
                                &rad2grad_e_refv);
...
const define_alias_ref_symbolic(grad2rad_e_symb, (size_t)&__prod, _VECT, _SEQ__VECT,
                                &grad2rad_e_refv);
...
static const alias_gen * const table_cos_alias[trig_deno + 1] = {
    &alias_minus_one,      &alias_minus_cos_pi_12, &alias_minus_sqrt3_2, &alias_minus_sqrt2_2,
    &alias_minus_one_half, &alias_minus_sin_pi_12, &alias_zero,          &alias_sin_pi_12,
    &alias_plus_one_half,  &alias_plus_sqrt2_2,    &alias_plus_sqrt3_2,  &alias_cos_pi_12,
    &alias_plus_one,       &alias_cos_pi_12,       &alias_plus_sqrt3_2,  &alias_plus_sqrt2_2,
    &alias_plus_one_half,  &alias_sin_pi_12,       &alias_zero,          &alias_minus_sin_pi_12,
    &alias_minus_one_half, &alias_minus_sqrt2_2,   &alias_minus_sqrt3_2, &alias_minus_cos_pi_12,
    &alias_minus_one};
const gen * const * table_cos = (const gen **)table_cos_alias;
static const alias_gen * const table_tan_alias[trig_deno / 2 + 1] = {
    &alias_zero,        &alias_tan_pi_12,  &alias_plus_sqrt3_3,  &alias_plus_one,
    &alias_plus_sqrt3,  &alias_tan_5pi_12, &alias_unsigned_inf,  &alias_minus_tan_5pi_12,
    &alias_minus_sqrt3, &alias_minus_one,  &alias_minus_sqrt3_3, &alias_minus_tan_pi_12,
    &alias_zero};
const gen * const * table_tan = (const gen **)table_tan_alias;

#endif // GIAC_GENERIC_CONSTANTS

  const alias_type reim_op_alias[] = {(alias_type)&__inv,       (alias_type)&__exp,
                                      (alias_type)&__cos,       (alias_type)&__sin,
                                      (alias_type)&__tan,       (alias_type)&__cosh,
                                      (alias_type)&__sinh,      (alias_type)&__tanh,
                                      (alias_type)&__atan,      (alias_type)&__lnGamma_minus,
                                      (alias_type)&__Gamma,     (alias_type)&__Psi_minus_ln,
                                      (alias_type)&__Psi,       (alias_type)&__Zeta,
                                      (alias_type)&__Eta,       (alias_type)&__sign,
                                      (alias_type)&__erf,       (alias_type)&__of,
                                      (alias_type)&__factorial, 0};
  const unary_function_ptr * const reim_op = (const unary_function_ptr * const)reim_op_alias;
  // for subst.cc
  const alias_type sincostan_tab_alias[] = {(alias_type)&__sin, (alias_type)&__cos,
                                            (alias_type)&__tan, 0};
  const unary_function_ptr * const sincostan_tab =
      (const unary_function_ptr * const)sincostan_tab_alias;

  const alias_type asinacosatan_tab_alias[] = {(alias_type)&__asin, (alias_type)&__acos,
                                               (alias_type)&__atan, 0};
  const unary_function_ptr * const asinacosatan_tab =
      (const unary_function_ptr * const)asinacosatan_tab_alias;

  const alias_type sinhcoshtanh_tab_alias[] = {alias_at_sinh, alias_at_cosh, alias_at_tanh, 0};
  const unary_function_ptr * const sinhcoshtanh_tab =
      (const unary_function_ptr * const)sinhcoshtanh_tab_alias;

  const alias_type sinhcoshtanhinv_tab_alias[] = {(alias_type)&__sinh, (alias_type)&__cosh,
                                                  (alias_type)&__tanh, (alias_type)&__inv, 0};
  const unary_function_ptr * const sinhcoshtanhinv_tab =
      (const unary_function_ptr * const)sinhcoshtanhinv_tab_alias;

  const alias_type sincostansinhcoshtanh_tab_alias[] = {(alias_type)&__sin,
                                                        (alias_type)&__cos,
                                                        (alias_type)&__tan,
                                                        (alias_type)&__sinh,
                                                        (alias_type)&__cosh,
                                                        (alias_type)&__tanh,
                                                        0};
  const unary_function_ptr * const sincostansinhcoshtanh_tab =
      (const unary_function_ptr * const)sincostansinhcoshtanh_tab_alias;
  ...
  const alias_type sign_floor_ceil_round_tab_alias[] = {(alias_type)&__sign, (alias_type)&__floor,
                                                        (alias_type)&__ceil, (alias_type)&__round,
                                                        (alias_type)&__sum,  0};
  const unary_function_ptr * const sign_floor_ceil_round_tab =
      (const unary_function_ptr * const)sign_floor_ceil_round_tab_alias;
  ...
  const unary_function_ptr * const Heaviside_tab =
      (const unary_function_ptr * const)Heaviside_tab_alias;

  const alias_type invpowtan_tab_alias[] = {alias_at_inv, alias_at_pow, alias_at_tan, 0};
  const unary_function_ptr * const invpowtan_tab =
      (const unary_function_ptr * const)invpowtan_tab_alias;
  ...
  const gen_op_context halftan_hyp2exp_tab[] = {sin2tan2, cos2tan2, tan2tan2, sinh2exp,
                                                cosh2exp, tanh2exp, 0};
  ...
  const unary_function_ptr * const factorial_tab =
      (const unary_function_ptr * const)factorial_tab_alias;

  const gen_op_context factorial2gamma_tab[] = {factorialtogamma, 0};

  // for integration
  const alias_type primitive_tab_op_alias[] = {
      (const alias_type)&__sin,   (const alias_type)&__cos,   (const alias_type)&__tan,
      (const alias_type)&__exp,   (const alias_type)&__sinh,  (const alias_type)&__cosh,
      (const alias_type)&__tanh,  (const alias_type)&__asin,  (const alias_type)&__acos,
      (const alias_type)&__atan,  (const alias_type)&__ln,    (const alias_type)&__asinh,
      (const alias_type)&__acosh, (const alias_type)&__atanh, 0};
  const unary_function_ptr * const primitive_tab_op =
      (const unary_function_ptr * const)primitive_tab_op_alias;
  const alias_type inverse_tab_op_alias[] = {(const alias_type)&__asin,  (const alias_type)&__acos,
                                             (const alias_type)&__atan,  (const alias_type)&__ln,
                                             (const alias_type)&__asinh, (const alias_type)&__acosh,
                                             (const alias_type)&__atanh, (const alias_type)&__erf,
                                             (const alias_type)&__erfc,  0};
  const unary_function_ptr * const inverse_tab_op =
      (const unary_function_ptr * const)inverse_tab_op_alias;

  const alias_type analytic_sommets_alias[] = {
      (const alias_type)&__plus,  (const alias_type)&__prod,  (const alias_type)&__neg,
      (const alias_type)&__inv,   (const alias_type)&__pow,   (const alias_type)&__sin,
      (const alias_type)&__cos,   (const alias_type)&__tan,   (const alias_type)&__exp,
      (const alias_type)&__sinh,  (const alias_type)&__cosh,  (const alias_type)&__tanh,
      (const alias_type)&__asin,  (const alias_type)&__acos,  (const alias_type)&__atan,
      (const alias_type)&__asinh, (const alias_type)&__atanh, (const alias_type)&__acosh,
      (const alias_type)&__ln,    (const alias_type)&__sqrt,  0};
  const unary_function_ptr * const analytic_sommets =
      (const unary_function_ptr * const)analytic_sommets_alias;
  // test if g is < > <= >=,
  const alias_type inequality_tab_alias[] = {(const alias_type)&__equal,
                                             (const alias_type)&__inferieur_strict,
                                             (const alias_type)&__inferieur_egal,
                                             (const alias_type)&__different,
                                             (const alias_type)&__superieur_strict,
                                             (const alias_type)&__superieur_egal,
                                             0};
  const unary_function_ptr * const inequality_tab =
      (const unary_function_ptr * const)inequality_tab_alias;
  ...
  const alias_type solve_fcns_tab_alias[] = {
      (const alias_type)&__exp,   (const alias_type)&__ln,    (const alias_type)&__sin,
      (const alias_type)&__cos,   (const alias_type)&__tan,   (const alias_type)&__asin,
      (const alias_type)&__acos,  (const alias_type)&__atan,  (const alias_type)&__sinh,
      (const alias_type)&__cosh,  (const alias_type)&__tanh,  (const alias_type)&__asinh,
      (const alias_type)&__acosh, (const alias_type)&__atanh, 0};
  const unary_function_ptr * const solve_fcns_tab =
      (const unary_function_ptr * const)solve_fcns_tab_alias;

  const alias_type limit_tab_alias[] = {(const alias_type)&__Gamma, (const alias_type)&__Psi,
                                        (const alias_type)&__erf,
                                        (const alias_type)&__lower_incomplete_gamma, 0};
  const unary_function_ptr * const limit_tab = (const unary_function_ptr * const)limit_tab_alias;
  const gen_op_context limit_replace[] = {Gamma_replace, Psi_replace, erf_replace, igamma_replace,
                                          0};
  ...
  nio::ios_base<T> & operator<<(nio::ios_base<T> & os, const alias_ref_vecteur & v) {
    ...
    os << &v << ":" << *(gen *)v.begin_immediate_vect << ","
       << *(gen *)(v.begin_immediate_vect + 1);
#else
    os << &v;
#endif
    return os;
...
#ifdef IMMEDIATE_VECTOR
  os << (unsigned long)&v << ":" << *(gen *)v.begin_immediate_vect << ","
     << *(gen *)(v.begin_immediate_vect + 1);
#else
  os << (unsigned long)&v;
#endif
