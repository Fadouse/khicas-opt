...
const int rand_max2 = RAND_MAX;
  ...
  static define_unary_function_eval4_index(147, __program, &quote_program, _program_s,
                                           &printasprogram, &texprintsommetasoperator);
  ...
  symbolic symb_ifte(const gen & test, const gen & oui, const gen & non) {
    return symbolic(at_ifte, makesequence(test, oui, non));
  }
  ...
  static gen to_increment(const gen & g) {
    ...
    if (type && av.front() == b) {
      ...
      if (type)
        return symbolic(at_increment,
                        makesequence(b, symb_plus(vecteur(av.begin() + 1, av.end()))));
    }
    return g;
  }
  ...
  gen _for(const gen & args, const context * contextptr) {
    ...
    try {
      ...
      gen gforin;
      size_t testsize = 0;
      if ((test.is_symb_of_sommet(at_for)) && test._SYMBptr->feuille.type == _VECT &&
            ...
            if (a.type == _DOUBLE_ && a._DOUBLE_val <= RAND_MAX && a._DOUBLE_val >= -RAND_MAX) {
              ...
              if (a.type != _INT_) {
                if (a.type != _ZINT || mpz_sizeinbase(*a._ZINTptr, 2) > 62) {
                  *logptr(contextptr)
                      << gettext("Unable to convert to integer ") + a.print(contextptr) << endl;
                  return -RAND_MAX;
                }
              }
            }
  ...
  gen _dollar(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type != _VECT) {
      ...
      vargs = makevecteur(
          ...
          symb_equal(
              tmp,
              args)); // makevecteur(tmp,symbolic(at_equal,gen(makevecteur(tmp,args),_SEQ__VECT)));
    } else
      vargs = *args._VECTptr;
...
void debug_loop(gen & res, GIAC_CONTEXT) {
  ...
  *logptr(contextptr) << gettext("Please try xcas on Linux or an Unix") << endl;
  ...
  debug_ptr(contextptr)->debug_allowed = true;
  if (!debug_ptr(contextptr)->args_stack.empty()) {
    w.push_back(makevecteur(debug_ptr(contextptr)->args_stack.back(),
                            vector_int_2_vecteur(debug_ptr(contextptr)->sst_at, contextptr)));
  } else
    w.push_back(undef);
  w.push_back(undef);
  w.push_back(res);
  ...
  public:
    bool operator()(const gen & a, const gen & b) {
      ...
      if (c.type != _INT_) {
#ifndef NO_STDEXCEPT
        setsizeerr(gettext("Unable to sort ") + sorting_function.print(contextptr) + "(" +
                   ab.print(contextptr) + ")=" + c.print(contextptr));
#else
      *logptr(contextptr) << gettext("Unable to sort ") + sorting_function.print(contextptr) + "(" +
                                 ab.print(contextptr) + ")=" + c.print(contextptr)
                          << endl;
#endif
        return true;
  ...
  */
  ...
  gen simplifier(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.type != _SYMB || g._SYMBptr->sommet == at_program || g._SYMBptr->sommet == at_pnt ||
        g._SYMBptr->sommet == at_unit || g._SYMBptr->sommet == at_integrate ||
        g._SYMBptr->sommet == at_superieur_strict || g._SYMBptr->sommet == at_superieur_egal ||
        g._SYMBptr->sommet == at_inferieur_strict || g._SYMBptr->sommet == at_inferieur_egal ||
        g._SYMBptr->sommet == at_and || g._SYMBptr->sommet == at_ou ||
    ...
    if (g.is_symb_of_sommet(at_sto) && g._SYMBptr->feuille.type == _VECT &&
      ...
      if (b.is_symb_of_sommet(at_sto)) {
        ...
        if (a.type == _VECT && c.type == _VECT) {
          ...
          av.back() = symb_equal(
              av.back(), cv.front()); // symbolic(at_equal,makesequence(av.back(),cv.front()));
          cv.erase(cv.begin());
  ...
  gen _write(const gen & args, GIAC_CONTEXT) {
    ...
    *logptr(contextptr) << "Write " << fname << endl;
    ...
    if (hFile < 0) {
      ...
      if (hFile < 0)
        return 0; // if it still fails, there's nothing we can do
    }
