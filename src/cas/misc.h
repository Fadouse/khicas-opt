...
extern unary_function_eval __input;
  ...
  int step_func(const gen & f, const gen & x, gen & xmin, gen & xmax, vecteur & poig, vecteur & tvi,
                gen & periode, vecteur & asym, vecteur & parab, vecteur & crit, vecteur & inflex,
                bool printtvi, bool exactlegende, GIAC_CONTEXT, int do_inflex_tabsign = 1);
  ...
  void draw_filled_polygon(std::vector<std::vector<int>> & L, int xmin, int xmax, int ymin,
                           int ymax, int color);
  void draw_line(int x1, int y1, int x2, int y2, int color);
  extern "C" void set_pixel(int x, int y, unsigned short c);
  void draw_circle(int xc, int yc, int r, int color, bool q1 = true, bool q2 = true, bool q3 = true,
                   bool q4 = true);
  void draw_filled_circle(int xc, int yc, int r, int color, bool left = true, bool right = true);
  inline void draw_filled_circle(int xc, int yc, int r, int color, bool left, bool right,
                                 const void *) {
    draw_filled_circle(xc, yc, r, color, left, right);
  };
  void draw_arc(int xc, int yc, int rx, int ry, int color, double t1, double t2, bool q1 = true,
                bool q2 = true, bool q3 = true, bool q4 = true);
  void draw_filled_arc(int x, int y, int rx, int ry, int color, int theta1_deg, int theta2_deg,
                       int xmin, int xmax, int ymin, int ymax, bool segment = false);
