...
namespace giac {
  ...
  static define_unary_function_eval(__accumulate_head_tail, &_accumulate_head_tail,
                                    _accumulate_head_tail_s);
  define_unary_function_ptr5(at_accumulate_head_tail, alias_at_accumulate_head_tail,
                             &__accumulate_head_tail, 0, true);
  ...
  static define_unary_function_eval2_quoted(__inverser, &_inverser, _inverser_s,
                                            &printastifunction);
  ...
  static gen maple_gcdigcd(const gen & a_orig, const unary_function_ptr * u, GIAC_CONTEXT) {
    ...
    if (res.type == _VECT && res._VECTptr->size() == 3 && v[s - 2].type == _IDNT &&
        v[s - 1].type == _IDNT) {
  ...
  define_unary_function_ptr5(at_restart_modes, alias_at_restart_modes, &__restart_modes, 0,
                             T_RETURN);
  ...
  gen _time(const gen & a, GIAC_CONTEXT) {
    ...
    if (a.type == _VECT && a.subtype == _SEQ__VECT) {
      ...
      if (a._VECTptr->size() == 2 && a._VECTptr->back().type == _STRNG &&
        ...
        if (!get_cas_memory_stats(before))
        ...
        if (!get_cas_memory_stats(after))
          return gensizeerr(contextptr);
        vecteur result;
        result.reserve(7);
        result.push_back(answer);
        result.push_back(double(elapsed) / 128);
        result.push_back(gen(size_t(before[0])));
        result.push_back(gen(size_t(after[0])));
        result.push_back(gen(size_t(after[2])));
        result.push_back(gen(size_t(after[3] - before[3])));
        result.push_back(gen(size_t(after[4] - before[4])));
        return result;
      }
      if (a._VECTptr->size() == 2 && a._VECTptr->front().type == _INT_ &&
  ...
  gen _pivot(const gen & a_orig, GIAC_CONTEXT) {
    ...
    for (; l1 <= l2; ++l1) {
      if (l1 != l && !is_zero(m[l1][c]))
        linear_combination(p, *m[l1]._VECTptr, -m[l1][c], *m[l]._VECTptr, plus_one, 1,
                           *m[l1]._VECTptr, epsilon(contextptr));
    }
    return m;
  }
  ...
  gen _row(const gen & g, GIAC_CONTEXT) {
    ...
    if (indice.is_symb_of_sommet(at_interval) && indice._SYMBptr->feuille.type == _VECT)
      indice = symbolic(at_interval,
                        indice._SYMBptr->feuille -
                            gen(shift) * vecteur(indice._SYMBptr->feuille._VECTptr->size(), 1));
    else
      indice -= shift;
  ...
  gen _trunc(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT) {
      ...
      if (args.subtype == _SEQ__VECT && args._VECTptr->size() == 2 &&
          args._VECTptr->front().type == _INT_) {
  ...
  static define_unary_function_eval4(__div, &_iquo, _div_s, &printsommetasoperator,
  ...
  gen _evalc(const gen & g, GIAC_CONTEXT) {
    ...
    if (lvar(makevecteur(re2, im2)).size() < lvar(makevecteur(r, i)).size())
      reim(inv(ratnormal(re2, contextptr) + cst_i * ratnormal(im2, contextptr), contextptr), r, i,
           contextptr);
...
#define S_IWUSR 00200
  ...
  gen _open(const gen & g, GIAC_CONTEXT) {
    if (g.type == _STRNG && g.subtype == -1)
      return g;
#if defined(VISUALC) || defined(__MINGW_H) || defined(FIR) || defined(FXCG) || defined(NSPIRE) ||  \
    defined(__ANDROID__) || defined(NSPIRE_NEWLIB) || defined(EMCC) || defined(EMCC2) ||           \
    defined(GIAC_GGB) || defined KHICAS
  ...
  gen _fprint(const gen & g, GIAC_CONTEXT) {
    ...
    if (f) {
      if (s > 1 && v[1] == gen("Unquoted", contextptr)) {
        for (int i = 2; i < s; ++i)
          fprintf(f, "%s",
                  v[i].type == _STRNG ? v[i]._STRNGptr->c_str()
                                      : unquote(v[i].print(contextptr)).c_str());
  ...
  static gen delrowscols(const gen & g, bool isrow, GIAC_CONTEXT) {
    ...
    gm = g._VECTptr->front();
    if (gm.type == _IDNT) {
      return sto(delrowscols(eval(g, eval_level(contextptr), contextptr), isrow, contextptr), gm,
                 contextptr);
    }
    ...
    if (!ckmatrix(gm) || !interval.is_symb_of_sommet(at_interval) ||
        (f = interval._SYMBptr->feuille).type != _VECT || f._VECTptr->size() != 2 ||
        !is_integral(fa = f._VECTptr->front()) || !is_integral(fb = f._VECTptr->back()))
      return gentypeerr();
    int shift =
        array_start(contextptr); //xcas_mode(contextptr)!=0 || abs_calc_mode(contextptr)==38;
  ...
  gen _border(const gen & g, GIAC_CONTEXT) {
    if (g.type == _STRNG && g.subtype == -1)
      return g;
    if (g.type != _VECT || g._VECTptr->size() != 2 || !ckmatrix(g._VECTptr->front()) ||
  ...
  gen _pade(const gen & g, GIAC_CONTEXT) {
    ...
    if (!egcd_pade(Np, fp, p, a, b, 0))
      *logptr(contextptr) << gettext(
                                 "Solution may be wrong since a and b are not prime together: ") +
                                 gen(a).print(contextptr) + "," + gen(b).print(contextptr)
                          << '\n';
  ...
  define_unary_function_ptr5(at_reverse_rsolve, alias_at_reverse_rsolve, &__reverse_rsolve, 0,
                             true);
  ...
  define_unary_function_ptr5(at_berlekamp_massey, alias_at_berlekamp_massey, &__berlekamp_massey, 0,
                             true);

  // Approx fft or exact if args=poly1,omega,n
  gen fft(const gen & g_orig, int direct, GIAC_CONTEXT) {
    if (g_orig.type == _VECT && g_orig.subtype == _SEQ__VECT && g_orig._VECTptr->size() >= 3 &&
        g_orig._VECTptr->front().type == _VECT) {
  ...
  define_unary_function_ptr5(at_implicitplot3d, alias_at_implicitplot3d, &__implicitplot3d, 0,
                             true);
  ...
  bool read_audio(vecteur & v, int & channels, int & sample_rate, int & bits_per_sample,
    ...
    if (channels <= 0 || channels > int(v.size())) {
      ...
      w = makevecteur(channels, 16, 44100);
      ws = 3;
      g = w;
      v.insert(v.begin(), g);
    }
...
#include <ao/ao.h>
  ...
  gen _playsnd(const gen & args, GIAC_CONTEXT) {
    ...
    if (data_size) {
      *logptr(contextptr) << gettext("Using sound parameters: channels, rate, bits, records ")
...
#if !defined GIAC_GGB &&                                                                           \
    (defined EMCC ||                                                                               \
     defined(EMCC2)) // must have EM_ASM code javascript inlined (emscripten 1.30.4 at least?)
#include <emscripten.h>
gen _playsnd(const gen & args, GIAC_CONTEXT) {
  if (args.type == _STRNG) {
    if (args.subtype == -1)
...
define_unary_function_ptr5(at_writewav, alias_at_writewav, &__writewav, 0, true);
    ...
    for (int i = 3; i < s; ++i) {
      if (is_equal(v[i])) {
        ...
        if (f.type == _VECT && f._VECTptr->size() == 2 && f._VECTptr->front().type == _INT_ &&
            f._VECTptr->front().val == _FRAMES && f._VECTptr->back().type == _INT_)
  ...
  static bool writergb(const string & filename, const vecteur & v) {
    ...
    if (v.size() != 5 || !ckmatrix(v[1]) || !ckmatrix(v[2]) || !ckmatrix(v[3]) || !ckmatrix(v[4]))
      return false;
...
static bool writergb(const string & s, const vecteur & w) {
  ...
  gen _writergb(const gen & g, GIAC_CONTEXT) {
    ...
    vecteur arg =
        makevecteur(w[0][2], 0, w[0][1] / w[0][2], symb_equal(tmp, makevecteur(v[0], undef)));
  ...
  gen _readrgb(const gen & g, GIAC_CONTEXT) {
    ...
    bool ok = false;
    if (readrgb_ptr)
      ok = readrgb_ptr(*v[0]._STRNGptr, ((s > 2 && v[1].type == _INT_) ? v[1].val : 0),
                       ((s > 2 && v[2].type == _INT_) ? v[2].val : 0), res);
    if (!ok)
      return gensizeerr(gettext("File not found or unrecognized image file format"));
    return res;
  }
  ...
  gen linear_apply(const gen & e, const gen & x, const gen & l, gen & remains, GIAC_CONTEXT,
                   gen (*f)(const gen &, const gen &, const gen &, gen &, const context *)) {
  ...
  gen product(const polynome & P, const vecteur & v, const gen & n, gen & remains, GIAC_CONTEXT) {
    ...
    if (!factor(P, Pcont, f, /* is_primitive*/ false, /* with_sqrt*/ true, /* complex */ true,
                divan, extra_div) ||
        extra_div != 1) {
    ...
    for (; it != itend; ++it) {
      ...
      if (it->fact.lexsorted_degree() != 1) {
        ...
        res = res * pow(a, it->mult * n, contextptr) *
              pow(symbolic(at_factorial, n + b / a - 1), it->mult, contextptr);
      }
    }
  ...
  gen product(const polynome & P, const vecteur & v, const gen & n, const gen & a, const gen & b,
              GIAC_CONTEXT) {
  ...
  static bool rsolve_particular(const modpoly & vnext, const modpoly & v, const modpoly & vcst,
                                modpoly & sol, GIAC_CONTEXT) {
    // find majoration of degree for solution
    int vnextdeg = int(vnext.size()) - 1, vdeg = int(v.size()) - 1, vcstdeg = int(vcst.size()) - 1,
        soldeg;
    ...
    modpoly equations =
        operator_plus(operator_times(vnext, taylor(vars, 1, 0), 0), operator_times(v, vars, 0), 0);
  ...
  static gen rsolve(const gen & e, const gen & n, const gen & l0, gen & remains, GIAC_CONTEXT) {
    ...
    if (a == l) { // let v(n)=u(n)/l^n, then v(n+1)=v(n)+P(n)/l
      return pow(l0, n, contextptr) * sum(r2e(P, v, contextptr), n, remains, contextptr) /
             r2e(l, v1, contextptr) / r2e(P0d, v, contextptr);
    }
  ...
  */
  ...
  gen _seqsolve(const gen & args, GIAC_CONTEXT) {
    ...
    if (!is_zero(derive(m, x, contextptr))) {
      if (f._VECTptr->size() == 1 && x._VECTptr->size() == 1 && is_zero(derive(f, n, contextptr))) {
        ...
        if (is_linear_wrt(tmpn, var, a, b, contextptr) &&
            is_linear_wrt(tmpd, var, c, d, contextptr) && !is_zero(c)) {
    ...
    vecteur cst =
        *normal(subvecteur(*f._VECTptr, multmatvecteur(m, *x._VECTptr)), contextptr)._VECTptr;
    if (m.size() == 1 && cst.size() == 1) {
      ...
      if (!is_zero(derive(l, n, contextptr))) {
        ...
        gen res = pow(subst(normal(l / pn, contextptr), n, 0, false, contextptr), n, contextptr) *
                  simplify(product(P, v, n, 0, n - 1, contextptr) /
                               product(Q, v, n, 0, n - 1, contextptr),
                           contextptr) *
                  pow(q0r0, n * (n - 1) / 2, contextptr);
        // then we might search for a polynomial particular solution to e
        if (is_zero(c))
          return u0 / subst(res, n, 0, false, contextptr) * res;
        if (!is_one(q0r0))
          return gensizeerr("Unable to find particular solution, general solution is " +
        ...
        if (is_zero(derive(tmpnum, n, contextptr)) && is_zero(derive(tmpnum, n, contextptr)) &&
        ...
        *logptr(contextptr) << gettext(
                                   "Unable to find a particular solution for inhomogeneous part")
                            << '\n';
        return symbolic(at_seqsolve, args);
      }
  ...
  static vecteur rsolve_initcond(const vecteur & initcond, const gen & un, const gen & u,
                                 const gen & n, const vecteur & uinit, GIAC_CONTEXT) {
    if (initcond.empty())
      return makevecteur(un);
    gen initc = subst(initcond, u, _unapply(gen(makevecteur(un, n), _SEQ__VECT), contextptr), false,
                      contextptr);
  ...
  static gen crsolve(const gen & f0, const gen & u, const gen & n, vecteur & initcond0,
                     GIAC_CONTEXT) {
    if (f0.type == _VECT) {
    ...
    for (const_iterateur it = vof0.begin(); it != vof0.end(); ++it) {
      ...
      int pos;
      if (itf.type == _VECT && itf._VECTptr->size() == 2 &&
          (pos = equalposcomp(uv, itf._VECTptr->front()))) {
    ...
    if (all_one) {
      ...
      if (uvs > 1) {
        ...
        vecteur Fv = gsolve(*F._VECTptr, idX, 1 /* complex */, 0 /* approx mode=no*/, contextptr),
                res;
        if (is_undef(Fv))
          return Fv;
        for (const_iterateur it = Fv.begin(); it != Fv.end(); ++it) {
          ...
          if (initc.type != _VECT)
            return gensizeerr();
          vecteur valv =
              gsolve(*initc._VECTptr, idx, /* complex mode */ true, /* approx=no */ 0, contextptr);
          if (is_undef(valv))
            return valv;
          for (unsigned int i = 0; i < valv.size(); ++i) {
  ...
  static gen rsolve(const gen & f0, const gen & u, const gen & n, vecteur & initcond0, int st,
                    GIAC_CONTEXT) {
    bool b = complex_mode(contextptr);
    complex_mode(true, contextptr);
  ...
  gen _array(const gen & g, GIAC_CONTEXT) {
    ...
    if (g.type == _VECT && g.subtype == _SEQ__VECT) {
      ...
      if (!args.empty()) {
        gen f = args.back();
        if (f == at_float ||
            (f.is_symb_of_sommet(at_equal) && f._SYMBptr->feuille[0] == at_dtype)) {
          args.pop_back();
          return _convert(
              makesequence(
                  _array(args.size() == 1 ? args.front() : gen(args, _SEQ__VECT), contextptr),
                  f.type == _FUNC ? f : f._SYMBptr->feuille[1]),
              contextptr);
        }
      }
    } else
  ...
  int is_int_or_double(const gen & g) {
    ...
    if (g.type == _SYMB) {
      if (g._SYMBptr->sommet == at_floor || g._SYMBptr->sommet == at_irem ||
          g._SYMBptr->sommet == at_iquo || g._SYMBptr->sommet == at_size)
        return 2;
      vecteur v = lvar(g._SYMBptr->feuille);
      return is_int_or_double(
          v); // this assumes that the function has the same type as the arguments
    }
    if (g.type != _VECT)
      return 0;
  ...
  int cpp_vartype(const gen & g, const vecteur & allnames, const vecteur & allchecks) {
    ...
    if (g.type == _SYMB) {
      ...
      if (g._SYMBptr->sommet == at_floor || g._SYMBptr->sommet == at_irem ||
          g._SYMBptr->sommet == at_iquo)
        return 2;
      if (g._SYMBptr->sommet == at_size)
        return 2;
      if (g._SYMBptr->sommet == at_abs &&
          cpp_vartype(g._SYMBptr->feuille, allnames, allchecks) == 3)
        return 2;
      vecteur v = lvar(g._SYMBptr->feuille);
      const_iterateur it = v.begin(), itend = v.end();
      int cur = -1;
  ...
  std::string cprintvars(const gen & f0, bool cst, bool ctx0, const string & sep, int * typeptr,
                         const vecteur & allnames, const vecteur & allchecks, GIAC_CONTEXT) {
    ...
    for (int i = 0;;) {
      ...
      if (eq || eqsto) {
        ...
        if (!i || i == gt || (i == 2 && gt == _DOUBLE_) || (i == _DOUBLE_ && gt == 2))
          ...
          vtype = vtype + " = cpp_convert_" + print_INT_(i) + "(" + cprint(g, 0, contextptr) +
                  (ctx0 ? ",giac::context0)" : ",contextptr)");
#else
      vtype = vtype + '=';
  ...
  std::string cpp_stoprint(const gen & g, const vecteur & allnames, const vecteur & allchecks,
                           GIAC_CONTEXT) {
    if (g.is_symb_of_sommet(at_at) && g._SYMBptr->feuille.type == _VECT &&
      ...
      if (t != 2)
        return cprint(f, 0, contextptr) + "[cpp_convert_2(" + cprint(i, 0, contextptr) +
               ",contextptr)]";
      else
  ...
  std::string cprint(const gen & args, const gen & name_, const vecteur & allnames,
                     const vecteur & allchecks, GIAC_CONTEXT) {
    ...
    if (args.type == _VECT) {
      ...
      for (int i = 0;; ++i) {
        s += cprint(*it, (name.type == _VECT && i < name._VECTptr->size()) ? name[i] : zero,
                    allnames, allchecks, contextptr);
        ++it;
        if (it == itend) {
          break;
        }
    ...
    if (args.type == _CPLX)
      return "gen(" + args._CPLXptr->print(contextptr) + "," +
             (args._CPLXptr + 1)->print(contextptr) + ")";
    if (args.type == _FUNC) {
      if (args == at_break)
        return "break";
      if (args == at_continue)
    ...
    if ((u == at_sto || u == at_array_sto) && f.type == _VECT && f._VECTptr->size() == 2) {
      ...
      if (f[1].type == _VECT) {
        ...
        int vs = v.size();
        res += "giac::vecteur __tmp_=giac::makevecteur(" +
               cprint(f[0], 0, allnames, allchecks, contextptr) + ");\n";
        for (int j = 0; j < vs; ++j) {
          gen var = f[1][j];
          int i = cpp_vartype(var, allnames, allchecks);
          res += cpp_stoprint(var, allnames, allchecks, contextptr) + " = cpp_convert_" +
                 print_INT_(i) + "(" + "__tmp_[" + print_INT_(j) + "],contextptr);";
        }
        return res + "\n}";
      }
      ...
      if (!i || i == f0t || (i == _DOUBLE_ && f0t == 2) || (i == 2 && f0t == _DOUBLE_))
        return cpp_stoprint(f[1], allnames, allchecks, contextptr) + '=' +
               cprint(f[0], 0, allnames, allchecks, contextptr);
      ;
      string res = cpp_stoprint(f[1], allnames, allchecks, contextptr) + " = cpp_convert_" +
                   print_INT_(i) + "(" + cprint(f[0], 0, allnames, allchecks, contextptr) +
                   ",contextptr)";
      return res;
#else
    ...
    if (u == at_equal && f.type == _VECT && f._VECTptr->size() == 2) {
      ...
      if (!i || i == cpp_vartype(f[1], allnames, allchecks))
        return cpp_stoprint(f[0], allnames, allchecks, contextptr) + '=' +
               cprint(f[1], 0, allnames, allchecks, contextptr);
      string res = cpp_stoprint(f[0], allnames, allchecks, contextptr) + " = cpp_convert_" +
                   print_INT_(i) + "(" + cprint(f[1], 0, allnames, allchecks, contextptr) +
                   ",contextptr)";
      return res;
#else
    ...
    if (u == at_size) {
      ...
      return "cpp_convert_7(" + cprint(f, 0, allnames, allchecks, contextptr) +
             ",contextptr).size()";
    }
    if (f.type == _VECT && f._VECTptr->size() == 2) {
      if (u == at_at) {
        ...
        if (i == 0 || j == 2)
          return f._VECTptr->front().print(contextptr) + "[" +
                 cprint(f._VECTptr->back(), 0, allnames, allchecks, contextptr) + "]";
        return f._VECTptr->front().print(contextptr) + "[cpp_convert_2(" +
               cprint(f._VECTptr->back(), 0, allnames, allchecks, contextptr) + ",contextptr)]";
      }
      if (u == at_of) {
          ...
          if (evaled_fnc.is_symb_of_sommet(at_program)) {
            ...
            if (F.type == _VECT && F._VECTptr->size() == 3) {
              ...
              if (argtype.size() == argF.size()) {
                cprintvars(F[0], false, true, ",", &argtype.front(), allnames, allchecks,
                           contextptr);
                ...
                for (size_t pos = 0; pos < argF.size(); ++pos) {
                  ...
                  if (i == j || (i == 0 || j == 2))
                    ...
                    res += "cpp_convert_" + print_INT_(i) + "(" +
                           cprint(argF[pos], 0, allnames, allchecks, contextptr) + ",contextptr)";
                  if (pos != argF.size() - 1)
                    res += ',';
                }
                res += ')';
      ...
      if (f[0].type != _VECT || f[0]._VECTptr->size() <= 1) {
        if (f[0].type == _VECT && f[0]._VECTptr->empty())
          ...
          if (0 && name_.is_symb_of_sommet(at_deuxpoints))
            core = "return cpp_convert_" + print_INT_(name_._SYMBptr->feuille[1].val) + "(" +
                   heads + "(cpp_convert_" +
                   print_INT_(cpp_vartype(f[0]._VECTptr->front(), allnames, allchecks)) +
                   "(g,contextptr),contextptr));";
          else
            core = "return " + heads + "(cpp_convert_" +
                   print_INT_(cpp_vartype(f[0]._VECTptr->front(), allnames, allchecks)) +
                   "(g,contextptr),contextptr);";
#else
        switch (convert) {
        case 0:
          ...
          break;
        case 2:
          core = "if (g.type!=_INT_) return gensizeerr(contextptr); else return " + heads +
                 "(g.val,contextptr);";
          break;
        case _DOUBLE_:
          core = "if (g.type!=_DOUBLE_) return gensizeerr(contextptr); else return " + heads +
                 "(g._DOUBLE_val,contextptr);";
          break;
        }
#endif
        }
        ...
        core = "if (g.type!=_VECT || g.subtype!=_SEQ__VECT || g._VECTptr->size()!=" +
               print_INT_(nargs) + ") return gendimerr(contextptr);\n";
    ...
    if (u == at_irem && f.type == _VECT && f._VECTptr->size() == 2) {
      ...
      if ((f0t == 2 || f0t == _DOUBLE_) && (f1t == 2 || f1t == _DOUBLE_))
        return "long(" + cprint(f[0], 0, allnames, allchecks, contextptr) + ") % long(" +
               cprint(f[1], 0, allnames, allchecks, contextptr) + ")";
    }
    if (u == at_floor) {
      ...
      if (f.type == _VECT && f._VECTptr->size() == 2) {
        ...
        if (cpp_vartype(f0, allnames, allchecks) == _VECT) {
          ...
          if (f1.is_symb_of_sommet(at_prepend))
            return f0s + ".insert(" + f0s + ".begin()," +
                   cprint(f1f, 0, allnames, allchecks, contextptr) + ")";
    ...
    if (u == at_sin || u == at_cos || u == at_tan || u == at_asin || u == at_acos || u == at_atan ||
        u == at_exp || u == at_ln || u == at_abs) {
      if (idf)
        return "std::" + args.print(contextptr);
    } else
      res = '_' + res;
    if (u.ptr()->printsommet == printsommetasoperator || u == at_division ||
        (idf0 && (u == at_inferieur_strict || u == at_inferieur_egal || u == at_superieur_strict ||
                  u == at_superieur_egal))) {
      int rs = int(res.size());
      if ((idf && (rs != 2 || res[1] != '^')) ||
          (rs == 2 && (res[1] == '+' || res[1] == '*' || res[1] == '>' || res[1] == '<')) ||
          (rs == 3 && (res[1] == '>' || res[1] == '<') && res[2] == '=')) {
    ...
    if (u == at_increment || u == at_decrement) {
      ...
      if (f.type == _VECT && f._VECTptr->size() == 2 && is_int_or_double(f._VECTptr->front()) &&
          is_int_or_double(f._VECTptr->back())) {
        return cprint(f._VECTptr->front(), 0, allnames, allchecks, contextptr) +
               (u == at_increment ? "+=" : "-=") +
               cprint(f._VECTptr->back(), 0, allnames, allchecks, contextptr);
      }
    }
    if (u == at_same || u == at_different || u == at_and || u == at_ou) {
      return cprint(args._SYMBptr->feuille[0], -1, allnames, allchecks, contextptr) + " " +
             u.ptr()->s + " " +
             cprint(args._SYMBptr->feuille[1], -1, allnames, allchecks, contextptr);
    }
    if (u == at_neg || u == at_not)
      return u.ptr()->s + cprint(args._SYMBptr->feuille, -1, allnames, allchecks, contextptr);
    if (u == at_inferieur_egal)
      return "giac::is_greater(" +
             cprint(args._SYMBptr->feuille[1], -1, allnames, allchecks, contextptr) + "," +
             cprint(args._SYMBptr->feuille[0], 0, allnames, allchecks, contextptr) + ",contextptr)";
    if (u == at_superieur_egal)
      return "giac::is_greater(" +
             cprint(args._SYMBptr->feuille[0], -1, allnames, allchecks, contextptr) + "," +
             cprint(args._SYMBptr->feuille[1], 0, allnames, allchecks, contextptr) + ",contextptr)";
  ...
  int cpp_write_compile(const string & filename, const string & funcname, const string & s,
                        GIAC_CONTEXT) {
    ofstream of(filename.c_str());
#ifdef __APPLE__
    of << "// -*- mode:c++; compile-command:\" c++ -Wall -fno-strict-aliasing "
          "-I/Applications/usr/include -I.. -I. -fPIC -DPIC -g -O2 -dynamiclib giac_"
       << funcname << ".cpp -o libgiac_" << funcname
       << ".dylib -L/Applications/usr/lib -L/Applications/usr/64/local/lib -lgiac \" -*-" << '\n';
#else
    of << "// -*- mode:c++; compile-command:\" c++ -Wall -fno-strict-aliasing -I.. -I. -fPIC -DPIC "
          "-g -O2 -c giac_"
       << funcname << ".cpp -o giac_" << funcname << ".lo && cc -shared giac_" << funcname
       << ".lo -lgiac -lc -Wl,-soname -Wl,libgiac_" << funcname << "so.0 -o libgiac_" << funcname
       << ".so.0.0.0 && ln -sf libgiac_" << funcname << ".so.0.0.0 libgiac_" << funcname
       << ".so\" -*-" << '\n';
#endif
    ...
    of << "inline bool operator <= (const gen & a,const gen &b){ return "
          "is_greater(b,a,giac::context0); }\ninline bool operator >= (const gen & a,const gen "
          "&b){ return is_greater(a,b,giac::context0); }"
       << '\n';
    of << s << '\n';
...
#endif
#ifdef __APPLE__
    cmd = "g++ -Wall -fno-strict-aliasing -dynamiclib -I/Applications/usr/include -I.. -I. -fPIC "
          "-DPIC -g -O2 -dy giac_" +
          funcname + ".cpp -o libgiac_" + funcname +
          ".dylib -L/Applications/usr/lib -L/Applications/usr/64/local/lib -lgiac";
#else
    cmd = "c++ -Wall -fno-strict-aliasing -I.. -I. -fPIC -DPIC -g -O2 -c giac_" + funcname +
          ".cpp -o giac_" + funcname + ".lo";
#endif
    *logptr(contextptr) << "Running\n" << cmd << '\n';
    ...
    //not_ok=system_no_deprecation(cmd.c_str());
#ifndef __APPLE__
    cmd = "cc -shared giac_" + funcname + ".lo -lgiac -lc -Wl,-soname -Wl,libgiac_" + funcname +
          ".so.0 -o libgiac_" + funcname + ".so.0.0.0";
  ...
  gen _cpp(const gen & args, GIAC_CONTEXT) {
    ...
    if (is_undef(tmp))
      return tmp;
    int x = xcas_mode(contextptr), l = language(contextptr), l0 = language(context0),
        p = python_compat(contextptr);
    if (args.type == _IDNT) {
      xcas_mode(0, contextptr);
      language(2, contextptr);
      language(2, context0);
      ...
      if (args_evaled.is_symb_of_sommet(at_program))
        ...
        if (args_evaled.type == _DOUBLE_)
          ...
          if (args_evaled.type == _INT_)
            ...
            s += "const gen " + arg.print(contextptr) + "(\"" + args_evaled.print(contextptr) +
                 "\",context0);";
        }
      }
    }
  ...
  define_unary_function_ptr5(at_giac_hex, alias_at_giac_hex, &__giac_hex, 0, true) gen
      _octprint(const gen & g, GIAC_CONTEXT) {
    if (g.type == _STRNG && g.subtype == -1)
      return g;
    if (g.type == _INT_)
