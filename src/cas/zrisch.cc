...
namespace giac {
  ...
  static bool in_risch(const gen & e, const identificateur & x, const vecteur & v,
                       const gen & allowed_lnarg, gen & prim, gen & lncoeff,
                       gen & remains_to_integrate, GIAC_CONTEXT);
  static bool risch_poly_part(const vecteur & e, int shift, const identificateur & x,
                              const vecteur & v, const gen & allowed_lnarg, gen & prim,
                              gen & lncoeff, gen & remains_to_integrate, GIAC_CONTEXT);
  static bool risch_desolve(const gen & f, const gen & g, const identificateur & x,
                            const vecteur & v, gen & y, bool f_is_derivative, GIAC_CONTEXT);
  ...
  static polynome rothstein_trager_resultant(const polynome & num, const polynome & den,
                                             const vecteur & vl, polynome & p1, GIAC_CONTEXT) {
    int s = num.dim;
  ...
  */
  ...
  // diff(n/d*exp(a*x))
  static bool diff(const polynome & n, const polynome & d, const gen & a, const vecteur & v,
                   polynome & resn, polynome & resd) {
    int s = n.dim;
  ...
  static bool spde_x(const polynome & S0, const polynome & R0, const polynome & T0,
                     const vecteur & vdiff, polynome & N1,
                     polynome & N2) { // Solve S*N+R*N'=T, R constant poly
  ...
  static bool SPDE(const polynome & R0, const polynome & S0, const polynome & T0,
                   const identificateur & x, const vecteur & v, const vecteur & vdiff,
                   const vecteur & lv, int ydeg, gen & prim, GIAC_CONTEXT) {
    ...
    if (R.lexsorted_degree()) {
      ...
      polynome dRnum(gen2poly(dR.num, s)), dRden(gen2poly(dR.den, s)), dVnum(gen2poly(dV.num, s)),
          dVden(gen2poly(dV.den, s));
      polynome newR(R * dRden * dVden), newS((S * dRden + dRnum) * dVden),
          newT((((U * dVden) / c) - dVnum) * dRden);
      ydeg = ydeg - r;
    ...
    for (int k = tdeg; k >= 0; --k) {
      if (Z.is_symb_of_sommet(at_exp))
        ...
        ok = risch_desolve(b, t[tdeg - k] - (k + 1) * previous * dz / z, x, v1, sol, false,
                           contextptr);
      if (!ok)
        return false;
  ...
  static bool risch_desolve(const gen & f, const gen & g, const identificateur & x,
    ...
    if (Z.is_symb_of_sommet(at_exp)) {
      ...
      int fdenv = fden.valuation(0), fnumv = fnum.valuation(0), gdenv = gden.valuation(0),
          gnumv = gnum.valuation(0);
      index_t ii(ss);
      ii[0] = -fdenv;
      ...
      if (gamma < 0) {
        ...
        if (beta <= 0) {
          ...
          if (in_risch(f0, x, v1, Z._SYMBptr->feuille, prim, lnc, remains, contextptr) &&
              lnc.type == _INT_)
            alpha = giacmin(lnc.val, alpha);
#endif
          }
    ...
    polynome R(D * lcmdeno), S(fnum * (lcmdeno / fden) * D - dDnum * (lcmdeno / dDden)),
        T(D * D * gnum * (lcmdeno / gden));
  ...
  static pf<gen> hermite_reduce(const pf<gen> & p_cst, const gen & a, const vecteur & v_derivatives,
                                const vecteur & lv, gen & prim, GIAC_CONTEXT) {
    pf<gen> p(p_cst);
    if (p.mult <= 0) {
  ...
  static bool integrate_ei(const gen & a, const gen & coeff, const identificateur & x, gen & prim,
                           gen & remains_to_integrate, GIAC_CONTEXT) {
    ...
    prim += _integrate(
        gen(makevecteur(expax * r2sym(ipnum, l, contextptr) / r2sym(ipden, l, contextptr), x),
            _SEQ__VECT),
        contextptr);
    ...
    for (; it != itend; ++it) {
      ...
      for (; f_it != f_itend; ++f_it) {
        ...
        if (!deg)
          continue;
        if (deg != 1) {
          remains_to_integrate = remains_to_integrate + expax * r2sym(it->num, l, contextptr) /
                                                            r2sym(it->den, l, contextptr);
          break;
        }
        // extract the root
        vecteur vtmp = polynome2poly1(f_it->fact, 1);
        ...
        if (is_zero(im(root, contextptr)) && is_zero(rea)) {
          prim += (cos(ima * root, contextptr) + cst_i * sin(ima * root, contextptr)) *
                  (r2sym(rootn / rootd, l1, contextptr) *
                   (symbolic(at_Ci, (imaneg ? (-ima) : ima) * (x - root)) +
                    (imaneg ? (-cst_i) : cst_i) *
                        symbolic(at_Si, (imaneg ? (-ima) : ima) * (x - root))));
        } else
          prim += r2sym(rootn / rootd, l1, contextptr) * exp(a * root, contextptr) *
                  symbolic(at_Ei, a * (x - root));
      }
    }
    return true;
  }
  ...
  static bool risch_poly_part(const vecteur & e, int shift, const identificateur & x,
    ...
    if (X.is_symb_of_sommet(at_ln)) {
      ...
      for (int j = s - 1; j > 0; --j) {
        ...
        if (!in_risch(e[s - 1 - j] - (j + 1) * eprim[s - 1 - j] * dX, x, v1, X._SYMBptr->feuille,
                      eprim[s - j], lnc, remains, contextptr)) {
      ...
      gen prim_add;
      bool ok =
          in_risch(e[s - 1] - eprim[s - 1] * dX, x, v1, zero, prim_add, lnc, remains, contextptr);
    ...
    for (int j = s - 1; j >= 0; --j) {
      if (j + shift == 0) {
        bool ok =
            in_risch(e[s - 1 - j], x, v1, allowed_lnarg, prim_add, lncoeff, remains, contextptr);
        prim = prim + prim_add;
        remains_to_integrate = remains_to_integrate + remains;
        if (!ok && !is_zero(allowed_lnarg))
          return ok;
      } else {
        if (!risch_desolve((j + shift) * dY, e[s - 1 - j], x, v1, eprim[s - 1 - j], true,
                           contextptr)) {
          gen coeff = e[s - 1 - j], pui = j + shift;
          gen a, b, c, d;
          if (is_zero(allowed_lnarg) && is_linear_wrt(X._SYMBptr->feuille, x, a, b, contextptr) &&
              lvarx(coeff, x) == vecteur(1, x) &&
              integrate_ei(pui * a, coeff, x, c, d, contextptr)) {
  ...
  static bool in_risch(const gen & e, const identificateur & x, const vecteur & v,
    ...
    for (; it != itend; ++it) {
      if (v.front().is_symb_of_sommet(at_exp) && it->fact.coord.size() == 1) {
        ...
        if (!risch_poly_part(tmp, -it->mult, x, v, allowed_lnarg, prim, lncoeff,
                             remains_to_integrate, contextptr) &&
            !is_zero(allowed_lnarg))
          return false;
        ...
        if (!is_zero(allowed_lnarg)) { // if only 1 log allowed
          ...
          lncoeff =
              normal(allowed_lnarg * r2sym(it->num, l, contextptr) /
                         (derive(allowed_lnarg, x, contextptr) * r2sym(it->den, l, contextptr)),
                     contextptr);
        ...
        for (; f_it != f_itend; ++f_it) {
          ...
          if (deg != 1) {
            ...
            remains_to_integrate = remains_to_integrate +
                                   r2sym(it->num, l, contextptr) / r2sym(it->den, l, contextptr);
            break;
          }
          // extract the root
          vecteur vtmp = polynome2poly1(f_it->fact, 1);
          ...
          if (!is_zero(derive(root, x, contextptr))) {
            ...
            remains_to_integrate = remains_to_integrate +
                                   r2sym(tmp.num, l, contextptr) / r2sym(tmp.den, l, contextptr);
            break;
          }
          identificateur t(" t");
          ...
          tmp1 = subst(tmp1, t, root, false, contextptr);
          gen tmparg =
              _gcd(makevecteur(recursive_normal(tmp1, contextptr),
                               recursive_normal(r2sym(it->fact, l, contextptr), contextptr)),
                   contextptr);
          ...
          ipshift = ipshift - root * _quo(makevecteur(derive(tmparg, x, contextptr), tmparg,
                                                      recursive_normal(v.front(), contextptr)),
                                          contextptr);
          if (is_undef(ipshift))
            return false;
        }
    ...
    if (tmp.empty())
      ...
      tmp.back() += ipshift;
    if (!risch_poly_part(tmp, 0, x, v, allowed_lnarg, prim, lncoeff, remains_to_integrate,
                         contextptr) &&
        !is_zero(allowed_lnarg))
      return false;
    return true;
  ...
  static gen risch_lin(const gen & e_orig, const identificateur & x, gen & remains_to_integrate,
                       GIAC_CONTEXT) {
  ...
  gen risch(const gen & e_orig, const identificateur & x, gen & remains_to_integrate,
            GIAC_CONTEXT) {
