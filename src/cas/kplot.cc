...
double gnuplot_xmin = -5, gnuplot_xmax = 5, gnuplot_ymin = -5, gnuplot_ymax = 5, gnuplot_zmin = -5,
  ...
  static bool in_autoscale(const gen & g, vector<double> & vx, vector<double> & vy,
                           vector<double> & vz, GIAC_CONTEXT) {
    ...
    if (g.is_symb_of_sommet(at_cercle)) {
      ...
      if (g._SYMBptr->feuille.type == _VECT && g._SYMBptr->feuille._VECTptr->size() >= 3) {
        ...
        v = makevecteur(c, c + r * expi((v1 - 0.1 * delta), contextptr),
                        ...
                        c + r * expi(v2, contextptr), c + r * expi((v2 + 0.1 * delta), contextptr));
      } else
  ...
  bool autoscaleg(const gen & g, vector<double> & vx, vector<double> & vy, vector<double> & vz,
                  GIAC_CONTEXT) {
    if (g.type == _VECT) {
      bool ortho = false;
      const_iterateur it = g._VECTptr->begin(), itend = g._VECTptr->end();
    ...
    if (g.is_symb_of_sommet(at_equal) && g._SYMBptr->feuille.type == _VECT &&
        ...
        g._SYMBptr->feuille._VECTptr->front() == _GL_ORTHO &&
        !is_zero(g._SYMBptr->feuille._VECTptr->back()))
      return true;
    return false;
  }
  ...
  define_unary_function_ptr5(at_parameter, alias_at_parameter, &__parameter, _QUOTE_ARGUMENTS,
                             true);
  ...
  gen _legende(const gen & args, GIAC_CONTEXT) {
    ...
    return symb_pnt1(
        makesequence(f, makevecteur(s == 3 ? (*args._VECTptr)[2] : FL_BLACK | _POINT_POINT),
                     (*args._VECTptr)[1]));
  }
  ...
  gen plotfunc(const gen & f_, const gen & vars, const vecteur & attributs, bool densityplot,
    ...
    if (vars.type == _IDNT) { // function plot
        ...
        if (yy.type != _DOUBLE_) {
          if (!chemin.empty())
            res.push_back(pnt_attrib(
                symb_curve(gen(makevecteur(gen(vars, f), vars, xmin, i, showeq), _PNT__VECT),
      ...
      if (!chemin.empty()) {
        res.push_back(pnt_attrib(
            symb_curve(gen(makevecteur(gen(vars, f), vars, xmin, i - step, showeq), _PNT__VECT),
                       gen(chemin, _GROUP__VECT)),
            attributs.empty() ? color : attributs, contextptr));
      }
  ...
  gen plotseq(const gen & f, const gen & x, double x0, double xmin, double xmax, int niter,
              const vecteur & attributs, const context * contextptr) {
    if (xmin > xmax)
    ...
    vecteur g(gen2vecteur(
        _plotfunc(makesequence(f, symb_equal(x, symb_interval(xmin, xmax))), contextptr)));
  ...
  gen _cercle(const gen & args, GIAC_CONTEXT) {
    ...
    gen res = pnt_attrib((symbolic(at_cercle, gen(makenewvecteur(diametre, a1, a2), _PNT__VECT))),
                         attributs, contextptr);
    if (s < 3 + narg)
      return res;
    vecteur w(1, res);
  ...
  static gen equation(const gen & arg, const gen & x, const gen & y, const gen & z, GIAC_CONTEXT) {
    ...
    if ((e.type == _SYMB) && (e._SYMBptr->sommet == at_cercle)) {
      ...
      return symb_equal(
          pow(x - re(centre, contextptr), 2) + pow(y - im(centre, contextptr), 2),
          r); //symbolic(at_equal,makesequence(pow(x-re(centre,contextptr),2)+pow(y-im(centre,contextptr),2),r));
    }
    if ((e.type == _VECT) && (e._VECTptr->size() == 2)) {
      ...
      if (!is_zero(b, contextptr))
        return symb_equal(
            y,
            normal(-a / b, contextptr) * x +
                normal(
                    c / b,
                    contextptr)); //symbolic(at_equal,makesequence(y,normal(-a/b,contextptr)*x+normal(c/b,contextptr)));
      return symb_equal(
          x,
          normal(
              c / a,
              contextptr)); //symbolic(at_equal,makesequence(x,normal(c/a,contextptr)));//symbolic(at_equal,makesequence(a*x+b*y,c));
    }
    if ((e.type == _SYMB) && (e._SYMBptr->sommet == at_curve)) {
      vecteur v = *e._SYMBptr->feuille._VECTptr->front()._VECTptr;
      if (v.size() >= 6 && !is_undef(v[5]))
        return symb_equal(
            subst(v[5], makevecteur(x__IDNT_e, y__IDNT_e, z__IDNT_e), makevecteur(x, y, z), false,
                  contextptr),
            0); //symbolic(at_equal,makesequence(subst(v[5],makevecteur(x__IDNT_e,y__IDNT_e,z__IDNT_e),makevecteur(x,y,z),false,contextptr),0));
      return gensizeerr(contextptr);
    }
    return e;
  }
    ...
    for (int i = 0; i < imax - 1; ++i) {
      ...
      for (int j = 0; j < jmax - 1; ++j) {
        ...
        for (int k = 0; k < nz; k++) {
          ...
          if (d == z)
            d += zeps;
          bool ab = (a - z) * (b - z) <= 0, ca = (a - z) * (c - z) <= 0,
               db = (d - z) * (b - z) <= 0, cd = (d - z) * (c - z) <= 0;
          gen AB, CA, DB, CD;
          // intercepts
          if (ab)
  ...
  gen _plot(const gen & g, const context * contextptr) {
    ...
    bool v0cst = lidnt(evalf(v0, 1, contextptr)).empty(),
    ...
    return plotfunc(v[0], xvar, attributs, false, xmin, xmax, ymin, ymax, zmin, zmax, nstep, 0,
                    showeq, contextptr);
  }
  ...
  gen plotparam(const gen & f, const gen & vars, const vecteur & attributs, bool densityplot,
                double function_xmin, double function_xmax, double function_ymin,
    ...
    for (int count = 0; !ctrl_c && !interrupted && count <= nstep; ++count, t += function_tstep) {
      ...
      if (xy.type == _VECT && xy._VECTptr->size() == 2) {
        ...
        if (xy_.type == _VECT && xy_._VECTptr->size() == 2) {
          ...
          reim(
              xy_, x_, y_,
              contextptr); // x_=re(xy_,newcontextptr); y_=im(xy_,newcontextptr).evalf_double(elevel,newcontextptr);
  ...
  gen paramplotparam(const gen & args, bool densityplot, const context * contextptr) {
    ...
    */
    ...
    if (s > 7)
      vparam = vargs[7];
    double tmin = gnuplot_tmin, tmax = gnuplot_tmax, xmin = gnuplot_xmin, xmax = gnuplot_xmax,
           ymin = gnuplot_ymin, ymax = gnuplot_ymax, tstep = -1;
    int elevel = eval_level(contextptr);
    if (param2d && s >= 3) {
      gen trange = vargs[2].evalf_double(elevel, contextptr);
      if ((trange.type == _SYMB) && (trange._SYMBptr->sommet == at_interval)) {
        tmin = evalf_double(trange._SYMBptr->feuille._VECTptr->front(), elevel, contextptr)
                   ._DOUBLE_val;
        tmax =
            evalf_double(trange._SYMBptr->feuille._VECTptr->back(), elevel, contextptr)._DOUBLE_val;
        if (s > 3)
    ...
    for (int i = 1; i < s; ++i) {
      ...
      if (vargsi.is_symb_of_sommet(at_equal) && vargsi._SYMBptr->feuille.type == _VECT &&
          vargsi._SYMBptr->feuille._VECTptr->front().type == _INT_) {
  ...
  gen _LineTan(const gen & g, GIAC_CONTEXT) {
    ...
    return put_attributs(_droite(makesequence(M0, M0 + direction), contextptr), attributs,
                         contextptr);
  }
    ...
    for (double curx = xmin; curx <= xmax; curx += scaling * xstep) {
      ...
      for (double cury = ymin; cury <= ymax; cury += scaling * ystep) {
        ...
        if ((xp_eval.type == _DOUBLE_) && (yp_eval.type == _DOUBLE_)) {
          ...
          res.push_back(
              pnt_attrib(gen(makevecteur(origine, origine + gen(echelle * xpd, echelle * ypd)),
