...
static double _epsilon_ = 1e-12;
  ...
  thread_param::thread_param()
      : _kill_thread(false), thread_eval_status(-1), v(6)
#ifdef HAVE_LIBPTHREAD
#ifdef __MINGW_H
        ,
  ...
  bool kill_thread(GIAC_CONTEXT) {
    thread_param * ptr =
        (contextptr && contextptr->globalptr) ? contextptr->globalptr->_thread_param_ptr : 0;
...
#ifdef HAVE_LIBPTHREAD
  pthread_mutex_t _mutexptr = PTHREAD_MUTEX_INITIALIZER,
                  _mutex_eval_status = PTHREAD_MUTEX_INITIALIZER;
...
tinymt32_t * rand_seed(GIAC_CONTEXT) {
  ...
  int array_start(GIAC_CONTEXT) {
    if (contextptr && contextptr->globalptr) {
      ...
      return (!contextptr->globalptr->_python_compat_ &&
              (contextptr->globalptr->_xcas_mode_ || hp38))
                 ? 1
                 : 0;
    }
...
int debug_infolevel = 0;
  ...
  int printprog = 0;
    ...
    defined NSPIRE || defined FXCG || defined NSPIRE_NEWLIB
  int threads = 1;
#else
int threads = sysconf(_SC_NPROCESSORS_ONLN);
  ...
  unsigned simult_primes = 16, simult_primes2 = 16, simult_primes3 = 16, simult_primes_seuil2 = -1,
           simult_primes_seuil3 = -1;
...
pid_t parent_id = 0;
  ...
  vector<context *> & context_list() {
    ...
    if (!ans)
      ans = new vector<context *>(1, (context *)0);
    return *ans;
  }
  context::context() {
  ...
  void clear_context(context * ptr) {
    ...
    ptr->globalcontextptr = ptr;
    ptr->previous = 0;
    ptr->globalptr = new global;
  ...
  void init_context(context * ptr) {
    ...
    ptr->globalptr->_step_infolevel_ =
        _step_infolevel_; // bit1= full simplify, bit2=1 for truncation
...
#define CLK_TCK 1
  ...
  bool make_thread(const gen & g, int level, const giac_callback & f, void * f_param,
                   context * contextptr) {
    return false;
  }
  ...
  debug_struct::debug_struct()
      : indent_spaces(0), debug_mode(false), sst_mode(false), sst_in_mode(false),
        debug_allowed(true), current_instruction(-1), debug_refresh(false) {
  ...
  global::global()
      : _xcas_mode_(0), _calc_mode_(0), _decimal_digits_(9), _minchar_for_quote_as_string_(1),
        _scientific_format_(0), _integer_format_(0), _latex_format_(0),
#ifdef BCD
        _bcd_decpoint_('.' | ('E' << 16) | (' ' << 24)), _bcd_mantissa_(12 + (15 << 8)),
        _bcd_flags_(0), _bcd_printdouble_(false),
#endif
        _expand_re_im_(true), _do_lnabs_(true), _eval_abs_(true), _eval_equaltosto_(true),
        _integer_mode_(true), _complex_mode_(false), _escape_real_(true),
        _complex_variables_(false), _increasing_power_(false), _approx_mode_(false),
        _variables_are_files_(false), _local_eval_(true), _withsqrt_(true), _show_point_(true),
        _io_graph_(false), _show_axes_(true), _all_trig_sol_(false),
#ifdef WITH_MYOSTREAM
        _ntl_on_(true), _lexer_close_parenthesis_(true), _rpn_mode_(false), _try_parse_i_(true),
        _specialtexprint_double_(false), _atan_tan_no_floor_(false), _keep_acosh_asinh_(false),
        _keep_algext_(false), _auto_assume_(false), _parse_e_(false), _convert_rootof_(false),
        _python_compat_(false), _angle_mode_(0), _bounded_function_no_(0), _series_flags_(0x3),
        _step_infolevel_(0), _default_color_(FL_BLACK), _epsilon_(1e-12), _proba_epsilon_(1e-15),
        _spread_Row_(-1), _spread_Col_(-1), _logptr_(&my_CERR), _prog_eval_level_val(1),
        _eval_level(DEFAULT_EVAL_LEVEL), _rand_seed(123457), _last_evaled_function_name_(0),
        _currently_scanned_(""), _last_evaled_argptr_(0), _max_sum_sqrt_(3),
#ifdef GIAC_HAS_STO_38 // Prime sum(x^2,x,0,100000) crash on hardware
        _max_sum_add_(10000),
#else
        _max_sum_add_(100000),
#endif
        _total_time_(0), _evaled_table_(0), _extra_ptr_(0), _series_variable_name_('h'),
        _series_default_order_(5),
#else
      _ntl_on_(true), _lexer_close_parenthesis_(true), _rpn_mode_(false), _try_parse_i_(true),
      _specialtexprint_double_(false), _atan_tan_no_floor_(false), _keep_acosh_asinh_(false),
      _keep_algext_(false), _auto_assume_(false), _parse_e_(false), _convert_rootof_(false),
...
#endif
#endif
      _prog_eval_level_val(1), _eval_level(DEFAULT_EVAL_LEVEL), _rand_seed(123457),
      _last_evaled_function_name_(0), _currently_scanned_(""), _last_evaled_argptr_(0),
      _max_sum_sqrt_(3),
#ifdef GIAC_HAS_STO_38 // Prime sum(x^2,x,0,100000) crash on hardware
      _max_sum_add_(10000),
#else
  ...
  global & global::operator=(const global & g) {
    ...
    _series_flags_ =
        g._series_flags_; // bit1= full simplify, bit2=1 for truncation, bit3=?, bit4=1 do not convert back SPOL1 to symbolic expression
  ...
  const char * const do_not_autosimplify[] = {"POLYFORM",
                                              "autosimplify",
                                              "canonical_form",
                                              "cfactor",
                                              "cpartfrac",
...
#endif
#if 1
      if (dichotomic_search(do_not_autosimplify, sizeof(do_not_autosimplify) / sizeof(char *) - 1,
                            c) != -1)
        return g;
#else
...
static void newlinestobr(string & s, const string & add) {
  ...
  void gprintf(unsigned special, const string & format, const vecteur & v, int step_info,
               GIAC_CONTEXT) {
    if (step_info == 0)
      return;
    if (my_gprintf) {
  ...
  int find_or_make_symbol(const string & s, gen & res, void * scanner, bool check38, GIAC_CONTEXT) {
    ...
    if (p.first != p.second && p.first != builtin_lexer_functions_end()) {
	  ...
	  if (builtin_lexer_functions[pos]._FUNC_%2){
        ...
        struct builtin_pointer_reference {
          const unary_function_ptr * const & pointer;
        };
...
#ifdef USTL
    ustl::pair<const lexer_tab_int_type *, const lexer_tab_int_type *> pp =
        ustl::equal_range(lexer_tab_int_values, lexer_tab_int_values_end, tst, tri1);
#else
  std::pair<const lexer_tab_int_type *, const lexer_tab_int_type *> pp =
      equal_range(lexer_tab_int_values, lexer_tab_int_values_end, tst, tri1);
#endif
...
#include "static_lexer.h"
  ...
  const unsigned builtin_lexer_functions_number =
      sizeof(builtin_lexer_functions) / sizeof(charptr_gen_unary);
...
#include "static_lexer_at.h"
  ...
  void convert_python(string & cur, GIAC_CONTEXT) {
    ...
    for (int pos = 1; pos < int(cur.size()); ++pos) {
      ...
      if ((curch == ']' && (prevch == ':' || prevch == ',')) || (curch == ',' && prevch == ':')) {
        cur[pos - 1] = '.';
      ...
      if (curch == '=' && openpar == 0 && prevch != '>' && prevch != '<' && prevch != '!' &&
          prevch != ':' && prevch != ';' && prevch != '=' && prevch != '+' && prevch != '-' &&
          prevch != '*' && prevch != '/' && prevch != '%' &&
          (pos == int(cur.size()) - 1 || (cur[pos + 1] != '=' && cur[pos + 1] != '<'))) {
        cur.insert(cur.begin() + pos, ':');
        ++pos;
        continue;
      }
  ...
  static void python_import(string & cur, int cs, int posturtle, int poscmath, int posmath,
                            int posnumpy, int posmatplotlib, GIAC_CONTEXT) {
    if (posmatplotlib >= 0 && posmatplotlib < cs) {
      cur += "np:=numpy:;xlim(a,b):=gl_x=a..b:;ylim(a,b):=gl_y=a..b:;scatter:=scatterplot:;bar:="
             "bar_plot:;text:=legend:;xlabel:=gl_x_axis_name:;ylabel:=gl_y_axis_name:;arrow:="
             "vector:;boxplot:=moustache:;";
      posnumpy = posmatplotlib;
    }
    if (posnumpy >= 0 && posnumpy < cs) {
      static bool alertnum = true;
      // add python numpy shortcuts
      cur += "mat:=matrix:;arange:=range:;resize:=redim:;shape:=dim:;conjugate:=conj:;full:=matrix:"
             ";eye:=identity:;ones(n,c):=matrix(n,c,1):; "
             "astype:=convert:;float64:=float:;asarray:=array:;astype:=convert:;reshape(m,n,c):="
             "matrix(n,c,flatten(m));";
      if (alertnum) {
        alertnum = false;
        alert("mat:=matrix;arange:=range;resize:=redim;shape:=dim;conjugate:=conj;full:=matrix;eye:"
              "=idn;ones(n,c):=matrix(n,c,1);reshape(m,n,c):=matrix(n,c,flatten(m));",
              contextptr);
      }
      return;
    }
...
#ifdef KHICAS
      cur += "fd:=forward:;bk:=backward:; rt:=right:; lt:=left:; pos:=position:; "
             "seth:=heading:;setheading:=heading:; ";
#else
    cur += "pu:=penup:;up:=penup:; pd:=pendown:;down:=pendown:; fd:=forward:;bk:=backward:; "
           "rt:=right:; lt:=left:; pos:=position:; seth:=heading:;setheading:=heading:; "
           "reset:=efface:;";
#endif
      if (alertturtle) {
        alertturtle = false;
        alert(
            "pu:=penup;up:=penup; pd:=pendown;down:=pendown; fd:=forward;bk:=backward; rt:=right; "
            "lt:=left; pos:=position; seth:=heading;setheading:=heading; reset:=efface",
            contextptr);
      }
      return;
    }
    ...
    if (posmath >= 0 && posmath < cs) {
      ...
      cur += "log2(x):=logb(x,2):;gamma:=Gamma:;fabs:=abs:;function modf(x) local y; y:=floor(x); "
             "return x-y,y; ffunction:;radians(x):=x/180*pi:;degrees(x):=x/pi*180";
  ...
  std::string python2xcas(const std::string & s_orig, GIAC_CONTEXT) {
    ...
    if (s_orig[first] == '#' || (s_orig[first] == '_' && !isalpha(s_orig[first + 1])) ||
        s_orig.substr(first, 4) == "from" || s_orig.substr(first, 7) == "import " ||
        s_orig.substr(first, 4) == "def ") {
      pythonmode = true;
      pythoncompat = true;
    }
    if (pythoncompat) {
    ...
    res = s_orig;
    if (res.size() > 18 && res.substr(0, 17) == "add_autosimplify(" && res[res.size() - 1] == ')')
    ...
    for (; res.size();) {
      ...
      for (pos = 0; pos < int(cur.size()); ++pos) {
        ...
        if (!instring && pythoncompat && ch == '\'' && pos < cur.size() - 2 &&
            cur[pos + 1] != '\\' &&
            (pos == 0 || (cur[pos - 1] != '\\' &&
                          cur[pos - 1] != '\''))) { // workaround for '' string delimiters
        ...
        if (ch == '#') {
          ...
          if (cur.size() > pos + 8 &&
              (cur.substr(pos, 8) == "# local " || cur.substr(pos, 7) == "#local ")) {
            cur.erase(cur.begin() + pos);
            if (cur[pos] == ' ')
              cur.erase(cur.begin() + pos);
          } else
        ...
        if (chkfrom && ch == 'f' && pos + 15 < int(cur.size()) && cur.substr(pos, 5) == "from ") {
          ...
          if (posi > pos + 5 && posi < int(cur.size())) {
            ...
            int cs = int(cur.size());
            cur = cur.substr(0, pos);
            python_import(cur, cs, posturtle, poscmath, posmath, posnumpy, posmatplotlib,
                          contextptr);
            pythonmode = true;
#ifdef KHICAS
            if ((posturtle < 0 || posturtle >= cs) && (poscmath < 0 || poscmath >= cs) &&
                (posmath < 0 || posmath >= cs) && (posnumpy < 0 || posnumpy >= cs) &&
        ...
        if (ch == 'l' && pos + 6 < int(cur.size()) && cur.substr(pos, 6) == "lambda" &&
            instruction_at(cur, pos, 6)) {
        ...
        if (ch == 'e' && pos + 4 < int(cur.size()) && cur.substr(pos, 3) == "end" &&
            instruction_at(cur, pos, 3)) {
      ...
      if (instring) {
        *logptr(contextptr)
            << "Warning: multi-line strings can not be converted from Python like syntax" << '\n';
        return s_orig;
      }
          ...
          if (elsepos > 0 && elsepos < cs) {
            ...
            if (ifpos > 0 && ifpos < elsepos) {
              ...
              if (retpos >= 0 && retpos < ifpos) {
                cur = cur.substr(0, endretpos) + " ifte(" +
                      cur.substr(ifpos + 2, elsepos - ifpos - 2) + "," +
                      cur.substr(endretpos, ifpos - endretpos) + "," +
                      cur.substr(elsepos + 4, cs - elsepos - 4) + ")";
              }
            }
          }
        }
      ...
      if (cur[pos] == ':') {
        ...
        if (progpos >= 0 && progpos < cs && instruction_at(cur, progpos, 3)) {
          ...
          // for _ -> for x_
          cur = cur.substr(0, pos);
          if (progpos + 5 < cs && cur[progpos + 3] == ' ' && cur[progpos + 4] == '_' &&
              cur[progpos + 5] == ' ') {
        ...
        char curpos = cur[pos];
        if (pythonmode && !res.empty() && pos >= 0 && curpos != ';' && curpos != ',' &&
