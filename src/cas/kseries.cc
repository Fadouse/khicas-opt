...
namespace giac {
  ...
  static bool taylor_(const gen & f_x, const gen & x, const gen & lim_point, int ordre, vecteur & v,
                      GIAC_CONTEXT) {
  ...
  bool taylor(const gen & f_x, const gen & x, const gen & lim_point, int ordre, vecteur & v,
              GIAC_CONTEXT) {
  ...
  gen taylor(const gen & lim_point, int ordre, const unary_function_ptr & f, int direction,
             gen & shift_coeff, GIAC_CONTEXT) {
    ...
    */
    ...
    if (is_undef(lim_point) || is_inf(lim_point)) {
      invalidserieserr(gettext("non tractable function ") +
                       (f.ptr()->print(contextptr) + (" at " + lim_point.print(contextptr))));
      return undef;
    }
    identificateur x(" ");
    vecteur v;
  ...
  bool padd(const sparse_poly1 & a, const sparse_poly1 & b, sparse_poly1 & res, GIAC_CONTEXT) {
    ...
    for (; (a_cur != a_end) && (b_cur != b_end);) {
      ...
      if (sum.type > _POLY && sum.type != _FRAC &&
          (res.empty() || (series_flags(contextptr) & 0x1))) {
        //cerr << sum << " ";
  ...
  bool pmul(const sparse_poly1 & celuici, const sparse_poly1 & other, sparse_poly1 & final_seq,
    ...
    gen c_max =
        min(normal(a_min + b_max, contextptr), normal(b_min + a_max, contextptr), contextptr);
    ...
    for (; ita != ita_end; ++ita) {
      ...
      for (;;) {
        ...
        if ((n_truncate && ck_is_strictly_greater(n_valuation, cur_pow, contextptr)) ||
            ck_is_greater(c_max, cur_pow, contextptr)) {
          if (cur_pow != old_pow) {
  ...
  bool pdiv(const sparse_poly1 & a, const sparse_poly1 & b_orig, sparse_poly1 & res, int ordre_orig,
            GIAC_CONTEXT) {
#ifdef TIMEOUT
    control_c();
#endif
  ...
  bool ppow(const sparse_poly1 & base, const gen & e, int ordre, int direction, sparse_poly1 & res,
            GIAC_CONTEXT) {
#ifdef TIMEOUT
    control_c();
#endif
  ...
  bool prevert(const sparse_poly1 & p_orig, sparse_poly1 & q, GIAC_CONTEXT) {
    ...
    if (p.front().exponent == 0)
      p.erase(p.begin());
    gen ak, k, invk, b1;
    if (p.empty() || is_undef((ak = p.front().coeff)) ||
        ck_is_positive(-(k = p.front().exponent), contextptr) || k.type != _INT_)
      return false; // setsizeerr(gettext("prevert"));
    invk = gen(1) / k;
  ...
  static bool in_series__SPOL1(const gen & e, const identificateur & x, const vecteur & lvx,
                               const vecteur & lvx_s, int ordre, int direction, sparse_poly1 & s,
                               GIAC_CONTEXT) {
#ifdef TIMEOUT
    control_c();
#endif
    ...
    if (e._SYMBptr->sommet == at_plus) {
      if (e._SYMBptr->feuille.type != _VECT) {
        return in_series__SPOL1(e._SYMBptr->feuille, x, lvx, lvx_s, ordre, direction, s,
                                contextptr);
      }
  ...
  static void find_image(const symbolic & temp__SYMB, gen & image_of_lim_point, sparse_poly1 & s,
      ...
      if (s.begin()->exponent == 0) {
        ...
        if (ck_is_strictly_positive(s.begin()->exponent, contextptr))
          ...
          if ((s.begin()->exponent.type == _INT_) &&
              !(s.begin()->exponent.val % 2)) { // odd negative exponent
  ...
  static int ck_is_greater(const sparse_poly1 & s1, sparse_poly1 & s2, int direction,
                           GIAC_CONTEXT) {
    sparse_poly1 s(s2);
    pneg(s, s, contextptr);
  ...
  static gen in_limit(const gen & e, const identificateur & x, const gen & lim_point, int direction,
                      GIAC_CONTEXT);

  static bool mrv_lead_term(const gen & e, const identificateur & x, gen & coeff, gen & mrv_var,
                            gen & exponent, sparse_poly1 & q, int begin_ordre, GIAC_CONTEXT,
                            bool series);

  vecteur integrate(const vecteur & p, const gen & shift_coeff);

  bool series(const sparse_poly1 & s_, const unary_function_ptr & u, int direction,
  ...
  sparse_poly1 series(const sparse_poly1 & s, const unary_function_ptr & u, int direction,
                      GIAC_CONTEXT) {
    sparse_poly1 res;
  ...
  bool series__SPOL1(const gen & e_orig, const identificateur & x, const gen & lim_point, int ordre,
                     int direction, sparse_poly1 & s, GIAC_CONTEXT) {
    gen e(e_orig);
    // fast check first
    if (!contains(e, x)) {
    ...
    for (; lvx_it != lvx_end; ++lvx_it) {
      ...
      if ((temp__SYMB.sommet == at_pow) && (!contains((*temp__SYMB.feuille._VECTptr)[1], x))) {
        if (!in_series__SPOL1((*temp__SYMB.feuille._VECTptr)[0], x, lvx, lvx_s, ordre, direction, s,
                              contextptr) ||
      ...
      if (temp__SYMB.sommet == at_pow)
        temp__SYMB = symbolic(at_exp, (*temp__SYMB.feuille._VECTptr)[1] *
                                          ln((*temp__SYMB.feuille._VECTptr)[0], contextptr));
      ...
      if ((temp__SYMB.feuille.type == _IDNT) && (temp__SYMB.sommet != at_abs)) {
        ...
        if (!temp__SYMB.sommet.ptr()->series_expansion) {
          *logptr(contextptr) << gettext("no taylor method for ")
                              << temp__SYMB.sommet.ptr()->print(contextptr) << endl;
          return false;
        }
        gen shift_coeff;
        gen res = temp__SYMB.sommet.ptr()->series_expansion(lim_point, ordre, temp__SYMB.sommet,
                                                            direction, shift_coeff, contextptr);
        if (res.type == _SPOL1)
      ...
      if (temp__SYMB.sommet != at_of && temp__SYMB.feuille.type == _VECT) {
        ...
        if (nargs == 4 && temp__SYMB.sommet == at_sum) {
          ...
          if (contains(k, x)) {
            invalidserieserr(
                gettext("Summation variable must be != from series expansion variable"));
            return false;
          }
        ...
        if (temp__SYMB.sommet == at_euler_mac_laurin) {
          ...
          eff += (limit(f, *k._IDNTptr, upper, -1, contextptr) +
                  limit(f, *k._IDNTptr, lower, 1, contextptr)) /
                 2;
          // then Bernoulli part
          if (is_undef(fdiff))
            return false;
          for (int i = 1; i < ordre; i++) {
            gen add = limit(fdiff, *k._IDNTptr, upper, -1, contextptr) -
                      limit(fdiff, *k._IDNTptr, lower, 1, contextptr);
        ...
        if (temp__SYMB.sommet == at_integrate) {
          ...
          if (contains(t, x)) {
            invalidserieserr(
                gettext("Integration variable must be != from series expansion variable"));
            return false;
          }
          if (!contains(tempfv[0], x)) {
            ...
            gen g = derive(tempfv[3], x, contextptr) *
                        subst(tempfv[0], tempfv[1], tempfv[3], false, contextptr) -
                    derive(tempfv[2], x, contextptr) *
                        subst(tempfv[0], tempfv[1], tempfv[2], false, contextptr);
          ...
          primit = subst(primit, t, tempfv[3], false, contextptr) -
        ...
        const_iterateur fit = temp__SYMB.feuille._VECTptr->begin(),
                        fitend = temp__SYMB.feuille._VECTptr->end();
        if (temp__SYMB.sommet == at_Psi || temp__SYMB.sommet == at_Eta ||
            temp__SYMB.sommet == at_Zeta) {
        ...
        if (temp__SYMB.sommet == at_of) {
          ...
          if (tf.type == _VECT && tf._VECTptr->size() == 2) {
            ...
            if (!in_series__SPOL1(tfx, x, lvx, lvx_s, ordre, direction, s, contextptr))
              return false; // s<-arg
          }
        } else {
          if (!in_series__SPOL1(temp__SYMB.feuille, x, lvx, lvx_s, ordre, direction, s, contextptr))
      ...
      if (temp__SYMB.sommet == at_of) {
        ...
        if (tf.type == _VECT && tf._VECTptr->size() == 2) {
          ...
          vecteur expansion(
              1, symb_of(
                     tff,
                     image_of_lim_point)); // symbolic(at_of,makesequence(tff,image_of_lim_point)));
          expansion.reserve(ordre);
          for (int i = 1; i <= ordre; i++) {
            gen fn;
            if (i == 1)
              fn = symb_of(
                  symbolic(at_function_diff, tff),
                  image_of_lim_point); //symbolic(at_of,makesequence(symbolic(at_function_diff,tff),image_of_lim_point));
            else
              fn = symb_of(
                  symb_of(symbolic(at_composepow, makesequence(at_function_diff, i)), tff),
                  image_of_lim_point); //symbolic(at_of,makesequence(symbolic(at_of,makesequence(symbolic(at_composepow,makesequence(at_function_diff,i)),tff)),image_of_lim_point));
      ...
      if ((temp__SYMB.sommet == at_Psi || temp__SYMB.sommet == at_Eta ||
        ...
        if (temp__SYMB.sommet == at_Psi)
          expansion = at_Psi_minus_ln->ptr()->series_expansion(
              image_of_lim_point, ordre + addorder, temp__SYMB.sommet, image_of_direction,
              shift_coeff, contextptr);
      }
      if (expansion == 0)
        expansion = temp__SYMB.sommet.ptr()->series_expansion(image_of_lim_point, ordre + addorder,
                                                              temp__SYMB.sommet, image_of_direction,
                                                              shift_coeff, contextptr);
  ...
  sparse_poly1 series__SPOL1(const gen & e, const identificateur & x, const gen & lim_point,
  ...
  static sparse_poly1 ck_series__SPOL1(const gen & e, const identificateur & x,
    ...
    if (true_order.type == _INT_ && true_order.val <= ordre) {
      if (series__SPOL1(e, x, lim_point, ordre + 1 + ordre - true_order.val, direction, s,
                        contextptr)) {
        if (!s.empty())
  ...
  vecteur lvarx(const gen & e, const gen & x, bool test) {
    ...
    for (; it != itend; ++it) {
      ...
      if ((it->type == _SYMB) && ((it->_SYMBptr->sommet == at_pow &&
                                   !contains((*(it->_SYMBptr->feuille._VECTptr))[1], x)) ||
                                  (it->_SYMBptr->sommet == at_NTHROOT &&
                                   !contains((*(it->_SYMBptr->feuille._VECTptr))[0], x)))) {
        vecteur tmp(lvarx(
            (*(it->_SYMBptr->feuille._VECTptr))[(it->_SYMBptr->sommet == at_pow) ? 0 : 1], x));
  ...
  void rlvarx(const gen & e, const gen & xgen, vecteur & res) {
    ...
    for (; it != itend; ++it) {
      ...
      // recursive call
      res.push_back(*it);
      if (it->is_symb_of_sommet(at_derive) && it->_SYMBptr->feuille.type == _VECT &&
          it->_SYMBptr->feuille._VECTptr->size() == 3 &&
          it->_SYMBptr->feuille._VECTptr->back().type == _INT_) {
        int n = it->_SYMBptr->feuille._VECTptr->back().val;
        for (--n; n > 1; --n) {
          res.push_back(symb_derive(
              it->_SYMBptr->feuille._VECTptr->front(), (*it->_SYMBptr->feuille._VECTptr)[1],
              n)); //res.push_back(symbolic(at_derive,makesequence(it->_SYMBptr->feuille._VECTptr->front(),(*it->_SYMBptr->feuille._VECTptr)[1],n)));
        }
        res.push_back(symb_derive(
            it->_SYMBptr->feuille._VECTptr->front(),
            (*it->_SYMBptr->feuille._VECTptr)
                [1])); //res.push_back(symbolic(at_derive,makesequence(it->_SYMBptr->feuille._VECTptr->front(),(*it->_SYMBptr->feuille._VECTptr)[1])));
      }
      if (it->type == _SYMB) {
  ...
  */
  ...
  // specialization
  static int equalposcomp(const std::vector<const unary_function_ptr *> & v,
                          unary_function_ptr * w) {
    int n = 1;
    for (std::vector<const unary_function_ptr *>::const_iterator it = v.begin(); it != v.end();
         ++it) {
      if (*(*it) == *w)
        return n;
      else
  ...
  gen limit_symbolic_preprocess(const gen & e0, const identificateur & x, const gen & lim_point,
                                int direction, GIAC_CONTEXT) {
    ...
    if (!contains(lidnt(first_try), unsigned_inf)) {
      ...
      if (hasnum && !is_undef(chk) && abs(chk - chknum, contextptr) > 1e-10 &&
          abs(1 - chk / chknum, contextptr) > 1e-10)
    ...
    for (int i = 0; i < vs; ++i) {
      if (v[i].type == _SYMB) {
        ...
        if (v[i].is_symb_of_sommet(at_sinh) || v[i].is_symb_of_sommet(at_cosh) ||
            v[i].is_symb_of_sommet(at_tanh)) {
          gen g = limit(v[i]._SYMBptr->feuille, x, lim_point, direction, contextptr);
          if (is_inf(g)) {
            v1.push_back(v[i]);
        ...
        if (v[i].is_symb_of_sommet(at_sum)) {
          gen tmp;
          if (v[i]._SYMBptr->feuille.type == _VECT &&
              v[i]._SYMBptr->feuille._VECTptr->size() == 4) {
          ...
          if (!convert_to_euler_mac_laurin(v[i], x, tmp, contextptr))
            return gensizeerr(gettext("Unable to convert sum to Euler Mac-Laurin ") +
                              v[i].print(contextptr));
          v1.push_back(v[i]);
          v2.push_back(tmp);
        }
        if (((pos1 = equalposcomp(limit_tab, v[i]._SYMBptr->sommet)) ||
             (pos2 = equalposcomp(limit_tractable_functions(), &v[i]._SYMBptr->sommet)))) {
          gen g = limit(v[i]._SYMBptr->feuille, x, lim_point, direction, contextptr);
          if (is_inf(g) ||
              (g.type == _VECT && !g._VECTptr->empty() && is_inf(g._VECTptr->back()))) {
            v1.push_back(v[i]);
            v2.push_back(
                pos1 ? limit_replace[pos1 - 1](v[i]._SYMBptr->feuille, contextptr)
                     : limit_tractable_replace()[pos2 - 1](v[i]._SYMBptr->feuille, contextptr));
          }
        }
      }
    }
  ...
  static gen unidirectional_limit(const gen & e0, const identificateur & x, const gen & lim_point,
                                  int direction, GIAC_CONTEXT) {
    gen e_copy = e0;
    ...
    gen coeff, mrv_var, exponent;
    sparse_poly1 p;
    if (!mrv_lead_term(e_copy, x, coeff, mrv_var, exponent, p, mrv_begin_order, contextptr,
                       false) ||
        is_undef(coeff)) {
    ...
    for (unsigned i = 0; i < p.size(); ++i) {
      if (check_bounded(p[i].coeff, contextptr) == -1)
        return gensizeerr("Limit probably undefined, algorithm unable to handle " +
                          p[i].coeff.print(contextptr));
    }
  ...
  static gen in_limit(const gen & e0, const identificateur & x, const gen & lim_point,
                      int direction, GIAC_CONTEXT) {
    if (direction == -2)
      return gensizeerr(contextptr);
    ...
    if (_about(x, contextptr) != x) {
      ...
      return in_limit(quotesubst(e0, x, xprime, contextptr), xprime, lim_point, direction,
                      contextptr);
    }
    ...
    if (0 && direction && !vsign.empty() &&
        !equalposcomp(sign_floor_ceil_round_tab, e._SYMBptr->sommet)) {
    ...
    if (vsign.empty()) {
	  ...
	  */
      ...
      first_try = eval(
          first_try, eval_level(contextptr),
          contextptr); // moved before eval_abs(false,contextptr), must be before simplifier below
      first_try = simplifier(
          first_try,
          contextptr); // for assume(a>0); limit((sqrt(2*a^3*x-x^4)-a*root(3,a^2*x))/(a-root(4,a*x^3)),x=a);
    ...
    if (has_i(e_copy)) {
      ...
      if (has_i(lop(e_copy, at_erfs)))
        return gensizeerr(
            gettext("erf/erfc/erfs with complex argument not yet implemented in limit"));
    }
    // Rewrite constants
    ...
    if (!direction) {
      if (!is_analytic(e_copy) || has_op(e_copy, *at_acos) || has_op(e_copy, *at_asin)) {
        ...
        if (g1 == g2 || is_zero(ratnormal(g1 - g2, contextptr)))
          return g1;
        return gensizeerr("Unidirectional limits are distinct " + g2.print(contextptr) + "," +
                          g1.print(contextptr));
      }
      ...
      for (; !p.empty() && is_undef(p.front().coeff) && (ordre < mrv_begin_order * 4);
           ordre = 1.5 * ordre + 1) {
        p = series__SPOL1(e_copy, x, lim_point, int(ordre), 0, contextptr);
        if (!p.empty() && !is_undef(p.front().coeff)) {
          if (p.front().coeff.type == _FRAC &&
              is_strictly_positive(-p.front().coeff._FRACptr->den, contextptr))
            p.front().coeff =
                fraction(-p.front().coeff._FRACptr->num, -p.front().coeff._FRACptr->den);
          break;
        }
      }
      if (ordre >= mrv_begin_order * 4) {
        ...
        if (is_zero(ratnormal(g1 - g2, contextptr)))
          return g1;
        return gensizeerr("Unidirectional limits are distincts " + g2.print(contextptr) + "," +
                          g1.print(contextptr));
      }
      ...
      if (is_zero(p.front().exponent)) {
        ...
        if (l == -1)
          return gensizeerr("Limit probably undefined, algorithm unable to handle " +
                            p.front().coeff.print(contextptr));
  ...
  static gen mrv_compare(const gen & a, const gen & b, const identificateur & x, GIAC_CONTEXT) {
    ...
    gen coeff, mrv_var, exponent;
    sparse_poly1 p;
    if (!mrv_lead_term(rdiv(lna, lnb, contextptr), x, coeff, mrv_var, exponent, p, mrv_begin_order,
                       contextptr, false))
      return gensizeerr(contextptr);
    if (ck_is_strictly_positive(exponent, contextptr))
      return 0;
  ...
  static bool mrv_max(const vecteur & a_faster_var, const vecteur & a_coeff_ln,
                      const vecteur & a_slower_var, const vecteur & b_faster_var,
                      const vecteur & b_coeff_ln, const vecteur & b_slower_var,
                      const identificateur & x, vecteur & faster_var, vecteur & coeff_ln,
                      vecteur & slower_var, GIAC_CONTEXT) {
    int pos_a, pos_b;
    gen s;
  ...
  static bool mrv(const gen & e, const identificateur & x, vecteur & faster_var, vecteur & coeff_ln,
                  vecteur & slower_var, GIAC_CONTEXT) {
    ...
    for (; it != itend; ++it) {
      if (equalposcomp(faster_var, *it) || equalposcomp(slower_var, *it))
        continue;
      if (it->type != _SYMB) {
        if (!mrv_max(faster_var, coeff_ln, slower_var, vecteur(1, *it), vecteur(1, plus_one),
                     vecteur(0), x, faster_var, coeff_ln, slower_var, contextptr))
          return false;
        continue;
      }
      gen temp = *it;
      ...
      if (temp._SYMBptr->sommet == at_lower_incomplete_gamma ||
          temp._SYMBptr->sommet == at_igamma_exp) {
      ...
      if (temp._SYMBptr->sommet == at_pow)
        temp = symb_exp((*(temp._SYMBptr->feuille._VECTptr))[1] *
                        ln((*(temp._SYMBptr->feuille._VECTptr))[0], contextptr));
      ...
      if (temp._SYMBptr->feuille.type == _VECT) {
        *logptr(contextptr) << gettext("Limit probably undefined, algorithm unable to handle ") +
                                   temp.print(contextptr)
                            << endl;
        return false;
      }
      gen l = in_limit(temp._SYMBptr->feuille, x, plus_inf, 0, contextptr);
      if (is_undef(l) || (l == unsigned_inf && temp._SYMBptr->sommet != at_cos &&
                          temp._SYMBptr->sommet != at_sin && temp._SYMBptr->sommet != at_erfs)) {
      ...
      if (temp._SYMBptr->sommet == at_exp) {
        if (!mrv(temp._SYMBptr->feuille, x, faster_var, coeff_ln, slower_var, contextptr) ||
            !mrv_max(faster_var, coeff_ln, slower_var, vecteur(1, temp), vecteur(1, plus_one),
                     vecteur(0), x, faster_var, coeff_ln, slower_var, contextptr))
          return false;
        continue;
      }
      ...
      if (!temp._SYMBptr->sommet.ptr()->series_expansion) {
        invalidserieserr(string(gettext("no taylor method for ")) +
                         temp._SYMBptr->sommet.ptr()->print(contextptr));
        return false;
      }
      gen test = temp._SYMBptr->sommet.ptr()->series_expansion(
          l, 0, temp._SYMBptr->sommet, 0, shift_coeff,
          contextptr); // fixme: 0 should be image_of_direction
      if (!is_undef(test)) {
        if (!mrv(temp._SYMBptr->feuille, x, faster_var, coeff_ln, slower_var, contextptr))
          return false;
        continue;
  ...
  static bool mrv_lead_term(const gen & e, const identificateur & x, gen & coeff, gen & mrv_var,
    ...
    for (; it != itend; ++it) {
      ...
      faster_var_subst.push_back(pow(w, c, contextptr) *
                                 exp(normal(f - c * g, contextptr), contextptr));
    }
    ...
    for (unsigned count = 0; (ordre < max_series_expansion_order) && !p.empty() &&
                             (p.size() >= 1) && is_undef(p.front().coeff);
         ++count, ordre = ordre * 1.5 + 1) {
      bool inv = false;
      ...
      if (!p.empty() && !is_undef(p.front().coeff)) {
        ...
        gen tmp = ratnormal(
            subst(p.front().coeff, ln(w, contextptr), (dont_invert ? g : -g), false, contextptr),
            contextptr);
        if (is_undef(tmp)) {
          inv = true;
    ...
    if (!p.empty()) {
      ...
      if (is_zero(p.front().exponent) && contains(p.front().coeff, x) &&
          !is_zero(derive(p.front().coeff, x, contextptr))) {
        // if p is uniquely composed of this coeff, expand
        if (!mrv_lead_term(p.front().coeff, x, coeff, mrv_var, exponent, p, mrv_begin_order,
                           contextptr, false))
          return false;
        if (p.empty()
  ...
  gen limit(const gen & e, const identificateur & x, const gen & lim_point_, int direction,
            GIAC_CONTEXT) {
    ...
    if (has_num_coeff(
            lim_point)) // otherwise A:=conic(x^2/4+y^2/3=1); B:=element(A,1)+nop(-1.666-1.243*i) runs forever
      return subst(e, x, lim_point, false, contextptr);
  ...
  gen quotedlimit(const gen & e, const identificateur & x, const gen & lim_point, int direction,
                  GIAC_CONTEXT) {
    ...
    if (s == 3) {
      ...
      if (e.type != _SYMB) {
        if (is_one(arg3) || is_minus_one(arg3))
          return quotedlimit(G, *ggb_var(G)._IDNTptr, e,
                             int(evalf_double(arg3, 1, contextptr)._DOUBLE_val), contextptr);
        return gentypeerr(contextptr);
      }
      if (!is_equal(e)) {
        if (is_one(arg3) || is_minus_one(arg3))
      ...
      if (x.type != _IDNT)
        return gensizeerr(contextptr);
      return quotedlimit(G, *x._IDNTptr, (*(e._SYMBptr->feuille._VECTptr))[1],
                         convert_to_direction(v[2]), contextptr);
    }
    if (s > 4)
  ...
  // expand it, l.1976
  static gen sparse_poly12gen_expand(const sparse_poly1 & s, const identificateur & x,
                                     const gen & mrv_var, int ordre, gen & remains,
                                     bool with_order_size, GIAC_CONTEXT) {
    ...
    if (!mrv_lead_term(afaire, x, coeff2, mrv_var2, exponent2, s2, ordre, contextptr, true))
      return false;
    return sparse_poly12gen_expand(s2, x, mrv_var2, ordre, remains, with_order_size, contextptr) *
           pow(mrv_fait, exponent, contextptr);
  }

  static gen in_series(const gen & e0, const identificateur & x, const gen & lim_point, int ordre,
                       int direction, GIAC_CONTEXT) {
    ...
    if (lim_point == minus_inf) {
      gen coeff, mrv_var, exponent, remains;
      sparse_poly1 s;
      if (!mrv_lead_term(subst(e, x, -x, false, contextptr), x, coeff, mrv_var, exponent, s, ordre,
                         contextptr, true))
        return gensizeerr(contextptr);
      if (series_flags(contextptr) & (1 << 4))
        return s; // no back conversion if bit 4 is set
      return subst(sparse_poly12gen_expand(s, x, mrv_var, ordre, remains, true, contextptr), x, -x,
                   false, contextptr);
    }
    if (direction == 1) {
      ...
      if (series_flags(contextptr) & (1 << 4))
        return s; // no back conversion if bit 4 is set
      return subst(sparse_poly12gen_expand(s, x, mrv_var, ordre, remains, true, contextptr), x,
                   inv(x - lim_point, contextptr), false, contextptr);
    }
    if (direction == -1) {
      gen ecopy = subst(e, x, lim_point - inv(x, contextptr), false, contextptr);
      gen coeff, mrv_var, exponent, remains;
      ...
      if (series_flags(contextptr) & (1 << 4))
        return s; // no back conversion if bit 4 is set
      return subst(sparse_poly12gen_expand(s, x, mrv_var, ordre, remains, true, contextptr), x,
                   inv(lim_point - x, contextptr), false, contextptr);
    }
    gen remains;
    switch (e.type) {
    case _INT_:
  ...
  gen series(const gen & e, const identificateur & x, const gen & lim_point, int ordre,
             int direction, GIAC_CONTEXT) {
    int save_series_flags = series_flags(contextptr);
    series_flags(save_series_flags | 8, contextptr);
    if (has_op(e, *at_surd) || has_op(e, *at_NTHROOT)) {
    ...
    if (e.type == _VECT) {
      ...
      int l = int(res.size());
      for (int i = 0; i < l; ++i) {
        res[i] = in_series(_pow2exp(tan2sincos(res[i], contextptr), contextptr), x, lim_point,
                           ordre, direction, contextptr);
      }
      series_flags(save_series_flags, contextptr);
      return res;
    }
    gen res = in_series(_pow2exp(tan2sincos(e, contextptr), contextptr), x, lim_point, ordre,
                        direction, contextptr);
    series_flags(save_series_flags, contextptr);
    return res;
  }
  ...
  gen series(const gen & e, const gen & vars, const gen & lim_point, const gen & ordre0,
             GIAC_CONTEXT) {
    gen ordre(ordre0);
  ...
  gen _series(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type == _STRNG && args._STRNGptr->size() == 1) {
      ...
      *logptr(contextptr) << "Setting " << ch << " as series variable name" << endl;
      ...
      if (eval(O, 1, contextptr) != O)
        *logptr(contextptr) << "Purge " << Os << " if you want to use " << Os << "(" << h
                            << "^...) notation" << endl;
      else {
        gen prog = symb_program(vx_var, 0, vx_var * symbolic(at_order_size, h), contextptr);
        *logptr(contextptr) << "Assigning " << Os << " so that you can use use " << Os << "(" << h
                            << "^...) notation" << endl;
    ...
    if (v.empty())
      return gensizeerr(contextptr);
    if (v.back().type == _INT_ && v.back().subtype == _INT_MAPLECONVERSION &&
        v.back().val == _POLY1__VECT) {
      gen p = v.back();
      v.pop_back();
    ...
    if (s == 3) {
      if ((v[1].type == _VECT && v[2].type == _VECT) ||
          (v[1].type == _IDNT || (v[1].type == _SYMB && (v[1]._SYMBptr->sommet == at_equal ||
                                                         v[1]._SYMBptr->sommet == at_equal2 ||
                                                         v[1]._SYMBptr->sommet == at_at))))
        return series(v[0], v[1], v[2], series_default_order(contextptr), contextptr);
      return series(v[0], symbolic(at_equal, makesequence(vx_var, v[1])), v[2],
                    series_default_order(contextptr), contextptr);
    }
    if (s == 4)
  ...
  define_unary_function_ptr5(at_bounded_function, alias_at_bounded_function, &__bounded_function, 0,
                             true);
  ...
  define_unary_function_ptr5(at_euler_mac_laurin, alias_at_euler_mac_laurin, &__euler_mac_laurin, 0,
                             true);

  bool convert_to_euler_mac_laurin(const gen & g, const identificateur & n, gen & res,
                                   GIAC_CONTEXT) {
