...
namespace giac {
  ...
  int is_meromorphic(const gen & g, const gen & x, gen & a, gen & b, gen & P, GIAC_CONTEXT) {
    ...
    if (g.type == _SYMB) {
      ...
      if (v.size() == 2) {
        ...
        if ((v[1].is_symb_of_sommet(at_cos) || v[1].is_symb_of_sommet(at_sin)) &&
            is_linear_wrt(P, x, a, b, contextptr) && is_zero(im(a, contextptr)))
          return 3;
      ...
      for (; it != itend; ++it) {
        ...
        if (u == at_sin || u == at_cos || u == at_tan || u == at_exp || u == at_sinh ||
            u == at_cosh || u == at_tanh) {
  ...
  int fast_is_even_odd(const gen & f, const gen & x, GIAC_CONTEXT) {
    ...
    if (res < 2)
      return res;
    // ff is odd
    if (u == at_plus || u == at_neg || u == at_inv || u == at_sin || u == at_tan || u == at_sinh ||
        u == at_tanh || u == at_atan || u == at_atanh)
      return res;
    if (u == at_prod) {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        if (is_strictly_positive(e1, contextptr)) {
          ...
          if (!has_sparse_poly1(res) && is_zero(derive(res, x, contextptr)))
            return 0;
          return gensizeerr(gettext("Non holomorphic function ") + g.print(contextptr) + " at " +
                            x.print(contextptr) + "=" + a.print(contextptr));
        }
        if (e1.type == _FRAC)
          return undef;
  ...
  bool assume_t_in_ab(const gen & t, const gen & a, const gen & b, bool exclude_a, bool exclude_b,
                      GIAC_CONTEXT) {
    ...
    return !is_undef(sto(
        gen(makevecteur(gen(_DOUBLE_).change_subtype(1), v_interval, v_excluded), _ASSUME__VECT), t,
        contextptr));
  }
  ...
  static bool intgab_sincos(const gen & g, const gen & x, const gen & a, const gen & b, gen & A,
                            gen & B, gen & res, GIAC_CONTEXT) {
    ...
    if (separate_variables(f, Xid, x, Xfact, xfact, contextptr)) {
      ...
      if (Qd) {
        ...
        decal[0] = -Dval;
        Dp = Dp.shift(decal);
        polynome XQd(gen(1), int(vX.size())), U(int(vX.size())), V(int(vX.size())),
            C(int(vX.size()));
        decal[0] = Dval;
        XQd = XQd.shift(decal);
...
#endif
  bool intgab_r(const gen & g0, const gen & x, const gen & a, const gen & b, bool rational,
    ...
    if (typeint == 6 && a == minus_inf && b == plus_inf && is_zero(P) &&
        is_constant_wrt(A, x, contextptr) && lvarxwithinv(B, x, contextptr).size() < 2 &&
        is_strictly_positive(-A[0], contextptr)) {
      ...
      res = sqrt(-cst_pi / A_, contextptr) * B *
            exp(ratnormal(C_ - B_ * B_ / 4 / A_, contextptr), contextptr);
      return true;
    }
    if (typeint == 5) {
      ...
      for (int Pi = 0; Pi < Pvs; ++Pi) {
        ...
        fxnd(P2, N, D);
        gen lncst =
            ln(_lcoeff(gen(makevecteur(r2sym(N, lv, contextptr), x), _SEQ__VECT), contextptr) /
                   _lcoeff(gen(makevecteur(r2sym(D, lv, contextptr), x), _SEQ__VECT), contextptr),
               contextptr);
          ...
          for (int j = 0; j < rPs; ++j) {
            ...
            if (racinePcplus) { // contour is C- for this pole/root of ln
              if (!estreel && !residucplus) {
                gen tmp =
                    residue(A * ln(x - racineP, contextptr), *x._IDNTptr, racineA, contextptr);
                if (is_undef(tmp))
                  return false;
                somme_residus -= tmp;
              }
  ...
  bool integration_rational_tail(const gen &, const gen &, int &, gen &, unsigned &, unsigned,
                                 GIAC_CONTEXT);
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool intgab_continuous_finite(const gen & g, const gen & x, unsigned & budget,
                                       unsigned depth, GIAC_CONTEXT) {
    if (!budget || depth > 8)
      return false;
    --budget;
    if (!has_op(g, *at_tan)) {
      vecteur atoms;
      return logarithmic_span_entire(g, x, atoms, budget, depth);
    }
    if (g.type != _SYMB)
      return false;
    ...
    if (g.is_symb_of_sommet(at_tan)) {
      ...
      if (waves.size() != 1)
        return false;
      gen A, B;
      if (!is_linear_wrt(f, waves[0], A, B, contextptr) || !equation_rational(A) ||
          !equation_rational(B))
        return false;
      gen margin = 1 - abs(A, contextptr) - abs(B, contextptr);
      vecteur atoms;
      return (is_zero(margin) || is_strictly_positive(margin, contextptr)) &&
             logarithmic_span_entire(waves[0]._SYMBptr->feuille, x, atoms, budget, depth + 1);
    }
    if (g.is_symb_of_sommet(at_neg) || g.is_symb_of_sommet(at_sin) || g.is_symb_of_sommet(at_cos) ||
        g.is_symb_of_sommet(at_exp))
      return intgab_continuous_finite(f, x, budget, depth + 1, contextptr);
    if (f.type != _VECT)
      return false;
    if (g.is_symb_of_sommet(at_pow) && f._VECTptr->size() == 2)
      return f[1].type == _INT_ && f[1].val > 0 && f[1].val <= 8 &&
             intgab_continuous_finite(f[0], x, budget, depth + 1, contextptr);
    if (!g.is_symb_of_sommet(at_plus) && !g.is_symb_of_sommet(at_prod))
      return false;
    for (unsigned j = 0; j < f._VECTptr->size(); ++j)
      if (!intgab_continuous_finite(f[j], x, budget, depth + 1, contextptr))
        return false;
    return true;
  }
  ...
  __attribute__((noinline, optimize("Os")))
#endif
  static bool intgab(const gen & g0, const gen & x, const gen & a, const gen & b, gen & res,
                     bool nonrecursive, GIAC_CONTEXT) {
    if (x.type != _IDNT)
      return false;
    if (is_zero(g0)) {
      res = zero;
      ...
      if (g0_.type == _VECT && g0_._VECTptr->size() == 2) {
        ...
        if (lv.size() == 1 && lv.front() == x) {
          ...
          if (derive(base, x, contextptr) == 0) {
            ...
            g0_ = symbolic(at_pow, makesequence(x - a, na * expo)) *
                  symbolic(at_pow, makesequence(b - x, nb * expo));
          ...
          if (nb == 1 && !na) {
            ...
            if (derive(base, x, contextptr) == 0) {
              ...
              res = pow(-base, expo, contextptr) * pow(b - a, 1 + na * expo, contextptr) *
                    Gamma(inv(na, contextptr), contextptr) * Gamma(expo + 1, contextptr) /
                    Gamma(expo + 1 + inv(na, contextptr), contextptr) / na;
              return true;
            }
          } // nb==1 && !na
    ...
    if (!is_inf(a) && !is_inf(b) && g0_.is_symb_of_sommet(at_prod) &&
        g0_._SYMBptr->feuille.type == _VECT &&
        g0_._SYMBptr->feuille._VECTptr->size() == 2) { // Beta?
      // rewrite ^ of powers
      ...
      if (v1.size() == 2 && v1.front().is_symb_of_sommet(at_pow) &&
          v1.back().is_symb_of_sommet(at_pow)) {
        gen va = v1.front()._SYMBptr->feuille, vb = v1.back()._SYMBptr->feuille;
        if (va.type == _VECT && va._VECTptr->size() == 2 && vb.type == _VECT &&
            vb._VECTptr->size() == 2) {
          gen va1 = va._VECTptr->front(), va1x, va1c, va2 = va._VECTptr->back(),
              vb1 = vb._VECTptr->front(), vb2 = vb._VECTptr->back(), vb1x, vb1c;
          if (va1.is_symb_of_sommet(at_pow) && va1._SYMBptr->feuille.type == _VECT &&
              va1._SYMBptr->feuille._VECTptr->size() == 2) {
            va2 = va1._SYMBptr->feuille._VECTptr->back() * va2;
            va1 = va1._SYMBptr->feuille._VECTptr->front();
          }
          if (vb1.is_symb_of_sommet(at_pow) && vb1._SYMBptr->feuille.type == _VECT &&
              vb1._SYMBptr->feuille._VECTptr->size() == 2) {
            vb2 = vb1._SYMBptr->feuille._VECTptr->back() * vb2;
            vb1 = vb1._SYMBptr->feuille._VECTptr->front();
          }
          if (is_linear_wrt(va1, x, va1x, va1c, contextptr) &&
              is_linear_wrt(vb1, x, vb1x, vb1c, contextptr)) {
            if (is_zero(recursive_normal(va1c / va1x + a, contextptr), contextptr) &&
              ...
              res = g0mult * pow(va1x, va2, contextptr) * pow(-vb1x, vb2, contextptr) *
                    pow(b - a, va2 + vb2 + 1, contextptr) * Beta(va2 + 1, vb2 + 1, contextptr);
              return true;
            }
            ...
            if (factors.size() <= 8) {
              for (unsigned i = 0; i < factors.size(); ++i) {
                ...
                if ((factor.is_symb_of_sommet(at_sin) || factor.is_symb_of_sommet(at_cos)) &&
                    is_zero(im(factor._SYMBptr->feuille, contextptr)))
                  continue;
              ...
              int degree;
              gen leading;
              if (integration_rational_tail(envelope, x, degree, leading, budget, 0, contextptr) &&
                  degree <= -2) {
                res = 0;
                return true;
              }
    ...
    if (b == plus_inf) {
      ...
      if (vax.size() == 2 && vax.front() == x && vax.back().is_symb_of_sommet(at_exp)) {
        ...
        if (is_linear_wrt(expo._SYMBptr->feuille, x, expo_a, expo_b, contextptr) &&
            is_strictly_positive(expo_a, contextptr)) {
          ...
          if (gand.type == _VECT && gand._VECTptr->size() == 2) {
            ...
            if (rlvarx(gan, x).size() == 1 && is_linear_wrt(gad, expo, gad_a, gad_b, contextptr)) {
              ...
              if (is_zero(test)) {
                ...
                if (ganv.type == _VECT && !ganv._VECTptr->empty() &&
                    is_zero(ganv._VECTptr->back())) {
                  res = 0;
              ...
              if (n > 1 && is_zero(derive(coeffden, y, contextptr))) {
                ...
                if (xfactc.type == _VECT && !xfactc._VECTptr->empty()) {
                  ...
                  if (check) {
                    ...
                    gen int1 = (derive(xfact, x, contextptr) / ((n - 1) * expo_a) - xfact) *
                               subst(exponum / pow(y - 1, n - 1, contextptr), y,
                                     exp(expo_a * x, contextptr), false, contextptr);
                    int1 = _integrate(makesequence(int1, x, 0, plus_inf), contextptr);
                    gen int2 = xfact / gen(n - 1) *
                               subst(derive(exponum, y, contextptr) / pow(y - 1, n - 1, contextptr),
                                     y, exp(expo_a * x, contextptr), false, contextptr);
                    int2 = _integrate(makesequence(int2, x, 0, plus_inf), contextptr);
                    gen int3 = xfact * subst(additional[0] / (y - 1), y,
                                             exp(expo_a * x, contextptr), false, contextptr);
    ...
    if (eo == 2) {
      ...
      unsigned budget = 64;
      if (!intgab_continuous_finite(g0, x, budget, 0, contextptr))
        return false;
      res = 0;
      return true;
    }
        ...
        if (s >= 2) {
          ...
          for (i = 0; i < s; ++i) {
            if (v[i].is_symb_of_sommet(at_exp)) {
              ...
              if (is_linear_wrt(v0arg, x, alphacur, betacur, contextptr) &&
                  is_integer(
                      (periodecur = normal(alphacur * (b - a) / cst_two_pi / cst_i, contextptr)))) {
          ...
          if (!is_zero(periode)) {
            ...
            if (is_zero(re(alpha, contextptr))) {
              ...
              vecteur v2 = lop(
                  recursive_normal(rlvarx(subst(v, vin, vout, false, contextptr), x), contextptr),
                  at_exp);
              ...
              if (debug_infolevel)
                *logptr(contextptr)
                    << gettext("Searching int of ") << gof << gettext(" where ") << x
                    << gettext(" is on the unit circle, using residues") << '\n';
              ...
              for (int i = 0; i < s; ++i) {
                ...
                if (is_strictly_greater(radius, wabs, contextptr)) {
                  ...
                  somme_residues =
                      somme_residues +
                      tmpresidue; // was residue(gof,x,w[i],contextptr) but no reason to compute it twice...
                  if (is_undef(somme_residues))
                    return false;
                }
              }
              res = ratnormal(normal(periode * cst_two_pi / alpha * cst_i, contextptr) *
                                  somme_residues,
                              contextptr);
              if (changesign)
                res = -res;
  ...
  bool intgab(const gen & g0, const gen & x, const gen & a, const gen & b, gen & res,
              GIAC_CONTEXT) {
    return intgab(g0, x, a, b, res, false, contextptr);
  }

  static gen quotesubstcheck(const gen & g, const gen & x, const gen & i, const vecteur & v,
                             GIAC_CONTEXT) {
    if (!equalposcomp(v, i))
  ...
  bool is_admissible_poly(const polynome & P, int & deg, polynome & lcoeff, vecteur & roots,
                          GIAC_CONTEXT) {
  ...
  static bool in_sumab_int(gen & tmp1, const gen & gx, const vecteur & decals, const gen & lcoeff,
                           GIAC_CONTEXT) {
    ...
    for (int i = 0; i < nstep; ++i) {
      if (decals[i].type == _FRAC) {
        ...
        if (is_inf(tmp1))
          return false; // for sum(1/((n+1)*(2*n-1)),n,0,inf);
        tmp1 = ratnormal(tmp1 * pow(gx, n, contextptr), contextptr);
        tmp1 =
            ratnormal(subst(tmp1, gx, pow(gx, inv(d, contextptr), contextptr), false, contextptr),
                      contextptr);
      } else {
  ...
  static bool sumab_int(gen & tmp1, const gen & gx, const vecteur & decals, const gen & lcoeff,
                        GIAC_CONTEXT) {
  ...
  static bool sumab_ps(const polynome & Q, const polynome & R, const vecteur & v, const gen & a,
                       const gen & x, const gen & g, bool est_reel, const polynome & p,
                       const polynome & s, gen & res, GIAC_CONTEXT) {
    ...
    if (Q.degree(0) == 0) {
      ...
      identificateur idx(" sumw"); // identificateur idx(" x"); //
        ...
        if (r == 1) {
          ...
          ind[0] = 0;
          gen Qg = r2e(Q, v, contextptr) /
                   r2e(polynome(monomial<gen>(R.coord.front().value, ind)), v, contextptr);
          ...
          for (int n = R0.val; n < a.val; ++n) {
            tmp1 -=
                subst(Pg, x, n, false, contextptr) * pow(Qg, n - R0.val) / factorial(n - R0.val);
          }
          // Integration step
          decals = subvecteur(decals, vecteur(decals.size(), R0));
          if (sumab_int(tmp1, gx, decals, lcoeff, contextptr)) {
            gen coeffa =
                limit(g * r2e(s, v, contextptr) / r2e(p, v, contextptr) * factorial(a.val - R0.val),
                      *x._IDNTptr, a, 1, contextptr);
            res =
                limit(coeffa * tmp1, *gx._IDNTptr, Qg, 1, contextptr) / pow(Qg, a - R0, contextptr);
            if (est_reel)
        ...
        if (r >= 2) {
          ...
          gen coeffa = g * r2e(s, v, contextptr) / r2e(p, v, contextptr) /
                       pow(Qg, r * a - R0, contextptr) * factorial(r * a.val - r0);
          ...
          gen tmp1 = 0, tmpadd;
          for (int k = 0; k < r; ++k) {
            tmpadd = subst(tmp, gx, gx * exp(2 * k * cst_i * cst_pi / r, contextptr), false,
                           contextptr) *
                     exp(-2 * k * N * cst_i * cst_pi / r, contextptr);
            tmp1 += tmpadd;
          }
          tmp1 = tmp1 / r;
          // Integration step
        ...
        if (!r) {
          ...
          if (sumab_int(tmp1, gx, decals, lcoeff, contextptr)) {
            ...
            gen coeffa = limit(g * r2e(s, v, contextptr) / r2e(p, v, contextptr), *x._IDNTptr, a, 1,
                               contextptr);
  ...
  static bool in_sumab(const gen & g, const gen & x, const gen & a_orig, const gen & b_orig,
                       gen & res, bool testi, bool dopartfrac, GIAC_CONTEXT) {
    //grad
    ...
    if (!vD.empty()) {
      ...
      if (is_linear_wrt(g, vD0, A, B, contextptr) &&
          is_linear_wrt(vD0._SYMBptr->feuille, x, a, b, contextptr) &&
          in_sumab(B, x, a_orig, b_orig, res, testi, false, contextptr)) {
        // sum(A*Kronecker(a*x+b),x,a_orig,b_orig)+res
        gen xval = -b / a;
        if (is_integer(xval) && is_greater(xval, a_orig, contextptr) &&
            is_greater(b_orig, xval, contextptr))
    ...
    if (!vD.empty()) {
      ...
      if (is_linear_wrt(g, vD0, A, B, contextptr) &&
        ...
        gen xval = -b / a, resadd;
        if (is_integer(xval) && is_strictly_positive(a, contextptr) &&
            in_sumab(A, x, max(a_orig, xval, contextptr), b_orig, resadd, testi, false,
                     contextptr)) {
          res += resadd;
          return true;
        }
    ...
    if (Hyper) {
      ...
      if (a.type == _INT_ && b == plus_inf && p.lexsorted_degree() == 0 && r.coord.size() == 1 &&
          q + r == 0) {
        ...
        res = -_sum(makesequence(R, x, a, plus_inf), contextptr) +
              pow(2, 1 - pui, contextptr) *
                  _sum(makesequence(R, x, (a.val + 1) / 2, plus_inf), contextptr);
      ...
      if (is_linear_wrt(Q, x, Qa, Qb, contextptr) && is_linear_wrt(R, x, Ra, Rb, contextptr)) {
        ...
        if (diffa.type == _INT_ && diffb.type == _INT_) {
          ...
          if (p.lexsorted_degree() == 0) {
            res = P * (-Qa / Ra) + 1;
            if (!is_zero(res))
              res = simplify(pow(res, truen, contextptr) *
                                 limit(g, *x._IDNTptr, truea, 0, contextptr),
                             contextptr);
  ...
  bool sumab(const gen & g, const gen & x, const gen & a_orig, const gen & b_orig, gen & res,
             bool testi, GIAC_CONTEXT) {
    if (x.type != _IDNT)
      return false;
