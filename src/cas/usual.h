...
namespace giac {
  ...
  bool check_vect_38(
      const std::string & s); // check if s is a string identifier of a vector, matrix or list
  ...
  gen sqrt_mod(const gen & a, const gen & b, bool isprime,
               GIAC_CONTEXT); // set isprime to true if b is prime
  ...
  extern bool (*storcl_38)(gen & value, const char * name_space, const char * idname, gen indice,
