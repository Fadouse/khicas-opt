...
#undef HAVE_GMPXX_H
  ...
  inline polynome operator*(const gen & fact, const polynome & th) {
    return th * fact;
  }
  // a*b+c*d
  ...
  bool divrem1(const polynome & a, const polynome & b, polynome & quo, polynome & r,
               int exactquo = 0, bool allowrational = false);
  bool divrem(const polynome & th, const polynome & other, polynome & quo, polynome & rem,
              bool allowrational = false);
  ...
  void ichrem_smod_inplace(mpz_t * Az, mpz_t * Bz, mpz_t * iz, mpz_t * tmpz, gen & i,
                           const gen & j);
  ...
  bool gcdheu(const polynome & p, const polynome & q, polynome & p_simp, gen & np_simp,
              polynome & q_simp, gen & nq_simp, polynome & d, gen & d_content,
              bool skip_test = false, bool compute_cofactors = true);
  ...
  bool factor(const polynome & p, polynome & p_content, factorization & f, bool isprimitive,
              bool withsqrt, bool complexmode, const gen & divide_by_an, gen & extra_div);
  ...
  void partfrac(const polynome & num, const polynome & den,
                const std::vector<facteur<polynome>> & v, std::vector<pf<gen>> & pfde_VECT,
                polynome & ipnum, polynome & ipden, bool rational = true);
  ...
  bool factorunivsqff(const polynome & q, environment * env, vectpoly & v, int & ithprime,
                      int debug, int modfactor_primes);
  // find linear factor only
  int linearfind(const polynome & q, environment * env, polynome & qrem, vectpoly & v,
                 int & ithprime);
  ...
  gen peval(const polynome & p, const vecteur & v, const gen & m, bool simplify_at_end = false,
            std::vector<int_unsigned> * pptr = 0);
  ...
  template <class U>
  bool convert_myint(const polynome & p, const index_t & deg,
                     std::vector<T_unsigned<my_mpz, U>> & v) {
  ...
  template <class U>
  bool convert_myint(const polynome & p, const index_t & deg,
                     std::vector<T_unsigned<mpz_class, U>> & v) {
    typename std::vector<monomial<gen>>::const_iterator it = p.coord.begin(), itend = p.coord.end();
    v.clear();
    v.reserve(itend - it);
    U u;
  ...
  template <class U>
  bool convert_double(const polynome & p, const index_t & deg,
                      std::vector<T_unsigned<double, U>> & v) {
    typename std::vector<monomial<gen>>::const_iterator it = p.coord.begin(), itend = p.coord.end();
    v.clear();
    v.reserve(itend - it);
  ...
  template <class U>
  bool convert_int32(const polynome & p, const index_t & deg, std::vector<T_unsigned<int, U>> & v,
                     int modulo = 0) {
    typename std::vector<monomial<gen>>::const_iterator it = p.coord.begin(), itend = p.coord.end();
    v.clear();
    v.reserve(itend - it);
  ...
  template <class U>
  bool convert_int(const polynome & p, const index_t & deg,
                   std::vector<T_unsigned<longlong, U>> & v, longlong & maxp) {
    typename std::vector<monomial<gen>>::const_iterator it = p.coord.begin(), itend = p.coord.end();
    v.clear();
    v.reserve(itend - it);
  ...
  template <class U>
  bool convert_int(const polynome & p, const index_t & deg,
                   std::vector<T_unsigned<int128_t, U>> & v, int128_t & maxp) {
    typename std::vector<monomial<gen>>::const_iterator it = p.coord.begin(), itend = p.coord.end();
    v.clear();
    v.reserve(itend - it);
  ...
  template <class U>
  void convert_longlong(const std::vector<T_unsigned<gen, U>> & p,
                        std::vector<T_unsigned<longlong, U>> & pd) {
    typename std::vector<T_unsigned<gen, U>>::const_iterator it = p.begin(), itend = p.end();
    pd.reserve(itend - it);
    for (; it != itend; ++it)
  ...
  void convert_from(const std::vector<T_unsigned<T, U>> & p, std::vector<T_unsigned<gen, U>> & pd) {
  ...
  template <class T, class U>
  void convert_from(typename std::vector<T_unsigned<T, U>>::const_iterator it,
                    typename std::vector<T_unsigned<T, U>>::const_iterator itend,
                    const index_t & deg, typename std::vector<monomial<gen>>::iterator jt,
                    int mode = 0) {
    if (mode == 1) {
      for (; it != itend; ++jt, ++it) {
        jt->value = gen(it->g);
    ...
    int k;
    int count = 0;
#if defined(GIAC_NO_OPTIMIZATIONS) ||                                                              \
    ((defined(VISUALC) || defined(__APPLE__)) && !defined(GIAC_VECTOR)) ||                         \
    defined __clang__ // || defined NSPIRE || defined(FXCG)
    if (0) {
      count = 0;
    }
#else
  ...
  template <class T, class U> struct convert_t {
  ...
  template <class T, class U> void * do_convert_from(void * ptr) {
  ...
  void convert_from(const std::vector<T_unsigned<T, U>> & v, const index_t & deg, polynome & p,
    ...
    if (nthreads > 1 && int(taille) > nthreads * 1000) {
      pthread_t tab[nthreads];
      std::vector<convert_t<T, U>> arg(nthreads);
      for (int i = 0; i < nthreads; i++) {
        convert_t<T, U> tmp = {it + i * (taille / nthreads), it + (i + 1) * taille / nthreads, &deg,
                               jt + i * (taille / nthreads), 0};
        if (i == nthreads - 1) {
          tmp.itend = itend;
          convert_from<T, U>(tmp.it, tmp.itend, deg, tmp.jt, tmp.mode);
        } else {
          arg[i] = tmp;
          int res = pthread_create(&tab[i], (pthread_attr_t *)NULL, do_convert_from<T, U>,
                                   (void *)&arg[i]);
          if (res)
            convert_from<T, U>(tmp.it, tmp.itend, deg, tmp.jt, tmp.mode);
        }
