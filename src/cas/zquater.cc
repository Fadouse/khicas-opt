...
namespace giac {
  ...
  gen _galois_field(const gen & args, GIAC_CONTEXT) {
    return gensizeerr("Not implemented. Requires RTTI support (no NO_RTTI in config.h/no -fno-rtti "
                      "flag in Makefile)");
  }
  static define_unary_function_eval(__galois_field, &_galois_field, _galois_field_s);
  define_unary_function_ptr5(at_galois_field, alias_at_galois_field, &__galois_field, 0,
                             true); // auto-register
#else
...
define_unary_function_ptr5(at_quaternion, alias_at_quaternion, &__quaternion, 0,
                           true); // auto-register

string quaternion::print(GIAC_CONTEXT) const {
  return string(_quaternion_s) + "(" + r.print() + "," + i.print() + "," + j.print() + "," +
         k.print() + ")";
  this->dbgprint();
}
...
int is_irreducible_primitive(const modpoly & v, const gen & p, modpoly & vmin, int primitive,
                             GIAC_CONTEXT) {
  vmin = v;
  int m = int(v.size()) - 1;
  if (m < 2)
  ...
  if (primitive != 1) {
    if (primitive == 0)
      *logptr(contextptr)
          << gettext("Warning, minimal polynomial is only irreducible, not necessarily primitive")
          << '\n';
    return 3;
  }
...
vecteur find_irreducible_primitive(const gen & p_, int m, bool primitive, GIAC_CONTEXT) {
  ...
  *logptr(contextptr) << gettext("Warning, random search for irreducible polynomial did not work, "
...
gen _galois_field(const gen & args, GIAC_CONTEXT) {
  ...
  if (is_integer(args)) { // must be a power of a prime
    if (_isprime(args, contextptr) != 0) {
      return gensizeerr(
          gettext(
              "GF is used for non-prime finite field. Use % or mod for prime fields, e.g. 1 % ") +
          args.print(contextptr) + '.');
    }
    v.push_back(args);
  } else {
    if (args.type != _VECT) {
  ...
  if (s == 2) {
    ...
    *logptr(contextptr) << gettext("Setting ") << g << gettext(" as generator for Galois field ")
                        << K << '\n'
                        << gettext("(auxiliary polynomial variable for addition representation ")
                        << k << ")" << '\n';
    ++s;
  }
  if (s == 3) {
    ...
    if (v[2].type == _IDNT) {
      ...
      if (v[1].type == _SYMB) {
        ...
        if (P.type == _VECT) {
          ...
          if (res == 0)
            return gensizeerr("Polynomial " + v[1].print(contextptr) +
                              " is not irreducible modulo " + v[0].print(contextptr));
    ...
    if (v.back().type == _VECT && v.back()._VECTptr->size() == 3 && fieldvalue.type == _USER) {
      ...
      *logptr(contextptr) << gettext("Now e.g. ") << g
                          << gettext("^200+1 will build an element of ") << K << '\n';
  ...
  if (s != 4)
    return gensizeerr(gettext("galois_field has 1 or 4 arguments (charac p, irred poly P, var name "
                              "x, value as a poly of x or as a vector)"));
  vecteur a, P, vmin;
  gen & x = v[2];
  gen xid(x);
  ...
  if (is_undef(v[3])) {
    ...
    if (res == 2)
      *logptr(contextptr) << gettext("Warning ") << symb_horner(*v[1]._VECTptr, xid)
                          << gettext(" is irreducible but not primitive. You could use ")
                          << symb_horner(vmin, xid) << gettext(" instead ") << '\n';
  }
...
string galois_field::print(GIAC_CONTEXT) const {
  ...
  return string(_galois_field_s) + "(" + p.print() + "," + r2e(unmod(P), xid, contextptr).print() +
         "," + x.print() + "," + r2e(A, xid, contextptr).print() + ")";
...
galois_field::galois_field(const gen & g, bool primitive, GIAC_CONTEXT) {
  if (g.type == _USER) {
    ...
    if (g.type != _VECT || g._VECTptr->size() < 2 || !is_integer(g._VECTptr->front()) ||
        (*g._VECTptr)[1].type != _INT_)
...
galois_field::galois_field(const gen p_, const gen & P_, const gen & x_, const gen & a_,
                           bool doreduce)
    : p(p_), P(P_), x(x_), a(a_) {
  if (doreduce)
    reduce();
  ...
  if (ag.type == _USER && bg.type == _USER) {
    ...
    if (agptr->P != bgptr->P) {
      ...
      if (tmp.type != _USER || !(tmpptr = dynamic_cast<galois_field *>(tmp._USERptr)))
        return 0;
      *logptr(contextptr) << "Minimal polynomial of field generator "
                          << symb_horner(*tmpptr->P._VECTptr, tmpptr->x[2]) << '\n';
      tmp = galois_field(tmpptr->p, tmpptr->P, tmpptr->x, makevecteur(1, 0),
                         false); // field generator
...
gen galois_field::operator+(const gen & g) const {
  ...
  if (g.type != _USER)
    return sym_add(*this, cleanup(*this, g),
                   context0); // ok symbolic(at_plus,makesequence(g,*this));
  if (galois_field * gptr = dynamic_cast<galois_field *>(g._USERptr)) {
    if (char2 && gptr->a.type == _INT_) {
      ...
      return galois_field(p, P, x, char2_uncoerce(a), false) +
             galois_field(gptr->p, gptr->P, gptr->x, char2_uncoerce(gptr->a), false);
    }
...
const unsigned char gf28_tab2mult[] = {
    0,   1,   2,   26,  3,   51,  27,  199, 4,   224, 52,  239, 28,  105, 200, 76,  5,   101, 225,
    15,  53,  142, 240, 130, 29,  194, 106, 249, 201, 9,   77,  114, 6,   139, 102, 48,  226, 37,
    16,  34,  54,  148, 143, 219, 241, 19,  131, 70,  30,  182, 195, 126, 107, 40,  250, 186, 202,
    155, 10,  121, 78,  229, 115, 167, 7,   192, 140, 99,  103, 222, 49,  254, 227, 153, 38,  180,
    17,  146, 35,  137, 55,  209, 149, 207, 144, 151, 220, 190, 242, 211, 20,  93,  132, 57,  71,
    65,  31,  67,  183, 164, 196, 73,  127, 111, 108, 59,  41,  85,  251, 134, 187, 62,  203, 95,
    156, 160, 11,  22,  122, 44,  79,  213, 230, 173, 116, 244, 168, 88,  8,   113, 193, 248, 141,
    129, 100, 14,  104, 75,  223, 238, 50,  198, 255, 25,  228, 166, 154, 120, 39,  185, 181, 125,
    18,  69,  147, 218, 36,  33,  138, 47,  56,  64,  210, 92,  150, 189, 208, 206, 145, 136, 152,
    179, 221, 253, 191, 98,  243, 87,  212, 172, 21,  43,  94,  159, 133, 61,  58,  84,  72,  110,
    66,  163, 32,  46,  68,  217, 184, 124, 165, 119, 197, 24,  74,  237, 128, 13,  112, 247, 109,
    162, 60,  83,  42,  158, 86,  171, 252, 97,  135, 178, 188, 205, 63,  91,  204, 90,  96,  177,
    157, 170, 161, 82,  12,  246, 23,  236, 123, 118, 45,  216, 80,  175, 214, 234, 231, 232, 174,
    233, 117, 215, 245, 235, 169, 81,  89,  176};
...
int gf_char2_inv(int A, int N) {
  if (N == 285) {
    if (A < 2)
      return A; // means undef for A==0, inv(1)==1
...
void gf_char2_addp(const vector<int> & a, int astart, int aend, const vector<int> & b, int bstart,
                   int bend, vector<int> & res, int M) {
  res.clear();
...
void gf_char2_multp(const vector<int> & a, int astart, int aend, const vector<int> & b, int bstart,
                    int bend, vector<int> & res, int M) {
...
bool gf_char2_multpoly(const std::vector<int> & a, const std::vector<int> & b,
                       std::vector<int> & res, int M) {
  if (a.empty() || b.empty()) {
    res.clear();
    return true;
  }
...
void gf_char2_vectorint2vecteur(const std::vector<int> & source, vecteur & target, int M,
                                const gen & x) {
...
void gf_char2_vectorvectorint2mat(const std::vector<std::vector<int>> & source, matrice & target,
                                  int M, const gen & x) {
  target.resize(source.size());
  for (int i = 0; i < source.size(); ++i) {
    vecteur T;
...
bool gf_char2_mmult_atranb(const std::vector<std::vector<int>> & A,
                           const std::vector<std::vector<int>> & tranB,
                           std::vector<std::vector<int>> & C, int M) {
  int r = A.size();
  if (r == 0)
    return false;
  int c = tranB.size();
...
// v1=v1+c2*v2
void gf_char2_linear_combination(std::vector<int> & v1, int c2, const std::vector<int> & v2,
                                 int cstart, int cend, int M) {
  if (c2) {
...
bool gf_char2_rref(std::vector<std::vector<int>> & N, const gen & x, int M, vecteur & pivots,
                   std::vector<int> & permutation, std::vector<int> & maxrankcols, gen & det, int l,
    ...
    if (pivot) {
      ...
      if (fullreduction) {
        for (int ltemp = linit; ltemp < lmax; ++ltemp) {
          if (ltemp == l)
            continue;
          gf_char2_linear_combination(N[ltemp], N[ltemp][pivotcol], N[l], (use_cstart ? c : cmax),
                                      effcmax, M);
        }
      } else {
        for (int ltemp = l + 1; ltemp < lmax; ++ltemp) {
          ...
          gf_char2_linear_combination(N[ltemp], -N[ltemp][pivotcol], N[l],
...
// a[astart..aend[ + b[bstart..bend[ to res
void gf_addp(const vector<vector<int>> & a, int astart, int aend, const vector<vector<int>> & b,
             int bstart, int bend, vector<vector<int>> & res, int modulo) {
  res.clear();
...
void gf_addp(const vector<vector<int>> & a, const vector<vector<int>> & b,
             vector<vector<int>> & res, int modulo) {
  gf_addp(a, 0, a.size(), b, 0, b.size(), res, modulo);
}

// a[astart..aend[ - b[bstart..bend[ to res
void gf_subp(const vector<vector<int>> & a, int astart, int aend, const vector<vector<int>> & b,
             int bstart, int bend, vector<vector<int>> & res, int modulo) {
  res.clear();
  res.resize(giacmax(aend - astart, bend - bstart));
  int pos = 0;
...
void gf_subp(const vector<vector<int>> & a, const vector<vector<int>> & b,
             vector<vector<int>> & res, int modulo) {
...
void kronecker_shrink(const std::vector<int> & A, int n, std::vector<std::vector<int>> & a,
                      const std::vector<int> & pmin, int modulo) {
  // multiplying 2 polynomials of size a by size b
  // returns a poly of size a+b-1 instead of a+b, must fill an initial 0
  std::vector<int>::const_iterator it = A.begin() + n - 1, itnext, itend = A.end();
  for (; it < itend;) {
...
void mulext(const vector<int> & a, const vector<int> & b, const vector<int> & pmin, int modulo,
            vector<int> & res) {
  if (b.empty()) {
    res.clear();
    return;
...
static void mulext(const vector<int> & a, const vector<int> & b, const vector<int> & pmin,
                   int modulo, vector<int> & res, vector<int> & tmp, vector<int> & q) {
  if (b.empty()) {
    res.clear();
    return;
  }
...
static void mulext(const vecteur & a, const vecteur & b, const vecteur & pmin, int modulo,
                   vecteur & res) {
  if (b.empty()) {
    res.clear();
    return;
...
static void mulext(vector<vector<int>> & a, const vector<int> & b, const vector<int> & pmin,
                   int modulo, vector<int> & tmp, vector<int> & q) {
  if (b.empty()) {
    a.clear();
    return;
  }
...
// a[astart..aend[ * b[bstart..bend[ to res
void gf_multp(const vector<vector<int>> & a, int astart, int aend, const vector<vector<int>> & b,
              int bstart, int bend, vector<vector<int>> & res, const vector<int> & pmin,
              int modulo) {
  int adeg = aend - astart - 1, bdeg = bend - bstart - 1;
  if (bdeg < adeg) {
...
bool gf_multpoly(const std::vector<std::vector<int>> & a, const std::vector<std::vector<int>> & b,
                 std::vector<std::vector<int>> & res, const std::vector<int> & pmin, int modulo) {
  if (a.empty() || b.empty()) {
    res.clear();
    return true;
  }
...
int gf_vecteur2vectorvectorint(const vecteur & v, std::vector<std::vector<int>> & V, gen & x,
                               std::vector<int> & pmin) {
  // quick check
  int i = 0, carac = 0;
  gen P;
...
int gf_matrice2vectorvectorvectorint(const matrice & m,
                                     std::vector<std::vector<std::vector<int>>> & M, gen & x,
                                     std::vector<int> & pmin) {
  M.resize(m.size());
  vector<int> b;
  int carac = 0, c;
...
void gf_vectorvectorint2vecteur(const std::vector<std::vector<int>> & source, vecteur & target,
                                const gen & carac, const vecteur & pmin, const gen & x) {
  //CERR << pmin << endl;
  target.resize(source.size());
  vecteur tmp;
  for (int i = 0; i < source.size(); ++i) {
...
void gf_vectorvectorint2vecteur(const std::vector<std::vector<int>> & source, vecteur & target,
                                int carac, const std::vector<int> & pmin, const gen & x) {
  vecteur Pmin;
...
void gf_vectorvectorvectorint2mat(const std::vector<std::vector<std::vector<int>>> & source,
                                  matrice & target, int carac, const std::vector<int> & pmin,
                                  const gen & x) {
  vecteur Pmin;
  vector_int2vecteur(pmin, Pmin);
  target.resize(source.size());
...
bool galois_field::is_zero() const {
  return a == zero || (a.type == _VECT && (a._VECTptr->empty() ||
                                           (a._VECTptr->size() == 1 && a._VECTptr->front() == 0)));
}
...
bool galois_field::is_minus_one() const {
  return a == plus_one ||
         (a.type == _VECT && a._VECTptr->size() == 1 &&
          smod(a._VECTptr->front(), p) == -1); // this is really plus_one because we are in char 2
}
        ...
        if (g.type == _IDNT)
          ...
          if (g.type == _SYMB) {
            ...
            if (g._SYMBptr->sommet == at_pow && g._SYMBptr->feuille.type == _VECT &&
              ...
              return pow((*this)(g._SYMBptr->feuille[0], contextptr), g._SYMBptr->feuille[1],
                         contextptr);
            return gensizeerr(
                x[1].print() + "(" + g.print() +
                gettext(") invalid, works only for integers or polynomials depending on ") +
                xid.print());
          }
        }
      } else
...
gen galois_field::polyfactor(const polynome & p0, factorization & f) const {
  ...
  if (!is_exactly_one(lcoeff))
    f.push_back(facteur<polynome>(polynome(monomial<gen>(lcoeff, 0, p.dim)), 1));
  return 0;
}
