...
namespace giac {
  ...
  inline bool lexer_functions_register(const unary_function_ptr * u, const char * s,
                                       int parser_token) {
    return lexer_functions_register(*u, s, parser_token);
  }
  ...
  int find_or_make_symbol(const std::string & s, gen & res, void * scanner, bool check38,
                          GIAC_CONTEXT);
  ...
  YY_BUFFER_STATE set_lexer_string(const std::string & s, void *& scanner,
                                   const context * contextptr, int maxsize = 0);
