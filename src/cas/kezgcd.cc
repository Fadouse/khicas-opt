...
namespace giac {
  ...
  static bool convert_from_truncate(const vector<T_unsigned<int, hashgcd_U>> & p, hashgcd_U var,
                                    polynome & P) {
    P.dim = 1;
    P.coord.clear();
  ...
  bool find_good_eval(const polynome & F, const polynome & G, polynome & Fb, polynome & Gb,
                      vecteur & b, bool debuglog, const gen & mod) {
    ...
    if ( //false &&
      ...
      if (convert(F, G, d, vars, f, g, modulo)) {
        ...
        for (;; ++essai) {
          ...
          if (!fb.empty() && !gb.empty() && int(fb.front().u / vars.front()) == Fdeg &&
              int(gb.front().u / vars.front()) == Gdeg) {
  ...
  bool ezgcd(const polynome & F_orig, const polynome & G_orig, polynome & GCD, bool is_sqff,
             bool is_primitive, int max_gcddeg, double maxop) {
    return false;
  }
  ...
  bool try_sparse_factor(const polynome & pcur, const factorization & v, int mult,
                         factorization & f) {
    return false;
  }
