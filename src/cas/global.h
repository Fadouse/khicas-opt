...
#define GIAC_CONTEXT0 const giac::context * contextptr = 0
    ...
    !defined(NSPIRE) && !defined(FXCG)
#include <sys/times.h>
#else
#if defined VISUALC || defined BESTA_OS || defined FREERTOS
typedef long pid_t;
#else // VISUALC
#if !defined(__MINGW_H) && !defined(NSPIRE) && !defined(FXCG) && !defined(__ANDROID__) &&          \
    !defined(NSPIRE_NEWLIB) && !defined(OSX) && !defined(IOS) && !defined(OSXIOS) &&               \
    !defined(FIR_LINUX)
...
#include "help.h"
#if !defined(                                                                                      \
    FXCG) // !defined(GIAC_HAS_STO_38) // && !defined(ConnectivityKit) && !defined(BESTA_OS)
#include "tinymt32.h"
#endif
...
/* This is for C++ and does no harm in C */
  ...
  unsigned int ConvertUTF8toUTF16(const UTF8 * sourceStart, const UTF8 * sourceEnd,
                                  UTF16 * targetStart, UTF16 * targetEnd, ConversionFlags flags);

  unsigned int ConvertUTF16toUTF8(const UTF16 * sourceStart, const UTF16 * sourceEnd,
...
#define GIAC_DATA_ESCAPE ((char)27)
  ...
  extern unsigned simult_primes, simult_primes2, simult_primes_seuil2, simult_primes3,
      simult_primes_seuil3;
...
#define control_c()                                                                                \
  if (ctrl_c) {                                                                                    \
    interrupted = true;                                                                            \
    CERR << "Throwing exception for user interruption." << std::endl;                              \
...
public:
  ...
  public:
    ...
    std_matrix(size_t i, const dbgprint_vector<T> & v)
        : std::vector<dbgprint_vector<T>>::vector(i, v) {};
    std_matrix(size_t i, size_t j)
        : std::vector<dbgprint_vector<T>>::vector(i, dbgprint_vector<T>(j)) {};
    std_matrix(size_t i, size_t j, const T & t)
        : std::vector<dbgprint_vector<T>>::vector(i, dbgprint_vector<T>(j, t)) {};
    // ~dbgprint_vector() { };
    // inherited destructors
  ...
  vecteur *
  keywords_vecteur_ptr(); // idnt assigned to a commandname for localization, like mediatrice for perpen_bissector
  ...
  struct logo_turtle {
    ...
    int s; //std::string s;
    logo_turtle()
        : x(100), y(100), theta(0), visible(true), mark(true), direct(true), color(0),
          turtle_length(1), radius(0) {}
    inline bool equal_except_nomark(const logo_turtle & t) const {
      return x == t.x && y == t.y && turtle_length == t.turtle_length && s == t.s &&
             radius == t.radius;
    }
  };
  ...
  bool make_thread(const giac::gen & g, int level, const giac_callback & f, void * f_param,
...
tinymt32_t * rand_seed(GIAC_CONTEXT);
  ...
  std::string & xcasrc();
  std::string & xcasroot();
  std::string add_extension(const std::string & s, const std::string & ext,
  ...
  */
  ...
  bool archive_save(void * f, const gen & g,
                    size_t writefunc(void const * p, size_t nbBytes, size_t NbElements,
                                     void * file),
                    GIAC_CONTEXT, bool noRecurse = false);

  bool archive_save(void * f, const gen & g, GIAC_CONTEXT);
  // restore a gen from an opened file
  gen archive_restore(void * f,
                      size_t readfunc(void * p, size_t nbBytes, size_t NbElements, void * file),
                      GIAC_CONTEXT);
  ...
  extern void (*my_gprintf)(unsigned special, const std::string & format, const vecteur & v,
                            GIAC_CONTEXT);
  ...
  void gprintf(unsigned special, const std::string & format, const vecteur & v, int step_info,
               GIAC_CONTEXT);
