...
namespace giac {
  ...
  vecteur gauss(const gen & q, const vecteur & x, vecteur & D, vecteur & U, vecteur & P,
                GIAC_CONTEXT) {
    int n = int(x.size()), b;
    gen u1, u2, q1, l1, l2;
    vecteur R(1);
  ...
  gen conique_ratparam(const gen & eq, const gen & M, GIAC_CONTEXT) {
    ...
    gen eqM = _quo(
        makesequence(
            subst(eq, makevecteur(x, y), makevecteur(Mx + x, My + t * x), false, contextptr), x),
        contextptr);
    gen a, b;
  ...
  vecteur conique_ratparams(const gen & eq, const gen & M, GIAC_CONTEXT) {
    ...
    if (!is_linear_wrt(eqM, x, deno, num, contextptr) || !is_linear_wrt(num, t, d, e, contextptr) ||
        !is_quadratic_wrt(deno, t, a, b, c, contextptr))
      return vecteur(1, undef);
    return makevecteur(at_ellipse, M, a, b, c, -d, -e);
  }

  int conique_reduite(const gen & equation_conique, const gen & pointsurconique,
                      const vecteur & nom_des_variables, gen & x0, gen & y0, vecteur & V0,
                      vecteur & V1, gen & propre, gen & equation_reduite, vecteur & param_curves,
                      gen & ratparam, bool numeric, GIAC_CONTEXT, gen * aptr, gen * bptr) {
    ...
    if (a * c - b * b == 0) {
      ...
      if (a != 0) {
        ...
        if (coeffx != 0) {
          // parabole
          X0 = rdiv((d * a + e * b) * (d * a + e * b) - f * (a * a + b * b) * vp1,
                    gen(2) * vp1 * (d * b - e * a) * norme, contextptr);
          equation_reduite = coeffy2 * pow(x[1], 2) + coeffx * x[0];
        } else {
          //si d*b-e*a==0 alors X0=0
          coeffcst = normal(f - (d * a + e * b) * (d * a + e * b) / ((a + c) * (a * a + b * b)),
                            contextptr);
      ...
      gen t(t__IDNT_e);
        ...
        if (sprodvp > 0) {
          ...
          *logptr(contextptr) << gettext("Ellipsis of center (") << x0 << "," << y0 << ")" << '\n';
          ...
          if (numeric) {
            if (!is_undef(ratparam))
              tmp = subst(evalf(ratparam, 1, contextptr), t, symbolic(at_tan, t / 2), false,
                          contextptr);
            else {
              tmp = evalf(vp0, 1, contextptr) * symb_cos(t) +
                    cst_i * evalf(vp1, 1, contextptr) * symb_sin(t);
          ...
          if (is_undef(ratparam))
            ratparam =
                z0 + zV0 * (vp0 * (1 - pow(t, 2)) + cst_i * vp1 * plus_two * t) / (1 + pow(t, 2));

          bool rad = angle_radian(contextptr), deg = angle_degree(contextptr);
          param_curves.push_back(makevecteur(tmp, t, 0, rad ? cst_two_pi : (deg ? 360 : 400),
                                             rad ? cst_two_pi / 60 : (deg ? 6 : rdiv(20, 3)), q,
                                             ratparam)); //grad
          ...
          if (is_zero(coeffcst)) {
            ...
            *logptr(contextptr) << gettext("2 secant lines at (") << x0 << "," << y0 << ")" << '\n';
            ...
            if (is_undef(ratparam))
              ratparam = makevecteur(z0 + zV0 * (1 + cst_i * directeur) * t,
                                     z0 + zV0 * (1 - cst_i * directeur) * t);
            param_curves.push_back(
                gen(makevecteur(z0, z0 + zV0 * (1 + cst_i * directeur)), _LINE__VECT));
            param_curves.push_back(
                gen(makevecteur(z0, z0 + zV0 * (1 - cst_i * directeur)), _LINE__VECT));
            return -4;
          }
          ...
          *logptr(contextptr) << gettext("Hyperbola of center (") << x0 << "," << y0 << ")" << '\n';
          ...
          if (numeric) {
            if (!is_undef(ratparam))
              ...
              tmp = evalf(vp0, 1, contextptr) * symbolic(sprodcoeff < 0 ? at_cosh : at_sinh, t) +
                    cst_i * evalf(vp1, 1, contextptr) *
                        symbolic(sprodcoeff < 0 ? at_sinh : at_cosh, t);
              tmp = evalf(z0, 1, contextptr) + evalf(zV0, 1, contextptr) * tmp;
            }
          } else {
            tmp = vp0 * symbolic(sprodcoeff < 0 ? at_cosh : at_sinh, t) +
                  cst_i * vp1 * symbolic(sprodcoeff < 0 ? at_sinh : at_cosh, t);
            tmp = z0 + zV0 * tmp;
          }
          bool noratparam = is_undef(ratparam);
          if (noratparam) {
            ratparam =
                vp0 * gen((sprodcoeff < 0) ? (t + plus_one / t) / 2 : (t - plus_one / t) / 2) +
                cst_i * vp1 * ((sprodcoeff < 0) ? (t - plus_one / t) / 2 : (t + plus_one / t) / 2);
        ...
        param_curves.push_back(makevecteur(tmp, t, -tt, tt, tt / 100, q, ratparam));
          ...
          if (noratparam) {
            if (numeric) {
              tmp = (sprodcoeff < 0 ? -1 : 1) * evalf(vp0, 1, contextptr) *
                        symbolic(sprodcoeff < 0 ? at_cosh : at_sinh, t) +
                    (sprodcoeff < 0 ? 1 : -1) * cst_i * evalf(vp1, 1, contextptr) *
                        symbolic(sprodcoeff < 0 ? at_sinh : at_cosh, t);
              tmp = evalf(z0, 1, contextptr) + evalf(zV0, 1, contextptr) * tmp;
            } else {
              tmp = (sprodcoeff < 0 ? -1 : 1) * vp0 *
                        symbolic(sprodcoeff < 0 ? at_cosh : at_sinh, t) +
                    (sprodcoeff < 0 ? 1 : -1) * cst_i * vp1 *
                        symbolic(sprodcoeff < 0 ? at_sinh : at_cosh, t);
              tmp = z0 + zV0 * tmp;
            }
#if defined POCKETCAS
...
#ifdef RTOS_THREADX
  bool quadrique_reduite(const gen & q, const gen & M, const vecteur & vxyz, gen & x, gen & y,
                         gen & z, vecteur & u, vecteur & v, vecteur & w, vecteur & propre,
                         gen & equation_reduite, vecteur & param_surface, vecteur & centre,
...
bool quadrique_reduite(const gen & q, const gen & M, const vecteur & vxyz, gen & x, gen & y,
  ...
  gen Q = normal(t * t * (subst(q, vxyz, makevecteur(x / t, y / t, z / t), false, contextptr)),
  ...
  if (s1g.type != _DOUBLE_ || s2g.type != _DOUBLE_ || s3g.type != _DOUBLE_) {
    *logptr(contextptr) << (gettext("Can't check sign ") + s1g.print(contextptr) + gettext(" or ") +
                            s2g.print(contextptr) + gettext(" or ") + s3g.print(contextptr))
                        << '\n';
    return false;
  }
        ...
        if (ds == 0) {
          ...
          if (s1s * s3s > 0) {
            if (s1s * ds > 0)
              *logptr(contextptr) << gettext("Empty ellipsoid") << '\n';
              ...
              gen a = sqrt(-d / s1, contextptr), b = sqrt(-d / s2, contextptr),
                  c = sqrt(-d / s3, contextptr);
              // x^2/a^2+y^2/b^2+z^2/c^2=1
              gen eq = makevecteur(a * symb_sin(upar) * symb_cos(vpar),
                                   b * symb_sin(upar) * symb_sin(vpar), c * symb_cos(upar));
            ...
            if (s1s * ds > 0) {
              ...
              gen eq = makevecteur(a * symb_sinh(upar) * symb_cos(vpar),
                                   b * symb_sinh(upar) * symb_sin(vpar), c * symb_cosh(upar));
              eq = centre + multmatvecteur(P, *eq._VECTptr);
              *logptr(contextptr) << gettext("2-fold hyperboloid of center ") << centre << '\n';
              ...
              vstep = symb_equal(vstep, cst_two_pi / 20);
              param_surface.push_back(makevecteur(eq, ueq, veq, ustep, vstep));
              eq = makevecteur(a * symb_sinh(upar) * symb_cos(vpar),
                               b * symb_sinh(upar) * symb_sin(vpar), -c * symb_cosh(upar));
              eq = centre + multmatvecteur(P, *eq._VECTptr);
              param_surface.push_back(makevecteur(eq, ueq, veq, ustep, vstep));
              ...
              gen eq = makevecteur(a * symb_cosh(upar) * symb_cos(vpar),
                                   b * symb_cosh(upar) * symb_sin(vpar), c * symb_sinh(upar));
              *logptr(contextptr) << gettext("2-fold hyperboloid of center ") << centre << '\n';
              eq = centre + multmatvecteur(P, *eq._VECTptr);
          ...
          *logptr(contextptr) << gettext("Elliptic cylinder around ") << centre << '\n';
          ...
          if (is_zero(d)) { // 2 plans
            *logptr(contextptr) << gettext("2 plans intersecting at ") << centre << '\n';
            ...
            param_surface.push_back(
                symbolic(at_hyperplan, gen(makevecteur(n, centre), _SEQ__VECT)));
    ...
    if (!is_zero(c2) || !is_zero(c3)) {
      ...
      *logptr(contextptr) << gettext("2 parallel plans") << '\n';
      ...
      if (s1s * ds < 0) { // 2 plans x = +/- sqrt(-dred/s1)
        ...
        param_surface.push_back(
            symbolic(at_hyperplan, gen(makevecteur(u, centre + a * u), _SEQ__VECT)));
        param_surface.push_back(
            symbolic(at_hyperplan, gen(makevecteur(u, centre - a * u), _SEQ__VECT)));
      }
      return true;
  ...
  gen conique_quadrique_reduite(const gen & args, GIAC_CONTEXT, bool conique) {
    ...
    if (s == 1) {
      v.push_back(conique ? makevecteur(x__IDNT_e, y__IDNT_e)
                          : makevecteur(x__IDNT_e, y__IDNT_e, z__IDNT_e));
    }
    if (v[0].type == _SYMB && v[1].type == _VECT) {
      ...
      if (v[1]._VECTptr->size() == 3) {
        quadrique_reduite(v[0], undef, *v[1]._VECTptr, x0, y0, z0, V0, V1, V2, proprev, eq_reduite,
                          param_curves, centre, false, contextptr);
        return makevecteur(centre, mtran(makevecteur(V0, V1, V2)), proprev, eq_reduite,
                           param_curves);
      } else {
        if (!conique_reduite(v[0], undef, *v[1]._VECTptr, x0, y0, V0, V1, propre, eq_reduite,
                             param_curves, ratparam, false, contextptr))
          return gensizeerr(contextptr);
        return makevecteur(makevecteur(x0, y0), mtran(makevecteur(V0, V1)), propre, eq_reduite,
                           param_curves);
      }
    }
  ...
  define_unary_function_ptr5(at_conique_reduite, alias_at_conique_reduite, &__conique_reduite, 0,
                             true);
  ...
  define_unary_function_ptr5(at_quadrique_reduite, alias_at_quadrique_reduite, &__quadrique_reduite,
                             0, true);
