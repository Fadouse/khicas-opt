...
namespace giac {
  ...
  bool hensel_lift(const polynome & p, const polynome & lcp, const polynome & qb,
                   const polynome & rb, const vecteur & b, polynome & q, polynome & r,
                   bool linear_lift = true, double maxop = -1);

  bool find_good_eval(const polynome & F, const polynome & G, polynome & Fb, polynome & Gb,
                      vecteur & b, bool debuglog = false, const gen & mod = zero);
  ...
  bool reduce_divrem(const polynome & a, const polynome & other, const vecteur & v, int n,
  ...
  bool try_sparse_factor(const polynome & pcur, const factorization & v0, int mult,
                         factorization & f);
  bool try_sparse_factor_bi(polynome & pcur, int mult, factorization & f);

  bool try_hensel_lift_factor(const polynome & pcur, const polynome & F0, const factorization & v0,
                              int mult, factorization & f);

  // find u,v,d s.t. u*p+v*q=d by Hensel lift
  bool try_hensel_egcd(const polynome & p, const polynome & q, polynome & u, polynome & v,
                       polynome & d);
  ...
  bool ezgcd(const polynome & F_orig, const polynome & G_orig, polynome & GCD, bool is_sqff = false,
             bool is_primitive = false, int max_gcddeg = 0, double maxop = -1);
