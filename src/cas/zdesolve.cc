...
namespace giac {
  ...
  static gen pf_ilaplace(const gen & e0, const gen & x, gen & remains, GIAC_CONTEXT) {
    ...
    for (; it != itend;) {
      ...
      if (glap.type != _POLY)
        return gensizeerr(gettext("desolve.cc/pf_ilaplace"));
      ...
      for (; it != itend; ++it) {
        ...
        case 1: // 1st order
          findde(it->den, a, b);
          lnpart = lnpart + rdiv(r2e(it->num, l, contextptr),
                                 r2e(firstcoeff(a), lprime, contextptr), contextptr) *
                                exp(r2e(rdiv(-b, a, contextptr), lprime, contextptr) * laplace_var,
                                    contextptr);
          break;
        case 2: // 2nd order
          findabcdelta(it->fact, a, b, c, delta);
          exppart = exp(r2e(rdiv(-b, gen(2) * a, contextptr), lprime, contextptr) * laplace_var,
                        contextptr);
          ...
          if (uselog) {
            ...
            lnpart = lnpart + rdiv(r2e(d, lprime, contextptr), r2e(lnpartden, lprime, contextptr),
                                   contextptr) *
                                  cosh(racine * laplace_var, contextptr) * exppart;
            gen aa = ratnormal(r2e(atannum, lprime, contextptr) / r2e(alpha, lprime, contextptr) /
                                   sqrtdelta,
                               contextptr);
            ...
            lnpart = lnpart + rdiv(r2e(d, lprime, contextptr), r2e(lnpartden, lprime, contextptr),
                                   contextptr) *
                                  cos(racine * laplace_var, contextptr) * exppart;
            gen aa = ratnormal(r2e(atannum, lprime, contextptr) / r2e(alpha, lprime, contextptr) /
                                   sqrtdelta,
                               contextptr);
      ...
      for (int i = 0; i < nums; ++i) {
        ...
        tmp = tmp * symbolic(at_Dirac, (i == nums - 1) ? laplace_var
                                                       : gen(makevecteur(laplace_var, nums - 1 - i),
                                                             _SEQ__VECT));
        res += tmp;
      }
      ...
      if (is_zero(expa))
        ...
        res += quotesubst(lnpart, laplace_var, laplace_var + expa, contextptr) *
               exp(expb, contextptr) * _Heaviside(laplace_var + expa, contextptr);
    }
    return res;
  }
  ...
  static bool is_linear_diffeq(const gen & f_orig, const gen & x, const gen & y, int order,
                               vecteur & v, int step_info, GIAC_CONTEXT) {
    v.clear();
    ...
    if (step_info && v.size() > 3)
      gprintf("Linear differential equation of coefficients %gen\nsecond member %gen",
              makevecteur(vecteur(v.begin(), v.end() - 1), -v.back()), step_info, contextptr);
    return true;
  }

  static bool find_n_derivatives_function(const gen & f, const gen & x, int & nder,
                                          gen & fonction) {
  ...
  static gen in_desolve_with_conditions(const vecteur & v_, const gen & x, const gen & y,
                                        const gen & solution_generale, const vecteur & parameters,
                                        const gen & f, int step_info, GIAC_CONTEXT) {
    gen yy(y);
    vecteur v(v_);
    if (yy.type != _IDNT)
      yy = function_of(y, x);
    ...
    if (solution_generale.type == _VECT && v.size() == 2) {
      ...
      if (init.is_symb_of_sommet(at_equal) && init._SYMBptr->feuille.type == _VECT &&
          init._SYMBptr->feuille._VECTptr->size() >= 2) {
        point = (*init._SYMBptr->feuille._VECTptr)[0];
        init = (*init._SYMBptr->feuille._VECTptr)[1];
        if (!point.is_symb_of_sommet(at_of) || point._SYMBptr->feuille.type != _VECT ||
            point._SYMBptr->feuille._VECTptr->size() < 2 ||
            point._SYMBptr->feuille._VECTptr->front() != y)
    ...
    for (unsigned ndiff = 0; jt != jtend; ++ndiff, ++jt) {
      if (jt->type == _VECT && jt->_VECTptr->size() == 2) {
        if (ndiff)
          *jt = symbolic(at_of, makesequence(symbolic(at_derive, makesequence(y, x, int(ndiff))),
                                             jt->_VECTptr->front())) -
                jt->_VECTptr->back();
        else
    ...
    conditions = *eval(subst(conditions, substin, substout, false, contextptr),
    ...
    vecteur parameters_solutions =
        *_solve(gen(makevecteur(conditions, parameters), _SEQ__VECT), contextptr)._VECTptr;
    if (step_info)
      gprintf(
          "General solution %gen\nSolving initial conditions\n%gen\nunknowns %gen\nSolutions %gen",
          makevecteur(solution_generale, conditions, parameters, parameters_solutions), step_info,
          contextptr);
    ...
    vecteur res;
    for (; it != itend; ++it) {
      gen solgen = eval(subst(solution_generale, parameters, *it, false, contextptr),
                        eval_level(contextptr), contextptr);
  ...
  static gen desolve_with_conditions(const vecteur & v, const gen & x, const gen & y, gen & f,
                                     int step_info, GIAC_CONTEXT) {
    if (v.empty())
      return gensizeerr(contextptr);
    int ordre;
    bool num = false;
    vecteur parameters;
    gen solution_generale(
        desolve_f(v.front(), x, y, ordre, parameters, f, step_info, num, contextptr));
    if (solution_generale.type != _VECT) {
      gen res = in_desolve_with_conditions(v, x, y, solution_generale, parameters, f, step_info,
                                           contextptr);
      return num ? evalf(res, 1, contextptr) : res;
    }
    solution_generale.subtype =
  ...
  bool separate_variables(const gen & f, const gen & x, const gen & y, gen & xfact, gen & yfact,
                          int step_info, GIAC_CONTEXT) {
    ...
    if (step_info)
      gprintf("Separable variables d%gen/%gen=%gen*d%gen", makevecteur(y, yfact, xfact, x),
              step_info, contextptr);
    return true;
  }
  ...
  static gen desolve_lin1(const gen & a, const gen & b, const gen & c, const gen & x,
                          vecteur & parameters, int step_info, GIAC_CONTEXT) {
    if (step_info)
      gprintf("Linear differential equation of order 1 a*y'+b*y+c=0\na=%gen, b=%gen, c=%gen",
              makevecteur(a, b, c), step_info, contextptr);
    if (a.type == _VECT) {
    ...
    gen C =
        integrate_without_lnabs(ratnormal(rdiv(-c, a, contextptr) * i, contextptr), x, contextptr);
    if (step_info && C != 0)
  ...
  bool desolve_linn(const gen & x, const gen & y, const gen & t, int n, vecteur & v,
                    vecteur & parameters, gen & result, int step_info, GIAC_CONTEXT) {
    // 1st order
    ...
    if (derive(v, x, contextptr) == vecteur(n + 1, zero)) {
      ...
      if (n <= 3) {
        ...
        if (n == 2 && rac.size() == 1) {
          parameters.push_back(diffeq_constante(int(parameters.size()), contextptr));
          parameters.push_back(diffeq_constante(int(parameters.size()), contextptr));
          gen sol = exp(rac.front() * x, contextptr) *
                    (parameters[parameters.size() - 2] * x + parameters.back());
          if (step_info)
          ...
          if (b)
            calc_mode(0, contextptr);
          gen part =
              _integrate(makesequence(-cst / v.front() * exp(-rac.front() * x, contextptr), x),
                         contextptr) *
                  x +
              _integrate(makesequence(cst / v.front() * x * exp(-rac.front() * x, contextptr), x),
                         contextptr);
          if (step_info)
        ...
        if (int(rac.size()) == n) {
          ...
          for (int j = 0; j < n;) {
            if (j < n - 1 &&
                is_zero(ratnormal(rac[j] - conj(rac[j + 1], contextptr), contextptr), contextptr)) {
              gen racr, raci;
              ...
              parameters.push_back(diffeq_constante(int(parameters.size()), contextptr));
              parameters.push_back(diffeq_constante(int(parameters.size()), contextptr));
              sol += exp(racr * x, contextptr) *
                     (parameters[parameters.size() - 2] * cos(raci * x, contextptr) +
                      parameters[parameters.size() - 1] * sin(raci * x, contextptr));
              j += 2;
              continue;
            }
          ...
          if (ckmatrix(M_)) {
            ...
            for (unsigned i = 0; i < c.size(); ++i) {
              ...
              if (b)
                calc_mode(0, contextptr);
              gen tmp =
                  _lin(makesequence(c[i] * exp(-rac[i] * x, contextptr), at_sqrt), contextptr);
	...
	if (desolve_linn(x,y,t,1,v1,parameters,result,step_info,contextptr)){
      ...
      */
      ...
      bool x2 = is_zero(derive(ratnormal(u * x, contextptr), x, contextptr)) &&
                is_zero(derive(ratnormal(v * x * x, contextptr), x, contextptr));
      if (cst || x2) {
        gen s, t;
        if (cst) {
          s = x;
      ...
      if (lvarxwithinv(makevecteur(a, b, c), x, contextptr) == vecteur(1, x)) {
        ...
        if (a.type == _VECT && b.type == _VECT && c.type == _VECT) {
          int A = int(a._VECTptr->size()) - 1, B = int(b._VECTptr->size()) - 1,
              C = int(c._VECTptr->size()) - 1, N = -1;
          if (C == B - 1) {
          ...
          if (C == A - 2 && B - 1 < C) {
            gen delta =
                (1 + sqrt(1 + 4 * c._VECTptr->front() / a._VECTptr->front(), contextptr)) / 2;
            if (delta.type == _INT_ && delta.val > N)
              N = delta.val;
          }
          if (N >= 0) {
            ...
            if (!m.empty()) {
              ...
              if (sol.type == _VECT) {
                ...
                *logptr(contextptr) << "Polynomial solution found " << sol << '\n';
                ...
                gen res = desolve_lin1(a0 * sol, 2 * a0 * derive(sol, x, contextptr) + b0 * sol, d,
                                       x, parameters, step_info, contextptr);
      ...
      if (d == 0 && lvarx(makevecteur(a, b, c), x, contextptr) == vecteur(1, x)) {
        ...
        if (k.type == _VECT && !k._VECTptr->empty()) {
          if (k._VECTptr->size() == 2) {
            parameters.push_back(diffeq_constante(int(parameters.size()), contextptr));
            parameters.push_back(diffeq_constante(int(parameters.size()), contextptr));
            result = parameters[parameters.size() - 2] * k._VECTptr->front() +
                     parameters[parameters.size() - 1] * k._VECTptr->back();
            return true;
          }
          if (k._VECTptr->size() == 1) {
            ...
            // a*(z''Y+2z'Y')+b*(z'Y)=0
            // z''*(aY)+z'*(2aY'+bY)=0
            result = desolve_lin1(a * Y, 2 * a * derive(Y, x, contextptr) + b * Y, 0, x, parameters,
                                  step_info, contextptr);
            result = _integrate(makesequence(result, x), contextptr);
            parameters.push_back(diffeq_constante(int(parameters.size()), contextptr));
  ...
  gen desolve_f(const gen & f_orig, const gen & x_orig, const gen & y_orig, int & ordre,
                vecteur & parameters, gen & fres, int step_info, bool & num, GIAC_CONTEXT) {
    num = false;
    ...
    if (x.type != _IDNT) {
      ...
      if (x_orig.type == _VECT)
        return desolve_with_conditions(makevecteur(f_orig, x_orig, y_orig), vx, vy, fres, step_info,
                                       contextptr);
      else
        return desolve_with_conditions(makevecteur(f_orig, makevecteur(x_orig, y_orig)), vx, vy,
                                       fres, step_info, contextptr);
    }
    ...
    for (unsigned i = 0; i < fid.size(); ++i) {
      ...
      if (fidi.type == _IDNT) {
        ...
        if (strlen(ch) == 2 && ch[0] == 'M' && ch[1] >= '0' && ch[1] <= '9')
          return gensizeerr("Home matrix variable " + string(ch) +
                            " not allowed. Store your matrix in a CAS variable first");
      }
    }
#endif
    ...
    if (n == 1) { // 1st order
      ...
      if (x.type == _IDNT && y.type == _IDNT && is_linear_wrt(f, y, fc, fd, contextptr) &&
        ...
        if (is_linear_wrt(fa, t, faa, fab, contextptr) && is_zero(fab) &&
            derive(faa, makevecteur(x, y, t), contextptr) == vecteur(3, 0) &&
            derive(fc, makevecteur(x, y, t), contextptr) == vecteur(3, 0) &&
            derive(fb, makevecteur(x, y), contextptr) == vecteur(2, 0)) {
          ...
          if (is_one(ratnormal(-faa / fc, contextptr))) {
            ...
            sol.push_back(parameters.back() * x +
                          subst(fm, t, parameters.back(), false, contextptr));
        ...
        if (is_zero(derive(makevecteur(fa, fb, fc), x, contextptr)) && !is_zero(fb)) {
          ...
          gen res = desolve_lin1(fa + t, derive(fa, t, contextptr), derive(fb, t, contextptr), t,
                                 parameters, step_info, contextptr);
      ...
      for (; it != itend; ++it) {
        ...
        if (separate_variables(f, x, y, xfact, yfact, step_info, contextptr)) { // y'/yfact=xfact
          ...
          gen pra, prb;
          if (!prv.empty() && prv[0].is_symb_of_sommet(at_ln) &&
              is_linear_wrt(pr, prv[0], pra, prb, contextptr)) {
        ...
        if (pr.is_symb_of_sommet(at_ln))
          ...
          if (is_undef(newsol)) {
            newsol.clear();
            *logptr(contextptr) << "Unable to solve implicit equation " << implicitsol << "=0 in "
                                << y << '\n';
          }
#else
        vecteur newsol;
        ...
        if (is_zero(derive(*it, x, contextptr))) { // x incomplete
          if (step_info)
            gprintf("Order 1 x-incomplete differential equation", vecteur(0), step_info,
                    contextptr);
          if (debug_infolevel)
        ...
        fa = derive(*it, x, contextptr);
        fb = derive(*it, y, contextptr);
        ...
        if (is_zero(derive(fc, x, contextptr)) && is_zero(derive(fc, y, contextptr))) {
          ...
          gen pr =
              integrate_without_lnabs(inv(eff + fc, contextptr), y, contextptr) + parameters.back();
        ...
        if (is_zero(f)) {
          ...
          gen pr =
              parameters.back() *
              _simplify(exp(integrate_without_lnabs(inv(f, contextptr), t, contextptr), contextptr),
                        contextptr);
          ...
          if (!v.empty() && !is_undef(v)) {
            *logptr(contextptr) << "solve(" << pr << "=" << x << "," << t << ") returned " << v
                                << ".\nIf solutions were missed consider paramplot("
                                << makevecteur(pr, t * pr) << "," << t << ")" << '\n';
        ...
        if (is_zero(derive(f, y, contextptr))) {
          ...
          parameters.push_back(diffeq_constante(int(parameters.size()), contextptr));
          F = F +
              integrate_without_lnabs(normal(P * M - derive(F, x, contextptr), contextptr), x,
                                      contextptr) +
              parameters.back();
        ...
        if (is_zero(derive(f, x, contextptr))) {
          ...
          if (step_info)
            gprintf("Order 1 Integrating factor %gen", makevecteur(P), step_info, contextptr);
          F = F +
              integrate_without_lnabs(normal(P * N - derive(F, y, contextptr), contextptr), y,
                                      contextptr) +
              diffeq_constante(int(parameters.size()), contextptr);
          sol = mergevecteur(sol, solve(F, *y._IDNTptr, 3, contextptr));
          continue;
        }
        // Bernoulli?
        ...
        if (is_quadratic_wrt(*it, y, R, Q, P, contextptr)) {
          ...
          vecteur v(makevecteur(1, -normal(Q + derive(R, x, contextptr) / R, contextptr),
                                normal(R * P, contextptr), 0));
    ...
    if (n == 2) {
      ...
      if (soly2.type == _VECT && !is_undef(soly2)) {
        ...
        for (unsigned i = 0; i < soly2v.size(); ++i) {
          gen soly2c = soly2v[i];
          gen a, b, c;
          if (is_quadratic_wrt(soly2c, der1, a, b, c, contextptr) && is_zero(c) &&
              is_zero(derive(a, x, contextptr)) && is_zero(derive(b, y, contextptr))) {
            parameters = paramsave;
            parameters.push_back(diffeq_constante(int(parameters.size()), contextptr));
            gen usolj = parameters.back() *
                        exp(integrate_without_lnabs(b, x, contextptr), contextptr) *
                        exp(integrate_without_lnabs(a, y, contextptr), contextptr);
            gen ysol = desolve(symb_equal(symbolic(at_derive, makesequence(y, x)), usolj), x, y,
                               ordre, parameters, contextptr);
            if (is_undef(ysol))
          ...
          if (is_zero(derive(soly2c, x, contextptr))) { // x-incomplete
            if (step_info)
              gprintf("Order 2 x-incomplete differential equation", vecteur(0), step_info,
                      contextptr);
            // desolve(u'=soly2c/der1,y,u)
            parameters = paramsave;
            gen usol =
                desolve(symb_equal(symbolic(at_derive, makesequence(der1, y)), soly2c / der1), y,
                        der1, ordre, parameters, contextptr);
            if (is_undef(usol))
              return unable_to_solve_diffeq();
            if (usol.type != _VECT)
              usol = vecteur(1, usol);
  ...
  gen _desolve(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.type != _VECT || args.subtype != _SEQ__VECT ||
        (!args._VECTptr->empty() && is_equal(args._VECTptr->back()) &&
         args._VECTptr->back()._SYMBptr->feuille[0].type != _IDNT)) {
      // guess x and y
      vecteur lv(lop(args, at_of));
      vecteur f;
      if (lv.size() >= 1 && lv[0]._SYMBptr->feuille.type == _VECT &&
          (f = *lv[0]._SYMBptr->feuille._VECTptr).size() == 2) {
    ...
    if (s == 2) {
      if ((v[1].type == _SYMB && v[1]._SYMBptr->sommet == at_of &&
           v[1]._SYMBptr->feuille.type == _VECT && v[1]._SYMBptr->feuille._VECTptr->size() == 2))
        return desolve(v[0], (*v[1]._SYMBptr->feuille._VECTptr)[1],
                       (*v[1]._SYMBptr->feuille._VECTptr)[0], ordre, parameters, contextptr);
      return ggbputinlist(desolve(v[0], vx_var, v[1], ordre, parameters, contextptr), contextptr);
    }
    gen f;
    if (s == 4)
      return ggbputinlist(
          desolve_with_conditions(makevecteur(v[0], v[3]), v[1], v[2], f, contextptr), contextptr);
    if (s == 5)
      return ggbputinlist(
          desolve_with_conditions(makevecteur(v[0], v[3], v[4]), v[1], v[2], f, contextptr),
          contextptr);
    if (s != 3)
      return gensizeerr(contextptr);
  ...
  gen desolve(const gen & f_orig, const gen & x_orig, const gen & y_orig, int & ordre,
  ...
  static gen invztranserr(GIAC_CONTEXT) {
    return gensizeerr(gettext("Inverse z-transform of non rational functions not implemented or "
                              "unable to fully factor rational function"));
  }
  ...
  gen invztrans(const gen & f, const gen & x, const gen & s, GIAC_CONTEXT) {
    ...
    if (varxs == 0)
      ...
      vecteur lprime(l);
      if (lprime.front().type != _VECT)
        return gensizeerr(gettext("desolve.cc/invztrans"));
      lprime.front() = cdr_VECT(*(lprime.front()._VECTptr));
      ...
      for (; it != itend; ++it) {
        ...
        if (is_zero(a)) {
          ...
          for (int i = 0; i < mult; ++i) {
            res0 +=
                r2e(vnum[i], lprime, contextptr) *
                symbolic(
                    at_Kronecker,
                    s - i); // symb_when(symb_equal(s,i),1,0) will not be handled correctly by ztrans
          }
          res += res0 / B;
        } else {
    ...
    gen A, B, a, b;
    if (vD.size() == 1 && is_linear_wrt(res, vD.front(), A, B, contextptr) &&
        is_linear_wrt(vD.front()._SYMBptr->feuille, s, a, b, contextptr)) {
  ...
  bool kovacic_iscase3(const gen & cpfr, const gen & x, const vecteur & poles, int dinf,
                       GIAC_CONTEXT) {
    if (dinf < 2)
      return false;
    vecteur alpha, beta;
  ...
  void build_E_families(const gen_map & E, const vecteur & cv, vecteur & family,
                        matrice & families) {
    int i = family.size();
    if (i >= int(cv.size()))
      return;
  ...
  gen strip_abs(const gen & g) {
    ...
    if (g.type == _SYMB) {
      ...
      if (g._SYMBptr->feuille.type == _VECT) {
        ...
        args = strip_abs(g._SYMBptr->feuille);
  ...
  void partialrad(const gen & g, const gen & deg, gen & outside, gen & inside, bool isdenom,
                  GIAC_CONTEXT) {
    gen s;
    if (g.is_integer() &&
        (s = _pow(makesequence(g, _inv(deg, contextptr)), contextptr)).is_integer()) {
      ...
      inside = inside * (isdenom ? _inv(g, contextptr) : g);
  }
  ...
  gen radsimp(const gen & g, GIAC_CONTEXT) {
    ...
    if (isroot(g, deg, contextptr)) {
      ...
      if (!(inum =
                _eval(_pow(makesequence(_numer(inside, contextptr), ideg), contextptr), contextptr))
               .is_symb_of_sommet(at_pow) ||
          inum._SYMBptr->feuille._VECTptr->at(1).is_integer())
        return _collect(outside, contextptr) * inum /
               _pow(makesequence(_collect(_denom(inside, contextptr), contextptr), ideg),
                    contextptr);
      return _collect(outside, contextptr) *
             _pow(makesequence(_collect(inside, contextptr), ideg), contextptr);
    }
  ...
  gen fullsimp(const gen & g, GIAC_CONTEXT) {
    return ratsimp_nonexp(
        _collect(
            radsimp(explnsimp(exp(_ratnormal(g, contextptr), contextptr), contextptr), contextptr),
            contextptr),
        contextptr);
  }
  ...
  gen kovacicsols(const gen & r_orig, const gen & x, const gen & dy_coeff, GIAC_CONTEXT) {
    ...
    gen s = _numer(r, contextptr), t = _denom(r, contextptr), a, b, c, e,
        w = identificateur("omega_");
    ...
    if (kovacic_iscase1(poles, dinf)) {
      ...
      for (const_iterateur it = poles.begin(); it != poles.end(); ++it) {
        ...
        if (order == 1) {
          ...
          b -= expansion_coeff(_cpartfrac(makesequence(sq(sqrt_r[c]), x), contextptr),
                               pow(x - c, -nu - 1), contextptr);
      ...
      for (int i = 0; i < N; ++i) {
        ...
        for (int j = 1; j < np; ++j) {
          fd -= (i & (1 << j)) != 0 ? alpha_plus[cv[j - 1]]
                                    : alpha_minus[cv[j - 1]]; // i & (int)std::pow(2.0,j)
        }
      ...
      for (const_iterateur it = fam.begin(); it != fam.end() && !success; ++it) {
        ...
        for (int i = 0; i < d; ++i)
          P += v[i] * pow(x, i);
        P += pow(x, d);
        e = _numer(_derive(makesequence(P, x, 2), contextptr) +
                       2 * fw * _derive(makesequence(P, x), contextptr) +
                       (_derive(makesequence(fw, x), contextptr) + sq(fw) - r) * P,
                   contextptr);
        ...
        if (lsol.type == _VECT && !lsol._VECTptr->empty()) {
          ...
          solutions.push_back(
              _subst(makesequence(P, v, lsol), contextptr) *
    ...
    if (!success && kovacic_iscase2(poles)) {
      ...
      for (const_iterateur it = fam.begin(); it != fam.end() && !success; ++it) {
        ...
        e = _derive(makesequence(P, x, 3), contextptr) +
            3 * th * _derive(makesequence(P, x, 2), contextptr) +
            (3 * sq(th) + 3 * dth - 4 * r) * _derive(makesequence(P, x), contextptr) +
            (_derive(makesequence(th, x, 2), contextptr) + 3 * th * dth + pow(th, 3) - 4 * r * th -
             2 * _derive(makesequence(r, x), contextptr)) *
                P;
        e = _numer(e, contextptr);
        gen cfs = deg == 0
                      ? undef
                      : _solve(makesequence(_coeff(makesequence(e, x), contextptr), v), contextptr);
        if (deg == 0 ? is_zero(e) : (cfs.type == _VECT && !cfs._VECTptr->empty())) {
          ...
          gen qsol =
              _solve(makesequence(symb_equal(sq(w) - w * ph +
                                                 (_derive(makesequence(ph, x), contextptr) / 2 +
                                                  sq(ph) / 2 - r),
                                             0),
                                  w),
                     contextptr);
          if (qsol.type == _VECT)
            for (const_iterateur jt = qsol._VECTptr->begin(); jt != qsol._VECTptr->end(); ++jt) {
              solutions.push_back(
                  fullsimp(_int(makesequence(*jt - dy_coeff / 2, x), contextptr), contextptr));
              success = true;
            }
        }
      }
    ...
    if (!success && kovacic_iscase3(cpfr, x, poles, dinf, contextptr)) {
      ...
      for (vector<int>::const_iterator nt = nv.begin(); nt != nv.end(); ++nt) {
        ...
        for (const_iterateur it = fam.begin(); it != fam.end(); ++it) {
          ...
          if (P[n].type == _SYMB)
            P[n] = _collect(P[n], contextptr);
          for (int i = n; i-- > 0;) {
            P[i] = -S * _derive(makesequence(P[i + 1], x), contextptr) +
                   ((n - i) * dS - S * th) * P[i + 1] - (n - i) * (i + 1) * sq(S) * r * P[i + 2];
            if (P[i].type == _SYMB)
              P[i] = _collect(P[i], contextptr);
          }
          gen cfs;
          if ((deg == 0 && is_zero(P[0])) ||
              (deg > 0 &&
               (cfs =
                    _solve(makesequence(_coeff(makesequence(P[0], x), contextptr), v), contextptr))
                       .type == _VECT &&
               !cfs._VECTptr->empty())) {
            if (deg > 0) {
              cfs = _subst(makesequence(cfs._VECTptr->front(), v, vecteur(deg, 0)), contextptr);
            ...
            if (!is_zero(dy_coeff)) {
              vecteur C =
                  *_coeff(
                       makesequence(_subst(makesequence(omg, w, w + dy_coeff / 2), contextptr), w),
                       contextptr)
                       ._VECTptr;
              for (int i = 0; i <= n; ++i) {
  ...
  gen _kovacicsols(const gen & g, GIAC_CONTEXT) {
    ...
    if (eq.type == _VECT) {
      ...
      eq = _subst(makesequence(eq,
                               makevecteur(_derive(makesequence(yx, x), contextptr),
                                           _derive(makesequence(yx, x, 2), contextptr)),
                               makevecteur(dy, d2y)),
                  contextptr);
      eq =
          _subst(makesequence(_subst(makesequence(_subst(makesequence(eq, diff2y, d2y), contextptr),
                                                  diffy, dy),
                                     contextptr),
                              yx, y),
                 contextptr);
  ...
  define_unary_function_ptr5(at_kovacicsols, alias_at_kovacicsols, &__kovacicsols, _QUOTE_ARGUMENTS,
                             true)
