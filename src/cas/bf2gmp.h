...
inline void mpz_init(mpz_t & a) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
}
inline void mpz_init2(mpz_t & a, int size) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
}
inline void mpz_init_set(mpz_t & a, const mpz_t & b) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
  bf_set(&a, &b);
}
inline void mpz_init_set_si(mpz_t & a, long b) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
  bf_set_si(&a, b);
}
inline void mpz_init_set_ui(mpz_t & a, unsigned long b) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
  bf_set_ui(&a, b);
}
...
inline int mpz_sgn(const mpz_t & a) {
  if (bf_is_zero(&a))
    return 0;
  return a.sign == 0 ? 1 : -1;
}
inline void mpz_set(mpz_t & a, const mpz_t & b) {
  bf_set(&a, &b);
}
inline void mpz_set_ui(mpz_t & a, unsigned long ui) {
  bf_set_ui(&a, ui);
}
inline void mpz_set_si(mpz_t & a, long ui) {
  bf_set_si(&a, ui);
}
...
inline void mpz_mod(mpz_t & r, const mpz_t & a, const mpz_t & b) {
  bf_rem(&r, &a, &b, BF_PREC_INF, BF_RNDZ, BF_DIVREM_EUCLIDIAN);
}
inline int mpz_add(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  return bf_add(&c, &a, &b, BF_PREC_INF, BF_RNDZ);
}
inline void mpz_add_ui(mpz_t & c, const mpz_t & a, unsigned long B) {
  bf_add_si(&c, &a, B, BF_PREC_INF, BF_RNDZ);
} // ASSUMES that B<1ULL<<63
inline int mpz_sub(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  return bf_sub(&c, &a, &b, BF_PREC_INF, BF_RNDZ);
}
inline void mpz_sub_ui(mpz_t & c, const mpz_t & a, long B) {
  bf_add_si(&c, &a, -B, BF_PREC_INF, BF_RNDZ);
} // ASSUMES that B<1ULL<<63
inline int mpz_mul(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  return bf_mul(&c, &a, &b, BF_PREC_INF, BF_RNDZ);
}
inline void mpz_mul_ui(mpz_t & c, const mpz_t & a, unsigned long B) {
  bf_mul_ui(&c, &a, B, BF_PREC_INF, BF_RNDZ);
}
inline void mpz_mul_si(mpz_t & c, const mpz_t & a, long B) {
  bf_mul_si(&c, &a, B, BF_PREC_INF, BF_RNDZ);
}
inline int mpz_mul_2exp(mpz_t & c, const mpz_t & a, long B) {
  if (&c != &a)
    mpz_set(c, a);
  return bf_mul_2exp(&c, B, BF_PREC_INF, BF_RNDZ);
}
...
inline int mpz_even_p(const mpz_t & p) {
  int i;
  bf_get_int32(&i, &p, BF_GET_INT_MOD);
  return i % 2 == 0;
}
inline int mpz_odd_p(const mpz_t & p) {
  int i;
  bf_get_int32(&i, &p, BF_GET_INT_MOD);
  return i % 2;
}
inline int mpz_pow_ui(mpz_t & c, const mpz_t & a, unsigned B) {
  return bf_pow_ui(&c, &a, B, BF_PREC_INF, BF_RNDZ);
}
inline void mpz_ui_pow_ui(mpz_t & c, unsigned A, unsigned B) {
  mpz_t a;
  mpz_init_set_ui(a, A);
  mpz_pow_ui(c, a, B);
  mpz_clear(a);
}
inline int mpz_ior(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  return bf_logic_or(&c, &a, &b);
}
inline int mpz_xor(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  return bf_logic_xor(&c, &a, &b);
}
inline int mpz_and(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  return bf_logic_and(&c, &a, &b);
}
inline void mpz_addmul(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  mpz_t ab;
  mpz_init(ab);
  mpz_mul(ab, a, b);
  mpz_add(c, c, ab);
...
inline void mpz_addmul_ui(mpz_t & c, const mpz_t & a, unsigned int B) {
  mpz_t ab;
  mpz_init(ab);
  mpz_mul_ui(ab, a, B);
  mpz_add(c, c, ab);
...
inline void mpz_submul(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  mpz_t ab;
  mpz_init(ab);
  mpz_mul(ab, a, b);
  mpz_sub(c, c, ab);
...
inline void mpz_submul_ui(mpz_t & c, const mpz_t & a, unsigned int B) {
  mpz_t ab;
  mpz_init(ab);
  mpz_mul_ui(ab, a, B);
  mpz_sub(c, c, ab);
...
inline int mpz_fdiv_qr(mpz_t & c, mpz_t & d, const mpz_t & a, const mpz_t & b) {
  return bf_divrem(&c, &d, &a, &b, BF_PREC_INF, BF_RNDZ, BF_RNDD);
}
inline int mpz_tdiv_qr(mpz_t & c, mpz_t & d, const mpz_t & a, const mpz_t & b) {
  return bf_divrem(&c, &d, &a, &b, BF_PREC_INF, BF_RNDZ, BF_DIVREM_EUCLIDIAN);
}
inline int mpz_fdiv_r(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  return bf_rem(&c, &a, &b, BF_PREC_INF, BF_RNDZ, BF_RNDD);
}
inline int mpz_tdiv_r(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  return bf_rem(&c, &a, &b, BF_PREC_INF, BF_RNDZ, BF_DIVREM_EUCLIDIAN);
}
inline void mpz_fdiv_r_ui(mpz_t & c, const mpz_t & a, unsigned B) {
  mpz_t b;
  mpz_init_set_ui(b, B);
  mpz_fdiv_r(c, a, b);
  mpz_clear(b);
...
inline void mpz_tdiv_r_ui(mpz_t & c, const mpz_t & a, unsigned B) {
  mpz_t b;
  mpz_init_set_ui(b, B);
  mpz_tdiv_r(c, a, b);
  mpz_clear(b);
}
inline long mpz_fdiv_qr_ui(mpz_t & c, mpz_t & d, const mpz_t & a, unsigned B) {
  mpz_t b;
  mpz_init_set_ui(b, B);
  mpz_fdiv_qr(c, d, a, b);
  mpz_clear(b);
...
inline long mpz_tdiv_qr_ui(mpz_t & c, mpz_t & d, const mpz_t & a, unsigned B) {
  mpz_t b;
  mpz_init_set_ui(b, B);
  mpz_tdiv_qr(c, d, a, b);
  mpz_clear(b);
...
inline void mpz_fdiv_q_ui(mpz_t & c, const mpz_t & a, unsigned B) {
  mpz_t b, r;
  mpz_init_set_ui(b, B);
  mpz_init(r);
  mpz_fdiv_qr(c, r, a, b);
...
inline void mpz_tdiv_q_ui(mpz_t & c, const mpz_t & a, unsigned B) {
  mpz_t b, r;
  mpz_init_set_ui(b, B);
  mpz_init(r);
  mpz_tdiv_qr(c, r, a, b);
...
inline void mpz_fdiv_q(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  mpz_t r;
  mpz_init(r);
  mpz_fdiv_qr(c, r, a, b);
  mpz_clear(r);
}
inline void mpz_tdiv_q(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  mpz_t r;
  mpz_init(r);
  mpz_tdiv_qr(c, r, a, b);
  mpz_clear(r);
}
inline void mpz_divexact(mpz_t & c, const mpz_t & a, const mpz_t & b) {
  mpz_fdiv_q(c, a, b);
}
inline void mpz_divexact_ui(mpz_t & c, const mpz_t & a, unsigned B) {
  mpz_t b, r;
  mpz_init_set_ui(b, B);
  mpz_fdiv_q(c, a, b);
  mpz_clear(b);
}
inline int mpz_divisible_ui_p(const mpz_t & c, unsigned long a) {
  return mpz_fdiv_ui(c, a) == 0;
}
...
inline void mpz_powm_ui(mpz_t & d, const mpz_t & a, unsigned int B, const mpz_t & c) {
  mpz_t b;
  mpz_init_set_ui(b, B);
  mpz_powm(d, a, b, c);
  mpz_clear(b);
...
inline int mpz_sqr(mpz_t & a, const mpz_t & b) {
  return bf_mul(&a, &b, &b, BF_PREC_INF, BF_RNDZ);
}
inline void mpz_neg(mpz_t & a, const mpz_t & b) {
  if (&a != &b)
    mpz_set(a, b);
  bf_neg(&a);
}
...
inline int mpfr_get_prec(const mpfr_t & a) {
  return bf_global_prec;
}
...
// init/clear
inline void mpfr_init(mpfr_t & a) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
}
inline void mpfr_init2(mpfr_t & a, int size) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
} /* precision is ignored */
inline void mpfr_init_set(mpfr_t & a, const mpfr_t & b, bf_flags_t flags) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
  bf_set(&a, &b);
}
inline void mpfr_init_set_si(mpfr_t & a, long B, bf_flags_t flags) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
  bf_set_si(&a, B);
}
inline void mpfr_init_set_d(mpfr_t & a, double d, bf_flags_t flags) {
  bf_init((bf_context_t *)bf_ctx_ptr, &a);
  bf_set_float64(&a, d);
}
inline void mpfr_set(mpfr_t & a, const mpfr_t & b, bf_flags_t) {
  bf_set(&a, &b);
}
inline void mpfr_set_ui(mpfr_t & a, unsigned long ui, bf_flags_t) {
  bf_set_ui(&a, ui);
}
inline void mpfr_set_si(mpfr_t & a, long ui, bf_flags_t) {
  bf_set_si(&a, ui);
}
inline void mpfr_set_d(mpfr_t & a, double d, bf_flags_t) {
  bf_set_float64(&a, d);
}
inline void mpfr_set_z(mpfr_t & rop, const mpz_t & op, mpfr_rnd_t rnd) {
  mpz_set(rop, op);
}
inline double mpfr_get_d(const mpfr_t & a, bf_rnd_t round) {
  double d;
  bf_get_float64(&a, &d, round);
  return d;
}
inline int mpfr_const_pi(mpfr_t & a, bf_flags_t flags) {
  return bf_const_pi(&a, bf_global_prec, flags);
}
inline void mpfr_clear(mpfr_t & f) {
  bf_delete(&f);
}
inline void mpfr_swap(mpfr_t & a, mpfr_t & b) {
  mpz_swap(a, b);
}
// + - * /
...
inline int mpfr_sgn(const mpfr_t & a) {
  return mpz_sgn(a);
}
// mpfr_get_str mpfr_set_str mpfr_strtofr
typedef size_t mp_exp_t;
inline char * mpfr_get_str(char * s, mp_exp_t * expo, int base, int prec, const mpz_t & z,
                           bf_rnd_t round) {
  //int n=z.len*sizeof(limb_t)*8;
  //prec=int(floor(M_LN2/M_LN10*n))+1;
  size_t len;
  char * ptr = bf_ftoa(&len, &z, base, prec, BF_FTOA_FORMAT_FIXED
                       //round
...
inline int mpfr_strtofr(mpfr_t & rop, const char * s, const char ** endptr, int base,
                        mpfr_rnd_t rnd) {
  slimb_t exponent;
...
inline int mpf_set_str(double & x, const char * s, int base) {
  if (base != 10)
    return 1;
  x = strtod(s, 0);
  return 0;
