...
namespace giac {
  ...
  void convolution(const gen & coeff, const gen & arg, const vecteur & w, vecteur & res,
                   GIAC_CONTEXT) {
  ...
  void lin(const gen & e, vecteur & v, bool convert_sqrt, GIAC_CONTEXT) {
    ...
    gen f = _lin(convert_sqrt ? e._SYMBptr->feuille : makesequence(e._SYMBptr->feuille, at_sqrt),
                 contextptr);
    if (s == at_exp) {
      v.push_back(1);
      v.push_back(f);
  ...
  static void tadd(vecteur & res, const gen & coeff, const gen & angle, GIAC_CONTEXT) {
    ...
    if ((newangle.type == _SYMB) &&
        ((newangle._SYMBptr->sommet == at_sin) || (newangle._SYMBptr->sommet == at_cos))) {
  ...
  void tconvolution(const gen & coeff, const gen & arg, const vecteur & w, vecteur & res,
                    GIAC_CONTEXT) {
    gen newcoeff, tmp;
  ...
  void tlin(const gen & e, vecteur & v, GIAC_CONTEXT) {
    ...
    if (s == at_pow && te.type == _VECT && te._VECTptr->size() == 2 &&
        te._VECTptr->back() == plus_one_half)
  ...
  define_unary_function_ptr5(at_lineariser_trigo, alias_at_lineariser_trigo, &__lineariser_trigo, 0,
                             true);
  ...
  */
  ...
  gen prod_expand(const gen & a, const gen & b, GIAC_CONTEXT) {
    ...
    const_iterateur ita = a._SYMBptr->feuille._VECTptr->begin(),
                    itaend = a._SYMBptr->feuille._VECTptr->end();
    const_iterateur itb = b._SYMBptr->feuille._VECTptr->begin(),
                    itbend = b._SYMBptr->feuille._VECTptr->end();
  ...
  static gen prod_expand(const const_iterateur it, const const_iterateur itend, GIAC_CONTEXT) {
    ...
    if (s == 1)
      return *it;
    return _simplifier(prod_expand(prod_expand(it, it + s / 2, contextptr),
                                   prod_expand(it + s / 2, itend, contextptr), contextptr),
                       contextptr);
  }
  ...
  static gen prod_expand_nosimp(const const_iterateur it, const const_iterateur itend,
                                GIAC_CONTEXT) {
    int s = int(itend - it);
    if (s == 0)
      return plus_one;
    if (s == 1)
      return *it;
    return prod_expand(prod_expand_nosimp(it, it + s / 2, contextptr),
                       prod_expand_nosimp(it + s / 2, itend, contextptr), contextptr);
  }
  ...
  static void pow_expand_add_res(vector<gen> & factn, int pos, int sumexpo, const gen & coeff,
                                 const vecteur & w, const gen & p, int k, int n, vecteur & res,
                                 GIAC_CONTEXT) {
    if (sumexpo == k) {
      // End recursion
    ...
    for (int i = k - sumexpo; i >= 0; --i) {
      pow_expand_add_res(factn, pos + 1, sumexpo + i, coeff / factn[i], w,
                         expand(p * pow(w[pos], i), contextptr), k, n, res, contextptr);
    }
  }
  ...
  static gen expand_pow_expand(const gen & e, GIAC_CONTEXT) {
    ...
    if (v[0].is_symb_of_sommet(at_plus) && v[0]._SYMBptr->feuille.type == _VECT &&
        v[1].type == _INT_ && v[1].val >= 0) {
      int k = v[1].val;
      if (!k)
        return plus_one;
      if (k == 1)
  ...
  static gen ln_expand0(const gen & e, GIAC_CONTEXT) {
    ...
    if (e._SYMBptr->sommet == at_prod)
      return _plus(
          apply(e._SYMBptr->feuille, ln_expand0, contextptr),
          contextptr); //symbolic(at_plus,apply(e._SYMBptr->feuille,ln_expand0,contextptr));
    ...
    if (e._SYMBptr->sommet == at_pow) {
      ...
      if (tmp.type == _VECT && tmp._VECTptr->size() == 2) {
        ...
        if (!complex_mode(contextptr) && do_lnabs(contextptr) && expo.type == _INT_ &&
            expo.val % 2 == 0)
  ...
  static gen sin_expand(const gen & e, GIAC_CONTEXT) {
    ...
    if (e._SYMBptr->sommet == at_plus) {
      ...
      return sin_expand(first, contextptr) * cos_expand(last, contextptr) +
             cos_expand(first, contextptr) * sin_expand(last, contextptr);
    }
  ...
  static gen cos_expand(const gen & e, GIAC_CONTEXT) {
    ...
    if (e._SYMBptr->sommet == at_plus) {
      ...
      if (v.size() == 1)
        ...
        first = symbolic(at_plus, v);
      return cos_expand(first, contextptr) * cos_expand(last, contextptr) -
             sin_expand(first, contextptr) * sin_expand(last, contextptr);
    }
    if (e._SYMBptr->sommet == at_neg)
  ...
  static gen tan_expand(const gen & e, GIAC_CONTEXT) {
    ...
    split(e, coeff, arg, contextptr);
    if (!is_one(coeff) && coeff.type == _INT_ && coeff.val < max_texpand_expansion_order) {
      gen g = rdiv(symhorner(tchebycheff(coeff.val, false), cos(arg, contextptr)) *
                       sin(arg, contextptr),
                   symhorner(tchebycheff(coeff.val, true), cos(arg, contextptr)), contextptr);
  ...
  static define_unary_function_eval(__developper_transcendant, &_texpand,
                                    _developper_transcendant_s);
  define_unary_function_ptr5(at_developper_transcendant, alias_at_developper_transcendant,
                             &__developper_transcendant, 0, true);
  ...
  gen factor_ineq(const gen & g_, GIAC_CONTEXT) {
    ...
    if (v.size() == 2) {
      gen res;
      if (sup)
        res = symbolic(at_ou, makesequence(symbolic(at_and, makesequence(res1[0], res1[1])),
                                           symbolic(at_and, makesequence(res2[0], res2[1]))));
      else
        res = symbolic(at_ou, makesequence(symbolic(at_and, makesequence(res1[0], res2[1])),
  ...
  bool and2mat(const vecteur & v, const gen & x, const gen & y, matrice & w, GIAC_CONTEXT) {
    ...
    for (size_t i = 0; i < v.size(); ++i) {
      ...
      if (evalf_double(a, 1, contextptr).type != _DOUBLE_ ||
          ...
          evalf_double(c, 1, contextptr).type != _DOUBLE_)
        return false;
  ...
  vecteur lin_ineq_inter(const matrice & m, double & xmin, double & xmax, double & ymin,
                         double & ymax, GIAC_CONTEXT) {
    size_t N = m.size();
    vecteur res;
    res.reserve((N * (N + 1)) / 2);
    for (size_t i = 0; i < N; ++i) {
  ...
  static void add_autoscale(vecteur & res, double xmin, double xmax, double ymin, double ymax,
                            bool ortho = true) {
    if (ortho) {
      ...
      if (r > 0.25 && r < 4) {
        ...
        if (r > 1)
          res.insert(res.begin(),
                     symb_equal(change_subtype(_GL_X, _INT_PLOT), symb_interval(xmin, xmax)));
        else
          res.insert(res.begin(),
                     symb_equal(change_subtype(_GL_Y, _INT_PLOT), symb_interval(ymin, ymax)));
        return;
      }
    }
    res.insert(res.begin(),
  ...
  static void adjust(double & xmin, double & xmax, double & ymin, double & ymax, vecteur & res,
                     bool zout) {
    if (xmin == 1e307)
      xmin = gnuplot_xmin;
    if (ymin == 1e307)
  ...
  bool lin_ineq_plot(const vecteur & vsymb, const gen & x, const gen & y, const vecteur & attr_,
                     vecteur & res, GIAC_CONTEXT) {
    vecteur attr(attr_);
    fill_attributs(attr);
  ...
  void adjust_xy(const gen & g, double & xmin, double & xmax, double & ymin, double & ymax,
                 GIAC_CONTEXT) {
  ...
  int cut(const gen & eq, const gen & vx, const gen & vy, vecteur & v, double xmin, double xmax,
          double ymin, double ymax, vector<double> & xymin, vector<double> & xymax,
          vector<double> & yxmin, vector<double> & yxmax, GIAC_CONTEXT) {
    if (v.size() < 2)
      return -1;
    gen g = v[0], x, y;
    double xd, yd, lastxd, lastyd;
    reim(g, x, y, contextptr);
    x = evalf_double(x, 1, contextptr);
    y = evalf_double(y, 1, contextptr);
    if (x.type != _DOUBLE_ || y.type != _DOUBLE_)
      return -1;
    lastxd = xd = x._DOUBLE_val;
    lastyd = yd = y._DOUBLE_val;
  ...
  int poly_ineq_inter(vecteur & m, matrice & res, gen & curves, const gen & vx, const gen & vy,
                      double & xmin, double & xmax, double & ymin, double & ymax, vecteur & cache,
                      GIAC_CONTEXT) {
    ...
    for (size_t i = 0; i < N; ++i) {
      ...
      for (size_t j = i + 1; j < N; ++j) {
        ...
        for (int k = 0; k < sol.size(); ++k) {
          ...
          dv1 = subst(dv1, vars, cur, false, contextptr),
          dv2 = subst(dv2, vars, cur, false, contextptr);
    ...
    vector<double> yxmin, yxmax, xymin,
        xymax; // contains intersections of curves with the 4 edges of the domain
    yxmin.push_back(ymin);
    yxmin.push_back(ymax); // y values at x=xmin
    yxmax.push_back(ymin);
    yxmax.push_back(ymax);
    xymin.push_back(xmin);
    xymin.push_back(xmax); // x values at y=ymin
    xymax.push_back(xmin);
    xymax.push_back(xmax);
    for (size_t i = 0; i < N; ++i) {
      gen Ci = C[i];
      if (is_zero(Ci)) {
        ...
        if (it != itend)
          ...
          if (nstep < 8000)
            nstep = 8000;
#endif
          Ci = plotimplicit(m[i], vx, vy, xmin, xmax, ymin, ymax, nstep, 0, epsilon(contextptr),
                            vecteur(1, default_color(contextptr)), false, true, contextptr, 3);
        ...
        for (int j = 0; j < v.size(); ++j) {
          ...
          if (cut(m[i], vx, vy, tmp, xmin, xmax, ymin, ymax, xymin, xymax, yxmin, yxmax,
                  contextptr) == -1)
            return -1;
          reverse(tmp.begin(), tmp.end());
          if (cut(m[i], vx, vy, tmp, xmin, xmax, ymin, ymax, xymin, xymax, yxmin, yxmax,
...
#else
                  gen br =
                      _plotparam(makesequence(eq, t__IDNT_e, t0, t1, (t1 - t0) / 100), contextptr);
      ...
      for (int j = 0; j < Ci._VECTptr->size(); ++j) {
        ...
        if (br.size() >= 2 && is_ok(m, i, vars, br.front(), contextptr) &&
            is_ok(m, i, vars, br.back(), contextptr)) {
  ...
  bool curves2polygons(const vecteur & C, vecteur & res, GIAC_CONTEXT) {
    ...
    for (;;) {
      ...
      if (maxin == -1)
        return true; // all curves were done
      done[J] = true;
  ...
  bool poly_ineq_plot(const vecteur & vsymb, const gen & x, const gen & y, double & xmin,
                      double & xmax, double & ymin, double & ymax, const vecteur & attr_,
                      vecteur & res, GIAC_CONTEXT) {
    vecteur attr(attr_);
    fill_attributs(attr);
    vecteur v(vsymb.size());
    ...
    matrice inter(vsymb.size()),
        curves(
            vsymb
                .size()); // one list of matrices of intersection points for each element of v (and one list of curves)
  ...
  gen expand(const gen & e, GIAC_CONTEXT) {
    ...
    if (e.type == _VECT && e.subtype == _SEQ__VECT && e._VECTptr->size() == 2) {
      ...
      if (last.type == _STRNG || last.type == _FUNC) {
        vector<const unary_function_ptr *> v;
        vector<gen_op_context> w;
        if (contains(last, gen(at_prod)) ||
            (last.type == _STRNG &&
             !strcmp(last._STRNGptr->c_str(), "*"))) { // expand * with no further simplification
          v.push_back(at_prod);
