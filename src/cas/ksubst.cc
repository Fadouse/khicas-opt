...
namespace giac {
  ...
  */
  ...
  gen exp2sincos(const gen & e, GIAC_CONTEXT) {
    ...
    return exp(a, contextptr) * gen(cos(radtoangle(b, contextptr), contextptr),
                                    sin(radtoangle(b, contextptr), contextptr));
  }
  ...
  gen exptopower(const gen & g, GIAC_CONTEXT) {
    if (is_zero(g))
      return 1;
    gen a, ar, ai, b;
    if (has_i(g) && !complex_mode(contextptr) && contains(g, cst_pi) &&
        is_linear_wrt(g, cst_pi, a, b, contextptr) && !is_zero(a)) {
      // exp(pi*a+b)
  ...
  static void subst_vecteur(const vecteur & v, const gen & i, const gen & newi, vecteur & w,
                            bool quotesubst, GIAC_CONTEXT) {
    if (&v == &w) {
  ...
  static bool has_subst(const gen & e, const gen & i, const gen & newi, gen & newe, bool quotesubst,
                        GIAC_CONTEXT);

  static bool has_subst_vect(const gen & e, const gen & i, const gen & newi, gen & newe,
                             bool quotesubst, GIAC_CONTEXT) {
  ...
  static gen subst_integrate(const gen & e, const gen & i, const gen & newi, bool quotesubst,
                             int intg, GIAC_CONTEXT) {
  ...
  static gen subst_derive(const gen & e, const gen & i, const gen & newi, bool quotesubst,
                          GIAC_CONTEXT) {
    ...
    if ((s == 2) && (v[1] == i)) {
      ...
      v[1] = l.front();
      v = subst(v, i, newi, quotesubst, contextptr);
      return rdiv(symbolic(at_derive, gen(v, _SEQ__VECT)), derive(newi, l.front(), contextptr),
                  contextptr);
    }
  ...
  static bool has_subst(const gen & e, const gen & i, const gen & newi, gen & newe, bool quotesubst,
    ...
    case _SYMB:
      ...
      if (e._SYMBptr->sommet == at_pow && i.type == _SYMB && i._SYMBptr->sommet == at_exp &&
          (*(e._SYMBptr->feuille._VECTptr))[1] *
                  ln((*(e._SYMBptr->feuille._VECTptr))[0], contextptr) ==
              i._SYMBptr->feuille) {
        CERR << e << "=" << i << endl;
        newe = newi;
        return true;
      }
      if ((e._SYMBptr->sommet == at_integrate ||
           !strcmp(e._SYMBptr->sommet.ptr()->s, "integration") ||
           !strcmp(e._SYMBptr->sommet.ptr()->s, "int")) &&
          (e._SYMBptr->feuille.type == _VECT) && (i.type == _IDNT)) {
      ...
      if ((e._SYMBptr->sommet == at_derive) && (e._SYMBptr->feuille.type == _VECT) &&
          (i.type == _IDNT)) {
    ...
    case _FRAC:
      ...
      newe = fraction(subst(e._FRACptr->num, i, newi, quotesubst, contextptr),
                      subst(e._FRACptr->den, i, newi, quotesubst, contextptr));
      return true;
    default:
#ifndef NO_STDEXCEPT
  ...
  gen subst(const gen & e, const gen & i, const gen & newi, bool quotesubst, GIAC_CONTEXT) {
    ...
    if (i.type != _IDNT && i.type != _SYMB && i.type != _FUNC)
      *logptr(contextptr) << gettext("Warning, replacing ") << i << gettext(" by ") << newi
                          << gettext(", a substitution variable should perhaps be purged.") << endl;
    gen res;
  ...
  static int multisubst(const gen & e, vecteur & res, const gen & x, const vecteur & xval,
                        GIAC_CONTEXT);

  static int multisubst_frac(const gen & e, vecteur & res, const gen & x, const vecteur & xval,
                             GIAC_CONTEXT) {
    vecteur resn, resd;
  ...
  static int multisubst_vect(const gen & e, vecteur & res, const gen & x, const vecteur & xval,
                             GIAC_CONTEXT) {
    const vecteur & v = *e._VECTptr;
    int s = int(v.size());
  ...
  static int multisubst_symb(const gen & e, vecteur & res, const gen & x, const vecteur & xval,
                             GIAC_CONTEXT) {
    ...
    iterateur it = res.begin(), itend = res.end();
    for (; it != itend; ++it) {
      *it = e._SYMBptr->sommet.quoted() ? symbolic(e._SYMBptr->sommet, *it)
                                        : e._SYMBptr->sommet(*it, contextptr);
    }
    return 1;
  }
  ...
  sparse_poly1 subst(const sparse_poly1 & v, const gen & i, const gen & newi, bool quotesubst,
                     GIAC_CONTEXT) {
    sparse_poly1 p;
  ...
  vecteur sortsubst(const vecteur & v, const vecteur & i, const vecteur & newi, bool quotesubst,
                    GIAC_CONTEXT) {
    if (i.empty())
      return v;
    if (v == i)
  ...
  static void sort2(vecteur & i, vecteur & newi, GIAC_CONTEXT) {
    for (unsigned k = 0; k < i.size(); ++k) {
      if (i[k].type != _IDNT && i[k].type != _SYMB && i[k].type != _FUNC &&
          !is_zero(i[k] - newi[k]))
        *logptr(contextptr) << gettext("Warning, replacing ") << i[k] << gettext(" by ") << newi[k]
                            << gettext(", a substitution variable should perhaps be purged.")
                            << endl;
    }
    int is = int(i.size());
    if (is < 2)
  ...
  vecteur subst(const vecteur & v, const vecteur & i, const vecteur & newi, bool quotesubst,
                GIAC_CONTEXT) {
  ...
  gen sortsubst(const gen & e, const vecteur & i, const vecteur & newi, bool quotesubst,
                GIAC_CONTEXT) {
    if (i.empty())
      return e;
    int pos;
    ...
    case _SYMB:
      ...
      if (!quotesubst && abs_calc_mode(contextptr) != 38 && e._SYMBptr->sommet == at_pow) {
        pos = findpos(i, exp((*(e._SYMBptr->feuille._VECTptr))[1] *
                                 ln((*(e._SYMBptr->feuille._VECTptr))[0], contextptr),
                             contextptr));
        if (pos)
          return newi[pos - 1];
      ...
      if (e._SYMBptr->feuille.type == _VECT) {
        ...
        if (quotesubst ||
            e._SYMBptr->sommet.quoted()
            // || e._SYMBptr->sommet==at_pow
            || (e._SYMBptr->sommet == at_pow && ef.type == _VECT && ef._VECTptr->size() == 2 &&
                ef._VECTptr->front().type > _POLY && !is_zero(ef._VECTptr->front())))
    ...
    case _FRAC:
      ...
      if (pos)
        return newi[pos - 1];
      return fraction(sortsubst(e._FRACptr->num, i, newi, quotesubst, contextptr),
                      sortsubst(e._FRACptr->den, i, newi, quotesubst, contextptr));
    default:
      pos = findpos(i, e);
      if (pos)
        return newi[pos - 1];
  ...
  gen subst(const gen & e, const vector<const unary_function_ptr *> & v,
            const vector<gen_op_context> & w, bool quotesubst, GIAC_CONTEXT) {
    if (v.empty())
      return e;
    if (e.type == _VECT) {
  ...
  gen subst(const gen & e, const unary_function_ptr * v, const gen_op_context * w, bool quotesubst,
            GIAC_CONTEXT, bool recursive_nonrat) {
    ...
    int pos = -1;
    if (recursive_nonrat || ((pos = equalposcomp(analytic_sommets, e._SYMBptr->sommet)) &&
                             (pos <= 4 || (pos == 5 && is_integer(e._SYMBptr->feuille[1])))))
  ...
  gen subst(const gen & e, const vector<const unary_function_ptr *> & v, const vector<gen_op> & w,
            bool quotesubst, GIAC_CONTEXT) {
    if (v.empty())
      return e;
    if (e.type == _VECT) {
  ...
  define_unary_function_ptr5(at_halftan_hyp2exp, alias_at_halftan_hyp2exp, &__halftan_hyp2exp, 0,
                             true);
    ...
    if (p.coord.empty() && cl.type != _POLY) { // search for a non poly in wrt
      ...
      for (i = 0; i < n; ++i) {
        if (v[i].type != _POLY) {
          ...
          if (gg.type == _INT_)
            break;
          if ((gg.type == _FRAC) && (gg._FRACptr->num.type == _INT_) &&
              (gg._FRACptr->den.type == _INT_))
            break;
        }
      }
      if (i == n) {
  ...
  bool is_unit(const gen & g) {
    ...
    if (g.type == _ZINT)
      return (g == 1) || (g == -1);
    if (g.type == _CPLX)
      return (g._CPLXptr->type == _INT_) && (g._CPLXptr->val == 0) &&
             ((g._CPLXptr + 1)->type == _INT_) &&
             (((g._CPLXptr + 1)->val == 1) || ((g._CPLXptr + 1)->val == -1));
    if (g.type == _POLY)
      return (g._POLYptr->coord.size() == 1) && (g._POLYptr->coord.front().index.is_zero()) &&
             is_unit(g._POLYptr->coord.front().value);
    return false;
  }
  ...
  static bool ext_relation(const gen & a0, const vecteur & ext, const vecteur & vars,
                           vecteur & coeffs, GIAC_CONTEXT) {
    if (ext.empty())
      return false;
    gen a = r2e(a0, vars, contextptr);
    gen converted = r2e(ext, vars, contextptr);
    if (converted.type != _VECT || converted._VECTptr->size() != ext.size())
      return false;
    ...
    for (int i = 0; i < s; i++) {
      vecteur tmp(s + 2);
      tmp[i] = 1;
      tmp[s] = _floor(precision * evalf(re(ln(w[i], contextptr), contextptr), 1, contextptr),
                      contextptr);
      tmp[s + 1] = _floor(precision * evalf(im(ln(w[i], contextptr), contextptr), 1, contextptr),
                          contextptr);
      test[i] = tmp;
    }
    matrice S, A, L, O;
  ...
  static void decompose(const gen & a_orig, vecteur & primeargs, vecteur & extargs, int begin,
                        const vecteur & vars, GIAC_CONTEXT) {
    if (is_unit(a_orig))
      return;
  ...
  static gen expanded_ln(const gen & a_orig, const vecteur & primeargs, const vecteur & extargs,
                         const vecteur & lnprimeargs, const vecteur & lnextargs,
                         const vecteur & vars, GIAC_CONTEXT) {
#ifdef TIMEOUT
    control_c();
#endif
    ...
    gg = re(branch_evalf(rdiv(ln(r2e(a_orig, vars, contextptr), contextptr) - res,
                              cst_pi_over_2 * cst_i, contextptr),
                         contextptr),
            contextptr);
    if (gg.type == _DOUBLE_)
  ...
  gen _pow2exp(const gen & e, GIAC_CONTEXT) {
    ...
    if ((e._SYMBptr->sommet == at_pow || e._SYMBptr->sommet == at_surd ||
         e._SYMBptr->sommet == at_NTHROOT) &&
	...
	return gensizeerr(gettext("Conversion of ^ of list/matrices to exp/ln not allowed. For symbolic power of square matrices, try matpow instead of ^"));
      */
      if (v[1].type != _INT_ && v[1].type != _FRAC) {
        gen tmp = -v[0];
        gen tmp1 = (e._SYMBptr->sommet == at_surd || e._SYMBptr->sommet == at_NTHROOT)
                       ? inv(v[1], contextptr)
                       : v[1];
        tmp1 = _pow2exp(tmp1, contextptr);
        if (is_strictly_positive(tmp, contextptr))
          return exp(tmp1 * _pow2exp(ln(tmp, contextptr), contextptr), contextptr) *
                 symb_exp(v[1] * cst_ipi());
        else
  ...
  gen pow2expln(const gen & e, const identificateur & x, GIAC_CONTEXT) {
    ...
    if (e._SYMBptr->feuille.type == _VECT) {
      ...
      if ((e._SYMBptr->sommet == at_pow) &&
          (contains(v[1], x) || (v[1].type != _INT_ && contains(v[0], x))))
  ...
  gen simplifypsi(const gen & g, GIAC_CONTEXT) {
    ...
    for (int i = 1; i < s; ++i) {
      ...
      for (int j = 0; j < i; ++j) {
        ...
        gen difference = current - base;
        difference = simplify(difference, contextptr); // recursive call
        if (difference.type == _VECT && difference._VECTptr->size() == 2 &&
            difference._VECTptr->back() == 0)
  ...
  gen tsimplify_common(const gen & e, GIAC_CONTEXT) {
    ...
    if (s > 1) {
      vecteur argln(s);
      for (int i = 0; i < s; ++i)
        argln[i] = tsimplify_common(
            l[i]._SYMBptr->feuille,
            contextptr); // was tsimplify, but replacing sin/cos in arg of ln is bad
      ...
      if (lp.type != _VECT || le.type != _VECT || lp._VECTptr->size() != primeargs.size() ||
          le._VECTptr->size() != extargs.size())
        return e;
      ...
      for (int i = 0; i < s; ++i) {
        ...
        if (ctrl_c || interrupted)
          return gensizeerr(contextptr);
        fxnd(argln[i], num, den);
        newln[i] = expanded_ln(num, primeargs, extargs, lnprimeargs, lnextargs, vars, contextptr) -
                   expanded_ln(den, primeargs, extargs, lnprimeargs, lnextargs, vars, contextptr);
#ifdef TIMEOUT
        control_c();
#endif
        if (ctrl_c || interrupted)
          return gensizeerr(contextptr);
        gen gg = evalf_double(
            re(branch_evalf(rdiv(l[i] - newln[i], cst_two_pi * cst_i, contextptr), contextptr),
               contextptr),
            0, contextptr);
#ifdef TIMEOUT
        control_c();
    ...
    for (int i = 0; i < s; ++i)
      newl[i] = tsimplify_common(
          l[i]._SYMBptr->feuille,
          contextptr); // was tsimplify, but replacing sin/cos in terms of complex exp is bad
    ...
    for (int i = 0; i < c; ++i) {
      ...
      if (i < n_ln)
        ...
        independant[i] = rewrite_strong_exp(
            r2e(rdiv(independant[i], lcmg, contextptr), vars, contextptr), contextptr);
    }
    m = mtran(mt);
    // compute exp[newl]
    for (int i = 0; i < s; ++i) {
...
#endif
        g = subst(e, sincostan_tab, trig2exp_tab, false, contextptr,
                  false); // g=trig2exp(e,contextptr);
    }
    if (s2 > 1 && angle_radian(contextptr))
      g = subst(g, asinacosatan_tab, atrig2ln_tab, false, contextptr,
  ...
  gen expln2trig(const gen & g, GIAC_CONTEXT) {
    ...
    if (g._SYMBptr->sommet == at_inv) {
      ...
      if (f.type == _SYMB) {
        ...
        if (s == at_pow)
          return pow(expln2trig(inv(ff._VECTptr->front(), contextptr), contextptr),
                     ff._VECTptr->back(), contextptr);
      }
    ...
    if (g._SYMBptr->sommet != at_plus && g._SYMBptr->sommet != at_prod &&
        g._SYMBptr->sommet != at_inv && g._SYMBptr->sommet != at_pow &&
        g._SYMBptr->sommet != at_neg)
    ...
    if (g._SYMBptr->sommet == at_ln) {
      ...
      if (is_zero(gim)) {
        if (complex_mode(contextptr))
          return rdiv(ln(pow(gre, 2), contextptr), plus_two, contextptr) +
                 cst_i * (plus_one - sign(gre, contextptr)) * cst_pi_over_2;
        else
          return ln(gre, contextptr);
      }
      return rdiv(ln(pow(gre, 2) + pow(gim, 2), contextptr), plus_two, contextptr) +
             cst_i * (atan(gim / gre, contextptr) +
                      sign(gim, contextptr) * (plus_one - sign(gre, contextptr)) * cst_pi_over_2);
    }
    ...
    for (unsigned i = 0; i < l.size(); ++i) {
      if (l[i].is_symb_of_sommet(at_exp)) {
        ...
        if (contains(li, cst_pi)) {
          ...
          if (li.type == _FRAC && li._FRACptr->num.type == _INT_ &&
              li._FRACptr->den.type == _INT_) {
  ...
  gen ataninvtoatan(const gen & g, GIAC_CONTEXT) {
    if (g.is_symb_of_sommet(at_inv))
      return cst_pi_over_2 * sign(g._SYMBptr->feuille, contextptr) -
             atan(g._SYMBptr->feuille, contextptr);
    return symb_atan(g);
  }
  ...
  gen lvar_ratnormal(const gen & e, GIAC_CONTEXT) {
    ...
    for (int j = 0; j < v.size(); ++j) {
      ...
      if (w[j]._SYMBptr->feuille.type == _SYMB)
        w[j] = symbolic(w[j]._SYMBptr->sommet,
                        expand(recursive_normal(w[j]._SYMBptr->feuille, contextptr), contextptr));
      else
  ...
  static bool simplify_preflight(const gen & input, bool & large_power) {
    ...
    frame pending[33];
    unsigned used = 1, remaining = 2048;
    pending[0].node = &input;
    pending[0].child = 0;
    pending[0].functions = 0;
    large_power = false;
    while (used) {
      frame & top = pending[used - 1];
      const gen & g = *top.node;
      if (!top.child) {
        if (!remaining--)
          return false;
        if (g.type == _SYMB) {
          const unary_function_ptr & op = g._SYMBptr->sommet;
          if (op != at_plus && op != at_prod && op != at_neg && op != at_inv && op != at_division &&
              op != at_equal) {
            if (++top.functions > 6)
              return false;
          }
      ...
      const gen * child = 0;
      unsigned index = top.child++;
      if (g.type == _SYMB) {
        if (!index)
          child = &g._SYMBptr->feuille;
      } else if (g.type == _VECT) {
        if (index < g._VECTptr->size())
          child = &(*g._VECTptr)[index];
      } else if (g.type == _FRAC) {
        if (index < 2)
          child = index ? &g._FRACptr->den : &g._FRACptr->num;
      } else if (g.type == _CPLX) {
        if (index < 2)
          child = &g._CPLXptr[index];
      }
      if (!child) {
        --used;
        continue;
      ...
      pending[used].node = child;
      pending[used].child = 0;
      pending[used].functions = functions;
      ++used;
    }
    return true;
  }
  ...
  static gen simplify_shallow_leaf(const gen & input, GIAC_CONTEXT) {
    const gen *shells[64], *leaf = &input;
    unsigned count = 0;
    while (count < 64) {
      const gen * next = 0;
      if (leaf->type == _VECT && leaf->_VECTptr->size() == 1)
        next = &leaf->_VECTptr->front();
      else if (leaf->type == _SYMB && leaf->_SYMBptr->feuille.type != _VECT) {
        const unary_function_ptr & op = leaf->_SYMBptr->sommet;
        if (op == at_sin || op == at_cos || op == at_tan || op == at_sinh || op == at_cosh ||
            op == at_tanh || op == at_exp || op == at_ln || op == at_erf || op == at_atan ||
            op == at_asin || op == at_acos || op == at_abs)
    ...
    bool large = false;
    if (!count || !simplify_preflight(*leaf, large) || large)
      return input;
    gen result = simplify(*leaf, contextptr);
    while (count) {
      const gen & shell = *shells[--count];
      if (shell.type == _VECT)
        result = gen(vecteur(1, result), shell.subtype);
      else
        result = symbolic(shell._SYMBptr->sommet, result);
    }
    return result;
  }
  ...
  static unsigned simplify_special_terms(const gen & g, bool & psi, unsigned & budget,
                                         unsigned depth) {
    if (!budget || depth > 32) {
      budget = 0;
      return 257;
    ...
    --budget;
    if (g.is_symb_of_sommet(at_Psi)) {
      psi = true;
      return 1;
    }
    if (g.is_symb_of_sommet(at_atan) || g.is_symb_of_sommet(at_asin) ||
        g.is_symb_of_sommet(at_acos) || g.is_symb_of_sommet(at_abs))
      psi = true;
    if (g.type == _FRAC) {
  ...
  static gen simplify_special_core(const gen & e_orig, GIAC_CONTEXT) {
    ...
    if (e_orig.is_symb_of_sommet(at_integrate) || e_orig.is_symb_of_sommet(at_when) ||
        e_orig.is_symb_of_sommet(at_piecewise))
      return e_orig;
    ...
    if (taille(e_orig, 129) <= 128 && contains(e_orig, *at_ln)) {
      if (contains(e_orig, *at_sqrt) && has_i(e_orig))
        return e_orig;
      vecteur logs = lop(e_orig, at_ln), from, to;
      bool complex_log = false;
      for (unsigned j = 0; j < logs.size(); ++j) {
        const gen & argument = logs[j]._SYMBptr->feuille;
        if (taille(argument, 33) > 32)
          continue;
        if (!is_zero(im(argument, contextptr)))
          complex_log = true;
        if (!argument.is_symb_of_sommet(at_pow) || argument._SYMBptr->feuille.type != _VECT)
          continue;
        const vecteur & p = *argument._SYMBptr->feuille._VECTptr;
        if (p.size() != 2 || p[1] != 2)
          continue;
        gen imaginary = im(p[0], contextptr);
        bool upper = is_strictly_positive(imaginary, contextptr),
             lower = is_strictly_positive(-imaginary, contextptr);
        if (!upper && !lower)
          continue;
        gen real = re(p[0], contextptr);
        gen condition = upper ? symb_superieur_egal(real, 0) : symb_superieur_strict(real, 0);
        gen correction =
            symbolic(at_when, makesequence(condition, 0, (upper ? -2 : 2) * cst_pi * cst_i));
        from.push_back(logs[j]);
        to.push_back(2 * symbolic(at_ln, p[0]) + correction);
      }
      if (complex_log || !from.empty())
        return ratnormal(from.empty() ? e_orig : quotesubst(e_orig, from, to, contextptr),
                         contextptr);
    }
    ...
    if (taille(e_orig, 129) <= 128) {
      ...
      bool logarithmic = true, trig_abs = false;
      for (unsigned j = 0; j < atoms.size(); ++j) {
        const gen & atom = atoms[j];
        if (atom.type == _IDNT)
          continue;
        if (!atom.is_symb_of_sommet(at_ln)) {
          logarithmic = false;
          break;
        }
        const gen & arg = atom._SYMBptr->feuille;
        trig_abs = trig_abs ||
                   (contains(arg, *at_abs) && (contains(arg, *at_sin) || contains(arg, *at_cos)));
      }
      if (logarithmic && trig_abs) {
        ...
        for (unsigned j = 0; j < atoms.size(); ++j) {
          ...
          if (!atom.is_symb_of_sommet(at_ln))
            continue;
          const gen & arg = atom._SYMBptr->feuille;
          vecteur absolute = lop(arg, at_abs);
          gen a, b;
          if (absolute.size() != 1 || !is_linear_wrt(arg, absolute[0], a, b, contextptr) ||
              !is_zero(b))
            continue;
          bool rational = a.type == _INT_ || a.type == _ZINT ||
                          (a.type == _FRAC &&
                           (a._FRACptr->num.type == _INT_ || a._FRACptr->num.type == _ZINT) &&
          ...
          if (!rational || !is_strictly_positive(a, contextptr) ||
              (!trig.is_symb_of_sommet(at_sin) && !trig.is_symb_of_sommet(at_cos)))
            continue;
          gen reduced =
              symbolic(trig._SYMBptr->sommet, ratnormal(trig._SYMBptr->feuille, contextptr));
          from.push_back(atom);
      ...
      if (contains(e_orig, *at_sin) && contains(e_orig, *at_cos)) {
        ...
        for (unsigned j = 0; j < inverses.size(); ++j) {
          ...
          if (!den.is_symb_of_sommet(at_plus))
            continue;
          ...
          for (unsigned k = 0; k < powers.size(); ++k) {
            ...
            if (n.type != _INT_ || n.val < 3 ||
                (!base.is_symb_of_sommet(at_sin) && !base.is_symb_of_sommet(at_cos)))
              continue;
    ...
    if (e_orig.is_symb_of_sommet(at_prod) && taille(e_orig, 97) <= 96) {
      ...
      if (logs.size() == 1) {
        ...
        if (variables > 1 && radical)
          return ratnormal(e_orig, contextptr);
      }
    }
    ...
    if (taille(e_orig, 257) <= 256) {
      ...
      if (trig.size() == 2 &&
          ...
          trig[0]._SYMBptr->feuille == trig[1]._SYMBptr->feuille &&
          !lidnt(trig[0]._SYMBptr->feuille).empty()) {
        gen a, b, c, d;
        if (is_linear_wrt(e_orig, trig[0], a, b, contextptr) && !contains(a, trig[1]) &&
            is_linear_wrt(b, trig[1], c, d, contextptr))
          return ratnormal(e_orig, contextptr);
      }
    }
    if (taille(e_orig, 129) <= 128) {
      ...
      if (angles.size() > 1 && !lidnt(angles).empty()) {
        ...
        if (angles.size() == 2) {
          ...
          reciprocal = is_zero(im(a, contextptr)) && is_zero(im(b, contextptr)) &&
                       is_zero(ratnormal(a * b - 1, contextptr));
        }
    ...
    for (unsigned j = 0; j < powers.size(); ++j) {
      if (!powers[j].is_symb_of_sommet(at_pow))
        continue;
      ...
      if (!deep) {
        ...
        for (unsigned k = 0; k < inner.size(); ++k) {
          ...
          if (p.type == _VECT && p._VECTptr->size() == 2 && p._VECTptr->back().type == _FRAC) {
            deep = true;
            break;
          }
        }
    ...
    e = quotesubst(e, vabs, vabs2, contextptr);
    e = quotesubst(e, vabs, vabs2,
                   contextptr); // second replacement because vabs2 might contain expression in vabs
    ...
    for (unsigned int i = 0; i < vabs2.size(); ++i) {
      if (vabs2[i].is_symb_of_sommet(at_abs) || vabs2[i].is_symb_of_sommet(at_pow) ||
          vabs2[i].is_symb_of_sommet(at_integrate)) {
    ...
    if (s1 > 1) {
      ...
      if (!existing_tangents.empty()) {
        ...
        vecteur tangents = lop(e2, at_tan);
        bool same_domain = true;
        for (unsigned j = 0; j < tangents.size(); ++j)
          if (!equalposcomp(existing_tangents, tangents[j])) {
            same_domain = false;
            break;
          }
        if (same_domain && int(loptab(e2, sincostan_tab).size()) < s1)
      ...
      if (!existing_tangents.empty()) {
        ...
        if (e2.type == _VECT) {
          ...
          for (unsigned j = 0; j < new_tangents.size(); ++j)
            if (!equalposcomp(existing_tangents, new_tangents[j])) {
              same_domain = false;
              break;
            }
...
#ifdef FXCG
      if (s1 != 2 || v1.size() != 2 || !v1[0].is_symb_of_sommet(at_tan) ||
          !v1[1].is_symb_of_sommet(at_tan))
#endif
    ...
    if (s1) {
      gen g1(normal(trigcos(reg, contextptr), contextptr),
             normal(trigcos(img, contextptr), contextptr));
      gen g2(normal(trigsin(reg, contextptr), contextptr),
             normal(trigsin(img, contextptr), contextptr));
    ...
    g = gen(normal(trigcos(reg, contextptr), contextptr),
            normal(trigcos(img, contextptr), contextptr));
    g = quotesubst(g, vabs2, vabs, contextptr);
    return g;
  }
  ...
  gen simplify(const gen & e_orig, GIAC_CONTEXT) {
    ...
    if (!simplify_preflight(e_orig, large_power))
      return simplify_shallow_leaf(e_orig, contextptr);
    if (large_power)
      return e_orig;
    ...
    if (e_orig.is_symb_of_sommet(at_prod) && e_orig._SYMBptr->feuille.type == _VECT &&
      ...
      for (unsigned i = 0; i < factors.size() && simple; ++i) {
        ...
        if (factor.type == _INT_ || factor.type == _ZINT || factor.type == _FRAC ||
            factor.type == _IDNT || factor.is_symb_of_sommet(at_sin) ||
            factor.is_symb_of_sommet(at_cos))
          continue;
        if (factor.is_symb_of_sommet(at_inv)) {
          const gen & den = factor._SYMBptr->feuille;
          if (den.type == _INT_ || den.type == _ZINT || den.type == _FRAC)
            continue;
        }
        if (!factor.is_symb_of_sommet(at_pow) || factor._SYMBptr->feuille.type != _VECT) {
          simple = false;
          break;
        }
        const vecteur & power = *factor._SYMBptr->feuille._VECTptr;
        if (power.size() != 2 || power[1].type != _INT_ || power[1].val < 1 ||
            (!power[0].is_symb_of_sommet(at_sin) && !power[0].is_symb_of_sommet(at_cos))) {
          simple = false;
          break;
        }
    ...
    if (terms > 64 && (lidnt(e_orig).size() > 1 || terms > 256))
      return e_orig;
    if (terms > 64) {
      if (contains(e_orig, *at_sqrt))
        return e_orig;
    ...
    if (terms > 64 && (contains(e_orig, *at_sin) || contains(e_orig, *at_cos)))
      return e_orig;
    ...
    if (contains(e_orig, *at_Li2) || contains(e_orig, *at_when) || contains(e_orig, *at_piecewise))
      return terms > 64 ? e_orig : ratnormal(e_orig, contextptr);
      ...
      if (constant_size > 128 && constant_size < 513) {
        ...
        for (unsigned j = 0; j < atoms.size(); ++j) {
          ...
          if (argument.is_symb_of_sommet(at_division) && argument._SYMBptr->feuille.type == _VECT) {
            ...
            if (v.size() != 2)
              continue;
            factors.push_back(symbolic(at_inv, v[1]));
            argument = gen(v[0]);
          }
          if (argument.is_symb_of_sommet(at_neg)) {
            factors.push_back(-1);
            argument = gen(argument._SYMBptr->feuille);
          }
          if (argument.is_symb_of_sommet(at_prod) && argument._SYMBptr->feuille.type == _VECT) {
            ...
            for (unsigned k = 0; k < v.size(); ++k)
              factors.push_back(v[k]);
          } else
            factors.push_back(argument);
          gen q = 1;
          unsigned pi_count = 0;
          bool valid = true;
          for (unsigned k = 0; k < factors.size(); ++k) {
            ...
            if (inverse)
              factor = gen(factor._SYMBptr->feuille);
            gen numerator = factor, denominator = 1;
            if (factor.type == _FRAC) {
              numerator = factor._FRACptr->num;
              denominator = factor._FRACptr->den;
            }
            if (numerator.type != _INT_ || denominator.type != _INT_ || numerator.val < -4096 ||
                numerator.val > 4096 || denominator.val < -4096 || denominator.val > 4096 ||
                !denominator.val || (inverse && !numerator.val)) {
              valid = false;
              break;
            }
            q = q * rdiv(inverse ? denominator : numerator, inverse ? numerator : denominator,
                         contextptr);
          }
          if (!valid || pi_count != 1 || q.type != _FRAC || q._FRACptr->num.type != _INT_ ||
              q._FRACptr->den.type != _INT_ || q._FRACptr->den.val <= 6 ||
              q._FRACptr->den.val > 4096)
            continue;
    ...
    if (!psi)
      return simplify_special_core(e_orig, contextptr);
    ...
    for (unsigned i = 0; i < root_powers.size(); ++i) {
      ...
      if (!base.is_symb_of_sommet(at_abs) || exponent.type != _FRAC ||
          !is_one(exponent._FRACptr->num) || exponent._FRACptr->den.type != _INT_)
        continue;
      int n = exponent._FRACptr->den.val;
      if (n < 3 || n > 9 || !(n % 2))
        continue;
      const gen & argument = base._SYMBptr->feuille;
      if (taille(argument, 129) > 128 || !is_zero(im(argument, contextptr)))
        continue;
      real_roots.push_back(root_powers[i]);
      for (unsigned j = 0; j < root_signs.size(); ++j)
        if (root_signs[j]._SYMBptr->feuille == argument && !equalposcomp(real_roots, root_signs[j]))
          real_roots.push_back(root_signs[j]);
      if (real_roots.size() > 32)
        return e_orig;
    }
    if (!real_roots.empty()) {
      unsigned k = 0;
    ...
    for (unsigned i = 0; i < psi_atoms.size(); ++i) {
      ...
      if (a.type == _FRAC)
        rational = (a._FRACptr->num.type == _INT_ || a._FRACptr->num.type == _ZINT) &&
                   (a._FRACptr->den.type == _INT_ || a._FRACptr->den.type == _ZINT) &&
                   !is_zero(a._FRACptr->den);
      if (rational && is_strictly_positive(a, contextptr))
        special.push_back(psi_atoms[i]);
    }
    ...
    vecteur atoms = lvar(e_orig);
    vecteur powers = lop(e_orig, at_pow);
    bool angle_power = false;
    for (unsigned i = 0; i < powers.size() && !angle_power; ++i) {
      ...
      if (exponent.type == _INT_ && exponent.val >= 2 &&
          (base.is_symb_of_sommet(at_atan) || base.is_symb_of_sommet(at_asin) ||
           base.is_symb_of_sommet(at_acos)) &&
          equalposcomp(atoms, base))
        angle_power = true;
    }
    ...
    if (!angle_power) {
      ...
      for (unsigned i = 0; i < products.size() && !angle_power; ++i) {
        ...
        for (unsigned j = 0; j < v.size() && !angle_power; ++j) {
          if (!(v[j].is_symb_of_sommet(at_atan) || v[j].is_symb_of_sommet(at_asin) ||
                v[j].is_symb_of_sommet(at_acos)) ||
              !equalposcomp(atoms, v[j]))
            continue;
    ...
    for (unsigned i = 0; i < atoms.size(); ++i) {
      ...
      if (!angle_power ||
          !(atom.is_symb_of_sommet(at_atan) || atom.is_symb_of_sommet(at_asin) ||
            atom.is_symb_of_sommet(at_acos)) ||
          taille(atom, 65) > 64)
        continue;
      vecteur names = lidnt(atom);
      bool closed = true;
  ...
  static bool simplify_root_domain(const gen & args) {
    ...
    if (args.is_symb_of_sommet(at_prod) && args._SYMBptr->feuille.type == _VECT) {
      ...
      for (unsigned j = 0; j < f.size(); ++j) {
        if (!f[j].is_symb_of_sommet(at_inv))
          continue;
        const gen & den = f[j]._SYMBptr->feuille;
        if (den.type == _INT_ || den.type == _ZINT)
          continue;
    ...
    if (has_op(args, *at_sqrt) || has_op(args, *at_pow)) {
      ...
      for (unsigned j = 0; j < quotients.size(); ++j) {
        ...
        if (quotients[j].is_symb_of_sommet(at_division)) {
          if (den.type != _VECT || den._VECTptr->size() != 2)
            continue;
          den = gen(den[1]);
        }
        if (!has_op(den, *at_plus))
          continue;
        if (has_op(den, *at_sqrt))
          return true;
        vecteur powers = lop(den, at_pow);
        for (unsigned k = 0; k < powers.size(); ++k) {
          const gen & f = powers[k]._SYMBptr->feuille;
    ...
    if (!has_i(args) && has_op(args, *at_ln)) {
      vecteur logs = lop(args, at_ln);
      bool square = false, quotient = false;
      for (unsigned j = 0; j < logs.size() && logs.size() > 1; ++j) {
        ...
        if (u.is_symb_of_sommet(at_pow) && u._SYMBptr->feuille.type == _VECT &&
            u._SYMBptr->feuille._VECTptr->size() == 2) {
          const gen & n = u._SYMBptr->feuille[1];
          if (n.type == _INT_ && n.val > 0 && n.val % 2 == 0)
            square = true;
        }
      }
      if (square && quotient)
    ...
    if (has_op(args, *at_atan) && (has_op(args, *at_sin) || has_op(args, *at_cos)))
      return true;
    ...
    if (has_op(args, *at_surd) || has_op(args, *at_NTHROOT)) {
      ...
      for (unsigned j = 0; j < roots.size(); ++j) {
        ...
        if (f.type == _VECT && f._VECTptr->size() == 2) {
          ...
          if ((n.type == _INT_ && n.val > 1 && n.val % 2) || has_op(u, *at_inv) ||
              has_op(u, *at_division))
            return true;
        }
      }
    }
  ...
  static bool simplify_atan_addition(const gen & g, gen & result, GIAC_CONTEXT) {
    ...
    for (unsigned j = 0; j < 3; ++j) {
      gen c, next;
      if (!is_linear_wrt(remaining, angles[j], c, next, contextptr) || !equation_rational(c) ||
          is_zero(c))
        return false;
      coefficients.push_back(c);
      remaining = next;
    }
    if (!equation_rational(remaining))
      return false;
    for (unsigned j = 0; j < 3; ++j) {
      unsigned k = (j + 1) % 3, l = (j + 2) % 3;
      if (coefficients[k] != coefficients[l] || coefficients[j] != -coefficients[k])
        continue;
      gen u = angles[k]._SYMBptr->feuille, v = angles[l]._SYMBptr->feuille;
      unsigned budget = 128;
      equation_polynomial_budget ub, vb, nb, db;
      if (!equation_polynomial_bound(u, budget, 0, ub) ||
          !equation_polynomial_bound(v, budget, 0, vb) || ub.degree > 4 || vb.degree > 4 ||
          !is_zero(im(u, contextptr)) || !is_zero(im(v, contextptr)))
        continue;
      const gen & argument = angles[j]._SYMBptr->feuille;
      gen N = 1, D = 1;
      vecteur factors =
          argument.is_symb_of_sommet(at_prod) && argument._SYMBptr->feuille.type == _VECT
              ? *argument._SYMBptr->feuille._VECTptr
              : makevecteur(argument);
      for (unsigned f = 0; f < factors.size(); ++f) {
        if (factors[f].is_symb_of_sommet(at_inv))
          D = D * factors[f]._SYMBptr->feuille;
        else if (factors[f].is_symb_of_sommet(at_division) &&
                 factors[f]._SYMBptr->feuille.type == _VECT &&
                 factors[f]._SYMBptr->feuille._VECTptr->size() == 2) {
          N = N * factors[f]._SYMBptr->feuille[0];
          D = D * factors[f]._SYMBptr->feuille[1];
        } else
          N = N * factors[f];
      }
      if (!equation_polynomial_bound(N, budget, 0, nb) ||
          !equation_polynomial_bound(D, budget, 0, db) || nb.degree > 8 || db.degree > 8)
        continue;
      ...
      if (!equation_rational(scale) || is_zero(scale) ||
          !is_zero(ratnormal(N - scale * (u + v), contextptr)))
        continue;
      gen value = symbolic(at_when, makesequence(symbolic(at_superieur_strict, makesequence(d, 0)),
                                                 0, cst_pi * symbolic(at_sign, u + v)));
      result = symbolic(at_when,
                        makesequence(symb_equal(d, 0), undef, remaining + coefficients[k] * value));
      return true;
    }
    return false;
  }
  ...
  static bool simplify_minmax_clamp(const gen & g, gen & result, GIAC_CONTEXT) {
    if (complex_mode(contextptr) || complex_variables(contextptr) ||
        !g.is_symb_of_sommet(at_plus) || taille(g, 65) > 64)
      return false;
    const gen & terms = g._SYMBptr->feuille;
    if (terms.type != _VECT || terms._VECTptr->size() != 2)
      return false;
    gen left = terms[0], right = terms[1], cl = equation_numeric_factor(left),
        cr = equation_numeric_factor(right);
    if (left.is_symb_of_sommet(at_min)) {
      gen tmp = left;
      left = right;
      right = tmp;
      tmp = cl;
    ...
    if (!left.is_symb_of_sommet(at_max) || !right.is_symb_of_sommet(at_min) ||
        !equation_rational(cl) || is_zero(cl) || cl != -cr)
      return false;
    const gen &lv = left._SYMBptr->feuille, &rv = right._SYMBptr->feuille;
    if (lv.type != _VECT || rv.type != _VECT || lv._VECTptr->size() != 2 ||
        rv._VECTptr->size() != 2)
      return false;
    for (unsigned j = 0; j < 2; ++j)
      for (unsigned k = 0; k < 2; ++k) {
        if (!lv[j].is_symb_of_sommet(at_min) || !rv[k].is_symb_of_sommet(at_max))
          continue;
        const gen &inner = lv[j]._SYMBptr->feuille, &other = rv[k]._SYMBptr->feuille;
        if (inner.type != _VECT || other.type != _VECT || inner._VECTptr->size() != 2 ||
            other._VECTptr->size() != 2)
          continue;
        gen a = lv[1 - j], b = rv[1 - k];
        for (unsigned l = 0; l < 2; ++l)
          for (unsigned m = 0; m < 2; ++m) {
            if (inner[l] != b || other[m] != a || inner[1 - l] != other[1 - m])
              continue;
            unsigned budget = 96;
            equation_polynomial_budget bound;
            gen vars = makevecteur(a, b, inner[1 - l]);
            bool real = true;
            for (unsigned n = 0; n < 3; ++n)
              if (!equation_polynomial_bound(vars[n], budget, 0, bound) || bound.degree > 8 ||
                  bound.terms > 32 || !is_zero(im(vars[n], contextptr)))
                real = false;
            if (!real)
              continue;
            // For a<=b both clamp orders agree. For a>b they are the
            // constants a and b, regardless of the middle argument.
            result = cl * (a - b + symbolic(at_abs, a - b)) / 2;
            return true;
          }
      }
    return false;
  ...
  static bool simplify_conjugate_roots(const gen & g, gen & result, GIAC_CONTEXT) {
    if (complex_mode(contextptr) || complex_variables(contextptr) ||
        !g.is_symb_of_sommet(at_plus) || taille(g, 97) > 96)
      return false;
    const gen & f = g._SYMBptr->feuille;
    if (f.type != _VECT || f._VECTptr->size() != 2)
      return false;
    ...
    for (unsigned j = 0; j < 2; ++j) {
      ...
      if (root.is_symb_of_sommet(at_sqrt))
        radicands[j] = root._SYMBptr->feuille;
        ...
        radicands[j] = root._SYMBptr->feuille[0];
      else
        return false;
    }
    ...
    for (unsigned j = 0; j < roots.size(); ++j) {
      ...
      unsigned budget = 128;
      equation_polynomial_budget wb, ub, vb;
      if (!equation_polynomial_bound(W, budget, 0, wb) || wb.degree > 4 || wb.terms > 16)
        continue;
      gen B, U;
      if (!is_linear_wrt(radicands[0], roots[j], B, U, contextptr) || !equation_rational(B) ||
          is_zero(B) || !equation_polynomial_bound(U, budget, 0, ub) || ub.degree > 2 ||
          ub.terms > 8 || !is_zero(im(U, contextptr)) || !is_zero(im(W, contextptr)) ||
          !is_zero(ratnormal(radicands[1] - U + B * roots[j], contextptr)))
        continue;
      gen V = ratnormal(U * U - B * B * W, contextptr);
      if (!equation_polynomial_bound(V, budget, 0, vb) || vb.terms > 16)
        continue;
      ...
      bool square = false;
      if (V.is_symb_of_sommet(at_pow) && V._SYMBptr->feuille.type == _VECT &&
          V._SYMBptr->feuille._VECTptr->size() == 2) {
        const gen & v = V._SYMBptr->feuille;
        if (v[1].type == _INT_ && v[1].val > 0 && v[1].val % 2 == 0) {
          magnitude = symbolic(at_abs, pow(v[0], v[1].val / 2, contextptr));
          square = true;
        }
      }
      gen value = symbolic(at_sqrt, 2 * (U + magnitude));
      value =
          symbolic(at_when, makesequence(symbolic(at_inferieur_strict, makesequence(U, magnitude)),
                                         undef, value));
      result = square ? value
                      : gen(symbolic(at_when,
                                     makesequence(symbolic(at_inferieur_strict, makesequence(V, 0)),
                                                  undef, value)));
      return true;
    }
    return false;
  ...
  gen _simplify(const gen & args, GIAC_CONTEXT) {
    ...
    if (args.is_symb_of_sommet(at_when) || args.is_symb_of_sommet(at_piecewise))
      return args;
    gen atan_addition;
    if (simplify_minmax_clamp(args, atan_addition, contextptr) ||
        simplify_conjugate_roots(args, atan_addition, contextptr) ||
        simplify_atan_addition(args, atan_addition, contextptr))
      return atan_addition;
    if (simplify_root_domain(args))
      return args;
    ...
    if (taille(args, 129) <= 128 && contains(args, *at_ln) && contains(args, *at_abs) &&
      ...
      return ratnormal(args, contextptr);
    bool large_power = false;
    if (!simplify_preflight(args, large_power))
      return simplify_shallow_leaf(args, contextptr);
    if (large_power)
      return args;
    gen var, res;
    if (is_algebraic_program(args, var, res))
    ...
    if (args.type == _VECT) {
      ...
      int vs = int(v.size());
      if ((vs == 2 || vs == 3) && args.subtype == _SEQ__VECT && args[1].type == _VECT &&
          !ckmatrix(args) && !ckmatrix(args._VECTptr->back())) {
    ...
    if (is_equal(args)) {
      if (equation_primitive(args, res, contextptr))
        return res;
  ...
  gen _trigsin(const gen & args, GIAC_CONTEXT) {
    ...
    gen g = ratnormal(_tan2sincos(args, contextptr), contextptr);
    g = trigsin(g, contextptr);
    g = recursive_ratnormal(
        g,
        contextptr); //normal(g,contextptr); // recursive_ratnormal(g,contextptr); // should be normal
  ...
  gen tcollect(const gen & args, bool output_cos, GIAC_CONTEXT) {
    ...
    for (int i = 1; i < s; i += 2) {
      ...
      if (j >= s)
        ...
        if (output_cos) {
          gen angle(normal(
              arg(gen(coscoeff, sincoeff), contextptr),
              contextptr)); // gen angle=normal(arg(coscoeff+cst_i*sincoeff,contextptr),contextptr);
          res = res + newcoeff * symb_cos(argu - angle);
        } else {
          gen angle(normal(
              arg(gen(sincoeff, coscoeff), contextptr),
              contextptr)); //gen angle=normal(arg(sincoeff+cst_i*coscoeff,contextptr),contextptr);
  ...
  define_unary_function_ptr5(at_rassembler_trigo, alias_at_rassembler_trigo, &__rassembler_trigo, 0,
                             true);
  ...
  static vecteur inlncollect(const gen & args, GIAC_CONTEXT) {
    ...
    if (u == at_prod) {
      if (g.type != _VECT)
        return inlncollect(g, contextptr);
      // is there a ln in g?
      const_iterateur it = g._VECTptr->begin(), it0 = g._VECTptr->begin(),
                      itend = g._VECTptr->end();
      for (; it != itend; ++it) {
    ...
    return makevecteur((u)(_lncollect(g, contextptr), contextptr), symb_exp(1));
  }
  ...
  define_unary_function_ptr5(at_mult_conjugate, alias_at_mult_conjugate, &__mult_conjugate, 0,
                             true);
  ...
  static define_unary_function_eval_quoted(__mult_c_conjugate, &_mult_c_conjugate,
                                           _mult_c_conjugate_s);
  define_unary_function_ptr5(at_mult_c_conjugate, alias_at_mult_c_conjugate, &__mult_c_conjugate,
                             _QUOTE_ARGUMENTS, true);

  static const char _multiplier_conjugue_s[] = "multiplier_conjugue";
  static define_unary_function_eval(__multiplier_conjugue, &_mult_conjugate,
                                    _multiplier_conjugue_s);
  define_unary_function_ptr5(at_multiplier_conjugue, alias_at_multiplier_conjugue,
                             &__multiplier_conjugue, 0, true);

  static const char _multiplier_conjugue_complexe_s[] = "multiplier_conjugue_complexe";
  static define_unary_function_eval(__multiplier_conjugue_complexe, &_mult_c_conjugate,
                                    _multiplier_conjugue_complexe_s);
  define_unary_function_ptr5(at_multiplier_conjugue_complexe, alias_at_multiplier_conjugue_complexe,
                             &__multiplier_conjugue_complexe, 0, true);
  ...
  define_unary_function_ptr5(at_rectangular2polar, alias_at_rectangular2polar, &__rectangular2polar,
                             0, true);
  ...
  define_unary_function_ptr5(at_polar2rectangular, alias_at_polar2rectangular, &__polar2rectangular,
                             0, true);
  ...
  static define_unary_function_eval(__rectangular2spherical, &_rectangular2spherical,
                                    _rectangular2spherical_s);
  define_unary_function_ptr5(at_rectangular2spherical, alias_at_rectangular2spherical,
                             &__rectangular2spherical, 0, true);
  ...
  static gen spherical2rectangular(const gen & g, const context * contextptr) {
    ...
    return makevecteur(rhocos * cos(phi, contextptr), rhocos * sin(phi, contextptr),
                       rho * sin(theta, contextptr));
  }
  ...
  static define_unary_function_eval(__spherical2rectangular, &_spherical2rectangular,
                                    _spherical2rectangular_s);
  define_unary_function_ptr5(at_spherical2rectangular, alias_at_spherical2rectangular,
                             &__spherical2rectangular, 0, true);
