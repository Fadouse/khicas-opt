...
namespace giac {
  ...
  bool has_improved_latex_export(const gen & g, std::string & s, bool override_texmacs,
                                 GIAC_CONTEXT);
  ...
  bool autoscaleg(const gen & g, std::vector<double> & vx, std::vector<double> & vy,
                  std::vector<double> & vz, GIAC_CONTEXT);
  // read inside g for gl_x=xmin..xmax, gl_y= gl_z options and overwrite window limits, return true if axes should be drawn
  bool overwrite_viewbox(const gen & g, double & window_xmin, double & window_xmax,
                         double & window_ymin, double & window_ymax, double & window_zmin,
                         double & window_zmax);
  ...
  int graph2tex(FILE * file, const vecteur & v, double X1, double X2, double Y1, double Y2,
                double xunit, double yunit, const char * filename, bool logo, GIAC_CONTEXT);
  int graph2tex(FILE * file, const vecteur & v, double X1, double X2, double Y1, double Y2,
                double Unit, const char * filename, bool logo, GIAC_CONTEXT);
  int graph2tex(const std::string & s, const vecteur & v, double X1, double X2, double Y1,
                double Y2, double xunit, double yunit, bool logo, GIAC_CONTEXT);
  int graph2tex(const std::string & s, const vecteur & v, double X1, double X2, double Y1,
                double Y2, double Unit, bool logo, GIAC_CONTEXT);
