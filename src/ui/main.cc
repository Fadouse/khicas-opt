...
int select_interpreter() {
  const char * choix[] = {
      "Xcas interpreter",    "XcasPy ^=**", "XcasPy ^=xor", "MicroPython 1.12", 0,
      "Javascript (QuickJS)"};
  return select_item(choix, "Syntax", false);
}
...
void c_draw_filled_polygon(int * x, int * y, int n, int xmin, int xmax, int ymin, int ymax,
                           int color) {
  giac::freeze = true;
  vector<vector<int>> v(n);
  c_convert(x, y, v);
...
void c_draw_filled_arc(int x, int y, int rx, int ry, int theta1_deg, int theta2_deg, int color,
...
const char * gettext(const char * s) {
  ...
  char4 s4 = {s};
  std::pair<char4 * const, char4 * const> pp =
      equal_range(aspen_giac_translations, aspen_giac_translations + aspen_giac_records, s4, tri2);
  if (pp.first != pp.second && pp.second != aspen_giac_translations + aspen_giac_records &&
      (*pp.first)[aspenlang]) {
...
#ifdef NSPIRE_NEWLIB
      DefineStatusMessage(
          (char *)((lang == 1) ? "Ecran fige. Taper esc" : "Screen frozen. Press esc."), 1, 0, 0);
#else
      DefineStatusMessage(
          (char *)((lang == 1) ? "Ecran fige. Taper EXIT" : "Screen frozen. Press EXIT."), 1, 0, 0);
#endif
      DisplayStatusArea();
      int key;
      ck_getkey(&key);
...
bool shiftstate = false, alphastate = false, alphalock = false, alphamaj = true,
     oldshiftstate = false, oldalphastate = false;
...
int in_ckgetkey(int * keyptr, int waitforkey, const char * menu, const char * shiftmenu,
                const char * alphamenu, int menucolorbg) {
  bool is_emulator = *(volatile uint32_t *)0xff000044 == 0x00000000;
  ...
  while (1) {
    ...
    int ret = GetKeyWait_OS(&col, &row, waitforkey ? 2 : 1, timeout_delay /*timeout_period*/,
                            1 /* 0: handle menu key*/, &keycode);
...
int chk_ram2M(const char * filename, int reload) {
  ...
  if (size > 3 * 1024 * 1024 - 4 || Bfile_ReadFile_OS(hf, ram2M, size, 0) != size) {
...
int kbd_convert(int r, int c) {
  ...
  if (r == 2) {
    ...
    if (c == 6)
      return KEY_CHAR_DP;
    if (c == 5)
      return KEY_CHAR_EXPN10;
    if (c == 4)
      return KEY_CHAR_PMINUS;
    if (c == 3)
      return 4; // KEY_CTRL_EXE;
  }
  if (r == 3) {
    if (c == 7)
      return KEY_CHAR_1;
  ...
  if (r == 4) {
    ...
    if (c == 5)
      return KEY_CHAR_6;
    if (c == 4)
      return KEY_CHAR_MULT;
    if (c == 3)
      return KEY_CHAR_DIV;
  }
  if (r == 5) {
    if (c == 7)
      return KEY_CHAR_FRAC;
    if (c == 6)
      return KEY_CHAR_H;
    if (c == 5)
      return KEY_CHAR_LPAR;
    if (c == 4)
      return KEY_CHAR_RPAR;
    if (c == 3)
      return KEY_CHAR_COMMA;
    if (c == 2)
      return KEY_CHAR_STORE;
  }
  if (r == 6) {
    if (c == 7)
      return KEY_CTRL_XTT;
  ...
  if (r == 8) {
    if (c == 7)
      return KEY_CTRL_ALPHA;
    if (c == 6)
      return KEY_CHAR_SQUARE;
    if (c == 5)
      return KEY_CHAR_POW;
    if (c == 4)
      return 5; // KEY_CTRL_EXIT;
    if (c == 3)
      return 2; // KEY_CTRL_DOWN;
    if (c == 2)
      return 3; // KEY_CTRL_RIGHT;
  }
  if (r == 9) {
    if (c == 7)
      return KEY_CTRL_SHIFT;
    if (c == 6)
      return KEY_CTRL_OPTN;
    if (c == 5)
      return KEY_CTRL_VARS;
    if (c == 4)
      return KEY_CTRL_MENU;
    if (c == 3)
      return 0; // KEY_CTRL_LEFT;
    if (c == 2)
      return 1; // KEY_CTRL_UP;
  }
  if (r == 10) {
    if (c == 7)
      return KEY_CTRL_F1;
...
#else
bool get_cas_memory_stats(unsigned *) {
  return false;
}
int get_free_memory() {
  return freeslotmem();
...
int main1() {
  ...
  if (1 || calculator == 1
    ...
    int key = QRdisp(
        lang == 1 ? "https://www-fourier.univ-grenoble-alpes.fr/~parisse/casio/khicasio.html"
                  : "https://www-fourier.univ-grenoble-alpes.fr/~parisse/casio/khicasioen.html",
        "KhiCAS doc QRcode", lang == 1 ? "*10^x: accelerer" : "*10^x: overclock",
        lang == 1 ? "EXE: vitesse normale" : "EXE: normal speed",
        lang == 1 ? "DEL: efface session" : "DEL: erase session");
    if ( //1 ||
        calculator == 1 && key == KEY_CHAR_EXP
    ...
    if (key == KEY_CTRL_DEL &&
        do_confirm(lang == 1 ? "Effacer session.xw?" : "Erase session.xw?")) {
      unsigned short pFile[MAX_FILENAME_SIZE + 1];
      const char filename[] = "\\\\fls0\\session.xw";
      Bfile_StrToName_ncpy(pFile, (const unsigned char *)filename, strlen(filename) + 1);
      int res = 0;
      res = Bfile_DeleteEntry(pFile);
    }
  }
  ...
  // confirm("main","2.5");
  unsigned addr = 0xE5200000;
  addr = 0xE5007000;
  //char bufmcs2[64]={0};printx(bufmcs2,addr); confirm("addr",bufmcs2);
  if ( //1 ||
      inexammode) {
    const unsigned patch[0x34] = {
        0x73657373, 0x696f6e00, 0x00000014, 0x00000020, 0x00000012, 0x6d617271, // 0x10
        0x75657572, 0x20486869, 0x43415300, 0,                                  // 0x20
        0,          0,
        0, // 0x30
    };
    bool doit = false;
...
static giac::gen eqw_presentation(const giac::gen & ge, bool editable,
                                  const giac::gen * raw_result);
...
static giac::gen eqw_presentation(const giac::gen & ge, bool editable,
  ...
  for (;;) {
    ...
    if (firstrun == 2) {
      DefineStatusMessage(
          (char *)(lang ? "EXE: quitte, resultat dans last" : "EXE: quit, result stored in last"),
          1, 0, 0);
      //EnableStatusArea(2);
      DisplayStatusArea();
      firstrun = 1;
...
#define EQW_TAILLE 54
    ...
    if (raw_result) {
      ...
      if (key == KEY_CTRL_CLIP) {
        copy_clipboard(parametric_display_selection(*raw_result, line, col).print(contextptr), true,
                       true);
        continue;
      }
      if (key == KEY_CHAR_STORE) {
        ustl::string name;
        if (inputline(lang ? "Stocker selection dans" : "Save selection in",
                      lang ? "Nom de variable: " : "Variable name: ", name, false) &&
      ...
      if (key != KEY_CTRL_LEFT && key != KEY_CTRL_RIGHT && key != KEY_CTRL_UP &&
          key != KEY_CTRL_DOWN && key != KEY_SHIFT_LEFT && key != KEY_SHIFT_RIGHT &&
          key != KEY_CTRL_PAGEUP && key != KEY_CTRL_PAGEDOWN)
        continue;
    }
    //cout << key << '\n';
    if (key == KEY_CTRL_SD) {
    ...
    bool ins = key == KEY_CHAR_STORE || key == KEY_CHAR_RPAR || key == KEY_CHAR_LPAR ||
               key == KEY_CHAR_COMMA || key == KEY_CTRL_PASTE;
    ...
    if (key == KEY_CHAR_STORE) {
      ...
      if (keyflag == 0)
        handle_f5();
      if (inputline(lang ? "Stocker selection dans" : "Save selection in",
                    lang ? "Nom de variable: " : "Variable name: ", varname, false) &&
    ...
    translate_fkey(key);
    const char * adds = (key == KEY_CHAR_PMINUS || (key >= KEY_CTRL_F7 && key <= KEY_CTRL_F20) ||
                         (key == char(key) && (isalphanum(key) || key == '.')))
                            ? keybuf
                            : keytostring(key, keyflag);
    if (key == KEY_CTRL_F1 || key == KEY_CTRL_F2 || (key >= KEY_CTRL_F7 && key <= KEY_CTRL_F20)) {
    ...
    if (key != KEY_CHAR_MINUS && key != KEY_CHAR_EQUAL &&
        (ins || key == KEY_CHAR_PI || key == KEY_CTRL_F5 ||
      ...
      if (line >= 0 && xcas::eqw_select(eq.data, line, col, true, value)) {
        ...
        vector<int> goto_sel;
        if (xcas::Equation_adjust_xy(eq.data, xleft, ytop, xright, ybottom, gsel, gselparent,
                                     gselpos, &goto_sel) &&
            gsel && xcas::do_select(*gsel, true, value) && value.type == _EQW) {
          ustl::string s;
          if (ins) {
    ...
    if (key == KEY_CTRL_EXIT || key == KEY_CTRL_AC) {
      if (!edited)
        return geq;
      if (confirm(lang ? "Vraiment abandonner?" : "Really leave",
                  lang ? "F1: retour editeur,  F6: confirmer"
                       : "F1: back to editor,  F6: confirm") == KEY_CTRL_F6)
        return undef;
    }
        ...
        if (op.type == _FUNC) {
          ...
          if (xcas::do_select(*gsel, true, value) && value.type == _EQW) {
            ...
            unary_function_ptr immediate_op[] = {
                *at_eval,       *at_evalf,     *at_evalc,          *at_regrouper, *at_simplify,
                *at_normal,     *at_ratnormal, *at_factor,         *at_cfactor,   *at_partfrac,
                *at_cpartfrac,  *at_expand,    *at_canonical_form, *at_exp2trig,  *at_trig2exp,
                *at_sincos,     *at_lin,       *at_tlin,           *at_tcollect,  *at_texpand,
                *at_trigexpand, *at_trigcos,   *at_trigsin,        *at_trigtan,   *at_halftan};
...
#ifndef TURTLETAB
    turtle_stack().erase(turtle_stack().begin() + 1,
                         turtle_stack().end()); // =vector<logo_turtle>(1,logo_turtle());
#endif
...
void displaylogo() {
  ...
  while (1) {
    ...
    if (key == KEY_CTRL_EXIT || key == KEY_PRGM_ACON || key == KEY_CTRL_MENU ||
        key == KEY_CTRL_EXE || key == KEY_CTRL_VARS)
      break;
    if (key == KEY_CTRL_UP) {
      t.turtley += 10;
    }
...
bool eqws(char * s, bool eval) { // s buffer must be at least GEN_PRINT_BUFSIZE char
  ...
  if (is_undef(ge))
    return textedit(s);
  if (ge.type == giac::_SYMB ||
      (ge.type == giac::_VECT && !ge._VECTptr->empty() && !is_numericv(*ge._VECTptr))) {
    if (islogo(ge)) {
      displaylogo();
      return false;
    }
...
void check_do_graph(giac::gen & ge, const giac::gen & gs, int do_logo_graph_eqw, GIAC_CONTEXT) {
  ...
  if (ge.type == giac::_SYMB ||
    ...
    if (do_logo_graph_eqw % 2 == 0)
      return;
    if (taille(ge, xcas::max_prettyprint_equation) >= xcas::max_prettyprint_equation ||
        ge.is_symb_of_sommet(at_program))
...
char * c_load_script(const char * filename) {
  ...
  unsigned char * asrc = (unsigned char *)malloc(size * sizeof(unsigned char) +
                                                 5); // 5 more bytes to make sure it fits...
  if (!asrc) {
    Bfile_CloseFile_OS(hFile);
    return 0;
  }
  memset(
      asrc, 0,
      size +
          5); //alloca does not clear the allocated space. Make sure the string is null-terminated this way.
...
int run_script(const char * filename) {
  ...
  if (hFile >= 0) // Check if it opened
  {
    ...
    unsigned char * asrc = (unsigned char *)alloca(size * sizeof(unsigned char) +
                                                   5); // 5 more bytes to make sure it fits...
    memset(
        asrc, 0,
        size +
    ...
    if (asrc[0] == '#' || (asrc[0] == 'd' && asrc[1] == 'e' && asrc[2] == 'f' && asrc[3] == ' '))
      return 2;
    if ((asrc[0] == '/' && asrc[1] == '/') ||
        (rsize > 8 && asrc[0] == 'f' && (asrc[1] == 'o' || asrc[1] == 'u') && asrc[2] == 'n' &&
         asrc[3] == 'c' && asrc[4] == 't' && asrc[5] == 'i' && asrc[6] == 'o' && asrc[7] == 'n' &&
         asrc[8] == ' '))
      return 3;
    return 1;
  }
...
void save(const char * fname) {
  ...
  save_console_state_smem(
      ("\\\\fls0\\" + filename + ".xw").c_str(),
      strcmp(
          filename,
          "session")); // call before save_khicas_symbols_smem(), because this calls create_data_folder if necessary!
...
void save_session() {
  if (strcmp(session_filename, "session") && console_changed) {
    ...
    if (confirm(tmp.c_str(), lang ? "F1: sauvegarder, F6: tant pis"
                                  : "F1: save, F6: discard changes") == KEY_CTRL_F1) {
...
int restore_session(const char * fname) {
  ...
  if (!load_console_state_smem((string("\\\\fls0\\") + filename + string(".xw")).c_str())) {
    int x = 0, y = 92;
    PrintMini(&x, &y, (unsigned char *)KHICAS_DISPLAY_VERSION, 0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK,
              COLOR_WHITE, 1, 0);
    x = 0;
    y += 18;
#ifdef MICROPY_LIB
    PrintMini(&x, &y, (unsigned char *)"MicroPython 1.12 (c) D. George et al", 0x02, 0xFFFFFFFF, 0,
              0, COLOR_BLACK, COLOR_WHITE, 1, 0);
    x = 0;
    y += 18;
    PrintMini(&x, &y, (unsigned char *)"License GPL2 (KhiCAS), MIT (mPython)", 0x02, 0xFFFFFFFF, 0,
              0, COLOR_BLACK, COLOR_WHITE, 1, 0);
    x = 0;
    y += 18;
#else
    PrintMini(&x, &y, (unsigned char *)"  License GPL 2", 0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK,
              COLOR_WHITE, 1, 0);
    x = 0;
    y += 18;
#endif
    PrintMini(&x, &y, (unsigned char *)"  Do not use if CAS is forbidden", 0x02, 0xFFFFFFFF, 0, 0,
              COLOR_RED, COLOR_WHITE, 1, 0);
#ifdef MICROPY_LIB
...
void edit_script(char * fname) {
  ...
  if (res) {
    ...
    if (s.empty()) {
      s = python_compat(contextptr)
              ? (lang ? "Prog. Python, sinon taper" : "Python prog., for Xcas")
              : (lang ? "Prog. Xcas, sinon taper" : "Xcas prog., for Python");
      s += " AC F6 12";
...
void run(const char * s, int do_logo_graph_eqw) {
  ...
  if (ge.type == giac::_STRNG)
    ...
    if (te >= 256)
      ...
      if (ge.is_symb_of_sommet(giac::at_pnt) ||
          (ge.type == giac::_VECT && !ge._VECTptr->empty() &&
