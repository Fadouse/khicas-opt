...
namespace giac {
  ...
  inline bool parametric_display_view(const gen & request, const gen & result, gen & view) {
    ...
    for (unsigned depth = 0; depth < 8; ++depth) {
      if (call->is_symb_of_sommet(at_simplify) || call->is_symb_of_sommet(at_normal) ||
          call->is_symb_of_sommet(at_ratnormal)) {
        call = &call->_SYMBptr->feuille;
        if (call->type == _VECT && call->subtype == _SEQ__VECT)
          return false;
        continue;
      }
      if (call->is_symb_of_sommet(at_sto)) {
        ...
        if (f.type != _VECT || f._VECTptr->size() != 2)
          return false;
        call = &f._VECTptr->front();
        continue;
      }
      break;
    }
    bool cart = call->is_symb_of_sommet(at_cart2param);
    if (!cart && !call->is_symb_of_sommet(at_polar2param))
      return false;
    const gen & f = call->_SYMBptr->feuille;
    if (f.type != _VECT || f._VECTptr->size() != 3)
      return false;
    const vecteur & args = *f._VECTptr;
    if (args[1].type != _VECT || args[1]._VECTptr->size() != 2 || args[2].type != _IDNT)
      return false;
    if (result.type != _VECT || result._VECTptr->empty() || result._VECTptr->size() > 16)
      return false;
    ...
    for (unsigned i = 0; i < result._VECTptr->size(); ++i) {
      ...
      if (branch.type != _VECT || branch._VECTptr->size() != 2)
        return false;
      branches.push_back(makevecteur(symb_equal(xcall, branch._VECTptr->front()),
                                     symb_equal(ycall, branch._VECTptr->back())));
    }
    view = branches;
    return true;
  }
  ...
  inline gen parametric_display_selection(const gen & result, int row, int column) {
    if (result.type != _VECT || row < 0 || unsigned(row) >= result._VECTptr->size())
      return result;
