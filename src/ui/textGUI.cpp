...
int check_parse(const ustl::vector<textElement> & v, int python) {
  ...
  if (lineerr) {
    ...
    if (pos >= 0)
      ...
      sprintf(status, lang ? "Erreur ligne %i %s" : "Error line %i %s", lineerr,
              (pos == -2 ? (lang ? ", : manquant ?" : ", missing :?") : ""));
    DefineStatusMessage(status, 1, 0, 0);
  } else {
    giac::set_abort();
    giac::gen gs(g);
...
void search_msg() {
  DefineStatusMessage((char *)(lang ? "EXE: suivant, AC: annuler" : "EXE: next, AC: cancel"), 1, 0,
                      0);
  DisplayStatusArea();
}
...
bool chk_replace(textArea * text, const ustl::string & search, const ustl::string & replace) {
  if (replace.size())
    DefineStatusMessage(
        (char *)(lang ? "Remplacer? EXE: Oui, 8 ou N: Non" : "Replace? EXE: Yes, 8 or N: No"), 1, 0,
        0);
  else
    search_msg();
  DisplayStatusArea();
  for (;;) {
    ...
    if (key == KEY_CHAR_MINUS || key == KEY_CHAR_Y || key == KEY_CHAR_9 || key == KEY_CHAR_O ||
      ...
      if (replace.size()) {
        ...
        s = s.substr(0, text->pos - search.size()) + replace +
            s.substr(text->pos, s.size() - text->pos);
        search_msg();
      }
      return true;
    }
    ...
    if (text->changed) {
      ...
      if (strcmp(tmp.c_str(), "temp.py") == 0) {
        if (confirm(lang ? "Les modifications seront perdues" : "Changes will be lost",
                    lang ? "F1: annuler,       F6: tant pis" : "F1: cancel,       F6: confirm") ==
            KEY_CTRL_F1)
          return 2;
        else {
          return 0;
...
void print(int & X, int & Y, const char * buf, int color, bool revert, bool fake, bool minimini) {
  if (minimini)
    ...
    PrintMini(&X, &Y, (unsigned char *)buf, revert ? 4 : 0, 0xFFFFFFFF, 0, 0, color, COLOR_WHITE,
              fake ? 0 : 1, 0);
}
...
ustl::string get_selection(textArea * text, bool erase) {
  ...
  int clipline = text->clipline, clippos = text->clippos, textline = text->line,
      textpos = text->pos;
  if (clipline >= 0) {
  ...
  if (erase) {
    ...
    text->line = sel_line1;
    text->pos = sel_pos1;
    text->elements[sel_line1].s =
        s.substr(0, sel_pos1) +
        text->elements[sel_line2].s.substr(sel_pos2, text->elements[sel_line2].s.size() - sel_pos2);
  }
  ...
  if (erase)
    text->elements.erase(text->elements.begin() + sel_line1_ + 1,
                         text->elements.begin() + sel_line2 + 1);
  return s;
}
...
void display(textArea * text, int & isFirstDraw, int & totalTextY, int & scroll, int & textY) {
  ...
  for (int cur = 0; cur < v.size(); ++cur) {
    ...
    if (cur == 0) {
      ...
      if (l >= 8 && src[0] == 'f' && (src[1] == 'o' || src[1] == 'u') && src[2] == 'n' &&
          src[3] == 'c' && src[4] == 't' && src[5] == 'i' && src[6] == 'o' && src[7] == 'n')
        change_mode(text, 0); // text->python=false;
    ...
    while (*src) {
      ...
      if ((text->python && *src == '#') || (!text->python && *src == '/' && *(src + 1) == '/')) {
        linecomment = true;
        couleur = 4;
      }
      if (linecomment || !text->editable)
        ...
        if (*src == '"') {
          ...
          if (i == 0) {
            ...
            for (; *src; ++i, ++src) {
              if (*src == ' ' || (i && *src == ',') || (text->python && *src == '#') ||
                  (!text->python && *src == '/' && *(src + 1) == '/') || *src == '"' ||
                  isalpha(*src))
                break;
            }
          }
      ...
      if (temptextX < text->width && temptextX + textX > text->width - 6) {
        if (editable)
          PrintMini(&textX, &textY, (unsigned char *)"\xe6\x9b", 0x02, 0xFFFFFFFF, 0, 0,
                    COLOR_MAGENTA, COLOR_WHITE, 1, 0);
        //time for a new line
        textX = text->x + deltax;
        textY = textY + text->lineHeight + v[cur].lineSpacing;
        ++nlines;
      ...
      if (textY >= -24 && textY < LCD_HEIGHT_PX) {
        ...
        if (editable) {
          if (match1 && oldsrc <= match1 && match1 < src)
            match_print(singleword, match1 - oldsrc, temptextX, textY, line2 != -1,
                        // match2,
                        minimini);
          if (match2 && oldsrc <= match2 && match2 < src)
            match_print(singleword, match2 - oldsrc, temptextX, textY, line1 != -1,
                        //match1,
                        minimini);
        }
...
bool move_to_word(textArea * text, const ustl::string & s, const ustl::string & replace,
                  int & isFirstDraw, int & totalTextY, int & scroll, int & textY) {
  if (!s.size())
    return false;
...
int doTextArea(textArea * text) {
  ...
  while (1) {
	...
	continue;
      ...
      if (clipline < 0) {
        ...
        if ((key >= KEY_CTRL_F1 && key <= KEY_CTRL_F3) ||
          ...
          const char * ptr =
              console_menu(key, (unsigned char *)(le_menu.c_str()), text->gr ? 5 : 2);
          if (!ptr) {
            show_status(text, search, replace);
            continue;
          }
        ...
        if (adds) {
          ...
          if (isex)
            ++adds;
          bool isif = strcmp(adds, "if ") == 0, iselse = strcmp(adds, "else ") == 0,
               isfor = strcmp(adds, "for ") == 0, isforin = strcmp(adds, "for in") == 0,
               isdef = (strcmp(adds, "f(x):=") == 0 || strcmp(adds, "def") == 0),
               iswhile = strcmp(adds, "while ") == 0, islist = strcmp(adds, "list ") == 0,
          ...
          insert(text, adds,
                 key != KEY_CTRL_PASTE); // was true, but we should not indent when pasting
          show_status(text, search, replace);
          continue;
        }
      }
    ...
    case KEY_CTRL_EXE:
      ...
      if (text->gr || text->allowEXE || !text->editable)
    ...
    case KEY_CTRL_F6:
      if (clipline < 0 && text->editable && text->filename.size()) {
        ...
        if (sres == MENU_RETURN_SELECTION) {
          ...
          if (sres == 2) {
            if (text->gr)
              confirm(lang == 1 ? "Tapez EXE puis sauvegardez" : "Type EXE then save",
                      lang == 1 ? "depuis le menu F6 item 11" : "from F6 menu item 11");
            else {
	    ...
	    CLIP_Store(s.c_str(),s.size()+1);
          ...
          if (sres == 7) {
            display(text, isFirstDraw, totalTextY, scroll, textY);
            int l = get_line_number(lang ? "Negatif: en partant de la fin"
