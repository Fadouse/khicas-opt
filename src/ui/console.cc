...
char menu_f1[maxfmenusize] = {0}, menu_f2[maxfmenusize] = {0}, menu_f3[maxfmenusize] = {0},
     menu_f4[maxfmenusize] = {0}, menu_f5[maxfmenusize] = {0}, menu_f6[maxfmenusize],
     menu_f7[maxfmenusize] = {0}, menu_f8[maxfmenusize] = {0}, menu_f9[maxfmenusize] = {0},
     menu_f10[maxfmenusize] = {0}, menu_f11[maxfmenusize] = {0}, menu_f12[maxfmenusize] = {0},
     menu_f13[maxfmenusize] = {0}, menu_f14[maxfmenusize] = {0}, menu_f15[maxfmenusize] = {0},
     menu_f16[maxfmenusize] = {0}, menu_f17[maxfmenusize] = {0}, menu_f18[maxfmenusize] = {0};
char session_filename[MAX_FILENAME_SIZE + 1] = "session";
char * FMenu_entries_name[] = {menu_f1,  menu_f2,  menu_f3,  menu_f4,  menu_f5,  menu_f6,
                               menu_f7,  menu_f8,  menu_f9,  menu_f10, menu_f11, menu_f12,
                               menu_f13, menu_f14, menu_f15, menu_f16, menu_f17, menu_f18};
location Cursor;
...
void set_xcas_status() {
  ...
  status += char('0' + (minute % 10));
  status += xthetat ? " t " : " x ";
  status += xcas_python_eval == 1
                ? " Python "
                : (giac::python_compat(contextptr)
                       ? (giac::python_compat(contextptr) == 2 ? " XcasPy ^=xor " : " XcasPy ^=** ")
                       : " Xcas ");
...
void menu_setup() {
  ...
  while (1) {
#ifdef MICROPY_LIB
    string heaps((lang ? "Tas Python " : "Python heap ") +
                 giac::print_INT_(pythonjs_heap_size / 1024) + "K");
    ...
    if (sres == MENU_RETURN_SELECTION) {
      ...
      if (smallmenu.selection == 5) {
        ...
        if (inputdouble(lang ? "Digits (9, 19, 28)?" : "Digits (9, 19, 28)?", d) && d == int(d) &&
            d >= 9 && d <= 1000) {
      ...
      if (smallmenu.selection == 6 && pythonjs_static_heap == 0) {
        ...
        if (inputdouble(lang ? "Tas en K (64-480)?" : "Heap size in K", d) && d == int(d) &&
            d >= 64 && d <= 480) {
      ...
      if (smallmenu.selection == 2) {
        ...
        if (c >= 0) {
          ...
          if (old_xcas_python_eval == 1 && ((int)python_heap) > 1 &&
              do_confirm((lang == 1) ? "Effacer le tas MicroPython?" : "Clear MicroPython heap?"))
            python_free();
#endif
#ifdef QUICKKS
          if (0 && old_xcas_python_eval == -1 &&
              do_confirm((lang == 1) ? "Effacer le tas QuickJS?" : "Clear QuickJS heap?"))
            js_end(global_js_context);
#endif
          // warn_python(p,false);
          console_disp_status();
...
void copy_clipboard(const ustl::string & s, bool status, bool clip_pasted) {
  ...
  if (status) {
    DefineStatusMessage(
        (char *)(lang ? "Selection copiee vers presse-papiers." : "Selection copied to clipboard"),
        1, 0, 0);
    DisplayStatusArea();
  }
}
...
int print_msg12(const char * msg1, const char * msg2, int textY) {
  ...
  int textX = 30;
  if (msg1)
    PrintMini(&textX, &textY, (unsigned char *)msg1, 0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK,
              COLOR_WHITE, 1, 0);
  textX = 10;
  textY += 25;
  if (msg2)
    PrintMini(&textX, &textY, (unsigned char *)msg2, 0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK,
              COLOR_WHITE, 1, 0);
  return textX;
}
...
int inputline(const char * msg1, const char * msg2, ustl::string & s, bool numeric, int ypos) {
  ...
  for (;;) {
    ...
    if (beg > pos)
      beg = pos;
    textX = X1;
    PrintMini(&textX, &textY, (unsigned char *)s.substr(beg, pos - beg).c_str(), 0x02, 0xFFFFFFFF,
              0, 0, COLOR_BLACK, COLOR_WHITE, 1, 0);
    int cursorpos = textX;
    PrintMini(&textX, &textY, (unsigned char *)s.substr(pos, s.size() - pos).c_str(), 0x02,
...
void warn_python(int mode, bool autochange) {
  if (mode == 0)
    confirm(autochange
                ...
                : (lang ? "Syntaxe Xcas." : "Xcas syntax."),
            "F1/F6: ok");
  if (mode == 1)
    if (autochange)
      confirm(lang ? "Source en syntaxe Python. Passage" : "Python syntax source detected. Setting",
              lang ? "en Python avec ^=**, F1/F6: ok" : "Python mode with ^=**, F1/F6:ok");
    else
      confirm(lang ? "Syntaxe Python avec ^==**, tapez" : "Python syntax with ^==**, type",
              lang ? "python_compat(2) pour xor. F1: ok" : "python_compat(2) for xor. F1: ok");
  if (mode == 2) {
    confirm(lang ? "Syntaxe Python avec ^==xor" : "Python syntax with ^==xor",
            lang ? "python_compat(1) pour **. F1: ok" : "python_compat(1) for **. F1: ok");
  }
}
...
const char * input_matrix(bool list) {
  ...
  *sptr = "";
  ...
  if (w.empty())
    ...
    msg = ((lang ? "Creer nouveau ou editer " : "Create new or edit ") +
           (w.size() == 1 ? w.front() : giac::gen(w, giac::_SEQ__VECT)).print(contextptr));
  handle_f5();
  if (inputline(msg.c_str(), (lang ? "Nom de variable:" : "Variable name:"), *sptr, false) &&
...
int get_filename(char * filename, const char * extension) {
  handle_f5();
  ustl::string str;
  int res = inputline(lang ? "EXIT ou chaine vide: annulation" : "EXIT or empty string: cancel",
                      lang ? "Nom de fichier:" : "Filename:", str, false);
  ...
  if (confirm(lang ? "     Le fichier existe!" : "     File exists!",
              lang ? "F1: ecraser,           F6: annuler" : "F1:overwrite,           F6: cancel") ==
      KEY_CTRL_F1)
    return 1;
  return 0;
}
...
ustl::string get_searchitem(ustl::string & replace) {
  ...
  handle_f5();
  int res = inputline(lang ? "EXIT ou chaine vide: annulation" : "EXIT or empty string: cancel",
                      lang ? "Chercher:" : "Search:", search, false);
...
// QR code
static int do_QRdisp(const uint8_t qrcode[], const char * msg1, const char * msg2,
                     const char * msg3, const char * msg4) {
  drawRectangle(0, 0, LCD_WIDTH_PX, LCD_HEIGHT_PX, 0xffff);
  EnableStatusArea(3);
  int x = 0, y = 182;
  PrintMiniMini(&x, &y,
                (unsigned char *)"EXE; ok, EXIT: cancel. QR Code generator (c) Project Nayuki.", 0,
                0, 0);
  ...
  if (scale) {
    for (int y = -border; y < size + border; y++) {
      for (int x = -border; x < size + border; x++) {
        drawRectangle(2 + scale * (border + x), 2 + scale * (border + y), scale, scale,
                      qrcodegen_getModule(qrcode, x, y) ? 0 : 0xffff);
      }
    }
  }
  x = 184;
  y = 60;
  if (msg1)
    PrintMini(&x, &y, (unsigned char *)msg1, 0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK, COLOR_WHITE, 1,
              0);
  x = 184;
  y += 20;
  if (msg2)
    PrintMini(&x, &y, (unsigned char *)msg2, 0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK, COLOR_WHITE, 1,
              0);
  x = 184;
  y += 20;
  if (msg3)
    PrintMini(&x, &y, (unsigned char *)msg3, 0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK, COLOR_WHITE, 1,
              0);
  x = 184;
  y += 20;
  if (msg4)
    PrintMini(&x, &y, (unsigned char *)msg4, 0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK, COLOR_WHITE, 1,
              0);
  while (1) {
    int key;
    ck_getkey(&key);
...
void save_console_state_smem(const char * filename, bool dispqr) {
  ...
  for (int i = start_row; i <= Last_Line; ++i) {
    ...
    memcpy(consoleptr, &l, sizeof(l));
    consoleptr += sizeof(l);
    unsigned short s = cur.start_col;
    memcpy(consoleptr, &s, sizeof(s));
    consoleptr += sizeof(s);
    unsigned char c = cur.type;
    memcpy(consoleptr, &c, sizeof(c));
    consoleptr += sizeof(c);
    ...
    memcpy(consoleptr, buf, l);
    consoleptr += l;
  }
  char BUF[2] = {0, 0};
  memcpy(consoleptr, BUF, sizeof(BUF));
  consoleptr += sizeof(BUF);
...
bool load_console_state_smem(const char * filename) {
  ...
  for (int pos = 0;; ++pos) {
    ...
    if (Bfile_ReadFile_OS(hf, &l, sizeof(l), -1) != sizeof(l) || l == 0)
      break;
    if (Bfile_ReadFile_OS(hf, &curs, sizeof(curs), -1) != sizeof(curs))
      break;
    if (Bfile_ReadFile_OS(hf, &type, sizeof(type), -1) != sizeof(type))
      break;
    if (Bfile_ReadFile_OS(hf, &readonly, sizeof(readonly), -1) != sizeof(readonly))
      break;
    char buf[l + 1];
    buf[l] = 0;
...
int Console_MoveCursor(int direction) {
  ...
  case CURSOR_UP:
    ...
    if ((Cursor.y > 0) || (Start_Line > 0)) {
      ...
      if (!Line[Current_Line].readonly) {
        if ((Line[Current_Line].str =
                 (unsigned char *)console_malloc(strlen((const char *)Edit_Line) + 1)) == NULL)
          return CONSOLE_MEM_ERR;
  ...
  case CURSOR_DOWN:
    ...
    if ((Cursor.y < LINE_DISP_MAX - 1) && (Current_Line < Last_Line) ||
        (Start_Line + LINE_DISP_MAX - 1 < Last_Line)) {
      //If the current line is not read-only, then Edit_Line copy to the current line.
      if (!Line[Current_Line].readonly) {
        if ((Line[Current_Line].str =
                 (unsigned char *)console_malloc(strlen((const char *)Edit_Line) + 1)) == NULL)
      ...
      //If the current cursor line is not read-only, then it is a string copy to Edit_Line for editing.
      if (!Line[Current_Line].readonly && Line[Current_Line].str) {
        strncpy((char *)Edit_Line, (const char *)Line[Current_Line].str, minbufsize);
        Edit_Line[minbufsize - 1] = 0;
        console_free(Line[Current_Line].str);
        Line[Current_Line].str = Edit_Line;
      }
    }
...
void chk_clearscreen() {
  ...
  if (confirm(lang ? "Effacer l'historique?" : "Clear history?",
              lang ? "F1: annuler,   F6: effacer" : "F1: cancel,   F6: erase",
              true) == KEY_CTRL_F6) {
    Console_Init();
    Console_Clear_EditLine();
  }
...
std::string _pnt2string(const giac::gen & g, const giac::context * contextptr) {
  ...
  if (g.is_symb_of_sommet(giac::at_pnt)) {
    ...
    if (fp.is_symb_of_sommet(giac::at_hyperplan)) {
      return gettext("plan") + string("(") + _equation(g, contextptr).print(contextptr) +
             string(")");
    }
    if (f.type == giac::_VECT && !f._VECTptr->empty()) {
      ...
      if (f0.is_symb_of_sommet(giac::at_cercle) && f0._SYMBptr->feuille.type == giac::_VECT) {
        ...
        if (!complex_mode(contextptr) && (centre.type < giac::_IDNT || centre.type == giac::_FRAC))
          return gettext("circle") + string("(point(") +
                 giac::re(centre, contextptr).print(contextptr) + "," +
      ...
      if ((f0.type != giac::_FRAC && f0.type >= giac::_IDNT) || is3d(g) ||
  ...
  if (g.type == giac::_VECT && !g._VECTptr->empty() &&
      g._VECTptr->back().is_symb_of_sommet(giac::at_pnt)) {
    std::string s = "[";
...
#ifdef TURTLETAB
  total += giac::syms().capacity() * (sizeof(string) + sizeof(giac::gen) + 8) +
           sizeof(giac::sym_string_tab) + sizeof(giac::context) +
           contextptr->tabptr->capacity() * (sizeof(const char *) + sizeof(giac::gen) + 8) +
           bytesize(giac::history_in(contextptr)) + bytesize(giac::history_out(contextptr));
#else
  total += giac::syms().capacity() * (sizeof(string) + sizeof(giac::gen) + 8) +
           sizeof(giac::sym_string_tab) +
           giac::turtle_stack().capacity() * sizeof(giac::logo_turtle) + sizeof(giac::context) +
           contextptr->tabptr->capacity() * (sizeof(const char *) + sizeof(giac::gen) + 8) +
           bytesize(giac::history_in(contextptr)) + bytesize(giac::history_out(contextptr));
#endif
...
const char * keytostring(int key, int keyflag, bool py, const giac::context * contextptr) {
  ...
  case KEY_CTRL_F3:
    return (alph) ? (py ? "\\" : "sum(")
                  : (keyflag == 1 ? (py ? "%" : "integrate(") : ((!py && xthetat) ? "diff(" : "'"));
  case KEY_CHAR_MAT:
  case KEY_CTRL_F8: {
...
int chartab() {
  ...
  for (;;) {
    ...
    os_draw_string(0, 24, COLOR_BLACK, COLOR_WHITE,
                   lang == 1 ? "shift INS: table de caracteres" : "shift INS: char table");
    int dy = 44;
    for (int r = 0; r < 6; ++r) {
      for (int c = 0; c < 16; ++c) {
        int currc = 32 + 16 * r + c;
  ...
  for (;;) {
    ...
    if (doit) {
      ...
      if (paramenu.selection == 2) {
        handle_f5();
        if (inputline(menu_name, lang ? "Nouvelle valeur?" : "New value?", s1, false) ==
                KEY_CTRL_EXE &&
            s1.size() > 0 && isalpha(s1[0])) {
          if (s1.size() > 10)
            s1 = s1.substr(0, 10);
...
int Console_GetKey() {
  ...
  for (;;) {
    ...
    if (key == KEY_CHAR_PLUS || key == KEY_CHAR_MINUS || key == KEY_CHAR_MULT ||
        key == KEY_CHAR_DIV) {
        ...
        if (sres == MENU_RETURN_SELECTION) {
          ...
          if (smallmenu.selection == 4) {
            ...
            if (fileBrowser(filename, (char *)"*.xw", (char *)"Sessions")) {
              if (console_changed == 0 || strcmp(session_filename, "session") == 0 ||
                  confirm(lang ? "Session courante perdue?" : "Current session will be lost",
                          lang ? "F1: annul, F6: ok" : "F1: cancel, F6: ok") == KEY_CTRL_F6) {
                giac::clear_context(
                    contextptr); // giac::_restart(giac::gen(giac::vecteur(0),giac::_SEQ__VECT),contextptr);
          ...
          if (smallmenu.selection == 7) {
            if (!edptr || merge_area(edptr->elements).size() < 2)
              edit_script((char *)("\\\\fls0\\" + giac::remove_extension(session_filename) + ".py")
                              .c_str());
            else
              doTextArea(edptr);
            break;
          ...
          if (smallmenu.selection == 17) {
            if (do_confirm("Quitter en reinitialisant KhiCAS?")) {
              ...
              SetQuitHandler(0);
              print_msg12(lang == 1 ? "Taper sur MENU, ouvrez une autre"
                                    : "Type MENU key, open another",
                          lang == 1 ? "application avant de reouvrir KhiCAS"
                                    : "application before reopening KhiCAS");
              for (;;) { //wait for ever
                int key;
                GetKey(&key);
              }
          ...
          if (smallmenu.selection == 17 || smallmenu.selection == 18) {
            Bdisp_AllClr_VRAM();
            confirm(lang == 1 ? "Taper sur la touche MENU" : "Type the MENU key",
                    lang == 1 ? "puis ouvrez une autre application"
                              : "then open any other application");
          }
        }
        break;
      } // end while(1)
    ...
    if ((key >= KEY_CTRL_F1 && key <= KEY_CTRL_F6) || (key >= KEY_CTRL_F7 && key <= KEY_CTRL_F20)) {
    ...
    if ((key == KEY_CTRL_DOWN || key == KEY_CTRL_F10 || key == KEY_CTRL_MIXEDFRAC) &&
    ...
    if (key == KEY_CTRL_EXIT) {
      if (Last_Line == Current_Line) {
        if (!edptr)
          edit_script(
              (char *)("\\\\fls0\\" + giac::remove_extension(session_filename) + ".py").c_str());
        else {
          edptr->y = 0;
          doTextArea(edptr);
        }
...
int Console_FMenu(int key) {
  const char *s = console_menu(
                 key, xcas_python_eval ? (unsigned char *)python_conf_standard : original_cfg, 0),
             *ptr = 0;
  if (!s) {
...
const char * console_menu(int key, int active_app) {
  return console_menu(key,
                      xcas_python_eval == 1 ? (unsigned char *)python_conf_standard : original_cfg,
                      active_app);
}
...
const char * console_menu(int key, unsigned char * cfg_, int active_app) {
  ...
  char temp[maxentry_size], menu1[maxentry_size], menu2[maxentry_size], menu3[maxentry_size],
      menu4[maxentry_size], menu5[maxentry_size], menu6[maxentry_size], menu7[maxentry_size],
      menu8[maxentry_size];
...
const char * Console_Draw_FMenu(int key, struct FMenu * menu, unsigned char * cfg, int active_app) {
  ...
  for (;;) {
    ...
    PrintMini(3 + position_x + quick_len * 4, box.bottom - 7 * (selector + 1), entries[selector],
              4);
    ck_getkey(&input_key);
    ...
    if (active_app == 0 && ((input_key >= KEY_CTRL_F1 && input_key <= KEY_CTRL_F6) ||
...
int Console_Init() {
  ...
  if ((Edit_Line = (unsigned char *)console_malloc(EDIT_LINE_MAX + 1)) == NULL)
    return CONSOLE_MEM_ERR;
  Edit_Line[0] = 0;
...
int print_color(int print_x, int print_y, const char * s, int color, bool invert, bool minimini,
  ...
  while (*src && print_y < LCD_WIDTH_PX) {
    ...
    if ((python && *src == '#') || (!python && *src == '/' && *(src + 1) == '/')) {
      linecomment = true;
      couleur = 4;
    }
    if (linecomment)
    ...
    if (couleur == 1)
      couleur = COLOR_BLUE;
    if (couleur == 2)
      couleur = 49432; //was COLOR_YELLOWDARK;
    if (couleur == 3)
      couleur = 51712; //33024;
    if (couleur == 4)
...
void get_current_console_menu(string & menu, string & shiftmenu, string & alphamenu,
                              int & menucolorbg, int app) {
  ...
  if (app == 3) {
    menu = (lang ? " outil | stat | edit | cmds | A<>a | menu"
  ...
  if (app == 1) {
    menu = lang == 1 ? " tests | struct | misc | cmds | A<>a |Fich. "
                     : " tests | loops | misc | cmds | A<>a |File ";
  } else {
    menu += string(menu_f1);
    while (menu.size() < 6)
      menu += " ";
  ...
  int xcas_color = python_compat(contextptr) == 0 ? 65055 : COLOR_CYAN, python_color = 65520,
      js_color = 63048;
  if (app == 1) {
  ...
  menucolorbg =
      xcas_python_eval == -1 ? js_color : (xcas_python_eval == 1 ? python_color : xcas_color);
}
...
void console_disp_status() {
  ...
  get_current_console_menu(menu, shiftmenu, alphamenu, menucolorbg, 0);
  int px = 0 * 3, py = 58 * 3;
  PrintMini(&px, &py, (unsigned char *)menu.c_str(), 0, 0xFFFFFFFF, 0, 0, COLOR_BLACK, menucolorbg,
            1, 0);
  // status, clock,
  set_xcas_status();
  Bdisp_PutDisp_DD();
...
int Console_Disp() {
  ...
  for (i = 0; (i < LINE_DISP_MAX) && (i + Start_Line <= Last_Line); i++) {
    ...
    if (i == Cursor.y) {
      // cursor line
      if (curline.type == LINE_TYPE_INPUT ||
          curline.type == LINE_TYPE_OUTPUT && curline.disp_len >= COL_DISP_MAX) {
        locate(1, i + 1);
        if (curline.readonly) {
#ifdef CURSOR
          Cursor_SetFlashOff();
        ...
        if (matchdirection) {
          ...
          if (pos >= fakestart) {
            ...
            print(fakex, fakey, fakes.c_str(), TEXT_COLOR_BLACK, false, true /* fake*/,
                  minimini); // fake print
            print(fakex, fakey, buf, ok ? COLOR_RED : COLOR_GREEN, true /* revert*/, false,
                  minimini);
          }
          if (ok) {
            fakex = 0;
...
void Console_Draw_TeX_Popup(unsigned char * str, int width, int height) {
  DISPBOX popup;
  DISPBOX temp;
  unsigned char arrows[4 * 3] = {0xE6, 0x9A, '\0', 0xE6, 0x9B, '\0',
                                 0xE6, 0x9C, '\0', 0xE6, 0x9D, '\0'};
  ...
  while (key != KEY_CTRL_EXIT) {
    ...
    Bdisp_AreaReverseVRAM(popup.left + border, popup.top + border, popup.right - border,
                          popup.bottom - border);

    TeX_drawComplex((char *)str, popup.left + border + margin + scroll_lateral,
                    popup.top + border + margin + scroll_vertical);
