...
int doMenu(
    Menu * menu,
    MenuItemIcon *
        icontable) { // returns code telling what user did. selection is on menu->selection. menu->selection starts at 1!
  int itemsStartY =
      menu->startY; // char Y where to start drawing the menu items. Having a title increases this by one
  ...
  while (1) {
    ...
    if (menu->statusText != NULL)
      DefineStatusMessage(menu->statusText, 1, 0, 0);
    // Clear the area of the screen we are going to draw on
    if (0 == menu->pBaRtR)
      drawRectangle(18 * (menu->startX - 1),
                    24 * (menu->miniMiniTitle ? itemsStartY : menu->startY),
                    18 * menu->width + ((menu->scrollbar && menu->scrollout) ? 6 : 0),
    ...
    if (menu->numitems > 0) {
        ...
        if (menu->scroll < curitem + 1 && menu->scroll > curitem - itemsHeight) {
          ...
          if (menu->items[curitem].type != MENUITEM_SEPARATOR) {
            ...
            for (int i = 0; i < fillerRequired; i++)
              strcat(menuitem, " ");
            mPrintXY(menu->startX, curitem + itemsStartY - menu->scroll, (char *)menuitem,
                     (menu->selection == curitem + 1 ? TEXT_MODE_INVERT
          ...
          if (menu->items[curitem].type == MENUITEM_CHECKBOX) {
            mPrintXY(
                menu->startX + menu->width - 1, curitem + itemsStartY - menu->scroll,
                (menu->items[curitem].value == MENUITEM_VALUE_CHECKED ? (char *)"\xe6\xa9"
                                                                      : (char *)"\xe6\xa5"),
                (menu->selection == curitem + 1
                     ? TEXT_MODE_INVERT
                     : (menu->pBaRtR == 1 ? TEXT_MODE_TRANSPARENT_BACKGROUND : TEXT_MODE_NORMAL)),
                menu->items[curitem].color);
          }
#ifdef FILEICON
          ...
          if (menu->type == MENUTYPE_MULTISELECT) {
            ...
            if (menu->items[curitem].isselected) {
              if (menu->selection == curitem + 1) {
                mPrintXY(menu->startX, curitem + itemsStartY - menu->scroll, (char *)"\xe6\x9b",
                         TEXT_MODE_TRANSPARENT_BACKGROUND,
                         (menu->items[curitem].color == TEXT_COLOR_GREEN ? TEXT_COLOR_BLUE
                                                                         : TEXT_COLOR_GREEN));
              } else {
                mPrintXY(menu->startX, curitem + itemsStartY - menu->scroll, (char *)"\xe6\x9b",
      ...
      printCentered(menu->nodatamsg, (itemsStartY * 24) + (itemsHeight * 24) / 2 - 12, COLOR_BLACK,
                    COLOR_WHITE);
    }
    if (showtitle) {
      ...
      if (menu->subtitle != NULL) {
        ...
        PrintMini(&textX, &textY, menu->subtitle, 0, 0xFFFFFFFF, 0, 0, COLOR_BLACK, COLOR_WHITE, 1,
                  0);
      }
