...
void chk_restart() {
  ...
  if (confirm(lang ? "Conserver les variables?" : "Keep variables?",
              lang ? "F1: conserver,   F6: effacer" : "F1: keep,   F6: erase") == KEY_CTRL_F6)
    do_restart();
}
...
void input_eval_loop(int isRecording) {
  char ** recHistory = NULL;
  int curRecHistEntry = 0;
  if (isRecording)
    recHistory = (char **)alloca(200); // space for 200 pointers to history entries
  exproffset = 0;
  int nodel = 0; // no delete
  while (1) {
    set_xcas_status();
    DefineStatusMessage((char *)(isRecording ? "Recording ('record' to stop)" : xcas_status), 1, 0,
                        0);
    has_drawn_graph = 0;
    if (!nodel)
    ...
    int res = gets(expr + exproffset, INPUTBUFLEN - exproffset, isRecording,
                   !!exproffset); // isRecording is provided for UI changes, no behavior changes.
    if (res == 2) {
      exproffset = 0;
      dConsolePutChar('\n');
    ...
    if (strcmp(expr, "testmode") == 0) {
      ...
      if (!isRecording)
        ...
        for (int i = 0; i <= maxHistory; i++) {
          size =
              size + strlen(recHistory[i]) + 1; // 1 byte for \n. we will use unix line termination
        }
...
giac::gen eqw(const giac::gen & ge, bool editable) {
  ...
  for (;;) {
    ...
    if (key == KEY_CHAR_STORE) {
      ...
      if (keyflag == 0)
        handle_f5();
      if (inputline(lang ? "Stocker la selection dans" : "Save selection in",
    ...
    const char * adds =
        (key == KEY_CHAR_PMINUS || (key == char(key) && (isalphanum(key) || key == '.')))
            ? keybuf
...
void displaygraph(const giac::gen & ge) {
  ...
  for (;;) {
    ...
    PrintMini(&x, &y, (unsigned char *)"menu", 0x04, 0xffffffff, 0, 0, COLOR_BLACK, COLOR_WHITE, 1,
              0);
    int keyflag = GetSetupSetting((unsigned int)0x14);
    int key;
    GetKey(&key);
    if (key == KEY_CTRL_F1) {
      ...
      if (sres == MENU_RETURN_SELECTION) {
        ...
        if (smallmenu.selection == 1) {
          inputline(menu_xmin, lang ? "Nouvelle valeur?" : "New value?", s1,
                    false); /* numeric value expected */
        ...
        if (smallmenu.selection == 2) {
          inputline(menu_xmax, lang ? "Nouvelle valeur?" : "New value?", s1,
                    false); /* numeric value expected */
          if (stringtodouble(s1, d)) {
            gr.window_xmax = d;
            gr.update();
        ...
        if (smallmenu.selection == 3) {
          inputline(menu_ymin, lang ? "Nouvelle valeur?" : "New value?", s1,
                    false); /* numeric value expected */
          if (stringtodouble(s1, d)) {
            gr.window_ymin = d;
            gr.update();
        ...
        if (smallmenu.selection == 4) {
          inputline(menu_ymax, lang ? "Nouvelle valeur?" : "New value?", s1,
                    false); /* numeric value expected */
          if (stringtodouble(s1, d)) {
            gr.window_ymax = d;
            gr.update();
...
void save(const char * fname) {
  ...
  save_console_state_smem(
      ("\\\\fls0\\" + filename + ".xw")
          .c_str()); // call before save_khicas_symbols_smem(), because this calls create_data_folder if necessary!
...
giac::gen run(char * s) {
  ...
  if (giac::freeze) {
    giac::freeze = false;
    DefineStatusMessage((char *)(lang ? "Taper sur une touche" : "Screen freezed. Press any key."),
                        1, 0, 0);
    DisplayStatusArea();
    //int keyflag = GetSetupSetting( (unsigned int)0x14);
    int key;
  ...
  if (ge.type == giac::_STRNG)
    ...
    if (te > 256)
      ...
      if (ge.is_symb_of_sommet(at_pnt) || (ge.type == _VECT && !ge._VECTptr->empty() &&
                                           ge._VECTptr->back().is_symb_of_sommet(at_pnt)))
        s_ = "Graphic object.";
      else
