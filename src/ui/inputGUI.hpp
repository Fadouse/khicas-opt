...
typedef struct {
  ...
  int y = 3;
  int width =
      21; // again, in character coordinates. note that last space of the input is reserved for scrolling and never gets hit by a char, only the cursor
  ...
  int key =
      0; // put a key here to provide for the initial keypress. also, when input returns INPUT_RETURN_KEYCODE, the keycode is here.
