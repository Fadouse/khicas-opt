...
char * outputRedirectBuffer =
    NULL; // if not null, console output will be redirected to the buffer this char* points to.
int remainingBytesInRedirect =
    0; // bytes remaining in the redirect buffer, respect to avoid overflows
...
void set_xcas_status() {
  ...
  status += char('0' + (minute % 10));
  status += xthetat ? " t " : " x ";
  status += giac::python_compat(contextptr)
                ? (giac::python_compat(contextptr) == 2 ? " Python ^=xor " : " Python ^=** ")
                : " Xcas ";
...
void print(unsigned char * msg) {
  ...
  if (linestart < 0)
    linestart = 0;
  if (myconsoley - linestart >= 0 && myconsoley - linestart < visible_lines)
    PrintMiniFix(myconsolex * 12, (myconsoley - linestart) * 17, msg,
                 (visible_lines == 12 ? 0x40 : 0), COLOR_BLACK, COLOR_WHITE,
                 (visible_lines == 12 ? 1 : 0));
...
void addStringToInput(char * dest, const char * src, int * pos, int max, int * refresh) {
  ...
  if ((int)strlen(dest) + srclen - 1 >= max)
    return;
...
void do_refresh(char * s, int start, int pos, int x, int y, int width, int isscrolling) {
  ...
  if (start == 0) {
    ...
    locate(x + pos, y);
    printCursor(); // if (pos>=0){ locate(x+pos,y); printCursor(); }
  ...
  if (!isscrolling) {
    ...
    if (start) {
      int px = 0; //12;
      PrintMini(&px, &py, (unsigned char *)"\xe6\x9a", 0x02, 0xFFFFFFFF, 0, 0, COLOR_MAGENTA,
                COLOR_WHITE, 1, 0);
    }
    if ((int)strlen(s + start) > width) {
      int px = 31 * 12;
      PrintMini(&px, &py, (unsigned char *)"\xe6\x9b", 0x02, 0xFFFFFFFF, 0, 0, COLOR_MAGENTA,
                COLOR_WHITE, 1, 0);
    }
  }
}
...
int dGetLine(char * s, int max, int isRecording, int ml) {
  ...
  while (1) {
    ...
    int keyflag = GetSetupSetting((unsigned int)0x14);
    DirectDrawRectangle(LCD_WIDTH_PX + 6, 24, LCD_WIDTH_PX + 6 + 5, 210,
                        COLOR_WHITE); // clear scroll indicator
    if (isscrolling) {
      int starty =
          (line_count == 0
               ? 0
               : ((line_count + myconsolescroll -
                   (line_count < visible_lines ? line_count : visible_lines)) *
                  162) /
                     (line_count < visible_lines ? line_count : line_count - visible_lines));
      DirectDrawRectangle(LCD_WIDTH_PX + 6, 24 + starty, LCD_WIDTH_PX + 6 + 5, 24 + starty + 8,
                          COLOR_BLACK);
    }
    GetKey(&key);
    set_xcas_status();
    //addStringToInput(s, giac::print_INT_(key).c_str(), &pos, max, &refresh);
    if (isscrolling && !(key == KEY_CTRL_PAGEUP || key == KEY_CTRL_PAGEDOWN || key == KEY_CTRL_UP ||
                         key == KEY_CTRL_DOWN)) {
      isscrolling = 0;
      myconsolescroll = 0;
      ...
      if (strlen(s) != 0) {
        ...
        if (lineerr) {
          for (int i = 0; i < sizeof(xcas_status); ++i)
            xcas_status[i] = 0;
          sprintf(xcas_status, lang ? "Erreur a %s" : "Error at %s",
                  giac::error_token_name(contextptr).c_str());
    ...
    if ((key == KEY_CHAR_STORE || key == KEY_CHAR_PLUS || key == KEY_CHAR_MINUS ||
         key == KEY_CHAR_MULT || key == KEY_CHAR_DIV || key == KEY_CHAR_POW ||
         key == KEY_CHAR_SQUARE) &&
        *s == 0 && firstLoopRun)
      addStringToInput(
          s, "ans()", &pos, max,
          &refresh); //start of line, append "last" as we're going to do a calculation on the previous value
    const char * adds = keytostring(key, keyflag);
    if (adds) {
      bool islist = strcmp(adds, "list ") == 0, ismat = strcmp(adds, "matrix ") == 0;
      if (islist)
        input_matrix(true);
      if (ismat)
        input_matrix(false);
    ...
    if (key == KEY_CTRL_CLIP) {
      ...
      if (sres == MENU_RETURN_SELECTION) {
        ...
        if (smallmenu.selection == 1) {
          ...
          if (!ans.empty()) {
            ...
            DefineStatusMessage((char *)(lang ? "Reponse copiee vers presse-papiers."
                                              : "Last result copied to clipboard"),
                                1, 0, 0);
            DisplayStatusArea();
          } else
            AUX_DisplayErrorMessage(0x15);
      ...
      if (isscrolling) {
        myconsolescroll--;
        if (line_count - visible_lines + myconsolescroll < 0)
          myconsolescroll++;
        dConsoleRedraw();
      } else {
      ...
      if (*s == 0) {
        if (confirm(lang ? "Effacer l'historique?" : "Clear history?",
                    lang ? "F1: oui,     F6: annuler" : "F1: yes,     F6: cancel") == KEY_CTRL_F1) {
          dConsoleCls();
          pos = 0;
          myconsolex = 1;
          myconsoley = 0;
      ...
      *s = 0;
      ...
      return 5;
#endif
    } else if (key != KEY_CTRL_SHIFT && key != KEY_CTRL_ALPHA &&
               get_custom_key_handler_state() == 1) {
      custom_key_to_handle = key;
      custom_key_to_handle_modifier = keyflag;
      return 3;
    }
    firstLoopRun = 0;
  }
...
void draw_menu(int editor) {
  drawFkeyLabels(get_custom_fkey_label(0), get_custom_fkey_label(1), get_custom_fkey_label(2),
                 get_custom_fkey_label(3), 0x0307, get_custom_fkey_label(5));
  ...
  if (editor == 2) {
    ...
    x = fkeyw / 2 - 20;
    PrintMini(&x, &y, (unsigned char *)(py ? ": " : ":="), 0, 0xffffffff, 0, 0, COLOR_BLACK,
              COLOR_WHITE, 1, 0);
    x += 6;
    Bdisp_MMPrint(x, y - 2, "!", 0, 0xffffffff, 0, 0, jaune, COLOR_WHITE, 1, 0);
    x += 18;
  ...
  Bdisp_MMPrint(x, y - 2, (py && editor < 2) ? " % " : "int", 0, 0xffffffff, 0, 0, jaune,
                COLOR_WHITE, 1, 0);
  x += 20;
  ...
  // >
  drawLine(fkeyw + fkeyw / 2, LCD_HEIGHT_PX - 19, fkeyw + fkeyw / 2 + 7, LCD_HEIGHT_PX - 12,
           TEXT_COLOR_BLACK);
  drawLine(fkeyw + fkeyw / 2, LCD_HEIGHT_PX - 18, fkeyw + fkeyw / 2 + 6, LCD_HEIGHT_PX - 12,
           TEXT_COLOR_BLACK);
  drawLine(fkeyw + fkeyw / 2, LCD_HEIGHT_PX - 5, fkeyw + fkeyw / 2 + 7, LCD_HEIGHT_PX - 12,
           TEXT_COLOR_BLACK);
  drawLine(fkeyw + fkeyw / 2, LCD_HEIGHT_PX - 6, fkeyw + fkeyw / 2 + 6, LCD_HEIGHT_PX - 12,
           TEXT_COLOR_BLACK);
  // '
  drawLine(2 * fkeyw + fkeyw / 2, LCD_HEIGHT_PX - 12, 2 * fkeyw + fkeyw / 2 + 6, LCD_HEIGHT_PX - 18,
           TEXT_COLOR_BLACK);
  drawLine(2 * fkeyw + fkeyw / 2, LCD_HEIGHT_PX - 11, 2 * fkeyw + fkeyw / 2 + 6, LCD_HEIGHT_PX - 17,
           TEXT_COLOR_BLACK);
#endif
...
#if 1
  Bdisp_MMPrint(3 * fkeyw, LCD_HEIGHT_PX - STATUS_AREA_PX - 18, " CATALOG", 0, 0xffffffff, 0, 0,
  ...
  if (editor == 2) {
    ...
    Bdisp_MMPrint(5 * fkeyw + 2, LCD_HEIGHT_PX - STATUS_AREA_PX - 18,
                  editor == 2 ? " eval" : (lang ? "Fich,Cfg" : "File,Cfg"), 0, 0xffffffff, 0, 0,
                  COLOR_WHITE, COLOR_BLACK, 1, 0);
  }
#else
...
void dConsolePutChar(char c) {
  ...
  if (line_x >= LINE_COL_MAX) {
    ...
    if (line_count < LINE_ROW_MAX)
      ...
      line_start++;
      if (line_start >= LINE_ROW_MAX)
        line_start = 0;
    }
    line_index++;
    if (line_index >= LINE_ROW_MAX)
      line_index = 0;
  }
  line_buf[line_index][line_x] = '\0';
}
...
bool load_console_state_smem(const char * filename) {
  ...
  Bfile_ReadFile_OS(hFile, line_buf, sizeof(line_row) * LINE_ROW_MAX,
                    -1); // read console scrollback buffer
