...
int fileBrowser(char * filename, const char * filter, const char * title) {
  ...
  while (res) {
    ...
    if (res == 2)
      return 1; // user selected a file
  }
  return 0;
}

int fileBrowserSub(char * browserbasepath, char * filename, const char * filter,
                   const char * title) {
  Menu menu;
  ...
  while (1) {
    ...
    int temptextY = 0;
    PrintMini(&temptextX, &temptextY, friendlypath, 0, 0xFFFFFFFF, 0, 0, COLOR_BLACK, COLOR_WHITE,
              0, 0); // fake draw
    if (temptextX > LCD_WIDTH_PX - 6) {
      ...
      if (strlen(friendlypath) > strlen(newfriendlypath) &&
          strlen(newfriendlypath) >
              3) { // check if len > 3 because shortenDisplayPath may return just "..." when the folder name is too big
  ...
  while (1) {
    ...
    case MENU_RETURN_EXIT:
      if (!strcmp(browserbasepath,
                  "\\\\fls0\\")) { //check that we aren't already in the root folder
...
void buildIconTable(MenuItemIcon * icontable) {
  ...
  const char * bogusFiles[] = {
      "t", // for folder
      "t.g3m", "t.g3e", "t.g3a", "t.g3p", "t.g3b", "t.bmp", "t.txt", "t.csv",
      "t.abc" //for "unsupported" files
  };

  for (int i = 0; i < 10; i++)
    SMEM_MapIconToExt((unsigned char *)bogusFiles[i],
                      (i == 0 ? folder : (unsigned short *)"\x000\x000"), &msgno,
                      icontable[i].data);
}
