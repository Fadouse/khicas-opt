...
namespace xcas {
  ...
  giac::gen Equation_compute_size(const giac::gen & g, const giac::attributs & a, int windowhsize,
  ...
  bool Equation_find_vector_pos(giac::const_iterateur it, giac::const_iterateur itend, int & i,
                                int & nrows);
  bool Equation_adjust_xy(giac::gen & g, int & xleft, int & ytop, int & xright, int & ybottom,
                          giac::gen *& gsel, giac::gen *& gselparent, int & gselpos,
                          std::vector<int> * gotosel = 0);
  ...
  void replace_selection(Equation & eq, const giac::gen & tmp, giac::gen * gsel = 0,
                         const std::vector<int> * gotoptr = 0);
  ...
  void get_axis_angle_deg(const quaternion_double & q, double & x, double & y, double & z,
                          double & theta); // q must be a quaternion of unit norm, theta is in deg
  ...
  inline bool operator<(const int2_double2 & a, const int2_double2 & b) {
    if (a.arg != b.arg)
      return a.arg < b.arg;
...
#define giac3d_default_downdowncolor 22539
  ...
  class Graph2d {
  public:
    double window_xmin, window_xmax, window_ymin, window_ymax, window_zmin, window_zmax, x_scale,
        y_scale, z_scale, x_tick, y_tick, z_tick;
    //double theta_x,theta_y,theta_z;
    quaternion_double q;
    float3d transform[16], invtransform[16];
    // only 12 used, last line [0,0,0,1], usual matrices, not transposed
    int display_mode, show_axes, show_edges, show_names, labelsize, lcdz, default_upcolor,
    ...
    float3d Ai, Aj, Bi, Bj, Ci, Cj, Di, Dj, Ei, Ej, Fi, Fj, Gi, Gj, Hi,
        Hj; // visualization cube coordinates
    ...
    std::vector<int4> hyp_color, plan_color, sphere_color, polyedre_color, line_color, curve_color,
        point_color;
    giac::gen g, trace_x0, trace_x1, trace_x2, trace_y0, trace_y1, trace_y2, trace_parameq,
        trace_z1, trace_z2,
        trace_t; //  normal graph instructions, trace_* are storing cached derivatives for curve study
    const giac::context * contextptr;
    /* geometry data */
    ...
    std::string title, x_axis_name, x_axis_unit, y_axis_name, y_axis_unit, z_axis_name, z_axis_unit,
        fcnfield, fcnvars;
    int npixels; // max # of pixels distance for click
    giac::vecteur plot_instructions, symbolic_instructions, animation_instructions,
        trace_instructions;
    ...
    bool moving_param;
    double param_orig, param_value, param_min, param_max, param_step;
    int nparams;
    int tracemode;
    int tracemode_n;
    double tracemode_i;
    string tracemode_add;
    giac::vecteur tracemode_disp;
    double tracemode_mark;
    /* end geometry data */
    ...
    void geometry_round(double x, double y, double z, double eps, giac::gen & tmp,
                        const giac::context *);
    giac::gen geometry_round(double x, double y, double z, double eps, giac::gen & original,
                             int & pos, bool selectfirstlevel = false, bool setscroller = false);
    ...
    void tracemode_set(
        int operation =
            0); // operation==1 if user is setting the value of t on a parametric curve, operation==2 for root, operation==3 for extremum, operation==4 mark current position, operation=5 for area
    void init_tracemode();
    void curve_infos();
    ...
    bool findij(const giac::gen & e0, double x_scale, double y_scale, double & i0, double & j0,
                const giac::context *) const;
    ...
    void xyz2ij(const float3d3 & d, float3d & i, float3d & j,
                float3d3 & d3) const;                        // d not transformed, d3 is
    ...
    void draw();
    bool glsurface(int w, int h, int lcdz, const giac::context *, int upcolor, int downcolor,
