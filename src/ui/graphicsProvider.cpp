...
#ifdef FILEICON
void CopySpriteMasked(unsigned short * data, int x, int y, int width, int height,
...
void plot(int x0, int y0, unsigned short color) {
  if (x0 < 0 || x0 >= LCD_WIDTH_PX || y0 < 0 || y0 >= LCD_HEIGHT_PX)
    return;
  unsigned short * VRAM = GetVRAMAddress(); //(unsigned short*)VRAM_base;
...
void drawScreenTitle(char * title, char * subtitle) {
  if (title != NULL)
    mPrintXY(1, 1, title, TEXT_MODE_TRANSPARENT_BACKGROUND, TEXT_COLOR_BLUE);
  if (subtitle != NULL)
    mPrintXY(1, 2, subtitle, TEXT_MODE_TRANSPARENT_BACKGROUND, TEXT_COLOR_BLACK);
}
