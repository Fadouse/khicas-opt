  ...
  while (!ret) {
...
#ifdef FILEICON
        if (fileinfo.fsize == 0)
          menuitems[*count].icon =
              FILE_ICON_FOLDER; // it would be a folder icon anyway, because isfolder is true
        else
          menuitems[*count].icon = fileIconFromName((char *)buffer);
#endif
        menuitems[*count].isselected =
            0; //clear selection. this means selection is cleared when changing directory (doesn't happen with native file manager)
    ...
    if (*count == MAX_ITEMS_IN_DIR) {
      ...
      if (files != NULL && menuitems != NULL)
        insertSortFileMenuArray(files, menuitems, *count);
      return GETFILES_MAX_FILES_REACHED; // Don't find more files, the array is full.
    } else
      ret = Bfile_FindNext_NON_SMEM(findhandle, found, (char *)&fileinfo);
  }
  Bfile_FindClose(findhandle);
...
int fileIconFromName(char * name) {
  if (EndsIWith(name, (char *)".g1m") || EndsIWith(name, (char *)".g2m") ||
      EndsIWith(name, (char *)".g3m"))
    return FILE_ICON_G3M;
  else if (EndsIWith(name, (char *)".g1e") || EndsIWith(name, (char *)".g2e") ||
           EndsIWith(name, (char *)".g3e"))
