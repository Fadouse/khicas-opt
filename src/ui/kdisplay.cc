...
namespace xcas {
  ...
  void glinter1(double z, double dz, double * zmin, double * zmax, double ZMIN, double ZMAX, int ih,
                int lcdz, int upcolor, int downcolor, int diffusionz, int diffusionz_limit,
                bool interval) {
    if (ZMIN < z && z < ZMAX)
      return;
  ...
  void glinter(double a, double b, double c, double xscale, double xc, double yscale, double yc,
               double * zmin, double * zmax, double ZMIN, double ZMAX, int i, int horiz, int j,
  ...
  void find_abc(double x1, double x2, double x3, double y1, double y2, double y3, double z1,
                double z2, double z3, double & a, double & b, double & c) {
  ...
  void glinter(double x1, double x2, double x3, double y1, double y2, double y3, double z1,
               double z2, double z3, double xscale, double xc, double yscale, double yc,
               double * zmin, double * zmax, double ZMIN, double ZMAX, int i, int horiz, int j,
               int w, int h, int lcdz, int upcolor, int downcolor, int diffusionz,
               int diffusionz_limit, bool interval) {
    double a, b, c;
  ...
  void update12(bool & found, bool & found2, double x1, double x2, double x3, double y1, double y2,
                double y3, double z1, double z2, double z3, int upcolor, int downcolor,
                int downupcolor, int downdowncolor, double & curx1, double & curx2, double & curx3,
                double & cury1, double & cury2, double & cury3, double & curz1, double & curz2,
                double & curz3, double & cur2x1, double & cur2x2, double & cur2x3, double & cur2y1,
                double & cur2y2, double & cur2y3, double & cur2z1, double & cur2z2, double & cur2z3,
  ...
  void update12(bool & found, bool & found2, double a, double b, double c, double z, int upcolor,
                int downcolor, int downupcolor, int downdowncolor, double & cura1, double & curb1,
                double & curc1, double & curz1, double & cura2, double & curb2, double & curc2,
                double & curz2, int & u, int & d, int & du, int & dd) {
    if (!found || z > curz1) {
  ...
  // 3d demo prototype
  inline void do_transform(const double mat[16], double x, double y, double z, double & X,
                           double & Y, double & Z) {
...
#define ABC3D
  ...
  void grmtv2ij(const Graph2d & gr, const double3 & m, const double3 & v, double t, int & i,
                int & j) {
    double x = m.x + t * v.x;
    double y = m.y + t * v.y;
    double z = m.z + t * v.z;
  ...
  const int4 tabcolorcplx[] = {
      {63488, 47104, 30720, 14336}, {63489, 47105, 30720, 14336}, {63491, 47106, 30721, 14336},
      {63492, 47107, 30722, 14337}, {63494, 47108, 30723, 14337}, {63495, 47109, 30723, 14337},
      {63497, 47110, 30724, 14338}, {63498, 47111, 30725, 14338}, {63500, 47113, 30726, 14339},
      {63501, 47114, 30726, 14339}, {63503, 47115, 30727, 14339}, {63504, 47116, 30728, 14340},
      {63506, 47117, 30729, 14340}, {63507, 47118, 30729, 14340}, {63509, 47119, 30730, 14341},
      {63510, 47120, 30731, 14341}, {63512, 47122, 30732, 14342}, {63513, 47123, 30732, 14342},
      {63515, 47124, 30733, 14342}, {63516, 47125, 30734, 14343}, {63518, 47126, 30735, 14343},
      {63519, 47127, 30735, 14343}, {59423, 45079, 28687, 14343}, {57375, 43031, 28687, 14343},
      {53279, 40983, 26639, 12295}, {51231, 38935, 24591, 12295}, {47135, 34839, 22543, 10247},
      {45087, 32791, 22543, 10247}, {40991, 30743, 20495, 10247}, {38943, 28695, 18447, 8199},
      {64320, 47744, 31136, 14528}, {64224, 47648, 31072, 14496}, {64128, 47584, 31040, 14496},
      {64032, 47520, 30976, 14464}, {63936, 47456, 30944, 14432}, {63840, 47360, 30880, 14400},
      {63744, 47296, 30848, 14400}, {63648, 47232, 30784, 14368}, {63552, 47168, 30752, 14336},
  };
...
#define HYPERQUAD
  ...
  void update_hypertri(const vector<hypertriangle_t> & hypertriangles, double x, double y,
                       bool & found, bool & found2, double3 & curabc1, double & curz1,
                       double3 & curabc2, double & curz2, int & upcolor, int & downcolor,
  ...
  bool Graph2d::glsurface(int w, int h, int lcdz, GIAC_CONTEXT, int upcolor_, int downcolor_,
    ...
    for (int i = imin - horiz; i < imax - horiz; i += w, ++count) {
      ...
      for (int k = 0; k < int(hypv.size()); k += 2) {
        ...
        for (sprec = sbeg, scur = sprec + 1; scur < send; ++sprec, ++scur) {
          ...
          for (itprec += 3, itcur += 3; itprec < itprecend; itprec += 3, itcur += 3) {
            ...
            double x123 = (x1 + x2 + x3 + x4) / 4, y123 = (y1 + y2 + y3 + y4) / 4,
            ...
            if ((yx > yx1 && yx > yx2 && yx > yx3) || (yx < yx1 && yx < yx2 && yx < yx3))
              ...
              double x123 = (x1 + x2 + x3) / 3, y123 = (y1 + y2 + y3) / 3,
            ...
            if ((yx > yx4 && yx > yx2 && yx > yx3) || (yx < yx4 && yx < yx2 && yx < yx3))
              ...
              double x423 = (x4 + x2 + x3) / 3, y423 = (y4 + y2 + y3) / 3,
      ...
      for (int k = 0; k < int(sphere_centerv.size()); ++k) {
        ...
        double m00 = m0[0]._DOUBLE_val, m01 = m0[1]._DOUBLE_val, m02 = m0[2]._DOUBLE_val,
	...
	*/
        double A = 4 * m02 * m02 + 4 * m12 * m12 - 4 * m00 * m22 - 8 * m01 * m22 + 8 * m02 * m12 -
                   4 * m11 * m22,
...
#else
      double curx1, curx2, curx3, cury1, cury2, cury3, curz1 = -1e306, curz2 = -1e306,
                                                       curz3 = -1e306;
      double cur2x1, cur2x2, cur2x3, cur2y1, cur2y2, cur2y3, cur2z1 = -1e306, cur2z2 = -1e306,
                                                             cur2z3 = -1e306;
#endif
      int u, d, du, dd;
      ...
      if (only_hypertri) {
        ...
        for (int j = jmax; j >= jmin; j -= h, x -= yscale * h, y -= yscale * h) {
          ...
          for (int k = 0; k < int(sphere_centerv.size()); ++k) {
            ...
            double a = m2[2]._DOUBLE_val, b = 2 * (m0[2]._DOUBLE_val * v0 + m1[2]._DOUBLE_val * v1),
  ...
  Graph2d::Graph2d(const giac::gen & g_, const giac::context * cptr)
      : window_xmin(gnuplot_xmin), window_xmax(gnuplot_xmax), window_ymin(gnuplot_ymin),
        ...
        display_mode(0x45), show_axes(1), show_edges(1), show_names(1), labelsize(16), precision(1),
  ...
  void Graph2d::adddepth(vector<int2> & polyg, const double3 & A, const double3 & B,
                         int2 & IJmin) const {
      ...
      if (tracemode) {
        if (tracemode_add.size())
          ...
          os_draw_string_small(LCD_WIDTH_PX - 20 -
                               ...
                               LCD_HEIGHT_PX - 14, _WHITE, _BLACK, s);
  ...
  void Graph2d::draw() {
    ...
    if (is3d) {
      ...
      double3 A(window_xmin, window_ymin, window_zmin), B(window_xmin, window_ymin, window_zmax),
  ...
  void Graph2d::tracemode_set(int operation) {
    ...
    if (G.is_symb_of_sommet(at_curve)) {
      ...
      if (operation == 2) { // root near curt
        sol = newton(y, t, curt, NEWTON_DEFAULT_ITERATION, eps, 1e-12, true, tmin._DOUBLE_val,
                     tmax._DOUBLE_val, 1, 0, 1, contextptr);
      ...
      if (operation == 6) { // inflexion
        sol = newton(x1 * y2 - x2 * y1, t, curt, NEWTON_DEFAULT_ITERATION, eps, 1e-12, true,
                     tmin._DOUBLE_val, tmax._DOUBLE_val, 1, 0, 1, contextptr);
        if (sol.type == _DOUBLE_) {
    ...
    if (Gx.type == _DOUBLE_ && Gy.type == _DOUBLE_) {
      tracemode_add +=
          "x=" + print_DOUBLE_(Gx._DOUBLE_val, 3) + ",y=" + print_DOUBLE_(Gy._DOUBLE_val, 3);
      if (tstep != 0) {
  ...
  void Graph2d::curve_infos() {
    if (!tracemode)
      init_tracemode();
    const char * tab[] = {lang == 1 ? "Infos objet (shift-2)" : "Object infos (shift-2)", // 0
#ifdef NUMWORKS
  ...
  void geosave(textArea * text, GIAC_CONTEXT) {
    ...
    if (tmp.type == _IDNT)
      ...
      confirm(lang == 1 ? "Impossible de sauvegarder" : "Unable to save",
              lang == 1 ? "Nom de fichier incompatible" : "Incompatible file name");
  }
    ...
    add(&text,
        lang == 1
            ? "haut/bas/droit/gauche: change point de vue\ny^x ou e^x: trace 3d precis\nEXIT: "
              "quitte ou interrompt le trace 3d en cours\n( et ): modifie le rendu des surfaces "
              "raides 3d\n0: surfaces cachees 3d ON/OFF\n.: remplissage surface 3d raide ON/OFF\n5 "
              "reset 3d view\n7,8,9,1,2,3: deplacement 3d\n\nGeometrie\nF4: change le mode\nLe "
              "mode repere (shift F1) permet de changer le point de vue\nLe mode pointeur (shift "
              "F2) permet de bouger un objet et les objets dependants avec EXE et les touches de "
              "deplacement\nLes autres modes permettent de creer des objets\nEXIT: permet de "
              "passer en vue symbolique et de creer/modifier des objets par des commandes, taper "
              "EXE pour revenir en vue graphique\n4,6: modifie la profondeur du clic"
            : "up/down/right/left: modify viewpoint\nON/Back: leave or interrupt 3d rendering\ny^x "
              ...
              "move 3d view\n\nGeometry\nF4: change geometry mode\nFrame mode (shift F1): modify "
              "viewpoint\nPointer mode (shift F2): select an object and move it with EXE and "
              "cursor keys\nOther modes: create an object\nEXIT: go to symbolic view where you can "
              "create/modify objects with commands, press EXE to go back to graphic view");
    ...
    for (;;) {
      ...
      if (!hp) {
        ...
        os_draw_string(0, LCD_HEIGHT_PX - STATUS_AREA_PX - 17, COLOR_BLACK, COLOR_WHITE,
                       "home: cfg");
#else
      os_draw_string_small(0, LCD_HEIGHT_PX - STATUS_AREA_PX - 17, COLOR_BLACK, COLOR_WHITE,
                           "F1: help");
#endif
      }
      int key = -1;
