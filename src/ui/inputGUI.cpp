...
int doTextInput(textInput * input) {
  ...
  if (input->width == 21) {
    widthForSyscalls = 20;
    clearLine(
        1, input->y); // remove aestethically unpleasing bit of background at the end of the field
  }
  while (1) {
    ...
    if (input->type == INPUTTYPE_DATE || input->type == INPUTTYPE_TIME) {
      ...
      drawLine(input->x * 18 - 18, input->y * 24 - 1, input->x * 18 - 18, input->y * 24 + 23,
               COLOR_GRAY);
      drawLine((input->x * 18 - 18) + 18 * input->width, input->y * 24 - 1,
               (input->x * 18 - 18) + 18 * input->width, input->y * 24 + 23, COLOR_GRAY);
    }
    if (input->type == INPUTTYPE_DATE) {
      ...
      drawLine((input->x * 18 - 18) + 18 * 2, input->y * 24 - 1, (input->x * 18 - 18) + 18 * 2,
               input->y * 24 + 23, COLOR_GRAY);
      drawLine((input->x * 18 - 18) + 18 * 4, input->y * 24 - 1, (input->x * 18 - 18) + 18 * 4,
               input->y * 24 + 23, COLOR_GRAY);
    }
    ...
    if (GetSetupSetting((unsigned int)0x14) == 0x01 ||
      ...
      keyflag =
          GetSetupSetting((unsigned int)0x14); //make sure the flag we're using is the updated one.
    ...
    if (input->key == KEY_CTRL_EXE || (input->key == KEY_CTRL_F6 && input->acceptF6)) {
      ...
      // Aborted
      Cursor_SetFlashOff();
      return INPUT_RETURN_EXIT;
    } else if (input->key == KEY_CTRL_F1 || input->key == KEY_CTRL_F2) {
      Cursor_SetFlashOff();
      return INPUT_RETURN_KEYCODE;
    ...
    if (input->key && input->key < 30000) {
      if (input->type == INPUTTYPE_NORMAL ||
          (input->key >= KEY_CHAR_0 &&
           input->key <= KEY_CHAR_9)) { // either a normal input, or only allow digits
      ...
      EditMBStringCtrl2((unsigned char *)input->buffer, input->charlimit + 1, &input->start,
