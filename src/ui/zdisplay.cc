...
const char * python_keywords[] = {
    ...
    "False", "None", "True",   "and", "break", "continue", "def",   "default",
    "elif",  "else", "except", "for", "from",  "global",   "if",    "import",
    "not",   "or",   "return", "try", "while", "xor",      "yield",
};
const char * const python_builtins[] = {
    "NoneType",     "__call__",     "__class__",   "__delitem__", "__dir__",  "__enter__",
    "__exit__",     "__getattr__",  "__getitem__", "__hash__",    "__init__", "__int__",
    "__iter__",     "__len__",      "__main__",    "__module__",  "__name__", "__new__",
    "__next__",     "__qualname__", "__repr__",    "__setitem__", "__str__",  "abs",
    "all",          "any",          "append",      "args",        "bool",     "builtins",
    "bytearray",    "bytecode",     "bytes",       "callable",    "chr",      "classmethod",
    "clear",        "close",        "const",       "copy",        "count",    "dict",
    "dir",          "divmod",       "end",         "endswith",    "eval",     "exec",
    "extend",       "find",         "format",      "from_bytes",  "get",      "getattr",
    "globals",      "hasattr",      "hash",        "id",          "index",    "insert",
    "int",          "isalpha",      "isdigit",     "isinstance",  "islower",  "isspace",
    "issubclass",   "isupper",      "items",       "iter",        "join",     "key",
    "keys",         "len",          "list",        "little",      "locals",   "lower",
    "lstrip",       "main",         "map",         "micropython", "next",     "object",
    "open",         "ord",          "pop",         "popitem",     "pow",      "print",
    "range",        "read",         "readinto",    "readline",    "remove",   "replace",
    "repr",         "reverse",      "rfind",       "rindex",      "round",    "rsplit",
    "rstrip",       "self",         "send",        "sep",         "set",      "setattr",
    "setdefault",   "sort",         "sorted",      "split",       "start",    "startswith",
    "staticmethod", "step",         "stop",        "str",         "strip",    "sum",
    "super",        "throw",        "to_bytes",    "tuple",       "type",     "update",
    "upper",        "utf-8",        "value",       "values",      "write",    "xcas",
    "zip",
};
...
namespace xcas {
  ...
  bool Equation_box_sizes(const gen & g, int & l, int & h, int & x, int & y, attributs & attr,
                          bool & selected) {
    if (g.type == _EQW) {
      eqwdata & w = *g._EQWptr;
      x = w.x;
  ...
  bool Equation_adjust_xy(gen & g, int & xleft, int & ytop, int & xright, int & ybottom,
                          gen *& gsel, gen *& gselparent, int & gselpos,
                          std::vector<int> * goto_ptr) {
    gsel = 0;
    gselparent = 0;
    gselpos = 0;
    int x, y, w, h;
    ...
    for (; it != itend; ++it) {
      if (Equation_adjust_xy(*it, xleft, ytop, xright, ybottom, gsel, gselparent, gselpos,
                             goto_ptr)) {
        if (goto_ptr) {
  ...
  int eqw_select_leftright(Equation & eq, bool left, int exchange) {
    ...
    gen *gsel, *gselparent;
    vector<int> goto_sel;
    if (Equation_adjust_xy(g, xleft, ytop, xright, ybottom, gsel, gselparent, gselpos, &goto_sel) &&
        gselparent && gselparent->type == _VECT) {
  ...
  void text_print(int fontsize, const char * s, int x, int y, int c = COLOR_BLACK,
                  int bg = COLOR_WHITE, int mode = 0) {
    ...
    if (fontsize >= 16 && ss == 2 && s[0] == char(0xe5) &&
        (s[1] == char(0xea) ||
         s[1] == char(0xeb))) // special handling for increasing and decreasing in tabvar output
      fontsize = 18;
    if (fontsize >= 18) {
  ...
  void fl_pie(int x, int y, int rx, int ry, int theta1_deg, int theta2_deg, int c = COLOR_BLACK,
              bool segment = false) {
  ...
  bool binary_op(const unary_function_ptr & u) {
    const unary_function_ptr binary_op_tab_ptr[] = {*at_plus,
                                                    *at_prod,
                                                    *at_pow,
                                                    *at_and,
                                                    *at_ou,
  ...
  gen Equation_compute_symb_size(const gen & g, const attributs & a, int windowhsize,
                                 GIAC_CONTEXT) {
    if (g.type != _SYMB)
    ...
    if (u == at_inv) {
      if ((is_integer(arg) && is_positive(-arg, contextptr)) || (arg.is_symb_of_sommet(at_neg)))
        return Equation_compute_size(symb_neg(Tfraction<gen>(plus_one, -arg)), a, windowhsize,
                                     contextptr);
    ...
    if (u == at_limit && arg.type == _VECT) { // limit
      ...
      if (s >= 3) {
        ...
        if (s == 4) {
          ...
          res.push_back(
              eqwdata(vf.dx + vx.dx + a.fontsize + 4 + vl.dx + vdir.dx, h, 0, y, a, u, 0));
        } else
    ...
    if ((u == at_of || u == at_at) && arg.type == _VECT && arg._VECTptr->size() == 2) {
      ...
      if (u == at_at && xcas_mode(contextptr) != 0) {
        if (arg2.type == _VECT)
          arg2 =
              gen(addvecteur(*arg2._VECTptr, vecteur(arg2._VECTptr->size(), plus_one)), _SEQ__VECT);
        else
          arg2 = arg2 + plus_one;
      }
    ...
    if (u == at_factorial) {
      ...
      eqwdata vv = Equation_total_size(varg);
      bool paren =
          need_parenthesis(vv.g) || vv.g == at_prod || vv.g == at_division || vv.g == at_pow;
      if (paren)
        x += llp;
      Equation_translate(varg, x, 0);
      Equation_vertical_adjust(vv.dy, vv.y, h, y);
    ...
    if (u != at_sto && u.ptr()->printsommet != NULL && !binaryop) {
      ...
      return Equation_compute_size(symbolic(at_expr, makesequence(tmp, xcas_mode(contextptr))), a,
  ...
  gen Equation_compute_size(const gen & g, const attributs & a, int windowhsize, GIAC_CONTEXT) {
    ...
    if (g.type == _FRAC) {
      if (is_integer(g._FRACptr->num) && is_positive(-g._FRACptr->num, contextptr))
        return Equation_compute_size(symb_neg(fraction(-g._FRACptr->num, g._FRACptr->den)), a,
                                     windowhsize, contextptr);
      ...
      v[2] = eqwdata(w, a.fontsize / 2 + vv1.dy + vv2.dy + 1, 0,
                     (a.fontsize <= 14 ? 4 : 3) - vv2.dy, a, at_division, 0);
      return gen(v, _SEQ__VECT);
    }
    /***************
     *   VECTORS   *
      ...
      if (gmat && g.subtype != _SEQ__VECT && g.subtype != _SET__VECT && g.subtype != _POLY1__VECT &&
        ...
        if (ncols) {
          ...
          for (int i = 0; it != itend; ++it, ++i) {
            ...
            vecteur tmp(
                Equation_subsizes(tmpg, a, max(windowhsize / ncols - a.fontsize, 230), contextptr));
          ...
          all_sizes.push_back(eqwdata(col_widths.back(), row_heights.back(), 0, -row_heights.back(),
  ...
  inline void check_fl_rectf(int x, int y, int w, int h, int imin, int jmin, int di, int dj,
                             int delta_i, int delta_j, int c) {
  ...
  void Equation_draw(const gen & g, int x, int y, int rightx, int lowery, Equation * equat) {
    ...
    if (oper.type == _FUNC) {
      ...
      if (u == at_at) {  // draw brackets around 2nd arg
        ...
        if (x0 < rightx)
          text_print(fontsize, "[", eqx + x0 - x - int(text_width(fontsize, ("["))),
                     eqy + y - varg2.baseline, text_color, background, mode);
        x0 += varg2.dx;
        if (x0 < rightx)
          text_print(fontsize, "]", eqx + x0 - x, eqy + y - varg2.baseline, text_color, background,
                     mode);
        return;
      }
      if (u == at_of) {  // do we need to draw some parenthesis?
        ...
        if (arg2.type != _VECT || arg2._VECTptr->back().type != _EQW ||
          ...
          x0 += varg2.dx;
          if (x0 < rightx)
            text_print(pfontsize, ")", eqx + x0 - x, eqy + y - varg2.baseline, text_color,
                       background, mode);
        }
        return;
      }
        ...
        if (oper.subtype != _SEQ__VECT && oper.subtype != _PRINT__VECT) {
          ...
          if (eqx + x0 - x + 1 >= 0) {
            draw_line(eqx + x0 - x + 1, eqy + y - y0 + 1, eqx + x0 - x + 1, eqy + y - y1 + 1,
                      draw_line_color);
            draw_line(eqx + x0 - x + decal, eqy + y - y0 + 1, eqx + x0 - x + decal,
                      eqy + y - y1 + 1, draw_line_color);
            draw_line(eqx + x0 - x + 1, eqy + y - y0 + 1, eqx + x0 - x + fontsize / 4,
                      eqy + y - y0 + 1, draw_line_color);
            draw_line(eqx + x0 - x + 1, eqy + y - y1 + 1, eqx + x0 - x + fontsize / 4,
                      eqy + y - y1 + 1, draw_line_color);
          }
          x0 += w.dx;
      ...
      if (u == at_pow) {
        ...
        if (tmp.x + tmp.dx < rightx)
          text_print(fontsize, ")", eqx + tmp.x + tmp.dx - x, eqy + y - tmp.baseline, text_color,
                     background, mode);
        return;
      }
      if (u == at_program) {
      ...
      if (u == at_sum) {
        if (x0 < rightx) {
          ...
          draw_line(eqx + x0, eqy + y0 - fontsize, eqx + x0 + (2 * fontsize) / 3,
                    eqy + y0 - fontsize, draw_line_color);
          draw_line(eqx + x0, eqy + y0, eqx + x0 + fontsize / 2, eqy + y0 - fontsize / 2,
                    draw_line_color);
      ...
      if (u == at_abs) {
        ...
        if (x0 < rightx) {
          ...
          draw_line(eqx + x0 + w.dx - 1, eqy + y0 - 1, eqx + x0 + w.dx - 1, eqy + y0 - h + 3,
                    draw_line_color);
          draw_line(eqx + x0 + w.dx, eqy + y0 - 1, eqx + x0 + w.dx, eqy + y0 - h + 3,
                    draw_line_color);
        }
        return;
      }
      if (u == at_sqrt) {
        ...
        int h = w.dy;
        if (x0 < rightx) {
          draw_line(eqx + x0 + 2, eqy + y0 - h / 2, eqx + x0 + fontsize / 2, eqy + y0 - 1,
                    draw_line_color);
      ...
      if (u == at_factorial) {
        text_print(fontsize, "!", eqx + w.x + w.dx - 4 - x, eqy + y - w.baseline, text_color,
                   background, mode);
        if (!need_parenthesis(tmp.g) && tmp.g != at_pow && tmp.g != at_prod && tmp.g != at_division)
          return;
        if (tmp.x - lpsize < rightx)
      ...
      if (u == at_integrate) {
        ...
        if (x0 < rightx) {
          ...
          draw_line(eqx + x0 + fontsize / 3, eqy + y0, eqx + x0 + fontsize / 3,
                    eqy + y0 - 2 * fontsize + 4, draw_line_color);
          fl_arc(eqx + x0 + fontsize / 3, eqy + y0 - 2 * fontsize + 3, fontsize / 3, fontsize / 3,
                 0, 180, draw_line_color);
        }
        if (v.size() != 2) { // if arg has size > 1 draw the d
          ...
          if (ptmp.x + ptmp.dx < rightx)
            text_print(fontsize, " dx", eqx + ptmp.x + ptmp.dx - x, eqy + y - ptmp.baseline,
                       text_color, background, mode);
        }
        return;
      }
      ...
      if (u == at_limit && v.size() >= 4) {
        if (x0 < rightx)
          text_print(fontsize, "lim", eqx + w.x - x, eqy + y - w.baseline, text_color, background,
                     mode);
        gen arg2 = v[1]; // 2nd arg of limit, i.e. the variable
        if (arg2.type == _EQW) {
          eqwdata & varg2 = *arg2._EQWptr;
          if (varg2.x + varg2.dx + 2 < rightx)
            text_print(fontsize, "\x1e", eqx + varg2.x + varg2.dx + 2 - x, eqy + y - varg2.y,
                       text_color, background, mode);
        }
        if (v.size() >= 5) {
      ...
      if ((itend - it == 1) && ((u == at_neg) || (u == at_plus) // uncommented for +infinity
        ...
        if (w.x < rightx) {
          text_print(fontsize, u.ptr()->s, eqx + w.x - x, eqy + y - w.baseline, text_color,
                     background, mode);
        }
        return;
      }
      ...
      if (u == at_plus && tmp.g != at_equal)
        ...
        if (parenthesis && need_parenthesis(tmp.g)) {
          if (w.x < rightx) {
            ...
            text_print(pfontsize, "(", eqx + w.x - x, eqy + y - tmp.baseline, text_color,
                       background, mode);
          }
        }
      }
      for (;;) {
        ...
        if (parenthesis && need_parenthesis(tmp.g)) {
          if (xx < rightx) {
            int pfontsize =
                min(max(fontsize, (fontsize + (tmp.baseline - tmp.y)) / 2), fontsize * 2);
            int deltapary = (2 * (pfontsize - fontsize)) / 3;
            text_print(pfontsize, ")", eqx + xx, eqy + y - tmp.baseline + deltapary, text_color,
                       background, mode);
          }
          xx += rpsize;
        }
        ...
        if (u == at_prod) {
          ...
          gen tmpgen;
          if (u == at_plus && ((it->type == _VECT && it->_VECTptr->back().type == _EQW &&
                                it->_VECTptr->back()._EQWptr->g == at_neg) ||
                               (it->type == _EQW &&
            ...
            if (xx + 1 < rightx)
              ...
              text_print(fontsize, opstring.c_str(), eqx + xx + 1, eqy + y - tmp.baseline,
                         text_color, background, mode);
          }
        }
      ...
      if (w.x < rightx) {
        s = u.ptr()->s;
        s += '(';
        text_print(fontsize, s.c_str(), eqx + w.x - x, eqy + y - w.baseline, text_color, background,
                   mode);
      }
      if (w.x + w.dx - rpsize < rightx)
        text_print(fontsize, ")", eqx + w.x + w.dx - x - rpsize + 2, eqy + y - w.baseline,
                   text_color, background, mode);
      return;
    }
  ...
  Equation::Equation(int x_, int y_, const gen & g) {
    ...
    if (taille(g, max_prettyprint_equation) < max_prettyprint_equation)
      ...
      data = Equation_compute_size(string2gen("Object_too_large", false), attr, LCD_WIDTH_PX,
                                   contextptr);
  ...
  inline bool inside(double x0, double x1, double x2, double y0, double y1, double y2, double x,
                     double y) {
#if 1
  ...
  void quaternion_double_to_euler_deg(const quaternion_double & q, double & phi, double & theta,
                                      double & psi) {
  ...
  // q must be a unit
  void get_axis_angle_deg(const quaternion_double & q, double & x, double & y, double & z,
                          double & theta) {
  ...
  void rotate(double rx, double ry, double rz, double theta, double x, double y, double z,
  ...
  void glinter1(float3d z, float3d dz, float3d * zmin, float3d * zmax, float3d ZMIN, float3d ZMAX,
                int ih, int lcdz, int upcolor, int downcolor, int diffusionz, int diffusionz_limit,
                bool interval) {
    if (ZMIN < z && z < ZMAX)
      return;
      ...
      if (diffus) {
        if (z <= *zmin) {
          ...
          drawRectangle(ih, z, 1, std::ceil(*zmin - z),
                        diffuse(upcolor, std::min(float3d(diffusionz), std::max(-dz, 1.0))));
          color = upcolor;
          *zmin = z;
        }
        if (z >= *zmax) {
          ...
          drawRectangle(ih, *zmax, 1, std::ceil(z - *zmax),
                        diffuse(downcolor, std::min(float3d(diffusionz), std::max(dz, 1.0))));
          *zmax = z;
        }
        return;
      }
  ...
  void glinter(float3d a, float3d b, float3d c, float3d xscale, float3d xc, float3d yscale,
               float3d yc, float3d * zmin, float3d * zmax, float3d ZMIN, float3d ZMAX, int i,
               int horiz, int j, int w, int h, int lcdz, int upcolor, int downcolor, int diffusionz,
               int diffusionz_limit, bool interval) {
    ...
    for (int I = i; I < i + w; ++I, ++zmax, ++zmin) {
      ...
      if ( //x-(h-1)*yscale>xmin && y-(h-1)*yscale>ymin &&
          z >= 0 && z + (h - 1) * dz >= 0 && z <= LCD_HEIGHT_PX &&
          z + (h - 1) * dz <= LCD_HEIGHT_PX) {
        int color = -1;
          ...
          if (diffusionz < diffusionz_limit) {
            if (dz > 0)
              drawRectangle(ih, z, 1, std::ceil(h * dz),
                            diffuse(color, std::min(float3d(diffusionz), std::max(dz, 1.0))));
            else
  ...
  void find_abc(float3d x1, float3d x2, float3d x3, float3d y1, float3d y2, float3d y3, float3d z1,
                float3d z2, float3d z3, float3d & a, float3d & b, float3d & c) {
  ...
  void glinter(float3d x1, float3d x2, float3d x3, float3d y1, float3d y2, float3d y3, float3d z1,
               float3d z2, float3d z3, float3d xscale, float3d xc, float3d yscale, float3d yc,
               float3d * zmin, float3d * zmax, float3d ZMIN, float3d ZMAX, int i, int horiz, int j,
               int w, int h, int lcdz, int upcolor, int downcolor, int diffusionz,
               int diffusionz_limit, bool interval) {
    float3d a, b, c;
    find_abc(x1, x2, x3, y1, y2, y3, z1, z2, z3, a, b, c);
    glinter(a, b, c, xscale, xc, yscale, yc, zmin, zmax, ZMIN, ZMAX, i, horiz, j, w, h, lcdz,
            upcolor, downcolor, diffusionz, diffusionz_limit, interval);
  }

  void update12(bool & found, bool & found2, float3d x1, float3d x2, float3d x3, float3d y1,
                float3d y2, float3d y3, float3d z1, float3d z2, float3d z3, int upcolor,
                int downcolor, int downupcolor, int downdowncolor, float3d & curx1, float3d & curx2,
                float3d & curx3, float3d & cury1, float3d & cury2, float3d & cury3, float3d & curz1,
                float3d & curz2, float3d & curz3, float3d & cur2x1, float3d & cur2x2,
                float3d & cur2x3, float3d & cur2y1, float3d & cur2y2, float3d & cur2y3,
                float3d & cur2z1, float3d & cur2z2, float3d & cur2z3, int & u, int & d, int & du,
                int & dd) {
    //cout << "update12\n";
    if (found) {
  ...
  void update12(bool & found, bool & found2, float3d a, float3d b, float3d c, float3d z,
                int upcolor, int downcolor, int downupcolor, int downdowncolor, float3d & cura1,
                float3d & curb1, float3d & curc1, float3d & curz1, float3d & cura2, float3d & curb2,
                float3d & curc2, float3d & curz2, int & u, int & d, int & du, int & dd) {
    if (!found || z > curz1) {
  ...
  // 3d demo prototype
  inline void do_transform(const float3d mat[16], float3d x, float3d y, float3d z, float3d & X,
                           float3d & Y, float3d & Z) {
  ...
  bool get_colors(gen attr, int & upcolor, int & downcolor, int & downupcolor,
                  int & downdowncolor) {
...
#define ABC3D
  ...
  void grmtv2ij(const Graph2d & gr, const float3d3 & m, const float3d3 & v, float3d t, int & i,
                int & j) {
    float3d x = m.x + t * v.x;
    float3d y = m.y + t * v.y;
    float3d z = m.z + t * v.z;
  ...
  //const
  int4 tabcolorcplx[] = {
      int4(63488, 47104, 30720, 14336), int4(63489, 47105, 30720, 14336),
      int4(63491, 47106, 30721, 14336), int4(63492, 47107, 30722, 14337),
      int4(63494, 47108, 30723, 14337), int4(63495, 47109, 30723, 14337),
      int4(63497, 47110, 30724, 14338), int4(63498, 47111, 30725, 14338),
      int4(63500, 47113, 30726, 14339), int4(63501, 47114, 30726, 14339),
      int4(63503, 47115, 30727, 14339), int4(63504, 47116, 30728, 14340),
      int4(63506, 47117, 30729, 14340), int4(63507, 47118, 30729, 14340),
      int4(63509, 47119, 30730, 14341), int4(63510, 47120, 30731, 14341),
      int4(63512, 47122, 30732, 14342), int4(63513, 47123, 30732, 14342),
      int4(63515, 47124, 30733, 14342), int4(63516, 47125, 30734, 14343),
      int4(63518, 47126, 30735, 14343), int4(63519, 47127, 30735, 14343),
      int4(59423, 45079, 28687, 14343), int4(57375, 43031, 28687, 14343),
      int4(53279, 40983, 26639, 12295), int4(51231, 38935, 24591, 12295),
      int4(47135, 34839, 22543, 10247), int4(45087, 32791, 22543, 10247),
      int4(40991, 30743, 20495, 10247), int4(38943, 28695, 18447, 8199),
      int4(34847, 26647, 16399, 8199),  int4(32799, 24599, 16399, 8199),
      int4(28703, 22551, 14351, 6151),  int4(26655, 20503, 12303, 6151),
      int4(22559, 16407, 10255, 4103),  int4(20511, 14359, 10255, 4103),
      int4(16415, 12311, 8207, 4103),   int4(14367, 10263, 6159, 2055),
      int4(10271, 8215, 4111, 2055),    int4(8223, 6167, 4111, 2055),
      int4(4127, 4119, 2063, 7),        int4(2079, 2071, 15, 7),
      int4(2079, 2071, 2063, 2055),     int4(2175, 2135, 2095, 2055),
      int4(2271, 2199, 2159, 2087),     int4(2367, 2263, 2191, 2119),
      int4(2463, 2359, 2255, 2151),     int4(2559, 2423, 2287, 2151),
      int4(2655, 2487, 2351, 2183),     int4(2751, 2551, 2383, 2215),
      int4(2847, 2647, 2447, 2247),     int4(2943, 2711, 2479, 2247),
      int4(3039, 2775, 2543, 2279),     int4(3135, 2839, 2575, 2311),
      int4(3231, 2935, 2639, 2343),     int4(3327, 2999, 2671, 2343),
      int4(3423, 3063, 2735, 2375),     int4(3519, 3127, 2767, 2407),
      int4(3615, 3223, 2831, 2439),     int4(3711, 3287, 2863, 2439),
      int4(3807, 3351, 2927, 2471),     int4(3903, 3415, 2959, 2503),
      int4(3999, 3511, 3023, 2535),     int4(4063, 3575, 3055, 2535),
      int4(4061, 3574, 3054, 2535),     int4(4060, 3573, 3054, 2535),
      int4(4058, 3572, 3053, 2534),     int4(4057, 3571, 3052, 2534),
      int4(4055, 3569, 3051, 2533),     int4(4054, 3568, 3051, 2533),
      int4(4052, 3567, 3050, 2533),     int4(4051, 3566, 3049, 2532),
      int4(4049, 3565, 3048, 2532),     int4(4048, 3564, 3048, 2532),
      int4(4046, 3563, 3047, 2531),     int4(4045, 3562, 3046, 2531),
      int4(4043, 3560, 3045, 2530),     int4(4042, 3559, 3045, 2530),
      int4(4040, 3558, 3044, 2530),     int4(4039, 3557, 3043, 2529),
      int4(4037, 3556, 3042, 2529),     int4(4036, 3555, 3042, 2529),
      int4(4034, 3554, 3041, 2528),     int4(4033, 3553, 3040, 2528),
      int4(4032, 3552, 3040, 2528),     int4(4032, 3552, 992, 480),
      int4(8128, 5600, 3040, 480),      int4(10176, 7648, 5088, 2528),
      int4(14272, 9696, 7136, 2528),    int4(16320, 11744, 7136, 2528),
      int4(20416, 13792, 9184, 4576),   int4(22464, 15840, 11232, 4576),
      int4(26560, 19936, 13280, 6624),  int4(28608, 21984, 13280, 6624),
      int4(32704, 24032, 15328, 6624),  int4(34752, 26080, 17376, 8672),
      int4(38848, 28128, 19424, 8672),  int4(40896, 30176, 19424, 8672),
      int4(44992, 32224, 21472, 10720), int4(47040, 34272, 23520, 10720),
      int4(51136, 38368, 25568, 12768), int4(53184, 40416, 25568, 12768),
      int4(57280, 42464, 27616, 12768), int4(59328, 44512, 29664, 14816),
      int4(63424, 46560, 31712, 14816), int4(65472, 48608, 31712, 14816),
      int4(65376, 48512, 31648, 14784), int4(65280, 48448, 31616, 14784),
      int4(65184, 48384, 31552, 14752), int4(65088, 48320, 31520, 14720),
      int4(64992, 48224, 31456, 14688), int4(64896, 48160, 31424, 14688),
      int4(64800, 48096, 31360, 14656), int4(64704, 48032, 31328, 14624),
      int4(64608, 47936, 31264, 14592), int4(64512, 47872, 31232, 14592),
      int4(64416, 47808, 31168, 14560), int4(64320, 47744, 31136, 14528),
      int4(64224, 47648, 31072, 14496), int4(64128, 47584, 31040, 14496),
      int4(64032, 47520, 30976, 14464), int4(63936, 47456, 30944, 14432),
      int4(63840, 47360, 30880, 14400), int4(63744, 47296, 30848, 14400),
      int4(63648, 47232, 30784, 14368), int4(63552, 47168, 30752, 14336),
  };
...
#define HYPERQUAD
  ...
  void compute(float3d yx, float3d3 * cur, hypertriangle_t & res) {
    ...
    res.ymin = ymin;
    res.ymax = ymax;
    find_abc(cur[0].x, cur[1].x, cur[2].x, cur[0].y, cur[1].y, cur[2].y, cur[0].z, cur[1].z,
             cur[2].z, res.a, res.b, res.c);
  }
  ...
  void update_hypertri(const vector<hypertriangle_t> & hypertriangles, float3d x, float3d y,
                       bool & found, bool & found2, float3d3 & curabc1, float3d & curz1,
                       float3d3 & curabc2, float3d & curz2, int & upcolor, int & downcolor,
                       int & downupcolor, int & downdowncolor) {
    vector<hypertriangle_t>::const_iterator it = hypertriangles.begin(),
                                            itend = hypertriangles.end();
    for (; it != itend; ++it) {
      if (x < it->xmin) {
        ++it;
        if (it == itend)
  ...
  bool Graph2d::glsurface(int w, int h, int lcdz, GIAC_CONTEXT, int upcolor_, int downcolor_,
                          int downupcolor_, int downdowncolor_) {
    if (g.type == _INT_ ||
        (g.type == _VECT && g._VECTptr->size() == 1 && g._VECTptr->front().type == _INT_)) {
    ...
    short int *jmintab = (short int *)alloca(jmintabsize * sizeof(short int)),
              *jmaxtab = (short int *)alloca(jmintabsize *
                                             sizeof(short int)); // assumes width3d<=jmintabsize
    ...
    float3d xmin = -1, ymin = -1, xmax = 1, ymax = 1, xscale = 0.6 * (xmax - xmin) / horiz,
            yscale = 0.6 * (ymax - ymin) / vert, x, y, z, xc = (xmin + xmax) / 2,
            yc = (ymin + ymax) / 2;
    ...
    for (int i = imin - horiz; i < imax - horiz; i += w, ++count) {
      ...
      drawRectangle(i + horiz, STATUS_AREA_PX, w, LCD_HEIGHT_PX - STATUS_AREA_PX,
                    COLOR_BLACK); // clear
      ...
      for (int k = 0; k < int(hypv.size()); k += 2) {
        bool cplx =
            hyp_color[k].u < 0 || hyp_color[k].d < 0 || hyp_color[k].du < 0 || hyp_color[k].dd < 0;
        ...
        for (sprec = sbeg, scur = sprec + 1; scur < send; ++sprec, ++scur) {
          ...
          for (itprec += 3, itcur += 3; itprec < itprecend; itprec += 3, itcur += 3) {
            ...
            yx4 = y4 - x4;
#ifdef HYPERQUAD
            if (precision <= 3 && (discard(this, x1, y1, z1) || discard(this, x2, y2, z2) ||
                                   discard(this, x3, y3, z3)))
              continue;
            tri[0] = float3d3(x1, y1, z1);
            tri[1] = float3d3(x2, y2, z2);
            tri[2] = float3d3(x4, y4, z4);
            tri[3] = float3d3(x3, y3, z3);
            float3d x123 = (x1 + x2 + x3 + x4) / 4, y123 = (y1 + y2 + y3 + y4) / 4,
                    z123 = (z1 + z2 + z3 + z4) / 4, X, Y, Z;
            ...
            if ((yx > yx1 && yx > yx2 && yx > yx3) || (yx < yx1 && yx < yx2 && yx < yx3))
              ; // not intersecting
            else {
              float3d x123 = (x1 + x2 + x3) / 3, y123 = (y1 + y2 + y3) / 3,
                      z123 = (z1 + z2 + z3) / 3, X, Y, Z;
              float3d xy123 = x123 + y123;
              if (xy123 < hyperxymin)
                hyperxymin = xy123;
              if (xy123 > hyperxymax)
            ...
            if ((yx > yx4 && yx > yx2 && yx > yx3) || (yx < yx4 && yx < yx2 && yx < yx3))
              ; // not intersecting
            else {
              float3d x423 = (x4 + x2 + x3) / 3, y423 = (y4 + y2 + y3) / 3,
                      z423 = (z4 + z2 + z3) / 3, X, Y, Z;
      ...
      for (int k = 0; k < int(sphere_centerv.size()); ++k) {
        ...
        float3d m00 = m0[0]._DOUBLE_val, m01 = m0[1]._DOUBLE_val, m02 = m0[2]._DOUBLE_val,
                m11 = m1[1]._DOUBLE_val, m12 = m1[2]._DOUBLE_val, m22 = m2[2]._DOUBLE_val;
	...
	*/
        float3d A = 4 * m02 * m02 + 4 * m12 * m12 - 4 * m00 * m22 - 8 * m01 * m22 + 8 * m02 * m12 -
                    4 * m11 * m22,
                B = -8 * m00 * m12 * xc + 8 * m00 * m12 * yc - 8 * m00 * m12 * yx +
                    8 * m01 * m02 * xc - 8 * m01 * m02 * yc + 8 * m01 * m02 * yx -
                    8 * m01 * m12 * xc + 8 * m01 * m12 * yc - 8 * m01 * m12 * yx +
                    8 * m02 * m11 * xc - 8 * m02 * m11 * yc + 8 * m02 * m11 * yx,
...
#else
      float3d curx1, curx2, curx3, cury1, cury2, cury3, curz1 = -1e306, curz2 = -1e306,
                                                        curz3 = -1e306;
      float3d cur2x1, cur2x2, cur2x3, cur2y1, cur2y2, cur2y3, cur2z1 = -1e306, cur2z2 = -1e306,
                                                              cur2z3 = -1e306;
#endif
      int u, d, du, dd;
      ...
      if (only_hypertri) {
        if (hypertriangles.empty())
          goto suite3d;
        int effjmax = (hyperxymax - xc - yc) / yscale / 2.0,
            effjmin = (hyperxymin - xc - yc) / yscale / 2.0;
        if (effjmax + 1 < jmax)
          jmax = effjmax + 1;
        if (effjmin - 1 > jmin)
          jmin = effjmin - 1;
        ...
        for (int j = jmax; j >= jmin; j -= h, x -= yscale * h, y -= yscale * h) {
          ...
          update_hypertri(hypertriangles, x, y, found, found2, curabc1, curz1, curabc2, curz2,
          ...
          if (h == 1 && w == 1) {
            if (found2 && !hide2nd) {
              ...
              z = LCD_HEIGHT_PX / 2 + j - lcdz * z;
              glinter1(z, dz, zmin2, zmax2, zmin[0], zmax[0], ih, lcdz, downupcolor, downdowncolor,
            ...
            glinter1(z, dz, zmin, zmax, 1e307, -1e307, ih, lcdz, upcolor, downcolor, diffusionz,
                     diffusionz_limit, interval);
          } else {
            if (found2 && !hide2nd)
              glinter(curabc2.x, curabc2.y, curabc2.z, xscale, xc, yscale, yc, zmin2, zmax2,
                      zmin[0], zmax[0], i, horiz, j, w, h, lcdz, downupcolor, downdowncolor,
                      diffusionz, diffusionz_limit, interval);
            glinter(curabc1.x, curabc1.y, curabc1.z, xscale, xc, yscale, yc, zmin, zmax, 1e307,
                    -1e307, i, horiz, j, w, h, lcdz, upcolor, downcolor, diffusionz,
                    diffusionz_limit, interval);
          }
        }
      } // end only_hypertri
        ...
        for (int j = jmax; j >= jmin; j -= h, x -= yscale * h, y -= yscale * h) {
          ...
          for (int ki = 0; ki < int(polyedrei.size()); ++ki) {
            ...
            if (
#if 1
                x >= polyedrexmin[ki] && x <= polyedrexmax[ki] && y >= polyedreymin[ki] &&
                y <= polyedreymax[ki]
#else
                inside(cur, x, y)
#endif
              ...
              if (!is_clipped) {
                ...
                is_clipped = X >= window_xmin && X <= window_xmax && Y >= window_ymin &&
                             Y <= window_ymax && Z >= window_zmin && Z <= window_zmax;
              }
              if (is_clipped) {
#ifdef ABC3D
                update12(found, found2, a, b, c, z, color.u, color.d, color.du, color.dd, curabc1.x,
                         curabc1.y, curabc1.z, curz1, curabc2.x, curabc2.y, curabc2.z, curz2,
                         upcolor, downcolor, downupcolor, downdowncolor);
#else
                update12(found, found2, x - .5, x - .5, x + 1, y + 0.866, y - 0.866, y,
                         z - .5 * a + .866 * b, z - .5 * a - .866 * b, z + a, color.u, color.d,
                         color.du, color.dd, curx1, curx2, curx3, cury1, cury2, cury3, curz1, curz2,
                         curz3, cur2x1, cur2x2, cur2x3, cur2y1, cur2y2, cur2y3, cur2z1, cur2z2,
                         cur2z3, upcolor, downcolor, downupcolor, downdowncolor);
#endif
              }
          ...
          for (int k = 0; k < int(sphere_centerv.size()); ++k) {
            ...
            float3d a = m2[2]._DOUBLE_val,
                    b = 2 * (m0[2]._DOUBLE_val * v0 + m1[2]._DOUBLE_val * v1),
                    C = (m0[0]._DOUBLE_val * v0 + 2 * m0[1]._DOUBLE_val * v1) * v0 +
            ...
            if (is_clipped) {
              ...
              update12(found, found2, a, b, c, z, color.u, color.d, color.du, color.dd, curabc1.x,
                       curabc1.y, curabc1.z, curz1, curabc2.x, curabc2.y, curabc2.z, curz2, upcolor,
                       downcolor, downupcolor, downdowncolor);
#else
              update12(found, found2,
                       //x-w2,x,x,y,y,y-w2,z+w0,z,z+w1,
                       x - 0.5, x - .5, x + 1, y + .866, y - .866, y,
                       z + .5 * w0 / w2 - .866 * w1 / w2, z + .5 * w0 / w2 + .866 * w1 / w2,
                       z - w0 / w2, color.u, color.d, color.du, color.dd, curx1, curx2, curx3,
                       cury1, cury2, cury3, curz1, curz2, curz3, cur2x1, cur2x2, cur2x3, cur2y1,
                       cur2y2, cur2y3, cur2z1, cur2z2, cur2z3, upcolor, downcolor, downupcolor,
                       downdowncolor);
#endif
            }
            if (delta <= 0)
              continue; // delta==0, twice the same point
            v2 = sol2;
            z = v2 + c.z;
...
#ifdef ABC3D
              update12(found, found2, abc.x, abc.y, abc.z, z, color.u, color.d, color.du, color.dd,
                       curabc1.x, curabc1.y, curabc1.z, curz1, curabc2.x, curabc2.y, curabc2.z,
                       curz2, upcolor, downcolor, downupcolor, downdowncolor);
#else
              update12(found, found2, x - 1, x, x, y, y, y + 1, z - abc.x, z, z + abc.y, color.u,
                       color.d, color.du, color.dd, curx1, curx2, curx3, cury1, cury2, cury3, curz1,
                       curz2, curz3, cur2x1, cur2x2, cur2x3, cur2y1, cur2y2, cur2y3, cur2z1, cur2z2,
                       cur2z3, upcolor, downcolor, downupcolor, downdowncolor);
#endif
          } // end hyperplan loop
          if (found) {
            ...
            if (found2) {
              if (!hide2nd)
                glinter(cur2x1, cur2x2, cur2x3, cur2y1, cur2y2, cur2y3, cur2z1, cur2z2, cur2z3,
                        xscale, xc, yscale, yc, zmin2, zmax2, zmin[0], zmax[0], i, horiz, j, w, h,
                        lcdz, downupcolor, downdowncolor, diffusionz, diffusionz_limit, interval);
              glinter(curx1, curx2, curx3, cury1, cury2, cury3, curz1, curz2, curz3, xscale, xc,
                      yscale, yc, zmin, zmax, 1e307, -1e307, i, horiz, j, w, h, lcdz, upcolor,
                      downcolor, diffusionz, diffusionz_limit, interval);
            } else
              glinter(curx1, curx2, curx3, cury1, cury2, cury3, curz1, curz2, curz3, xscale, xc,
                      yscale, yc, zmin, zmax, 1e307, -1e307, i, horiz, j, w, h, lcdz, upcolor,
                      downcolor, diffusionz, diffusionz_limit, interval);
    ...
    for (int j = 0; j < linetypev.size(); j++) {
      ...
      float3d tmin_ = usetmax ? tmin : tmax, tmax_ = usetmax ? tmin : tmax;
      ...
      for (int k = 0; k < sphere_centerv.size(); ++k) {
        ...
        float3d a = vx * vx + vy * vy + vz * vz,
                b = (vx * (mx - cx) + vy * (my - cy) + vz * (mz - cz)),
                C = (mx - cx) * (mx - cx) + (my - cy) * (my - cy) + (mz - cz) * (mz - cz) - R * R;
        ...
        if (surf.is_symb_of_sommet(at_hypersurface)) {
          ...
          if (hyp.size() > 2 && !is_undef(hyp[1])) {
            ...
            if (_is_polynomial(makesequence(eq, vars[0]), contextptr) == 1 &&
              ...
              gen gradeq =
                  subst(derive(eq, vars, contextptr), vars, V, false, contextptr); // not used
      ...
      if (tmin < tmin_) {
        ...
        if (tmin_ != tmax)
          os_fill_rect(i2 - 2, j2 - 2, 4, 4, curcolor);
      }
      if (tmax > tmax_) {
        ...
        if (tmax_ != tmin)
          os_fill_rect(i1 - 2, j1 - 2, 4, 4, curcolor);
      }
    }
  ...
  bool geoparse(textArea * text, GIAC_CONTEXT) {
    ...
    for (; i < int(v.size()); ++i) {
      ...
      if (lineerr) {
        ...
        if (pos >= 0)
          sprintf(status, (lang == 1) ? "Erreur ligne %i a %s" : "Error line %i at %s", i + 1,
                  tok.c_str());
        else
          sprintf(status, (lang == 1) ? "Erreur ligne %i %s" : "Error line %i %s", i + 1,
                  (pos == -2 ? ((lang == 1) ? ", : manquant ?" : ", missing :?") : ""));
        if (confirm(status, (lang == 1) ? "OK: corrige, back: continue" : "OK: fix", 1) ==
            KEY_CTRL_F1) {
          text->line = i;
          if (pos >= 0 && pos < v[i].s.size())
            text->pos = pos;
          return true;
        }
      }
  ...
  Graph2d::Graph2d(const giac::gen & g_, const giac::context * cptr)
      : window_xmin(gnuplot_xmin), window_xmax(gnuplot_xmax), window_ymin(gnuplot_ymin),
        window_ymax(gnuplot_ymax), window_zmin(gnuplot_zmin), window_zmax(gnuplot_zmax), g(g_),
        display_mode(0x45), show_axes(1), show_edges(1), show_names(1), labelsize(16), precision(2),
        contextptr(cptr), hp(0), npixels(5), couleur(0), nparams(0) {
    tracemode = 0;
    tracemode_n = 0;
    tracemode_i = 0;
    current_i = width3d / 3;
  ...
  void inv4(float3d * c, float3d * res) {
    res[0] = c[5] * c[10] * c[15] - c[5] * c[14] * c[11] - c[10] * c[7] * c[13] -
             c[15] * c[9] * c[6] + c[14] * c[9] * c[7] + c[11] * c[6] * c[13];
    res[1] = -c[1] * c[10] * c[15] + c[1] * c[14] * c[11] + c[10] * c[3] * c[13] +
             c[15] * c[9] * c[2] - c[14] * c[9] * c[3] - c[11] * c[2] * c[13];
    ...
    res[4] = -c[4] * c[10] * c[15] + c[4] * c[14] * c[11] + c[10] * c[7] * c[12] +
             c[15] * c[8] * c[6] - c[14] * c[8] * c[7] - c[11] * c[6] * c[12];
    res[5] = c[0] * c[10] * c[15] - c[0] * c[14] * c[11] - c[10] * c[3] * c[12] -
             c[15] * c[8] * c[2] + c[14] * c[8] * c[3] + c[11] * c[2] * c[12];
    res[6] = -c[0] * c[6] * c[15] + c[0] * c[14] * c[7] + c[6] * c[3] * c[12] +
             c[15] * c[4] * c[2] - c[14] * c[4] * c[3] - c[7] * c[2] * c[12];
    res[7] = c[0] * c[6] * c[11] - c[0] * c[10] * c[7] - c[6] * c[3] * c[8] - c[11] * c[4] * c[2] +
             c[10] * c[4] * c[3] + c[7] * c[2] * c[8];
    res[8] = c[4] * c[9] * c[15] - c[4] * c[13] * c[11] - c[9] * c[7] * c[12] -
             c[15] * c[8] * c[5] + c[13] * c[8] * c[7] + c[11] * c[5] * c[12];
    res[9] = -c[0] * c[9] * c[15] + c[0] * c[13] * c[11] + c[9] * c[3] * c[12] +
             c[15] * c[8] * c[1] - c[13] * c[8] * c[3] - c[11] * c[1] * c[12];
    res[10] = c[0] * c[5] * c[15] - c[0] * c[13] * c[7] - c[5] * c[3] * c[12] -
              c[15] * c[4] * c[1] + c[13] * c[4] * c[3] + c[7] * c[1] * c[12];
    res[11] = -c[0] * c[5] * c[11] + c[0] * c[9] * c[7] + c[5] * c[3] * c[8] + c[11] * c[4] * c[1] -
              c[9] * c[4] * c[3] - c[7] * c[1] * c[8];
    res[12] = -c[4] * c[9] * c[14] + c[4] * c[13] * c[10] + c[9] * c[6] * c[12] +
              c[14] * c[8] * c[5] - c[13] * c[8] * c[6] - c[10] * c[5] * c[12];
    res[13] = c[0] * c[9] * c[14] - c[0] * c[13] * c[10] - c[9] * c[2] * c[12] -
              c[14] * c[8] * c[1] + c[13] * c[8] * c[2] + c[10] * c[1] * c[12];
    res[14] = -c[0] * c[5] * c[14] + c[0] * c[13] * c[6] + c[5] * c[2] * c[12] +
              c[14] * c[4] * c[1] - c[13] * c[4] * c[2] - c[6] * c[1] * c[12];
  ...
  void Graph2d::update_rotation() {
    ...
    float3d a11 = invr2 * (rx * rx + ry * ry * c + rz * rz * c),
            a12 = invr2 * (rx * ry - rx * ry * c - rz * s * r),
            a13 = invr2 * (rx * rz - rx * rz * c + ry * s * r);
    float3d a21 = invr2 * (rx * ry - rx * ry * c + rz * s * r),
            a22 = invr2 * (ry * ry + rx * rx * c + rz * rz * c),
            a23 = invr2 * (ry * rz - rx * s * r - ry * rz * c);
    float3d a31 = invr2 * (rx * rz - rx * rz * c - ry * s * r),
            a32 = invr2 * (ry * rz + rx * s * r - ry * rz * c),
            a33 = invr2 * (rz * rz + rx * rx * c + ry * ry * c);
    float3d xt = (window_xmin + window_xmax) / 2, xs = 2.0 / (window_xmax - window_xmin),
            yt = (window_ymin + window_ymax) / 2, ys = 2.0 / (window_ymax - window_ymin),
            zt = (window_zmin + window_zmax) / 2, zs = 2.0 / (window_zmax - window_zmin);
    ...
    for (int i = 0; i < v.size(); ++i) {
      int u = default_upcolor, d = default_downcolor, du = default_downupcolor,
          dd = default_downdowncolor;
      const char * ptr = 0;
        ...
        if (ckmatrix(f)) {
          ...
          for (int k = 0; k < n; ++k) {
            ...
            if (P.type == _VECT && P._VECTptr->size() == 3) {
              float3d X, Y, Z;
              do_transform(transform, P[0]._DOUBLE_val, P[1]._DOUBLE_val, P[2]._DOUBLE_val, X, Y,
                           Z);
      ...
      bool line = G.subtype == _LINE__VECT, halfline = G.subtype == _HALFLINE__VECT,
           segment = G.subtype == _GROUP__VECT;
      if (G.type == _VECT && G._VECTptr->size() >= 2 && (line || halfline || segment)) {
        for (int n = 1; n < G._VECTptr->size(); ++n) {
          gen a = evalf_double((*G._VECTptr)[n - 1], 1, contextptr),
              b = evalf_double((*G._VECTptr)[n], 1, contextptr);
          if (a.type == _VECT && b.type == _VECT && a._VECTptr->size() == 3 &&
              b._VECTptr->size() == 3) {
      ...
      if (G.is_symb_of_sommet(at_hypersphere)) {
        ...
        matrice qmat(makevecteur(makevecteur(mat[0], mat[4], mat[8]),
        ...
        bool isclipped = x >= window_xmin + r && x <= window_xmax - r && y >= window_ymin + r &&
                         y <= window_ymax - r && z >= window_zmin + r && z <= window_zmax - r;
        ...
        if (h.type == _VECT && h.subtype == _POLYEDRE__VECT)
          ...
          for (int j = 0; j < V.size(); ++j) {
            ...
            for (int k = 0; k < vj.size(); k += 3) {
              float3d X, Y, Z;
              if (cplx)
                do_transform(mat, vj[k]._DOUBLE_val, vj[k + 1]._DOUBLE_val,
                             absarg(vj[k + 2], argcplx), X, Y, Z);
              else
                do_transform(mat, vj[k]._DOUBLE_val, vj[k + 1]._DOUBLE_val, vj[k + 2]._DOUBLE_val,
                             X, Y, Z);
          ...
          if (g.type == _VECT) {
            ...
            for (int k = 0; k < w.size(); ++k) {
              ...
              if (P.type == _VECT && P._VECTptr->size() == 3) {
                ...
                if (is_clipped && (x < window_xmin || x > window_xmax || y < window_ymin ||
                                   y > window_ymax || z < window_zmin || z > window_zmax))
                  is_clipped = false;
                float3d X, Y, Z;
                do_transform(transform, x, y, z, X, Y, Z);
            ...
            if (cur.size() >= 3) {
              ...
              for (; l < cur.size() - 2; ++l) {
                ...
                float3d X3 = Y1 * Z2 - Y2 * Z1, Y3 = X2 * Z1 - X1 * Z2;
                Z3 = X1 * Y2 - X2 * Y1;
                float3d prec = 1e-3;
#if 1
              ...
              polyedrev.push_back(vector<float3d3>(0));
  ...
  bool Graph2d::findij(const gen & e0, double x_scale, double y_scale, double & i0, double & j0,
                       GIAC_CONTEXT) const {
    gen e, f0, f1;
  ...
  void check_fl_draw(int fontsize, const char * ch, int i0, int j0, int imin, int jmin, int di,
                     int dj, int delta_i, int delta_j, int c) {
    /* int n=fl_size();
  ...
  inline void check_fl_point(int i0, int j0, int imin, int jmin, int di, int dj, int delta_i,
                             int delta_j, int c) {
  ...
  inline void check_fl_line(int i0, int j0, int i1, int j1, int imin, int jmin, int di, int dj,
                            int delta_i, int delta_j, int c) {
  ...
  void checklog_fl_line(double i0, double j0, double i1, double j1, double deltax, double deltay,
                        bool logx, bool logy, double window_xmin, double x_scale,
                        double window_ymax, double y_scale, int c) {
    if (!logx && !logy) {
  ...
  void draw_legende(const vecteur & f, int i0, int j0, int labelpos, const Graph2d * iptr,
                    int clip_x, int clip_y, int clip_w, int clip_h, int deltax, int deltay, int c) {
    ...
    check_fl_draw(fontsize, legendes.c_str(), i0 + dx, j0 + dy, clip_x, clip_y, clip_w, clip_h,
                  deltax, deltay, c);
  }
  ...
  void petite_fleche(double i1, double j1, double dx, double dy, int deltax, int deltay, int width,
    ...
    if (dxy) {
      ...
      fl_polygon(round(i1) + deltax, round(j1) + deltay, round(i1 + dx + dxp) + deltax,
                 round(j1 + dy + dyp) + deltay, round(i1 + dx - dxp) + deltax,
                 round(j1 + dy - dyp) + deltay, c);
    }
  }
  ...
  void fltk_point(int deltax, int deltay, int i0, int j0, int epaisseur_point, int type_point,
    ...
    switch (type_point) {
    case 1: // losange
      fl_line(deltax + i0 - epaisseur_point, deltay + j0, deltax + i0,
              deltay + j0 - epaisseur_point, c);
      fl_line(deltax + i0, deltay + j0 - epaisseur_point, deltax + i0 + epaisseur_point,
              deltay + j0, c);
      fl_line(deltax + i0 - epaisseur_point, deltay + j0, deltax + i0,
              deltay + j0 + epaisseur_point, c);
      fl_line(deltax + i0, deltay + j0 + epaisseur_point, deltax + i0 + epaisseur_point,
    ...
    case 2: // croix verticale
      fl_line(deltax + i0, deltay + j0 - epaisseur_point, deltax + i0,
              deltay + j0 + epaisseur_point, c);
      fl_line(deltax + i0 - epaisseur_point, deltay + j0, deltax + i0 + epaisseur_point,
              deltay + j0, c);
      break;
    case 3: // carre
      fl_line(deltax + i0 - epaisseur_point, deltay + j0 - epaisseur_point,
              deltax + i0 - epaisseur_point, deltay + j0 + epaisseur_point, c);
      fl_line(deltax + i0 + epaisseur_point, deltay + j0 - epaisseur_point,
              deltax + i0 + epaisseur_point, deltay + j0 + epaisseur_point, c);
      fl_line(deltax + i0 - epaisseur_point, deltay + j0 - epaisseur_point,
              deltax + i0 + epaisseur_point, deltay + j0 - epaisseur_point, c);
  ...
  void fltk_draw(Graph2d & Mon_image, const gen & g, double x_scale, double y_scale, int clip_x,
    ...
    if (s == at_parameter && fs >= 4 && f[0].type == _IDNT) {
      ...
      if (Mon_image.pushed && Mon_image.moving_param) {
        ...
        os_draw_string_small(65, ypos - fheight - 2, _WHITE, _BLACK,
                             f[1].print(contextptr).c_str());
        os_draw_string_small(193, ypos - fheight - 2, _WHITE, _BLACK,
                             f[2].print(contextptr).c_str());
    ...
    if (s == at_pnt) {
      ...
      if (point.type == _VECT && point.subtype == _POINT__VECT)
        return;
      if ((f[0].type == _SYMB) && (f[0]._SYMBptr->sommet == at_curve) &&
          (f[0]._SYMBptr->feuille.type == _VECT) && (f[0]._SYMBptr->feuille._VECTptr->size())) {
        ...
        if (type_line >= 4 && point.type == _VECT && point._VECTptr->size() > 2) {
          ...
          if (Mon_image.findij(v[vs], x_scale, y_scale, i0, j0, contextptr) &&
            ...
            checklog_fl_line(i0, j0, i1, j1, deltax, deltay, logx, logy, Mon_image.window_xmin,
                             x_scale, Mon_image.window_ymax, y_scale, couleur);
      ...
      if (point.type == _SYMB) {
        if (point._SYMBptr->sommet == at_cercle) {
          ...
          gen e1 = diametre._VECTptr->front().evalf_double(1, contextptr),
              e2 = diametre._VECTptr->back().evalf_double(1, contextptr);
          gen centre = rdiv(e1 + e2, 2.0, contextptr);
          gen e12 = e2 - e1;
          double ex = evalf_double(re(e12, contextptr), 1, contextptr)._DOUBLE_val,
                 ey = evalf_double(im(e12, contextptr), 1, contextptr)._DOUBLE_val;
          ...
          if ((diam.type == _DOUBLE_) && (a1.type == _DOUBLE_) && (a2.type == _DOUBLE_)) {
            ...
            if (fill_polygon)
              fl_pie(deltax + round(i0 - i1), deltay + round(j0 - j1), round(2 * i1), round(2 * j1),
                     full ? 0 : anglei * 180 / M_PI + .5, full ? 360 : anglef * 180 / M_PI + .5,
                     couleur, false);
            else {
              ...
              if (v.size() >= 4) { // if cercle has the optionnal 5th arg
                if (v[3] == 2)
                  petite_fleche(i0 + i1 * std::cos(anglem), j0 - j1 * std::sin(anglem),
                                -i1 * std::sin(anglem), -j1 * std::cos(anglem), deltax, deltay,
                                width, couleur);
                else {
                  if (changer_sens)
            ...
            if (!hidden_name)
              draw_legende(f, round(i0), round(j0), labelpos, &Mon_image, clip_x, clip_y, clip_w,
                           clip_h, 0, 0, couleur);
            return;
          }
        } // end circle
      ...
      if (point.type != _VECT ||
          (point.type == _VECT &&
           (point.subtype == _GROUP__VECT || point.subtype == _VECTOR__VECT) &&
           point._VECTptr->size() == 2 &&
           is_zero(point._VECTptr->back() - point._VECTptr->front()))) { // single point
        if (!Mon_image.findij((point.type == _VECT ? point._VECTptr->front() : point), x_scale,
                              y_scale, i0, j0, contextptr))
          return;
      ...
      if (fill_polygon) {
        if (jtend - jt == 5 && *(jt + 4) == *jt) {
          ...
          if (cote1 && horiz_or_vert(jt + 1, contextptr) == 3 - cote1 &&
              horiz_or_vert(jt + 2, contextptr) == cote1 &&
              horiz_or_vert(jt + 3, contextptr) == 3 - cote1) {
      ...
      if (jt == jtend) {
        if (i0 > 0 && i0 < mxw && j0 > 0 && j0 < myw)
          check_fl_point(deltax + round(i0), deltay + round(j0), clip_x, clip_y, clip_w, clip_h, 0,
                         0, couleur);
        if (!hidden_name)
          draw_legende(f, round(i0), round(j0), labelpos, &Mon_image, clip_x, clip_y, clip_w,
                       clip_h, 0, 0, couleur);
      ...
      bool seghalfline = (point.subtype == _LINE__VECT || point.subtype == _HALFLINE__VECT) &&
                         (point._VECTptr->size() == 2);
      // rest of the path
      for (;;) {
      ...
      if (seghalfline) {
        ...
        if (point.subtype == _LINE__VECT) {
          if (deltai == 0)
            checklog_fl_line(i1, 0, i1, clip_h, deltax, deltay, logx, logy, Mon_image.window_xmin,
                             x_scale, Mon_image.window_ymax, y_scale, couleur);
          else {
            if (deltaj == 0)
              ...
              if (pts.size() >= 2)
                checklog_fl_line(pts[0].real(), pts[0].imag(), pts[1].real(), pts[1].imag(), deltax,
                                 deltay, Mon_image.display_mode & 0x400,
                                 Mon_image.display_mode & 0x800, Mon_image.window_xmin, x_scale,
                                 Mon_image.window_ymax, y_scale, couleur);
  ...
  void Graph2d::adddepth(vector<int2> & polyg, const float3d3 & A, const float3d3 & B,
                         int2 & IJmin) const {
  ...
  void Graph2d::addpolyg(vector<int2> & polyg, float3d x, float3d y, float3d z,
                         int2 & IJmin) const {
    int I, J;
  ...
  void Graph2d::draw_decorations(const gen & title_tmp) {
    if (args_tmp.empty()) { // add selected names
      ...
      if (tracemode) {
        ...
        os_draw_string_small(LCD_WIDTH_PX - 20 -
                                 os_draw_string_small(0, 0, _WHITE, _BLACK, s, true),
                             tracemode ? 24 : LCD_HEIGHT_PX - 14, _WHITE, _BLACK, s);
    }
    if (mode && title.size() < 100 && (!title.empty() || !is_zero(title_tmp))) {
      ...
      if (!is_zero(title_tmp) && function_final.type == _FUNC) {
        if (function_final == at_point && title_tmp.is_symb_of_sommet(at_point))
          ...
          mytitle = function_final._FUNCptr->ptr()->s +
                    ('(' + title_tmp.print(contextptr) +
      ...
      if (!mytitle.empty()) {
        ...
        if (dt > LCD_WIDTH_PX)
          dt = LCD_WIDTH_PX;
        os_draw_string_small((LCD_WIDTH_PX - dt) / 2, LCD_HEIGHT_PX - 14, _WHITE, _BLACK,
                             mytitle.c_str());
      }
    }
  ...
  void displaypolyg(const vector<int2> & polyg, const int2 & IJmin, int color, int & Px, int & Py,
                    GIAC_CONTEXT) {
    if (polyg.empty())
      return;
  ...
  void Graph2d::draw() {
    ...
    if (is3d) {
      ...
      if (lang == 1)
        ...
        statuslinemsg("DEL: stop");
      float3d3 A(window_xmin, window_ymin, window_zmin), B(window_xmin, window_ymin, window_zmax),
          C(window_xmax, window_ymin, window_zmin), D(window_xmax, window_ymin, window_zmax),
          E(window_xmin, window_ymax, window_zmin), F(window_xmin, window_ymax, window_zmax),
          G(window_xmax, window_ymax, window_zmin), H(window_xmax, window_ymax, window_zmax);
      ...
      if (mode == 0 && precision < 3)
        precision += 2;
      //do_confirm("draw 2");
      glsurface(precision, precision, lcdz, contextptr, default_upcolor, default_downcolor,
                default_downupcolor, default_downdowncolor);
      //do_confirm("draw 3");
      precision = prec;
      clear_abort();
      if (show_edges) {
    ...
    clip_ymin = STATUS_AREA_PX;
    int horizontal_pixels = LCD_WIDTH_PX, vertical_pixels = LCD_HEIGHT_PX - STATUS_AREA_PX,
        deltax = 0, deltay = STATUS_AREA_PX, clip_x = 0, clip_y = 0, clip_w = horizontal_pixels,
        clip_h = vertical_pixels;
    ...
    if (show_axes && (window_ymax >= 0) && (window_ymin <= 0)) { // X-axis
      ...
      int affs;
      char ch[256];
      check_fl_line(deltax, deltay + j_0, deltax + horizontal_pixels, deltay + j_0, clip_x, clip_y,
                    clip_w, clip_h, 0, 0, _GREEN);
      check_fl_line(deltax + i_0, deltay + j_0, deltax + i_0 + int(x_scale), deltay + j_0, clip_x,
                    clip_y, clip_w, clip_h, 0, 0, _CYAN);
      ...
      check_fl_draw(labelsize, "x", deltax + horizontal_pixels - 40, deltay + j_0 - 4, clip_x,
                    clip_y, clip_w, clip_h, 0, 0, _GREEN);
    }
    if (show_axes && (window_xmax >= 0) && (window_xmin <= 0)) { // Y-axis
      vecteur aff;
      int affs;
      char ch[256];
      check_fl_line(deltax + i_0, deltay, deltax + i_0, deltay + vertical_pixels, clip_x, clip_y,
                    clip_w, clip_h, 0, 0, _RED);
      check_fl_line(deltax + i_0, deltay + j_0, deltax + i_0, deltay + j_0 - int(y_scale), clip_x,
                    clip_y, clip_w, clip_h, 0, 0, _CYAN);
      ...
      for (int j = 0; j < affs; ++j) {
        ...
        if (delta >= taille && delta <= vertical_pixels - taille) {
          fl_line(deltax + i_0, STATUS_AREA_PX + delta, deltax + i_0 + 4, STATUS_AREA_PX + delta,
                  _RED);
        }
      }
      check_fl_draw(labelsize, "y", deltax + i_0 + 2, deltay + labelsize, clip_x, clip_y, clip_w,
                    clip_h, 0, 0, _RED);
    }
    ...
    if (show_axes) {
      ...
      for (int i = 0; i < affs; ++i) {
        ...
        taille = strlen(ch) * 9;
        fl_line(delta, vertical_pixels + STATUS_AREA_PX - 6, delta,
                vertical_pixels + STATUS_AREA_PX - 1, _GREEN);
      ...
      for (int j = 0; j < affs; ++j) {
        ...
        if (delta >= taille && delta <= vertical_pixels - taille) {
          ...
          text_print(10, ch, horizontal_pixels - strlen(ch) * 9, STATUS_AREA_PX + delta + taille,
                     _RED);
        }
      }
    }
  ...
  int displaygraph(const giac::gen & ge, const gen & gs, GIAC_CONTEXT) {
    ...
    if (gs != 0)
      gr.symbolic_instructions = gen2vecteur(gs);
  ...
  vecteur Graph2d::get_current_animation() const {
    if (animation_instructions_pos >= 0 &&
        animation_instructions_pos < animation_instructions.size())
    ...
    for (size_t i = start; i < symbolic_instructions.size(); ++i) {
      ...
      const int maxtrace = 512;
        ...
        if (trace_instructions.size() >= maxtrace)
          trace_instructions.erase(trace_instructions.begin(),
                                   trace_instructions.begin() + maxtrace / 2);
  ...
  void Graph2d::find_xyz(double i, double j, double k, double & x, double & y, double & z) const {
    if (is3d) {                                  // FIXME
      ...
      double xmin = -1, ymin = -1, xmax = 1, ymax = 1, xscale = 0.6 * (xmax - xmin) / horiz,
             yscale = 0.6 * (ymax - ymin) / vert;
  ...
  gen geometry_round_numeric(double x, double y, double z, double eps, bool approx) {
    return gen(approx
                   ? makevecteur(x, y, z)
                   : makevecteur(exact_double(x, eps), exact_double(y, eps), exact_double(z, eps)),
               _POINT__VECT);
  }
  ...
  giac::gen add_attributs(const giac::gen & g, int couleur_, GIAC_CONTEXT) {
    ...
    return symbolic(g._SYMBptr->sommet, (v.size() == 1 && f.type != _VECT)
                                            ? f
                                            : gen(v, f.type == _VECT ? f.subtype : _SEQ__VECT));
  }
  ...
  void Graph2d::tracemode_set(int operation) {
    ...
    for (int i = 0; i < plot_instructions.size(); ++i) {
      gen g = plot_instructions[i];
      if (g.type == _VECT && !g._VECTptr->empty() &&
          g._VECTptr->front().is_symb_of_sommet(at_curve)) {
        vecteur & v = *g._VECTptr;
        for (int j = 0; j < v.size(); ++j)
    ...
    if (G.is_symb_of_sommet(at_curve)) {
      ...
      if (operation == -1) {
        ...
        if ((x != t && c.type == _VECT && c._VECTptr->size() > 7 &&
          ...
          if (x == t)
            curve_infos1 = "Function " + y.print(contextptr);
          else
            curve_infos1 = "Parametric " + x.print(contextptr) + "," + y.print(contextptr);
          curve_infos2 = t.print(contextptr) + "=" + tmin.print(contextptr) + ".." +
                         tmax.print(contextptr) + ',';
      ...
      if (operation == 2) { // root near curt
        sol = newton(y, t, curt, NEWTON_DEFAULT_ITERATION, eps, 1e-12, true, tmin._DOUBLE_val,
                     tmax._DOUBLE_val, tmin._DOUBLE_val, tmax._DOUBLE_val, 1, contextptr);
      ...
      if (operation == 4) { // horizontal tangent near curt
        sol = newton(y1, t, curt, NEWTON_DEFAULT_ITERATION, eps, 1e-12, true, tmin._DOUBLE_val,
                     tmax._DOUBLE_val, tmin._DOUBLE_val, tmax._DOUBLE_val, 1, contextptr);
        if (sol.type == _DOUBLE_) {
          confirm(lang == 1 ? "y'=0, extremum/pt singulier en" : "y'=0, extremum/singular pt at",
                  sol.print(contextptr).c_str());
          sto(sol, gen("Extremum", contextptr), contextptr);
        }
      }
      if (operation == 5) { // vertical tangent near curt
        if (x1 == 1)
          do_confirm(lang == 1 ? "Outil pour courbes parametriques!"
                               : "Tool for parametric curves!");
        else {
          sol = newton(x1, t, curt, NEWTON_DEFAULT_ITERATION, eps, 1e-12, true, tmin._DOUBLE_val,
                       tmax._DOUBLE_val, tmin._DOUBLE_val, tmax._DOUBLE_val, 1, contextptr);
          if (sol.type == _DOUBLE_) {
      ...
      if (operation == 6) { // inflexion
        sol = newton(x1 * y2 - x2 * y1, t, curt, NEWTON_DEFAULT_ITERATION, eps, 1e-12, true,
                     tmin._DOUBLE_val, tmax._DOUBLE_val, tmin._DOUBLE_val, tmax._DOUBLE_val, 1,
                     contextptr);
        if (sol.type == _DOUBLE_) {
      ...
      gen M(put_attributs(_point(subst(parameq, t, tracemode_mark, false, contextptr), contextptr),
                          vecteur(1, _POINT_WIDTH_4 | _BLUE), contextptr));
    ...
    if (operation == 3) { // intersect this curve with all other curves
      ...
      if (!V.empty()) {
        ...
        for (int i = 0; i < V.size(); ++i) {
          ...
          if (i == 0) {
            I1 = cur;
            d1 = distance2pp(I1, G, contextptr);
            continue;
          }
          if (i == 1) {
            I2 = cur;
            d2 = distance2pp(I2, G, contextptr);
    ...
    if (Gx.type == _DOUBLE_ && Gy.type == _DOUBLE_) {
      ...
      if (implicit) {
        ...
        gen T(_droite(
            makesequence(_point(G, contextptr), _point(G + m[0] + cst_i * m[1], contextptr)),
            contextptr));
        m = m[1] / m[0];
        if (m.type == _DOUBLE_) {
          tracemode_add += " ,m=";
      ...
      if (tstep != 0) {
        ...
        if (curt.type == _DOUBLE_) {
          ...
          if (tracemode & 2) {
            ...
            gen T(_vector(makesequence(_point(G, contextptr), _point(G + G1t, contextptr),
                                       symb_equal(at_display, _BLUE)),
                          contextptr));
            tracemode_disp.push_back(T);
            if (tracemode & 0xc) {
              ...
              if (tracemode & 4) {
                gen N(_vector(makesequence(_point(G, contextptr), _point(centre, contextptr),
                                           symb_equal(at_display, _BLUE)),
                              contextptr));
              ...
              if (tracemode & 8) {
                ...
                tracemode_disp.push_back(
                    _cercle(makesequence(centre, R, symb_equal(at_display, _BLUE)), contextptr));
              }
            }
          }
        }
  ...
  void Graph2d::curve_infos() {
    if (!tracemode)
      init_tracemode();
    const char * tab[] = {lang == 1 ? "Infos objet (F2)" : "Object infos (F2)", // 0
  ...
  void Graph2d::set_mode(const giac::gen & f_tmp, const giac::gen & f_final, int m,
                         const string & help) {
    approx = true;
    mode = m;
    selected.clear();
  ...
  void Graph2d::geometry_round(double x, double y, double z, double eps, gen & tmp, GIAC_CONTEXT) {
    tmp = is3d ? geometry_round_numeric(x, y, z, eps, approx)
               : geometry_round_numeric(x, y, eps, approx);
    selected = nearest_point(plot_instructions,
                             is3d ? geometry_round_numeric(x, y, z, eps, true)
                                  : geometry_round_numeric(x, y, eps, true),
                             eps, contextptr);
  ...
  gen Graph2d::geometry_round(double x, double y, double z, double eps, gen & original, int & pos,
                              bool selectfirstlevel, bool setscroller) {
    if (!hp)
      return undef;
    gen tmp;
    pos = -1;
    ...
    if (selected.empty())
      return tmp;
    if (function_final == at_areaatraw || function_final == at_areaat ||
        function_final == at_perimeteratraw || function_final == at_perimeterat) {
    ...
    if (g.is_symb_of_sommet(at_plus) && g._SYMBptr->feuille.type == _VECT &&
        !g._SYMBptr->feuille._VECTptr->empty())
  ...
  int Graph2d::geo_handle(int event, int key) {
    ...
    if (in_area && ((mode != 1 && event == FL_DRAG) || event == FL_PUSH || event == FL_RELEASE ||
      ...
      tmp = geometry_round(newx, newy, newz, eps, tmp2, pos,
                           mode == 0 ||
                               (args_tmp.size() == mode && function_final.type == _FUNC &&
    ...
    if (mode == 0 || mode == 255) {
      ...
      if (moving_param && (event == FL_DRAG || event == FL_RELEASE)) {
        ...
        if (param_step <= 0)
          ...
          gen newval =
              symbolic(at_element, makesequence(symb_interval(param_min, param_max), param_value));
      ...
      if (moving && (event == FL_DRAG || event == FL_RELEASE)) {
        ...
        if (in_area && key != KEY_CTRL_EXIT) {
          if (drag_original_value.is_symb_of_sommet(at_plus) &&
            ...
            newval = symbolic(
                at_plus,
                makevecteur(v[0], symbolic(at_nop, ratnormal(_plus(vecteur(v.begin() + 1, v.end()),
                                                                   contextptr) +
                                                             decal))));
          } else {
            newval =
                is_zero(decal)
                    ? drag_original_value
                    : symbolic(at_plus, makevecteur(drag_original_value, symbolic(at_nop, decal)));
          }
        }
    ...
    if (mode == 1) {
      ...
      if (event == FL_RELEASE) {
        ...
        if (hp && !args_tmp.empty() &&
          ...
          if (in_area && args_tmp.front().is_symb_of_sommet(at_point)) {
            ...
            set_gen_value(
                -1,
                symbolic(at_sto,
                         makevecteur(add_attributs(args_tmp.front(), couleur, contextptr), val1)),
                false);
          ...
          if (in_area) {
            gen tmp3 =
                add_attributs(symbolic(at_segment, makevecteur(val1, val2)), couleur, contextptr);
        ...
        if (tmp.type == _IDNT && tmp2.type == _SYMB &&
          ...
          gen tmp3 = symbolic(
              at_element,
              ((t.type < _IDNT || t.type == _VECT) ? gen(makevecteur(tmp, t), _SEQ__VECT) : tmp));
      ...
      if (mode > 1 && s >= mode) {
        ...
        if (in_area && function_final.type == _FUNC) {
          ...
          for (int i = 0; i < ifinal; ++i) {
            ...
            if (tmp_plot.is_symb_of_sommet(at_point)) {
              tmp_plot = symbolic(
                  at_sto, makevecteur(add_attributs(tmp_plot, couleur, contextptr), objname));
              args_tmp[i] = objname;
        ...
        try {
...
#endif
          tmp_plot =
              symbolic(at_sto, makevecteur(add_attributs(tmp_plot, couleur, contextptr), objname));
          set_gen_value(hp_pos, tmp_plot, false);
          add_entry(hp_pos + 1);
          ++hp_pos;
          if (res.is_symb_of_sommet(at_pnt)) {
            ...
            int ns = 0;
            if (res.type == _VECT && res.subtype == _GROUP__VECT &&
                (ns = res._VECTptr->size()) > 2) {
              vecteur l;
              ...
              if (function_final.type == _FUNC &&
                ...
                for (int i = 1; i <= ns; ++i) {
                  ...
                  set_gen_value(
                      hp_pos,
                      symbolic(at_sto, makevecteur(add_attributs(tmp_plot, couleur, contextptr),
                                                   gen(autoname(contextptr) + print_INT_(ns + i),
                                                       contextptr))),
                      false);
                  add_entry(hp_pos + 1);
                  ++hp_pos;
                ...
                for (int i = mode; i < ns; ++i) {
                  tmp_plot = symb_at(symbolic(at_sommets, gen(autoname(contextptr), contextptr)), i,
                                     contextptr);
  ...
  void Graph2d::update_g() {
    if (hp) {
      ...
      if (!is_undef(plot_tmp))
        v.push_back(plot_tmp);
      // geometry: update g from plot_instructions
      g = mergevecteur(selected.empty() ? plot_instructions
                                        : mark_selected(plot_instructions, selected, is3d),
                       v);
      if (is3d)
        update_rotation();
    }
  ...
  void geohelp() {
    ...
    add(&text,
        lang == 1
            ? "x,theta,t: infos courbes (uniquement en 2d)\nhaut/bas/droit/gauche: deplace "
              "pointeur ou change point de vue\nSIN: axes ON/OFF\n*: "
              "autoscale\n/:orthonormalise\n+- zoom\n\n3d\ny^x ou e^x: trace 3d precis\nF2: "
              "+rapide/-precis\nF3: -rapide/+precis\nEXIT: quitte ou interrompt le trace 3d en "
              "cours\n( et ): modifie le rendu des surfaces raides 3d\n0: surfaces cachees 3d "
              "ON/OFF\n.: remplissage surface 3d raide ON/OFF\n5 reset 3d view\n7,8,9,1,2,3: "
              "deplacement 3d\n\nGeometrie\nF4: change le mode\nLe mode repere (shift F1) permet "
              "de changer le point de vue\nLe mode pointeur (shift F2) permet de bouger un objet "
              "et les objets dependants avec EXE et les touches de deplacement\nLes autres modes "
              "permettent de creer des objets\nEXIT: permet de passer en vue symbolique et de "
              "creer/modifier des objets par des commandes, taper EXE pour revenir en vue "
              "graphique\n4,6: modifie la profondeur du clic"
            : "x,theta,t: curves infos (2d only)\nup/down/right/left: move pointer or modify "
              "viewpoint\nSIN: axes ON/OFF\n*: autoscale\n:/: orthonormalize\n+- "
              "zoom\n\n3d\nON/Back: leave or interrupt 3d rendering\ny^x or e^x: precise 3d\nF2: "
              "faster&less precise\nF3: slower&more precise\n( and ): modify stiff surfaces 3d "
              "rendering\n0: hidden 3d surfaces ON/OFF\n.: fill stiff 3d surfacesON/OFF\n5 reset "
              "3d view\n7,8,9,1,2,3: move 3d view\n\nGeometry\nF4: change geometry mode\nFrame "
              "mode (shift F1): modify viewpoint\nPointer mode (shift F2): select an object and "
              "move it with EXE and cursor keys\nOther modes: create an object\nEXIT: go to "
              "symbolic view where you can create/modify objects with commands, press EXE to go "
              "back to graphic view");
    int exec = doTextArea(&text);
  }

  int Graph2d::ui() {
    Graph2d & gr = *this;
    if (!hp && gr.symbolic_instructions.size() == 1 &&
    ...
    for (;;) {
      ...
      if (0 && !hp) {
#ifdef NUMWORKS
        os_draw_string(0, LCD_HEIGHT_PX - STATUS_AREA_PX - 17, COLOR_BLACK, COLOR_WHITE,
                       "home: cfg");
#else
    ...
    ck_getkey(&key);
        ...
        if (key >= 'A' && key <= 'Z') {
          ...
          if (tmp.type == _IDNT) {
            ...
            if (tmp.is_symb_of_sommet(at_pnt)) {
              ...
              if (is3d && tmp.type == _VECT && tmp._VECTptr->size() == 3 &&
      ...
      if (hp && (key == KEY_CTRL_CATALOG || key == KEY_BOOK || key == KEY_CTRL_F4)) {
        const char * tab[] = {lang == 1 ? "Mode repere" : "Frame mode", // 0
        ...
        if (choix == 5) { // Points
          const char * tab[] = {lang == 1 ? "Point" : "Point",
          ...
          const char * help[] = {
              "Point", "Point1,Point2", "Circle", "Line1,Line2", "Curve1,Curve2", "Curve",
          };
          set_mode(ftmp[choix], ffinal[choix], mode[choix], help[choix]);
          continue;
        }
        if (choix == 6) { // Droites
          const char * tab[] = {lang == 1 ? "Segment" : "Segment",
          ...
          if (choix < 0 || choix > s)
            continue;
          gen ftmp[] = {at_segment, at_vector,    at_demi_droite,     at_droite,
                        at_segment, at_parallele, at_perpendiculaire, at_mediatrice,
                        at_segment, at_segment,   at_segment};
          gen ffinal[] = {at_segment,     at_vector,    at_demi_droite,     at_droite,
                          at_plan,        at_parallele, at_perpendiculaire, at_mediatrice,
                          at_bissectrice, at_mediane,   at_tangent};
          int mode[] = {2, 2, 2, 2, 2, 3, 2, 2, 3, 3, 2};
          const char * help[] = {"Point1,Point2",
                                 "Point1,Point2",
                                 "Point1,Point2",
                                 "Point1,Point2",
        ...
        if (choix == 7) { // Polygons
          const char * tab[] = {lang == 1 ? "Triangle" : "Triangle",
          ...
          if (choix < 0 || choix > s)
            continue;
          gen ftmp[] = {at_polygone_ouvert, at_segment,         at_segment,
                        at_polygone_ouvert, at_polygone_ouvert, at_polygone_ouvert,
                        at_polygone_ouvert, at_polygone_ouvert, at_polygone_ouvert,
                        at_polygone_ouvert, at_polygone_ouvert};
          gen ffinal[] = {at_triangle,  at_triangle_equilateral,
                          at_carre,     at_quadrilatere,
                          at_polygone,  at_tetraedre,
                          at_tetraedre, at_cube,
                          at_octaedre,  at_dodecaedre,
                          at_icosaedre};
          int mode[] = {3, 2, 2, 4, 5, 4, 3, 3, 3, 3, 3};
          int m = mode[choix];
          if (choix == 4) {
            double d = 5;
            if (inputdouble(lang == 1 ? "Nombre de sommets?" : "Number of vertices?", d,
                            contextptr) &&
                d == int(d) && d >= 3 && d < 20) {
              m = d;
            } else
        ...
        if (choix == 8) { // Conics
          const char * tab[] = {
              lang == 1 ? "cercle" : "circle",       lang == 1 ? "circonscrit" : "circumcircle",
              lang == 1 ? "inscrit" : "incircle",    lang == 1 ? "ellipse" : "ellipse",
              lang == 1 ? "hyperbole" : "hyperbola", lang == 1 ? "parabole" : "parabola",
              lang == 1 ? "sphere" : "sphere",       0};
          const int s = sizeof(tab) / sizeof(char *);
          int choix = select_item(tab, "Conic", true);
          if (choix < 0 || choix > s)
            continue;
          gen ftmp[] = {at_segment, at_segment, at_segment, at_segment,
                        at_segment, at_segment, at_segment};
          gen ffinal[] = {at_cercle,    at_circonscrit, at_inscrit, at_ellipse,
                          at_hyperbole, at_parabole,    at_sphere};
        ...
        if (choix == 9) { // Curves
          const char * tab[] = {lang == 1 ? "Fonction plot(sin(x))" : "Function plot(sin(x))",
                                lang == 1 ? "Param. plotparam([x^2,x^3])"
                                          : "Param. plotparam([x^2,x^3])",
          ...
          if (choix < 0 || choix > s)
            continue;
          const char * cmd[] = {"plot()", "plotparam()", "plotpolar()",
                                "plot()", "plotfield()", "plotode()"};
        ...
        if (choix == 10) {
          ...
          if (param == 0) {
            confirm(lang == 1 ? "Plus de variables libres." : "No more free variable available",
                    lang == 1 ? "Essayez purge(a) ou purge(b) ou ..."
                              : "Try purge(a) or purge(b) or ...");
            continue;
          }
        ...
        if (choix == 11) { // Transforms
          const char * tab[] = {
              lang == 1 ? "symetrie" : "reflexion", lang == 1 ? "rotation" : "rotation",
              lang == 1 ? "translation" : "translation", lang == 1 ? "projection" : "projection",
              lang == 1 ? "homothetie" : "homothety", lang == 1 ? "similitude" : "similarity",
              // lang==1?"":"",
              // lang==1?"":"",
              0};
          const int s = sizeof(tab) / sizeof(char *);
          ...
          if (choix < 0 || choix > s)
            continue;
          gen ftmp[] = {at_segment, at_polygone_ouvert, at_segment,
                        at_segment, at_segment,         at_polygone_ouvert};
          gen ffinal[] = {at_symetrie,   at_rotation,   at_translation,
                          at_projection, at_homothetie, at_similitude};
          int mode[] = {2, 3, 2, 2, 2, 3};
          const char * help[] = {"Symmetry_center_axis,Object",
                                 "Center,Angle,Object",
                                 "Vector,Object",
                                 "Curve,Object",
                                 "Center,Ratio,Object",
        ...
        if (choix == 12) { // Mesures
          const char * tab[] = {lang == 1 ? "distance" : "distance",
                                lang == 1 ? "angle" : "angle",
                                lang == 1 ? "aire" : "area",
          ...
          if (choix < 0 || choix > s)
            continue;
          gen ftmp[] = {at_segment, at_triangle, at_areaat,    at_perimeterat,    at_slopeat,
                        at_segment, at_triangle, at_areaatraw, at_perimeteratraw, at_slopeatraw};
          gen ffinal[] = {at_distanceat,     at_angleat,       at_areaat,     at_perimeterat,
                          at_slopeat,        at_distanceatraw, at_angleatraw, at_areaatraw,
                          at_perimeteratraw, at_slopeatraw};
      ...
      if (key == KEY_CTRL_MENU || key == KEY_CTRL_F6 || (!hp && key == KEY_CTRL_F4)) {
        char menu_xmin[32], menu_xmax[32], menu_ymin[32], menu_ymax[32], menu_zmin[32],
            menu_zmax[32], menu_depth[32];
        for (;;) {
          string s;
          ...
          smallmenuitems[1].text =
              (char *)((lang == 1) ? "Etude graphe (xtt)" : "Curve study (xtt)");
          ...
          smallmenuitems[18].text =
              (char *)((lang == 1) ? "Effacer traces geometrie" : "Clear geometry traces");
  ...
  void draw_turtle(double x1, double x2, double xc, double y, double yc, double theta1,
                   double theta2, double cottheta1, double cottheta2, int c, GIAC_CONTEXT) {
    int Y = my_round(yc - y);
  ...
  void draw_turtle_arc(double xc, double yc, double r1, double r2, int c, double theta1,
                       double theta2, GIAC_CONTEXT) {
    if (theta1 > theta2)
    ...
    for (int y = r2; y >= 0; --y) {
      ...
      if (xmax1 < 0) {
        ...
        if (y) {
          draw_turtle(-xmax2, -xmax1, xc, -y, yc, theta1, theta2, cotheta1, cotheta2, c,
                      contextptr);
          draw_turtle(xmax1, xmax2, xc, -y, yc, theta1, theta2, cotheta1, cotheta2, c, contextptr);
        }
        // update xmax1
          ...
          if (current.s >= 0) { // Write a string
            ...
            if (current.s < ecristab().size())
              text_print(current.radius, ecristab()[current.s].c_str(),
                         ...
                         int(deltay + LCD_HEIGHT_PX - turtlezoom * (current.y - turtley)),
                         current.color);
          } else {
              ...
              if (current.direct) {
                if (rempli)
                  ...
                  if (width > 1) {
                    ...
                    draw_turtle_arc(deltax + x, deltay + LCD_HEIGHT_PX - y, R - width / 2.0,
                                    R + width / 2.0, current.color, theta1 - 90, theta2 - 90,
                                    context0);
                  } else
                ...
                if (rempli)
                  ...
                  if (width > 1) {
                    ...
                    R = int(turtlezoom * r + .5);
                    draw_turtle_arc(deltax + x, deltay + LCD_HEIGHT_PX - y, R - width / 2.0,
                                    R + width / 2.0, current.color, 90 + theta2, 90 + theta1,
                                    context0);
                  } else
            ...
            if (current.radius < -1 && k + current.radius >= 0) {
              ...
              logo_turtle & t = (*turtleptr)[k + i];
                ...
                if (t.radius > 0) {
                  ...
                  fl_pie(deltax + x, deltay + LCD_HEIGHT_PX - y, R, R, 0, 360, current.color,
                         false);
                }
...
#endif
      drawRectangle(deltax + horizontal_pixels, deltay, LCD_WIDTH_PX - horizontal_pixels,
                    2 * COORD_SIZE, COLOR_YELLOW);
      // drawRectangle(deltax, deltay, LCD_WIDTH_PX, LCD_HEIGHT_PX,COLOR_BLACK);
      char buf[32];
      sprintf(buf, "x %i   ", int(turtle.x + .5));
      text_print(18, buf, deltax + horizontal_pixels, deltay + (2 * COORD_SIZE) / 3 - 2,
                 COLOR_BLACK, COLOR_YELLOW);
      sprintf(buf, "y %i   ", int(turtle.y + .5));
      text_print(18, buf, deltax + horizontal_pixels, deltay + (4 * COORD_SIZE) / 3 - 3,
                 COLOR_BLACK, COLOR_YELLOW);
      sprintf(buf, "t %i   ", int(turtle.theta + .5));
      text_print(18, buf, deltax + horizontal_pixels, deltay + 2 * COORD_SIZE - 4, COLOR_BLACK,
                 COLOR_YELLOW);
    }
  }
